...
/-
**CompactlyGenerated** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：Type (w + 1)
参数：w + 1。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`CompactlyGenerated.{u, w}` is the type of `u`-compactly generated `w`-small top
ological spaces.
This should always be used with explicit universe parameters.
-/
...
/-
**CompactlyGenerated.** 是 Mathlib 中的一个实例，位于命名空间 `CompactlyGenerated`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CompactlyGenerated.of** 是 Mathlib 中的一个缩写定义，位于命名空间 `CompactlyGenerated`。
形式化陈述：of : CompactlyGenerated.{u, w} where toTop
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CompactlyGenerated.ofHom** 是 Mathlib 中的一个缩写定义，位于命名空间 `CompactlyGenerated`。
形式化陈述：ofHom (f : C(X, Y)) : of X ⟶ of Y
参数：f : C(X, Y)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CompactlyGenerated.compactlyGeneratedToTop** 是 Mathlib 中的一个定义，位于命名空间 `Compactl
yGenerated`。
形式化陈述：compactlyGeneratedToTop : CompactlyGenerated.{u, w} ⥤ TopCat.{w}
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CompactlyGenerated.fullyFaithfulCompactlyGeneratedToTop** 是 Mathlib 中的一个定义，位于命
名空间 `CompactlyGenerated`。
形式化陈述：fullyFaithfulCompactlyGeneratedToTop : compactlyGeneratedToTop.{u, w}.Full
yFaithful
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps hom inv]
/-
**CompactlyGenerated.isoOfHomeo** 是 Mathlib 中的一个定义，位于命名空间 `CompactlyGenerated`。
形式化陈述：isoOfHomeo {X Y : CompactlyGenerated.{u, w}} (f : X ≃ₜ Y) : X ≅ Y where ho
m
参数：f : X ≃ₜ Y。
该定义给出了上述对象。
...
@[simps]
/-
**CompactlyGenerated.homeoOfIso** 是 Mathlib 中的一个定义，位于命名空间 `CompactlyGenerated`。
形式化陈述：homeoOfIso {X Y : CompactlyGenerated.{u, w}} (f : X ≅ Y) : X ≃ₜ Y where to
Fun
参数：f : X ≅ Y。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CompactlyGenerated.isoEquivHomeo** 是 Mathlib 中的一个定义，位于命名空间 `CompactlyGenerated
`。
形式化陈述：isoEquivHomeo {X Y : CompactlyGenerated.{u, w}} : (X ≅ Y) ≃ (X ≃ₜ Y) where
 toFun
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
