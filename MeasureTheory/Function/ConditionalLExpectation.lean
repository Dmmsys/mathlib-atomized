/-
Copyright (c) 2025 David Ledvinka. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: David Ledvinka
-/
module

public import Mathlib.MeasureTheory.Measure.Decomposition.Lebesgue

import Mathlib.MeasureTheory.Measure.Decomposition.RadonNikodym
import Mathlib.Probability.Notation

/-! # Conditional Lebesgue expectation

We define the conditional expectation of a `ℝ≥0∞`-valued function using the Lebesgue integral.
Given a measure `P : Measure[mΩ₀] Ω` and a sub-σ-algebra `mΩ` of `mΩ₀` (meaning `hm : mΩ ≤ mΩ₀`)
and a function `X : Ω → ℝ≥0∞`, if `P.trim hm` is σ-finite, then the conditional (Lebesgue)
expectation `P⁻[X|mΩ]` of `X` is the `mΩ`-measurable function such that for all
`mΩ`-measurable sets `s`, `∫⁻ ω in s, P⁻[X|mΩ] ω ∂P = ∫⁻ ω in s, X ω ∂P`
(see `setLIntegral_condLExp`). This is unique up to `P`-ae equality (see `ae_eq_condLExp`).

## Main definitions

* `condLExp` : conditional (Lebesgue) expectation of `X` with respect to `mΩ`.
* `setLIntegral_condLExp`: For any `mΩ`-measurable set `s`,
  `∫⁻ ω in s, P⁻[X|mΩ] ω ∂P = ∫⁻ ω in s, X ω ∂P`.
* `ae_eq_condLExp` : the conditional (Lebesgue) expectation is characterized by its (Lebesgue)
  integral on `mΩ`-measurable sets up to `P`-ae equality.

## Notation

For a measure `P : Measure[mΩ₀] Ω`, and another `mΩ : MeasurableSpace Ω`, we define the notation
* `P⁻[X|mΩ] = condLExp mΩ P X`

## Design decisions

`P⁻[X|mΩ]` is assigned the junk value `0` when either `¬ mΩ ≤ mΩ₀` (`mΩ` is not a sub-σ-algebra)
or `h : mΩ ≤ mΩ₀` but `¬ SigmaFinite (P.trim hm)` (the latter always holds when `P` is a
probability measure). When both these hold, in some sense the "user definition" of `P⁻[X|mΩ]`
should be considered "the" measurable function which satisfies `setLIntegral_condLExp`
(which is proven unique up to `P`-ae measurable equality in `ae_eq_condLExp`). The actual definition
is just used to show existence. However for (potential) convenience the actual definition assigns
`P⁻[X|mΩ] := X` in the case when `X` is `mΩ`-measurable (which can be invoked using
`condLExp_eq_self`).

## To do

* Prove the pullout property
* Prove a dominated convergence theorem.

-/

public section

open MeasureTheory ProbabilityTheory Measure

open scoped ENNReal

namespace MeasureTheory

variable {Ω : Type*} {mΩ₀ mΩ : MeasurableSpace Ω} {P : Measure[mΩ₀] Ω} {X Y : Ω -> Real>=0∞}

open scoped Classical in
/-- Conditional (Lebesgue) expectation of a function, with notation `P⁻[X|mΩ]`.

It is defined as `0` if either `¬ mΩ ≤ mΩ₀` or `hm : mΩ ≤ mΩ₀` but `¬ SigmaFinite (P.trim hm)`.

One should typically not use the definition directly.
-/
noncomputable irreducible_def condLExp (mΩ : MeasurableSpace Ω) (P : Measure[mΩ₀] Ω)
    (X : Ω -> Real>=0∞) : Ω -> Real>=0∞ :=
  if hm : mΩ <= mΩ₀ then
    if SigmaFinite (P.trim hm) then
      if Measurable[mΩ] X then X else
      ∂((P.withDensity X).trim hm)/∂(P.trim hm)
    else 0
  else 0

@[inherit_doc MeasureTheory.condLExp]
scoped macro:max P:term noWs "⁻[" X:term " | " mΩ:term "]" : term =>
  `(MeasureTheory.condLExp $mΩ $P $X)

/-- Unexpander for `μ⁻[f|m]` notation. -/
@[app_unexpander MeasureTheory.condLExp]
meta def condLExpUnexpander : Lean.PrettyPrinter.Unexpander
  | `($_ $mΩ $P $X) => `($P⁻[$X|$mΩ])
  | _ => throw ()

/-- info: P⁻[X | mΩ] : Ω → ℝ≥0∞ -/
#guard_msgs in
#check P⁻[X|mΩ]
/-- info: P⁻[X | mΩ] sorry : ℝ≥0∞ -/
#guard_msgs in
#check P⁻[X|mΩ] (sorry : Ω)

/--
theorem `condLExp_of_not_le` / 定理 `condLExp_of_not_le`

English:
theorem condLExp_of_not_le
  given: (hm_not : ¬mΩ <= mΩ₀)
  statement: P⁻[X|mΩ] = 0
  proof: by
  rw [condLExp]; rw [dif_neg hm_not]

中文:
定理 condLExp_of_not_le
  条件: (hm_not : ¬mΩ <= mΩ₀)
  结论: P⁻[X|mΩ] = 0
  证明: by
  rw [condLExp]; rw [dif_neg hm_not]

Depends on / 依赖: condLExp, dif_neg, hm_not
-/
theorem condLExp_of_not_le (hm_not : ¬mΩ <= mΩ₀) : P⁻[X|mΩ] = 0 := by
  rw [condLExp]; rw [dif_neg hm_not]

/--
theorem `condLExp_of_not_sigmaFinite` / 定理 `condLExp_of_not_sigmaFinite`

English:
theorem condLExp_of_not_sigmaFinite
  given: (hm : mΩ <= mΩ₀) (hμm_not : ¬SigmaFinite (P.trim hm))
  proof: by simp [condLExp, dif_pos hm, hμm_not]

中文:
定理 condLExp_of_not_sigmaFinite
  条件: (hm : mΩ <= mΩ₀) (hμm_not : ¬σ有限 (P.trim hm))
  证明: by simp [condLExp, dif_pos hm, hμm_not]

Depends on / 依赖: condLExp, dif_pos
-/
theorem condLExp_of_not_sigmaFinite (hm : mΩ <= mΩ₀) (hμm_not : ¬SigmaFinite (P.trim hm)) :
    P⁻[X|mΩ] = 0 := by simp [condLExp, dif_pos hm, hμm_not]

/--
theorem `condLExp_eq_self` / 定理 `condLExp_eq_self`

English:
theorem condLExp_eq_self
  statement: (hm : mΩ <= mΩ₀) (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
  proof: by
  simp [condLExp, hm, hσ, hX]

中文:
定理 condLExp_eq_self
  结论: (hm : mΩ <= mΩ₀) (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)]
  证明: by
  simp [condLExp, hm, hσ, hX]

Depends on / 依赖: condLExp
-/
theorem condLExp_eq_self (hm : mΩ <= mΩ₀) (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
    (hX : Measurable[mΩ] X) : P⁻[X|mΩ] = X := by
  simp [condLExp, hm, hσ, hX]

/--
theorem `condLExp_of_not_sub_sigma_measurable` / 定理 `condLExp_of_not_sub_sigma_measurable`

English:
theorem condLExp_of_not_sub_sigma_measurable
  statement: (hm : mΩ <= mΩ₀) (P : Measure[mΩ₀] Ω)
  proof: by
  simp [condLExp, hm, hσ, hX]

@[fun_prop]

中文:
定理 condLExp_of_not_sub_sigma_measurable
  结论: (hm : mΩ <= mΩ₀) (P : 测度[mΩ₀] Ω)
  证明: by
  simp [condLExp, hm, hσ, hX]

@[fun_prop]

Depends on / 依赖: condLExp
-/
theorem condLExp_of_not_sub_sigma_measurable (hm : mΩ <= mΩ₀) (P : Measure[mΩ₀] Ω)
    [hσ : SigmaFinite (P.trim hm)] {X : Ω -> Real>=0∞} (hX : ¬Measurable[mΩ] X) :
    P⁻[X|mΩ] = ∂((P.withDensity X).trim hm)/∂(P.trim hm) := by
  simp [condLExp, hm, hσ, hX]

@[fun_prop]
/--
theorem `measurable_condLExp` / 定理 `measurable_condLExp`

English:
theorem measurable_condLExp
  given: (mΩ : MeasurableSpace Ω) (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · by_cases hX : Measurable[mΩ] X
      · simp [condLExp_eq_self hm, hX]
      simp [condLExp_of_not_sub_sigma_measurable hm _ hX, measurable_rnDeriv]
    simp [condLExp_of_not_sigmaFinite hm hσ, measurable_zero]
  simp [condLExp_of_not_le hm, measurable_zero]

@[fun_prop]

中文:
定理 measurable_condLExp
  条件: (mΩ : 可测空间 Ω) (P : 测度[mΩ₀] Ω) (X : Ω -> 实数>=0∞)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · by_cases hX : Measurable[mΩ] X
      · simp [condLExp_eq_self hm, hX]
      simp [condLExp_of_not_sub_sigma_measurable hm _ hX, measurable_rnDeriv]
    simp [condLExp_of_not_sigmaFinite hm hσ, measurable_zero]
  simp [condLExp_of_not_le hm, measurable_zero]

@[fun_prop]

Depends on / 依赖: Measurable, P.trim, SigmaFinite, condLExp_eq_self, condLExp_of_not_le, condLExp_of_not_sigmaFinite, condLExp_of_not_sub_sigma_measurable, measurable_rnDeriv, measurable_zero
-/
theorem measurable_condLExp (mΩ : MeasurableSpace Ω) (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞) :
    Measurable[mΩ] P⁻[X|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · by_cases hX : Measurable[mΩ] X
      · simp [condLExp_eq_self hm, hX]
      simp [condLExp_of_not_sub_sigma_measurable hm _ hX, measurable_rnDeriv]
    simp [condLExp_of_not_sigmaFinite hm hσ, measurable_zero]
  simp [condLExp_of_not_le hm, measurable_zero]

@[fun_prop]
/--
theorem `measurable_condLExp'` / 定理 `measurable_condLExp'`

English:
theorem measurable_condLExp'
  given: (mΩ : MeasurableSpace Ω) (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  · exact (measurable_condLExp _ _ _).mono hm (le_refl _)
  · simp [condLExp_of_not_le hm, measurable_zero]

中文:
定理 measurable_condLExp'
  条件: (mΩ : 可测空间 Ω) (P : 测度[mΩ₀] Ω) (X : Ω -> 实数>=0∞)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  · exact (measurable_condLExp _ _ _).mono hm (le_refl _)
  · simp [condLExp_of_not_le hm, measurable_zero]

Depends on / 依赖: condLExp_of_not_le, le_refl, measurable_condLExp, measurable_zero
-/
theorem measurable_condLExp' (mΩ : MeasurableSpace Ω) (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞) :
    Measurable[mΩ₀] P⁻[X|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  · exact (measurable_condLExp _ _ _).mono hm (le_refl _)
  · simp [condLExp_of_not_le hm, measurable_zero]

variable (hm : mΩ <= mΩ₀)

/--
theorem `setLIntegral_condLExp` / 定理 `setLIntegral_condLExp`

English:
theorem setLIntegral_condLExp
  statement: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
  proof: by
  by_cases hX : Measurable[mΩ] X
  · simp [condLExp_eq_self hm _ hX]
  have h := AbsolutelyContinuous.trim (withDensity_absolutelyContinuous P X) hm
  have : SFinite ((P.withDensity X).trim hm) := sFinite_of_absolutelyContinuous h
  rw [condLExp_of_not_sub_sigma_measurable hm _ hX]; rw [← lintegral_indicator (hm s hs)]; rw [← lintegral_trim hm (by measurability)]; rw [lintegral_indicator hs]; rw [setLIntegral_rnDeriv' h hs]; rw [trim_measurableSet_eq hm hs]; rw [withDensity_apply _ (hm s hs)]

中文:
定理 setL整数egral_condLExp
  结论: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)]
  证明: by
  by_cases hX : Measurable[mΩ] X
  · simp [condLExp_eq_self hm _ hX]
  have h := AbsolutelyContinuous.trim (withDensity_absolutelyContinuous P X) hm
  have : SFinite ((P.withDensity X).trim hm) := sFinite_of_absolutelyContinuous h
  rw [condLExp_of_not_sub_sigma_measurable hm _ hX]; rw [← lintegral_indicator (hm s hs)]; rw [← lintegral_trim hm (by measurability)]; rw [lintegral_indicator hs]; rw [setLIntegral_rnDeriv' h hs]; rw [trim_measurableSet_eq hm hs]; rw [withDensity_apply _ (hm s hs)]

Depends on / 依赖: AbsolutelyContinuous, AbsolutelyContinuous.trim, Measurable, P.withDensity, SFinite, condLExp_eq_self, condLExp_of_not_sub_sigma_measurable, lintegral_indicator, lintegral_trim, measurability, sFinite_of_absolutelyContinuous, setLIntegral_rnDeriv, trim_measurableSet_eq, withDensity, withDensity_absolutelyContinuous, withDensity_apply
-/
theorem setLIntegral_condLExp (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
    (X : Ω -> Real>=0∞) {s : Set Ω} (hs : MeasurableSet[mΩ] s) :
    ∫⁻ ω in s, P⁻[X|mΩ] ω ∂P = ∫⁻ ω in s, X ω ∂P := by
  by_cases hX : Measurable[mΩ] X
  · simp [condLExp_eq_self hm _ hX]
  have h := AbsolutelyContinuous.trim (withDensity_absolutelyContinuous P X) hm
  have : SFinite ((P.withDensity X).trim hm) := sFinite_of_absolutelyContinuous h
  rw [condLExp_of_not_sub_sigma_measurable hm _ hX]; rw [← lintegral_indicator (hm s hs)]; rw [← lintegral_trim hm (by measurability)]; rw [lintegral_indicator hs]; rw [setLIntegral_rnDeriv' h hs]; rw [trim_measurableSet_eq hm hs]; rw [withDensity_apply _ (hm s hs)]

/--
theorem `setLIntegral_condLExp_trim` / 定理 `setLIntegral_condLExp_trim`

English:
theorem setLIntegral_condLExp_trim
  statement: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
  proof: by
  rw [setLIntegral_trim hm (measurable_condLExp _ _ _) hs]; rw [setLIntegral_condLExp _ _ _ hs]

中文:
定理 setL整数egral_condLExp_trim
  结论: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)]
  证明: by
  rw [setLIntegral_trim hm (measurable_condLExp _ _ _) hs]; rw [setLIntegral_condLExp _ _ _ hs]

Depends on / 依赖: measurable_condLExp, setLIntegral_condLExp, setLIntegral_trim
-/
theorem setLIntegral_condLExp_trim (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
    (X : Ω -> Real>=0∞) {s : Set Ω} (hs : MeasurableSet[mΩ] s) :
    ∫⁻ ω in s, P⁻[X|mΩ] ω ∂P.trim hm = ∫⁻ ω in s, X ω ∂P := by
  rw [setLIntegral_trim hm (measurable_condLExp _ _ _) hs]; rw [setLIntegral_condLExp _ _ _ hs]

/--
theorem `lintegral_condLExp` / 定理 `lintegral_condLExp`

English:
theorem lintegral_condLExp
  given: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)] (X : Ω -> Real>=0∞)
  proof: by
  simpa [← setLIntegral_univ] using setLIntegral_condLExp _ _ _ .univ

中文:
定理 lintegral_condLExp
  条件: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)] (X : Ω -> 实数>=0∞)
  证明: by
  simpa [← setLIntegral_univ] using setLIntegral_condLExp _ _ _ .univ

Depends on / 依赖: setLIntegral_condLExp, setLIntegral_univ
-/
theorem lintegral_condLExp (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)] (X : Ω -> Real>=0∞) :
    ∫⁻ ω, P⁻[X|mΩ] ω ∂P = ∫⁻ ω, X ω ∂P := by
  simpa [← setLIntegral_univ] using setLIntegral_condLExp _ _ _ .univ

/--
lemma `condLExp_lt_top` / 引理 `condLExp_lt_top`

English:
lemma condLExp_lt_top
  given: {f : Ω -> Real>=0∞} (hf : ∫⁻ x, f x ∂P != ∞)
  statement: forallᵐ x ∂P, P⁻[f|mΩ] x < ∞
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  · exact ae_lt_top' (by fun_prop) (by rwa [lintegral_condLExp])
  · simp [condLExp_of_not_sigmaFinite hm hσ]

中文:
引理 condLExp_lt_top
  条件: {f : Ω -> 实数>=0∞} (hf : ∫⁻ x, f x ∂P != ∞)
  结论: 对任意ᵐ x ∂P, P⁻[f|mΩ] x < ∞
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  · exact ae_lt_top' (by fun_prop) (by rwa [lintegral_condLExp])
  · simp [condLExp_of_not_sigmaFinite hm hσ]

Depends on / 依赖: P.trim, SigmaFinite, ae_lt_top, condLExp_of_not_le, condLExp_of_not_sigmaFinite, fun_prop, lintegral_condLExp
-/
lemma condLExp_lt_top {f : Ω -> Real>=0∞} (hf : ∫⁻ x, f x ∂P != ∞) : forallᵐ x ∂P, P⁻[f|mΩ] x < ∞ := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  · exact ae_lt_top' (by fun_prop) (by rwa [lintegral_condLExp])
  · simp [condLExp_of_not_sigmaFinite hm hσ]

/--
lemma `condLExp_ne_top` / 引理 `condLExp_ne_top`

English:
lemma condLExp_ne_top
  given: {f : Ω -> Real>=0∞} (hf : ∫⁻ x, f x ∂P != ∞)
  statement: forallᵐ x ∂P, P⁻[f|mΩ] x != ∞
  proof: by
  filter_upwards [condLExp_lt_top hf] with x hx using hx.ne

中文:
引理 condLExp_ne_top
  条件: {f : Ω -> 实数>=0∞} (hf : ∫⁻ x, f x ∂P != ∞)
  结论: 对任意ᵐ x ∂P, P⁻[f|mΩ] x != ∞
  证明: by
  filter_upwards [condLExp_lt_top hf] with x hx using hx.ne

Depends on / 依赖: condLExp_lt_top, filter_upwards, hx.ne
-/
lemma condLExp_ne_top {f : Ω -> Real>=0∞} (hf : ∫⁻ x, f x ∂P != ∞) : forallᵐ x ∂P, P⁻[f|mΩ] x != ∞ := by
  filter_upwards [condLExp_lt_top hf] with x hx using hx.ne

/--
theorem `ae_eq_condLExp₀` / 定理 `ae_eq_condLExp₀`

English:
theorem ae_eq_condLExp₀
  statement: {P : Measure[mΩ₀] Ω} [hσ : SigmaFinite (P.trim hm)]
  proof: by
  apply ae_eq_of_ae_eq_trim
  apply ae_eq_of_forall_setLIntegral_eq_of_sigmaFinite₀ hY (by fun_prop)
  intro s hs _
  rw [setLIntegral_trim_ae hm hY hs]; rw [setLIntegral_condLExp_trim _ _ _ hs]
  exact hXY s hs

中文:
定理 ae_eq_condLExp₀
  结论: {P : 测度[mΩ₀] Ω} [hσ : σ有限 (P.trim hm)]
  证明: by
  apply ae_eq_of_ae_eq_trim
  apply ae_eq_of_forall_setLIntegral_eq_of_sigmaFinite₀ hY (by fun_prop)
  intro s hs _
  rw [setLIntegral_trim_ae hm hY hs]; rw [setLIntegral_condLExp_trim _ _ _ hs]
  exact hXY s hs

Depends on / 依赖: ae_eq_of_ae_eq_trim, fun_prop, setLIntegral_condLExp_trim, setLIntegral_trim_ae
-/
theorem ae_eq_condLExp₀ {P : Measure[mΩ₀] Ω} [hσ : SigmaFinite (P.trim hm)]
    (X : Ω -> Real>=0∞) (hY : AEMeasurable[mΩ] Y (P.trim hm))
    (hXY : forall s, MeasurableSet[mΩ] s -> ∫⁻ ω in s, Y ω ∂P = ∫⁻ ω in s, X ω ∂P) :
    Y =ᵐ[P] P⁻[X|mΩ] := by
  apply ae_eq_of_ae_eq_trim
  apply ae_eq_of_forall_setLIntegral_eq_of_sigmaFinite₀ hY (by fun_prop)
  intro s hs _
  rw [setLIntegral_trim_ae hm hY hs]; rw [setLIntegral_condLExp_trim _ _ _ hs]
  exact hXY s hs

/--
theorem `ae_eq_condLExp` / 定理 `ae_eq_condLExp`

English:
theorem ae_eq_condLExp
  statement: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
  proof: ae_eq_condLExp₀ _ _ hY.aemeasurable hXY

中文:
定理 ae_eq_condLExp
  结论: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)]
  证明: ae_eq_condLExp₀ _ _ hY.aemeasurable hXY

Depends on / 依赖: aemeasurable, hY.aemeasurable
-/
theorem ae_eq_condLExp (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
    (X : Ω -> Real>=0∞) (hY : Measurable[mΩ] Y)
    (hXY : forall s, MeasurableSet[mΩ] s -> ∫⁻ ω in s, Y ω ∂P = ∫⁻ ω in s, X ω ∂P) :
    Y =ᵐ[P] P⁻[X|mΩ] := ae_eq_condLExp₀ _ _ hY.aemeasurable hXY

/--
theorem `condLExp_const` / 定理 `condLExp_const`

English:
theorem condLExp_const
  given: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)] (c : Real>=0∞)
  proof: condLExp_eq_self _ _ (measurable_const)

@[gcongr]

中文:
定理 condLExp_const
  条件: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)] (c : 实数>=0∞)
  证明: condLExp_eq_self _ _ (measurable_const)

@[gcongr]

Depends on / 依赖: condLExp_eq_self, measurable_const
-/
theorem condLExp_const (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)] (c : Real>=0∞) :
    P⁻[fun _ : Ω => c|mΩ] = fun _ => c := condLExp_eq_self _ _ (measurable_const)

@[gcongr]
/--
theorem `condLExp_congr_ae` / 定理 `condLExp_congr_ae`

English:
theorem condLExp_congr_ae
  statement: {P : Measure[mΩ₀] Ω}
  proof: by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · refine ae_eq_condLExp _ _ _ (measurable_condLExp _ _ _) (fun s hs => ?_)
      rw [setLIntegral_condLExp _ _ _ hs]
      apply setLIntegral_congr_fun_ae (hm s hs)
      filter_upwards [hXY] with _ h _ using h
    simp [condLExp_of_not_sigmaFinite hm hσ]
  simp [condLExp_of_not_le hm]

@[simp]

中文:
定理 condLExp_congr_ae
  结论: {P : 测度[mΩ₀] Ω}
  证明: by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · refine ae_eq_condLExp _ _ _ (measurable_condLExp _ _ _) (fun s hs => ?_)
      rw [setLIntegral_condLExp _ _ _ hs]
      apply setLIntegral_congr_fun_ae (hm s hs)
      filter_upwards [hXY] with _ h _ using h
    simp [condLExp_of_not_sigmaFinite hm hσ]
  simp [condLExp_of_not_le hm]

@[simp]

Depends on / 依赖: P.trim, SigmaFinite, ae_eq_condLExp, condLExp_of_not_le, condLExp_of_not_sigmaFinite, filter_upwards, measurable_condLExp, setLIntegral_condLExp, setLIntegral_congr_fun_ae
-/
theorem condLExp_congr_ae {P : Measure[mΩ₀] Ω}
    {X Y : Ω -> Real>=0∞} (hXY : X =ᵐ[P] Y) : P⁻[X|mΩ] =ᵐ[P] P⁻[Y|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  · by_cases hσ : SigmaFinite (P.trim hm)
    · refine ae_eq_condLExp _ _ _ (measurable_condLExp _ _ _) (fun s hs => ?_)
      rw [setLIntegral_condLExp _ _ _ hs]
      apply setLIntegral_congr_fun_ae (hm s hs)
      filter_upwards [hXY] with _ h _ using h
    simp [condLExp_of_not_sigmaFinite hm hσ]
  simp [condLExp_of_not_le hm]

@[simp]
/--
theorem `condLExp_zero` / 定理 `condLExp_zero`

English:
theorem condLExp_zero
  given: (P : Measure[mΩ₀] Ω)
  statement: P⁻[0|mΩ] = 0
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  exact condLExp_const hm P 0

@[simp]

中文:
定理 condLExp_zero
  条件: (P : 测度[mΩ₀] Ω)
  结论: P⁻[0|mΩ] = 0
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  exact condLExp_const hm P 0

@[simp]

Depends on / 依赖: P.trim, SigmaFinite, condLExp_const, condLExp_of_not_le, condLExp_of_not_sigmaFinite
-/
theorem condLExp_zero (P : Measure[mΩ₀] Ω) : P⁻[0|mΩ] = 0 := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  exact condLExp_const hm P 0

@[simp]
/--
theorem `condLExp_one` / 定理 `condLExp_one`

English:
theorem condLExp_one
  given: (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)]
  proof: condLExp_const hm P 1

@[gcongr]

中文:
定理 condLExp_one
  条件: (P : 测度[mΩ₀] Ω) [hσ : σ有限 (P.trim hm)]
  证明: condLExp_const hm P 1

@[gcongr]

Depends on / 依赖: condLExp_const
-/
theorem condLExp_one (P : Measure[mΩ₀] Ω) [hσ : SigmaFinite (P.trim hm)] :
    P⁻[1|mΩ] = 1 := condLExp_const hm P 1

@[gcongr]
/--
theorem `condLExp_congr_ae_trim` / 定理 `condLExp_congr_ae_trim`

English:
theorem condLExp_congr_ae_trim
  given: {P : Measure[mΩ₀] Ω} {X Y : Ω -> Real>=0∞} (hXY : X =ᵐ[P] Y)
  proof: by
  apply ae_eq_trim_of_measurable hm (measurable_condLExp _ _ X) (measurable_condLExp _ _ Y)
  exact condLExp_congr_ae hXY

中文:
定理 condLExp_congr_ae_trim
  条件: {P : 测度[mΩ₀] Ω} {X Y : Ω -> 实数>=0∞} (hXY : X =ᵐ[P] Y)
  证明: by
  apply ae_eq_trim_of_measurable hm (measurable_condLExp _ _ X) (measurable_condLExp _ _ Y)
  exact condLExp_congr_ae hXY

Depends on / 依赖: ae_eq_trim_of_measurable, condLExp_congr_ae, measurable_condLExp
-/
theorem condLExp_congr_ae_trim {P : Measure[mΩ₀] Ω} {X Y : Ω -> Real>=0∞} (hXY : X =ᵐ[P] Y) :
    P⁻[X|mΩ] =ᵐ[P.trim hm] P⁻[Y|mΩ] := by
  apply ae_eq_trim_of_measurable hm (measurable_condLExp _ _ X) (measurable_condLExp _ _ Y)
  exact condLExp_congr_ae hXY

/--
theorem `condLExp_bot'` / 定理 `condLExp_bot'`

English:
theorem condLExp_bot'
  given: (P : Measure[mΩ₀] Ω) [NeZero P] (X : Ω -> Real>=0∞)
  proof: by
  by_cases hP : IsFiniteMeasure P; swap
  · have hσ : ¬SigmaFinite (P.trim bot_le) := by rwa [sigmaFinite_trim_bot_iff]
    rw [not_isFiniteMeasure_iff] at hP
    rw [condLExp_of_not_sigmaFinite bot_le hσ]
    simp [hP, Pi.zero_def]
  obtain ⟨c, h_eq⟩ := eq_const_of_measurable_bot (measurable_condLExp ⊥ P X)
  ext _
  rw [← lintegral_condLExp bot_le]
  simp [h_eq, mul_comm, mul_assoc, ENNReal.mul_inv_cancel
    (NeZero.ne (P .univ)) (measure_ne_top _ _)]

中文:
定理 condLExp_bot'
  条件: (P : 测度[mΩ₀] Ω) [NeZero P] (X : Ω -> 实数>=0∞)
  证明: by
  by_cases hP : IsFiniteMeasure P; swap
  · have hσ : ¬SigmaFinite (P.trim bot_le) := by rwa [sigmaFinite_trim_bot_iff]
    rw [not_isFiniteMeasure_iff] at hP
    rw [condLExp_of_not_sigmaFinite bot_le hσ]
    simp [hP, Pi.zero_def]
  obtain ⟨c, h_eq⟩ := eq_const_of_measurable_bot (measurable_condLExp ⊥ P X)
  ext _
  rw [← lintegral_condLExp bot_le]
  simp [h_eq, mul_comm, mul_assoc, ENNReal.mul_inv_cancel
    (NeZero.ne (P .univ)) (measure_ne_top _ _)]

Depends on / 依赖: ENNReal, ENNReal.mul_inv_cancel, IsFiniteMeasure, NeZero, NeZero.ne, P.trim, Pi.zero_def, SigmaFinite, bot_le, condLExp_of_not_sigmaFinite, eq_const_of_measurable_bot, h_eq, lintegral_condLExp, measurable_condLExp, measure_ne_top, mul_assoc, mul_comm, mul_inv_cancel, not_isFiniteMeasure_iff, sigmaFinite_trim_bot_iff
-/
theorem condLExp_bot' (P : Measure[mΩ₀] Ω) [NeZero P] (X : Ω -> Real>=0∞) :
    P⁻[X|⊥] = fun _ => (P .univ)⁻¹ • ∫⁻ ω, X ω ∂P := by
  by_cases hP : IsFiniteMeasure P; swap
  · have hσ : ¬SigmaFinite (P.trim bot_le) := by rwa [sigmaFinite_trim_bot_iff]
    rw [not_isFiniteMeasure_iff] at hP
    rw [condLExp_of_not_sigmaFinite bot_le hσ]
    simp [hP, Pi.zero_def]
  obtain ⟨c, h_eq⟩ := eq_const_of_measurable_bot (measurable_condLExp ⊥ P X)
  ext _
  rw [← lintegral_condLExp bot_le]
  simp [h_eq, mul_comm, mul_assoc, ENNReal.mul_inv_cancel
    (NeZero.ne (P .univ)) (measure_ne_top _ _)]

/--
theorem `condLExp_bot_ae_eq` / 定理 `condLExp_bot_ae_eq`

English:
theorem condLExp_bot_ae_eq
  given: (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞)
  proof: by
  rcases eq_zero_or_neZero P with rfl | hP
  · rw [ae_zero]; exact Filter.eventually_bot
exact ae_of_all P congr_fun (condLExp_bot' P X)

中文:
定理 condLExp_bot_ae_eq
  条件: (P : 测度[mΩ₀] Ω) (X : Ω -> 实数>=0∞)
  证明: by
  rcases eq_zero_or_neZero P with rfl | hP
  · rw [ae_zero]; exact Filter.eventually_bot
exact ae_of_all P congr_fun (condLExp_bot' P X)

Depends on / 依赖: Filter, Filter.eventually_bot, ae_of_all, ae_zero, condLExp_bot, congr_fun, eq_zero_or_neZero, eventually_bot
-/
theorem condLExp_bot_ae_eq (P : Measure[mΩ₀] Ω) (X : Ω -> Real>=0∞) :
    P⁻[X|⊥] =ᵐ[P] fun _ => (P .univ)⁻¹ • ∫⁻ ω, X ω ∂P := by
  rcases eq_zero_or_neZero P with rfl | hP
  · rw [ae_zero]; exact Filter.eventually_bot
exact ae_of_all P congr_fun (condLExp_bot' P X)

/--
theorem `condLExp_bot` / 定理 `condLExp_bot`

English:
theorem condLExp_bot
  given: (P : Measure[mΩ₀] Ω) [IsProbabilityMeasure P] (X : Ω -> Real>=0∞)
  proof: (condLExp_bot' P X).trans (by simp)

中文:
定理 condLExp_bot
  条件: (P : 测度[mΩ₀] Ω) [是概率测度 P] (X : Ω -> 实数>=0∞)
  证明: (condLExp_bot' P X).trans (by simp)

Depends on / 依赖: condLExp_bot
-/
theorem condLExp_bot (P : Measure[mΩ₀] Ω) [IsProbabilityMeasure P] (X : Ω -> Real>=0∞) :
    P⁻[X|⊥] = fun _ => ∫⁻ ω, X ω ∂P :=
  (condLExp_bot' P X).trans (by simp)

/--
theorem `condLExp_mono` / 定理 `condLExp_mono`

English:
theorem condLExp_mono
  given: (hXY : X <=ᵐ[P] Y)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm, Filter.EventuallyLE.rfl]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ, Filter.EventuallyLE.rfl]
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  apply setLIntegral_mono_ae' (hm s hs)
  filter_upwards [hXY] using fun _ h _ => h

中文:
定理 condLExp_mono
  条件: (hXY : X <=ᵐ[P] Y)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm, Filter.EventuallyLE.rfl]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ, Filter.EventuallyLE.rfl]
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  apply setLIntegral_mono_ae' (hm s hs)
  filter_upwards [hXY] using fun _ h _ => h

Depends on / 依赖: EventuallyLE, Filter, Filter.EventuallyLE.rfl, P.trim, SigmaFinite, ae_le_of_ae_le_trim, ae_le_of_forall_setLIntegral_le_of_sigmaFinite, condLExp_of_not_le, condLExp_of_not_sigmaFinite, filter_upwards, fun_prop, repeat, setLIntegral_condLExp_trim, setLIntegral_mono_ae, simp_rw
-/
theorem condLExp_mono (hXY : X <=ᵐ[P] Y) :
    P⁻[X|mΩ] <=ᵐ[P] P⁻[Y|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm, Filter.EventuallyLE.rfl]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ, Filter.EventuallyLE.rfl]
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  apply setLIntegral_mono_ae' (hm s hs)
  filter_upwards [hXY] using fun _ h _ => h

/--
theorem `condLExp_add_le` / 定理 `condLExp_add_le`

English:
theorem condLExp_add_le
  given: (X Y : Ω -> Real>=0∞)
  proof: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by fun_prop)]
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  grw [le_lintegral_add]
  simp

中文:
定理 condLExp_add_le
  条件: (X Y : Ω -> 实数>=0∞)
  证明: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by fun_prop)]
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  grw [le_lintegral_add]
  simp

Depends on / 依赖: P.trim, Pi.add_apply, SigmaFinite, add_apply, ae_le_of_ae_le_trim, ae_le_of_forall_setLIntegral_le_of_sigmaFinite, condLExp_of_not_le, condLExp_of_not_sigmaFinite, filter_upwards, fun_prop, le_lintegral_add, lintegral_add_left, repeat, setLIntegral_condLExp_trim, simp_rw
-/
theorem condLExp_add_le (X Y : Ω -> Real>=0∞) :
    P⁻[X|mΩ] + P⁻[Y|mΩ] <=ᵐ[P] P⁻[X + Y|mΩ] := by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by fun_prop)]
  repeat rw [setLIntegral_condLExp_trim hm _ _ hs]
  grw [le_lintegral_add]
  simp

/--
theorem `condLExp_add_left` / 定理 `condLExp_add_left`

English:
theorem condLExp_add_left
  given: {X : Ω -> Real>=0∞} (Y : Ω -> Real>=0∞) (hX : AEMeasurable[mΩ₀] X P)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm]; simp
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by measurability)]
  repeat rw [setLIntegral_condLExp hm _ _ hs]
  rw [lintegral_add_left' (by fun_prop)]

中文:
定理 condLExp_add_left
  条件: {X : Ω -> 实数>=0∞} (Y : Ω -> 实数>=0∞) (hX : 几乎处处可测[mΩ₀] X P)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm]; simp
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by measurability)]
  repeat rw [setLIntegral_condLExp hm _ _ hs]
  rw [lintegral_add_left' (by fun_prop)]

Depends on / 依赖: P.trim, Pi.add_apply, SigmaFinite, add_apply, ae_eq_condLExp, condLExp_of_not_le, condLExp_of_not_sigmaFinite, fun_prop, lintegral_add_left, measurability, repeat, setLIntegral_condLExp, simp_rw
-/
theorem condLExp_add_left {X : Ω -> Real>=0∞} (Y : Ω -> Real>=0∞) (hX : AEMeasurable[mΩ₀] X P) :
    P⁻[X + Y|mΩ] =ᵐ[P] P⁻[X|mΩ] + P⁻[Y|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp_rw [condLExp_of_not_le hm]; simp
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.add_apply]
  rw [lintegral_add_left (by measurability)]
  repeat rw [setLIntegral_condLExp hm _ _ hs]
  rw [lintegral_add_left' (by fun_prop)]

/--
theorem `condLExp_add_right` / 定理 `condLExp_add_right`

English:
theorem condLExp_add_right
  given: (X : Ω -> Real>=0∞) {Y : Ω -> Real>=0∞} (hY : AEMeasurable[mΩ₀] Y P)
  proof: by
  rw [add_comm]; rw [add_comm P⁻[X|mΩ]]
  exact condLExp_add_left X hY

中文:
定理 condLExp_add_right
  条件: (X : Ω -> 实数>=0∞) {Y : Ω -> 实数>=0∞} (hY : 几乎处处可测[mΩ₀] Y P)
  证明: by
  rw [add_comm]; rw [add_comm P⁻[X|mΩ]]
  exact condLExp_add_left X hY

Depends on / 依赖: add_comm, condLExp_add_left
-/
theorem condLExp_add_right (X : Ω -> Real>=0∞) {Y : Ω -> Real>=0∞} (hY : AEMeasurable[mΩ₀] Y P) :
    P⁻[X + Y|mΩ] =ᵐ[P] P⁻[X|mΩ] + P⁻[Y|mΩ] := by
  rw [add_comm]; rw [add_comm P⁻[X|mΩ]]
  exact condLExp_add_left X hY

/--
theorem `condLExp_smul` / 定理 `condLExp_smul`

English:
theorem condLExp_smul
  given: (X : Ω -> Real>=0∞) (hX : AEMeasurable[mΩ₀] X P) (c : Real>=0∞)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul]; rw [lintegral_const_mul'']; rw [setLIntegral_condLExp _ _ _ hs]
  all_goals fun_prop

中文:
定理 condLExp_smul
  条件: (X : Ω -> 实数>=0∞) (hX : 几乎处处可测[mΩ₀] X P) (c : 实数>=0∞)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul]; rw [lintegral_const_mul'']; rw [setLIntegral_condLExp _ _ _ hs]
  all_goals fun_prop

Depends on / 依赖: P.trim, Pi.smul_apply, SigmaFinite, ae_eq_condLExp, all_goals, condLExp_of_not_le, condLExp_of_not_sigmaFinite, fun_prop, lintegral_const_mul, setLIntegral_condLExp, smul_apply, smul_eq_mul
-/
theorem condLExp_smul (X : Ω -> Real>=0∞) (hX : AEMeasurable[mΩ₀] X P) (c : Real>=0∞) :
    P⁻[c • X|mΩ] =ᵐ[P] c • P⁻[X|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul]; rw [lintegral_const_mul'']; rw [setLIntegral_condLExp _ _ _ hs]
  all_goals fun_prop

/--
theorem `condLExp_smul_le` / 定理 `condLExp_smul_le`

English:
theorem condLExp_smul_le
  given: (X : Ω -> Real>=0∞) {c : Real>=0∞}
  proof: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp [setLIntegral_condLExp_trim _ _ _ hs, lintegral_const_mul _ (measurable_condLExp _ P X),
    lintegral_const_mul_le]

中文:
定理 condLExp_smul_le
  条件: (X : Ω -> 实数>=0∞) {c : 实数>=0∞}
  证明: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp [setLIntegral_condLExp_trim _ _ _ hs, lintegral_const_mul _ (measurable_condLExp _ P X),
    lintegral_const_mul_le]

Depends on / 依赖: P.trim, SigmaFinite, ae_le_of_ae_le_trim, ae_le_of_forall_setLIntegral_le_of_sigmaFinite, condLExp_of_not_le, condLExp_of_not_sigmaFinite, filter_upwards, fun_prop, lintegral_const_mul, lintegral_const_mul_le, measurable_condLExp, setLIntegral_condLExp_trim, simp_rw
-/
theorem condLExp_smul_le (X : Ω -> Real>=0∞) {c : Real>=0∞} :
    c • P⁻[X|mΩ] <=ᵐ[P] P⁻[c • X|mΩ] := by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  apply ae_le_of_ae_le_trim
  apply ae_le_of_forall_setLIntegral_le_of_sigmaFinite (μ := P.trim hm) (by fun_prop)
  intro s hs _
  simp [setLIntegral_condLExp_trim _ _ _ hs, lintegral_const_mul _ (measurable_condLExp _ P X),
    lintegral_const_mul_le]

/--
theorem `condLExp_smul'` / 定理 `condLExp_smul'`

English:
theorem condLExp_smul'
  given: (X : Ω -> Real>=0∞) {c : Real>=0∞} (hc : c != ∞)
  proof: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul' _ _ hc]; rw [lintegral_const_mul' _ _ hc]; rw [setLIntegral_condLExp _ _ _ hs]

中文:
定理 condLExp_smul'
  条件: (X : Ω -> 实数>=0∞) {c : 实数>=0∞} (hc : c != ∞)
  证明: by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul' _ _ hc]; rw [lintegral_const_mul' _ _ hc]; rw [setLIntegral_condLExp _ _ _ hs]

Depends on / 依赖: P.trim, Pi.smul_apply, SigmaFinite, ae_eq_condLExp, condLExp_of_not_le, condLExp_of_not_sigmaFinite, fun_prop, lintegral_const_mul, setLIntegral_condLExp, smul_apply, smul_eq_mul
-/
theorem condLExp_smul' (X : Ω -> Real>=0∞) {c : Real>=0∞} (hc : c != ∞) :
    P⁻[c • X|mΩ] =ᵐ[P] c • P⁻[X|mΩ] := by
  by_cases hm : mΩ <= mΩ₀
  swap; · simp [condLExp_of_not_le hm]
  by_cases hσ : SigmaFinite (P.trim hm)
  swap; · simp [condLExp_of_not_sigmaFinite hm hσ]
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [Pi.smul_apply, smul_eq_mul]
  rw [lintegral_const_mul' _ _ hc]; rw [lintegral_const_mul' _ _ hc]; rw [setLIntegral_condLExp _ _ _ hs]

section Sum

variable {ι : Type*} (mΩ : MeasurableSpace Ω)

/--
theorem `condLExp_tsum` / 定理 `condLExp_tsum`

English:
theorem condLExp_tsum
  statement: [Countable ι] {X : ι -> Ω -> Real>=0∞}
  proof: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [ENNReal.tsum_apply]
  repeat rw [lintegral_tsum (by measurability)]
  congr with i
  exact setLIntegral_condLExp hm P (X i) hs

中文:
定理 condLExp_tsum
  结论: [可数 ι] {X : ι -> Ω -> 实数>=0∞}
  证明: by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [ENNReal.tsum_apply]
  repeat rw [lintegral_tsum (by measurability)]
  congr with i
  exact setLIntegral_condLExp hm P (X i) hs

Depends on / 依赖: ENNReal, ENNReal.tsum_apply, P.trim, SigmaFinite, ae_eq_condLExp, condLExp_of_not_le, condLExp_of_not_sigmaFinite, filter_upwards, fun_prop, lintegral_tsum, measurability, repeat, setLIntegral_condLExp, simp_rw, tsum_apply
-/
theorem condLExp_tsum [Countable ι] {X : ι -> Ω -> Real>=0∞}
    (hX : forall i, AEMeasurable[mΩ₀] (X i) P) :
    P⁻[∑' i, X i|mΩ] =ᵐ[P] ∑' i, P⁻[X i|mΩ] := by
  by_cases hm : mΩ <= mΩ₀; swap
  · simp_rw [condLExp_of_not_le hm]; filter_upwards; simp
  by_cases hσ : SigmaFinite (P.trim hm); swap
  · simp_rw [condLExp_of_not_sigmaFinite hm hσ]; filter_upwards; simp
  refine (ae_eq_condLExp _ _ _ (by fun_prop) ?_).symm
  intro s hs
  simp only [ENNReal.tsum_apply]
  repeat rw [lintegral_tsum (by measurability)]
  congr with i
  exact setLIntegral_condLExp hm P (X i) hs

/--
theorem `condLExp_finsetSum` / 定理 `condLExp_finsetSum`

English:
theorem condLExp_finsetSum
  statement: (s : Finset ι) {X : ι -> Ω -> Real>=0∞}
  proof: by
  convert! condLExp_tsum mΩ (fun i : s => hX i)
  · simp [Finset.sum_attach]
  · simp [Finset.sum_attach _ (f := (P⁻[X ·|mΩ]))]

@[deprecated (since := "2026-04-08")] alias condLExp_finset_sum := condLExp_finsetSum

中文:
定理 condLExp_finsetSum
  结论: (s : 有限集 ι) {X : ι -> Ω -> 实数>=0∞}
  证明: by
  convert! condLExp_tsum mΩ (fun i : s => hX i)
  · simp [Finset.sum_attach]
  · simp [Finset.sum_attach _ (f := (P⁻[X ·|mΩ]))]

@[deprecated (since := "2026-04-08")] alias condLExp_finset_sum := condLExp_finsetSum

Depends on / 依赖: Finset, Finset.sum_attach, condLExp_tsum, convert, sum_attach
-/
theorem condLExp_finsetSum (s : Finset ι) {X : ι -> Ω -> Real>=0∞}
    (hX : forall i, AEMeasurable[mΩ₀] (X i) P) :
    P⁻[∑ i in s, X i|mΩ] =ᵐ[P] ∑ i in s, P⁻[X i|mΩ] := by
  convert! condLExp_tsum mΩ (fun i : s => hX i)
  · simp [Finset.sum_attach]
  · simp [Finset.sum_attach _ (f := (P⁻[X ·|mΩ]))]

@[deprecated (since := "2026-04-08")] alias condLExp_finset_sum := condLExp_finsetSum

end Sum

end MeasureTheory
