...
/--
lemma `exists_polynomial_eval_sub_aux` / 引理 `exists_polynomial_eval_sub_aux`
...
lemma exists_polynomial_eval_sub_aux
    ...
    {E : Submodule R K} (a : ι -> K) (ha : forall i, a i in E) (f : E ->+ R) :
    exists r : K[X], forall i j, r.eval (a i - a j) =
...
/--
theorem `isNilpotent_derivedSeries_of_traceForm_eq_zero_aux` / 定理 `isNilpotent_derivedSeries_of_traceForm_eq_zero_aux`

English:
theorem isNilpotent_derivedSeries_of_traceForm_eq_zero_aux
  statement: {K : Type*}
  proof: by
  set φ := toEnd K L M
...
public theorem isNilpotent_derivedSeries_of_traceForm_eq_zero
  ...
  have nilp_ext : IsNilpotent (derivedSeries A (A otimes[R] L) 1) (A otimes[R] M) :=
isNilpotent_derivedSeries_of_traceForm_eq_zero_aux by simpa
  ...
  intro ⟨x, hx⟩
  have hx_ext : 1 otimesₜ[R] x in derivedSeries A (A otimes[R] L) 1 := by
  ...
  rw [← IsNilpotent.map_iff hbc_inj]; rw [aux]; rw [← toEnd_baseChange]
...
public theorem LieIdeal.isSolvable_of_killingForm_apply_lie_eq_zero (I : LieIdeal R L)
    (h : forall x in I, forall y in ⁅I, I⁆, killingForm R L x y = 0) :
    IsSolvable I := by
...
public lemma isSolvable_of_killingForm_apply_lie_eq_zero
    (h : forall x, forall y in derivedSeries R L 1, killingForm R L x y = 0) :
    IsSolvable L := by
...
example (A : Type*) [LieRing A] [Module.Finite Int A] [Module.Free Int A] :
    HasTrivialRadical Int A ↔ IsKilling Int A :=
  hasTrivialRadical_iff_isKilling Int A
