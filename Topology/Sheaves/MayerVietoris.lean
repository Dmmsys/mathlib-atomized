...
@[simps! toSquare]
/-
**Opens.mayerVietorisSquare'** 是 Mathlib 中的一个定义，位于命名空间 `Opens`。
形式化陈述：mayerVietorisSquare' (sq : Square (Opens T)) (h₄ : sq.X₄ = sq.X₂ ⊔ sq.X₃) 
(h₁ : sq.X₁ = sq.X₂ ⊓ sq.X₃) : (Opens.grothendieckTopology T).MayerVietorisSquar
e
参数：sq : Square (Opens T)；h₄ : sq.X₄ = sq.X₂ ⊔ sq.X₃；h₁ : sq.X₁ = sq.X₂ ⊓ sq.X₃。
该定义给出了上述对象。
...
/-
**Opens.mayerVietorisSquare** 是 Mathlib 中的一个定义，位于命名空间 `Opens`。
形式化陈述：mayerVietorisSquare (U V : Opens T) : (Opens.grothendieckTopology T).Mayer
VietorisSquare
参数：U V : Opens T。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
