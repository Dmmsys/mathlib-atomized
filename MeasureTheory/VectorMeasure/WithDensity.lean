...
variable {f g : α → E}
/-
**MeasureTheory.withDensity** 是 Mathlib 中的一个定理，位于命名空间 `MeasureTheory`。
...
/-
**MeasureTheory.Integrable.ae_eq_of_withDensity** 是 Mathlib 中的一个定理，位于命名空间 `Measu
reTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Having the same density implies the underlying functions are equal almost everyw
here.
-/
...
/-
**MeasureTheory.Integrable.withDensity** 是 Mathlib 中的一个定理，位于命名空间 `MeasureTheory`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
