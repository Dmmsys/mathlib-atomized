...
@[simps!]
/--
Definition of `fiberwiseColimitLimitIso` / `fiberwiseColimitLimitIso` 的定义

English:
definition fiberwiseColimitLimitIso
  signature: (K : J ⥤ Grothendieck F ⥤ H)
...
中文:
定义 fiberwiseColimitLimitIso
  签名: (K : J ⥤ Grothendieck F ⥤ H)
...
def fiberwiseColimitLimitIso (K : J ⥤ Grothendieck F ⥤ H)
    [forall (c : C), HasColimitsOfShape (↑(F.obj c)) H] [HasLimitsOfShape J H]
    [forall c, PreservesLimitsOfShape J (colim (J := F.obj c) (C := H))] :
...
variable (C) (F) in
/--
Instance `preservesLimitsOfShape_colim_grothendieck` / 实例 `preservesLimitsOfShape_colim_grothendieck`

English:
instance preservesLimitsOfShape_colim_grothendieck
  signature: [HasColimitsOfShape C H] [HasLimitsOfShape J H]
  body: by
  constructor
  intro K
...
instance preservesLimitsOfShape_colim_grothendieck [HasColimitsOfShape C H] [HasLimitsOfShape J H]
    [forall c, HasColimitsOfShape (↑(F.obj c)) H] [PreservesLimitsOfShape J (colim (J := C) (C := H))]
    [forall c, PreservesLimitsOfShape J (colim (J := F.obj c) (C := H))] :
