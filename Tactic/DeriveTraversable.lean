...
definition mkCasesOnMatch
  ...
  let lhss ← iinfo.ctors.mapM fun c
...
definition mkOneInstance
  ...
  -- incrementally build up target expression `(hp :
...
if ← withNewMCtxDepth isDefEq t v then
    ...
    mkAppOptM ``Traversable.traverse #[t.appFn!, inst, none, n
...
definition traverseConstructor
  ...
  let gargs := args'.filterMap (fun y => i
