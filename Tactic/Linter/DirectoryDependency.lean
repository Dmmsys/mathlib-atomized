...
.erase `Init .map (fun imp => imp.module)

-- XXX: is there a better location for these?
/--
Definition of `Lean.Name.findPrefix` / `Lean.Name.findPrefix` 的定义

English:
definition Lean.Name.findPrefix
  signature: {α} (f : Name -> Option α) (n : Name)
  body: do
f n > match n with
    | anonymous => none
...
/--
Definition of `Lean.Name.prefixes` / `Lean.Name.prefixes` 的定义

English:
definition Lean.Name.prefixes
  signature: (n : Name)
  body: NameSet.insert (n := n) match n with
    | anonymous => ∅
    | str n' _ => n'.prefixes
    | num n' _ => n'.prefixes

中文:
定义 Lean.Name.prefixes
  签名: (n : Name)
  定义体: NameSet.insert (n := n) match n with
    | anonymous => ∅
    | str n' _ => n'.prefixes
    | num n' _ => n'.prefixes

Depends on / 依赖: NameSet, NameSet.insert, anonymous, insert, prefixes
-/
...
/--
Definition of `Lean.Name.prefix?` / `Lean.Name.prefix?` 的定义

English:
definition Lean.Name.prefix?
  signature: (n : Name)
  body: match n with
    | anonymous => none
    | str n' _ => some n'
...
/--
Definition of `Lean.Name.collectPrefixes` / `Lean.Name.collectPrefixes` 的定义

English:
definition Lean.Name.collectPrefixes
  signature: (ns : Array Name)
  body: ns.foldl (fun ns n => ns.append n.prefixes) ∅
...
/--
Definition of `Lean.Name.prefixToName` / `Lean.Name.prefixToName` 的定义

English:
definition Lean.Name.prefixToName
  signature: (p : Name) (ns : Array Name)
  body: ns.find? p.isPrefixOf
...
/--
Definition of `NamePrefixRel` / `NamePrefixRel` 的定义

English:
definition NamePrefixRel
  body: NameMap NameSet

中文:
定义 NamePrefixRel
  定义体: NameMap NameSet

Depends on / 依赖: NameMap, NameSet
-/
...
English:
instance :
  signature: EmptyCollection NamePrefixRel
  body: inferInstanceAs (EmptyCollection (NameMap _))

中文:
实例 :
  签名: EmptyCollection NamePrefixRel
  定义体: inferInstanceAs (EmptyCollection (NameMap _))

Depends on / 依赖: EmptyCollection, NameMap
-/
...
English:
definition insert
  signature: (r : NamePrefixRel) (n₁ n₂ : Name)
  body: match r.find? n₁ with
    | some ns => NameMap.insert r n₁ (ns.insert n₂)
    | none => NameMap.insert r n₁ (.insert ∅ n₂)

中文:
定义 insert
  签名: (r : NamePrefixRel) (n₁ n₂ : Name)
  定义体: match r.find? n₁ with
    | some ns => NameMap.insert r n₁ (ns.insert n₂)
    | none => NameMap.insert r n₁ (.insert ∅ n₂)

Depends on / 依赖: NameMap, NameMap.insert, insert, ns.insert, r.find
-/
...
Depends on / 依赖: insert, r.insert, xs.foldl
-/
...
Depends on / 依赖: contains, findPrefix, ns.contains, r.find
-/
...
/--
Definition of `findAny` / `findAny` 的定义

English:
definition findAny
  signature: (r : NamePrefixRel) (n₁ : Name) (ns : Array Name)
  body: let prefixes := Lean.Name.collectPrefixes ns
  n₁.findPrefix fun n₁' => do
    let ns ← r.find? n₁'
...
Depends on / 依赖: Lean.Name.collectPrefixes, collectPrefixes, contains, findPrefix, ns.contains, prefixes, r.find, return
-/
...
/--
Definition of `containsKey` / `containsKey` 的定义

English:
definition containsKey
  signature: (r : NamePrefixRel) (n : Name)
  body: NameMap.contains r n

中文:
定义 containsKey
  签名: (r : NamePrefixRel) (n : Name)
  定义体: NameMap.contains r n

Depends on / 依赖: NameMap, NameMap.contains, contains
-/
...
definition contains
  signature: (r : NamePrefixRel) (n₁ n₂ : Name)
  body: (r.find n₁ n₂).isSome

中文:
定义 contains
  签名: (r : NamePrefixRel) (n₁ n₂ : Name)
  定义体: (r.find n₁ n₂).isSome

Depends on / 依赖: isSome, r.find
-/
def contains (r : NamePrefixRel) (n₁ n₂ : Name) : Bool := (r.find n₁ n₂).isSome

/--
Definition of `getAllLeft` / `getAllLeft` 的定义

English:
definition getAllLeft
  signature: (r : NamePrefixRel) (n : Name)
  body: Id.run do
...
/--
Definition of `allowedImportDirs` / `allowedImportDirs` 的定义

English:
definition allowedImportDirs
  signature: : NamePrefixRel
  body: .ofArray #[
...
/--
Definition of `forbiddenImportDirs` / `forbiddenImportDirs` 的定义

English:
definition forbiddenImportDirs
  signature: : NamePrefixRel
  body: .ofArray #[
...
/--
Definition of `overrideAllowedImportDirs` / `overrideAllowedImportDirs` 的定义

English:
definition overrideAllowedImportDirs
  signature: : NamePrefixRel
  body: .ofArray #[
...
/--
Definition of `checkBlocklist` / `checkBlocklist` 的定义

English:
definition checkBlocklist
  signature: (env : Environment) (mainModule : Name) (imports : Array Name)
  body: Id.run do
  ...
  | some (n₁, n₂) => do
    if let some imported := n₂.prefixToName imports then
      if !overrideAllowedImportDirs.contains mainModule imported then
        let mut msg := m!"Modules starting with {n₁} are not allowed to import 
