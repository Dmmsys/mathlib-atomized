/-
Copyright (c) 2026 Thomas Browning. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Thomas Browning
-/
module

public import Mathlib.Order.Filter.TendstoCofinite

/-!
# Northcott Functions

In number theory, the height function `h` satisfies the *Northcott property* that the sets
`{a | h a ≤ b}` are finite. This file extracts this notion as a typeclass and provides some API.

## Main definitions

* `Northcott h`: A function `h : α → β` is Northcott if the sets `{a : α | h a ≤ b}` are all finite.

## Main theorems

* `Northcott.exists_min_image h s hs`: If `h` is Northcott to a linear order, then `h` has an
  absolute minimum on every nonempty set `s`.

## References

* [D. Northcott, *An inequality in the theory of arithmetic on algebraic varieties*](northcott1949)
-/

@[expose] public noncomputable section

variable {α β γ : Type*} (h : α -> β) (h' : β -> γ)

/-- A function `h : α → β` is Northcott if the sets `{a : α | h a ≤ b}` are all finite. -/
@[mk_iff]
/--
Definition of `Northcott` / `Northcott` 的定义

English:
class Northcott
  parameters: [LE β]
  axioms and operations (1):
    - finite_le : forall b, {a : α | h a <= b}.Finite

中文:
类 Northcott
  参数: [LE β]
  公理与运算 (1 个):
    - finite_le : 对任意 b, {a : α | h a <= b}.Finite
-/
class Northcott [LE β] : Prop where
  finite_le : forall b, {a : α | h a <= b}.Finite

open Filter in
/--
theorem `northcott_iff_tendsto` / 定理 `northcott_iff_tendsto`

English:
theorem northcott_iff_tendsto
  given: [LinearOrder β] [NoMaxOrder β]
  proof: by
  simp_rw [northcott_iff, tendsto_atTop, eventually_cofinite, not_le]
  refine ⟨fun H b => (H b).subset fun x => le_of_lt, fun H b => ?_⟩
  obtain ⟨b', hc⟩ := exists_gt b
  exact (H b').subset fun x hx => lt_of_le_of_lt hx hc

中文:
定理 northcott_iff_tendsto
  条件: [LinearOrder β] [NoMaxOrder β]
  证明: by
  simp_rw [northcott_iff, tendsto_atTop, eventually_cofinite, not_le]
  refine ⟨fun H b => (H b).subset fun x => le_of_lt, fun H b => ?_⟩
  obtain ⟨b', hc⟩ := exists_gt b
  exact (H b').subset fun x hx => lt_of_le_of_lt hx hc

Depends on / 依赖: eventually_cofinite, exists_gt, le_of_lt, lt_of_le_of_lt, northcott_iff, not_le, simp_rw, subset, tendsto_atTop
-/
theorem northcott_iff_tendsto [LinearOrder β] [NoMaxOrder β] :
    Northcott h ↔ Tendsto h cofinite atTop := by
  simp_rw [northcott_iff, tendsto_atTop, eventually_cofinite, not_le]
  refine ⟨fun H b => (H b).subset fun x => le_of_lt, fun H b => ?_⟩
  obtain ⟨b', hc⟩ := exists_gt b
  exact (H b').subset fun x hx => lt_of_le_of_lt hx hc

namespace Northcott

/--
theorem `exists_min_image` / 定理 `exists_min_image`

English:
theorem exists_min_image
  given: [LinearOrder β] [Northcott h] (s : Set α) (hs : s.Nonempty)
  proof: by
  obtain ⟨a₁, h₁⟩ := hs
  obtain ⟨a₂, h₂, h₃⟩ := Set.exists_min_image ({a | h a <= h a₁} inter s) h
    ((finite_le (h a₁)).inter_of_left s) ⟨a₁, le_rfl, h₁⟩
  grind

中文:
定理 exists_min_image
  条件: [LinearOrder β] [Northcott h] (s : Set α) (hs : s.Nonempty)
  证明: by
  obtain ⟨a₁, h₁⟩ := hs
  obtain ⟨a₂, h₂, h₃⟩ := Set.exists_min_image ({a | h a <= h a₁} inter s) h
    ((finite_le (h a₁)).inter_of_left s) ⟨a₁, le_rfl, h₁⟩
  grind

Depends on / 依赖: Set.exists_min_image, exists_min_image, finite_le, inter_of_left, le_rfl
-/
theorem exists_min_image [LinearOrder β] [Northcott h] (s : Set α) (hs : s.Nonempty) :
    exists a in s, forall a' in s, h a <= h a' := by
  obtain ⟨a₁, h₁⟩ := hs
  obtain ⟨a₂, h₂, h₃⟩ := Set.exists_min_image ({a | h a <= h a₁} inter s) h
    ((finite_le (h a₁)).inter_of_left s) ⟨a₁, le_rfl, h₁⟩
  grind

/--
lemma `comp_of_bddAbove` / 引理 `comp_of_bddAbove`

English:
lemma comp_of_bddAbove
  given: [Preorder β] [LE γ] [Northcott h] (H : forall c, BddAbove (h' ⁻¹' {x | x <= c}))
  proof: by
    obtain ⟨b, hb⟩ := bddAbove_def.mp (H c)
exact (finite_le (h := h) b).subset by grind

中文:
引理 comp_of_bddAbove
  条件: [Preorder β] [LE γ] [Northcott h] (H : 对任意 c, BddAbove (h' ⁻¹' {x | x <= c}))
  证明: by
    obtain ⟨b, hb⟩ := bddAbove_def.mp (H c)
exact (finite_le (h := h) b).subset by grind

Depends on / 依赖: bddAbove_def, bddAbove_def.mp, finite_le, subset
-/
lemma comp_of_bddAbove [Preorder β] [LE γ] [Northcott h] (H : forall c, BddAbove (h' ⁻¹' {x | x <= c})) :
    Northcott (h' ∘ h) where
  finite_le c := by
    obtain ⟨b, hb⟩ := bddAbove_def.mp (H c)
exact (finite_le (h := h) b).subset by grind

/--
lemma `comp_of_finite_fibers` / 引理 `comp_of_finite_fibers`

English:
lemma comp_of_finite_fibers
  given: [LE γ] [Northcott h'] [Filter.TendstoCofinite h]
  proof: by
    refine Set.Finite.of_finite_fibers h ?_ fun x _ =>
      (Filter.TendstoCofinite.finite_preimage_singleton h x).inter_of_right _
exact (finite_le (h := h') c).subset by grind

中文:
引理 comp_of_finite_fibers
  条件: [LE γ] [Northcott h'] [Filter.TendstoCofinite h]
  证明: by
    refine Set.Finite.of_finite_fibers h ?_ fun x _ =>
      (Filter.TendstoCofinite.finite_preimage_singleton h x).inter_of_right _
exact (finite_le (h := h') c).subset by grind

Depends on / 依赖: Filter, Filter.TendstoCofinite.finite_preimage_singleton, Finite, Set.Finite.of_finite_fibers, TendstoCofinite, finite_le, finite_preimage_singleton, inter_of_right, of_finite_fibers, subset
-/
lemma comp_of_finite_fibers [LE γ] [Northcott h'] [Filter.TendstoCofinite h] :
    Northcott (h' ∘ h) where
  finite_le c := by
    refine Set.Finite.of_finite_fibers h ?_ fun x _ =>
      (Filter.TendstoCofinite.finite_preimage_singleton h x).inter_of_right _
exact (finite_le (h := h') c).subset by grind

end Northcott
