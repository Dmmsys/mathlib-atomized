...
中文:
类 Functor.ReflectsIsomorphisms
  参数: (F : C ⥤ D)
  公理与运算 (1 个):
    - reflects : 对任意 {A B : C} (f : A ⟶ B) [IsIso (F.map f)], IsIso f
-/
...
定理 isIso_of_reflects_iso
  结论: {A B : C} (f : A ⟶ B) (F : C ⥤ D) [IsIso (F.map f)]
...
引理 isIso_iff_of_reflects_iso
  条件: {A B : C} (f : A ⟶ B) (F : C ⥤ D) [F.ReflectsIsomorphisms]
...
中文:
引理 Functor.FullyFaithful.reflectsIsomorphisms
...
引理 reflectsIsomorphisms_of_iso
  条件: {F G : C ⥤ D} (α : F ≅ G) [F.ReflectsIsomorphisms]
  证明: by
    rw [← isIso_iff_of_reflects_iso _ F]; rw [← NatIso.naturality_2 α f]
    infer_instance
