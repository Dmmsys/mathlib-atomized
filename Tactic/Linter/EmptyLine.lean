...
/-
**Lean.Substring.Raw.getRange** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Substring.Raw`。
形式化陈述：Substring.Raw → Syntax.Range
...
partial
/-
**Lean.Syntax.filterMapM** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Syntax`。
形式化陈述：filterMapM {m : Type -> Type} [Monad m] {α} (stx : Syntax) (f : Syntax -> 
m (Option α)) : m (Array α)
参数：stx : Syntax；f : Syntax -> m (Option α)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Lean.Syntax.filterMap** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Syntax`。
形式化陈述：filterMap {α} (stx : Syntax) (f : Syntax -> Option α) : Array α
参数：stx : Syntax；f : Syntax -> Option α。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Lean.Syntax.filter** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Syntax`。
形式化陈述：filter (stx : Syntax) (f : Syntax -> Bool) : Array Syntax
参数：stx : Syntax；f : Syntax -> Bool。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.EmptyLine.AllowEmptyLines** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Li
nter.EmptyLine`。
形式化陈述：Std.HashSet SyntaxNodeKind
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.EmptyLine.SkippedFileSegments** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Linter.EmptyLine`。
形式化陈述：Std.HashSet Name
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If a file contains one of these names as segments, we disable the `emptyLine` li
nter.
-/
...
/-
**Mathlib.Linter.EmptyLine.emptyLineLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Li
nter.EmptyLine`。
形式化陈述：Linter
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
