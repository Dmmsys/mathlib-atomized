...
theorem fourier_deriv
  ...
  have : 𝓕 (deriv f) x = 𝓕 (fderiv Real f) x 1 := by
    simp only [fourier_continuousLinearMap_apply I, fderiv_a
...
theorem iteratedDeriv_fourier
  ...
  rw [iteratedDeriv]; rw [iteratedFDeriv_fourier A B hn]; rw [fourier_continuousM
