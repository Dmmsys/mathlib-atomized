/-
Copyright (c) 2015 Jeremy Avigad. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Jeremy Avigad, Robert Y. Lewis, Johannes Hölzl, Mario Carneiro, Sébastien Gouëzel
-/
module

public import Mathlib.Topology.MetricSpace.Pseudo.Lemmas
public import Mathlib.Topology.EMetricSpace.Basic

/-!
## Cauchy sequences in (pseudo-)metric spaces

Various results on Cauchy sequences in (pseudo-)metric spaces, including

* `Metric.complete_of_cauchySeq_tendsto` A pseudo-metric space is complete iff each Cauchy sequences
  converges to some limit point.
* `cauchySeq_bdd`: a Cauchy sequence on the natural numbers is bounded
* various characterisation of Cauchy and uniformly Cauchy sequences

## Tags

metric, pseudometric space, Cauchy sequence
-/

public section

open Filter
open scoped Uniformity Topology

universe u v w

variable {α : Type u} {β : Type v} {X ι : Type*}
variable [PseudoMetricSpace α]

/--
theorem `Metric.complete_of_convergent_controlled_sequences` / 定理 `Metric.complete_of_convergent_controlled_sequences`

English:
theorem Metric.complete_of_convergent_controlled_sequences
  statement: (B : Nat -> Real) (hB : forall n, 0 < B n)
  proof: UniformSpace.complete_of_convergent_controlled_sequences
    (fun n => { p : α × α | dist p.1 p.2 < B n }) (fun n => dist_mem_uniformity <| hB n) H

中文:
定理 Metric.complete_of_convergent_controlled_sequences
  结论: (B : 自然数 -> 实数) (hB : 对任意 n, 0 < B n)
  证明: UniformSpace.complete_of_convergent_controlled_sequences
    (fun n => { p : α × α | dist p.1 p.2 < B n }) (fun n => dist_mem_uniformity <| hB n) H

Depends on / 依赖: UniformSpace, UniformSpace.complete_of_convergent_controlled_sequences, complete_of_convergent_controlled_sequences, dist_mem_uniformity
-/
theorem Metric.complete_of_convergent_controlled_sequences (B : Nat -> Real) (hB : forall n, 0 < B n)
    (H : forall u : Nat -> α, (forall N n m : Nat, N <= n -> N <= m -> dist (u n) (u m) < B N) ->
      exists x, Tendsto u atTop (𝓝 x)) :
    CompleteSpace α :=
  UniformSpace.complete_of_convergent_controlled_sequences
    (fun n => { p : α × α | dist p.1 p.2 < B n }) (fun n => dist_mem_uniformity <| hB n) H

/--
theorem `Metric.complete_of_cauchySeq_tendsto` / 定理 `Metric.complete_of_cauchySeq_tendsto`

English:
theorem Metric.complete_of_cauchySeq_tendsto
  proof: EMetric.complete_of_cauchySeq_tendsto

中文:
定理 Metric.complete_of_cauchySeq_tendsto
  证明: EMetric.complete_of_cauchySeq_tendsto

Depends on / 依赖: EMetric, EMetric.complete_of_cauchySeq_tendsto, complete_of_cauchySeq_tendsto
-/
theorem Metric.complete_of_cauchySeq_tendsto :
    (forall u : Nat -> α, CauchySeq u -> exists a, Tendsto u atTop (𝓝 a)) -> CompleteSpace α :=
  EMetric.complete_of_cauchySeq_tendsto

section CauchySeq

variable [Nonempty β] [SemilatticeSup β]

/--
theorem `Metric.cauchySeq_iff` / 定理 `Metric.cauchySeq_iff`

English:
theorem Metric.cauchySeq_iff
  given: {u : β -> α}
  proof: uniformity_basis_dist.cauchySeq_iff

中文:
定理 Metric.cauchySeq_iff
  条件: {u : β -> α}
  证明: uniformity_basis_dist.cauchySeq_iff

Depends on / 依赖: cauchySeq_iff, uniformity_basis_dist, uniformity_basis_dist.cauchySeq_iff
-/
theorem Metric.cauchySeq_iff {u : β -> α} :
    CauchySeq u ↔ forall ε > 0, exists N, forall m >= N, forall n >= N, dist (u m) (u n) < ε :=
  uniformity_basis_dist.cauchySeq_iff

/--
theorem `Metric.cauchySeq_iff'` / 定理 `Metric.cauchySeq_iff'`

English:
theorem Metric.cauchySeq_iff'
  given: {u : β -> α}
  proof: uniformity_basis_dist.cauchySeq_iff'

中文:
定理 Metric.cauchySeq_iff'
  条件: {u : β -> α}
  证明: uniformity_basis_dist.cauchySeq_iff'

Depends on / 依赖: cauchySeq_iff, uniformity_basis_dist, uniformity_basis_dist.cauchySeq_iff
-/
theorem Metric.cauchySeq_iff' {u : β -> α} :
    CauchySeq u ↔ forall ε > 0, exists N, forall n >= N, dist (u n) (u N) < ε :=
  uniformity_basis_dist.cauchySeq_iff'

/--
theorem `Metric.uniformCauchySeqOn_iff` / 定理 `Metric.uniformCauchySeqOn_iff`

English:
theorem Metric.uniformCauchySeqOn_iff
  given: {γ : Type*} {F : β -> γ -> α} {s : Set γ}
  proof: by
  constructor
  · intro h ε hε
    let u := { a : α × α | dist a.fst a.snd < ε }
    have hu : u in 𝓤 α := Metric.mem_uniformity_dist.mpr ⟨ε, hε, by simp [u]⟩
    rw [← Filter.eventually_atTop_prod_self' (p := fun m =>
      forall x in s]; rw [dist (F m.fst x) (F m.snd x) < ε)]
    specialize h 

中文:
定理 Metric.uniformCauchySeqOn_iff
  条件: {γ : 类型} {F : β -> γ -> α} {s : Set γ}
  证明: by
  constructor
  · intro h ε hε
    let u := { a : α × α | dist a.fst a.snd < ε }
    have hu : u in 𝓤 α := Metric.mem_uniformity_dist.mpr ⟨ε, hε, by simp [u]⟩
    rw [← Filter.eventually_atTop_prod_self' (p := fun m =>
      forall x in s]; rw [dist (F m.fst x) (F m.snd x) < ε)]
    specialize h 

Depends on / 依赖: Filter, Filter.eventually_atTop_prod_self, Metric, Metric.mem_uniformity_dist.mp, Metric.mem_uniformity_dist.mpr, a.fst, a.snd, eventually_atTop, eventually_atTop_prod_self, h.mono, m.fst, m.snd, mem_uniformity_dist, prod_atTop_atTop_eq, specialize
-/
theorem Metric.uniformCauchySeqOn_iff {γ : Type*} {F : β -> γ -> α} {s : Set γ} :
    UniformCauchySeqOn F atTop s ↔ forall ε > (0 : Real),
      exists N : β, forall m >= N, forall n >= N, forall x in s, dist (F m x) (F n x) < ε := by
  constructor
  · intro h ε hε
    let u := { a : α × α | dist a.fst a.snd < ε }
    have hu : u in 𝓤 α := Metric.mem_uniformity_dist.mpr ⟨ε, hε, by simp [u]⟩
    rw [← Filter.eventually_atTop_prod_self' (p := fun m =>
      forall x in s]; rw [dist (F m.fst x) (F m.snd x) < ε)]
    specialize h u hu
    rw [prod_atTop_atTop_eq] at h
    exact h.mono fun n h x hx => h x hx
  · intro h u hu
    rcases Metric.mem_uniformity_dist.mp hu with ⟨ε, hε, hab⟩
    rcases h ε hε with ⟨N, hN⟩
    rw [prod_atTop_atTop_eq]; rw [eventually_atTop]
    use (N, N)
    intro b hb x hx
    rcases hb with ⟨hbl, hbr⟩
    exact hab (hN b.fst hbl.ge b.snd hbr.ge x hx)

/--
theorem `cauchySeq_of_le_tendsto_0'` / 定理 `cauchySeq_of_le_tendsto_0'`

English:
theorem cauchySeq_of_le_tendsto_0'
  statement: {s : β -> α} (b : β -> Real)
  proof: Metric.cauchySeq_iff'.2 fun ε ε0 => (h₀.eventually (gt_mem_nhds ε0)).exists.imp fun N hN n hn =>
    calc dist (s n) (s N) = dist (s N) (s n) := dist_comm _ _
    _ <= b N := h _ _ hn
    _ < ε := hN

中文:
定理 cauchySeq_of_le_tendsto_0'
  结论: {s : β -> α} (b : β -> 实数)
  证明: Metric.cauchySeq_iff'.2 fun ε ε0 => (h₀.eventually (gt_mem_nhds ε0)).exists.imp fun N hN n hn =>
    calc dist (s n) (s N) = dist (s N) (s n) := dist_comm _ _
    _ <= b N := h _ _ hn
    _ < ε := hN

Depends on / 依赖: Metric, Metric.cauchySeq_iff, cauchySeq_iff, dist_comm, eventually, exists.imp, gt_mem_nhds
-/
theorem cauchySeq_of_le_tendsto_0' {s : β -> α} (b : β -> Real)
    (h : forall n m : β, n <= m -> dist (s n) (s m) <= b n) (h₀ : Tendsto b atTop (𝓝 0)) : CauchySeq s :=
  Metric.cauchySeq_iff'.2 fun ε ε0 => (h₀.eventually (gt_mem_nhds ε0)).exists.imp fun N hN n hn =>
    calc dist (s n) (s N) = dist (s N) (s n) := dist_comm _ _
    _ <= b N := h _ _ hn
    _ < ε := hN

/--
theorem `cauchySeq_of_le_tendsto_0` / 定理 `cauchySeq_of_le_tendsto_0`

English:
theorem cauchySeq_of_le_tendsto_0
  statement: {s : β -> α} (b : β -> Real)
  proof: cauchySeq_of_le_tendsto_0' b (fun _n _m hnm => h _ _ _ le_rfl hnm) h₀

中文:
定理 cauchySeq_of_le_tendsto_0
  结论: {s : β -> α} (b : β -> 实数)
  证明: cauchySeq_of_le_tendsto_0' b (fun _n _m hnm => h _ _ _ le_rfl hnm) h₀

Depends on / 依赖: cauchySeq_of_le_tendsto_0, le_rfl
-/
theorem cauchySeq_of_le_tendsto_0 {s : β -> α} (b : β -> Real)
    (h : forall n m N : β, N <= n -> N <= m -> dist (s n) (s m) <= b N) (h₀ : Tendsto b atTop (𝓝 0)) :
    CauchySeq s :=
  cauchySeq_of_le_tendsto_0' b (fun _n _m hnm => h _ _ _ le_rfl hnm) h₀

/--
theorem `cauchySeq_bdd` / 定理 `cauchySeq_bdd`

English:
theorem cauchySeq_bdd
  given: {u : Nat -> α} (hu : CauchySeq u)
  statement: exists R > 0, forall m n, dist (u m) (u n) < R
  proof: by
  rcases Metric.cauchySeq_iff'.1 hu 1 zero_lt_one with ⟨N, hN⟩
  rsuffices ⟨R, R0, H⟩ : exists R > 0, forall n, dist (u n) (u N) < R
  · exact ⟨_, add_pos R0 R0, fun m n =>
      lt_of_le_of_lt (dist_triangle_right _ _ _) (add_lt_add (H m) (H n))⟩
  let R := Finset.sup (Finset.range N) fun n => n

中文:
定理 cauchySeq_bdd
  条件: {u : 自然数 -> α} (hu : CauchySeq u)
  结论: 存在 R > 0, 对任意 m n, dist (u m) (u n) < R
  证明: by
  rcases Metric.cauchySeq_iff'.1 hu 1 zero_lt_one with ⟨N, hN⟩
  rsuffices ⟨R, R0, H⟩ : exists R > 0, forall n, dist (u n) (u N) < R
  · exact ⟨_, add_pos R0 R0, fun m n =>
      lt_of_le_of_lt (dist_triangle_right _ _ _) (add_lt_add (H m) (H n))⟩
  let R := Finset.sup (Finset.range N) fun n => n

Depends on / 依赖: Finset, Finset.le_sup, Finset.range, Finset.sup, Metric, Metric.cauchySeq_iff, add_lt_add, add_pos, add_pos_of_nonneg_of_pos, cauchySeq_iff, dist_triangle_right, le_add_of_nonneg_left, le_or_gt, le_sup, lt_of_le_of_lt, lt_of_lt_of_le, nndist, rsuffices, zero_lt_one
-/
theorem cauchySeq_bdd {u : Nat -> α} (hu : CauchySeq u) : exists R > 0, forall m n, dist (u m) (u n) < R := by
  rcases Metric.cauchySeq_iff'.1 hu 1 zero_lt_one with ⟨N, hN⟩
  rsuffices ⟨R, R0, H⟩ : exists R > 0, forall n, dist (u n) (u N) < R
  · exact ⟨_, add_pos R0 R0, fun m n =>
      lt_of_le_of_lt (dist_triangle_right _ _ _) (add_lt_add (H m) (H n))⟩
  let R := Finset.sup (Finset.range N) fun n => nndist (u n) (u N)
  refine ⟨↑R + 1, add_pos_of_nonneg_of_pos R.2 zero_lt_one, fun n => ?_⟩
  rcases le_or_gt N n with h | h
  · exact lt_of_lt_of_le (hN _ h) (le_add_of_nonneg_left R.2)
  · have : _ <= R := Finset.le_sup (Finset.mem_range.2 h)
    exact lt_of_le_of_lt this (lt_add_of_pos_right _ zero_lt_one)

/--
theorem `cauchySeq_iff_le_tendsto_0` / 定理 `cauchySeq_iff_le_tendsto_0`

English:
theorem cauchySeq_iff_le_tendsto_0
  given: {s : Nat -> α}
  proof: ⟨fun hs => by
    /- `s` is a Cauchy sequence. The sequence `b` will be constructed by taking
      the supremum of the distances between `s n` and `s m` for `n m ≥ N`.
      First, we prove that all these distances are bounded, as otherwise the Sup
      would not make sense. -/
    let S N := (fun

中文:
定理 cauchySeq_iff_le_tendsto_0
  条件: {s : 自然数 -> α}
  证明: ⟨fun hs => by
    /- `s` is a Cauchy sequence. The sequence `b` will be constructed by taking
      the supremum of the distances between `s n` and `s m` for `n m ≥ N`.
      First, we prove that all these distances are bounded, as otherwise the Sup
      would not make sense. -/
    let S N := (fun
-/
theorem cauchySeq_iff_le_tendsto_0 {s : Nat -> α} :
    CauchySeq s ↔
      exists b : Nat -> Real,
        (forall n, 0 <= b n) ∧
          (forall n m N : Nat, N <= n -> N <= m -> dist (s n) (s m) <= b N) ∧ Tendsto b atTop (𝓝 0) :=
  ⟨fun hs => by
    /- `s` is a Cauchy sequence. The sequence `b` will be constructed by taking
      the supremum of the distances between `s n` and `s m` for `n m ≥ N`.
      First, we prove that all these distances are bounded, as otherwise the Sup
      would not make sense. -/
    let S N := (fun p : Nat × Nat => dist (s p.1) (s p.2)) '' { p | p.1 >= N ∧ p.2 >= N }
    have hS : forall N, exists x, forall y in S N, y <= x := by
      rcases cauchySeq_bdd hs with ⟨R, -, hR⟩
      refine fun N => ⟨R, ?_⟩
      rintro _ ⟨⟨m, n⟩, _, rfl⟩
      exact le_of_lt (hR m n)
    -- Prove that it bounds the distances of points in the Cauchy sequence
    have ub : forall m n N, N <= m -> N <= n -> dist (s m) (s n) <= sSup (S N) := fun m n N hm hn =>
      le_csSup (hS N) ⟨⟨_, _⟩, ⟨hm, hn⟩, rfl⟩
    have S0m : forall n, (0 : Real) in S n := fun n => ⟨⟨n, n⟩, ⟨le_rfl, le_rfl⟩, dist_self _⟩
    have S0 := fun n => le_csSup (hS n) (S0m n)
    -- Prove that it tends to `0`, by using the Cauchy property of `s`
    refine ⟨fun N => sSup (S N), S0, ub, Metric.tendsto_atTop.2 fun ε ε0 => ?_⟩
    refine (Metric.cauchySeq_iff.1 hs (ε / 2) (half_pos ε0)).imp fun N hN n hn => ?_
    rw [Real.dist_0_eq_abs]; rw [abs_of_nonneg (S0 n)]
    refine lt_of_le_of_lt (csSup_le ⟨_, S0m _⟩ ?_) (half_lt_self ε0)
    rintro _ ⟨⟨m', n'⟩, ⟨hm', hn'⟩, rfl⟩
    exact le_of_lt (hN _ (le_trans hn hm') _ (le_trans hn hn')),
   fun ⟨b, _, b_bound, b_lim⟩ => cauchySeq_of_le_tendsto_0 b b_bound b_lim⟩

/--
lemma `Metric.exists_subseq_bounded_of_cauchySeq` / 引理 `Metric.exists_subseq_bounded_of_cauchySeq`

English:
lemma Metric.exists_subseq_bounded_of_cauchySeq
  statement: (u : Nat -> α) (hu : CauchySeq u) (b : Nat -> Real)
  proof: by
  rw [cauchySeq_iff] at hu
  have hu' : forall k, forallᶠ (n : Nat) in atTop, forall m >= n, dist (u m) (u n) < b k := by
    intro k
    rw [eventually_atTop]
    obtain ⟨N, hN⟩ := hu (b k) (hb k)
    exact ⟨N, fun m hm r hr => hN r (hm.trans hr) m hm⟩
  exact Filter.extraction_forall_of_eventua

中文:
引理 Metric.exists_subseq_bounded_of_cauchySeq
  结论: (u : 自然数 -> α) (hu : CauchySeq u) (b : 自然数 -> 实数)
  证明: by
  rw [cauchySeq_iff] at hu
  have hu' : forall k, forallᶠ (n : Nat) in atTop, forall m >= n, dist (u m) (u n) < b k := by
    intro k
    rw [eventually_atTop]
    obtain ⟨N, hN⟩ := hu (b k) (hb k)
    exact ⟨N, fun m hm r hr => hN r (hm.trans hr) m hm⟩
  exact Filter.extraction_forall_of_eventua

Depends on / 依赖: Filter, Filter.extraction_forall_of_eventually, cauchySeq_iff, eventually_atTop, extraction_forall_of_eventually, hm.trans
-/
lemma Metric.exists_subseq_bounded_of_cauchySeq (u : Nat -> α) (hu : CauchySeq u) (b : Nat -> Real)
    (hb : forall n, 0 < b n) :
    exists f : Nat -> Nat, StrictMono f ∧ forall n, forall m >= f n, dist (u m) (u (f n)) < b n := by
  rw [cauchySeq_iff] at hu
  have hu' : forall k, forallᶠ (n : Nat) in atTop, forall m >= n, dist (u m) (u n) < b k := by
    intro k
    rw [eventually_atTop]
    obtain ⟨N, hN⟩ := hu (b k) (hb k)
    exact ⟨N, fun m hm r hr => hN r (hm.trans hr) m hm⟩
  exact Filter.extraction_forall_of_eventually hu'

end CauchySeq
