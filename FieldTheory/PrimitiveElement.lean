...
/--
theorem `exists_primitive_element_of_finite_top` / 定理 `exists_primitive_element_of_finite_top`

English:
theorem exists_primitive_element_of_finite_top
  given: [Finite E]
  statement: exists α : E, F⟮α⟯ = ⊤
  proof: by
  ...
  · obtain ⟨n, hn⟩ := Set.mem_range.mp (hα (Units.mk0 x hx))
    rw [show x = α ^ n by norm_cast; rw [hn]; rw [Units.val_mk0]]
    exact zpow_
...
Depends on / 依赖: IsCyclic, IsCyclic.exists_generator, Set.mem_range.mp, Units.mk0, Units.val_mk0, eq_top_iff, exists_generator, mem_adjoin_simple_self, mem_range, val_mk0, zero_mem, zpow_mem
-/
theorem exists_primitive_element_of_finite_top [Finite E] : exists α : E, F⟮α⟯ = ⊤ := by
  obtain ⟨α, hα⟩ := @IsCyclic.exists_generator Eˣ _ _
  use α
  rw [eq_top_iff]
  rintro x -
...
/--
theorem `exists_primitive_element_of_finite_bot` / 定理 `exists_primitive_element_of_finite_bot`

English:
theorem exists_primitive_element_of_finite_bot
  given: [Finite F] [FiniteDimensional F E]
  proof: haveI : Finite E := Module.finite_of_finite F
...
Depends on / 依赖: Finite, Module, Module.finite_of_finite, exists_primitive_element_of_finite_top, finite_of_finite
-/
...
/--
theorem `primitive_element_inf_aux_exists_c` / 定理 `primitive_element_inf_aux_exists_c`
...
Depends on / 依赖: Finset, Finset.mem_preimage, Infinite, Infinite.exists_notMem_finset, Multiset, Multiset.mem_bind, Multiset.mem_map, Multiset.mem_toFinset, classical, exists_notMem_finset, f.map, g.map, injective, mem_bind, mem_map, mem_preimage, mem_toFinset, preimage, s.preimage, sf.bind
-/
theorem primitive_element_inf_aux_exists_c (f g : F[X]) :
    exists c : F, forall α' in (f.map ϕ).roots, forall β' in (g.map ϕ).roots, -(α' - α) / (β' - β) != ϕ c := by
  let sf := (f.map ϕ).roots
  let sg := (g.map ϕ).roots
  classical
  let s := (sf.bind fun α' => sg.map fun β' => -(α' - α) / (β' - β)).toFinset
...
/--
theorem `primitive_element_inf_aux` / 定理 `primitive_element_inf_aux`

English:
theorem primitive_element_inf_aux
  given: [Algebra.IsSeparable F E]
  statement: exists γ : E, F⟮α, β⟯ = F⟮γ⟯
  proof: by
  ...
  obtain ⟨c, hc⟩ := primitive_element_inf_aux_exists_c (ιEE'.comp ιFE) 
...
Depends on / 依赖: Algebra, Algebra.IsSeparable.isIntegral, IsSeparable, SplittingField, add_sub_cancel_right, adjoin_le_iff, algebraMap, g.map, isIntegral, le_antisymm, mem_adj, minpoly, primitive_element_inf_aux_exists_c, sub_mem
-/
theorem primitive_element_inf_aux [Algebra.IsSeparable F E] : exists γ : E, F⟮α, β⟯ = F⟮γ⟯ := by
  have hα := Algebra.IsSeparable.isIntegral F α
  have hβ := Algebra.IsSeparable.isIntegral F β
  let f := minpoly F α
  let g := minpoly F β
  ...
  let γ := α + c • β
  suffices β_in_Fγ : β in F⟮γ⟯ by
    use γ
    apply le_antisymm
    · rw [adjoin_le_iff]
      have α_in_Fγ : α in F⟮γ⟯ := by
    ...
    · rw [adjoin_simple_le_iff]
      have α_in_Fαβ : α in F⟮α, β⟯ := subset_adjoin F {α, β} (Set.mem_insert α {β})
      have β_in_Fαβ : β in F⟮α, β⟯ := subset_adjoin F {α, β} (Set.mem_insert_of_mem α rfl)
  ...
  have h_roots : forall x in (h.map ιEE').roots, x = ιEE' β := by
    intro x hx
    rw [mem_roots_map h_ne_zero] at hx
    specialize hc (ιEE' γ - ιEE' (ιFE c) * x) (by
      have f_root := root_left_of_root_gcd hx
      rw [eval₂_comp]; rw [eval₂_sub]; rw [eval₂_mul]; rw [eval₂_C]; rw [eval₂_C]; rw [eval₂_X]; rw [eval₂_map] at f_root
...
/--
theorem `primitive_element_inf_aux_of_finite_intermediateField` / 定理 `primitive_element_inf_aux_of_finite_intermediateField`
...
theorem primitive_element_inf_aux_of_finite_intermediateField
  ...
  apply le_antisymm
  · rw [adjoin_le_iff]
    have αxβ_in_K : α + x • β in F⟮α + x • β⟯ := mem_adjoin_simple_self F _
    have αyβ_in_K : α + y • β in

中文:
定理 primitive_element_inf_aux_of_finite_intermediateField
  证明: by
  let f : F -> IntermediateField F E := fun x => F⟮α + x • β⟯
  obtain ⟨x, y, hneq, heq⟩ := Finite.exists_ne_map_eq_of_infinite f
  use α + x • β
...
private theorem primitive_element_inf_aux_of_finite_intermediateField
    [Finite (IntermediateField F E)] : exists γ : E, F⟮α, β⟯ = F⟮γ⟯ := by
  let f : F -> IntermediateField F E := fun x => F⟮α + x • β⟯
  obtain ⟨x, y, hneq, heq⟩ := Finite.exists_ne_map_eq_of_infinite f
  use α + x • β
  apply le_antisymm
  · rw [adjoin_le_iff]
    have αxβ_in_K : α + x • β in F⟮α + x • β⟯ := mem_adjoin_simple_self F _
    have αyβ_in_K : α + y • β in F⟮α + y • β⟯ := mem_adjoin_simple_self F _
    dsimp [f] at *
    ...
    rw [smul_smul]; rw [inv_mul_eq_div]; rw [div_self (sub_ne_zero.2 hneq)]; rw [one_smul] at β_in_K
    have α_in_K : α in F⟮α + x • β⟯ := by
...
/--
theorem `exists_primitive_element` / 定理 `exists_primitive_element`
...
theorem exists_primitive_element
  ...
  · let P : IntermediateField F E -> Prop := fun K => exists α : E, F⟮α⟯ = K
    have base : P ⊥ := ⟨0, adjoin_zero⟩
    have ih : forall (K : IntermediateField F E) (x : E), P K -> P (K⟮x⟯.restrictScalars F) := by
      intro K
...
Depends on / 依赖: F_finite, F_inf, Fintype, Infinite, IntermediateField, adjoin_simple_adjoin_simple, adjoin_zero, induction_on_adjoin, isEmpty_fintype, isEmpty_fintype.mp, isEmpty_or_nonempty, primitive_element_inf_aux, restrictScalars
-/
theorem exists_primitive_element : exists α : E, F⟮α⟯ = ⊤ := by
  rcases isEmpty_or_nonempty (Fintype F) with (F_inf | ⟨⟨F_finite⟩⟩)
  · let P : IntermediateField F E -> Prop := fun K => exists α : E, F⟮α⟯ = K
    have base : P ⊥ := ⟨0, adjoin_zero⟩
    have ih : forall (K : IntermediateField F E) (x : E), P K -> P (K⟮x⟯.restrictScalars F) := by
...
/--
Definition of `powerBasisOfFiniteOfSeparable` / `powerBasisOfFiniteOfSeparable` 的定义

English:
definition powerBasisOfFiniteOfSeparable
  signature: : PowerBasis F E
  body: let α := (exists_primitive_element F E).choose
...
中文:
定义 powerBasisOfFiniteOfSeparable
  签名: : PowerBasis F E
  定义体: let α := (exists_primitive_element F E).choose
  let pb := adjoin.powerBasis (Algebra.IsSeparable.isIntegral F α)
  have e : F⟮α⟯ = ⊤ := (exists_primitive_element F E).choose_spec
  pb.map ((IntermediateField.equivOfEq e).trans IntermediateField.topEquiv)

Depends on / 依赖: Algebra, Algebra.IsSeparable.isIntegral, IntermediateField, IntermediateField.equivOfEq, IntermediateField.topEquiv, IsSeparable, adjoin, adjoin.powerBasis, choose_spec, equivOfEq, exists_primitive_element, isIntegral, pb.map, powerBasis, topEquiv
-/
...
/--
theorem `isAlgebraic_of_adjoin_eq_adjoin` / 定理 `isAlgebraic_of_adjoin_eq_adjoin`

English:
theorem isAlgebraic_of_adjoin_eq_adjoin
  statement: {α : E} {m n : Nat} (hneq : m != n)
  proof: by
  wlog hmn : m < n
  ...
  · rw [hm] at heq hmn
    ...
    refine ⟨X ^ n - C y, X_pow_sub_C_ne_z

中文:
定理 isAlgebraic_of_adjoin_eq_adjoin
  结论: {α : E} {m n : 自然数} (hneq : m != n)
  证明: by
  wlog hmn : m < n
  · exact this F E hneq.symm heq.symm (hneq.lt_or_gt.resolve_left hmn)
  by_cases hm : m = 0
...
Depends on / 依赖: X_pow_sub_C_ne_zero, adjoin_one, aeval_C, aeval_X, heq.symm, hneq.lt_or_gt.resolve_left, hneq.symm, lt_or_gt, map_pow, map_sub, mem_adjoin_simple_iff, mem_adjoin_simple_self, mem_bot, pow_zero, resolve_left, sub_self
-/
theorem isAlgebraic_of_adjoin_eq_adjoin {α : E} {m n : Nat} (hneq : m != n)
  ...
  rw [eq_div_iff hzero]; rw [← sub_eq_zero] at h
...
/--
theorem `isAlgebraic_of_finite_intermediateField` / 定理 `isAlgebraic_of_finite_intermediateField`
...
中文:
定理 isAlgebraic_of_finite_intermediateField
  证明: ⟨fun α =>
  have ⟨_m, _n, hneq, heq⟩ := Finite.exists_ne_map_eq_of_infinite fun n => F⟮α ^ n⟯
  isAlgebraic_of_adjoin_eq_adjoin F E hneq heq⟩
-/
...
/--
theorem `FiniteDimensional.of_finite_intermediateField` / 定理 `FiniteDimensional.of_finite_intermediateField`
...
theorem FiniteDimensional.of_finite_intermediateField
  ...
  have : forall K : IF, FiniteDimensional F K.1 := fun ⟨_, x, rfl⟩ => adjoin.finiteDimensional
...
Depends on / 依赖: Algebra, Algebra.IsIntegral.isIntegral, FiniteDimensional, IntermediateField, IsIntegral, adjoin, adjoin.finiteDimensional, antisymm, finiteDi, finiteDimensional, finiteDimensional_iSup_of_finite, isAlgebraic_of_finite_intermediateField, isIntegral, le_iSup, le_top, le_top.antisymm, mem_adjoin_simple_self, toLinearEquiv, topEquiv, topEquiv.toLinearEquiv.finiteDi
-/
theorem FiniteDimensional.of_finite_intermediateField
...
/--
theorem `exists_primitive_element_of_finite_intermediateField` / 定理 `exists_primitive_element_of_finite_intermediateField`
...
Depends on / 依赖: FiniteDimensional, FiniteDimensional.of_finite_intermediateField, adjoin_simple_adjoin_simple, adjoin_zero, congr_arg, eq_comm, exists_primitive_element_of_finite_bot, finite_or_infinite, induction_on_adjoin, lift_adjoin_simple, lift_top, of_finite_intermediateField, primitive_element_inf_aux_of_finite_intermediateField, simp_rw
-/
theorem exists_primitive_element_of_finite_intermediateField
    [Finite (IntermediateField F E)] (K : IntermediateField F E) : exists α : E, F⟮α⟯ = K := by
  have := FiniteDimensional.of_finite_intermediateField F E
  rcases finite_or_infinite F with (_ | _)
  · obtain ⟨α, h⟩ := exists_primitive_element_of_finite_bot F K
    exact ⟨α, by simpa only [lift_adjoin_simple, lift_top] using congr_arg lift h⟩
  · apply induction_on_adjoin (fun K => exists α : E, F⟮α⟯ = K) ⟨0, adjoin_zero⟩
    rintro K β ⟨α, rfl⟩
    simp_rw [adjoin_simple_adjoin_simple, eq_comm]
    exact primitive_element_inf_aux_of_finite_intermediateField F α β

/--
theorem `FiniteDimensional.of_exists_primitive_element` / 定理 `FiniteDimensional.of_exists_primitive_element`

English:
theorem FiniteDimensional.of_exists_primitive_element
...
Depends on / 依赖: Algebra, Algebra.IsIntegral.isIntegral, IsIntegral, adjoin, adjoin.finiteDimensional, finiteDimensional, isIntegral, toLinearEquiv, topEquiv, topEquiv.toLinearEquiv.finiteDimensional
-/
theorem FiniteDimensional.of_exists_primitive_element [Algebra.IsAlgebraic F E]
    (h : exists α : E, F⟮α⟯ = ⊤) : FiniteDimensional F E := by
  obtain ⟨α, hprim⟩ := h
  have hfin := adjoin.finiteDimensional (Algebra.IsIntegral.isIntegral (R := F) α)
  rw [hprim] at hfin
  exact topEquiv.toLinearEquiv.finiteDimensional
...
/--
theorem `finite_intermediateField_of_exists_primitive_element` / 定理 `finite_intermediateField_of_exists_primitive_element`
...
Depends on / 依赖: FiniteDimensional, FiniteDimensional.of_exists_primitive_element, of_exists_primitive_element
-/
theorem finite_intermediateField_of_exists_primitive_element [Algebra.IsAlgebraic F E]
    (h : exists α : E, F⟮α⟯ = ⊤) : Finite (IntermediateField F E) := by
  have := FiniteDimensional.of_exists_primitive_element F E h
  obtain ⟨α, hprim⟩ := h
  -- Let `f` be the minimal polynomial of `α ∈ E` over `F`
  let f : F[X] := minpoly F α
...
/--
theorem `exists_primitive_element_iff_finite_intermediateField` / 定理 `exists_primitive_element_iff_finite_intermediateField`

English:
theorem exists_primitive_element_iff_finite_intermediateField
  proof: ⟨fun ⟨_, h⟩ => finite_intermediateField_of_exists_primitive_element F E h,
...
Depends on / 依赖: exists_primitive_element_of_finite_intermediateField, finite_intermediateField_of_exists_primitive_element, isAlgebraic_of_finite_intermediateField
-/
theorem exists_primitive_element_iff_finite_intermediateField :
    (Algebra.IsAlgebraic F E ∧ exists α : E, F⟮α⟯ = ⊤) ↔ Finite (IntermediateField F E) :=
...
/--
theorem `AlgHom.natCard_of_splits` / 定理 `AlgHom.natCard_of_splits`

English:
theorem AlgHom.natCard_of_splits
  statement: (L : Type*) [Field L] [Algebra F L]
  proof: (AlgHom.natCard_of_powerBasis (L := L) (Field.powerBasisOfFiniteOfSeparable F E)
...
Depends on / 依赖: AlgHom, AlgHom.natCard_of_powerBasis, Algebra, Algebra.IsSeparable.isSeparable, Field.powerBasisOfFiniteOfSeparable, IsSeparable, PowerBasis, PowerBasis.finrank, finrank, isSeparable, natCard_of_powerBasis, powerBasisOfFiniteOfSeparable
-/
theorem AlgHom.natCard_of_splits (L : Type*) [Field L] [Algebra F L]
    (hL : forall x : E, ((minpoly F x).map (algebraMap F L)).Splits) :
...
@[simp]
/--
theorem `AlgHom.card_of_splits` / 定理 `AlgHom.card_of_splits`

English:
theorem AlgHom.card_of_splits
  statement: (L : Type*) [Field L] [Algebra F L]
  proof: by
...
Depends on / 依赖: AlgHom, AlgHom.natCard_of_splits, Fintype, Fintype.card_eq_nat_card, card_eq_nat_card, natCard_of_splits
-/
...
@[simp]
/--
theorem `AlgHom.card` / 定理 `AlgHom.card`

English:
theorem AlgHom.card
  given: (K : Type*) [Field K] [IsAlgClosed K] [Algebra F K]
...
/--
theorem `primitive_element_iff_minpoly_natDegree_eq` / 定理 `primitive_element_iff_minpoly_natDegree_eq`

English:
theorem primitive_element_iff_minpoly_natDegree_eq
  given: (α : E)
  proof: by
  rw [← adjoin.finrank (IsIntegral.of_finite F α)]; rw [← finrank_top F E]
  refine ⟨fun h => ?_, fun h => eq_of_le_of_finrank_eq le_top h⟩
  exact congr_arg (fun K : IntermediateField F E => finrank F K) h

中文:
定理 primitive_element_iff_minpoly_natDegree_eq
  条件: (α : E)
  证明: by
  rw [← adjoin.finrank (IsIntegral.of_finite F α)]; rw [← finrank_top F E]
  refine ⟨fun h => ?_, fun h => eq_of_le_of_finrank_eq le_top h⟩
...
Depends on / 依赖: IntermediateField, IsIntegral, IsIntegral.of_finite, adjoin, adjoin.finrank, congr_arg, eq_of_le_of_finrank_eq, finrank, finrank_top, le_top, of_finite
-/
...
/--
theorem `primitive_element_iff_minpoly_degree_eq` / 定理 `primitive_element_iff_minpoly_degree_eq`

English:
theorem primitive_element_iff_minpoly_degree_eq
  given: (α : E)
  proof: by
  rw [degree_eq_iff_natDegree_eq]; rw [primitive_element_iff_minpoly_natDegree_eq]
  exact minpoly.ne_zero_of_finite F α

中文:
定理 primitive_element_iff_minpoly_degree_eq
  条件: (α : E)
  证明: by
  rw [degree_eq_iff_natDegree_eq]; rw [primitive_element_iff_minpoly_natDegree_eq]
  exact minpoly.ne_zero_of_finite F α

Depends on / 依赖: degree_eq_iff_natDegree_eq, minpoly, minpoly.ne_zero_of_finite, ne_zero_of_finite, primitive_element_iff_minpoly_natDegree_eq
-/
...
variable [Algebra.IsSeparable F E] (A : Type*) [Field A] [Algebra F A]
  (hA : forall x : E, ((minpoly F x).map (algebraMap F A)).Splits)
include hA

/--
theorem `primitive_element_iff_algHom_eq_of_eval'` / 定理 `primitive_element_iff_algHom_eq_of_eval'`
...
theorem primitive_element_iff_algHom_eq_of_eval'
  ...
  simp_rw [primitive_element_iff_minpoly_natDegree_eq, ← card_rootSet_eq_natDegree (K := A)
    ...
    ← AlgHom.card_of_splits F E A hA

中文:
定理 primitive_element_iff_algHom_eq_of_eval'
  条件: (α : E)
  证明: by
  classical
  simp_rw [primitive_element_iff_minpoly_natDegree_eq, ← card_rootSet_eq_natDegree (K := A)
...
Depends on / 依赖: AlgHom, AlgHom.card_of_splits, Algebra, Algebra.IsAlgebraic.of_finite, Algebra.IsSeparable.isSeparable, Finset, Finset.card_image_iff, Finset.coe_univ, Fintype, Fintype.card, IsAlgebraic, IsSeparable, card_image_iff, card_of_splits, card_rootSet_eq_natDegree, classical, coe_univ, injOn_univ, isSeparable, of_finite
-/
...
theorem primitive_element_iff_algHom_eq_of_eval
  ...
  rw [IntermediateField.finrank_top]; rw [← AlgHom.card_of_splits _ _ A]; rw [Fintype.card_eq_one_iff]
·
...
Depends on / 依赖: AdjoinSimple, AdjoinSimple.gen, AlgHom, AlgHom.card_of_splits, AlgHom.restrictScalars_injective, Algebra, Eq.symm, Field.primitive_element_iff_algHom_eq_of_eval, Fintype, Fintype.card_eq_one_iff, IntermediateField, IntermediateField.finrank_top, IsIntegral, IsIntegral.of_finite, card_eq_one_iff, card_of_splits, commutes, eq_of_le_of_finrank_eq, finrank_top, le_top
-/
theorem primitive_element_iff_algHom_eq_of_eval (α : E)
    (φ : E ->ₐ[F] A) : F⟮α⟯ = ⊤ ↔ forall ψ : E ->ₐ[F] A, φ α = ψ α -> φ = ψ := by
  refine ⟨fun h ψ hψ => (Field.primitive_element_iff_algHom_eq_of_eval' F A hA α).mp h hψ,
    fun h => eq_of_le_of_finrank_eq' le_top ?_⟩
  let : Algebra F⟮α⟯ A := (φ.comp F⟮α⟯.val).toAlgebra
  rw [IntermediateField.finrank_top]; rw [← AlgHom.card_of_splits _ _ A]; rw [Fintype.card_eq_one_iff]
