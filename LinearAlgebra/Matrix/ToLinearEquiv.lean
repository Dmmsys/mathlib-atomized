...
中文:
定义 toLinearEquiv'
  签名: (P : Matrix n n R) (_ : Invertible P)
...
定理 toLinearEquiv'_apply
  条件: (P : Matrix n n R) (h : Invertible P)
  证明: rfl
...
中文:
定义 toLinearEquiv
  签名: [DecidableEq n] (A : Matrix n n R) (hA : IsUnit A.det)
  定义体: toLin b b A
  toFun := toLin b b A
  invFun := toLin b b A⁻¹
  left_inv x := by
...
中文:
定理 ker_toLin_eq_bot
  条件: [DecidableEq n] (A : Matrix n n R) (hA : IsUnit A.det)
...
中文:
定理 exists_mulVec_eq_zero_iff_aux
  结论: {K : 类型} [DecidableEq n] [Field K]
  证明: by
  constructor
  · rintro ⟨v, hv, mul_eq⟩
    contrapose! hv
...
中文:
定理 exists_mulVec_eq_zero_iff'
  结论: {A : 类型} (K : 类型) [DecidableEq n] [CommRing A]
  证明: by
  have : (exists v != 0, (algebraMap A K).mapMatrix M *ᵥ v = 0) ↔ _ :=
    exists_mulVec_eq_zero_iff_aux
  rw [← RingHom.map_det]; rw [IsFractionRing.to_map_eq_zero_iff] at this
...
定理 nondegenerate_iff_separatingLeft
  条件: [Finite n]
  结论: M.Nondegenerate ↔ M.SeparatingLeft
  证明: by
  classical
  have := Fintype.ofFinite n
  rw [nondegenerate_iff_det_ne_zero]; rw [separatingLeft_iff_det_ne_zero]
...
定理 nondegenerate_iff_separatingRight
  条件: [Finite n]
  结论: M.Nondegenerate ↔ M.SeparatingRight
  证明: by
  classical
  have := Fintype.ofFinite n
  rw [nondegenerate_iff_det_ne_zero]; rw [separatingRight_iff_det_ne_zero]
...
中文:
定理 Nondegenerate.mul_iff_right
  条件: (h : N.Nondegenerate)
  证明: by
  classical
  simp only [nondegenerate_iff_det_ne_zero, det_mul] at h ⊢
  exact mul_ne_zero_iff_right h
...
中文:
定理 Nondegenerate.smul_iff
  条件: [Finite n] {t : A} (h : t != 0)
  证明: by
  have := Fintype.ofFinite
  rw [nondegenerate_def]; rw [nondegenerate_def]
  simp [smul_mulVec, mul_eq_zero_iff_left h]
