/-
Copyright (c) 2022 Sébastien Gouëzel. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Sébastien Gouëzel
-/
module

public import Mathlib.Analysis.SpecialFunctions.Gaussian.GaussianIntegral
public import Mathlib.Analysis.Complex.CauchyIntegral
public import Mathlib.MeasureTheory.Integral.Pi
public import Mathlib.Analysis.Fourier.FourierTransform

/-!
# Fourier transform of the Gaussian

We prove that the Fourier transform of the Gaussian function is another Gaussian:

* `integral_cexp_quadratic`: general formula for `∫ (x : ℝ), exp (b * x ^ 2 + c * x + d)`
* `fourierIntegral_gaussian`: for all complex `b` and `t` with `0 < re b`, we have
  `∫ x:ℝ, exp (I * t * x) * exp (-b * x^2) = (π / b) ^ (1 / 2) * exp (-t ^ 2 / (4 * b))`.
* `fourierIntegral_gaussian_pi`: a variant with `b` and `t` scaled to give a more symmetric
  statement, and formulated in terms of the Fourier transform operator `𝓕`.

We also give versions of these formulas in finite-dimensional inner product spaces, see
`integral_cexp_neg_mul_sq_norm_add` and `fourierIntegral_gaussian_innerProductSpace`.

-/

@[expose] public section

/-!
## Fourier integral of Gaussian functions
-/

open Real Set MeasureTheory Filter Asymptotics intervalIntegral

open scoped Real Topology FourierTransform RealInnerProductSpace

open Complex hiding exp continuous_exp

noncomputable section

namespace GaussianFourier

variable {b : Complex}

/--
Definition of `verticalIntegral` / `verticalIntegral` 的定义

English:
definition verticalIntegral
  signature: (b : Complex) (c T : Real)
  body: ∫ y : Real in (0 : Real)..c, I * (cexp (-b * (T + y * I) ^ 2) - cexp (-b * (T - y * I) ^ 2))

中文:
定义 verticalIntegral
  签名: (b : Complex) (c T : 实数)
  定义体: ∫ y : Real in (0 : Real)..c, I * (cexp (-b * (T + y * I) ^ 2) - cexp (-b * (T - y * I) ^ 2))
-/
def verticalIntegral (b : Complex) (c T : Real) : Complex :=
  ∫ y : Real in (0 : Real)..c, I * (cexp (-b * (T + y * I) ^ 2) - cexp (-b * (T - y * I) ^ 2))

/--
theorem `norm_cexp_neg_mul_sq_add_mul_I` / 定理 `norm_cexp_neg_mul_sq_add_mul_I`

English:
theorem norm_cexp_neg_mul_sq_add_mul_I
  given: (b : Complex) (c T : Real)
  proof: by
  rw [Complex.norm_exp]; rw [neg_mul]; rw [neg_re]; rw [← re_add_im b]
  simp only [sq, re_add_im, mul_re, mul_im, add_re, add_im, ofReal_re, ofReal_im, I_re, I_im]
  ring_nf

中文:
定理 norm_cexp_neg_mul_sq_add_mul_I
  条件: (b : Complex) (c T : 实数)
  证明: by
  rw [Complex.norm_exp]; rw [neg_mul]; rw [neg_re]; rw [← re_add_im b]
  simp only [sq, re_add_im, mul_re, mul_im, add_re, add_im, ofReal_re, ofReal_im, I_re, I_im]
  ring_nf

Depends on / 依赖: Complex.norm_exp, I_im, I_re, add_im, add_re, mul_im, mul_re, neg_mul, neg_re, norm_exp, ofReal_im, ofReal_re, re_add_im, ring_nf
-/
theorem norm_cexp_neg_mul_sq_add_mul_I (b : Complex) (c T : Real) :
    ‖cexp (-b * (T + c * I) ^ 2)‖ = exp (-(b.re * T ^ 2 - 2 * b.im * c * T - b.re * c ^ 2)) := by
  rw [Complex.norm_exp]; rw [neg_mul]; rw [neg_re]; rw [← re_add_im b]
  simp only [sq, re_add_im, mul_re, mul_im, add_re, add_im, ofReal_re, ofReal_im, I_re, I_im]
  ring_nf

/--
theorem `norm_cexp_neg_mul_sq_add_mul_I'` / 定理 `norm_cexp_neg_mul_sq_add_mul_I'`

English:
theorem norm_cexp_neg_mul_sq_add_mul_I'
  given: (hb : b.re != 0) (c T : Real)
  proof: by
  have :
    b.re * T ^ 2 - 2 * b.im * c * T - b.re * c ^ 2 =
      b.re * (T - b.im * c / b.re) ^ 2 - c ^ 2 * (b.im ^ 2 / b.re + b.re) := by
    field
  rw [norm_cexp_neg_mul_sq_add_mul_I]; rw [this]

中文:
定理 norm_cexp_neg_mul_sq_add_mul_I'
  条件: (hb : b.re != 0) (c T : 实数)
  证明: by
  have :
    b.re * T ^ 2 - 2 * b.im * c * T - b.re * c ^ 2 =
      b.re * (T - b.im * c / b.re) ^ 2 - c ^ 2 * (b.im ^ 2 / b.re + b.re) := by
    field
  rw [norm_cexp_neg_mul_sq_add_mul_I]; rw [this]

Depends on / 依赖: b.im, b.re, norm_cexp_neg_mul_sq_add_mul_I
-/
theorem norm_cexp_neg_mul_sq_add_mul_I' (hb : b.re != 0) (c T : Real) :
    ‖cexp (-b * (T + c * I) ^ 2)‖ =
      exp (-(b.re * (T - b.im * c / b.re) ^ 2 - c ^ 2 * (b.im ^ 2 / b.re + b.re))) := by
  have :
    b.re * T ^ 2 - 2 * b.im * c * T - b.re * c ^ 2 =
      b.re * (T - b.im * c / b.re) ^ 2 - c ^ 2 * (b.im ^ 2 / b.re + b.re) := by
    field
  rw [norm_cexp_neg_mul_sq_add_mul_I]; rw [this]

/--
theorem `verticalIntegral_norm_le` / 定理 `verticalIntegral_norm_le`

English:
theorem verticalIntegral_norm_le
  given: (hb : 0 < b.re) (c : Real) {T : Real} (hT : 0 <= T)
  proof: by
  -- first get uniform bound for integrand
  have vert_norm_bound :
    forall {T : Real},
      0 <= T ->
        forall {c y : Real},
          |y| <= |c| ->
            ‖cexp (-b * (T + y * I) ^ 2)‖ <=
              exp (-(b.re * T ^ 2 - (2 : Real) * |b.im| * |c| * T - b.re * c ^ 2)) := by
   

中文:
定理 verticalIntegral_norm_le
  条件: (hb : 0 < b.re) (c : 实数) {T : 实数} (hT : 0 <= T)
  证明: by
  -- first get uniform bound for integrand
  have vert_norm_bound :
    forall {T : Real},
      0 <= T ->
        forall {c y : Real},
          |y| <= |c| ->
            ‖cexp (-b * (T + y * I) ^ 2)‖ <=
              exp (-(b.re * T ^ 2 - (2 : Real) * |b.im| * |c| * T - b.re * c ^ 2)) := by
   
-/
theorem verticalIntegral_norm_le (hb : 0 < b.re) (c : Real) {T : Real} (hT : 0 <= T) :
    ‖verticalIntegral b c T‖ <=
      (2 : Real) * |c| * exp (-(b.re * T ^ 2 - (2 : Real) * |b.im| * |c| * T - b.re * c ^ 2)) := by
  -- first get uniform bound for integrand
  have vert_norm_bound :
    forall {T : Real},
      0 <= T ->
        forall {c y : Real},
          |y| <= |c| ->
            ‖cexp (-b * (T + y * I) ^ 2)‖ <=
              exp (-(b.re * T ^ 2 - (2 : Real) * |b.im| * |c| * T - b.re * c ^ 2)) := by
    intro T hT c y hy
    rw [norm_cexp_neg_mul_sq_add_mul_I b]
    gcongr exp (-(_ - ?_ * _ - _ * ?_))
    · (conv_lhs => rw [mul_assoc]); (conv_rhs => rw [mul_assoc])
      gcongr _ * ?_
      refine (le_abs_self _).trans ?_
      rw [abs_mul]
      gcongr
    · rwa [sq_le_sq]
  -- now main proof
  apply (intervalIntegral.norm_integral_le_of_norm_le_const _).trans
  · rw [sub_zero]
    conv_lhs => simp only [mul_comm _ |c|]
    conv_rhs =>
      conv =>
        congr
        rw [mul_comm]
      rw [mul_assoc]
  · intro y hy
    have absy : |y| <= |c| := by
      simpa using abs_sub_left_of_mem_uIcc (uIoc_subset_uIcc hy)
    rw [norm_mul]; rw [norm_I]; rw [one_mul]; rw [two_mul]
    refine (norm_sub_le _ _).trans (add_le_add (vert_norm_bound hT absy) ?_)
    rw [← abs_neg y] at absy
    simpa only [neg_mul, ofReal_neg] using! vert_norm_bound hT absy

/--
theorem `tendsto_verticalIntegral` / 定理 `tendsto_verticalIntegral`

English:
theorem tendsto_verticalIntegral
  given: (hb : 0 < b.re) (c : Real)
  proof: by
  -- complete proof using squeeze theorem:
  rw [tendsto_zero_iff_norm_tendsto_zero]
  refine
    tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds ?_
      (Eventually.of_forall fun _ => norm_nonneg _)
      ((eventually_ge_atTop (0 : Real)).mp
        (Eventually.of_forall fun T hT 

中文:
定理 tendsto_verticalIntegral
  条件: (hb : 0 < b.re) (c : 实数)
  证明: by
  -- complete proof using squeeze theorem:
  rw [tendsto_zero_iff_norm_tendsto_zero]
  refine
    tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds ?_
      (Eventually.of_forall fun _ => norm_nonneg _)
      ((eventually_ge_atTop (0 : Real)).mp
        (Eventually.of_forall fun T hT 
-/
theorem tendsto_verticalIntegral (hb : 0 < b.re) (c : Real) :
    Tendsto (verticalIntegral b c) atTop (𝓝 0) := by
  -- complete proof using squeeze theorem:
  rw [tendsto_zero_iff_norm_tendsto_zero]
  refine
    tendsto_of_tendsto_of_tendsto_of_le_of_le' tendsto_const_nhds ?_
      (Eventually.of_forall fun _ => norm_nonneg _)
      ((eventually_ge_atTop (0 : Real)).mp
        (Eventually.of_forall fun T hT => verticalIntegral_norm_le hb c hT))
  rw [(by ring : 0 = 2 * |c| * 0)]
  refine (tendsto_exp_atBot.comp (tendsto_neg_atTop_atBot.comp ?_)).const_mul _
  apply tendsto_atTop_add_const_right
  simp_rw [sq, ← mul_assoc, ← sub_mul]
  refine Tendsto.atTop_mul_atTop₀ (tendsto_atTop_add_const_right _ _ ?_) tendsto_id
  exact (tendsto_const_mul_atTop_of_pos hb).mpr tendsto_id

/--
theorem `integrable_cexp_neg_mul_sq_add_real_mul_I` / 定理 `integrable_cexp_neg_mul_sq_add_real_mul_I`

English:
theorem integrable_cexp_neg_mul_sq_add_real_mul_I
  given: (hb : 0 < b.re) (c : Real)
  proof: by
  refine ⟨by fun_prop, ?_⟩
  rw [← hasFiniteIntegral_norm_iff]
  simp_rw [norm_cexp_neg_mul_sq_add_mul_I' hb.ne', neg_sub _ (c ^ 2 * _),
    sub_eq_add_neg _ (b.re * _), Real.exp_add]
  suffices Integrable fun x : Real => exp (-(b.re * x ^ 2)) by
    exact (Integrable.comp_sub_right this (b.im * 

中文:
定理 integrable_cexp_neg_mul_sq_add_real_mul_I
  条件: (hb : 0 < b.re) (c : 实数)
  证明: by
  refine ⟨by fun_prop, ?_⟩
  rw [← hasFiniteIntegral_norm_iff]
  simp_rw [norm_cexp_neg_mul_sq_add_mul_I' hb.ne', neg_sub _ (c ^ 2 * _),
    sub_eq_add_neg _ (b.re * _), Real.exp_add]
  suffices Integrable fun x : Real => exp (-(b.re * x ^ 2)) by
    exact (Integrable.comp_sub_right this (b.im * 

Depends on / 依赖: Integrable, Integrable.comp_sub_right, Real.exp_add, b.im, b.re, comp_sub_right, const_mul, exp_add, fun_prop, hasFiniteIntegral, hasFiniteIntegral.const_mul, hasFiniteIntegral_norm_iff, hb.ne, integrable_exp_neg_mul_sq, neg_mul, neg_sub, norm_cexp_neg_mul_sq_add_mul_I, simp_rw, sub_eq_add_neg
-/
theorem integrable_cexp_neg_mul_sq_add_real_mul_I (hb : 0 < b.re) (c : Real) :
    Integrable fun x : Real => cexp (-b * (x + c * I) ^ 2) := by
  refine ⟨by fun_prop, ?_⟩
  rw [← hasFiniteIntegral_norm_iff]
  simp_rw [norm_cexp_neg_mul_sq_add_mul_I' hb.ne', neg_sub _ (c ^ 2 * _),
    sub_eq_add_neg _ (b.re * _), Real.exp_add]
  suffices Integrable fun x : Real => exp (-(b.re * x ^ 2)) by
    exact (Integrable.comp_sub_right this (b.im * c / b.re)).hasFiniteIntegral.const_mul _
  simp_rw [← neg_mul]
  apply integrable_exp_neg_mul_sq hb

/--
theorem `integral_cexp_neg_mul_sq_add_real_mul_I` / 定理 `integral_cexp_neg_mul_sq_add_real_mul_I`

English:
theorem integral_cexp_neg_mul_sq_add_real_mul_I
  given: (hb : 0 < b.re) (c : Real)
  proof: by
  refine
    tendsto_nhds_unique
      (intervalIntegral_tendsto_integral (integrable_cexp_neg_mul_sq_add_real_mul_I hb c)
        tendsto_neg_atTop_atBot tendsto_id)
      ?_
  set I₁ := fun T => ∫ x : Real in -T..T, cexp (-b * (x + c * I) ^ 2) with HI₁
  let I₂ := fun T : Real => ∫ x : Real in 

中文:
定理 integral_cexp_neg_mul_sq_add_real_mul_I
  条件: (hb : 0 < b.re) (c : 实数)
  证明: by
  refine
    tendsto_nhds_unique
      (intervalIntegral_tendsto_integral (integrable_cexp_neg_mul_sq_add_real_mul_I hb c)
        tendsto_neg_atTop_atBot tendsto_id)
      ?_
  set I₁ := fun T => ∫ x : Real in -T..T, cexp (-b * (x + c * I) ^ 2) with HI₁
  let I₂ := fun T : Real => ∫ x : Real in 

Depends on / 依赖: integrable_cexp_neg_mul_sq_add_real_mul_I, intervalIntegral_tendsto_integral, tendsto_id, tendsto_neg_atTop_atBot, tendsto_nhds_unique
-/
theorem integral_cexp_neg_mul_sq_add_real_mul_I (hb : 0 < b.re) (c : Real) :
    ∫ x : Real, cexp (-b * (x + c * I) ^ 2) = (π / b) ^ (1 / 2 : Complex) := by
  refine
    tendsto_nhds_unique
      (intervalIntegral_tendsto_integral (integrable_cexp_neg_mul_sq_add_real_mul_I hb c)
        tendsto_neg_atTop_atBot tendsto_id)
      ?_
  set I₁ := fun T => ∫ x : Real in -T..T, cexp (-b * (x + c * I) ^ 2) with HI₁
  let I₂ := fun T : Real => ∫ x : Real in -T..T, cexp (-b * (x : Complex) ^ 2)
  let I₄ := fun T : Real => ∫ y : Real in (0 : Real)..c, cexp (-b * (T + y * I) ^ 2)
  let I₅ := fun T : Real => ∫ y : Real in (0 : Real)..c, cexp (-b * (-T + y * I) ^ 2)
  have C : forall T : Real, I₂ T - I₁ T + I * I₄ T - I * I₅ T = 0 := by
    intro T
    have :=
      integral_boundary_rect_eq_zero_of_differentiableOn (fun z => cexp (-b * z ^ 2)) (-T)
        (T + c * I)
        (by
          refine Differentiable.differentiableOn (Differentiable.const_mul ?_ _).cexp
          exact differentiable_pow 2)
    simpa only [neg_im, ofReal_im, neg_zero, ofReal_zero, zero_mul, add_zero, neg_re,
      ofReal_re, add_re, mul_re, I_re, mul_zero, I_im, tsub_zero, add_im, mul_im,
      mul_one, zero_add, smul_eq_mul, ofReal_neg] using this
  simp_rw [id, ← HI₁]
  have : I₁ = fun T : Real => I₂ T + verticalIntegral b c T := by
    ext1 T
    specialize C T
    rw [sub_eq_zero] at C
    unfold verticalIntegral
    rw [intervalIntegral.integral_const_mul]; rw [intervalIntegral.integral_sub]
    · simp_rw [(fun a b => by rw [sq]; ring_nf : forall a b : Complex, (a - b * I) ^ 2 = (-a + b * I) ^ 2)]
      change I₁ T = I₂ T + I * (I₄ T - I₅ T)
      rw [mul_sub]; rw [← C]
      abel
    all_goals apply Continuous.intervalIntegrable; fun_prop
  rw [this]; rw [← add_zero ((π / b : Complex) ^ (1 / 2 : Complex))]; rw [← integral_gaussian_complex hb]
  refine Tendsto.add ?_ (tendsto_verticalIntegral hb c)
  exact
    intervalIntegral_tendsto_integral (integrable_cexp_neg_mul_sq hb) tendsto_neg_atTop_atBot
      tendsto_id

/--
theorem `_root_.integral_cexp_quadratic` / 定理 `_root_.integral_cexp_quadratic`

English:
theorem _root_.integral_cexp_quadratic
  given: (hb : b.re < 0) (c d : Complex)
  proof: by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h (x : Real) : cexp (b * x ^ 2 + c * x + d) =
      cexp (- -b * (x + c / (2 * b)) ^ 2) * cexp (d - c ^ 2 / (4 * b)) := by
    simp_rw [← Complex.exp_add]
    congr 1
    field
  simp_rw [h, MeasureTheory.integral_mul_const]
  rw [

中文:
定理 _root_.integral_cexp_quadratic
  条件: (hb : b.re < 0) (c d : Complex)
  证明: by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h (x : Real) : cexp (b * x ^ 2 + c * x + d) =
      cexp (- -b * (x + c / (2 * b)) ^ 2) * cexp (d - c ^ 2 / (4 * b)) := by
    simp_rw [← Complex.exp_add]
    congr 1
    field
  simp_rw [h, MeasureTheory.integral_mul_const]
  rw [

Depends on / 依赖: Complex.exp_add, MeasureTheory, MeasureTheory.integral_mul_const, add_assoc, contrapose, exp_add, integral_add_right_eq_self, integral_cexp_neg_mul_sq_add_real_mul_I, integral_mul_const, neg_re, ofReal_add, re_add_im, simp_rw, zero_re
-/
theorem _root_.integral_cexp_quadratic (hb : b.re < 0) (c d : Complex) :
    ∫ x : Real,
      cexp (b * x ^ 2 + c * x + d) = (π / -b) ^ (1 / 2 : Complex) * cexp (d - c ^ 2 / (4 * b)) := by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h (x : Real) : cexp (b * x ^ 2 + c * x + d) =
      cexp (- -b * (x + c / (2 * b)) ^ 2) * cexp (d - c ^ 2 / (4 * b)) := by
    simp_rw [← Complex.exp_add]
    congr 1
    field
  simp_rw [h, MeasureTheory.integral_mul_const]
  rw [← re_add_im (c / (2 * b))]
  simp_rw [← add_assoc, ← ofReal_add]
  rw [integral_add_right_eq_self fun a : Real => cexp (- -b * (↑a + ↑(c / (2 * b)).im * I) ^ 2)]; rw [integral_cexp_neg_mul_sq_add_real_mul_I ((neg_re b).symm ▸ (neg_pos.mpr hb))]

/--
lemma `_root_.integrable_cexp_quadratic'` / 引理 `_root_.integrable_cexp_quadratic'`

English:
lemma _root_.integrable_cexp_quadratic'
  given: (hb : b.re < 0) (c d : Complex)
  proof: by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  by_contra H
  simpa [hb', pi_ne_zero, Complex.exp_ne_zero, integral_undef H]
    using integral_cexp_quadratic hb c d

中文:
引理 _root_.integrable_cexp_quadratic'
  条件: (hb : b.re < 0) (c d : Complex)
  证明: by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  by_contra H
  simpa [hb', pi_ne_zero, Complex.exp_ne_zero, integral_undef H]
    using integral_cexp_quadratic hb c d

Depends on / 依赖: Complex.exp_ne_zero, contrapose, exp_ne_zero, integral_cexp_quadratic, integral_undef, pi_ne_zero, zero_re
-/
lemma _root_.integrable_cexp_quadratic' (hb : b.re < 0) (c d : Complex) :
    Integrable (fun (x : Real) => cexp (b * x ^ 2 + c * x + d)) := by
  have hb' : b != 0 := by contrapose! hb; rw [hb, zero_re]
  by_contra H
  simpa [hb', pi_ne_zero, Complex.exp_ne_zero, integral_undef H]
    using integral_cexp_quadratic hb c d

/--
lemma `_root_.integrable_cexp_quadratic` / 引理 `_root_.integrable_cexp_quadratic`

English:
lemma _root_.integrable_cexp_quadratic
  given: (hb : 0 < b.re) (c d : Complex)
  proof: by
  have : (-b).re < 0 := by simpa using hb
  exact integrable_cexp_quadratic' this c d

中文:
引理 _root_.integrable_cexp_quadratic
  条件: (hb : 0 < b.re) (c d : Complex)
  证明: by
  have : (-b).re < 0 := by simpa using hb
  exact integrable_cexp_quadratic' this c d

Depends on / 依赖: integrable_cexp_quadratic
-/
lemma _root_.integrable_cexp_quadratic (hb : 0 < b.re) (c d : Complex) :
    Integrable (fun (x : Real) => cexp (-b * x ^ 2 + c * x + d)) := by
  have : (-b).re < 0 := by simpa using hb
  exact integrable_cexp_quadratic' this c d

/--
theorem `_root_.fourierIntegral_gaussian` / 定理 `_root_.fourierIntegral_gaussian`

English:
theorem _root_.fourierIntegral_gaussian
  given: (hb : 0 < b.re) (t : Complex)
  proof: by
  conv => enter [1, 2, x]; rw [← Complex.exp_add, add_comm, ← add_zero (-b * x ^ 2 + I * t * x)]
  rw [integral_cexp_quadratic (show (-b).re < 0 by rwa [neg_re]; rw [neg_lt_zero]), neg_neg, zero_sub,
    mul_neg, div_neg, neg_neg, mul_pow, I_sq, neg_one_mul, mul_comm]

中文:
定理 _root_.fourierIntegral_gaussian
  条件: (hb : 0 < b.re) (t : Complex)
  证明: by
  conv => enter [1, 2, x]; rw [← Complex.exp_add, add_comm, ← add_zero (-b * x ^ 2 + I * t * x)]
  rw [integral_cexp_quadratic (show (-b).re < 0 by rwa [neg_re]; rw [neg_lt_zero]), neg_neg, zero_sub,
    mul_neg, div_neg, neg_neg, mul_pow, I_sq, neg_one_mul, mul_comm]

Depends on / 依赖: Complex.exp_add, I_sq, add_comm, add_zero, div_neg, exp_add, integral_cexp_quadratic, mul_comm, mul_neg, mul_pow, neg_lt_zero, neg_neg, neg_one_mul, neg_re, zero_sub
-/
theorem _root_.fourierIntegral_gaussian (hb : 0 < b.re) (t : Complex) :
    ∫ x : Real, cexp (I * t * x) * cexp (-b * x ^ 2) =
    (π / b) ^ (1 / 2 : Complex) * cexp (-t ^ 2 / (4 * b)) := by
  conv => enter [1, 2, x]; rw [← Complex.exp_add, add_comm, ← add_zero (-b * x ^ 2 + I * t * x)]
  rw [integral_cexp_quadratic (show (-b).re < 0 by rwa [neg_re]; rw [neg_lt_zero]), neg_neg, zero_sub,
    mul_neg, div_neg, neg_neg, mul_pow, I_sq, neg_one_mul, mul_comm]

/--
theorem `_root_.fourier_gaussian_pi'` / 定理 `_root_.fourier_gaussian_pi'`

English:
theorem _root_.fourier_gaussian_pi'
  given: (hb : 0 < b.re) (c : Complex)
  proof: by
  have : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h : (-↑π * b).re < 0 := by
    simpa only [neg_mul, neg_re, re_ofReal_mul, neg_lt_zero] using mul_pos pi_pos hb
  ext1 t
  simp_rw [fourier_real_eq_integral_exp_smul, smul_eq_mul, ← Complex.exp_add, ← add_assoc]
  have (x : Real) : ↑(-

中文:
定理 _root_.fourier_gaussian_pi'
  条件: (hb : 0 < b.re) (c : Complex)
  证明: by
  have : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h : (-↑π * b).re < 0 := by
    simpa only [neg_mul, neg_re, re_ofReal_mul, neg_lt_zero] using mul_pos pi_pos hb
  ext1 t
  simp_rw [fourier_real_eq_integral_exp_smul, smul_eq_mul, ← Complex.exp_add, ← add_assoc]
  have (x : Real) : ↑(-

Depends on / 依赖: Complex.exp_add, add_assoc, contrapose, div_div, exp_add, fourier_real_eq_integral_exp_smul, integral_cexp_quadratic, mul_pos, neg_lt_zero, neg_mul, neg_neg, neg_re, pi_pos, re_ofReal_mul, simp_rw, smul_eq_mul, zero_re
-/
theorem _root_.fourier_gaussian_pi' (hb : 0 < b.re) (c : Complex) :
    (𝓕 fun x : Real => cexp (-π * b * x ^ 2 + 2 * π * c * x)) = fun t : Real =>
    1 / b ^ (1 / 2 : Complex) * cexp (-π / b * (t + I * c) ^ 2) := by
  have : b != 0 := by contrapose! hb; rw [hb, zero_re]
  have h : (-↑π * b).re < 0 := by
    simpa only [neg_mul, neg_re, re_ofReal_mul, neg_lt_zero] using mul_pos pi_pos hb
  ext1 t
  simp_rw [fourier_real_eq_integral_exp_smul, smul_eq_mul, ← Complex.exp_add, ← add_assoc]
  have (x : Real) : ↑(-2 * π * x * t) * I + -π * b * x ^ 2 + 2 * π * c * x =
    -π * b * x ^ 2 + (-2 * π * I * t + 2 * π * c) * x + 0 := by push_cast; ring
  simp_rw [this, integral_cexp_quadratic h, neg_mul, neg_neg]
  congr 2
  · rw [← div_div, div_self <| ofReal_ne_zero.mpr pi_ne_zero, one_div, inv_cpow, ← one_div]
    rw [Ne]; rw [arg_eq_pi_iff]; rw [not_and_or]; rw [not_lt]
    exact Or.inl hb.le
  · field_simp
    ring_nf
    simp only [I_sq]
    ring

/--
theorem `_root_.fourier_gaussian_pi` / 定理 `_root_.fourier_gaussian_pi`

English:
theorem _root_.fourier_gaussian_pi
  given: (hb : 0 < b.re)
  proof: by
  simpa only [mul_zero, zero_mul, add_zero] using fourier_gaussian_pi' hb 0

中文:
定理 _root_.fourier_gaussian_pi
  条件: (hb : 0 < b.re)
  证明: by
  simpa only [mul_zero, zero_mul, add_zero] using fourier_gaussian_pi' hb 0

Depends on / 依赖: add_zero, fourier_gaussian_pi, mul_zero, zero_mul
-/
theorem _root_.fourier_gaussian_pi (hb : 0 < b.re) :
    (𝓕 fun (x : Real) => cexp (-π * b * x ^ 2)) =
    fun t : Real => 1 / b ^ (1 / 2 : Complex) * cexp (-π / b * t ^ 2) := by
  simpa only [mul_zero, zero_mul, add_zero] using fourier_gaussian_pi' hb 0

section InnerProductSpace

variable {V : Type*} [NormedAddCommGroup V] [InnerProductSpace Real V] [FiniteDimensional Real V]
  [MeasurableSpace V] [BorelSpace V]

/--
theorem `integrable_cexp_neg_sum_mul_add` / 定理 `integrable_cexp_neg_sum_mul_add`

English:
theorem integrable_cexp_neg_sum_mul_add
  statement: {ι : Type*} [Fintype ι] {b : ι -> Complex}
  proof: by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  apply Integrable.fintype_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v)) (fun i => ?_)
  convert! integrable_cexp_quadratic (hb i) (c i) 0 using 3 with x
  simp only [add_zero]

中文:
定理 integrable_cexp_neg_sum_mul_add
  结论: {ι : 类型} [Fintype ι] {b : ι -> Complex}
  证明: by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  apply Integrable.fintype_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v)) (fun i => ?_)
  convert! integrable_cexp_quadratic (hb i) (c i) 0 using 3 with x
  simp only [add_zero]

Depends on / 依赖: Complex.exp_sum, Finset, Finset.sum_add_distrib, Finset.sum_neg_distrib, Integrable, Integrable.fintype_prod, add_zero, convert, exp_sum, fintype_prod, integrable_cexp_quadratic, neg_mul, simp_rw, sum_add_distrib, sum_neg_distrib
-/
theorem integrable_cexp_neg_sum_mul_add {ι : Type*} [Fintype ι] {b : ι -> Complex}
    (hb : forall i, 0 < (b i).re) (c : ι -> Complex) :
    Integrable (fun (v : ι -> Real) => cexp (-∑ i, b i * (v i : Complex) ^ 2 + ∑ i, c i * v i)) := by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  apply Integrable.fintype_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v)) (fun i => ?_)
  convert! integrable_cexp_quadratic (hb i) (c i) 0 using 3 with x
  simp only [add_zero]

/--
theorem `integrable_cexp_neg_mul_sum_add` / 定理 `integrable_cexp_neg_mul_sum_add`

English:
theorem integrable_cexp_neg_mul_sum_add
  given: {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex)
  proof: by
  simp_rw [neg_mul, Finset.mul_sum]
  exact integrable_cexp_neg_sum_mul_add (fun _ => hb) c

中文:
定理 integrable_cexp_neg_mul_sum_add
  条件: {ι : 类型} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex)
  证明: by
  simp_rw [neg_mul, Finset.mul_sum]
  exact integrable_cexp_neg_sum_mul_add (fun _ => hb) c

Depends on / 依赖: Finset, Finset.mul_sum, integrable_cexp_neg_sum_mul_add, mul_sum, neg_mul, simp_rw
-/
theorem integrable_cexp_neg_mul_sum_add {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex) :
    Integrable (fun (v : ι -> Real) => cexp (-b * ∑ i, (v i : Complex) ^ 2 + ∑ i, c i * v i)) := by
  simp_rw [neg_mul, Finset.mul_sum]
  exact integrable_cexp_neg_sum_mul_add (fun _ => hb) c

/--
theorem `integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace` / 定理 `integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace`

English:
theorem integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  proof: by
  rw [← (PiLp.volume_preserving_toLp ι).integrable_comp_emb
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul, Function.comp_def]
  convert! integrable_cexp_neg_mul_sum_add hb (fun i => c * w i) using 3 with v
  simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, PiLp.i

中文:
定理 integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  证明: by
  rw [← (PiLp.volume_preserving_toLp ι).integrable_comp_emb
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul, Function.comp_def]
  convert! integrable_cexp_neg_mul_sum_add hb (fun i => c * w i) using 3 with v
  simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, PiLp.i

Depends on / 依赖: EuclideanSpace, EuclideanSpace.norm_eq, Finset, Finset.mul_sum, Finset.sum_neg_distrib, Finset.sum_nonneg, Function, Function.comp_def, MeasurableEquiv, MeasurableEquiv.toLp, PiLp.inner_apply, PiLp.volume_preserving_toLp, RCLike, RCLike.inner_apply, comp_def, conj_trivial, convert, inner_apply, integrable_cexp_neg_mul_sum_add, integrable_comp_emb
-/
theorem integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace
    {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : Complex) (w : EuclideanSpace Real ι) :
    Integrable (fun (v : EuclideanSpace Real ι) => cexp (- b * ‖v‖ ^ 2 + c * ⟪w, v⟫)) := by
  rw [← (PiLp.volume_preserving_toLp ι).integrable_comp_emb
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul, Function.comp_def]
  convert! integrable_cexp_neg_mul_sum_add hb (fun i => c * w i) using 3 with v
  simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, PiLp.inner_apply, RCLike.inner_apply,
    conj_trivial, ofReal_sum, ofReal_mul, Finset.mul_sum, neg_mul, Finset.sum_neg_distrib,
    mul_assoc]
  norm_cast
  rw [sq_sqrt]
  · simp [Finset.mul_sum, mul_comm]
  · exact Finset.sum_nonneg (fun i _hi => by positivity)

/--
theorem `integrable_cexp_neg_mul_sq_norm_add` / 定理 `integrable_cexp_neg_mul_sq_norm_add`

English:
theorem integrable_cexp_neg_mul_sq_norm_add
  given: (hb : 0 < b.re) (c : Complex) (w : V)
  proof: by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integrable_comp_emb e.toHomeomorph.measurableEmbedding]
  convert! integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) with v
  simp only [neg_mul, Function.comp_apply, LinearIsometryEquiv.norm_map,
  

中文:
定理 integrable_cexp_neg_mul_sq_norm_add
  条件: (hb : 0 < b.re) (c : Complex) (w : V)
  证明: by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integrable_comp_emb e.toHomeomorph.measurableEmbedding]
  convert! integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) with v
  simp only [neg_mul, Function.comp_apply, LinearIsometryEquiv.norm_map,
  

Depends on / 依赖: Function, Function.comp_apply, LinearIsometryEquiv, LinearIsometryEquiv.inner_map_eq_flip, LinearIsometryEquiv.norm_map, LinearIsometryEquiv.symm_symm, comp_apply, convert, e.measurePreserving.integrable_comp_emb, e.symm, e.toHomeomorph.measurableEmbedding, inner_map_eq_flip, integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace, integrable_comp_emb, measurableEmbedding, measurePreserving, neg_mul, norm_map, repr.symm, stdOrthonormalBasis
-/
theorem integrable_cexp_neg_mul_sq_norm_add (hb : 0 < b.re) (c : Complex) (w : V) :
    Integrable (fun (v : V) => cexp (-b * ‖v‖ ^ 2 + c * ⟪w, v⟫)) := by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integrable_comp_emb e.toHomeomorph.measurableEmbedding]
  convert! integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) with v
  simp only [neg_mul, Function.comp_apply, LinearIsometryEquiv.norm_map,
    LinearIsometryEquiv.symm_symm,
    LinearIsometryEquiv.inner_map_eq_flip]

/--
theorem `integral_cexp_neg_sum_mul_add` / 定理 `integral_cexp_neg_sum_mul_add`

English:
theorem integral_cexp_neg_sum_mul_add
  statement: {ι : Type*} [Fintype ι] {b : ι -> Complex}
  proof: by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  rw [integral_fintype_prod_volume_eq_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v))]
  congr with i
  have : (-b i).re < 0 := by simpa using hb i
  convert! integral_cexp_quadratic this (c i

中文:
定理 integral_cexp_neg_sum_mul_add
  结论: {ι : 类型} [Fintype ι] {b : ι -> Complex}
  证明: by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  rw [integral_fintype_prod_volume_eq_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v))]
  congr with i
  have : (-b i).re < 0 := by simpa using hb i
  convert! integral_cexp_quadratic this (c i

Depends on / 依赖: Complex.exp_sum, Finset, Finset.sum_add_distrib, Finset.sum_neg_distrib, convert, div_neg, exp_sum, integral_cexp_quadratic, integral_fintype_prod_volume_eq_prod, neg_mul, simp_rw, sum_add_distrib, sum_neg_distrib
-/
theorem integral_cexp_neg_sum_mul_add {ι : Type*} [Fintype ι] {b : ι -> Complex}
    (hb : forall i, 0 < (b i).re) (c : ι -> Complex) :
    ∫ v : ι -> Real, cexp (-∑ i, b i * (v i : Complex) ^ 2 + ∑ i, c i * v i)
      = ∏ i, (π / b i) ^ (1 / 2 : Complex) * cexp (c i ^ 2 / (4 * b i)) := by
  simp_rw [← Finset.sum_neg_distrib, ← Finset.sum_add_distrib, Complex.exp_sum, ← neg_mul]
  rw [integral_fintype_prod_volume_eq_prod (f := fun i (v : Real) => cexp (-b i * v ^ 2 + c i * v))]
  congr with i
  have : (-b i).re < 0 := by simpa using hb i
  convert! integral_cexp_quadratic this (c i) 0 using 1 <;> simp [div_neg]

/--
theorem `integral_cexp_neg_mul_sum_add` / 定理 `integral_cexp_neg_mul_sum_add`

English:
theorem integral_cexp_neg_mul_sum_add
  given: {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex)
  proof: by
  simp_rw [neg_mul, Finset.mul_sum, integral_cexp_neg_sum_mul_add (fun _ => hb) c, one_div,
    Finset.prod_mul_distrib, Finset.prod_const, ← cpow_nat_mul, ← Complex.exp_sum, Fintype.card,
    Finset.sum_div, div_eq_mul_inv]

中文:
定理 integral_cexp_neg_mul_sum_add
  条件: {ι : 类型} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex)
  证明: by
  simp_rw [neg_mul, Finset.mul_sum, integral_cexp_neg_sum_mul_add (fun _ => hb) c, one_div,
    Finset.prod_mul_distrib, Finset.prod_const, ← cpow_nat_mul, ← Complex.exp_sum, Fintype.card,
    Finset.sum_div, div_eq_mul_inv]

Depends on / 依赖: Complex.exp_sum, Finset, Finset.mul_sum, Finset.prod_const, Finset.prod_mul_distrib, Finset.sum_div, Fintype, Fintype.card, cpow_nat_mul, div_eq_mul_inv, exp_sum, integral_cexp_neg_sum_mul_add, mul_sum, neg_mul, one_div, prod_const, prod_mul_distrib, simp_rw, sum_div
-/
theorem integral_cexp_neg_mul_sum_add {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : ι -> Complex) :
    ∫ v : ι -> Real, cexp (-b * ∑ i, (v i : Complex) ^ 2 + ∑ i, c i * v i)
      = (π / b) ^ (Fintype.card ι / 2 : Complex) * cexp ((∑ i, c i ^ 2) / (4 * b)) := by
  simp_rw [neg_mul, Finset.mul_sum, integral_cexp_neg_sum_mul_add (fun _ => hb) c, one_div,
    Finset.prod_mul_distrib, Finset.prod_const, ← cpow_nat_mul, ← Complex.exp_sum, Fintype.card,
    Finset.sum_div, div_eq_mul_inv]

/--
theorem `integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace` / 定理 `integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace`

English:
theorem integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  proof: by
  rw [← (PiLp.volume_preserving_toLp ι).integral_comp
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul]
  convert! integral_cexp_neg_mul_sum_add hb (fun i => c * w i) using 5 with _x y
  · simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, neg_mul, neg_inj, mul_eq_mul

中文:
定理 integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  证明: by
  rw [← (PiLp.volume_preserving_toLp ι).integral_comp
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul]
  convert! integral_cexp_neg_mul_sum_add hb (fun i => c * w i) using 5 with _x y
  · simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, neg_mul, neg_inj, mul_eq_mul

Depends on / 依赖: EuclideanSpace, EuclideanSpace.norm_eq, Finset, Finset.mul_sum, Finset.sum_nonneg, MeasurableEquiv, MeasurableEquiv.toLp, PiLp.inner_apply, PiLp.volume_preserving_toLp, RCLike, RCLike.inner_apply, conj_trivial, convert, inner_apply, integral_cexp_neg_mul_sum_add, integral_comp, measurableEmbedding, mul_assoc, mul_eq_mul_left_iff, mul_sum
-/
theorem integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace
    {ι : Type*} [Fintype ι] (hb : 0 < b.re) (c : Complex) (w : EuclideanSpace Real ι) :
    ∫ v : EuclideanSpace Real ι, cexp (- b * ‖v‖ ^ 2 + c * ⟪w, v⟫) =
      (π / b) ^ (Fintype.card ι / 2 : Complex) * cexp (c ^ 2 * ‖w‖ ^ 2 / (4 * b)) := by
  rw [← (PiLp.volume_preserving_toLp ι).integral_comp
    (MeasurableEquiv.toLp 2 _).measurableEmbedding]
  simp only [neg_mul]
  convert! integral_cexp_neg_mul_sum_add hb (fun i => c * w i) using 5 with _x y
  · simp only [EuclideanSpace.norm_eq, norm_eq_abs, sq_abs, neg_mul, neg_inj, mul_eq_mul_left_iff]
    norm_cast
    left
    rw [sq_sqrt]
    exact Finset.sum_nonneg (fun i _hi => by positivity)
  · simp only [PiLp.inner_apply, RCLike.inner_apply, conj_trivial, ofReal_sum, ofReal_mul,
      Finset.mul_sum, mul_assoc]
    simp_rw [mul_comm]
  · simp only [EuclideanSpace.norm_eq, Real.norm_eq_abs, sq_abs, mul_pow, ← Finset.mul_sum]
    congr
    norm_cast
    rw [sq_sqrt]
    exact Finset.sum_nonneg (fun i _hi => by positivity)

/--
theorem `integral_cexp_neg_mul_sq_norm_add` / 定理 `integral_cexp_neg_mul_sq_norm_add`

English:
theorem integral_cexp_neg_mul_sq_norm_add
  proof: by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integral_comp e.toHomeomorph.measurableEmbedding]
  convert! integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) <;>
    simp [LinearIsometryEquiv.inner_map_eq_flip]

中文:
定理 integral_cexp_neg_mul_sq_norm_add
  证明: by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integral_comp e.toHomeomorph.measurableEmbedding]
  convert! integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) <;>
    simp [LinearIsometryEquiv.inner_map_eq_flip]

Depends on / 依赖: LinearIsometryEquiv, LinearIsometryEquiv.inner_map_eq_flip, convert, e.measurePreserving.integral_comp, e.symm, e.toHomeomorph.measurableEmbedding, inner_map_eq_flip, integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace, integral_comp, measurableEmbedding, measurePreserving, repr.symm, stdOrthonormalBasis, toHomeomorph
-/
theorem integral_cexp_neg_mul_sq_norm_add
    (hb : 0 < b.re) (c : Complex) (w : V) :
    ∫ v : V, cexp (-b * ‖v‖ ^ 2 + c * ⟪w, v⟫) =
      (π / b) ^ (Module.finrank Real V / 2 : Complex) * cexp (c ^ 2 * ‖w‖ ^ 2 / (4 * b)) := by
  let e := (stdOrthonormalBasis Real V).repr.symm
  rw [← e.measurePreserving.integral_comp e.toHomeomorph.measurableEmbedding]
  convert! integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace hb c (e.symm w) <;>
    simp [LinearIsometryEquiv.inner_map_eq_flip]

/--
theorem `integral_cexp_neg_mul_sq_norm` / 定理 `integral_cexp_neg_mul_sq_norm`

English:
theorem integral_cexp_neg_mul_sq_norm
  given: (hb : 0 < b.re)
  proof: by
  simpa using integral_cexp_neg_mul_sq_norm_add hb 0 (0 : V)

中文:
定理 integral_cexp_neg_mul_sq_norm
  条件: (hb : 0 < b.re)
  证明: by
  simpa using integral_cexp_neg_mul_sq_norm_add hb 0 (0 : V)

Depends on / 依赖: integral_cexp_neg_mul_sq_norm_add
-/
theorem integral_cexp_neg_mul_sq_norm (hb : 0 < b.re) :
    ∫ v : V, cexp (-b * ‖v‖ ^ 2) = (π / b) ^ (Module.finrank Real V / 2 : Complex) := by
  simpa using integral_cexp_neg_mul_sq_norm_add hb 0 (0 : V)

set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `integral_rexp_neg_mul_sq_norm` / 定理 `integral_rexp_neg_mul_sq_norm`

English:
theorem integral_rexp_neg_mul_sq_norm
  given: {b : Real} (hb : 0 < b)
  proof: by
  rw [← ofReal_inj]
  convert! integral_cexp_neg_mul_sq_norm (show 0 < (b : Complex).re from hb) (V := V)
  · change ofRealLI (∫ (v : V), rexp (-b * ‖v‖ ^ 2)) = ∫ (v : V), cexp (-↑b * ↑‖v‖ ^ 2)
    rw [← ofRealLI.integral_comp_comm]
    simp [ofRealLI]
  · rw [← ofReal_div, ofReal_cpow (by positi

中文:
定理 integral_rexp_neg_mul_sq_norm
  条件: {b : 实数} (hb : 0 < b)
  证明: by
  rw [← ofReal_inj]
  convert! integral_cexp_neg_mul_sq_norm (show 0 < (b : Complex).re from hb) (V := V)
  · change ofRealLI (∫ (v : V), rexp (-b * ‖v‖ ^ 2)) = ∫ (v : V), cexp (-↑b * ↑‖v‖ ^ 2)
    rw [← ofRealLI.integral_comp_comm]
    simp [ofRealLI]
  · rw [← ofReal_div, ofReal_cpow (by positi

Depends on / 依赖: convert, integral_cexp_neg_mul_sq_norm, integral_comp_comm, ofRealLI, ofRealLI.integral_comp_comm, ofReal_cpow, ofReal_div, ofReal_inj
-/
theorem integral_rexp_neg_mul_sq_norm {b : Real} (hb : 0 < b) :
    ∫ v : V, rexp (-b * ‖v‖ ^ 2) = (π / b) ^ (Module.finrank Real V / 2 : Real) := by
  rw [← ofReal_inj]
  convert! integral_cexp_neg_mul_sq_norm (show 0 < (b : Complex).re from hb) (V := V)
  · change ofRealLI (∫ (v : V), rexp (-b * ‖v‖ ^ 2)) = ∫ (v : V), cexp (-↑b * ↑‖v‖ ^ 2)
    rw [← ofRealLI.integral_comp_comm]
    simp [ofRealLI]
  · rw [← ofReal_div, ofReal_cpow (by positivity)]
    simp

/--
theorem `_root_.fourier_gaussian_innerProductSpace'` / 定理 `_root_.fourier_gaussian_innerProductSpace'`

English:
theorem _root_.fourier_gaussian_innerProductSpace'
  given: (hb : 0 < b.re) (x w : V)
  proof: by
  simp only [neg_mul, fourier_eq', ofReal_neg, ofReal_mul, ofReal_ofNat,
    smul_eq_mul, ← Complex.exp_add, real_inner_comm w]
  convert! integral_cexp_neg_mul_sq_norm_add hb (2 * π * Complex.I) (x - w) using 3 with v
  · congr 1
    simp [inner_sub_left]
    ring
  · have : b != 0 := by contrap

中文:
定理 _root_.fourier_gaussian_innerProductSpace'
  条件: (hb : 0 < b.re) (x w : V)
  证明: by
  simp only [neg_mul, fourier_eq', ofReal_neg, ofReal_mul, ofReal_ofNat,
    smul_eq_mul, ← Complex.exp_add, real_inner_comm w]
  convert! integral_cexp_neg_mul_sq_norm_add hb (2 * π * Complex.I) (x - w) using 3 with v
  · congr 1
    simp [inner_sub_left]
    ring
  · have : b != 0 := by contrap

Depends on / 依赖: Complex.I, Complex.exp_add, contrapose, convert, exp_add, fourier_eq, inner_sub_left, integral_cexp_neg_mul_sq_norm_add, mul_pow, neg_mul, ofReal_mul, ofReal_neg, ofReal_ofNat, real_inner_comm, smul_eq_mul, zero_re
-/
theorem _root_.fourier_gaussian_innerProductSpace' (hb : 0 < b.re) (x w : V) :
    𝓕 (fun v => cexp (-b * ‖v‖ ^ 2 + 2 * π * Complex.I * ⟪x, v⟫)) w =
      (π / b) ^ (Module.finrank Real V / 2 : Complex) * cexp (-π ^ 2 * ‖x - w‖ ^ 2 / b) := by
  simp only [neg_mul, fourier_eq', ofReal_neg, ofReal_mul, ofReal_ofNat,
    smul_eq_mul, ← Complex.exp_add, real_inner_comm w]
  convert! integral_cexp_neg_mul_sq_norm_add hb (2 * π * Complex.I) (x - w) using 3 with v
  · congr 1
    simp [inner_sub_left]
    ring
  · have : b != 0 := by contrapose! hb; rw [hb, zero_re]
    simp [mul_pow]
    ring

/--
theorem `_root_.fourier_gaussian_innerProductSpace` / 定理 `_root_.fourier_gaussian_innerProductSpace`

English:
theorem _root_.fourier_gaussian_innerProductSpace
  given: (hb : 0 < b.re) (w : V)
  proof: by
  simpa using fourier_gaussian_innerProductSpace' hb 0 w

中文:
定理 _root_.fourier_gaussian_innerProductSpace
  条件: (hb : 0 < b.re) (w : V)
  证明: by
  simpa using fourier_gaussian_innerProductSpace' hb 0 w

Depends on / 依赖: fourier_gaussian_innerProductSpace
-/
theorem _root_.fourier_gaussian_innerProductSpace (hb : 0 < b.re) (w : V) :
    𝓕 (fun (v : V) => cexp (-b * ‖v‖ ^ 2)) w =
      (π / b) ^ (Module.finrank Real V / 2 : Complex) * cexp (-π ^ 2 * ‖w‖ ^ 2 / b) := by
  simpa using fourier_gaussian_innerProductSpace' hb 0 w

end InnerProductSpace

end GaussianFourier
