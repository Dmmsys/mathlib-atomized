...
/-
**TopModuleCat.ker** 是 Mathlib 中的一个缩写定义，位于命名空间 `TopModuleCat`。
形式化陈述：ker : TopModuleCat R
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Kernel in `TopModuleCat R` is the kernel of the linear map with the subspace top
ology.
-/
...
/-
**TopModuleCat.isLimitKer** 是 Mathlib 中的一个定义，位于命名空间 `TopModuleCat`。
形式化陈述：isLimitKer : IsLimit (KernelFork.ofι (kerι φ) (kerι_comp φ))
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `TopModuleCat.kerι_comp`：∀ {R : Type u} [inst : Ring R] [inst_1 : Topolog
icalSpace R] {M N : TopModuleCat R} (φ : M ⟶ N),   CategoryTheory.CategoryStruct
.comp (TopMo…
...
/-
**TopModuleCat.coker** 是 Mathlib 中的一个缩写定义，位于命名空间 `TopModuleCat`。
形式化陈述：coker : TopModuleCat R
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Cokernel in `TopModuleCat R` is the cokernel of the linear map with the quotient
 topology.
-/
...
@[simp]
/-
**TopModuleCat.hom_coker** 是 Mathlib 中的一个引理，位于命名空间 `TopModuleCat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**TopModuleCat.comp_coker** 是 Mathlib 中的一个引理，位于命名空间 `TopModuleCat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**TopModuleCat.isColimitCoker** 是 Mathlib 中的一个定义，位于命名空间 `TopModuleCat`。
形式化陈述：isColimitCoker : IsColimit (CokernelCofork.ofπ (cokerπ φ) (comp_cokerπ φ))
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `TopModuleCat.comp_cokerπ`：∀ {R : Type u} [inst : Ring R] [inst_1 : Topol
ogicalSpace R] {M N : TopModuleCat R} (φ : M ⟶ N),   CategoryTheory.CategoryStru
ct.comp φ (Top…
