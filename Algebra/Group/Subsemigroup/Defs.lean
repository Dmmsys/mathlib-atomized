/-
Copyright (c) 2018 Johannes Hölzl. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Johannes Hölzl, Kenny Lau, Johan Commelin, Mario Carneiro, Kevin Buzzard,
Amelia Livingston, Yury Kudryashov, Yakov Pechersky
-/
module

public import Mathlib.Algebra.Group.Hom.Defs
public import Mathlib.Algebra.Group.InjSurj
public import Mathlib.Data.SetLike.Basic
public import Mathlib.Tactic.FastInstance

/-!
# Subsemigroups: definition

This file defines bundled multiplicative and additive subsemigroups.

## Main definitions

* `Subsemigroup M`: the type of bundled subsemigroup of a magma `M`; the underlying set is given in
  the `carrier` field of the structure, and should be accessed through coercion as in `(S : Set M)`.
* `AddSubsemigroup M` : the type of bundled subsemigroups of an additive magma `M`.

For each of the following definitions in the `Subsemigroup` namespace, there is a corresponding
definition in the `AddSubsemigroup` namespace.

* `Subsemigroup.copy` : copy of a subsemigroup with `carrier` replaced by a set that is equal but
  possibly not definitionally equal to the carrier of the original `Subsemigroup`.

Similarly, for each of these definitions in the `MulHom` namespace, there is a corresponding
definition in the `AddHom` namespace.

* `MulHom.eqLocus f g`: the subsemigroup of those `x` such that `f x = g x`

## Implementation notes

Subsemigroup inclusion is denoted `≤` rather than `⊆`, although `∈` is defined as
membership of a subsemigroup's underlying set.

Note that `Subsemigroup M` does not actually require `Semigroup M`,
instead requiring only the weaker `Mul M`.

This file is designed to have very few dependencies. In particular, it should not use natural
numbers.

## Tags
subsemigroup, subsemigroups
-/

@[expose] public section

assert_not_exists RelIso CompleteLattice MonoidWithZero

variable {M : Type*} {N : Type*}

section NonAssoc

variable [Mul M] {s : Set M}

/-- `MulMemClass S M` says `S` is a type of sets `s : Set M` that are closed under `(*)` -/
/-
**MulMemClass** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：(S : Type u_3) → (M : outParam (Type u_4)) → [Mul M] → [SetLike S M] → Pro
p
参数：Type u_4。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`MulMemClass S M` says `S` is a type of sets `s : Set M` that are closed under `
(*)`
-/
class MulMemClass (S : Type*) (M : outParam Type*) [Mul M] [SetLike S M] : Prop where
  /-- A substructure satisfying `MulMemClass` is closed under multiplication. -/
  mul_mem : ∀ {s : S} {a b : M}, a ∈ s → b ∈ s → a * b ∈ s

export MulMemClass (mul_mem)

/-- `AddMemClass S M` says `S` is a type of sets `s : Set M` that are closed under `(+)` -/
/-
**AddMemClass** 是 Mathlib 中的一个类，位于命名空间 ``。
形式化陈述：AddMemClass (S : Type*) (M : outParam Type*) [Add M] [SetLike S M] : Prop 
where /-- A substructure satisfying `AddMemClass` is closed under addition. -/ a
dd_mem : forall {s : S} {a b : M}, a in s -> b in s -> a + b in s  export AddMem
Class (add_mem)  attribute [to_additive] MulMemClass  attribute [aesop 90% (rule
_sets
参数：S : Type*；M : outParam Type*。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`AddMemClass S M` says `S` is a type of sets `s : Set M` that are closed under `
(+)`
-/
class AddMemClass (S : Type*) (M : outParam Type*) [Add M] [SetLike S M] : Prop where
  /-- A substructure satisfying `AddMemClass` is closed under addition. -/
  add_mem : ∀ {s : S} {a b : M}, a ∈ s → b ∈ s → a + b ∈ s

export AddMemClass (add_mem)

attribute [to_additive] MulMemClass

attribute [aesop 90% (rule_sets := [SetLike])] mul_mem add_mem

/-- A subsemigroup of a magma `M` is a subset closed under multiplication. -/
/-
**Subsemigroup** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：(M : Type u_3) → [Mul M] → Type u_3
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A subsemigroup of a magma `M` is a subset closed under multiplication.
-/
structure Subsemigroup (M : Type*) [Mul M] where
  /-- The carrier of a subsemigroup. -/
  carrier : Set M
  /-- The product of two elements of a subsemigroup belongs to the subsemigroup. -/
  mul_mem' {a b} : a ∈ carrier → b ∈ carrier → a * b ∈ carrier

/-- An additive subsemigroup of an additive magma `M` is a subset closed under addition. -/
/-
**AddSubsemigroup** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：(M : Type u_3) → [Add M] → Type u_3
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
An additive subsemigroup of an additive magma `M` is a subset closed under addit
ion.
-/
structure AddSubsemigroup (M : Type*) [Add M] where
  /-- The carrier of an additive subsemigroup. -/
  carrier : Set M
  /-- The sum of two elements of an additive subsemigroup belongs to the subsemigroup. -/
  add_mem' {a b} : a ∈ carrier → b ∈ carrier → a + b ∈ carrier

attribute [to_additive AddSubsemigroup] Subsemigroup

namespace Subsemigroup

@[to_additive]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : SetLike (Subsemigroup M) M :=
  ⟨Subsemigroup.carrier, fun p q h => by cases p; cases q; congr⟩
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[to_additive] instance : PartialOrder (Subsemigroup M) := .ofSetLike (Subsemigroup M) M

initialize_simps_projections Subsemigroup (carrier → coe, as_prefix coe)
initialize_simps_projections AddSubsemigroup (carrier → coe, as_prefix coe)

/-- The actual `Subsemigroup` obtained from an element of a `MulMemClass`. -/
@[to_additive (attr := simps) /-- The actual `AddSubsemigroup` obtained from an element of a
`AddMemClass` -/]
/-
**Subsemigroup.ofClass** 是 Mathlib 中的一个定义，位于命名空间 `Subsemigroup`。
形式化陈述：ofClass {S M : Type*} [Mul M] [SetLike S M] [MulMemClass S M] (s : S) : Su
bsemigroup M
参数：s : S。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `MulMemClass.mul_mem`：∀ {S : Type u_3} {M : outParam (Type u_4)} {inst : 
Mul M} {inst_1 : SetLike S M} [self : MulMemClass S M] {s : S}   {a b : M}, a ∈ 
s → b ∈ s…
-/
def ofClass {S M : Type*} [Mul M] [SetLike S M] [MulMemClass S M] (s : S) : Subsemigroup M :=
  ⟨s, MulMemClass.mul_mem⟩

@[to_additive]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (priority := 100) : CanLift (Set M) (Subsemigroup M) (↑)
    (fun s ↦ ∀ {x y}, x ∈ s → y ∈ s → x * y ∈ s) where
  prf s h := ⟨{ carrier := s, mul_mem' := h }, rfl⟩

@[to_additive]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : MulMemClass (Subsemigroup M) M where mul_mem := fun {_ _ _} => Subsemigroup.mul_mem' _

@[to_additive (attr := simp)]
/-
**Subsemigroup.mem_carrier** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：mem_carrier {s : Subsemigroup M} {x : M} : x in s.carrier ↔ x in s
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_carrier {s : Subsemigroup M} {x : M} : x ∈ s.carrier ↔ x ∈ s :=
  Iff.rfl

@[to_additive (attr := simp)]
/-
**Subsemigroup.mem_mk** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：mem_mk {s : Set M} {x : M} (h_mul) : x in mk s h_mul ↔ x in s
参数：h_mul。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_mk {s : Set M} {x : M} (h_mul) : x ∈ mk s h_mul ↔ x ∈ s :=
  Iff.rfl

@[to_additive (attr := simp, norm_cast)]
/-
**Subsemigroup.coe_set_mk** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：coe_set_mk (s : Set M) (h_mul) : (mk s h_mul : Set M) = s
参数：s : Set M；h_mul。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_set_mk (s : Set M) (h_mul) : (mk s h_mul : Set M) = s :=
  rfl

@[to_additive (attr := simp)]
/-
**Subsemigroup.mk_le_mk** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：mk_le_mk {s t : Set M} (h_mul) (h_mul') : mk s h_mul <= mk t h_mul' ↔ s su
bseteq t
参数：h_mul；h_mul'。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mk_le_mk {s t : Set M} (h_mul) (h_mul') : mk s h_mul ≤ mk t h_mul' ↔ s ⊆ t :=
  Iff.rfl

/-- Two subsemigroups are equal if they have the same elements. -/
@[to_additive (attr := ext) /-- Two `AddSubsemigroup`s are equal if they have the same elements. -/]
/-
**Subsemigroup.ext** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：ext {S T : Subsemigroup M} (h : forall x, x in S ↔ x in T) : S = T
参数：h : forall x, x in S ↔ x in T。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `SetLike.ext`：ext (h : forall x, x in p ↔ x in q) : p = q

--- 原说明 ---
Two subsemigroups are equal if they have the same elements.
-/
theorem ext {S T : Subsemigroup M} (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T :=
  SetLike.ext h

/-- Copy a subsemigroup replacing `carrier` with a set that is equal to it. -/
@[to_additive
/-- Copy an additive subsemigroup replacing `carrier` with a set that is equal to it. -/]
/-
**Subsemigroup.copy** 是 Mathlib 中的一个定义，位于命名空间 `Subsemigroup`。
形式化陈述：{M : Type u_1} → [inst : Mul M] → (S : Subsemigroup M) → (s : Set M) → s =
 ↑S → Subsemigroup M
参数：S : Subsemigroup M；s : Set M。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def copy (S : Subsemigroup M) (s : Set M) (hs : s = S) :
    Subsemigroup M where
  carrier := s
  mul_mem' := hs.symm ▸ S.mul_mem'

variable {S : Subsemigroup M}

@[to_additive (attr := simp, norm_cast)]
/-
**Subsemigroup.coe_copy** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：coe_copy {s : Set M} (hs : s = S) : (S.copy s hs : Set M) = s
参数：hs : s = S。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_copy {s : Set M} (hs : s = S) : (S.copy s hs : Set M) = s :=
  rfl

@[to_additive]
/-
**Subsemigroup.copy_eq** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：copy_eq {s : Set M} (hs : s = S) : S.copy s hs = S
参数：hs : s = S。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `SetLike.coe_injective`：∀ {A : Type u_1} {B : outParam (Type u_2)} [self 
: SetLike A B], Function.Injective SetLike.coe
-/
theorem copy_eq {s : Set M} (hs : s = S) : S.copy s hs = S :=
  SetLike.coe_injective hs

variable (S)

/-- A subsemigroup is closed under multiplication. -/
@[to_additive /-- An `AddSubsemigroup` is closed under addition. -/]
/-
**Subsemigroup.mul_mem** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：∀ {M : Type u_1} [inst : Mul M] (S : Subsemigroup M) {x y : M}, x ∈ S → y 
∈ S → x * y ∈ S
参数：S : Subsemigroup M。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Subsemigroup.mul_mem'`：∀ {M : Type u_3} [inst : Mul M] (self : Subsemigr
oup M) {a b : M},   a ∈ self.carrier → b ∈ self.carrier → a * b ∈ self.carrier

--- 原说明 ---
A subsemigroup is closed under multiplication.
-/
protected theorem mul_mem {x y : M} : x ∈ S → y ∈ S → x * y ∈ S :=
  Subsemigroup.mul_mem' S

/-- The subsemigroup `M` of the magma `M`. -/
@[to_additive /-- The additive subsemigroup `M` of the magma `M`. -/]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The subsemigroup `M` of the magma `M`.
-/
instance : Top (Subsemigroup M) :=
  ⟨{  carrier := Set.univ
      mul_mem' := fun _ _ => Set.mem_univ _ }⟩

/-- The trivial subsemigroup `∅` of a magma `M`. -/
@[to_additive /-- The trivial `AddSubsemigroup` `∅` of an additive magma `M`. -/]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The trivial subsemigroup `∅` of a magma `M`.
-/
instance : Bot (Subsemigroup M) :=
  ⟨{  carrier := ∅
      mul_mem' := False.elim }⟩

@[to_additive]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : Inhabited (Subsemigroup M) :=
  ⟨⊥⟩

@[to_additive]
/-
**Subsemigroup.notMem_bot** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：notMem_bot {x : M} : x ∉ (⊥ : Subsemigroup M)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.notMem_empty`：notMem_empty (x : α) : x ∉ (∅ : Set α)
-/
theorem notMem_bot {x : M} : x ∉ (⊥ : Subsemigroup M) :=
  Set.notMem_empty x

@[to_additive (attr := simp)]
/-
**Subsemigroup.mem_top** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：mem_top (x : M) : x in (⊤ : Subsemigroup M)
参数：x : M。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mem_univ`：mem_univ (x : α) : x in @univ α
-/
theorem mem_top (x : M) : x ∈ (⊤ : Subsemigroup M) :=
  Set.mem_univ x

@[to_additive (attr := simp, norm_cast)]
/-
**Subsemigroup.coe_top** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：coe_top : ((⊤ : Subsemigroup M) : Set M) = Set.univ
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_top : ((⊤ : Subsemigroup M) : Set M) = Set.univ :=
  rfl

@[to_additive (attr := simp, norm_cast)]
/-
**Subsemigroup.coe_bot** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：coe_bot : ((⊥ : Subsemigroup M) : Set M) = ∅
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_bot : ((⊥ : Subsemigroup M) : Set M) = ∅ :=
  rfl

@[to_additive (attr := simp)]
/-
**Subsemigroup.mk_eq_top** 是 Mathlib 中的一个引理，位于命名空间 `Subsemigroup`。
形式化陈述：mk_eq_top (carrier : Set M) (mul_mem') : mk carrier mul_mem' = ⊤ ↔ carrier
 = .univ
参数：carrier : Set M；mul_mem'。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
lemma mk_eq_top (carrier : Set M) (mul_mem') : mk carrier mul_mem' = ⊤ ↔ carrier = .univ := by
  simp [← SetLike.coe_set_eq]

@[to_additive (attr := simp)]
/-
**Subsemigroup.mk_eq_bot** 是 Mathlib 中的一个引理，位于命名空间 `Subsemigroup`。
形式化陈述：mk_eq_bot (carrier : Set M) (mul_mem') : mk carrier mul_mem' = ⊥ ↔ carrier
 = ∅
参数：carrier : Set M；mul_mem'。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
lemma mk_eq_bot (carrier : Set M) (mul_mem') : mk carrier mul_mem' = ⊥ ↔ carrier = ∅ := by
  simp [← SetLike.coe_set_eq]

/-- The inf of two subsemigroups is their intersection. -/
@[to_additive /-- The inf of two `AddSubsemigroup`s is their intersection. -/]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The inf of two subsemigroups is their intersection.
-/
instance : Min (Subsemigroup M) :=
  ⟨fun S₁ S₂ =>
    { carrier := S₁ ∩ S₂
      mul_mem' := fun ⟨hx, hx'⟩ ⟨hy, hy'⟩ => ⟨S₁.mul_mem hx hy, S₂.mul_mem hx' hy'⟩ }⟩

@[to_additive (attr := simp, norm_cast)]
/-
**Subsemigroup.coe_inf** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：coe_inf (p p' : Subsemigroup M) : ((p ⊓ p' : Subsemigroup M) : Set M) = (p
 : Set M) inter p'
参数：p p' : Subsemigroup M。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_inf (p p' : Subsemigroup M) : ((p ⊓ p' : Subsemigroup M) : Set M) = (p : Set M) ∩ p' :=
  rfl

@[to_additive (attr := simp)]
/-
**Subsemigroup.mem_inf** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigroup`。
形式化陈述：mem_inf {p p' : Subsemigroup M} {x : M} : x in p ⊓ p' ↔ x in p ∧ x in p'
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_inf {p p' : Subsemigroup M} {x : M} : x ∈ p ⊓ p' ↔ x ∈ p ∧ x ∈ p' :=
  Iff.rfl

@[to_additive]
/-
**Subsemigroup.subsingleton_of_subsingleton** 是 Mathlib 中的一个定理，位于命名空间 `Subsemigr
oup`。
形式化陈述：subsingleton_of_subsingleton [Subsingleton (Subsemigroup M)] : Subsingleto
n M
参数：Subsemigroup M。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Subsingleton.elim`：∀ {α : Sort u} [h : Subsingleton α] (a b : α), a = b
· 使用定理 `implies_true`：∀ (α : Sort u), (∀ (a : α), True) = True
· 使用定理 `Subsemigroup.notMem_bot`：notMem_bot {x : M} : x ∉ (⊥ : Subsemigroup M)
-/
theorem subsingleton_of_subsingleton [Subsingleton (Subsemigroup M)] : Subsingleton M := by
  constructor; intro x y
  have : ∀ a : M, a ∈ (⊥ : Subsemigroup M) := by simp [Subsingleton.elim (⊥ : Subsemigroup M) ⊤]
  exact absurd (this x) notMem_bot

@[to_additive]
/-
**Subsemigroup.** 是 Mathlib 中的一个实例，位于命名空间 `Subsemigroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance [hn : Nonempty M] : Nontrivial (Subsemigroup M) :=
  ⟨⟨⊥, ⊤, fun h => by
      obtain ⟨x⟩ := id hn
      refine absurd (?_ : x ∈ ⊥) notMem_bot
      simp [h]⟩⟩

end Subsemigroup

namespace MulHom

variable [Mul N]

open Subsemigroup

/-- The subsemigroup of elements `x : M` such that `f x = g x` -/
@[to_additive /-- The additive subsemigroup of elements `x : M` such that `f x = g x` -/]
/-
**MulHom.eqLocus** 是 Mathlib 中的一个定义，位于命名空间 `MulHom`。
形式化陈述：eqLocus (f g : M ->ₙ* N) : Subsemigroup M where carrier
参数：f g : M ->ₙ* N。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The subsemigroup of elements `x : M` such that `f x = g x`
-/
def eqLocus (f g : M →ₙ* N) : Subsemigroup M where
  carrier := { x | f x = g x }
  mul_mem' (hx : _ = _) (hy : _ = _) := by simp [*]

@[to_additive (attr := simp)]
/-
**MulHom.mem_eqLocus** 是 Mathlib 中的一个定理，位于命名空间 `MulHom`。
形式化陈述：mem_eqLocus {f g : M ->ₙ* N} {x : M} : x in f.eqLocus g ↔ f x = g x
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_eqLocus {f g : M →ₙ* N} {x : M} : x ∈ f.eqLocus g ↔ f x = g x := Iff.rfl

@[to_additive]
/-
**MulHom.eq_of_eqOn_top** 是 Mathlib 中的一个定理，位于命名空间 `MulHom`。
形式化陈述：eq_of_eqOn_top {f g : M ->ₙ* N} (h : Set.EqOn f g (⊤ : Subsemigroup M)) : 
f = g
参数：h : Set.EqOn f g (⊤ : Subsemigroup M)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `MulHom.ext`：MulHom.ext [Mul M] [Mul N] ⦃f g : M ->ₙ* N⦄ (h : forall x, f
 x = g x) : f = g
· 使用定理 `trivial`：True
-/
theorem eq_of_eqOn_top {f g : M →ₙ* N} (h : Set.EqOn f g (⊤ : Subsemigroup M)) : f = g :=
  ext fun _ => h trivial

end MulHom

end NonAssoc

namespace MulMemClass

variable {A : Type*} [Mul M] [SetLike A M] [hA : MulMemClass A M] (S' : A)

-- lower priority so other instances are found first
/-- A submagma of a magma inherits a multiplication. -/
@[to_additive /-- An additive submagma of an additive magma inherits an addition. -/]
/-
**MulMemClass.** 是 Mathlib 中的一个实例，位于命名空间 `MulMemClass`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A submagma of a magma inherits a multiplication.
-/
instance (priority := 900) mul : Mul S' :=
  ⟨fun a b => ⟨a.1 * b.1, mul_mem a.2 b.2⟩⟩

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[to_additive (attr := simp low, norm_cast)]
/-
**MulMemClass.coe_mul** 是 Mathlib 中的一个定理，位于命名空间 `MulMemClass`。
形式化陈述：coe_mul (x y : S') : (↑(x * y) : M) = ↑x * ↑y
参数：x y : S'。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_mul (x y : S') : (↑(x * y) : M) = ↑x * ↑y :=
  rfl

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[to_additive (attr := simp low)]
/-
**MulMemClass.mk_mul_mk** 是 Mathlib 中的一个定理，位于命名空间 `MulMemClass`。
形式化陈述：mk_mul_mk (x y : M) (hx : x in S') (hy : y in S') : (⟨x, hx⟩ : S') * ⟨y, h
y⟩ = ⟨x * y, mul_mem hx hy⟩
参数：x y : M；hx : x in S'；hy : y in S'。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem mk_mul_mk (x y : M) (hx : x ∈ S') (hy : y ∈ S') :
    (⟨x, hx⟩ : S') * ⟨y, hy⟩ = ⟨x * y, mul_mem hx hy⟩ :=
  rfl

@[to_additive]
/-
**MulMemClass.mul_def** 是 Mathlib 中的一个定理，位于命名空间 `MulMemClass`。
形式化陈述：mul_def (x y : S') : x * y = ⟨x * y, mul_mem x.2 y.2⟩
参数：x y : S'。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem mul_def (x y : S') : x * y = ⟨x * y, mul_mem x.2 y.2⟩ :=
  rfl

/-- A subsemigroup of a semigroup inherits a semigroup structure. -/
@[to_additive
/-- An `AddSubsemigroup` of an `AddSemigroup` inherits an `AddSemigroup` structure. -/]
/-
**MulMemClass.toSemigroup** 是 Mathlib 中的一个实例，位于命名空间 `MulMemClass`。
形式化陈述：toSemigroup {M : Type*} [Semigroup M] {A : Type*} [SetLike A M] [MulMemCla
ss A M] (S : A) : Semigroup S
参数：S : A。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance toSemigroup {M : Type*} [Semigroup M] {A : Type*} [SetLike A M] [MulMemClass A M]
    (S : A) : Semigroup S := fast_instance%
  Subtype.coe_injective.semigroup Subtype.val fun _ _ => rfl

/-- A subsemigroup of a `CommSemigroup` is a `CommSemigroup`. -/
@[to_additive /-- An `AddSubsemigroup` of an `AddCommSemigroup` is an `AddCommSemigroup`. -/]
/-
**MulMemClass.toCommSemigroup** 是 Mathlib 中的一个实例，位于命名空间 `MulMemClass`。
形式化陈述：toCommSemigroup {M} [CommSemigroup M] {A : Type*} [SetLike A M] [MulMemCla
ss A M] (S : A) : CommSemigroup S
参数：S : A。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A subsemigroup of a `CommSemigroup` is a `CommSemigroup`.
-/
instance toCommSemigroup {M} [CommSemigroup M] {A : Type*} [SetLike A M] [MulMemClass A M]
    (S : A) : CommSemigroup S := fast_instance%
  Subtype.coe_injective.commSemigroup Subtype.val fun _ _ => rfl

/-- A submagma of a left cancellative magma inherits left cancellation. -/
@[to_additive
/-- An additive submagma of a left cancellative additive magma inherits left cancellation. -/]
/-
**MulMemClass.isLeftCancelMul** 是 Mathlib 中的一个实例，位于命名空间 `MulMemClass`。
形式化陈述：isLeftCancelMul [IsLeftCancelMul M] (S : A) : IsLeftCancelMul S
参数：S : A。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Injective.isLeftCancelMul`：∀ {M₁ : Type u_1} {M₂ : Type u_2} [i
nst : Mul M₁] [inst_1 : Mul M₂] [IsLeftCancelMul M₂] (f : M₁ → M₂),   Function.I
njective f → (∀ (x y : M…
· 使用定理 `Subtype.coe_injective`：coe_injective : Injective (fun (a : Subtype p) =>
 (a : α))
-/
instance isLeftCancelMul [IsLeftCancelMul M] (S : A) : IsLeftCancelMul S :=
  Subtype.coe_injective.isLeftCancelMul Subtype.val fun _ _ => rfl

/-- A submagma of a right cancellative magma inherits right cancellation. -/
@[to_additive
/-- An additive submagma of a right cancellative additive magma inherits right cancellation. -/]
/-
**MulMemClass.isRightCancelMul** 是 Mathlib 中的一个实例，位于命名空间 `MulMemClass`。
形式化陈述：isRightCancelMul [IsRightCancelMul M] (S : A) : IsRightCancelMul S
参数：S : A。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Injective.isRightCancelMul`：∀ {M₁ : Type u_1} {M₂ : Type u_2} [
inst : Mul M₁] [inst_1 : Mul M₂] [IsRightCancelMul M₂] (f : M₁ → M₂),   Function
.Injective f → (∀ (x y : …
· 使用定理 `Subtype.coe_injective`：coe_injective : Injective (fun (a : Subtype p) =>
 (a : α))
-/
instance isRightCancelMul [IsRightCancelMul M] (S : A) : IsRightCancelMul S :=
  Subtype.coe_injective.isRightCancelMul Subtype.val fun _ _ => rfl

/-- A submagma of a cancellative magma inherits cancellation. -/
@[to_additive /-- An additive submagma of a cancellative additive magma inherits cancellation. -/]
/-
**MulMemClass.isCancelMul** 是 Mathlib 中的一个定理，位于命名空间 `MulMemClass`。
形式化陈述：∀ {M : Type u_1} {A : Type u_3} [inst : Mul M] [inst_1 : SetLike A M] [hA 
: MulMemClass A M] [IsCancelMul M] (S : A),   IsCancelMul ↥S
参数：S : A。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `IsCancelMul.toIsLeftCancelMul`：∀ {G : Type u} {inst : Mul G} [self : IsC
ancelMul G], IsLeftCancelMul G
· 使用定理 `IsCancelMul.toIsRightCancelMul`：∀ {G : Type u} {inst : Mul G} [self : Is
CancelMul G], IsRightCancelMul G

--- 原说明 ---
A submagma of a cancellative magma inherits cancellation.
-/
instance isCancelMul [IsCancelMul M] (S : A) : IsCancelMul S where

/-- The natural semigroup hom from a subsemigroup of semigroup `M` to `M`. -/
@[to_additive /-- The natural semigroup hom from an `AddSubsemigroup` of
`AddSubsemigroup` `M` to `M`. -/]
/-
**MulMemClass.subtype** 是 Mathlib 中的一个定义，位于命名空间 `MulMemClass`。
形式化陈述：subtype : S' ->ₙ* M where toFun
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def subtype : S' →ₙ* M where
  toFun := Subtype.val; map_mul' := fun _ _ => rfl

variable {S'} in
@[to_additive (attr := simp)]
/-
**MulMemClass.subtype_apply** 是 Mathlib 中的一个引理，位于命名空间 `MulMemClass`。
形式化陈述：subtype_apply (x : S') : MulMemClass.subtype S' x = x
参数：x : S'。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma subtype_apply (x : S') :
    MulMemClass.subtype S' x = x := rfl

@[to_additive]
/-
**MulMemClass.subtype_injective** 是 Mathlib 中的一个引理，位于命名空间 `MulMemClass`。
形式化陈述：subtype_injective : Function.Injective (MulMemClass.subtype S')
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Subtype.coe_injective`：coe_injective : Injective (fun (a : Subtype p) =>
 (a : α))
-/
lemma subtype_injective :
    Function.Injective (MulMemClass.subtype S') :=
  Subtype.coe_injective

@[to_additive (attr := simp)]
/-
**MulMemClass.coe_subtype** 是 Mathlib 中的一个定理，位于命名空间 `MulMemClass`。
形式化陈述：coe_subtype : (MulMemClass.subtype S' : S' -> M) = Subtype.val
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_subtype : (MulMemClass.subtype S' : S' → M) = Subtype.val :=
  rfl

end MulMemClass

@[to_additive]
/-
**isMulCommutative_iff_of_setLike** 是 Mathlib 中的一个引理，位于命名空间 ``。
形式化陈述：isMulCommutative_iff_of_setLike {S M : Type*} [SetLike S M] [Mul M] [MulMe
mClass S M] {s : S} : IsMulCommutative s ↔ forall a in s, forall b in s, a * b =
 b * a
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `MulMemClass.mul_mem`：∀ {S : Type u_3} {M : outParam (Type u_4)} {inst : 
Mul M} {inst_1 : SetLike S M} [self : MulMemClass S M] {s : S}   {a b : M}, a ∈ 
s → b ∈ s…
· 使用定理 `Subtype.mk.injEq`：∀ {α : Sort u} {p : α → Prop} (val : α) (property : p 
val) (val_1 : α) (property_1 : p val_1),   (⟨val, property⟩ = ⟨val_1, property_1
⟩) = (…
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
lemma isMulCommutative_iff_of_setLike {S M : Type*} [SetLike S M] [Mul M] [MulMemClass S M]
    {s : S} : IsMulCommutative s ↔ ∀ a ∈ s, ∀ b ∈ s, a * b = b * a := by
  simp [isMulCommutative_iff]

@[to_additive]
alias ⟨_, IsMulCommutative.of_setLike_mul_comm⟩ := isMulCommutative_iff_of_setLike

/-- Commutativity of multiplication in commutative subobjects. -/
@[to_additive /-- Commutativity of addition in commutative subobjects. -/ ]
/-
**setLike_mul_comm** 是 Mathlib 中的一个引理，位于命名空间 ``。
形式化陈述：setLike_mul_comm {S M : Type*} [SetLike S M] [Mul M] [MulMemClass S M] {s 
: S} [IsMulCommutative s] ⦃a b : M⦄ (ha : a in s) (hb : b in s) : a * b = b * a
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用引理 `isMulCommutative_iff_of_setLike`：isMulCommutative_iff_of_setLike {S M : 
Type*} [SetLike S M] [Mul M] [MulMemClass S M] {s : S} : IsMulCommutative s ↔ fo
rall a in s, forall b…

--- 原说明 ---
Commutativity of multiplication in commutative subobjects.
-/
lemma setLike_mul_comm {S M : Type*} [SetLike S M] [Mul M] [MulMemClass S M]
    {s : S} [IsMulCommutative s] ⦃a b : M⦄ (ha : a ∈ s) (hb : b ∈ s) :
    a * b = b * a :=
  isMulCommutative_iff_of_setLike.mp ‹_› a ha b hb
