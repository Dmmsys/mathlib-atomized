...
/-
**CategoryTheory.coherentTopology.struct** 是 Mathlib 中的一个结构，位于命名空间 `CategoryTheo
ry.coherentTopology`。
...
include hF in
/-
**CategoryTheory.coherentTopology.exists_effectiveEpi** 是 Mathlib 中的一个引理，位于命名空间 
...
private lemma exists_effectiveEpi (n : ℕ) (X : C) (y : (F.obj ⟨n⟩).obj.obj ⟨X⟩) :
  ...
  exact this X y
/-
**CategoryTheory.coherentTopology.preimage** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTh
eory.coherentTopology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.coherentTopology.preimageStruct** 是 Mathlib 中的一个定义，位于命名空间 `Cate
goryTheory.coherentTopology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.coherentTopology.preimageDiagram** 是 Mathlib 中的一个定义，位于命名空间 `Cat
...
/-
**CategoryTheory.coherentTopology.cone** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory
.coherentTopology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include hF h hc in
/-
**CategoryTheory.coherentTopology.isLocallySurjective_** 是 Mathlib 中的一个引理，位于命名空间
 `CategoryTheory.coherentTopology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include h in
/-
**CategoryTheory.coherentTopology.epi_** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory
.coherentTopology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
