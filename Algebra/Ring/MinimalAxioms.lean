...
/-
**Ring.ofMinimalAxioms** 是 Mathlib 中的一个缩写定义，位于命名空间 ``。
形式化陈述：Ring.ofMinimalAxioms {R : Type u} [Add R] [Mul R] [Neg R] [Zero R] [One R]
 (add_assoc : forall a b c : R, a + b + c = a + (b + c)) (zero_add : forall a : 
R, 0 + a = a) (neg_add_cancel : forall a : R, -a + a = 0) (mul_assoc : forall a 
b c : R, a * b * c = a * (b * c)) (one_mul : forall a : R, 1 * a = a) (mul_one :
 forall a : R, a * 1 = a) (left_distrib : forall a b c : R, a * (b + c) = a * b 
+ a * c) (right_distrib : forall a b c : R, (a + b) * c = a * c + b * c) : Ring 
R
参数：add_assoc : forall a b c : R, a + b + c = a + (b + c)；zero_add : forall a : R
, 0 + a = a；neg_add_cancel : forall a : R, -a + a = 0；mul_assoc : forall a b c :
 R, a * b * c = a * (b * c)；one_mul : forall a : R, 1 * a = a；mul_one : forall a
 : R, a * 1 = a；left_distrib : forall a b c : R, a * (b + c) = a * b + a * c；rig
ht_distrib : forall a b c : R, (a + b) * c = a * c + b * c。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Define a `Ring` structure on a Type by proving a minimized set of axioms.
Note that this uses the default definitions for `npow`, `nsmul`, `zsmul` and `su
b`
See note [reducible non-instances].
-/
...
See note [reducible non-instances]. -/
/-
**CommRing.ofMinimalAxioms** 是 Mathlib 中的一个缩写定义，位于命名空间 ``。
形式化陈述：CommRing.ofMinimalAxioms {R : Type u} [Add R] [Mul R] [Neg R] [Zero R] [On
e R] (add_assoc : forall a b c : R, a + b + c = a + (b + c)) (zero_add : forall 
a : R, 0 + a = a) (neg_add_cancel : forall a : R, -a + a = 0) (mul_assoc : foral
l a b c : R, a * b * c = a * (b * c)) (mul_comm : forall a b : R, a * b = b * a)
 (one_mul : forall a : R, 1 * a = a) (left_distrib : forall a b c : R, a * (b + 
c) = a * b + a * c) : CommRing R
参数：add_assoc : forall a b c : R, a + b + c = a + (b + c)；zero_add : forall a : R
, 0 + a = a；neg_add_cancel : forall a : R, -a + a = 0；mul_assoc : forall a b c :
 R, a * b * c = a * (b * c)；mul_comm : forall a b : R, a * b = b * a；one_mul : f
orall a : R, 1 * a = a；left_distrib : forall a b c : R, a * (b + c) = a * b + a 
* c。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
