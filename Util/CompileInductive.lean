...
/-
**Mathlib.Util.replaceConst** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
...
/-
**Mathlib.Util.mkRecNames** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
形式化陈述：mkRecNames (all : List Name) (numMotives : Nat) : List Name
参数：all : List Name；numMotives : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Util.addAndCompile'** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Util.compileDefn** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
形式化陈述：compileDefn (dv : DefinitionVal) : MetaM Unit
参数：dv : DefinitionVal。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Compile the definition `dv` by adding a second definition `dv✝` with the same bo
dy,
and registering a `csimp`-lemma `dv = dv✝`.
-/
...
/-
**Mathlib.Util.hasCSimpLemma** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
形式化陈述：hasCSimpLemma (env : Environment) (n : Name) : Bool
参数：env : Environment；n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Util.compileStructOnly** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Util.compileInductiveOnly** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Util`。
形式化陈述：compileInductiveOnly (iv : InductiveVal) (rv : RecursorVal) (warn
参数：iv : InductiveVal；rv : RecursorVal。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Generate compiled code for the recursor for `iv`, excluding the `sizeOf` functio
n.
-/
...
-/
/-
**Mathlib.Util.compileInductive** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Util`。
形式化陈述：InductiveVal → optParam Bool true → MetaM Unit
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Util.compileSizeOf** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Util`。
形式化陈述：InductiveVal → RecursorVal → MetaM Unit
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Compiles the `sizeOf` auxiliary functions. It also recursively compiles any indu
ctives required to
compile the `sizeOf` definition (because `sizeOf` definitions depend on `T.rec`)
.
-/
