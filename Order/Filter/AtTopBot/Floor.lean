...
/-
**FloorSemiring.eventually_mul_pow_lt_factorial_sub** 是 Mathlib 中的一个定理，位于命名空间 ``
。
形式化陈述：FloorSemiring.eventually_mul_pow_lt_factorial_sub (a c : K) (d : Nat) : fo
rallᶠ n in atTop, a * c ^ n < (n - d)!
参数：a c : K；d : Nat。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Filter.mp_mem`：mp_mem (hs : s in f) (h : { x | x in s -> x in t } in f) 
: t in f
· 使用定理 `Nat.eventually_mul_pow_lt_factorial_sub`：eventually_mul_pow_lt_factorial
_sub (a c d : Nat) : forallᶠ n in atTop, a * c ^ n < (n - d)!
