...
/--
Definition of `IsSymmOp` / `IsSymmOp` 的定义
...
/--
Definition of `LeftCommutative` / `LeftCommutative` 的定义

English:
class LeftCommutative
  parameters: (op : α -> β -> β)
  axioms and operations (1):
...
/--
Definition of `RightCommutative` / `RightCommutative` 的定义

English:
class RightCommutative
  parameters: (op : β -> α -> β)
  axioms and operations (1):
...
/--
theorem `IsSymmOp.flip_eq` / 定理 `IsSymmOp.flip_eq`

English:
theorem IsSymmOp.flip_eq
  given: (op : α -> α -> β) [IsSymmOp op]
  statement: flip op = op
...
Depends on / 依赖: IsSymmOp, IsSymmOp.symm_op, symm_op
-/
