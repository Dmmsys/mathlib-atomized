...
/--
theorem `iff_exists_tensorProduct` / 定理 `iff_exists_tensorProduct`

English:
theorem iff_exists_tensorProduct
  ...
  have : forall t : S otimes[R] S, TensorProduct.lmul' R t = 1 ↔ 1 - t in KaehlerDifferential.ideal R S := by
    intro 
...
Depends on / 依赖: Ideal.cotangent_subsingleton_iff, Ideal.isIdempotentElem_iff_of_fg, Ideal.span, KaehlerDifferential, KaehlerDifferential.ideal, KaehlerDifferential.ideal_fg, KaehlerDifferential.span_range_eq_ideal, RingHom, RingHom.mem_ker, TensorProduct, TensorProduct.lmul, cotangent_subsingleton_iff, eq_comm, formallyUnramified_iff, ideal_fg, isIdempotentElem_iff_of_fg, map_one, map_sub, mem_ker, otimes
-/
theorem iff_exists_tensorProduct [EssFiniteType R S] :
    FormallyUnramified R S ↔ exists t : S otimes[R] S,
      (forall s, ((1 : S) otimesₜ[R] s - s otimesₜ[R] (1 : S)) * t = 0) ∧ TensorProduct.lmul' R t = 1 := by
  rw [formallyUnramified_iff]; rw [KaehlerDifferential]; rw [Ideal.cotangent_subsingleton_iff]; rw [Ideal.isIdempotentElem_iff_of_fg _
      (KaehlerDifferential.ideal_fg R S)]
  have : forall t : S otimes[R] S, TensorProduct.lmul' R t = 1 ↔ 1 - t in KaehlerDifferential.ideal R S := by
    intro t
  ...
  · rintro ⟨e, he₁, he₂ : _ = Ideal.span _⟩
    refine ⟨1 - e, ?_, ?_⟩
    · intro s
      obtain ⟨x, hx⟩ : e ∣ 1 otimesₜ[R] s - s otimesₜ[R] 1 := by
        rw [← Ideal.mem_span_singleton]; rw [← he₂]
        exact Ideal.subset_span ⟨s, rfl⟩
      rw [hx]; rw [mul_comm]; rw [← mul_assoc]; rw [sub_mul]; rw [one_mul]; rw [he₁.eq]; rw [sub_self]; rw [zero_mul]
    · rw [sub_sub_cancel, he₂, Ideal.mem_span_singleton]
  · rintro ⟨t, ht₁, ht₂⟩
    use 1 - t
    rw [← sub_sub_self 1 t] at ht₁; generalize 1 - t = e at *
    constructor
    · suffices e in (Submodule.span (S otimes[R] S) {1 - e}).annihilator by
    ...
    · apply le_antisymm <;> simp only [Ideal.submodule_span_eq, Ideal.mem_span_singleton, ht₂,
      ...
      intro s
      use 1 otimesₜ[R] s - s otimesₜ[R] 1
      linear_combination ht₁ s

/--
lemma `finite_of_free_aux` / 引理 `finite_of_free_aux`

English:
lemma finite_of_free_aux
  statement: (I) [DecidableEq I] (b : Basis I R S)
  proof: by
  ...
  conv_lhs =>
    simp only [TensorProduct.tmul_mul_tmul, one_mul, mul_comm x (b _),
      ← show forall i, Finsupp.linearCombination _ b (a i) = b i * x from
...
Depends on / 依赖: Finset, Finset.mul_sum, Finsupp, Finsupp.coe_lsum, Finsupp.linearCombination, Finsupp.sum, LinearMap, LinearMap.coe_smulRight, LinearMap.id_coe, TensorProduct, TensorProduct.smul_tmul, TensorProduct.tmul_mul_tmul, TensorProduct.tmul_sum, b.linearCombination_repr, b.repr, coe_lsum, coe_smulRight, conv_lhs, id_coe, id_eq
-/
lemma finite_of_free_aux (I) [DecidableEq I] (b : Basis I R S)
    (f : I ->₀ S) (x : S) (a : I -> I ->₀ R) (ha : a = fun i => b.repr (b i * x)) :
    (1 otimesₜ[R] x * Finsupp.sum f fun i y => y otimesₜ[R] b i) =
      Finset.sum (f.support.biUnion fun i => (a i).support) fun k =>
      Finsupp.sum (b.repr (f.sum fun i y => a i k • y)) fun j c => c • b j otimesₜ[R] b k := by
  rw [Finsupp.sum]; rw [Finset.mul_sum]
  subst ha
  let a i := b.repr (b i * x)
  conv_lhs =>
  ...
  have h₁ : forall k,
    (Finsupp.sum (Finsupp.sum f fun i y => a i k • b.repr y) fun j z => z • b j otimesₜ[R] b k) =
      (f.sum fun i y => (b.repr y).sum fun j z => a i k • z • b j otimesₜ[R] b k) := by
    intro i
  ...
  have h₂ : forall (x : S), ((b.repr x).support.sum fun a => b.repr x a • b a) = x := by
...
English:
definition elem
  signature: : S otimes[R] S
  body: (iff_exists_tensorProduct.mp inferInstance).choose

中文:
定义 elem
  签名: : S otimes[R] S
  定义体: (iff_exists_tensorProduct.mp inferInstance).choose

Depends on / 依赖: iff_exists_tensorProduct, iff_exists_tensorProduct.mp
-/
def elem : S otimes[R] S :=
  (iff_exists_tensorProduct.mp inferInstance).choose

/--
lemma `one_tmul_sub_tmul_one_mul_elem` / 引理 `one_tmul_sub_tmul_one_mul_elem`

English:
lemma one_tmul_sub_tmul_one_mul_elem
  proof: (iff_exists_tensorProduct.mp inferInstance).choose_spec.1 s

中文:
引理 one_tmul_sub_tmul_one_mul_elem
  证明: (iff_exists_tensorProduct.mp inferInstance).choose_spec.1 s

Depends on / 依赖: choose_spec, iff_exists_tensorProduct, iff_exists_tensorProduct.mp
-/
lemma one_tmul_sub_tmul_one_mul_elem
    (s : S) : (1 otimesₜ s - s otimesₜ 1) * elem R S = 0 :=
  (iff_exists_tensorProduct.mp inferInstance).choose_spec.1 s

/--
lemma `one_tmul_mul_elem` / 引理 `one_tmul_mul_elem`

English:
lemma one_tmul_mul_elem
  proof: by
  rw [← sub_eq_zero]; rw [← sub_mul]; rw [one_tmul_sub_tmul_one_mul_elem]

中文:
引理 one_tmul_mul_elem
  证明: by
  rw [← sub_eq_zero]; rw [← sub_mul]; rw [one_tmul_sub_tmul_one_mul_elem]

Depends on / 依赖: one_tmul_sub_tmul_one_mul_elem, sub_eq_zero, sub_mul
-/
lemma one_tmul_mul_elem
    (s : S) : (1 otimesₜ s) * elem R S = (s otimesₜ 1) * elem R S := by
  rw [← sub_eq_zero]; rw [← sub_mul]; rw [one_tmul_sub_tmul_one_mul_elem]

/--
lemma `lmul_elem` / 引理 `lmul_elem`

English:
lemma lmul_elem
  proof: (iff_exists_tensorProduct.mp inferInstance).choose_spec.2

中文:
引理 lmul_elem
  证明: (iff_exists_tensorProduct.mp inferInstance).choose_spec.2
...
/--
lemma `finite_of_free` / 引理 `finite_of_free`

English:
lemma finite_of_free
  given: [Module.Free R S]
  ...
  -- `∑ₛ fᵢbᵢ = 1` and `∀ x : S, xfᵢ ⊗ bᵢ = aᵢ ⊗ xfᵢ` which ex
...
Depends on / 依赖: ChooseBasisIndex, Module, Module.Free.ChooseBasisIndex, classical
-/
lemma finite_of_free [Module.Free R S] : Module.Finite R S := by
  ...
  have ⟨f, hf⟩ : exists (a : I ->₀ S), elem R S = a.sum (fun i x => x otimesₜ b i) := by
  ...
  have hG : forall j ∉ (Finset.biUnion f.support fun i => (a i).support),
  ...
  have : F = G := by
    ...
    suffices (F.sum fun a f => f.sum fun b' c => c • b b' otimesₜ[R] b a) =
        G.sum fun a f => f.sum fun b' c => c • b b' otimesₜ[R] b a by
      simpa [Finsupp.linearCombination_apply, Finsupp.sum_uncurry_index]
    have : forall i, ((b.repr (x * f i)).sum fun j k => k • b j otimesₜ[R] b i) = (x * f i) otimesₜ[R] b i := by
      intro i
      simp_rw [Finsupp.sum, TensorProduct.smul_tmul', ← TensorProduct.sum_tmul]
      congr 1
      exact b.linearCombination_repr _
    rw [Finsupp.onFinset_sum]; rw [Finsupp.onFinset_sum]
    · trans (x otimesₜ 1) * elem R S
  ...
  have : forall j, x * f j = f.sum fun i y => a i j • y := by
    intro j
    apply b.repr.injective
    exact DFunLike.congr_fun this j
  -- Since `∑ₛ fⱼbⱼ = 1`, `x = ∑ₛ aᵢⱼfᵢbⱼ` is indeed in the span of `{ fᵢbⱼ | i, j ∈ s }`.
  rw [← mul_one x]; rw [← @lmul_elem R]; rw [hf]; rw [map_finsuppSum]; rw [Finsupp.sum]; rw [Finset.mul_sum]
...
definition sec
  signature: :
  body: ((TensorProduct.AlgebraTensorModule.mapBilinear R S S S S S M
  ...
  map_smul' r m := by
    simp only [AddHom.toFun_eq_coe, LinearMap.coe_toAddHom, LinearMap.coe_comp, Function.comp_apply,
      LinearMap.flip_apply, TensorProduct.Al

中文:
定义 sec
  签名: :
  定义体: ((TensorProduct.AlgebraTensorModule.mapBilinear R S S S S S M
    LinearMap.id).flip (elem R S)).comp (lsmul R R M).toLinearMap.flip
  map_smul' r m := by
    simp only [AddHom.toFun_eq_coe, LinearMap.coe_toAddHom, LinearMap.coe_comp, Function.comp_apply,
      LinearMap.flip_apply, TensorProduct.Al

Depends on / 依赖: AlgebraTensorModule, TensorProduct, TensorProduct.AlgebraTensorModule.mapBilinear, mapBilinear
-/
def sec :
    M ->ₗ[S] S otimes[R] M where
  ...
  map_smul' r m := by
    ...
    trans (TensorProduct.AlgebraTensorModule.map (LinearMap.id (R := S) (M := S))
      ((LinearMap.flip (AlgHom.toLinearMap (lsmul R R M))) m)) ((1 otimesₜ r) * elem R S)
    · induction elem R S using TensorProduct.induction_on
      · simp
      · simp [smul_comm r]
      · simp only [map_add, mul_add, *]
    · have := one_tmul_sub_tmul_one_mul_elem (R := R) r
      rw [sub_mul]; rw [sub_eq_zero] at this
      rw [this]
      induction elem R S using TensorProduct.induction_on
      · simp
...
Depends on / 依赖: AlgebraTensorModule, Function, Function.comp_apply, LinearMap, LinearMap.coe_comp, LinearMap.coe_mk, LinearMap.coe_toAddHom, LinearMap.flip_apply, LinearMap.id_coe, TensorProduct, TensorProduct.AlgebraTensorModule.lift_apply, TensorProduct.AlgebraTensorModule.mapBilinear_apply, TensorProduct.induction_on, TensorProduct.lmul, add_smul, coe_comp, coe_mk, coe_toAddHom, comp_apply, flip_apply
-/
lemma comp_sec :
...
/--
lemma `flat_of_restrictScalars` / 引理 `flat_of_restrictScalars`

English:
lemma flat_of_restrictScalars
  given: [Module.Flat R M]
  statement: Module.Flat S M
  proof: Module.Flat.of_retract _ _ (comp_sec R S M)
...
/--
lemma `projective_of_restrictScalars` / 引理 `projective_of_restrictScalars`

English:
lemma projective_of_restrictScalars
  given: [Module.Projective R M]
  statement: Module.Projective S M
  proof: Module.Projective.of_split _ _ (comp_sec R S M)
