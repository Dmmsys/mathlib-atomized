...
/-
**Fin.val_intCast** 是 Mathlib 中的一个定理，位于命名空间 `Fin`。
形式化陈述：∀ {n : ℕ} [inst : NeZero n] (x : ℤ), ↑↑x = (x % ↑n).toNat
参数：x : ℤ；x % ↑n。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Fin.intCast_def'`：intCast_def' {n : Nat} [NeZero n] (x : Int) : (x : Fin
 n) = if 0 <= x then ↑x.natAbs else -↑x.natAbs
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `if_neg`：∀ {c : Prop} {h : Decidable c}, ¬c → ∀ {α : Sort u} {t e : α}, (
if c then t else e) = e
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `Fin.val_neg`：∀ {n : ℕ} [inst : NeZero n] (x : Fin n), ↑(-x) = if x = 0 t
hen 0 else n - ↑x
· 使用定理 `ite_congr`：∀ {α : Sort u_1} {b c : Prop} {x y u v : α} {s : Decidable b}
 [inst : Decidable c],   b = c → (c → x = u) → (¬c → y = v) → (if b then x else…
· 使用定理 `Int.emod_natAbs_of_neg`：∀ {x : ℤ}, x < 0 → ∀ {n : ℕ}, n ≠ 0 → x.natAbs %
 n = if ↑n ∣ x then 0 else n - (x % ↑n).toNat
...
· 使用定理 `Int.toNat_zero`：Int.toNat 0 = 0
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Int.emod_natAbs_of_nonneg`：∀ {x : ℤ}, 0 ≤ x → ∀ {n : ℕ}, x.natAbs % n = 
(x % ↑n).toNat
...
/-
**Fin.instCommSemigroup** 是 Mathlib 中的一个实例，位于命名空间 `Fin`。
形式化陈述：instCommSemigroup (n : Nat) : CommSemigroup (Fin n) where mul_assoc
参数：n : Nat。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Fin.mul_comm`：∀ {n : ℕ} (a b : Fin n), a * b = b * a
...
/-
**Fin.left_distrib_aux** 是 Mathlib 中的一个定理，位于命名空间 `Fin`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Fin.instDistrib** 是 Mathlib 中的一个实例，位于命名空间 `Fin`。
形式化陈述：instDistrib (n : Nat) : Distrib (Fin n) where left_distrib
参数：n : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Fin.instNonUnitalCommRing** 是 Mathlib 中的一个实例，位于命名空间 `Fin`。
形式化陈述：instNonUnitalCommRing (n : Nat) [NeZero n] : NonUnitalCommRing (Fin n) whe
re zero_mul
参数：n : Nat。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Fin.zero_mul`：∀ {n : ℕ} [inst : NeZero n] (k : Fin n), 0 * k = 0
· 使用定理 `Fin.mul_zero`：∀ {n : ℕ} [inst : NeZero n] (k : Fin n), k * 0 = 0
-/
instance instNonUnitalCommRing (n : ℕ) [NeZero n] : NonUnitalCommRing (Fin n) where
  zero_mul := Fin.zero_mul
  mul_zero := Fin.mul_zero
/-
**Fin.instCommMonoid** 是 Mathlib 中的一个实例，位于命名空间 `Fin`。
形式化陈述：instCommMonoid (n : Nat) [NeZero n] : CommMonoid (Fin n) where one_mul
参数：n : Nat。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Fin.one_mul`：∀ {n : ℕ} [inst : NeZero n] (k : Fin n), 1 * k = k
· 使用定理 `Fin.mul_one`：∀ {n : ℕ} [i : NeZero n] (k : Fin n), k * 1 = k
-/
...
/-
**Fin.instHasDistribNeg** 是 Mathlib 中的一个实例，位于命名空间 `Fin`。
形式化陈述：instHasDistribNeg (n : Nat) : HasDistribNeg (Fin n) where mul_neg
参数：n : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Note this is more general than `Fin.instCommRing` as it applies (vacuously) to `
Fin 0` too.
-/
...
@[instance_reducible]
/-
**Fin.instCommRing** 是 Mathlib 中的一个定义，位于命名空间 `Fin`。
形式化陈述：instCommRing (n : Nat) [NeZero n] : CommRing (Fin n) where intCast n
参数：n : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
This is not a global instance, but can introduced locally using `open Fin.CommRi
ng in ...`.
...
def ZMod : ℕ → Type
  ...
  | n + 1 => Fin (n + 1)
/-
**ZMod.decidableEq** 是 Mathlib 中的一个定义，位于命名空间 `ZMod`。
形式化陈述：(n : ℕ) → DecidableEq (ZMod n)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ZMod.repr** 是 Mathlib 中的一个定义，位于命名空间 `ZMod`。
形式化陈述：(n : ℕ) → Repr (ZMod n)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ZMod.instUnique** 是 Mathlib 中的一个实例，位于命名空间 `ZMod`。
形式化陈述：instUnique : Unique (ZMod 1)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance instUnique : Unique (ZMod 1) := Fin.instUnique
/-
**ZMod.fintype** 是 Mathlib 中的一个定义，位于命名空间 `ZMod`。
形式化陈述：(n : ℕ) → [NeZero n] → Fintype (ZMod n)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ZMod.infinite** 是 Mathlib 中的一个实例，位于命名空间 `ZMod`。
形式化陈述：infinite : Infinite (ZMod 0)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**ZMod.card** 是 Mathlib 中的一个定理，位于命名空间 `ZMod`。
形式化陈述：card (n : Nat) [Fintype (ZMod n)] : Fintype.card (ZMod n) = n
参数：n : Nat；ZMod n。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
field. -/
/-
**ZMod.commRing** 是 Mathlib 中的一个实例，位于命名空间 `ZMod`。
形式化陈述：commRing (n : Nat) : CommRing (ZMod n) where add
参数：n : Nat。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
...
--- 原说明 ---
We define each field by cases, to ensure that the eta-expanded `ZMod.commRing` i
s defeq to the
original, this helps avoid diamonds with instances coming from classes extending
 `CommRing` such as
field.
-/
...
/-
**ZMod.inhabited** 是 Mathlib 中的一个实例，位于命名空间 `ZMod`。
形式化陈述：inhabited (n : Nat) : Inhabited (ZMod n)
参数：n : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
