...
-/
/-
**Lean.Elab.Tactic.applyCongr** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Elab.Tactic.applyCongr (q : Option Expr) : TacticM Unit
参数：q : Option Expr。
该定义给出了上述对象。
...
When called without an argument `apply_congr` will try applying all lemmas marke
d with `@[congr]`.
Otherwise `apply_congr e` will apply the lemma `e`.
...
As usual, `apply_congr` can create new goals;
any of these which are _not_ equations with a metavariable on the right-hand sid
e
will be hard to deal with in `conv` mode.
Thus `apply_congr` automatically calls `intros` on any new goals,
and fails if they are not then equations.
...
example (f g : ℤ → ℤ) (S : Finset ℤ) (h : ∀ m ∈ S, f m = g m) :
  ...
  conv_lhs =>
    ...
    -- because we are only given the opportunity to rewrite `f`.
    -- However `apply_congr` uses the appropriate `@[congr]` lemma,
    -- so we get to rewrite `f x`, in the presence of the crucial `H : x ∈ S` hy
pothesis.
    apply_congr
    · skip
    · simp [*]
...
In the above example, when the `apply_congr` tactic is called it gives the hypot
hesis `H : x ∈ S`
which is then used to rewrite the `f x` to `g x`.
-/
