...
/-
**SmoothBumpCovering.embeddingPiTangent** 是 Mathlib 中的一个定义，位于命名空间 `SmoothBumpCov
ering`。
形式化陈述：embeddingPiTangent : C^∞⟮I, M; 𝓘(Real, ι -> E × Real), ι -> E × Real⟯ wher
e val x i
该定义给出了上述对象。
...
@[local simp]
/-
**SmoothBumpCovering.embeddingPiTangent_coe** 是 Mathlib 中的一个定理，位于命名空间 `SmoothBum
pCovering`。
形式化陈述：embeddingPiTangent_coe : ⇑f.embeddingPiTangent = fun x i => (f i x • extCh
artAt I (f.c i) x, f i x)
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem embeddingPiTangent_coe :
  ...
  (rfl)
/-
**SmoothBumpCovering.embeddingPiTangent_injOn** 是 Mathlib 中的一个定理，位于命名空间 `SmoothB
umpCovering`。
形式化陈述：embeddingPiTangent_injOn : InjOn f.embeddingPiTangent s
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SmoothBumpCovering.embeddingPiTangent_injective** 是 Mathlib 中的一个定理，位于命名空间 `Smo
othBumpCovering`。
形式化陈述：embeddingPiTangent_injective (f : SmoothBumpCovering ι I M) : Injective f.
embeddingPiTangent
参数：f : SmoothBumpCovering ι I M。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SmoothBumpCovering.comp_embeddingPiTangent_mfderiv** 是 Mathlib 中的一个定理，位于命名空间 `
SmoothBumpCovering`。
形式化陈述：comp_embeddingPiTangent_mfderiv (x : M) (hx : x in s) : ((ContinuousLinear
Map.fst Real E Real).comp (@ContinuousLinearMap.proj Real _ ι (fun _ => E × Real
) _ _ (fun _ => inferInstance) (f.ind x hx))).comp (mfderiv I 𝓘(Real, ι -> E × R
eal) f.embeddingPiTangent x) = mfderiv% (chartAt H (f.c (f.ind x hx))) x
参数：x : M；hx : x in s。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SmoothBumpCovering.embeddingPiTangent_ker_mfderiv** 是 Mathlib 中的一个定理，位于命名空间 `S
moothBumpCovering`。
形式化陈述：embeddingPiTangent_ker_mfderiv (x : M) (hx : x in s) : (mfderiv I 𝓘(Real, 
ι -> E × Real) f.embeddingPiTangent x).ker = ⊥
参数：x : M；hx : x in s。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SmoothBumpCovering.embeddingPiTangent_injective_mfderiv** 是 Mathlib 中的一个定理，位于命
名空间 `SmoothBumpCovering`。
形式化陈述：embeddingPiTangent_injective_mfderiv (x : M) (hx : x in s) : Injective (mf
deriv I 𝓘(Real, ι -> E × Real) f.embeddingPiTangent x)
参数：x : M；hx : x in s。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
