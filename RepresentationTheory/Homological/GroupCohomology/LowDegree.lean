...
to `A` as a `k`-module. -/
/-
**groupCohomology.cochainsIso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The 0th object in the complex of inhomogeneous cochains of `A : Rep k G` is isom
orphic
to `A` as a `k`-module.
-/
...
--- 原说明 ---
The 1st object in the complex of inhomogeneous cochains of `A : Rep k G` is isom
orphic
to `Fun(G, A)` as a `k`-module.
-/
...
--- 原说明 ---
The 2nd object in the complex of inhomogeneous cochains of `A : Rep k G` is isom
orphic
to `Fun(G², A)` as a `k`-module.
-/
...
--- 原说明 ---
The 3rd object in the complex of inhomogeneous cochains of `A : Rep k G` is isom
orphic
to `Fun(G³, A)` as a `k`-module.
-/
...
@[simps!]
/-
**groupCohomology.d** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**groupCohomology.d** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.subtype_comp_d** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**groupCohomology.comp_d** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Let `C(G, A)` denote the complex of inhomogeneous cochains of `A : Rep k G`. Thi
s lemma
...
@[reassoc, elementwise]
/-
**groupCohomology.eq_d** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps! -isSimp f g]
/-
**groupCohomology.shortComplexH0** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：shortComplexH0 : ShortComplex (ModuleCat k)
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用引理 `groupCohomology.subtype_comp_d₀₁`：subtype_comp_d₀₁ : ModuleCat.ofHom (A.
ρ.invariants.subtype) ≫ d₀₁ A = 0
...
@[simps! -isSimp f g]
/-
**groupCohomology.shortComplexH1** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：shortComplexH1 : ShortComplex (ModuleCat k)
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `groupCohomology.d₀₁_comp_d₁₂`：d₀₁_comp_d₁₂ : d₀₁ A ≫ d₁₂ A = 0
...
@[simps! -isSimp f g]
/-
**groupCohomology.shortComplexH2** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
...
--- 原说明 ---
The 1-cocycles `Z¹(G, A)` of `A : Rep k G`, defined as the kernel of the map
`Fun(G, A) → Fun(G × G, A)` sending `(f, (g₁, g₂)) ↦ ρ_A(g₁)(f(g₂)) - f(g₁g₂) + 
f(g₁).`
-/
...
--- 原说明 ---
When `A : Rep k G` is a trivial representation of `G`, `Z¹(G, A)` is isomorphic 
to the
group homs `G → A`.
-/
...
/-
**groupCohomology.coboundaries** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**groupCohomology.coboundaries** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
variable (A) in
/-
**groupCohomology.coboundaries** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.coboundariesToCocycles** 是 Mathlib 中的一个缩写定义，位于命名空间 `groupCohom
ology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**groupCohomology.coboundariesToCocycles** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomol
ogy`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.IsCocycle** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**groupCohomology.map_one_of_isCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomolog
y`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_one_fst_of_isCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohom
ology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_one_snd_of_isCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohom
ology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_inv_of_isCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomolog
y`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.smul_map_inv_sub_map_inv_of_isCocycle** 是 Mathlib 中的一个定理，位于命名空
间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.IsCoboundary** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A function `f : G → A` satisfies the 1-coboundary condition if there's `x : A` s
uch that
`g • x - x = f(g)` for all `g : G`.
-/
...
--- 原说明 ---
A function `f : G × G → A` satisfies the 2-coboundary condition if there's `x : 
G → A` such
that `g • x(h) - x(gh) + x(g) = f(g, h)` for all `g, h : G`.
-/
...
@[simps]
/-
**groupCohomology.cocyclesOfIsCocycle** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology
`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a `k`-module `A` with a compatible `DistribMulAction` of `G`, and a functi
on
`f : G → A` satisfying the 1-cocycle condition, produces a 1-cocycle for the rep
resentation on
`A` induced by the `DistribMulAction`.
-/
...
/-
**groupCohomology.isCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps]
/-
**groupCohomology.coboundariesOfIsCoboundary** 是 Mathlib 中的一个定义，位于命名空间 `groupCoh
omology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Given a `k`-module `A` with a compatible `DistribMulAction` of `G`, and a functi
on
`f : G → A` satisfying the 1-coboundary condition, produces a 1-coboundary for t
he representation
on `A` induced by the `DistribMulAction`.
-/
...
/-
**groupCohomology.isCoboundary** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
Given a `k`-module `A` with a compatible `DistribMulAction` of `G`, and a functi
on
`f : G × G → A` satisfying the 2-cocycle condition, produces a 2-cocycle for the
 representation on
`A` induced by the `DistribMulAction`.
-/
...
Given a `k`-module `A` with a compatible `DistribMulAction` of `G`, and a functi
on
`f : G × G → A` satisfying the 2-coboundary condition, produces a 2-coboundary f
or the
representation on `A` induced by the `DistribMulAction`.
-/
...
`f(gh) = g • f(h) * f(g)` for all `g, h : G`. -/
/-
**groupCohomology.IsMulCocycle** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**groupCohomology.map_one_of_isMulCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomo
logy`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_one_fst_of_isMulCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCo
homology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_one_snd_of_isMulCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCo
homology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.map_inv_of_isMulCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomo
logy`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.smul_map_inv_div_map_inv_of_isMulCocycle** 是 Mathlib 中的一个定理，位于
命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.IsMulCoboundary** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A function `f : G → M` satisfies the multiplicative 1-coboundary condition if th
ere's `x : M`
such that `g • x / x = f(g)` for all `g : G`.
-/
...
--- 原说明 ---
A function `f : G × G → M` satisfies the 2-coboundary condition if there's `x : 
G → M` such
that `g • x(h) / x(gh) * x(g) = f(g, h)` for all `g, h : G`.
-/
...
@[simps]
/-
**groupCohomology.cocyclesOfIsMulCocycle** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomol
ogy`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given an abelian group `M` with a `MulDistribMulAction` of `G`, and a function
`f : G → M` satisfying the multiplicative 1-cocycle condition, produces a 1-cocy
cle for the
representation on `Additive M` induced by the `MulDistribMulAction`.
-/
...
/-
**groupCohomology.isMulCocycle** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps]
/-
**groupCohomology.coboundariesOfIsMulCoboundary** 是 Mathlib 中的一个定义，位于命名空间 `group
Cohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Given an abelian group `M` with a `MulDistribMulAction` of `G`, and a function
`f : G → M` satisfying the multiplicative 1-coboundary condition, produces a
1-coboundary for the representation on `Additive M` induced by the `MulDistribMu
lAction`.
-/
...
--- 原说明 ---
Given an abelian group `M` with a `MulDistribMulAction` of `G`, and a function
`f : G × G → M` satisfying the multiplicative 2-cocycle condition, produces a 2-
cocycle for the
representation on `cochainsIso₁Additive M` induced by the `MulDistribMulAction`.
-/
...
/-
**groupCohomology.shortComplexH0_exact** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomolog
y`。
...
/-
**groupCohomology.dArrowIso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**groupCohomology.cocyclesIso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[reassoc (attr := simp), elementwise (attr := simp)]
/-
**groupCohomology.cocyclesIso** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps! hom inv]
/-
**groupCohomology.isoShortComplexH1** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：isoShortComplexH1 : (inhomogeneousCochains A).sc 1 ≅ shortComplexH1 A
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `groupCohomology.comp_d₀₁_eq`：comp_d₀₁_eq : (cochainsIso₀ A).hom ≫ d₀₁ A 
= (inhomogeneousCochains A).d 0 1 ≫ (cochainsIso₁ A).hom
· 使用定理 `groupCohomology.comp_d₁₂_eq`：comp_d₁₂_eq : (cochainsIso₁ A).hom ≫ d₁₂ A 
= (inhomogeneousCochains A).d 1 2 ≫ (cochainsIso₂ A).hom

--- 原说明 ---
The short complex `A --d₀₁--> Fun(G, A) --d₁₂--> Fun(G × G, A)` is isomorphic to
 the 1st
short complex associated to the complex of inhomogeneous cochains of `A`.
-/
...
/-
**groupCohomology.isoCocycles** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[reassoc (attr := simp), elementwise (attr := simp)]
/-
**groupCohomology.isoCocycles** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[reassoc (attr := simp), elementwise (attr := simp)]
/-
**groupCohomology.toCocycles_comp_isoCocycles** 是 Mathlib 中的一个引理，位于命名空间 `groupCo
homology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps! hom inv]
/-
**groupCohomology.isoShortComplexH2** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：isoShortComplexH2 : (inhomogeneousCochains A).sc 2 ≅ shortComplexH2 A
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `groupCohomology.comp_d₁₂_eq`：comp_d₁₂_eq : (cochainsIso₁ A).hom ≫ d₁₂ A 
= (inhomogeneousCochains A).d 1 2 ≫ (cochainsIso₂ A).hom
· 使用定理 `groupCohomology.comp_d₂₃_eq`：comp_d₂₃_eq : (cochainsIso₂ A).hom ≫ d₂₃ A 
= (inhomogeneousCochains A).d 2 3 ≫ (cochainsIso₃ A).hom

--- 原说明 ---
The short complex `Fun(G, A) --d₁₂--> Fun(G × G, A) --dTwo--> Fun(G × G × G, A)`
 is
isomorphic to the 2nd short complex associated to the complex of inhomogeneous c
ochains of `A`.
-/
...
/-
**groupCohomology.H0** 是 Mathlib 中的一个缩写定义，位于命名空间 `groupCohomology`。
形式化陈述：H0
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Shorthand for the 0th group cohomology of a `k`-linear `G`-representation `A`, `
H⁰(G, A)`,
defined as the 0th cohomology of the complex of inhomogeneous cochains of `A`.
-/
...
/-
**groupCohomology.H0Iso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：H0Iso : H0 A ≅ ModuleCat.of k A.ρ.invariants
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The 0th group cohomology of `A`, defined as the 0th cohomology of the complex of
 inhomogeneous
cochains, is isomorphic to the invariants of the representation on `A`.
-/
...
@[reassoc (attr := simp), elementwise (attr := simp)]
/-
**groupCohomology.** 是 Mathlib 中的一个引理，位于命名空间 `groupCohomology`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[elab_as_elim]
/-
**groupCohomology.H0_induction_on** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
形式化陈述：H0_induction_on {C : H0 A -> Prop} (x : H0 A) (h : forall x : A.ρ.invarian
ts, C ((H0Iso A).inv x)) : C x
参数：x : H0 A；h : forall x : A.ρ.invariants, C ((H0Iso A).inv x)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
...
/-
**groupCohomology.H0IsoOfIsTrivial** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
...
@[simp]
/-
**groupCohomology.H0IsoOfIsTrivial_hom** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomolog
y`。
形式化陈述：H0IsoOfIsTrivial_hom : (H0IsoOfIsTrivial A).hom = (H0Iso A).hom ≫ (shortCo
mplexH0 A).f
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**groupCohomology.H0IsoOfIsTrivial_inv_apply** 是 Mathlib 中的一个定理，位于命名空间 `groupCoh
omology`。
形式化陈述：H0IsoOfIsTrivial_inv_apply (x : A) : (H0IsoOfIsTrivial A).inv x = (H0Iso A
).inv ⟨x, by simp⟩
参数：x : A。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
Shorthand for the 1st group cohomology of a `k`-linear `G`-representation `A`, `
H¹(G, A)`,
defined as the 1st cohomology of the complex of inhomogeneous cochains of `A`.
-/
...
--- 原说明 ---
The quotient map from the 1-cocycles of `A`, as a submodule of `G → A`, to `H¹(G
, A)`.
-/
...
@[elab_as_elim]
/-
**groupCohomology.H1_induction_on** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
形式化陈述：H1_induction_on {C : H1 A -> Prop} (x : H1 A) (h : forall x : cocycles₁ A,
 C (H1π A x)) : C x
参数：x : H1 A；h : forall x : cocycles₁ A, C (H1π A x)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `groupCohomology_induction_on`：groupCohomology_induction_on [Group G] {A 
: Rep k G} {n : Nat} {C : groupCohomology A n -> Prop} (x : groupCohomology A n)
 (h : forall x : c…
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `CategoryTheory.Iso.hom_inv_id_apply`：∀ {C : Type u} [inst : CategoryTheo
...
/-
**groupCohomology.H1Iso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
...
The 1st group cohomology of `A`, defined as the 1st cohomology of the complex of
 inhomogeneous
cochains, is isomorphic to `cocycles₁ A ⧸ coboundaries₁ A`, which is a simpler t
ype.
-/
...
group homs `G → A`. -/
/-
**groupCohomology.H1IsoOfIsTrivial** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：H1IsoOfIsTrivial : H1 A ≅ ModuleCat.of k (Additive G ->+ A)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
When `A : Rep k G` is a trivial representation of `G`, `H¹(G, A)` is isomorphic 
to the
group homs `G → A`.
-/
...
variable {A}
/-
**groupCohomology.H1IsoOfIsTrivial_H1** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology
`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**groupCohomology.H1IsoOfIsTrivial_inv_apply** 是 Mathlib 中的一个定理，位于命名空间 `groupCoh
omology`。
形式化陈述：H1IsoOfIsTrivial_inv_apply (f : Additive G ->+ A) : (H1IsoOfIsTrivial A).i
nv f = H1π A ((cocycles₁IsoOfIsTrivial A).inv f)
参数：f : Additive G ->+ A。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
--- 原说明 ---
Shorthand for the 2nd group cohomology of a `k`-linear `G`-representation `A`, `
H²(G, A)`,
defined as the 2nd cohomology of the complex of inhomogeneous cochains of `A`.
-/
...
--- 原说明 ---
The quotient map from the 2-cocycles of `A`, as a submodule of `G × G → A`, to `
H²(G, A)`.
-/
...
@[elab_as_elim]
/-
**groupCohomology.H2_induction_on** 是 Mathlib 中的一个定理，位于命名空间 `groupCohomology`。
形式化陈述：H2_induction_on {C : H2 A -> Prop} (x : H2 A) (h : forall x : cocycles₂ A,
 C (H2π A x)) : C x
参数：x : H2 A；h : forall x : cocycles₂ A, C (H2π A x)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `groupCohomology_induction_on`：groupCohomology_induction_on [Group G] {A 
: Rep k G} {n : Nat} {C : groupCohomology A n -> Prop} (x : groupCohomology A n)
...
cochains, is isomorphic to `cocycles₂ A ⧸ coboundaries₂ A`, which is a simpler type. -/
/-
**groupCohomology.H2Iso** 是 Mathlib 中的一个定义，位于命名空间 `groupCohomology`。
形式化陈述：H2Iso : H2 A ≅ (shortComplexH2 A).moduleCatLeftHomologyData.H
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The 2nd group cohomology of `A`, defined as the 2nd cohomology of the complex of
 inhomogeneous
cochains, is isomorphic to `cocycles₂ A ⧸ coboundaries₂ A`, which is a simpler t
ype.
-/
