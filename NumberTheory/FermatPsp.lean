...
-/
/-
**Nat.ProbablePrime** 是 Mathlib 中的一个定义，位于命名空间 `Nat`。
形式化陈述：ProbablePrime (n b : Nat) : Prop
参数：n b : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`n` is a probable prime to base `b` if `n` passes the Fermat primality test; tha
t is, `n` divides
`b ^ (n - 1) - 1`.
This definition implies that all numbers are probable primes to base 0 or 1, and
 that 0 and 1 are
probable primes to any base.
-/
...
-/
/-
**Nat.FermatPsp** 是 Mathlib 中的一个定义，位于命名空间 `Nat`。
形式化陈述：FermatPsp (n b : Nat) : Prop
参数：n b : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`n` is a Fermat pseudoprime to base `b` if `n` is a probable prime to base `b` a
nd is composite. By
this definition, all composite natural numbers are pseudoprimes to base 0 and 1.
 This definition
also permits `n` to be less than `b`, so that 4 is a pseudoprime to base 5, for 
example.
-/
def FermatPsp (n b : ℕ) : Prop :=
  ProbablePrime n b ∧ ¬n.Prime ∧ 1 < n
/-
**Nat.decidableProbablePrime** 是 Mathlib 中的一个实例，位于命名空间 `Nat`。
形式化陈述：decidableProbablePrime (n b : Nat) : Decidable (ProbablePrime n b)
参数：n b : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.decidablePsp** 是 Mathlib 中的一个实例，位于命名空间 `Nat`。
形式化陈述：decidablePsp (n b : Nat) : Decidable (FermatPsp n b)
参数：n b : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Nat.coprime_of_probablePrime** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：coprime_of_probablePrime {n b : Nat} (h : ProbablePrime n b) (h₁ : 1 <= n)
 (h₂ : 1 <= b) : Nat.Coprime n b
参数：h : ProbablePrime n b；h₁ : 1 <= n；h₂ : 1 <= b。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
--- 原说明 ---
If `n` passes the Fermat primality test to base `b`, then `n` is coprime with `b
`, assuming that
`n` and `b` are both positive.
-/
theorem coprime_of_probablePrime {n b : ℕ} (h : ProbablePrime n b) (h₁ : 1 ≤ n) (h₂ : 1 ≤ b) :
  ...
  · rw [show n = 1 by lia]
    simp
/-
**Nat.probablePrime_iff_modEq** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：probablePrime_iff_modEq (n : Nat) {b : Nat} (h : 1 <= b) : ProbablePrime n
 b ↔ b ^ (n - 1) ≡ 1 [MOD n]
参数：n : Nat；h : 1 <= b。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
-/
/-
**Nat.coprime_of_fermatPsp** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：coprime_of_fermatPsp {n b : Nat} (h : FermatPsp n b) (h₁ : 1 <= b) : Nat.C
oprime n b
参数：h : FermatPsp n b；h₁ : 1 <= b。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.coprime_of_probablePrime`：coprime_of_probablePrime {n b : Nat} (h : 
ProbablePrime n b) (h₁ : 1 <= n) (h₂ : 1 <= b) : Nat.Coprime n b

--- 原说明 ---
If `n` is a Fermat pseudoprime to base `b`, then `n` is coprime with `b`, assumi
ng that `b` is
positive.
...
-/
/-
**Nat.fermatPsp_base_one** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：fermatPsp_base_one {n : Nat} (h₁ : 1 < n) (h₂ : ¬n.Prime) : FermatPsp n 1
参数：h₁ : 1 < n；h₂ : ¬n.Prime。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Nat.a_id_helper** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.b_id_helper** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.AB_id_helper** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Nat.bp_helper** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Nat.psp_from_prime** 是 Mathlib 中的一个定义，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a prime `p` which does not divide `b * (b ^ 2 - 1)`, we can produce a numb
er `n` which is
larger than `p` and pseudoprime to base `b`. We do this by defining
`n = ((b ^ p - 1) / (b - 1)) * ((b ^ p + 1) / (b + 1))`

The primary purpose of this definition is to help prove `exists_infinite_pseudop
rimes`. For a proof
that `n` is actually pseudoprime to base `b`, see `psp_from_prime_psp`, and for 
a proof that `n` is
greater than `p`, see `psp_from_prime_gt_p`.

This lemma is intended to be used when `2 ≤ b`, `2 < p`, `p` is prime, and `¬p ∣
 b * (b ^ 2 - 1)`,
because those are the hypotheses for `psp_from_prime_psp`.
-/
...
-/
/-
**Nat.psp_from_prime_psp** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
This is a proof that the number produced using `psp_from_prime` is actually pseu
doprime to base `b`.
The primary purpose of this lemma is to help prove `exists_infinite_pseudoprimes
`.

We use <https://primes.utm.edu/notes/proofs/a_pseudoprimes.html> as a rough outl
ine of the proof.
-/
...
-/
/-
**Nat.psp_from_prime_gt_p** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
This is a proof that the number produced using `psp_from_prime` is greater than 
the prime `p` used
to create it. The primary purpose of this lemma is to help prove `exists_infinit
e_pseudoprimes`.
-/
...
-/
/-
**Nat.exists_infinite_pseudoprimes** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：exists_infinite_pseudoprimes {b : Nat} (h : 1 <= b) (m : Nat) : exists n :
 Nat, FermatPsp n b ∧ m <= n
参数：h : 1 <= b；m : Nat。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `_private.Mathlib.NumberTheory.FermatPsp.0.Nat.psp_from_prime_gt_p`：∀ {b 
: ℕ}, 2 ≤ b → ∀ {p : ℕ}, 2 < p → p < Nat.psp_from_prime✝ b p
· 使用定理 `_private.Mathlib.NumberTheory.FermatPsp.0.Nat.psp_from_prime_psp`：∀ {b :
 ℕ}, 2 ≤ b → ∀ {p : ℕ}, Nat.Prime p → 2 < p → ¬p ∣ b * (b ^ 2 - 1) → (Nat.psp_fr
om_prime✝ b p).FermatPsp b
· 使用定理 `le_of_lt`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Nat.not_prime_mul`：not_prime_mul {a b : Nat} (a1 : a != 1) (b1 : b != 1)
 : ¬Prime (a * b)
· 使用定理 `Nat.fermatPsp_base_one`：fermatPsp_base_one {n : Nat} (h₁ : 1 < n) (h₂ : 
¬n.Prime) : FermatPsp n 1

--- 原说明 ---
For all positive bases, there exist infinitely many **Fermat pseudoprimes** to t
hat base.
Given in this form: for all numbers `b ≥ 1` and `m`, there exists a pseudoprime 
`n` to base `b` such
that `m ≤ n`. This form is similar to `Nat.exists_infinite_primes`.
-/
...
/-
**Nat.frequently_atTop_fermatPsp** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：frequently_atTop_fermatPsp {b : Nat} (h : 1 <= b) : existsᶠ n in Filter.at
Top, FermatPsp n b
参数：h : 1 <= b。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `Nat.exists_infinite_pseudoprimes`：exists_infinite_pseudoprimes {b : Nat}
 (h : 1 <= b) (m : Nat) : exists n : Nat, FermatPsp n b ∧ m <= n
...
-/
/-
**Nat.infinite_setOfPred_pseudoprimes** 是 Mathlib 中的一个定理，位于命名空间 `Nat`。
形式化陈述：infinite_setOfPred_pseudoprimes {b : Nat} (h : 1 <= b) : Set.Infinite { n 
: Nat | FermatPsp n b }
参数：h : 1 <= b。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `Nat.frequently_atTop_fermatPsp`：frequently_atTop_fermatPsp {b : Nat} (h 
: 1 <= b) : existsᶠ n in Filter.atTop, FermatPsp n b
