...
/--
theorem `hofer` / 定理 `hofer`

English:
theorem hofer
  statement: {X : Type*} [MetricSpace X] [CompleteSpace X] (x : X) (ε : Real) (ε_pos : 0 < ε)
  proof: by
  by_contra! H
  have reformulation : forall (x') (k : Nat), ε * ϕ x <= ε / 2 ^ k * ϕ x' ↔ 2 ^ k * ϕ x <= ϕ x' := by
    intro x' k
    rw [div_mul_eq_mul_div]; rw [le_div_iff₀]; rw [mul_assoc]; rw [mul_le_mul_iff_right₀ ε_pos]; rw [mul_comm]
    positivity
  -- Now let's specialize to `ε/2^k`
  replace H : forall k : Nat, forall x', d x' x <= 2 * ε ∧ 2 ^ k * ϕ x <= ϕ x' ->
      exists y, d x' y <= ε / 2 ^ k ∧ 2 * ϕ x' < ϕ y := by
    intro k x'
  ...
  let u : Nat -> X := fun n => Nat.recOn n x F
  ...
  have key : forall n, d (u n) (u (n + 1)) <= ε / 2 ^ n ∧ 2 * ϕ (u n) < ϕ (u (n + 1)) := by
    intro n
  ...
  obtain ⟨y, limy⟩ : exists y, Tendsto u atTop (𝓝 y) := CompleteSpace.complete cauchy_u
  -- And ϕ ∘ u goes to +∞
...
Depends on / 依赖: div_mul_eq_mul_div, mul_assoc, mul_comm, reformulation
-/
theorem hofer {X : Type*} [MetricSpace X] [CompleteSpace X] (x : X) (ε : Real) (ε_pos : 0 < ε)
    {ϕ : X -> Real} (cont : Continuous ϕ) (nonneg : forall y, 0 <= ϕ y) : exists ε' > 0, exists x' : X,
    ε' <= ε ∧ d x' x <= 2 * ε ∧ ε * ϕ x <= ε' * ϕ x' ∧ forall y, d x' y <= ε' -> ϕ y <= 2 * ϕ x' := by
  by_contra! H
  have reformulation : forall (x') (k : Nat), ε * ϕ x <= ε / 2 ^ k * ϕ x' ↔ 2 ^ k * ϕ x <= ϕ x' := by
    intro x' k
    rw [div_mul_eq_mul_div]; rw [le_div_iff₀]; rw [mul_assoc]; rw [mul_le_mul_iff_right₀ ε_pos]; rw [mul_comm]
