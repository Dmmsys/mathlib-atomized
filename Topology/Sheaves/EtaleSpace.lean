...
variable {X : TopCat.{v}} {C : Type u} [Category.{v} C] {CC : C -> Type v} {FC : C -> C -> Type w}
  [forall X Y, FunLike (FC X Y) (CC X) (CC Y)] [ConcreteCategory C FC] [Limits.HasColimits.{v} C]

/--
Definition of `EtaleSpace` / `EtaleSpace` 的定义

English:
structure EtaleSpace
  parameters: (F : Presheaf C X)
...
instance (F : Presheaf C X) : TopologicalSpace F.EtaleSpace :=
  .generateFrom
    {{g | exists h, g.germ = F.germ U g.base h f} | (U : Opens X) (f : ToType (F.obj <| op U))}
...
theorem eventually_nhds
  statement: (g : EtaleSpace F) {U : Opens X} (h : g.base in U)
  proof: by
...
mem_iInf_of_mem rfl mem_principal_self _
  simp [*]
-/
protected theorem eventually_nhds (g : EtaleSpace F) {U : Opens X} (h : g.base in U)
    (s : ToType (F.obj (op U))) (hs : F.germ U g.base h s = g.germ) :
    forallᶠ g' : EtaleSpace F in 𝓝 g, exists hgU : g'.base in U, g'.germ = F.germ U g'.base hgU s := by
  simp only [nhds_generateFrom, Filter.Eventually, mem_ofPred_eq, iInf_and, iInf_exists]
refine mem_iInf_of_mem _ mem_iInf_of_mem ?_ mem_iInf_of_mem U mem_iInf_of_mem s
mem_iInf_of_mem rfl mem_principal_self _
  simp [*]
...
@[fun_prop]
/--
theorem `continuous_base` / 定理 `continuous_base`

English:
theorem continuous_base
  statement: Continuous (base (F := F))
  proof: by
  rw [continuous_iff_continuousAt]
  intro x
  rw [ContinuousAt]; rw [(nhds_basis_opens _).tendsto_right_iff]
  rintro U ⟨hxU, hUo⟩
  lift U to Opens X using hUo
  rcases F.exists_le_germ_eq x.germ hxU with ⟨V, hVU, hxV, f, hf⟩
.mono ?_ refine x.eventually_nhds hxV f hf
  aesop
...
Depends on / 依赖: ContinuousAt, F.exists_le_germ_eq, continuous_iff_continuousAt, eventually_nhds, exists_le_germ_eq, nhds_basis_opens, tendsto_right_iff, x.eventually_nhds, x.germ
-/
...
/--
theorem `exists_section_of_tendsto` / 定理 `exists_section_of_tendsto`

English:
theorem exists_section_of_tendsto
  statement: {α : Type*} {l : Filter α} {g : α -> F.EtaleSpace}
  proof: by
...
Depends on / 依赖: F.exists_germ_eq, eventually, eventually_nhds, exists_germ_eq, h.eventually
-/
theorem exists_section_of_tendsto {α : Type*} {l : Filter α} {g : α -> F.EtaleSpace}
    {g₀ : F.EtaleSpace} (h : Tendsto g l (𝓝 g₀)) :
    exists (U : Opens X), g₀.base in U ∧ exists (f : ToType (F.obj (op U))),
      forallᶠ a in l, exists ha : (g a).base in U, (g a).germ = F.germ U (g a).base ha f := by
  rcases F.exists_germ_eq g₀.germ with ⟨U, hU, s, hs⟩
  use U, hU, s
exact h.eventually g₀.eventually_nhds hU s hs
...
noncomputable def homeomorph (U : Opens X)
    (hF_bij : forall (x : X) (hx : x in U), Bijective (F.germ U x hx))
    (x : X) (hx : x in U) :
...
/--
theorem `isCoveringMap_base` / 定理 `isCoveringMap_base`
...
Depends on / 依赖: F.stalk, ToType, U.isOpen, U.isOpen.preimage, WithDiscreteTopology, continuous_base, hF_bij, hU_bij, homeomorph, isOpen, preimage, to_isEvenlyCovered_preimage
-/
theorem isCoveringMap_base
    (hF_bij : forall x, exists (U : Opens X), x in U ∧ forall y (hyU : y in U), Bijective (F.germ U y hyU)) :
