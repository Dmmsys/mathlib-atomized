...
variable {X E ι : Type*} [MeasurableSpace X] [NormedAddCommGroup E] [NormedSpace Real E]
  [CompleteSpace E]

/--
theorem `Filter.Tendsto.integral_sub_linear_isLittleO_ae` / 定理 `Filter.Tendsto.integral_sub_linear_isLittleO_ae`
...
/--
theorem `ContinuousWithinAt.integral_sub_linear_isLittleO_ae` / 定理 `ContinuousWithinAt.integral_sub_linear_isLittleO_ae`

English:
theorem ContinuousWithinAt.integral_sub_linear_isLittleO_ae
  statement: [TopologicalSpace X]
  proof: haveI : (𝓝[t] x).IsMeasurablyGenerated := ht.nhdsWithin_isMeasurablyGenerated _
...
theorem ContinuousWithinAt.integral_sub_linear_isLittleO_ae [TopologicalSpace X]
    ...
    {li : Filter ι} (hs : Tendsto s li (𝓝[t] x).smallSets) (m : ι -> Real := fun i => μ.real (s i))
    (hsμ : (fun i => μ.real (s i)) =ᶠ[li] m := by rfl) :
...
/--
theorem `ContinuousAt.integral_sub_linear_isLittleO_ae` / 定理 `ContinuousAt.integral_sub_linear_isLittleO_ae`

English:
theorem ContinuousAt.integral_sub_linear_isLittleO_ae
  statement: [TopologicalSpace X] [OpensMeasurableSpace X]
  proof: (hx.mono_left inf_le_left).integral_sub_linear_isLittleO_ae hfm (μ.finiteAt_nhds x) hs m hsμ
...
theorem ContinuousAt.integral_sub_linear_isLittleO_ae [TopologicalSpace X] [OpensMeasurableSpace X]
    ...
    {li : Filter ι} (hs : Tendsto s li (𝓝 x).smallSets) (m : ι -> Real := fun i => μ.real (s i))
    (hsμ : (fun i => μ.real (s i)) =ᶠ[li] m := by rfl) :
    (fun i => (∫ x in s i, f x ∂μ) - m i • f x) =o[li] m :=
  (hx.mono_left inf_le_left).integral_sub_linear_isLittleO_ae hfm (μ.finiteAt_nhds x) hs m hsμ

/--
theorem `ContinuousOn.integral_sub_linear_isLittleO_ae` / 定理 `ContinuousOn.integral_sub_linear_isLittleO_ae`

English:
theorem ContinuousOn.integral_sub_linear_isLittleO_ae
  statement: [TopologicalSpace X] [OpensMeasurableSpace X]
  proof: (hft x hx).integral_sub_linear_isLittleO_ae ht
...
theorem ContinuousOn.integral_sub_linear_isLittleO_ae [TopologicalSpace X] [OpensMeasurableSpace X]
    ...
    [IsLocallyFiniteMeasure μ] {x : X} {t : Set X} {f : X -> E} (hft : ContinuousOn f t) (hx : x in t)
