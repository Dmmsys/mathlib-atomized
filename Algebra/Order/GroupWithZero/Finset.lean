/-
Copyright (c) 2022 Eric Wieser. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Eric Wieser, Yaël Dillies, Andrew Yang
-/
module

public import Mathlib.Algebra.Order.GroupWithZero.Canonical
public import Mathlib.Algebra.Order.GroupWithZero.OrderIso
public import Mathlib.Data.Finset.Lattice.Fold

/-!
# `Finset.sup` in a group with zero
-/

public section

namespace Finset
variable {ι M₀ G₀ : Type*}

section MonoidWithZero
variable [MonoidWithZero M₀] {s : Finset ι} {a b : ι -> M₀}

/--
lemma `sup_mul_le_mul_sup_of_nonneg` / 引理 `sup_mul_le_mul_sup_of_nonneg`

English:
lemma sup_mul_le_mul_sup_of_nonneg
  statement: [SemilatticeSup M₀] [OrderBot M₀] [PosMulMono M₀] [MulPosMono M₀]
  proof: Finset.sup_le fun _i hi =>
    mul_le_mul (le_sup hi) (le_sup hi) (hb _ hi) ((ha _ hi).trans <| le_sup hi)

中文:
引理 sup_mul_le_mul_sup_of_nonneg
  结论: [SemilatticeSup M₀] [有底序 M₀] [正乘递增 M₀] [乘正递增 M₀]
  证明: Finset.sup_le fun _i hi =>
    mul_le_mul (le_sup hi) (le_sup hi) (hb _ hi) ((ha _ hi).trans <| le_sup hi)

Depends on / 依赖: Finset, Finset.sup_le, le_sup, mul_le_mul, sup_le
-/
lemma sup_mul_le_mul_sup_of_nonneg [SemilatticeSup M₀] [OrderBot M₀] [PosMulMono M₀] [MulPosMono M₀]
    (ha : forall i in s, 0 <= a i) (hb : forall i in s, 0 <= b i) : s.sup (a * b) <= s.sup a * s.sup b :=
  Finset.sup_le fun _i hi =>
    mul_le_mul (le_sup hi) (le_sup hi) (hb _ hi) ((ha _ hi).trans <| le_sup hi)

/--
lemma `mul_inf_le_inf_mul_of_nonneg` / 引理 `mul_inf_le_inf_mul_of_nonneg`

English:
lemma mul_inf_le_inf_mul_of_nonneg
  statement: [SemilatticeInf M₀] [OrderTop M₀] [PosMulMono M₀] [MulPosMono M₀]
  proof: Finset.le_inf fun i hi => mul_le_mul (inf_le hi) (inf_le hi) (Finset.le_inf hb) (ha i hi)

中文:
引理 mul_inf_le_inf_mul_of_nonneg
  结论: [SemilatticeInf M₀] [有顶序 M₀] [正乘递增 M₀] [乘正递增 M₀]
  证明: Finset.le_inf fun i hi => mul_le_mul (inf_le hi) (inf_le hi) (Finset.le_inf hb) (ha i hi)

Depends on / 依赖: Finset, Finset.le_inf, inf_le, le_inf, mul_le_mul
-/
lemma mul_inf_le_inf_mul_of_nonneg [SemilatticeInf M₀] [OrderTop M₀] [PosMulMono M₀] [MulPosMono M₀]
    (ha : forall i in s, 0 <= a i) (hb : forall i in s, 0 <= b i) : s.inf a * s.inf b <= s.inf (a * b) :=
  Finset.le_inf fun i hi => mul_le_mul (inf_le hi) (inf_le hi) (Finset.le_inf hb) (ha i hi)

/--
lemma `sup'_mul_le_mul_sup'_of_nonneg` / 引理 `sup'_mul_le_mul_sup'_of_nonneg`

English:
lemma sup'_mul_le_mul_sup'_of_nonneg
  statement: [SemilatticeSup M₀] [PosMulMono M₀] [MulPosMono M₀]
  proof: sup'_le _ _ fun _i hi =>
    mul_le_mul (le_sup' _ hi) (le_sup' _ hi) (hb _ hi) ((ha _ hi).trans <| le_sup' _ hi)

中文:
引理 上确界'_mul_le_mul_sup'_of_nonneg
  结论: [SemilatticeSup M₀] [正乘递增 M₀] [乘正递增 M₀]
  证明: sup'_le _ _ fun _i hi =>
    mul_le_mul (le_sup' _ hi) (le_sup' _ hi) (hb _ hi) ((ha _ hi).trans <| le_sup' _ hi)
-/
lemma sup'_mul_le_mul_sup'_of_nonneg [SemilatticeSup M₀] [PosMulMono M₀] [MulPosMono M₀]
    (ha : forall i in s, 0 <= a i) (hb : forall i in s, 0 <= b i) (hs) :
    s.sup' hs (a * b) <= s.sup' hs a * s.sup' hs b :=
  sup'_le _ _ fun _i hi =>
    mul_le_mul (le_sup' _ hi) (le_sup' _ hi) (hb _ hi) ((ha _ hi).trans <| le_sup' _ hi)

/--
lemma `inf'_mul_le_mul_inf'_of_nonneg` / 引理 `inf'_mul_le_mul_inf'_of_nonneg`

English:
lemma inf'_mul_le_mul_inf'_of_nonneg
  statement: [SemilatticeInf M₀] [PosMulMono M₀] [MulPosMono M₀]
  proof: le_inf' _ _ fun _i hi => mul_le_mul (inf'_le _ hi) (inf'_le _ hi) (le_inf' _ _ hb) (ha _ hi)

中文:
引理 下确界'_mul_le_mul_inf'_of_nonneg
  结论: [SemilatticeInf M₀] [正乘递增 M₀] [乘正递增 M₀]
  证明: le_inf' _ _ fun _i hi => mul_le_mul (inf'_le _ hi) (inf'_le _ hi) (le_inf' _ _ hb) (ha _ hi)
-/
lemma inf'_mul_le_mul_inf'_of_nonneg [SemilatticeInf M₀] [PosMulMono M₀] [MulPosMono M₀]
    (ha : forall i in s, 0 <= a i) (hb : forall i in s, 0 <= b i) (hs) :
    s.inf' hs a * s.inf' hs b <= s.inf' hs (a * b) :=
  le_inf' _ _ fun _i hi => mul_le_mul (inf'_le _ hi) (inf'_le _ hi) (le_inf' _ _ hb) (ha _ hi)

end MonoidWithZero

section GroupWithZero
variable [GroupWithZero G₀] [SemilatticeSup G₀] {s : Finset ι} {a : G₀}

/--
lemma `sup'_mul₀` / 引理 `sup'_mul₀`

English:
lemma sup'_mul₀
  given: [MulPosReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs)
  proof: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulRight₀ _ (lt_of_le_of_ne ha h)) hs f

中文:
引理 上确界'_mul₀
  条件: [乘正反映严格偏序 G₀] (ha : 0 <= a) (f : ι -> G₀) (s : 有限集 ι) (hs)
  证明: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulRight₀ _ (lt_of_le_of_ne ha h)) hs f
-/
lemma sup'_mul₀ [MulPosReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs) :
    s.sup' hs f * a = s.sup' hs fun i => f i * a := by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulRight₀ _ (lt_of_le_of_ne ha h)) hs f

set_option linter.docPrime false in
/--
lemma `mul₀_sup'` / 引理 `mul₀_sup'`

English:
lemma mul₀_sup'
  given: [PosMulReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs)
  proof: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulLeft₀ _ (lt_of_le_of_ne ha h)) hs f

中文:
引理 mul₀_sup'
  条件: [正乘反映严格偏序 G₀] (ha : 0 <= a) (f : ι -> G₀) (s : 有限集 ι) (hs)
  证明: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulLeft₀ _ (lt_of_le_of_ne ha h)) hs f

Depends on / 依赖: OrderIso, OrderIso.mulLeft, lt_of_le_of_ne, map_finset_sup
-/
lemma mul₀_sup' [PosMulReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs) :
    a * s.sup' hs f = s.sup' hs fun i => a * f i := by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.mulLeft₀ _ (lt_of_le_of_ne ha h)) hs f

/--
lemma `sup'_div₀` / 引理 `sup'_div₀`

English:
lemma sup'_div₀
  given: [MulPosReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs)
  proof: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.divRight₀ _ (lt_of_le_of_ne ha h)) hs f

中文:
引理 上确界'_div₀
  条件: [乘正反映严格偏序 G₀] (ha : 0 <= a) (f : ι -> G₀) (s : 有限集 ι) (hs)
  证明: by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.divRight₀ _ (lt_of_le_of_ne ha h)) hs f
-/
lemma sup'_div₀ [MulPosReflectLT G₀] (ha : 0 <= a) (f : ι -> G₀) (s : Finset ι) (hs) :
    s.sup' hs f / a = s.sup' hs fun i => f i / a := by
  by_cases! h : 0 = a
  · simp [← h]
  exact map_finset_sup' (OrderIso.divRight₀ _ (lt_of_le_of_ne ha h)) hs f

end GroupWithZero

/--
lemma `sup_div₀` / 引理 `sup_div₀`

English:
lemma sup_div₀
  statement: [LinearOrderedCommGroupWithZero G₀] {a : G₀} (ha : 0 <= a)
  proof: by
  obtain rfl | hs := s.eq_empty_or_nonempty
  · simp [bot_eq_zero]
  rw [← Finset.sup'_eq_sup hs]; rw [← Finset.sup'_eq_sup hs]; rw [sup'_div₀ (ha := ha)]

中文:
引理 sup_div₀
  结论: [带零LinearOrderedComm群 G₀] {a : G₀} (ha : 0 <= a)
  证明: by
  obtain rfl | hs := s.eq_empty_or_nonempty
  · simp [bot_eq_zero]
  rw [← Finset.sup'_eq_sup hs]; rw [← Finset.sup'_eq_sup hs]; rw [sup'_div₀ (ha := ha)]

Depends on / 依赖: Finset, Finset.sup, _eq_sup, bot_eq_zero, eq_empty_or_nonempty, s.eq_empty_or_nonempty
-/
lemma sup_div₀ [LinearOrderedCommGroupWithZero G₀] {a : G₀} (ha : 0 <= a)
    (s : Finset ι) (f : ι -> G₀) : s.sup f / a = s.sup fun i => f i / a := by
  obtain rfl | hs := s.eq_empty_or_nonempty
  · simp [bot_eq_zero]
  rw [← Finset.sup'_eq_sup hs]; rw [← Finset.sup'_eq_sup hs]; rw [sup'_div₀ (ha := ha)]

end Finset
