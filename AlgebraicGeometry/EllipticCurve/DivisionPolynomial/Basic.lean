...
/-
**WeierstrassCurve.C_** 是 Mathlib 中的一个引理，位于命名空间 `WeierstrassCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
lemma ψ₂_sq : W.ψ₂ ^ 2 = C W.Ψ₂Sq + 4 * W.toAffine.polynomial := by
  simp [C_Ψ₂Sq]
/-
**WeierstrassCurve.Affine.CoordinateRing.mk_** 是 Mathlib 中的一个引理，位于命名空间 `Weierstr
assCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
`ψ₄ = Ψ₄ = preΨ₄ψ₂`. -/
/-
**WeierstrassCurve.pre** 是 Mathlib 中的一个定义，位于命名空间 `WeierstrassCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The univariate polynomial `preΨ₄`, which is auxiliary to the 4-division polynomi
al
`ψ₄ = Ψ₄ = preΨ₄ψ₂`.
-/
...
--- 原说明 ---
The univariate polynomials `preΨₙ` for `n ∈ ℕ`, which are auxiliary to the bivar
iate polynomials
`Ψₙ` congruent to the bivariate `n`-division polynomials `ψₙ`.
-/
...
--- 原说明 ---
The univariate polynomials `preΨₙ` for `n ∈ ℤ`, which are auxiliary to the bivar
iate polynomials
`Ψₙ` congruent to the bivariate `n`-division polynomials `ψₙ`.
-/
...
@[simp]
/-
**WeierstrassCurve.map_pre** 是 Mathlib 中的一个引理，位于命名空间 `WeierstrassCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**WeierstrassCurve.baseChange_** 是 Mathlib 中的一个引理，位于命名空间 `WeierstrassCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
lemma baseChange_Ψ₃ : (W⁄B).Ψ₃ = (W⁄A).Ψ₃.map f := by
  rw [← map_Ψ₃, map_baseChange]
/-
**WeierstrassCurve.baseChange_pre** 是 Mathlib 中的一个引理，位于命名空间 `WeierstrassCurve`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
