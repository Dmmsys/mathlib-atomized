/-
Copyright (c) 2026 Thomas Browning. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Thomas Browning
-/
module

public import Mathlib.NumberTheory.RamificationInertia.Inertia
public import Mathlib.RingTheory.QuasiFinite.Basic

/-!
# Inertia degree

Given a prime ideal `q` of an `R`-algebra `S`, the inertia degree of `q` over `R` is defined
to be the degree of the residue field of `q` over the residue field of its preimage `p` in `R`.

## Main definitions

* `Ideal.inertiaDeg q R`: The inertia degree of `q` over `R`.

## Main statements

* `inertiaDeg'_eq_inertiaDeg`: The inertia degree agrees with the usual definition in the case of
  maximal ideals.
* `inertiaDeg_tower`: Inertia degree is multiplicative in towers.
-/

@[expose] public section

namespace Ideal

section

variable {S : Type*} [CommRing S] (q : Ideal S) (R : Type*) [CommRing R] [Algebra R S]

open scoped Classical in
/--
Definition of `inertiaDeg` / `inertiaDeg` 的定义

English:
definition inertiaDeg
  signature: : Nat
  body: if _ : q.IsPrime then
    letI := Localization.AtPrime.algebraOfLiesOver (q.under R) q
    Module.finrank (q.under R).ResidueField q.ResidueField else 0

中文:
定义 inertiaDeg
  签名: : 自然数
  定义体: if _ : q.IsPrime then
    letI := Localization.AtPrime.algebraOfLiesOver (q.under R) q
    Module.finrank (q.under R).ResidueField q.ResidueField else 0

Depends on / 依赖: AtPrime, IsPrime, Localization, Localization.AtPrime.algebraOfLiesOver, Module, Module.finrank, ResidueField, algebraOfLiesOver, finrank, q.IsPrime, q.ResidueField, q.under
-/
noncomputable def inertiaDeg : Nat :=
  if _ : q.IsPrime then
    letI := Localization.AtPrime.algebraOfLiesOver (q.under R) q
    Module.finrank (q.under R).ResidueField q.ResidueField else 0

/--
theorem `inertiaDeg_def` / 定理 `inertiaDeg_def`

English:
theorem inertiaDeg_def
  statement: [hq : q.IsPrime]
  proof: by
  convert! dif_pos hq
  simp [Algebra.algebra_ext_iff, Localization.AtPrime.IsLiesOverAlgebra.algebraMap_eq]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_def := inertiaDeg_def

中文:
定理 inertiaDeg_def
  结论: [hq : q.是素]
  证明: by
  convert! dif_pos hq
  simp [Algebra.algebra_ext_iff, Localization.AtPrime.IsLiesOverAlgebra.algebraMap_eq]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_def := inertiaDeg_def

Depends on / 依赖: Algebra, Algebra.algebra_ext_iff, AtPrime, IsLiesOverAlgebra, Localization, Localization.AtPrime.IsLiesOverAlgebra.algebraMap_eq, algebraMap_eq, algebra_ext_iff, convert, dif_pos
-/
theorem inertiaDeg_def [hq : q.IsPrime]
    [Algebra (Localization.AtPrime (q.under R)) (Localization.AtPrime q)]
    [Localization.AtPrime.IsLiesOverAlgebra (q.under R) q] :
    q.inertiaDeg R = Module.finrank (q.under R).ResidueField q.ResidueField := by
  convert! dif_pos hq
  simp [Algebra.algebra_ext_iff, Localization.AtPrime.IsLiesOverAlgebra.algebraMap_eq]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_def := inertiaDeg_def

/--
theorem `inertiaDeg_of_not_isPrime` / 定理 `inertiaDeg_of_not_isPrime`

English:
theorem inertiaDeg_of_not_isPrime
  given: (hq : ¬ q.IsPrime)
  statement: q.inertiaDeg R = 0
  proof: dif_neg hq

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_of_not_isPrime :=
  inertiaDeg_of_not_isPrime

中文:
定理 inertiaDeg_of_not_isPrime
  条件: (hq : ¬ q.是素)
  结论: q.inertiaDeg R = 0
  证明: dif_neg hq

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_of_not_isPrime :=
  inertiaDeg_of_not_isPrime

Depends on / 依赖: dif_neg
-/
theorem inertiaDeg_of_not_isPrime (hq : ¬ q.IsPrime) : q.inertiaDeg R = 0 :=
  dif_neg hq

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_of_not_isPrime :=
  inertiaDeg_of_not_isPrime

/--
theorem `inertiaDeg_pos` / 定理 `inertiaDeg_pos`

English:
theorem inertiaDeg_pos
  given: [hq : q.IsPrime] [Module.Finite R S]
  statement: 0 < q.inertiaDeg R
  proof: by
  let := Localization.AtPrime.algebraOfLiesOver (q.under R) q
  rw [inertiaDeg_def]
  apply Module.finrank_pos

中文:
定理 inertiaDeg_pos
  条件: [hq : q.是素] [模.有限 R S]
  结论: 0 < q.inertiaDeg R
  证明: by
  let := Localization.AtPrime.algebraOfLiesOver (q.under R) q
  rw [inertiaDeg_def]
  apply Module.finrank_pos

Depends on / 依赖: AtPrime, Localization, Localization.AtPrime.algebraOfLiesOver, Module, Module.finrank_pos, algebraOfLiesOver, finrank_pos, inertiaDeg_def, q.under
-/
theorem inertiaDeg_pos [hq : q.IsPrime] [Module.Finite R S] : 0 < q.inertiaDeg R := by
  let := Localization.AtPrime.algebraOfLiesOver (q.under R) q
  rw [inertiaDeg_def]
  apply Module.finrank_pos

end

section

variable {R S T : Type*} [CommRing R] [CommRing S] [CommRing T]
  [Algebra R S] [Algebra R T] [Algebra S T] [IsScalarTower R S T]
  (p : Ideal R) (q : Ideal S) (r : Ideal T)

/--
theorem `inertiaDeg_eq` / 定理 `inertiaDeg_eq`

English:
theorem inertiaDeg_eq
  statement: [q.LiesOver p] [q.IsPrime] [p.IsPrime]
  proof: by
  have := Ideal.over_def q p
  subst this
  exact inertiaDeg_def q R

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq := inertiaDeg_eq

中文:
定理 inertiaDeg_eq
  结论: [q.LiesOver p] [q.是素] [p.是素]
  证明: by
  have := Ideal.over_def q p
  subst this
  exact inertiaDeg_def q R

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq := inertiaDeg_eq

Depends on / 依赖: Ideal.over_def, inertiaDeg_def, over_def
-/
theorem inertiaDeg_eq [q.LiesOver p] [q.IsPrime] [p.IsPrime]
    [Algebra (Localization.AtPrime p) (Localization.AtPrime q)]
    [Localization.AtPrime.IsLiesOverAlgebra p q] :
    q.inertiaDeg R = Module.finrank p.ResidueField q.ResidueField := by
  have := Ideal.over_def q p
  subst this
  exact inertiaDeg_def q R

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq := inertiaDeg_eq

/--
theorem `inertiaDeg_eq_of_isFractionRing` / 定理 `inertiaDeg_eq_of_isFractionRing`

English:
theorem inertiaDeg_eq_of_isFractionRing
  statement: [q.LiesOver p] [p.IsPrime] [q.IsPrime]
  proof: by
  let := Localization.AtPrime.algebraOfLiesOver p q
  rw [inertiaDeg_eq p q]
  apply Algebra.finrank_eq_of_equiv_equiv
    (IsFractionRing.algEquivOfAlgEquiv (R := R) (A := R ⧸ p) (K := p.ResidueField) (L := K) .refl)
    (IsFractionRing.algEquivOfAlgEquiv (R := S) (A := S ⧸ q) (K := q.ResidueFie

中文:
定理 inertiaDeg_eq_of_isFractionRing
  结论: [q.LiesOver p] [p.是素] [q.是素]
  证明: by
  let := Localization.AtPrime.algebraOfLiesOver p q
  rw [inertiaDeg_eq p q]
  apply Algebra.finrank_eq_of_equiv_equiv
    (IsFractionRing.algEquivOfAlgEquiv (R := R) (A := R ⧸ p) (K := p.ResidueField) (L := K) .refl)
    (IsFractionRing.algEquivOfAlgEquiv (R := S) (A := S ⧸ q) (K := q.ResidueFie

Depends on / 依赖: Algebra, Algebra.finrank_eq_of_equiv_equiv, AtPrime, Ideal.Quotient.mk_surjective, IsFractionRing, IsFractionRing.algEquivOfAlgEquiv, IsFractionRing.ringHom_ext, IsScalarTower, IsScalarTower.alge, IsScalarTower.algebraMap_apply, Localization, Localization.AtPrime.algebraOfLiesOver, Quotient, ResidueField, algEquivOfAlgEquiv, algebraMap_apply, algebraOfLiesOver, finrank_eq_of_equiv_equiv, inertiaDeg_eq, mk_surjective
-/
theorem inertiaDeg_eq_of_isFractionRing [q.LiesOver p] [p.IsPrime] [q.IsPrime]
    (K L : Type*) [Field K] [Field L]
    [Algebra (R ⧸ p) K] [IsFractionRing (R ⧸ p) K]
    [Algebra (S ⧸ q) L] [IsFractionRing (S ⧸ q) L]
    [Algebra R K] [IsScalarTower R (R ⧸ p) K]
    [Algebra S L] [IsScalarTower S (S ⧸ q) L]
    [Algebra R L] [IsScalarTower R S L]
    [Algebra K L] [IsScalarTower R K L] :
    q.inertiaDeg R = Module.finrank K L := by
  let := Localization.AtPrime.algebraOfLiesOver p q
  rw [inertiaDeg_eq p q]
  apply Algebra.finrank_eq_of_equiv_equiv
    (IsFractionRing.algEquivOfAlgEquiv (R := R) (A := R ⧸ p) (K := p.ResidueField) (L := K) .refl)
    (IsFractionRing.algEquivOfAlgEquiv (R := S) (A := S ⧸ q) (K := q.ResidueField) (L := L) .refl)
  apply IsFractionRing.ringHom_ext (A := R ⧸ p)
  intro x
  obtain ⟨x, rfl⟩ := Ideal.Quotient.mk_surjective x
  simp [← IsScalarTower.algebraMap_apply R p.ResidueField q.ResidueField,
    IsScalarTower.algebraMap_apply R S q.ResidueField,
    ← IsScalarTower.algebraMap_apply R K L, ← IsScalarTower.algebraMap_apply R S L]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq_of_isFractionRing :=
inertiaDeg_eq_of_isFractionRing

/--
theorem `inertiaDeg_eq_of_isMaximal` / 定理 `inertiaDeg_eq_of_isMaximal`

English:
theorem inertiaDeg_eq_of_isMaximal
  given: [q.LiesOver p] [p.IsMaximal] [q.IsMaximal]
  proof: by
  let : Field (R ⧸ p) := Quotient.field p
  let : Field (S ⧸ q) := Quotient.field q
  exact inertiaDeg_eq_of_isFractionRing p q (R ⧸ p) (S ⧸ q)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq_of_isMaximal :=
  inertiaDeg_eq_of_isMaximal

中文:
定理 inertiaDeg_eq_of_isMaximal
  条件: [q.LiesOver p] [p.是极大] [q.是极大]
  证明: by
  let : Field (R ⧸ p) := Quotient.field p
  let : Field (S ⧸ q) := Quotient.field q
  exact inertiaDeg_eq_of_isFractionRing p q (R ⧸ p) (S ⧸ q)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq_of_isMaximal :=
  inertiaDeg_eq_of_isMaximal

Depends on / 依赖: Quotient, Quotient.field, inertiaDeg_eq_of_isFractionRing
-/
theorem inertiaDeg_eq_of_isMaximal [q.LiesOver p] [p.IsMaximal] [q.IsMaximal] :
    q.inertiaDeg R = Module.finrank (R ⧸ p) (S ⧸ q) := by
  let : Field (R ⧸ p) := Quotient.field p
  let : Field (S ⧸ q) := Quotient.field q
  exact inertiaDeg_eq_of_isFractionRing p q (R ⧸ p) (S ⧸ q)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_eq_of_isMaximal :=
  inertiaDeg_eq_of_isMaximal

/--
theorem `inertiaDeg'_eq_inertiaDeg` / 定理 `inertiaDeg'_eq_inertiaDeg`

English:
theorem inertiaDeg'_eq_inertiaDeg
  given: [q.LiesOver p] [p.IsMaximal] [q.IsMaximal]
  proof: by
  rw [inertiaDeg'_algebraMap]; rw [inertiaDeg_eq_of_isMaximal p q]

@[deprecated (since := "2026-07-03")] alias inertiaDeg_eq_inertiaDeg' := inertiaDeg'_eq_inertiaDeg

中文:
定理 inertiaDeg'_eq_inertiaDeg
  条件: [q.LiesOver p] [p.是极大] [q.是极大]
  证明: by
  rw [inertiaDeg'_algebraMap]; rw [inertiaDeg_eq_of_isMaximal p q]

@[deprecated (since := "2026-07-03")] alias inertiaDeg_eq_inertiaDeg' := inertiaDeg'_eq_inertiaDeg
-/
theorem inertiaDeg'_eq_inertiaDeg [q.LiesOver p] [p.IsMaximal] [q.IsMaximal] :
    p.inertiaDeg' q = q.inertiaDeg R := by
  rw [inertiaDeg'_algebraMap]; rw [inertiaDeg_eq_of_isMaximal p q]

@[deprecated (since := "2026-07-03")] alias inertiaDeg_eq_inertiaDeg' := inertiaDeg'_eq_inertiaDeg

/--
theorem `inertiaDeg_tower` / 定理 `inertiaDeg_tower`

English:
theorem inertiaDeg_tower
  given: [r.LiesOver q]
  proof: by
  by_cases hr : r.IsPrime
  · have : q.IsPrime := isPrime_of_liesOver r q
    have : q.LiesOver (r.under R) := LiesOver.tower_bot r q (r.under R)
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) r
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) q
    let := Localization

中文:
定理 inertiaDeg_tower
  条件: [r.LiesOver q]
  证明: by
  by_cases hr : r.IsPrime
  · have : q.IsPrime := isPrime_of_liesOver r q
    have : q.LiesOver (r.under R) := LiesOver.tower_bot r q (r.under R)
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) r
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) q
    let := Localization

Depends on / 依赖: AtPrime, IsPrime, LiesOver, LiesOver.tower_bot, Localization, Localization.AtPrime.algebraOfLiesOver, Module, Module.finrank_mul_finrank, algebraOfLiesOver, eq_comm, finrank_mul_finrank, inertiaDeg_def, inertiaDeg_eq, inertiaDeg_of_not_isPrime, isPrime_of_liesOver, q.IsPrime, q.LiesOver, r.IsPrime, r.under, tower_bot
-/
theorem inertiaDeg_tower [r.LiesOver q] :
    r.inertiaDeg R = q.inertiaDeg R * r.inertiaDeg S := by
  by_cases hr : r.IsPrime
  · have : q.IsPrime := isPrime_of_liesOver r q
    have : q.LiesOver (r.under R) := LiesOver.tower_bot r q (r.under R)
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) r
    let := Localization.AtPrime.algebraOfLiesOver (r.under R) q
    let := Localization.AtPrime.algebraOfLiesOver q r
    rw [inertiaDeg_def]; rw [inertiaDeg_eq (r.under R)]; rw [inertiaDeg_eq q]; rw [eq_comm]
    apply Module.finrank_mul_finrank
  · rw [inertiaDeg_of_not_isPrime r R hr, inertiaDeg_of_not_isPrime r S hr, mul_zero]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_tower := inertiaDeg_tower

/--
theorem `inertiaDeg_below_dvd` / 定理 `inertiaDeg_below_dvd`

English:
theorem inertiaDeg_below_dvd
  given: [r.LiesOver q]
  proof: by
  use r.inertiaDeg S
  rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_dvd := inertiaDeg_below_dvd

中文:
定理 inertiaDeg_below_dvd
  条件: [r.LiesOver q]
  证明: by
  use r.inertiaDeg S
  rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_dvd := inertiaDeg_below_dvd

Depends on / 依赖: inertiaDeg, inertiaDeg_tower, r.inertiaDeg
-/
theorem inertiaDeg_below_dvd [r.LiesOver q] :
    q.inertiaDeg R ∣ r.inertiaDeg R := by
  use r.inertiaDeg S
  rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_dvd := inertiaDeg_below_dvd

/--
theorem `inertiaDeg_above_dvd` / 定理 `inertiaDeg_above_dvd`

English:
theorem inertiaDeg_above_dvd
  given: [r.LiesOver q]
  proof: by
  use q.inertiaDeg R
  rw [mul_comm]; rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_dvd := inertiaDeg_above_dvd

中文:
定理 inertiaDeg_above_dvd
  条件: [r.LiesOver q]
  证明: by
  use q.inertiaDeg R
  rw [mul_comm]; rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_dvd := inertiaDeg_above_dvd

Depends on / 依赖: inertiaDeg, inertiaDeg_tower, mul_comm, q.inertiaDeg
-/
theorem inertiaDeg_above_dvd [r.LiesOver q] :
    r.inertiaDeg S ∣ r.inertiaDeg R := by
  use q.inertiaDeg R
  rw [mul_comm]; rw [← inertiaDeg_tower]

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_dvd := inertiaDeg_above_dvd

/--
theorem `inertiaDeg_below_le` / 定理 `inertiaDeg_below_le`

English:
theorem inertiaDeg_below_le
  given: [r.IsPrime] [r.LiesOver q] [Module.Finite R T]
  proof: Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_below_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_le := inertiaDeg_below_le

中文:
定理 inertiaDeg_below_le
  条件: [r.是素] [r.LiesOver q] [模.有限 R T]
  证明: Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_below_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_le := inertiaDeg_below_le

Depends on / 依赖: Nat.le_of_dvd, inertiaDeg_below_dvd, inertiaDeg_pos, le_of_dvd, q.inertiaDeg_below_dvd, r.inertiaDeg_pos
-/
theorem inertiaDeg_below_le [r.IsPrime] [r.LiesOver q] [Module.Finite R T] :
    q.inertiaDeg R <= r.inertiaDeg R :=
  Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_below_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_below_le := inertiaDeg_below_le

/--
theorem `inertiaDeg_above_le` / 定理 `inertiaDeg_above_le`

English:
theorem inertiaDeg_above_le
  given: [r.IsPrime] [r.LiesOver q] [Module.Finite R T]
  proof: Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_above_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_le := inertiaDeg_above_le

中文:
定理 inertiaDeg_above_le
  条件: [r.是素] [r.LiesOver q] [模.有限 R T]
  证明: Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_above_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_le := inertiaDeg_above_le

Depends on / 依赖: Nat.le_of_dvd, inertiaDeg_above_dvd, inertiaDeg_pos, le_of_dvd, q.inertiaDeg_above_dvd, r.inertiaDeg_pos
-/
theorem inertiaDeg_above_le [r.IsPrime] [r.LiesOver q] [Module.Finite R T] :
    r.inertiaDeg S <= r.inertiaDeg R :=
  Nat.le_of_dvd (r.inertiaDeg_pos R) (q.inertiaDeg_above_dvd r)

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_above_le := inertiaDeg_above_le

variable (R) in
open Pointwise in
@[simp]
/--
theorem `inertiaDeg_smul` / 定理 `inertiaDeg_smul`

English:
theorem inertiaDeg_smul
  statement: {G : Type*} [Group G] [MulSemiringAction G S] [SMulCommClass G R S]
  proof: by
  by_cases hq : q.IsPrime; swap
  · rw [inertiaDeg_of_not_isPrime, inertiaDeg_of_not_isPrime] <;> simpa
  · let p := q.under R
    let f₀ := MulSemiringAction.toAlgAut G R S g
    let := Localization.AtPrime.algebraOfLiesOver p q
    let := Localization.AtPrime.algebraOfLiesOver p (g • q)
    rw 

中文:
定理 inertiaDeg_smul
  结论: {G : 类型} [群 G] [MulSemiring作用 G S] [标量交换类 G R S]
  证明: by
  by_cases hq : q.IsPrime; swap
  · rw [inertiaDeg_of_not_isPrime, inertiaDeg_of_not_isPrime] <;> simpa
  · let p := q.under R
    let f₀ := MulSemiringAction.toAlgAut G R S g
    let := Localization.AtPrime.algebraOfLiesOver p q
    let := Localization.AtPrime.algebraOfLiesOver p (g • q)
    rw 

Depends on / 依赖: AtPrime, Ideal.residueFieldAlgEquiv, IsPrime, Localization, Localization.AtPrime.algebraOfLiesOver, MulSemiringAction, MulSemiringAction.toAlgAut, algebraOfLiesOver, comap_symm, finrank_eq, inertiaDeg_eq, inertiaDeg_of_not_isPrime, q.IsPrime, q.under, residueFieldAlgEquiv, toAlgAut, toLinearEquiv, toLinearEquiv.finrank_eq, toRingEquiv
-/
theorem inertiaDeg_smul {G : Type*} [Group G] [MulSemiringAction G S] [SMulCommClass G R S]
    (g : G) : (g • q).inertiaDeg R = q.inertiaDeg R := by
  by_cases hq : q.IsPrime; swap
  · rw [inertiaDeg_of_not_isPrime, inertiaDeg_of_not_isPrime] <;> simpa
  · let p := q.under R
    let f₀ := MulSemiringAction.toAlgAut G R S g
    let := Localization.AtPrime.algebraOfLiesOver p q
    let := Localization.AtPrime.algebraOfLiesOver p (g • q)
    rw [inertiaDeg_eq p q]; rw [inertiaDeg_eq p (g • q)]
    let e₂ := Ideal.residueFieldAlgEquiv' p (g • q) q f₀.symm (comap_symm f₀.toRingEquiv).symm
    exact e₂.toLinearEquiv.finrank_eq

@[deprecated (since := "2026-07-03")] alias inertiaDeg'_smul := inertiaDeg_smul

/--
theorem `cardQuot_pow_inertiaDeg` / 定理 `cardQuot_pow_inertiaDeg`

English:
theorem cardQuot_pow_inertiaDeg
  given: [Module.Finite R S] [p.IsMaximal] [q.IsMaximal] [q.LiesOver p]
  proof: by
  let _ : Field (R ⧸ p) := Quotient.field p
  rw [← inertiaDeg'_eq_inertiaDeg p q]; rw [inertiaDeg'_algebraMap p q]
  exact Module.natCard_eq_pow_finrank.symm

@[deprecated (since := "2026-07-03")] alias cardQuot_pow_inertiaDeg' := cardQuot_pow_inertiaDeg

中文:
定理 cardQuot_pow_inertiaDeg
  条件: [模.有限 R S] [p.是极大] [q.是极大] [q.LiesOver p]
  证明: by
  let _ : Field (R ⧸ p) := Quotient.field p
  rw [← inertiaDeg'_eq_inertiaDeg p q]; rw [inertiaDeg'_algebraMap p q]
  exact Module.natCard_eq_pow_finrank.symm

@[deprecated (since := "2026-07-03")] alias cardQuot_pow_inertiaDeg' := cardQuot_pow_inertiaDeg

Depends on / 依赖: Module, Module.natCard_eq_pow_finrank.symm, Quotient, Quotient.field, _algebraMap, _eq_inertiaDeg, inertiaDeg, natCard_eq_pow_finrank
-/
theorem cardQuot_pow_inertiaDeg [Module.Finite R S] [p.IsMaximal] [q.IsMaximal] [q.LiesOver p] :
    p.cardQuot ^ q.inertiaDeg R = q.cardQuot := by
  let _ : Field (R ⧸ p) := Quotient.field p
  rw [← inertiaDeg'_eq_inertiaDeg p q]; rw [inertiaDeg'_algebraMap p q]
  exact Module.natCard_eq_pow_finrank.symm

@[deprecated (since := "2026-07-03")] alias cardQuot_pow_inertiaDeg' := cardQuot_pow_inertiaDeg

/--
theorem `absNorm_pow_inertiaDeg` / 定理 `absNorm_pow_inertiaDeg`

English:
theorem absNorm_pow_inertiaDeg
  statement: [Module.Finite R S] [q.IsPrime] [q.LiesOver p]
  proof: by
  by_cases hp : p = ⊥
  · subst hp
    simpa [eq_bot_of_liesOver_bot R q] using (inertiaDeg_pos q R).ne'
  have := isPrime_of_liesOver q p
  have := isMaximal_of_isPrime_of_ne_bot p hp
  have := IsMaximal.of_liesOver_isMaximal q p
  exact cardQuot_pow_inertiaDeg p q

@[deprecated (since := "2026-

中文:
定理 absNorm_pow_inertiaDeg
  结论: [模.有限 R S] [q.是素] [q.LiesOver p]
  证明: by
  by_cases hp : p = ⊥
  · subst hp
    simpa [eq_bot_of_liesOver_bot R q] using (inertiaDeg_pos q R).ne'
  have := isPrime_of_liesOver q p
  have := isMaximal_of_isPrime_of_ne_bot p hp
  have := IsMaximal.of_liesOver_isMaximal q p
  exact cardQuot_pow_inertiaDeg p q

@[deprecated (since := "2026-

Depends on / 依赖: IsMaximal, IsMaximal.of_liesOver_isMaximal, cardQuot_pow_inertiaDeg, eq_bot_of_liesOver_bot, inertiaDeg_pos, isMaximal_of_isPrime_of_ne_bot, isPrime_of_liesOver, of_liesOver_isMaximal
-/
theorem absNorm_pow_inertiaDeg [Module.Finite R S] [q.IsPrime] [q.LiesOver p]
    [IsDedekindDomain R] [IsDedekindDomain S] [Module.Free Int R] [Module.Free Int S] :
    p.absNorm ^ q.inertiaDeg R = q.absNorm := by
  by_cases hp : p = ⊥
  · subst hp
    simpa [eq_bot_of_liesOver_bot R q] using (inertiaDeg_pos q R).ne'
  have := isPrime_of_liesOver q p
  have := isMaximal_of_isPrime_of_ne_bot p hp
  have := IsMaximal.of_liesOver_isMaximal q p
  exact cardQuot_pow_inertiaDeg p q

@[deprecated (since := "2026-07-03")] alias absNorm_pow_inertiaDeg' := absNorm_pow_inertiaDeg

/--
theorem `natAbs_pow_inertiaDeg` / 定理 `natAbs_pow_inertiaDeg`

English:
theorem natAbs_pow_inertiaDeg
  statement: [IsDedekindDomain R] [Module.Free Int R] [Module.Finite Int R] (p : Int)
  proof: by
  simpa using absNorm_pow_inertiaDeg (span {p}) P

@[deprecated (since := "2026-07-03")] alias natAbs_pow_inertiaDeg' := natAbs_pow_inertiaDeg

中文:
定理 natAbs_pow_inertiaDeg
  结论: [是Dedekind整环 R] [模.自由 整数 R] [模.有限 整数 R] (p : 整数)
  证明: by
  simpa using absNorm_pow_inertiaDeg (span {p}) P

@[deprecated (since := "2026-07-03")] alias natAbs_pow_inertiaDeg' := natAbs_pow_inertiaDeg

Depends on / 依赖: absNorm_pow_inertiaDeg
-/
theorem natAbs_pow_inertiaDeg [IsDedekindDomain R] [Module.Free Int R] [Module.Finite Int R] (p : Int)
    (P : Ideal R) [P.IsPrime] [P.LiesOver (span {p})] :
    p.natAbs ^ P.inertiaDeg Int = absNorm P := by
  simpa using absNorm_pow_inertiaDeg (span {p}) P

@[deprecated (since := "2026-07-03")] alias natAbs_pow_inertiaDeg' := natAbs_pow_inertiaDeg

/--
theorem `pow_inertiaDeg` / 定理 `pow_inertiaDeg`

English:
theorem pow_inertiaDeg
  statement: [IsDedekindDomain R] [Module.Free Int R] [Module.Finite Int R] (p : Nat)
  proof: natAbs_pow_inertiaDeg p P

中文:
定理 pow_inertiaDeg
  结论: [是Dedekind整环 R] [模.自由 整数 R] [模.有限 整数 R] (p : 自然数)
  证明: natAbs_pow_inertiaDeg p P

Depends on / 依赖: natAbs_pow_inertiaDeg
-/
theorem pow_inertiaDeg [IsDedekindDomain R] [Module.Free Int R] [Module.Finite Int R] (p : Nat)
    (P : Ideal R) [P.IsPrime] [P.LiesOver (span {(p : Int)})] :
    p ^ P.inertiaDeg Int = absNorm P :=
  natAbs_pow_inertiaDeg p P

end

end Ideal
