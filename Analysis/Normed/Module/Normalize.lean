...
/-
**NormedSpace.normalize** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：NormedSpace.normalize (x : V) : V
参数：x : V。
该定义给出了上述对象。
...
@[simp]
/-
**NormedSpace.normalize_zero_eq_zero** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_zero_eq_zero : normalize (0 : V) = 0
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[simp]
/-
**NormedSpace.normalize_eq_zero_iff** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_eq_zero_iff (x : V) : normalize x = 0 ↔ x = 0
参数：x : V。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
@[simp]
/-
**NormedSpace.norm_smul_normalize** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：norm_smul_normalize (x : V) : ‖x‖ • normalize x = x
参数：x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
@[simp]
/-
**NormedSpace.norm_normalize_eq_one_iff** 是 Mathlib 中的一个引理，位于命名空间 `NormedSpace`。
形式化陈述：norm_normalize_eq_one_iff {x : V} : ‖normalize x‖ = 1 ↔ x != 0
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
...
/-
**NormedSpace.normalize_eq_self_of_norm_eq_one** 是 Mathlib 中的一个引理，位于命名空间 `Normed
Space`。
形式化陈述：normalize_eq_self_of_norm_eq_one {x : V} (h : ‖x‖ = 1) : normalize x = x
参数：h : ‖x‖ = 1。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
@[simp]
/-
**NormedSpace.normalize_normalize** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_normalize (x : V) : normalize (normalize x) = normalize x
参数：x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用引理 `NormedSpace.normalize_eq_self_of_norm_eq_one`：normalize_eq_self_of_norm_
eq_one {x : V} (h : ‖x‖ = 1) : normalize x = x
· 使用定理 `eq_false`：∀ {p : Prop}, ¬p → p = False
· 使用定理 `not_false_eq_true`：(¬False) = True
-/
...
@[simp]
/-
**NormedSpace.normalize_neg** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_neg (x : V) : normalize (-x) = - normalize x
参数：x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
theorem normalize_neg (x : V) : normalize (-x) = - normalize x := by
  simp [normalize]
/-
**NormedSpace.normalize_smul_of_pos** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_smul_of_pos {r : Real} (hr : 0 < r) (x : V) : normalize (r • x) 
= normalize x
参数：hr : 0 < r；x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**NormedSpace.normalize_smul_of_neg** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_smul_of_neg {r : Real} (hr : r < 0) (x : V) : normalize (r • x) 
= - normalize x
参数：hr : r < 0；x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `NormedSpace.normalize_neg`：normalize_neg (x : V) : normalize (-x) = - no
rmalize x
· 使用定理 `NormedSpace.normalize_smul_of_pos`：normalize_smul_of_pos {r : Real} (hr 
: 0 < r) (x : V) : normalize (r • x) = normalize x
...
/-
**NormedSpace.normalize_smul** 是 Mathlib 中的一个定理，位于命名空间 `NormedSpace`。
形式化陈述：normalize_smul (r : Real) (x : V) : normalize (r • x) = (SignType.sign r :
 Real) • normalize x
参数：r : Real；x : V。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `NormedSpace.normalize_smul_of_neg`：normalize_smul_of_neg {r : Real} (hr 
: r < 0) (x : V) : normalize (r • x) = - normalize x
