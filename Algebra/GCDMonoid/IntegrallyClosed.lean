...
/--
theorem `IsLocalization.surj_of_gcd_domain` / 定理 `IsLocalization.surj_of_gcd_domain`

English:
theorem IsLocalization.surj_of_gcd_domain
  statement: [GCDMonoid R] (M : Submonoid R) [IsLocalization M A]
  proof: by
...
Depends on / 依赖: IsLocalization, IsLocalization.exists_mk, IsLocalization.mk, IsLocalization.mul_mk, _eq_mk, _mul_cancel_left, _of_mul, convert, exists_mk, extract_gcd, mul_comm, mul_mk
-/
theorem IsLocalization.surj_of_gcd_domain [GCDMonoid R] (M : Submonoid R) [IsLocalization M A]
    (z : A) : exists a b : R, IsUnit (gcd a b) ∧ z * algebraMap R A b = algebraMap R A a := by
  obtain ⟨x, ⟨y, hy⟩, rfl⟩ := IsLocalization.exists_mk'_eq M z
  obtain ⟨x', y', hx', hy', hu⟩ := extract_gcd x y
  use x', y', hu
  rw [mul_comm]; rw [IsLocalization.mul_mk'_eq_mk'_of_mul]
...
instance (priority := 100) GCDMonoid.toIsIntegrallyClosed
  ...
  (isIntegrallyClosed_iff (FractionRing R)).mpr fun {X} ⟨p, hp₁, hp₂⟩ => by
    ...
    rw [← coe_map_inv]; rw [eq_comm]; rw [Units.eq_mul_inv_iff_mul_eq]
    exact he
