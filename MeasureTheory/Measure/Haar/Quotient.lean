...
/--
Instance `QuotientGroup.measurableSMul` / 实例 `QuotientGroup.measurableSMul`

English:
instance QuotientGroup.measurableSMul
...
/--
theorem `measurePreserving_quotientGroup_mk_of_QuotientMeasureEqMeasurePreimage` / 定理 `measurePreserving_quotientGroup_mk_of_QuotientMeasureEqMeasurePreimage`

English:
theorem measurePreserving_quotientGroup_mk_of_QuotientMeasureEqMeasurePreimage
  proof: h𝓕.measurePreserving_quotient_mk μ

local notation "π" => @QuotientGroup.mk G _ Γ

中文:
定理 measurePreserving_quotientGroup_mk_of_QuotientMeasureEqMeasurePreimage
  证明: h𝓕.measurePreserving_quotient_mk μ

local notation "π" => @QuotientGroup.mk G _ Γ

Depends on / 依赖: measurePreserving_quotient_mk
-/
theorem measurePreserving_quotientGroup_mk_of_QuotientMeasureEqMeasurePreimage
...
/--
lemma `MeasureTheory.QuotientMeasureEqMeasurePreimage.smulInvariantMeasure_quotient` / 引理 `MeasureTheory.QuotientMeasureEqMeasurePreimage.smulInvariantMeasure_quotient`
...
/--
lemma `MeasureTheory.QuotientMeasureEqMeasurePreimage.mulInvariantMeasure_quotient` / 引理 `MeasureTheory.QuotientMeasureEqMeasurePreimage.mulInvariantMeasure_quotient`
...
left invariant measures, if normalized by a single positive finite-measured set. -/]
/--
theorem `MeasureTheory.Measure.IsMulLeftInvariant.quotientMeasureEqMeasurePreimage_of_set` / 定理 `MeasureTheory.Measure.IsMulLeftInvariant.quotientMeasureEqMeasurePreimage_of_set`

English:
theorem MeasureTheory.Measure.IsMulLeftInvariant.quotientMeasureEqMeasurePreimage_of_set
  statement: {s : Set G}
  proof: by
...
/--
theorem `MeasureTheory.leftInvariantIsQuotientMeasureEqMeasurePreimage` / 定理 `MeasureTheory.leftInvariantIsQuotientMeasureEqMeasurePreimage`

English:
theorem MeasureTheory.leftInvariantIsQuotientMeasureEqMeasurePreimage
...
itself Haar. -/]
/--
theorem `MeasureTheory.QuotientMeasureEqMeasurePreimage.haarMeasure_quotient` / 定理 `MeasureTheory.QuotientMeasureEqMeasurePreimage.haarMeasure_quotient`
...
/--
theorem `IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_HaarMeasure` / 定理 `IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_HaarMeasure`

English:
theorem IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_HaarMeasure
  statement: {𝓕 : Set G}
  proof: by
...
theorem IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_HaarMeasure {𝓕 : Set G}
    ...
    (hμK : μ V = ν ((π ⁻¹' V) inter 𝓕)) (neTopV : μ V != ⊤) :
...
properly normalized, satisfies `AddQuotientMeasureEqMeasurePreimage`. -/]
/--
theorem `IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_smulHaarMeasure` / 定理 `IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_smulHaarMeasure`

English:
theorem IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_smulHaarMeasure
  statement: {𝓕 : Set G}
  proof: by
...
theorem IsFundamentalDomain.QuotientMeasureEqMeasurePreimage_smulHaarMeasure {𝓕 : Set G}
    ...
    QuotientMeasureEqMeasurePreimage ν
      ((ν ((π ⁻¹' (K : Set (G ⧸ Γ))) inter 𝓕)) • haarMeasure K) := by
  set c := ν ((π ⁻¹' (K : Set (G ⧸ Γ))) inter 𝓕)
  have c_ne_top : c != ∞ := measure_inter_ne_top_of_right_ne_top h𝓕_finite
  set μ := c • haarMeasure K
  have hμK : μ K = c := by simp [μ, haarMeasure_self]
...
@[to_additive /-- The `essSup` of a function `g` on the additive quotient space `G ⧸ Γ` with respect
  ...
  to `μ`. -/]
/--
lemma `essSup_comp_quotientGroup_mk` / 引理 `essSup_comp_quotientGroup_mk`

English:
lemma essSup_comp_quotientGroup_mk
  statement: [μ.IsMulRightInvariant] {g : G ⧸ Γ -> Real>=0∞}
  proof: by
...
Depends on / 依赖: Measurable, QuotientGroup, QuotientGroup.mk, QuotientGroup.mk_mul_of_mem, aemeasurable, continuous_quotient_mk, essSup_map_measure, essSup_measure_restrict, g_ae_measurable, measurable, mk_mul_of_mem
-/
lemma essSup_comp_quotientGroup_mk [μ.IsMulRightInvariant] {g : G ⧸ Γ -> Real>=0∞}
...
@[to_additive /-- Given an additive quotient space `G ⧸ Γ` where `Γ` is `Countable`, and the
  ...
  will take the value `∞` on any open set in the quotient! -/]
/--
lemma `_root_.MeasureTheory.IsFundamentalDomain.absolutelyContinuous_map` / 引理 `_root_.MeasureTheory.IsFundamentalDomain.absolutelyContinuous_map`
...
/--
lemma `QuotientGroup.integral_eq_integral_automorphize` / 引理 `QuotientGroup.integral_eq_integral_automorphize`

English:
lemma QuotientGroup.integral_eq_integral_automorphize
...
lemma QuotientGroup.integral_eq_integral_automorphize {E : Type*} [NormedAddCommGroup E]
    [NormedSpace Real E] [μ.IsMulRightInvariant] {f : G -> E}
...
/--
lemma `QuotientGroup.integral_mul_eq_integral_automorphize_mul` / 引理 `QuotientGroup.integral_mul_eq_integral_automorphize_mul`

English:
lemma QuotientGroup.integral_mul_eq_integral_automorphize_mul
  statement: {K : Type*} [NormedField K]
  proof: by
...
lemma QuotientGroup.integral_mul_eq_integral_automorphize_mul {K : Type*} [NormedField K]
    [NormedSpace Real K] [μ.IsMulRightInvariant] {f : G -> K}
...
/--
lemma `QuotientAddGroup.integral_mul_eq_integral_automorphize_mul` / 引理 `QuotientAddGroup.integral_mul_eq_integral_automorphize_mul`

English:
lemma QuotientAddGroup.integral_mul_eq_integral_automorphize_mul
  statement: {K : Type*} [NormedField K]
  proof: by
...
lemma QuotientAddGroup.integral_mul_eq_integral_automorphize_mul {K : Type*} [NormedField K]
    [NormedSpace Real K] [μ'.IsAddRightInvariant] {f : G' -> K}
    (f_ℒ_1 : Integrable f μ') {g : G' ⧸ Γ' -> K} (hg : AEStronglyMeasurable g μ_𝓕)
