/-
Copyright (c) 2024 Joël Riou. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Joël Riou
-/
module

public import Mathlib.Algebra.Homology.Embedding.Basic
public import Mathlib.Algebra.Homology.Additive

/-!
# The restriction functor of an embedding of complex shapes

Given `c` and `c'` complex shapes on two types, and `e : c.Embedding c'`
(satisfying `[e.IsRelIff]`), we define the restriction functor
`e.restrictionFunctor C : HomologicalComplex C c' ⥤ HomologicalComplex C c`.

-/

@[expose] public section

open CategoryTheory Category Limits ZeroObject

variable {ι ι' : Type*} {c : ComplexShape ι} {c' : ComplexShape ι'}

namespace HomologicalComplex

variable {C : Type*} [Category* C] [HasZeroMorphisms C]
  (K L M : HomologicalComplex C c') (φ : K ⟶ L) (φ' : L ⟶ M)
  (e : c.Embedding c') [e.IsRelIff]

/-- Given `K : HomologicalComplex C c'` and `e : c.Embedding c'` (satisfying `[e.IsRelIff]`),
this is the homological complex in `HomologicalComplex C c` obtained by restriction. -/
@[simps]
/--
Definition of `restriction` / `restriction` 的定义

English:
definition restriction
  signature: : HomologicalComplex C c where
  body: K.X (e.f i)
  d _ _ := K.d _ _
  shape i j hij := K.shape _ _ (by simpa only [← e.rel_iff] using hij)

中文:
定义 restriction
  签名: : HomologicalComplex C c where
  定义体: K.X (e.f i)
  d _ _ := K.d _ _
  shape i j hij := K.shape _ _ (by simpa only [← e.rel_iff] using hij)
-/
def restriction : HomologicalComplex C c where
  X i := K.X (e.f i)
  d _ _ := K.d _ _
  shape i j hij := K.shape _ _ (by simpa only [← e.rel_iff] using hij)

/--
Definition of `restrictionXIso` / `restrictionXIso` 的定义

English:
definition restrictionXIso
  signature: {i : ι} {i' : ι'} (h : e.f i = i')
  body: eqToIso (h ▸ rfl)

中文:
定义 restrictionXIso
  签名: {i : ι} {i' : ι'} (h : e.f i = i')
  定义体: eqToIso (h ▸ rfl)

Depends on / 依赖: eqToIso
-/
def restrictionXIso {i : ι} {i' : ι'} (h : e.f i = i') :
    (K.restriction e).X i ≅ K.X i' :=
  eqToIso (h ▸ rfl)

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency.types false in
@[reassoc]
/--
lemma `restriction_d_eq` / 引理 `restriction_d_eq`

English:
lemma restriction_d_eq
  given: {i j : ι} {i' j' : ι'} (hi : e.f i = i') (hj : e.f j = j')
  proof: by
  subst hi hj
  simp [restrictionXIso]

中文:
引理 restriction_d_eq
  条件: {i j : ι} {i' j' : ι'} (hi : e.f i = i') (hj : e.f j = j')
  证明: by
  subst hi hj
  simp [restrictionXIso]

Depends on / 依赖: restrictionXIso
-/
lemma restriction_d_eq {i j : ι} {i' j' : ι'} (hi : e.f i = i') (hj : e.f j = j') :
    (K.restriction e).d i j = (K.restrictionXIso e hi).hom ≫ K.d i' j' ≫
      (K.restrictionXIso e hj).inv := by
  subst hi hj
  simp [restrictionXIso]

variable {K L}

set_option backward.defeqAttrib.useBackward true in
/-- The morphism `K.restriction e ⟶ L.restriction e` induced by a morphism `φ : K ⟶ L`. -/
@[simps]
/--
Definition of `restrictionMap` / `restrictionMap` 的定义

English:
definition restrictionMap
  signature: : K.restriction e ⟶ L.restriction e where
  body: φ.f (e.f i)

中文:
定义 restrictionMap
  签名: : K.restriction e ⟶ L.restriction e where
  定义体: φ.f (e.f i)
-/
def restrictionMap : K.restriction e ⟶ L.restriction e where
  f i := φ.f (e.f i)

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency.types false in
@[reassoc]
/--
lemma `restrictionMap_f'` / 引理 `restrictionMap_f'`

English:
lemma restrictionMap_f'
  given: {i : ι} {i' : ι'} (hi : e.f i = i')
  proof: by
  subst hi
  simp [restrictionXIso]

中文:
引理 restrictionMap_f'
  条件: {i : ι} {i' : ι'} (hi : e.f i = i')
  证明: by
  subst hi
  simp [restrictionXIso]

Depends on / 依赖: restrictionXIso
-/
lemma restrictionMap_f' {i : ι} {i' : ι'} (hi : e.f i = i') :
    (restrictionMap φ e).f i = (K.restrictionXIso e hi).hom ≫
      φ.f i' ≫ (L.restrictionXIso e hi).inv := by
  subst hi
  simp [restrictionXIso]

variable (K)

@[simp]
/--
lemma `restrictionMap_id` / 引理 `restrictionMap_id`

English:
lemma restrictionMap_id
  statement: restrictionMap (𝟙 K) e = 𝟙 _
  proof: rfl

@[simp, reassoc]

中文:
引理 restrictionMap_id
  结论: restrictionMap (𝟙 K) e = 𝟙 _
  证明: rfl

@[simp, reassoc]
-/
lemma restrictionMap_id : restrictionMap (𝟙 K) e = 𝟙 _ := rfl

@[simp, reassoc]
/--
lemma `restrictionMap_comp` / 引理 `restrictionMap_comp`

English:
lemma restrictionMap_comp
  proof: rfl

中文:
引理 restrictionMap_comp
  证明: rfl
-/
lemma restrictionMap_comp :
    restrictionMap (φ ≫ φ') e = restrictionMap φ e ≫ restrictionMap φ' e := rfl

end HomologicalComplex

namespace ComplexShape.Embedding

variable (e : Embedding c c') (C : Type*) [Category* C] [HasZeroObject C] [e.IsRelIff]

/-- Given `e : ComplexShape.Embedding c c'`, this is the restriction
functor `HomologicalComplex C c' ⥤ HomologicalComplex C c`. -/
@[simps]
/--
Definition of `restrictionFunctor` / `restrictionFunctor` 的定义

English:
definition restrictionFunctor
  signature: [HasZeroMorphisms C]
  body: K.restriction e
  map φ := HomologicalComplex.restrictionMap φ e

中文:
定义 restrictionFunctor
  签名: [HasZeroMorphisms C]
  定义体: K.restriction e
  map φ := HomologicalComplex.restrictionMap φ e

Depends on / 依赖: K.restriction, restriction
-/
def restrictionFunctor [HasZeroMorphisms C] :
    HomologicalComplex C c' ⥤ HomologicalComplex C c where
  obj K := K.restriction e
  map φ := HomologicalComplex.restrictionMap φ e

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [HasZeroMorphisms
  signature: C] : (e.restrictionFunctor C).PreservesZeroMorphisms where

中文:
实例 [HasZeroMorphisms
  签名: C] : (e.restrictionFunctor C).PreservesZeroMorphisms where
-/
instance [HasZeroMorphisms C] : (e.restrictionFunctor C).PreservesZeroMorphisms where

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [Preadditive
  signature: C] : (e.restrictionFunctor C).Additive where

中文:
实例 [Preadditive
  签名: C] : (e.restrictionFunctor C).Additive where
-/
instance [Preadditive C] : (e.restrictionFunctor C).Additive where

end ComplexShape.Embedding
