...
/-
**Polynomial.deriv_gaussian_eq_hermite_mul_gaussian** 是 Mathlib 中的一个定理，位于命名空间 `P
olynomial`。
形式化陈述：deriv_gaussian_eq_hermite_mul_gaussian (n : Nat) (x : Real) : deriv^[n] (f
un y => Real.exp (-(y ^ 2 / 2))) x = (-1 : Real) ^ n * aeval x (hermite n) * Rea
l.exp (-(x ^ 2 / 2))
参数：n : Nat；x : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Polynomial.hermite_eq_deriv_gaussian** 是 Mathlib 中的一个定理，位于命名空间 `Polynomial`。
形式化陈述：hermite_eq_deriv_gaussian (n : Nat) (x : Real) : aeval x (hermite n) = (-1
 : Real) ^ n * deriv^[n] (fun y => Real.exp (-(y ^ 2 / 2))) x / Real.exp (-(x ^ 
2 / 2))
参数：n : Nat；x : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Polynomial.deriv_gaussian_eq_hermite_mul_gaussian`：deriv_gaussian_eq_her
mite_mul_gaussian (n : Nat) (x : Real) : deriv^[n] (fun y => Real.exp (-(y ^ 2 /
 2))) x = (-1 : Real) ^ n * aeval x (he…
...
/-
**Polynomial.hermite_eq_deriv_gaussian'** 是 Mathlib 中的一个定理，位于命名空间 `Polynomial`。
形式化陈述：hermite_eq_deriv_gaussian' (n : Nat) (x : Real) : aeval x (hermite n) = (-
1 : Real) ^ n * deriv^[n] (fun y => Real.exp (-(y ^ 2 / 2))) x * Real.exp (x ^ 2
 / 2)
参数：n : Nat；x : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Polynomial.hermite_eq_deriv_gaussian`：hermite_eq_deriv_gaussian (n : Nat
) (x : Real) : aeval x (hermite n) = (-1 : Real) ^ n * deriv^[n] (fun y => Real.
exp (-(y ^ 2 / 2))) x / Re…
