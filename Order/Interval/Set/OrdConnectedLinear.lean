...
/--
lemma `Set.Nonempty.ordConnected_iff_of_bdd` / 引理 `Set.Nonempty.ordConnected_iff_of_bdd`

English:
lemma Set.Nonempty.ordConnected_iff_of_bdd
  proof: have h₄ : I.Finite := h₁.finite_of_bddAbove h₂
...
Depends on / 依赖: Finite, I.Finite, I.Icc_subset, Icc_subset, csInf_mem, csSup_mem, finite_of_bddAbove, le_antisymm, ordConnected_Icc, subset_Icc_csInf_csSup
-/
lemma Set.Nonempty.ordConnected_iff_of_bdd
...
English:
lemma Set.Nonempty.ordConnected_iff_of_bdd'
  statement: [ConditionallyCompleteLinearOrder α]
...
/--
lemma `Set.ordConnected_iff_disjoint_Ioo_empty` / 引理 `Set.ordConnected_iff_disjoint_Ioo_empty`

English:
lemma Set.ordConnected_iff_disjoint_Ioo_empty
  given: [LinearOrder α] [LocallyFiniteOrder α]
  proof: by
...
lemma Set.ordConnected_iff_disjoint_Ioo_empty [LinearOrder α] [LocallyFiniteOrder α] :
    I.OrdConnected ↔ forallᵉ (x in I) (y in I), Disjoint (Ioo x y) I -> Ioo x y = ∅ := by
  simp_rw [← Set.subset_compl_iff_disjoint_right]
  refine ⟨fun h' x hx y hy hxy => ?_, fun h' => ordConnected_of_Ioo fun x hx y hy hxy z hz => ?_⟩
  · suffices forall z, x < z -> y <= z by ext z; simpa using this z
    intro z hz
...
/--
lemma `Set.Nonempty.eq_Icc_iff_nat` / 引理 `Set.Nonempty.eq_Icc_iff_nat`

English:
lemma Set.Nonempty.eq_Icc_iff_nat
  statement: {I : Set Nat}
  proof: by
...
Depends on / 依赖: OrderBot, OrderBot.bddBelow, bddBelow, ordConnected_iff_disjoint_Ioo_empty, ordConnected_iff_of_bdd
-/
lemma Set.Nonempty.eq_Icc_iff_nat {I : Set Nat}
    (h₀ : I.Nonempty) (h₂ : BddAbove I) :
    I = Icc (sInf I) (sSup I) ↔ forallᵉ (x in I) (y in I), Disjoint (Ioo x y) I -> y <= x + 1 := by
  simp [← h₀.ordConnected_iff_of_bdd (OrderBot.bddBelow I) h₂, ordConnected_iff_disjoint_Ioo_empty]

/--
lemma `Set.Nonempty.eq_Icc_iff_int` / 引理 `Set.Nonempty.eq_Icc_iff_int`

English:
lemma Set.Nonempty.eq_Icc_iff_int
  statement: {I : Set Int}
  proof: by
...
Depends on / 依赖: ordConnected_iff_disjoint_Ioo_empty, ordConnected_iff_of_bdd
-/
lemma Set.Nonempty.eq_Icc_iff_int {I : Set Int}
    (h₀ : I.Nonempty) (h₁ : BddBelow I) (h₂ : BddAbove I) :
    I = Icc (sInf I) (sSup I) ↔ forallᵉ (x in I) (y in I), Disjoint (Ioo x y) I -> y <= x + 1 := by
  simp [← h₀.ordConnected_iff_of_bdd h₁ h₂, ordConnected_iff_disjoint_Ioo_empty]
