/-
Copyright (c) 2024 Markus Himmel. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Markus Himmel, Christian Merten
-/
module

public import Mathlib.CategoryTheory.Limits.FunctorCategory.Filtered
public import Mathlib.CategoryTheory.Limits.FunctorCategory.Shapes.Products
public import Mathlib.CategoryTheory.Limits.Types.Filtered
public import Mathlib.CategoryTheory.Limits.Types.Products

/-!
# The IPC property

Given a family of categories `I i` (`i : α`) and a family of functors `F i : I i ⥤ C`, we consider
the diagram `pointwiseProduct F : Π i, I i ⥤ C` defined by `(Xᵢ)ᵢ ↦ ∏ᶜ Xᵢ`. Given a cocone
`cᵢ` on `Fᵢ` for each `i`, there is a natural cocone on `pointwiseProduct F` with point `∏ᶜ cᵢ`.

Similarly to the study of finite limits commuting with filtered colimits, we then study sufficient
conditions for this cocone to be colimiting if all the `cᵢ` are colimiting. We say that `C`
satisfies the `w`-IPC property if the morphism is an isomorphism as long as `α` is `w`-small and
`I i` is `w`-small and filtered for all `i`.

## Main definitions

- `CategoryTheory.Limits.IsIPCOfShape`: `C` satisfies `w`-IPC of shape `α` if `w`-sized filtered
  colimits commute products of shape `α`, i.e. if the joint cocone from above is colimiting if the
  components are.
- `CategoryTheory.Limits.IsIPC`: `C` satisfies the `w`-IPC property if it satisfies `w`-IPC for
  every `α : Type w`.

## Main results

- The category `Type u` satisfies the `u`-IPC property (available by `inferInstance`).
- If `C` satisfies the `w`-IPC property, then `D ⥤ C` satisfies the `w`-IPC property
  (available by `inferInstance`).

These results will be used to show that if a category `C` has products indexed by `α`, then so
does the category of Ind-objects of `C`.

## References
* [M. Kashiwara, P. Schapira, *Categories and Sheaves*][Kashiwara2006], 3.1.10, 3.1.11, 3.1.12.
-/

@[expose] public section

universe w v v₁ v₂ u u₁ u₂

namespace CategoryTheory.Limits

open ConcreteCategory

section

variable {C : Type u} [Category.{v} C] {α : Type*} {I : α -> Type*} [forall i, Category* (I i)]
  [HasProductsOfShape α C] (F : forall i, I i ⥤ C)

/--
Definition of `pointwiseProduct` / `pointwiseProduct` 的定义

English:
abbreviation pointwiseProduct
  signature: : (forall i, I i) ⥤ C
  body: Functor.pi F ⋙ Pi.functor α

中文:
缩写 pointwiseProduct
  签名: : (对任意 i, I i) ⥤ C
  定义体: Functor.pi F ⋙ Pi.functor α

Depends on / 依赖: Functor, Functor.pi, Pi.functor, functor
-/
noncomputable abbrev pointwiseProduct : (forall i, I i) ⥤ C :=
  Functor.pi F ⋙ Pi.functor α

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
attribute [local simp] Functor.pi in
/-- `pointwiseProduct` is invariant under re-indexing. -/
@[simps!]
noncomputable
/--
Definition of `Pi.equivalenceOfEquivCompPointwiseProduct` / `Pi.equivalenceOfEquivCompPointwiseProduct` 的定义

English:
definition Pi.equivalenceOfEquivCompPointwiseProduct
  signature: {β : Type*} (f : β ≃ α) [HasProductsOfShape β C]
  body: (NatIso.ofComponents
    (fun a => (Pi.whiskerEquiv f (fun j => (Iso.refl ((F (f j)).obj <| a (f j))))).symm)).symm

中文:
定义 Pi.equivalenceOfEquivCompPointwiseProduct
  签名: {β : 类型} (f : β ≃ α) [HasProductsOfShape β C]
  定义体: (NatIso.ofComponents
    (fun a => (Pi.whiskerEquiv f (fun j => (Iso.refl ((F (f j)).obj <| a (f j))))).symm)).symm

Depends on / 依赖: Iso.refl, NatIso, NatIso.ofComponents, Pi.whiskerEquiv, ofComponents, whiskerEquiv
-/
def Pi.equivalenceOfEquivCompPointwiseProduct {β : Type*} (f : β ≃ α) [HasProductsOfShape β C] :
    (Pi.equivalenceOfEquiv I f).inverse ⋙ pointwiseProduct (fun i => F (f i)) ≅
      pointwiseProduct F :=
  (NatIso.ofComponents
    (fun a => (Pi.whiskerEquiv f (fun j => (Iso.refl ((F (f j)).obj <| a (f j))))).symm)).symm

set_option backward.defeqAttrib.useBackward true in
variable {F} in
/-- The inclusions `(F s).obj (k s) ⟶ colimit (F s)` induce a cocone on `pointwiseProduct F` with
cone point `∏ᶜ (fun s : α) => colimit (F s)`. -/
@[simps]
/--
Definition of `coconePointwiseProduct` / `coconePointwiseProduct` 的定义

English:
definition coconePointwiseProduct
  signature: (c : forall i, Cocone (F i))
  body: ∏ᶜ fun i => (c i).pt
  ι := Functor.whiskerRight (NatTrans.pi fun i => (c i).ι) _ ≫ (Pi.constCompPiIsoConst _).hom

中文:
定义 coconePointwiseProduct
  签名: (c : 对任意 i, Cocone (F i))
  定义体: ∏ᶜ fun i => (c i).pt
  ι := Functor.whiskerRight (NatTrans.pi fun i => (c i).ι) _ ≫ (Pi.constCompPiIsoConst _).hom
-/
noncomputable def coconePointwiseProduct (c : forall i, Cocone (F i)) :
    Cocone (pointwiseProduct F) where
  pt := ∏ᶜ fun i => (c i).pt
  ι := Functor.whiskerRight (NatTrans.pi fun i => (c i).ι) _ ≫ (Pi.constCompPiIsoConst _).hom

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `coconePointwiseProductIso` / `coconePointwiseProductIso` 的定义

English:
definition coconePointwiseProductIso
  signature: {c c' : forall i, Cocone (F i)} (e : forall i, c i ≅ c' i)
  body: Cocone.ext (Pi.mapIso fun i => (Cocone.forget _).mapIso (e i)) fun i => by
    dsimp
    ext
    simp [Functor.pi]

中文:
定义 coconePointwiseProductIso
  签名: {c c' : 对任意 i, Cocone (F i)} (e : 对任意 i, c i ≅ c' i)
  定义体: Cocone.ext (Pi.mapIso fun i => (Cocone.forget _).mapIso (e i)) fun i => by
    dsimp
    ext
    simp [Functor.pi]

Depends on / 依赖: Cocone, Cocone.ext, Cocone.forget, Functor, Functor.pi, Pi.mapIso, forget, mapIso
-/
noncomputable def coconePointwiseProductIso {c c' : forall i, Cocone (F i)} (e : forall i, c i ≅ c' i) :
    coconePointwiseProduct c ≅ coconePointwiseProduct c' :=
  Cocone.ext (Pi.mapIso fun i => (Cocone.forget _).mapIso (e i)) fun i => by
    dsimp
    ext
    simp [Functor.pi]

/--
Definition of `colimitPointwiseProductToProductColimit` / `colimitPointwiseProductToProductColimit` 的定义

English:
definition colimitPointwiseProductToProductColimit
  signature: [forall i, HasColimit (F i)]
  body: colimit.desc _ (coconePointwiseProduct _)

中文:
定义 colimitPointwiseProductToProductColimit
  签名: [对任意 i, HasColimit (F i)]
  定义体: colimit.desc _ (coconePointwiseProduct _)

Depends on / 依赖: coconePointwiseProduct, colimit, colimit.desc
-/
noncomputable def colimitPointwiseProductToProductColimit [forall i, HasColimit (F i)]
    [HasColimit (Functor.pi F ⋙ Pi.functor α)] :
    colimit (pointwiseProduct F) ⟶ ∏ᶜ fun (s : α) => colimit (F s) :=
  colimit.desc _ (coconePointwiseProduct _)

variable [forall i, HasColimit (F i)] [HasColimit (pointwiseProduct F)]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
@[reassoc (attr := simp)]
/--
theorem `ι_colimitPointwiseProductToProductColimit_π` / 定理 `ι_colimitPointwiseProductToProductColimit_π`

English:
theorem ι_colimitPointwiseProductToProductColimit_π
  given: (k : forall i, I i) (s : α)
  proof: by
  simp [colimitPointwiseProductToProductColimit, Functor.pi, Pi.functor]

中文:
定理 ι_colimitPointwiseProductToProductColimit_π
  条件: (k : 对任意 i, I i) (s : α)
  证明: by
  simp [colimitPointwiseProductToProductColimit, Functor.pi, Pi.functor]

Depends on / 依赖: Functor, Functor.pi, Pi.functor, colimitPointwiseProductToProductColimit, functor
-/
theorem ι_colimitPointwiseProductToProductColimit_π (k : forall i, I i) (s : α) :
    colimit.ι (pointwiseProduct F) k ≫
      colimitPointwiseProductToProductColimit F ≫ Pi.π _ s =
      Pi.π _ s ≫ colimit.ι (F s) (k s) := by
  simp [colimitPointwiseProductToProductColimit, Functor.pi, Pi.functor]

end

section functorCategory

variable {C : Type*} [Category* C] {D : Type*} [Category* D]
  {α : Type*} {I : α -> Type*} [forall i, Category (I i)]
  [HasLimitsOfShape (Discrete α) C]
  (F : forall i, I i ⥤ D ⥤ C)

set_option backward.defeqAttrib.useBackward true in
/-- Evaluating the pointwise product `k ↦ ∏ᶜ fun (s : α) => (F s).obj (k s)` at `d` is the same as
taking the pointwise product `k ↦ ∏ᶜ fun (s : α) => ((F s).obj (k s)).obj d`. -/
@[simps!]
/--
Definition of `pointwiseProductCompEvaluation` / `pointwiseProductCompEvaluation` 的定义

English:
definition pointwiseProductCompEvaluation
  signature: (d : D)
  body: NatIso.ofComponents (fun k => piObjIso _ _)
    (fun f => Pi.hom_ext _ _ (by simp [Functor.pi, ← NatTrans.comp_app]))

中文:
定义 pointwiseProductCompEvaluation
  签名: (d : D)
  定义体: NatIso.ofComponents (fun k => piObjIso _ _)
    (fun f => Pi.hom_ext _ _ (by simp [Functor.pi, ← NatTrans.comp_app]))

Depends on / 依赖: Functor, Functor.pi, NatIso, NatIso.ofComponents, NatTrans, NatTrans.comp_app, Pi.hom_ext, comp_app, hom_ext, ofComponents, piObjIso
-/
noncomputable def pointwiseProductCompEvaluation (d : D) :
    pointwiseProduct F ⋙ (evaluation D C).obj d ≅
      pointwiseProduct (fun s => F s ⋙ (evaluation _ _).obj d) :=
  NatIso.ofComponents (fun k => piObjIso _ _)
    (fun f => Pi.hom_ext _ _ (by simp [Functor.pi, ← NatTrans.comp_app]))

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `evaluationCoconePointwiseProductIso` / `evaluationCoconePointwiseProductIso` 的定义

English:
definition evaluationCoconePointwiseProductIso
  signature: (X : D) (c : forall i, Cocone (F i))
  body: Cocone.ext (piObjIso (fun i => (c i).pt) X) fun j => by
    dsimp
    ext
    simp [Functor.pi, NatTrans.pi, ← NatTrans.comp_app]

中文:
定义 evaluationCoconePointwiseProductIso
  签名: (X : D) (c : 对任意 i, Cocone (F i))
  定义体: Cocone.ext (piObjIso (fun i => (c i).pt) X) fun j => by
    dsimp
    ext
    simp [Functor.pi, NatTrans.pi, ← NatTrans.comp_app]

Depends on / 依赖: Cocone, Cocone.ext, Functor, Functor.pi, NatTrans, NatTrans.comp_app, NatTrans.pi, comp_app, piObjIso
-/
noncomputable def evaluationCoconePointwiseProductIso (X : D) (c : forall i, Cocone (F i)) :
    ((evaluation D C).obj X).mapCocone (coconePointwiseProduct c) ≅
      (Cocone.precompose <| (pointwiseProductCompEvaluation F X).hom).obj
        (coconePointwiseProduct fun i => ((evaluation D C).obj X).mapCocone (c i)) :=
  Cocone.ext (piObjIso (fun i => (c i).pt) X) fun j => by
    dsimp
    ext
    simp [Functor.pi, NatTrans.pi, ← NatTrans.comp_app]

variable [forall i, HasColimitsOfShape (I i) C] [HasColimitsOfShape (forall i, I i) C]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `colimitPointwiseProductToProductColimit_app` / 定理 `colimitPointwiseProductToProductColimit_app`

English:
theorem colimitPointwiseProductToProductColimit_app
  given: (d : D)
  proof: by
  rw [← Iso.inv_comp_eq]
  simp only [← Category.assoc]
  rw [Iso.eq_comp_inv]
  refine Pi.hom_ext _ _ (fun s => colimit.hom_ext (fun k => ?_))
  simp [← NatTrans.comp_app, Functor.pi]

中文:
定理 colimitPointwiseProductToProductColimit_app
  条件: (d : D)
  证明: by
  rw [← Iso.inv_comp_eq]
  simp only [← Category.assoc]
  rw [Iso.eq_comp_inv]
  refine Pi.hom_ext _ _ (fun s => colimit.hom_ext (fun k => ?_))
  simp [← NatTrans.comp_app, Functor.pi]

Depends on / 依赖: Category, Category.assoc, Functor, Functor.pi, Iso.eq_comp_inv, Iso.inv_comp_eq, NatTrans, NatTrans.comp_app, Pi.hom_ext, colimit, colimit.hom_ext, comp_app, eq_comp_inv, hom_ext, inv_comp_eq
-/
theorem colimitPointwiseProductToProductColimit_app (d : D) :
    (colimitPointwiseProductToProductColimit F).app d =
      (colimitObjIsoColimitCompEvaluation _ _).hom ≫
        (HasColimit.isoOfNatIso (pointwiseProductCompEvaluation F d)).hom ≫
          colimitPointwiseProductToProductColimit _ ≫
            (Pi.mapIso fun _ => (colimitObjIsoColimitCompEvaluation _ _).symm).hom ≫
              (piObjIso _ _).inv := by
  rw [← Iso.inv_comp_eq]
  simp only [← Category.assoc]
  rw [Iso.eq_comp_inv]
  refine Pi.hom_ext _ _ (fun s => colimit.hom_ext (fun k => ?_))
  simp [← NatTrans.comp_app, Functor.pi]

end functorCategory

section

variable {C : Type*} [Category* C]
variable {ι : Type*} [HasProductsOfShape ι C]

/-- A category `C` has the `w`-IPC property for shape `ι` if `w`-sized filtered colimits commute
with products of shape `ι`. -/
@[pp_with_univ]
/--
Definition of `IsIPCOfShape` / `IsIPCOfShape` 的定义

English:
class IsIPCOfShape
  parameters: (ι : Type*) (C : Type*) [Category* C] [HasProductsOfShape ι C]
  axioms and operations (1):
    - nonempty_isColimit(⦃J) : ι -> Type w⦄ [forall i, SmallCategory (J i)] [forall i, IsFiltered (J i)] ⦃F : forall i, J i ⥤ C⦄ ⦃c : forall i, Cocone (F i)⦄ : (forall i, IsColimit (c i)) -> Nonempty (IsColimit (coconePointwiseProduct c))

中文:
类 IsIPCOfShape
  参数: (ι : 类型) (C : 类型) [Category* C] [HasProductsOfShape ι C]
  公理与运算 (1 个):
    - nonempty_isColimit(⦃J) : ι -> Type w⦄ [对任意 i, SmallCategory (J i)] [对任意 i, IsFiltered (J i)] ⦃F : 对任意 i, J i ⥤ C⦄ ⦃c : 对任意 i, Cocone (F i)⦄ : (对任意 i, IsColimit (c i)) -> Nonempty (IsColimit (coconePointwiseProduct c))
-/
class IsIPCOfShape (ι : Type*) (C : Type*) [Category* C] [HasProductsOfShape ι C] : Prop where
  nonempty_isColimit ⦃J : ι -> Type w⦄ [forall i, SmallCategory (J i)]
    [forall i, IsFiltered (J i)] ⦃F : forall i, J i ⥤ C⦄ ⦃c : forall i, Cocone (F i)⦄ :
    (forall i, IsColimit (c i)) -> Nonempty (IsColimit (coconePointwiseProduct c))

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [IsIPCOfShape.{w}
  signature: ι C] {J
  body: by
  rw [colimitPointwiseProductToProductColimit]; rw [colimit.desc]
  refine ((colimit.isColimit (pointwiseProduct F)).nonempty_isColimit_iff_isIso_desc).mp ?_
  exact IsIPCOfShape.nonempty_isColimit fun i => colimit.isColimit _

中文:
实例 [IsIPCOfShape.{w}
  签名: ι C] {J
  定义体: by
  rw [colimitPointwiseProductToProductColimit]; rw [colimit.desc]
  refine ((colimit.isColimit (pointwiseProduct F)).nonempty_isColimit_iff_isIso_desc).mp ?_
  exact IsIPCOfShape.nonempty_isColimit fun i => colimit.isColimit _

Depends on / 依赖: IsIPCOfShape, IsIPCOfShape.nonempty_isColimit, colimit, colimit.desc, colimit.isColimit, colimitPointwiseProductToProductColimit, isColimit, nonempty_isColimit, nonempty_isColimit_iff_isIso_desc, pointwiseProduct
-/
instance [IsIPCOfShape.{w} ι C] {J : ι -> Type w} [forall i, SmallCategory (J i)]
    [forall i, IsFiltered (J i)] (F : forall i, J i ⥤ C)
    [forall i, HasColimit (F i)] [HasColimit (pointwiseProduct F)] :
    IsIso (colimitPointwiseProductToProductColimit F) := by
  rw [colimitPointwiseProductToProductColimit]; rw [colimit.desc]
  refine ((colimit.isColimit (pointwiseProduct F)).nonempty_isColimit_iff_isIso_desc).mp ?_
  exact IsIPCOfShape.nonempty_isColimit fun i => colimit.isColimit _

/--
lemma `IsIPCOfShape.of_forall_exists` / 引理 `IsIPCOfShape.of_forall_exists`

English:
lemma IsIPCOfShape.of_forall_exists
  proof: by
    have (i : ι) : HasColimit (F i) := ⟨_, hc i⟩
    obtain ⟨c', hc', _⟩ := H F
    let e : coconePointwiseProduct c ≅ coconePointwiseProduct c' :=
      coconePointwiseProductIso _ fun i => (hc i).uniqueUpToIso (hc' i)
    rwa [(IsColimit.equivIsoColimit e).nonempty_congr]

中文:
引理 IsIPCOfShape.of_forall_exists
  证明: by
    have (i : ι) : HasColimit (F i) := ⟨_, hc i⟩
    obtain ⟨c', hc', _⟩ := H F
    let e : coconePointwiseProduct c ≅ coconePointwiseProduct c' :=
      coconePointwiseProductIso _ fun i => (hc i).uniqueUpToIso (hc' i)
    rwa [(IsColimit.equivIsoColimit e).nonempty_congr]

Depends on / 依赖: HasColimit, IsColimit, IsColimit.equivIsoColimit, coconePointwiseProduct, coconePointwiseProductIso, equivIsoColimit, nonempty_congr, uniqueUpToIso
-/
lemma IsIPCOfShape.of_forall_exists
    (H : forall ⦃J : ι -> Type w⦄ [forall i, SmallCategory (J i)]
      [forall i, IsFiltered (J i)] (F : forall i, J i ⥤ C) [forall i, HasColimit (F i)], exists (c : forall i, Cocone (F i))
      (_ : forall i, IsColimit (c i)), Nonempty (IsColimit (coconePointwiseProduct c))) :
    IsIPCOfShape.{w} ι C where
  nonempty_isColimit J _ _ F c hc := by
    have (i : ι) : HasColimit (F i) := ⟨_, hc i⟩
    obtain ⟨c', hc', _⟩ := H F
    let e : coconePointwiseProduct c ≅ coconePointwiseProduct c' :=
      coconePointwiseProductIso _ fun i => (hc i).uniqueUpToIso (hc' i)
    rwa [(IsColimit.equivIsoColimit e).nonempty_congr]

/--
lemma `IsIPCOfShape.of_isIso` / 引理 `IsIPCOfShape.of_isIso`

English:
lemma IsIPCOfShape.of_isIso
  proof: by
  refine .of_forall_exists fun J _ _ F _ => ?_
  refine ⟨fun i => colimit.cocone _, fun i => colimit.isColimit _, ?_⟩
  obtain ⟨_, h⟩ := H J F
  rwa [IsColimit.nonempty_isColimit_iff_isIso_desc (colimit.isColimit _)]

中文:
引理 IsIPCOfShape.of_isIso
  证明: by
  refine .of_forall_exists fun J _ _ F _ => ?_
  refine ⟨fun i => colimit.cocone _, fun i => colimit.isColimit _, ?_⟩
  obtain ⟨_, h⟩ := H J F
  rwa [IsColimit.nonempty_isColimit_iff_isIso_desc (colimit.isColimit _)]

Depends on / 依赖: IsColimit, IsColimit.nonempty_isColimit_iff_isIso_desc, cocone, colimit, colimit.cocone, colimit.isColimit, isColimit, nonempty_isColimit_iff_isIso_desc, of_forall_exists
-/
lemma IsIPCOfShape.of_isIso
    (H : forall (J : ι -> Type w) [forall i, SmallCategory (J i)] [forall i, IsFiltered (J i)]
      (F : forall i, J i ⥤ C) [forall (i : ι), HasColimit (F i)],
      exists (_ : HasColimit (pointwiseProduct F)),
        IsIso (colimitPointwiseProductToProductColimit F)) :
    IsIPCOfShape.{w} ι C := by
  refine .of_forall_exists fun J _ _ F _ => ?_
  refine ⟨fun i => colimit.cocone _, fun i => colimit.isColimit _, ?_⟩
  obtain ⟨_, h⟩ := H J F
  rwa [IsColimit.nonempty_isColimit_iff_isIso_desc (colimit.isColimit _)]

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
attribute [local simp] Functor.pi in
/--
lemma `IsIPCOfShape.of_equiv` / 引理 `IsIPCOfShape.of_equiv`

English:
lemma IsIPCOfShape.of_equiv
  statement: {ι' : Type*} [HasProductsOfShape ι' C] [IsIPCOfShape.{w} ι C]
  proof: by
    obtain ⟨h⟩ := nonempty_isColimit fun i : ι => hc (e i)
    constructor
apply IsColimit.equivOfNatIsoOfIso _ _ _ _
        h.whiskerEquivalence (Pi.equivalenceOfEquiv J e).symm
    · exact (Pi.equivalenceOfEquivCompPointwiseProduct F e)
    · -- Without the double `symm`, one runs into DTT hel

中文:
引理 IsIPCOfShape.of_equiv
  结论: {ι' : 类型} [HasProductsOfShape ι' C] [IsIPCOfShape.{w} ι C]
  证明: by
    obtain ⟨h⟩ := nonempty_isColimit fun i : ι => hc (e i)
    constructor
apply IsColimit.equivOfNatIsoOfIso _ _ _ _
        h.whiskerEquivalence (Pi.equivalenceOfEquiv J e).symm
    · exact (Pi.equivalenceOfEquivCompPointwiseProduct F e)
    · -- Without the double `symm`, one runs into DTT hel

Depends on / 依赖: Cocone, Cocone.ext, IsColimit, IsColimit.equivOfNatIsoOfIso, Iso.refl, Pi.equivalenceOfEquiv, Pi.equivalenceOfEquivCompPointwiseProduct, Pi.whiskerEquiv, Without, double, equivOfNatIsoOfIso, equivalenceOfEquiv, equivalenceOfEquivCompPointwiseProduct, h.whiskerEquivalence, inl_inr, nonempty_isColimit, whiskerEquiv, whiskerEquivalence
-/
lemma IsIPCOfShape.of_equiv {ι' : Type*} [HasProductsOfShape ι' C] [IsIPCOfShape.{w} ι C]
    (e : ι ≃ ι') :
    IsIPCOfShape.{w} ι' C where
  nonempty_isColimit J _ _ F c hc := by
    obtain ⟨h⟩ := nonempty_isColimit fun i : ι => hc (e i)
    constructor
apply IsColimit.equivOfNatIsoOfIso _ _ _ _
        h.whiskerEquivalence (Pi.equivalenceOfEquiv J e).symm
    · exact (Pi.equivalenceOfEquivCompPointwiseProduct F e)
    · -- Without the double `symm`, one runs into DTT hell
      exact (Cocone.ext (Pi.whiskerEquiv e fun _ => Iso.refl _).symm).symm

variable (C) in
/--
Definition of `IsIPC` / `IsIPC` 的定义

English:
class IsIPC
  parameters: [HasProducts.{w} C] [HasFilteredColimitsOfSize.{w} C]
  axioms and operations (1):
    - isIPCOfShape((ι : Type w)) : IsIPCOfShape.{w} ι C  [default: by infer_instance]

中文:
类 IsIPC
  参数: [HasProducts.{w} C] [HasFilteredColimitsOfSize.{w} C]
  公理与运算 (1 个):
    - isIPCOfShape((ι : Type w)) : IsIPCOfShape.{w} ι C  [默认: by infer_instance]

Depends on / 依赖: infer_instance
-/
class IsIPC [HasProducts.{w} C] [HasFilteredColimitsOfSize.{w} C] : Prop where
  isIPCOfShape (ι : Type w) : IsIPCOfShape.{w} ι C := by infer_instance

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [HasProducts.{w}
  signature: C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] (ι
  body: by
  suffices IsIPCOfShape (Shrink.{w} ι) C from .of_equiv (equivShrink ι).symm
  apply IsIPC.isIPCOfShape

中文:
实例 [HasProducts.{w}
  签名: C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] (ι
  定义体: by
  suffices IsIPCOfShape (Shrink.{w} ι) C from .of_equiv (equivShrink ι).symm
  apply IsIPC.isIPCOfShape

Depends on / 依赖: IsIPC.isIPCOfShape, IsIPCOfShape, Shrink, equivShrink, isIPCOfShape, of_equiv
-/
instance [HasProducts.{w} C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] (ι : Type*)
    [Small.{w} ι] [HasProductsOfShape ι C] :
    IsIPCOfShape.{w} ι C := by
  suffices IsIPCOfShape (Shrink.{w} ι) C from .of_equiv (equivShrink ι).symm
  apply IsIPC.isIPCOfShape

end

section types

variable {α : Type u} {I : α -> Type u} [forall i, SmallCategory (I i)] [forall i, IsFiltered (I i)]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `Types.isIso_colimitPointwiseProductToProductColimit` / 定理 `Types.isIso_colimitPointwiseProductToProductColimit`

English:
theorem Types.isIso_colimitPointwiseProductToProductColimit
  given: (F : forall i, I i ⥤ Type u)
  proof: by
  -- We follow the proof in [Kashiwara2006], Prop. 3.1.11(ii)
  refine (isIso_iff_bijective _).2 ⟨fun y y' hy => ?_, fun x => ?_⟩
  · obtain ⟨ky, yk₀, hyk₀⟩ := Types.jointly_surjective' y
    obtain ⟨ky', yk₀', hyk₀'⟩ := Types.jointly_surjective' y'
    let k := IsFiltered.max ky ky'
    let yk :

中文:
定理 Types.isIso_colimitPointwiseProductToProductColimit
  条件: (F : 对任意 i, I i ⥤ 类型u)
  证明: by
  -- We follow the proof in [Kashiwara2006], Prop. 3.1.11(ii)
  refine (isIso_iff_bijective _).2 ⟨fun y y' hy => ?_, fun x => ?_⟩
  · obtain ⟨ky, yk₀, hyk₀⟩ := Types.jointly_surjective' y
    obtain ⟨ky', yk₀', hyk₀'⟩ := Types.jointly_surjective' y'
    let k := IsFiltered.max ky ky'
    let yk :
-/
theorem Types.isIso_colimitPointwiseProductToProductColimit (F : forall i, I i ⥤ Type u) :
    IsIso (colimitPointwiseProductToProductColimit F) := by
  -- We follow the proof in [Kashiwara2006], Prop. 3.1.11(ii)
  refine (isIso_iff_bijective _).2 ⟨fun y y' hy => ?_, fun x => ?_⟩
  · obtain ⟨ky, yk₀, hyk₀⟩ := Types.jointly_surjective' y
    obtain ⟨ky', yk₀', hyk₀'⟩ := Types.jointly_surjective' y'
    let k := IsFiltered.max ky ky'
    let yk : (pointwiseProduct F).obj k :=
      (pointwiseProduct F).map (IsFiltered.leftToMax ky ky') yk₀
    let yk' : (pointwiseProduct F).obj k :=
      (pointwiseProduct F).map (IsFiltered.rightToMax ky ky') yk₀'
    obtain rfl : y = colimit.ι (pointwiseProduct F) k yk := by
      simp only [k, yk, colimit.w_apply, hyk₀]
    obtain rfl : y' = colimit.ι (pointwiseProduct F) k yk' := by
      simp only [k, yk', colimit.w_apply, hyk₀']
    dsimp at yk yk'
    have hch : forall (s : α), exists (i' : I s) (hi' : k s ⟶ i'),
        (F s).map hi' (Pi.π (fun s => (F s).obj (k s)) s yk) =
          (F s).map hi' (Pi.π (fun s => (F s).obj (k s)) s yk') := by
      intro s
      have hy₁ := congr_hom (ι_colimitPointwiseProductToProductColimit_π F k s) yk
      have hy₂ := congr_hom (ι_colimitPointwiseProductToProductColimit_π F k s) yk'
      dsimp at hy₁ hy₂ hy
      rw [← hy]; rw [hy₁]; rw [Types.FilteredColimit.colimit_eq_iff] at hy₂
      obtain ⟨i₀, f₀, g₀, h₀⟩ := hy₂
      refine ⟨IsFiltered.coeq f₀ g₀, f₀ ≫ IsFiltered.coeqHom f₀ g₀, ?_⟩
      conv_rhs => rw [IsFiltered.coeq_condition]
      dsimp [Functor.pi] at h₀
      simp [h₀]
    choose k' f hk' using hch
    apply Types.colimit_sound' f f
    exact Types.limit_ext' _ _ _ (fun ⟨s⟩ => by simpa [Functor.pi, Pi.map_π_apply] using hk' s)
  · have hch : forall (s : α), exists (i : I s) (xi : (F s).obj i), colimit.ι (F s) i xi =
        Pi.π (fun s => colimit (F s)) s x := fun s => Types.jointly_surjective' _
    choose k p hk using hch
    refine ⟨colimit.ι (pointwiseProduct F) k ((Types.productIso _).inv p), ?_⟩
    refine Types.limit_ext' _ _ _ (fun ⟨s⟩ => ?_)
    have := congr_hom (ι_colimitPointwiseProductToProductColimit_π F k s)
      ((Types.productIso _).inv p)
    exact this.trans (by simpa [Functor.pi] using hk _)

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: IsIPC.{u} (Type u)
  body: .of_isIso fun _ _ _ _ _ => ⟨inferInstance, Types.isIso_colimitPointwiseProductToProductColimit _⟩

中文:
实例 :
  签名: IsIPC.{u} (类型u)
  定义体: .of_isIso fun _ _ _ _ _ => ⟨inferInstance, Types.isIso_colimitPointwiseProductToProductColimit _⟩

Depends on / 依赖: Types.isIso_colimitPointwiseProductToProductColimit, isIso_colimitPointwiseProductToProductColimit, of_isIso
-/
instance : IsIPC.{u} (Type u) where
  isIPCOfShape _ :=
    .of_isIso fun _ _ _ _ _ => ⟨inferInstance, Types.isIso_colimitPointwiseProductToProductColimit _⟩

end types

section functorCategory

variable {C : Type u} [Category.{v} C]

instance (ι : Type*) [HasProductsOfShape ι C] [IsIPCOfShape.{w} ι C]
    [HasFilteredColimitsOfSize.{w, w} C] {D : Type u₁}
    [Category.{v₁} D] : IsIPCOfShape.{w} ι (D ⥤ C) where
  nonempty_isColimit J _ _ F c hc := by
    refine ⟨evaluationJointlyReflectsColimits _ fun X => ?_⟩
    exact IsColimit.equivOfNatIsoOfIso (pointwiseProductCompEvaluation F X).symm _ _
      (evaluationCoconePointwiseProductIso F X c).symm
      (IsIPCOfShape.nonempty_isColimit fun i => isColimitOfPreserves _ (hc i)).some

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [HasProducts.{w}
  signature: C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] {D

中文:
实例 [HasProducts.{w}
  签名: C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] {D
-/
instance [HasProducts.{w} C] [HasFilteredColimitsOfSize.{w, w} C] [IsIPC.{w} C] {D : Type u₁}
    [Category.{v₁} D] : IsIPC.{w} (D ⥤ C) where

end functorCategory

end CategoryTheory.Limits
