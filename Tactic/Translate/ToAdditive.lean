...
English:
theorem mul_comm'
  given: {α} [CommSemigroup α] (x y : α)
  statement: x * y = y * x
  proof: CommSemigroup.mul_comm
```
...
Depends on / 依赖: ObjectProperty, ObjectProperty.homMk, StructuredArrow, StructuredArrow.mk, V.homToIndex, V.index, homToIndex, single
-/
