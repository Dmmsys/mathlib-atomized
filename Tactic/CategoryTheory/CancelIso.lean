...
@[to_dual none]
/--
lemma `hom_inv_id_of_eq` / 引理 `hom_inv_id_of_eq`

English:
lemma hom_inv_id_of_eq
  statement: {C : Type*} [Category* C] {x y : C}
  proof: by
  rw [← h]
...
Depends on / 依赖: IsIso.hom_inv_id, hom_inv_id
-/
...
@[to_dual none]
/--
lemma `hom_inv_id_of_eq_assoc` / 引理 `hom_inv_id_of_eq_assoc`

English:
lemma hom_inv_id_of_eq_assoc
  statement: {C : Type*} [Category* C] {x y : C}
  proof: by
  rw [← h]
...
Depends on / 依赖: IsIso.hom_inv_id_assoc, hom_inv_id_assoc
-/
...
/--
Definition of `tryCancelPair` / `tryCancelPair` 的定义

English:
definition tryCancelPair
  signature: (C x y z f g : Expr)
  body: do
...
/--
Definition of `cancelIsoSimproc` / `cancelIsoSimproc` 的定义

English:
definition cancelIsoSimproc
