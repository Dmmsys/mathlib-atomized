/-
Copyright (c) 2025 Joseph Myers. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Joseph Myers
-/
module

public import Mathlib.Analysis.InnerProductSpace.Projection.FiniteDimensional
public import Mathlib.Geometry.Euclidean.Projection
public import Mathlib.Geometry.Euclidean.Sphere.Basic

/-!
# Spaces orthogonal to the radius vector in spheres.

This file defines the affine subspace orthogonal to the radius vector at a point.

## Main definitions

* `EuclideanGeometry.Sphere.orthRadius`: the affine subspace orthogonal to the radius vector at
  a point (the tangent space, if that point lies in the sphere; more generally, the polar of the
  inversion of that point in the sphere).

-/

@[expose] public section


namespace EuclideanGeometry

namespace Sphere

open AffineSubspace Function RealInnerProductSpace
open scoped Affine

variable {V P : Type*}
variable [NormedAddCommGroup V] [InnerProductSpace Real V] [MetricSpace P] [NormedAddTorsor V P]

/--
Definition of `orthRadius` / `orthRadius` 的定义

English:
definition orthRadius
  signature: (s : Sphere P) (p : P)
  body: .mk' p (Real ∙ (p -ᵥ s.center))ᗮ

中文:
定义 orthRadius
  签名: (s : Sphere P) (p : P)
  定义体: .mk' p (Real ∙ (p -ᵥ s.center))ᗮ

Depends on / 依赖: center, s.center
-/
noncomputable def orthRadius (s : Sphere P) (p : P) : AffineSubspace Real P :=
  .mk' p (Real ∙ (p -ᵥ s.center))ᗮ

instance (s : Sphere P) (p : P) : Nonempty (s.orthRadius p) := by
  rw [orthRadius]
  infer_instance

/--
lemma `self_mem_orthRadius` / 引理 `self_mem_orthRadius`

English:
lemma self_mem_orthRadius
  given: (s : Sphere P) (p : P)
  statement: p in s.orthRadius p
  proof: self_mem_mk' _ _

中文:
引理 self_mem_orthRadius
  条件: (s : Sphere P) (p : P)
  结论: p in s.orthRadius p
  证明: self_mem_mk' _ _
-/
@[simp] lemma self_mem_orthRadius (s : Sphere P) (p : P) : p in s.orthRadius p :=
  self_mem_mk' _ _

/--
lemma `mem_orthRadius_iff_inner_left` / 引理 `mem_orthRadius_iff_inner_left`

English:
lemma mem_orthRadius_iff_inner_left
  given: {s : Sphere P} {p x : P}
  proof: by
  rw [orthRadius]; rw [mem_mk']; rw [Submodule.mem_orthogonal_singleton_iff_inner_left]

中文:
引理 mem_orthRadius_iff_inner_left
  条件: {s : Sphere P} {p x : P}
  证明: by
  rw [orthRadius]; rw [mem_mk']; rw [Submodule.mem_orthogonal_singleton_iff_inner_left]

Depends on / 依赖: Submodule, Submodule.mem_orthogonal_singleton_iff_inner_left, mem_mk, mem_orthogonal_singleton_iff_inner_left, orthRadius
-/
lemma mem_orthRadius_iff_inner_left {s : Sphere P} {p x : P} :
    x in s.orthRadius p ↔ ⟪x -ᵥ p, p -ᵥ s.center⟫ = 0 := by
  rw [orthRadius]; rw [mem_mk']; rw [Submodule.mem_orthogonal_singleton_iff_inner_left]

/--
lemma `mem_orthRadius_iff_inner_right` / 引理 `mem_orthRadius_iff_inner_right`

English:
lemma mem_orthRadius_iff_inner_right
  given: {s : Sphere P} {p x : P}
  proof: by
  rw [mem_orthRadius_iff_inner_left]; rw [inner_eq_zero_symm]

中文:
引理 mem_orthRadius_iff_inner_right
  条件: {s : Sphere P} {p x : P}
  证明: by
  rw [mem_orthRadius_iff_inner_left]; rw [inner_eq_zero_symm]

Depends on / 依赖: inner_eq_zero_symm, mem_orthRadius_iff_inner_left
-/
lemma mem_orthRadius_iff_inner_right {s : Sphere P} {p x : P} :
    x in s.orthRadius p ↔ ⟪p -ᵥ s.center, x -ᵥ p⟫ = 0 := by
  rw [mem_orthRadius_iff_inner_left]; rw [inner_eq_zero_symm]

/--
lemma `direction_orthRadius` / 引理 `direction_orthRadius`

English:
lemma direction_orthRadius
  given: (s : Sphere P) (p : P)
  proof: by
  rw [orthRadius]; rw [direction_mk']

中文:
引理 direction_orthRadius
  条件: (s : Sphere P) (p : P)
  证明: by
  rw [orthRadius]; rw [direction_mk']
-/
@[simp] lemma direction_orthRadius (s : Sphere P) (p : P) :
    (s.orthRadius p).direction = (Real ∙ (p -ᵥ s.center))ᗮ := by
  rw [orthRadius]; rw [direction_mk']

instance (s : Sphere P) (p : P) : (s.orthRadius p).direction.HasOrthogonalProjection := by
  rw [direction_orthRadius]
  infer_instance

/--
lemma `orthRadius_center` / 引理 `orthRadius_center`

English:
lemma orthRadius_center
  given: (s : Sphere P)
  statement: s.orthRadius s.center = ⊤
  proof: by
  simp [orthRadius]

中文:
引理 orthRadius_center
  条件: (s : Sphere P)
  结论: s.orthRadius s.center = ⊤
  证明: by
  simp [orthRadius]
-/
@[simp] lemma orthRadius_center (s : Sphere P) : s.orthRadius s.center = ⊤ := by
  simp [orthRadius]

/--
lemma `center_mem_orthRadius_iff` / 引理 `center_mem_orthRadius_iff`

English:
lemma center_mem_orthRadius_iff
  given: {s : Sphere P} {p : P}
  proof: by
  rw [mem_orthRadius_iff_inner_left]; rw [← neg_vsub_eq_vsub_rev]; rw [inner_neg_left]
  simp

中文:
引理 center_mem_orthRadius_iff
  条件: {s : Sphere P} {p : P}
  证明: by
  rw [mem_orthRadius_iff_inner_left]; rw [← neg_vsub_eq_vsub_rev]; rw [inner_neg_left]
  simp
-/
@[simp] lemma center_mem_orthRadius_iff {s : Sphere P} {p : P} :
    s.center in s.orthRadius p ↔ p = s.center := by
  rw [mem_orthRadius_iff_inner_left]; rw [← neg_vsub_eq_vsub_rev]; rw [inner_neg_left]
  simp

set_option backward.isDefEq.respectTransparency false in
/--
lemma `orthogonalProjection_orthRadius_center` / 引理 `orthogonalProjection_orthRadius_center`

English:
lemma orthogonalProjection_orthRadius_center
  given: (s : Sphere P) (p : P)
  proof: by
  simp_rw [orthRadius, coe_orthogonalProjection_eq_iff_mem]
  rw [← Submodule.neg_mem_iff]
  simp

中文:
引理 orthogonalProjection_orthRadius_center
  条件: (s : Sphere P) (p : P)
  证明: by
  simp_rw [orthRadius, coe_orthogonalProjection_eq_iff_mem]
  rw [← Submodule.neg_mem_iff]
  simp
-/
@[simp] lemma orthogonalProjection_orthRadius_center (s : Sphere P) (p : P) :
    orthogonalProjection (s.orthRadius p) s.center = p := by
  simp_rw [orthRadius, coe_orthogonalProjection_eq_iff_mem]
  rw [← Submodule.neg_mem_iff]
  simp

/--
lemma `orthRadius_le_orthRadius_iff` / 引理 `orthRadius_le_orthRadius_iff`

English:
lemma orthRadius_le_orthRadius_iff
  given: {s : Sphere P} {p q : P}
  proof: by
  refine ⟨fun h => ?_, fun h => ?_⟩
  · have h' := direction_le h
    simp only [direction_orthRadius] at h'
    have h'' := Submodule.orthogonal_le h'
    simp only [Submodule.orthogonal_orthogonal, Submodule.span_singleton_le_iff_mem,
      Submodule.mem_span_singleton] at h''
    rcases h'' wi

中文:
引理 orthRadius_le_orthRadius_iff
  条件: {s : Sphere P} {p q : P}
  证明: by
  refine ⟨fun h => ?_, fun h => ?_⟩
  · have h' := direction_le h
    simp only [direction_orthRadius] at h'
    have h'' := Submodule.orthogonal_le h'
    simp only [Submodule.orthogonal_orthogonal, Submodule.span_singleton_le_iff_mem,
      Submodule.mem_span_singleton] at h''
    rcases h'' wi

Depends on / 依赖: Submodule, Submodule.mem_span_singleton, Submodule.orthogonal_le, Submodule.orthogonal_orthogonal, Submodule.span_singleton_le_iff_mem, center, direction_le, direction_orthRadius, inner_sub_left, mem_orthRadius_iff_inner_left, mem_span_singleton, orthRadius, orthogonal_le, orthogonal_orthogonal, real_in, real_inner_smul_left, s.center, s.orthRadius, s.self_mem_orthRadius, self_mem_orthRadius
-/
lemma orthRadius_le_orthRadius_iff {s : Sphere P} {p q : P} :
    s.orthRadius p <= s.orthRadius q ↔ p = q ∨ q = s.center := by
  refine ⟨fun h => ?_, fun h => ?_⟩
  · have h' := direction_le h
    simp only [direction_orthRadius] at h'
    have h'' := Submodule.orthogonal_le h'
    simp only [Submodule.orthogonal_orthogonal, Submodule.span_singleton_le_iff_mem,
      Submodule.mem_span_singleton] at h''
    rcases h'' with ⟨r, hr⟩
    have hp : p in s.orthRadius q := h (s.self_mem_orthRadius p)
    rw [mem_orthRadius_iff_inner_left]; rw [← vsub_sub_vsub_cancel_right p q s.center]; rw [← hr]; rw [inner_sub_left]; rw [real_inner_smul_left]; rw [real_inner_smul_right]; rw [← mul_assoc]; rw [← sub_mul]; rw [mul_eq_zero] at hp
    rcases hp with hp | hp
    · nth_rw 1 [← one_mul r] at hp
      rw [← sub_mul]; rw [mul_eq_zero] at hp
      rcases hp with hp | rfl
      · rw [sub_eq_zero] at hp
        rw [← hp]; rw [one_smul]; rw [vsub_left_cancel_iff] at hr
        exact .inl hr
      · rw [zero_smul, eq_comm, vsub_eq_zero_iff_eq] at hr
        exact .inr hr
    · simp only [inner_self_eq_zero, vsub_eq_zero_iff_eq] at hp
      rw [hp]; rw [vsub_self]; rw [smul_zero]; rw [eq_comm]; rw [vsub_eq_zero_iff_eq] at hr
      exact .inr hr
  · rcases h with rfl | rfl <;> simp

/--
lemma `orthRadius_eq_orthRadius_iff` / 引理 `orthRadius_eq_orthRadius_iff`

English:
lemma orthRadius_eq_orthRadius_iff
  given: {s : Sphere P} {p q : P}
  proof: by
  refine ⟨fun h => ?_, fun h => h ▸ rfl⟩
  have hpq := orthRadius_le_orthRadius_iff.1 h.le
  have hqp := orthRadius_le_orthRadius_iff.1 h.symm.le
  grind

中文:
引理 orthRadius_eq_orthRadius_iff
  条件: {s : Sphere P} {p q : P}
  证明: by
  refine ⟨fun h => ?_, fun h => h ▸ rfl⟩
  have hpq := orthRadius_le_orthRadius_iff.1 h.le
  have hqp := orthRadius_le_orthRadius_iff.1 h.symm.le
  grind
-/
@[simp] lemma orthRadius_eq_orthRadius_iff {s : Sphere P} {p q : P} :
    s.orthRadius p = s.orthRadius q ↔ p = q := by
  refine ⟨fun h => ?_, fun h => h ▸ rfl⟩
  have hpq := orthRadius_le_orthRadius_iff.1 h.le
  have hqp := orthRadius_le_orthRadius_iff.1 h.symm.le
  grind

/--
lemma `orthRadius_injective` / 引理 `orthRadius_injective`

English:
lemma orthRadius_injective
  given: (s : Sphere P)
  statement: Injective s.orthRadius
  proof: fun _ _ => orthRadius_eq_orthRadius_iff.1

中文:
引理 orthRadius_injective
  条件: (s : Sphere P)
  结论: Injective s.orthRadius
  证明: fun _ _ => orthRadius_eq_orthRadius_iff.1

Depends on / 依赖: orthRadius_eq_orthRadius_iff
-/
lemma orthRadius_injective (s : Sphere P) : Injective s.orthRadius :=
  fun _ _ => orthRadius_eq_orthRadius_iff.1

/--
lemma `finrank_orthRadius` / 引理 `finrank_orthRadius`

English:
lemma finrank_orthRadius
  given: [FiniteDimensional Real V] {s : Sphere P} {p : P} (hp : p != s.center)
  proof: by
  rw [orthRadius]; rw [add_comm]; rw [direction_mk']
  convert! (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  exact (finrank_span_singleton (vsub_ne_zero.2 hp)).symm

中文:
引理 finrank_orthRadius
  条件: [FiniteDimensional 实数 V] {s : Sphere P} {p : P} (hp : p != s.center)
  证明: by
  rw [orthRadius]; rw [add_comm]; rw [direction_mk']
  convert! (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  exact (finrank_span_singleton (vsub_ne_zero.2 hp)).symm

Depends on / 依赖: add_comm, center, convert, direction_mk, finrank_add_finrank_orthogonal, finrank_span_singleton, orthRadius, s.center, vsub_ne_zero
-/
lemma finrank_orthRadius [FiniteDimensional Real V] {s : Sphere P} {p : P} (hp : p != s.center) :
    Module.finrank Real (s.orthRadius p).direction + 1 = Module.finrank Real V := by
  rw [orthRadius]; rw [add_comm]; rw [direction_mk']
  convert! (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  exact (finrank_span_singleton (vsub_ne_zero.2 hp)).symm

/--
lemma `orthRadius_map` / 引理 `orthRadius_map`

English:
lemma orthRadius_map
  given: {s : Sphere P} (p : P) {f : P ≃ᵃⁱ[Real] P} (h : f s.center = s.center)
  proof: by
  rw [orthRadius]; rw [map_mk']; rw [orthRadius]
  convert! rfl using 2
  convert! (Submodule.map_orthogonal_equiv (Real ∙ (p -ᵥ s.center)) f.linearIsometryEquiv).symm
  simp [Submodule.map_span, Set.image_singleton, h]

中文:
引理 orthRadius_map
  条件: {s : Sphere P} (p : P) {f : P ≃ᵃⁱ[实数] P} (h : f s.center = s.center)
  证明: by
  rw [orthRadius]; rw [map_mk']; rw [orthRadius]
  convert! rfl using 2
  convert! (Submodule.map_orthogonal_equiv (Real ∙ (p -ᵥ s.center)) f.linearIsometryEquiv).symm
  simp [Submodule.map_span, Set.image_singleton, h]

Depends on / 依赖: Set.image_singleton, Submodule, Submodule.map_orthogonal_equiv, Submodule.map_span, center, convert, f.linearIsometryEquiv, image_singleton, linearIsometryEquiv, map_mk, map_orthogonal_equiv, map_span, orthRadius, s.center
-/
lemma orthRadius_map {s : Sphere P} (p : P) {f : P ≃ᵃⁱ[Real] P} (h : f s.center = s.center) :
    (s.orthRadius p).map f.toAffineMap = s.orthRadius (f p) := by
  rw [orthRadius]; rw [map_mk']; rw [orthRadius]
  convert! rfl using 2
  convert! (Submodule.map_orthogonal_equiv (Real ∙ (p -ᵥ s.center)) f.linearIsometryEquiv).symm
  simp [Submodule.map_span, Set.image_singleton, h]

/--
lemma `direction_orthRadius_le_iff` / 引理 `direction_orthRadius_le_iff`

English:
lemma direction_orthRadius_le_iff
  given: {s : Sphere P} {p q : P}
  proof: by
  simp [Submodule.orthogonal_le_orthogonal_iff, Submodule.mem_span_singleton, eq_comm]

中文:
引理 direction_orthRadius_le_iff
  条件: {s : Sphere P} {p q : P}
  证明: by
  simp [Submodule.orthogonal_le_orthogonal_iff, Submodule.mem_span_singleton, eq_comm]

Depends on / 依赖: Submodule, Submodule.mem_span_singleton, Submodule.orthogonal_le_orthogonal_iff, eq_comm, mem_span_singleton, orthogonal_le_orthogonal_iff
-/
lemma direction_orthRadius_le_iff {s : Sphere P} {p q : P} :
    (s.orthRadius p).direction <= (s.orthRadius q).direction ↔
      exists r : Real, q -ᵥ s.center = r • (p -ᵥ s.center) := by
  simp [Submodule.orthogonal_le_orthogonal_iff, Submodule.mem_span_singleton, eq_comm]

/--
lemma `orthRadius_parallel_orthRadius_iff` / 引理 `orthRadius_parallel_orthRadius_iff`

English:
lemma orthRadius_parallel_orthRadius_iff
  given: {s : Sphere P} {p q : P}
  proof: by
  simp_rw [orthRadius, parallel_iff_direction_eq_and_eq_bot_iff_eq_bot, direction_mk',
    Submodule.orthogonalComplement_eq_orthogonalComplement,
    Submodule.span_singleton_eq_span_singleton, ← coe_eq_bot_iff,
    ← Set.not_nonempty_iff_eq_empty, mk'_nonempty, and_true, ← Units.exists_iff_ne_z

中文:
引理 orthRadius_parallel_orthRadius_iff
  条件: {s : Sphere P} {p q : P}
  证明: by
  simp_rw [orthRadius, parallel_iff_direction_eq_and_eq_bot_iff_eq_bot, direction_mk',
    Submodule.orthogonalComplement_eq_orthogonalComplement,
    Submodule.span_singleton_eq_span_singleton, ← coe_eq_bot_iff,
    ← Set.not_nonempty_iff_eq_empty, mk'_nonempty, and_true, ← Units.exists_iff_ne_z

Depends on / 依赖: Set.not_nonempty_iff_eq_empty, Submodule, Submodule.orthogonalComplement_eq_orthogonalComplement, Submodule.span_singleton_eq_span_singleton, Units.exists_iff_ne_zero, Units.smul_def, _nonempty, and_true, coe_eq_bot_iff, direction_mk, eq_comm, exists_iff_ne_zero, not_nonempty_iff_eq_empty, orthRadius, orthogonalComplement_eq_orthogonalComplement, parallel_iff_direction_eq_and_eq_bot_iff_eq_bot, simp_rw, smul_def, span_singleton_eq_span_singleton
-/
lemma orthRadius_parallel_orthRadius_iff {s : Sphere P} {p q : P} :
    s.orthRadius p ∥ s.orthRadius q ↔ exists r : Real, r != 0 ∧ q -ᵥ s.center = r • (p -ᵥ s.center) := by
  simp_rw [orthRadius, parallel_iff_direction_eq_and_eq_bot_iff_eq_bot, direction_mk',
    Submodule.orthogonalComplement_eq_orthogonalComplement,
    Submodule.span_singleton_eq_span_singleton, ← coe_eq_bot_iff,
    ← Set.not_nonempty_iff_eq_empty, mk'_nonempty, and_true, ← Units.exists_iff_ne_zero, eq_comm,
    Units.smul_def]

/--
lemma `dist_sq_eq_iff_mem_orthRadius` / 引理 `dist_sq_eq_iff_mem_orthRadius`

English:
lemma dist_sq_eq_iff_mem_orthRadius
  given: {s : Sphere P} {p q : P}
  proof: by
  simp_rw [dist_eq_norm_vsub, pow_two]
  rw [← vsub_add_vsub_cancel q p s.center]
  nth_rw 3 [add_comm]
  rw [norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]; rw [← mem_orthRadius_iff_inner_left]

alias ⟨_, dist_sq_eq_of_mem_orthRadius⟩ := dist_sq_eq_iff_mem_orthRadius

中文:
引理 dist_sq_eq_iff_mem_orthRadius
  条件: {s : Sphere P} {p q : P}
  证明: by
  simp_rw [dist_eq_norm_vsub, pow_two]
  rw [← vsub_add_vsub_cancel q p s.center]
  nth_rw 3 [add_comm]
  rw [norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]; rw [← mem_orthRadius_iff_inner_left]

alias ⟨_, dist_sq_eq_of_mem_orthRadius⟩ := dist_sq_eq_iff_mem_orthRadius

Depends on / 依赖: add_comm, center, dist_eq_norm_vsub, mem_orthRadius_iff_inner_left, norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero, nth_rw, pow_two, s.center, simp_rw, vsub_add_vsub_cancel
-/
lemma dist_sq_eq_iff_mem_orthRadius {s : Sphere P} {p q : P} :
    (dist q s.center) ^ 2 = (dist p s.center) ^ 2 + (dist q p) ^ 2 ↔ q in s.orthRadius p := by
  simp_rw [dist_eq_norm_vsub, pow_two]
  rw [← vsub_add_vsub_cancel q p s.center]
  nth_rw 3 [add_comm]
  rw [norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]; rw [← mem_orthRadius_iff_inner_left]

alias ⟨_, dist_sq_eq_of_mem_orthRadius⟩ := dist_sq_eq_iff_mem_orthRadius

/--
lemma `mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq` / 引理 `mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq`

English:
lemma mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq
  given: {s : Sphere P} {p q : P}
  proof: by
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe,
    ← dist_sq_eq_iff_mem_orthRadius, ← direction_orthRadius,
    vsub_right_mem_direction_iff_mem (s.self_mem_orthRadius p), ← dist_eq_norm_vsub]
  nth_rw 3 [and_comm]
  rw [← and_assoc]; rw [and_congr_left_iff]
  intro

中文:
引理 mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq
  条件: {s : Sphere P} {p q : P}
  证明: by
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe,
    ← dist_sq_eq_iff_mem_orthRadius, ← direction_orthRadius,
    vsub_right_mem_direction_iff_mem (s.self_mem_orthRadius p), ← dist_eq_norm_vsub]
  nth_rw 3 [and_comm]
  rw [← and_assoc]; rw [and_congr_left_iff]
  intro

Depends on / 依赖: Metric, Metric.mem_sphere, Set.mem_inter_iff, SetLike, SetLike.mem_coe, and_assoc, and_comm, and_congr_left_iff, direction_orthRadius, dist_eq_norm_vsub, dist_sq_eq_iff_mem_orthRadius, false_and, h0.not_ge, iff_false, le_or_gt, mem_coe, mem_inter_iff, mem_sphere, not_ge, nth_rw
-/
lemma mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq {s : Sphere P} {p q : P} :
    q in (s inter s.orthRadius p : Set P) ↔ 0 <= s.radius ∧
      q -ᵥ p in (Real ∙ (p -ᵥ s.center))ᗮ ∧ ‖q -ᵥ p‖ ^ 2 = s.radius ^ 2 - (dist p s.center) ^ 2 := by
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe,
    ← dist_sq_eq_iff_mem_orthRadius, ← direction_orthRadius,
    vsub_right_mem_direction_iff_mem (s.self_mem_orthRadius p), ← dist_eq_norm_vsub]
  nth_rw 3 [and_comm]
  rw [← and_assoc]; rw [and_congr_left_iff]
  intro h
  rw [← sub_eq_iff_eq_add'] at h
  rw [← h]
  rcases le_or_gt 0 s.radius with h0 | h0
  · simp [h0]
  · simp only [h0.not_ge, sub_left_inj, false_and, iff_false]
    intro hm
    exact h0.not_ge (radius_nonneg_of_mem hm)

/--
lemma `mem_inter_orthRadius_iff_vsub_mem_and_norm_sq` / 引理 `mem_inter_orthRadius_iff_vsub_mem_and_norm_sq`

English:
lemma mem_inter_orthRadius_iff_vsub_mem_and_norm_sq
  given: {s : Sphere P} {p q : P} (h : 0 <= s.radius)
  proof: by
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp [h]

中文:
引理 mem_inter_orthRadius_iff_vsub_mem_and_norm_sq
  条件: {s : Sphere P} {p q : P} (h : 0 <= s.radius)
  证明: by
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp [h]

Depends on / 依赖: mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq
-/
lemma mem_inter_orthRadius_iff_vsub_mem_and_norm_sq {s : Sphere P} {p q : P} (h : 0 <= s.radius) :
    q in (s inter s.orthRadius p : Set P) ↔
      q -ᵥ p in (Real ∙ (p -ᵥ s.center))ᗮ ∧ ‖q -ᵥ p‖ ^ 2 = s.radius ^ 2 - (dist p s.center) ^ 2 := by
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp [h]

/--
lemma `vadd_mem_inter_orthRadius_iff_norm_sq` / 引理 `vadd_mem_inter_orthRadius_iff_norm_sq`

English:
lemma vadd_mem_inter_orthRadius_iff_norm_sq
  statement: {s : Sphere P} {p : P} {v : V} (h : 0 <= s.radius)
  proof: by
  rw [mem_inter_orthRadius_iff_vsub_mem_and_norm_sq h]
  simp [hv]

中文:
引理 vadd_mem_inter_orthRadius_iff_norm_sq
  结论: {s : Sphere P} {p : P} {v : V} (h : 0 <= s.radius)
  证明: by
  rw [mem_inter_orthRadius_iff_vsub_mem_and_norm_sq h]
  simp [hv]

Depends on / 依赖: mem_inter_orthRadius_iff_vsub_mem_and_norm_sq
-/
lemma vadd_mem_inter_orthRadius_iff_norm_sq {s : Sphere P} {p : P} {v : V} (h : 0 <= s.radius)
    (hv : v in (Real ∙ (p -ᵥ s.center))ᗮ) :
    v +ᵥ p in (s inter s.orthRadius p : Set P) ↔ ‖v‖ ^ 2 = s.radius ^ 2 - (dist p s.center) ^ 2 := by
  rw [mem_inter_orthRadius_iff_vsub_mem_and_norm_sq h]
  simp [hv]

attribute [local instance] FiniteDimensional.of_fact_finrank_eq_two

/--
lemma `inter_orthRadius_eq_singleton_of_dist_eq_radius` / 引理 `inter_orthRadius_eq_singleton_of_dist_eq_radius`

English:
lemma inter_orthRadius_eq_singleton_of_dist_eq_radius
  statement: {s : Sphere P} {p : P}
  proof: by
  ext p'
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe, Set.mem_singleton_iff]
  constructor
  · rintro ⟨hp's, hp'i⟩
    have h' := dist_sq_eq_of_mem_orthRadius hp'i
    rw [hp's]; rw [hp] at h'
    simpa using h'
  · rintro rfl
    simpa using hp

中文:
引理 inter_orthRadius_eq_singleton_of_dist_eq_radius
  结论: {s : Sphere P} {p : P}
  证明: by
  ext p'
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe, Set.mem_singleton_iff]
  constructor
  · rintro ⟨hp's, hp'i⟩
    have h' := dist_sq_eq_of_mem_orthRadius hp'i
    rw [hp's]; rw [hp] at h'
    simpa using h'
  · rintro rfl
    simpa using hp

Depends on / 依赖: Metric, Metric.mem_sphere, Set.mem_inter_iff, Set.mem_singleton_iff, SetLike, SetLike.mem_coe, dist_sq_eq_of_mem_orthRadius, mem_coe, mem_inter_iff, mem_singleton_iff, mem_sphere
-/
lemma inter_orthRadius_eq_singleton_of_dist_eq_radius {s : Sphere P} {p : P}
    (hp : dist p s.center = s.radius) : (s inter s.orthRadius p : Set P) = {p} := by
  ext p'
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe, Set.mem_singleton_iff]
  constructor
  · rintro ⟨hp's, hp'i⟩
    have h' := dist_sq_eq_of_mem_orthRadius hp'i
    rw [hp's]; rw [hp] at h'
    simpa using h'
  · rintro rfl
    simpa using hp

/--
lemma `inter_orthRadius_eq_singleton_iff` / 引理 `inter_orthRadius_eq_singleton_iff`

English:
lemma inter_orthRadius_eq_singleton_iff
  given: {s : Sphere P} {p q : P}
  proof: by
  constructor
  · intro h
    have hq : q in (s inter s.orthRadius p : Set P) := h ▸ Set.mem_singleton _
    have h' (q' : P) : q' in (s inter s.orthRadius p : Set P) ↔ q' = q := by simp [h]
    have hr : 0 <= s.radius := radius_nonneg_of_mem hq.1
    simp_rw [mem_inter_orthRadius_iff_vsub_mem_an

中文:
引理 inter_orthRadius_eq_singleton_iff
  条件: {s : Sphere P} {p q : P}
  证明: by
  constructor
  · intro h
    have hq : q in (s inter s.orthRadius p : Set P) := h ▸ Set.mem_singleton _
    have h' (q' : P) : q' in (s inter s.orthRadius p : Set P) ↔ q' = q := by simp [h]
    have hr : 0 <= s.radius := radius_nonneg_of_mem hq.1
    simp_rw [mem_inter_orthRadius_iff_vsub_mem_an

Depends on / 依赖: Set.mem_singleton, center, mem_inter_orthRadius_iff_vsub_mem_and_norm_sq, mem_singleton, neg_vsub_eq_vsub_rev, orthRadius, radius, radius_nonneg_of_mem, s.center, s.orthRadius, s.radius, simp_rw
-/
lemma inter_orthRadius_eq_singleton_iff {s : Sphere P} {p q : P} :
    (s inter s.orthRadius p : Set P) = {q} ↔ q = p ∧ dist p s.center = s.radius := by
  constructor
  · intro h
    have hq : q in (s inter s.orthRadius p : Set P) := h ▸ Set.mem_singleton _
    have h' (q' : P) : q' in (s inter s.orthRadius p : Set P) ↔ q' = q := by simp [h]
    have hr : 0 <= s.radius := radius_nonneg_of_mem hq.1
    simp_rw [mem_inter_orthRadius_iff_vsub_mem_and_norm_sq hr] at h'
    have hq' := (h' q).2 rfl
    have hq'' : (-(q -ᵥ p) +ᵥ p) -ᵥ p in (Real ∙ (p -ᵥ s.center))ᗮ ∧
        ‖(-(q -ᵥ p) +ᵥ p) -ᵥ p‖ ^ 2 = s.radius ^ 2 - dist p s.center ^ 2 := by
      simpa [-neg_vsub_eq_vsub_rev] using hq'
    have hqq := (h' _).1 hq''
    rw [eq_comm]; rw [eq_vadd_iff_vsub_eq]; rw [eq_neg_iff_add_eq_zero]; rw [← two_smul Real]; rw [smul_eq_zero_iff_right (by norm_num)]; rw [vsub_eq_zero_iff_eq] at hqq
    refine ⟨hqq, ?_⟩
    subst hqq
    exact hq.1
  · rintro ⟨rfl, h⟩
    exact inter_orthRadius_eq_singleton_of_dist_eq_radius h

/--
lemma `inter_orthRadius_eq_empty_of_radius_lt_dist` / 引理 `inter_orthRadius_eq_empty_of_radius_lt_dist`

English:
lemma inter_orthRadius_eq_empty_of_radius_lt_dist
  statement: {s : Sphere P} {p : P}
  proof: by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  rintro hle - h
  nlinarith

中文:
引理 inter_orthRadius_eq_empty_of_radius_lt_dist
  结论: {s : Sphere P} {p : P}
  证明: by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  rintro hle - h
  nlinarith

Depends on / 依赖: Set.mem_empty_iff_false, iff_false, mem_empty_iff_false, mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq, not_and
-/
lemma inter_orthRadius_eq_empty_of_radius_lt_dist {s : Sphere P} {p : P}
    (hp : s.radius < dist p s.center) : (s inter s.orthRadius p : Set P) = ∅ := by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  rintro hle - h
  nlinarith

/--
lemma `inter_orthRadius_eq_empty_of_finrank_eq_one` / 引理 `inter_orthRadius_eq_empty_of_finrank_eq_one`

English:
lemma inter_orthRadius_eq_empty_of_finrank_eq_one
  statement: {s : Sphere P} {p : P} (hpc : p != s.center)
  proof: by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  intro hr hpo
  have : FiniteDimensional Real V := Module.finite_of_finrank_eq_succ hf
  have ha := (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  

中文:
引理 inter_orthRadius_eq_empty_of_finrank_eq_one
  结论: {s : Sphere P} {p : P} (hpc : p != s.center)
  证明: by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  intro hr hpo
  have : FiniteDimensional Real V := Module.finite_of_finrank_eq_succ hf
  have ha := (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  

Depends on / 依赖: FiniteDimensional, Module, Module.finite_of_finrank_eq_succ, Nat.add_eq_left, Set.mem_empty_iff_false, Submodule, Submodule.finrank_eq_zero, Submodule.mem_bot, Submodule.orthogonal_eq_bot_iff, Submodule.top_orthogonal_eq_bot, add_eq_left, center, finite_of_finrank_eq_succ, finrank_add_finrank_orthogonal, finrank_eq_zero, finrank_span_singleton, iff_false, mem_bot, mem_empty_iff_false, mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq
-/
lemma inter_orthRadius_eq_empty_of_finrank_eq_one {s : Sphere P} {p : P} (hpc : p != s.center)
    (hp : dist p s.center != s.radius) (hf : Module.finrank Real V = 1) :
    (s inter s.orthRadius p : Set P) = ∅ := by
  ext p'
  rw [mem_inter_orthRadius_iff_radius_nonneg_and_vsub_mem_and_norm_sq]
  simp only [Set.mem_empty_iff_false, iff_false, not_and]
  intro hr hpo
  have : FiniteDimensional Real V := Module.finite_of_finrank_eq_succ hf
  have ha := (Real ∙ (p -ᵥ s.center)).finrank_add_finrank_orthogonal
  simp only [finrank_span_singleton (vsub_ne_zero.2 hpc), hf, Nat.add_eq_left,
    Submodule.finrank_eq_zero, Submodule.orthogonal_eq_bot_iff] at ha
  simp only [ha, Submodule.top_orthogonal_eq_bot, Submodule.mem_bot, vsub_eq_zero_iff_eq] at hpo
  simp only [hpo, vsub_self, norm_zero, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true, zero_pow]
  rw [eq_comm]; rw [sub_eq_zero]; rw [eq_comm]; rw [sq_eq_sq₀ dist_nonneg hr]
  exact hp

/--
lemma `inter_orthRadius_eq_empty_iff` / 引理 `inter_orthRadius_eq_empty_iff`

English:
lemma inter_orthRadius_eq_empty_iff
  given: {s : Sphere P} {p : P}
  proof: by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · simp only [h.not_gt, h, ne_eq, true_and, (dist_nonneg.trans_lt h).ne', not_false_eq_true,
      and_true, false_or]
    obtain rfl | hp := eq_or_ne p s.center
    · rcases subsingleton_or_nontrivial V with hs | hs
      · have h

中文:
引理 inter_orthRadius_eq_empty_iff
  条件: {s : Sphere P} {p : P}
  证明: by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · simp only [h.not_gt, h, ne_eq, true_and, (dist_nonneg.trans_lt h).ne', not_false_eq_true,
      and_true, false_or]
    obtain rfl | hp := eq_or_ne p s.center
    · rcases subsingleton_or_nontrivial V with hs | hs
      · have h

Depends on / 依赖: AddTorsor, AddTorsor.subsingleton_iff, Metric, Metric.sphere_eq_empty_of_subsingleton, Module, Module.Free, Set.inter_univ, _root_, _root_.Module.Free.of_divisionRing, and_false, and_true, center, dist_nonneg, dist_nonneg.trans_lt, eq_or_ne, false_or, h.not_gt, inter_univ, lt_trichotomy, ne_eq
-/
lemma inter_orthRadius_eq_empty_iff {s : Sphere P} {p : P} :
    (s inter s.orthRadius p : Set P) = ∅ ↔ s.radius < dist p s.center ∨
      (Module.finrank Real V = 1 ∧ dist p s.center < s.radius ∧ p != s.center) ∨
      (Subsingleton V ∧ s.radius != 0) := by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · simp only [h.not_gt, h, ne_eq, true_and, (dist_nonneg.trans_lt h).ne', not_false_eq_true,
      and_true, false_or]
    obtain rfl | hp := eq_or_ne p s.center
    · rcases subsingleton_or_nontrivial V with hs | hs
      · have hs' := (AddTorsor.subsingleton_iff V P).1 hs
        simp [hs, Metric.sphere_eq_empty_of_subsingleton (dist_nonneg.trans_lt h).ne']
      · simp only [orthRadius_center, top_coe, Set.inter_univ, not_true_eq_false, and_false,
          not_subsingleton_iff_nontrivial.2 hs, or_self, iff_false, ← Set.nonempty_iff_ne_empty]
        obtain ⟨v, hv⟩ := exists_norm_eq V (dist_nonneg.trans_lt h).le
        exact ⟨v +ᵥ s.center, by simp [hv]⟩
    · simp only [hp, not_false_eq_true, and_true]
      rcases eq_or_ne (Real ∙ (p -ᵥ s.center)) ⊤ with hb | hb
      · have hb' := hb
        rw [Submodule.span_singleton_eq_top_iff] at hb'
        have hf := finrank_eq_one_iff'.2 ⟨p -ᵥ s.center, vsub_ne_zero.2 hp, hb'⟩
        simpa [hf] using inter_orthRadius_eq_empty_of_finrank_eq_one hp h.ne
      · have hn : ¬Subsingleton V := by
          rw [AddTorsor.subsingleton_iff V P]
          intro hs
          simp [Subsingleton.elim p s.center] at hp
        have hnf : Module.finrank Real V != 1 := by
          intro hf
          apply hb
          rw [Submodule.span_singleton_eq_top_iff]
          rw [finrank_eq_one_iff'] at hf
          obtain ⟨v, hv0, hv⟩ := hf
          obtain ⟨c, hc⟩ := hv (p -ᵥ s.center)
          have hc0 : c != 0 := by
            rintro rfl
            rw [zero_smul]; rw [eq_comm]; rw [vsub_eq_zero_iff_eq] at hc
            simp [hc] at hp
          intro v'
          obtain ⟨c', rfl⟩ := hv v'
          refine ⟨c' / c, ?_⟩
          simp [← hc, smul_smul, hc0]
        simp only [hnf, hn, or_self, iff_false, ← Set.nonempty_iff_ne_empty]
        rw [ne_eq]; rw [← Submodule.orthogonal_eq_bot_iff] at hb
        obtain ⟨v, hvm, hv0⟩ := Submodule.exists_mem_ne_zero_of_ne_bot hb
        refine ⟨(√(s.radius ^ 2 - (dist p s.center) ^ 2) / ‖v‖) • v +ᵥ p, ?_⟩
        rw [vadd_mem_inter_orthRadius_iff_norm_sq (dist_nonneg.trans_lt h).le
          (Submodule.smul_mem _ _ hvm)]
        rw [norm_smul]; rw [norm_div]; rw [norm_norm]; rw [div_mul_cancel₀ _ (norm_ne_zero_iff.2 hv0)]
        simp only [Real.norm_eq_abs, sq_abs]
        refine Real.sq_sqrt ?_
        rw [sub_nonneg]; rw [sq_le_sq]; rw [abs_of_nonneg dist_nonneg]
        exact h.le.trans (le_abs_self _)
  · rw [inter_orthRadius_eq_singleton_of_dist_eq_radius h]
    simp only [Set.singleton_ne_empty, h, lt_self_iff_false, ne_eq, false_and, and_false, false_or,
      false_iff, not_and, not_not]
    intro hs
    rw [AddTorsor.subsingleton_iff V P] at hs
    rw [Subsingleton.elim p s.center] at h
    simpa using h.symm
  · simp [h, inter_orthRadius_eq_empty_of_radius_lt_dist]

/--
lemma `inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one` / 引理 `inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one`

English:
lemma inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one
  statement: [hf2 : Fact (Module.finrank Real V = 2)]
  proof: by
  have hr : 0 <= s.radius := dist_nonneg.trans hp
  have hv0 : v != 0 := by rw [← norm_ne_zero_iff, hv1]; simp
  rw [neg_smul]
  have hf := finrank_orthRadius hpc
  rw [direction_orthRadius] at hf
  simp only [hf2.out, Nat.reduceEqDiff] at hf
  rw [finrank_eq_one_iff_of_nonzero' ⟨v]; rw [hv⟩ (by 

中文:
引理 inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one
  结论: [hf2 : Fact (Module.finrank 实数 V = 2)]
  证明: by
  have hr : 0 <= s.radius := dist_nonneg.trans hp
  have hv0 : v != 0 := by rw [← norm_ne_zero_iff, hv1]; simp
  rw [neg_smul]
  have hf := finrank_orthRadius hpc
  rw [direction_orthRadius] at hf
  simp only [hf2.out, Nat.reduceEqDiff] at hf
  rw [finrank_eq_one_iff_of_nonzero' ⟨v]; rw [hv⟩ (by 

Depends on / 依赖: Nat.reduceEqDiff, center, direction_orthRadius, dist_nonneg, dist_nonneg.trans, finrank_eq_one_iff_of_nonzero, finrank_orthRadius, hf2.out, neg_smul, norm_ne_zero_iff, radius, reduceEqDiff, s.center, s.radius, sub_nonneg
-/
lemma inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one [hf2 : Fact (Module.finrank Real V = 2)]
    {s : Sphere P} {p : P} (hp : dist p s.center <= s.radius) (hpc : p != s.center) {v : V}
    (hv : v in (Real ∙ (p -ᵥ s.center))ᗮ) (hv1 : ‖v‖ = 1) :
    (s inter s.orthRadius p : Set P) = {√(s.radius ^ 2 - (dist p s.center) ^ 2) • v +ᵥ p,
      -√(s.radius ^ 2 - (dist p s.center) ^ 2) • v +ᵥ p} := by
  have hr : 0 <= s.radius := dist_nonneg.trans hp
  have hv0 : v != 0 := by rw [← norm_ne_zero_iff, hv1]; simp
  rw [neg_smul]
  have hf := finrank_orthRadius hpc
  rw [direction_orthRadius] at hf
  simp only [hf2.out, Nat.reduceEqDiff] at hf
  rw [finrank_eq_one_iff_of_nonzero' ⟨v]; rw [hv⟩ (by simpa using hv0)] at hf
  have hvc : forall w in (Real ∙ (p -ᵥ s.center))ᗮ, exists c : Real, c • v = w := by
    intro w hw
    simpa using hf ⟨w, hw⟩
  have hvp : 0 <= s.radius ^ 2 - (dist p s.center) ^ 2 := by
    rw [sub_nonneg]; rw [sq_le_sq]; rw [abs_of_nonneg dist_nonneg]
    exact le_abs.2 (.inl hp)
  ext p'
  refine ⟨fun h => ?_, fun h => ?_⟩
  · rw [← vsub_vadd p' p, Set.mem_insert_iff, Set.mem_singleton_iff, vadd_right_cancel_iff,
      vadd_right_cancel_iff]
    have h' : p' -ᵥ p in (Real ∙ (p -ᵥ s.center))ᗮ := by
      rw [← direction_orthRadius]; rw [vsub_right_mem_direction_iff_mem (s.self_mem_orthRadius p)]
      exact h.2
    rw [← vsub_vadd p' p]; rw [vadd_mem_inter_orthRadius_iff_norm_sq hr h'] at h
    obtain ⟨c, hc⟩ := hvc _ h'
    rw [← hc] at h ⊢
    rw [← neg_smul]
    simp_rw [(smul_left_injective Real hv0).eq_iff, ← sq_eq_sq_iff_eq_or_eq_neg, Real.sq_sqrt hvp]
    simpa [norm_smul, hv1] using h
  · rw [← neg_smul] at h
    rcases h with rfl | rfl <;>
      rw [vadd_mem_inter_orthRadius_iff_norm_sq hr (Submodule.smul_mem _ _ hv)] <;>
      simp [norm_smul, hv1, hvp]

/--
lemma `inter_orthRadius_eq_of_dist_le_radius` / 引理 `inter_orthRadius_eq_of_dist_le_radius`

English:
lemma inter_orthRadius_eq_of_dist_le_radius
  statement: [hf2 : Fact (Module.finrank Real V = 2)]
  proof: by
  convert!
    inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one hp hpc (v := ‖v‖⁻¹ • v)
      (Submodule.smul_mem _ _ hv) ?_ using 2
  · simp [div_eq_mul_inv, smul_smul]
  · simp [div_eq_mul_inv, smul_smul]
  · simp [norm_smul, norm_ne_zero_iff.2 hv0]

中文:
引理 inter_orthRadius_eq_of_dist_le_radius
  结论: [hf2 : Fact (Module.finrank 实数 V = 2)]
  证明: by
  convert!
    inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one hp hpc (v := ‖v‖⁻¹ • v)
      (Submodule.smul_mem _ _ hv) ?_ using 2
  · simp [div_eq_mul_inv, smul_smul]
  · simp [div_eq_mul_inv, smul_smul]
  · simp [norm_smul, norm_ne_zero_iff.2 hv0]

Depends on / 依赖: Submodule, Submodule.smul_mem, convert, div_eq_mul_inv, inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one, norm_ne_zero_iff, norm_smul, smul_mem, smul_smul
-/
lemma inter_orthRadius_eq_of_dist_le_radius [hf2 : Fact (Module.finrank Real V = 2)]
    {s : Sphere P} {p : P} (hp : dist p s.center <= s.radius) (hpc : p != s.center) {v : V}
    (hv : v in (Real ∙ (p -ᵥ s.center))ᗮ) (hv0 : v != 0) :
    (s inter s.orthRadius p : Set P) = {(√(s.radius ^ 2 - (dist p s.center) ^ 2) / ‖v‖) • v +ᵥ p,
      -(√(s.radius ^ 2 - (dist p s.center) ^ 2) / ‖v‖) • v +ᵥ p} := by
  convert!
    inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one hp hpc (v := ‖v‖⁻¹ • v)
      (Submodule.smul_mem _ _ hv) ?_ using 2
  · simp [div_eq_mul_inv, smul_smul]
  · simp [div_eq_mul_inv, smul_smul]
  · simp [norm_smul, norm_ne_zero_iff.2 hv0]

/--
lemma `ncard_inter_orthRadius_eq_two_of_dist_lt_radius` / 引理 `ncard_inter_orthRadius_eq_two_of_dist_lt_radius`

English:
lemma ncard_inter_orthRadius_eq_two_of_dist_lt_radius
  statement: [hf2 : Fact (Module.finrank Real V = 2)]
  proof: by
  have hf := finrank_orthRadius hpc
  simp only [hf2.out, Nat.reduceEqDiff, finrank_eq_one_iff'] at hf
  obtain ⟨v, hv0, hv⟩ := hf
  replace hv0 : (v : V) != 0 := by simpa using hv0
  rw [inter_orthRadius_eq_of_dist_le_radius hp.le hpc (by simpa using v.property) hv0]; rw [Submodule.norm_coe]; rw

中文:
引理 ncard_inter_orthRadius_eq_two_of_dist_lt_radius
  结论: [hf2 : Fact (Module.finrank 实数 V = 2)]
  证明: by
  have hf := finrank_orthRadius hpc
  simp only [hf2.out, Nat.reduceEqDiff, finrank_eq_one_iff'] at hf
  obtain ⟨v, hv0, hv⟩ := hf
  replace hv0 : (v : V) != 0 := by simpa using hv0
  rw [inter_orthRadius_eq_of_dist_le_radius hp.le hpc (by simpa using v.property) hv0]; rw [Submodule.norm_coe]; rw

Depends on / 依赖: Nat.reduceEqDiff, Set.ncard_pair, Submodule, Submodule.norm_coe, add_eq_zero_iff_eq_neg, div_eq_, finrank_eq_one_iff, finrank_orthRadius, hf2.out, hp.le, inter_orthRadius_eq_of_dist_le_radius, ncard_pair, ne_eq, neg_smul, norm_coe, property, reduceEqDiff, replace, smul_eq_zero_iff_left, smul_eq_zero_iff_right
-/
lemma ncard_inter_orthRadius_eq_two_of_dist_lt_radius [hf2 : Fact (Module.finrank Real V = 2)]
    {s : Sphere P} {p : P} (hp : dist p s.center < s.radius) (hpc : p != s.center) :
    (s inter s.orthRadius p : Set P).ncard = 2 := by
  have hf := finrank_orthRadius hpc
  simp only [hf2.out, Nat.reduceEqDiff, finrank_eq_one_iff'] at hf
  obtain ⟨v, hv0, hv⟩ := hf
  replace hv0 : (v : V) != 0 := by simpa using hv0
  rw [inter_orthRadius_eq_of_dist_le_radius hp.le hpc (by simpa using v.property) hv0]; rw [Submodule.norm_coe]; rw [neg_smul]; rw [Set.ncard_pair]
  rw [ne_eq]; rw [vadd_right_cancel_iff]; rw [← add_eq_zero_iff_eq_neg]; rw [← two_smul Real]; rw [smul_eq_zero_iff_right two_ne_zero]; rw [smul_eq_zero_iff_left hv0]; rw [div_eq_iff]; rw [zero_mul]
  · have hvp : 0 < √(s.radius ^ 2 - dist p s.center ^ 2) := by
      rw [Real.sqrt_pos]; rw [sub_pos]; rw [sq_lt_sq]; rw [abs_of_nonneg dist_nonneg]
      exact lt_abs.2 (.inl hp)
    exact hvp.ne'
  · simpa using hv0

/--
lemma `ncard_inter_orthRadius_le_two` / 引理 `ncard_inter_orthRadius_le_two`

English:
lemma ncard_inter_orthRadius_le_two
  statement: [hf2 : Fact (Module.finrank Real V = 2)]
  proof: by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · exact (ncard_inter_orthRadius_eq_two_of_dist_lt_radius h hpc).le
  · simp [inter_orthRadius_eq_singleton_of_dist_eq_radius h]
  · simp [inter_orthRadius_eq_empty_of_radius_lt_dist h]

中文:
引理 ncard_inter_orthRadius_le_two
  结论: [hf2 : Fact (Module.finrank 实数 V = 2)]
  证明: by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · exact (ncard_inter_orthRadius_eq_two_of_dist_lt_radius h hpc).le
  · simp [inter_orthRadius_eq_singleton_of_dist_eq_radius h]
  · simp [inter_orthRadius_eq_empty_of_radius_lt_dist h]

Depends on / 依赖: center, inter_orthRadius_eq_empty_of_radius_lt_dist, inter_orthRadius_eq_singleton_of_dist_eq_radius, lt_trichotomy, ncard_inter_orthRadius_eq_two_of_dist_lt_radius, radius, s.center, s.radius
-/
lemma ncard_inter_orthRadius_le_two [hf2 : Fact (Module.finrank Real V = 2)]
    {s : Sphere P} {p : P} (hpc : p != s.center) : (s inter s.orthRadius p : Set P).ncard <= 2 := by
  rcases lt_trichotomy (dist p s.center) s.radius with h | h | h
  · exact (ncard_inter_orthRadius_eq_two_of_dist_lt_radius h hpc).le
  · simp [inter_orthRadius_eq_singleton_of_dist_eq_radius h]
  · simp [inter_orthRadius_eq_empty_of_radius_lt_dist h]

end Sphere

end EuclideanGeometry
