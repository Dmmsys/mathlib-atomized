...
English:
class Regular
  parameters: extends HasFiniteLimits C
  extends: HasFiniteLimits C
...
instance :
  ...
  body: ⟨_, pullback.snd g f, inferInstance, pullback.fst g f, pullback.condition⟩

中文:
实例 :
  签名: Preregular C
  定义体: ⟨_, pullback.snd g f, inferInstance, pullback.fst g f, pullback.condition⟩

Depends on / 依赖: condition, hI.isBasis_inter_ground.inter_eq_of_subset_indep, hJ.subset_ground, inter_assoc, inter_comm, inter_eq_of_subset_indep, inter_eq_self_of_subset_left, isBasis_inter_ground, pullback, pullback.condition, pullback.fst, pullback.snd, subset_ground
-/
...
English:
instance :
  signature: Mono (coequalizer.desc f pullback.condition)
  body: by
...
English:
definition strongEpiMonoFactorisation
  signature: : StrongEpiMonoFactorisation f where
  body: coequalizer (pullback.fst f f) (pullback.snd f f)
  m := coequalizer.desc f pullback.condition
  e := coequalizer.π (pullback.fst f f) (pullback.snd f f)

中文:
定义 strongEpiMonoFactorisation
  签名: : StrongEpiMonoFactorisation f where
  定义体: coequalizer (pullback.fst f f) (pullback.snd f f)
  m := coequalizer.desc f pullback.condition
  e := coequalizer.π (pullback.fst f f) (pullback.snd f f)

Depends on / 依赖: coequalizer, pullback, pullback.fst, pullback.snd
-/
...
English:
instance :
  signature: IsRegularEpi (strongEpiMonoFactorisation f).e
  body: by
...
Depends on / 依赖: infer_instance, strongEpiMonoFactorisation
-/
...
/--
Instance `hasStrongEpiMonoFactorisations` / 实例 `hasStrongEpiMonoFactorisations`

English:
instance hasStrongEpiMonoFactorisations
  signature: : HasStrongEpiMonoFactorisations C where
  body: ⟨strongEpiMonoFactorisation f⟩
...
set_option backward.isDefEq.respectTransparency false in
/--
Definition of `regularEpiOfExtremalEpi` / `regularEpiOfExtremalEpi` 的定义

English:
definition regularEpiOfExtremalEpi
  signature: [h : ExtremalEpi f]
  body: have := h.isIso (strongEpiMonoFactorisation f).e (strongEpiMonoFactorisation f).m (by simp)
...
Depends on / 依赖: Arrow.isoMk, IsRegularEpi, IsRegularEpi.getStruct, Iso.refl, RegularEpi, RegularEpi.ofArrowIso, getStruct, h.isIso, ofArrowIso, strongEpiMonoFactorisation
-/
...
/--
Instance `isRegularEpi_of_extremalEpi` / 实例 `isRegularEpi_of_extremalEpi`

English:
instance isRegularEpi_of_extremalEpi
  signature: (f : X ⟶ Y) [ExtremalEpi f]
  body: ⟨⟨regularEpiOfExtremalEpi f⟩⟩
...
definition frobeniusMorphism
  signature: :
  body: (inf_isPullback ((«exists» f).obj A') B').flip.lift
...
定义 frobeniusMorphism
  签名: :
  定义体: (inf_isPullback ((«exists» f).obj A') B').flip.lift
    ((ofLE _ _ (inf_le_right A' ((Subobject.pullback f).obj B'))) ≫ (pullbackπ _ _))
    ((ofLE _ _ (inf_le_left A' ((Subobject.pullback f).obj B'))) ≫ (imageFactorisation f A').F.e)
    (by simp [← imageFactorisation_F_m, (isPullback _ _).w])

Depends on / 依赖: Subobject, Subobject.pullback, flip.lift, imageFactorisation, imageFactorisation_F_m, inf_isPullback, inf_le_left, inf_le_right, isPullback, pullback
-/
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `frobeniusMorphism_isPullback` / 引理 `frobeniusMorphism_isPullback`
...
English:
instance :
  signature: IsRegularEpi (frobeniusMorphism f A' B')
  body: by
...
Depends on / 依赖: MorphismProperty, MorphismProperty.regularEpi_iff, frobeniusMorphism_isPullback, imageFactorisation, infer_instance, isImage, of_isPullback, regularEpiIsStableUnderBaseChange, regularEpiIsStableUnderBaseChange.of_isPullback, regularEpi_iff, strongEpiMonoFactorisation, strongEpi_of_strongEpiMonoFactorisation
-/
...
@[simps!]
/--
Definition of `frobeniusStrongEpiMonoFactorisation` / `frobeniusStrongEpiMonoFactorisation` 的定义

English:
definition frobeniusStrongEpiMonoFactorisation
  signature: :
  body: underlying.obj («exists» f).obj A' ⊓ B'
  m := ((«exists» f).obj A' ⊓ B').arrow
  e := frobeniusMorphism f A' B'
  fac := by
    rw [frobeniusMorphism]; rw [← inf_comp_left]; rw [← Category.assoc]; rw [(inf_isPullback ((«exists» f).obj A') B').flip.lift_snd]
    simp [← imageFactorisation_F_m]

中文:
定义 frobeniusStrongEpiMonoFactorisation
  签名: :
  定义体: underlying.obj («exists» f).obj A' ⊓ B'
  m := ((«exists» f).obj A' ⊓ B').arrow
  e := frobeniusMorphism f A' B'
  fac := by
    rw [frobeniusMorphism]; rw [← inf_comp_left]; rw [← Category.assoc]; rw [(inf_isPullback ((«exists» f).obj A') B').flip.lift_snd]
...
/--
theorem `exists_inf_pullback_eq_exists_inf` / 定理 `exists_inf_pullback_eq_exists_inf`
...
Depends on / 依赖: IsImage, IsImage.isoExt, IsImage.isoExt_hom_m, eq_of_comm, frobeniusStrongEpiMonoFactorisation, imageFactorisation, isImage, isoExt, isoExt_hom_m, toMonoIsImage
-/
theorem exists_inf_pullback_eq_exists_inf :
