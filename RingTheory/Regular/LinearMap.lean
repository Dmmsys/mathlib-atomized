...
/--
lemma `linearMap_subsingleton_of_mem_annihilator` / 引理 `linearMap_subsingleton_of_mem_annihilator`

English:
lemma linearMap_subsingleton_of_mem_annihilator
  statement: {r : R} (reg : IsSMulRegular M r)
  proof: by
  apply subsingleton_of_forall_eq 0 (fun f => ext fun x => ?_)
  have : r • (f x) = r • 0 := by
    rw [smul_zero]; rw [← map_smul]; rw [Module.mem_annihilator.mp mem_ann x]; rw [map_zero]
  simpa using reg this

中文:
引理 linearMap_subsingleton_of_mem_annihilator
  结论: {r : R} (reg : IsSMulRegular M r)
  证明: by
  apply subsingleton_of_forall_eq 0 (fun f => ext fun x => ?_)
  have : r • (f x) = r • 0 := by
    rw [smul_zero]; rw [← map_smul]; rw [Module.mem_annihilator.mp mem_ann x]; rw [map_zero]
...
Depends on / 依赖: Module, Module.mem_annihilator.mp, map_smul, map_zero, mem_ann, mem_annihilator, smul_zero, subsingleton_of_forall_eq
-/
lemma linearMap_subsingleton_of_mem_annihilator {r : R} (reg : IsSMulRegular M r)
    (mem_ann : r in Module.annihilator R N) : Subsingleton (N ->ₗ[R] M) := by
  apply subsingleton_of_forall_eq 0 (fun f => ext fun x => ?_)
  have : r • (f x) = r • 0 := by
    rw [smul_zero]; rw [← map_smul]; rw [Module.mem_annihilator.mp mem_ann x]; rw [map_zero]
  simpa using reg this
...
/--
lemma `subsingleton_linearMap_iff` / 引理 `subsingleton_linearMap_iff`

English:
lemma subsingleton_linearMap_iff
  given: [IsNoetherianRing R] [Module.Finite R M] [Module.Finite R N]
  proof: by
  ...
  · by_contra! h
    have hexist : exists p in associatedPrimes R M, 
...
Depends on / 依赖: Ideal.subset_union_prime_finite, IsSMulRegular, IsSMulRegular.zero, Module, Module.annihilator, Submodule, Submodule.zero_mem, annihilator, associatedPrimes, associatedPrimes.finite, associatedPrimes.nonempty, biUnion_associatedPrimes_eq_compl_regular, finite, hexist, linearMap_subsingleton_of_mem_annihilator, mem_ann, nonempty, subset_union_prime_finite, subsingleton_or_nontrivial, zero_mem
-/
lemma subsingleton_linearMap_iff [IsNoetherianRing R] [Module.Finite R M] [Module.Finite R N] :
    Subsingleton (N ->ₗ[R] M) ↔ exists r in Module.annihilator R N, IsSMulRegular M r := by
  refine ⟨fun hom0 => ?_, fun ⟨r, mem_ann, reg⟩ =>
    linearMap_subsingleton_of_mem_annihilator reg mem_ann⟩
  cases subsingleton_or_nontrivial M
  · exact ⟨0, ⟨Submodule.zero_mem (Module.annihilator R N), IsSMulRegular.zero⟩⟩
  · by_contra! h
    have hexist : exists p in associatedPrimes R M, Module.annihilator R N <= p := by
    ...
    have loc_ne_zero : p' in Module.support R N := Module.mem_support_iff_of_finite.mpr hp
