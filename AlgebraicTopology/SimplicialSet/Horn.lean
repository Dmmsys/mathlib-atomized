...
lemma horn_obj_zero
  ...
  simp only [horn_eq_iSup, Subfunctor.iSup_obj, Set.iUnion_coe_set,
    ...
    Nat.reduceAdd, Finset.mem_compl, Finset.mem_singleton, exists_prop, Set.top_eq
...
lemma subcomplex_le_horn_iff
  ...
  · replace h := h.trans hA
    ...
    apply h
    rw [stdSimplex.coe_asOrderHom_objEquiv_symm]; rw [SimplexCategory.coe_δ]; rw [Fin.range_succAbove]; rw [Set.compl_u
...
definition primitiveEdge
  ...
  obtain rfl | hn : n = 2 ∨ 2 < n := by
    rw [eq_comm]; rw [or_comm]; rw [← le_iff_lt_
