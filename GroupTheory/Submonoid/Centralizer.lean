...
theorem centralizer_toSubsemigroup
  statement: (centralizer S).toSubsemigroup = Subsemigroup.centralizer S
  proof: rfl
...
lemma le_centralizer_centralizer
  given: {s : Submonoid M}
  statement: s <= centralizer (centralizer (s : Set M))
...
commutative monoid. -/]
/--
Definition of `closureCommMonoidOfComm` / `closureCommMonoidOfComm` 的定义

English:
abbreviation closureCommMonoidOfComm
  signature: {s : Set M} (hcomm : forall a in s, forall b in s, a * b = b * a)
  body: haveI := isMulCommutative_closure _ hcomm
  inferInstance
...
-/
abbrev closureCommMonoidOfComm {s : Set M} (hcomm : forall a in s, forall b in s, a * b = b * a) :
