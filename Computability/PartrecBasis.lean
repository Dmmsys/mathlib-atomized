...
/-
**Nat.Partrec.to_part** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
theorem of_eq {n} {f g : List.Vector ℕ n →. ℕ} (hf : Partrec' f) (H : ∀ i, f i = g i) :
  ...
  (funext H : f = g) ▸ hf
/-
**Nat.Partrec.of_prim** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem of_prim {n} {f : List.Vector ℕ n → ℕ} (hf : Primrec f) : @Partrec' n f :=
  prim (Nat.Primrec'.of_prim hf)
/-
**Nat.Partrec.head** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.tail** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency.types false in
/-
**Nat.Partrec.bind** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.map** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.Vec** 是 Mathlib 中的一个定义，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Analogous to `Nat.Partrec'` for `ℕ`-valued functions, a predicate for partial re
cursive
  vector-valued functions.
-/
...
/-
**Nat.Partrec.nil** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected theorem nil {n} : @Vec n 0 fun _ => nil := fun i => i.elim0
/-
**Nat.Partrec.cons** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.idv** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency.types false in
/-
**Nat.Partrec.comp'** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem comp' {n m f g} (hf : @Partrec' m f) (hg : @Vec n m g) : Partrec' fun v => f (g v) :=
  (hf.comp _ hg).of_eq fun v => by simp
/-
**Nat.Partrec.comp** 是 Mathlib 中的一个ctor，位于命名空间 `Nat.Partrec`。
形式化陈述：∀ {f g : ℕ →. ℕ}, Nat.Partrec f → Nat.Partrec g → Nat.Partrec fun n => g n
 >>= f
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.rfindOpt** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.of_part** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.part_iff** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.Partrec.vec_iff** 是 Mathlib 中的一个定理，位于命名空间 `Nat.Partrec`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
