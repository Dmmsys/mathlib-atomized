...
variable {X M E : Type*}
  ...
  {p : Real>=0∞} [Fact (1 <= p)] [hp : Fact (p != ∞)]

@[to_additive]
/--
Instance `Lp.instContinuousSMulDomMulAct` / 实例 `Lp.instContinuousSMulDomMulAct`

English:
instance Lp.instContinuousSMulDomMulAct
  signature: : ContinuousSMul Mᵈᵐᵃ (Lp E p μ) where
  body: let g : C(Mᵈᵐᵃ × Lp E p μ, C(X, X)) :=
...
Depends on / 依赖: ContinuousMap, ContinuousMap.fst, ContinuousMap.mk, DomMulAct, DomMulAct.mk.symm, Fact.out, compMeasurePreservingLp, continuous, continuous_smul, continuous_snd, continuous_snd.compMeasurePreservingLp, curry.comp, g.continuous
-/
