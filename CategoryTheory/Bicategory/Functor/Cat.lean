/-
Copyright (c) 2025 Joël Riou. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Joël Riou, Christian Merten
-/
module

public import Mathlib.CategoryTheory.Bicategory.Functor.Pseudofunctor

/-!
# Pseudofunctors to Cat

In this file, we state naturality properties of `mapId'` and `mapComp'`
for pseudofunctors to `Cat`.

-/

public section

universe w v v' u u'

namespace CategoryTheory

open Bicategory

namespace Pseudofunctor

variable {B : Type u} [Bicategory.{w, v} B] (F : B ⥤ᵖ Cat.{v', u'})

section naturality

variable {b₀ b₁ b₂ : B} {X Y : F.obj b₀}

section

variable (f : b₀ ⟶ b₀) (hf : f = 𝟙 b₀) (a : X ⟶ Y)

#adaptation_note
/-- `respectTransparency.types true` changes the auto-generated lemmas' signature -/
set_option backward.isDefEq.respectTransparency.types false in
@[reassoc]
/--
lemma `mapId'_hom_naturality` / 引理 `mapId'_hom_naturality`

English:
lemma mapId'_hom_naturality
  proof: (F.mapId' f hf).hom.toNatTrans.naturality a

#adaptation_note

中文:
引理 mapId'_hom_naturality
  证明: (F.mapId' f hf).hom.toNatTrans.naturality a

#adaptation_note

Depends on / 依赖: F.mapId, hom.toNatTrans.naturality, naturality, toNatTrans
-/
lemma mapId'_hom_naturality :
    (F.map f).toFunctor.map a ≫ (F.mapId' f hf).hom.toNatTrans.app Y =
    (F.mapId' f hf).hom.toNatTrans.app X ≫ a :=
  (F.mapId' f hf).hom.toNatTrans.naturality a

#adaptation_note
/-- `respectTransparency.types true` changes the auto-generated lemmas' signature -/
set_option backward.isDefEq.respectTransparency.types false in
@[reassoc]
/--
lemma `mapId'_inv_naturality` / 引理 `mapId'_inv_naturality`

English:
lemma mapId'_inv_naturality
  proof: ((F.mapId' f hf).inv.toNatTrans.naturality a).symm

中文:
引理 mapId'_inv_naturality
  证明: ((F.mapId' f hf).inv.toNatTrans.naturality a).symm
-/
lemma mapId'_inv_naturality :
    (F.mapId' f hf).inv.toNatTrans.app X ≫ (F.map f).toFunctor.map a =
    a ≫ (F.mapId' f hf).inv.toNatTrans.app Y :=
  ((F.mapId' f hf).inv.toNatTrans.naturality a).symm

end

section

variable (f : b₀ ⟶ b₁) (g : b₁ ⟶ b₂) (fg : b₀ ⟶ b₂)
  (hfg : f ≫ g = fg) (a : X ⟶ Y)

#adaptation_note
/-- `respectTransparency.types true` changes the auto-generated lemmas' signature -/
set_option backward.isDefEq.respectTransparency.types false in
@[reassoc]
/--
lemma `mapComp'_hom_naturality` / 引理 `mapComp'_hom_naturality`

English:
lemma mapComp'_hom_naturality
  proof: (F.mapComp' f g fg hfg).hom.toNatTrans.naturality a

中文:
引理 mapComp'_hom_naturality
  证明: (F.mapComp' f g fg hfg).hom.toNatTrans.naturality a

Depends on / 依赖: F.mapComp, hom.toNatTrans.naturality, mapComp, naturality, toNatTrans
-/
lemma mapComp'_hom_naturality :
    (F.map fg).toFunctor.map a ≫ (F.mapComp' f g fg hfg).hom.toNatTrans.app Y =
    (F.mapComp' f g fg hfg).hom.toNatTrans.app X ≫ (F.map g).toFunctor.map
      ((F.map f).toFunctor.map a) :=
  (F.mapComp' f g fg hfg).hom.toNatTrans.naturality a

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in -- Needed in Sites/Descent/IsPrestack.lean
@[reassoc (attr := simp)]
/--
lemma `mapComp'_inv_naturality` / 引理 `mapComp'_inv_naturality`

English:
lemma mapComp'_inv_naturality
  proof: (F.mapComp' f g fg hfg).inv.toNatTrans.naturality a

@[reassoc]

中文:
引理 mapComp'_inv_naturality
  证明: (F.mapComp' f g fg hfg).inv.toNatTrans.naturality a

@[reassoc]
-/
lemma mapComp'_inv_naturality :
    (F.map g).toFunctor.map ((F.map f).toFunctor.map a) ≫
    (F.mapComp' f g fg hfg).inv.toNatTrans.app Y =
    (F.mapComp' f g fg hfg).inv.toNatTrans.app X ≫ (F.map fg).toFunctor.map a :=
  (F.mapComp' f g fg hfg).inv.toNatTrans.naturality a

@[reassoc]
/--
lemma `mapComp'_naturality_1` / 引理 `mapComp'_naturality_1`

English:
lemma mapComp'_naturality_1
  proof: NatIso.naturality_1 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a

@[reassoc]

中文:
引理 mapComp'_naturality_1
  证明: NatIso.naturality_1 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a

@[reassoc]
-/
lemma mapComp'_naturality_1 :
    (F.mapComp' f g fg hfg).inv.toNatTrans.app X ≫
      (F.map fg).toFunctor.map a ≫ (F.mapComp' f g fg hfg).hom.toNatTrans.app Y =
    (F.map g).toFunctor.map ((F.map f).toFunctor.map a) :=
  NatIso.naturality_1 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a

@[reassoc]
/--
lemma `mapComp'_naturality_2` / 引理 `mapComp'_naturality_2`

English:
lemma mapComp'_naturality_2
  proof: NatIso.naturality_2 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a

中文:
引理 mapComp'_naturality_2
  证明: NatIso.naturality_2 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a
-/
lemma mapComp'_naturality_2 :
    (F.mapComp' f g fg hfg).hom.toNatTrans.app X ≫
      (F.map g).toFunctor.map ((F.map f).toFunctor.map a) ≫
        (F.mapComp' f g fg hfg).inv.toNatTrans.app Y =
    (F.map fg).toFunctor.map a :=
  NatIso.naturality_2 (Cat.Hom.toNatIso (F.mapComp' f g fg hfg)) a

end

end naturality

end Pseudofunctor

end CategoryTheory
