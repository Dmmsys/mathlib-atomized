...
abbreviation pullback
  signature: (f : CompactlySupportedContinuousMap B E) (b : B)
  body: f.pullback_monoidHom H.isClosedEmbedding b
...
Depends on / 依赖: H.isClosedEmbedding, f.pullback_monoidHom, isClosedEmbedding, pullback_monoidHom
-/
...
@[to_additive]
/--
theorem `pullback_def` / 定理 `pullback_def`

English:
theorem pullback_def
  given: (f : CompactlySupportedContinuousMap B E) (b : B) (a : A)
  proof: f.pullback_monoidHom_def H.isClosedEmbedding b a
...
@[to_additive]
/--
theorem `integral_pullback_invFun_apply` / 定理 `integral_pullback_invFun_apply`

English:
theorem integral_pullback_invFun_apply
  given: (f : CompactlySupportedContinuousMap B E) (b : B)
  proof: by
...
Depends on / 依赖: Function, Function.apply_invFun_apply, Function.invFun, H.mulExact.monoidHom_ker_eq, apply_invFun_apply, integral_mul_left_eq_self, invFun, mem_ker, monoidHom_ker_eq, mulExact, mul_assoc, pullback_def
-/
...
definition pushforward
  signature: :
  body: { toFun := fun c => ∫ a, pullback H f (Function.invFun ψ c) a ∂μA
...
定义 pushforward
  签名: :
  定义体: { toFun := fun c => ∫ a, pullback H f (Function.invFun ψ c) a ∂μA
    hasCompactSupport' := by
      obtain ⟨K, hK, hf⟩ := exists_compact_iff_hasCompactSupport.mpr f.hasCompactSupport
      refine exists_compact_iff_hasCompactSupport.mp
        ⟨ψ '' K, hK.image H.isOpenQuotientMap.continuous, fun x hx => ?_⟩
...
noncomputable def pushforward :
    CompactlySupportedContinuousMap B E ->ₗ[Real] CompactlySupportedContinuousMap C E where
  toFun f :=
...
@[to_additive]
/--
theorem `pushforward_def` / 定理 `pushforward_def`

English:
theorem pushforward_def
  given: (f : CompactlySupportedContinuousMap B E) (c : C)
  proof: rfl
...
@[to_additive]
/--
theorem `pushforward_apply_apply` / 定理 `pushforward_apply_apply`

English:
theorem pushforward_apply_apply
  given: (f : CompactlySupportedContinuousMap B E) (b : B)
  proof: integral_pullback_invFun_apply H μA f b
...
Depends on / 依赖: integral_pullback_invFun_apply
-/
...
@[to_additive]
/--
theorem `pushforward_mono` / 定理 `pushforward_mono`

English:
theorem pushforward_mono
  given: {f g : CompactlySupportedContinuousMap B Real} (h : f <= g)
  proof: fun _ => integral_mono (pullback H f _).integrable (pullback H g _).integrable (fun _ => h _)
...
-/
theorem pushforward_mono {f g : CompactlySupportedContinuousMap B Real} (h : f <= g) :
...
/--
Definition of `integrate` / `integrate` 的定义

English:
definition integrate
  signature: : CompactlySupportedContinuousMap B E ->ₗ[Real] E where
  body: ∫ c, pushforward H μA f c ∂μC
  map_add' f g := by
    rw [map_add]
...
-/
noncomputable def integrate : CompactlySupportedContinuousMap B E ->ₗ[Real] E where
...
@[to_additive]
/--
theorem `integrate_apply` / 定理 `integrate_apply`

English:
theorem integrate_apply
  given: (f : CompactlySupportedContinuousMap B E)
  proof: rfl
...
@[to_additive]
/--
theorem `integrate_mono` / 定理 `integrate_mono`

English:
theorem integrate_mono
  given: {f g : CompactlySupportedContinuousMap B Real} (h : f <= g)
  proof: integral_mono (pushforward H μA f).integrable (pushforward H μA g).integrable
...
Depends on / 依赖: integrable, integral_mono, pushforward, pushforward_mono
-/
theorem integrate_mono {f g : CompactlySupportedContinuousMap B Real} (h : f <= g) :
...
and `C`. -/]
/--
Definition of `inducedMeasure` / `inducedMeasure` 的定义

English:
definition inducedMeasure
  signature: : Measure B
  body: RealRMK.rieszMeasure ⟨integrate H μA μC, fun _ _ => integrate_mono H μA μC⟩
...
Depends on / 依赖: RealRMK, RealRMK.rieszMeasure, integrate, integrate_mono, rieszMeasure
-/
...
@[to_additive]
/--
Instance `inducedMeasure_regular` / 实例 `inducedMeasure_regular`

English:
instance inducedMeasure_regular
  signature: : (inducedMeasure H μA μC).Regular
  body: RealRMK.regular_rieszMeasure _
...
Depends on / 依赖: RealRMK, RealRMK.regular_rieszMeasure, regular_rieszMeasure
-/
...
@[to_additive]
/--
theorem `integral_inducedMeasure` / 定理 `integral_inducedMeasure`

English:
theorem integral_inducedMeasure
  given: (f : CompactlySupportedContinuousMap B Real)
  proof: by
...
Depends on / 依赖: RealRMK, RealRMK.integral_rieszMeasure, integral_rieszMeasure
-/
theorem integral_inducedMeasure (f : CompactlySupportedContinuousMap B Real) :
    ∫ b : B, f b ∂(inducedMeasure H μA μC) = integrate H μA μC f := by
  apply RealRMK.integral_rieszMeasure

@[to_additive]
/--
Instance `isHaarMeasure_inducedMeasure` / 实例 `isHaarMeasure_inducedMeasure`

English:
instance isHaarMeasure_inducedMeasure
  signature: : IsHaarMeasure (inducedMeasure H μA μC) where
  body: by
...
/--
theorem `inducedMeasure_lt_of_injOn` / 定理 `inducedMeasure_lt_of_injOn`

English:
theorem inducedMeasure_lt_of_injOn
  statement: {U : Set B} (hU : IsOpen U) [DiscreteTopology A]
  proof: by
  contrapose! h
