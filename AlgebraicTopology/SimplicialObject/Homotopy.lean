...
structure Homotopy
  axioms and operations (8):
    - h({n : Nat} (i : Fin (n + 1))) : X _⦋n⦌ ⟶ Y _⦋n + 1⦌
    - h_zero_comp_δ_zero((n : Nat)) : h 0 ≫ Y.δ 0 = g.app (op ⦋n⦌)
    - h_last_comp_δ_last((n : Nat)) : h (Fin.last n) ≫ Y.δ (Fin.last (n + 1)) = f.app (op ⦋n⦌)
    - h_succ_comp_δ_castSucc_of_lt({n : Nat} (i : Fin (n + 2)) (j : Fin (n + 1)) (hij : i <= j.castSucc)) : h j.succ ≫ Y.δ i.castSucc = X.δ i ≫ h j
    - h_succ_comp_δ_castSucc_succ({n : Nat} (j : Fin (n + 1))) : h j.succ ≫ Y.δ j.castSucc.succ = h j.castSucc ≫ Y.δ j.castSucc.succ
    - h_castSucc_comp_δ_succ_of_lt({n : Nat} (i : Fin (n + 2)) (j : Fin (n + 1)) (hji : j.castSucc < i)) : h j.castSucc ≫ Y.δ i.succ = X.δ i ≫ h j
    - h_comp_σ_castSucc_of_le({n : Nat} (i j : Fin (n + 1)) (hij : i <= j)) : h j ≫ Y.σ i.castSucc = X.σ i ≫ h j.succ
...
structure Homotopy
  ...
  h_zero_comp_δ_zero (n : Nat) : h 0 ≫ Y.δ 0 = g.app (op ⦋n⦌)
  h_last_comp_δ_last (n : Nat) : h (Fin.last n) ≫ Y.δ (Fin.last (n + 1)) = f.app (op ⦋n⦌)
  h_succ_comp_δ_castSucc_of_lt {n : Nat} (i : Fin (n + 2)) (j : Fin (n + 1)) (hij : i <= j.castSucc) :
    h j.succ ≫ Y.δ i.castSucc = X.δ i ≫ h j
  h_succ_comp_δ_castSucc_succ {n : Nat} (j : Fin (n + 1)) :
    h j.succ ≫ Y.δ j.castSucc.succ = h j.castSucc ≫ Y.δ j.castSucc.succ
  h_castSucc_comp_δ_succ_of_lt {n : Nat} (i : Fin (n + 2)) (j : Fin (n + 1)) (hji : j.castSucc < i) :
    h j.castSucc ≫ Y.δ i.succ = X.δ i ≫ h j
  h_comp_σ_castSucc_of_le {n : Nat} (i j : Fin (n + 1)) (hij : i <= j) :
    h j ≫ Y.σ i.castSucc = X.σ i ≫ h j.succ
  h_comp_σ_succ_of_lt {n : Nat} (i j : Fin (n + 1)) (hji : j <= i) :
...
definition whiskerRight
  signature: (H : Homotopy f g) {D : Type u'} [Category.{v'} D] (F : C ⥤ D)
  body: F.map (H.h i)
  h_zero_comp_δ_zero n := by
...
English:
definition postcomp
  signature: (H : Homotopy f g) {Y' : SimplicialObject C} (p : Y ⟶ Y')
  body: H.h i ≫ p.app _
  h_zero_comp_δ_zero n := by
...
English:
definition precomp
  signature: (H : Homotopy f g) {X' : SimplicialObject C} (p : X' ⟶ X)
  body: p.app _ ≫ H.h i
  h_zero_comp_δ_zero n := by
