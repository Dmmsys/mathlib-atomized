...
中文:
结构 ConvexBody
  参数: (V : 类型) [TopologicalSpace V] [AddCommMonoid V] [SMul 实数 V]
  公理与运算 (4 个):
    - carrier : Set V
    - convex' : Convex 实数 carrier
    - isCompact' : IsCompact carrier
    - nonempty' : carrier.Nonempty
-/
...
中文:
定理 ext
  条件: {K L : ConvexBody V} (h : (K : Set V) = L)
  结论: K = L
  证明: SetLike.ext' h
...
定理 zero_mem_of_symmetric
  条件: (K : ConvexBody V) (h_symm : 对任意 x in K, -x in K)
  结论: 0 in K
  证明: by
  obtain ⟨x, hx⟩ := K.nonempty
  rw [show 0 = (1 / 2 : Real) • x + (1 / 2 : Real) • (-x) by simp]
...
中文:
定理 coe_nsmul
  结论: 对任意 (n : 自然数) (K : ConvexBody V), ↑(n • K) = n • (K : Set V)
-/
...
中文:
定理 coe_smul
  条件: (c : 实数) (K : ConvexBody V)
  结论: (↑(c • K) : Set V) = c • (K : Set V)
  证明: rfl
...
中文:
定理 smul_le_of_le
  条件: (K : ConvexBody V) (h_zero : 0 in K) {a b : 实数>=0} (h : a <= b)
  证明: by
  rw [← SetLike.coe_subset_coe]; rw [coe_smul']; rw [coe_smul']
  obtain rfl | ha := eq_zero_or_pos a
  · rw [Set.zero_smul_set K.nonempty, Set.zero_subset]
...
中文:
实例 :
  签名: DistribMulAction 实数 (ConvexBody V)
...
中文:
定理 isBounded
  结论: Bornology.IsBounded (K : Set V)
...
定理 hausdorffEDist_ne_top
  条件: {K L : ConvexBody V}
  结论: Metric.hausdorffEDist (K : Set V) L != ⊤
  证明: by
  apply_rules [Metric.hausdorffEDist_ne_top_of_nonempty_of_bounded, ConvexBody.nonempty,
    ConvexBody.isBounded]
...
中文:
实例 :
  签名: PseudoMetricSpace (ConvexBody V)
...
定理 hausdorffDist_coe
  结论: Metric.hausdorffDist (K : Set V) L = dist K L
  证明: rfl
...
定理 hausdorffEDist_coe
  结论: Metric.hausdorffEDist (K : Set V) L = edist K L
  证明: by
  rw [edist_dist]
  exact (ENNReal.ofReal_toReal hausdorffEDist_ne_top).symm
...
中文:
定理 iInter_smul_eq_self
  结论: [T2Space V] {u : 自然数 -> 实数>=0} (K : ConvexBody V) (h_zero : 0 in K)
  证明: by
  ext x
  refine ⟨fun h => ?_, fun h => ?_⟩
  · obtain ⟨C, hC_pos, hC_bdd⟩ := K.isBounded.exists_pos_norm_le
