...
variable {C D : Type _} [Category* C] [Category* D]
  ...
  (s : A -> D ⥤ D) (i : forall a, F ⋙ s a ≅ shiftFunctor C a ⋙ F)
...
definition zero
  signature: : s 0 ≅ 𝟭 D
  body: ((whiskeringLeft C D D).obj F).preimageIso ((i 0) ≪≫
    isoWhiskerRight (shiftFunctorZero C A) F ≪≫ F.leftUnitor ≪≫ F.rightUnitor.symm)

中文:
定义 zero
  签名: : s 0 ≅ 𝟭 D
  定义体: ((whiskeringLeft C D D).obj F).preimageIso ((i 0) ≪≫
    isoWhiskerRight (shiftFunctorZero C A) F ≪≫ F.leftUnitor ≪≫ F.rightUnitor.symm)

Depends on / 依赖: F.leftUnitor, F.rightUnitor.symm, isoWhiskerRight, leftUnitor, preimageIso, rightUnitor, shiftFunctorZero, whiskeringLeft
-/
...
definition add
  signature: (a b : A)
  body: ((whiskeringLeft C D D).obj F).preimageIso
...
定义 add
  签名: (a b : A)
  定义体: ((whiskeringLeft C D D).obj F).preimageIso
    (i (a + b) ≪≫ isoWhiskerRight (shiftFunctorAdd C a b) F ≪≫
      Functor.associator _ _ _ ≪≫
        isoWhiskerLeft _ (i b).symm ≪≫ (Functor.associator _ _ _).symm ≪≫
        isoWhiskerRight (i a).symm _ ≪≫ Functor.associator _ _ _)

Depends on / 依赖: Functor, Functor.associator, associator, isoWhiskerLeft, isoWhiskerRight, preimageIso, shiftFunctorAdd, whiskeringLeft
-/
...
@[simp]
/--
lemma `zero_hom_app_obj` / 引理 `zero_hom_app_obj`

English:
lemma zero_hom_app_obj
  given: (X : C)
  proof: by
...
Depends on / 依赖: NatTrans, NatTrans.congr_app, congr_app, map_preimage, whiskerLeft, whiskeringLeft
-/
...
@[simp]
/--
lemma `zero_inv_app_obj` / 引理 `zero_inv_app_obj`

English:
lemma zero_inv_app_obj
  given: (X : C)
  proof: by
...
@[simp]
/--
lemma `add_hom_app_obj` / 引理 `add_hom_app_obj`

English:
lemma add_hom_app_obj
  given: (a b : A) (X : C)
  proof: by
...
@[simp]
/--
lemma `add_inv_app_obj` / 引理 `add_inv_app_obj`

English:
lemma add_inv_app_obj
  given: (a b : A) (X : C)
  proof: by
...
/--
lemma `shiftFunctor_of_induced` / 引理 `shiftFunctor_of_induced`

English:
lemma shiftFunctor_of_induced
  given: (a : A)
  proof: HasShift.induced F A s i
    shiftFunctor D a = s a :=
  rfl

中文:
引理 shiftFunctor_of_induced
  条件: (a : A)
...
@[simp]
/--
lemma `shiftFunctorZero_hom_app_obj_of_induced` / 引理 `shiftFunctorZero_hom_app_obj_of_induced`

English:
lemma shiftFunctorZero_hom_app_obj_of_induced
  given: (X : C)
  proof: HasShift.induced F A s i
...
中文:
引理 shiftFunctorZero_hom_app_obj_of_induced
  条件: (X : C)
  证明: HasShift.induced F A s i
    (shiftFunctorZero D A).hom.app (F.obj X) =
      (i 0).hom.app X ≫ F.map ((shiftFunctorZero C A).hom.app X) := by
...
@[simp]
/--
lemma `shiftFunctorZero_inv_app_obj_of_induced` / 引理 `shiftFunctorZero_inv_app_obj_of_induced`

English:
lemma shiftFunctorZero_inv_app_obj_of_induced
  given: (X : C)
  proof: HasShift.induced F A s i
...
中文:
引理 shiftFunctorZero_inv_app_obj_of_induced
  条件: (X : C)
  证明: HasShift.induced F A s i
    (shiftFunctorZero D A).inv.app (F.obj X) =
      F.map ((shiftFunctorZero C A).inv.app X) ≫ (i 0).inv.app X := by
...
@[simp]
/--
lemma `shiftFunctorAdd_hom_app_obj_of_induced` / 引理 `shiftFunctorAdd_hom_app_obj_of_induced`

English:
lemma shiftFunctorAdd_hom_app_obj_of_induced
  given: (a b : A) (X : C)
  proof: HasShift.induced F A s i
...
@[simp]
/--
lemma `shiftFunctorAdd_inv_app_obj_of_induced` / 引理 `shiftFunctorAdd_inv_app_obj_of_induced`

English:
lemma shiftFunctorAdd_inv_app_obj_of_induced
  given: (a b : A) (X : C)
  proof: HasShift.induced F A s i
...
@[instance_reducible]
/--
Definition of `Functor.CommShift.ofInduced` / `Functor.CommShift.ofInduced` 的定义

English:
definition Functor.CommShift.ofInduced
  signature: :
  body: HasShift.induced F A s i
    F.CommShift A := by
  letI := HasShift.induced F A s i
  exact
...
/--
lemma `Functor.commShiftIso_eq_ofInduced` / 引理 `Functor.commShiftIso_eq_ofInduced`

English:
lemma Functor.commShiftIso_eq_ofInduced
  given: (a : A)
  proof: HasShift.induced F A s i
