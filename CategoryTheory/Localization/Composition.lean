...
/--
Definition of `StrictUniversalPropertyFixedTarget.comp` / `StrictUniversalPropertyFixedTarget.comp` 的定义

English:
definition StrictUniversalPropertyFixedTarget.comp
  body: hW₃
...
中文:
定义 StrictUniversalPropertyFixedTarget.comp
  定义体: hW₃
  lift F hF := h₂.lift (h₁.lift F (MorphismProperty.IsInvertedBy.of_le _ _ F hF hW₁₃)) (by
    refine MorphismProperty.IsInvertedBy.of_le _ _ _ ?_ hW₂₃
    simpa only [MorphismProperty.IsInvertedBy.map_iff, h₁.fac F] using hF)
...
English:
lemma comp
  statement: [L₁.IsLocalization W₁] [L₂.IsLocalization W₂]
  proof: by
...
English:
lemma of_comp
  statement: (W₃ : MorphismProperty C₁)
  proof: by
...
Depends on / 依赖: Functor, Functor.associator, IsLocalization, IsLocalization.of_equivalence_target, Localization, Localization.compUniqFunctor, Localization.inverts, Localization.uniq, associator, compUniqFunctor, inverts, liftNatIso, map_mem_map, of_equivalence_target
-/
