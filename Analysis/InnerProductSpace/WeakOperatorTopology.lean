/-
Copyright (c) 2024 Frédéric Dupuis. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Frédéric Dupuis
-/
module

public import Mathlib.Algebra.Star.TransferInstance
public import Mathlib.Analysis.InnerProductSpace.Adjoint
public import Mathlib.Analysis.InnerProductSpace.Dual
public import Mathlib.Analysis.LocallyConvex.WeakOperatorTopology

/-!
# The weak operator topology in Hilbert spaces

This file gives a few properties of the weak operator topology that are specific to operators on
Hilbert spaces. This mostly involves using the Fréchet-Riesz representation to convert between
applications of elements of the dual and inner products with vectors in the space.

## Main results

+ `ContinuousLinearMapWOT.tendsto_iff_forall_inner_apply_tendsto`: a function `f : α → E →WOT[𝕜] F`
  tends to `𝓝 A` if and only if `fun a ↦ ⟪y, (f a) x⟫_𝕜` tends to `𝓝 ⟪y, A x⟫_𝕜` for all
  `x : E`, `y : F`. Also included are the corresponding characterizations of continuity.
+ The adjoint operation is continuous in the weak operator topology, declared as an instance of
  `ContinuousStar (F →WOT[𝕜] F)`.
-/

public section

open scoped Topology InnerProductSpace

namespace ContinuousLinearMapWOT

variable {𝕜 : Type*} {E : Type*} {F : Type*} [RCLike 𝕜] [AddCommGroup E] [TopologicalSpace E]
  [Module 𝕜 E] [NormedAddCommGroup F] [InnerProductSpace 𝕜 F]

@[ext]
/--
lemma `ext_inner` / 引理 `ext_inner`

English:
lemma ext_inner
  given: {A B : E ->WOT[𝕜] F} (h : forall x y, ⟪y, A x⟫_𝕜 = ⟪y, B x⟫_𝕜)
  statement: A = B
  proof: by
  rw [ContinuousLinearMapWOT.ext_iff]
  exact fun x => ext_inner_left 𝕜 fun y => h x y

中文:
引理 ext_inner
  条件: {A B : E ->WOT[𝕜] F} (h : 对任意 x y, ⟪y, A x⟫_𝕜 = ⟪y, B x⟫_𝕜)
  结论: A = B
  证明: by
  rw [ContinuousLinearMapWOT.ext_iff]
  exact fun x => ext_inner_left 𝕜 fun y => h x y

Depends on / 依赖: ContinuousLinearMapWOT, ContinuousLinearMapWOT.ext_iff, ext_iff, ext_inner_left
-/
lemma ext_inner {A B : E ->WOT[𝕜] F} (h : forall x y, ⟪y, A x⟫_𝕜 = ⟪y, B x⟫_𝕜) : A = B := by
  rw [ContinuousLinearMapWOT.ext_iff]
  exact fun x => ext_inner_left 𝕜 fun y => h x y

variable [CompleteSpace F]

open Filter in
/--
lemma `tendsto_iff_forall_inner_apply_tendsto` / 引理 `tendsto_iff_forall_inner_apply_tendsto`

English:
lemma tendsto_iff_forall_inner_apply_tendsto
  statement: {α : Type*} {l : Filter α}
  proof: by
  simp_rw [tendsto_iff_forall_dual_apply_tendsto]
exact .symm forall_congr' fun _ =>
    Equiv.forall_congr (InnerProductSpace.toDual 𝕜 F) fun _ => Iff.rfl

中文:
引理 tendsto_iff_对任意_inner_apply_tendsto
  结论: {α : 类型} {l : 滤子 α}
  证明: by
  simp_rw [tendsto_iff_forall_dual_apply_tendsto]
exact .symm forall_congr' fun _ =>
    Equiv.forall_congr (InnerProductSpace.toDual 𝕜 F) fun _ => Iff.rfl

Depends on / 依赖: Equiv.forall_congr, Iff.rfl, InnerProductSpace, InnerProductSpace.toDual, forall_congr, simp_rw, tendsto_iff_forall_dual_apply_tendsto, toDual
-/
lemma tendsto_iff_forall_inner_apply_tendsto {α : Type*} {l : Filter α}
    {f : α -> E ->WOT[𝕜] F} {A : E ->WOT[𝕜] F} :
    Tendsto f l (𝓝 A) ↔ forall x y, Tendsto (fun a => ⟪y, (f a) x⟫_𝕜) l (𝓝 ⟪y, A x⟫_𝕜) := by
  simp_rw [tendsto_iff_forall_dual_apply_tendsto]
exact .symm forall_congr' fun _ =>
    Equiv.forall_congr (InnerProductSpace.toDual 𝕜 F) fun _ => Iff.rfl

/--
lemma `le_nhds_iff_forall_inner_apply_le_nhds` / 引理 `le_nhds_iff_forall_inner_apply_le_nhds`

English:
lemma le_nhds_iff_forall_inner_apply_le_nhds
  statement: {l : Filter (E ->WOT[𝕜] F)}
  proof: tendsto_iff_forall_inner_apply_tendsto (f := id)

中文:
引理 le_nhds_iff_对任意_inner_apply_le_nhds
  结论: {l : 滤子 (E ->WOT[𝕜] F)}
  证明: tendsto_iff_forall_inner_apply_tendsto (f := id)

Depends on / 依赖: tendsto_iff_forall_inner_apply_tendsto
-/
lemma le_nhds_iff_forall_inner_apply_le_nhds {l : Filter (E ->WOT[𝕜] F)}
    {A : E ->WOT[𝕜] F} : l <= 𝓝 A ↔ forall x y, l.map (fun T => ⟪y, T x⟫_𝕜) <= 𝓝 (⟪y, A x⟫_𝕜) :=
  tendsto_iff_forall_inner_apply_tendsto (f := id)

/--
lemma `continuousWithinAt_iff` / 引理 `continuousWithinAt_iff`

English:
lemma continuousWithinAt_iff
  statement: {α : Type*} [TopologicalSpace α]
  proof: tendsto_iff_forall_inner_apply_tendsto

中文:
引理 continuousWithinAt_iff
  结论: {α : 类型} [拓扑空间 α]
  证明: tendsto_iff_forall_inner_apply_tendsto

Depends on / 依赖: tendsto_iff_forall_inner_apply_tendsto
-/
lemma continuousWithinAt_iff {α : Type*} [TopologicalSpace α]
    {f : α -> E ->WOT[𝕜] F} {s : Set α} {a : α} :
    ContinuousWithinAt f s a ↔ forall x y, ContinuousWithinAt (⟪y, f · x⟫_𝕜) s a :=
  tendsto_iff_forall_inner_apply_tendsto

/--
lemma `continuousAt_iff` / 引理 `continuousAt_iff`

English:
lemma continuousAt_iff
  given: {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F} {a : α}
  proof: tendsto_iff_forall_inner_apply_tendsto

中文:
引理 continuousAt_iff
  条件: {α : 类型} [拓扑空间 α] {f : α -> E ->WOT[𝕜] F} {a : α}
  证明: tendsto_iff_forall_inner_apply_tendsto

Depends on / 依赖: tendsto_iff_forall_inner_apply_tendsto
-/
lemma continuousAt_iff {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F} {a : α} :
    ContinuousAt f a ↔ forall x y, ContinuousAt (⟪y, f · x⟫_𝕜) a :=
  tendsto_iff_forall_inner_apply_tendsto

/--
lemma `continuousOn_iff` / 引理 `continuousOn_iff`

English:
lemma continuousOn_iff
  given: {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F} {s : Set α}
  proof: by
  simp_rw [ContinuousOn, forall_comm (α := E), forall_comm (α := F), continuousWithinAt_iff]

中文:
引理 continuousOn_iff
  条件: {α : 类型} [拓扑空间 α] {f : α -> E ->WOT[𝕜] F} {s : 集合 α}
  证明: by
  simp_rw [ContinuousOn, forall_comm (α := E), forall_comm (α := F), continuousWithinAt_iff]

Depends on / 依赖: ContinuousOn, continuousWithinAt_iff, forall_comm, simp_rw
-/
lemma continuousOn_iff {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F} {s : Set α} :
    ContinuousOn f s ↔ forall x y, ContinuousOn (⟪y, f · x⟫_𝕜) s := by
  simp_rw [ContinuousOn, forall_comm (α := E), forall_comm (α := F), continuousWithinAt_iff]

/--
lemma `continuous_iff` / 引理 `continuous_iff`

English:
lemma continuous_iff
  given: {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F}
  proof: by
  simp_rw [continuous_iff_continuousAt, forall_comm (α := E), forall_comm (α := F),
    continuousAt_iff]

@[fun_prop] alias ⟨continuousWithinAt_inner_apply, continuousWithinAt⟩ := continuousWithinAt_iff
@[fun_prop] alias ⟨continuousOn_inner_apply, continuousOn⟩ := continuousOn_iff
@[fun_prop] al

中文:
引理 continuous_iff
  条件: {α : 类型} [拓扑空间 α] {f : α -> E ->WOT[𝕜] F}
  证明: by
  simp_rw [continuous_iff_continuousAt, forall_comm (α := E), forall_comm (α := F),
    continuousAt_iff]

@[fun_prop] alias ⟨continuousWithinAt_inner_apply, continuousWithinAt⟩ := continuousWithinAt_iff
@[fun_prop] alias ⟨continuousOn_inner_apply, continuousOn⟩ := continuousOn_iff
@[fun_prop] al

Depends on / 依赖: continuousAt_iff, continuous_iff_continuousAt, forall_comm, simp_rw
-/
lemma continuous_iff {α : Type*} [TopologicalSpace α] {f : α -> E ->WOT[𝕜] F} :
    Continuous f ↔ forall x y, Continuous (⟪y, f · x⟫_𝕜) := by
  simp_rw [continuous_iff_continuousAt, forall_comm (α := E), forall_comm (α := F),
    continuousAt_iff]

@[fun_prop] alias ⟨continuousWithinAt_inner_apply, continuousWithinAt⟩ := continuousWithinAt_iff
@[fun_prop] alias ⟨continuousOn_inner_apply, continuousOn⟩ := continuousOn_iff
@[fun_prop] alias ⟨continuousAt_inner_apply, continuousAt⟩ := continuousAt_iff
@[fun_prop] alias ⟨continuous_inner_apply, continuous⟩ := continuous_iff

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: StarRing (F ->WOT[𝕜] F)
  body: equiv.starRing

中文:
实例 :
  签名: 对合环 (F ->WOT[𝕜] F)
  定义体: equiv.starRing

Depends on / 依赖: equiv.starRing, starRing
-/
noncomputable instance : StarRing (F ->WOT[𝕜] F) := equiv.starRing

/--
lemma `star_apply` / 引理 `star_apply`

English:
lemma star_apply
  given: (A : F ->WOT[𝕜] F) (x : F)
  statement: star A x = star (toCLM A) x
  proof: rfl

中文:
引理 star_apply
  条件: (A : F ->WOT[𝕜] F) (x : F)
  结论: star A x = star (toCLM A) x
  证明: rfl
-/
lemma star_apply (A : F ->WOT[𝕜] F) (x : F) : star A x = star (toCLM A) x := rfl

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: StarModule 𝕜 (F ->WOT[𝕜] F)
  body: equiv.starModule 𝕜

中文:
实例 :
  签名: 对合模 𝕜 (F ->WOT[𝕜] F)
  定义体: equiv.starModule 𝕜

Depends on / 依赖: equiv.starModule, starModule
-/
instance : StarModule 𝕜 (F ->WOT[𝕜] F) := equiv.starModule 𝕜

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: ContinuousStar (F ->WOT[𝕜] F)
  body: by
    simp_rw [continuous_iff, star_apply, ContinuousLinearMap.star_eq_adjoint,
      ContinuousLinearMap.adjoint_inner_right, coe_toCLM]
    rw [forall_comm]
    conv in ⟪_, _⟫_𝕜 => rw [← inner_conj_symm, ← RCLike.star_def]
    fun_prop

中文:
实例 :
  签名: 余ntinuousStar (F ->WOT[𝕜] F)
  定义体: by
    simp_rw [continuous_iff, star_apply, ContinuousLinearMap.star_eq_adjoint,
      ContinuousLinearMap.adjoint_inner_right, coe_toCLM]
    rw [forall_comm]
    conv in ⟪_, _⟫_𝕜 => rw [← inner_conj_symm, ← RCLike.star_def]
    fun_prop

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.adjoint_inner_right, ContinuousLinearMap.star_eq_adjoint, RCLike, RCLike.star_def, adjoint_inner_right, coe_toCLM, continuous_iff, forall_comm, fun_prop, inner_conj_symm, simp_rw, star_apply, star_def, star_eq_adjoint
-/
instance : ContinuousStar (F ->WOT[𝕜] F) where
  continuous_star := by
    simp_rw [continuous_iff, star_apply, ContinuousLinearMap.star_eq_adjoint,
      ContinuousLinearMap.adjoint_inner_right, coe_toCLM]
    rw [forall_comm]
    conv in ⟪_, _⟫_𝕜 => rw [← inner_conj_symm, ← RCLike.star_def]
    fun_prop

end ContinuousLinearMapWOT
