...
theorem infinite_rat_abs_sub_lt_one_div_den_sq_of_irrational
  ...
  obtain ⟨q', hmem, hbetter⟩ := exists_rat_abs_sub_lt_and_lt_of_irra
...
theorem aux₁
  ...
  have H : (2 * v - 1 : Real) < 1 := by
    refine (mul_lt_iff_lt_one_right hv₀).1 ((inv_lt_inv
...
theorem aux₂
  ...
  rw [← one_div]; rw [lt_div_iff₀ (mul_pos hv₀ hv₀')]; rw [← abs_of_pos (mul_pos hv₀ hv₀')]; rw [← abs_mul]; rw [sub_mul]; rw [← mul_assoc]; rw [← mul_assoc]; rw [
...
theorem aux₃
  ...
  obtain ⟨Hu, Hu'⟩ := aux₀ hu₀
  obtain ⟨Hv, Hv'⟩ := aux₀ (zero_lt_two.tra
