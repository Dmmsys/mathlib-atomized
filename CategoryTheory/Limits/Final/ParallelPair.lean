...
/--
lemma `parallelPair_initial_mk'` / 引理 `parallelPair_initial_mk'`

English:
lemma parallelPair_initial_mk'
  statement: {X Y : C} (f g : X ⟶ Y)
  proof: by
    have : Nonempty (CostructuredArrow (parallelPair f g) Z) :=
      ⟨mk (Y := zero) (h₁ Z).some⟩
    have : forall (x : CostructuredArrow (parallelPair f g) Z), Zigzag x
...
lemma parallelPair_initial_mk' {X Y : C} (f g : X ⟶ Y)
    (h₁ : forall Z, Nonempty (X ⟶ Z))
    (h₂ : forall ⦃Z : C⦄ (i j : X ⟶ Z),
...
lemma parallelPair_initial_mk
  statement: {X Y : C} (f g : X ⟶ Y)
  proof: parallelPair_initial_mk' f g h₁ (fun Z i j => by
...
引理 parallelPair_initial_mk
  结论: {X Y : C} (f g : X ⟶ Y)
  证明: parallelPair_initial_mk' f g h₁ (fun Z i j => by
    obtain ⟨a, rfl, rfl⟩ := h₂ i j
    let f₁ : (mk (Y := zero) (f ≫ a) : CostructuredArrow (parallelPair f g) Z) ⟶ mk (Y := one) a :=
      homMk left
    let f₂ : (mk (Y := zero) (g ≫ a) : CostructuredArrow (parallelPair f g) Z) ⟶ mk (Y := one) a :=
...
lemma parallelPair_initial_mk {X Y : C} (f g : X ⟶ Y)
    (h₁ : forall Z, Nonempty (X ⟶ Z))
    (h₂ : forall ⦃Z : C⦄ (i j : X ⟶ Z), exists (a : Y ⟶ Z), i = f ≫ a ∧ j = g ≫ a) :
