...
定理 isLeftRegular_of_non_zero_divisor
  结论: [NonUnitalNonAssocRing α] (k : α)
  证明: by
  refine fun x y (h' : k * x = k * y) => sub_eq_zero.mp (h _ ?_)
  rw [mul_sub]; rw [sub_eq_zero]; rw [h']
...
定理 IsRegular.of_ne_zero'
  条件: [NonUnitalNonAssocRing α] [NoZeroDivisors α] {k : α} (hk : k != 0)
...
定理 isRegular_iff_ne_zero'
  结论: [Nontrivial α] [NonUnitalNonAssocRing α] [NoZeroDivisors α]
  证明: ⟨fun h => by
    rintro rfl
    exact not_not.mpr h.left not_isLeftRegular_zero, .of_ne_zero'⟩
...
中文:
引理 NoZeroDivisors.toIsCancelMulZero
  条件: [NonUnitalNonAssocRing α] [NoZeroDivisors α]
