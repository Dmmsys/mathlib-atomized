...
-/
/-
**Mathlib.Linter.UnusedInstancesInType.Parameter** 是 Mathlib 中的一个结构，位于命名空间 `Math
lib.Linter.UnusedInstancesInType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A structure for storing information about a parameter of some declaration, usual
ly within some
telescope. We use this for recording the expressions and index associated to a p
arameter which was
unused in the remainder of the type.

`m!"{param}"` displays as `[{param.type?.get}] (#{param.idx + 1})` if `type?` is
 `some _`, and as
`parameter #{param.idx + 1}` as a failsafe if `type?` is `none`. (We always expe
ct `type?` to be
`some _`, but would like a fallback if there are unexpected issues in telescopin
g.)
-/
...
/-
**Mathlib.Linter.UnusedInstancesInType.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Linte
r.UnusedInstancesInType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Linter.UnusedInstancesInType._root_.Lean.Name.unusedInstancesMsg** 是 M
athlib 中的一个定义，位于命名空间 `Mathlib.Linter.UnusedInstancesInType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`#[p₁, p₂, ..., pₙ]`, constructs the message:
```null
`{foo}` does not use the following hypothes(is/es) in its type{ outside of proof
s}:
  • {p₁}
  • {p₂}
  ⋮
  • {pₙ}
```
where the bracketed "outside of proofs" is only included if some parameter appea
rs in a proof in
the type.
-/
...
/-
**Mathlib.Linter.UnusedInstancesInType.collectFVarsOutsideOfProofs** 是 Mathlib 中
的一个定义，位于命名空间 `Mathlib.Linter.UnusedInstancesInType`。
形式化陈述：collectFVarsOutsideOfProofs (e : Expr) : StateRefT FVarIdSet MetaM Unit
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Collects free variables that do not appear in proofs. Ignores `sorry`s (and thei
r types).
-/
...
/-
**Mathlib.Linter.UnusedInstancesInType.InstanceOfConcern** 是 Mathlib 中的一个结构，位于命名
空间 `Mathlib.Linter.UnusedInstancesInType`。
形式化陈述：InstanceOfConcern where /-- The `FVarId` of the instance of concern within
 the current telescope. -/ fvarId : FVarId /-- The index of the binder that this
 instance came from. The first binder is `0`, and we do not increment this index
 when seeing through `let`s. -/ idx : Nat  /-- Gets the indices `i` (in ascendin
g order) of the binders of a nested `.forallE`, `(x₀ : A₀) → (x₁ : A₁) → ⋯ → X`,
 such that - the binder `[xᵢ : Aᵢ]` has `instImplicit` `binderInfo` - `p Aᵢ` is 
`true` - The rest of the t
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The information we keep track of when encountering a binder for an instance of c
oncern. -/
-/
...
-/
/-
**Mathlib.Linter.UnusedInstancesInType._root_.Lean.Expr.collectUnnecessaryInstan
ceBinderIdxsWhere** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.UnusedInstancesInTyp
e`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
- the binder `[xᵢ : Aᵢ]` has `instImplicit` `binderInfo`
-  `p Aᵢ` is `true`
- The rest of the type `(xᵢ₊₁ : Aᵢ₊₁) → ⋯ → X` does not depend on `xᵢ` outside o
f proofs.

This is like `getForallUnusedInstanceBinderIdxsWhere`, but ignores dependence th
at arises from
within proof terms.
...
-/
/-
**Mathlib.Linter.UnusedInstancesInType._root_.Lean.ConstantVal.onUnusedInstances
Where** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.UnusedInstancesInType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Gathers instance hypotheses in the type of `decl` that are unused in the remaind
er of the type and
whose types satisfy `p`. (Does not consider the body of the declaration.) Collec
ts them into an
`Array Parameter`, and if (and only if) this array is nonempty, feeds it to `log
OnUnused`.

Since `logOnUnused` will only be run if its argument is nonempty, it is allowed 
to be expensive.

Note that `p` is non-monadic, and may encounter loose bvars in its argument. Thi
s is a performance
optimization. However, the `Parameter`s are created in a telescope, and their fi
elds will *not*
have loose bound variables.
-/
...
/-
**Mathlib.Linter.UnusedInstancesInType._root_.Lean.Syntax.logUnusedInstancesInTh
eoremsWhere** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.UnusedInstancesInType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Finds theorems whose bodies were elaborated in the current infotrees and whose (
full)
declaration names satisfy `nameFilter`. Checks their type to see if it contains 
instance hypotheses
that (1) are unused in the remainder of the type (2) have types which satisfy `i
nstanceTypeFilter`.
(Note: `instanceTypeFilter` is non-monadic, and may encounter bound variables in
 its argument. This
is a performance optimization. `isAppOrForallOfConstP` may be useful in detectin
g constant
applications and types of the form `∀ (...), bar ..` here.)

If any such parameters are found in the type of a theorem `foo`, we create a tel
escope in which the
types and free variables of the unused parameters are available as
`unusedParams : Array Parameter := #[p₁, p₂, ..., pₙ]`, as well as the theorem `
thm : ConstantVal`
and current infotree `t`, and run `log t thm unusedParams`.
...
-/
/-
**Mathlib.Linter.UnusedInstancesInType.isDecidableVariant** 是 Mathlib 中的一个定义，位于命
名空间 `Mathlib.Linter.UnusedInstancesInType`。
形式化陈述：isDecidableVariant (type : Expr) : Bool
参数：type : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Checks if `type` is an application of (or forall with return type which is an ap
plication of) a
`Decidable*` constant. Specifically, checks if the constant is one of:
- `Decidable`
- `DecidablePred`
...
/-
**Mathlib.Linter.UnusedInstancesInType.unusedDecidableInType** 是 Mathlib 中的一个定义，
位于命名空间 `Mathlib.Linter.UnusedInstancesInType`。
形式化陈述：unusedDecidableInType : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Detects `Decidable*` instance hypotheses in the types of theorems which are not 
used in the
remainder of the type, and suggests replacing them with a use of `classical` in 
the proof or
`open scoped Classical in` at the term level.
-/
...
/-
**Mathlib.Linter.UnusedInstancesInType.unusedFintypeInType** 是 Mathlib 中的一个定义，位于
命名空间 `Mathlib.Linter.UnusedInstancesInType`。
形式化陈述：unusedFintypeInType : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Detects `Fintype` instance hypotheses in the types of theorems which are not use
d in the
remainder of the type, and suggests replacing them with the corresponding hypoth
esis of `Finite`
and the use of `Fintype.ofFinite` in the proof.
-/
