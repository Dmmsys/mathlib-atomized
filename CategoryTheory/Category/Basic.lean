...
@[pp_with_univ]
/-
**CategoryTheory.CategoryStruct** 是 Mathlib 中的一个类，位于命名空间 `CategoryTheory`。
形式化陈述：CategoryStruct (obj : Type u) : Type max u (v + 1) extends Quiver.{v} obj 
where /-- The identity morphism on an object. -/ id : forall X : obj, Hom X X /-
- Composition of morphisms in a category, written `f ≫ g`. -/ comp : forall {X Y
 Z : obj}, (X ⟶ Y) -> (Y ⟶ Z) -> (X ⟶ Z)  attribute [trans, to_dual self (reorde
r
参数：obj : Type u。
继承自：Quiver.{v} obj。
...
/-
**CategoryTheory.Category** 是 Mathlib 中的一个类，位于命名空间 `CategoryTheory`。
形式化陈述：Category (obj : Type u) : Type max u (v + 1) extends CategoryStruct.{v} ob
j where /-- Identity morphisms are left identities for composition. -/ id_comp :
 forall {X Y : obj} (f : X ⟶ Y), 𝟙 X ≫ f = f
参数：obj : Type u。
继承自：CategoryStruct.{v} obj。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[to_dual existing mk]
/-
**CategoryTheory.Category.mk'** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Category
`。
形式化陈述：{obj : Type u} →   [inst : CategoryTheory.CategoryStruct.{v, u} obj] →    
 (∀ {X Y : obj} (f : Y ⟶ X), CategoryTheory.CategoryStruct.comp f (CategoryTheor
y.CategoryStruct.id X) = f) →       (∀ {X Y : obj} (f : Y ⟶ X), CategoryTheory.C
ategoryStruct.comp (CategoryTheory.CategoryStruct.id Y) f = f) →         (∀ {W X
 Y Z : obj} (f : X ⟶ W) (g : Y ⟶ X) (h : Z ⟶ Y),             CategoryTheory.Cate
goryStruct.comp h (CategoryTheory.CategoryStruct.comp g f) =               Categ
oryTheory.CategoryStruct.comp (CategoryTheory.CategoryStruct.comp h g) f) →     
      CategoryTheory.Category.{v, u} obj
参数：∀ {X Y : obj} (f : Y ⟶ X), CategoryTheory.CategoryStruct.comp f (CategoryTheo
ry.CategoryStruct.id X) = f；∀ {X Y : obj} (f : Y ⟶ X), CategoryTheory.CategorySt
ruct.comp (CategoryTheory.CategoryStruct.id Y) f = f；∀ {W X Y Z : obj} (f : X ⟶ 
W) (g : Y ⟶ X) (h : Z ⟶ Y),             CategoryTheory.CategoryStruct.comp h (Ca
tegoryTheory.CategoryStruct.comp g f) =               CategoryTheory.CategoryStr
uct.comp (CategoryTheory.CategoryStruct.comp h g) f。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**CategoryTheory.LargeCategory** 是 Mathlib 中的一个缩写定义，位于命名空间 `CategoryTheory`。
形式化陈述：LargeCategory (C : Type (u + 1)) : Type (u + 1)
参数：C : Type (u + 1)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A `LargeCategory` has objects in one universe level higher than the universe lev
el of
the morphisms. It is useful for examples such as the category of types, or the c
ategory
of groups, etc.
-/
...
-/
/-
**CategoryTheory.SmallCategory** 是 Mathlib 中的一个缩写定义，位于命名空间 `CategoryTheory`。
形式化陈述：SmallCategory (C : Type u) : Type (u + 1)
参数：C : Type u。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CategoryTheory.Category.assoc'** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory.Categ
ory`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {W X Y Z : C} (f 
: X ⟶ W) (g : Y ⟶ X) (h : Z ⟶ Y),   CategoryTheory.CategoryStruct.comp h (Catego
ryTheory.CategoryStruct.comp g f) =     CategoryTheory.CategoryStruct.comp (Cate
goryTheory.CategoryStruct.comp h g) f
参数：f : X ⟶ W；g : Y ⟶ X；h : Z ⟶ Y；CategoryTheory.CategoryStruct.comp g f；Category
Theory.CategoryStruct.comp h g。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.eq_whisker** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：eq_whisker {f g : X ⟶ Y} (w : f = g) (h : Y ⟶ Z) : f ≫ h = g ≫ h
参数：w : f = g；h : Y ⟶ Z。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.eq_of_comp_left_eq** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：eq_of_comp_left_eq {f g : X ⟶ Y} (w : forall {Z : C} (h : Y ⟶ Z), f ≫ h = 
g ≫ h) : f = g
参数：w : forall {Z : C} (h : Y ⟶ Z), f ≫ h = g ≫ h。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.eq_of_comp_left_eq'** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：eq_of_comp_left_eq' (f g : X ⟶ Y) (w : (fun {Z} (h : Y ⟶ Z) => f ≫ h) = fu
n {Z} (h : Y ⟶ Z) => g ≫ h) : f = g
参数：f g : X ⟶ Y；w : (fun {Z} (h : Y ⟶ Z) => f ≫ h) = fun {Z} (h : Y ⟶ Z) => g ≫ h
。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.eq_of_comp_left_eq`：eq_of_comp_left_eq {f g : X ⟶ Y} (w :
 forall {Z : C} (h : Y ⟶ Z), f ≫ h = g ≫ h) : f = g
...
/-
**CategoryTheory.id_of_comp_left_id** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：id_of_comp_left_id (f : X ⟶ X) (w : forall {Y : C} (g : X ⟶ Y), f ≫ g = g)
 : f = 𝟙 X
参数：f : X ⟶ X；w : forall {Y : C} (g : X ⟶ Y), f ≫ g = g。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `eq_of_heq`：∀ {α : Sort u} {a a' : α}, a ≍ a' → a = a'
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
...
/-
**CategoryTheory.comp_ite** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：comp_ite {P : Prop} [Decidable P] {X Y Z : C} (f : X ⟶ Y) (g g' : Y ⟶ Z) :
...
/-
**CategoryTheory.comp_dite** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：comp_dite {P : Prop} [Decidable P] {X Y Z : C} (f : X ⟶ Y) (g : P -> (Y ⟶ 
Z)) (g' : ¬P -> (Y ⟶ Z)) : (f ≫ if h : P then g h else g' h) = if h : P then f ≫
 g h else f ≫ g' h
参数：f : X ⟶ Y；g : P -> (Y ⟶ Z)；g' : ¬P -> (Y ⟶ Z)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
...
`f ≫ g = f ≫ h` implies `g = h`. -/
/-
**CategoryTheory.Epi** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTheory`。
...
/-
**CategoryTheory.Mono** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTheory`。
形式化陈述：{C : Type u} → [inst : CategoryTheory.Category.{v, u} C] → {X Y : C} → (X 
⟶ Y) → Prop
参数：X ⟶ Y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[to_dual]
/-
**CategoryTheory.cancel_epi** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：cancel_epi (f : X ⟶ Y) [Epi f] {g h : Y ⟶ Z} : f ≫ g = f ≫ h ↔ g = h
参数：f : X ⟶ Y。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual]
/-
**CategoryTheory.cancel_epi_assoc_iff** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`
。
形式化陈述：cancel_epi_assoc_iff (f : X ⟶ Y) [Epi f] {g h : Y ⟶ Z} {W : C} {k l : Z ⟶ 
W} : (f ≫ g) ≫ k = (f ≫ h) ≫ l ↔ g ≫ k = h ≫ l
参数：f : X ⟶ Y。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual]
/-
**CategoryTheory.cancel_epi_id** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：cancel_epi_id (f : X ⟶ Y) [Epi f] {h : Y ⟶ Y} : f ≫ h = f ↔ h = 𝟙 Y
参数：f : X ⟶ Y。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `eq_of_heq`：∀ {α : Sort u} {a a' : α}, a ≍ a' → a = a'
...
-/]
/-
**CategoryTheory.epi_comp** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory`。
形式化陈述：epi_comp (f : X ⟶ Y) [Epi f] (g : Y ⟶ Z) [Epi g] : Epi (f ≫ g)
参数：f : X ⟶ Y；g : Y ⟶ Z。
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用定理 `CategoryTheory.cancel_epi`：cancel_epi (f : X ⟶ Y) [Epi f] {g h : Y ⟶ Z} 
: f ≫ g = f ≫ h ↔ g = h
· 使用定理 `CategoryTheory.cancel_epi_assoc_iff`：cancel_epi_assoc_iff (f : X ⟶ Y) [E
pi f] {g h : Y ⟶ Z} {W : C} {k l : Z ⟶ W} : (f ≫ g) ≫ k = (f ≫ h) ≫ l ↔ g ≫ k = 
h ≫ l
-/
...
-/]
/-
**CategoryTheory.epi_comp'** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：epi_comp' {f : X ⟶ Y} {g : Y ⟶ Z} (hf : Epi f) (hg : Epi g) : Epi (f ≫ g)
参数：hf : Epi f；hg : Epi g。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[to_dual (reorder := f g)]
/-
**CategoryTheory.epi_of_epi** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：epi_of_epi (f : X ⟶ Y) (g : Y ⟶ Z) [Epi (f ≫ g)] : Epi g
参数：f : X ⟶ Y；g : Y ⟶ Z；f ≫ g。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
...
@[to_dual]
/-
**CategoryTheory.epi_of_epi_fac** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory`。
形式化陈述：epi_of_epi_fac {f : X ⟶ Y} {g : Y ⟶ Z} {h : X ⟶ Z} [Epi h] (w : f ≫ g = h)
 : Epi g
参数：w : f ≫ g = h。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
is injective. -/]
/-
**CategoryTheory.epi_iff_forall_injective** 是 Mathlib 中的一个引理，位于命名空间 `CategoryThe
ory`。
形式化陈述：epi_iff_forall_injective (f : X ⟶ Y) : Epi f ↔ forall Z, (fun g : Y ⟶ Z =>
 f ≫ g).Injective
参数：f : X ⟶ Y。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[instance_reducible]
/-
**CategoryTheory.uliftCategory** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory`。
形式化陈述：uliftCategory : Category.{v} (ULift.{u'} C) where Hom X Y
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
