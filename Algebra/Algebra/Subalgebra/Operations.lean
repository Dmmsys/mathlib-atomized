/-
Copyright (c) 2018 Kenny Lau. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Andrew Yang, Antoine Chambert-Loir
-/
module

public import Mathlib.Algebra.Algebra.Subalgebra.Basic
public import Mathlib.RingTheory.Ideal.Maps
public import Mathlib.Algebra.Ring.Action.Submonoid

/-!
# More operations on subalgebras

In this file we relate the definitions in `Mathlib/RingTheory/Ideal/Operations.lean` to subalgebras.
The contents of this file are somewhat random since both
`Mathlib/Algebra/Algebra/Subalgebra/Basic.lean` and `Mathlib/RingTheory/Ideal/Operations.lean` are
somewhat of a grab-bag of definitions, and this is whatever ends up in the intersection.
-/

@[expose] public section

assert_not_exists Cardinal

namespace AlgHom

variable {R A B : Type*} [CommSemiring R] [Semiring A] [Algebra R A] [Semiring B] [Algebra R B]

/--
theorem `ker_rangeRestrict` / 定理 `ker_rangeRestrict`

English:
theorem ker_rangeRestrict
  given: (f : A ->ₐ[R] B)
  statement: RingHom.ker f.rangeRestrict = RingHom.ker f
  proof: Ideal.ext fun _ => Subtype.ext_iff

中文:
定理 ker_rangeRestrict
  条件: (f : A ->ₐ[R] B)
  结论: RingHom.ker f.rangeRestrict = RingHom.ker f
  证明: Ideal.ext fun _ => Subtype.ext_iff

Depends on / 依赖: Ideal.ext, Subtype, Subtype.ext_iff, ext_iff
-/
theorem ker_rangeRestrict (f : A ->ₐ[R] B) : RingHom.ker f.rangeRestrict = RingHom.ker f :=
  Ideal.ext fun _ => Subtype.ext_iff

end AlgHom

namespace Subalgebra

open Algebra

variable {R S : Type*} [CommSemiring R] [CommSemiring S] [Algebra R S]
variable (S' : Subalgebra R S)

/--
theorem `mem_of_finsetSum_eq_one_of_pow_smul_mem` / 定理 `mem_of_finsetSum_eq_one_of_pow_smul_mem`

English:
theorem mem_of_finsetSum_eq_one_of_pow_smul_mem
  proof: by
  suffices x in Subalgebra.toSubmodule (Algebra.ofId S' S).range by
    obtain ⟨x, rfl⟩ := this
    exact x.2
  choose n hn using H
  let s' : ι -> S' := fun x => ⟨s x, hs x⟩
  let l' : ι -> S' := fun x => ⟨l x, hl x⟩
  have e' : ∑ i in ι', l' i * s' i = 1 := by
    ext
    change S'.subtype (∑ i

中文:
定理 mem_of_finsetSum_eq_one_of_pow_smul_mem
  证明: by
  suffices x in Subalgebra.toSubmodule (Algebra.ofId S' S).range by
    obtain ⟨x, rfl⟩ := this
    exact x.2
  choose n hn using H
  let s' : ι -> S' := fun x => ⟨s x, hs x⟩
  let l' : ι -> S' := fun x => ⟨l x, hl x⟩
  have e' : ∑ i in ι', l' i * s' i = 1 := by
    ext
    change S'.subtype (∑ i

Depends on / 依赖: Algebra, Algebra.ofId, Ideal.eq_top_iff_one, Ideal.mul_mem_left, Ideal.span, Ideal.subset_span, Set.mem_image_, Subalgebra, Subalgebra.toSubmodule, eq_top_iff_one, map_mul, map_sum, mem_image_, mul_mem_left, subset_span, subtype, sum_mem, toSubmodule
-/
theorem mem_of_finsetSum_eq_one_of_pow_smul_mem
    {ι : Type*} (ι' : Finset ι) (s : ι -> S) (l : ι -> S)
    (e : ∑ i in ι', l i * s i = 1) (hs : forall i, s i in S') (hl : forall i, l i in S') (x : S)
    (H : forall i, exists n : Nat, (s i ^ n : S) • x in S') : x in S' := by
  suffices x in Subalgebra.toSubmodule (Algebra.ofId S' S).range by
    obtain ⟨x, rfl⟩ := this
    exact x.2
  choose n hn using H
  let s' : ι -> S' := fun x => ⟨s x, hs x⟩
  let l' : ι -> S' := fun x => ⟨l x, hl x⟩
  have e' : ∑ i in ι', l' i * s' i = 1 := by
    ext
    change S'.subtype (∑ i in ι', l' i * s' i) = 1
    simpa only [map_sum, map_mul] using! e
  have : Ideal.span (s' '' ι') = ⊤ := by
    rw [Ideal.eq_top_iff_one]; rw [← e']
    apply sum_mem
    intro i hi
exact Ideal.mul_mem_left _ _ Ideal.subset_span Set.mem_image_of_mem s' hi
  let N := ι'.sup n
  have hN := Ideal.span_pow_eq_top _ this N
  apply (Algebra.ofId S' S).range.toSubmodule.mem_of_span_top_of_smul_mem _ hN
  rintro ⟨_, _, ⟨i, hi, rfl⟩, rfl⟩
  change s' i ^ N • x in _
  rw [← tsub_add_cancel_of_le (show n i <= N from Finset.le_sup hi)]; rw [pow_add]; rw [mul_smul]
  refine Submodule.smul_mem _ (⟨_, pow_mem (hs i) _⟩ : S') ?_
  exact ⟨⟨_, hn i⟩, rfl⟩

@[deprecated (since := "2026-04-08")]
alias mem_of_finset_sum_eq_one_of_pow_smul_mem := mem_of_finsetSum_eq_one_of_pow_smul_mem

/--
theorem `mem_of_span_eq_top_of_smul_pow_mem` / 定理 `mem_of_span_eq_top_of_smul_pow_mem`

English:
theorem mem_of_span_eq_top_of_smul_pow_mem
  proof: mem_of_finsetSum_eq_one_of_pow_smul_mem S' l.support (↑) l hs (fun x => hs' x.2) hl x H

中文:
定理 mem_of_span_eq_top_of_smul_pow_mem
  证明: mem_of_finsetSum_eq_one_of_pow_smul_mem S' l.support (↑) l hs (fun x => hs' x.2) hl x H

Depends on / 依赖: l.support, mem_of_finsetSum_eq_one_of_pow_smul_mem, support
-/
theorem mem_of_span_eq_top_of_smul_pow_mem
    (s : Set S) (l : s ->₀ S) (hs : Finsupp.linearCombination S ((↑) : s -> S) l = 1)
    (hs' : s subseteq S') (hl : forall i, l i in S') (x : S) (H : forall r : s, exists n : Nat, (r : S) ^ n • x in S') :
    x in S' :=
  mem_of_finsetSum_eq_one_of_pow_smul_mem S' l.support (↑) l hs (fun x => hs' x.2) hl x H

end Subalgebra

section MulSemiringAction

variable (A B B' : Type*) [CommSemiring A] [Ring B] [Semiring B'] [Algebra A B] [Algebra A B']
variable (G : Type*) [Monoid G] [MulSemiringAction G B] [SMulCommClass G A B]
  [MulSemiringAction G B'] [SMulCommClass G A B']

/--
Definition of `FixedPoints.subsemiring` / `FixedPoints.subsemiring` 的定义

English:
definition FixedPoints.subsemiring
  signature: : Subsemiring B' where
  body: FixedPoints.addSubmonoid G B'
  __ := FixedPoints.submonoid G B'

中文:
定义 FixedPoints.subsemiring
  签名: : Subsemiring B' where
  定义体: FixedPoints.addSubmonoid G B'
  __ := FixedPoints.submonoid G B'

Depends on / 依赖: FixedPoints, FixedPoints.addSubmonoid, addSubmonoid
-/
def FixedPoints.subsemiring : Subsemiring B' where
  __ := FixedPoints.addSubmonoid G B'
  __ := FixedPoints.submonoid G B'

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: SMulCommClass G (FixedPoints.subsemiring B' G) B'
  body: inferInstanceAs (SMulCommClass G (FixedPoints.submonoid G B') B')

中文:
实例 :
  签名: SMulCommClass G (FixedPoints.subsemiring B' G) B'
  定义体: inferInstanceAs (SMulCommClass G (FixedPoints.submonoid G B') B')

Depends on / 依赖: FixedPoints, FixedPoints.submonoid, SMulCommClass, submonoid
-/
instance : SMulCommClass G (FixedPoints.subsemiring B' G) B' :=
  inferInstanceAs (SMulCommClass G (FixedPoints.submonoid G B') B')

/--
Definition of `FixedPoints.subring` / `FixedPoints.subring` 的定义

English:
definition FixedPoints.subring
  signature: : Subring B where
  body: FixedPoints.addSubgroup G B
  __ := FixedPoints.submonoid G B

中文:
定义 FixedPoints.subring
  签名: : Subring B where
  定义体: FixedPoints.addSubgroup G B
  __ := FixedPoints.submonoid G B

Depends on / 依赖: FixedPoints, FixedPoints.addSubgroup, addSubgroup
-/
def FixedPoints.subring : Subring B where
  __ := FixedPoints.addSubgroup G B
  __ := FixedPoints.submonoid G B

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: SMulCommClass G (FixedPoints.subring B G) B
  body: inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B G) B)

中文:
实例 :
  签名: SMulCommClass G (FixedPoints.subring B G) B
  定义体: inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B G) B)

Depends on / 依赖: FixedPoints, FixedPoints.subsemiring, SMulCommClass, subsemiring
-/
instance : SMulCommClass G (FixedPoints.subring B G) B :=
  inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B G) B)

/--
Definition of `FixedPoints.subalgebra` / `FixedPoints.subalgebra` 的定义

English:
definition FixedPoints.subalgebra
  signature: : Subalgebra A B' where
  body: FixedPoints.subsemiring B' G
  algebraMap_mem' r g := smul_algebraMap g r

中文:
定义 FixedPoints.subalgebra
  签名: : Subalgebra A B' where
  定义体: FixedPoints.subsemiring B' G
  algebraMap_mem' r g := smul_algebraMap g r

Depends on / 依赖: FixedPoints, FixedPoints.subsemiring, subsemiring
-/
def FixedPoints.subalgebra : Subalgebra A B' where
  __ := FixedPoints.subsemiring B' G
  algebraMap_mem' r g := smul_algebraMap g r

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: SMulCommClass G (FixedPoints.subalgebra A B' G) B'
  body: inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B' G) B')

中文:
实例 :
  签名: SMulCommClass G (FixedPoints.subalgebra A B' G) B'
  定义体: inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B' G) B')

Depends on / 依赖: FixedPoints, FixedPoints.subsemiring, SMulCommClass, subsemiring
-/
instance : SMulCommClass G (FixedPoints.subalgebra A B' G) B' :=
  inferInstanceAs (SMulCommClass G (FixedPoints.subsemiring B' G) B')

end MulSemiringAction
