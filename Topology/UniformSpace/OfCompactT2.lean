...
@[instance_reducible]
/-
**uniformSpaceOfCompactR1** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：uniformSpaceOfCompactR1 [TopologicalSpace γ] [CompactSpace γ] [R1Space γ] 
: UniformSpace γ where uniformity
该定义给出了上述对象。
