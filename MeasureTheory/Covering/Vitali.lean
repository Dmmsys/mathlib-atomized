...
/--
theorem `exists_disjoint_subfamily_covering_enlargement` / 定理 `exists_disjoint_subfamily_covering_enlargement`

English:
theorem exists_disjoint_subfamily_covering_enlargement
  statement: (B : ι -> Set α) (t : Set ι) (δ : ι -> Real)
  proof: by
...
定理 exists_disjoint_subfamily_covering_enlargement
  ...
  the already chosen one, pick another element with large `δ`. Go on forever (transfinitely) unti
-/
theorem exists_disjoint_subfamily_covering_enlargement (B : ι -> Set α) (t : Set ι) (δ : ι -> Real)
    (τ : Real) (hτ : 1 < τ) (δnonneg : forall a in t, 0 <= δ a) (R : Real) (δle : forall a in t, δ a <= R)
    (hne : forall a in t, (B a).Nonempty) :
    exists u subseteq t,
      u.PairwiseDisjoint B ∧ forall a in t, exists b in u, (B a inter B b).Nonempty ∧ δ a <= τ * δ b := by
  /- The proof could be formulated as a transfinite induction. First pick an element of `t` with `δ`
  as large as possible (up to a factor of `τ`). Then among the remaining elements not intersecting
  ...
  -/
  let T : Set (Set ι) := { u | u subseteq t ∧ u.PairwiseDisjoint B ∧
    forall a in t, forall b in u, (B a inter B b).Nonempty -> exists c in u, (B a inter B c).Nonempty ∧ δ a <= τ * δ c }
  -- By Zorn, choose a maximal family in the good set `T` of disjoint families.
  obtain ⟨u, hu⟩ : exists m, Maximal (fun x => x in T) m := by
    ...
    obtain ⟨c, cu, ac, hc⟩ : exists c, c in u ∧ (B a inter B c).Nonempty ∧ δ a <= τ * δ c :=
  ...
  by_contra! hcon
  have a_disj : forall c in u, Disjoint (B a) (B c) := by
    intro c hc
    by_contra h
    rw [not_disjoint_iff_nonempty_inter] at h
    obtain ⟨d, du, ad, hd⟩ : exists d, d in u ∧ (B a inter B d).Nonempty ∧ δ a <= τ * δ d :=
  ...
  let A := { a' | a' in t ∧ forall c in u, Disjoint (B a') (B c) }
  ...
  have bddA : BddAbove (δ '' A) := by
    ...
    exact δle a' ha'.1
  obtain ⟨a', a'A, ha'⟩ : exists a' in A, m / τ <= δ a' := by
  ...
  · -- check that every element `c` of `t` intersecting `u ∪ {a'}` intersects an element of this
    ...
    by_cases! H : exists d in u, (B c inter B d).Nonempty
...
/--
theorem `exists_disjoint_subfamily_covering_enlargement_closedBall` / 定理 `exists_disjoint_subfamily_covering_enlargement_closedBall`
...
theorem exists_disjoint_subfamily_covering_enlargement_closedBall
  ...
  by_cases! ht : forall a in t, r a < 0
...
Depends on / 依赖: Function, Function.onFun, Subset, Subset.refl, Subset.rfl, closedBall, closedBall_eq_empty, empty_disjoint, empty_subset, eq_empty_or_nonempty, exists_disjoint_subfamily_covering_enlargement, linari, pairwiseDisjoint_empty
-/
theorem exists_disjoint_subfamily_covering_enlargement_closedBall
    [PseudoMetricSpace α] (t : Set ι)
    (x : ι -> α) (r : ι -> Real) (R : Real) (hr : forall a in t, r a <= R) (τ : Real) (hτ : 3 < τ) :
    exists u subseteq t,
      (u.PairwiseDisjoint fun a => closedBall (x a) (r a)) ∧
        forall a in t, exists b in u, closedBall (x a) (r a) subseteq closedBall (x b) (τ * r b) := by
  rcases eq_empty_or_nonempty t with (rfl | _)
  · exact ⟨∅, Subset.refl _, pairwiseDisjoint_empty, by simp⟩
  by_cases! ht : forall a in t, r a < 0
  · exact ⟨t, Subset.rfl, fun a ha b _ _ => by
  ...
  rcases exists_disjoint_subfamily_covering_enlargement (fun a => closedBall (x a) (r a)) t' r
    ...
    ⟨u, ut', u_disj, hu⟩
  have A : forall a in t', exists b in u, closedBall (x a) (r a) subseteq closedBall (x b) (τ * r b) := by
    intro a ha
...
/--
theorem `exists_disjoint_subfamily_covering_enlargement_ball` / 定理 `exists_disjoint_subfamily_covering_enlargement_ball`
...
Depends on / 依赖: Function, Function.onFun, Subset, Subset.refl, Subset.rfl, ball_eq_empty, empty_disjoint, empty_subset, eq_empty_or_nonempty, exists_disjoint_subfamily_covering_enlargement, pairwiseDisjoint_empty
-/
theorem exists_disjoint_subfamily_covering_enlargement_ball
    [PseudoMetricSpace α] (t : Set ι)
    (x : ι -> α) (r : ι -> Real) (R : Real) (hr : forall a in t, r a <= R) (τ : Real) (hτ : 3 < τ) :
    exists u subseteq t,
      (u.PairwiseDisjoint fun a => ball (x a) (r a)) ∧
        forall a in t, exists b in u, ball (x a) (r a) subseteq ball (x b) (τ * r b) := by
  rcases eq_empty_or_nonempty t with (rfl | _)
  · exact ⟨∅, Subset.refl _, pairwiseDisjoint_empty, by simp⟩
  by_cases! ht : forall a in t, r a <= 0
  · exact ⟨t, Subset.rfl, fun a ha b _ _ => by
  ...
  rcases exists_disjoint_subfamily_covering_enlargement (fun a => ball (x a) (r a)) t' r
    ...
    ⟨u, ut', u_disj, hu⟩
  have A : forall a in t', exists b in u, ball (x a) (r a) subseteq ball (x b) (τ * r b) := by
    intro a ha
    rcases hu a ha with ⟨b, bu, hb, rb⟩
    refine ⟨b, bu, ?_⟩
...
theorem exists_disjoint_covering_ae
    ...
    (C : Real>=0) (r : ι -> Real) (c : ι -> α) (B : ι -> Set α) (hB : forall a in t, B a subseteq closedBall (c a) (r a))
    (μB : forall a in t, μ (closedBall (c a) (3 * r a)) <= C * μ (B a))
    (ht : forall a in t, (interior (B a)).Nonempty) (h't : forall a in t, IsClosed (B a))
    (hf : forall x in s, forall ε > (0 : Real), exists a in t, r a <= ε ∧ c a = x) :
    exists u subseteq t, u.Countable ∧ u.PairwiseDisjoint B ∧ μ (s \ ⋃ a in u, B a) = 0 := by
  /- The idea of the proof is the following. Assume for simplicity that `μ` is finite. Applying the
  abstract Vitali covering theorem with `δ = r` given by `hf`, one obtains a disjoint subfamily `u`,
  such that any element of `t` intersects an element of `u` with comparable radius. Fix `ε > 0`.
  ...
  have : forall x, exists R, 0 < R ∧ R <= 1 ∧ μ (closedBall x (20 * R)) < ∞ := fun x => by
  ...
  let t' := { a in t | r a <= R (c a) }
  -- extract a disjoint subfamily `u` of `t'` thanks to the abstract Vitali covering theorem.
  obtain ⟨u, ut', u_disj, hu⟩ : exists u subseteq t',
      u.PairwiseDisjoint B ∧ forall a in t', exists b in u, (B a inter B b).Nonempty ∧ r a <= 2 * r b := by
    have A : forall a in t', r a <= 1 := by
      intro a ha
      apply ha.2.trans (hR1 (c a))
    have A' : forall a in t', (B a).Nonempty :=
  ...
  have ut : u subseteq t := fun a hau => (ut' hau).1
  ...
  let v := { a in u | (B a inter ball x (R x)).Nonempty }
  have vu : v subseteq u := fun a ha => ha.1
  -- they are all contained in a fixed ball of finite measure, thanks to our choice of `t'`
  obtain ⟨K, μK, hK⟩ : exists K, μ (closedBall x K) < ∞ ∧
      forall a in u, (B a inter ball x (R x)).Nonempty -> B a subseteq closedBall x K := by
    have Idist_v : forall a in v, dist (c a) x <= r a + R x := by
      intro a hav
    ...
    · have R0pos : 0 < R0 := (hR0 x).trans_le H
      have vnonempty : v.Nonempty := by
        by_contra h
        rw [nonempty_iff_ne_empty]; rw [Classical.not_not] at h
        rw [h]; rw [image_empty]; rw [Real.sSup_empty] at R0_def
        exact lt_irrefl _ (R0pos.trans_le (le_of_eq R0_def))
      obtain ⟨a, hav, R0a⟩ : exists a in v, R0 / 2 < r a := by
        obtain ⟨r', r'mem, hr'⟩ : exists r' in r '' v, R0 / 2 < r' :=
  ...
  refine ⟨_ inter ball x (R x), inter_mem_nhdsWithin _ (ball_mem_nhds _ (hR0 _)),
  ...
  obtain ⟨w, hw⟩ : exists w : Finset v, (∑' a : { a // a ∉ w }, μ (B a)) < ε / C :=
    haveI : 0 < ε / C := by
  ...
  have M : (s \ ⋃ a in u, B a) inter
      ball x (R x) subseteq ⋃ a : { a // a ∉ w }, closedBall (c a) (3 * r a) := by
    intro z hz
    set k := ⋃ (a : v) (_ : a in w), B a
    ...
    have z_notmem_k : z ∉ k := by
      ...
      intro b hbv _ h'z
      have : z in (s \ ⋃ a in u, B a) inter ⋃ a in u, B a :=
    ...
    obtain ⟨b, bu, ab, bdiam⟩ : exists b in u, (B a inter B b).Nonempty ∧ r a <= 2 * r b :=
    ...
    have b'_notmem_w : b' ∉ w := by
      intro b'w
      have b'k : B b' subseteq k := @Finset.subset_set_biUnion_of_mem _ _ _ (fun y : v => B y) _ b'w
      have : (ball x (R x) \ k inter k).Nonempty := by
    ...
    -- enlargement of `b`
    have zb : c a in closedBall (c b) (3 * r b) := by
    ...
    suffices H : closedBall (c b'') (3 * r b'')
        subseteq ⋃ a : { a // a ∉ w }, closedBall (c a) (3 * r a) from H zb
  ...
  calc
    μ ((s \ ⋃ a in u, B a) inter ball x (R x)) <= μ (⋃ a : { a // a ∉ w }, closedBall (c a) (3 * r a)) :=
      measure_mono M
...
theorem exists_disjoint_covering_ae'
  ...
  intro x hx ε hε
  specialize hf x hx
  rw [frequently_nhdsWithin_iff]; rw [frequently_nhds_iff] at hf
...
Depends on / 依赖: exists_disjoint_covering_ae, frequently_nhdsWithin_iff, frequently_nhds_iff, isOpen_Ioo, specialize
-/
theorem exists_disjoint_covering_ae'
    [PseudoMetricSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    [SecondCountableTopology α] (μ : Measure α) [IsLocallyFiniteMeasure μ] (s : Set α) (t : Set ι)
    ...
    (μB : forall a in t, μ (closedBall (c a) (3 * r a)) <= C * μ (B a))
    (ht : forall a in t, (interior (B a)).Nonempty) (h't : forall a in t, IsClosed (B a))
    (hf : forall x in s, existsᶠ ε in 𝓝[>] 0, exists a in t, r a = ε ∧ c a = x) :
    exists u subseteq t, u.Countable ∧ u.PairwiseDisjoint B ∧ μ (s \ ⋃ a in u, B a) = 0 := by
  suffices forall x in s, forall ε > (0 : Real), exists a in t, r a <= ε ∧ c a = x from
    exists_disjoint_covering_ae μ s t C r c B hB μB ht h't this
  intro x hx ε hε
...
definition vitaliFamily
  signature: [PseudoMetricSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
  body: { a | IsClosed a ∧ (interior a).Nonempty ∧
...
protected def vitaliFamily [PseudoMetricSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    [SecondCountableTopology α] (μ : Measure α) [IsLocallyFiniteMeasure μ] (C : Real>=0)
    (h : forall x, existsᶠ r in 𝓝[>] 0, μ (closedBall x (3 * r)) <= C * μ (closedBall x r)) :
  ...
  nontrivial x ε εpos := by
    obtain ⟨r, μr, rpos, rε⟩ :
        exists r, μ (closedBall x (3 * r)) <= C * μ (closedBall x r) ∧ r in Ioc (0 : Real) ε :=
  ...
  covering := by
    ...
    let t : Set (Real × α × Set α) :=
      { p | p.2.2 subseteq closedBall p.2.1 p.1 ∧ μ (closedBall p.2.1 (3 * p.1)) <= C * μ p.2.2 ∧
            (interior p.2.2).Nonempty ∧ IsClosed p.2.2 ∧ p.2.2 in f p.2.1 ∧ p.2.1 in s }
    have A : forall x in s, forall ε : Real, ε > 0 -> exists p, p in t ∧ p.1 <= ε ∧ p.2.1 = x := by
      intro x xs ε εpos
    ...
    refine ⟨(fun p : Real × α × Set α => p.2) '' t', ?_, ?_, ?_, ?_⟩
    · rintro - ⟨q, hq, rfl⟩
