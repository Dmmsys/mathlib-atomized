/-
Copyright (c) 2023 Anatole Dedecker. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Anatole Dedecker, Etienne Marion
-/
module

public import Mathlib.Topology.Homeomorph.Lemmas

/-!
# Proper maps between topological spaces

This file develops the basic theory of proper maps between topological spaces. A map `f : X → Y`
between two topological spaces is said to be **proper** if it is continuous and satisfies
the following equivalent conditions:
1. `f` is closed and has compact fibers.
2. `f` is **universally closed**, in the sense that for any topological space `Z`, the map
  `Prod.map f id : X × Z → Y × Z` is closed.
3. For any `ℱ : Filter X`, all cluster points of `map f ℱ` are images by `f` of some cluster point
  of `ℱ`.

We take 3 as the definition in `IsProperMap`, and we show the equivalence with 1, 2, and some
other variations.

## Main statements

* `isProperMap_iff_ultrafilter`: characterization of proper maps in terms of limits of ultrafilters
  instead of cluster points of filters.
* `IsProperMap.pi_map`: any product of proper maps is proper.
* `isProperMap_iff_isClosedMap_and_compact_fibers`: a map is proper if and only if it is
  continuous, closed, and has compact fibers

## Implementation notes

In algebraic geometry, it is common to also ask that proper maps are *separated*, in the sense of
[Stacks: definition OCY1](https://stacks.math.columbia.edu/tag/0CY1). We don't follow this
convention because it is unclear whether it would give the right notion in all cases, and in
particular for the theory of proper group actions. That means that our terminology does **NOT**
align with that of [Stacks: Characterizing proper maps](https://stacks.math.columbia.edu/tag/005M),
instead our definition of `IsProperMap` coincides with what they call "Bourbaki-proper".

Regarding the proofs, we don't really follow Bourbaki and go for more filter-heavy proofs,
as usual. In particular, their arguments rely heavily on restriction of closed maps (see
`IsClosedMap.restrictPreimage`), which makes them somehow annoying to formalize in type theory.
In contrast, the filter-based proofs work really well thanks to the existing API.

In fact, the filter proofs work so well that I thought this would be a great pedagogical resource
about how we use filters. For that reason, **all interesting proofs in this file are commented**,
so don't hesitate to have a look!

## TODO

* prove the equivalence with condition 3 of
  [Stacks: Theorem 005R](https://stacks.math.columbia.edu/tag/005R). Note that they mean something
  different by "universally closed".

## References

* [N. Bourbaki, *General Topology*][bourbaki1966]
* [Stacks: Characterizing proper maps](https://stacks.math.columbia.edu/tag/005M)
-/

public section

assert_not_exists StoneCech

open Filter Topology Function Set
open Prod (fst snd)

variable {X Y Z W ι : Type*} [TopologicalSpace X] [TopologicalSpace Y] [TopologicalSpace Z]
  [TopologicalSpace W] {f : X -> Y} {g : Y -> Z}

/-- A map `f : X → Y` between two topological spaces is said to be **proper** if it is continuous
and, for all `ℱ : Filter X`, any cluster point of `map f ℱ` is the image by `f` of a cluster point
of `ℱ`. -/
@[mk_iff isProperMap_iff_clusterPt, fun_prop]
/--
Definition of `IsProperMap` / `IsProperMap` 的定义

English:
structure IsProperMap
  parameters: (f : X -> Y)
  extends: Continuous f
  axioms and operations (1):
    - clusterPt_of_mapClusterPt : forall ⦃ℱ : Filter X⦄, forall ⦃y : Y⦄, MapClusterPt y ℱ f -> exists x, f x = y ∧ ClusterPt x ℱ

中文:
结构 是真映射
  参数: (f : X -> Y)
  继承: 连续 f
  公理与运算 (1 个):
    - clusterPt_of_mapClusterPt : 对任意 ⦃ℱ : 滤子 X⦄, 对任意 ⦃y : Y⦄, MapClusterPt y ℱ f -> 存在 x, f x = y ∧ ClusterPt x ℱ
-/
structure IsProperMap (f : X -> Y) : Prop extends Continuous f where
  /-- By definition, if `f` is a proper map and `ℱ` is any filter on `X`, then any cluster point of
  `map f ℱ` is the image by `f` of some cluster point of `ℱ`. -/
  clusterPt_of_mapClusterPt :
    forall ⦃ℱ : Filter X⦄, forall ⦃y : Y⦄, MapClusterPt y ℱ f -> exists x, f x = y ∧ ClusterPt x ℱ

/-- Definition of proper maps. See also `isClosedMap_iff_clusterPt` for a related criterion
for closed maps. -/
add_decl_doc isProperMap_iff_clusterPt

/-- By definition, a proper map is continuous. -/
@[fun_prop]
/--
lemma `IsProperMap.continuous` / 引理 `IsProperMap.continuous`

English:
lemma IsProperMap.continuous
  given: (h : IsProperMap f)
  statement: Continuous f
  proof: h.toContinuous

中文:
引理 是真映射.continuous
  条件: (h : 是真映射 f)
  结论: 连续 f
  证明: h.toContinuous

Depends on / 依赖: h.toContinuous, toContinuous
-/
lemma IsProperMap.continuous (h : IsProperMap f) : Continuous f := h.toContinuous

/--
lemma `IsProperMap.isClosedMap` / 引理 `IsProperMap.isClosedMap`

English:
lemma IsProperMap.isClosedMap
  given: (h : IsProperMap f)
  statement: IsClosedMap f
  proof: by
  rw [isClosedMap_iff_clusterPt]
  exact fun s y => h.clusterPt_of_mapClusterPt (ℱ := 𝓟 s) (y := y)

中文:
引理 是真映射.isClosedMap
  条件: (h : 是真映射 f)
  结论: 是闭映射 f
  证明: by
  rw [isClosedMap_iff_clusterPt]
  exact fun s y => h.clusterPt_of_mapClusterPt (ℱ := 𝓟 s) (y := y)

Depends on / 依赖: clusterPt_of_mapClusterPt, h.clusterPt_of_mapClusterPt, isClosedMap_iff_clusterPt
-/
lemma IsProperMap.isClosedMap (h : IsProperMap f) : IsClosedMap f := by
  rw [isClosedMap_iff_clusterPt]
  exact fun s y => h.clusterPt_of_mapClusterPt (ℱ := 𝓟 s) (y := y)

/--
lemma `isProperMap_iff_ultrafilter` / 引理 `isProperMap_iff_ultrafilter`

English:
lemma isProperMap_iff_ultrafilter
  statement: IsProperMap f ↔ Continuous f ∧
  proof: by
  -- This is morally trivial since ultrafilters give all the information about cluster points.
  rw [isProperMap_iff_clusterPt]
  refine and_congr_right (fun _ => ?_)
  constructor <;> intro H
  · intro 𝒰 y (hY : (Ultrafilter.map f 𝒰 : Filter Y) <= _)
    simp_rw [← Ultrafilter.clusterPt_iff] at 

中文:
引理 isProperMap_iff_ultrafilter
  结论: 是真映射 f ↔ 连续 f ∧
  证明: by
  -- This is morally trivial since ultrafilters give all the information about cluster points.
  rw [isProperMap_iff_clusterPt]
  refine and_congr_right (fun _ => ?_)
  constructor <;> intro H
  · intro 𝒰 y (hY : (Ultrafilter.map f 𝒰 : Filter Y) <= _)
    simp_rw [← Ultrafilter.clusterPt_iff] at 
-/
lemma isProperMap_iff_ultrafilter : IsProperMap f ↔ Continuous f ∧
    forall ⦃𝒰 : Ultrafilter X⦄, forall ⦃y : Y⦄, Tendsto f 𝒰 (𝓝 y) -> exists x, f x = y ∧ 𝒰 <= 𝓝 x := by
  -- This is morally trivial since ultrafilters give all the information about cluster points.
  rw [isProperMap_iff_clusterPt]
  refine and_congr_right (fun _ => ?_)
  constructor <;> intro H
  · intro 𝒰 y (hY : (Ultrafilter.map f 𝒰 : Filter Y) <= _)
    simp_rw [← Ultrafilter.clusterPt_iff] at hY ⊢
    exact H hY
  · simp_rw [MapClusterPt, ClusterPt, ← Filter.push_pull', map_neBot_iff, ← exists_ultrafilter_iff,
      forall_exists_index]
    intro ℱ y 𝒰 hy
    rcases H (tendsto_iff_comap.mpr <| hy.trans inf_le_left) with ⟨x, hxy, hx⟩
    exact ⟨x, hxy, 𝒰, le_inf hx (hy.trans inf_le_right)⟩

/--
lemma `isProperMap_iff_ultrafilter_of_t2` / 引理 `isProperMap_iff_ultrafilter_of_t2`

English:
lemma isProperMap_iff_ultrafilter_of_t2
  given: [T2Space Y]
  statement: IsProperMap f ↔ Continuous f ∧
  proof: isProperMap_iff_ultrafilter.trans and_congr_right fun hc => forall₃_congr fun _𝒰 _y hy =>
    exists_congr fun x => and_iff_right_of_imp fun h =>
      tendsto_nhds_unique ((hc.tendsto x).mono_left h) hy

中文:
引理 isProperMap_iff_ultrafilter_of_t2
  条件: [T2空间 Y]
  结论: 是真映射 f ↔ 连续 f ∧
  证明: isProperMap_iff_ultrafilter.trans and_congr_right fun hc => forall₃_congr fun _𝒰 _y hy =>
    exists_congr fun x => and_iff_right_of_imp fun h =>
      tendsto_nhds_unique ((hc.tendsto x).mono_left h) hy

Depends on / 依赖: and_congr_right, and_iff_right_of_imp, exists_congr, hc.tendsto, isProperMap_iff_ultrafilter, isProperMap_iff_ultrafilter.trans, mono_left, tendsto, tendsto_nhds_unique
-/
lemma isProperMap_iff_ultrafilter_of_t2 [T2Space Y] : IsProperMap f ↔ Continuous f ∧
    forall ⦃𝒰 : Ultrafilter X⦄, forall ⦃y : Y⦄, Tendsto f 𝒰 (𝓝 y) -> exists x, 𝒰.1 <= 𝓝 x :=
isProperMap_iff_ultrafilter.trans and_congr_right fun hc => forall₃_congr fun _𝒰 _y hy =>
    exists_congr fun x => and_iff_right_of_imp fun h =>
      tendsto_nhds_unique ((hc.tendsto x).mono_left h) hy

/--
lemma `IsProperMap.ultrafilter_le_nhds_of_tendsto` / 引理 `IsProperMap.ultrafilter_le_nhds_of_tendsto`

English:
lemma IsProperMap.ultrafilter_le_nhds_of_tendsto
  given: (h : IsProperMap f) ⦃𝒰
  statement: Ultrafilter X⦄ ⦃y : Y⦄
  proof: (isProperMap_iff_ultrafilter.mp h).2 hy

中文:
引理 是真映射.ultrafilter_le_nhds_of_tendsto
  条件: (h : 是真映射 f) ⦃𝒰
  结论: Ultrafilter X⦄ ⦃y : Y⦄
  证明: (isProperMap_iff_ultrafilter.mp h).2 hy

Depends on / 依赖: isProperMap_iff_ultrafilter, isProperMap_iff_ultrafilter.mp
-/
lemma IsProperMap.ultrafilter_le_nhds_of_tendsto (h : IsProperMap f) ⦃𝒰 : Ultrafilter X⦄ ⦃y : Y⦄
    (hy : Tendsto f 𝒰 (𝓝 y)) : exists x, f x = y ∧ 𝒰 <= 𝓝 x :=
  (isProperMap_iff_ultrafilter.mp h).2 hy

/--
lemma `IsProperMap.comp` / 引理 `IsProperMap.comp`

English:
lemma IsProperMap.comp
  given: (hg : IsProperMap g) (hf : IsProperMap f)
  proof: by
  refine ⟨by fun_prop, fun ℱ z h => ?_⟩
  rw [mapClusterPt_comp] at h
  rcases hg.clusterPt_of_mapClusterPt h with ⟨y, rfl, hy⟩
  rcases hf.clusterPt_of_mapClusterPt hy with ⟨x, rfl, hx⟩
  use x, rfl

中文:
引理 是真映射.comp
  条件: (hg : 是真映射 g) (hf : 是真映射 f)
  证明: by
  refine ⟨by fun_prop, fun ℱ z h => ?_⟩
  rw [mapClusterPt_comp] at h
  rcases hg.clusterPt_of_mapClusterPt h with ⟨y, rfl, hy⟩
  rcases hf.clusterPt_of_mapClusterPt hy with ⟨x, rfl, hx⟩
  use x, rfl

Depends on / 依赖: clusterPt_of_mapClusterPt, fun_prop, hf.clusterPt_of_mapClusterPt, hg.clusterPt_of_mapClusterPt, mapClusterPt_comp
-/
lemma IsProperMap.comp (hg : IsProperMap g) (hf : IsProperMap f) :
    IsProperMap (g ∘ f) := by
  refine ⟨by fun_prop, fun ℱ z h => ?_⟩
  rw [mapClusterPt_comp] at h
  rcases hg.clusterPt_of_mapClusterPt h with ⟨y, rfl, hy⟩
  rcases hf.clusterPt_of_mapClusterPt hy with ⟨x, rfl, hx⟩
  use x, rfl

/--
lemma `isProperMap_of_comp_of_surj` / 引理 `isProperMap_of_comp_of_surj`

English:
lemma isProperMap_of_comp_of_surj
  statement: (hf : Continuous f)
  proof: by
  refine ⟨hg, fun ℱ z h => ?_⟩
  rw [← ℱ.map_comap_of_surjective f_surj]; rw [← mapClusterPt_comp] at h
  rcases hgf.clusterPt_of_mapClusterPt h with ⟨x, rfl, hx⟩
  rw [← ℱ.map_comap_of_surjective f_surj]
  exact ⟨f x, rfl, hx.map hf.continuousAt tendsto_map⟩

中文:
引理 isProperMap_of_comp_of_surj
  结论: (hf : 连续 f)
  证明: by
  refine ⟨hg, fun ℱ z h => ?_⟩
  rw [← ℱ.map_comap_of_surjective f_surj]; rw [← mapClusterPt_comp] at h
  rcases hgf.clusterPt_of_mapClusterPt h with ⟨x, rfl, hx⟩
  rw [← ℱ.map_comap_of_surjective f_surj]
  exact ⟨f x, rfl, hx.map hf.continuousAt tendsto_map⟩

Depends on / 依赖: clusterPt_of_mapClusterPt, continuousAt, f_surj, hf.continuousAt, hgf.clusterPt_of_mapClusterPt, hx.map, mapClusterPt_comp, map_comap_of_surjective, tendsto_map
-/
lemma isProperMap_of_comp_of_surj (hf : Continuous f)
    (hg : Continuous g) (hgf : IsProperMap (g ∘ f)) (f_surj : f.Surjective) : IsProperMap g := by
  refine ⟨hg, fun ℱ z h => ?_⟩
  rw [← ℱ.map_comap_of_surjective f_surj]; rw [← mapClusterPt_comp] at h
  rcases hgf.clusterPt_of_mapClusterPt h with ⟨x, rfl, hx⟩
  rw [← ℱ.map_comap_of_surjective f_surj]
  exact ⟨f x, rfl, hx.map hf.continuousAt tendsto_map⟩

/--
lemma `isProperMap_of_comp_of_inj` / 引理 `isProperMap_of_comp_of_inj`

English:
lemma isProperMap_of_comp_of_inj
  statement: {f : X -> Y} {g : Y -> Z} (hf : Continuous f) (hg : Continuous g)
  proof: by
  refine ⟨hf, fun ℱ y h => ?_⟩
  rcases hgf.clusterPt_of_mapClusterPt (h.map hg.continuousAt tendsto_map) with ⟨x, hx1, hx2⟩
  exact ⟨x, g_inj hx1, hx2⟩

中文:
引理 isProperMap_of_comp_of_inj
  结论: {f : X -> Y} {g : Y -> Z} (hf : 连续 f) (hg : 连续 g)
  证明: by
  refine ⟨hf, fun ℱ y h => ?_⟩
  rcases hgf.clusterPt_of_mapClusterPt (h.map hg.continuousAt tendsto_map) with ⟨x, hx1, hx2⟩
  exact ⟨x, g_inj hx1, hx2⟩

Depends on / 依赖: clusterPt_of_mapClusterPt, continuousAt, g_inj, h.map, hg.continuousAt, hgf.clusterPt_of_mapClusterPt, tendsto_map
-/
lemma isProperMap_of_comp_of_inj {f : X -> Y} {g : Y -> Z} (hf : Continuous f) (hg : Continuous g)
    (hgf : IsProperMap (g ∘ f)) (g_inj : g.Injective) : IsProperMap f := by
  refine ⟨hf, fun ℱ y h => ?_⟩
  rcases hgf.clusterPt_of_mapClusterPt (h.map hg.continuousAt tendsto_map) with ⟨x, hx1, hx2⟩
  exact ⟨x, g_inj hx1, hx2⟩

/--
lemma `isProperMap_of_comp_of_t2` / 引理 `isProperMap_of_comp_of_t2`

English:
lemma isProperMap_of_comp_of_t2
  statement: [T2Space Y] (hf : Continuous f) (hg : Continuous g)
  proof: by
  rw [isProperMap_iff_ultrafilter_of_t2]
  refine ⟨hf, fun 𝒰 y h => ?_⟩
  rw [isProperMap_iff_ultrafilter] at hgf
  rcases hgf.2 ((hg.tendsto y).comp h) with ⟨x, -, hx⟩
  exact ⟨x, hx⟩

中文:
引理 isProperMap_of_comp_of_t2
  结论: [T2空间 Y] (hf : 连续 f) (hg : 连续 g)
  证明: by
  rw [isProperMap_iff_ultrafilter_of_t2]
  refine ⟨hf, fun 𝒰 y h => ?_⟩
  rw [isProperMap_iff_ultrafilter] at hgf
  rcases hgf.2 ((hg.tendsto y).comp h) with ⟨x, -, hx⟩
  exact ⟨x, hx⟩

Depends on / 依赖: hg.tendsto, isProperMap_iff_ultrafilter, isProperMap_iff_ultrafilter_of_t2, tendsto
-/
lemma isProperMap_of_comp_of_t2 [T2Space Y] (hf : Continuous f) (hg : Continuous g)
    (hgf : IsProperMap (g ∘ f)) : IsProperMap f := by
  rw [isProperMap_iff_ultrafilter_of_t2]
  refine ⟨hf, fun 𝒰 y h => ?_⟩
  rw [isProperMap_iff_ultrafilter] at hgf
  rcases hgf.2 ((hg.tendsto y).comp h) with ⟨x, -, hx⟩
  exact ⟨x, hx⟩

/--
lemma `IsProperMap.prodMap` / 引理 `IsProperMap.prodMap`

English:
lemma IsProperMap.prodMap
  given: {g : Z -> W} (hf : IsProperMap f) (hg : IsProperMap g)
  proof: by
  simp_rw [isProperMap_iff_ultrafilter] at hf hg ⊢
  constructor
  -- Continuity is clear.
  · exact hf.1.prodMap hg.1
  -- Let `𝒰 : Ultrafilter (X × Z)`, and assume that `f × g` tends to some `(y, w) : Y × W`
  -- along `𝒰`.
  · intro 𝒰 ⟨y, w⟩ hyw
  -- That means that `f` tends to `y` along `map

中文:
引理 是真映射.prodMap
  条件: {g : Z -> W} (hf : 是真映射 f) (hg : 是真映射 g)
  证明: by
  simp_rw [isProperMap_iff_ultrafilter] at hf hg ⊢
  constructor
  -- Continuity is clear.
  · exact hf.1.prodMap hg.1
  -- Let `𝒰 : Ultrafilter (X × Z)`, and assume that `f × g` tends to some `(y, w) : Y × W`
  -- along `𝒰`.
  · intro 𝒰 ⟨y, w⟩ hyw
  -- That means that `f` tends to `y` along `map

Depends on / 依赖: isProperMap_iff_ultrafilter, simp_rw
-/
lemma IsProperMap.prodMap {g : Z -> W} (hf : IsProperMap f) (hg : IsProperMap g) :
    IsProperMap (Prod.map f g) := by
  simp_rw [isProperMap_iff_ultrafilter] at hf hg ⊢
  constructor
  -- Continuity is clear.
  · exact hf.1.prodMap hg.1
  -- Let `𝒰 : Ultrafilter (X × Z)`, and assume that `f × g` tends to some `(y, w) : Y × W`
  -- along `𝒰`.
  · intro 𝒰 ⟨y, w⟩ hyw
  -- That means that `f` tends to `y` along `map fst 𝒰` and `g` tends to `w` along `map snd 𝒰`.
    simp_rw [nhds_prod_eq, tendsto_prod_iff'] at hyw
  -- Thus, by properness of `f` and `g`, we get some `x : X` and `z : Z` such that `f x = y`,
  -- `g z = w`, `map fst 𝒰` tends to `x`, and `map snd 𝒰` tends to `y`.
    rcases hf.2 (show Tendsto f (Ultrafilter.map fst 𝒰) (𝓝 y) by simpa using! hyw.1) with
      ⟨x, hxy, hx⟩
    rcases hg.2 (show Tendsto g (Ultrafilter.map snd 𝒰) (𝓝 w) by simpa using! hyw.2) with
      ⟨z, hzw, hz⟩
  -- By the properties of the product topology, that means that `𝒰` tends to `(x, z)`,
  -- which completes the proof since `(f × g)(x, z) = (y, w)`.
    refine ⟨⟨x, z⟩, Prod.ext hxy hzw, ?_⟩
    rw [nhds_prod_eq]; rw [le_prod]
    exact ⟨hx, hz⟩

/--
lemma `IsProperMap.pi_map` / 引理 `IsProperMap.pi_map`

English:
lemma IsProperMap.pi_map
  statement: {X Y : ι -> Type*} [forall i, TopologicalSpace (X i)]
  proof: by
  simp_rw [isProperMap_iff_ultrafilter] at h ⊢
  constructor
  -- Continuity is clear.
  · exact continuous_pi fun i => (h i).1.comp (continuous_apply i)
  -- Let `𝒰 : Ultrafilter (Π i, X i)`, and assume that `Π i, f i` tends to some `y : Π i, Y i`
  -- along `𝒰`.
  · intro 𝒰 y hy
  -- That means

中文:
引理 是真映射.pi_map
  结论: {X Y : ι -> 类型} [对任意 i, 拓扑空间 (X i)]
  证明: by
  simp_rw [isProperMap_iff_ultrafilter] at h ⊢
  constructor
  -- Continuity is clear.
  · exact continuous_pi fun i => (h i).1.comp (continuous_apply i)
  -- Let `𝒰 : Ultrafilter (Π i, X i)`, and assume that `Π i, f i` tends to some `y : Π i, Y i`
  -- along `𝒰`.
  · intro 𝒰 y hy
  -- That means

Depends on / 依赖: isProperMap_iff_ultrafilter, simp_rw
-/
lemma IsProperMap.pi_map {X Y : ι -> Type*} [forall i, TopologicalSpace (X i)]
    [forall i, TopologicalSpace (Y i)] {f : (i : ι) -> X i -> Y i} (h : forall i, IsProperMap (f i)) :
    IsProperMap (fun (x : forall i, X i) i => f i (x i)) := by
  simp_rw [isProperMap_iff_ultrafilter] at h ⊢
  constructor
  -- Continuity is clear.
  · exact continuous_pi fun i => (h i).1.comp (continuous_apply i)
  -- Let `𝒰 : Ultrafilter (Π i, X i)`, and assume that `Π i, f i` tends to some `y : Π i, Y i`
  -- along `𝒰`.
  · intro 𝒰 y hy
  -- That means that each `f i` tends to `y i` along `map (eval i) 𝒰`.
    have : forall i, Tendsto (f i) (Ultrafilter.map (eval i) 𝒰) (𝓝 (y i)) := by
      simpa [tendsto_pi_nhds] using! hy
  -- Thus, by properness of all the `f i`s, we can choose some `x : Π i, X i` such that, for all
  -- `i`, `f i (x i) = y i` and `map (eval i) 𝒰` tends to `x i`.
    choose x hxy hx using fun i => (h i).2 (this i)
  -- By the properties of the product topology, that means that `𝒰` tends to `x`,
  -- which completes the proof since `(Π i, f i) x = y`.
    refine ⟨x, funext hxy, ?_⟩
    rwa [nhds_pi, le_pi]

/--
lemma `IsProperMap.isCompact_preimage` / 引理 `IsProperMap.isCompact_preimage`

English:
lemma IsProperMap.isCompact_preimage
  given: (h : IsProperMap f) {K : Set Y} (hK : IsCompact K)
  proof: by
  rw [isCompact_iff_ultrafilter_le_nhds]
  -- Let `𝒰 ≤ 𝓟 (f ⁻¹' K)` an ultrafilter.
  intro 𝒰 h𝒰
  -- In other words, we have `map f 𝒰 ≤ 𝓟 K`
  rw [← comap_principal]; rw [← map_le_iff_le_comap]; rw [← Ultrafilter.coe_map] at h𝒰
  -- Thus, by compactness of `K`, the ultrafilter `map f 𝒰` tends to

中文:
引理 是真映射.isCompact_preimage
  条件: (h : 是真映射 f) {K : 集合 Y} (hK : 是紧集 K)
  证明: by
  rw [isCompact_iff_ultrafilter_le_nhds]
  -- Let `𝒰 ≤ 𝓟 (f ⁻¹' K)` an ultrafilter.
  intro 𝒰 h𝒰
  -- In other words, we have `map f 𝒰 ≤ 𝓟 K`
  rw [← comap_principal]; rw [← map_le_iff_le_comap]; rw [← Ultrafilter.coe_map] at h𝒰
  -- Thus, by compactness of `K`, the ultrafilter `map f 𝒰` tends to

Depends on / 依赖: isCompact_iff_ultrafilter_le_nhds
-/
lemma IsProperMap.isCompact_preimage (h : IsProperMap f) {K : Set Y} (hK : IsCompact K) :
    IsCompact (f ⁻¹' K) := by
  rw [isCompact_iff_ultrafilter_le_nhds]
  -- Let `𝒰 ≤ 𝓟 (f ⁻¹' K)` an ultrafilter.
  intro 𝒰 h𝒰
  -- In other words, we have `map f 𝒰 ≤ 𝓟 K`
  rw [← comap_principal]; rw [← map_le_iff_le_comap]; rw [← Ultrafilter.coe_map] at h𝒰
  -- Thus, by compactness of `K`, the ultrafilter `map f 𝒰` tends to some `y ∈ K`.
  rcases hK.ultrafilter_le_nhds _ h𝒰 with ⟨y, hyK, hy⟩
  -- Then, by properness of `f`, that means that `𝒰` tends to some `x ∈ f ⁻¹' {y} ⊆ f ⁻¹' K`,
  -- which completes the proof.
  rcases h.ultrafilter_le_nhds_of_tendsto hy with ⟨x, rfl, hx⟩
  exact ⟨x, hyK, hx⟩

/--
theorem `isProperMap_iff_isClosedMap_and_compact_fibers` / 定理 `isProperMap_iff_isClosedMap_and_compact_fibers`

English:
theorem isProperMap_iff_isClosedMap_and_compact_fibers
  proof: by
  constructor <;> intro H
  -- Note: In Bourbaki, the direct implication is proved by going through universally closed maps.
  -- We could do the same (using a `TFAE` cycle) but proving it directly from
  -- `IsProperMap.isCompact_preimage` is nice enough already so we don't bother with that.
  ·

中文:
定理 isProperMap_iff_isClosedMap_and_compact_fibers
  证明: by
  constructor <;> intro H
  -- Note: In Bourbaki, the direct implication is proved by going through universally closed maps.
  -- We could do the same (using a `TFAE` cycle) but proving it directly from
  -- `IsProperMap.isCompact_preimage` is nice enough already so we don't bother with that.
  ·
-/
theorem isProperMap_iff_isClosedMap_and_compact_fibers :
    IsProperMap f ↔ Continuous f ∧ IsClosedMap f ∧ forall y, IsCompact (f ⁻¹' {y}) := by
  constructor <;> intro H
  -- Note: In Bourbaki, the direct implication is proved by going through universally closed maps.
  -- We could do the same (using a `TFAE` cycle) but proving it directly from
  -- `IsProperMap.isCompact_preimage` is nice enough already so we don't bother with that.
  · exact ⟨H.continuous, H.isClosedMap, fun y => H.isCompact_preimage isCompact_singleton⟩
  · rw [isProperMap_iff_clusterPt]
  -- Let `ℱ : Filter X` and `y` some cluster point of `map f ℱ`.
    refine ⟨H.1, fun ℱ y hy => ?_⟩
  -- That means that the singleton `pure y` meets the "closure" of `map f ℱ`, by which we mean
  -- `Filter.lift' (map f ℱ) closure`. But `f` is closed, so
  -- `closure (map f ℱ) = map f (closure ℱ)` (see `IsClosedMap.lift'_closure_map_eq`).
  -- Thus `map f (closure ℱ ⊓ 𝓟 (f ⁻¹' {y})) = map f (closure ℱ) ⊓ 𝓟 {y} ≠ ⊥`, hence
  -- `closure ℱ ⊓ 𝓟 (f ⁻¹' {y}) ≠ ⊥`.
    rw [H.2.1.mapClusterPt_iff_lift'_closure H.1] at hy
  -- Now, applying the compactness of `f ⁻¹' {y}` to the nontrivial filter
  -- `closure ℱ ⊓ 𝓟 (f ⁻¹' {y})`, we obtain that it has a cluster point `x ∈ f ⁻¹' {y}`.
    rcases H.2.2 y (f := Filter.lift' ℱ closure ⊓ 𝓟 (f ⁻¹' {y})) inf_le_right with ⟨x, hxy, hx⟩
    refine ⟨x, hxy, ?_⟩
  -- In particular `x` is a cluster point of `closure ℱ`. Since cluster points of `closure ℱ`
  -- are exactly cluster points of `ℱ` (see `clusterPt_lift'_closure_iff`), this completes
  -- the proof.
    rw [← clusterPt_lift'_closure_iff]
    exact hx.mono inf_le_left

/--
lemma `isProperMap_iff_isClosedMap_of_inj` / 引理 `isProperMap_iff_isClosedMap_of_inj`

English:
lemma isProperMap_iff_isClosedMap_of_inj
  given: (f_cont : Continuous f) (f_inj : f.Injective)
  proof: by
  refine ⟨fun h => h.isClosedMap, fun h => ?_⟩
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  exact ⟨f_cont, h, fun y => (subsingleton_singleton.preimage f_inj).isCompact⟩

中文:
引理 isProperMap_iff_isClosedMap_of_inj
  条件: (f_cont : 连续 f) (f_inj : f.单射)
  证明: by
  refine ⟨fun h => h.isClosedMap, fun h => ?_⟩
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  exact ⟨f_cont, h, fun y => (subsingleton_singleton.preimage f_inj).isCompact⟩

Depends on / 依赖: f_cont, f_inj, h.isClosedMap, isClosedMap, isCompact, isProperMap_iff_isClosedMap_and_compact_fibers, preimage, subsingleton_singleton, subsingleton_singleton.preimage
-/
lemma isProperMap_iff_isClosedMap_of_inj (f_cont : Continuous f) (f_inj : f.Injective) :
    IsProperMap f ↔ IsClosedMap f := by
  refine ⟨fun h => h.isClosedMap, fun h => ?_⟩
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  exact ⟨f_cont, h, fun y => (subsingleton_singleton.preimage f_inj).isCompact⟩

/--
lemma `isProperMap_of_isClosedMap_of_inj` / 引理 `isProperMap_of_isClosedMap_of_inj`

English:
lemma isProperMap_of_isClosedMap_of_inj
  statement: (f_cont : Continuous f) (f_inj : f.Injective)
  proof: (isProperMap_iff_isClosedMap_of_inj f_cont f_inj).2 f_closed

中文:
引理 isProperMap_of_isClosedMap_of_inj
  结论: (f_cont : 连续 f) (f_inj : f.单射)
  证明: (isProperMap_iff_isClosedMap_of_inj f_cont f_inj).2 f_closed

Depends on / 依赖: f_closed, f_cont, f_inj, isProperMap_iff_isClosedMap_of_inj
-/
lemma isProperMap_of_isClosedMap_of_inj (f_cont : Continuous f) (f_inj : f.Injective)
    (f_closed : IsClosedMap f) : IsProperMap f :=
  (isProperMap_iff_isClosedMap_of_inj f_cont f_inj).2 f_closed

/--
lemma `Homeomorph.isProperMap` / 引理 `Homeomorph.isProperMap`

English:
lemma Homeomorph.isProperMap
  given: (e : X ≃ₜ Y)
  statement: IsProperMap e
  proof: isProperMap_of_isClosedMap_of_inj e.continuous e.injective e.isClosedMap

中文:
引理 同胚.isProperMap
  条件: (e : X ≃ₜ Y)
  结论: 是真映射 e
  证明: isProperMap_of_isClosedMap_of_inj e.continuous e.injective e.isClosedMap
-/
@[simp] lemma Homeomorph.isProperMap (e : X ≃ₜ Y) : IsProperMap e :=
  isProperMap_of_isClosedMap_of_inj e.continuous e.injective e.isClosedMap

/--
lemma `IsHomeomorph.isProperMap` / 引理 `IsHomeomorph.isProperMap`

English:
lemma IsHomeomorph.isProperMap
  given: (hf : IsHomeomorph f)
  statement: IsProperMap f
  proof: isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap

中文:
引理 是同胚.isProperMap
  条件: (hf : 是同胚 f)
  结论: 是真映射 f
  证明: isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap
-/
protected lemma IsHomeomorph.isProperMap (hf : IsHomeomorph f) : IsProperMap f :=
  isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap

/--
lemma `isProperMap_id` / 引理 `isProperMap_id`

English:
lemma isProperMap_id
  statement: IsProperMap (id : X -> X)
  proof: IsHomeomorph.id.isProperMap

中文:
引理 isProperMap_id
  结论: 是真映射 (id : X -> X)
  证明: IsHomeomorph.id.isProperMap
-/
@[simp] lemma isProperMap_id : IsProperMap (id : X -> X) := IsHomeomorph.id.isProperMap

/--
lemma `Topology.IsClosedEmbedding.isProperMap` / 引理 `Topology.IsClosedEmbedding.isProperMap`

English:
lemma Topology.IsClosedEmbedding.isProperMap
  given: (hf : IsClosedEmbedding f)
  statement: IsProperMap f
  proof: isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap

中文:
引理 拓扑.是闭嵌入.isProperMap
  条件: (hf : 是闭嵌入 f)
  结论: 是真映射 f
  证明: isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap

Depends on / 依赖: continuous, hf.continuous, hf.injective, hf.isClosedMap, injective, isClosedMap, isProperMap_of_isClosedMap_of_inj
-/
lemma Topology.IsClosedEmbedding.isProperMap (hf : IsClosedEmbedding f) : IsProperMap f :=
  isProperMap_of_isClosedMap_of_inj hf.continuous hf.injective hf.isClosedMap

/--
lemma `IsClosed.isProperMap_subtypeVal` / 引理 `IsClosed.isProperMap_subtypeVal`

English:
lemma IsClosed.isProperMap_subtypeVal
  given: {C : Set X} (hC : IsClosed C)
  statement: IsProperMap ((↑) : C -> X)
  proof: hC.isClosedEmbedding_subtypeVal.isProperMap

中文:
引理 是闭集.isProperMap_subtypeVal
  条件: {C : 集合 X} (hC : 是闭集 C)
  结论: 是真映射 ((↑) : C -> X)
  证明: hC.isClosedEmbedding_subtypeVal.isProperMap

Depends on / 依赖: hC.isClosedEmbedding_subtypeVal.isProperMap, isClosedEmbedding_subtypeVal, isProperMap
-/
lemma IsClosed.isProperMap_subtypeVal {C : Set X} (hC : IsClosed C) : IsProperMap ((↑) : C -> X) :=
  hC.isClosedEmbedding_subtypeVal.isProperMap

/--
lemma `IsProperMap.restrict` / 引理 `IsProperMap.restrict`

English:
lemma IsProperMap.restrict
  given: {C : Set X} (hf : IsProperMap f) (hC : IsClosed C)
  proof: hf.comp hC.isProperMap_subtypeVal

中文:
引理 是真映射.restrict
  条件: {C : 集合 X} (hf : 是真映射 f) (hC : 是闭集 C)
  证明: hf.comp hC.isProperMap_subtypeVal

Depends on / 依赖: hC.isProperMap_subtypeVal, hf.comp, isProperMap_subtypeVal
-/
lemma IsProperMap.restrict {C : Set X} (hf : IsProperMap f) (hC : IsClosed C) :
    IsProperMap fun x : C => f x := hf.comp hC.isProperMap_subtypeVal

/--
lemma `IsProperMap.isClosed_range` / 引理 `IsProperMap.isClosed_range`

English:
lemma IsProperMap.isClosed_range
  given: (hf : IsProperMap f)
  statement: IsClosed (range f)
  proof: hf.isClosedMap.isClosed_range

中文:
引理 是真映射.isClosed_range
  条件: (hf : 是真映射 f)
  结论: 是闭集 (range f)
  证明: hf.isClosedMap.isClosed_range

Depends on / 依赖: hf.isClosedMap.isClosed_range, isClosedMap, isClosed_range
-/
lemma IsProperMap.isClosed_range (hf : IsProperMap f) : IsClosed (range f) :=
  hf.isClosedMap.isClosed_range

/--
lemma `isProperMap_iff_isClosedMap_and_tendsto_cofinite` / 引理 `isProperMap_iff_isClosedMap_and_tendsto_cofinite`

English:
lemma isProperMap_iff_isClosedMap_and_tendsto_cofinite
  given: [T1Space Y]
  proof: by
  simp_rw [isProperMap_iff_isClosedMap_and_compact_fibers, Tendsto,
    le_cofinite_iff_compl_singleton_mem, mem_map, preimage_compl]
  refine and_congr_right fun f_cont => and_congr_right fun _ =>
    ⟨fun H y => (H y).compl_mem_cocompact, fun H y => ?_⟩
  rcases mem_cocompact.mp (H y) with ⟨K, 

中文:
引理 isProperMap_iff_isClosedMap_and_tendsto_cofinite
  条件: [T1空间 Y]
  证明: by
  simp_rw [isProperMap_iff_isClosedMap_and_compact_fibers, Tendsto,
    le_cofinite_iff_compl_singleton_mem, mem_map, preimage_compl]
  refine and_congr_right fun f_cont => and_congr_right fun _ =>
    ⟨fun H y => (H y).compl_mem_cocompact, fun H y => ?_⟩
  rcases mem_cocompact.mp (H y) with ⟨K, 

Depends on / 依赖: Tendsto, and_congr_right, compl_le_compl_iff_le, compl_le_compl_iff_le.mp, compl_mem_cocompact, f_cont, hK.of_isClosed_subset, isClosed_singleton, isClosed_singleton.preimage, isProperMap_iff_isClosedMap_and_compact_fibers, le_cofinite_iff_compl_singleton_mem, mem_cocompact, mem_cocompact.mp, mem_map, of_isClosed_subset, preimage, preimage_compl, simp_rw
-/
lemma isProperMap_iff_isClosedMap_and_tendsto_cofinite [T1Space Y] :
    IsProperMap f ↔ Continuous f ∧ IsClosedMap f ∧ Tendsto f (cocompact X) cofinite := by
  simp_rw [isProperMap_iff_isClosedMap_and_compact_fibers, Tendsto,
    le_cofinite_iff_compl_singleton_mem, mem_map, preimage_compl]
  refine and_congr_right fun f_cont => and_congr_right fun _ =>
    ⟨fun H y => (H y).compl_mem_cocompact, fun H y => ?_⟩
  rcases mem_cocompact.mp (H y) with ⟨K, hK, hKy⟩
  exact hK.of_isClosed_subset (isClosed_singleton.preimage f_cont)
    (compl_le_compl_iff_le.mp hKy)

/--
theorem `Continuous.isProperMap` / 定理 `Continuous.isProperMap`

English:
theorem Continuous.isProperMap
  given: [CompactSpace X] [T2Space Y] (hf : Continuous f)
  statement: IsProperMap f
  proof: isProperMap_iff_isClosedMap_and_tendsto_cofinite.2 ⟨hf, hf.isClosedMap, by simp⟩

中文:
定理 连续.isProperMap
  条件: [紧空间 X] [T2空间 Y] (hf : 连续 f)
  结论: 是真映射 f
  证明: isProperMap_iff_isClosedMap_and_tendsto_cofinite.2 ⟨hf, hf.isClosedMap, by simp⟩

Depends on / 依赖: hf.isClosedMap, isClosedMap, isProperMap_iff_isClosedMap_and_tendsto_cofinite
-/
theorem Continuous.isProperMap [CompactSpace X] [T2Space Y] (hf : Continuous f) : IsProperMap f :=
  isProperMap_iff_isClosedMap_and_tendsto_cofinite.2 ⟨hf, hf.isClosedMap, by simp⟩

/--
theorem `isProperMap_const_iff` / 定理 `isProperMap_const_iff`

English:
theorem isProperMap_const_iff
  given: [T1Space Y] (y : Y)
  proof: by
  classical
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  constructor
  · rintro ⟨-, -, h⟩
    exact ⟨by simpa using h y⟩
  · intro H
    refine ⟨continuous_const, isClosedMap_const, fun y' => ?_⟩
    simp [preimage_const, mem_singleton_iff, apply_ite, isCompact_univ]

中文:
定理 isProperMap_const_iff
  条件: [T1空间 Y] (y : Y)
  证明: by
  classical
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  constructor
  · rintro ⟨-, -, h⟩
    exact ⟨by simpa using h y⟩
  · intro H
    refine ⟨continuous_const, isClosedMap_const, fun y' => ?_⟩
    simp [preimage_const, mem_singleton_iff, apply_ite, isCompact_univ]

Depends on / 依赖: apply_ite, classical, continuous_const, isClosedMap_const, isCompact_univ, isProperMap_iff_isClosedMap_and_compact_fibers, mem_singleton_iff, preimage_const
-/
theorem isProperMap_const_iff [T1Space Y] (y : Y) :
    IsProperMap (fun _ : X => y) ↔ CompactSpace X := by
  classical
  rw [isProperMap_iff_isClosedMap_and_compact_fibers]
  constructor
  · rintro ⟨-, -, h⟩
    exact ⟨by simpa using h y⟩
  · intro H
    refine ⟨continuous_const, isClosedMap_const, fun y' => ?_⟩
    simp [preimage_const, mem_singleton_iff, apply_ite, isCompact_univ]

/--
theorem `isProperMap_const` / 定理 `isProperMap_const`

English:
theorem isProperMap_const
  given: [h : CompactSpace X] [T1Space Y] (y : Y)
  proof: .mpr h isProperMap_const_iff y

中文:
定理 isProperMap_const
  条件: [h : 紧空间 X] [T1空间 Y] (y : Y)
  证明: .mpr h isProperMap_const_iff y

Depends on / 依赖: isProperMap_const_iff
-/
theorem isProperMap_const [h : CompactSpace X] [T1Space Y] (y : Y) :
    IsProperMap (fun _ : X => y) :=
.mpr h isProperMap_const_iff y

/--
theorem `isProperMap_fst_of_compactSpace` / 定理 `isProperMap_fst_of_compactSpace`

English:
theorem isProperMap_fst_of_compactSpace
  given: [CompactSpace Y]
  proof: .isProperMap.comp (isProperMap_id.prodMap (isProperMap_const ())) Homeomorph.prodPUnit X

中文:
定理 isProperMap_fst_of_compactSpace
  条件: [紧空间 Y]
  证明: .isProperMap.comp (isProperMap_id.prodMap (isProperMap_const ())) Homeomorph.prodPUnit X

Depends on / 依赖: Homeomorph, Homeomorph.prodPUnit, isProperMap, isProperMap.comp, isProperMap_const, isProperMap_id, isProperMap_id.prodMap, prodMap, prodPUnit
-/
theorem isProperMap_fst_of_compactSpace [CompactSpace Y] :
    IsProperMap (Prod.fst : X × Y -> X) :=
.isProperMap.comp (isProperMap_id.prodMap (isProperMap_const ())) Homeomorph.prodPUnit X

/--
theorem `isProperMap_snd_of_compactSpace` / 定理 `isProperMap_snd_of_compactSpace`

English:
theorem isProperMap_snd_of_compactSpace
  given: [CompactSpace X]
  proof: .isProperMap.comp ((isProperMap_const ()).prodMap isProperMap_id) Homeomorph.punitProd Y

中文:
定理 isProperMap_snd_of_compactSpace
  条件: [紧空间 X]
  证明: .isProperMap.comp ((isProperMap_const ()).prodMap isProperMap_id) Homeomorph.punitProd Y

Depends on / 依赖: Homeomorph, Homeomorph.punitProd, isProperMap, isProperMap.comp, isProperMap_const, isProperMap_id, prodMap, punitProd
-/
theorem isProperMap_snd_of_compactSpace [CompactSpace X] :
    IsProperMap (Prod.snd : X × Y -> Y) :=
.isProperMap.comp ((isProperMap_const ()).prodMap isProperMap_id) Homeomorph.punitProd Y

/--
theorem `isClosedMap_fst_of_compactSpace` / 定理 `isClosedMap_fst_of_compactSpace`

English:
theorem isClosedMap_fst_of_compactSpace
  given: [CompactSpace Y]
  proof: isProperMap_fst_of_compactSpace.isClosedMap

中文:
定理 isClosedMap_fst_of_compactSpace
  条件: [紧空间 Y]
  证明: isProperMap_fst_of_compactSpace.isClosedMap

Depends on / 依赖: isClosedMap, isProperMap_fst_of_compactSpace, isProperMap_fst_of_compactSpace.isClosedMap
-/
theorem isClosedMap_fst_of_compactSpace [CompactSpace Y] :
    IsClosedMap (Prod.fst : X × Y -> X) :=
  isProperMap_fst_of_compactSpace.isClosedMap

/--
theorem `isClosedMap_snd_of_compactSpace` / 定理 `isClosedMap_snd_of_compactSpace`

English:
theorem isClosedMap_snd_of_compactSpace
  given: [CompactSpace X]
  proof: isProperMap_snd_of_compactSpace.isClosedMap

中文:
定理 isClosedMap_snd_of_compactSpace
  条件: [紧空间 X]
  证明: isProperMap_snd_of_compactSpace.isClosedMap

Depends on / 依赖: isClosedMap, isProperMap_snd_of_compactSpace, isProperMap_snd_of_compactSpace.isClosedMap
-/
theorem isClosedMap_snd_of_compactSpace [CompactSpace X] :
    IsClosedMap (Prod.snd : X × Y -> Y) :=
  isProperMap_snd_of_compactSpace.isClosedMap

/--
theorem `IsProperMap.universally_closed` / 定理 `IsProperMap.universally_closed`

English:
theorem IsProperMap.universally_closed
  given: (Z) [TopologicalSpace Z] (h : IsProperMap f)
  proof: -- `f × id` is proper as a product of proper maps, hence closed.
  (h.prodMap isProperMap_id).isClosedMap

中文:
定理 是真映射.universally_closed
  条件: (Z) [拓扑空间 Z] (h : 是真映射 f)
  证明: -- `f × id` is proper as a product of proper maps, hence closed.
  (h.prodMap isProperMap_id).isClosedMap
-/
theorem IsProperMap.universally_closed (Z) [TopologicalSpace Z] (h : IsProperMap f) :
    IsClosedMap (Prod.map f id : X × Z -> Y × Z) :=
  -- `f × id` is proper as a product of proper maps, hence closed.
  (h.prodMap isProperMap_id).isClosedMap
