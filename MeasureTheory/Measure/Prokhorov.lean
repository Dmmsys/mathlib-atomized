...
variable (E) in
/--
theorem `isCompact_setOfPred_finiteMeasure_le_of_compactSpace` / 定理 `isCompact_setOfPred_finiteMeasure_le_of_compactSpace`

English:
theorem isCompact_setOfPred_finiteMeasure_le_of_compactSpace
  given: [CompactSpace E] (C : Real>=0)
  proof: by
...
-/
theorem isCompact_setOfPred_finiteMeasure_le_of_compactSpace [CompactSpace E] (C : Real>=0) :
  ...
  have L (g : C_c(E, Real)) :
      exists x in Icc (-C * ‖g.toBoundedContinuousFunction‖) (C * ‖g.toBoundedContinuousFunction‖),
...
variable (E) in
/--
lemma `isCompact_setOfPred_finiteMeasure_eq_of_compactSpace` / 引理 `isCompact_setOfPred_finiteMeasure_eq_of_compactSpace`

English:
lemma isCompact_setOfPred_finiteMeasure_eq_of_compactSpace
  given: [CompactSpace E] (C : Real>=0)
  proof: by
...
Depends on / 依赖: FiniteMeasure, IsCompact, IsCompact.inter_right, fun_prop, inter_right, isClosed_eq, isCompact_setOfPred_finiteMeasure_le_of_compactSpace
-/
lemma isCompact_setOfPred_finiteMeasure_eq_of_compactSpace [CompactSpace E] (C : Real>=0) :
    IsCompact {μ : FiniteMeasure E | μ.mass = C} := by
  have : {μ : FiniteMeasure E | μ.mass = C} = {μ | μ.mass <= C} inter {μ | μ.mass = C} := by grind
  rw [this]
  apply IsCompact.inter_right (isCompact_setOfPred_finiteMeasure_le_of_compactSpace E C)
...
instance [CompactSpace
  signature: E] : CompactSpace (ProbabilityMeasure E)
  body: by
  constructor
...
Depends on / 依赖: ProbabilityMeasure, ProbabilityMeasure.toFiniteMeasure_isEmbedding, isCompact_iff, isCompact_setOfPred_finiteMeasure_eq_of_compactSpace, toFiniteMeasure_isEmbedding
-/
...
/--
lemma `isCompact_setOfPred_finiteMeasure_le_of_isCompact` / 引理 `isCompact_setOfPred_finiteMeasure_le_of_isCompact`
...
中文:
引理 isCompact_setOfPred_finiteMeasure_le_of_isCompact
  证明: by
  let f : K -> E := Subtype.val
  have hf : IsClosedEmbedding f := IsClosedEmbedding.subtypeVal hK.isClosed
  have rf : range f = K := Subtype.range_val
...
Depends on / 依赖: FiniteMeasure, FiniteMeasure.mass_comap_le, IsClosedEmbedding, IsClosedEmbedding.subtypeVal, Subset, Subset.antisymm, Subtype, Subtype.range_val, Subtype.val, antisymm, hK.isClosed, isClosed, mass_comap_le, mem_image, range_val, subtypeVal, toMeasure_map
-/
lemma isCompact_setOfPred_finiteMeasure_le_of_isCompact
    (C : Real>=0) {K : Set E} (hK : IsCompact K) :
  ...
  have : {μ : FiniteMeasure E | μ.mass <= C ∧ μ Kᶜ = 0} = F '' T := by
    ...
    · intro μ hμ
      ...
      rw [Measure.comap_apply _ (Subtype.val_injective)]; rw [image_preimage_eq_inter_range]
...
/--
lemma `isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le` / 引理 `isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le`
...
lemma isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le
  ...
  each of these sets (which is contained in the 

中文:
引理 isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le
  证明: by
  /- Consider a sequence of measures with mass at most `C` and giving mass at most `uₙ` to `Kₙᶜ`,
  for which we want to find a converging subsequence.
  We want to write `⋃ n, Kₙ` as the disjoint union of `disjointed K n`, restrict the measures to
...
-/
lemma isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le
    {u : Nat -> Real>=0} {K : Nat -> Set E} (C : Real>=0)
    (hu : Tendsto u atTop (𝓝 0)) (hK : forall n, IsCompact (K n)) (h : NormalSpace E ∨ Monotone K) :
    IsCompact {μ : FiniteMeasure E | μ.mass <= C ∧ forall n, μ (K n)ᶜ <= u n} := by
  /- Consider a sequence of measures with mass at most `C` and giving mass at most `uₙ` to `Kₙᶜ`,
  for which we want to find a converging subsequence.
  We want to write `⋃ n, Kₙ` as the disjoint union of `disjointed K n`, restrict the measures to
  ...
  have I (μ : FiniteMeasure E) (n : Nat) :
      ∑ i in Finset.range (n + 1), μ.restrict (disjointed K i) = μ.restrict (partialSups K n) := by
    rw [← biUnion_range_succ_disjointed]; rw [FiniteMeasure.restrict_biUnion_finset]
  ...
  have M n : exists (ν : FiniteMeasure E), Measure.InnerRegular (ν : Measure E) ∧
    ...
    obtain ⟨ν, hν, ν_lim⟩ : exists ν in {ρ : FiniteMeasure E | ρ.mass <= C ∧ ρ (partialSups K n)ᶜ = 0},
        Tendsto (fun (ρ : FiniteMeasure E) => ρ.restrict (disjointed K n)) f (𝓝 ν) := by
      simp only [Tendsto]
    ...
    obtain ⟨ν', ν'_reg, ν'_fin, ν'K, hν'⟩ : exists ν', ν'.InnerRegular ∧ IsFiniteMeasure ν' ∧
        ν' (partialSups K n)ᶜ = 0 ∧ forall (g : E ->ᵇ Real), ∫ x, g x ∂ν = ∫ x, g x ∂ν' := by
  ...
  have B : (Measure.sum (fun n => (ν n : Measure E))) univ <= C := by
    ...
    have : Tendsto (fun n => ∑ i in Finset.range (n + 1), (ν i : Measure E) univ) atTop
        (𝓝 (∑' i, (ν i : Measure E) univ)) :=
      (ENNReal.tendsto_nat_tsum _).comp (tendsto_add_atTop_nat 1)
    apply le_of_tendsto' this (fun n => ?_)
    have : ∑ i in Finset.range (n + 1), (ν i : Measure E) univ
        = (∑ i in Finset.range (n + 1), ν i).toMeasure univ := by simp
    rw [this]
    suffices (∑ i in Finset.range (n + 1), ν i).mass <= C by
      convert! ENNReal.coe_le_coe.2 this
      simp
    have : Tendsto (fun (μ : FiniteMeasure E) =>
        (∑ i in Finset.range (n + 1), μ.restrict (disjointed K i)).mass) f
        (𝓝 ((∑ i in Finset.range (n + 1), ν i).mass)) := by
      apply Tendsto.mass
      exact tendsto_finsetSum _ (fun i hi => hν i)
    apply le_of_tendsto this
    filter_upwards [hf] with μ hμ
    rw [I]; rw [restrict_mass]
  ...
  have L : Tendsto id f (𝓝 μ) := by
    ...
    have I1 : forallᶠ n in atTop,
        dist (∫ x, g x ∂(∑ i in Finset.range (n + 1), ν i)) (∫ x, g x ∂μ) < ε / 3 := by
      have : Tendsto (fun n => ∫ x, g x ∂(∑ i in Finset.range n, ν i)) atTop (𝓝 (∫ x, g x ∂μ)) := by
    ...
    -- appear below).
    have I2 : forallᶠ n in atTop, ‖g‖ * u n < ε / 3 := by
    ...
    have : Tendsto (fun (ρ : FiniteMeasure E) =>
        ∫ x, g x ∂(∑ i in Finset.range (n + 1), ρ.restrict (disjointed K i) : FiniteMeasure E)) f
        (𝓝 (∫ x, g x ∂(∑ i in Finset.range (n + 1), ν i : FiniteMeasure E))) := by
    ...
    _ <= dist (∫ (x : E), g x ∂ρ)
          (∫ x, g x ∂(∑ i in Finset.range (n + 1), ρ.restrict (disjointed K i)))
        + dist (∫ x, g x ∂(∑ i in Finset.range (n + 1), ρ.restrict (disjointed K i)))
          (∫ x, g x ∂(∑ i in Finset.range (n + 1), ν i))
        + dist (∫ x, g x ∂(∑ i in Finset.range (n + 1), ν i)) (∫ (x : E), g x ∂μ) :=
      dist_triangle4 _ _ _ _
    -- each term is bounded by `ε / 3` by design.
    _ < ε / 3 + ε / 3 + ε / 3 := by
      gcongr
      · have : ρ = ρ.restrict (partialSups K n)ᶜ +
            ∑ i in Finset.range (n + 1), ρ.restrict (disjointed K i) := by
          rw [I]; rw [← FiniteMeasure.restrict_union disjoint_compl_left (A n).measurableSet]
          simp
        nth_rewrite 1 [this]
        rw [toMeasure_add]; rw [integral_add_measure (g.integrable _) (g.integrable _)]
  ...
  · simp only [mass, mk_apply, μ]
    rw [show C = (C : Real>=0∞).toNNReal by simp]
  ...
  · -- To show that `μ (Kₙᶜ) ≤ uₙ` when the space is normal, we argue that `μ (Kₙᶜ)` is the
    ...
    rw [← ENNReal.coe_le_coe]; rw [ennreal_coeFn_eq_coeFn_toMeasure]; rw [(hK n).isClosed.isOpen_compl.measure_eq_biSup_integral_continuous]
  ...
  · -- to show that `μ (Kₙᶜ) ≤ uₙ` when the sequence `K` is monotone, we argue that the only
    ...
    have : Tendsto (fun m => ∑ i in Finset.range (m + 1), (ν i : Measure E) (K n)ᶜ) atTop
        (𝓝 (∑' i, (ν i : Measure E) (K n)ᶜ)) :=
      (ENNReal.tendsto_nat_tsum _).comp (tendsto_add_atTop_nat 1)
    apply le_of_tendsto this
    filter_upwards [Ici_mem_atTop n] with m (hm : n <= m)
    have : ∑ i in Finset.range (m + 1), (ν i : Measure E) (K n)ᶜ
        = ∑ i in Finset.Ioc n m, (ν i : Measure E) (K n)ᶜ := by
    ...
    rw [this]
    suffices (∑ i in Finset.Ioc n m, ν i).toMeasure univ <= u n by
      apply le_trans _ this
      simp only [toMeasure_sum, Measure.coe_finsetSum, Finset.sum_apply]
      gcongr
      simp
    suffices (∑ i in Finset.Ioc n m, ν i).mass <= u n by
      convert! ENNReal.coe_le_coe.2 this
      simp
    have : Tendsto (fun (μ : FiniteMeasure E) =>
        (∑ i in Finset.Ioc n m, μ.restrict (disjointed K i)).mass) f
        (𝓝 ((∑ i in Finset.Ioc n m, ν i).mass)) := by
      apply Tendsto.mass
      exact tendsto_finsetSum _ (fun i hi => hν i)
    apply le_of_tendsto this
    filter_upwards [hf] with μ hμ
    have : ∑ i in Finset.Ioc n m, μ.restrict (disjointed K i) = μ.restrict (K m \ K n) := by
      rw [← biUnion_Ioc_disjointed_of_monotone h hm]; rw [FiniteMeasure.restrict_biUnion_finset]
      · exact (disjoint_disjointed K).set_pairwise _
      · exact MeasurableSet.disjointed (fun i => (hK i).measurableSet)
    rw [this]; rw [restrict_mass]
...
/--
lemma `isCompact_setOfPred_finiteMeasure_mass_eq_compl_isCompact_le` / 引理 `isCompact_setOfPred_finiteMeasure_mass_eq_compl_isCompact_le`

English:
lemma isCompact_setOfPred_finiteMeasure_mass_eq_compl_isCompact_le
  statement: {u : Nat -> Real>=0}
  proof: by
  have : {μ : FiniteMeasure E | μ.mass = C ∧ forall n, μ (K n)ᶜ <= u n} =
    {μ | μ.mass <= C ∧ forall n, μ (K n)ᶜ <= u n} inter {μ | μ.mass = C} := by ext; grind
  rw [this]
...
中文:
引理 isCompact_setOfPred_finiteMeasure_mass_eq_compl_isCompact_le
  结论: {u : 自然数 -> 实数>=0}
  证明: by
  have : {μ : FiniteMeasure E | μ.mass = C ∧ forall n, μ (K n)ᶜ <= u n} =
    {μ | μ.mass <= C ∧ forall n, μ (K n)ᶜ <= u n} inter {μ | μ.mass = C} := by ext; grind
...
Depends on / 依赖: FiniteMeasure, IsCompact, IsCompact.inter_right, fun_prop, inter_right, isClosed_eq, isCompact_setOfPred_finiteMeasure_mass_le_compl_isCompact_le
-/
lemma isCompact_setOfPred_finiteMeasure_mass_eq_compl_isCompact_le {u : Nat -> Real>=0}
    {K : Nat -> Set E} (C : Real>=0) (hu : Tendsto u atTop (𝓝 0)) (hK : forall n, IsCompact (K n))
...
/--
lemma `isCompact_setOfPred_probabilityMeasure_mass_eq_compl_isCompact_le` / 引理 `isCompact_setOfPred_probabilityMeasure_mass_eq_compl_isCompact_le`

English:
lemma isCompact_setOfPred_probabilityMeasure_mass_eq_compl_isCompact_le
  statement: {u : Nat -> Real>=0}
  proof: by
  apply (ProbabilityMeasure.toFiniteMeasure_isEmbedding E).isCompact_iff.2
  have : ProbabilityMeasure.toFiniteMeasure '' {μ | forall (n : Nat), μ (K n)ᶜ <= u n}
      = {μ : FiniteMeasure E | μ.mass = 1 ∧ forall n, μ (K n)ᶜ <= u n} := by
    ext μ
    simp only [mem_image, mem_ofPred_eq]
    ref

中文:
引理 isCompact_setOfPred_probabilityMeasure_mass_eq_compl_isCompact_le
  结论: {u : 自然数 -> 实数>=0}
  证明: by
  apply (ProbabilityMeasure.toFiniteMeasure_isEmbedding E).isCompact_iff.2
  have : ProbabilityMeasure.toFiniteMeasure '' {μ | forall (n : Nat), μ (K n)ᶜ <= u n}
...
Depends on / 依赖: FiniteMeasure, ProbabilityMeasure, ProbabilityMeasure.toFiniteMeasure, ProbabilityMeasure.toFiniteMeasure_isEmbedding, isCompact_iff, isProbabilityMeasure_iff_real, mem_image, mem_ofPred_eq, toFiniteMeasure, toFiniteMeasure_isEmbedding
-/
lemma isCompact_setOfPred_probabilityMeasure_mass_eq_compl_isCompact_le {u : Nat -> Real>=0}
    {K : Nat -> Set E} (hu : Tendsto u atTop (𝓝 0)) (hK : forall n, IsCompact (K n))
    (h : NormalSpace E ∨ Monotone K) :
    IsCompact {μ : ProbabilityMeasure E | forall n, μ (K n)ᶜ <= u n} := by
  apply (ProbabilityMeasure.toFiniteMeasure_isEmbedding E).isCompact_iff.2
  have : ProbabilityMeasure.toFiniteMeasure '' {μ | forall (n : Nat), μ (K n)ᶜ <= u n}
      = {μ : FiniteMeasure E | μ.mass = 1 ∧ forall n, μ (K n)ᶜ <= u n} := by
    ext μ
...
/--
lemma `isCompact_closure_of_isTightMeasureSet` / 引理 `isCompact_closure_of_isTightMeasureSet`

English:
lemma isCompact_closure_of_isTightMeasureSet
  statement: {S : Set (ProbabilityMeasure E)}
  proof: by
  obtain ⟨u, -, u_pos, u_lim⟩ :
      exists u : Nat -> Real>=0, StrictAnti u ∧ (forall n, 0 < u n) ∧ Tendsto u atTop (𝓝 0) :=
    exists_seq_strictAnti_tendsto 0
  have A n : exists (K : Set E), IsCompact K ∧ forall μ in S, μ Kᶜ <= u n := by
    rcases isTightMeasureSet_iff_exists_isCompact_meas
...
Depends on / 依赖: ENNReal, ENNReal.coe_le_coe, IsCompact, K_comp, Measure, StrictAnti, Tendsto, coe_le_coe, exists_seq_strictAnti_tendsto, isTightMeasureSet_iff_exists_isCompact_measure_compl_le, u_lim, u_pos
-/
lemma isCompact_closure_of_isTightMeasureSet {S : Set (ProbabilityMeasure E)}
    (hS : IsTightMeasureSet {((μ : ProbabilityMeasure E) : Measure E) | μ in S}) :
  ...
  let K' n := ⋃ i in Iic n, K i
  have h'K : IsCompact {μ : ProbabilityMeasure E | forall n, μ (K' n)ᶜ <= u n} := by
...
/--
lemma `exists_measure_iUnion_gt_of_isCompact_closure` / 引理 `exists_measure_iUnion_gt_of_isCompact_closure`
...
lemma exists_measure_iUnion_gt_of_isCompact_closure
  ...
  obtain ⟨ε, hε', rfl⟩ : exists (ε' : Real) (hε' : 0 <= ε'), ε = .mk ε' hε' := ⟨↑ε, ε.2, rfl⟩
...
Depends on / 依赖: ENNReal, ENNReal.coe_pos, Measurebound, NNReal, NNReal.coe_le_coe, NNReal.coe_lt_coe, NNReal.coe_one, NNReal.coe_zero, coe_le_coe, coe_le_one_iff, coe_lt_coe, coe_mk, coe_one, coe_pos, coe_zero, hcomp.isSeqCompact, hcontradiction, hsubmono, isSeqCompact, ne_top_of_le_ne_top
-/
lemma exists_measure_iUnion_gt_of_isCompact_closure
    (U : Nat -> Set 𝓧) (O : forall i, IsOpen (U i)) (Cov : ⋃ i, U i = univ) (hcomp : IsCompact (closure S))
    (ε : Real>=0∞) (hε : 0 < ε) (hεbound : ε <= 1) :
    exists (k : Nat), forall μ in S, 1 - ε < μ (⋃ i <= k, U i) := by
  have εfin : ε != ∞ := ne_top_of_le_ne_top (by simp) hεbound
  lift ε to Real>=0 using εfin
  obtain ⟨ε, hε', rfl⟩ : exists (ε' : Real) (hε' : 0 <= ε'), ε = .mk ε' hε' := ⟨↑ε, ε.2, rfl⟩
  simp only [ENNReal.coe_pos, ← NNReal.coe_lt_coe, NNReal.coe_zero, coe_mk, coe_le_one_iff,
  ...
  have Measurebound n : (μlim (⋃ (i <= n), U i) : Real) <= 1 - ε := calc
    (μlim (⋃ (i <= n), U i) : Real)
    _ <= liminf (fun k => (μ (sub k) (⋃ (i <= n), U i) : Real)) atTop := by
...
(h x (by simp)).trans ProbabilityMeasure.apply_le_one (μ (sub x)) (⋃ i <= n, U i)
    _ <= liminf (fun k => (μ (sub k) (⋃ (i <= sub k), U i) : Real)) atTop := by
 ...
 Set.biUnion_mono (fun i (hi : i <= n) => hi.trans ?_) fun _ _ => le_rfl
  ...
  rw [Cov]; rw [coeFn_univ]; rw [← NNReal.tendsto_coe] at accumulation
  have exceeds_bound : forallᶠ n in atTop, (1 - ε / 2 : Real) <= μlim (⋃ i <= n, U i) :=
      Tendsto.eventually_const_le (v := 1)
        (by simp only [sub_lt_self_iff, Nat.ofNat_pos, div_pos_iff_of_pos_right]; positivity)
        accumulation
  suffices forallᶠ n : Nat in atTop, False from this.exists.choose_spec
...
/--
theorem `isTightMeasureSet_of_isCompact_closure` / 定理 `isTightMeasureSet_of_isCompact_closure`

English:
theorem isTightMeasureSet_of_isCompact_closure
  given: (hcomp : IsCompact (closure S))
  proof: by
...
Depends on / 依赖: StrictAnti, Tendsto, denseRange_iff, exists_dense_seq, exists_seq_strictAnti_tendsto, hempty, hnonempty, hu_anti, hu_pos, isCompact_empty, isEmpty_or_nonempty, isTightMeasureSet_iff_exists_isCompact_measure_compl_le, univ_eq_empty_iff
-/
theorem isTightMeasureSet_of_isCompact_closure (hcomp : IsCompact (closure S)) :
    IsTightMeasureSet {((μ : ProbabilityMeasure 𝓧) : Measure 𝓧) | μ in S} := by
  rw [isTightMeasureSet_iff_exists_isCompact_measure_compl_le]
  rcases isEmpty_or_nonempty 𝓧 with hempty | hnonempty
  · rw [← univ_eq_empty_iff] at hempty
    exact fun ε εpos => ⟨∅, isCompact_empty, by simp [hempty]⟩
  obtain ⟨D, hD⟩ := exists_dense_seq 𝓧
  obtain ⟨u, hu_anti, hu_pos, hu⟩ : exists u, StrictAnti u ∧ (forall n, 0 < u n) ∧ Tendsto u atTop (𝓝 0) :=
    exists_seq_strictAnti_tendsto (0 : Real)
  have hcov (m : Nat) : ⋃ i, ball (D i) (u m) = univ := by
...
exact ⟨fun a => trivial, fun _ => mem_iUnion.mpr hD p (u m) (hu_pos m)⟩
  ...
  · refine ⟨∅, isCompact_empty, fun μ hμ => ?_⟩
    ...
    obtain ⟨μ', hμ', rfl⟩ := hμ
    rw [compl_empty]; rw [measure_univ]
    exact le_of_lt hεbound
  have byclaim (m : Nat) : exists k, forall μ in S, 1 - (ε * 2 ^ (-m : Int) : Real>=0∞) <
      μ (⋃ i <= k, ball (D i) (u m)) := by
    refine exists_measure_iUnion_gt_of_isCompact_closure
      (fun i => ball (D i) (u m)) (fun _ => isOpen_ball) (hcov m) hcomp (ε * 2 ^ (-m : Int)) ?_ ?_
...
· exact Left.mul_le_one hεbound zpow_le_one_of_nonpos (by linarith) (by simp)
  ...
  have bigcalc (μ : ProbabilityMeasure 𝓧) (hs : μ in S) : μ.toMeasure bigKᶜ <= ε := calc
    μ.toMeasure bigKᶜ
  ...
  · refine Metric.totallyBounded_iff.mpr fun δ δpos => ?_
    have ⟨δ_inv, hδ_inv⟩ : exists x, u x < δ := (Tendsto.eventually_lt_const δpos hu).exists
    ...
    _ subseteq ⋃ i <= km (δ_inv + 1), closure (ball (D i) (u δ_inv)) := iInter_subset ..
    _ subseteq ⋃ i <= km (δ_inv + 1), ball (D i) δ := by
        gcongr
