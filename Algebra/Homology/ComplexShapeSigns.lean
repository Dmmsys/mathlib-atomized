...
/-
**TotalComplexShape** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：{I₁ : Type u_1} →   {I₂ : Type u_2} →     {I₁₂ : Type u_4} → ComplexShape 
I₁ → ComplexShape I₂ → ComplexShape I₁₂ → Type (max (max u_1 u_2) u_4)
参数：max (max u_1 u_2) u_4。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A total complex shape for three complex shapes `c₁`, `c₂`, `c₁₂` on three types
`I₁`, `I₂` and `I₁₂` consists of the data and properties that will allow the con
struction
...
variable {c₁}
/-
**ComplexShape.rel_** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ComplexShape.next_** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ComplexShape.prev_** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ComplexShape.TensorSigns** 是 Mathlib 中的一个归纳类型，位于命名空间 `ComplexShape`。
形式化陈述：{I : Type u_7} → [AddMonoid I] → ComplexShape I → Type u_7
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `I` is an additive monoid and `c : ComplexShape I`, `c.TensorSigns` contains 
the data of
map `ε : I → ℤˣ` and properties which allows the construction of a `TotalComplex
Shape c c c`.
-/
...
/-
**ComplexShape.rel_add** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
形式化陈述：rel_add {p q : I} (hpq : c.Rel p q) (r : I) : c.Rel (p + r) (q + r)
参数：hpq : c.Rel p q；r : I。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `ComplexShape.TensorSigns.rel_add`：∀ {I : Type u_7} {inst : AddMonoid I} 
{c : ComplexShape I} [self : c.TensorSigns] (p q r : I),   c.Rel p q → c.Rel (p 
+ r) (q + r)
-/
lemma rel_add {p q : I} (hpq : c.Rel p q) (r : I) : c.Rel (p + r) (q + r) :=
  TensorSigns.rel_add _ _ _ hpq
/-
**ComplexShape.add_rel** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
形式化陈述：add_rel (r : I) {p q : I} (hpq : c.Rel p q) : c.Rel (r + p) (r + q)
参数：r : I；hpq : c.Rel p q。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `ComplexShape.TensorSigns.add_rel`：∀ {I : Type u_7} {inst : AddMonoid I} 
{c : ComplexShape I} [self : c.TensorSigns] (p q r : I),   c.Rel p q → c.Rel (r 
+ p) (r + q)
-/
...
lemma ε_add (p q : I) : c.ε (p + q) = c.ε p * c.ε q := by
  apply map_mul
/-
**ComplexShape.next_add** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
形式化陈述：next_add (p q : I) (hp : c.Rel p (c.next p)) : c.next (p + q) = c.next p +
 q
参数：p q : I；hp : c.Rel p (c.next p)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `ComplexShape.next_eq'`：next_eq' (c : ComplexShape ι) {i j : ι} (h : c.Re
l i j) : c.next i = j
· 使用引理 `ComplexShape.rel_add`：rel_add {p q : I} (hpq : c.Rel p q) (r : I) : c.Re
l (p + r) (q + r)
-/
...
/-
**ComplexShape.next_add'** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
形式化陈述：next_add' (p q : I) (hq : c.Rel q (c.next q)) : c.next (p + q) = p + c.nex
t q
参数：p q : I；hq : c.Rel q (c.next q)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `ComplexShape.next_eq'`：next_eq' (c : ComplexShape ι) {i j : ι} (h : c.Re
l i j) : c.next i = j
· 使用引理 `ComplexShape.add_rel`：add_rel (r : I) {p q : I} (hpq : c.Rel p q) : c.Re
l (r + p) (r + q)
-/
...
/-
**ComplexShape.Associative** 是 Mathlib 中的一个归纳类型，位于命名空间 `ComplexShape`。
形式化陈述：{I₁ : Type u_1} →   {I₂ : Type u_2} →     {I₃ : Type u_3} →       {I₁₂ : T
ype u_4} →         {I₂₃ : Type u_5} →           {J : Type u_6} →             (c₁
 : ComplexShape I₁) →               (c₂ : ComplexShape I₂) →                 (c₃
 : ComplexShape I₃) →                   (c₁₂ : ComplexShape I₁₂) →              
       (c₂₃ : ComplexShape I₂₃) →                       (c : ComplexShape J) →  
                       [TotalComplexShape c₁ c₂ c₁₂] →                          
 [TotalComplexShape c₁₂ c₃ c] →                             [TotalComplexShape c
₂ c₃ c₂₃] → [TotalComplexShape c₁ c₂₃ c] → Prop
参数：c₁ : ComplexShape I₁；c₂ : ComplexShape I₂；c₃ : ComplexShape I₃；c₁₂ : ComplexS
hape I₁₂；c₂₃ : ComplexShape I₂₃；c : ComplexShape J。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
When we have six complex shapes `c₁`, `c₂`, `c₃`, `c₁₂`, `c₂₃`, `c`, and total f
unctors
`HomologicalComplex₂ C c₁ c₂ ⥤ HomologicalComplex C c₁₂`,
`HomologicalComplex₂ C c₁₂ c₃ ⥤ HomologicalComplex C c`,
`HomologicalComplex₂ C c₂ c₃ ⥤ HomologicalComplex C c₂₃`,
...
/-
**ComplexShape.assoc** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
形式化陈述：assoc (i₁ : I₁) (i₂ : I₂) (i₃ : I₃) : π c₁₂ c₃ c ⟨π c₁ c₂ c₁₂ ⟨i₁, i₂⟩, i₃
⟩ = π c₁ c₂₃ c ⟨i₁, π c₂ c₃ c₂₃ ⟨i₂, i₃⟩⟩
参数：i₁ : I₁；i₂ : I₂；i₃ : I₃。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `ComplexShape.Associative.assoc`：∀ {I₁ : Type u_1} {I₂ : Type u_2} {I₃ : 
Type u_3} {I₁₂ : Type u_4} {I₂₃ : Type u_5} {J : Type u_6}   {c₁ : ComplexShape 
I₁} {c₂ : ComplexSha…
-/
...
/-
**ComplexShape.associative_** 是 Mathlib 中的一个引理，位于命名空间 `ComplexShape`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ComplexShape.r** 是 Mathlib 中的一个定义，位于命名空间 `ComplexShape`。
形式化陈述：r : I₁ × I₂ × I₃ -> J
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
The map `I₁ × I₂ × I₃ → j` that is obtained using `TotalComplexShape c₁ c₂ c₁₂`
and `TotalComplexShape c₁₂ c₃ c` when `c₁ : ComplexShape I₁`, `c₂ : ComplexShape
 I₂`,
`c₃ : ComplexShape I₃`, `c₁₂ : ComplexShape I₁₂` and `c : ComplexShape J`.
-/
...
@[reducible]
/-
**ComplexShape.** 是 Mathlib 中的一个定义，位于命名空间 `ComplexShape`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[instance_reducible]
/-
**TotalComplexShape.symm** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：TotalComplexShape.symm [TotalComplexShape c₁ c₂ c₁₂] : TotalComplexShape c
₂ c₁ c₁₂ where π
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用引理 `ComplexShape.rel_π₂`：rel_π₂ (i₁ : I₁) {i₂ i₂' : I₂} (h : c₂.Rel i₂ i₂') 
: c₁₂.Rel (π c₁ c₂ c₁₂ ⟨i₁, i₂⟩) (π c₁ c₂ c₁₂ ⟨i₁, i₂'⟩)
· 使用引理 `ComplexShape.rel_π₁`：rel_π₁ {i₁ i₁' : I₁} (h : c₁.Rel i₁ i₁') (i₂ : I₂) 
: c₁₂.Rel (π c₁ c₂ c₁₂ ⟨i₁, i₂⟩) (π c₁ c₂ c₁₂ ⟨i₁', i₂⟩)
...
/-
**TotalComplexShapeSymmetry** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：{I₁ : Type u_1} →   {I₂ : Type u_2} →     {I₁₂ : Type u_4} →       (c₁ : C
omplexShape I₁) →         (c₂ : ComplexShape I₂) →           (c₁₂ : ComplexShape
 I₁₂) → [TotalComplexShape c₁ c₂ c₁₂] → [TotalComplexShape c₂ c₁ c₁₂] → Type (ma
x u_1 u_2)
参数：c₁ : ComplexShape I₁；c₂ : ComplexShape I₂；c₁₂ : ComplexShape I₁₂；max u_1 u_2。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[instance_reducible]
/-
**TotalComplexShape.symmSymmetry** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：TotalComplexShape.symmSymmetry [TotalComplexShape c₁ c₂ c₁₂] : letI
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**ComplexShape.symmetryEquiv** 是 Mathlib 中的一个定义，位于命名空间 `ComplexShape`。
形式化陈述：symmetryEquiv (j : I₁₂) : (π c₂ c₁ c₁₂ ⁻¹' {j}) ≃ (π c₁ c₂ c₁₂ ⁻¹' {j}) wh
ere toFun
参数：j : I₁₂。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[instance_reducible]
/-
**TotalComplexShapeSymmetry.symmetry** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：TotalComplexShapeSymmetry.symmetry [TotalComplexShape c₁ c₂ c₁₂] [TotalCom
plexShape c₂ c₁ c₁₂] [TotalComplexShapeSymmetry c₁ c₂ c₁₂] : TotalComplexShapeSy
mmetry c₂ c₁ c₁₂ where symm i₂ i₁
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**TotalComplexShapeSymmetrySymmetry** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：{I₁ : Type u_1} →   {I₂ : Type u_2} →     {I₁₂ : Type u_4} →       (c₁ : C
omplexShape I₁) →         (c₂ : ComplexShape I₂) →           (c₁₂ : ComplexShape
 I₁₂) →             [inst : TotalComplexShape c₁ c₂ c₁₂] →               [inst_1
 : TotalComplexShape c₂ c₁ c₁₂] →                 [TotalComplexShapeSymmetry c₁ 
c₂ c₁₂] → [TotalComplexShapeSymmetry c₂ c₁ c₁₂] → Prop
参数：c₁ : ComplexShape I₁；c₂ : ComplexShape I₂；c₁₂ : ComplexShape I₁₂。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
This typeclass expresses that the signs given by `[TotalComplexShapeSymmetry c₁ 
c₂ c₁₂]`
and by `[TotalComplexShapeSymmetry c₂ c₁ c₁₂]` are compatible.
-/
