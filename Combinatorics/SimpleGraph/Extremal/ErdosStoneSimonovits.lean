...
/-
**SimpleGraph.ErdosStone.filter** 是 Mathlib 中的一个定义，位于命名空间 `SimpleGraph.ErdosSton
e`。
形式化陈述：filter (t : Nat) : Finset (Fin n)
参数：t : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`ErdosStone.filter` is the set of vertices in the complement of a complete equip
artite
subgraph, in `r` parts each of size `t'`, adjacent to at least `t` vertices in e
ach part of the
complete equipartite subgraph.
...
/-
**SimpleGraph.ErdosStone.filter_subset_compl_verts** 是 Mathlib 中的一个定理，位于命名空间 `Si
mpleGraph.ErdosStone`。
形式化陈述：filter_subset_compl_verts : filter K t subseteq K.vertsᶜ
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SimpleGraph.ErdosStone.between_verts_isBipartiteWith** 是 Mathlib 中的一个定理，位于命名空间
 `SimpleGraph.ErdosStone`。
形式化陈述：between_verts_isBipartiteWith : (G.between K.verts K.vertsᶜ).IsBipartiteWi
th K.verts ↑K.vertsᶜ
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SimpleGraph.ErdosStone.le_card_edgeFinset_between_verts** 是 Mathlib 中的一个引理，位于命
名空间 `SimpleGraph.ErdosStone`。
形式化陈述：le_card_edgeFinset_between_verts : (#K.verts * (G.minDegree - #K.verts) : 
Real) <= #(G.between K.verts K.vertsᶜ).edgeFinset
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SimpleGraph.ErdosStone.degree_between_verts_lt_of_mem_sdiff** 是 Mathlib 中的一个引理
，位于命名空间 `SimpleGraph.ErdosStone`。
形式化陈述：degree_between_verts_lt_of_mem_sdiff {v : Fin n} (hv : v in K.vertsᶜ \ fil
ter K t) (ht'_pos : 0 < t') : (G.between K.verts K.vertsᶜ).degree v < #K.verts -
 t' + t
参数：hv : v in K.vertsᶜ \ filter K t；ht'_pos : 0 < t'。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
For `v ∈ K.vertsᶜ \ ErdosStone.filter`, since `v` is adjacent to fewer than `t`
vertices in at least one part of the complete equipartite subgraph, it follows t
hat `v` is
adjacent to fewer than `#K.verts - (t' - t)` vertices in `K.verts`.
...
lemma degree_between_verts_lt_of_mem_sdiff
  ...
  · contrapose! hs
    ...
    exact hadj.symm
/-
**SimpleGraph.ErdosStone.card_edgeFinset_between_verts_le** 是 Mathlib 中的一个引理，位于命
名空间 `SimpleGraph.ErdosStone`。
形式化陈述：card_edgeFinset_between_verts_le (hr_pos : 0 < r) (ht'_pos : 0 < t') : (#(
G.between K.verts K.vertsᶜ).edgeFinset : Real) <= (n - #K.verts) * (#K.verts - (
t' - t)) + #(filter K t) * (t' - t)
参数：hr_pos : 0 < r；ht'_pos : 0 < t'。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
This is an auxiliary lemma for the Erdős-Stone theorem. -/
/-
**SimpleGraph.ErdosStone.mul_le_card_filter_mul** 是 Mathlib 中的一个引理，位于命名空间 `Simpl
eGraph.ErdosStone`。
形式化陈述：mul_le_card_filter_mul (hr_pos : 0 < r) (ht'_pos : 0 < t') (hδ : G.minDegr
ee >= (1 - 1 / r + ε) * n) {N : Nat} (hN : (N + r * t') * (t' - t) <= n * (r * t
' * ε - t)) : (N * (t' - t) : Real) <= (#(filter K t) * (t' - t) : Real)
参数：hr_pos : 0 < r；ht'_pos : 0 < t'；hδ : G.minDegree >= (1 - 1 / r + ε) * n；hN : 
(N + r * t') * (t' - t) <= n * (r * t' * ε - t)。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
This is an auxiliary definition for the Erdős-Stone theorem. -/
/-
**SimpleGraph.ErdosStone.filter.pi** 是 Mathlib 中的一个定义，位于命名空间 `SimpleGraph.ErdosS
tone`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
For `w ∈ ErdosStone.filter`, there exists a `r` subets of vertices of size `t < 
t'`
adjacent to `w`.
...
/-
**SimpleGraph.ErdosStone.filter.pi.mem_val** 是 Mathlib 中的一个定理，位于命名空间 `SimpleGrap
h.ErdosStone`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**SimpleGraph.ErdosStone.filter.pi.exists_le_card_fiber** 是 Mathlib 中的一个定理，位于命名空
间 `SimpleGraph.ErdosStone`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `#ErdosStone.filter` is sufficiently large, then there exist a `y` such that 
there
are least `t` vertices in the fiber `ErdosStone.filter.pi A · = y`.
