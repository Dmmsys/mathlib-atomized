/-
Copyright (c) 2021 Yury Kudryashov. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yury Kudryashov
-/
module

public import Mathlib.Analysis.Analytic.CPolynomial
public import Mathlib.Analysis.Analytic.Inverse
public import Mathlib.Analysis.Analytic.Within
public import Mathlib.Analysis.Calculus.Deriv.Basic
public import Mathlib.Analysis.Calculus.ContDiff.FTaylorSeries
public import Mathlib.Analysis.Calculus.FDeriv.Add
public import Mathlib.Analysis.Calculus.FDeriv.Prod
public import Mathlib.Analysis.Normed.Module.Completion

/-!
# Fréchet derivatives of analytic functions.

A function expressible as a power series at a point has a Fréchet derivative there.
Also the special case in terms of `deriv` when the domain is 1-dimensional.

As an application, we show that continuous multilinear maps are smooth. We also compute their
iterated derivatives, in `ContinuousMultilinearMap.iteratedFDeriv_eq`.

## Main definitions and results

* `AnalyticAt.differentiableAt` : an analytic function at a point is differentiable there.
* `AnalyticOnNhd.fderiv` : in a complete space, if a function is analytic on a
  neighborhood of a set `s`, so is its derivative.
* `AnalyticOnNhd.fderiv_of_isOpen` : if a function is analytic on a neighborhood of an
  open set `s`, so is its derivative.
* `AnalyticOn.fderivWithin` : if a function is analytic on a set of unique differentiability,
  so is its derivative within this set.
* `OpenPartialHomeomorph.analyticAt_symm` : if an open partial homeomorphism `f` is analytic at a
  point `f.symm a`, with invertible derivative, then its inverse is analytic at `a`.

## Comments on completeness

Some theorems need a complete space, some don't, for the following reason.

(1) If a function is analytic at a point `x`, then it is differentiable there (with derivative given
by the first term in the power series). There is no issue of convergence here.

(2) If a function has a power series on a ball `B (x, r)`, there is no guarantee that the power
series for the derivative will converge at `y ≠ x`, if the space is not complete. So, to deduce
that `f` is differentiable at `y`, one needs completeness in general.

(3) However, if a function `f` has a power series on a ball `B (x, r)`, and is a priori known to be
differentiable at some point `y ≠ x`, then the power series for the derivative of `f` will
automatically converge at `y`, towards the given derivative: this follows from the facts that this
is true in the completion (thanks to the previous point) and that the map to the completion is
an embedding.

(4) Therefore, if one assumes `AnalyticOn 𝕜 f s` where `s` is an open set, then `f` is analytic
therefore differentiable at every point of `s`, by (1), so by (3) the power series for its
derivative converges on whole balls. Therefore, the derivative of `f` is also analytic on `s`. The
same holds if `s` is merely a set with unique differentials.

(5) However, this does not work for `AnalyticOnNhd 𝕜 f s`, as we don't get for free
differentiability at points in a neighborhood of `s`. Therefore, the theorem that deduces
`AnalyticOnNhd 𝕜 (fderiv 𝕜 f) s` from `AnalyticOnNhd 𝕜 f s` requires completeness of the space.

-/

public section

open Filter Asymptotics Set

open scoped ENNReal Topology

universe u v

variable {𝕜 : Type*} [NontriviallyNormedField 𝕜]
variable {E : Type u} [NormedAddCommGroup E] [NormedSpace 𝕜 E]
variable {F : Type v} [NormedAddCommGroup F] [NormedSpace 𝕜 F]

section fderiv

variable {p : FormalMultilinearSeries 𝕜 E F} {r : Real>=0∞}
variable {f : E -> F} {x : E} {s : Set E}

/--
theorem `HasFPowerSeriesWithinAt.hasStrictFDerivWithinAt` / 定理 `HasFPowerSeriesWithinAt.hasStrictFDerivWithinAt`

English:
theorem HasFPowerSeriesWithinAt.hasStrictFDerivWithinAt
  given: (h : HasFPowerSeriesWithinAt f p s x)
  proof: by
  refine h.isBigO_image_sub_norm_mul_norm_sub.trans_isLittleO (IsLittleO.of_norm_right ?_)
  refine isLittleO_iff_exists_eq_mul.2 ⟨fun y => ‖y - (x, x)‖, ?_, EventuallyEq.rfl⟩
  apply Tendsto.mono_left _ nhdsWithin_le_nhds
  refine (continuous_id.fun_sub continuous_const).norm.tendsto' _ _ ?_
  r

中文:
定理 HasFPowerSeriesWithinAt.hasStrictFDerivWithinAt
  条件: (h : HasFPowerSeriesWithinAt f p s x)
  证明: by
  refine h.isBigO_image_sub_norm_mul_norm_sub.trans_isLittleO (IsLittleO.of_norm_right ?_)
  refine isLittleO_iff_exists_eq_mul.2 ⟨fun y => ‖y - (x, x)‖, ?_, EventuallyEq.rfl⟩
  apply Tendsto.mono_left _ nhdsWithin_le_nhds
  refine (continuous_id.fun_sub continuous_const).norm.tendsto' _ _ ?_
  r

Depends on / 依赖: EventuallyEq, EventuallyEq.rfl, IsLittleO, IsLittleO.of_norm_right, Tendsto, Tendsto.mono_left, _root_, _root_.id, continuous_const, continuous_id, continuous_id.fun_sub, fun_sub, h.isBigO_image_sub_norm_mul_norm_sub.trans_isLittleO, isBigO_image_sub_norm_mul_norm_sub, isLittleO_iff_exists_eq_mul, mono_left, nhdsWithin_le_nhds, norm.tendsto, norm_zero, of_norm_right
-/
theorem HasFPowerSeriesWithinAt.hasStrictFDerivWithinAt (h : HasFPowerSeriesWithinAt f p s x) :
    (fun y => f y.1 - f y.2 - (continuousMultilinearCurryFin1 𝕜 E F (p 1)) (y.1 - y.2))
      =o[𝓝[insert x s ×ˢ insert x s] (x, x)] fun y => y.1 - y.2 := by
  refine h.isBigO_image_sub_norm_mul_norm_sub.trans_isLittleO (IsLittleO.of_norm_right ?_)
  refine isLittleO_iff_exists_eq_mul.2 ⟨fun y => ‖y - (x, x)‖, ?_, EventuallyEq.rfl⟩
  apply Tendsto.mono_left _ nhdsWithin_le_nhds
  refine (continuous_id.fun_sub continuous_const).norm.tendsto' _ _ ?_
  rw [_root_.id]; rw [sub_self]; rw [norm_zero]

/--
theorem `HasFPowerSeriesAt.hasStrictFDerivAt` / 定理 `HasFPowerSeriesAt.hasStrictFDerivAt`

English:
theorem HasFPowerSeriesAt.hasStrictFDerivAt
  given: (h : HasFPowerSeriesAt f p x)
  proof: by
  simpa only [hasStrictFDerivAt_iff_isLittleO, Set.insert_eq_of_mem, Set.mem_univ,
      Set.univ_prod_univ, nhdsWithin_univ]
    using (h.hasFPowerSeriesWithinAt (s := Set.univ)).hasStrictFDerivWithinAt

中文:
定理 HasFPowerSeriesAt.hasStrictFDerivAt
  条件: (h : HasFPowerSeriesAt f p x)
  证明: by
  simpa only [hasStrictFDerivAt_iff_isLittleO, Set.insert_eq_of_mem, Set.mem_univ,
      Set.univ_prod_univ, nhdsWithin_univ]
    using (h.hasFPowerSeriesWithinAt (s := Set.univ)).hasStrictFDerivWithinAt

Depends on / 依赖: Set.insert_eq_of_mem, Set.mem_univ, Set.univ, Set.univ_prod_univ, h.hasFPowerSeriesWithinAt, hasFPowerSeriesWithinAt, hasStrictFDerivAt_iff_isLittleO, hasStrictFDerivWithinAt, insert_eq_of_mem, mem_univ, nhdsWithin_univ, univ_prod_univ
-/
theorem HasFPowerSeriesAt.hasStrictFDerivAt (h : HasFPowerSeriesAt f p x) :
    HasStrictFDerivAt f (continuousMultilinearCurryFin1 𝕜 E F (p 1)) x := by
  simpa only [hasStrictFDerivAt_iff_isLittleO, Set.insert_eq_of_mem, Set.mem_univ,
      Set.univ_prod_univ, nhdsWithin_univ]
    using (h.hasFPowerSeriesWithinAt (s := Set.univ)).hasStrictFDerivWithinAt

/--
theorem `HasFPowerSeriesWithinAt.hasFDerivWithinAt` / 定理 `HasFPowerSeriesWithinAt.hasFDerivWithinAt`

English:
theorem HasFPowerSeriesWithinAt.hasFDerivWithinAt
  given: (h : HasFPowerSeriesWithinAt f p s x)
  proof: by
  rw [hasFDerivWithinAt_iff_isLittleO]; rw [isLittleO_iff]
  intro c hc
  have : Tendsto (fun y => (y, x)) (𝓝[insert x s] x) (𝓝[insert x s ×ˢ insert x s] (x, x)) := by
    rw [nhdsWithin_prod_eq]
    exact Tendsto.prodMk tendsto_id (tendsto_const_nhdsWithin (by simp))
  exact this (isLittleO_iff.

中文:
定理 HasFPowerSeriesWithinAt.hasFDerivWithinAt
  条件: (h : HasFPowerSeriesWithinAt f p s x)
  证明: by
  rw [hasFDerivWithinAt_iff_isLittleO]; rw [isLittleO_iff]
  intro c hc
  have : Tendsto (fun y => (y, x)) (𝓝[insert x s] x) (𝓝[insert x s ×ˢ insert x s] (x, x)) := by
    rw [nhdsWithin_prod_eq]
    exact Tendsto.prodMk tendsto_id (tendsto_const_nhdsWithin (by simp))
  exact this (isLittleO_iff.

Depends on / 依赖: Tendsto, Tendsto.prodMk, h.hasStrictFDerivWithinAt, hasFDerivWithinAt_iff_isLittleO, hasStrictFDerivWithinAt, insert, isLittleO_iff, nhdsWithin_prod_eq, prodMk, tendsto_const_nhdsWithin, tendsto_id
-/
theorem HasFPowerSeriesWithinAt.hasFDerivWithinAt (h : HasFPowerSeriesWithinAt f p s x) :
    HasFDerivWithinAt f (continuousMultilinearCurryFin1 𝕜 E F (p 1)) (insert x s) x := by
  rw [hasFDerivWithinAt_iff_isLittleO]; rw [isLittleO_iff]
  intro c hc
  have : Tendsto (fun y => (y, x)) (𝓝[insert x s] x) (𝓝[insert x s ×ˢ insert x s] (x, x)) := by
    rw [nhdsWithin_prod_eq]
    exact Tendsto.prodMk tendsto_id (tendsto_const_nhdsWithin (by simp))
  exact this (isLittleO_iff.1 h.hasStrictFDerivWithinAt hc)

/--
theorem `HasFPowerSeriesAt.hasFDerivAt` / 定理 `HasFPowerSeriesAt.hasFDerivAt`

English:
theorem HasFPowerSeriesAt.hasFDerivAt
  given: (h : HasFPowerSeriesAt f p x)
  proof: h.hasStrictFDerivAt.hasFDerivAt

中文:
定理 HasFPowerSeriesAt.hasFDerivAt
  条件: (h : HasFPowerSeriesAt f p x)
  证明: h.hasStrictFDerivAt.hasFDerivAt

Depends on / 依赖: h.hasStrictFDerivAt.hasFDerivAt, hasFDerivAt, hasStrictFDerivAt
-/
theorem HasFPowerSeriesAt.hasFDerivAt (h : HasFPowerSeriesAt f p x) :
    HasFDerivAt f (continuousMultilinearCurryFin1 𝕜 E F (p 1)) x :=
  h.hasStrictFDerivAt.hasFDerivAt

/--
theorem `HasFPowerSeriesWithinAt.differentiableWithinAt` / 定理 `HasFPowerSeriesWithinAt.differentiableWithinAt`

English:
theorem HasFPowerSeriesWithinAt.differentiableWithinAt
  given: (h : HasFPowerSeriesWithinAt f p s x)
  proof: h.hasFDerivWithinAt.differentiableWithinAt

中文:
定理 HasFPowerSeriesWithinAt.differentiableWithinAt
  条件: (h : HasFPowerSeriesWithinAt f p s x)
  证明: h.hasFDerivWithinAt.differentiableWithinAt

Depends on / 依赖: differentiableWithinAt, h.hasFDerivWithinAt.differentiableWithinAt, hasFDerivWithinAt
-/
theorem HasFPowerSeriesWithinAt.differentiableWithinAt (h : HasFPowerSeriesWithinAt f p s x) :
    DifferentiableWithinAt 𝕜 f (insert x s) x :=
  h.hasFDerivWithinAt.differentiableWithinAt

/--
theorem `HasFPowerSeriesAt.differentiableAt` / 定理 `HasFPowerSeriesAt.differentiableAt`

English:
theorem HasFPowerSeriesAt.differentiableAt
  given: (h : HasFPowerSeriesAt f p x)
  statement: DifferentiableAt 𝕜 f x
  proof: h.hasFDerivAt.differentiableAt

中文:
定理 HasFPowerSeriesAt.differentiableAt
  条件: (h : HasFPowerSeriesAt f p x)
  结论: DifferentiableAt 𝕜 f x
  证明: h.hasFDerivAt.differentiableAt

Depends on / 依赖: differentiableAt, h.hasFDerivAt.differentiableAt, hasFDerivAt
-/
theorem HasFPowerSeriesAt.differentiableAt (h : HasFPowerSeriesAt f p x) : DifferentiableAt 𝕜 f x :=
  h.hasFDerivAt.differentiableAt

/--
theorem `AnalyticWithinAt.differentiableWithinAt` / 定理 `AnalyticWithinAt.differentiableWithinAt`

English:
theorem AnalyticWithinAt.differentiableWithinAt
  given: (h : AnalyticWithinAt 𝕜 f s x)
  proof: by
  obtain ⟨p, hp⟩ := h
  exact hp.differentiableWithinAt

@[fun_prop]

中文:
定理 AnalyticWithinAt.differentiableWithinAt
  条件: (h : AnalyticWithinAt 𝕜 f s x)
  证明: by
  obtain ⟨p, hp⟩ := h
  exact hp.differentiableWithinAt

@[fun_prop]

Depends on / 依赖: differentiableWithinAt, hp.differentiableWithinAt
-/
theorem AnalyticWithinAt.differentiableWithinAt (h : AnalyticWithinAt 𝕜 f s x) :
    DifferentiableWithinAt 𝕜 f (insert x s) x := by
  obtain ⟨p, hp⟩ := h
  exact hp.differentiableWithinAt

@[fun_prop]
/--
theorem `AnalyticAt.differentiableAt` / 定理 `AnalyticAt.differentiableAt`

English:
theorem AnalyticAt.differentiableAt
  statement: AnalyticAt 𝕜 f x -> DifferentiableAt 𝕜 f x

中文:
定理 AnalyticAt.differentiableAt
  结论: AnalyticAt 𝕜 f x -> DifferentiableAt 𝕜 f x
-/
theorem AnalyticAt.differentiableAt : AnalyticAt 𝕜 f x -> DifferentiableAt 𝕜 f x
  | ⟨_, hp⟩ => hp.differentiableAt

/--
theorem `AnalyticAt.differentiableWithinAt` / 定理 `AnalyticAt.differentiableWithinAt`

English:
theorem AnalyticAt.differentiableWithinAt
  given: (h : AnalyticAt 𝕜 f x)
  statement: DifferentiableWithinAt 𝕜 f s x
  proof: h.differentiableAt.differentiableWithinAt

中文:
定理 AnalyticAt.differentiableWithinAt
  条件: (h : AnalyticAt 𝕜 f x)
  结论: DifferentiableWithinAt 𝕜 f s x
  证明: h.differentiableAt.differentiableWithinAt

Depends on / 依赖: differentiableAt, differentiableWithinAt, h.differentiableAt.differentiableWithinAt
-/
theorem AnalyticAt.differentiableWithinAt (h : AnalyticAt 𝕜 f x) : DifferentiableWithinAt 𝕜 f s x :=
  h.differentiableAt.differentiableWithinAt

/--
theorem `HasFPowerSeriesWithinAt.fderivWithin_eq` / 定理 `HasFPowerSeriesWithinAt.fderivWithin_eq`

English:
theorem HasFPowerSeriesWithinAt.fderivWithin_eq
  proof: h.hasFDerivWithinAt.fderivWithin hu

中文:
定理 HasFPowerSeriesWithinAt.fderivWithin_eq
  证明: h.hasFDerivWithinAt.fderivWithin hu

Depends on / 依赖: fderivWithin, h.hasFDerivWithinAt.fderivWithin, hasFDerivWithinAt
-/
theorem HasFPowerSeriesWithinAt.fderivWithin_eq
    (h : HasFPowerSeriesWithinAt f p s x) (hu : UniqueDiffWithinAt 𝕜 (insert x s) x) :
    fderivWithin 𝕜 f (insert x s) x = continuousMultilinearCurryFin1 𝕜 E F (p 1) :=
  h.hasFDerivWithinAt.fderivWithin hu

/--
theorem `HasFPowerSeriesAt.fderiv_eq` / 定理 `HasFPowerSeriesAt.fderiv_eq`

English:
theorem HasFPowerSeriesAt.fderiv_eq
  given: (h : HasFPowerSeriesAt f p x)
  proof: h.hasFDerivAt.fderiv

中文:
定理 HasFPowerSeriesAt.fderiv_eq
  条件: (h : HasFPowerSeriesAt f p x)
  证明: h.hasFDerivAt.fderiv

Depends on / 依赖: fderiv, h.hasFDerivAt.fderiv, hasFDerivAt
-/
theorem HasFPowerSeriesAt.fderiv_eq (h : HasFPowerSeriesAt f p x) :
    fderiv 𝕜 f x = continuousMultilinearCurryFin1 𝕜 E F (p 1) :=
  h.hasFDerivAt.fderiv

/--
theorem `AnalyticAt.hasStrictFDerivAt` / 定理 `AnalyticAt.hasStrictFDerivAt`

English:
theorem AnalyticAt.hasStrictFDerivAt
  given: (h : AnalyticAt 𝕜 f x)
  proof: by
  rcases h with ⟨p, hp⟩
  rw [hp.fderiv_eq]
  exact hp.hasStrictFDerivAt

中文:
定理 AnalyticAt.hasStrictFDerivAt
  条件: (h : AnalyticAt 𝕜 f x)
  证明: by
  rcases h with ⟨p, hp⟩
  rw [hp.fderiv_eq]
  exact hp.hasStrictFDerivAt

Depends on / 依赖: fderiv_eq, hasStrictFDerivAt, hp.fderiv_eq, hp.hasStrictFDerivAt
-/
theorem AnalyticAt.hasStrictFDerivAt (h : AnalyticAt 𝕜 f x) :
    HasStrictFDerivAt f (fderiv 𝕜 f x) x := by
  rcases h with ⟨p, hp⟩
  rw [hp.fderiv_eq]
  exact hp.hasStrictFDerivAt

/--
lemma `AnalyticAt.hasStrictDerivAt` / 引理 `AnalyticAt.hasStrictDerivAt`

English:
lemma AnalyticAt.hasStrictDerivAt
  given: {f : 𝕜 -> F} {x : 𝕜} (hf : AnalyticAt 𝕜 f x)
  proof: by
  simpa [hasStrictDerivAt_iff_hasStrictFDerivAt, toSpanSingleton_deriv] using hf.hasStrictFDerivAt

中文:
引理 AnalyticAt.hasStrictDerivAt
  条件: {f : 𝕜 -> F} {x : 𝕜} (hf : AnalyticAt 𝕜 f x)
  证明: by
  simpa [hasStrictDerivAt_iff_hasStrictFDerivAt, toSpanSingleton_deriv] using hf.hasStrictFDerivAt

Depends on / 依赖: hasStrictDerivAt_iff_hasStrictFDerivAt, hasStrictFDerivAt, hf.hasStrictFDerivAt, toSpanSingleton_deriv
-/
lemma AnalyticAt.hasStrictDerivAt {f : 𝕜 -> F} {x : 𝕜} (hf : AnalyticAt 𝕜 f x) :
    HasStrictDerivAt f (deriv f x) x := by
  simpa [hasStrictDerivAt_iff_hasStrictFDerivAt, toSpanSingleton_deriv] using hf.hasStrictFDerivAt

/--
theorem `HasFPowerSeriesWithinOnBall.differentiableOn` / 定理 `HasFPowerSeriesWithinOnBall.differentiableOn`

English:
theorem HasFPowerSeriesWithinOnBall.differentiableOn
  statement: [CompleteSpace F]
  proof: by
  intro y hy
  have Z := (h.analyticWithinAt_of_mem hy).differentiableWithinAt
  rcases eq_or_ne y x with rfl | hy
  · exact Z.mono inter_subset_left
  · apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    suffices s in 𝓝[insert x s] y from nhdsWithin_mono _ inter_subset_left this
    r

中文:
定理 有FPowerSeriesWithinOnBall.differentiableOn
  结论: [完备空间 F]
  证明: by
  intro y hy
  have Z := (h.analyticWithinAt_of_mem hy).differentiableWithinAt
  rcases eq_or_ne y x with rfl | hy
  · exact Z.mono inter_subset_left
  · apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    suffices s in 𝓝[insert x s] y from nhdsWithin_mono _ inter_subset_left this
    r

Depends on / 依赖: Z.mono, analyticWithinAt_of_mem, differentiableWithinAt, eq_or_ne, h.analyticWithinAt_of_mem, insert, inter_subset_left, mono_of_mem_nhdsWithin, nhdsWithin_insert_of_ne, nhdsWithin_mono, self_mem_nhdsWithin, subset_insert
-/
theorem HasFPowerSeriesWithinOnBall.differentiableOn [CompleteSpace F]
    (h : HasFPowerSeriesWithinOnBall f p s x r) :
    DifferentiableOn 𝕜 f (insert x s inter Metric.eball x r) := by
  intro y hy
  have Z := (h.analyticWithinAt_of_mem hy).differentiableWithinAt
  rcases eq_or_ne y x with rfl | hy
  · exact Z.mono inter_subset_left
  · apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    suffices s in 𝓝[insert x s] y from nhdsWithin_mono _ inter_subset_left this
    rw [nhdsWithin_insert_of_ne hy]
    exact self_mem_nhdsWithin

/--
theorem `HasFPowerSeriesOnBall.differentiableOn` / 定理 `HasFPowerSeriesOnBall.differentiableOn`

English:
theorem HasFPowerSeriesOnBall.differentiableOn
  statement: [CompleteSpace F]
  proof: fun _ hy =>
  (h.analyticAt_of_mem hy).differentiableWithinAt

中文:
定理 有FPowerSeriesOnBall.differentiableOn
  结论: [完备空间 F]
  证明: fun _ hy =>
  (h.analyticAt_of_mem hy).differentiableWithinAt
-/
theorem HasFPowerSeriesOnBall.differentiableOn [CompleteSpace F]
    (h : HasFPowerSeriesOnBall f p x r) : DifferentiableOn 𝕜 f (Metric.eball x r) := fun _ hy =>
  (h.analyticAt_of_mem hy).differentiableWithinAt

/--
theorem `HasFPowerSeriesAt.eventually_differentiableAt` / 定理 `HasFPowerSeriesAt.eventually_differentiableAt`

English:
theorem HasFPowerSeriesAt.eventually_differentiableAt
  proof: by
  obtain ⟨r, hp⟩ := hp
  exact hp.differentiableOn.eventually_differentiableAt (Metric.eball_mem_nhds _ hp.r_pos)

中文:
定理 HasFPowerSeriesAt.eventually_differentiableAt
  证明: by
  obtain ⟨r, hp⟩ := hp
  exact hp.differentiableOn.eventually_differentiableAt (Metric.eball_mem_nhds _ hp.r_pos)

Depends on / 依赖: ContractibleSpace, Metric, Metric.eball_mem_nhds, RealTopologicalVectorSpace, RealTopologicalVectorSpace.contractibleSpace, contractibleSpace, differentiableOn, eball_mem_nhds, eventually_differentiableAt, hp.differentiableOn.eventually_differentiableAt, hp.r_pos, r_pos
-/
theorem HasFPowerSeriesAt.eventually_differentiableAt
    [CompleteSpace F] (hp : HasFPowerSeriesAt f p x) :
    forallᶠ z in 𝓝 x, DifferentiableAt 𝕜 f z := by
  obtain ⟨r, hp⟩ := hp
  exact hp.differentiableOn.eventually_differentiableAt (Metric.eball_mem_nhds _ hp.r_pos)

/--
theorem `AnalyticOn.differentiableOn` / 定理 `AnalyticOn.differentiableOn`

English:
theorem AnalyticOn.differentiableOn
  given: (h : AnalyticOn 𝕜 f s)
  statement: DifferentiableOn 𝕜 f s
  proof: fun y hy => (h y hy).differentiableWithinAt.mono (by simp)

中文:
定理 AnalyticOn.differentiableOn
  条件: (h : AnalyticOn 𝕜 f s)
  结论: DifferentiableOn 𝕜 f s
  证明: fun y hy => (h y hy).differentiableWithinAt.mono (by simp)

Depends on / 依赖: differentiableWithinAt, differentiableWithinAt.mono
-/
theorem AnalyticOn.differentiableOn (h : AnalyticOn 𝕜 f s) : DifferentiableOn 𝕜 f s :=
  fun y hy => (h y hy).differentiableWithinAt.mono (by simp)

/--
theorem `AnalyticOnNhd.differentiableOn` / 定理 `AnalyticOnNhd.differentiableOn`

English:
theorem AnalyticOnNhd.differentiableOn
  given: (h : AnalyticOnNhd 𝕜 f s)
  statement: DifferentiableOn 𝕜 f s
  proof: fun y hy => (h y hy).differentiableWithinAt

中文:
定理 AnalyticOnNhd.differentiableOn
  条件: (h : AnalyticOnNhd 𝕜 f s)
  结论: DifferentiableOn 𝕜 f s
  证明: fun y hy => (h y hy).differentiableWithinAt

Depends on / 依赖: differentiableWithinAt
-/
theorem AnalyticOnNhd.differentiableOn (h : AnalyticOnNhd 𝕜 f s) : DifferentiableOn 𝕜 f s :=
  fun y hy => (h y hy).differentiableWithinAt

/--
theorem `HasFPowerSeriesWithinOnBall.hasFDerivWithinAt` / 定理 `HasFPowerSeriesWithinOnBall.hasFDerivWithinAt`

English:
theorem HasFPowerSeriesWithinOnBall.hasFDerivWithinAt
  statement: [CompleteSpace F]
  proof: by
  rcases eq_or_ne y 0 with rfl | h''y
  · convert! (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    simp
  · have Z := (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    rw [nhdsWithin_insert_of_

中文:
定理 有FPowerSeriesWithinOnBall.hasFDerivWithinAt
  结论: [完备空间 F]
  证明: by
  rcases eq_or_ne y 0 with rfl | h''y
  · convert! (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    simp
  · have Z := (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    rw [nhdsWithin_insert_of_

Depends on / 依赖: Z.mono, changeOrigin, convert, eq_or_ne, h.changeOrigin, hasFDerivWithinAt, hasFPowerSeriesWithinAt, hasFPowerSeriesWithinAt.hasFDerivWithinAt, mono_of_mem_nhdsWithin, nhdsWithin_insert_of_ne, self_mem_nhdsWithin, subset_insert
-/
theorem HasFPowerSeriesWithinOnBall.hasFDerivWithinAt [CompleteSpace F]
    (h : HasFPowerSeriesWithinOnBall f p s x r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) (h'y : x + y in insert x s) :
    HasFDerivWithinAt f (continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1))
      (insert x s) (x + y) := by
  rcases eq_or_ne y 0 with rfl | h''y
  · convert! (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    simp
  · have Z := (h.changeOrigin hy h'y).hasFPowerSeriesWithinAt.hasFDerivWithinAt
    apply (Z.mono (subset_insert _ _)).mono_of_mem_nhdsWithin
    rw [nhdsWithin_insert_of_ne]
    · exact self_mem_nhdsWithin
    · simpa using h''y

/--
theorem `HasFPowerSeriesOnBall.hasFDerivAt` / 定理 `HasFPowerSeriesOnBall.hasFDerivAt`

English:
theorem HasFPowerSeriesOnBall.hasFDerivAt
  statement: [CompleteSpace F] (h : HasFPowerSeriesOnBall f p x r)
  proof: (h.changeOrigin hy).hasFPowerSeriesAt.hasFDerivAt

中文:
定理 有FPowerSeriesOnBall.hasFDerivAt
  结论: [完备空间 F] (h : 有FPowerSeriesOnBall f p x r)
  证明: (h.changeOrigin hy).hasFPowerSeriesAt.hasFDerivAt

Depends on / 依赖: changeOrigin, h.changeOrigin, hasFDerivAt, hasFPowerSeriesAt, hasFPowerSeriesAt.hasFDerivAt
-/
theorem HasFPowerSeriesOnBall.hasFDerivAt [CompleteSpace F] (h : HasFPowerSeriesOnBall f p x r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) :
    HasFDerivAt f (continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1)) (x + y) :=
  (h.changeOrigin hy).hasFPowerSeriesAt.hasFDerivAt

/--
theorem `HasFPowerSeriesWithinOnBall.fderivWithin_eq` / 定理 `HasFPowerSeriesWithinOnBall.fderivWithin_eq`

English:
theorem HasFPowerSeriesWithinOnBall.fderivWithin_eq
  statement: [CompleteSpace F]
  proof: (h.hasFDerivWithinAt hy h'y).fderivWithin (hu _ h'y)

中文:
定理 有FPowerSeriesWithinOnBall.fderivWithin_eq
  结论: [完备空间 F]
  证明: (h.hasFDerivWithinAt hy h'y).fderivWithin (hu _ h'y)

Depends on / 依赖: fderivWithin, h.hasFDerivWithinAt, hasFDerivWithinAt
-/
theorem HasFPowerSeriesWithinOnBall.fderivWithin_eq [CompleteSpace F]
    (h : HasFPowerSeriesWithinOnBall f p s x r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) (h'y : x + y in insert x s) (hu : UniqueDiffOn 𝕜 (insert x s)) :
    fderivWithin 𝕜 f (insert x s) (x + y) =
      continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1) :=
  (h.hasFDerivWithinAt hy h'y).fderivWithin (hu _ h'y)

/--
theorem `HasFPowerSeriesOnBall.fderiv_eq` / 定理 `HasFPowerSeriesOnBall.fderiv_eq`

English:
theorem HasFPowerSeriesOnBall.fderiv_eq
  statement: [CompleteSpace F] (h : HasFPowerSeriesOnBall f p x r)
  proof: (h.hasFDerivAt hy).fderiv

中文:
定理 有FPowerSeriesOnBall.fderiv_eq
  结论: [完备空间 F] (h : 有FPowerSeriesOnBall f p x r)
  证明: (h.hasFDerivAt hy).fderiv

Depends on / 依赖: fderiv, h.hasFDerivAt, hasFDerivAt
-/
theorem HasFPowerSeriesOnBall.fderiv_eq [CompleteSpace F] (h : HasFPowerSeriesOnBall f p x r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) :
    fderiv 𝕜 f (x + y) = continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1) :=
  (h.hasFDerivAt hy).fderiv

/--
theorem `HasFPowerSeriesOnBall.fderiv` / 定理 `HasFPowerSeriesOnBall.fderiv`

English:
theorem HasFPowerSeriesOnBall.fderiv
  statement: [CompleteSpace F]
  proof: by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesOnBall ?_
    simpa using! ((p.hasFPowerSeriesOnBall_changeOrigi

中文:
定理 有FPowerSeriesOnBall.fderiv
  结论: [完备空间 F]
  证明: by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesOnBall ?_
    simpa using! ((p.hasFPowerSeriesOnBall_changeOrigi
-/
protected theorem HasFPowerSeriesOnBall.fderiv [CompleteSpace F]
    (h : HasFPowerSeriesOnBall f p x r) :
    HasFPowerSeriesOnBall (fderiv 𝕜 f) p.derivSeries x r := by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesOnBall ?_
    simpa using! ((p.hasFPowerSeriesOnBall_changeOrigin 1
      (h.r_pos.trans_le h.r_le)).mono h.r_pos h.r_le).comp_sub x
  dsimp only
  rw [← h.fderiv_eq]; rw [add_sub_cancel]
  simpa only [edist_eq_enorm_sub, Metric.mem_eball] using! hz

/--
theorem `FormalMultilinearSeries.fderiv_sum` / 定理 `FormalMultilinearSeries.fderiv_sum`

English:
theorem FormalMultilinearSeries.fderiv_sum
  given: [CompleteSpace F] (h : ‖x‖ₑ < p.radius)
  proof: by
  simpa using (p.hasFPowerSeriesOnBall (zero_le.trans_lt h)).fderiv.sum (by simpa using h)

中文:
定理 FormalMultilinearSeries.fderiv_sum
  条件: [完备空间 F] (h : ‖x‖ₑ < p.radius)
  证明: by
  simpa using (p.hasFPowerSeriesOnBall (zero_le.trans_lt h)).fderiv.sum (by simpa using h)
-/
protected theorem FormalMultilinearSeries.fderiv_sum [CompleteSpace F] (h : ‖x‖ₑ < p.radius) :
    fderiv 𝕜 p.sum x = p.derivSeries.sum x := by
  simpa using (p.hasFPowerSeriesOnBall (zero_le.trans_lt h)).fderiv.sum (by simpa using h)

/--
theorem `FormalMultilinearSeries.hasFDerivAt_sum` / 定理 `FormalMultilinearSeries.hasFDerivAt_sum`

English:
theorem FormalMultilinearSeries.hasFDerivAt_sum
  given: [CompleteSpace F] (h : ‖x‖ₑ < p.radius)
  proof: by
  rw [← FormalMultilinearSeries.fderiv_sum h]
  exact p.hasFPowerSeriesOnBall (zero_le.trans_lt h)
.differentiableAt.hasFDerivAt .analyticAt_of_mem (by simpa using h)

中文:
定理 FormalMultilinearSeries.hasFDerivAt_sum
  条件: [完备空间 F] (h : ‖x‖ₑ < p.radius)
  证明: by
  rw [← FormalMultilinearSeries.fderiv_sum h]
  exact p.hasFPowerSeriesOnBall (zero_le.trans_lt h)
.differentiableAt.hasFDerivAt .analyticAt_of_mem (by simpa using h)
-/
protected theorem FormalMultilinearSeries.hasFDerivAt_sum [CompleteSpace F] (h : ‖x‖ₑ < p.radius) :
    HasFDerivAt p.sum (p.derivSeries.sum x) x := by
  rw [← FormalMultilinearSeries.fderiv_sum h]
  exact p.hasFPowerSeriesOnBall (zero_le.trans_lt h)
.differentiableAt.hasFDerivAt .analyticAt_of_mem (by simpa using h)

/--
theorem `HasFPowerSeriesWithinOnBall.fderivWithin` / 定理 `HasFPowerSeriesWithinOnBall.fderivWithin`

English:
theorem HasFPowerSeriesWithinOnBall.fderivWithin
  statement: [CompleteSpace F]
  proof: by
  refine .congr' (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    (fun z hz => ?_)
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesWithinOnBall ?_
    apply HasFPowerSeriesOnBall.hasFPowerSeri

中文:
定理 有FPowerSeriesWithinOnBall.fderivWithin
  结论: [完备空间 F]
  证明: by
  refine .congr' (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    (fun z hz => ?_)
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesWithinOnBall ?_
    apply HasFPowerSeriesOnBall.hasFPowerSeri
-/
protected theorem HasFPowerSeriesWithinOnBall.fderivWithin [CompleteSpace F]
    (h : HasFPowerSeriesWithinOnBall f p s x r) (hu : UniqueDiffOn 𝕜 (insert x s)) :
    HasFPowerSeriesWithinOnBall (fderivWithin 𝕜 f (insert x s)) p.derivSeries s x r := by
  refine .congr' (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    (fun z hz => ?_)
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFPowerSeriesWithinOnBall ?_
    apply HasFPowerSeriesOnBall.hasFPowerSeriesWithinOnBall
    simpa using! ((p.hasFPowerSeriesOnBall_changeOrigin 1
      (h.r_pos.trans_le h.r_le)).mono h.r_pos h.r_le).comp_sub x
  · dsimp only
    rw [← h.fderivWithin_eq _ _ hu]; rw [add_sub_cancel]
    · simpa only [edist_eq_enorm_sub, Metric.mem_eball] using! hz.2
    · simpa using! hz.1

/--
theorem `HasFPowerSeriesWithinOnBall.fderivWithin_of_mem` / 定理 `HasFPowerSeriesWithinOnBall.fderivWithin_of_mem`

English:
theorem HasFPowerSeriesWithinOnBall.fderivWithin_of_mem
  statement: [CompleteSpace F]
  proof: by
  have : insert x s = s := insert_eq_of_mem hx
  rw [← this] at hu
  convert! h.fderivWithin hu
  exact this.symm

中文:
定理 有FPowerSeriesWithinOnBall.fderivWithin_of_mem
  结论: [完备空间 F]
  证明: by
  have : insert x s = s := insert_eq_of_mem hx
  rw [← this] at hu
  convert! h.fderivWithin hu
  exact this.symm
-/
protected theorem HasFPowerSeriesWithinOnBall.fderivWithin_of_mem [CompleteSpace F]
    (h : HasFPowerSeriesWithinOnBall f p s x r) (hu : UniqueDiffOn 𝕜 s) (hx : x in s) :
    HasFPowerSeriesWithinOnBall (fderivWithin 𝕜 f s) p.derivSeries s x r := by
  have : insert x s = s := insert_eq_of_mem hx
  rw [← this] at hu
  convert! h.fderivWithin hu
  exact this.symm

/-- If a function is analytic on a set `s`, so is its Fréchet derivative. -/
@[fun_prop]
/--
theorem `AnalyticAt.fderiv` / 定理 `AnalyticAt.fderiv`

English:
theorem AnalyticAt.fderiv
  given: [CompleteSpace F] (h : AnalyticAt 𝕜 f x)
  proof: by
  rcases h with ⟨p, r, hp⟩
  exact hp.fderiv.analyticAt

中文:
定理 AnalyticAt.fderiv
  条件: [完备空间 F] (h : AnalyticAt 𝕜 f x)
  证明: by
  rcases h with ⟨p, r, hp⟩
  exact hp.fderiv.analyticAt
-/
protected theorem AnalyticAt.fderiv [CompleteSpace F] (h : AnalyticAt 𝕜 f x) :
    AnalyticAt 𝕜 (fderiv 𝕜 f) x := by
  rcases h with ⟨p, r, hp⟩
  exact hp.fderiv.analyticAt

/--
theorem `AnalyticOnNhd.fderiv` / 定理 `AnalyticOnNhd.fderiv`

English:
theorem AnalyticOnNhd.fderiv
  given: [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s)
  proof: fun y hy => AnalyticAt.fderiv (h y hy)

中文:
定理 AnalyticOnNhd.fderiv
  条件: [完备空间 F] (h : AnalyticOnNhd 𝕜 f s)
  证明: fun y hy => AnalyticAt.fderiv (h y hy)
-/
protected theorem AnalyticOnNhd.fderiv [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) :
    AnalyticOnNhd 𝕜 (fderiv 𝕜 f) s :=
  fun y hy => AnalyticAt.fderiv (h y hy)

/--
theorem `AnalyticOnNhd.iteratedFDeriv` / 定理 `AnalyticOnNhd.iteratedFDeriv`

English:
theorem AnalyticOnNhd.iteratedFDeriv
  given: [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) (n : Nat)
  proof: by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_analyticOnNhd h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    -- Porting note: for reasons that I do not understand at all, `

中文:
定理 AnalyticOnNhd.iteratedFDeriv
  条件: [完备空间 F] (h : AnalyticOnNhd 𝕜 f s) (n : 自然数)
  证明: by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_analyticOnNhd h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    -- Porting note: for reasons that I do not understand at all, `
-/
protected theorem AnalyticOnNhd.iteratedFDeriv [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) (n : Nat) :
    AnalyticOnNhd 𝕜 (iteratedFDeriv 𝕜 n f) s := by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_analyticOnNhd h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    -- Porting note: for reasons that I do not understand at all, `?g` cannot be inlined.
    convert! ContinuousLinearMap.comp_analyticOnNhd ?g IH.fderiv
    case g => exact ↑(continuousMultilinearCurryLeftEquiv 𝕜 (fun _ : Fin (n + 1) => E) F).symm
    simp

/--
lemma `AnalyticOnNhd.hasFTaylorSeriesUpToOn` / 引理 `AnalyticOnNhd.hasFTaylorSeriesUpToOn`

English:
lemma AnalyticOnNhd.hasFTaylorSeriesUpToOn
  statement: [CompleteSpace F]
  proof: by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · apply HasFDerivAt.hasFDerivWithinAt
    exact ((h.iteratedFDeriv m x hx).differentiableAt).hasFDerivAt
  · apply (DifferentiableAt.continuousAt (𝕜 := 𝕜) ?_).continuousWithinAt
    exact (h.iteratedFDeriv m x hx).different

中文:
引理 AnalyticOnNhd.hasFTaylorSeriesUpToOn
  结论: [完备空间 F]
  证明: by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · apply HasFDerivAt.hasFDerivWithinAt
    exact ((h.iteratedFDeriv m x hx).differentiableAt).hasFDerivAt
  · apply (DifferentiableAt.continuousAt (𝕜 := 𝕜) ?_).continuousWithinAt
    exact (h.iteratedFDeriv m x hx).different

Depends on / 依赖: DifferentiableAt, DifferentiableAt.continuousAt, HasFDerivAt, HasFDerivAt.hasFDerivWithinAt, continuousAt, continuousWithinAt, differentiableAt, h.iteratedFDeriv, hasFDerivAt, hasFDerivWithinAt, iteratedFDeriv
-/
lemma AnalyticOnNhd.hasFTaylorSeriesUpToOn [CompleteSpace F]
    (n : WithTop Nat∞) (h : AnalyticOnNhd 𝕜 f s) :
    HasFTaylorSeriesUpToOn n f (ftaylorSeries 𝕜 f) s := by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · apply HasFDerivAt.hasFDerivWithinAt
    exact ((h.iteratedFDeriv m x hx).differentiableAt).hasFDerivAt
  · apply (DifferentiableAt.continuousAt (𝕜 := 𝕜) ?_).continuousWithinAt
    exact (h.iteratedFDeriv m x hx).differentiableAt

/--
lemma `AnalyticWithinAt.exists_hasFTaylorSeriesUpToOn` / 引理 `AnalyticWithinAt.exists_hasFTaylorSeriesUpToOn`

English:
lemma AnalyticWithinAt.exists_hasFTaylorSeriesUpToOn
  statement: [CompleteSpace F]
  proof: by
  rcases h.exists_analyticAt with ⟨g, -, fg, hg⟩
  rcases hg.exists_mem_nhds_analyticOnNhd with ⟨v, vx, hv⟩
  refine ⟨insert x s inter v, inter_mem_nhdsWithin _ vx, ftaylorSeries 𝕜 g, ?_, fun i => ?_⟩
  · suffices HasFTaylorSeriesUpToOn n g (ftaylorSeries 𝕜 g) (insert x s inter v) from
      this

中文:
引理 AnalyticWithinAt.存在_hasFTaylorSeriesUpToOn
  结论: [完备空间 F]
  证明: by
  rcases h.exists_analyticAt with ⟨g, -, fg, hg⟩
  rcases hg.exists_mem_nhds_analyticOnNhd with ⟨v, vx, hv⟩
  refine ⟨insert x s inter v, inter_mem_nhdsWithin _ vx, ftaylorSeries 𝕜 g, ?_, fun i => ?_⟩
  · suffices HasFTaylorSeriesUpToOn n g (ftaylorSeries 𝕜 g) (insert x s inter v) from
      this

Depends on / 依赖: AnalyticOnNhd, AnalyticOnNhd.hasFTaylorSeriesUpToOn, HasFTaylorSeriesUpToOn, Set.inter_subset_right, analyticOn, analyticOn.mono, exists_analyticAt, exists_mem_nhds_analyticOnNhd, ftaylorSeries, h.exists_analyticAt, hasFTaylorSeriesUpToOn, hg.exists_mem_nhds_analyticOnNhd, hv.iteratedFDeriv, hv.mono, insert, inter_mem_nhdsWithin, inter_subset_right, iteratedFDeriv, this.congr
-/
lemma AnalyticWithinAt.exists_hasFTaylorSeriesUpToOn [CompleteSpace F]
    (n : WithTop Nat∞) (h : AnalyticWithinAt 𝕜 f s x) :
    exists u in 𝓝[insert x s] x, exists (p : E -> FormalMultilinearSeries 𝕜 E F),
    HasFTaylorSeriesUpToOn n f p u ∧ forall i, AnalyticOn 𝕜 (fun x => p x i) u := by
  rcases h.exists_analyticAt with ⟨g, -, fg, hg⟩
  rcases hg.exists_mem_nhds_analyticOnNhd with ⟨v, vx, hv⟩
  refine ⟨insert x s inter v, inter_mem_nhdsWithin _ vx, ftaylorSeries 𝕜 g, ?_, fun i => ?_⟩
  · suffices HasFTaylorSeriesUpToOn n g (ftaylorSeries 𝕜 g) (insert x s inter v) from
      this.congr (fun y hy => fg hy.1)
    exact AnalyticOnNhd.hasFTaylorSeriesUpToOn _ (hv.mono Set.inter_subset_right)
  · exact (hv.iteratedFDeriv i).analyticOn.mono Set.inter_subset_right

/--
theorem `HasFPowerSeriesWithinOnBall.hasSum_derivSeries_of_hasFDerivWithinAt` / 定理 `HasFPowerSeriesWithinOnBall.hasSum_derivSeries_of_hasFDerivWithinAt`

English:
theorem HasFPowerSeriesWithinOnBall.hasSum_derivSeries_of_hasFDerivWithinAt
  proof: by
  /- In the completion of the space, the derivative series is summable, and its sum is a derivative
  of the function. Therefore, by uniqueness of derivatives, its sum is the image of `f'` under
  the canonical embedding. As this is an embedding, it means that there was also convergence in
  the 

中文:
定理 有FPowerSeriesWithinOnBall.hasSum_derivSeries_of_hasFDerivWithinAt
  证明: by
  /- In the completion of the space, the derivative series is summable, and its sum is a derivative
  of the function. Therefore, by uniqueness of derivatives, its sum is the image of `f'` under
  the canonical embedding. As this is an embedding, it means that there was also convergence in
  the 
-/
theorem HasFPowerSeriesWithinOnBall.hasSum_derivSeries_of_hasFDerivWithinAt
    (h : HasFPowerSeriesWithinOnBall f p s x r)
    {f' : E ->L[𝕜] F}
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) (h'y : x + y in insert x s)
    (hf' : HasFDerivWithinAt f f' (insert x s) (x + y))
    (hu : UniqueDiffOn 𝕜 (insert x s)) :
    HasSum (fun n => p.derivSeries n (fun _ => y)) f' := by
  /- In the completion of the space, the derivative series is summable, and its sum is a derivative
  of the function. Therefore, by uniqueness of derivatives, its sum is the image of `f'` under
  the canonical embedding. As this is an embedding, it means that there was also convergence in
  the original space, to `f'`. -/
  let F' := UniformSpace.Completion F
  let a : F ->L[𝕜] F' := UniformSpace.Completion.toComplL
  let b : (E ->L[𝕜] F) ->ₗᵢ[𝕜] (E ->L[𝕜] F') := UniformSpace.Completion.toComplₗᵢ.postcomp
  rw [← b.isEmbedding.hasSum_iff]
  have : HasFPowerSeriesWithinOnBall (a ∘ f) (a.compFormalMultilinearSeries p) s x r :=
    a.comp_hasFPowerSeriesWithinOnBall h
  have Z := (this.fderivWithin hu).hasSum h'y (by simpa [edist_zero_right] using! hy)
  have : fderivWithin 𝕜 (a ∘ f) (insert x s) (x + y) = a ∘L f' := by
    apply HasFDerivWithinAt.fderivWithin _ (hu _ h'y)
    exact a.hasFDerivAt.comp_hasFDerivWithinAt (x + y) hf'
  rw [this] at Z
  convert! Z with n
  ext v
  simp only [FormalMultilinearSeries.derivSeries, sum_apply,
    ContinuousLinearMap.compFormalMultilinearSeries_apply,
    FormalMultilinearSeries.changeOriginSeries,
    ContinuousLinearMap.compContinuousMultilinearMap_coe, ContinuousLinearEquiv.coe_coe,
    LinearIsometryEquiv.coe_coe, Function.comp_apply, sum_apply, map_sum]
  rfl

/--
theorem `HasFPowerSeriesWithinOnBall.fderivWithin_of_mem_of_analyticOn` / 定理 `HasFPowerSeriesWithinOnBall.fderivWithin_of_mem_of_analyticOn`

English:
theorem HasFPowerSeriesWithinOnBall.fderivWithin_of_mem_of_analyticOn
  proof: by
  refine ⟨hr.r_le.trans p.radius_le_radius_derivSeries, hr.r_pos, fun {y} hy h'y => ?_⟩
  apply hr.hasSum_derivSeries_of_hasFDerivWithinAt (by simpa [edist_zero_right] using! h'y) hy
  · rw [insert_eq_of_mem hx] at hy ⊢
    apply DifferentiableWithinAt.hasFDerivWithinAt
    exact h.differentiable

中文:
定理 有FPowerSeriesWithinOnBall.fderivWithin_of_mem_of_analyticOn
  证明: by
  refine ⟨hr.r_le.trans p.radius_le_radius_derivSeries, hr.r_pos, fun {y} hy h'y => ?_⟩
  apply hr.hasSum_derivSeries_of_hasFDerivWithinAt (by simpa [edist_zero_right] using! h'y) hy
  · rw [insert_eq_of_mem hx] at hy ⊢
    apply DifferentiableWithinAt.hasFDerivWithinAt
    exact h.differentiable
-/
protected theorem HasFPowerSeriesWithinOnBall.fderivWithin_of_mem_of_analyticOn
    (hr : HasFPowerSeriesWithinOnBall f p s x r)
    (h : AnalyticOn 𝕜 f s) (hs : UniqueDiffOn 𝕜 s) (hx : x in s) :
    HasFPowerSeriesWithinOnBall (fderivWithin 𝕜 f s) p.derivSeries s x r := by
  refine ⟨hr.r_le.trans p.radius_le_radius_derivSeries, hr.r_pos, fun {y} hy h'y => ?_⟩
  apply hr.hasSum_derivSeries_of_hasFDerivWithinAt (by simpa [edist_zero_right] using! h'y) hy
  · rw [insert_eq_of_mem hx] at hy ⊢
    apply DifferentiableWithinAt.hasFDerivWithinAt
    exact h.differentiableOn _ hy
  · rwa [insert_eq_of_mem hx]

/--
theorem `AnalyticOn.fderivWithin` / 定理 `AnalyticOn.fderivWithin`

English:
theorem AnalyticOn.fderivWithin
  given: (h : AnalyticOn 𝕜 f s) (hu : UniqueDiffOn 𝕜 s)
  proof: by
  intro x hx
  rcases h x hx with ⟨p, r, hr⟩
  refine ⟨p.derivSeries, r, hr.fderivWithin_of_mem_of_analyticOn h hu hx⟩

中文:
定理 AnalyticOn.fderivWithin
  条件: (h : AnalyticOn 𝕜 f s) (hu : UniqueDiffOn 𝕜 s)
  证明: by
  intro x hx
  rcases h x hx with ⟨p, r, hr⟩
  refine ⟨p.derivSeries, r, hr.fderivWithin_of_mem_of_analyticOn h hu hx⟩
-/
protected theorem AnalyticOn.fderivWithin (h : AnalyticOn 𝕜 f s) (hu : UniqueDiffOn 𝕜 s) :
    AnalyticOn 𝕜 (fderivWithin 𝕜 f s) s := by
  intro x hx
  rcases h x hx with ⟨p, r, hr⟩
  refine ⟨p.derivSeries, r, hr.fderivWithin_of_mem_of_analyticOn h hu hx⟩

/--
theorem `AnalyticOn.iteratedFDerivWithin` / 定理 `AnalyticOn.iteratedFDerivWithin`

English:
theorem AnalyticOn.iteratedFDerivWithin
  statement: (h : AnalyticOn 𝕜 f s)
  proof: by
  induction n with
  | zero =>
    rw [iteratedFDerivWithin_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F)
.comp_analyticOn h
  | succ n IH =>
    rw [iteratedFDerivWithin_succ_eq_comp_left]
    apply AnalyticOnNhd.comp_analyticOn _ (IH.fderivWithi

中文:
定理 AnalyticOn.iteratedFDerivWithin
  结论: (h : AnalyticOn 𝕜 f s)
  证明: by
  induction n with
  | zero =>
    rw [iteratedFDerivWithin_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F)
.comp_analyticOn h
  | succ n IH =>
    rw [iteratedFDerivWithin_succ_eq_comp_left]
    apply AnalyticOnNhd.comp_analyticOn _ (IH.fderivWithi
-/
protected theorem AnalyticOn.iteratedFDerivWithin (h : AnalyticOn 𝕜 f s)
    (hu : UniqueDiffOn 𝕜 s) (n : Nat) :
    AnalyticOn 𝕜 (iteratedFDerivWithin 𝕜 n f s) s := by
  induction n with
  | zero =>
    rw [iteratedFDerivWithin_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F)
.comp_analyticOn h
  | succ n IH =>
    rw [iteratedFDerivWithin_succ_eq_comp_left]
    apply AnalyticOnNhd.comp_analyticOn _ (IH.fderivWithin hu) (mapsTo_univ _ _)
    apply LinearIsometryEquiv.analyticOnNhd

/--
lemma `AnalyticOn.hasFTaylorSeriesUpToOn` / 引理 `AnalyticOn.hasFTaylorSeriesUpToOn`

English:
lemma AnalyticOn.hasFTaylorSeriesUpToOn
  statement: {n : WithTop Nat∞}
  proof: by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · have := (h.iteratedFDerivWithin hu m x hx).differentiableWithinAt.hasFDerivWithinAt
    rwa [insert_eq_of_mem hx] at this
  · exact (h.iteratedFDerivWithin hu m x hx).continuousWithinAt

中文:
引理 AnalyticOn.hasFTaylorSeriesUpToOn
  结论: {n : WithTop 自然数∞}
  证明: by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · have := (h.iteratedFDerivWithin hu m x hx).differentiableWithinAt.hasFDerivWithinAt
    rwa [insert_eq_of_mem hx] at this
  · exact (h.iteratedFDerivWithin hu m x hx).continuousWithinAt
-/
protected lemma AnalyticOn.hasFTaylorSeriesUpToOn {n : WithTop Nat∞}
    (h : AnalyticOn 𝕜 f s) (hu : UniqueDiffOn 𝕜 s) :
    HasFTaylorSeriesUpToOn n f (ftaylorSeriesWithin 𝕜 f s) s := by
  refine ⟨fun x _hx => rfl, fun m _hm x hx => ?_, fun m _hm x hx => ?_⟩
  · have := (h.iteratedFDerivWithin hu m x hx).differentiableWithinAt.hasFDerivWithinAt
    rwa [insert_eq_of_mem hx] at this
  · exact (h.iteratedFDerivWithin hu m x hx).continuousWithinAt

/--
lemma `AnalyticOn.exists_hasFTaylorSeriesUpToOn` / 引理 `AnalyticOn.exists_hasFTaylorSeriesUpToOn`

English:
lemma AnalyticOn.exists_hasFTaylorSeriesUpToOn
  proof: ⟨ftaylorSeriesWithin 𝕜 f s, h.hasFTaylorSeriesUpToOn hu, h.iteratedFDerivWithin hu⟩

中文:
引理 AnalyticOn.存在_hasFTaylorSeriesUpToOn
  证明: ⟨ftaylorSeriesWithin 𝕜 f s, h.hasFTaylorSeriesUpToOn hu, h.iteratedFDerivWithin hu⟩

Depends on / 依赖: ftaylorSeriesWithin, h.hasFTaylorSeriesUpToOn, h.iteratedFDerivWithin, hasFTaylorSeriesUpToOn, iteratedFDerivWithin
-/
lemma AnalyticOn.exists_hasFTaylorSeriesUpToOn
    (h : AnalyticOn 𝕜 f s) (hu : UniqueDiffOn 𝕜 s) :
    exists p : E -> FormalMultilinearSeries 𝕜 E F,
      HasFTaylorSeriesUpToOn ⊤ f p s ∧ forall i, AnalyticOn 𝕜 (fun x => p x i) s :=
  ⟨ftaylorSeriesWithin 𝕜 f s, h.hasFTaylorSeriesUpToOn hu, h.iteratedFDerivWithin hu⟩

/--
theorem `AnalyticOnNhd.fderiv_of_isOpen` / 定理 `AnalyticOnNhd.fderiv_of_isOpen`

English:
theorem AnalyticOnNhd.fderiv_of_isOpen
  given: (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s)
  proof: by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.fderivWithin hs.uniqueDiffOn).congr (fun x hx => (fderivWithin_of_isOpen hs hx).symm)

中文:
定理 AnalyticOnNhd.fderiv_of_isOpen
  条件: (h : AnalyticOnNhd 𝕜 f s) (hs : 是开集 s)
  证明: by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.fderivWithin hs.uniqueDiffOn).congr (fun x hx => (fderivWithin_of_isOpen hs hx).symm)

Depends on / 依赖: analyticOn_iff_analyticOnNhd, fderivWithin, fderivWithin_of_isOpen, h.fderivWithin, hs.analyticOn_iff_analyticOnNhd, hs.uniqueDiffOn, uniqueDiffOn
-/
theorem AnalyticOnNhd.fderiv_of_isOpen (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s) :
    AnalyticOnNhd 𝕜 (fderiv 𝕜 f) s := by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.fderivWithin hs.uniqueDiffOn).congr (fun x hx => (fderivWithin_of_isOpen hs hx).symm)

/--
theorem `AnalyticOnNhd.iteratedFDeriv_of_isOpen` / 定理 `AnalyticOnNhd.iteratedFDeriv_of_isOpen`

English:
theorem AnalyticOnNhd.iteratedFDeriv_of_isOpen
  given: (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s) (n : Nat)
  proof: by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.iteratedFDerivWithin hs.uniqueDiffOn n).congr
    (fun x hx => (iteratedFDerivWithin_of_isOpen n hs hx).symm)

中文:
定理 AnalyticOnNhd.iteratedFDeriv_of_isOpen
  条件: (h : AnalyticOnNhd 𝕜 f s) (hs : 是开集 s) (n : 自然数)
  证明: by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.iteratedFDerivWithin hs.uniqueDiffOn n).congr
    (fun x hx => (iteratedFDerivWithin_of_isOpen n hs hx).symm)

Depends on / 依赖: analyticOn_iff_analyticOnNhd, h.iteratedFDerivWithin, hs.analyticOn_iff_analyticOnNhd, hs.uniqueDiffOn, iteratedFDerivWithin, iteratedFDerivWithin_of_isOpen, uniqueDiffOn
-/
theorem AnalyticOnNhd.iteratedFDeriv_of_isOpen (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s) (n : Nat) :
    AnalyticOnNhd 𝕜 (iteratedFDeriv 𝕜 n f) s := by
  rw [← hs.analyticOn_iff_analyticOnNhd] at h ⊢
  exact (h.iteratedFDerivWithin hs.uniqueDiffOn n).congr
    (fun x hx => (iteratedFDerivWithin_of_isOpen n hs hx).symm)

/--
theorem `OpenPartialHomeomorph.analyticAt_symm'` / 定理 `OpenPartialHomeomorph.analyticAt_symm'`

English:
theorem OpenPartialHomeomorph.analyticAt_symm'
  statement: (f : OpenPartialHomeomorph E F) {a : E}
  proof: by
  rcases h with ⟨p, hp⟩
  have : p 1 = (continuousMultilinearCurryFin1 𝕜 E F).symm i := by simp [← h', hp.fderiv_eq]
  exact (f.hasFPowerSeriesAt_symm h0 hp this).analyticAt

中文:
定理 OpenPartialHomeomorph.analyticAt_symm'
  结论: (f : OpenPartialHomeomorph E F) {a : E}
  证明: by
  rcases h with ⟨p, hp⟩
  have : p 1 = (continuousMultilinearCurryFin1 𝕜 E F).symm i := by simp [← h', hp.fderiv_eq]
  exact (f.hasFPowerSeriesAt_symm h0 hp this).analyticAt

Depends on / 依赖: analyticAt, continuousMultilinearCurryFin1, f.hasFPowerSeriesAt_symm, fderiv_eq, hasFPowerSeriesAt_symm, hp.fderiv_eq
-/
theorem OpenPartialHomeomorph.analyticAt_symm' (f : OpenPartialHomeomorph E F) {a : E}
    {i : E ≃L[𝕜] F} (h0 : a in f.source) (h : AnalyticAt 𝕜 f a) (h' : fderiv 𝕜 f a = i) :
    AnalyticAt 𝕜 f.symm (f a) := by
  rcases h with ⟨p, hp⟩
  have : p 1 = (continuousMultilinearCurryFin1 𝕜 E F).symm i := by simp [← h', hp.fderiv_eq]
  exact (f.hasFPowerSeriesAt_symm h0 hp this).analyticAt

/--
theorem `OpenPartialHomeomorph.analyticAt_symm` / 定理 `OpenPartialHomeomorph.analyticAt_symm`

English:
theorem OpenPartialHomeomorph.analyticAt_symm
  statement: (f : OpenPartialHomeomorph E F) {a : F}
  proof: by
  have : a = f (f.symm a) := by simp [h0]
  rw [this]
  exact f.analyticAt_symm' (by simp [h0]) h h'

中文:
定理 OpenPartialHomeomorph.analyticAt_symm
  结论: (f : OpenPartialHomeomorph E F) {a : F}
  证明: by
  have : a = f (f.symm a) := by simp [h0]
  rw [this]
  exact f.analyticAt_symm' (by simp [h0]) h h'

Depends on / 依赖: analyticAt_symm, f.analyticAt_symm, f.symm
-/
theorem OpenPartialHomeomorph.analyticAt_symm (f : OpenPartialHomeomorph E F) {a : F}
    {i : E ≃L[𝕜] F} (h0 : a in f.target) (h : AnalyticAt 𝕜 f (f.symm a))
    (h' : fderiv 𝕜 f (f.symm a) = i) :
    AnalyticAt 𝕜 f.symm a := by
  have : a = f (f.symm a) := by simp [h0]
  rw [this]
  exact f.analyticAt_symm' (by simp [h0]) h h'

end fderiv

section deriv

variable {p : FormalMultilinearSeries 𝕜 𝕜 F} {r : Real>=0∞}
variable {f : 𝕜 -> F} {x : 𝕜} {s : Set 𝕜}

/--
theorem `HasFPowerSeriesAt.hasStrictDerivAt` / 定理 `HasFPowerSeriesAt.hasStrictDerivAt`

English:
theorem HasFPowerSeriesAt.hasStrictDerivAt
  given: (h : HasFPowerSeriesAt f p x)
  proof: h.hasStrictFDerivAt.hasStrictDerivAt

中文:
定理 HasFPowerSeriesAt.hasStrictDerivAt
  条件: (h : HasFPowerSeriesAt f p x)
  证明: h.hasStrictFDerivAt.hasStrictDerivAt
-/
protected theorem HasFPowerSeriesAt.hasStrictDerivAt (h : HasFPowerSeriesAt f p x) :
    HasStrictDerivAt f (p 1 fun _ => 1) x :=
  h.hasStrictFDerivAt.hasStrictDerivAt

/--
theorem `HasFPowerSeriesAt.hasDerivAt` / 定理 `HasFPowerSeriesAt.hasDerivAt`

English:
theorem HasFPowerSeriesAt.hasDerivAt
  given: (h : HasFPowerSeriesAt f p x)
  proof: h.hasStrictDerivAt.hasDerivAt

中文:
定理 HasFPowerSeriesAt.hasDerivAt
  条件: (h : HasFPowerSeriesAt f p x)
  证明: h.hasStrictDerivAt.hasDerivAt
-/
protected theorem HasFPowerSeriesAt.hasDerivAt (h : HasFPowerSeriesAt f p x) :
    HasDerivAt f (p 1 fun _ => 1) x :=
  h.hasStrictDerivAt.hasDerivAt

/--
theorem `HasFPowerSeriesAt.deriv` / 定理 `HasFPowerSeriesAt.deriv`

English:
theorem HasFPowerSeriesAt.deriv
  given: (h : HasFPowerSeriesAt f p x)
  proof: h.hasDerivAt.deriv

中文:
定理 HasFPowerSeriesAt.deriv
  条件: (h : HasFPowerSeriesAt f p x)
  证明: h.hasDerivAt.deriv
-/
protected theorem HasFPowerSeriesAt.deriv (h : HasFPowerSeriesAt f p x) :
    deriv f x = p 1 fun _ => 1 :=
  h.hasDerivAt.deriv

/--
theorem `AnalyticOnNhd.deriv` / 定理 `AnalyticOnNhd.deriv`

English:
theorem AnalyticOnNhd.deriv
  given: [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s)
  proof: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd h.fderiv

中文:
定理 AnalyticOnNhd.deriv
  条件: [完备空间 F] (h : AnalyticOnNhd 𝕜 f s)
  证明: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd h.fderiv
-/
protected theorem AnalyticOnNhd.deriv [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) :
    AnalyticOnNhd 𝕜 (deriv f) s :=
  (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd h.fderiv

/--
theorem `AnalyticOnNhd.deriv_of_isOpen` / 定理 `AnalyticOnNhd.deriv_of_isOpen`

English:
theorem AnalyticOnNhd.deriv_of_isOpen
  given: (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s)
  proof: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd (h.fderiv_of_isOpen hs)

中文:
定理 AnalyticOnNhd.deriv_of_isOpen
  条件: (h : AnalyticOnNhd 𝕜 f s) (hs : 是开集 s)
  证明: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd (h.fderiv_of_isOpen hs)

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.apply, comp_analyticOnNhd, fderiv_of_isOpen, h.fderiv_of_isOpen
-/
theorem AnalyticOnNhd.deriv_of_isOpen (h : AnalyticOnNhd 𝕜 f s) (hs : IsOpen s) :
    AnalyticOnNhd 𝕜 (deriv f) s :=
  (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_analyticOnNhd (h.fderiv_of_isOpen hs)

/--
theorem `AnalyticOnNhd.iterated_deriv` / 定理 `AnalyticOnNhd.iterated_deriv`

English:
theorem AnalyticOnNhd.iterated_deriv
  given: [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) (n : Nat)
  proof: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

中文:
定理 AnalyticOnNhd.iterated_deriv
  条件: [完备空间 F] (h : AnalyticOnNhd 𝕜 f s) (n : 自然数)
  证明: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

Depends on / 依赖: Function, Function.comp_apply, Function.iterate_succ, IH.deriv, comp_apply, iterate_succ
-/
theorem AnalyticOnNhd.iterated_deriv [CompleteSpace F] (h : AnalyticOnNhd 𝕜 f s) (n : Nat) :
    AnalyticOnNhd 𝕜 (deriv^[n] f) s := by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

/--
theorem `AnalyticAt.deriv` / 定理 `AnalyticAt.deriv`

English:
theorem AnalyticAt.deriv
  given: [CompleteSpace F] (h : AnalyticAt 𝕜 f x)
  proof: by
  obtain ⟨r, hr, h⟩ := h.exists_ball_analyticOnNhd
  exact h.deriv x (by simp [hr])

中文:
定理 AnalyticAt.deriv
  条件: [完备空间 F] (h : AnalyticAt 𝕜 f x)
  证明: by
  obtain ⟨r, hr, h⟩ := h.exists_ball_analyticOnNhd
  exact h.deriv x (by simp [hr])
-/
@[fun_prop] protected theorem AnalyticAt.deriv [CompleteSpace F] (h : AnalyticAt 𝕜 f x) :
    AnalyticAt 𝕜 (deriv f) x := by
  obtain ⟨r, hr, h⟩ := h.exists_ball_analyticOnNhd
  exact h.deriv x (by simp [hr])

/--
theorem `AnalyticAt.iterated_deriv` / 定理 `AnalyticAt.iterated_deriv`

English:
theorem AnalyticAt.iterated_deriv
  given: [CompleteSpace F] (h : AnalyticAt 𝕜 f x) (n : Nat)
  proof: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

中文:
定理 AnalyticAt.iterated_deriv
  条件: [完备空间 F] (h : AnalyticAt 𝕜 f x) (n : 自然数)
  证明: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv
-/
@[fun_prop] theorem AnalyticAt.iterated_deriv [CompleteSpace F] (h : AnalyticAt 𝕜 f x) (n : Nat) :
    AnalyticAt 𝕜 (deriv^[n] f) x := by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

end deriv
section fderiv

variable {p : FormalMultilinearSeries 𝕜 E F} {r : Real>=0∞} {n : Nat}
variable {f : E -> F} {x : E} {s : Set E}


/--
theorem `HasFiniteFPowerSeriesOnBall.differentiableOn` / 定理 `HasFiniteFPowerSeriesOnBall.differentiableOn`

English:
theorem HasFiniteFPowerSeriesOnBall.differentiableOn
  proof: fun _ hy => (h.cpolynomialAt_of_mem hy).analyticAt.differentiableWithinAt

中文:
定理 有FiniteFPowerSeriesOnBall.differentiableOn
  证明: fun _ hy => (h.cpolynomialAt_of_mem hy).analyticAt.differentiableWithinAt

Depends on / 依赖: analyticAt, analyticAt.differentiableWithinAt, cpolynomialAt_of_mem, differentiableWithinAt, h.cpolynomialAt_of_mem
-/
theorem HasFiniteFPowerSeriesOnBall.differentiableOn
    (h : HasFiniteFPowerSeriesOnBall f p x n r) : DifferentiableOn 𝕜 f (Metric.eball x r) :=
  fun _ hy => (h.cpolynomialAt_of_mem hy).analyticAt.differentiableWithinAt

/--
theorem `HasFiniteFPowerSeriesOnBall.hasStrictFDerivAt` / 定理 `HasFiniteFPowerSeriesOnBall.hasStrictFDerivAt`

English:
theorem HasFiniteFPowerSeriesOnBall.hasStrictFDerivAt
  statement: (h : HasFiniteFPowerSeriesOnBall f p x n r)
  proof: (h.changeOrigin hy).toHasFPowerSeriesOnBall.hasFPowerSeriesAt.hasStrictFDerivAt

中文:
定理 有FiniteFPowerSeriesOnBall.hasStrictFDerivAt
  结论: (h : 有FiniteFPowerSeriesOnBall f p x n r)
  证明: (h.changeOrigin hy).toHasFPowerSeriesOnBall.hasFPowerSeriesAt.hasStrictFDerivAt

Depends on / 依赖: changeOrigin, h.changeOrigin, hasFPowerSeriesAt, hasStrictFDerivAt, toHasFPowerSeriesOnBall, toHasFPowerSeriesOnBall.hasFPowerSeriesAt.hasStrictFDerivAt
-/
theorem HasFiniteFPowerSeriesOnBall.hasStrictFDerivAt (h : HasFiniteFPowerSeriesOnBall f p x n r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) :
    HasStrictFDerivAt f (continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1)) (x + y) :=
  (h.changeOrigin hy).toHasFPowerSeriesOnBall.hasFPowerSeriesAt.hasStrictFDerivAt

/--
theorem `HasFiniteFPowerSeriesOnBall.hasFDerivAt` / 定理 `HasFiniteFPowerSeriesOnBall.hasFDerivAt`

English:
theorem HasFiniteFPowerSeriesOnBall.hasFDerivAt
  statement: (h : HasFiniteFPowerSeriesOnBall f p x n r)
  proof: (h.hasStrictFDerivAt hy).hasFDerivAt

中文:
定理 有FiniteFPowerSeriesOnBall.hasFDerivAt
  结论: (h : 有FiniteFPowerSeriesOnBall f p x n r)
  证明: (h.hasStrictFDerivAt hy).hasFDerivAt

Depends on / 依赖: h.hasStrictFDerivAt, hasFDerivAt, hasStrictFDerivAt
-/
theorem HasFiniteFPowerSeriesOnBall.hasFDerivAt (h : HasFiniteFPowerSeriesOnBall f p x n r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) :
    HasFDerivAt f (continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1)) (x + y) :=
  (h.hasStrictFDerivAt hy).hasFDerivAt

/--
theorem `HasFiniteFPowerSeriesOnBall.fderiv_eq` / 定理 `HasFiniteFPowerSeriesOnBall.fderiv_eq`

English:
theorem HasFiniteFPowerSeriesOnBall.fderiv_eq
  statement: (h : HasFiniteFPowerSeriesOnBall f p x n r)
  proof: (h.hasFDerivAt hy).fderiv

中文:
定理 有FiniteFPowerSeriesOnBall.fderiv_eq
  结论: (h : 有FiniteFPowerSeriesOnBall f p x n r)
  证明: (h.hasFDerivAt hy).fderiv

Depends on / 依赖: fderiv, h.hasFDerivAt, hasFDerivAt
-/
theorem HasFiniteFPowerSeriesOnBall.fderiv_eq (h : HasFiniteFPowerSeriesOnBall f p x n r)
    {y : E} (hy : (‖y‖₊ : Real>=0∞) < r) :
    fderiv 𝕜 f (x + y) = continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin y 1) :=
  (h.hasFDerivAt hy).fderiv

/--
theorem `HasFiniteFPowerSeriesOnBall.fderiv` / 定理 `HasFiniteFPowerSeriesOnBall.fderiv`

English:
theorem HasFiniteFPowerSeriesOnBall.fderiv
  proof: by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFiniteFPowerSeriesOnBall ?_
    simpa using!
      ((p.hasFiniteFPowerSeries

中文:
定理 有FiniteFPowerSeriesOnBall.fderiv
  证明: by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFiniteFPowerSeriesOnBall ?_
    simpa using!
      ((p.hasFiniteFPowerSeries
-/
protected theorem HasFiniteFPowerSeriesOnBall.fderiv
    (h : HasFiniteFPowerSeriesOnBall f p x (n + 1) r) :
    HasFiniteFPowerSeriesOnBall (fderiv 𝕜 f) p.derivSeries x n r := by
  refine .congr (f := fun z => continuousMultilinearCurryFin1 𝕜 E F (p.changeOrigin (z - x) 1)) ?_
    fun z hz => ?_
  · refine continuousMultilinearCurryFin1 𝕜 E F
.toContinuousLinearEquiv.toContinuousLinearMap.comp_hasFiniteFPowerSeriesOnBall ?_
    simpa using!
      ((p.hasFiniteFPowerSeriesOnBall_changeOrigin 1 h.finite).mono h.r_pos le_top).comp_sub x
  dsimp only
  rw [← h.fderiv_eq]; rw [add_sub_cancel]
  simpa only [edist_eq_enorm_sub, Metric.mem_eball] using! hz

/--
theorem `HasFiniteFPowerSeriesOnBall.fderiv'` / 定理 `HasFiniteFPowerSeriesOnBall.fderiv'`

English:
theorem HasFiniteFPowerSeriesOnBall.fderiv'
  given: (h : HasFiniteFPowerSeriesOnBall f p x n r)
  proof: by
  obtain rfl | hn := eq_or_ne n 0
  · rw [zero_tsub]
    refine HasFiniteFPowerSeriesOnBall.bound_zero_of_eq_zero (fun y hy => ?_) h.r_pos fun n => ?_
    · rw [Filter.EventuallyEq.fderiv_eq (f := fun _ => 0)]
      · simp
      · exact Filter.eventuallyEq_iff_exists_mem.mpr ⟨Metric.eball x r,
  

中文:
定理 有FiniteFPowerSeriesOnBall.fderiv'
  条件: (h : 有FiniteFPowerSeriesOnBall f p x n r)
  证明: by
  obtain rfl | hn := eq_or_ne n 0
  · rw [zero_tsub]
    refine HasFiniteFPowerSeriesOnBall.bound_zero_of_eq_zero (fun y hy => ?_) h.r_pos fun n => ?_
    · rw [Filter.EventuallyEq.fderiv_eq (f := fun _ => 0)]
      · simp
      · exact Filter.eventuallyEq_iff_exists_mem.mpr ⟨Metric.eball x r,
  

Depends on / 依赖: ContinuousMultilinearMap, ContinuousMultilinearMap.ext, EventuallyEq, Filter, Filter.EventuallyEq.fderiv_eq, Filter.eventuallyEq_iff_exists_mem.mpr, HasFiniteFPowerSeriesOnBall, HasFiniteFPowerSeriesOnBall.bound_zero_of_eq_zero, Metric, Metric.eball, Metric.isOpen_eball.mem_nhds, bound_zero_of_eq_zero, changeOrigin, changeOriginSeries, continuousMultilinearCurryFin1, eq_or_ne, eq_zero_of_bound_zero, eventuallyEq_iff_exists_mem, fderiv_eq, h.eq_zero_of_bound_zero
-/
theorem HasFiniteFPowerSeriesOnBall.fderiv' (h : HasFiniteFPowerSeriesOnBall f p x n r) :
    HasFiniteFPowerSeriesOnBall (fderiv 𝕜 f) p.derivSeries x (n - 1) r := by
  obtain rfl | hn := eq_or_ne n 0
  · rw [zero_tsub]
    refine HasFiniteFPowerSeriesOnBall.bound_zero_of_eq_zero (fun y hy => ?_) h.r_pos fun n => ?_
    · rw [Filter.EventuallyEq.fderiv_eq (f := fun _ => 0)]
      · simp
      · exact Filter.eventuallyEq_iff_exists_mem.mpr ⟨Metric.eball x r,
          Metric.isOpen_eball.mem_nhds hy, fun z hz => by rw [h.eq_zero_of_bound_zero z hz]⟩
    · apply ContinuousMultilinearMap.ext; intro a
      change (continuousMultilinearCurryFin1 𝕜 E F) (p.changeOriginSeries 1 n a) = 0
      rw [p.changeOriginSeries_finite_of_finite h.finite 1 (Nat.zero_le _)]
      exact map_zero _
  · rw [← Nat.succ_pred hn] at h
    exact h.fderiv

/--
theorem `CPolynomialOn.fderiv` / 定理 `CPolynomialOn.fderiv`

English:
theorem CPolynomialOn.fderiv
  given: (h : CPolynomialOn 𝕜 f s)
  proof: by
  intro y hy
  rcases h y hy with ⟨p, r, n, hp⟩
  exact hp.fderiv'.cpolynomialAt

中文:
定理 CPolynomialOn.fderiv
  条件: (h : CPolynomialOn 𝕜 f s)
  证明: by
  intro y hy
  rcases h y hy with ⟨p, r, n, hp⟩
  exact hp.fderiv'.cpolynomialAt

Depends on / 依赖: cpolynomialAt, fderiv, hp.fderiv
-/
theorem CPolynomialOn.fderiv (h : CPolynomialOn 𝕜 f s) :
    CPolynomialOn 𝕜 (fderiv 𝕜 f) s := by
  intro y hy
  rcases h y hy with ⟨p, r, n, hp⟩
  exact hp.fderiv'.cpolynomialAt

/--
theorem `CPolynomialOn.iteratedFDeriv` / 定理 `CPolynomialOn.iteratedFDeriv`

English:
theorem CPolynomialOn.iteratedFDeriv
  given: (h : CPolynomialOn 𝕜 f s) (n : Nat)
  proof: by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_cpolynomialOn h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    convert! ContinuousLinearMap.comp_cpolynomialOn ?g IH.fderiv
  

中文:
定理 CPolynomialOn.iteratedFDeriv
  条件: (h : CPolynomialOn 𝕜 f s) (n : 自然数)
  证明: by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_cpolynomialOn h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    convert! ContinuousLinearMap.comp_cpolynomialOn ?g IH.fderiv
  

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.comp_cpolynomialOn, IH.fderiv, comp_cpolynomialOn, continuousMultilinearCurryFin0, continuousMultilinearCurryLeftEquiv, convert, fderiv, iteratedFDeriv_succ_eq_comp_left, iteratedFDeriv_zero_eq_comp
-/
theorem CPolynomialOn.iteratedFDeriv (h : CPolynomialOn 𝕜 f s) (n : Nat) :
    CPolynomialOn 𝕜 (iteratedFDeriv 𝕜 n f) s := by
  induction n with
  | zero =>
    rw [iteratedFDeriv_zero_eq_comp]
    exact ((continuousMultilinearCurryFin0 𝕜 E F).symm : F ->L[𝕜] E [×0]->L[𝕜] F).comp_cpolynomialOn h
  | succ n IH =>
    rw [iteratedFDeriv_succ_eq_comp_left]
    convert! ContinuousLinearMap.comp_cpolynomialOn ?g IH.fderiv
    case g => exact ↑(continuousMultilinearCurryLeftEquiv 𝕜 (fun _ : Fin (n + 1) => E) F).symm
    simp

end fderiv

section deriv

variable {p : FormalMultilinearSeries 𝕜 𝕜 F} {r : Real>=0∞}
variable {f : 𝕜 -> F} {x : 𝕜} {s : Set 𝕜}

/--
theorem `CPolynomialOn.deriv` / 定理 `CPolynomialOn.deriv`

English:
theorem CPolynomialOn.deriv
  given: (h : CPolynomialOn 𝕜 f s)
  statement: CPolynomialOn 𝕜 (deriv f) s
  proof: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_cpolynomialOn h.fderiv

中文:
定理 CPolynomialOn.deriv
  条件: (h : CPolynomialOn 𝕜 f s)
  结论: CPolynomialOn 𝕜 (deriv f) s
  证明: (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_cpolynomialOn h.fderiv
-/
protected theorem CPolynomialOn.deriv (h : CPolynomialOn 𝕜 f s) : CPolynomialOn 𝕜 (deriv f) s :=
  (ContinuousLinearMap.apply 𝕜 F (1 : 𝕜)).comp_cpolynomialOn h.fderiv

/--
theorem `CPolynomialOn.iterated_deriv` / 定理 `CPolynomialOn.iterated_deriv`

English:
theorem CPolynomialOn.iterated_deriv
  given: (h : CPolynomialOn 𝕜 f s) (n : Nat)
  proof: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

中文:
定理 CPolynomialOn.iterated_deriv
  条件: (h : CPolynomialOn 𝕜 f s) (n : 自然数)
  证明: by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

Depends on / 依赖: Function, Function.comp_apply, Function.iterate_succ, IH.deriv, comp_apply, iterate_succ
-/
theorem CPolynomialOn.iterated_deriv (h : CPolynomialOn 𝕜 f s) (n : Nat) :
    CPolynomialOn 𝕜 (deriv^[n] f) s := by
  induction n with
  | zero => exact h
  | succ n IH => simpa only [Function.iterate_succ', Function.comp_apply] using IH.deriv

end deriv

namespace ContinuousMultilinearMap

variable {ι : Type*} {E : ι -> Type*} [forall i, NormedAddCommGroup (E i)] [forall i, NormedSpace 𝕜 (E i)]
  [Fintype ι] (f : ContinuousMultilinearMap 𝕜 E F)

open FormalMultilinearSeries

/--
theorem `changeOriginSeries_support` / 定理 `changeOriginSeries_support`

English:
theorem changeOriginSeries_support
  given: {k l : Nat} (h : k + l != Fintype.card ι)
  proof: Finset.sum_eq_zero fun _ _ => by
    simp_rw [FormalMultilinearSeries.changeOriginSeriesTerm,
      toFormalMultilinearSeries, dif_neg h.symm, LinearIsometryEquiv.map_zero]

中文:
定理 changeOriginSeries_support
  条件: {k l : 自然数} (h : k + l != 有限类型.card ι)
  证明: Finset.sum_eq_zero fun _ _ => by
    simp_rw [FormalMultilinearSeries.changeOriginSeriesTerm,
      toFormalMultilinearSeries, dif_neg h.symm, LinearIsometryEquiv.map_zero]

Depends on / 依赖: Finset, Finset.sum_eq_zero, FormalMultilinearSeries, FormalMultilinearSeries.changeOriginSeriesTerm, LinearIsometryEquiv, LinearIsometryEquiv.map_zero, changeOriginSeriesTerm, dif_neg, h.symm, map_zero, simp_rw, sum_eq_zero, toFormalMultilinearSeries
-/
theorem changeOriginSeries_support {k l : Nat} (h : k + l != Fintype.card ι) :
    f.toFormalMultilinearSeries.changeOriginSeries k l = 0 :=
  Finset.sum_eq_zero fun _ _ => by
    simp_rw [FormalMultilinearSeries.changeOriginSeriesTerm,
      toFormalMultilinearSeries, dif_neg h.symm, LinearIsometryEquiv.map_zero]

variable {n : WithTop Nat∞} (x : forall i, E i)

open Finset in
/--
theorem `changeOrigin_toFormalMultilinearSeries` / 定理 `changeOrigin_toFormalMultilinearSeries`

English:
theorem changeOrigin_toFormalMultilinearSeries
  given: [DecidableEq ι]
  proof: by
  ext y
  rw [continuousMultilinearCurryFin1_apply]; rw [linearDeriv_apply]; rw [changeOrigin]; rw [FormalMultilinearSeries.sum]
  cases isEmpty_or_nonempty ι
  · have (l : _) : 1 + l != Fintype.card ι := by
      rw [add_comm]; rw [Fintype.card_eq_zero]; exact Nat.succ_ne_zero _
    simp_rw [Fin

中文:
定理 changeOrigin_toFormalMultilinearSeries
  条件: [DecidableEq ι]
  证明: by
  ext y
  rw [continuousMultilinearCurryFin1_apply]; rw [linearDeriv_apply]; rw [changeOrigin]; rw [FormalMultilinearSeries.sum]
  cases isEmpty_or_nonempty ι
  · have (l : _) : 1 + l != Fintype.card ι := by
      rw [add_comm]; rw [Fintype.card_eq_zero]; exact Nat.succ_ne_zero _
    simp_rw [Fin

Depends on / 依赖: Fintype, Fintype.card, Fintype.card_eq_zero, Fintype.sum_empty, FormalMultilinearSeries, FormalMultilinearSeries.sum, Nat.succ_ne_zero, _root_, _root_.zero_apply, add_comm, card_eq_zero, changeOrigin, changeOriginSeries, changeOriginSeries_support, continuousMultilinearCurryFin1_apply, eq_tsub_iff_add_eq_of_le, isEmpty_or_nonempty, linearDeriv_apply, simp_rw, succ_ne_zero
-/
theorem changeOrigin_toFormalMultilinearSeries [DecidableEq ι] :
    continuousMultilinearCurryFin1 𝕜 (forall i, E i) F (f.toFormalMultilinearSeries.changeOrigin x 1) =
    f.linearDeriv x := by
  ext y
  rw [continuousMultilinearCurryFin1_apply]; rw [linearDeriv_apply]; rw [changeOrigin]; rw [FormalMultilinearSeries.sum]
  cases isEmpty_or_nonempty ι
  · have (l : _) : 1 + l != Fintype.card ι := by
      rw [add_comm]; rw [Fintype.card_eq_zero]; exact Nat.succ_ne_zero _
    simp_rw [Fintype.sum_empty, changeOriginSeries_support _ (this _), _root_.zero_apply _,
      tsum_zero]; rfl
  rw [tsum_eq_single (Fintype.card ι - 1)]; rw [changeOriginSeries]; swap
  · intro m hm
    rw [Ne]; rw [eq_tsub_iff_add_eq_of_le (by exact Fintype.card_pos)]; rw [add_comm] at hm
    rw [f.changeOriginSeries_support hm]; rw [_root_.zero_apply]
  rw [_root_.sum_apply]; rw [_root_.sum_apply]; rw [Fin.snoc_zero]
  simp_rw [changeOriginSeriesTerm_apply]
  refine (Fintype.sum_bijective (?_ ∘ Fintype.equivFinOfCardEq (Nat.add_sub_of_le
    Fintype.card_pos).symm) (.comp ?_ <| Equiv.bijective _) _ _ fun i => ?_).symm
  · exact (⟨{·}ᶜ, by
      rw [card_compl]; rw [Fintype.card_fin]; rw [Finset.card_singleton]; rw [Nat.add_sub_cancel_left]⟩)
  · use fun _ _ => (singleton_injective <| compl_injective <| Subtype.ext_iff.mp ·)
    intro ⟨s, hs⟩
    have h : #sᶜ = 1 := by rw [card_compl, hs, Fintype.card_fin, Nat.add_sub_cancel]
    obtain ⟨a, ha⟩ := card_eq_one.mp h
    exact ⟨a, Subtype.ext (compl_eq_comm.mp ha)⟩
  rw [Function.comp_apply]; rw [Subtype.coe_mk]; rw [compl_singleton]; rw [piecewise_erase_univ]; rw [toFormalMultilinearSeries]; rw [dif_pos (Nat.add_sub_of_le Fintype.card_pos).symm]
  simp_rw [domDomCongr_apply, compContinuousLinearMap_apply, ContinuousLinearMap.proj_apply,
    Function.update_apply, (Equiv.injective _).eq_iff, ite_apply]
  congr
  grind

/--
theorem `hasStrictFDerivAt` / 定理 `hasStrictFDerivAt`

English:
theorem hasStrictFDerivAt
  given: [DecidableEq ι]
  statement: HasStrictFDerivAt f (f.linearDeriv x) x
  proof: by
  rw [← changeOrigin_toFormalMultilinearSeries]
  convert! f.hasFiniteFPowerSeriesOnBall.hasStrictFDerivAt (y := x) ENNReal.coe_lt_top
  rw [zero_add]

中文:
定理 hasStrictFDerivAt
  条件: [DecidableEq ι]
  结论: HasStrictFDerivAt f (f.linearDeriv x) x
  证明: by
  rw [← changeOrigin_toFormalMultilinearSeries]
  convert! f.hasFiniteFPowerSeriesOnBall.hasStrictFDerivAt (y := x) ENNReal.coe_lt_top
  rw [zero_add]
-/
protected theorem hasStrictFDerivAt [DecidableEq ι] : HasStrictFDerivAt f (f.linearDeriv x) x := by
  rw [← changeOrigin_toFormalMultilinearSeries]
  convert! f.hasFiniteFPowerSeriesOnBall.hasStrictFDerivAt (y := x) ENNReal.coe_lt_top
  rw [zero_add]

/--
theorem `hasFDerivAt` / 定理 `hasFDerivAt`

English:
theorem hasFDerivAt
  given: [DecidableEq ι]
  statement: HasFDerivAt f (f.linearDeriv x) x
  proof: (f.hasStrictFDerivAt _).hasFDerivAt

中文:
定理 hasFDerivAt
  条件: [DecidableEq ι]
  结论: 在点处Fréchet可导 f (f.linearDeriv x) x
  证明: (f.hasStrictFDerivAt _).hasFDerivAt
-/
protected theorem hasFDerivAt [DecidableEq ι] : HasFDerivAt f (f.linearDeriv x) x :=
  (f.hasStrictFDerivAt _).hasFDerivAt

/--
theorem `hasStrictFDerivAt_uncurry` / 定理 `hasStrictFDerivAt_uncurry`

English:
theorem hasStrictFDerivAt_uncurry
  statement: [DecidableEq ι]
  proof: by
  let f := ContinuousLinearMap.id 𝕜 (ContinuousMultilinearMap 𝕜 E F)
.continuousMultilinearMapOption
  have Hf := (f.hasStrictFDerivAt (fun _ => fa)).comp (f := fun fx _ => fx) fa
    (hasStrictFDerivAt_pi.2 fun _ => hasStrictFDerivAt_id _)
  convert! Hf using 1
  ext g
  · suffices ∑ i, fa.1 (Fu

中文:
定理 hasStrictFDerivAt_uncurry
  结论: [DecidableEq ι]
  证明: by
  let f := ContinuousLinearMap.id 𝕜 (ContinuousMultilinearMap 𝕜 E F)
.continuousMultilinearMapOption
  have Hf := (f.hasStrictFDerivAt (fun _ => fa)).comp (f := fun fx _ => fx) fa
    (hasStrictFDerivAt_pi.2 fun _ => hasStrictFDerivAt_id _)
  convert! Hf using 1
  ext g
  · suffices ∑ i, fa.1 (Fu
-/
protected theorem hasStrictFDerivAt_uncurry [DecidableEq ι]
    (fa : ContinuousMultilinearMap 𝕜 E F × forall i, E i) :
    HasStrictFDerivAt (fun fx : ContinuousMultilinearMap 𝕜 E F × forall i, E i => fx.1 fx.2)
      (apply 𝕜 E F fa.2 ∘L .fst _ _ _ + fa.1.linearDeriv fa.2 ∘L .snd _ _ _) fa := by
  let f := ContinuousLinearMap.id 𝕜 (ContinuousMultilinearMap 𝕜 E F)
.continuousMultilinearMapOption
  have Hf := (f.hasStrictFDerivAt (fun _ => fa)).comp (f := fun fx _ => fx) fa
    (hasStrictFDerivAt_pi.2 fun _ => hasStrictFDerivAt_id _)
  convert! Hf using 1
  ext g
  · suffices ∑ i, fa.1 (Function.update fa.2 i 0) =
        ∑ i, fa.1 fun j => (Function.update (fun _ => fa) (some i) (g, 0) (some j)).2 j by
      simpa [f, ContinuousLinearMap.continuousMultilinearMapOption]
    congr with i
    congr with j
    rcases eq_or_ne j i with rfl | hij <;> simp [*]
  · suffices ∑ i, fa.1 (Function.update fa.2 i (g i)) =
        ∑ x, fa.1 fun i => (Function.update (fun x => fa) (some x) (0, g) (some i)).2 i by
      simpa [f, ContinuousLinearMap.continuousMultilinearMapOption]
    congr with i
    congr with j
    rcases eq_or_ne j i with rfl | hij <;> simp [*]

/--
theorem `_root_.HasStrictFDerivAt.continuousMultilinearMap_apply` / 定理 `_root_.HasStrictFDerivAt.continuousMultilinearMap_apply`

English:
theorem _root_.HasStrictFDerivAt.continuousMultilinearMap_apply
  statement: {G : Type*}
  proof: by
  convert!
.comp x ContinuousMultilinearMap.hasStrictFDerivAt_uncurry (f x, (g · x))
      (hf.prodMk (hasStrictFDerivAt_pi.2 hg))
  ext
  simp

中文:
定理 _root_.HasStrictFDerivAt.continuousMultilinearMap_apply
  结论: {G : 类型}
  证明: by
  convert!
.comp x ContinuousMultilinearMap.hasStrictFDerivAt_uncurry (f x, (g · x))
      (hf.prodMk (hasStrictFDerivAt_pi.2 hg))
  ext
  simp

Depends on / 依赖: ContinuousMultilinearMap, ContinuousMultilinearMap.hasStrictFDerivAt_uncurry, convert, hasStrictFDerivAt_pi, hasStrictFDerivAt_uncurry, hf.prodMk, prodMk
-/
theorem _root_.HasStrictFDerivAt.continuousMultilinearMap_apply {G : Type*}
    [NormedAddCommGroup G] [NormedSpace 𝕜 G] [DecidableEq ι] {x : G}
    {f : G -> ContinuousMultilinearMap 𝕜 E F} {g : forall i, G -> E i}
    {f' : G ->L[𝕜] ContinuousMultilinearMap 𝕜 E F} {g' : forall i, G ->L[𝕜] E i}
    (hf : HasStrictFDerivAt f f' x) (hg : forall i, HasStrictFDerivAt (g i) (g' i) x) :
    HasStrictFDerivAt (fun x => f x (g · x))
      (ContinuousMultilinearMap.apply 𝕜 E F (g · x) ∘L f' +
        ∑ i, (f x).toContinuousLinearMap (g · x) i ∘L g' i) x := by
  convert!
.comp x ContinuousMultilinearMap.hasStrictFDerivAt_uncurry (f x, (g · x))
      (hf.prodMk (hasStrictFDerivAt_pi.2 hg))
  ext
  simp

/--
theorem `_root_.HasFDerivWithinAt.continuousMultilinearMap_apply` / 定理 `_root_.HasFDerivWithinAt.continuousMultilinearMap_apply`

English:
theorem _root_.HasFDerivWithinAt.continuousMultilinearMap_apply
  statement: {G : Type*}
  proof: by
  convert!
    ContinuousMultilinearMap.hasStrictFDerivAt_uncurry
.hasFDerivAt.comp_hasFDerivWithinAt (f x, (g · x))
      x (hf.prodMk (hasFDerivWithinAt_pi.2 hg))
  ext
  simp

中文:
定理 _root_.HasFDerivWithinAt.continuousMultilinearMap_apply
  结论: {G : 类型}
  证明: by
  convert!
    ContinuousMultilinearMap.hasStrictFDerivAt_uncurry
.hasFDerivAt.comp_hasFDerivWithinAt (f x, (g · x))
      x (hf.prodMk (hasFDerivWithinAt_pi.2 hg))
  ext
  simp

Depends on / 依赖: ContinuousMultilinearMap, ContinuousMultilinearMap.hasStrictFDerivAt_uncurry, comp_hasFDerivWithinAt, convert, hasFDerivAt, hasFDerivAt.comp_hasFDerivWithinAt, hasFDerivWithinAt_pi, hasStrictFDerivAt_uncurry, hf.prodMk, prodMk
-/
theorem _root_.HasFDerivWithinAt.continuousMultilinearMap_apply {G : Type*}
    [NormedAddCommGroup G] [NormedSpace 𝕜 G] [DecidableEq ι] {s : Set G} {x : G}
    {f : G -> ContinuousMultilinearMap 𝕜 E F} {g : forall i, G -> E i}
    {f' : G ->L[𝕜] ContinuousMultilinearMap 𝕜 E F} {g' : forall i, G ->L[𝕜] E i}
    (hf : HasFDerivWithinAt f f' s x) (hg : forall i, HasFDerivWithinAt (g i) (g' i) s x) :
    HasFDerivWithinAt (fun x => f x (g · x))
      (ContinuousMultilinearMap.apply 𝕜 E F (g · x) ∘L f' +
        ∑ i, (f x).toContinuousLinearMap (g · x) i ∘L g' i) s x := by
  convert!
    ContinuousMultilinearMap.hasStrictFDerivAt_uncurry
.hasFDerivAt.comp_hasFDerivWithinAt (f x, (g · x))
      x (hf.prodMk (hasFDerivWithinAt_pi.2 hg))
  ext
  simp

/--
theorem `_root_.HasFDerivAt.continuousMultilinearMap_apply` / 定理 `_root_.HasFDerivAt.continuousMultilinearMap_apply`

English:
theorem _root_.HasFDerivAt.continuousMultilinearMap_apply
  statement: {G : Type*}
  proof: by
  simp only [← hasFDerivWithinAt_univ] at *
  exact hf.continuousMultilinearMap_apply hg

中文:
定理 _root_.在点处Fréchet可导.continuousMultilinearMap_apply
  结论: {G : 类型}
  证明: by
  simp only [← hasFDerivWithinAt_univ] at *
  exact hf.continuousMultilinearMap_apply hg

Depends on / 依赖: continuousMultilinearMap_apply, hasFDerivWithinAt_univ, hf.continuousMultilinearMap_apply
-/
theorem _root_.HasFDerivAt.continuousMultilinearMap_apply {G : Type*}
    [NormedAddCommGroup G] [NormedSpace 𝕜 G] [DecidableEq ι] {x : G}
    {f : G -> ContinuousMultilinearMap 𝕜 E F} {g : forall i, G -> E i}
    {f' : G ->L[𝕜] ContinuousMultilinearMap 𝕜 E F} {g' : forall i, G ->L[𝕜] E i}
    (hf : HasFDerivAt f f' x) (hg : forall i, HasFDerivAt (g i) (g' i) x) :
    HasFDerivAt (fun x => f x (g · x))
      (ContinuousMultilinearMap.apply 𝕜 E F (g · x) ∘L f' +
        ∑ i, (f x).toContinuousLinearMap (g · x) i ∘L g' i) x := by
  simp only [← hasFDerivWithinAt_univ] at *
  exact hf.continuousMultilinearMap_apply hg

/--
theorem `_root_.HasFDerivWithinAt.multilinear_comp` / 定理 `_root_.HasFDerivWithinAt.multilinear_comp`

English:
theorem _root_.HasFDerivWithinAt.multilinear_comp
  proof: by
  simpa using (hasFDerivWithinAt_const f x s).continuousMultilinearMap_apply hg

中文:
定理 _root_.HasFDerivWithinAt.multilinear_comp
  证明: by
  simpa using (hasFDerivWithinAt_const f x s).continuousMultilinearMap_apply hg

Depends on / 依赖: continuousMultilinearMap_apply, hasFDerivWithinAt_const
-/
theorem _root_.HasFDerivWithinAt.multilinear_comp
    [DecidableEq ι] {G : Type*} [NormedAddCommGroup G] [NormedSpace 𝕜 G]
    {g : forall i, G -> E i} {g' : forall i, G ->L[𝕜] E i} {s : Set G} {x : G}
    (hg : forall i, HasFDerivWithinAt (g i) (g' i) s x) :
    HasFDerivWithinAt (fun x => f (fun i => g i x))
      ((∑ i : ι, (f.toContinuousLinearMap (fun j => g j x) i) ∘L (g' i))) s x := by
  simpa using (hasFDerivWithinAt_const f x s).continuousMultilinearMap_apply hg

/--
theorem `_root_.HasFDerivAt.multilinear_comp` / 定理 `_root_.HasFDerivAt.multilinear_comp`

English:
theorem _root_.HasFDerivAt.multilinear_comp
  proof: by
  simpa using (hasFDerivAt_const f x).continuousMultilinearMap_apply hg

中文:
定理 _root_.在点处Fréchet可导.multilinear_comp
  证明: by
  simpa using (hasFDerivAt_const f x).continuousMultilinearMap_apply hg

Depends on / 依赖: continuousMultilinearMap_apply, hasFDerivAt_const
-/
theorem _root_.HasFDerivAt.multilinear_comp
    [DecidableEq ι] {G : Type*} [NormedAddCommGroup G] [NormedSpace 𝕜 G]
    {g : forall i, G -> E i} {g' : forall i, G ->L[𝕜] E i} {x : G}
    (hg : forall i, HasFDerivAt (g i) (g' i) x) :
    HasFDerivAt (fun x => f (fun i => g i x))
      ((∑ i : ι, (f.toContinuousLinearMap (fun j => g j x) i) ∘L (g' i))) x := by
  simpa using (hasFDerivAt_const f x).continuousMultilinearMap_apply hg

/--
lemma `_root_.Equiv.succ_embeddingFinSucc_fst_symm_apply` / 引理 `_root_.Equiv.succ_embeddingFinSucc_fst_symm_apply`

English:
lemma _root_.Equiv.succ_embeddingFinSucc_fst_symm_apply
  statement: {ι : Type*} [DecidableEq ι]
  proof: by
  rcases hk with ⟨j, rfl⟩
  have hj : j != 0 := by
    rintro rfl
    simp at h'k
  simp only [Function.Embedding.toEquivRange_symm_apply_self]
  have : e j = (Equiv.embeddingFinSucc n ι e).1 (Fin.pred j hj) := by simp
  simp_rw [this]
  simp [-Equiv.embeddingFinSucc_fst]

中文:
引理 _root_.等价.succ_embeddingFinSucc_fst_symm_apply
  结论: {ι : 类型} [DecidableEq ι]
  证明: by
  rcases hk with ⟨j, rfl⟩
  have hj : j != 0 := by
    rintro rfl
    simp at h'k
  simp only [Function.Embedding.toEquivRange_symm_apply_self]
  have : e j = (Equiv.embeddingFinSucc n ι e).1 (Fin.pred j hj) := by simp
  simp_rw [this]
  simp [-Equiv.embeddingFinSucc_fst]
-/
private lemma _root_.Equiv.succ_embeddingFinSucc_fst_symm_apply {ι : Type*} [DecidableEq ι]
    {n : Nat} (e : Fin (n + 1) ↪ ι) {k : ι}
    (h'k : k in Set.range (Equiv.embeddingFinSucc n ι e).1) (hk : k in Set.range e) :
    Fin.succ ((Equiv.embeddingFinSucc n ι e).1.toEquivRange.symm ⟨k, h'k⟩)
      = e.toEquivRange.symm ⟨k, hk⟩ := by
  rcases hk with ⟨j, rfl⟩
  have hj : j != 0 := by
    rintro rfl
    simp at h'k
  simp only [Function.Embedding.toEquivRange_symm_apply_self]
  have : e j = (Equiv.embeddingFinSucc n ι e).1 (Fin.pred j hj) := by simp
  simp_rw [this]
  simp [-Equiv.embeddingFinSucc_fst]

set_option backward.isDefEq.respectTransparency false in
/--
theorem `hasFTaylorSeriesUpTo_iteratedFDeriv` / 定理 `hasFTaylorSeriesUpTo_iteratedFDeriv`

English:
theorem hasFTaylorSeriesUpTo_iteratedFDeriv
  proof: by
  classical
  constructor
  · simp [ContinuousMultilinearMap.iteratedFDeriv]
  · rintro n - x
    suffices H : curryLeft (f.iteratedFDeriv (Nat.succ n) x) = (∑ e : Fin n ↪ ι,
          ((iteratedFDerivComponent f e.toEquivRange).linearDeriv
            (Pi.compRightL 𝕜 _ Subtype.val x)) ∘L (Pi.co

中文:
定理 hasFTaylorSeriesUpTo_iteratedFDeriv
  证明: by
  classical
  constructor
  · simp [ContinuousMultilinearMap.iteratedFDeriv]
  · rintro n - x
    suffices H : curryLeft (f.iteratedFDeriv (Nat.succ n) x) = (∑ e : Fin n ↪ ι,
          ((iteratedFDerivComponent f e.toEquivRange).linearDeriv
            (Pi.compRightL 𝕜 _ Subtype.val x)) ∘L (Pi.co

Depends on / 依赖: ContinuousMultilinearMap, ContinuousMultilinearMap.iteratedFDeriv, HasFDerivAt, Nat.succ, Pi.compRightL, Subtype, Subtype.val, classical, compRightL, curryLeft, e.toEquivRange, f.iteratedFDeriv, iteratedFDeriv, iteratedFDerivComponent, linearDeriv, toEquivRange
-/
theorem hasFTaylorSeriesUpTo_iteratedFDeriv :
    HasFTaylorSeriesUpTo ⊤ f (fun v n => f.iteratedFDeriv n v) := by
  classical
  constructor
  · simp [ContinuousMultilinearMap.iteratedFDeriv]
  · rintro n - x
    suffices H : curryLeft (f.iteratedFDeriv (Nat.succ n) x) = (∑ e : Fin n ↪ ι,
          ((iteratedFDerivComponent f e.toEquivRange).linearDeriv
            (Pi.compRightL 𝕜 _ Subtype.val x)) ∘L (Pi.compRightL 𝕜 _ Subtype.val)) by
      have A : HasFDerivAt (f.iteratedFDeriv n) (∑ e : Fin n ↪ ι,
          ((iteratedFDerivComponent f e.toEquivRange).linearDeriv (Pi.compRightL 𝕜 _ Subtype.val x))
            ∘L (Pi.compRightL 𝕜 _ Subtype.val)) x := by
        apply HasFDerivAt.fun_sum (fun s _hs => ?_)
        exact (ContinuousMultilinearMap.hasFDerivAt _ _).comp x (ContinuousLinearMap.hasFDerivAt _)
      rwa [← H] at A
    ext v m
    simp only [ContinuousMultilinearMap.iteratedFDeriv, curryLeft_apply, sum_apply,
      iteratedFDerivComponent_apply, Finset.univ_sigma_univ,
      Pi.compRightL_apply, _root_.sum_apply, ContinuousLinearMap.comp_apply, linearDeriv_apply,
      Finset.sum_sigma']
    rw [← (Equiv.embeddingFinSucc n ι).sum_comp]
    congr with e
    congr with k
    by_cases hke : k in Set.range e
    · simp only [hke, ↓reduceDIte]
      split_ifs with hkf
      · simp only [← Equiv.succ_embeddingFinSucc_fst_symm_apply e hkf hke, Fin.cons_succ]
      · obtain rfl : k = e 0 := by
          rcases hke with ⟨j, rfl⟩
          simpa using hkf
        simp only [Function.Embedding.toEquivRange_symm_apply_self, Fin.cons_zero, Function.update,
          Pi.compRightL_apply]
        split_ifs with h
        · congr!
        · exfalso
          apply h
          simp_rw [← Equiv.embeddingFinSucc_snd e]
    · have hkf : k ∉ Set.range (Equiv.embeddingFinSucc n ι e).1 := by
        contrapose hke
        rw [Equiv.embeddingFinSucc_fst] at hke
        exact Set.range_comp_subset_range _ _ hke
      simp only [hke, hkf, ↓reduceDIte, Pi.compRightL,
        ContinuousLinearMap.coe_mk', LinearMap.coe_mk, AddHom.coe_mk]
      rw [Function.update_of_ne]
      contrapose hke
      rw [show k = _ from Subtype.ext_iff.1 hke]; rw [Equiv.embeddingFinSucc_snd e]
      exact Set.mem_range_self _
  · rintro n -
    apply continuous_finsetSum _ (fun e _ => ?_)
    exact (ContinuousMultilinearMap.coe_continuous _).comp (ContinuousLinearMap.continuous _)

/--
theorem `iteratedFDeriv_eq` / 定理 `iteratedFDeriv_eq`

English:
theorem iteratedFDeriv_eq
  given: (n : Nat)
  proof: funext fun x => (f.hasFTaylorSeriesUpTo_iteratedFDeriv.eq_iteratedFDeriv (m := n) le_top x).symm

中文:
定理 iteratedFDeriv_eq
  条件: (n : 自然数)
  证明: funext fun x => (f.hasFTaylorSeriesUpTo_iteratedFDeriv.eq_iteratedFDeriv (m := n) le_top x).symm

Depends on / 依赖: eq_iteratedFDeriv, f.hasFTaylorSeriesUpTo_iteratedFDeriv.eq_iteratedFDeriv, hasFTaylorSeriesUpTo_iteratedFDeriv, le_top
-/
theorem iteratedFDeriv_eq (n : Nat) :
    iteratedFDeriv 𝕜 n f = f.iteratedFDeriv n :=
  funext fun x => (f.hasFTaylorSeriesUpTo_iteratedFDeriv.eq_iteratedFDeriv (m := n) le_top x).symm

/--
theorem `norm_iteratedFDeriv_le` / 定理 `norm_iteratedFDeriv_le`

English:
theorem norm_iteratedFDeriv_le
  given: (n : Nat) (x : (i : ι) -> E i)
  proof: by
  rw [f.iteratedFDeriv_eq]
  exact f.norm_iteratedFDeriv_le' n x

中文:
定理 norm_iteratedFDeriv_le
  条件: (n : 自然数) (x : (i : ι) -> E i)
  证明: by
  rw [f.iteratedFDeriv_eq]
  exact f.norm_iteratedFDeriv_le' n x

Depends on / 依赖: f.iteratedFDeriv_eq, f.norm_iteratedFDeriv_le, iteratedFDeriv_eq, norm_iteratedFDeriv_le
-/
theorem norm_iteratedFDeriv_le (n : Nat) (x : (i : ι) -> E i) :
    ‖iteratedFDeriv 𝕜 n f x‖
      <= Nat.descFactorial (Fintype.card ι) n * ‖f‖ * ‖x‖ ^ (Fintype.card ι - n) := by
  rw [f.iteratedFDeriv_eq]
  exact f.norm_iteratedFDeriv_le' n x

end ContinuousMultilinearMap

namespace FormalMultilinearSeries

variable (p : FormalMultilinearSeries 𝕜 E F)

open Fintype ContinuousLinearMap in
/--
theorem `derivSeries_apply_diag` / 定理 `derivSeries_apply_diag`

English:
theorem derivSeries_apply_diag
  given: (n : Nat) (x : E)
  proof: by
  simp only [derivSeries, compFormalMultilinearSeries_apply, changeOriginSeries,
    compContinuousMultilinearMap_coe, ContinuousLinearEquiv.coe_coe, LinearIsometryEquiv.coe_coe,
    Function.comp_apply, map_sum, _root_.sum_apply, continuousMultilinearCurryFin1_apply,
    Matrix.zero_empty]
  con

中文:
定理 derivSeries_apply_diag
  条件: (n : 自然数) (x : E)
  证明: by
  simp only [derivSeries, compFormalMultilinearSeries_apply, changeOriginSeries,
    compContinuousMultilinearMap_coe, ContinuousLinearEquiv.coe_coe, LinearIsometryEquiv.coe_coe,
    Function.comp_apply, map_sum, _root_.sum_apply, continuousMultilinearCurryFin1_apply,
    Matrix.zero_empty]
  con

Depends on / 依赖: ContinuousLinearEquiv, ContinuousLinearEquiv.coe_coe, Fin.snoc_zero, Finset, Finset.card_powersetCa, Finset.piecewise_same, Finset.powersetCard_eq_filter, Finset.powerset_univ, Finset.sum_const, Function, Function.comp_apply, LinearIsometryEquiv, LinearIsometryEquiv.coe_coe, Matrix, Matrix.zero_empty, _root_, _root_.sum_apply, add_comm, card_powersetCa, card_subtype
-/
theorem derivSeries_apply_diag (n : Nat) (x : E) :
    derivSeries p n (fun _ => x) x = (n + 1) • p (n + 1) fun _ => x := by
  simp only [derivSeries, compFormalMultilinearSeries_apply, changeOriginSeries,
    compContinuousMultilinearMap_coe, ContinuousLinearEquiv.coe_coe, LinearIsometryEquiv.coe_coe,
    Function.comp_apply, map_sum, _root_.sum_apply, continuousMultilinearCurryFin1_apply,
    Matrix.zero_empty]
  convert! Finset.sum_const _
  · rw [Fin.snoc_zero, changeOriginSeriesTerm_apply, Finset.piecewise_same, add_comm]
  · rw [← card, card_subtype, ← Finset.powerset_univ, ← Finset.powersetCard_eq_filter,
      Finset.card_powersetCard, ← card, card_fin, eq_comm, add_comm, Nat.choose_succ_self_right]

@[simp]
/--
lemma `derivSeries_coeff_one` / 引理 `derivSeries_coeff_one`

English:
lemma derivSeries_coeff_one
  given: (p : FormalMultilinearSeries 𝕜 𝕜 F) (n : Nat)
  proof: p.derivSeries_apply_diag _ _

中文:
引理 derivSeries_coeff_one
  条件: (p : FormalMultilinearSeries 𝕜 𝕜 F) (n : 自然数)
  证明: p.derivSeries_apply_diag _ _

Depends on / 依赖: derivSeries_apply_diag, p.derivSeries_apply_diag
-/
lemma derivSeries_coeff_one (p : FormalMultilinearSeries 𝕜 𝕜 F) (n : Nat) :
    p.derivSeries.coeff n 1 = (n + 1) • p.coeff (n + 1) :=
  p.derivSeries_apply_diag _ _

end FormalMultilinearSeries

namespace HasFPowerSeriesOnBall

open FormalMultilinearSeries ENNReal Nat

variable {p : FormalMultilinearSeries 𝕜 E F} {f : E -> F} {x : E} {r : Real>=0∞}
  (h : HasFPowerSeriesOnBall f p x r) (y : E)

include h in
/--
theorem `iteratedFDeriv_zero_apply_diag` / 定理 `iteratedFDeriv_zero_apply_diag`

English:
theorem iteratedFDeriv_zero_apply_diag
  statement: iteratedFDeriv 𝕜 0 f x = p 0
  proof: by
  ext
  simpa using (coeff_zero h _).symm

中文:
定理 iteratedFDeriv_zero_apply_diag
  结论: iteratedFDeriv 𝕜 0 f x = p 0
  证明: by
  ext
  simpa using (coeff_zero h _).symm

Depends on / 依赖: coeff_zero
-/
theorem iteratedFDeriv_zero_apply_diag : iteratedFDeriv 𝕜 0 f x = p 0 := by
  ext
  simpa using (coeff_zero h _).symm

open ContinuousLinearMap

/--
theorem `factorial_smul'` / 定理 `factorial_smul'`

English:
theorem factorial_smul'
  given: {n : Nat}
  statement: forall {F : Type max u v} [NormedAddCommGroup F]
  proof: by
  induction n with | zero => _ | succ n ih => _ <;> intro F _ _ _ p f h
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, ih h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl

中文:
定理 factorial_smul'
  条件: {n : 自然数}
  结论: 对任意 {F : 类型 最大值 u v} [赋范交换加群 F]
  证明: by
  induction n with | zero => _ | succ n ih => _ <;> intro F _ _ _ p f h
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, ih h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl
-/
private theorem factorial_smul' {n : Nat} : forall {F : Type max u v} [NormedAddCommGroup F]
    [NormedSpace 𝕜 F] [CompleteSpace F] {p : FormalMultilinearSeries 𝕜 E F}
    {f : E -> F}, HasFPowerSeriesOnBall f p x r ->
    n ! • p n (fun _ => y) = iteratedFDeriv 𝕜 n f x (fun _ => y) := by
  induction n with | zero => _ | succ n ih => _ <;> intro F _ _ _ p f h
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, ih h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl

variable [CompleteSpace F]
include h

/--
theorem `factorial_smul` / 定理 `factorial_smul`

English:
theorem factorial_smul
  given: (n : Nat)
  proof: by
  cases n
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, factorial_smul' _ h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl

中文:
定理 factorial_smul
  条件: (n : 自然数)
  证明: by
  cases n
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, factorial_smul' _ h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl

Depends on / 依赖: _root_, _root_.smul_apply, derivSeries_apply_diag, factorial_smul, factorial_succ, factorial_zero, fderiv, h.fderiv, h.iteratedFDeriv_zero_apply_diag, iteratedFDeriv_succ_apply_right, iteratedFDeriv_zero_apply_diag, mul_comm, mul_smul, one_smul, smul_apply
-/
theorem factorial_smul (n : Nat) :
    n ! • p n (fun _ => y) = iteratedFDeriv 𝕜 n f x (fun _ => y) := by
  cases n
  · rw [factorial_zero, one_smul, h.iteratedFDeriv_zero_apply_diag]
  · rw [factorial_succ, mul_comm, mul_smul, ← derivSeries_apply_diag,
      ← _root_.smul_apply, factorial_smul' _ h.fderiv, iteratedFDeriv_succ_apply_right]
    rfl

/--
theorem `hasSum_iteratedFDeriv` / 定理 `hasSum_iteratedFDeriv`

English:
theorem hasSum_iteratedFDeriv
  given: [CharZero 𝕜] {y : E} (hy : y in Metric.eball 0 r)
  proof: by
  convert! h.hasSum hy with n
  rw [← h.factorial_smul y n]; rw [smul_comm]; rw [← smul_assoc]; rw [nsmul_eq_mul]; rw [mul_inv_cancel₀ cast_ne_zero.mpr n.factorial_ne_zero]; rw [one_smul]

中文:
定理 hasSum_iteratedFDeriv
  条件: [特征零 𝕜] {y : E} (hy : y in Metric.eball 0 r)
  证明: by
  convert! h.hasSum hy with n
  rw [← h.factorial_smul y n]; rw [smul_comm]; rw [← smul_assoc]; rw [nsmul_eq_mul]; rw [mul_inv_cancel₀ cast_ne_zero.mpr n.factorial_ne_zero]; rw [one_smul]

Depends on / 依赖: cast_ne_zero, cast_ne_zero.mpr, convert, factorial_ne_zero, factorial_smul, h.factorial_smul, h.hasSum, hasSum, n.factorial_ne_zero, nsmul_eq_mul, one_smul, smul_assoc, smul_comm
-/
theorem hasSum_iteratedFDeriv [CharZero 𝕜] {y : E} (hy : y in Metric.eball 0 r) :
    HasSum (fun n => (n ! : 𝕜)⁻¹ • iteratedFDeriv 𝕜 n f x fun _ => y) (f (x + y)) := by
  convert! h.hasSum hy with n
  rw [← h.factorial_smul y n]; rw [smul_comm]; rw [← smul_assoc]; rw [nsmul_eq_mul]; rw [mul_inv_cancel₀ cast_ne_zero.mpr n.factorial_ne_zero]; rw [one_smul]

end HasFPowerSeriesOnBall

/-!
### Derivative of a linear map into multilinear maps
-/

namespace ContinuousLinearMap

variable {ι : Type*} {G : ι -> Type*} [forall i, NormedAddCommGroup (G i)] [forall i, NormedSpace 𝕜 (G i)]
  [Fintype ι] {H : Type*} [NormedAddCommGroup H]
  [NormedSpace 𝕜 H]

/--
theorem `hasFDerivAt_uncurry_of_multilinear` / 定理 `hasFDerivAt_uncurry_of_multilinear`

English:
theorem hasFDerivAt_uncurry_of_multilinear
  statement: [DecidableEq ι]
  proof: (f ∘L .fst 𝕜 E (forall i, G i)).hasFDerivAt.continuousMultilinearMap_apply
    (hasFDerivAt_pi'.mp (hasFDerivAt_snd (E := E) (F := forall i, G i)))

中文:
定理 hasFDerivAt_uncurry_of_multilinear
  结论: [DecidableEq ι]
  证明: (f ∘L .fst 𝕜 E (forall i, G i)).hasFDerivAt.continuousMultilinearMap_apply
    (hasFDerivAt_pi'.mp (hasFDerivAt_snd (E := E) (F := forall i, G i)))

Depends on / 依赖: continuousMultilinearMap_apply, hasFDerivAt, hasFDerivAt.continuousMultilinearMap_apply, hasFDerivAt_pi, hasFDerivAt_snd
-/
theorem hasFDerivAt_uncurry_of_multilinear [DecidableEq ι]
    (f : E ->L[𝕜] ContinuousMultilinearMap 𝕜 G F) (v : E × Π i, G i) :
    HasFDerivAt (fun (p : E × Π i, G i) => f p.1 p.2)
      ((f.flipMultilinear v.2) ∘L (.fst _ _ _) +
        ∑ i : ι, ((f v.1).toContinuousLinearMap v.2 i) ∘L (.proj _) ∘L (.snd _ _ _)) v :=
  (f ∘L .fst 𝕜 E (forall i, G i)).hasFDerivAt.continuousMultilinearMap_apply
    (hasFDerivAt_pi'.mp (hasFDerivAt_snd (E := E) (F := forall i, G i)))

/--
theorem `_root_.HasFDerivWithinAt.linear_multilinear_comp` / 定理 `_root_.HasFDerivWithinAt.linear_multilinear_comp`

English:
theorem _root_.HasFDerivWithinAt.linear_multilinear_comp
  proof: (f.hasFDerivAt.comp_hasFDerivWithinAt x ha).continuousMultilinearMap_apply hb

中文:
定理 _root_.HasFDerivWithinAt.linear_multilinear_comp
  证明: (f.hasFDerivAt.comp_hasFDerivWithinAt x ha).continuousMultilinearMap_apply hb

Depends on / 依赖: comp_hasFDerivWithinAt, continuousMultilinearMap_apply, f.hasFDerivAt.comp_hasFDerivWithinAt, hasFDerivAt
-/
theorem _root_.HasFDerivWithinAt.linear_multilinear_comp
    [DecidableEq ι] {a : H -> E} {a' : H ->L[𝕜] E}
    {b : forall i, H -> G i} {b' : forall i, H ->L[𝕜] G i} {s : Set H} {x : H}
    (ha : HasFDerivWithinAt a a' s x) (hb : forall i, HasFDerivWithinAt (b i) (b' i) s x)
    (f : E ->L[𝕜] ContinuousMultilinearMap 𝕜 G F) :
    HasFDerivWithinAt (fun y => f (a y) (fun i => b i y))
      ((f.flipMultilinear (fun i => b i x)) ∘L a' +
        ∑ i, ((f (a x)).toContinuousLinearMap (fun j => b j x) i) ∘L (b' i)) s x :=
  (f.hasFDerivAt.comp_hasFDerivWithinAt x ha).continuousMultilinearMap_apply hb

/--
theorem `_root_.HasFDerivAt.linear_multilinear_comp` / 定理 `_root_.HasFDerivAt.linear_multilinear_comp`

English:
theorem _root_.HasFDerivAt.linear_multilinear_comp
  statement: [DecidableEq ι] {a : H -> E} {a' : H ->L[𝕜] E}
  proof: (f.hasFDerivAt.comp x ha).continuousMultilinearMap_apply hb

中文:
定理 _root_.在点处Fréchet可导.linear_multilinear_comp
  结论: [DecidableEq ι] {a : H -> E} {a' : H ->L[𝕜] E}
  证明: (f.hasFDerivAt.comp x ha).continuousMultilinearMap_apply hb

Depends on / 依赖: continuousMultilinearMap_apply, f.hasFDerivAt.comp, hasFDerivAt
-/
theorem _root_.HasFDerivAt.linear_multilinear_comp [DecidableEq ι] {a : H -> E} {a' : H ->L[𝕜] E}
    {b : forall i, H -> G i} {b' : forall i, H ->L[𝕜] G i} {x : H}
    (ha : HasFDerivAt a a' x) (hb : forall i, HasFDerivAt (b i) (b' i) x)
    (f : E ->L[𝕜] ContinuousMultilinearMap 𝕜 G F) :
    HasFDerivAt (fun y => f (a y) (fun i => b i y))
      ((f.flipMultilinear (fun i => b i x)) ∘L a' +
        ∑ i, ((f (a x)).toContinuousLinearMap (fun j => b j x) i) ∘L (b' i)) x :=
  (f.hasFDerivAt.comp x ha).continuousMultilinearMap_apply hb

end ContinuousLinearMap
