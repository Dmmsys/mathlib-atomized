...
-/]
/--
lemma `MulAction.properSMul_iff_isCompact_setOfPred_inter_nonempty` / 引理 `MulAction.properSMul_iff_isCompact_setOfPred_inter_nonempty`

English:
lemma MulAction.properSMul_iff_isCompact_setOfPred_inter_nonempty
  given: [ContinuousSMul G X]
  proof: by
...
lemma MulAction.properSMul_iff_isCompact_setOfPred_inter_nonempty [ContinuousSMul G X] :
    ProperSMul G X ↔
    (forall {U V : Set X}, IsCompact U -> IsCompact V -> IsCompact {g : G | (g • U inter V).Nonempty}) := by
  refine ⟨fun h => ProperSMul.isCompact_setOfPred_inter_nonempty, fun h => ⟨?_⟩⟩
  refine isProperMap_iff_isCompact_preimage.mpr ⟨by fun_prop, fun {K} hK => ?_⟩
  -- First reduce to the case `K = U × V`.
  let U := Prod.fst '' K
...
@[to_additive]
/--
theorem `properlyDiscontinuousSMul_iff_properSMul` / 定理 `properlyDiscontinuousSMul_iff_properSMul`

English:
theorem properlyDiscontinuousSMul_iff_properSMul
  given: [DiscreteTopology G] [ContinuousConstSMul G X]
  proof: by
...
Depends on / 依赖: ContinuousSMul, MulAction, MulAction.properSMul_iff_isCompact_setOfPred_inter_nonempty, continuous_const_smul, continuous_prod_of_discrete_left, continuous_prod_of_discrete_left.mpr, isCompact_iff_finite, properSMul_iff_isCompact_setOfPred_inter_nonempty, properlyDiscontinuousSMul_iff
-/
