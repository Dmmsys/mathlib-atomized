...
English:
instance :
  signature: BicategoryLike.Context Monoidal.Context
  body: Monoidal.mkContext?
...
/--
Definition of `MonoidalM` / `MonoidalM` 的定义

English:
abbreviation MonoidalM
...
/--
Definition of `synthMonoidalError` / `synthMonoidalError` 的定义

English:
definition synthMonoidalError
  signature: {α : Type}
  body: do
...
instance : MonadMor₁ MonoidalM where
  ...
  comp₁M f g := do
    ...
    let f_e : Q($ctx.C) := f.e
    let g_e : Q($ctx.C) := g.e
    return .comp (q($f_e otimes $g_e)) f g
...
/--
theorem `StructuralOfExpr_monoidalComp` / 定理 `StructuralOfExpr_monoidalComp`

English:
theorem StructuralOfExpr_monoidalComp
  statement: {f g h i : C} [MonoidalCoherence g h]
  proof: by
...
/--
theorem `structuralIsoOfExpr_horizontalComp` / 定理 `structuralIsoOfExpr_horizontalComp`

English:
theorem structuralIsoOfExpr_horizontalComp
  statement: {f₁ g₁ f₂ g₂ : C}
  proof: by
  simp [ih_η, ih_θ]

中文:
定理 structuralIsoOfExpr_horizontalComp
  结论: {f₁ g₁ f₂ g₂ : C}
  证明: by
  simp [ih_η, ih_θ]
-/
theorem structuralIsoOfExpr_horizontalComp {f₁ g₁ f₂ g₂ : C}
    ...
    (η' otimesᵢ θ').hom = η otimesₘ θ := by
  simp [ih_η, ih_θ]
...
English:
instance :
  signature: MonadMor₂Iso MonoidalM
  body: do
    let ctx ← read
    let some _monoidal := ctx.instMonoidal? | synthMonoidalError
    let f_e : Q($ctx.C) := f.e
  ...
  leftUnitorM f := do
    let ctx ← read
    let some _monoidal := ctx

中文:
实例 :
  签名: MonadMor₂Iso MonoidalM
  定义体: do
    let ctx ← read
    let some _monoidal := ctx.instMonoidal? | synthMonoidalError
    let f_e : Q($ctx.C) := f.e
...
return .id e ⟨.structuralAtom ← id₂M f, eq⟩ f
  ...
  whiskerLeftM f η := do
    ...
    let iso_lift? ← (match η.isoLift? with
      ...
      | _ => return none)
    let e : Q($f_e otimes $g_e ⟶ $f_e otimes $h_e) := q($f_e ◁ $η_e)
  ...
  whiskerRightM η h := do
    ...
    let iso_lift? ← (match η.isoLift? with
      ...
      | _ => return none)
    let e : Q($f_e otimes $h_e ⟶ $g_e otimes $h_e) := q($η_e ▷ $h_e)
  ...
  horizontalCompM η θ := do
    ...
    let iso_lift? ← (match (η.isoLift?, θ.isoLift?) with
      ...
      | _ => return none)
    let e : Q($f₁_e otimes $f₂_e ⟶ $g₁_e otimes $g₂_e) := q($η_e otimesₘ $θ_e)
...
definition comp?
  ...
  match ctx.instMonoidal? with
    | some _monoidal => do
if ← withDefault isDefEq e q($f otimes $g) then
        let f ← instantiateMVars f
        let g ← instantiateMVars g
        return some ((.of ⟨f, ⟨none⟩, 
...
English:
instance :
  signature: BicategoryLike.MkMor₂ MonoidalM
  body: Mor₂OfExpr

中文:
实例 :
  签名: BicategoryLike.MkMor₂ MonoidalM
  定义体: Mor₂OfExpr
-/
...
English:
instance :
  signature: MonadCoherehnceHom MonoidalM
