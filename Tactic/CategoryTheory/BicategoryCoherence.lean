...
/-
**Mathlib.Tactic.BicategoryCoherence.LiftHom** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathli
b.Tactic.BicategoryCoherence`。
...
--- 原说明 ---
A typeclass carrying a choice of lift of a 1-morphism from `B` to `FreeBicategor
y B`.
-/
...
/-
**Mathlib.Tactic.BicategoryCoherence.liftHomId** 是 Mathlib 中的一个实例，位于命名空间 `Mathli
b.Tactic.BicategoryCoherence`。
形式化陈述：liftHomId : LiftHom (𝟙 a) where lift
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance liftHomId : LiftHom (𝟙 a) where lift := 𝟙 (of.obj a)
/-
**Mathlib.Tactic.BicategoryCoherence.liftHomComp** 是 Mathlib 中的一个实例，位于命名空间 `Math
lib.Tactic.BicategoryCoherence`。
形式化陈述：liftHomComp (f : a ⟶ b) (g : b ⟶ c) [LiftHom f] [LiftHom g] : LiftHom (f ≫
 g) where lift
参数：f : a ⟶ b；g : b ⟶ c。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.BicategoryCoherence.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Tactic.
BicategoryCoherence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
--- 原说明 ---
A typeclass carrying a choice of lift of a 2-morphism from `B` to `FreeBicategor
y B`.
-/
class LiftHom₂ {f g : a ⟶ b} [LiftHom f] [LiftHom g] (η : f ⟶ g) where
  ...
  lift : LiftHom.lift f ⟶ LiftHom.lift g
/-
**Mathlib.Tactic.BicategoryCoherence.liftHom** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.
Tactic.BicategoryCoherence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Tactic.BicategoryCoherence.assoc_liftHom** 是 Mathlib 中的一个定理，位于命名空间 `Ma
thlib.Tactic.BicategoryCoherence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
