...
/--
theorem `isLittleO_sub_sub_fderiv` / 定理 `isLittleO_sub_sub_fderiv`
...
theorem isLittleO_sub_sub_fderiv
    ...
    [NormedSpace Real E] [NormedSpace 𝕜 E] [NormedAddCommGroup F] [NormedSpace 𝕜 F]
    {u : E} {v w : α -> E} {l : Filter α} (hv : Tendsto v l (𝓝 u)) (hw : Tendsto w l (𝓝 u))
    (s : Set E := Set.univ) (seg : forallᶠ χ in l, [w χ -[Real] v χ] subseteq s := by simp)
    {f : α -> E -> F} {f' : α -> E -> E ->L[𝕜] F}
    (df' : forallᶠ p in l ×ˢ 𝓝[s] u, HasFDerivWithinAt (f p.1) (f' p.1 p.2) s p.2)
...
public theorem hasStrictFDerivAt_uncurry_coprod
    ...
    {f₂ : E₁ -> E₂ -> E₂ ->L[𝕜] F} (df₁ : forallᶠ v in 𝓝 u, HasFDerivAt (f · v.2) (↿f₁ v) v.1)
    (df₂ : forallᶠ v in 𝓝 u, HasFDerivAt (f v.1 ·) (↿f₂ v) v.2) (cf₁ : ContinuousAt ↿f₁ u)
...
have h := tendsto_snd.prodMk tendsto_snd.comp tendsto_snd.comp
            ...
            let : NormedSpace Real E₁ := RestrictScalars.normedSpace Real 𝕜 E₁
