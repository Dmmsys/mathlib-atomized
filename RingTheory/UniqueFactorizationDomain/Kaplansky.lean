...
/-
**UniqueFactorizationMonoid.kaplanskySet** 是 Mathlib 中的一个定义，位于命名空间 `UniqueFactor
izationMonoid`。
形式化陈述：kaplanskySet [Semiring R] (S : Subsemigroup R)
参数：S : Subsemigroup R。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The set of ideals of a semiring `R` that are disjoint from a given subsemigroup 
`S`.
-/
...
/-
**UniqueFactorizationMonoid.mem_kaplanskySet_iff** 是 Mathlib 中的一个定理，位于命名空间 `Uniq
ueFactorizationMonoid`。
形式化陈述：mem_kaplanskySet_iff : P in kaplanskySet S ↔ (P : Set R) inter S = ∅
该定理/引理刻画了左右两侧的等价关系。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**UniqueFactorizationMonoid.exists_mem_kaplanskySet_le** 是 Mathlib 中的一个定理，位于命名空间
 `UniqueFactorizationMonoid`。
形式化陈述：exists_mem_kaplanskySet_le {C : Set (Ideal R)} (hS : 0 ∉ S) (hC : C subset
eq kaplanskySet S) (hC₂ : IsChain (· <= ·) C) : exists P in kaplanskySet S, fora
ll J in C, J <= P
参数：Ideal R；hS : 0 ∉ S；hC : C subseteq kaplanskySet S；hC₂ : IsChain (· <= ·) C。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**UniqueFactorizationMonoid.exists_mem_kaplanskySet_eq_of_le** 是 Mathlib 中的一个定理，
位于命名空间 `UniqueFactorizationMonoid`。
形式化陈述：exists_mem_kaplanskySet_eq_of_le (hS : 0 ∉ S) : exists P in kaplanskySet S
, forall I in kaplanskySet S, P <= I -> I = P
参数：hS : 0 ∉ S。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**UniqueFactorizationMonoid.span_notMem_kaplanskySet** 是 Mathlib 中的一个定理，位于命名空间 `
UniqueFactorizationMonoid`。
形式化陈述：span_notMem_kaplanskySet {a : R} (ha : a != 0) (H : forall I != (⊥ : Ideal
 R), I.IsPrime -> exists x in I, Prime x) : span {a} ∉ kaplanskySet (closure {r 
: R | Prime r}).toSubsemigroup
参数：ha : a != 0；H : forall I != (⊥ : Ideal R), I.IsPrime -> exists x in I, Prime 
x。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
nonzero prime ideal contains a prime element. -/
/-
**UniqueFactorizationMonoid.of_exists_prime_mem_of_isPrime** 是 Mathlib 中的一个定理，位于
命名空间 `UniqueFactorizationMonoid`。
形式化陈述：of_exists_prime_mem_of_isPrime (H : forall I != (⊥ : Ideal R), I.IsPrime -
> exists x in I, Prime x) : UniqueFactorizationMonoid R
参数：H : forall I != (⊥ : Ideal R), I.IsPrime -> exists x in I, Prime x。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The nontrivial implication of Kaplansky's criterion: an integral domain is a UFD
 if every
nonzero prime ideal contains a prime element.
-/
