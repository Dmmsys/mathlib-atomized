...
Depends on / 依赖: Finset, Finset.sum_const_zero, Finset.sum_eq_ite, Fintype, Fintype.sum_sum_type, Matrix, Matrix.fromBlocks_apply, Matrix.mul_apply, Matrix.of_apply, Matrix.sub_apply, Matrix.zero_apply, Ring.lie_def, add_zero, classical, indexNeg_neg, lie_def, mul_apply, mul_ite, of_apply, sub_apply
-/
lemma lie_h_e :
  ...
  · simp only [h, e, indexNeg_neg, Ring.lie_def, Matrix.sub_apply, Matrix.mul_apply,
    ...
    rw [← Finset.sum_sub_distrib]; rw [← Finset.sum_subset (Finset.subset_univ {k]; rw [l}) (by aesop)]
...
Depends on / 依赖: IsZeroOrProbabilityMeasure, IsZeroOrProbabilityMeasure.toIsFiniteMeasure, Measure, Ring.lie_def, b.cartanMatrix, cartanMatrix, classical, congr_arg, lie_def, lie_h_e, mul_a, mul_assoc, mul_sub, replace, toIsFiniteMeasure
-/
lemma lie_h_f :
...
/--
lemma `lie_e_f_same_aux` / 引理 `lie_e_f_same_aux`

English:
lemma lie_e_f_same_aux
  given: (k : ι) (hki : k != i) (hki' : k != P.reflectionPerm i i)
  proof: by
  classical
  have h_lin_ind : LinearIndependent R ![P.root i, P.root k] := by
    rw [LinearIndependent.pair_symm_iff]; rw [IsReduced.linearIndependent_iff]
    refine ⟨hki, ?_⟩
...
中文:
引理 lie_e_f_same_aux
  条件: (k : ι) (hki : k != i) (hki' : k != P.reflectionPerm i i)
  证明: by
  classical
  have h_lin_ind : LinearIndependent R ![P.root i, P.root k] := by
    rw [LinearIndependent.pair_symm_iff]; rw [IsReduced.linearIndependent_iff]
...
private lemma lie_e_f_same_aux (k : ι) (hki : k != i) (hki' : k != P.reflectionPerm i i) :
  ...
  suffices (∑ x, if P.root k = P.root i + P.root x then
    ...
    have aux (x : ι) : P.root x = P.root k - P.root i ↔ P.root k = P.root i + P.root x := by
      rw [eq_sub_iff_add_eq']; rw [eq_comm]
  ...
  · have h_lin_ind_x : LinearIndependent R ![P.root i, P.root x] := by simpa [hx] using h_lin_ind
    have h_lin_ind_y : LinearIndependent R ![P.root i, P.root y] := by
      rw [← add_eq_of_eq_sub hy]; rw [add_comm]; simpa
    ...
    rw [Finset.sum_eq_single_of_mem x (Finset.mem_univ _) (by aesop)]; rw [Finset.sum_eq_single_of_mem y (Finset.mem_univ _) (by aesop)]
...
/--
lemma `lie_e_f_same` / 引理 `lie_e_f_same`
...
Depends on / 依赖: IsAddTorsionFree, IsReflexive, Module, Module.IsReflexive, P.indexNeg, P.ne_zero, P.root, P.toLinearMap, classical, contra, indexNeg, ne_zero, not_and, of_isPerfPair, of_isTorsionFree, sub_eq_add_neg, toLinearMap
-/
lemma lie_e_f_same :
    ⁅e i, f i⁆ = h i := by
  let := P.indexNeg
  ...
  · rcases eq_or_ne k i with rfl | hki
    · have hx (x : ι) : ¬ (P.root x = P.root i + P.root l ∧ P.root i = P.root x - P.root i) := by
        ...
        rwa [eq_sub_iff_add_eq, ← two_smul Nat, eq_comm] at contra
...
/--
lemma `isSl2Triple` / 引理 `isSl2Triple`

English:
lemma isSl2Triple
  given: [DecidableEq ι]
  proof: fun contra => by simpa [h] using congr_fun₂ contra (.inr i) (.inr i)
...
引理 isSl2Triple
  条件: [DecidableEq ι]
  证明: fun contra => by simpa [h] using congr_fun₂ contra (.inr i) (.inr i)
  lie_e_f := by rw [lie_e_f_same]
  lie_h_e_nsmul := by rw [lie_h_e]; simp
  lie_h_f_nsmul := by rw [lie_h_f]; simp

Depends on / 依赖: IsProbabilityMeasure, Measure, contra
-/
...
/--
lemma `lie_e_f_ne_aux₀` / 引理 `lie_e_f_ne_aux₀`

English:
lemma lie_e_f_ne_aux₀
  given: (k : b.support) (l : ι)
  proof: by
  classical
  let := P.indexNeg
  have aux₁ : forall x in Finset.univ, ¬ (P.root x = P.root i + P.root l ∧ k = j ∧ x = j) := by
    rintro x - ⟨hl, -, rfl⟩
    exact b.sub_notMem_range_root i.property j.property ⟨-l, by simp [hl]⟩
  have aux₂ : forall x in Finset.univ, ¬ (P.root x = P.root l - P.
...
private lemma lie_e_f_ne_aux₀ (k : b.support) (l : ι) :
  ...
  have aux₁ : forall x in Finset.univ, ¬ (P.root x = P.root i + P.root l ∧ k = j ∧ x = j) := by
    rintro x - ⟨hl, -, rfl⟩
    exact b.sub_notMem_range_root i.property j.property ⟨-l, by simp [hl]⟩
  have aux₂ : forall x in Finset.univ, ¬ (P.root x = P.root l - P.root j ∧ k = i ∧ x = -i) := by
    rintro x - ⟨hl, -, rfl⟩
...
lemma lie_e_f_ne_aux₁
  ...
  · suffices ((if k = i then ↑|b.cartanMatrix i j| else (0 : R)) -
        ∑ x, if P.root x = P.root i + P.root j ∧ P.root 
...
private lemma lie_e_f_ne_aux₁ :
  ...
  · suffices ((if k = i then ↑|b.cartanMatrix i j| else (0 : R)) -
      ...
      have aux : forall x in Finset.univ,
    ...
    by_cases hij_mem : P.root i + P.root j in range P.root
    · obtain ⟨m, hm⟩ := hij_mem
      rw [Finset.sum_eq_single_of_mem m (Finset.mem_univ _) (by rintro x - hx; simp [← hm]; rw [hx]),
...
Depends on / 依赖: IsProbabilityMeasure, IsProbabilityMeasure.neZero, Measure, neZero
-/
private lemma lie_e_f_ne_aux₂ :
    letI := P.indexNeg
    ⁅e i, f j⁆ᵀ (Sum.inr (-i)) = 0 := by
  ...
  · rw [Matrix.transpose_apply, lie_e_f_ne_aux₀, Pi.zero_apply]
  · have aux : ⁅e i, f j⁆ (.inr k) (.inr (-i)) = (⁅e i, f j⁆ * ω b) (.inr k) (.inr i) := by simp [ω]
    rw [Matrix.transpose_apply]; rw [aux]; rw [lie_e_f_mul_ω]; rw [← (-ω b * ⁅e j]; rw [f i⁆).transpose_apply]; rw [Matrix.transpose_mul]; rw [Matrix.mul_apply']; rw [lie_e_f_ne_aux₁ hij.symm]
    simp

/--
lemma `lie_e_f_ne` / 引理 `lie_e_f_ne`
...
lemma lie_e_f_ne
  ...
  · have aux₁ : P.root k != P.root i - P.root j :=
      fun contra => b.sub_notMem_range_
...
Depends on / 依赖: P.indexNeg, P.root, SetLike, SetLike.coe_eq_coe.mp, auxiliary, b.sub_notMem_range_root, classical, coe_eq_coe, contra, covered, eq_or_ne, i.property, if_neg, indexNeg, indexNeg_neg, j.property, lemmas, lie_e_f_ne_, ne_comm, property
-/
lemma lie_e_f_ne [P.IsReduced] [P.IsIrreducible] :
  ...
  · /- Geck Case 1 (covered by the auxiliary lemmas above). -/
    ...
    suffices
      ...
      have h₁ : forall x in Finset.univ, ¬ ((x = i ∧ l = -i) ∧ k = -j) := by
        rintro - - ⟨⟨-, contra⟩, -⟩; contradiction
      have h₂ : forall x in Finset.univ, ¬ ((x = j ∧ l = j) ∧ k = i) := by
        rintro - - ⟨⟨-, contra⟩, -⟩; contradiction
      rw [← sub_eq_zero] at this
      simpa [e, f, ← ite_and, Finset.sum_ite_of_false h₁, Finset.sum_ite_of_false h₂, -indexNeg_neg,
        -Finset.univ_eq_attach]
    by_cases h₅ : P.root l + P.root i - P.root j in range P.root; swap
    · have aux₃ : forall x in Finset.univ,
    ...
    by_cases h₆ : P.root l + P.root i in range P.root; swap
