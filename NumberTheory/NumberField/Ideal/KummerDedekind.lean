/-
Copyright (c) 2025 Xavier Roblot. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Xavier Roblot
-/
module

public import Mathlib.NumberTheory.KummerDedekind
public import Mathlib.NumberTheory.NumberField.Basic
public import Mathlib.RingTheory.RamificationInertia.Basic
public import Mathlib.RingTheory.Ideal.Int

/-!
# Kummer-Dedekind criterion for the splitting of prime numbers

In this file, we give a specialized version of the Kummer-Dedekind criterion for the case of the
splitting of rational primes in number fields.

## Main definitions

Let `K` be a number field and `θ` an algebraic integer of `K`.

* `RingOfIntegers.exponent`: the smallest positive integer `d` contained in the conductor of `θ`.
  It is the smallest integer such that `d • 𝓞 K ⊆ ℤ[θ]`, see `RingOfIntegers.exponent_eq_sInf`.

* `RingOfIntegers.ZModXQuotSpanEquivQuotSpan`: The isomorphism between `(ℤ / pℤ)[X] / (minpoly θ)`
  and `𝓞 K / p(𝓞 K)` for a prime `p` which doesn't divide the exponent of `θ`.

* `NumberField.Ideal.primesOverSpanEquivMonicFactorsMod`: The bijection between the prime ideals
  of `K` above `p` and the monic irreducible factors of `minpoly ℤ θ` modulo `p` for a prime `p`
  which doesn't divide the exponent of `θ`.

## Main results

* `NumberField.Ideal.primesOverSpanEquivMonicFactorsMod`: The ideal corresponding to the class
  of `Q ∈ ℤ[X]` modulo `p` via `NumberField.Ideal.primesOverSpanEquivMonicFactorsMod` is spanned
  by `p` and `Q(θ)`.

* `NumberField.Ideal.inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply`: The residual degree
  of the ideal corresponding to the class of `Q ∈ ℤ[X]` modulo `p` via
  `NumberField.Ideal.primesOverSpanEquivMonicFactorsMod` is equal to the degree of `Q mod p`.

* `NumberField.Ideal.ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply`: The
  ramification index of the ideal corresponding to the class of `Q ∈ ℤ[X]` modulo `p` via
  `NumberField.Ideal.primesOverSpanEquivMonicFactorsMod` is equal to the multiplicity of `Q mod p`
  in `minpoly ℤ θ`.

-/

@[expose] public section

noncomputable section

open Polynomial NumberField Ideal KummerDedekind UniqueFactorizationMonoid

variable {K : Type*} [Field K]

namespace RingOfIntegers

/--
Definition of `exponent` / `exponent` 的定义

English:
definition exponent
  signature: (θ : 𝓞 K)
  body: absNorm (under Int (conductor Int θ))

中文:
定义 exponent
  签名: (θ : 𝓞 K)
  定义体: absNorm (under Int (conductor Int θ))

Depends on / 依赖: absNorm, conductor
-/
def exponent (θ : 𝓞 K) : Nat := absNorm (under Int (conductor Int θ))

variable {θ : 𝓞 K}

/--
theorem `exponent_eq_one_iff` / 定理 `exponent_eq_one_iff`

English:
theorem exponent_eq_one_iff
  statement: exponent θ = 1 ↔ Algebra.adjoin Int {θ} = ⊤
  proof: by
  rw [exponent]; rw [absNorm_eq_one_iff]; rw [comap_eq_top_iff]; rw [conductor_eq_top_iff_adjoin_eq_top]

中文:
定理 exponent_eq_one_iff
  结论: exponent θ = 1 ↔ Algebra.adjoin 整数 {θ} = ⊤
  证明: by
  rw [exponent]; rw [absNorm_eq_one_iff]; rw [comap_eq_top_iff]; rw [conductor_eq_top_iff_adjoin_eq_top]

Depends on / 依赖: absNorm_eq_one_iff, comap_eq_top_iff, conductor_eq_top_iff_adjoin_eq_top, exponent
-/
theorem exponent_eq_one_iff : exponent θ = 1 ↔ Algebra.adjoin Int {θ} = ⊤ := by
  rw [exponent]; rw [absNorm_eq_one_iff]; rw [comap_eq_top_iff]; rw [conductor_eq_top_iff_adjoin_eq_top]

/--
theorem `not_dvd_exponent_iff` / 定理 `not_dvd_exponent_iff`

English:
theorem not_dvd_exponent_iff
  given: {p : Nat} [Fact (Nat.Prime p)]
  proof: by
  rw [codisjoint_comm]; rw [← IsCoatom.not_le_iff_codisjoint]; rw [← under_def]; rw [← Ideal.dvd_iff_le]; rw [← Int.ideal_span_absNorm_eq_self (under Int (conductor Int θ))]; rw [Ideal.span_singleton_dvd_span_singleton_iff_dvd]; rw [Int.natCast_dvd_natCast]; rw [exponent]
exact isMaximal_def.mp I

中文:
定理 not_dvd_exponent_iff
  条件: {p : 自然数} [Fact (自然数.Prime p)]
  证明: by
  rw [codisjoint_comm]; rw [← IsCoatom.not_le_iff_codisjoint]; rw [← under_def]; rw [← Ideal.dvd_iff_le]; rw [← Int.ideal_span_absNorm_eq_self (under Int (conductor Int θ))]; rw [Ideal.span_singleton_dvd_span_singleton_iff_dvd]; rw [Int.natCast_dvd_natCast]; rw [exponent]
exact isMaximal_def.mp I

Depends on / 依赖: Ideal.dvd_iff_le, Ideal.span_singleton_dvd_span_singleton_iff_dvd, Int.ideal_span_absNorm_eq_self, Int.ideal_span_isMaximal_of_prime, Int.natCast_dvd_natCast, IsCoatom, IsCoatom.not_le_iff_codisjoint, codisjoint_comm, conductor, dvd_iff_le, exponent, ideal_span_absNorm_eq_self, ideal_span_isMaximal_of_prime, isMaximal_def, isMaximal_def.mp, natCast_dvd_natCast, not_le_iff_codisjoint, span_singleton_dvd_span_singleton_iff_dvd, under_def
-/
theorem not_dvd_exponent_iff {p : Nat} [Fact (Nat.Prime p)] :
    ¬ p ∣ exponent θ ↔ Codisjoint (comap (algebraMap Int (𝓞 K)) (conductor Int θ)) (span {↑p}) := by
  rw [codisjoint_comm]; rw [← IsCoatom.not_le_iff_codisjoint]; rw [← under_def]; rw [← Ideal.dvd_iff_le]; rw [← Int.ideal_span_absNorm_eq_self (under Int (conductor Int θ))]; rw [Ideal.span_singleton_dvd_span_singleton_iff_dvd]; rw [Int.natCast_dvd_natCast]; rw [exponent]
exact isMaximal_def.mp Int.ideal_span_isMaximal_of_prime p

/--
theorem `exponent_eq_sInf` / 定理 `exponent_eq_sInf`

English:
theorem exponent_eq_sInf
  statement: exponent θ = sInf {d : Nat | 0 < d ∧ (d : 𝓞 K) in conductor Int θ}
  proof: by
  rw [exponent]; rw [Int.absNorm_under_eq_sInf]

中文:
定理 exponent_eq_sInf
  结论: exponent θ = sInf {d : 自然数 | 0 < d ∧ (d : 𝓞 K) in conductor 整数 θ}
  证明: by
  rw [exponent]; rw [Int.absNorm_under_eq_sInf]

Depends on / 依赖: Int.absNorm_under_eq_sInf, absNorm_under_eq_sInf, exponent
-/
theorem exponent_eq_sInf : exponent θ = sInf {d : Nat | 0 < d ∧ (d : 𝓞 K) in conductor Int θ} := by
  rw [exponent]; rw [Int.absNorm_under_eq_sInf]

variable [NumberField K] {θ : 𝓞 K} {p : Nat} [Fact p.Prime]

/--
Definition of `ZModXQuotSpanEquivQuotSpan` / `ZModXQuotSpanEquivQuotSpan` 的定义

English:
definition ZModXQuotSpanEquivQuotSpan
  signature: (hp : ¬ p ∣ exponent θ)
  body: (quotientEquivAlgOfEq Int (by simp [Ideal.map_span, Polynomial.map_map])).toRingEquiv.trans
    ((quotientEquiv _ _ (mapEquiv (Int.quotientSpanNatEquivZMod p)) rfl).symm.trans
      ((quotMapEquivQuotQuotMap (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral).symm.trans
        (quotientEquivAlgOfEq I

中文:
定义 ZModXQuotSpanEquivQuotSpan
  签名: (hp : ¬ p ∣ exponent θ)
  定义体: (quotientEquivAlgOfEq Int (by simp [Ideal.map_span, Polynomial.map_map])).toRingEquiv.trans
    ((quotientEquiv _ _ (mapEquiv (Int.quotientSpanNatEquivZMod p)) rfl).symm.trans
      ((quotMapEquivQuotQuotMap (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral).symm.trans
        (quotientEquivAlgOfEq I

Depends on / 依赖: Ideal.map_span, Int.quotientSpanNatEquivZMod, Polynomial, Polynomial.map_map, eq_top, isIntegral, mapEquiv, map_map, map_span, not_dvd_exponent_iff, not_dvd_exponent_iff.mp, quotMapEquivQuotQuotMap, quotientEquiv, quotientEquivAlgOfEq, quotientSpanNatEquivZMod, symm.trans, toRingEquiv, toRingEquiv.trans
-/
def ZModXQuotSpanEquivQuotSpan (hp : ¬ p ∣ exponent θ) :
    (ZMod p)[X] ⧸ span {map (Int.castRingHom (ZMod p)) (minpoly Int θ)} ≃+*
      𝓞 K ⧸ span {(p : 𝓞 K)} :=
  (quotientEquivAlgOfEq Int (by simp [Ideal.map_span, Polynomial.map_map])).toRingEquiv.trans
    ((quotientEquiv _ _ (mapEquiv (Int.quotientSpanNatEquivZMod p)) rfl).symm.trans
      ((quotMapEquivQuotQuotMap (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral).symm.trans
        (quotientEquivAlgOfEq Int (by simp [map_span])).toRingEquiv))

/--
theorem `ZModXQuotSpanEquivQuotSpan_mk_apply` / 定理 `ZModXQuotSpanEquivQuotSpan_mk_apply`

English:
theorem ZModXQuotSpanEquivQuotSpan_mk_apply
  given: (hp : ¬ p ∣ exponent θ) (Q : Int[X])
  proof: by
  simp only [ZModXQuotSpanEquivQuotSpan, algebraMap_int_eq,
    RingEquiv.trans_apply, AlgEquiv.coe_ringEquiv, quotientEquivAlgOfEq_mk,
    quotientEquiv_symm_apply, quotientMap_mk, RingHom.coe_coe, mapEquiv_symm_apply,
    Polynomial.map_map, Int.quotientSpanNatEquivZMod_comp_castRingHom]
exact 

中文:
定理 ZModXQuotSpanEquivQuotSpan_mk_apply
  条件: (hp : ¬ p ∣ exponent θ) (Q : 整数[X])
  证明: by
  simp only [ZModXQuotSpanEquivQuotSpan, algebraMap_int_eq,
    RingEquiv.trans_apply, AlgEquiv.coe_ringEquiv, quotientEquivAlgOfEq_mk,
    quotientEquiv_symm_apply, quotientMap_mk, RingHom.coe_coe, mapEquiv_symm_apply,
    Polynomial.map_map, Int.quotientSpanNatEquivZMod_comp_castRingHom]
exact 

Depends on / 依赖: AlgEquiv, AlgEquiv.coe_ringEquiv, Int.quotientSpanNatEquivZMod_comp_castRingHom, Polynomial, Polynomial.map_map, RingEquiv, RingEquiv.trans_apply, RingHom, RingHom.coe_coe, ZModXQuotSpanEquivQuotSpan, algebraMap_int_eq, coe_coe, coe_ringEquiv, congr_arg, eq_top, isIntegral, mapEquiv_symm_apply, map_map, map_span, not_dvd_exponent_iff
-/
theorem ZModXQuotSpanEquivQuotSpan_mk_apply (hp : ¬ p ∣ exponent θ) (Q : Int[X]) :
    (ZModXQuotSpanEquivQuotSpan hp)
      (Ideal.Quotient.mk (span {map (Int.castRingHom (ZMod p)) (minpoly Int θ)})
      (map (Int.castRingHom (ZMod p)) Q)) = Ideal.Quotient.mk (span {(p : 𝓞 K)}) (aeval θ Q) := by
  simp only [ZModXQuotSpanEquivQuotSpan, algebraMap_int_eq,
    RingEquiv.trans_apply, AlgEquiv.coe_ringEquiv, quotientEquivAlgOfEq_mk,
    quotientEquiv_symm_apply, quotientMap_mk, RingHom.coe_coe, mapEquiv_symm_apply,
    Polynomial.map_map, Int.quotientSpanNatEquivZMod_comp_castRingHom]
exact congr_arg (quotientEquivAlgOfEq Int (by simp [map_span]))
    quotMapEquivQuotQuotMap_symm_apply (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral Q

variable (p θ) in
/--
Definition of `monicFactorsMod` / `monicFactorsMod` 的定义

English:
abbreviation monicFactorsMod
  signature: : Finset ((ZMod p)[X])
  body: (normalizedFactors (map (Int.castRingHom (ZMod p)) (minpoly Int θ))).toFinset

中文:
缩写 monicFactorsMod
  签名: : Finset ((ZMod p)[X])
  定义体: (normalizedFactors (map (Int.castRingHom (ZMod p)) (minpoly Int θ))).toFinset

Depends on / 依赖: Int.castRingHom, castRingHom, minpoly, normalizedFactors, toFinset
-/
abbrev monicFactorsMod : Finset ((ZMod p)[X]) :=
  (normalizedFactors (map (Int.castRingHom (ZMod p)) (minpoly Int θ))).toFinset

/--
Definition of `ZModXQuotSpanEquivQuotSpanPair` / `ZModXQuotSpanEquivQuotSpanPair` 的定义

English:
definition ZModXQuotSpanEquivQuotSpanPair
  signature: (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
  body: have h₀ : map (Int.castRingHom (ZMod p)) (minpoly Int θ) != 0 :=
      map_monic_ne_zero (minpoly.monic θ.isIntegral)
  have h_eq₁ : span {map (Int.castRingHom (ZMod p)) Q} =
      span {map (Int.castRingHom (ZMod p)) (minpoly Int θ)} ⊔
        span {map (Int.castRingHom (ZMod p)) Q} := by
    rw [←

中文:
定义 ZModXQuotSpanEquivQuotSpanPair
  签名: (hp : ¬ p ∣ exponent θ) {Q : 整数[X]}
  定义体: have h₀ : map (Int.castRingHom (ZMod p)) (minpoly Int θ) != 0 :=
      map_monic_ne_zero (minpoly.monic θ.isIntegral)
  have h_eq₁ : span {map (Int.castRingHom (ZMod p)) Q} =
      span {map (Int.castRingHom (ZMod p)) (minpoly Int θ)} ⊔
        span {map (Int.castRingHom (ZMod p)) Q} := by
    rw [←

Depends on / 依赖: Int.castRingHom, Multiset, Multiset.mem_toFinset, Polynomial, Polynomial.mem_normalizedFactors_iff, castRingHom, isIntegral, map_monic_ne_zero, mem_normalizedFactors_iff, mem_toFinset, minpoly, minpoly.monic, span_insert, span_pair_comm, span_pair_eq_span_left_iff_dvd, span_pair_eq_span_left_iff_dvd.mpr
-/
def ZModXQuotSpanEquivQuotSpanPair (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
    (hQ : Q.map (Int.castRingHom (ZMod p)) in monicFactorsMod θ p) :
    (ZMod p)[X] ⧸ span {Polynomial.map (Int.castRingHom (ZMod p)) Q} ≃+*
      𝓞 K ⧸ span {(p : 𝓞 K), (aeval θ) Q} :=
  have h₀ : map (Int.castRingHom (ZMod p)) (minpoly Int θ) != 0 :=
      map_monic_ne_zero (minpoly.monic θ.isIntegral)
  have h_eq₁ : span {map (Int.castRingHom (ZMod p)) Q} =
      span {map (Int.castRingHom (ZMod p)) (minpoly Int θ)} ⊔
        span {map (Int.castRingHom (ZMod p)) Q} := by
    rw [← span_insert]; rw [span_pair_comm]; rw [span_pair_eq_span_left_iff_dvd.mpr]
    simp only [Multiset.mem_toFinset] at hQ
    exact ((Polynomial.mem_normalizedFactors_iff h₀).mp hQ).2.2
  have h_eq₂ : span {↑p} ⊔ span {(aeval θ) Q} = span {↑p, (aeval θ) Q} := by
    rw [span_insert]
((Ideal.quotEquivOfEq h_eq₁).trans (DoubleQuot.quotQuotEquivQuotSup _ _).symm).trans
    (Ideal.quotientEquiv
      (Ideal.map (Ideal.Quotient.mk _) (span {(Polynomial.map (Int.castRingHom (ZMod p)) Q)}))
      (Ideal.map (Ideal.Quotient.mk _) (span {aeval θ Q})) (ZModXQuotSpanEquivQuotSpan hp) (by
        simp [map_span, ZModXQuotSpanEquivQuotSpan_mk_apply])).trans <|
    (DoubleQuot.quotQuotEquivQuotSup _ _).trans (Ideal.quotEquivOfEq h_eq₂)

end RingOfIntegers

open RingOfIntegers IsDedekindDomain
namespace NumberField.Ideal

variable {θ : 𝓞 K} {p : Nat} [Fact (Nat.Prime p)]

attribute [local instance] Int.ideal_span_isMaximal_of_prime Ideal.Quotient.field

set_option backward.privateInPublic true in
open scoped Classical in
/--
Definition of `primesOverSpanEquivMonicFactorsModAux` / `primesOverSpanEquivMonicFactorsModAux` 的定义

English:
definition primesOverSpanEquivMonicFactorsModAux
  signature: (A : Int[X])
  body: (normalizedFactorsEquiv (f := (mapEquiv (Int.quotientSpanNatEquivZMod p)).toMulEquiv)
    (by simp) (map (Ideal.Quotient.mk (span {(p : Int)})) A)).trans
      (Equiv.subtypeEquivRight (fun _ => by simp [Polynomial.map_map]))

中文:
定义 primesOverSpanEquivMonicFactorsModAux
  签名: (A : 整数[X])
  定义体: (normalizedFactorsEquiv (f := (mapEquiv (Int.quotientSpanNatEquivZMod p)).toMulEquiv)
    (by simp) (map (Ideal.Quotient.mk (span {(p : Int)})) A)).trans
      (Equiv.subtypeEquivRight (fun _ => by simp [Polynomial.map_map]))
-/
private def primesOverSpanEquivMonicFactorsModAux (A : Int[X]) :
    {Q // Q in normalizedFactors (map (Ideal.Quotient.mk (span {(p : Int)})) A)} ≃
    (normalizedFactors (map (Int.castRingHom (ZMod p)) A)).toFinset :=
  (normalizedFactorsEquiv (f := (mapEquiv (Int.quotientSpanNatEquivZMod p)).toMulEquiv)
    (by simp) (map (Ideal.Quotient.mk (span {(p : Int)})) A)).trans
      (Equiv.subtypeEquivRight (fun _ => by simp [Polynomial.map_map]))

/--
theorem `primesOverSpanEquivMonicFactorsModAux_symm_apply` / 定理 `primesOverSpanEquivMonicFactorsModAux_symm_apply`

English:
theorem primesOverSpanEquivMonicFactorsModAux_symm_apply
  statement: (A : Int[X]) {Q : (ZMod p)[X]}
  proof: rfl

中文:
定理 primesOverSpanEquivMonicFactorsModAux_symm_apply
  结论: (A : 整数[X]) {Q : (ZMod p)[X]}
  证明: rfl
-/
private theorem primesOverSpanEquivMonicFactorsModAux_symm_apply (A : Int[X]) {Q : (ZMod p)[X]}
    (hQ : Q in (normalizedFactors (map (Int.castRingHom (ZMod p)) A)).toFinset) :
    ((primesOverSpanEquivMonicFactorsModAux A).symm ⟨Q, hQ⟩ : (Int ⧸ span {(p : Int)})[X]) =
      Polynomial.map ((Int.quotientSpanNatEquivZMod p).symm) Q := rfl

variable [NumberField K]

set_option backward.privateInPublic true in
set_option backward.privateInPublic.warn false in
/--
Definition of `primesOverSpanEquivMonicFactorsMod` / `primesOverSpanEquivMonicFactorsMod` 的定义

English:
definition primesOverSpanEquivMonicFactorsMod
  signature: (hp : ¬ p ∣ exponent θ)
  body: have h : span {(p : Int)} != ⊥ := by simp [NeZero.ne p]
  ((Equiv.setCongr (by ext; simp [mem_primesOver_iff_mem_normalizedFactors _ h])).trans
    (normalizedFactorsMapEquivNormalizedFactorsMinPolyMk
    (Int.ideal_span_isMaximal_of_prime p) h
      (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral)

中文:
定义 primesOverSpanEquivMonicFactorsMod
  签名: (hp : ¬ p ∣ exponent θ)
  定义体: have h : span {(p : Int)} != ⊥ := by simp [NeZero.ne p]
  ((Equiv.setCongr (by ext; simp [mem_primesOver_iff_mem_normalizedFactors _ h])).trans
    (normalizedFactorsMapEquivNormalizedFactorsMinPolyMk
    (Int.ideal_span_isMaximal_of_prime p) h
      (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral)

Depends on / 依赖: Equiv.setCongr, Int.ideal_span_isMaximal_of_prime, NeZero, NeZero.ne, eq_top, ideal_span_isMaximal_of_prime, isIntegral, mem_primesOver_iff_mem_normalizedFactors, normalizedFactorsMapEquivNormalizedFactorsMinPolyMk, not_dvd_exponent_iff, not_dvd_exponent_iff.mp, primesOverSpanEquivMonicFactorsModAux, setCongr
-/
def primesOverSpanEquivMonicFactorsMod (hp : ¬ p ∣ exponent θ) :
    primesOver (span {(p : Int)}) (𝓞 K) ≃ monicFactorsMod θ p :=
  have h : span {(p : Int)} != ⊥ := by simp [NeZero.ne p]
  ((Equiv.setCongr (by ext; simp [mem_primesOver_iff_mem_normalizedFactors _ h])).trans
    (normalizedFactorsMapEquivNormalizedFactorsMinPolyMk
    (Int.ideal_span_isMaximal_of_prime p) h
      (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral)).trans <|
        (primesOverSpanEquivMonicFactorsModAux _)

/--
theorem `primesOverSpanEquivMonicFactorsMod_symm_apply` / 定理 `primesOverSpanEquivMonicFactorsMod_symm_apply`

English:
theorem primesOverSpanEquivMonicFactorsMod_symm_apply
  statement: (hp : ¬ p ∣ exponent θ)
  proof: rfl

中文:
定理 primesOverSpanEquivMonicFactorsMod_symm_apply
  结论: (hp : ¬ p ∣ exponent θ)
  证明: rfl
-/
theorem primesOverSpanEquivMonicFactorsMod_symm_apply (hp : ¬ p ∣ exponent θ)
    {Q : (ZMod p)[X]} (hQ : Q in monicFactorsMod θ p) :
    ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨Q, hQ⟩ : Ideal (𝓞 K)) =
      (normalizedFactorsMapEquivNormalizedFactorsMinPolyMk
        inferInstance (by simp [NeZero.ne p]) (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral).symm
        ⟨Q.map (Int.quotientSpanNatEquivZMod p).symm, by
          rw [← primesOverSpanEquivMonicFactorsModAux_symm_apply]
          exact ((primesOverSpanEquivMonicFactorsModAux _).symm ⟨Q, hQ⟩).prop⟩ := rfl

set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span` / 定理 `primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span`

English:
theorem primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span
  statement: (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
  proof: by
  simp only [primesOverSpanEquivMonicFactorsMod_symm_apply, Polynomial.map_map,
    Int.quotientSpanNatEquivZMod_comp_castRingHom]
  rw [normalizedFactorsMapEquivNormalizedFactorsMinPolyMk_symm_apply_eq_span]; rw [span_union]; rw [span_eq]; rw [map_span]; rw [Set.image_singleton]; rw [map_natCast

中文:
定理 primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span
  结论: (hp : ¬ p ∣ exponent θ) {Q : 整数[X]}
  证明: by
  simp only [primesOverSpanEquivMonicFactorsMod_symm_apply, Polynomial.map_map,
    Int.quotientSpanNatEquivZMod_comp_castRingHom]
  rw [normalizedFactorsMapEquivNormalizedFactorsMinPolyMk_symm_apply_eq_span]; rw [span_union]; rw [span_eq]; rw [map_span]; rw [Set.image_singleton]; rw [map_natCast

Depends on / 依赖: Int.quotientSpanNatEquivZMod_comp_castRingHom, Polynomial, Polynomial.map_map, Set.image_singleton, image_singleton, map_map, map_natCast, map_span, normalizedFactorsMapEquivNormalizedFactorsMinPolyMk_symm_apply_eq_span, primesOverSpanEquivMonicFactorsMod_symm_apply, quotientSpanNatEquivZMod_comp_castRingHom, span_eq, span_insert, span_union
-/
theorem primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
    (hQ : Q.map (Int.castRingHom (ZMod p)) in monicFactorsMod θ p) :
    ((primesOverSpanEquivMonicFactorsMod hp).symm
      ⟨Q.map (Int.castRingHom (ZMod p)), hQ⟩ : Ideal (𝓞 K)) =
        span {(p : (𝓞 K)), aeval θ Q} := by
  simp only [primesOverSpanEquivMonicFactorsMod_symm_apply, Polynomial.map_map,
    Int.quotientSpanNatEquivZMod_comp_castRingHom]
  rw [normalizedFactorsMapEquivNormalizedFactorsMinPolyMk_symm_apply_eq_span]; rw [span_union]; rw [span_eq]; rw [map_span]; rw [Set.image_singleton]; rw [map_natCast]; rw [← span_insert]

/--
theorem `liesOver_primesOverSpanEquivMonicFactorsMod_symm` / 定理 `liesOver_primesOverSpanEquivMonicFactorsMod_symm`

English:
theorem liesOver_primesOverSpanEquivMonicFactorsMod_symm
  statement: (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
  proof: by
  rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
  exact ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨_, hQ⟩).prop.2

中文:
定理 liesOver_primesOverSpanEquivMonicFactorsMod_symm
  结论: (hp : ¬ p ∣ exponent θ) {Q : 整数[X]}
  证明: by
  rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
  exact ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨_, hQ⟩).prop.2

Depends on / 依赖: Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span, primesOverSpanEquivMonicFactorsMod, primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span
-/
theorem liesOver_primesOverSpanEquivMonicFactorsMod_symm (hp : ¬ p ∣ exponent θ) {Q : Int[X]}
    (hQ : Q.map (Int.castRingHom (ZMod p)) in monicFactorsMod θ p) :
    LiesOver (span {(p : (𝓞 K)), aeval θ Q}) (span {(p : Int)}) := by
  rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
  exact ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨_, hQ⟩).prop.2

/--
theorem `inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply` / 定理 `inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply`

English:
theorem inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply
  statement: (hp : ¬ p ∣ exponent θ)
  proof: by
  -- This is needed for `inertiaDeg_algebraMap` below to work
  have : (span {↑p, (aeval θ) Q}).IsMaximal := by
    rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
    apply Ideal.primesOver.isMaximal
  have := liesOver_primesOverSpanEquivMonicFactorsMod_symm hp hQ
  rw [

中文:
定理 inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply
  结论: (hp : ¬ p ∣ exponent θ)
  证明: by
  -- This is needed for `inertiaDeg_algebraMap` below to work
  have : (span {↑p, (aeval θ) Q}).IsMaximal := by
    rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
    apply Ideal.primesOver.isMaximal
  have := liesOver_primesOverSpanEquivMonicFactorsMod_symm hp hQ
  rw [
-/
theorem inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply (hp : ¬ p ∣ exponent θ)
    {Q : Int[X]} (hQ : Q.map (Int.castRingHom (ZMod p)) in monicFactorsMod θ p) :
    inertiaDeg ((primesOverSpanEquivMonicFactorsMod hp).symm
      ⟨Q.map (Int.castRingHom (ZMod p)), hQ⟩ : Ideal (𝓞 K)) Int =
        natDegree (Q.map (Int.castRingHom (ZMod p))) := by
  -- This is needed for `inertiaDeg_algebraMap` below to work
  have : (span {↑p, (aeval θ) Q}).IsMaximal := by
    rw [← Ideal.primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span hp hQ]
    apply Ideal.primesOver.isMaximal
  have := liesOver_primesOverSpanEquivMonicFactorsMod_symm hp hQ
  rw [primesOverSpanEquivMonicFactorsMod_symm_apply_eq_span]; rw [inertiaDeg_eq_of_isMaximal (span {(p : Int)})]; rw [← finrank_quotient_span_eq_natDegree]
  refine Algebra.finrank_eq_of_equiv_equiv (Int.quotientSpanNatEquivZMod p) ?_ (by ext; simp)
  exact (ZModXQuotSpanEquivQuotSpanPair hp hQ).symm

/--
theorem `inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply'` / 定理 `inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply'`

English:
theorem inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply'
  statement: (hp : ¬ p ∣ exponent θ)
  proof: by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply]

中文:
定理 inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply'
  结论: (hp : ¬ p ∣ exponent θ)
  证明: by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply]

Depends on / 依赖: Int.castRingHom, ZMod.ringHom_surjective, castRingHom, inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply, map_surjective, ringHom_surjective
-/
theorem inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply' (hp : ¬ p ∣ exponent θ)
    {Q : (ZMod p)[X]} (hQ : Q in monicFactorsMod θ p) :
    inertiaDeg
      ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨Q, hQ⟩ : Ideal (𝓞 K)) Int = natDegree Q := by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [inertiaDeg_primesOverSpanEquivMonicFactorsMod_symm_apply]

/--
theorem `ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply` / 定理 `ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply`

English:
theorem ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply
  statement: (hp : ¬ p ∣ exponent θ)
  proof: by
  rw [ramificationIdx_eq_multiplicity (span {↑p}) _ (map_ne_bot_of_ne_bot (by simp [NeZero.ne p]))]
  · apply multiplicity_eq_of_emultiplicity_eq
    rw [← emultiplicity_map_eq (mapEquiv (Int.quotientSpanNatEquivZMod p).symm)]; rw [emultiplicity_factors_map_eq_emultiplicity inferInstance (by simp

中文:
定理 ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply
  结论: (hp : ¬ p ∣ exponent θ)
  证明: by
  rw [ramificationIdx_eq_multiplicity (span {↑p}) _ (map_ne_bot_of_ne_bot (by simp [NeZero.ne p]))]
  · apply multiplicity_eq_of_emultiplicity_eq
    rw [← emultiplicity_map_eq (mapEquiv (Int.quotientSpanNatEquivZMod p).symm)]; rw [emultiplicity_factors_map_eq_emultiplicity inferInstance (by simp

Depends on / 依赖: Equiv.apply_symm_apply, Int.quotientSpanNatEquivZMod, NeZero, NeZero.ne, apply_symm_apply, emultiplicity_factors_map_eq_emultiplicity, emultiplicity_map_eq, eq_top, isIntegral, mapEquiv, map_ne_bot_of_ne_bot, multiplicity_eq_of_emultiplicity_eq, normalizedFactorsMapEquivNormalizedFactorsMinPolyMk, not_dvd_exponent_iff, not_dvd_exponent_iff.mp, primesOverSpanEquivMonicFactorsMod_symm_apply, quotientSpanNatEquivZMod, ramificationIdx_eq_multiplicity
-/
theorem ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply (hp : ¬ p ∣ exponent θ)
    {Q : Int[X]} (hQ : Q.map (Int.castRingHom (ZMod p)) in monicFactorsMod θ p) :
    ramificationIdx
      ((primesOverSpanEquivMonicFactorsMod hp).symm
        ⟨Q.map (Int.castRingHom (ZMod p)), hQ⟩ : Ideal (𝓞 K)) Int =
          multiplicity (Q.map (Int.castRingHom (ZMod p)))
            ((minpoly Int θ).map (Int.castRingHom (ZMod p))) := by
  rw [ramificationIdx_eq_multiplicity (span {↑p}) _ (map_ne_bot_of_ne_bot (by simp [NeZero.ne p]))]
  · apply multiplicity_eq_of_emultiplicity_eq
    rw [← emultiplicity_map_eq (mapEquiv (Int.quotientSpanNatEquivZMod p).symm)]; rw [emultiplicity_factors_map_eq_emultiplicity inferInstance (by simp [NeZero.ne p])
      (not_dvd_exponent_iff.mp hp).eq_top θ.isIntegral]
    · simp only [primesOverSpanEquivMonicFactorsMod_symm_apply,
        Equiv.apply_symm_apply (normalizedFactorsMapEquivNormalizedFactorsMinPolyMk _ _ _ _),
        Polynomial.map_map, Int.quotientSpanNatEquivZMod_comp_castRingHom, mapEquiv_apply]
    · rw [← mem_primesOver_iff_mem_normalizedFactors _ (by simp [NeZero.ne p])]
      exact ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨_, hQ⟩).coe_prop

/--
theorem `ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply'` / 定理 `ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply'`

English:
theorem ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply'
  statement: (hp : ¬ p ∣ exponent θ)
  proof: by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply]

中文:
定理 ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply'
  结论: (hp : ¬ p ∣ exponent θ)
  证明: by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply]

Depends on / 依赖: Int.castRingHom, ZMod.ringHom_surjective, castRingHom, map_surjective, ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply, ringHom_surjective
-/
theorem ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply' (hp : ¬ p ∣ exponent θ)
    {Q : (ZMod p)[X]} (hQ : Q in monicFactorsMod θ p) :
    ramificationIdx
      ((primesOverSpanEquivMonicFactorsMod hp).symm ⟨Q, hQ⟩ : Ideal (𝓞 K)) Int =
        multiplicity Q ((minpoly Int θ).map (Int.castRingHom (ZMod p))) := by
  obtain ⟨S, rfl⟩ := (map_surjective _ (ZMod.ringHom_surjective (Int.castRingHom (ZMod p)))) Q
  rw [ramificationIdx_primesOverSpanEquivMonicFactorsMod_symm_apply]

end NumberField.Ideal
