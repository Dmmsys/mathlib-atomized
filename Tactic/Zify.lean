...
/--
Definition of `mkZifyContext` / `mkZifyContext` 的定义

English:
definition mkZifyContext
  signature: (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ","))
  body: do
.getD #[] let args := simpArgs.map (·.getElems)
  mkSimpContext
...
/--
Definition of `applySimpResultToProp'` / `applySimpResultToProp'` 的定义

English:
definition applySimpResultToProp'
  signature: (proof : Expr) (prop : Expr) (r : Simp.Result)
  body: do
  match r.proof? with
...
中文:
定义 applySimpResultToProp'
  签名: (proof : Expr) (prop : Expr) (r : Simp.Result)
  定义体: do
  match r.proof? with
  | some eqProof => return (← mkExpectedTypeHint (← mkEqMP eqProof proof) r.expr, r.expr)
  | none =>
...
Depends on / 依赖: eqProof, mkEqMP, mkExpectedTypeHint, r.expr, r.proof, return
-/
...
/--
Definition of `zifyProof` / `zifyProof` 的定义

English:
definition zifyProof
  signature: (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ","))
  body: do
...
lemma natCast_ne
  given: (a b : Nat)
  statement: a != b ↔ (a : Int) != (b : Int)
  proof: not_congr Int.ofNat_inj.symm

中文:
引理 natCast_ne
  条件: (a b : 自然数)
  结论: a != b ↔ (a : 整数) != (b : 整数)
  证明: not_congr Int.ofNat_inj.symm
-/
...
/--
theorem `Nat.cast_sub_of_add_le` / 定理 `Nat.cast_sub_of_add_le`

English:
theorem Nat.cast_sub_of_add_le
  given: {m n k} (h : m + k <= n)
...
Depends on / 依赖: Nat.cast_sub, cast_sub, le_add_right, m.le_add_right
-/
theorem Nat.cast_sub_of_add_le {m n k} (h : m + k <= n) :
    ((n - m : Nat) : R) = n - m := Nat.cast_sub (m.le_add_right k |>.trans h)

/--
theorem `Nat.cast_sub_of_lt` / 定理 `Nat.cast_sub_of_lt`

English:
theorem Nat.cast_sub_of_lt
  given: {m n} (h : m < n)
...
@[norm_cast] theorem Nat.cast_sub_of_lt {m n} (h : m < n) :
    ((n - m : Nat) : R) = n - m := Nat.cast_sub h.le
