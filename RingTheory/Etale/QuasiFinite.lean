...
noncomputable
/--
Definition of `Ideal.fiberIsoOfBijectiveResidueField` / `Ideal.fiberIsoOfBijectiveResidueField` 的定义

English:
definition Ideal.fiberIsoOfBijectiveResidueField
  body: let := Localization.AtPrime.algebraOfLiesOver p q
...
Depends on / 依赖: Algebra, Algebra.TensorProduct.cancelBaseChange, Algebra.TensorProduct.congr, Algebra.ofId, AtPrime, Localization, Localization.AtPrime.algebraOfLiesOver, PrimeSpectrum, PrimeSpectrum.comapEquiv, PrimeSpectrum.primesOverOrderIsoFiber, ResidueField, TensorProduct, algebraOfLiesOver, cancelBaseChange, comapEquiv, e.toRingEquiv, ofBijective, otimes, p.Fiber, p.ResidueField
-/
def Ideal.fiberIsoOfBijectiveResidueField
    (H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p))) :
    q.primesOver (R' otimes[R] S) ≃o p.primesOver S :=
...
/--
lemma `Ideal.comap_fiberIsoOfBijectiveResidueField_symm` / 引理 `Ideal.comap_fiberIsoOfBijectiveResidueField_symm`
...
Depends on / 依赖: Ideal.fiberIsoOfBijectiveResidueField, PrimeSpectrum, PrimeSpectrum.preimageEquivFiber, PrimeSpectrum.preimageOrderIsoFiber, PrimeSpectrum.primesOverOrderIsoFiber, fiberIsoOfBijectiveResidueField, preimageEquivFiber, preimageOrderIsoFiber, primesOverOrderIsoFiber
-/
lemma Ideal.comap_fiberIsoOfBijectiveResidueField_symm
    (H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p)))
    (Q : p.primesOver S) :
...
@[simp]
/--
lemma `Ideal.comap_fiberIsoOfBijectiveResidueField_apply` / 引理 `Ideal.comap_fiberIsoOfBijectiveResidueField_apply`
...
Depends on / 依赖: Ideal.comap_fiberIsoOfBijectiveResidueField_symm, Ideal.fiberIsoOfBijectiveResidueField, comap_fiberIsoOfBijectiveResidueField_symm, fiberIsoOfBijectiveResidueField
-/
lemma Ideal.comap_fiberIsoOfBijectiveResidueField_apply
    (H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p)))
    (Q : q.primesOver (R' otimes[R] S)) :
...
/--
lemma `Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap` / 引理 `Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap`
...
Depends on / 依赖: Ideal.fiberIsoOfBijectiveResidueField, Subtype, Subtype.val, congr_arg, fiberIsoOfBijectiveResidueField, injective
-/
lemma Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap
    (H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p)))
    (P₁ P₂ : Ideal (R' otimes[R] S)) [P₁.IsPrime] [P₂.IsPrime] [P₁.LiesOver q] [P₂.LiesOver q]
...
@[stacks 00UI]
/--
lemma `Localization.exists_finite_awayMapₐ_of_surjective_awayMapₐ` / 引理 `Localization.exists_finite_awayMapₐ_of_surjective_awayMapₐ`
...
lemma Localization.exists_finite_awayMapₐ_of_surjective_awayMapₐ
    ...
    (hgp : IsUnit (M := p.Fiber S) (1 otimesₜ g)) :
    exists r ∉ p, (Localization.awayMapₐ (Algebra.ofId R T) r).Finite := by
  have := PrimeSpectrum.isClosedMap_comap_of_isIntegral (algebraMap R S)
    (algebraMap_isIntegral_iff.mpr ‹_›) _ (PrimeSpectrum.isClosed_zeroLocus {g})
  obtain ⟨_, ⟨_, ⟨r, rfl⟩, rfl⟩, hpr, hrg⟩ := PrimeSpectrum.isBasis_basic_opens
.exists_subset_of_mem_open (a := ⟨p, ‹_›⟩) (ou := this.isOpen_compl) by
    ...
    have : 1 otimesₜ g ∉ (PrimeSpectrum.preimageEquivFiber R S ⟨p, ‹_›⟩ ⟨q, e⟩).asIdeal :=
  ...
  · have : IsLocalization.Away (f.toRingHom (algebraMap R S r))
    ...
    have h₂ : Function.Surjective (IsLocalization.Away.map (Localization.Away (algebraMap R S r))
      ...
      suffices exists a k l, algebraMap R T r ^ (l + n) * f a =
...
/--
lemma `Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver` / 引理 `Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver`
...
Depends on / 依赖: Algebra, Algebra.ZariskisMainProperty.of_finiteType, Ideal.exists_not_mem_forall_mem_of_ne_of_liesOver, IsLocalization, IsLocalization.Away.map, IsLocalization.eq_, IsLocalization.exists_mk, IsLocalization.map_mk, IsLocalization.mk, ZariskisMainProperty, _eq_iff_eq_mul, algebraMap, exists_mk, exists_not_mem_forall_mem_of_ne_of_liesOver, map_mk, map_mul, map_pow, of_finiteType, powers
-/
lemma Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver
    ...
    exists s ∉ q, exists hs : IsIntegral R s, (forall q' : Ideal S, q'.IsPrime -> q' != q -> q'.LiesOver p -> s in q') ∧
      forall (q' : Ideal (integralClosure R S)), q'.IsPrime ->
        q' != q.under _ -> q'.LiesOver p -> ⟨s, hs⟩ in q' := by
  obtain ⟨s₁, hs₁q, hs₁⟩ := Ideal.exists_not_mem_forall_mem_of_ne_of_liesOver (R := R) p q
  obtain ⟨s₂, hs₂q, hs₂⟩ := Algebra.ZariskisMainProperty.of_finiteType (R := R) q
  obtain ⟨s₃, hs₃⟩ := hs₂.2 (algebraMap _ _ s₁)
  obtain ⟨s₃, ⟨_, n, rfl⟩, rfl⟩ := IsLocalization.exists_mk'_eq (.powers s₂) s₃
  obtain ⟨m, hm⟩ : exists m, ↑s₂ ^ m * ↑s₃ = ↑s₂ ^ m * (s₁ * ↑s₂ ^ n) := by
  ...
  have := hs₁ (q's.comap (e.symm.toAlgHom.comp (IsScalarTower.toAlgHom _ _ _)).toRingHom)
    inferInstance (by
      contrapose hq'q
      rw [← hq'q]; rw [Ideal.under]; rw [Ideal.comap_comap]; rw [AlgHom.toRingHom_eq_coe]; rw [AlgHom.comp_algebraMap]; rw [q's.over_def q'])
  ...
  have : e.symm (algebraMap S (Localization.Away
      ((integralClosure R S).val.toRingHom s₂)) s₁) in q's := by
    simpa using! this
  rw [← hs₃]; rw [← Ideal.IsPrime.mul_mem_left_iff (x := algebraMap _ _ (s₂ ^ n))] at this
  · dsimp [Localization.awayMap, IsLocalization.Away.map] at this
    rw [IsLocalization.map_mk']; rw [← e.symm.commutes]; rw [← map_mul]; rw [IsScalarTower.algebraMap_eq _ S _] at this
    replace this : e.symm ((algebraMap _ (Localization.Away s₂.1)) s₃) in q's := by
      simpa [-map_mul, -map_pow, -AlgEquiv.commutes] using! this
    replace this : s₃ in q' := by simpa [← Ideal.mem_comap, ← q's.over_def q'] using! this
...
/--
lemma `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux` / 引理 `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux`
...
Depends on / 依赖: IsCoprime, ResidueField, algebraMap, b.Monic, exists_eq_pow_rootMultiplicity_mul_and_not_dvd, exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver, f.Monic, f.map, minpoly, minpoly.monic, ne_zero, p.ResidueField
-/
lemma Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux
    ...
    exists (R' : Type u) (_ : CommRing R') (_ : Algebra R R') (_ : Algebra.Etale R R') (P : Ideal R')
      (_ : P.IsPrime) (_ : P.LiesOver p) (e : R' otimes[R] S) (_ : IsIdempotentElem e)
      (e₀ : R' otimes[R] integralClosure R S) (_ : IsIdempotentElem e₀)
      (_ : Algebra.TensorProduct.map (.id R' R') (integralClosure R S).val e₀ = e)
      (P' : Ideal (R' otimes[R] S)) (_ : P'.IsPrime) (_ : P'.LiesOver P), P'.comap
      ...
      (forall P'' : Ideal (R' otimes[R] integralClosure R S),
          P''.IsPrime -> P''.LiesOver P -> e₀ ∉ P'' -> P'' =
            P'.comap (Algebra.TensorProduct.map (.id R' R') (integralClosure R S).val).toRingHom) ∧
      forall P'' : Ideal (R' otimes[R] S), P''.IsPrime -> P''.LiesOver P -> e ∉ P'' -> P'' = P' := by
  obtain ⟨s, hsq, hRs, hs, hs₀⟩ := exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver p q
  obtain ⟨m, f, b, hfm, hbm, hab, hfab, hf⟩ : exists (m : Nat) (f : R[X])
      (b : p.ResidueField[X]), f.Monic ∧ b.Monic ∧ IsCoprime (X ^ (m + 1)) b ∧
  ...
  have ha'P' : aeval s' a' ∉ P'.1 := by
    simpa using! show IsScalarTower.toAlgHom R' _ P'.1.ResidueField (aeval s' a') != 0 by
      rw [← aeval_algHom_apply]; rw [← aeval_map_algebraMap P.ResidueField]; rw [← ha']; simpa
  have hb'P' : aeval s' b' in P'.1 := by
    rw [← Ideal.IsPrime.mul_mem_left_iff ha'P']; rw [← map_mul]; rw [← hfab']
    simp [hs'f]
  have heP' : e ∉ P'.1 := by
    intro H
    have := P'.1.mul_mem_left (aeval s' d) hb'P'
    rw [← map_mul]; rw [eq_sub_iff_add_eq'.mpr hcd]; rw [map_sub]; rw [Submodule.sub_mem_iff_left _ H]; rw [map_one] at this
  ...
  · have : (P'.1.comap φ.toRingHom).LiesOver P := inferInstanceAs ((P'.1.comap φ).LiesOver P)
    ...
    contrapose! H
    have : 1 otimesₜ s₀ in P'' := hs₀ _ inferInstance H (by simp [Ideal.liesOver_iff, Ideal.under,
      Ideal.comap_comap, Ideal.over_def P p, Ideal.over_def P'' P, ← IsScalarTower.algebraMap_eq])
    rw [← Ideal.algebraMap_residueField_eq_zero]; rw [← aeval_algebraMap_apply]; rw [Ideal.algebraMap_residueField_eq_zero.mpr this]; rw [← eval_map_algebraMap]; rw [Polynomial.map_mul]; rw [mul_comm]; rw [← (Ideal.ResidueField.mapₐ P P'' (Algebra.ofId _ _)
      (P''.over_def P)).comp_algebraMap]; rw [← Polynomial.map_map]; rw [← ha']
    simp
  · apply Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap hP
    rw [hP'q]
    contrapose! H
    have : s' in P'' := hs _ inferInstance H (by simp [Ideal.liesOver_iff, Ideal.under,
      Ideal.comap_comap, Ideal.over_def P p, Ideal.over_def P'' P, ← IsScalarTower.algebraMap_eq])
    rw [← Ideal.algebraMap_residueField_eq_zero]; rw [← aeval_algebraMap_apply]; rw [Ideal.algebraMap_residueField_eq_zero.mpr this]; rw [← eval_map_algebraMap]; rw [Polynomial.map_mul]; rw [mul_comm]; rw [← (Ideal.ResidueField.mapₐ P P'' (Algebra.ofId _ _)
      (P''.over_def P)).comp_algebraMap]; rw [← Polynomial.map_map]; rw [← ha']
    simp
...
Depends on / 依赖: Algebra, Algebra.IsIntegral, Algebra.TensorProduct.map, IsIntegral, IsLocalization, IsLocalization.Away.algebraMap_surjective_of_isIdempotentElem, IsScalarTower, IsScalarTower.toAlgHom, Localization, Localization.Away, Localization.awayM, Localization.awayMap, Localization.awayMap_awayMap_surjective, Localization.exists_finite_awayMap, TensorProduct, algebraMap, algebraMap_surjective_of_isIdempotentElem, awayMap_awayMap_surjective, of_surjective, otimes
-/
lemma Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux₂
    ...
    (e : R' otimes[R] S) (e₀ : R' otimes[R] R'') (he₀ : IsIdempotentElem e₀)
    (he₀e : Algebra.TensorProduct.map (.id R' R') (IsScalarTower.toAlgHom R R'' S) e₀ = e)
    (P' : Ideal (R' otimes[R] S))
    (hP'q : P'.comap Algebra.TensorProduct.includeRight.toRingHom = q)
    (H : forall P'' : Ideal (R' otimes[R] R''), P''.IsPrime -> P''.LiesOver P -> e₀ ∉ P'' -> P'' =
    ...
    exists f ∉ P, Module.Finite (Localization.Away f) (Localization.Away
  ...
  obtain ⟨f, hfP, hf⟩ := Localization.exists_finite_awayMapₐ_of_surjective_awayMapₐ
    (Localization.awayMapₐ (Algebra.TensorProduct.map (.id R' R')
      (IsScalarTower.toAlgHom R R'' S)) e₀) (algebraMap (R' otimes[R] R'') _ (1 otimesₜ g)) (by
    ...
    | tmul a b =>
      ...
      refine ⟨e₀ ^ m * a otimesₜ b', m, ?_⟩
      simp [e, mul_pow, mul_assoc]
    | add a b ha hb =>
      obtain ⟨xa, ma, ea⟩ := ha
      obtain ⟨xb, mb, eb⟩ := hb
      refine ⟨(e₀ * 1 otimesₜ[R] g) ^ mb * xa + (e₀ * 1 otimesₜ[R] g) ^ ma * xb, ma + mb, ?_⟩
      dsimp at ea eb ⊢
  ...
  rw [he₀e] at hf
  refine ⟨f, hfP, ?_⟩
  let φ : R' otimes[R] S ->ₐ[R'] Localization.Away f otimes[R] S :=
    Algebra.TensorProduct.map (Algebra.ofId _ _) (.id _ _)
  algebraize [φ.toRingHom]
  have : IsLocalization.Away (f otimesₜ 1 : R' otimes[R] S) (Localization.Away f otimes[R] S) := by
    have : .powers (f otimesₜ[R] 1) = Algebra.algebraMapSubmonoid (R' otimes[R] S) (.powers f) := by simp
    rw [IsLocalization.Away]; rw [this]
    refine IsLocalization.tensorProduct_tensorProduct _ _ _ _ ?_
    ext; simp [RingHom.algebraMap_toAlgebra, φ]
  have : IsLocalization.Away (e * (f otimesₜ 1)) (Localization.Away (φ e)) :=
    .mul (Localization.Away f otimes[R] S) (Localization.Away (algebraMap _ _ e)) _ _
  have : IsLocalization.Away (e * (f otimesₜ 1)) (Localization.Away (Algebra.ofId R'
      (Localization.Away e) f)) :=
    .mul' (Localization.Away e)
      (Localization.Away (algebraMap (R' otimes[R] S) (Localization.Away e) (f otimesₜ[R] 1))) _ _
  let equiv := IsLocalization.algEquiv (.powers (e * (f otimesₜ 1)))
...
@[stacks 00UJ]
/--
lemma `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq` / 引理 `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq`
...
Depends on / 依赖: Algebra, Algebra.ZariskisMainProperty.of_finiteType, Ideal.disjo, IsLocalization, IsLocalization.isPrime_of_isPrime_disjoint, IsPrime, Localization, Localization.Away, P.map, Pf.IsPrime, ZariskisMainProperty, algebraMap, exists_etale_isIdempotentElem_forall_liesOver_eq_aux, isPrime_of_isPrime_disjoint, of_finiteType, powers
-/
lemma Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq
    {R : Type u} {S : Type v} [CommRing R] [CommRing S] [Algebra R S] [Algebra.FiniteType R S]
    (p : Ideal R) [p.IsPrime] (q : Ideal S) [q.IsPrime] [q.LiesOver p] [Algebra.QuasiFiniteAt R q] :
  ...
  have : IsLocalization.Away (R := R' otimes[R] S) (f otimesₜ 1) (Localization.Away f otimes[R] S) := by
    have : .powers (f otimesₜ[R] 1) = Algebra.algebraMapSubmonoid (R' otimes[R] S) (.powers f) := by simp
    rw [IsLocalization.Away]; rw [this]
    refine IsLocalization.tensorProduct_tensorProduct _ _ _ _ ?_
    ext; simp [RingHom.algebraMap_toAlgebra, φ]
  let P'f := P'.map (algebraMap _ (Localization.Away f otimes[R] S))
  have hP'f : Disjoint (Submonoid.powers (f otimesₜ 1 : R' otimes[R] S) : Set (R' otimes[R] S)) ↑P' := by
    rw [Ideal.disjoint_powers_iff_notMem_of_isPrime]
    change f ∉ P'.under _
    rwa [← P'.over_def P]
  ...
  · intro P'' _ _ hP''
    ...
    refine congr($(PrimeSpectrum.localization_comap_injective (R := R' otimes[R] S)
      (Localization.Away f otimes[R] S) (.powers (f otimesₜ 1)) (a₁ := ⟨P'', ‹_›⟩)
...
/--
theorem `Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq_aux` / 定理 `Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq_aux`

English:
theorem Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq_aux
  proof: by
...
private theorem Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq_aux
    ...
    [P.IsPrime] [P.LiesOver p] (e : R' otimes[R] S) (P' : Ideal (R' otimes[R] S))
    ...
    (H : forall (P'' : Ideal (R' otimes[R] S)), P''.IsPrime -> P''.LiesOver P -> e ∉ P'' -> P'' = P')
    (R'' : Type u) [CommRing R''] [Algebra R' R''] [Algebra R R''] [IsScalarTower R R' R'']
    [Algebra.Etale R' R''] (Q : Ideal R'')
    [Q.IsPrime] [Q.LiesOver P] (n : Nat)
    (e' : Fin ((n + 1) + 1) -> R'' otimes[R] S)
    (he' : CompleteOrthogonalIdempotents e')
    (he'0 : e' 0 = Algebra.TensorProduct.map (Algebra.ofId R' R'') (AlgHom.id R S) e)
    (Q' : Fin n -> Ideal (R'' otimes[R] S)) [forall i, (Q' i).IsPrime] [forall i, (Q' i).LiesOver Q]
    (hPQ : Function.Bijective (Ideal.ResidueField.mapₐ P Q (Algebra.ofId _ _) (Q.over_def P)))
    (hQ' : forall (i : Fin n), e' i.succ.castSucc ∉ Q' i)
    (H' : forall (P'' : Ideal (R'' otimes[R] S)), e' 0 in P'' -> P''.IsPrime -> P''.LiesOver Q ->
      e' (.last _) in P'' ∧ forall (i : Fin n), e' i.succ.castSucc ∉ P'' -> P'' = Q' i) :
    exists (R' : Type u) (_ : CommRing R') (_ : Algebra R R') (_ : Algebra.Etale R R') (P : Ideal R')
      (_ : P.IsPrime) (_ : P.LiesOver p) (n : Nat) (e : Fin (n + 1) -> R' otimes[R] S)
      (_ : CompleteOrthogonalIdempotents e) (P' : Fin n -> Ideal (R' otimes[R] S))
      (_ : forall i, (P' i).IsPrime) (_ : forall i, (P' i).LiesOver P),
      Function.Bijective (Ideal.ResidueField.mapₐ p P (Algebra.ofId _ _) (P.over_def p)) ∧
      (forall i, e i.castSucc ∉ P' i) ∧
      forall (P'' : Ideal (R' otimes[R] S)), P''.IsPrime -> P''.LiesOver P ->
        e (.last n) in P'' ∧ forall i, e i.castSucc ∉ P'' -> P'' = P' i := by
  let φ := Algebra.TensorProduct.map (Algebra.ofId R' R'') (AlgHom.id R S)
  have : Q.LiesOver p := .trans _ P _
  have hpQ :
    Function.Bijective (Ideal.ResidueField.mapₐ p Q (Algebra.ofId _ _) (Q.over_def p)) := by
    convert hPQ.comp hpP
    rw [← @AlgHom.coe_restrictScalars' R R']; rw [← AlgHom.coe_comp]; congr 1; ext
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq'` / 引理 `Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq'`
...
lemma Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq'
  ...
  | h n IH =>
    have : IsArtinianRing (p.ResidueField otimes[R] S) := IsArtinianRing.of_finite p.ResidueField _
...
private lemma Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq'
  ...
  | h n IH =>
    ...
    have : (P.primesOver (R' otimes[R] S ⧸ Ideal.span {e})).ncard < n + 1 := by
      ...
      rw [← h]; rw [← Nat.card_coe_set_eq]; rw [← Nat.card_coe_set_eq]; rw [Nat.card_congr F.toEquiv]
    obtain ⟨R'', _, _, _, Q, _, _, n, e' : _ -> R'' otimes[R'] (R' otimes[R] S ⧸ Ideal.span {e}),
      he', Q' : _ -> Ideal (R'' otimes[R'] (R' otimes[R] S ⧸ Ideal.span {e})), _, _, hPQ, hQ', H'⟩ :=
      IH _ this (R := R') (S := R' otimes[R] S ⧸ Ideal.span {e}) P rfl
    let : Algebra R R'' := .compHom _ (algebraMap R R')
    have : IsScalarTower R R' R'' := .of_algebraMap_eq' rfl
    let φ := Algebra.TensorProduct.map (Algebra.ofId R' R'') (AlgHom.id R S)
    let e₁ : R'' otimes[R'] (R' otimes[R] S ⧸ Ideal.span {e}) ≃ₐ[R''] (R'' otimes[R] S ⧸ Ideal.span {φ e}) :=
    ...
    have : (P''.map (Ideal.Quotient.mk (.span {φ e}))).LiesOver Q := ⟨by
      ...
      rw [← Ideal.under_under (B := R'' otimes[R] S)]
...
lemma Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq
  ...
  let e₁ : R' otimes[R] S ≃ₐ[R'] R' otimes[R] ULift.{u} S :=
...
Depends on / 依赖: Algebra, Algebra.TensorProduct.congr, TensorProduct, ULift.algEquiv.symm, algEquiv, exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq, he.map, otimes, symm.toAlgHom, toAlgHom
-/
lemma Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq
