...
@[instance_reducible]
/-
**CliffordAlgebra.invertible** 是 Mathlib 中的一个定义，位于命名空间 `CliffordAlgebra`。
...
/-
**CliffordAlgebra.invOf_** 是 Mathlib 中的一个定理，位于命名空间 `CliffordAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
theorem invOf_ι (m : M) [Invertible (Q m)] [Invertible (ι Q m)] :
  ...
  convert! (rfl : ⅟(ι Q m) = _)
/-
**CliffordAlgebra.isUnit_** 是 Mathlib 中的一个定理，位于命名空间 `CliffordAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[instance_reducible]
/-
**CliffordAlgebra.invertibleOfInvertible** 是 Mathlib 中的一个定义，位于命名空间 `CliffordAlge
bra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CliffordAlgebra.isUnit_of_isUnit_** 是 Mathlib 中的一个定理，位于命名空间 `CliffordAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
