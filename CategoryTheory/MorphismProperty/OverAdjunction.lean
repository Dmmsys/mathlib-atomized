...
/--
Definition of `Over.map` / `Over.map` 的定义

English:
definition Over.map
  signature: {f : X ⟶ Y} (hPf : P f)
  body: Comma.mapRight _ (Discrete.natTrans fun _ => f) fun X => P.comp_mem _ _ X.prop hPf

中文:
定义 Over.map
  签名: {f : X ⟶ Y} (hPf : P f)
  定义体: Comma.mapRight _ (Discrete.natTrans fun _ => f) fun X => P.comp_mem _ _ X.prop hPf

Depends on / 依赖: Comma.mapRight, Discrete, Discrete.natTrans, P.comp_mem, X.prop, comp_mem, mapRight, natTrans
-/
...
/--
lemma `Over.map_comp` / 引理 `Over.map_comp`

English:
lemma Over.map_comp
  given: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g)
  proof: by
  fapply Functor.ext
...
Depends on / 依赖: CategoryTheory, CategoryTheory.Comma.mapRight, Comma.lift, Comma.mapRight, Functor, Functor.ext, fapply, mapRight
-/
...
@[simps!]
/--
Definition of `Over.mapCongr` / `Over.mapCongr` 的定义

English:
definition Over.mapCongr
  signature: [Q.RespectsIso] {X Y : T} {f g : X ⟶ Y} (hfg : f = g) (hf : P f)
  body: NatIso.ofComponents (fun Y => Over.isoMk (Iso.refl _))

中文:
定义 Over.mapCongr
  签名: [Q.RespectsIso] {X Y : T} {f g : X ⟶ Y} (hfg : f = g) (hf : P f)
...
@[simps!]
/--
Definition of `Over.mapId` / `Over.mapId` 的定义

English:
definition Over.mapId
  signature: [P.IsMultiplicative] [Q.RespectsIso] (X : T) (f : X ⟶ X := 𝟙 X)
  body: NatIso.ofComponents (fun Y => Over.isoMk (Iso.refl _))
...
/--
Definition of `Over.mapComp` / `Over.mapComp` 的定义

English:
definition Over.mapComp
  signature: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g) [Q.RespectsIso]
  body: NatIso.ofComponents (fun X => Over.isoMk (Iso.refl _))

中文:
定义 Over.mapComp
  签名: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g) [Q.RespectsIso]
  定义体: NatIso.ofComponents (fun X => Over.isoMk (Iso.refl _))
...
@[simps! obj_left obj_hom map_left]
/--
Definition of `Over.pullback` / `Over.pullback` 的定义

English:
definition Over.pullback
  signature: (f : X ⟶ Y) [P.HasPullbacksAlong f]
  body: Over.mk Q (Limits.pullback.snd A.hom f)
...
Depends on / 依赖: A.hom, Limits, Limits.pullback.snd, Over.mk, pullback
-/
...
@[simps! hom_app_left inv_app_left]
/--
Definition of `Over.pullbackComp` / `Over.pullbackComp` 的定义

English:
definition Over.pullbackComp
  signature: (f : X ⟶ Y) (g : Y ⟶ Z)
  body: by subst hfg; infer_instance
...
set_option backward.defeqAttrib.useBackward true in
/--
lemma `Over.pullbackComp_left_fst_fst` / 引理 `Over.pullbackComp_left_fst_fst`

English:
lemma Over.pullbackComp_left_fst_fst
  statement: (f : X ⟶ Y) (g : Y ⟶ Z) [P.IsStableUnderBaseChangeAlong f]
  proof: by
  simp
...
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `Over.pullbackCongr` / `Over.pullbackCongr` 的定义

English:
definition Over.pullbackCongr
  signature: {f : X ⟶ Y} [P.HasPullbacksAlong f]
  body: by subst h; infer_instance
...
@[reassoc (attr := simp)]
/--
lemma `Over.pullbackCongr_hom_app_left_fst` / 引理 `Over.pullbackCongr_hom_app_left_fst`

English:
lemma Over.pullbackCongr_hom_app_left_fst
  statement: {f : X ⟶ Y} [P.HasPullbacksAlong f] {g : X ⟶ Y}
  proof: by subst h; infer_instance
...
Depends on / 依赖: A.hom, Over.pullbackCongr, hom.app, infer_instance, pullback, pullback.fst, pullbackCongr
-/
...
@[simps]
/--
Definition of `Over.pullbackMapHomPullback` / `Over.pullbackMapHomPullback` 的定义

English:
definition Over.pullbackMapHomPullback
  signature: [P.IsStableUnderComposition]
  body: by subst hfg; infer_instance
...
noncomputable
/--
Definition of `Over.pullbackCompForgetIso` / `Over.pullbackCompForgetIso` 的定义

English:
definition Over.pullbackCompForgetIso
  signature: {X Y : T} (f : X ⟶ Y) [HasPullbacksAlong f]
  body: Iso.refl _
...
/--
Definition of `Over.mapPullbackAdj` / `Over.mapPullbackAdj` 的定义

English:
definition Over.mapPullbackAdj
  signature: (f : X ⟶ Y) [P.HasPullbacksAlong f]
  body: Over.homMk (pullback.lift (𝟙 _) A.hom (by simp)) (by simp) by
...
/--
lemma `isRightAdjoint_pullback` / 引理 `isRightAdjoint_pullback`

English:
lemma isRightAdjoint_pullback
  statement: (f : X ⟶ Y) [P.HasPullbacksAlong f] [P.IsStableUnderBaseChangeAlong f]
  proof: (Over.mapPullbackAdj P ⊤ f hPf trivial).isRightAdjoint
...
/--
Definition of `Under.map` / `Under.map` 的定义

English:
definition Under.map
  signature: {f : X ⟶ Y} (hPf : P f)
  body: Comma.mapLeft _ (Discrete.natTrans fun _ => f) fun X => P.comp_mem _ _ hPf X.prop

中文:
定义 Under.map
  签名: {f : X ⟶ Y} (hPf : P f)
  定义体: Comma.mapLeft _ (Discrete.natTrans fun _ => f) fun X => P.comp_mem _ _ hPf X.prop

Depends on / 依赖: Comma.mapLeft, Discrete, Discrete.natTrans, P.comp_mem, X.prop, comp_mem, mapLeft, natTrans
-/
...
set_option backward.defeqAttrib.useBackward true in
/--
lemma `Under.map_comp` / 引理 `Under.map_comp`

English:
lemma Under.map_comp
  given: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g)
  proof: by
  fapply Functor.ext
...
Depends on / 依赖: CategoryTheory, CategoryTheory.Comma.mapLeft, Comma.lift, Comma.mapLeft, Functor, Functor.ext, fapply, mapLeft
-/
...
@[simps!]
/--
Definition of `Under.mapCongr` / `Under.mapCongr` 的定义

English:
definition Under.mapCongr
  signature: [Q.RespectsIso] {X Y : T} {f g : X ⟶ Y} (hfg : f = g) (hf : P f)
  body: NatIso.ofComponents (fun Y => Under.isoMk (Iso.refl _))
...
@[simps!]
/--
Definition of `Under.mapId` / `Under.mapId` 的定义

English:
definition Under.mapId
  signature: [P.IsMultiplicative] [Q.RespectsIso] (X : T) (f : X ⟶ X := 𝟙 X)
  body: NatIso.ofComponents (fun Y => Under.isoMk (Iso.refl _))
...
@[simps! hom_app_left]
/--
Definition of `Under.mapComp` / `Under.mapComp` 的定义

English:
definition Under.mapComp
  signature: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g) [Q.RespectsIso]
  body: NatIso.ofComponents (fun X => Under.isoMk (Iso.refl _))

中文:
定义 Under.mapComp
  签名: {f : X ⟶ Y} (hf : P f) {g : Y ⟶ Z} (hg : P g) [Q.RespectsIso]
  定义体: NatIso.ofComponents (fun X => Under.isoMk (Iso.refl _))
...
@[simps! obj_right obj_hom map_right]
/--
Definition of `Under.pushout` / `Under.pushout` 的定义

English:
definition Under.pushout
  signature: (f : X ⟶ Y) [P.HasPushoutsAlong f]
  body: Under.mk Q (Limits.pushout.inr A.hom f)
...
/--
Definition of `Under.pushoutComp` / `Under.pushoutComp` 的定义

English:
definition Under.pushoutComp
  signature: (f : X ⟶ Y) (g : Y ⟶ Z)
  body: by subst hfg; infer_instance
...
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `Under.pushoutCongr` / `Under.pushoutCongr` 的定义

English:
definition Under.pushoutCongr
  signature: {f : X ⟶ Y} [P.HasPushoutsAlong f]
  body: by subst h; infer_instance
...
@[reassoc (attr := simp)]
/--
lemma `Under.pushoutCongr_hom_app_left_fst` / 引理 `Under.pushoutCongr_hom_app_left_fst`

English:
lemma Under.pushoutCongr_hom_app_left_fst
  statement: {f : X ⟶ Y} [P.HasPushoutsAlong f] {g : X ⟶ Y}
  proof: by subst h; infer_instance
...
Depends on / 依赖: Under.pushoutCongr, hom.app, infer_instance, pushout, pushout.inl, pushoutCongr
-/
...
noncomputable
/--
Definition of `Under.pushoutCompForgetIso` / `Under.pushoutCompForgetIso` 的定义

English:
definition Under.pushoutCompForgetIso
  signature: {X Y : T} (f : X ⟶ Y) [HasPushoutsAlong f]
  body: Iso.refl _
...
@[simps! unit_app counit_app]
/--
Definition of `Under.mapPushoutAdj` / `Under.mapPushoutAdj` 的定义

English:
definition Under.mapPushoutAdj
  signature: (f : X ⟶ Y) [P.HasPushoutsAlong f]
  body: Under.homMk (pushout.inl _ _) pushout.condition (Q.pushout_inl _ _ hQf)
...
/--
lemma `isLeftAdjoint_pushout` / 引理 `isLeftAdjoint_pushout`

English:
lemma isLeftAdjoint_pushout
  statement: (f : X ⟶ Y) [P.HasPushoutsAlong f] [P.IsStableUnderCobaseChangeAlong f]
  proof: (Under.mapPushoutAdj P ⊤ f hPf trivial).isLeftAdjoint
