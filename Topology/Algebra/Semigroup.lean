...
/--
theorem `exists_idempotent_of_compact_t2_of_continuous_mul_left` / 定理 `exists_idempotent_of_compact_t2_of_continuous_mul_left`

English:
theorem exists_idempotent_of_compact_t2_of_continuous_mul_left
  statement: {M} [Nonempty M] [Semigroup M]
  proof: by
  ...
  let S : Set (Set M) :=
    { N | IsClosed N ∧ N.Nonempty ∧ forall (m) (_ : m in N) (m') (_ : m' in N), m * m' in N }
  rsuffices ⟨N, hN⟩ : exists N', Minimal (· in S) N'
  · obtain ⟨N_closed, ⟨m, hm⟩, N_mul⟩ := hN.prop
    ...
    have absorbing_eq_self : N inter { m' | m' * m = m } = N := by
      apply hN.eq_of_subset
      · refine ⟨N_closed.inter ((T1Space.t1 m).preimage (continuous_const_mul m)), ?_, ?_⟩
        ...
        · rintro m'' ⟨mem'', eq'' : _ = m⟩ m' ⟨mem', eq' : _ = m⟩
          ...
          rw [Set.mem_ofPred_eq]; rw [mul_assoc]; rw [eq']; rw [eq'']
...
theorem exists_idempotent_of_compact_t2_of_continuous_mul_left {M} [Nonempty M] [Semigroup M]
    ...
    (continuous_const_mul : forall r : M, Continuous (· * r)) : exists m : M, m * m = m := by
  /- We apply Zorn's lemma to the poset of nonempty closed subsemigroups of `M`.
     It will turn out that any minimal element is `{m}` for an idempotent `m : M`. -/
  let S : Set (Set M) :=
    { N | IsClosed N ∧ N.Nonempty ∧ forall (m) (_ : m in N) (m') (_ : m' in N), m * m' in N }
...
/--
theorem `exists_idempotent_in_compact_subsemigroup` / 定理 `exists_idempotent_in_compact_subsemigroup`

English:
theorem exists_idempotent_in_compact_subsemigroup
  statement: {M} [Semigroup M] [TopologicalSpace M] [T2Space M]
  proof: by
  let M' := { m // m in s }
  let : Semigroup M' :=
  ...
  have : forall p : M', Continuous (· * p) := fun p =>
...
Depends on / 依赖: CompactSpace, Continuous, Nonempty, Semigroup, Subtype, Subtype.ext, continuous_const_mul, continuous_subtype_val, exists_idempotent_of_compact_t2_of_continuous_mul_left, isCompact_iff_compactSpace, isCompact_iff_compactSpace.mp, mul_assoc, nonempty_subtype, nonempty_subtype.mpr, s_add, s_compact, subtype_mk
-/
theorem exists_idempotent_in_compact_subsemigroup {M} [Semigroup M] [TopologicalSpace M] [T2Space M]
    (continuous_const_mul : forall r : M, Continuous (· * r)) (s : Set M) (snemp : s.Nonempty)
    (s_compact : IsCompact s) (s_add : forallᵉ (x in s) (y in s), x * y in s) :
    exists m in s, m * m = m := by
  let M' := { m // m in s }
  let : Semigroup M' :=
    { mul := fun p q => ⟨p.1 * q.1, s_add _ p.2 _ q.2⟩
      mul_assoc := fun p q r => Subtype.ext (mul_assoc _ _ _) }
