...
variable {A : Type w} [Category.{w'} A] [forall X, Limits.HasLimitsOfShape (StructuredArrow X G.op) A]
...
include K in
/--
lemma `isIso_ranCounit_app_of_isDenseSubsite` / 引理 `isIso_ranCounit_app_of_isDenseSubsite`

English:
lemma isIso_ranCounit_app_of_isDenseSubsite
  given: (Y : Sheaf J A) (U X)
  proof: by
  ...
  · intro f₁ f₂ e
    ...
    refine (Y.2 X _ (IsDenseSubsite.imageSieve_mem J K G g)).isSeparatedFo
...
Depends on / 依赖: Category, Category.assoc, G.obj, G.op, IsDenseSubsite, IsDenseSubsite.imageSieve_mem, NatTrans, NatTrans.naturality, g.mk_surjective, hom_ext, iVU.op, imageSieve_mem, isIso_iff_bijective, isPointwiseRightKanExtensionRanCounit, isSeparatedFor, isSeparatedFor.ext, mk_surjective, naturality
-/
...
English:
instance :
  signature: (G.sheafPushforwardContinuous A J K).IsEquivalence
  body: (G.sheafAdjunctionCocontinuous A J K).toEquivalence.isEquivalence_functor
