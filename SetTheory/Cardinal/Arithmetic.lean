/-
Copyright (c) 2017 Johannes Hölzl. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Johannes Hölzl, Mario Carneiro, Floris van Doorn
-/
module

public import Mathlib.SetTheory.Cardinal.Aleph

/-!
# Cardinal arithmetic

Arithmetic operations on cardinals are defined in `Mathlib/SetTheory/Cardinal/Order.lean`. However,
proving the important theorem `c * c = c` for infinite cardinals and its corollaries requires the
use of ordinal numbers. This is done within this file.

## Main statements

* `Cardinal.mul_eq_max` and `Cardinal.add_eq_max` state that the product (resp. sum) of two infinite
  cardinals is just their maximum. Several variations around this fact are also given.
* `Cardinal.mk_list_eq_mk`: when `α` is infinite, `α` and `List α` have the same cardinality.

## Tags

cardinal arithmetic (for infinite cardinals)
-/

public section

assert_not_exists Module Finsupp Ordinal.log

noncomputable section

open Function Set Cardinal Equiv Order Ordinal

universe u v w

namespace Cardinal

/-! ### Properties of `mul` -/
section mul

set_option backward.isDefEq.respectTransparency false in
/--
theorem `mul_eq_self` / 定理 `mul_eq_self`

English:
theorem mul_eq_self
  given: {c : Cardinal} (hc : ℵ₀ <= c)
  statement: c * c = c
  proof: by
  -- The only nontrivial part is `c * c ≤ c`. We prove it inductively.
  induction c using WellFoundedLT.induction with | ind c IH
  refine le_antisymm ?_ (by simpa using mul_le_mul_right (one_le_aleph0.trans hc) c)
  -- Consider the minimal well-order on `α` (a type with cardinality `c`).
  induction c using Cardinal.inductionOn with | mk α
  obtain ⟨_, _, hα⟩ := exists_ord_eq_type_lt α
  have : NoMaxOrder α := by
    rw [← isSuccPrelimit_type_lt_iff]; rw [← hα]
    exact (isSuccLimit_ord hc).isSuccPrelimit
  -- Define an order `s` on `α × α`, comparing first by `max x.1 x.2`, then by `toLex (x.1, x.2)`.
  let g : α × α -> α := uncurry max
  let f : α × α ↪ α ×ₗ (α ×ₗ α) := ⟨fun p => toLex (g p, toLex p), fun p q => congrArg Prod.snd⟩
  let s := f ⁻¹'o (· < ·)
  have : IsWellOrder _ s := (RelEmbedding.preimage ..).isWellOrder
  -- Every initial segment of `s` is contained in `β × β` for some `β` of cardinality `< c`.
  -- By the inductive hypothesis, this means `#(β × β) < c`. Thus, `α × α` must have
  -- cardinality `≤ c`.
refine @card_le_card (type s) (typeLT α) le_of_forall_lt fun o h => ?_
  obtain ⟨p, rfl⟩ := typein_surj s h
  obtain ⟨q, hq'⟩ := exists_gt (g p)
  rw [← hα]; rw [lt_ord]
  apply lt_of_le_of_lt (b := #(Iio q) * #(Iio q))
  · apply (Set.embeddingOfSubset { x | s x p } ..).cardinal_le.trans_eq (mk_setProd ..)
    simp [s, f, Prod.Lex.lt_iff, subset_def]
    grind
  rcases lt_or_ge #(Iio q) ℵ₀ with hq | hq
  · exact (mul_lt_aleph0 hq hq).trans_le hc
  · have := mk_Iio_lt q hα
    rwa [IH _ this hq]

中文:
定理 mul_eq_self
  条件: {c : 基数} (hc : ℵ₀ <= c)
  结论: c * c = c
  证明: by
  -- The only nontrivial part is `c * c ≤ c`. We prove it inductively.
  induction c using WellFoundedLT.induction with | ind c IH
  refine le_antisymm ?_ (by simpa using mul_le_mul_right (one_le_aleph0.trans hc) c)
  -- Consider the minimal well-order on `α` (a type with cardinality `c`).
  induction c using Cardinal.inductionOn with | mk α
  obtain ⟨_, _, hα⟩ := exists_ord_eq_type_lt α
  have : NoMaxOrder α := by
    rw [← isSuccPrelimit_type_lt_iff]; rw [← hα]
    exact (isSuccLimit_ord hc).isSuccPrelimit
  -- Define an order `s` on `α × α`, comparing first by `max x.1 x.2`, then by `toLex (x.1, x.2)`.
  let g : α × α -> α := uncurry max
  let f : α × α ↪ α ×ₗ (α ×ₗ α) := ⟨fun p => toLex (g p, toLex p), fun p q => congrArg Prod.snd⟩
  let s := f ⁻¹'o (· < ·)
  have : IsWellOrder _ s := (RelEmbedding.preimage ..).isWellOrder
  -- Every initial segment of `s` is contained in `β × β` for some `β` of cardinality `< c`.
  -- By the inductive hypothesis, this means `#(β × β) < c`. Thus, `α × α` must have
  -- cardinality `≤ c`.
refine @card_le_card (type s) (typeLT α) le_of_forall_lt fun o h => ?_
  obtain ⟨p, rfl⟩ := typein_surj s h
  obtain ⟨q, hq'⟩ := exists_gt (g p)
  rw [← hα]; rw [lt_ord]
  apply lt_of_le_of_lt (b := #(Iio q) * #(Iio q))
  · apply (Set.embeddingOfSubset { x | s x p } ..).cardinal_le.trans_eq (mk_setProd ..)
    simp [s, f, Prod.Lex.lt_iff, subset_def]
    grind
  rcases lt_or_ge #(Iio q) ℵ₀ with hq | hq
  · exact (mul_lt_aleph0 hq hq).trans_le hc
  · have := mk_Iio_lt q hα
    rwa [IH _ this hq]
-/
theorem mul_eq_self {c : Cardinal} (hc : ℵ₀ <= c) : c * c = c := by
  -- The only nontrivial part is `c * c ≤ c`. We prove it inductively.
  induction c using WellFoundedLT.induction with | ind c IH
  refine le_antisymm ?_ (by simpa using mul_le_mul_right (one_le_aleph0.trans hc) c)
  -- Consider the minimal well-order on `α` (a type with cardinality `c`).
  induction c using Cardinal.inductionOn with | mk α
  obtain ⟨_, _, hα⟩ := exists_ord_eq_type_lt α
  have : NoMaxOrder α := by
    rw [← isSuccPrelimit_type_lt_iff]; rw [← hα]
    exact (isSuccLimit_ord hc).isSuccPrelimit
  -- Define an order `s` on `α × α`, comparing first by `max x.1 x.2`, then by `toLex (x.1, x.2)`.
  let g : α × α -> α := uncurry max
  let f : α × α ↪ α ×ₗ (α ×ₗ α) := ⟨fun p => toLex (g p, toLex p), fun p q => congrArg Prod.snd⟩
  let s := f ⁻¹'o (· < ·)
  have : IsWellOrder _ s := (RelEmbedding.preimage ..).isWellOrder
  -- Every initial segment of `s` is contained in `β × β` for some `β` of cardinality `< c`.
  -- By the inductive hypothesis, this means `#(β × β) < c`. Thus, `α × α` must have
  -- cardinality `≤ c`.
refine @card_le_card (type s) (typeLT α) le_of_forall_lt fun o h => ?_
  obtain ⟨p, rfl⟩ := typein_surj s h
  obtain ⟨q, hq'⟩ := exists_gt (g p)
  rw [← hα]; rw [lt_ord]
  apply lt_of_le_of_lt (b := #(Iio q) * #(Iio q))
  · apply (Set.embeddingOfSubset { x | s x p } ..).cardinal_le.trans_eq (mk_setProd ..)
    simp [s, f, Prod.Lex.lt_iff, subset_def]
    grind
  rcases lt_or_ge #(Iio q) ℵ₀ with hq | hq
  · exact (mul_lt_aleph0 hq hq).trans_le hc
  · have := mk_Iio_lt q hα
    rwa [IH _ this hq]

/--
theorem `mul_eq_max` / 定理 `mul_eq_max`

English:
theorem mul_eq_max
  given: {a b : Cardinal} (ha : ℵ₀ <= a) (hb : ℵ₀ <= b)
  statement: a * b = max a b
  proof: le_antisymm
      (mul_eq_self (ha.trans (le_max_left a b)) ▸
        mul_le_mul' (le_max_left _ _) (le_max_right _ _)) <|
    max_le (by simpa only [mul_one] using mul_le_mul_right (one_le_aleph0.trans hb) a)
      (by simpa only [one_mul] using mul_le_mul_left (one_le_aleph0.trans ha) b)

@[simp]

中文:
定理 mul_eq_max
  条件: {a b : 基数} (ha : ℵ₀ <= a) (hb : ℵ₀ <= b)
  结论: a * b = 最大值 a b
  证明: le_antisymm
      (mul_eq_self (ha.trans (le_max_left a b)) ▸
        mul_le_mul' (le_max_left _ _) (le_max_right _ _)) <|
    max_le (by simpa only [mul_one] using mul_le_mul_right (one_le_aleph0.trans hb) a)
      (by simpa only [one_mul] using mul_le_mul_left (one_le_aleph0.trans ha) b)

@[simp]

Depends on / 依赖: ha.trans, le_antisymm, le_max_left, le_max_right, max_le, mul_eq_self, mul_le_mul, mul_le_mul_left, mul_le_mul_right, mul_one, one_le_aleph0, one_le_aleph0.trans, one_mul
-/
theorem mul_eq_max {a b : Cardinal} (ha : ℵ₀ <= a) (hb : ℵ₀ <= b) : a * b = max a b :=
  le_antisymm
      (mul_eq_self (ha.trans (le_max_left a b)) ▸
        mul_le_mul' (le_max_left _ _) (le_max_right _ _)) <|
    max_le (by simpa only [mul_one] using mul_le_mul_right (one_le_aleph0.trans hb) a)
      (by simpa only [one_mul] using mul_le_mul_left (one_le_aleph0.trans ha) b)

@[simp]
/--
theorem `mul_mk_eq_max` / 定理 `mul_mk_eq_max`

English:
theorem mul_mk_eq_max
  given: {α β : Type u} [Infinite α] [Infinite β]
  statement: #α * #β = max #α #β
  proof: mul_eq_max (aleph0_le_mk α) (aleph0_le_mk β)

@[simp]

中文:
定理 mul_mk_eq_max
  条件: {α β : 类型u} [无限 α] [无限 β]
  结论: #α * #β = 最大值 #α #β
  证明: mul_eq_max (aleph0_le_mk α) (aleph0_le_mk β)

@[simp]

Depends on / 依赖: aleph0_le_mk, mul_eq_max
-/
theorem mul_mk_eq_max {α β : Type u} [Infinite α] [Infinite β] : #α * #β = max #α #β :=
  mul_eq_max (aleph0_le_mk α) (aleph0_le_mk β)

@[simp]
/--
theorem `aleph_mul_aleph` / 定理 `aleph_mul_aleph`

English:
theorem aleph_mul_aleph
  given: (o₁ o₂ : Ordinal)
  statement: ℵ_ o₁ * ℵ_ o₂ = ℵ_ (max o₁ o₂)
  proof: by
  rw [Cardinal.mul_eq_max (aleph0_le_aleph o₁) (aleph0_le_aleph o₂)]; rw [aleph_max]

@[simp]

中文:
定理 aleph_mul_aleph
  条件: (o₁ o₂ : 序数)
  结论: ℵ_ o₁ * ℵ_ o₂ = ℵ_ (最大值 o₁ o₂)
  证明: by
  rw [Cardinal.mul_eq_max (aleph0_le_aleph o₁) (aleph0_le_aleph o₂)]; rw [aleph_max]

@[simp]

Depends on / 依赖: Cardinal, Cardinal.mul_eq_max, aleph0_le_aleph, aleph_max, mul_eq_max
-/
theorem aleph_mul_aleph (o₁ o₂ : Ordinal) : ℵ_ o₁ * ℵ_ o₂ = ℵ_ (max o₁ o₂) := by
  rw [Cardinal.mul_eq_max (aleph0_le_aleph o₁) (aleph0_le_aleph o₂)]; rw [aleph_max]

@[simp]
/--
theorem `aleph0_mul_eq` / 定理 `aleph0_mul_eq`

English:
theorem aleph0_mul_eq
  given: {a : Cardinal} (ha : ℵ₀ <= a)
  statement: ℵ₀ * a = a
  proof: (mul_eq_max le_rfl ha).trans (max_eq_right ha)

@[simp]

中文:
定理 aleph0_mul_eq
  条件: {a : 基数} (ha : ℵ₀ <= a)
  结论: ℵ₀ * a = a
  证明: (mul_eq_max le_rfl ha).trans (max_eq_right ha)

@[simp]

Depends on / 依赖: le_rfl, max_eq_right, mul_eq_max
-/
theorem aleph0_mul_eq {a : Cardinal} (ha : ℵ₀ <= a) : ℵ₀ * a = a :=
  (mul_eq_max le_rfl ha).trans (max_eq_right ha)

@[simp]
/--
theorem `mul_aleph0_eq` / 定理 `mul_aleph0_eq`

English:
theorem mul_aleph0_eq
  given: {a : Cardinal} (ha : ℵ₀ <= a)
  statement: a * ℵ₀ = a
  proof: (mul_eq_max ha le_rfl).trans (max_eq_left ha)

中文:
定理 mul_aleph0_eq
  条件: {a : 基数} (ha : ℵ₀ <= a)
  结论: a * ℵ₀ = a
  证明: (mul_eq_max ha le_rfl).trans (max_eq_left ha)

Depends on / 依赖: le_rfl, max_eq_left, mul_eq_max
-/
theorem mul_aleph0_eq {a : Cardinal} (ha : ℵ₀ <= a) : a * ℵ₀ = a :=
  (mul_eq_max ha le_rfl).trans (max_eq_left ha)

/--
theorem `aleph0_mul_mk_eq` / 定理 `aleph0_mul_mk_eq`

English:
theorem aleph0_mul_mk_eq
  given: {α : Type*} [Infinite α]
  statement: ℵ₀ * #α = #α
  proof: aleph0_mul_eq (aleph0_le_mk α)

中文:
定理 aleph0_mul_mk_eq
  条件: {α : 类型} [无限 α]
  结论: ℵ₀ * #α = #α
  证明: aleph0_mul_eq (aleph0_le_mk α)

Depends on / 依赖: aleph0_le_mk, aleph0_mul_eq
-/
theorem aleph0_mul_mk_eq {α : Type*} [Infinite α] : ℵ₀ * #α = #α :=
  aleph0_mul_eq (aleph0_le_mk α)

/--
theorem `mk_mul_aleph0_eq` / 定理 `mk_mul_aleph0_eq`

English:
theorem mk_mul_aleph0_eq
  given: {α : Type*} [Infinite α]
  statement: #α * ℵ₀ = #α
  proof: mul_aleph0_eq (aleph0_le_mk α)

中文:
定理 mk_mul_aleph0_eq
  条件: {α : 类型} [无限 α]
  结论: #α * ℵ₀ = #α
  证明: mul_aleph0_eq (aleph0_le_mk α)

Depends on / 依赖: aleph0_le_mk, mul_aleph0_eq
-/
theorem mk_mul_aleph0_eq {α : Type*} [Infinite α] : #α * ℵ₀ = #α :=
  mul_aleph0_eq (aleph0_le_mk α)

/--
theorem `aleph0_mul_aleph` / 定理 `aleph0_mul_aleph`

English:
theorem aleph0_mul_aleph
  given: (o : Ordinal)
  statement: ℵ₀ * ℵ_ o = ℵ_ o
  proof: by
  simp

中文:
定理 aleph0_mul_aleph
  条件: (o : 序数)
  结论: ℵ₀ * ℵ_ o = ℵ_ o
  证明: by
  simp
-/
theorem aleph0_mul_aleph (o : Ordinal) : ℵ₀ * ℵ_ o = ℵ_ o := by
  simp

/--
theorem `aleph_mul_aleph0` / 定理 `aleph_mul_aleph0`

English:
theorem aleph_mul_aleph0
  given: (o : Ordinal)
  statement: ℵ_ o * ℵ₀ = ℵ_ o
  proof: by
  simp

中文:
定理 aleph_mul_aleph0
  条件: (o : 序数)
  结论: ℵ_ o * ℵ₀ = ℵ_ o
  证明: by
  simp
-/
theorem aleph_mul_aleph0 (o : Ordinal) : ℵ_ o * ℵ₀ = ℵ_ o := by
  simp

/--
theorem `mul_le_of_le` / 定理 `mul_le_of_le`

English:
theorem mul_le_of_le
  given: {a b c : Cardinal} (hc : ℵ₀ <= c) (ha : a <= c) (hb : b <= c)
  statement: a * b <= c
  proof: by
  rw [← Cardinal.mul_eq_self hc]
  exact mul_le_mul' ha hb

中文:
定理 mul_le_of_le
  条件: {a b c : 基数} (hc : ℵ₀ <= c) (ha : a <= c) (hb : b <= c)
  结论: a * b <= c
  证明: by
  rw [← Cardinal.mul_eq_self hc]
  exact mul_le_mul' ha hb

Depends on / 依赖: Cardinal, Cardinal.mul_eq_self, mul_eq_self, mul_le_mul
-/
theorem mul_le_of_le {a b c : Cardinal} (hc : ℵ₀ <= c) (ha : a <= c) (hb : b <= c) : a * b <= c := by
  rw [← Cardinal.mul_eq_self hc]
  exact mul_le_mul' ha hb

/--
theorem `mul_lt_of_lt` / 定理 `mul_lt_of_lt`

English:
theorem mul_lt_of_lt
  given: {a b c : Cardinal} (hc : ℵ₀ <= c) (ha : a < c) (hb : b < c)
  statement: a * b < c
  proof: by
  apply (mul_le_mul' (le_max_left a b) (le_max_right a b)).trans_lt
  obtain h | h := lt_or_ge (max a b) ℵ₀
  · exact (mul_lt_aleph0 h h).trans_le hc
  · rw [mul_eq_self h]
    exact max_lt ha hb

中文:
定理 mul_lt_of_lt
  条件: {a b c : 基数} (hc : ℵ₀ <= c) (ha : a < c) (hb : b < c)
  结论: a * b < c
  证明: by
  apply (mul_le_mul' (le_max_left a b) (le_max_right a b)).trans_lt
  obtain h | h := lt_or_ge (max a b) ℵ₀
  · exact (mul_lt_aleph0 h h).trans_le hc
  · rw [mul_eq_self h]
    exact max_lt ha hb

Depends on / 依赖: le_max_left, le_max_right, lt_or_ge, max_lt, mul_eq_self, mul_le_mul, mul_lt_aleph0, trans_le, trans_lt
-/
theorem mul_lt_of_lt {a b c : Cardinal} (hc : ℵ₀ <= c) (ha : a < c) (hb : b < c) : a * b < c := by
  apply (mul_le_mul' (le_max_left a b) (le_max_right a b)).trans_lt
  obtain h | h := lt_or_ge (max a b) ℵ₀
  · exact (mul_lt_aleph0 h h).trans_le hc
  · rw [mul_eq_self h]
    exact max_lt ha hb

/--
theorem `mul_le_max_of_aleph0_le_left` / 定理 `mul_le_max_of_aleph0_le_left`

English:
theorem mul_le_max_of_aleph0_le_left
  given: {a b : Cardinal} (h : ℵ₀ <= a)
  statement: a * b <= max a b
  proof: by
  convert! mul_le_mul' (le_max_left a b) (le_max_right a b) using 1
  rw [mul_eq_self]
  exact h.trans (le_max_left a b)

中文:
定理 mul_le_max_of_aleph0_le_left
  条件: {a b : 基数} (h : ℵ₀ <= a)
  结论: a * b <= 最大值 a b
  证明: by
  convert! mul_le_mul' (le_max_left a b) (le_max_right a b) using 1
  rw [mul_eq_self]
  exact h.trans (le_max_left a b)

Depends on / 依赖: convert, h.trans, le_max_left, le_max_right, mul_eq_self, mul_le_mul
-/
theorem mul_le_max_of_aleph0_le_left {a b : Cardinal} (h : ℵ₀ <= a) : a * b <= max a b := by
  convert! mul_le_mul' (le_max_left a b) (le_max_right a b) using 1
  rw [mul_eq_self]
  exact h.trans (le_max_left a b)

/--
theorem `mul_eq_max_of_aleph0_le_left` / 定理 `mul_eq_max_of_aleph0_le_left`

English:
theorem mul_eq_max_of_aleph0_le_left
  given: {a b : Cardinal} (h : ℵ₀ <= a) (h' : b != 0)
  proof: by
  rcases le_or_gt ℵ₀ b with hb | hb
  · exact mul_eq_max h hb
  refine (mul_le_max_of_aleph0_le_left h).antisymm ?_
  have : b <= a := hb.le.trans h
  rw [max_eq_left this]
  convert! mul_le_mul_right (Cardinal.one_le_iff_ne_zero.mpr h') a
  rw [mul_one]

中文:
定理 mul_eq_max_of_aleph0_le_left
  条件: {a b : 基数} (h : ℵ₀ <= a) (h' : b != 0)
  证明: by
  rcases le_or_gt ℵ₀ b with hb | hb
  · exact mul_eq_max h hb
  refine (mul_le_max_of_aleph0_le_left h).antisymm ?_
  have : b <= a := hb.le.trans h
  rw [max_eq_left this]
  convert! mul_le_mul_right (Cardinal.one_le_iff_ne_zero.mpr h') a
  rw [mul_one]

Depends on / 依赖: Cardinal, Cardinal.one_le_iff_ne_zero.mpr, antisymm, convert, hb.le.trans, le_or_gt, max_eq_left, mul_eq_max, mul_le_max_of_aleph0_le_left, mul_le_mul_right, mul_one, one_le_iff_ne_zero
-/
theorem mul_eq_max_of_aleph0_le_left {a b : Cardinal} (h : ℵ₀ <= a) (h' : b != 0) :
    a * b = max a b := by
  rcases le_or_gt ℵ₀ b with hb | hb
  · exact mul_eq_max h hb
  refine (mul_le_max_of_aleph0_le_left h).antisymm ?_
  have : b <= a := hb.le.trans h
  rw [max_eq_left this]
  convert! mul_le_mul_right (Cardinal.one_le_iff_ne_zero.mpr h') a
  rw [mul_one]

/--
theorem `mul_le_max_of_aleph0_le_right` / 定理 `mul_le_max_of_aleph0_le_right`

English:
theorem mul_le_max_of_aleph0_le_right
  given: {a b : Cardinal} (h : ℵ₀ <= b)
  statement: a * b <= max a b
  proof: by
  simpa only [mul_comm b, max_comm b] using mul_le_max_of_aleph0_le_left h

中文:
定理 mul_le_max_of_aleph0_le_right
  条件: {a b : 基数} (h : ℵ₀ <= b)
  结论: a * b <= 最大值 a b
  证明: by
  simpa only [mul_comm b, max_comm b] using mul_le_max_of_aleph0_le_left h

Depends on / 依赖: max_comm, mul_comm, mul_le_max_of_aleph0_le_left
-/
theorem mul_le_max_of_aleph0_le_right {a b : Cardinal} (h : ℵ₀ <= b) : a * b <= max a b := by
  simpa only [mul_comm b, max_comm b] using mul_le_max_of_aleph0_le_left h

/--
theorem `mul_eq_max_of_aleph0_le_right` / 定理 `mul_eq_max_of_aleph0_le_right`

English:
theorem mul_eq_max_of_aleph0_le_right
  given: {a b : Cardinal} (h' : a != 0) (h : ℵ₀ <= b)
  proof: by
  rw [mul_comm]; rw [max_comm]
  exact mul_eq_max_of_aleph0_le_left h h'

中文:
定理 mul_eq_max_of_aleph0_le_right
  条件: {a b : 基数} (h' : a != 0) (h : ℵ₀ <= b)
  证明: by
  rw [mul_comm]; rw [max_comm]
  exact mul_eq_max_of_aleph0_le_left h h'

Depends on / 依赖: max_comm, mul_comm, mul_eq_max_of_aleph0_le_left
-/
theorem mul_eq_max_of_aleph0_le_right {a b : Cardinal} (h' : a != 0) (h : ℵ₀ <= b) :
    a * b = max a b := by
  rw [mul_comm]; rw [max_comm]
  exact mul_eq_max_of_aleph0_le_left h h'

/--
theorem `mul_eq_max'` / 定理 `mul_eq_max'`

English:
theorem mul_eq_max'
  given: {a b : Cardinal} (h : ℵ₀ <= a * b)
  statement: a * b = max a b
  proof: by
  rcases aleph0_le_mul_iff.mp h with ⟨ha, hb, ha' | hb'⟩
  · exact mul_eq_max_of_aleph0_le_left ha' hb
  · exact mul_eq_max_of_aleph0_le_right ha hb'

中文:
定理 mul_eq_max'
  条件: {a b : 基数} (h : ℵ₀ <= a * b)
  结论: a * b = 最大值 a b
  证明: by
  rcases aleph0_le_mul_iff.mp h with ⟨ha, hb, ha' | hb'⟩
  · exact mul_eq_max_of_aleph0_le_left ha' hb
  · exact mul_eq_max_of_aleph0_le_right ha hb'

Depends on / 依赖: aleph0_le_mul_iff, aleph0_le_mul_iff.mp, mul_eq_max_of_aleph0_le_left, mul_eq_max_of_aleph0_le_right
-/
theorem mul_eq_max' {a b : Cardinal} (h : ℵ₀ <= a * b) : a * b = max a b := by
  rcases aleph0_le_mul_iff.mp h with ⟨ha, hb, ha' | hb'⟩
  · exact mul_eq_max_of_aleph0_le_left ha' hb
  · exact mul_eq_max_of_aleph0_le_right ha hb'

/--
theorem `mul_le_max` / 定理 `mul_le_max`

English:
theorem mul_le_max
  given: (a b : Cardinal)
  statement: a * b <= max (max a b) ℵ₀
  proof: by
  rcases eq_or_ne a 0 with (rfl | ha0); · simp
  rcases eq_or_ne b 0 with (rfl | hb0); · simp
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [mul_eq_max_of_aleph0_le_left ha hb0]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [mul_comm, mul_eq_max_of_aleph0_le_left hb ha0, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (mul_lt_aleph0 ha hb).le

中文:
定理 mul_le_max
  条件: (a b : 基数)
  结论: a * b <= 最大值 (最大值 a b) ℵ₀
  证明: by
  rcases eq_or_ne a 0 with (rfl | ha0); · simp
  rcases eq_or_ne b 0 with (rfl | hb0); · simp
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [mul_eq_max_of_aleph0_le_left ha hb0]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [mul_comm, mul_eq_max_of_aleph0_le_left hb ha0, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (mul_lt_aleph0 ha hb).le

Depends on / 依赖: eq_or_ne, le_max_left, le_max_of_le_right, le_or_gt, max_comm, mul_comm, mul_eq_max_of_aleph0_le_left, mul_lt_aleph0
-/
theorem mul_le_max (a b : Cardinal) : a * b <= max (max a b) ℵ₀ := by
  rcases eq_or_ne a 0 with (rfl | ha0); · simp
  rcases eq_or_ne b 0 with (rfl | hb0); · simp
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [mul_eq_max_of_aleph0_le_left ha hb0]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [mul_comm, mul_eq_max_of_aleph0_le_left hb ha0, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (mul_lt_aleph0 ha hb).le

/--
theorem `mul_eq_left` / 定理 `mul_eq_left`

English:
theorem mul_eq_left
  given: {a b : Cardinal} (ha : ℵ₀ <= a) (hb : b <= a) (hb' : b != 0)
  statement: a * b = a
  proof: by
  rw [mul_eq_max_of_aleph0_le_left ha hb']; rw [max_eq_left hb]

中文:
定理 mul_eq_left
  条件: {a b : 基数} (ha : ℵ₀ <= a) (hb : b <= a) (hb' : b != 0)
  结论: a * b = a
  证明: by
  rw [mul_eq_max_of_aleph0_le_left ha hb']; rw [max_eq_left hb]

Depends on / 依赖: max_eq_left, mul_eq_max_of_aleph0_le_left
-/
theorem mul_eq_left {a b : Cardinal} (ha : ℵ₀ <= a) (hb : b <= a) (hb' : b != 0) : a * b = a := by
  rw [mul_eq_max_of_aleph0_le_left ha hb']; rw [max_eq_left hb]

/--
theorem `mul_eq_right` / 定理 `mul_eq_right`

English:
theorem mul_eq_right
  given: {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a <= b) (ha' : a != 0)
  statement: a * b = b
  proof: by
  rw [mul_comm]; rw [mul_eq_left hb ha ha']

中文:
定理 mul_eq_right
  条件: {a b : 基数} (hb : ℵ₀ <= b) (ha : a <= b) (ha' : a != 0)
  结论: a * b = b
  证明: by
  rw [mul_comm]; rw [mul_eq_left hb ha ha']

Depends on / 依赖: mul_comm, mul_eq_left
-/
theorem mul_eq_right {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a <= b) (ha' : a != 0) : a * b = b := by
  rw [mul_comm]; rw [mul_eq_left hb ha ha']

/--
theorem `le_mul_left` / 定理 `le_mul_left`

English:
theorem le_mul_left
  given: {a b : Cardinal} (h : b != 0)
  statement: a <= b * a
  proof: by
  convert! mul_le_mul_left (Cardinal.one_le_iff_ne_zero.mpr h) a
  rw [one_mul]

中文:
定理 le_mul_left
  条件: {a b : 基数} (h : b != 0)
  结论: a <= b * a
  证明: by
  convert! mul_le_mul_left (Cardinal.one_le_iff_ne_zero.mpr h) a
  rw [one_mul]

Depends on / 依赖: Cardinal, Cardinal.one_le_iff_ne_zero.mpr, convert, mul_le_mul_left, one_le_iff_ne_zero, one_mul
-/
theorem le_mul_left {a b : Cardinal} (h : b != 0) : a <= b * a := by
  convert! mul_le_mul_left (Cardinal.one_le_iff_ne_zero.mpr h) a
  rw [one_mul]

/--
theorem `le_mul_right` / 定理 `le_mul_right`

English:
theorem le_mul_right
  given: {a b : Cardinal} (h : b != 0)
  statement: a <= a * b
  proof: by
  rw [mul_comm]
  exact le_mul_left h

中文:
定理 le_mul_right
  条件: {a b : 基数} (h : b != 0)
  结论: a <= a * b
  证明: by
  rw [mul_comm]
  exact le_mul_left h

Depends on / 依赖: le_mul_left, mul_comm
-/
theorem le_mul_right {a b : Cardinal} (h : b != 0) : a <= a * b := by
  rw [mul_comm]
  exact le_mul_left h

/--
theorem `mul_eq_left_iff` / 定理 `mul_eq_left_iff`

English:
theorem mul_eq_left_iff
  given: {a b : Cardinal}
  statement: a * b = a ↔ max ℵ₀ b <= a ∧ b != 0 ∨ b = 1 ∨ a = 0
  proof: by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · have : a != 0 := by
        rintro rfl
        exact ha.not_gt aleph0_pos
      left
      rw [and_assoc]
      use ha
      constructor
      · rw [← not_lt]
        exact fun hb => ne_of_gt (hb.trans_le (le_mul_left this)) h
      · rintro rfl
        apply this
        rw [mul_zero] at h
        exact h.symm
    right
    by_cases h2a : a = 0
    · exact Or.inr h2a
    have hb : b != 0 := by
      rintro rfl
      apply h2a
      rw [mul_zero] at h
      exact h.symm
    left
    rw [← h]; rw [mul_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with (rfl | rfl | ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩)
    · contradiction
    · contradiction
    rw [← Ne] at h2a
    rw [← Cardinal.one_le_iff_ne_zero] at h2a hb
    norm_cast at h2a hb h ⊢
    apply le_antisymm _ hb
    rw [← not_lt]
    apply fun h2b => ne_of_gt _ h
    conv_rhs => left; rw [← mul_one n]
    rw [Nat.mul_lt_mul_left]
    · exact id
    apply Nat.lt_of_succ_le h2a
  · rintro (⟨⟨ha, hab⟩, hb⟩ | rfl | rfl)
    · rw [mul_eq_max_of_aleph0_le_left ha hb, max_eq_left hab]
    all_goals simp

中文:
定理 mul_eq_left_iff
  条件: {a b : 基数}
  结论: a * b = a ↔ 最大值 ℵ₀ b <= a ∧ b != 0 ∨ b = 1 ∨ a = 0
  证明: by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · have : a != 0 := by
        rintro rfl
        exact ha.not_gt aleph0_pos
      left
      rw [and_assoc]
      use ha
      constructor
      · rw [← not_lt]
        exact fun hb => ne_of_gt (hb.trans_le (le_mul_left this)) h
      · rintro rfl
        apply this
        rw [mul_zero] at h
        exact h.symm
    right
    by_cases h2a : a = 0
    · exact Or.inr h2a
    have hb : b != 0 := by
      rintro rfl
      apply h2a
      rw [mul_zero] at h
      exact h.symm
    left
    rw [← h]; rw [mul_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with (rfl | rfl | ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩)
    · contradiction
    · contradiction
    rw [← Ne] at h2a
    rw [← Cardinal.one_le_iff_ne_zero] at h2a hb
    norm_cast at h2a hb h ⊢
    apply le_antisymm _ hb
    rw [← not_lt]
    apply fun h2b => ne_of_gt _ h
    conv_rhs => left; rw [← mul_one n]
    rw [Nat.mul_lt_mul_left]
    · exact id
    apply Nat.lt_of_succ_le h2a
  · rintro (⟨⟨ha, hab⟩, hb⟩ | rfl | rfl)
    · rw [mul_eq_max_of_aleph0_le_left ha hb, max_eq_left hab]
    all_goals simp

Depends on / 依赖: Or.inr, aleph0_pos, and_assoc, h.symm, ha.not_gt, hb.trans_le, le_mul_left, le_or_gt, lt_aleph, lt_aleph0, max_le_iff, mul_lt_aleph0_iff, mul_zero, ne_of_gt, not_gt, not_lt, trans_le
-/
theorem mul_eq_left_iff {a b : Cardinal} : a * b = a ↔ max ℵ₀ b <= a ∧ b != 0 ∨ b = 1 ∨ a = 0 := by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · have : a != 0 := by
        rintro rfl
        exact ha.not_gt aleph0_pos
      left
      rw [and_assoc]
      use ha
      constructor
      · rw [← not_lt]
        exact fun hb => ne_of_gt (hb.trans_le (le_mul_left this)) h
      · rintro rfl
        apply this
        rw [mul_zero] at h
        exact h.symm
    right
    by_cases h2a : a = 0
    · exact Or.inr h2a
    have hb : b != 0 := by
      rintro rfl
      apply h2a
      rw [mul_zero] at h
      exact h.symm
    left
    rw [← h]; rw [mul_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with (rfl | rfl | ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩)
    · contradiction
    · contradiction
    rw [← Ne] at h2a
    rw [← Cardinal.one_le_iff_ne_zero] at h2a hb
    norm_cast at h2a hb h ⊢
    apply le_antisymm _ hb
    rw [← not_lt]
    apply fun h2b => ne_of_gt _ h
    conv_rhs => left; rw [← mul_one n]
    rw [Nat.mul_lt_mul_left]
    · exact id
    apply Nat.lt_of_succ_le h2a
  · rintro (⟨⟨ha, hab⟩, hb⟩ | rfl | rfl)
    · rw [mul_eq_max_of_aleph0_le_left ha hb, max_eq_left hab]
    all_goals simp

end mul

/-! ### Properties of `add` -/
section add

/--
theorem `add_eq_self` / 定理 `add_eq_self`

English:
theorem add_eq_self
  given: {c : Cardinal} (h : ℵ₀ <= c)
  statement: c + c = c
  proof: le_antisymm
    (by simpa [two_mul, mul_eq_self h] using mul_le_mul_left (natCast_le_aleph0 (n := 2).trans h) c)
    (self_le_add_left c c)

中文:
定理 add_eq_self
  条件: {c : 基数} (h : ℵ₀ <= c)
  结论: c + c = c
  证明: le_antisymm
    (by simpa [two_mul, mul_eq_self h] using mul_le_mul_left (natCast_le_aleph0 (n := 2).trans h) c)
    (self_le_add_left c c)

Depends on / 依赖: LocallyPathConnectedSpace, instLocallyPathConnectedSpace, le_antisymm, mul_eq_self, mul_le_mul_left, natCast_le_aleph0, self_le_add_left, two_mul
-/
theorem add_eq_self {c : Cardinal} (h : ℵ₀ <= c) : c + c = c :=
  le_antisymm
    (by simpa [two_mul, mul_eq_self h] using mul_le_mul_left (natCast_le_aleph0 (n := 2).trans h) c)
    (self_le_add_left c c)

/--
theorem `add_eq_max` / 定理 `add_eq_max`

English:
theorem add_eq_max
  given: {a b : Cardinal} (ha : ℵ₀ <= a)
  statement: a + b = max a b
  proof: le_antisymm
      (add_eq_self (ha.trans (le_max_left a b)) ▸
        add_le_add (le_max_left _ _) (le_max_right _ _)) <|
    max_le (self_le_add_right _ _) (self_le_add_left _ _)

中文:
定理 add_eq_max
  条件: {a b : 基数} (ha : ℵ₀ <= a)
  结论: a + b = 最大值 a b
  证明: le_antisymm
      (add_eq_self (ha.trans (le_max_left a b)) ▸
        add_le_add (le_max_left _ _) (le_max_right _ _)) <|
    max_le (self_le_add_right _ _) (self_le_add_left _ _)

Depends on / 依赖: add_eq_self, add_le_add, ha.trans, le_antisymm, le_max_left, le_max_right, max_le, self_le_add_left, self_le_add_right
-/
theorem add_eq_max {a b : Cardinal} (ha : ℵ₀ <= a) : a + b = max a b :=
  le_antisymm
      (add_eq_self (ha.trans (le_max_left a b)) ▸
        add_le_add (le_max_left _ _) (le_max_right _ _)) <|
    max_le (self_le_add_right _ _) (self_le_add_left _ _)

/--
theorem `add_eq_max'` / 定理 `add_eq_max'`

English:
theorem add_eq_max'
  given: {a b : Cardinal} (ha : ℵ₀ <= b)
  statement: a + b = max a b
  proof: by
  rw [add_comm]; rw [max_comm]; rw [add_eq_max ha]

@[simp]

中文:
定理 add_eq_max'
  条件: {a b : 基数} (ha : ℵ₀ <= b)
  结论: a + b = 最大值 a b
  证明: by
  rw [add_comm]; rw [max_comm]; rw [add_eq_max ha]

@[simp]

Depends on / 依赖: add_comm, add_eq_max, max_comm
-/
theorem add_eq_max' {a b : Cardinal} (ha : ℵ₀ <= b) : a + b = max a b := by
  rw [add_comm]; rw [max_comm]; rw [add_eq_max ha]

@[simp]
/--
theorem `add_mk_eq_max` / 定理 `add_mk_eq_max`

English:
theorem add_mk_eq_max
  given: {α β : Type u} [Infinite α]
  statement: #α + #β = max #α #β
  proof: add_eq_max (aleph0_le_mk α)

@[simp]

中文:
定理 add_mk_eq_max
  条件: {α β : 类型u} [无限 α]
  结论: #α + #β = 最大值 #α #β
  证明: add_eq_max (aleph0_le_mk α)

@[simp]

Depends on / 依赖: add_eq_max, aleph0_le_mk
-/
theorem add_mk_eq_max {α β : Type u} [Infinite α] : #α + #β = max #α #β :=
  add_eq_max (aleph0_le_mk α)

@[simp]
/--
theorem `add_mk_eq_max'` / 定理 `add_mk_eq_max'`

English:
theorem add_mk_eq_max'
  given: {α β : Type u} [Infinite β]
  statement: #α + #β = max #α #β
  proof: add_eq_max' (aleph0_le_mk β)

中文:
定理 add_mk_eq_max'
  条件: {α β : 类型u} [无限 β]
  结论: #α + #β = 最大值 #α #β
  证明: add_eq_max' (aleph0_le_mk β)

Depends on / 依赖: add_eq_max, aleph0_le_mk
-/
theorem add_mk_eq_max' {α β : Type u} [Infinite β] : #α + #β = max #α #β :=
  add_eq_max' (aleph0_le_mk β)

/--
theorem `add_mk_eq_self` / 定理 `add_mk_eq_self`

English:
theorem add_mk_eq_self
  given: {α : Type*} [Infinite α]
  statement: #α + #α = #α
  proof: by
  simp

中文:
定理 add_mk_eq_self
  条件: {α : 类型} [无限 α]
  结论: #α + #α = #α
  证明: by
  simp
-/
theorem add_mk_eq_self {α : Type*} [Infinite α] : #α + #α = #α := by
  simp

/--
theorem `add_le_max` / 定理 `add_le_max`

English:
theorem add_le_max
  given: (a b : Cardinal)
  statement: a + b <= max (max a b) ℵ₀
  proof: by
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [add_eq_max ha]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [add_comm, add_eq_max hb, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (add_lt_aleph0 ha hb).le

中文:
定理 add_le_max
  条件: (a b : 基数)
  结论: a + b <= 最大值 (最大值 a b) ℵ₀
  证明: by
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [add_eq_max ha]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [add_comm, add_eq_max hb, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (add_lt_aleph0 ha hb).le

Depends on / 依赖: add_comm, add_eq_max, add_lt_aleph0, le_max_left, le_max_of_le_right, le_or_gt, max_comm
-/
theorem add_le_max (a b : Cardinal) : a + b <= max (max a b) ℵ₀ := by
  rcases le_or_gt ℵ₀ a with ha | ha
  · rw [add_eq_max ha]
    exact le_max_left _ _
  · rcases le_or_gt ℵ₀ b with hb | hb
    · rw [add_comm, add_eq_max hb, max_comm]
      exact le_max_left _ _
    · exact le_max_of_le_right (add_lt_aleph0 ha hb).le

/--
theorem `add_le_of_le` / 定理 `add_le_of_le`

English:
theorem add_le_of_le
  given: {a b c : Cardinal} (hc : ℵ₀ <= c) (h1 : a <= c) (h2 : b <= c)
  statement: a + b <= c
  proof: (add_le_add h1 h2).trans le_of_eq add_eq_self hc

中文:
定理 add_le_of_le
  条件: {a b c : 基数} (hc : ℵ₀ <= c) (h1 : a <= c) (h2 : b <= c)
  结论: a + b <= c
  证明: (add_le_add h1 h2).trans le_of_eq add_eq_self hc

Depends on / 依赖: add_eq_self, add_le_add, le_of_eq
-/
theorem add_le_of_le {a b c : Cardinal} (hc : ℵ₀ <= c) (h1 : a <= c) (h2 : b <= c) : a + b <= c :=
(add_le_add h1 h2).trans le_of_eq add_eq_self hc

/--
theorem `add_lt_of_lt` / 定理 `add_lt_of_lt`

English:
theorem add_lt_of_lt
  given: {a b c : Cardinal} (hc : ℵ₀ <= c) (h1 : a < c) (h2 : b < c)
  statement: a + b < c
  proof: (add_le_add (le_max_left a b) (le_max_right a b)).trans_lt
    (lt_or_ge (max a b) ℵ₀).elim (fun h => (add_lt_aleph0 h h).trans_le hc) fun h => by
      rw [add_eq_self h]; exact max_lt h1 h2

中文:
定理 add_lt_of_lt
  条件: {a b c : 基数} (hc : ℵ₀ <= c) (h1 : a < c) (h2 : b < c)
  结论: a + b < c
  证明: (add_le_add (le_max_left a b) (le_max_right a b)).trans_lt
    (lt_or_ge (max a b) ℵ₀).elim (fun h => (add_lt_aleph0 h h).trans_le hc) fun h => by
      rw [add_eq_self h]; exact max_lt h1 h2

Depends on / 依赖: add_eq_self, add_le_add, add_lt_aleph0, le_max_left, le_max_right, lt_or_ge, max_lt, trans_le, trans_lt
-/
theorem add_lt_of_lt {a b c : Cardinal} (hc : ℵ₀ <= c) (h1 : a < c) (h2 : b < c) : a + b < c :=
(add_le_add (le_max_left a b) (le_max_right a b)).trans_lt
    (lt_or_ge (max a b) ℵ₀).elim (fun h => (add_lt_aleph0 h h).trans_le hc) fun h => by
      rw [add_eq_self h]; exact max_lt h1 h2

/--
theorem `add_one_lt_of_lt` / 定理 `add_one_lt_of_lt`

English:
theorem add_one_lt_of_lt
  given: {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a < b)
  statement: a + 1 < b
  proof: add_lt_of_lt hb ha (one_lt_aleph0.trans_le hb)

中文:
定理 add_one_lt_of_lt
  条件: {a b : 基数} (hb : ℵ₀ <= b) (ha : a < b)
  结论: a + 1 < b
  证明: add_lt_of_lt hb ha (one_lt_aleph0.trans_le hb)

Depends on / 依赖: add_lt_of_lt, one_lt_aleph0, one_lt_aleph0.trans_le, trans_le
-/
theorem add_one_lt_of_lt {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a < b) : a + 1 < b :=
  add_lt_of_lt hb ha (one_lt_aleph0.trans_le hb)

/--
theorem `one_add_lt_of_lt` / 定理 `one_add_lt_of_lt`

English:
theorem one_add_lt_of_lt
  given: {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a < b)
  statement: 1 + a < b
  proof: add_lt_of_lt hb (one_lt_aleph0.trans_le hb) ha

中文:
定理 one_add_lt_of_lt
  条件: {a b : 基数} (hb : ℵ₀ <= b) (ha : a < b)
  结论: 1 + a < b
  证明: add_lt_of_lt hb (one_lt_aleph0.trans_le hb) ha

Depends on / 依赖: add_lt_of_lt, one_lt_aleph0, one_lt_aleph0.trans_le, trans_le
-/
theorem one_add_lt_of_lt {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a < b) : 1 + a < b :=
  add_lt_of_lt hb (one_lt_aleph0.trans_le hb) ha

/--
theorem `eq_of_add_eq_of_aleph0_le` / 定理 `eq_of_add_eq_of_aleph0_le`

English:
theorem eq_of_add_eq_of_aleph0_le
  given: {a b c : Cardinal} (h : a + b = c) (ha : a < c) (hc : ℵ₀ <= c)
  proof: by
  apply le_antisymm
  · rw [← h]
    apply self_le_add_left
  rw [← not_lt]; intro hb
  have : a + b < c := add_lt_of_lt hc ha hb
  simp [h] at this

中文:
定理 eq_of_add_eq_of_aleph0_le
  条件: {a b c : 基数} (h : a + b = c) (ha : a < c) (hc : ℵ₀ <= c)
  证明: by
  apply le_antisymm
  · rw [← h]
    apply self_le_add_left
  rw [← not_lt]; intro hb
  have : a + b < c := add_lt_of_lt hc ha hb
  simp [h] at this

Depends on / 依赖: add_lt_of_lt, le_antisymm, not_lt, self_le_add_left
-/
theorem eq_of_add_eq_of_aleph0_le {a b c : Cardinal} (h : a + b = c) (ha : a < c) (hc : ℵ₀ <= c) :
    b = c := by
  apply le_antisymm
  · rw [← h]
    apply self_le_add_left
  rw [← not_lt]; intro hb
  have : a + b < c := add_lt_of_lt hc ha hb
  simp [h] at this

/--
theorem `add_eq_left` / 定理 `add_eq_left`

English:
theorem add_eq_left
  given: {a b : Cardinal} (ha : ℵ₀ <= a) (hb : b <= a)
  statement: a + b = a
  proof: by
  rw [add_eq_max ha]; rw [max_eq_left hb]

中文:
定理 add_eq_left
  条件: {a b : 基数} (ha : ℵ₀ <= a) (hb : b <= a)
  结论: a + b = a
  证明: by
  rw [add_eq_max ha]; rw [max_eq_left hb]

Depends on / 依赖: add_eq_max, max_eq_left
-/
theorem add_eq_left {a b : Cardinal} (ha : ℵ₀ <= a) (hb : b <= a) : a + b = a := by
  rw [add_eq_max ha]; rw [max_eq_left hb]

/--
theorem `add_eq_right` / 定理 `add_eq_right`

English:
theorem add_eq_right
  given: {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a <= b)
  statement: a + b = b
  proof: by
  rw [add_comm]; rw [add_eq_left hb ha]

中文:
定理 add_eq_right
  条件: {a b : 基数} (hb : ℵ₀ <= b) (ha : a <= b)
  结论: a + b = b
  证明: by
  rw [add_comm]; rw [add_eq_left hb ha]

Depends on / 依赖: add_comm, add_eq_left
-/
theorem add_eq_right {a b : Cardinal} (hb : ℵ₀ <= b) (ha : a <= b) : a + b = b := by
  rw [add_comm]; rw [add_eq_left hb ha]

/--
theorem `add_eq_left_iff` / 定理 `add_eq_left_iff`

English:
theorem add_eq_left_iff
  given: {a b : Cardinal}
  statement: a + b = a ↔ max ℵ₀ b <= a ∨ b = 0
  proof: by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · left
      use ha
      rw [← not_lt]
      apply fun hb => ne_of_gt _ h
      intro hb
      exact hb.trans_le (self_le_add_left b a)
    right
    rw [← h]; rw [add_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩
    norm_cast at h ⊢
    rw [← add_right_inj]; rw [h]; rw [add_zero]
  · rintro (⟨h1, h2⟩ | h3)
    · rw [add_eq_max h1, max_eq_left h2]
    · rw [h3, add_zero]

中文:
定理 add_eq_left_iff
  条件: {a b : 基数}
  结论: a + b = a ↔ 最大值 ℵ₀ b <= a ∨ b = 0
  证明: by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · left
      use ha
      rw [← not_lt]
      apply fun hb => ne_of_gt _ h
      intro hb
      exact hb.trans_le (self_le_add_left b a)
    right
    rw [← h]; rw [add_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩
    norm_cast at h ⊢
    rw [← add_right_inj]; rw [h]; rw [add_zero]
  · rintro (⟨h1, h2⟩ | h3)
    · rw [add_eq_max h1, max_eq_left h2]
    · rw [h3, add_zero]

Depends on / 依赖: add_eq_max, add_lt_aleph0_iff, add_right_inj, add_zero, hb.trans_le, le_or_gt, lt_aleph0, max_eq_left, max_le_iff, ne_of_gt, not_lt, self_le_add_left, trans_le
-/
theorem add_eq_left_iff {a b : Cardinal} : a + b = a ↔ max ℵ₀ b <= a ∨ b = 0 := by
  rw [max_le_iff]
  refine ⟨fun h => ?_, ?_⟩
  · rcases le_or_gt ℵ₀ a with ha | ha
    · left
      use ha
      rw [← not_lt]
      apply fun hb => ne_of_gt _ h
      intro hb
      exact hb.trans_le (self_le_add_left b a)
    right
    rw [← h]; rw [add_lt_aleph0_iff]; rw [lt_aleph0]; rw [lt_aleph0] at ha
    rcases ha with ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩
    norm_cast at h ⊢
    rw [← add_right_inj]; rw [h]; rw [add_zero]
  · rintro (⟨h1, h2⟩ | h3)
    · rw [add_eq_max h1, max_eq_left h2]
    · rw [h3, add_zero]

/--
theorem `add_eq_right_iff` / 定理 `add_eq_right_iff`

English:
theorem add_eq_right_iff
  given: {a b : Cardinal}
  statement: a + b = b ↔ max ℵ₀ a <= b ∨ a = 0
  proof: by
  rw [add_comm]; rw [add_eq_left_iff]

中文:
定理 add_eq_right_iff
  条件: {a b : 基数}
  结论: a + b = b ↔ 最大值 ℵ₀ a <= b ∨ a = 0
  证明: by
  rw [add_comm]; rw [add_eq_left_iff]

Depends on / 依赖: add_comm, add_eq_left_iff
-/
theorem add_eq_right_iff {a b : Cardinal} : a + b = b ↔ max ℵ₀ a <= b ∨ a = 0 := by
  rw [add_comm]; rw [add_eq_left_iff]

/--
theorem `add_nat_eq` / 定理 `add_nat_eq`

English:
theorem add_nat_eq
  given: {a : Cardinal} (n : Nat) (ha : ℵ₀ <= a)
  statement: a + n = a
  proof: add_eq_left ha (natCast_le_aleph0.trans ha)

中文:
定理 add_nat_eq
  条件: {a : 基数} (n : 自然数) (ha : ℵ₀ <= a)
  结论: a + n = a
  证明: add_eq_left ha (natCast_le_aleph0.trans ha)

Depends on / 依赖: add_eq_left, natCast_le_aleph0, natCast_le_aleph0.trans
-/
theorem add_nat_eq {a : Cardinal} (n : Nat) (ha : ℵ₀ <= a) : a + n = a :=
  add_eq_left ha (natCast_le_aleph0.trans ha)

/--
theorem `nat_add_eq` / 定理 `nat_add_eq`

English:
theorem nat_add_eq
  given: {a : Cardinal} (n : Nat) (ha : ℵ₀ <= a)
  statement: n + a = a
  proof: by
  rw [add_comm]; rw [add_nat_eq n ha]

中文:
定理 nat_add_eq
  条件: {a : 基数} (n : 自然数) (ha : ℵ₀ <= a)
  结论: n + a = a
  证明: by
  rw [add_comm]; rw [add_nat_eq n ha]

Depends on / 依赖: add_comm, add_nat_eq
-/
theorem nat_add_eq {a : Cardinal} (n : Nat) (ha : ℵ₀ <= a) : n + a = a := by
  rw [add_comm]; rw [add_nat_eq n ha]

/--
theorem `add_one_eq` / 定理 `add_one_eq`

English:
theorem add_one_eq
  given: {a : Cardinal} (ha : ℵ₀ <= a)
  statement: a + 1 = a
  proof: add_one_of_aleph0_le ha

中文:
定理 add_one_eq
  条件: {a : 基数} (ha : ℵ₀ <= a)
  结论: a + 1 = a
  证明: add_one_of_aleph0_le ha

Depends on / 依赖: add_one_of_aleph0_le
-/
theorem add_one_eq {a : Cardinal} (ha : ℵ₀ <= a) : a + 1 = a :=
  add_one_of_aleph0_le ha

/--
theorem `mk_add_one_eq` / 定理 `mk_add_one_eq`

English:
theorem mk_add_one_eq
  given: {α : Type*} [Infinite α]
  statement: #α + 1 = #α
  proof: add_one_eq (aleph0_le_mk α)

中文:
定理 mk_add_one_eq
  条件: {α : 类型} [无限 α]
  结论: #α + 1 = #α
  证明: add_one_eq (aleph0_le_mk α)

Depends on / 依赖: add_one_eq, aleph0_le_mk
-/
theorem mk_add_one_eq {α : Type*} [Infinite α] : #α + 1 = #α :=
  add_one_eq (aleph0_le_mk α)

/--
theorem `mk_Iic_lt` / 定理 `mk_Iic_lt`

English:
theorem mk_Iic_lt
  statement: {α : Type*} [LinearOrder α] [WellFoundedLT α] (i : α)
  proof: by
  rw [← Iio_insert]; rw [mk_insert self_notMem_Iio]
  exact add_one_lt_of_lt hα (mk_Iio_lt i h)

中文:
定理 mk_Iic_lt
  结论: {α : 类型} [线性序 α] [WellFoundedLT α] (i : α)
  证明: by
  rw [← Iio_insert]; rw [mk_insert self_notMem_Iio]
  exact add_one_lt_of_lt hα (mk_Iio_lt i h)

Depends on / 依赖: Iio_insert, add_one_lt_of_lt, mk_Iio_lt, mk_insert, self_notMem_Iio
-/
theorem mk_Iic_lt {α : Type*} [LinearOrder α] [WellFoundedLT α] (i : α)
    (h : ord #α = typeLT α) (hα : ℵ₀ <= #α) : #(Iic i) < #α := by
  rw [← Iio_insert]; rw [mk_insert self_notMem_Iio]
  exact add_one_lt_of_lt hα (mk_Iio_lt i h)

/--
theorem `mk_Ici_lt` / 定理 `mk_Ici_lt`

English:
theorem mk_Ici_lt
  statement: {α : Type*} [LinearOrder α] [WellFoundedGT α] (i : α)
  proof: mk_Iic_lt (OrderDual.toDual i) h hα

中文:
定理 mk_Ici_lt
  结论: {α : 类型} [线性序 α] [WellFoundedGT α] (i : α)
  证明: mk_Iic_lt (OrderDual.toDual i) h hα

Depends on / 依赖: OrderDual, OrderDual.toDual, mk_Iic_lt, toDual
-/
theorem mk_Ici_lt {α : Type*} [LinearOrder α] [WellFoundedGT α] (i : α)
    (h : ord #α = typeLT αᵒᵈ) (hα : ℵ₀ <= #α) : #(Ici i) < #α :=
  mk_Iic_lt (OrderDual.toDual i) h hα

/--
theorem `eq_of_add_eq_add_left` / 定理 `eq_of_add_eq_add_left`

English:
theorem eq_of_add_eq_add_left
  given: {a b c : Cardinal} (h : a + b = a + c) (ha : a < ℵ₀)
  proof: by
  rcases le_or_gt ℵ₀ b with hb | hb
  · have : a < b := ha.trans_le hb
    rw [add_eq_right hb this.le]; rw [eq_comm] at h
    rw [eq_of_add_eq_of_aleph0_le h this hb]
  · have hc : c < ℵ₀ := by
      rw [← not_le]
      intro hc
      apply lt_irrefl ℵ₀
      apply (hc.trans (self_le_add_left _ a)).trans_lt
      rw [← h]
      apply add_lt_aleph0 ha hb
    rw [lt_aleph0] at *
    rcases ha with ⟨n, rfl⟩
    rcases hb with ⟨m, rfl⟩
    rcases hc with ⟨k, rfl⟩
    norm_cast at h ⊢
    apply add_left_cancel h

中文:
定理 eq_of_add_eq_add_left
  条件: {a b c : 基数} (h : a + b = a + c) (ha : a < ℵ₀)
  证明: by
  rcases le_or_gt ℵ₀ b with hb | hb
  · have : a < b := ha.trans_le hb
    rw [add_eq_right hb this.le]; rw [eq_comm] at h
    rw [eq_of_add_eq_of_aleph0_le h this hb]
  · have hc : c < ℵ₀ := by
      rw [← not_le]
      intro hc
      apply lt_irrefl ℵ₀
      apply (hc.trans (self_le_add_left _ a)).trans_lt
      rw [← h]
      apply add_lt_aleph0 ha hb
    rw [lt_aleph0] at *
    rcases ha with ⟨n, rfl⟩
    rcases hb with ⟨m, rfl⟩
    rcases hc with ⟨k, rfl⟩
    norm_cast at h ⊢
    apply add_left_cancel h
-/
protected theorem eq_of_add_eq_add_left {a b c : Cardinal} (h : a + b = a + c) (ha : a < ℵ₀) :
    b = c := by
  rcases le_or_gt ℵ₀ b with hb | hb
  · have : a < b := ha.trans_le hb
    rw [add_eq_right hb this.le]; rw [eq_comm] at h
    rw [eq_of_add_eq_of_aleph0_le h this hb]
  · have hc : c < ℵ₀ := by
      rw [← not_le]
      intro hc
      apply lt_irrefl ℵ₀
      apply (hc.trans (self_le_add_left _ a)).trans_lt
      rw [← h]
      apply add_lt_aleph0 ha hb
    rw [lt_aleph0] at *
    rcases ha with ⟨n, rfl⟩
    rcases hb with ⟨m, rfl⟩
    rcases hc with ⟨k, rfl⟩
    norm_cast at h ⊢
    apply add_left_cancel h

/--
theorem `eq_of_add_eq_add_right` / 定理 `eq_of_add_eq_add_right`

English:
theorem eq_of_add_eq_add_right
  given: {a b c : Cardinal} (h : a + b = c + b) (hb : b < ℵ₀)
  proof: by
  rw [add_comm a b]; rw [add_comm c b] at h
  exact Cardinal.eq_of_add_eq_add_left h hb

中文:
定理 eq_of_add_eq_add_right
  条件: {a b c : 基数} (h : a + b = c + b) (hb : b < ℵ₀)
  证明: by
  rw [add_comm a b]; rw [add_comm c b] at h
  exact Cardinal.eq_of_add_eq_add_left h hb
-/
protected theorem eq_of_add_eq_add_right {a b c : Cardinal} (h : a + b = c + b) (hb : b < ℵ₀) :
    a = c := by
  rw [add_comm a b]; rw [add_comm c b] at h
  exact Cardinal.eq_of_add_eq_add_left h hb

end add

/--
theorem `exists_rel_mk_fibers_lt` / 定理 `exists_rel_mk_fibers_lt`

English:
theorem exists_rel_mk_fibers_lt
  given: (α : Type*) [Infinite α]
  proof: by
  obtain ⟨α, _, hα⟩ := exists_ord_eq_type_lt α
  refine ⟨LT.lt, fun x => ?_, fun y => mk_Iio_lt _ hα⟩
  simpa using! mk_Iic_lt _ hα (aleph0_le_mk _)

中文:
定理 存在_rel_mk_fibers_lt
  条件: (α : 类型) [无限 α]
  证明: by
  obtain ⟨α, _, hα⟩ := exists_ord_eq_type_lt α
  refine ⟨LT.lt, fun x => ?_, fun y => mk_Iio_lt _ hα⟩
  simpa using! mk_Iic_lt _ hα (aleph0_le_mk _)

Depends on / 依赖: LT.lt, aleph0_le_mk, exists_ord_eq_type_lt, mk_Iic_lt, mk_Iio_lt
-/
theorem exists_rel_mk_fibers_lt (α : Type*) [Infinite α] :
    exists r : α -> α -> Prop, (forall x, #{y // ¬ r x y} < #α) ∧ (forall y, #{x // r x y} < #α) := by
  obtain ⟨α, _, hα⟩ := exists_ord_eq_type_lt α
  refine ⟨LT.lt, fun x => ?_, fun y => mk_Iio_lt _ hα⟩
  simpa using! mk_Iic_lt _ hα (aleph0_le_mk _)

/-! ### Properties of `ciSup` -/
section ciSup

variable {ι : Type u} {ι' : Type w} (f : ι -> Cardinal.{v})

section add

variable [Nonempty ι] [Nonempty ι']

/--
theorem `ciSup_add` / 定理 `ciSup_add`

English:
theorem ciSup_add
  given: (hf : BddAbove (range f)) (c : Cardinal.{v})
  proof: by
  have (i : ι) : f i + c <= (⨆ i, f i) + c := by grw [le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · + c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [add_eq_max hs]; rw [max_le_iff]
  exact ⟨ciSup_mono bdd fun i => self_le_add_right _ c,
    (self_le_add_left _ _).trans (le_ciSup bdd <| Classical.arbitrary ι)⟩

中文:
定理 ciSup_add
  条件: (hf : BddAbove (range f)) (c : 基数.{v})
  证明: by
  have (i : ι) : f i + c <= (⨆ i, f i) + c := by grw [le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · + c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [add_eq_max hs]; rw [max_le_iff]
  exact ⟨ciSup_mono bdd fun i => self_le_add_right _ c,
    (self_le_add_left _ _).trans (le_ciSup bdd <| Classical.arbitrary ι)⟩
-/
protected theorem ciSup_add (hf : BddAbove (range f)) (c : Cardinal.{v}) :
    (⨆ i, f i) + c = ⨆ i, f i + c := by
  have (i : ι) : f i + c <= (⨆ i, f i) + c := by grw [le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · + c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [add_eq_max hs]; rw [max_le_iff]
  exact ⟨ciSup_mono bdd fun i => self_le_add_right _ c,
    (self_le_add_left _ _).trans (le_ciSup bdd <| Classical.arbitrary ι)⟩

/--
theorem `add_ciSup` / 定理 `add_ciSup`

English:
theorem add_ciSup
  given: (hf : BddAbove (range f)) (c : Cardinal.{v})
  proof: by
  rw [add_comm]; rw [Cardinal.ciSup_add f hf]; simp_rw [add_comm]

中文:
定理 add_ciSup
  条件: (hf : BddAbove (range f)) (c : 基数.{v})
  证明: by
  rw [add_comm]; rw [Cardinal.ciSup_add f hf]; simp_rw [add_comm]
-/
protected theorem add_ciSup (hf : BddAbove (range f)) (c : Cardinal.{v}) :
    c + (⨆ i, f i) = ⨆ i, c + f i := by
  rw [add_comm]; rw [Cardinal.ciSup_add f hf]; simp_rw [add_comm]

/--
theorem `ciSup_add_ciSup` / 定理 `ciSup_add_ciSup`

English:
theorem ciSup_add_ciSup
  statement: (hf : BddAbove (range f)) (g : ι' -> Cardinal.{v})
  proof: by
  simp_rw [Cardinal.ciSup_add f hf, Cardinal.add_ciSup g hg]

中文:
定理 ciSup_add_ciSup
  结论: (hf : BddAbove (range f)) (g : ι' -> 基数.{v})
  证明: by
  simp_rw [Cardinal.ciSup_add f hf, Cardinal.add_ciSup g hg]
-/
protected theorem ciSup_add_ciSup (hf : BddAbove (range f)) (g : ι' -> Cardinal.{v})
    (hg : BddAbove (range g)) :
    (⨆ i, f i) + (⨆ j, g j) = ⨆ (i) (j), f i + g j := by
  simp_rw [Cardinal.ciSup_add f hf, Cardinal.add_ciSup g hg]

end add

/--
theorem `ciSup_mul` / 定理 `ciSup_mul`

English:
theorem ciSup_mul
  given: (c : Cardinal.{v})
  statement: (⨆ i, f i) * c = ⨆ i, f i * c
  proof: by
  cases isEmpty_or_nonempty ι; · simp
  obtain rfl | h0 := eq_or_ne c 0; · simp
  by_cases hf : BddAbove (range f); swap
  · have hfc : ¬ BddAbove (range (f · * c)) := fun bdd => hf
      ⟨⨆ i, f i * c, forall_mem_range.mpr fun i => (le_mul_right h0).trans (le_ciSup bdd i)⟩
    simp [iSup, csSup_of_not_bddAbove, hf, hfc]
  have (i : ι) : f i * c <= (⨆ i, f i) * c := by grw [← le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · * c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [mul_eq_max_of_aleph0_le_left hs h0]; rw [max_le_iff]
  obtain ⟨i, hi⟩ := exists_lt_of_lt_ciSup' (one_lt_aleph0.trans_le hs)
  exact ⟨ciSup_mono bdd fun i => le_mul_right h0,
    (le_mul_left (zero_lt_one.trans hi).ne').trans (le_ciSup bdd i)⟩

中文:
定理 ciSup_mul
  条件: (c : 基数.{v})
  结论: (⨆ i, f i) * c = ⨆ i, f i * c
  证明: by
  cases isEmpty_or_nonempty ι; · simp
  obtain rfl | h0 := eq_or_ne c 0; · simp
  by_cases hf : BddAbove (range f); swap
  · have hfc : ¬ BddAbove (range (f · * c)) := fun bdd => hf
      ⟨⨆ i, f i * c, forall_mem_range.mpr fun i => (le_mul_right h0).trans (le_ciSup bdd i)⟩
    simp [iSup, csSup_of_not_bddAbove, hf, hfc]
  have (i : ι) : f i * c <= (⨆ i, f i) * c := by grw [← le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · * c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [mul_eq_max_of_aleph0_le_left hs h0]; rw [max_le_iff]
  obtain ⟨i, hi⟩ := exists_lt_of_lt_ciSup' (one_lt_aleph0.trans_le hs)
  exact ⟨ciSup_mono bdd fun i => le_mul_right h0,
    (le_mul_left (zero_lt_one.trans hi).ne').trans (le_ciSup bdd i)⟩
-/
protected theorem ciSup_mul (c : Cardinal.{v}) : (⨆ i, f i) * c = ⨆ i, f i * c := by
  cases isEmpty_or_nonempty ι; · simp
  obtain rfl | h0 := eq_or_ne c 0; · simp
  by_cases hf : BddAbove (range f); swap
  · have hfc : ¬ BddAbove (range (f · * c)) := fun bdd => hf
      ⟨⨆ i, f i * c, forall_mem_range.mpr fun i => (le_mul_right h0).trans (le_ciSup bdd i)⟩
    simp [iSup, csSup_of_not_bddAbove, hf, hfc]
  have (i : ι) : f i * c <= (⨆ i, f i) * c := by grw [← le_ciSup hf i]
  refine le_antisymm ?_ (ciSup_le' this)
  have bdd : BddAbove (range (f · * c)) := ⟨_, forall_mem_range.mpr this⟩
  obtain hs | hs := lt_or_ge (⨆ i, f i) ℵ₀
  · obtain ⟨i, hi⟩ := exists_eq_ciSup_of_not_isSuccLimit hf (not_isSuccLimit_of_lt_aleph0 hs)
    exact hi ▸ le_ciSup bdd i
  rw [mul_eq_max_of_aleph0_le_left hs h0]; rw [max_le_iff]
  obtain ⟨i, hi⟩ := exists_lt_of_lt_ciSup' (one_lt_aleph0.trans_le hs)
  exact ⟨ciSup_mono bdd fun i => le_mul_right h0,
    (le_mul_left (zero_lt_one.trans hi).ne').trans (le_ciSup bdd i)⟩

/--
theorem `mul_ciSup` / 定理 `mul_ciSup`

English:
theorem mul_ciSup
  given: (c : Cardinal.{v})
  statement: c * (⨆ i, f i) = ⨆ i, c * f i
  proof: by
  rw [mul_comm]; rw [Cardinal.ciSup_mul f]; simp_rw [mul_comm]

中文:
定理 mul_ciSup
  条件: (c : 基数.{v})
  结论: c * (⨆ i, f i) = ⨆ i, c * f i
  证明: by
  rw [mul_comm]; rw [Cardinal.ciSup_mul f]; simp_rw [mul_comm]
-/
protected theorem mul_ciSup (c : Cardinal.{v}) : c * (⨆ i, f i) = ⨆ i, c * f i := by
  rw [mul_comm]; rw [Cardinal.ciSup_mul f]; simp_rw [mul_comm]

/--
theorem `ciSup_mul_ciSup` / 定理 `ciSup_mul_ciSup`

English:
theorem ciSup_mul_ciSup
  given: (g : ι' -> Cardinal.{v})
  proof: by
  simp_rw [Cardinal.ciSup_mul f, Cardinal.mul_ciSup g]

中文:
定理 ciSup_mul_ciSup
  条件: (g : ι' -> 基数.{v})
  证明: by
  simp_rw [Cardinal.ciSup_mul f, Cardinal.mul_ciSup g]
-/
protected theorem ciSup_mul_ciSup (g : ι' -> Cardinal.{v}) :
    (⨆ i, f i) * (⨆ j, g j) = ⨆ (i) (j), f i * g j := by
  simp_rw [Cardinal.ciSup_mul f, Cardinal.mul_ciSup g]

/--
theorem `sum_eq_lift_iSup_of_lift_mk_le_lift_iSup` / 定理 `sum_eq_lift_iSup_of_lift_mk_le_lift_iSup`

English:
theorem sum_eq_lift_iSup_of_lift_mk_le_lift_iSup
  statement: [Small.{v} ι] {f : ι -> Cardinal.{v}} (hι : ℵ₀ <= #ι)
  proof: by
  rw [lift_iSup bddAbove_of_small] at h
  apply (lift_iSup_le_sum f).antisymm'
  convert! sum_le_lift_mk_mul_iSup_lift f
  rw [mul_eq_max (aleph0_le_lift.mpr hι) ((aleph0_le_lift.mpr hι).trans h)]; rw [max_eq_right h]; rw [lift_iSup bddAbove_of_small]

中文:
定理 sum_eq_lift_iSup_of_lift_mk_le_lift_iSup
  结论: [Small.{v} ι] {f : ι -> 基数.{v}} (hι : ℵ₀ <= #ι)
  证明: by
  rw [lift_iSup bddAbove_of_small] at h
  apply (lift_iSup_le_sum f).antisymm'
  convert! sum_le_lift_mk_mul_iSup_lift f
  rw [mul_eq_max (aleph0_le_lift.mpr hι) ((aleph0_le_lift.mpr hι).trans h)]; rw [max_eq_right h]; rw [lift_iSup bddAbove_of_small]

Depends on / 依赖: aleph0_le_lift, aleph0_le_lift.mpr, antisymm, bddAbove_of_small, convert, lift_iSup, lift_iSup_le_sum, max_eq_right, mul_eq_max, sum_le_lift_mk_mul_iSup_lift
-/
theorem sum_eq_lift_iSup_of_lift_mk_le_lift_iSup [Small.{v} ι] {f : ι -> Cardinal.{v}} (hι : ℵ₀ <= #ι)
    (h : lift.{v} #ι <= lift.{u} (⨆ i, f i)) : sum f = lift (⨆ i, f i) := by
  rw [lift_iSup bddAbove_of_small] at h
  apply (lift_iSup_le_sum f).antisymm'
  convert! sum_le_lift_mk_mul_iSup_lift f
  rw [mul_eq_max (aleph0_le_lift.mpr hι) ((aleph0_le_lift.mpr hι).trans h)]; rw [max_eq_right h]; rw [lift_iSup bddAbove_of_small]

/--
theorem `sum_eq_iSup_of_lift_mk_le_iSup` / 定理 `sum_eq_iSup_of_lift_mk_le_iSup`

English:
theorem sum_eq_iSup_of_lift_mk_le_iSup
  statement: {f : ι -> Cardinal.{max u v}} (hι : ℵ₀ <= #ι)
  proof: by
  rw [← lift_id'.{u]; rw [v} (iSup _)]
  apply sum_eq_lift_iSup_of_lift_mk_le_lift_iSup hι
  rwa [lift_umax, lift_id'.{u, v}]

中文:
定理 sum_eq_iSup_of_lift_mk_le_iSup
  结论: {f : ι -> 基数.{最大值 u v}} (hι : ℵ₀ <= #ι)
  证明: by
  rw [← lift_id'.{u]; rw [v} (iSup _)]
  apply sum_eq_lift_iSup_of_lift_mk_le_lift_iSup hι
  rwa [lift_umax, lift_id'.{u, v}]

Depends on / 依赖: lift_id, lift_umax, sum_eq_lift_iSup_of_lift_mk_le_lift_iSup
-/
theorem sum_eq_iSup_of_lift_mk_le_iSup {f : ι -> Cardinal.{max u v}} (hι : ℵ₀ <= #ι)
    (h : lift.{v} #ι <= ⨆ i, f i) : sum f = ⨆ i, f i := by
  rw [← lift_id'.{u]; rw [v} (iSup _)]
  apply sum_eq_lift_iSup_of_lift_mk_le_lift_iSup hι
  rwa [lift_umax, lift_id'.{u, v}]

/--
theorem `sum_eq_iSup_of_mk_le_iSup` / 定理 `sum_eq_iSup_of_mk_le_iSup`

English:
theorem sum_eq_iSup_of_mk_le_iSup
  given: {f : ι -> Cardinal.{u}} (hι : ℵ₀ <= #ι) (h : #ι <= iSup f)
  proof: sum_eq_iSup_of_lift_mk_le_iSup hι ((lift_id #ι).symm ▸ h)

中文:
定理 sum_eq_iSup_of_mk_le_iSup
  条件: {f : ι -> 基数.{u}} (hι : ℵ₀ <= #ι) (h : #ι <= iSup f)
  证明: sum_eq_iSup_of_lift_mk_le_iSup hι ((lift_id #ι).symm ▸ h)

Depends on / 依赖: lift_id, sum_eq_iSup_of_lift_mk_le_iSup
-/
theorem sum_eq_iSup_of_mk_le_iSup {f : ι -> Cardinal.{u}} (hι : ℵ₀ <= #ι) (h : #ι <= iSup f) :
    sum f = ⨆ i, f i :=
  sum_eq_iSup_of_lift_mk_le_iSup hι ((lift_id #ι).symm ▸ h)

end ciSup

/-! ### Properties of `aleph` -/
section aleph

@[simp]
/--
theorem `aleph_add_aleph` / 定理 `aleph_add_aleph`

English:
theorem aleph_add_aleph
  given: (o₁ o₂ : Ordinal)
  statement: ℵ_ o₁ + ℵ_ o₂ = ℵ_ (max o₁ o₂)
  proof: by
  rw [Cardinal.add_eq_max (aleph0_le_aleph o₁)]; rw [aleph_max]

中文:
定理 aleph_add_aleph
  条件: (o₁ o₂ : 序数)
  结论: ℵ_ o₁ + ℵ_ o₂ = ℵ_ (最大值 o₁ o₂)
  证明: by
  rw [Cardinal.add_eq_max (aleph0_le_aleph o₁)]; rw [aleph_max]

Depends on / 依赖: Cardinal, Cardinal.add_eq_max, add_eq_max, aleph0_le_aleph, aleph_max
-/
theorem aleph_add_aleph (o₁ o₂ : Ordinal) : ℵ_ o₁ + ℵ_ o₂ = ℵ_ (max o₁ o₂) := by
  rw [Cardinal.add_eq_max (aleph0_le_aleph o₁)]; rw [aleph_max]

/--
theorem `add_right_inj_of_lt_aleph0` / 定理 `add_right_inj_of_lt_aleph0`

English:
theorem add_right_inj_of_lt_aleph0
  given: {α β γ : Cardinal} (γ₀ : γ < aleph0)
  statement: α + γ = β + γ ↔ α = β
  proof: ⟨fun h => Cardinal.eq_of_add_eq_add_right h γ₀, fun h => congr_arg (· + γ) h⟩

@[simp]

中文:
定理 add_right_inj_of_lt_aleph0
  条件: {α β γ : 基数} (γ₀ : γ < aleph0)
  结论: α + γ = β + γ ↔ α = β
  证明: ⟨fun h => Cardinal.eq_of_add_eq_add_right h γ₀, fun h => congr_arg (· + γ) h⟩

@[simp]

Depends on / 依赖: Cardinal, Cardinal.eq_of_add_eq_add_right, congr_arg, eq_of_add_eq_add_right
-/
theorem add_right_inj_of_lt_aleph0 {α β γ : Cardinal} (γ₀ : γ < aleph0) : α + γ = β + γ ↔ α = β :=
  ⟨fun h => Cardinal.eq_of_add_eq_add_right h γ₀, fun h => congr_arg (· + γ) h⟩

@[simp]
/--
theorem `add_nat_inj` / 定理 `add_nat_inj`

English:
theorem add_nat_inj
  given: {α β : Cardinal} (n : Nat)
  statement: α + n = β + n ↔ α = β
  proof: add_right_inj_of_lt_aleph0 natCast_lt_aleph0

@[simp]

中文:
定理 add_nat_inj
  条件: {α β : 基数} (n : 自然数)
  结论: α + n = β + n ↔ α = β
  证明: add_right_inj_of_lt_aleph0 natCast_lt_aleph0

@[simp]

Depends on / 依赖: add_right_inj_of_lt_aleph0, natCast_lt_aleph0
-/
theorem add_nat_inj {α β : Cardinal} (n : Nat) : α + n = β + n ↔ α = β :=
  add_right_inj_of_lt_aleph0 natCast_lt_aleph0

@[simp]
/--
theorem `add_one_inj` / 定理 `add_one_inj`

English:
theorem add_one_inj
  given: {α β : Cardinal}
  statement: α + 1 = β + 1 ↔ α = β
  proof: add_right_inj_of_lt_aleph0 one_lt_aleph0

中文:
定理 add_one_inj
  条件: {α β : 基数}
  结论: α + 1 = β + 1 ↔ α = β
  证明: add_right_inj_of_lt_aleph0 one_lt_aleph0

Depends on / 依赖: add_right_inj_of_lt_aleph0, one_lt_aleph0
-/
theorem add_one_inj {α β : Cardinal} : α + 1 = β + 1 ↔ α = β :=
  add_right_inj_of_lt_aleph0 one_lt_aleph0

/--
theorem `add_le_add_iff_of_lt_aleph0` / 定理 `add_le_add_iff_of_lt_aleph0`

English:
theorem add_le_add_iff_of_lt_aleph0
  given: {α β γ : Cardinal} (γ₀ : γ < ℵ₀)
  proof: by
  refine ⟨fun h => ?_, fun h => by gcongr⟩
  contrapose h
  rw [not_le]; rw [lt_iff_le_and_ne]; rw [Ne] at h ⊢
  exact ⟨by grw [h.1], mt (add_right_inj_of_lt_aleph0 γ₀).1 h.2⟩

@[simp]

中文:
定理 add_le_add_iff_of_lt_aleph0
  条件: {α β γ : 基数} (γ₀ : γ < ℵ₀)
  证明: by
  refine ⟨fun h => ?_, fun h => by gcongr⟩
  contrapose h
  rw [not_le]; rw [lt_iff_le_and_ne]; rw [Ne] at h ⊢
  exact ⟨by grw [h.1], mt (add_right_inj_of_lt_aleph0 γ₀).1 h.2⟩

@[simp]

Depends on / 依赖: add_right_inj_of_lt_aleph0, contrapose, lt_iff_le_and_ne, not_le
-/
theorem add_le_add_iff_of_lt_aleph0 {α β γ : Cardinal} (γ₀ : γ < ℵ₀) :
    α + γ <= β + γ ↔ α <= β := by
  refine ⟨fun h => ?_, fun h => by gcongr⟩
  contrapose h
  rw [not_le]; rw [lt_iff_le_and_ne]; rw [Ne] at h ⊢
  exact ⟨by grw [h.1], mt (add_right_inj_of_lt_aleph0 γ₀).1 h.2⟩

@[simp]
/--
theorem `add_nat_le_add_nat_iff` / 定理 `add_nat_le_add_nat_iff`

English:
theorem add_nat_le_add_nat_iff
  given: {α β : Cardinal} (n : Nat)
  statement: α + n <= β + n ↔ α <= β
  proof: add_le_add_iff_of_lt_aleph0 natCast_lt_aleph0

@[simp]

中文:
定理 add_nat_le_add_nat_iff
  条件: {α β : 基数} (n : 自然数)
  结论: α + n <= β + n ↔ α <= β
  证明: add_le_add_iff_of_lt_aleph0 natCast_lt_aleph0

@[simp]

Depends on / 依赖: add_le_add_iff_of_lt_aleph0, natCast_lt_aleph0
-/
theorem add_nat_le_add_nat_iff {α β : Cardinal} (n : Nat) : α + n <= β + n ↔ α <= β :=
  add_le_add_iff_of_lt_aleph0 natCast_lt_aleph0

@[simp]
/--
theorem `add_one_le_add_one_iff` / 定理 `add_one_le_add_one_iff`

English:
theorem add_one_le_add_one_iff
  given: {α β : Cardinal}
  statement: α + 1 <= β + 1 ↔ α <= β
  proof: add_le_add_iff_of_lt_aleph0 one_lt_aleph0

中文:
定理 add_one_le_add_one_iff
  条件: {α β : 基数}
  结论: α + 1 <= β + 1 ↔ α <= β
  证明: add_le_add_iff_of_lt_aleph0 one_lt_aleph0

Depends on / 依赖: add_le_add_iff_of_lt_aleph0, one_lt_aleph0
-/
theorem add_one_le_add_one_iff {α β : Cardinal} : α + 1 <= β + 1 ↔ α <= β :=
  add_le_add_iff_of_lt_aleph0 one_lt_aleph0

/--
lemma `add_lt_add_iff_of_right_lt_aleph0` / 引理 `add_lt_add_iff_of_right_lt_aleph0`

English:
lemma add_lt_add_iff_of_right_lt_aleph0
  given: {a b c : Cardinal} (hc : c < ℵ₀)
  proof: by
  constructor <;> contrapose! <;> simp [add_le_add_iff_of_lt_aleph0 hc]

中文:
引理 add_lt_add_iff_of_right_lt_aleph0
  条件: {a b c : 基数} (hc : c < ℵ₀)
  证明: by
  constructor <;> contrapose! <;> simp [add_le_add_iff_of_lt_aleph0 hc]

Depends on / 依赖: add_le_add_iff_of_lt_aleph0, contrapose
-/
lemma add_lt_add_iff_of_right_lt_aleph0 {a b c : Cardinal} (hc : c < ℵ₀) :
    a + c < b + c ↔ a < b := by
  constructor <;> contrapose! <;> simp [add_le_add_iff_of_lt_aleph0 hc]

/--
lemma `add_lt_add_iff_of_left_lt_aleph0` / 引理 `add_lt_add_iff_of_left_lt_aleph0`

English:
lemma add_lt_add_iff_of_left_lt_aleph0
  given: {a b c : Cardinal} (hc : c < ℵ₀)
  proof: by
  simpa [add_comm] using add_lt_add_iff_of_right_lt_aleph0 (a := a) (b := b) hc

中文:
引理 add_lt_add_iff_of_left_lt_aleph0
  条件: {a b c : 基数} (hc : c < ℵ₀)
  证明: by
  simpa [add_comm] using add_lt_add_iff_of_right_lt_aleph0 (a := a) (b := b) hc

Depends on / 依赖: add_comm, add_lt_add_iff_of_right_lt_aleph0
-/
lemma add_lt_add_iff_of_left_lt_aleph0 {a b c : Cardinal} (hc : c < ℵ₀) :
    c + a < c + b ↔ a < b := by
  simpa [add_comm] using add_lt_add_iff_of_right_lt_aleph0 (a := a) (b := b) hc

/--
lemma `add_lt_add` / 引理 `add_lt_add`

English:
lemma add_lt_add
  statement: {κ₁ κ₂ μ₁ μ₂ : Cardinal}
  proof: by
  rcases le_or_gt ℵ₀ (κ₂ + μ₂) with hinf | hfin
  · refine add_lt_of_lt hinf ?_ ?_ <;> apply lt_of_lt_of_le <;> solve | assumption | simp
  · have hfin_ : κ₂ < ℵ₀ ∧ μ₂ < ℵ₀ := add_lt_aleph0_iff.1 hfin
    apply lt_of_le_of_lt
    · exact (add_le_add_iff_of_lt_aleph0 (hμ.trans hfin_.right)).mpr hκ.le
    · simpa [add_comm] using (add_lt_add_iff_of_right_lt_aleph0 hfin_.left).mpr hμ

中文:
引理 add_lt_add
  结论: {κ₁ κ₂ μ₁ μ₂ : 基数}
  证明: by
  rcases le_or_gt ℵ₀ (κ₂ + μ₂) with hinf | hfin
  · refine add_lt_of_lt hinf ?_ ?_ <;> apply lt_of_lt_of_le <;> solve | assumption | simp
  · have hfin_ : κ₂ < ℵ₀ ∧ μ₂ < ℵ₀ := add_lt_aleph0_iff.1 hfin
    apply lt_of_le_of_lt
    · exact (add_le_add_iff_of_lt_aleph0 (hμ.trans hfin_.right)).mpr hκ.le
    · simpa [add_comm] using (add_lt_add_iff_of_right_lt_aleph0 hfin_.left).mpr hμ
-/
protected lemma add_lt_add {κ₁ κ₂ μ₁ μ₂ : Cardinal}
    (hκ : κ₁ < κ₂) (hμ : μ₁ < μ₂) : κ₁ + μ₁ < κ₂ + μ₂ := by
  rcases le_or_gt ℵ₀ (κ₂ + μ₂) with hinf | hfin
  · refine add_lt_of_lt hinf ?_ ?_ <;> apply lt_of_lt_of_le <;> solve | assumption | simp
  · have hfin_ : κ₂ < ℵ₀ ∧ μ₂ < ℵ₀ := add_lt_aleph0_iff.1 hfin
    apply lt_of_le_of_lt
    · exact (add_le_add_iff_of_lt_aleph0 (hμ.trans hfin_.right)).mpr hκ.le
    · simpa [add_comm] using (add_lt_add_iff_of_right_lt_aleph0 hfin_.left).mpr hμ

end aleph

section mul_strictMono

variable {n : Nat} {a b : Cardinal}

/--
lemma `natCast_mul_strictMono` / 引理 `natCast_mul_strictMono`

English:
lemma natCast_mul_strictMono
  given: {n : Nat} (hn : n != 0)
  statement: StrictMono fun a : Cardinal => n * a
  proof: by
  match n, hn with
  | 1, _ => simpa using! strictMono_id
  | (n + 1) + 1, hneq1 =>
    intro a μ hlt
    push_cast
    conv_lhs => rw [add_mul, one_mul]
    conv_rhs => rw [add_mul, one_mul]
    refine Cardinal.add_lt_add ?_ hlt
    simpa using! (natCast_mul_strictMono (Nat.succ_ne_zero n) hlt)

中文:
引理 natCast_mul_strictMono
  条件: {n : 自然数} (hn : n != 0)
  结论: 严格递增 fun a : 基数 => n * a
  证明: by
  match n, hn with
  | 1, _ => simpa using! strictMono_id
  | (n + 1) + 1, hneq1 =>
    intro a μ hlt
    push_cast
    conv_lhs => rw [add_mul, one_mul]
    conv_rhs => rw [add_mul, one_mul]
    refine Cardinal.add_lt_add ?_ hlt
    simpa using! (natCast_mul_strictMono (Nat.succ_ne_zero n) hlt)

Depends on / 依赖: Cardinal, Cardinal.add_lt_add, Nat.succ_ne_zero, add_lt_add, add_mul, conv_lhs, conv_rhs, natCast_mul_strictMono, one_mul, strictMono_id, succ_ne_zero
-/
lemma natCast_mul_strictMono {n : Nat} (hn : n != 0) : StrictMono fun a : Cardinal => n * a := by
  match n, hn with
  | 1, _ => simpa using! strictMono_id
  | (n + 1) + 1, hneq1 =>
    intro a μ hlt
    push_cast
    conv_lhs => rw [add_mul, one_mul]
    conv_rhs => rw [add_mul, one_mul]
    refine Cardinal.add_lt_add ?_ hlt
    simpa using! (natCast_mul_strictMono (Nat.succ_ne_zero n) hlt)

/--
lemma `mul_natCast_strictMono` / 引理 `mul_natCast_strictMono`

English:
lemma mul_natCast_strictMono
  given: (hn : n != 0)
  statement: StrictMono fun a : Cardinal => a * n
  proof: fun _ _ hlt => by simpa [mul_comm] using natCast_mul_strictMono hn hlt

@[simp]

中文:
引理 mul_natCast_strictMono
  条件: (hn : n != 0)
  结论: 严格递增 fun a : 基数 => a * n
  证明: fun _ _ hlt => by simpa [mul_comm] using natCast_mul_strictMono hn hlt

@[simp]

Depends on / 依赖: mul_comm, natCast_mul_strictMono
-/
lemma mul_natCast_strictMono (hn : n != 0) : StrictMono fun a : Cardinal => a * n :=
  fun _ _ hlt => by simpa [mul_comm] using natCast_mul_strictMono hn hlt

@[simp]
/--
lemma `natCast_mul_inj` / 引理 `natCast_mul_inj`

English:
lemma natCast_mul_inj
  given: (hn : n != 0)
  statement: n * a = n * b ↔ a = b
  proof: (natCast_mul_strictMono hn).injective.eq_iff

@[simp]

中文:
引理 natCast_mul_inj
  条件: (hn : n != 0)
  结论: n * a = n * b ↔ a = b
  证明: (natCast_mul_strictMono hn).injective.eq_iff

@[simp]

Depends on / 依赖: eq_iff, injective, injective.eq_iff, natCast_mul_strictMono
-/
lemma natCast_mul_inj (hn : n != 0) : n * a = n * b ↔ a = b :=
  (natCast_mul_strictMono hn).injective.eq_iff

@[simp]
/--
lemma `mul_natCast_inj` / 引理 `mul_natCast_inj`

English:
lemma mul_natCast_inj
  given: (hn : n != 0)
  statement: a * n = b * n ↔ a = b
  proof: (mul_natCast_strictMono hn).injective.eq_iff

@[simp]

中文:
引理 mul_natCast_inj
  条件: (hn : n != 0)
  结论: a * n = b * n ↔ a = b
  证明: (mul_natCast_strictMono hn).injective.eq_iff

@[simp]

Depends on / 依赖: eq_iff, injective, injective.eq_iff, mul_natCast_strictMono
-/
lemma mul_natCast_inj (hn : n != 0) : a * n = b * n ↔ a = b :=
  (mul_natCast_strictMono hn).injective.eq_iff

@[simp]
/--
lemma `natCast_mul_le_natCast_mul` / 引理 `natCast_mul_le_natCast_mul`

English:
lemma natCast_mul_le_natCast_mul
  given: (hn : n != 0)
  statement: n * a <= n * b ↔ a <= b
  proof: (natCast_mul_strictMono hn).le_iff_le

@[simp]

中文:
引理 natCast_mul_le_natCast_mul
  条件: (hn : n != 0)
  结论: n * a <= n * b ↔ a <= b
  证明: (natCast_mul_strictMono hn).le_iff_le

@[simp]

Depends on / 依赖: le_iff_le, natCast_mul_strictMono
-/
lemma natCast_mul_le_natCast_mul (hn : n != 0) : n * a <= n * b ↔ a <= b :=
  (natCast_mul_strictMono hn).le_iff_le

@[simp]
/--
lemma `mul_natCast_le_mul_natCast` / 引理 `mul_natCast_le_mul_natCast`

English:
lemma mul_natCast_le_mul_natCast
  given: (hn : n != 0)
  statement: a * n <= b * n ↔ a <= b
  proof: (mul_natCast_strictMono hn).le_iff_le

@[simp]

中文:
引理 mul_natCast_le_mul_natCast
  条件: (hn : n != 0)
  结论: a * n <= b * n ↔ a <= b
  证明: (mul_natCast_strictMono hn).le_iff_le

@[simp]

Depends on / 依赖: le_iff_le, mul_natCast_strictMono
-/
lemma mul_natCast_le_mul_natCast (hn : n != 0) : a * n <= b * n ↔ a <= b :=
  (mul_natCast_strictMono hn).le_iff_le

@[simp]
/--
lemma `natCast_mul_lt_natCast_mul` / 引理 `natCast_mul_lt_natCast_mul`

English:
lemma natCast_mul_lt_natCast_mul
  given: (hn : n != 0)
  statement: n * a < n * b ↔ a < b
  proof: (natCast_mul_strictMono hn).lt_iff_lt

@[simp]

中文:
引理 natCast_mul_lt_natCast_mul
  条件: (hn : n != 0)
  结论: n * a < n * b ↔ a < b
  证明: (natCast_mul_strictMono hn).lt_iff_lt

@[simp]

Depends on / 依赖: lt_iff_lt, natCast_mul_strictMono
-/
lemma natCast_mul_lt_natCast_mul (hn : n != 0) : n * a < n * b ↔ a < b :=
  (natCast_mul_strictMono hn).lt_iff_lt

@[simp]
/--
lemma `mul_natCast_lt_mul_natCast` / 引理 `mul_natCast_lt_mul_natCast`

English:
lemma mul_natCast_lt_mul_natCast
  given: (hn : n != 0)
  statement: a * n < b * n ↔ a < b
  proof: (mul_natCast_strictMono hn).lt_iff_lt

中文:
引理 mul_natCast_lt_mul_natCast
  条件: (hn : n != 0)
  结论: a * n < b * n ↔ a < b
  证明: (mul_natCast_strictMono hn).lt_iff_lt

Depends on / 依赖: lt_iff_lt, mul_natCast_strictMono
-/
lemma mul_natCast_lt_mul_natCast (hn : n != 0) : a * n < b * n ↔ a < b :=
  (mul_natCast_strictMono hn).lt_iff_lt

end mul_strictMono

/-! ### Properties about `power` -/
section power

set_option backward.isDefEq.respectTransparency false in
/--
theorem `pow_le` / 定理 `pow_le`

English:
theorem pow_le
  given: {κ μ : Cardinal.{u}} (H1 : ℵ₀ <= κ) (H2 : μ < ℵ₀)
  statement: κ ^ μ <= κ
  proof: let ⟨n, H3⟩ := lt_aleph0.1 H2
  H3.symm ▸
    Quotient.inductionOn κ
      (fun α H1 =>
        Nat.recOn n
          (by grw [Nat.cast_zero, power_zero, one_lt_aleph0, H1])
          fun n ih => by grw [Nat.cast_succ, power_add, power_one, ih, mul_eq_self H1])
      H1

中文:
定理 pow_le
  条件: {κ μ : 基数.{u}} (H1 : ℵ₀ <= κ) (H2 : μ < ℵ₀)
  结论: κ ^ μ <= κ
  证明: let ⟨n, H3⟩ := lt_aleph0.1 H2
  H3.symm ▸
    Quotient.inductionOn κ
      (fun α H1 =>
        Nat.recOn n
          (by grw [Nat.cast_zero, power_zero, one_lt_aleph0, H1])
          fun n ih => by grw [Nat.cast_succ, power_add, power_one, ih, mul_eq_self H1])
      H1

Depends on / 依赖: H3.symm, Nat.cast_succ, Nat.cast_zero, Nat.recOn, Quotient, Quotient.inductionOn, cast_succ, cast_zero, inductionOn, lt_aleph0, mul_eq_self, one_lt_aleph0, power_add, power_one, power_zero
-/
theorem pow_le {κ μ : Cardinal.{u}} (H1 : ℵ₀ <= κ) (H2 : μ < ℵ₀) : κ ^ μ <= κ :=
  let ⟨n, H3⟩ := lt_aleph0.1 H2
  H3.symm ▸
    Quotient.inductionOn κ
      (fun α H1 =>
        Nat.recOn n
          (by grw [Nat.cast_zero, power_zero, one_lt_aleph0, H1])
          fun n ih => by grw [Nat.cast_succ, power_add, power_one, ih, mul_eq_self H1])
      H1

/--
theorem `pow_eq` / 定理 `pow_eq`

English:
theorem pow_eq
  given: {κ μ : Cardinal.{u}} (H1 : ℵ₀ <= κ) (H2 : 1 <= μ) (H3 : μ < ℵ₀)
  statement: κ ^ μ = κ
  proof: (pow_le H1 H3).antisymm self_le_power κ H2

中文:
定理 pow_eq
  条件: {κ μ : 基数.{u}} (H1 : ℵ₀ <= κ) (H2 : 1 <= μ) (H3 : μ < ℵ₀)
  结论: κ ^ μ = κ
  证明: (pow_le H1 H3).antisymm self_le_power κ H2

Depends on / 依赖: antisymm, pow_le, self_le_power
-/
theorem pow_eq {κ μ : Cardinal.{u}} (H1 : ℵ₀ <= κ) (H2 : 1 <= μ) (H3 : μ < ℵ₀) : κ ^ μ = κ :=
(pow_le H1 H3).antisymm self_le_power κ H2

/--
theorem `power_self_eq` / 定理 `power_self_eq`

English:
theorem power_self_eq
  given: {c : Cardinal} (h : ℵ₀ <= c)
  statement: c ^ c = 2 ^ c
  proof: by
  apply ((power_le_power_right <| (cantor c).le).trans _).antisymm
  · exact power_le_power_right (natCast_le_aleph0.trans h)
  · rw [← power_mul, mul_eq_self h]

中文:
定理 power_self_eq
  条件: {c : 基数} (h : ℵ₀ <= c)
  结论: c ^ c = 2 ^ c
  证明: by
  apply ((power_le_power_right <| (cantor c).le).trans _).antisymm
  · exact power_le_power_right (natCast_le_aleph0.trans h)
  · rw [← power_mul, mul_eq_self h]

Depends on / 依赖: antisymm, cantor, mul_eq_self, natCast_le_aleph0, natCast_le_aleph0.trans, power_le_power_right, power_mul
-/
theorem power_self_eq {c : Cardinal} (h : ℵ₀ <= c) : c ^ c = 2 ^ c := by
  apply ((power_le_power_right <| (cantor c).le).trans _).antisymm
  · exact power_le_power_right (natCast_le_aleph0.trans h)
  · rw [← power_mul, mul_eq_self h]

/--
theorem `prod_eq_two_power` / 定理 `prod_eq_two_power`

English:
theorem prod_eq_two_power
  statement: {ι : Type u} [Infinite ι] {c : ι -> Cardinal.{v}} (h₁ : forall i, 2 <= c i)
  proof: by
  rw [← lift_id'.{u]; rw [v} (prod.{u]; rw [v} c)]; rw [lift_prod]; rw [← lift_two_power]
  apply le_antisymm
  · refine (prod_le_prod _ _ h₂).trans_eq ?_
    rw [prod_const]; rw [lift_lift]; rw [← lift_power]; rw [power_self_eq (aleph0_le_mk ι)]; rw [lift_umax.{u]; rw [v}]
  · rw [← prod_const', lift_prod]
    refine prod_le_prod _ _ fun i => ?_
    rw [lift_two]; rw [← lift_two.{u]; rw [v}]; rw [lift_le]
    exact h₁ i

中文:
定理 prod_eq_two_power
  结论: {ι : 类型u} [无限 ι] {c : ι -> 基数.{v}} (h₁ : 对任意 i, 2 <= c i)
  证明: by
  rw [← lift_id'.{u]; rw [v} (prod.{u]; rw [v} c)]; rw [lift_prod]; rw [← lift_two_power]
  apply le_antisymm
  · refine (prod_le_prod _ _ h₂).trans_eq ?_
    rw [prod_const]; rw [lift_lift]; rw [← lift_power]; rw [power_self_eq (aleph0_le_mk ι)]; rw [lift_umax.{u]; rw [v}]
  · rw [← prod_const', lift_prod]
    refine prod_le_prod _ _ fun i => ?_
    rw [lift_two]; rw [← lift_two.{u]; rw [v}]; rw [lift_le]
    exact h₁ i

Depends on / 依赖: aleph0_le_mk, le_antisymm, lift_id, lift_le, lift_lift, lift_power, lift_prod, lift_two, lift_two_power, lift_umax, power_self_eq, prod_const, prod_le_prod, trans_eq
-/
theorem prod_eq_two_power {ι : Type u} [Infinite ι] {c : ι -> Cardinal.{v}} (h₁ : forall i, 2 <= c i)
    (h₂ : forall i, lift.{u} (c i) <= lift.{v} #ι) : prod c = 2 ^ lift.{v} #ι := by
  rw [← lift_id'.{u]; rw [v} (prod.{u]; rw [v} c)]; rw [lift_prod]; rw [← lift_two_power]
  apply le_antisymm
  · refine (prod_le_prod _ _ h₂).trans_eq ?_
    rw [prod_const]; rw [lift_lift]; rw [← lift_power]; rw [power_self_eq (aleph0_le_mk ι)]; rw [lift_umax.{u]; rw [v}]
  · rw [← prod_const', lift_prod]
    refine prod_le_prod _ _ fun i => ?_
    rw [lift_two]; rw [← lift_two.{u]; rw [v}]; rw [lift_le]
    exact h₁ i

/--
theorem `power_eq_two_power` / 定理 `power_eq_two_power`

English:
theorem power_eq_two_power
  given: {c₁ c₂ : Cardinal} (h₁ : ℵ₀ <= c₁) (h₂ : 2 <= c₂) (h₂' : c₂ <= c₁)
  proof: le_antisymm (power_self_eq h₁ ▸ power_le_power_right h₂') (power_le_power_right h₂)

中文:
定理 power_eq_two_power
  条件: {c₁ c₂ : 基数} (h₁ : ℵ₀ <= c₁) (h₂ : 2 <= c₂) (h₂' : c₂ <= c₁)
  证明: le_antisymm (power_self_eq h₁ ▸ power_le_power_right h₂') (power_le_power_right h₂)

Depends on / 依赖: le_antisymm, power_le_power_right, power_self_eq
-/
theorem power_eq_two_power {c₁ c₂ : Cardinal} (h₁ : ℵ₀ <= c₁) (h₂ : 2 <= c₂) (h₂' : c₂ <= c₁) :
    c₂ ^ c₁ = 2 ^ c₁ :=
  le_antisymm (power_self_eq h₁ ▸ power_le_power_right h₂') (power_le_power_right h₂)

/--
theorem `nat_power_eq` / 定理 `nat_power_eq`

English:
theorem nat_power_eq
  given: {c : Cardinal.{u}} (h : ℵ₀ <= c) {n : Nat} (hn : 2 <= n)
  proof: power_eq_two_power h (by assumption_mod_cast) (natCast_le_aleph0.trans h)

中文:
定理 nat_power_eq
  条件: {c : 基数.{u}} (h : ℵ₀ <= c) {n : 自然数} (hn : 2 <= n)
  证明: power_eq_two_power h (by assumption_mod_cast) (natCast_le_aleph0.trans h)

Depends on / 依赖: assumption_mod_cast, natCast_le_aleph0, natCast_le_aleph0.trans, power_eq_two_power
-/
theorem nat_power_eq {c : Cardinal.{u}} (h : ℵ₀ <= c) {n : Nat} (hn : 2 <= n) :
    (n : Cardinal.{u}) ^ c = 2 ^ c :=
  power_eq_two_power h (by assumption_mod_cast) (natCast_le_aleph0.trans h)

/--
theorem `power_nat_le` / 定理 `power_nat_le`

English:
theorem power_nat_le
  given: {c : Cardinal.{u}} {n : Nat} (h : ℵ₀ <= c)
  statement: c ^ n <= c
  proof: pow_le h natCast_lt_aleph0

中文:
定理 power_nat_le
  条件: {c : 基数.{u}} {n : 自然数} (h : ℵ₀ <= c)
  结论: c ^ n <= c
  证明: pow_le h natCast_lt_aleph0

Depends on / 依赖: natCast_lt_aleph0, pow_le
-/
theorem power_nat_le {c : Cardinal.{u}} {n : Nat} (h : ℵ₀ <= c) : c ^ n <= c :=
  pow_le h natCast_lt_aleph0

/--
theorem `power_nat_eq` / 定理 `power_nat_eq`

English:
theorem power_nat_eq
  given: {c : Cardinal.{u}} {n : Nat} (h1 : ℵ₀ <= c) (h2 : 1 <= n)
  statement: c ^ n = c
  proof: pow_eq h1 (mod_cast h2) natCast_lt_aleph0

中文:
定理 power_nat_eq
  条件: {c : 基数.{u}} {n : 自然数} (h1 : ℵ₀ <= c) (h2 : 1 <= n)
  结论: c ^ n = c
  证明: pow_eq h1 (mod_cast h2) natCast_lt_aleph0

Depends on / 依赖: mod_cast, natCast_lt_aleph0, pow_eq
-/
theorem power_nat_eq {c : Cardinal.{u}} {n : Nat} (h1 : ℵ₀ <= c) (h2 : 1 <= n) : c ^ n = c :=
  pow_eq h1 (mod_cast h2) natCast_lt_aleph0

/--
theorem `power_nat_le_max` / 定理 `power_nat_le_max`

English:
theorem power_nat_le_max
  given: {c : Cardinal.{u}} {n : Nat}
  statement: c ^ (n : Cardinal.{u}) <= max c ℵ₀
  proof: by
  rcases le_or_gt ℵ₀ c with hc | hc
  · exact le_max_of_le_left (power_nat_le hc)
  · exact le_max_of_le_right (power_lt_aleph0 hc natCast_lt_aleph0).le

中文:
定理 power_nat_le_max
  条件: {c : 基数.{u}} {n : 自然数}
  结论: c ^ (n : 基数.{u}) <= 最大值 c ℵ₀
  证明: by
  rcases le_or_gt ℵ₀ c with hc | hc
  · exact le_max_of_le_left (power_nat_le hc)
  · exact le_max_of_le_right (power_lt_aleph0 hc natCast_lt_aleph0).le

Depends on / 依赖: le_max_of_le_left, le_max_of_le_right, le_or_gt, natCast_lt_aleph0, power_lt_aleph0, power_nat_le
-/
theorem power_nat_le_max {c : Cardinal.{u}} {n : Nat} : c ^ (n : Cardinal.{u}) <= max c ℵ₀ := by
  rcases le_or_gt ℵ₀ c with hc | hc
  · exact le_max_of_le_left (power_nat_le hc)
  · exact le_max_of_le_right (power_lt_aleph0 hc natCast_lt_aleph0).le

/--
lemma `power_le_aleph0` / 引理 `power_le_aleph0`

English:
lemma power_le_aleph0
  given: {a b : Cardinal.{u}} (ha : a <= ℵ₀) (hb : b < ℵ₀)
  statement: a ^ b <= ℵ₀
  proof: by
  lift b to Nat using hb; simpa [ha] using power_nat_le_max (c := a)

中文:
引理 power_le_aleph0
  条件: {a b : 基数.{u}} (ha : a <= ℵ₀) (hb : b < ℵ₀)
  结论: a ^ b <= ℵ₀
  证明: by
  lift b to Nat using hb; simpa [ha] using power_nat_le_max (c := a)

Depends on / 依赖: power_nat_le_max
-/
lemma power_le_aleph0 {a b : Cardinal.{u}} (ha : a <= ℵ₀) (hb : b < ℵ₀) : a ^ b <= ℵ₀ := by
  lift b to Nat using hb; simpa [ha] using power_nat_le_max (c := a)

/--
theorem `powerlt_aleph0` / 定理 `powerlt_aleph0`

English:
theorem powerlt_aleph0
  given: {c : Cardinal} (h : ℵ₀ <= c)
  statement: c ^< ℵ₀ = c
  proof: by
  apply le_antisymm
  · rw [powerlt_le]
    exact fun _ a => pow_le h a
  convert! le_powerlt c one_lt_aleph0; rw [power_one]

中文:
定理 powerlt_aleph0
  条件: {c : 基数} (h : ℵ₀ <= c)
  结论: c ^< ℵ₀ = c
  证明: by
  apply le_antisymm
  · rw [powerlt_le]
    exact fun _ a => pow_le h a
  convert! le_powerlt c one_lt_aleph0; rw [power_one]

Depends on / 依赖: convert, le_antisymm, le_powerlt, one_lt_aleph0, pow_le, power_one, powerlt_le
-/
theorem powerlt_aleph0 {c : Cardinal} (h : ℵ₀ <= c) : c ^< ℵ₀ = c := by
  apply le_antisymm
  · rw [powerlt_le]
    exact fun _ a => pow_le h a
  convert! le_powerlt c one_lt_aleph0; rw [power_one]

/--
theorem `powerlt_aleph0_le` / 定理 `powerlt_aleph0_le`

English:
theorem powerlt_aleph0_le
  given: (c : Cardinal)
  statement: c ^< ℵ₀ <= max c ℵ₀
  proof: by
  rcases le_or_gt ℵ₀ c with h | h
  · rw [powerlt_aleph0 h]
    apply le_max_left
  rw [powerlt_le]
  exact fun c' hc' => (power_lt_aleph0 h hc').le.trans (le_max_right _ _)

中文:
定理 powerlt_aleph0_le
  条件: (c : 基数)
  结论: c ^< ℵ₀ <= 最大值 c ℵ₀
  证明: by
  rcases le_or_gt ℵ₀ c with h | h
  · rw [powerlt_aleph0 h]
    apply le_max_left
  rw [powerlt_le]
  exact fun c' hc' => (power_lt_aleph0 h hc').le.trans (le_max_right _ _)

Depends on / 依赖: le.trans, le_max_left, le_max_right, le_or_gt, power_lt_aleph0, powerlt_aleph0, powerlt_le
-/
theorem powerlt_aleph0_le (c : Cardinal) : c ^< ℵ₀ <= max c ℵ₀ := by
  rcases le_or_gt ℵ₀ c with h | h
  · rw [powerlt_aleph0 h]
    apply le_max_left
  rw [powerlt_le]
  exact fun c' hc' => (power_lt_aleph0 h hc').le.trans (le_max_right _ _)

end power

/-! ### Computing cardinality of various types -/
section computing

section Function

variable {α β : Type u} {β' : Type v}

/--
theorem `mk_equiv_eq_zero_iff_lift_ne` / 定理 `mk_equiv_eq_zero_iff_lift_ne`

English:
theorem mk_equiv_eq_zero_iff_lift_ne
  statement: #(α ≃ β') = 0 ↔ lift.{v} #α != lift.{u} #β'
  proof: by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_eq']

中文:
定理 mk_equiv_eq_zero_iff_lift_ne
  结论: #(α ≃ β') = 0 ↔ lift.{v} #α != lift.{u} #β'
  证明: by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_eq']

Depends on / 依赖: lift_mk_eq, mk_eq_zero_iff, not_nonempty_iff
-/
theorem mk_equiv_eq_zero_iff_lift_ne : #(α ≃ β') = 0 ↔ lift.{v} #α != lift.{u} #β' := by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_eq']

/--
theorem `mk_equiv_eq_zero_iff_ne` / 定理 `mk_equiv_eq_zero_iff_ne`

English:
theorem mk_equiv_eq_zero_iff_ne
  statement: #(α ≃ β) = 0 ↔ #α != #β
  proof: by
  rw [mk_equiv_eq_zero_iff_lift_ne]; rw [lift_id]; rw [lift_id]

中文:
定理 mk_equiv_eq_zero_iff_ne
  结论: #(α ≃ β) = 0 ↔ #α != #β
  证明: by
  rw [mk_equiv_eq_zero_iff_lift_ne]; rw [lift_id]; rw [lift_id]

Depends on / 依赖: lift_id, mk_equiv_eq_zero_iff_lift_ne
-/
theorem mk_equiv_eq_zero_iff_ne : #(α ≃ β) = 0 ↔ #α != #β := by
  rw [mk_equiv_eq_zero_iff_lift_ne]; rw [lift_id]; rw [lift_id]

/--
theorem `mk_equiv_comm` / 定理 `mk_equiv_comm`

English:
theorem mk_equiv_comm
  statement: #(α ≃ β') = #(β' ≃ α)
  proof: (ofBijective _ symm_bijective).cardinal_eq

中文:
定理 mk_equiv_comm
  结论: #(α ≃ β') = #(β' ≃ α)
  证明: (ofBijective _ symm_bijective).cardinal_eq

Depends on / 依赖: cardinal_eq, ofBijective, symm_bijective
-/
theorem mk_equiv_comm : #(α ≃ β') = #(β' ≃ α) :=
  (ofBijective _ symm_bijective).cardinal_eq

/--
theorem `mk_embedding_eq_zero_iff_lift_lt` / 定理 `mk_embedding_eq_zero_iff_lift_lt`

English:
theorem mk_embedding_eq_zero_iff_lift_lt
  statement: #(α ↪ β') = 0 ↔ lift.{u} #β' < lift.{v} #α
  proof: by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_le']; rw [not_le]

中文:
定理 mk_embedding_eq_zero_iff_lift_lt
  结论: #(α ↪ β') = 0 ↔ lift.{u} #β' < lift.{v} #α
  证明: by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_le']; rw [not_le]

Depends on / 依赖: lift_mk_le, mk_eq_zero_iff, not_le, not_nonempty_iff
-/
theorem mk_embedding_eq_zero_iff_lift_lt : #(α ↪ β') = 0 ↔ lift.{u} #β' < lift.{v} #α := by
  rw [mk_eq_zero_iff]; rw [← not_nonempty_iff]; rw [← lift_mk_le']; rw [not_le]

/--
theorem `mk_embedding_eq_zero_iff_lt` / 定理 `mk_embedding_eq_zero_iff_lt`

English:
theorem mk_embedding_eq_zero_iff_lt
  statement: #(α ↪ β) = 0 ↔ #β < #α
  proof: by
  rw [mk_embedding_eq_zero_iff_lift_lt]; rw [lift_lt]

中文:
定理 mk_embedding_eq_zero_iff_lt
  结论: #(α ↪ β) = 0 ↔ #β < #α
  证明: by
  rw [mk_embedding_eq_zero_iff_lift_lt]; rw [lift_lt]

Depends on / 依赖: lift_lt, mk_embedding_eq_zero_iff_lift_lt
-/
theorem mk_embedding_eq_zero_iff_lt : #(α ↪ β) = 0 ↔ #β < #α := by
  rw [mk_embedding_eq_zero_iff_lift_lt]; rw [lift_lt]

/--
theorem `mk_arrow_eq_zero_iff` / 定理 `mk_arrow_eq_zero_iff`

English:
theorem mk_arrow_eq_zero_iff
  statement: #(α -> β') = 0 ↔ #α != 0 ∧ #β' = 0
  proof: by
  simp_rw [mk_eq_zero_iff, mk_ne_zero_iff, isEmpty_fun]

中文:
定理 mk_arrow_eq_zero_iff
  结论: #(α -> β') = 0 ↔ #α != 0 ∧ #β' = 0
  证明: by
  simp_rw [mk_eq_zero_iff, mk_ne_zero_iff, isEmpty_fun]

Depends on / 依赖: isEmpty_fun, mk_eq_zero_iff, mk_ne_zero_iff, simp_rw
-/
theorem mk_arrow_eq_zero_iff : #(α -> β') = 0 ↔ #α != 0 ∧ #β' = 0 := by
  simp_rw [mk_eq_zero_iff, mk_ne_zero_iff, isEmpty_fun]

/--
theorem `mk_surjective_eq_zero_iff_lift` / 定理 `mk_surjective_eq_zero_iff_lift`

English:
theorem mk_surjective_eq_zero_iff_lift
  proof: by
  contrapose! +distrib
  rw [lift_mk_le']; rw [and_comm]
  simp_rw [mk_ne_zero_iff, mk_eq_zero_iff, nonempty_coe_sort,
    Set.Nonempty, mem_ofPred, exists_surjective_iff, nonempty_fun]

中文:
定理 mk_surjective_eq_zero_iff_lift
  证明: by
  contrapose! +distrib
  rw [lift_mk_le']; rw [and_comm]
  simp_rw [mk_ne_zero_iff, mk_eq_zero_iff, nonempty_coe_sort,
    Set.Nonempty, mem_ofPred, exists_surjective_iff, nonempty_fun]

Depends on / 依赖: Nonempty, Set.Nonempty, and_comm, contrapose, distrib, exists_surjective_iff, lift_mk_le, mem_ofPred, mk_eq_zero_iff, mk_ne_zero_iff, nonempty_coe_sort, nonempty_fun, simp_rw
-/
theorem mk_surjective_eq_zero_iff_lift :
    #{f : α -> β' | Surjective f} = 0 ↔ lift.{v} #α < lift.{u} #β' ∨ (#α != 0 ∧ #β' = 0) := by
  contrapose! +distrib
  rw [lift_mk_le']; rw [and_comm]
  simp_rw [mk_ne_zero_iff, mk_eq_zero_iff, nonempty_coe_sort,
    Set.Nonempty, mem_ofPred, exists_surjective_iff, nonempty_fun]

/--
theorem `mk_surjective_eq_zero_iff` / 定理 `mk_surjective_eq_zero_iff`

English:
theorem mk_surjective_eq_zero_iff
  proof: by
  rw [mk_surjective_eq_zero_iff_lift]; rw [lift_lt]

中文:
定理 mk_surjective_eq_zero_iff
  证明: by
  rw [mk_surjective_eq_zero_iff_lift]; rw [lift_lt]

Depends on / 依赖: lift_lt, mk_surjective_eq_zero_iff_lift
-/
theorem mk_surjective_eq_zero_iff :
    #{f : α -> β | Surjective f} = 0 ↔ #α < #β ∨ (#α != 0 ∧ #β = 0) := by
  rw [mk_surjective_eq_zero_iff_lift]; rw [lift_lt]

variable (α β')

/--
theorem `mk_equiv_le_embedding` / 定理 `mk_equiv_le_embedding`

English:
theorem mk_equiv_le_embedding
  statement: #(α ≃ β') <= #(α ↪ β')
  proof: ⟨⟨_, Equiv.toEmbedding_injective⟩⟩

中文:
定理 mk_equiv_le_embedding
  结论: #(α ≃ β') <= #(α ↪ β')
  证明: ⟨⟨_, Equiv.toEmbedding_injective⟩⟩

Depends on / 依赖: Equiv.toEmbedding_injective, toEmbedding_injective
-/
theorem mk_equiv_le_embedding : #(α ≃ β') <= #(α ↪ β') := ⟨⟨_, Equiv.toEmbedding_injective⟩⟩

/--
theorem `mk_embedding_le_arrow` / 定理 `mk_embedding_le_arrow`

English:
theorem mk_embedding_le_arrow
  statement: #(α ↪ β') <= #(α -> β')
  proof: ⟨⟨_, DFunLike.coe_injective⟩⟩

中文:
定理 mk_embedding_le_arrow
  结论: #(α ↪ β') <= #(α -> β')
  证明: ⟨⟨_, DFunLike.coe_injective⟩⟩

Depends on / 依赖: DFunLike, DFunLike.coe_injective, coe_injective
-/
theorem mk_embedding_le_arrow : #(α ↪ β') <= #(α -> β') := ⟨⟨_, DFunLike.coe_injective⟩⟩

variable [Infinite α] {α β'}

/--
theorem `mk_perm_eq_self_power` / 定理 `mk_perm_eq_self_power`

English:
theorem mk_perm_eq_self_power
  statement: #(Equiv.Perm α) = #α ^ #α
  proof: ((mk_equiv_le_embedding α α).trans (mk_embedding_le_arrow α α)).antisymm by
    suffices Nonempty ((α -> Bool) ↪ Equiv.Perm (α × Bool)) by
      obtain ⟨e⟩ : Nonempty (α ≃ α × Bool) := by simp [← Cardinal.eq, mul_two]
      simp only [← le_def, mk_pi, mk_fintype, Fintype.card_bool, Nat.cast_ofNat,
        prod_const, lift_ofNat, lift_uzero] at this
      rwa [← power_def, power_self_eq (aleph0_le_mk α), e.permCongr.cardinal_eq]
    refine ⟨⟨fun f => Involutive.toPerm (fun x => ⟨x.1, xor (f x.1) x.2⟩) fun x => ?_, fun f g h => ?_⟩⟩
    · simp_rw [← Bool.xor_assoc, Bool.xor_self, Bool.false_xor]
    · ext a; rw [← (f a).xor_false, ← (g a).xor_false]; exact congr(($h ⟨a, false⟩).2)

中文:
定理 mk_perm_eq_self_power
  结论: #(等价.置换 α) = #α ^ #α
  证明: ((mk_equiv_le_embedding α α).trans (mk_embedding_le_arrow α α)).antisymm by
    suffices Nonempty ((α -> Bool) ↪ Equiv.Perm (α × Bool)) by
      obtain ⟨e⟩ : Nonempty (α ≃ α × Bool) := by simp [← Cardinal.eq, mul_two]
      simp only [← le_def, mk_pi, mk_fintype, Fintype.card_bool, Nat.cast_ofNat,
        prod_const, lift_ofNat, lift_uzero] at this
      rwa [← power_def, power_self_eq (aleph0_le_mk α), e.permCongr.cardinal_eq]
    refine ⟨⟨fun f => Involutive.toPerm (fun x => ⟨x.1, xor (f x.1) x.2⟩) fun x => ?_, fun f g h => ?_⟩⟩
    · simp_rw [← Bool.xor_assoc, Bool.xor_self, Bool.false_xor]
    · ext a; rw [← (f a).xor_false, ← (g a).xor_false]; exact congr(($h ⟨a, false⟩).2)

Depends on / 依赖: Cardinal, Cardinal.eq, Equiv.Perm, Fintype, Fintype.card_bool, Involutive, Involutive.toPerm, Nat.cast_ofNat, Nonempty, aleph0_le_mk, antisymm, card_bool, cardinal_eq, cast_ofNat, e.permCongr.cardinal_eq, le_def, lift_ofNat, lift_uzero, mk_embedding_le_arrow, mk_equiv_le_embedding
-/
theorem mk_perm_eq_self_power : #(Equiv.Perm α) = #α ^ #α :=
((mk_equiv_le_embedding α α).trans (mk_embedding_le_arrow α α)).antisymm by
    suffices Nonempty ((α -> Bool) ↪ Equiv.Perm (α × Bool)) by
      obtain ⟨e⟩ : Nonempty (α ≃ α × Bool) := by simp [← Cardinal.eq, mul_two]
      simp only [← le_def, mk_pi, mk_fintype, Fintype.card_bool, Nat.cast_ofNat,
        prod_const, lift_ofNat, lift_uzero] at this
      rwa [← power_def, power_self_eq (aleph0_le_mk α), e.permCongr.cardinal_eq]
    refine ⟨⟨fun f => Involutive.toPerm (fun x => ⟨x.1, xor (f x.1) x.2⟩) fun x => ?_, fun f g h => ?_⟩⟩
    · simp_rw [← Bool.xor_assoc, Bool.xor_self, Bool.false_xor]
    · ext a; rw [← (f a).xor_false, ← (g a).xor_false]; exact congr(($h ⟨a, false⟩).2)

/--
theorem `mk_perm_eq_two_power` / 定理 `mk_perm_eq_two_power`

English:
theorem mk_perm_eq_two_power
  statement: #(Equiv.Perm α) = 2 ^ #α
  proof: by
  rw [mk_perm_eq_self_power]; rw [power_self_eq (aleph0_le_mk α)]

中文:
定理 mk_perm_eq_two_power
  结论: #(等价.置换 α) = 2 ^ #α
  证明: by
  rw [mk_perm_eq_self_power]; rw [power_self_eq (aleph0_le_mk α)]

Depends on / 依赖: aleph0_le_mk, mk_perm_eq_self_power, power_self_eq
-/
theorem mk_perm_eq_two_power : #(Equiv.Perm α) = 2 ^ #α := by
  rw [mk_perm_eq_self_power]; rw [power_self_eq (aleph0_le_mk α)]

/--
theorem `mk_equiv_eq_arrow_of_lift_eq` / 定理 `mk_equiv_eq_arrow_of_lift_eq`

English:
theorem mk_equiv_eq_arrow_of_lift_eq
  given: (leq : lift.{v} #α = lift.{u} #β')
  proof: by
  obtain ⟨e⟩ := lift_mk_eq'.mp leq
  have e₁ := lift_mk_eq'.mpr ⟨.equivCongr (.refl α) e⟩
  have e₂ := lift_mk_eq'.mpr ⟨.arrowCongr (.refl α) e⟩
  rw [lift_id'.{u]; rw [v}] at e₁ e₂
  rw [← e₁]; rw [← e₂]; rw [lift_inj]; rw [mk_perm_eq_self_power]; rw [power_def]

中文:
定理 mk_equiv_eq_arrow_of_lift_eq
  条件: (leq : lift.{v} #α = lift.{u} #β')
  证明: by
  obtain ⟨e⟩ := lift_mk_eq'.mp leq
  have e₁ := lift_mk_eq'.mpr ⟨.equivCongr (.refl α) e⟩
  have e₂ := lift_mk_eq'.mpr ⟨.arrowCongr (.refl α) e⟩
  rw [lift_id'.{u]; rw [v}] at e₁ e₂
  rw [← e₁]; rw [← e₂]; rw [lift_inj]; rw [mk_perm_eq_self_power]; rw [power_def]

Depends on / 依赖: arrowCongr, equivCongr, lift_id, lift_inj, lift_mk_eq, mk_perm_eq_self_power, power_def
-/
theorem mk_equiv_eq_arrow_of_lift_eq (leq : lift.{v} #α = lift.{u} #β') :
    #(α ≃ β') = #(α -> β') := by
  obtain ⟨e⟩ := lift_mk_eq'.mp leq
  have e₁ := lift_mk_eq'.mpr ⟨.equivCongr (.refl α) e⟩
  have e₂ := lift_mk_eq'.mpr ⟨.arrowCongr (.refl α) e⟩
  rw [lift_id'.{u]; rw [v}] at e₁ e₂
  rw [← e₁]; rw [← e₂]; rw [lift_inj]; rw [mk_perm_eq_self_power]; rw [power_def]

/--
theorem `mk_equiv_eq_arrow_of_eq` / 定理 `mk_equiv_eq_arrow_of_eq`

English:
theorem mk_equiv_eq_arrow_of_eq
  given: (eq : #α = #β)
  statement: #(α ≃ β) = #(α -> β)
  proof: mk_equiv_eq_arrow_of_lift_eq congr(lift $eq)

中文:
定理 mk_equiv_eq_arrow_of_eq
  条件: (eq : #α = #β)
  结论: #(α ≃ β) = #(α -> β)
  证明: mk_equiv_eq_arrow_of_lift_eq congr(lift $eq)

Depends on / 依赖: mk_equiv_eq_arrow_of_lift_eq
-/
theorem mk_equiv_eq_arrow_of_eq (eq : #α = #β) : #(α ≃ β) = #(α -> β) :=
  mk_equiv_eq_arrow_of_lift_eq congr(lift $eq)

/--
theorem `mk_equiv_of_lift_eq` / 定理 `mk_equiv_of_lift_eq`

English:
theorem mk_equiv_of_lift_eq
  given: (leq : lift.{v} #α = lift.{u} #β')
  statement: #(α ≃ β') = 2 ^ lift.{v} #α
  proof: by
  simp [mk_equiv_eq_arrow_of_lift_eq leq, ← leq, power_self_eq]

中文:
定理 mk_equiv_of_lift_eq
  条件: (leq : lift.{v} #α = lift.{u} #β')
  结论: #(α ≃ β') = 2 ^ lift.{v} #α
  证明: by
  simp [mk_equiv_eq_arrow_of_lift_eq leq, ← leq, power_self_eq]

Depends on / 依赖: mk_equiv_eq_arrow_of_lift_eq, power_self_eq
-/
theorem mk_equiv_of_lift_eq (leq : lift.{v} #α = lift.{u} #β') : #(α ≃ β') = 2 ^ lift.{v} #α := by
  simp [mk_equiv_eq_arrow_of_lift_eq leq, ← leq, power_self_eq]

/--
theorem `mk_equiv_of_eq` / 定理 `mk_equiv_of_eq`

English:
theorem mk_equiv_of_eq
  given: (eq : #α = #β)
  statement: #(α ≃ β) = 2 ^ #α
  proof: by
  rw [mk_equiv_of_lift_eq (lift_inj.mpr eq)]; rw [lift_id]

中文:
定理 mk_equiv_of_eq
  条件: (eq : #α = #β)
  结论: #(α ≃ β) = 2 ^ #α
  证明: by
  rw [mk_equiv_of_lift_eq (lift_inj.mpr eq)]; rw [lift_id]

Depends on / 依赖: lift_id, lift_inj, lift_inj.mpr, mk_equiv_of_lift_eq
-/
theorem mk_equiv_of_eq (eq : #α = #β) : #(α ≃ β) = 2 ^ #α := by
  rw [mk_equiv_of_lift_eq (lift_inj.mpr eq)]; rw [lift_id]

/--
theorem `mk_embedding_eq_arrow_of_lift_le` / 定理 `mk_embedding_eq_arrow_of_lift_le`

English:
theorem mk_embedding_eq_arrow_of_lift_le
  given: (lle : lift.{u} #β' <= lift.{v} #α)
  proof: (mk_embedding_le_arrow _ _).antisymm by
    conv_rhs => rw [← (Equiv.embeddingCongr (.refl _)
      (Cardinal.eq.mp <| mul_eq_self <| aleph0_le_mk α).some).cardinal_eq]
    obtain ⟨e⟩ := lift_mk_le'.mp lle
    exact ⟨⟨fun f => ⟨fun b => ⟨e b, f b⟩, fun _ _ h => e.injective congr(Prod.fst $h)⟩,
      fun f g h => funext fun b => congr(Prod.snd <| $h b)⟩⟩

中文:
定理 mk_embedding_eq_arrow_of_lift_le
  条件: (lle : lift.{u} #β' <= lift.{v} #α)
  证明: (mk_embedding_le_arrow _ _).antisymm by
    conv_rhs => rw [← (Equiv.embeddingCongr (.refl _)
      (Cardinal.eq.mp <| mul_eq_self <| aleph0_le_mk α).some).cardinal_eq]
    obtain ⟨e⟩ := lift_mk_le'.mp lle
    exact ⟨⟨fun f => ⟨fun b => ⟨e b, f b⟩, fun _ _ h => e.injective congr(Prod.fst $h)⟩,
      fun f g h => funext fun b => congr(Prod.snd <| $h b)⟩⟩

Depends on / 依赖: Cardinal, Cardinal.eq.mp, Equiv.embeddingCongr, Prod.fst, Prod.snd, aleph0_le_mk, antisymm, cardinal_eq, conv_rhs, e.injective, embeddingCongr, injective, lift_mk_le, mk_embedding_le_arrow, mul_eq_self
-/
theorem mk_embedding_eq_arrow_of_lift_le (lle : lift.{u} #β' <= lift.{v} #α) :
    #(β' ↪ α) = #(β' -> α) :=
(mk_embedding_le_arrow _ _).antisymm by
    conv_rhs => rw [← (Equiv.embeddingCongr (.refl _)
      (Cardinal.eq.mp <| mul_eq_self <| aleph0_le_mk α).some).cardinal_eq]
    obtain ⟨e⟩ := lift_mk_le'.mp lle
    exact ⟨⟨fun f => ⟨fun b => ⟨e b, f b⟩, fun _ _ h => e.injective congr(Prod.fst $h)⟩,
      fun f g h => funext fun b => congr(Prod.snd <| $h b)⟩⟩

/--
theorem `mk_embedding_eq_arrow_of_le` / 定理 `mk_embedding_eq_arrow_of_le`

English:
theorem mk_embedding_eq_arrow_of_le
  given: (le : #β <= #α)
  statement: #(β ↪ α) = #(β -> α)
  proof: mk_embedding_eq_arrow_of_lift_le (lift_le.mpr le)

中文:
定理 mk_embedding_eq_arrow_of_le
  条件: (le : #β <= #α)
  结论: #(β ↪ α) = #(β -> α)
  证明: mk_embedding_eq_arrow_of_lift_le (lift_le.mpr le)

Depends on / 依赖: lift_le, lift_le.mpr, mk_embedding_eq_arrow_of_lift_le
-/
theorem mk_embedding_eq_arrow_of_le (le : #β <= #α) : #(β ↪ α) = #(β -> α) :=
  mk_embedding_eq_arrow_of_lift_le (lift_le.mpr le)

/--
theorem `mk_surjective_eq_arrow_of_lift_le` / 定理 `mk_surjective_eq_arrow_of_lift_le`

English:
theorem mk_surjective_eq_arrow_of_lift_le
  given: (lle : lift.{u} #β' <= lift.{v} #α)
  proof: (mk_set_le _).antisymm
    have ⟨e⟩ : Nonempty (α ≃ α oplus β') := by
      simp_rw [← lift_mk_eq', mk_sum, lift_add, lift_lift]; rw [lift_umax.{u, v}, eq_comm]
      exact add_eq_left (aleph0_le_lift.mpr <| aleph0_le_mk α) lle
    ⟨⟨fun f => ⟨fun a => (e a).elim f id, fun b => ⟨e.symm (.inr b), congr_arg _ (e.right_inv _)⟩⟩,
      fun f g h => funext fun a => by
        simpa only [e.apply_symm_apply] using! congr_fun (Subtype.ext_iff.mp h) (e.symm <| .inl a)⟩⟩

中文:
定理 mk_surjective_eq_arrow_of_lift_le
  条件: (lle : lift.{u} #β' <= lift.{v} #α)
  证明: (mk_set_le _).antisymm
    have ⟨e⟩ : Nonempty (α ≃ α oplus β') := by
      simp_rw [← lift_mk_eq', mk_sum, lift_add, lift_lift]; rw [lift_umax.{u, v}, eq_comm]
      exact add_eq_left (aleph0_le_lift.mpr <| aleph0_le_mk α) lle
    ⟨⟨fun f => ⟨fun a => (e a).elim f id, fun b => ⟨e.symm (.inr b), congr_arg _ (e.right_inv _)⟩⟩,
      fun f g h => funext fun a => by
        simpa only [e.apply_symm_apply] using! congr_fun (Subtype.ext_iff.mp h) (e.symm <| .inl a)⟩⟩

Depends on / 依赖: Nonempty, Subtype, Subtype.ext_iff.mp, add_eq_left, aleph0_le_lift, aleph0_le_lift.mpr, aleph0_le_mk, antisymm, apply_symm_apply, congr_arg, congr_fun, e.apply_symm_apply, e.right_inv, e.symm, eq_comm, ext_iff, lift_add, lift_lift, lift_mk_eq, lift_umax
-/
theorem mk_surjective_eq_arrow_of_lift_le (lle : lift.{u} #β' <= lift.{v} #α) :
    #{f : α -> β' | Surjective f} = #(α -> β') :=
(mk_set_le _).antisymm
    have ⟨e⟩ : Nonempty (α ≃ α oplus β') := by
      simp_rw [← lift_mk_eq', mk_sum, lift_add, lift_lift]; rw [lift_umax.{u, v}, eq_comm]
      exact add_eq_left (aleph0_le_lift.mpr <| aleph0_le_mk α) lle
    ⟨⟨fun f => ⟨fun a => (e a).elim f id, fun b => ⟨e.symm (.inr b), congr_arg _ (e.right_inv _)⟩⟩,
      fun f g h => funext fun a => by
        simpa only [e.apply_symm_apply] using! congr_fun (Subtype.ext_iff.mp h) (e.symm <| .inl a)⟩⟩

/--
theorem `mk_surjective_eq_arrow_of_le` / 定理 `mk_surjective_eq_arrow_of_le`

English:
theorem mk_surjective_eq_arrow_of_le
  given: (le : #β <= #α)
  statement: #{f : α -> β | Surjective f} = #(α -> β)
  proof: mk_surjective_eq_arrow_of_lift_le (lift_le.mpr le)

中文:
定理 mk_surjective_eq_arrow_of_le
  条件: (le : #β <= #α)
  结论: #{f : α -> β | 满射 f} = #(α -> β)
  证明: mk_surjective_eq_arrow_of_lift_le (lift_le.mpr le)

Depends on / 依赖: lift_le, lift_le.mpr, mk_surjective_eq_arrow_of_lift_le
-/
theorem mk_surjective_eq_arrow_of_le (le : #β <= #α) : #{f : α -> β | Surjective f} = #(α -> β) :=
  mk_surjective_eq_arrow_of_lift_le (lift_le.mpr le)

end Function

@[simp]
/--
theorem `mk_list_eq_mk` / 定理 `mk_list_eq_mk`

English:
theorem mk_list_eq_mk
  given: (α : Type u) [Infinite α]
  statement: #(List α) = #α
  proof: have H1 : ℵ₀ <= #α := aleph0_le_mk α
Eq.symm
le_antisymm ((le_def _ _).2 ⟨⟨fun a => [a], fun _ => by simp⟩⟩)
      calc
        #(List α) = sum fun n : Nat => #α ^ (n : Cardinal.{u}) := mk_list_eq_sum_pow α
        _ <= sum fun _ : Nat => #α := sum_le_sum _ _ fun n => pow_le H1 natCast_lt_aleph0
        _ = #α := by simp [H1]

中文:
定理 mk_list_eq_mk
  条件: (α : 类型u) [无限 α]
  结论: #(列表 α) = #α
  证明: have H1 : ℵ₀ <= #α := aleph0_le_mk α
Eq.symm
le_antisymm ((le_def _ _).2 ⟨⟨fun a => [a], fun _ => by simp⟩⟩)
      calc
        #(List α) = sum fun n : Nat => #α ^ (n : Cardinal.{u}) := mk_list_eq_sum_pow α
        _ <= sum fun _ : Nat => #α := sum_le_sum _ _ fun n => pow_le H1 natCast_lt_aleph0
        _ = #α := by simp [H1]

Depends on / 依赖: Cardinal, Eq.symm, aleph0_le_mk, le_antisymm, le_def, mk_list_eq_sum_pow, natCast_lt_aleph0, pow_le, sum_le_sum
-/
theorem mk_list_eq_mk (α : Type u) [Infinite α] : #(List α) = #α :=
  have H1 : ℵ₀ <= #α := aleph0_le_mk α
Eq.symm
le_antisymm ((le_def _ _).2 ⟨⟨fun a => [a], fun _ => by simp⟩⟩)
      calc
        #(List α) = sum fun n : Nat => #α ^ (n : Cardinal.{u}) := mk_list_eq_sum_pow α
        _ <= sum fun _ : Nat => #α := sum_le_sum _ _ fun n => pow_le H1 natCast_lt_aleph0
        _ = #α := by simp [H1]

/--
theorem `mk_list_eq_aleph0` / 定理 `mk_list_eq_aleph0`

English:
theorem mk_list_eq_aleph0
  given: (α : Type u) [Countable α] [Nonempty α]
  statement: #(List α) = ℵ₀
  proof: mk_le_aleph0.antisymm (aleph0_le_mk _)

中文:
定理 mk_list_eq_aleph0
  条件: (α : 类型u) [可数 α] [非空 α]
  结论: #(列表 α) = ℵ₀
  证明: mk_le_aleph0.antisymm (aleph0_le_mk _)

Depends on / 依赖: aleph0_le_mk, antisymm, mk_le_aleph0, mk_le_aleph0.antisymm
-/
theorem mk_list_eq_aleph0 (α : Type u) [Countable α] [Nonempty α] : #(List α) = ℵ₀ :=
  mk_le_aleph0.antisymm (aleph0_le_mk _)

/--
theorem `mk_list_eq_max` / 定理 `mk_list_eq_max`

English:
theorem mk_list_eq_max
  given: (α : Type u) [Nonempty α]
  statement: #(List α) = max ℵ₀ #α
  proof: by
  cases finite_or_infinite α
  · rw [mk_list_eq_aleph0, eq_comm, max_eq_left]
    exact mk_le_aleph0
  · rw [mk_list_eq_mk, eq_comm, max_eq_right]
    exact aleph0_le_mk α

中文:
定理 mk_list_eq_max
  条件: (α : 类型u) [非空 α]
  结论: #(列表 α) = 最大值 ℵ₀ #α
  证明: by
  cases finite_or_infinite α
  · rw [mk_list_eq_aleph0, eq_comm, max_eq_left]
    exact mk_le_aleph0
  · rw [mk_list_eq_mk, eq_comm, max_eq_right]
    exact aleph0_le_mk α

Depends on / 依赖: aleph0_le_mk, eq_comm, finite_or_infinite, max_eq_left, max_eq_right, mk_le_aleph0, mk_list_eq_aleph0, mk_list_eq_mk
-/
theorem mk_list_eq_max (α : Type u) [Nonempty α] : #(List α) = max ℵ₀ #α := by
  cases finite_or_infinite α
  · rw [mk_list_eq_aleph0, eq_comm, max_eq_left]
    exact mk_le_aleph0
  · rw [mk_list_eq_mk, eq_comm, max_eq_right]
    exact aleph0_le_mk α

-- TODO: standardize whether we write `max ℵ₀ x` or `max x ℵ₀`.
/--
theorem `mk_list_eq_max_mk_aleph0` / 定理 `mk_list_eq_max_mk_aleph0`

English:
theorem mk_list_eq_max_mk_aleph0
  given: (α : Type u) [Nonempty α]
  statement: #(List α) = max #α ℵ₀
  proof: by
  rw [mk_list_eq_max]; rw [max_comm]

中文:
定理 mk_list_eq_max_mk_aleph0
  条件: (α : 类型u) [非空 α]
  结论: #(列表 α) = 最大值 #α ℵ₀
  证明: by
  rw [mk_list_eq_max]; rw [max_comm]

Depends on / 依赖: max_comm, mk_list_eq_max
-/
theorem mk_list_eq_max_mk_aleph0 (α : Type u) [Nonempty α] : #(List α) = max #α ℵ₀ := by
  rw [mk_list_eq_max]; rw [max_comm]

/--
theorem `sum_pow_eq_max_aleph0` / 定理 `sum_pow_eq_max_aleph0`

English:
theorem sum_pow_eq_max_aleph0
  given: {x : Cardinal} (h : x != 0)
  statement: sum (fun n => x ^ n) = max ℵ₀ x
  proof: by
  have := nonempty_out h
  conv_lhs => rw [← x.mk_out, ← mk_list_eq_sum_pow, mk_list_eq_max, mk_out]

中文:
定理 sum_pow_eq_max_aleph0
  条件: {x : 基数} (h : x != 0)
  结论: 求和 (fun n => x ^ n) = 最大值 ℵ₀ x
  证明: by
  have := nonempty_out h
  conv_lhs => rw [← x.mk_out, ← mk_list_eq_sum_pow, mk_list_eq_max, mk_out]

Depends on / 依赖: conv_lhs, mk_list_eq_max, mk_list_eq_sum_pow, mk_out, nonempty_out, x.mk_out
-/
theorem sum_pow_eq_max_aleph0 {x : Cardinal} (h : x != 0) : sum (fun n => x ^ n) = max ℵ₀ x := by
  have := nonempty_out h
  conv_lhs => rw [← x.mk_out, ← mk_list_eq_sum_pow, mk_list_eq_max, mk_out]

/--
theorem `mk_list_le_max` / 定理 `mk_list_le_max`

English:
theorem mk_list_le_max
  given: (α : Type u)
  statement: #(List α) <= max ℵ₀ #α
  proof: by
  cases finite_or_infinite α
  · exact mk_le_aleph0.trans (le_max_left _ _)
  · rw [mk_list_eq_mk]
    apply le_max_right

中文:
定理 mk_list_le_max
  条件: (α : 类型u)
  结论: #(列表 α) <= 最大值 ℵ₀ #α
  证明: by
  cases finite_or_infinite α
  · exact mk_le_aleph0.trans (le_max_left _ _)
  · rw [mk_list_eq_mk]
    apply le_max_right

Depends on / 依赖: finite_or_infinite, le_max_left, le_max_right, mk_le_aleph0, mk_le_aleph0.trans, mk_list_eq_mk
-/
theorem mk_list_le_max (α : Type u) : #(List α) <= max ℵ₀ #α := by
  cases finite_or_infinite α
  · exact mk_le_aleph0.trans (le_max_left _ _)
  · rw [mk_list_eq_mk]
    apply le_max_right

/--
theorem `sum_pow_le_max_aleph0` / 定理 `sum_pow_le_max_aleph0`

English:
theorem sum_pow_le_max_aleph0
  given: (x : Cardinal)
  statement: sum (fun n => x ^ n) <= max ℵ₀ x
  proof: by
  rw [← x.mk_out]; rw [← mk_list_eq_sum_pow]
  exact mk_list_le_max _

@[simp]

中文:
定理 sum_pow_le_max_aleph0
  条件: (x : 基数)
  结论: 求和 (fun n => x ^ n) <= 最大值 ℵ₀ x
  证明: by
  rw [← x.mk_out]; rw [← mk_list_eq_sum_pow]
  exact mk_list_le_max _

@[simp]

Depends on / 依赖: mk_list_eq_sum_pow, mk_list_le_max, mk_out, x.mk_out
-/
theorem sum_pow_le_max_aleph0 (x : Cardinal) : sum (fun n => x ^ n) <= max ℵ₀ x := by
  rw [← x.mk_out]; rw [← mk_list_eq_sum_pow]
  exact mk_list_le_max _

@[simp]
/--
theorem `mk_finset_of_infinite` / 定理 `mk_finset_of_infinite`

English:
theorem mk_finset_of_infinite
  given: (α : Type u) [Infinite α]
  statement: #(Finset α) = #α
  proof: by
  classical
exact Eq.symm
le_antisymm (mk_le_of_injective fun _ _ => Finset.singleton_inj.1)
      calc
        #(Finset α) <= #(List α) := mk_le_of_surjective List.toFinset_surjective
        _ = #α := mk_list_eq_mk α

中文:
定理 mk_finset_of_infinite
  条件: (α : 类型u) [无限 α]
  结论: #(有限集 α) = #α
  证明: by
  classical
exact Eq.symm
le_antisymm (mk_le_of_injective fun _ _ => Finset.singleton_inj.1)
      calc
        #(Finset α) <= #(List α) := mk_le_of_surjective List.toFinset_surjective
        _ = #α := mk_list_eq_mk α

Depends on / 依赖: Eq.symm, Finset, Finset.singleton_inj, List.toFinset_surjective, classical, le_antisymm, mk_le_of_injective, mk_le_of_surjective, mk_list_eq_mk, singleton_inj, toFinset_surjective
-/
theorem mk_finset_of_infinite (α : Type u) [Infinite α] : #(Finset α) = #α := by
  classical
exact Eq.symm
le_antisymm (mk_le_of_injective fun _ _ => Finset.singleton_inj.1)
      calc
        #(Finset α) <= #(List α) := mk_le_of_surjective List.toFinset_surjective
        _ = #α := mk_list_eq_mk α

/--
theorem `mk_bounded_set_le_of_infinite` / 定理 `mk_bounded_set_le_of_infinite`

English:
theorem mk_bounded_set_le_of_infinite
  given: (α : Type u) [Infinite α] (c : Cardinal)
  proof: by
  rw [← add_one_eq (aleph0_le_mk α)]
  induction c using Cardinal.inductionOn with | _ β
  fapply mk_le_of_surjective
  · intro f
    use Sum.inl ⁻¹' range f
    refine le_trans (mk_preimage_of_injective _ _ fun x y => Sum.inl.inj) ?_
    apply mk_range_le
  rintro ⟨s, ⟨g⟩⟩
  classical
  use fun y => if h : exists x : s, g x = y then Sum.inl (Classical.choose h).val
               else Sum.inr (ULift.up 0)
  apply Subtype.ext; ext x
  constructor
  · rintro ⟨y, h⟩
    dsimp only at h
    by_cases h' : exists z : s, g z = y
    · rw [dif_pos h'] at h
      cases Sum.inl.inj h
      exact (Classical.choose h').2
    · rw [dif_neg h'] at h
      cases h
  · intro h
    have : exists z : s, g z = g ⟨x, h⟩ := ⟨⟨x, h⟩, rfl⟩
    use g ⟨x, h⟩
    dsimp only
    rw [dif_pos this]
    congr
    suffices Classical.choose this = ⟨x, h⟩ from congr_arg Subtype.val this
    apply g.2
    exact Classical.choose_spec this

中文:
定理 mk_bounded_set_le_of_infinite
  条件: (α : 类型u) [无限 α] (c : 基数)
  证明: by
  rw [← add_one_eq (aleph0_le_mk α)]
  induction c using Cardinal.inductionOn with | _ β
  fapply mk_le_of_surjective
  · intro f
    use Sum.inl ⁻¹' range f
    refine le_trans (mk_preimage_of_injective _ _ fun x y => Sum.inl.inj) ?_
    apply mk_range_le
  rintro ⟨s, ⟨g⟩⟩
  classical
  use fun y => if h : exists x : s, g x = y then Sum.inl (Classical.choose h).val
               else Sum.inr (ULift.up 0)
  apply Subtype.ext; ext x
  constructor
  · rintro ⟨y, h⟩
    dsimp only at h
    by_cases h' : exists z : s, g z = y
    · rw [dif_pos h'] at h
      cases Sum.inl.inj h
      exact (Classical.choose h').2
    · rw [dif_neg h'] at h
      cases h
  · intro h
    have : exists z : s, g z = g ⟨x, h⟩ := ⟨⟨x, h⟩, rfl⟩
    use g ⟨x, h⟩
    dsimp only
    rw [dif_pos this]
    congr
    suffices Classical.choose this = ⟨x, h⟩ from congr_arg Subtype.val this
    apply g.2
    exact Classical.choose_spec this

Depends on / 依赖: Cardinal, Cardinal.inductionOn, Classical, Classical.choose, Subtype, Subtype.ext, Sum.inl, Sum.inl.inj, Sum.inr, ULift.up, add_one_eq, aleph0_le_mk, classical, dif_pos, fapply, inductionOn, le_trans, mk_le_of_surjective, mk_preimage_of_injective, mk_range_le
-/
theorem mk_bounded_set_le_of_infinite (α : Type u) [Infinite α] (c : Cardinal) :
    #{ t : Set α // #t <= c } <= #α ^ c := by
  rw [← add_one_eq (aleph0_le_mk α)]
  induction c using Cardinal.inductionOn with | _ β
  fapply mk_le_of_surjective
  · intro f
    use Sum.inl ⁻¹' range f
    refine le_trans (mk_preimage_of_injective _ _ fun x y => Sum.inl.inj) ?_
    apply mk_range_le
  rintro ⟨s, ⟨g⟩⟩
  classical
  use fun y => if h : exists x : s, g x = y then Sum.inl (Classical.choose h).val
               else Sum.inr (ULift.up 0)
  apply Subtype.ext; ext x
  constructor
  · rintro ⟨y, h⟩
    dsimp only at h
    by_cases h' : exists z : s, g z = y
    · rw [dif_pos h'] at h
      cases Sum.inl.inj h
      exact (Classical.choose h').2
    · rw [dif_neg h'] at h
      cases h
  · intro h
    have : exists z : s, g z = g ⟨x, h⟩ := ⟨⟨x, h⟩, rfl⟩
    use g ⟨x, h⟩
    dsimp only
    rw [dif_pos this]
    congr
    suffices Classical.choose this = ⟨x, h⟩ from congr_arg Subtype.val this
    apply g.2
    exact Classical.choose_spec this

/--
theorem `mk_bounded_set_le` / 定理 `mk_bounded_set_le`

English:
theorem mk_bounded_set_le
  given: (α : Type u) (c : Cardinal)
  proof: by
  trans #{ t : Set ((ULift.{u} Nat) oplus α) // #t <= c }
  · refine ⟨Embedding.subtypeMap ?_ ?_⟩
    · apply Embedding.image
      use Sum.inr
      apply Sum.inr.inj
    intro s hs
    exact mk_image_le.trans hs
  apply (mk_bounded_set_le_of_infinite ((ULift.{u} Nat) oplus α) c).trans
  rw [max_comm]; rw [← add_eq_max] <;> rfl

中文:
定理 mk_bounded_set_le
  条件: (α : 类型u) (c : 基数)
  证明: by
  trans #{ t : Set ((ULift.{u} Nat) oplus α) // #t <= c }
  · refine ⟨Embedding.subtypeMap ?_ ?_⟩
    · apply Embedding.image
      use Sum.inr
      apply Sum.inr.inj
    intro s hs
    exact mk_image_le.trans hs
  apply (mk_bounded_set_le_of_infinite ((ULift.{u} Nat) oplus α) c).trans
  rw [max_comm]; rw [← add_eq_max] <;> rfl

Depends on / 依赖: Embedding, Embedding.image, Embedding.subtypeMap, Sum.inr, Sum.inr.inj, add_eq_max, max_comm, mk_bounded_set_le_of_infinite, mk_image_le, mk_image_le.trans, subtypeMap
-/
theorem mk_bounded_set_le (α : Type u) (c : Cardinal) :
    #{ t : Set α // #t <= c } <= max #α ℵ₀ ^ c := by
  trans #{ t : Set ((ULift.{u} Nat) oplus α) // #t <= c }
  · refine ⟨Embedding.subtypeMap ?_ ?_⟩
    · apply Embedding.image
      use Sum.inr
      apply Sum.inr.inj
    intro s hs
    exact mk_image_le.trans hs
  apply (mk_bounded_set_le_of_infinite ((ULift.{u} Nat) oplus α) c).trans
  rw [max_comm]; rw [← add_eq_max] <;> rfl

/--
theorem `mk_bounded_subset_le` / 定理 `mk_bounded_subset_le`

English:
theorem mk_bounded_subset_le
  given: {α : Type u} (s : Set α) (c : Cardinal.{u})
  proof: by
  refine le_trans ?_ (mk_bounded_set_le s c)
  refine ⟨Embedding.codRestrict _ ?_ ?_⟩
  · use fun t => (↑) ⁻¹' t.1
    rintro ⟨t, ht1, ht2⟩ ⟨t', h1t', h2t'⟩ h
    apply Subtype.ext
    dsimp only at h ⊢
    refine (preimage_eq_preimage' ?_ ?_).1 h <;> rw [Subtype.range_coe] <;> assumption
  rintro ⟨t, _, h2t⟩; exact (mk_preimage_of_injective _ _ Subtype.val_injective).trans h2t

中文:
定理 mk_bounded_subset_le
  条件: {α : 类型u} (s : 集合 α) (c : 基数.{u})
  证明: by
  refine le_trans ?_ (mk_bounded_set_le s c)
  refine ⟨Embedding.codRestrict _ ?_ ?_⟩
  · use fun t => (↑) ⁻¹' t.1
    rintro ⟨t, ht1, ht2⟩ ⟨t', h1t', h2t'⟩ h
    apply Subtype.ext
    dsimp only at h ⊢
    refine (preimage_eq_preimage' ?_ ?_).1 h <;> rw [Subtype.range_coe] <;> assumption
  rintro ⟨t, _, h2t⟩; exact (mk_preimage_of_injective _ _ Subtype.val_injective).trans h2t

Depends on / 依赖: Embedding, Embedding.codRestrict, Subtype, Subtype.ext, Subtype.range_coe, Subtype.val_injective, codRestrict, le_trans, mk_bounded_set_le, mk_preimage_of_injective, preimage_eq_preimage, range_coe, val_injective
-/
theorem mk_bounded_subset_le {α : Type u} (s : Set α) (c : Cardinal.{u}) :
    #{ t : Set α // t subseteq s ∧ #t <= c } <= max #s ℵ₀ ^ c := by
  refine le_trans ?_ (mk_bounded_set_le s c)
  refine ⟨Embedding.codRestrict _ ?_ ?_⟩
  · use fun t => (↑) ⁻¹' t.1
    rintro ⟨t, ht1, ht2⟩ ⟨t', h1t', h2t'⟩ h
    apply Subtype.ext
    dsimp only at h ⊢
    refine (preimage_eq_preimage' ?_ ?_).1 h <;> rw [Subtype.range_coe] <;> assumption
  rintro ⟨t, _, h2t⟩; exact (mk_preimage_of_injective _ _ Subtype.val_injective).trans h2t

end computing

/-! ### Properties of `compl` -/
section compl

/--
theorem `mk_compl_of_infinite` / 定理 `mk_compl_of_infinite`

English:
theorem mk_compl_of_infinite
  given: {α : Type*} [Infinite α] (s : Set α) (h2 : #s < #α)
  proof: by
  refine eq_of_add_eq_of_aleph0_le ?_ h2 (aleph0_le_mk α)
  exact mk_sum_compl s

中文:
定理 mk_compl_of_infinite
  条件: {α : 类型} [无限 α] (s : 集合 α) (h2 : #s < #α)
  证明: by
  refine eq_of_add_eq_of_aleph0_le ?_ h2 (aleph0_le_mk α)
  exact mk_sum_compl s

Depends on / 依赖: aleph0_le_mk, eq_of_add_eq_of_aleph0_le, mk_sum_compl
-/
theorem mk_compl_of_infinite {α : Type*} [Infinite α] (s : Set α) (h2 : #s < #α) :
    #(sᶜ : Set α) = #α := by
  refine eq_of_add_eq_of_aleph0_le ?_ h2 (aleph0_le_mk α)
  exact mk_sum_compl s

/--
theorem `mk_compl_finset_of_infinite` / 定理 `mk_compl_finset_of_infinite`

English:
theorem mk_compl_finset_of_infinite
  given: {α : Type*} [Infinite α] (s : Finset α)
  proof: by
  apply mk_compl_of_infinite
  exact (finset_card_lt_aleph0 s).trans_le (aleph0_le_mk α)

中文:
定理 mk_compl_finset_of_infinite
  条件: {α : 类型} [无限 α] (s : 有限集 α)
  证明: by
  apply mk_compl_of_infinite
  exact (finset_card_lt_aleph0 s).trans_le (aleph0_le_mk α)

Depends on / 依赖: aleph0_le_mk, finset_card_lt_aleph0, mk_compl_of_infinite, trans_le
-/
theorem mk_compl_finset_of_infinite {α : Type*} [Infinite α] (s : Finset α) :
    #((↑s)ᶜ : Set α) = #α := by
  apply mk_compl_of_infinite
  exact (finset_card_lt_aleph0 s).trans_le (aleph0_le_mk α)

/--
theorem `mk_compl_eq_mk_compl_infinite` / 定理 `mk_compl_eq_mk_compl_infinite`

English:
theorem mk_compl_eq_mk_compl_infinite
  statement: {α : Type*} [Infinite α] {s t : Set α} (hs : #s < #α)
  proof: by
  rw [mk_compl_of_infinite s hs]; rw [mk_compl_of_infinite t ht]

中文:
定理 mk_compl_eq_mk_compl_infinite
  结论: {α : 类型} [无限 α] {s t : 集合 α} (hs : #s < #α)
  证明: by
  rw [mk_compl_of_infinite s hs]; rw [mk_compl_of_infinite t ht]

Depends on / 依赖: mk_compl_of_infinite
-/
theorem mk_compl_eq_mk_compl_infinite {α : Type*} [Infinite α] {s t : Set α} (hs : #s < #α)
    (ht : #t < #α) : #(sᶜ : Set α) = #(tᶜ : Set α) := by
  rw [mk_compl_of_infinite s hs]; rw [mk_compl_of_infinite t ht]

/--
theorem `mk_compl_eq_mk_compl_finite_lift` / 定理 `mk_compl_eq_mk_compl_finite_lift`

English:
theorem mk_compl_eq_mk_compl_finite_lift
  statement: {α : Type u} {β : Type v} [Finite α] {s : Set α}
  proof: by
  cases nonempty_fintype α
  rcases lift_mk_eq'.1 h1 with ⟨e⟩; let : Fintype β := Fintype.ofEquiv α e
  replace h1 : Fintype.card α = Fintype.card β := (Fintype.ofEquiv_card _).symm
  classical
    lift s to Finset α using s.toFinite
    lift t to Finset β using t.toFinite
    simp only [Finset.coe_sort_coe, mk_fintype, Fintype.card_coe, lift_natCast, Nat.cast_inj] at h2
    simp only [← Finset.coe_compl, Finset.coe_sort_coe, mk_coe_finset, Finset.card_compl,
      lift_natCast, h1, h2]

中文:
定理 mk_compl_eq_mk_compl_finite_lift
  结论: {α : 类型u} {β : 类型v} [有限 α] {s : 集合 α}
  证明: by
  cases nonempty_fintype α
  rcases lift_mk_eq'.1 h1 with ⟨e⟩; let : Fintype β := Fintype.ofEquiv α e
  replace h1 : Fintype.card α = Fintype.card β := (Fintype.ofEquiv_card _).symm
  classical
    lift s to Finset α using s.toFinite
    lift t to Finset β using t.toFinite
    simp only [Finset.coe_sort_coe, mk_fintype, Fintype.card_coe, lift_natCast, Nat.cast_inj] at h2
    simp only [← Finset.coe_compl, Finset.coe_sort_coe, mk_coe_finset, Finset.card_compl,
      lift_natCast, h1, h2]

Depends on / 依赖: Finset, Finset.card_compl, Finset.coe_compl, Finset.coe_sort_coe, Fintype, Fintype.card, Fintype.card_coe, Fintype.ofEquiv, Fintype.ofEquiv_card, Nat.cast_inj, card_coe, card_compl, cast_inj, classical, coe_compl, coe_sort_coe, lift_mk_eq, lift_natCast, mk_coe_finset, mk_fintype
-/
theorem mk_compl_eq_mk_compl_finite_lift {α : Type u} {β : Type v} [Finite α] {s : Set α}
    {t : Set β} (h1 : (lift.{v, u} #α) = (lift.{u, v} #β))
    (h2 : lift.{v, u} #s = lift.{u, v} #t) :
    lift.{v} #(sᶜ : Set α) = lift.{u} #(tᶜ : Set β) := by
  cases nonempty_fintype α
  rcases lift_mk_eq'.1 h1 with ⟨e⟩; let : Fintype β := Fintype.ofEquiv α e
  replace h1 : Fintype.card α = Fintype.card β := (Fintype.ofEquiv_card _).symm
  classical
    lift s to Finset α using s.toFinite
    lift t to Finset β using t.toFinite
    simp only [Finset.coe_sort_coe, mk_fintype, Fintype.card_coe, lift_natCast, Nat.cast_inj] at h2
    simp only [← Finset.coe_compl, Finset.coe_sort_coe, mk_coe_finset, Finset.card_compl,
      lift_natCast, h1, h2]

/--
theorem `mk_compl_eq_mk_compl_finite` / 定理 `mk_compl_eq_mk_compl_finite`

English:
theorem mk_compl_eq_mk_compl_finite
  statement: {α β : Type u} [Finite α] {s : Set α} {t : Set β}
  proof: by
  rw [← lift_inj.{u]; rw [u}]
  apply mk_compl_eq_mk_compl_finite_lift.{u, u}
  <;> rwa [lift_inj]

中文:
定理 mk_compl_eq_mk_compl_finite
  结论: {α β : 类型u} [有限 α] {s : 集合 α} {t : 集合 β}
  证明: by
  rw [← lift_inj.{u]; rw [u}]
  apply mk_compl_eq_mk_compl_finite_lift.{u, u}
  <;> rwa [lift_inj]

Depends on / 依赖: lift_inj, mk_compl_eq_mk_compl_finite_lift
-/
theorem mk_compl_eq_mk_compl_finite {α β : Type u} [Finite α] {s : Set α} {t : Set β}
    (h1 : #α = #β) (h : #s = #t) : #(sᶜ : Set α) = #(tᶜ : Set β) := by
  rw [← lift_inj.{u]; rw [u}]
  apply mk_compl_eq_mk_compl_finite_lift.{u, u}
  <;> rwa [lift_inj]

/--
theorem `mk_compl_eq_mk_compl_finite_same` / 定理 `mk_compl_eq_mk_compl_finite_same`

English:
theorem mk_compl_eq_mk_compl_finite_same
  given: {α : Type u} [Finite α] {s t : Set α} (h : #s = #t)
  proof: mk_compl_eq_mk_compl_finite.{u} rfl h

中文:
定理 mk_compl_eq_mk_compl_finite_same
  条件: {α : 类型u} [有限 α] {s t : 集合 α} (h : #s = #t)
  证明: mk_compl_eq_mk_compl_finite.{u} rfl h

Depends on / 依赖: mk_compl_eq_mk_compl_finite
-/
theorem mk_compl_eq_mk_compl_finite_same {α : Type u} [Finite α] {s t : Set α} (h : #s = #t) :
    #(sᶜ : Set α) = #(tᶜ : Set α) :=
  mk_compl_eq_mk_compl_finite.{u} rfl h

end compl

/-! ### Extending an injection to an equiv -/
section extend

/--
theorem `extend_function` / 定理 `extend_function`

English:
theorem extend_function
  statement: {α β : Type*} {s : Set α} (f : s ↪ β)
  proof: by
  classical
  have := h; obtain ⟨g⟩ := this
  let h : α ≃ β :=
    (Set.sumCompl (s : Set α)).symm.trans
      ((sumCongr (Equiv.ofInjective f f.2) g).trans (Set.sumCompl (range f)))
  refine ⟨h, ?_⟩; rintro ⟨x, hx⟩; simp [h, Set.sumCompl_symm_apply_of_mem, hx]

中文:
定理 extend_function
  结论: {α β : 类型} {s : 集合 α} (f : s ↪ β)
  证明: by
  classical
  have := h; obtain ⟨g⟩ := this
  let h : α ≃ β :=
    (Set.sumCompl (s : Set α)).symm.trans
      ((sumCongr (Equiv.ofInjective f f.2) g).trans (Set.sumCompl (range f)))
  refine ⟨h, ?_⟩; rintro ⟨x, hx⟩; simp [h, Set.sumCompl_symm_apply_of_mem, hx]

Depends on / 依赖: Equiv.ofInjective, Set.sumCompl, Set.sumCompl_symm_apply_of_mem, classical, ofInjective, sumCompl, sumCompl_symm_apply_of_mem, sumCongr, symm.trans
-/
theorem extend_function {α β : Type*} {s : Set α} (f : s ↪ β)
    (h : Nonempty ((sᶜ : Set α) ≃ ((range f)ᶜ : Set β))) : exists g : α ≃ β, forall x : s, g x = f x := by
  classical
  have := h; obtain ⟨g⟩ := this
  let h : α ≃ β :=
    (Set.sumCompl (s : Set α)).symm.trans
      ((sumCongr (Equiv.ofInjective f f.2) g).trans (Set.sumCompl (range f)))
  refine ⟨h, ?_⟩; rintro ⟨x, hx⟩; simp [h, Set.sumCompl_symm_apply_of_mem, hx]

/--
theorem `extend_function_finite` / 定理 `extend_function_finite`

English:
theorem extend_function_finite
  statement: {α : Type u} {β : Type v} [Finite α] {s : Set α} (f : s ↪ β)
  proof: by
  apply extend_function.{u, v} f
  rw [← lift_mk_eq'] at h
  rw [← lift_mk_eq']; rw [mk_compl_eq_mk_compl_finite_lift h]
  rw [mk_range_eq_of_injective]; exact f.2

中文:
定理 extend_function_finite
  结论: {α : 类型u} {β : 类型v} [有限 α] {s : 集合 α} (f : s ↪ β)
  证明: by
  apply extend_function.{u, v} f
  rw [← lift_mk_eq'] at h
  rw [← lift_mk_eq']; rw [mk_compl_eq_mk_compl_finite_lift h]
  rw [mk_range_eq_of_injective]; exact f.2

Depends on / 依赖: extend_function, lift_mk_eq, mk_compl_eq_mk_compl_finite_lift, mk_range_eq_of_injective
-/
theorem extend_function_finite {α : Type u} {β : Type v} [Finite α] {s : Set α} (f : s ↪ β)
    (h : Nonempty (α ≃ β)) : exists g : α ≃ β, forall x : s, g x = f x := by
  apply extend_function.{u, v} f
  rw [← lift_mk_eq'] at h
  rw [← lift_mk_eq']; rw [mk_compl_eq_mk_compl_finite_lift h]
  rw [mk_range_eq_of_injective]; exact f.2

/--
theorem `extend_function_of_lt` / 定理 `extend_function_of_lt`

English:
theorem extend_function_of_lt
  statement: {α β : Type*} {s : Set α} (f : s ↪ β) (hs : #s < #α)
  proof: by
  cases fintypeOrInfinite α
  · exact extend_function_finite f h
  · apply extend_function f
    obtain ⟨g⟩ := id h
    have := Infinite.of_injective _ g.injective
    rw [← lift_mk_eq'] at h ⊢
    rwa [mk_compl_of_infinite s hs, mk_compl_of_infinite]
    rwa [← lift_lt, mk_range_eq_of_injective f.injective, ← h, lift_lt]

中文:
定理 extend_function_of_lt
  结论: {α β : 类型} {s : 集合 α} (f : s ↪ β) (hs : #s < #α)
  证明: by
  cases fintypeOrInfinite α
  · exact extend_function_finite f h
  · apply extend_function f
    obtain ⟨g⟩ := id h
    have := Infinite.of_injective _ g.injective
    rw [← lift_mk_eq'] at h ⊢
    rwa [mk_compl_of_infinite s hs, mk_compl_of_infinite]
    rwa [← lift_lt, mk_range_eq_of_injective f.injective, ← h, lift_lt]

Depends on / 依赖: Infinite, Infinite.of_injective, extend_function, extend_function_finite, f.injective, fintypeOrInfinite, g.injective, injective, lift_lt, lift_mk_eq, mk_compl_of_infinite, mk_range_eq_of_injective, of_injective
-/
theorem extend_function_of_lt {α β : Type*} {s : Set α} (f : s ↪ β) (hs : #s < #α)
    (h : Nonempty (α ≃ β)) : exists g : α ≃ β, forall x : s, g x = f x := by
  cases fintypeOrInfinite α
  · exact extend_function_finite f h
  · apply extend_function f
    obtain ⟨g⟩ := id h
    have := Infinite.of_injective _ g.injective
    rw [← lift_mk_eq'] at h ⊢
    rwa [mk_compl_of_infinite s hs, mk_compl_of_infinite]
    rwa [← lift_lt, mk_range_eq_of_injective f.injective, ← h, lift_lt]

end extend
end Cardinal
