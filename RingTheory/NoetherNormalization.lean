...
variable {f v} in
/-
**NoetherNormalization.lt_up** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormalization`。
...
and `X_0` to `X_0`. -/
/-
**NoetherNormalization.T1** 是 Mathlib 中的一个缩写定义，位于命名空间 `NoetherNormalization`。
形式化陈述：T1 (c : k) : MvPolynomial (Fin (n + 1)) k ->ₐ[k] MvPolynomial (Fin (n + 1)
) k
参数：c : k。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**NoetherNormalization.t1_comp_t1_neg** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormaliz
ation`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**NoetherNormalization.T** 是 Mathlib 中的一个缩写定义，位于命名空间 `NoetherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**NoetherNormalization.sum_r_mul_ne** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormalizat
ion`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**NoetherNormalization.degreeOf_zero_t** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormali
zation`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**NoetherNormalization.degreeOf_t_ne_of_ne** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNor
malization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`T` maps different monomials of `f` to polynomials with different degrees in `X_
0`.
-/
private lemma degreeOf_t_ne_of_ne (hv : v ∈ f.support) (hw : w ∈ f.support) (ne : v ≠ w) :
  ...
  exact lt_of_le_of_lt ((monomial_le_degreeOf i ‹_›).trans (degreeOf_le_totalDegree f i))
    (by lia)
/-
**NoetherNormalization.leadingCoeff_finSuccEquiv_t** 是 Mathlib 中的一个引理，位于命名空间 `No
etherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**NoetherNormalization.T_leadingcoeff_isUnit** 是 Mathlib 中的一个引理，位于命名空间 `NoetherN
ormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`T` maps `f` into some polynomial in `X_0` such that the leading coefficient is 
invertible.
-/
...
/-
**NoetherNormalization.hom1** 是 Mathlib 中的一个缩写定义，位于命名空间 `NoetherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**NoetherNormalization.hom1_isIntegral** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormali
zation`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**NoetherNormalization.eqv1** 是 Mathlib 中的一个缩写定义，位于命名空间 `NoetherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
induced by `T`. -/
/-
**NoetherNormalization.eqv2** 是 Mathlib 中的一个缩写定义，位于命名空间 `NoetherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**NoetherNormalization.hom2** 是 Mathlib 中的一个定义，位于命名空间 `NoetherNormalization`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`hom2` is the composition of maps above, from `k[X_0,...X_(n-1)]` to `k[X_0,...X
_n]/I`.
-/
...
/-
**NoetherNormalization.hom2_isIntegral** 是 Mathlib 中的一个引理，位于命名空间 `NoetherNormali
zation`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**exists_integral_inj_algHom_of_quotient** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：exists_integral_inj_algHom_of_quotient (I : Ideal (MvPolynomial (Fin n) k)
) (hi : I != ⊤) : exists s <= n, exists g : (MvPolynomial (Fin s) k) ->ₐ[k] ((Mv
Polynomial (Fin n) k) ⧸ I), Function.Injective g ∧ g.IsIntegral
参数：I : Ideal (MvPolynomial (Fin n) k)；hi : I != ⊤。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `Ideal.Quotient.eq`：∀ {R : Type u} [inst : Ring R] {I : Ideal R} {x y : R
 ...
 x - y …
· 使用定理 `Ideal.Quotient.mkₐ_eq_mk`：∀ (R₁ : Type u_1) {A : Type u_3} [inst : CommS
emiring R₁] [inst_1 : Ring A] [inst_2 : Algebra R₁ A] (I : Ideal A)   [inst_3 : 
I.IsTwoSided],…
...
_3 : I.IsTwoSided],…
· 使用定理 `Ideal.Quotient.mk_bijective_iff_eq_bot`：∀ {A : Type u_3} [inst : Ring A]
 (I : Ideal A) [inst_1 : I.IsTwoSided],   Function.Bijective ⇑(Ideal.Quotient.mk
 I) ↔ I = ⊥
...
@[stacks 00OW]
/-
**exists_integral_inj_algHom_of_fg** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：exists_integral_inj_algHom_of_fg : exists s, exists g : (MvPolynomial (Fin
 s) k) ->ₐ[k] R, Function.Injective g ∧ g.IsIntegral
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
...
mClass F R S] (…
· 使用定理 `exists_integral_inj_algHom_of_quotient`：exists_integral_inj_algHom_of_qu
otient (I : Ideal (MvPolynomial (Fin n) k)) (hi : I != ⊤) : exists s <= n, exist
s g : (MvPolynomial (Fin s) …
...
For a finitely generated algebra `A` over a field `k`,
there exists a natural number `s` and an injective homomorphism
from `k[X_0, X_1, ..., X_(s-1)]` to `A` such that `A` is integral over `k[X_0, X
_1, ..., X_(s-1)]`.
-/
...
/-
**exists_finite_inj_algHom_of_fg** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：exists_finite_inj_algHom_of_fg : exists s, exists g : (MvPolynomial (Fin s
) k) ->ₐ[k] R, Function.Injective g ∧ g.Finite
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `exists_integral_inj_algHom_of_fg`：exists_integral_inj_algHom_of_fg : exi
sts s, exists g : (MvPolynomial (Fin s) k) ->ₐ[k] R, Function.Injective g ∧ g.Is
Integral
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
...
For a finitely generated algebra `A` over a field `k`,
there exists a natural number `s` and an injective homomorphism
from `k[X_0, X_1, ..., X_(s-1)]` to `A` such that `A` is finite over `k[X_0, X_1
, ..., X_(s-1)]`.
-/
