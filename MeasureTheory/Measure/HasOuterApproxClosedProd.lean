...
variable {ι κ Z T : Type*} {X : ι -> Type*} {Y : κ -> Type*}
  {mX : forall i, MeasurableSpace (X i)} [forall i, TopologicalSpace (X i)] [forall i, BorelSpace (X i)]
  [forall i, HasOuterApproxClosed (X i)]
  {mY : forall j, MeasurableSpace (Y j)} [forall j, TopologicalSpace (Y j)] [forall j, BorelSpace (Y j)]
  [forall j, HasOuterApproxClosed (Y j)]
...
/--
lemma `ext_of_lintegral_prod_mul_prod_boundedContinuousFunction` / 引理 `ext_of_lintegral_prod_mul_prod_boundedContinuousFunction`
...
lemma ext_of_lintegral_prod_mul_prod_boundedContinuousFunction
  ...
  let π : Set (Set ((Π i, X i) × (Π j, Y j))) :=
    Set.image2 (fun s t => s ×ˢ t) (Set.univ.pi '' (Set.univ.pi fun _ => {s | IsClosed s}))
      (Set.univ.pi '' (Set.univ.pi fun _ => {t | 

中文:
引理 ext_of_lintegral_prod_mul_prod_boundedContinuousFunction
  证明: by
  have hμν : μ univ = ν univ := by convert! h 1 1 <;> simp
  have : IsFiniteMeasure ν := ⟨by simp [← hμν]⟩
  let π : Set (Set ((Π i, X i) × (Π j, Y j))) :=
...
Depends on / 依赖: IsClosed, IsFiniteMeasure, IsPiSystem, Set.image2, Set.univ.pi, convert, image2
-/
lemma ext_of_lintegral_prod_mul_prod_boundedContinuousFunction
    {μ ν : Measure ((Π i, X i) × (Π j, Y j))} [IsFiniteMeasure μ]
    (h : forall (f : (i : ι) -> X i ->ᵇ Real>=0) (g : (j : κ) -> Y j ->ᵇ Real>=0),
  ...
  have hπ1 : IsPiSystem π := by
    ...
    refine ⟨_, ⟨fun i => s₁ i inter s₂ i, ?_, rfl⟩, _, ⟨fun j => t₁ j inter t₂ j, ?_, rfl⟩, ?_⟩
...
.comp have (p : (Π i, X i) × (Π j, Y j)) := ENNReal.continuous_coe.tendsto _
  ...
  have hp1 (x : Π i, X i) : ∏ i, (s i).indicator (fun _ => (1 : Real>=0)) (x i) =
  ...
  have hp2 (y : Π j, Y j) : ∏ j, (t j).indicator (fun _ => (1 : Real>=0)) (y j) =
  ...
  have h1 : Tendsto (fun n => ∫⁻ p, ((∏ i, (hs i).apprSeq n (p.1 i)) *
        ∏ j, (ht j).apprSeq n (p.2 j) : Real>=0) ∂μ)
      atTop (𝓝 (∫⁻ p, (((Set.univ.pi s) ×ˢ (Set.univ.pi t)).indicator 1 p : Real>=0) ∂μ)) := by
  ...
  have h2 : Tendsto (fun n => ∫⁻ p, ((∏ i, (hs i).apprSeq n (p.1 i)) *
        ∏ j, (ht j).apprSeq n (p.2 j) : Real>=0) ∂μ)
      atTop (𝓝 (∫⁻ p, (((Set.univ.pi s) ×ˢ (Set.univ.pi t)).indicator 1 p : Real>=0) ∂ν)) := by
    simp_rw [coe_mul, h]
    refine tendsto_lintegral_filter_of_dominated_convergence 1
...
/--
lemma `ext_of_integral_prod_mul_prod_boundedContinuousFunction` / 引理 `ext_of_integral_prod_mul_prod_boundedContinuousFunction`
...
lemma ext_of_integral_prod_mul_prod_boundedContinuousFunction
  ...
  · simp only [ofNNReal_finsetProd]
    have {μ : Measure ((Π i, X i) × Π j, Y j)} :
        (∫⁻ p, (∏ i, (f i (p.1 i) : Real>=0∞)) * ∏ j, (g j (p.2 j) : Real>=0∞) ∂μ).toReal =
         

中文:
引理 ext_of_integral_prod_mul_prod_boundedContinuousFunction
  证明: by
  refine ext_of_lintegral_prod_mul_prod_boundedContinuousFunction fun f g => ?_
  rw [← toReal_eq_toReal_iff']
  · simp only [ofNNReal_finsetProd]
...
Depends on / 依赖: AEStrong, Eventually, Eventually.of_forall, Finset, Finset.prod_nonneg, Measure, ext_of_lintegral_prod_mul_prod_boundedContinuousFunction, integral_eq_lintegral_of_nonneg_ae, ofNNReal_finsetProd, ofReal_prod_of_nonneg, of_forall, prod_nonneg, toReal, toReal_eq_toReal_iff
-/
lemma ext_of_integral_prod_mul_prod_boundedContinuousFunction
    {μ ν : Measure ((Π i, X i) × (Π j, Y j))} [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : (i : ι) -> X i ->ᵇ Real) (g : (j : κ) -> Y j ->ᵇ Real),
      ∫ p, (∏ i, f i (p.1 i)) * ∏ j, g j (p.2 j) ∂μ =
      ∫ p, (∏ i, f i (p.1 i)) * ∏ j, g j (p.2 j) ∂ν) :
    μ = ν := by
  refine ext_of_lintegral_prod_mul_prod_boundedContinuousFunction fun f g => ?_
...
/--
lemma `eq_prod_of_integral_prod_mul_prod_boundedContinuousFunction` / 引理 `eq_prod_of_integral_prod_mul_prod_boundedContinuousFunction`

English:
lemma eq_prod_of_integral_prod_mul_prod_boundedContinuousFunction
  statement: {μ : Measure (Π i, X i)}
  proof: ext_of_integral_prod_mul_prod_boundedContinuousFunction fun f g => by rw [h, ← integral_prod_mul]
...
/--
lemma `ext_of_integral_prod_mul_boundedContinuousFunction` / 引理 `ext_of_integral_prod_mul_boundedContinuousFunction`

English:
lemma ext_of_integral_prod_mul_boundedContinuousFunction
  statement: {μ ν : Measure ((Π i, X i) × T)}
  proof: by
  ...
  refine ext_of_integral_prod_mul_prod_
...
Depends on / 依赖: e.map_measurableEquiv_injective.eq_iff, eq_iff, ext_of_integral_prod_mul_prod_boundedContinuousFunction, integral_map_equiv, invFun, left_inv, map_measurableEquiv_injective, right_inv
-/
lemma ext_of_integral_prod_mul_boundedContinuousFunction {μ ν : Measure ((Π i, X i) × T)}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : (i : ι) -> X i ->ᵇ Real) (g : T ->ᵇ Real),
  ...
  rw [integral_map_equiv]; rw [integral_map_equiv]
  simpa [e] using h f (g ())

/--
lemma `eq_prod_of_integral_prod_mul_boundedContinuousFunction` / 引理 `eq_prod_of_integral_prod_mul_boundedContinuousFunction`

English:
lemma eq_prod_of_integral_prod_mul_boundedContinuousFunction
  statement: {μ : Measure (Π i, X i)}
  proof: ext_of_integral_prod_mul_boundedContinuousFunction fun f g => by rw [h, ← integral_prod_mul]
...
/--
lemma `ext_of_integral_mul_prod_boundedContinuousFunction` / 引理 `ext_of_integral_mul_prod_boundedContinuousFunction`

English:
lemma ext_of_integral_mul_prod_boundedContinuousFunction
  statement: {μ ν : Measure (Z × (Π j, Y j))}
  proof: by
...
Depends on / 依赖: e.map_measurableEquiv_injective.eq_iff, eq_iff, ext_of_integral_prod_mul_boundedContinuousFunction, integral_map_equiv, map_measurableEquiv_injective, mul_comm, prodComm
-/
lemma ext_of_integral_mul_prod_boundedContinuousFunction {μ ν : Measure (Z × (Π j, Y j))}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : Z ->ᵇ Real) (g : (j : κ) -> Y j ->ᵇ Real),
...
/--
lemma `eq_prod_of_integral_mul_prod_boundedContinuousFunction` / 引理 `eq_prod_of_integral_mul_prod_boundedContinuousFunction`

English:
lemma eq_prod_of_integral_mul_prod_boundedContinuousFunction
  statement: {μ : Measure Z}
  proof: ext_of_integral_mul_prod_boundedContinuousFunction fun f g => by rw [h, ← integral_prod_mul]
...
set_option linter.flexible false in -- simp followed by fun_prop
/--
lemma `ext_of_integral_mul_boundedContinuousFunction` / 引理 `ext_of_integral_mul_boundedContinuousFunction`

English:
lemma ext_of_integral_mul_boundedContinuousFunction
  statement: {μ ν : Measure (Z × T)}
  proof: by
...
Depends on / 依赖: e.map_measurableEquiv_injective.eq_iff, eq_iff, ext_of_integral_prod_mul_prod_boundedContinuousFunction, funUnique, integral_map_equiv, map_measurableEquiv_injective, prodCongr
-/
lemma ext_of_integral_mul_boundedContinuousFunction {μ ν : Measure (Z × T)}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : Z ->ᵇ Real) (g : T ->ᵇ Real), ∫ p, f p.1 * g p.2 ∂μ = ∫ p, f p.1 * g p.2 ∂ν) :
    μ = ν := by
  let e : (Z × T) ≃ᵐ ((Unit -> Z) × (Unit -> T)) :=
.symm .prodCongr (.funUnique ..) (.funUnique ..)
  rw [← e.map_measurableEquiv_injective.eq_iff]
...
/--
lemma `eq_prod_of_integral_mul_boundedContinuousFunction` / 引理 `eq_prod_of_integral_mul_boundedContinuousFunction`

English:
lemma eq_prod_of_integral_mul_boundedContinuousFunction
  statement: {μ : Measure Z}
  proof: ext_of_integral_mul_boundedContinuousFunction fun f g => by rw [h, ← integral_prod_mul]
...
lemma eq_prod_of_integral_mul_boundedContinuousFunction {μ : Measure Z}
    ...
    [IsFiniteMeasure μ] [IsFiniteMeasure ν] [IsFiniteMeasure ξ]
    (h : forall (f : Z ->ᵇ Real) (g : T ->ᵇ Real), ∫ p, f p.1 * g p.2 ∂ξ = (∫ z, f z ∂μ) * (∫ t, g t ∂ν)) :
    ξ = μ.prod ν :=
...
Depends on / 依赖: Fintype, Fintype.ofFinite, Function, Function.eval, compContinuous, convert, ext_of_integral_prod_mul_prod_boundedContinuousFunction, fun_prop, ofFinite
-/
lemma ext_of_integral_prod_mul_prod_boundedContinuousFunction'
    {μ ν : Measure ((Π i, X i) × (Π j, Y j))} [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : (Π i, X i) ->ᵇ Real) (g : (Π j, Y j) ->ᵇ Real),
...
Depends on / 依赖: Fintype, Fintype.ofFinite, Function, Function.eval, compContinuous, convert, ext_of_integral_prod_mul_boundedContinuousFunction, fun_prop, ofFinite
-/
lemma ext_of_integral_prod_mul_boundedContinuousFunction' {μ ν : Measure ((Π i, X i) × T)}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : (Π i, X i) ->ᵇ Real) (g : T ->ᵇ Real),
      ∫ p, f p.1 * g p.2 ∂μ = ∫ p, f p.1 * g p.2 ∂ν) :
    μ = ν := by
  have := Fintype.ofFinite ι
  refine ext_of_integral_prod_mul_boundedContinuousFunction fun f g => ?_
...
English:
lemma ext_of_integral_mul_prod_boundedContinuousFunction'
  statement: {μ ν : Measure (Z × (Π i, Y i))}
  proof: by
...
Depends on / 依赖: Fintype, Fintype.ofFinite, Function, Function.eval, compContinuous, convert, ext_of_integral_mul_prod_boundedContinuousFunction, fun_prop, ofFinite
-/
lemma ext_of_integral_mul_prod_boundedContinuousFunction' {μ ν : Measure (Z × (Π i, Y i))}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h : forall (f : Z ->ᵇ Real) (g : (Π j, Y j) ->ᵇ Real), ∫ p, f p.1 * g p.2 ∂μ = ∫ p, f p.1 * g p.2 ∂ν) :
    μ = ν := by
  have := Fintype.ofFinite κ
  refine ext_of_integral_mul_prod_boundedContinuousFunction fun f g => ?_
  convert! h f (∏ j, (g j).compContinuous ⟨Function.eval j, by fun_prop⟩) <;> simp
...
lemma eq_prod_of_integral_mul_prod_boundedContinuousFunction' {μ : Measure Z}
    {ν : Measure (Π j, Y j)} {ξ : Measure (Z × (Π j, Y j))}
    [IsFiniteMeasure μ] [IsFiniteMeasure ν] [IsFiniteMeasure ξ]
    (h : forall (f : Z ->ᵇ Real) (g : (Π j, Y j) ->ᵇ Real),
