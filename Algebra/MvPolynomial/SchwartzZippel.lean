...
/--
lemma `schwartz_zippel_sup_sum` / 引理 `schwartz_zippel_sup_sum`

English:
lemma schwartz_zippel_sup_sum
  proof: finSuccEquiv R n p with hp'
    ...
    calc
      ...
      #{x in S ^^ (n + 1) | eval x p = 0} / ∏ i, (#(S i) : Rat>=0)
          -- In the first case, `pₖ` evaluates to `0`.
        = #{x in S ^^ (n + 1) | eval x p = 0 ∧ eval (tail x) pₖ = 0} / ∏ i, (#(S i) : Rat>=0)
          -- In the second case, `pₖ` does not evaluate to `0`.
          + #{x in S ^^ (n + 1) | eval x p = 0 ∧ eval (tail x) pₖ != 0} / ∏ i, (#(S i) : Rat>=0) := by
        rw [← add_div]; rw [← Nat.cast_add]; rw [← card_union_add_card_inter]; rw [filter_union_right]; rw [← filter_and]
        simp [← and_or_left, em, and_and_and_comm]
      _ <= (pₖ.support.sup fun s => ∑ i, (s i / #(S i.succ) : Rat>=0)) + p.degreeOf 0 / #(S 0) := ?_
      _ <= p.support.sup fun s => ∑ i, (s i / #(S i) : Rat>=0) := ?_
    · gcongr ?_ + ?_
      · -- We bound the size of the first set by induction
        calc
          #{x in S ^^ (n + 1) | eval x p = 0 ∧ eval (tail x) pₖ = 0} / ∏ i, (#(S i) : Rat>=0)
            <= #{x in S ^^ (n + 1) | eval (tail x) pₖ = 0} / ∏ i, (#(S i) : Rat>=0) := by
            gcongr with x; exact And.right
          _ = #(S 0) * #{xₜ in tail S ^^ n | eval xₜ pₖ = 0}
              / (#(S 0) * (∏ i, #(S (.succ i)) : Rat>=0)) := by
            rw [card_consEquiv_filter_piFinset S fun x => eval x pₖ = 0]; rw [prod_univ_succ]; rw [tail_def]
            norm_cast
          _ <= #{xₜ in tail S ^^ n | eval xₜ pₖ = 0} / ∏ i, (#(S (.succ i)) : Rat>=0) :=
            mul_div_mul_left_le (by positivity)
          _ <= (pₖ.support.sup fun s => ∑ i, (s i / #(S (.succ i)) : Rat>=0)) :=
      ...
      · -- We bound the second set by noting that if `x` is in it, then `x₀` is the root of
        ...
        calc
          #{x in S ^^ (n + 1) | eval x p = 0 ∧ eval (tail x) pₖ != 0} / ∏ i, (#(S i) : Rat>=0)
            <= ↑(p.degreeOf 0 * ∏ i, #(S (.succ i))) / ∏ i, (#(S i) : Rat>=0) := ?_
          _ = p.degreeOf 0 * (∏ i, #(S (.succ i))) / (#(S 0) * ∏ i, #(S (.succ i))) := by
            norm_cast; rw [prod_univ_succ]
          _ <= (p.degreeOf 0 / #(S 0) : Rat>=0) := mul_div_mul_right_le (by positivity)
        gcongr
        calc
          #{x in S ^^ (n + 1) | eval x p = 0 ∧ eval (tail x) pₖ != 0}
            = #{x in S ^^ (n + 1) | eval (tail x) pₖ != 0 ∧ eval x p = 0} := by simp_rw [and_comm]
          _ = #({xₜ in tail S ^^ n | eval xₜ pₖ != 0}.biUnion fun xₜ => image (fun x₀ => (x₀, xₜ))
                {x₀ in S 0 | eval (cons x₀ xₜ) p = 0}) := by
            rw [← filter_filter]; rw [filter_piFinset_eq_map_consEquiv S (fun r => eval r pₖ != 0)]; rw [filter_map]; rw [card_map]; rw [product_eq_biUnion_right]; rw [filter_biUnion]
            simp [filter_image]
            rfl
          _ <= ∑ xₜ in tail S ^^ n with eval xₜ pₖ != 0,
                #(image (fun x₀ => (x₀, xₜ)) {x₀ in S 0 | eval (cons x₀ xₜ) p = 0}) :=
            card_biUnion_le
          _ <= ∑ xₜ in tail S ^^ n with eval xₜ pₖ != 0, #{x₀ in S 0 | eval (cons x₀ xₜ) p = 0} := by
            gcongr; exact card_image_le
          _ <= ∑ xₜ in tail S ^^ n with eval xₜ pₖ != 0, p.degreeOf 0 := ?_
          _ <= ∑ _xₜ in tail S ^^ n, p.degreeOf 0 := by gcongr; exact filter_subset ..
          _ = p.degreeOf 0 * ∏ i, #(S (.succ i)) := by simp [mul_comm, tail]
        gcongr with xₜ hxₜ
        set pₓ := p'.map (eval xₜ) with hpₓ
        have hpₓdeg : pₓ.natDegree = k := by
          rw [hpₓ]; rw [hk]; rw [Polynomial.natDegree_map_of_leadingCoeff_ne_zero _ (mem_filter.1 hxₜ).2]
have hpₓ₀ : pₓ != 0 := fun h => (mem_filter.1 hxₜ).2 by
          rw [hpₖ]; rw [Polynomial.leadingCoeff]; rw [← hk]; rw [← hpₓdeg]; rw [h]; rw [Polynomial.natDegree_zero]; rw [← Polynomial.coeff_map]; rw [← hpₓ]; rw [h]; rw [Polynomial.coeff_zero]
        calc
          #{x₀ in S 0 | eval (cons x₀ xₜ) p = 0} <= #pₓ.roots.toFinset := by
            gcongr
          ...
          _ <= p.degreeOf 0 := by
            have :
              (ofLex (AddMonoidAlgebra.supDegree toLex p'.leadingCoeff)).cons k in p.support := by
    ...
    · rw [Finset.sup_add (support_nonempty.mpr hpₖ₀)]
      ...
      rw [Fin.sum_univ_succ]; rw [add_comm]
      dsimp
      gcongr
      simp [natDegree_finSuccEquiv, p']

中文:
引理 schwartz_zippel_sup_sum
  证明: finSuccEquiv R n p with hp'
    -- Since `p` is not identically zero, there is some `k` such that `pₖ` is not identically zero.
    -- WLOG `k` is the largest such.
    set k := p'.natDegree with hk
    set pₖ := p'.leadingCoeff with hpₖ
...
-/
lemma schwartz_zippel_sup_sum :
    forall {n} {p : MvPolynomial (Fin n) R} (hp : p != 0) (S : Fin n -> Finset R),
      #{x in S ^^ n | eval x p = 0} / ∏ i, (#(S i) : Rat>=0) <=
        p.support.sup fun s => ∑ i, (s i / #(S i) : Rat>=0)
  | 0, p, hp, S => by
...
/--
lemma `schwartz_zippel_sum_degreeOf` / 引理 `schwartz_zippel_sum_degreeOf`

English:
lemma schwartz_zippel_sum_degreeOf
  statement: {n} {p : MvPolynomial (Fin n) R} (hp : p != 0)
  proof: by
  calc
    _ <= p.support.sup fun s => ∑ i, (s i / #(S i) : Rat>=0) := schwartz_zippel_sup_sum hp S
    _ <= ∑ i, (p.degreeOf i / #(S i) : Rat>=0) := Finset.sup_le fun s hs => by
...
Depends on / 依赖: Finset, Finset.sup_le, degreeOf, monomial_le_degreeOf, p.degreeOf, p.support.sup, schwartz_zippel_sup_sum, sup_le, support
-/
lemma schwartz_zippel_sum_degreeOf {n} {p : MvPolynomial (Fin n) R} (hp : p != 0)
    (S : Fin n -> Finset R) :
    #{x in S ^^ n | eval x p = 0} / ∏ i, (#(S i) : Rat>=0) <= ∑ i, (p.degreeOf i / #(S i) : Rat>=0) := by
  calc
    _ <= p.support.sup fun s => ∑ i, (s i / #(S i) : Rat>=0) := schwartz_zippel_sup_sum hp S
    _ <= ∑ i, (p.degreeOf i / #(S i) : Rat>=0) := Finset.sup_le fun s hs => by
      gcongr with i; exact monomial_le_degreeOf i hs
...
/--
lemma `schwartz_zippel_totalDegree` / 引理 `schwartz_zippel_totalDegree`

English:
lemma schwartz_zippel_totalDegree
  given: {n} {p : MvPolynomial (Fin n) R} (hp : p != 0) (S : Finset R)
  proof: calc
...
Depends on / 依赖: Finsupp, Finsupp.sum_fintype, Nat.cast_finsetSup, S.eq_empty_or_nonempty, cast_finsetSup, eq_empty_or_nonempty, p.support.sup, p.totalDegree, piFinset, schwartz_zippel_sup_sum, simp_rw, sum_div, sum_fintype, support, totalDegree
-/
lemma schwartz_zippel_totalDegree {n} {p : MvPolynomial (Fin n) R} (hp : p != 0) (S : Finset R) :
    #{f in piFinset fun _ => S | eval f p = 0} / (#S ^ n : Rat>=0) <= p.totalDegree / #S :=
  calc
    _ = #{f in piFinset fun _ => S | eval f p = 0} / (∏ i : Fin n, #S : Rat>=0) := by simp
    _ <= p.support.sup fun s => ∑ i, (s i / #S : Rat>=0) := schwartz_zippel_sup_sum hp _
    _ = p.totalDegree / #S := by
