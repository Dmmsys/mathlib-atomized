...
/--
lemma `generalizedEisenstein_aux` / 引理 `generalizedEisenstein_aux`

English:
lemma generalizedEisenstein_aux
  statement: {q f g : R[X]} {p : Nat}
  proof: by
...
private lemma generalizedEisenstein_aux {q f g : R[X]} {p : Nat}
    ...
    (hg_div : g ∣ f) :
    exists m r, g = C g.leadingCoeff * q ^ m + r ∧
...
/--
theorem `generalizedEisenstein` / 定理 `generalizedEisenstein`

English:
theorem generalizedEisenstein
  statement: {q f : R[X]} {p : Nat}
  proof: mt degree_eq_zero_of_isUnit fun h => by
...
定理 generalizedEisenstein
  结论: {q f : R[X]} {p : 自然数}
  证明: mt degree_eq_zero_of_isUnit fun h => by
    simp_all [natDegree_pos_iff_degree_pos]
  isUnit_or_isUnit g h h_eq := by
    -- We have to show that factorizations `f = g * h` are trivial
    set P : Ideal R := ker (algebraMap R K)
...
-/
theorem generalizedEisenstein {q f : R[X]} {p : Nat}
...
/--
theorem `irreducible_of_eisenstein_criterion` / 定理 `irreducible_of_eisenstein_criterion`

English:
theorem irreducible_of_eisenstein_criterion
  statement: {f : R[X]} {P : Ideal R} (hP : P.IsPrime)
  proof: by
...
theorem irreducible_of_eisenstein_criterion {f : R[X]} {P : Ideal R} (hP : P.IsPrime)
    ...
    (hfP : forall n : Nat, ↑n < degree f -> f.coeff n in P) (hfd0 : 0 < degree f)
