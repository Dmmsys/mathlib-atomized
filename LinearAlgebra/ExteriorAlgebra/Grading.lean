...
/-
**ExteriorAlgebra.GradedAlgebra.** 是 Mathlib 中的一个定义，位于命名空间 `ExteriorAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A version of `ExteriorAlgebra.ι` that maps directly into the graded structure. T
his is
primarily an auxiliary construction used to provide `ExteriorAlgebra.gradedAlgeb
ra`.
-/
...
/-
**ExteriorAlgebra.GradedAlgebra.** 是 Mathlib 中的一个定理，位于命名空间 `ExteriorAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
primarily an auxiliary construction used to provide `ExteriorAlgebra.gradedAlgebra`. -/
/-
**ExteriorAlgebra.GradedAlgebra.lift** 是 Mathlib 中的一个定义，位于命名空间 `ExteriorAlgebra`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**ExteriorAlgebra.GradedAlgebra.lift** 是 Mathlib 中的一个定理，位于命名空间 `ExteriorAlgebra`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**ExteriorAlgebra.gradedAlgebra** 是 Mathlib 中的一个实例，位于命名空间 `ExteriorAlgebra`。
形式化陈述：gradedAlgebra : GradedAlgebra (fun i : Nat => ⋀[R]^i M)
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
· 使用定理 `ExteriorAlgebra.instGradedMonoidNatSubmoduleExteriorPower`：∀ (R : Type u
_1) (M : Type u_2) [inst : CommRing R] [inst_1 : AddCommGroup M] [inst_2 : _root
_.Module R M],   SetLike.GradedMonoid fun i => …

--- 原说明 ---
The exterior algebra is graded by the powers of the submodule `(ExteriorAlgebra.
ι R).range`.
-/
...
--- 原说明 ---
The union of the images of the maps `ExteriorAlgebra.ιMulti R n` for `n` running
 through
all natural numbers spans the exterior algebra.
-/
lemma ιMulti_span :
