...
-/
/-
**Mathlib.Linter.deindentString** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：deindentString (currIndent : Nat) (docString : String) : String
参数：currIndent : Nat；docString : String。
该定义给出了上述对象。
...
-/
/-
**Mathlib.Linter.checkVersoSyntax** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：checkVersoSyntax (docComment : String) (fileName : Option String
参数：docComment : String。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Try to parse `docComment` as a Verso docstring, and report any parse errors.

`fileName` is the file where this docstring is defined (default: the file curren
tly being
elaborated).

This is a copy of the first half of `versoDocStringFromString`, which also does 
elaboration
(but here we only report parse errors).
-/
...
-/
/-
**Mathlib.Linter.isSilencedVersoWarning** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linte
r`。
形式化陈述：isSilencedVersoWarning (err : Parser.Error) : Bool
参数：err : Parser.Error。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Determines if a given Verso parse error should be silenced in the Verso syntax l
inter.
This happens when it is valid Markdown syntax that we will migrate all at once.
-/
...
-/
/-
**Mathlib.Linter.lintVersoSyntax** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：lintVersoSyntax (docComment : String) (fileName : Option String
参数：docComment : String。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.Style.docStringLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter
.Style`。
形式化陈述：docStringLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.moduleDocVersoLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.Style`。
形式化陈述：moduleDocVersoLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Verso syntax in declaration docstrings is already handled by the `docStringLinte
r`.
-/
