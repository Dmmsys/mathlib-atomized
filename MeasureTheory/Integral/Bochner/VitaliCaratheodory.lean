...
/--
theorem `SimpleFunc.exists_le_lowerSemicontinuous_lintegral_ge` / 定理 `SimpleFunc.exists_le_lowerSemicontinuous_lintegral_ge`

English:
theorem SimpleFunc.exists_le_lowerSemicontinuous_lintegral_ge
  statement: (f : α ->ₛ Real>=0) {ε : Real>=0∞}
  proof: by
...
Depends on / 依赖: MeasureTheory, MeasureTheory.SimpleFunc.induction, Set.indicator_le_self, Set.piecewise_eq_indicator, SimpleFunc, SimpleFunc.coe_const, SimpleFunc.coe_piecewise, SimpleFunc.coe_zero, SimpleFunc.const, SimpleFunc.const_zero, SimpleFunc.piecewise, _root_, _root_.top_add, coe_const, coe_piecewise, coe_zero, const_zero, generalizing, indicator_le_self, le_top
-/
theorem SimpleFunc.exists_le_lowerSemicontinuous_lintegral_ge (f : α ->ₛ Real>=0) {ε : Real>=0∞}
    (ε0 : ε != 0) :
    exists g : α -> Real>=0, (forall x, f x <= g x) ∧ LowerSemicontinuous g ∧
  ...
  | @const c s hs =>
    ...
    have : μ s < μ s + ε / c := by
      have : (0 : Real>=0∞) < ε / c := ENNReal.div_pos_iff.2 ⟨ε0, ENNReal.coe_ne_top⟩
      simpa using ENNReal.add_lt_add_left ne_top this
    obtain ⟨u, su, u_open, μu⟩ : exists (u : _), u ⊇ s ∧ IsOpen u ∧ μ u < μ s + ε / c :=
    ...
    · simp only [SimpleFunc.coe_const, SimpleFunc.const_zero, SimpleFunc.coe_zero,
      ...
      grw [su]
    · suffices (c : Real>=0∞) * μ u <= c * μ s + ε by
      ...
      calc
        (c : Real>=0∞) * μ u <= c * (μ s + ε / c) := by grw [μu]
        _ = c * μ s + ε := by
          simp_rw [mul_add]
...
open SimpleFunc in
/--
theorem `exists_le_lowerSemicontinuous_lintegral_ge` / 定理 `exists_le_lowerSemicontinuous_lintegral_ge`

English:
theorem exists_le_lowerSemicontinuous_lintegral_ge
  statement: (f : α -> Real>=0∞) (hf : Measurable f) {ε : Real>=0∞}
  proof: by
  rcases ENNReal.exists_pos_sum_of_countable' εpos Nat with ⟨δ, δpos, hδ⟩
  have :
    forall n,
      exists g : α -> Real>=0,
        (forall x, eapproxDiff f n x <= g x) ∧
...
Depends on / 依赖: ENNReal, ENNReal.coe_le, ENNReal.exists_pos_sum_of_countable, ENNReal.tsum_le_tsum, LowerSemicontinuous, SimpleFunc, SimpleFunc.exists_le_lowerSemicontinuous_lintegral_ge, coe_le, eapproxDiff, exists_le_lowerSemicontinuous_lintegral_ge, exists_pos_sum_of_countable, f_le_g, tsum_eapproxDiff, tsum_le_tsum
-/
theorem exists_le_lowerSemicontinuous_lintegral_ge (f : α -> Real>=0∞) (hf : Measurable f) {ε : Real>=0∞}
    (εpos : ε != 0) :
    exists g : α -> Real>=0∞,
      (forall x, f x <= g x) ∧ LowerSemicontinuous g ∧ (∫⁻ x, g x ∂μ) <= (∫⁻ x, f x ∂μ) + ε := by
  rcases ENNReal.exists_pos_sum_of_countable' εpos Nat with ⟨δ, δpos, hδ⟩
  have :
    forall n,
      exists g : α -> Real>=0,
  ...
  · calc
      ∫⁻ x, ∑' n : Nat, g n x ∂μ = ∑' n, ∫⁻ x, g n x ∂μ := by
...
/--
theorem `exists_lt_lowerSemicontinuous_lintegral_ge` / 定理 `exists_lt_lowerSemicontinuous_lintegral_ge`

English:
theorem exists_lt_lowerSemicontinuous_lintegral_ge
...
Depends on / 依赖: ENNReal, ENNReal.coe_lt_coe, ENNReal.half_pos, add_lt_add_right, add_zero, coe_lt_coe, coe_nnreal_ennreal, exists_le_lowerSemicontinuous_lintegral_ge, exists_pos_lintegral_lt_of_sigmaFinite, fmeas.add, half_pos, le_g
-/
theorem exists_lt_lowerSemicontinuous_lintegral_ge [SigmaFinite μ] (f : α -> Real>=0)
    (fmeas : Measurable f) {ε : Real>=0∞} (ε0 : ε != 0) :
    exists g : α -> Real>=0∞,
      (forall x, (f x : Real>=0∞) < g x) ∧ LowerSemicontinuous g ∧ (∫⁻ x, g x ∂μ) <= (∫⁻ x, f x ∂μ) + ε := by
  have : ε / 2 != 0 := (ENNReal.half_pos ε0).ne'
  rcases exists_pos_lintegral_lt_of_sigmaFinite μ this with ⟨w, wpos, wmeas, wint⟩
  let f' x := ((f x + w x : Real>=0) : Real>=0∞)
  rcases exists_le_lowerSemicontinuous_lintegral_ge μ f' (fmeas.add wmeas).coe_nnreal_ennreal
...
/--
theorem `exists_lt_lowerSemicontinuous_lintegral_ge_of_aemeasurable` / 定理 `exists_lt_lowerSemicontinuous_lintegral_ge_of_aemeasurable`

English:
theorem exists_lt_lowerSemicontinuous_lintegral_ge_of_aemeasurable
  statement: [SigmaFinite μ] (f : α -> Real>=0)
  proof: by
  have : ε / 2 != 0 := (ENNReal.half_pos ε0).ne'
...
Depends on / 依赖: ENNReal, ENNReal.half_pos, ae_eq_mk, exists_le_lowerSemicontinuous_lintegral_ge, exists_lt_lowerSemicontinuous_lintegral_ge, exists_measurable_superset_of_null, f_lt_g0, fmeas.ae_eq_mk, fmeas.measurable_mk, fmeas.mk, g0_cont, g0_cont.add, g0_int, g1_cont, g1_int, half_pos, indicator, le_g1, measurable_const, measurable_const.indicator
-/
theorem exists_lt_lowerSemicontinuous_lintegral_ge_of_aemeasurable [SigmaFinite μ] (f : α -> Real>=0)
    (fmeas : AEMeasurable f μ) {ε : Real>=0∞} (ε0 : ε != 0) :
    exists g : α -> Real>=0∞,
      (forall x, (f x : Real>=0∞) < g x) ∧ LowerSemicontinuous g ∧ (∫⁻ x, g x ∂μ) <= (∫⁻ x, f x ∂μ) + ε := by
  have : ε / 2 != 0 := (ENNReal.half_pos ε0).ne'
  rcases exists_lt_lowerSemicontinuous_lintegral_ge μ (fmeas.mk f) fmeas.measurable_mk this with
...
/--
theorem `exists_lt_lowerSemicontinuous_integral_gt_nnreal` / 定理 `exists_lt_lowerSemicontinuous_integral_gt_nnreal`

English:
theorem exists_lt_lowerSemicontinuous_integral_gt_nnreal
  statement: [SigmaFinite μ] (f : α -> Real>=0)
  proof: by
  ...
  obtain ⟨δ, δpos, hδε⟩ : exists δ : Real>=0, 0 < δ ∧ δ < ε := exists_between εpos
...
Depends on / 依赖: AEMeasurable, ENNReal, ENNReal.coe_ne_zero, Real.toNNReal_coe, aemeasurable, aestronglyMeasurable, coe_ne_zero, convert, exists_between, exists_lt_lowerSemicontinuous_lintegral_ge_of_aemeasurable, f_lt_g, fint.aestronglyMeasurable.real_toNNReal.aemeasurable, fint.hasFiniteIntegral, hasFiniteIntegral, hasFiniteIntegral_iff_ofNNReal, int_f_ne_top, pos.le, pos.ne, real_toNNReal, toNNReal_coe
-/
theorem exists_lt_lowerSemicontinuous_integral_gt_nnreal [SigmaFinite μ] (f : α -> Real>=0)
    (fint : Integrable (fun x => (f x : Real)) μ) {ε : Real} (εpos : 0 < ε) :
    exists g : α -> Real>=0∞,
      (forall x, (f x : Real>=0∞) < g x) ∧
      LowerSemicontinuous g ∧
      (forallᵐ x ∂μ, g x < ⊤) ∧
  ...
  have g_lt_top : forallᵐ x : α ∂μ, g x < ∞ := ae_lt_top gcont.measurable gint_ne
...
/--
theorem `SimpleFunc.exists_upperSemicontinuous_le_lintegral_le` / 定理 `SimpleFunc.exists_upperSemicontinuous_le_lintegral_le`

English:
theorem SimpleFunc.exists_upperSemicontinuous_le_lintegral_le
  statement: (f : α ->ₛ Real>=0)
  proof: by
  induction f using MeasureTheory.SimpleFunc.induction generalizing ε with
  | @const c s hs =>
    by_cases hc : c = 0
...
Depends on / 依赖: ENNReal, ENNReal.coe_ne_top, ENNReal.div_pos_iff, ENNReal.mul_eq_top, F_closed, IsClosed, MeasureTheory, MeasureTheory.SimpleFunc.induction, SimpleFunc, coe_ne_top, div_pos_iff, exists_isClosed_lt_add, generalizing, hs.exists_isClosed_lt_add, int_f, lt_top_iff_ne_top, mul_eq_top, s_lt_top.ne, subseteq, this.ne
-/
theorem SimpleFunc.exists_upperSemicontinuous_le_lintegral_le (f : α ->ₛ Real>=0)
    (int_f : (∫⁻ x, f x ∂μ) != ∞) {ε : Real>=0∞} (ε0 : ε != 0) :
    exists g : α -> Real>=0, (forall x, g x <= f x) ∧ UpperSemicontinuous g ∧
  ...
  | @const c s hs =>
    ...
    have μs_lt_top : μ s < ∞ := by simpa [hs, hc, ENNReal.mul_eq_top, lt_top_iff_ne_top] using int_f
    have : (0 : Real>=0∞) < ε / c := ENNReal.div_pos_iff.2 ⟨ε0, ENNReal.coe_ne_top⟩
    obtain ⟨F, Fs, F_closed, μF⟩ : exists (F : _), F subseteq s ∧ IsClosed F ∧ μ s < μ F + ε / c :=
    ...
    · simp only [SimpleFunc.coe_const, SimpleFunc.const_zero, SimpleFunc.coe_zero,
        Set.piecewise_eq_indicator, SimpleFunc.coe_piecewise, ← Function.const_def]
      grw [Fs]
    · suffices (c : Real>=0∞) * μ s <= c * μ F + ε by
      ...
      calc
        (c : Real>=0∞) * μ s <= c * (μ F + ε / c) := by grw [μF]
        _ = c * μ F + ε := by
          simp_rw [mul_add]
          rw [ENNReal.mul_div_cancel _ ENNReal.coe_ne_top]
          simpa using hc
...
/--
theorem `exists_upperSemicontinuous_le_lintegral_le` / 定理 `exists_upperSemicontinuous_le_lintegral_le`

English:
theorem exists_upperSemicontinuous_le_lintegral_le
  statement: (f : α -> Real>=0) (int_f : (∫⁻ x, f x ∂μ) != ∞)
  proof: by
  obtain ⟨fs, fs_le_f, int_fs⟩ :
    exists fs : α ->ₛ Real>=0, (forall x, fs x <= f x) ∧ (∫⁻ x, f x ∂μ) <= (∫⁻ x, fs x ∂μ) + ε / 2 := by
    have := ENNReal.lt_add_right int_f (ENNReal.half_pos ε0).ne'
    conv_rhs at this => rw [lintegral_eq_nnreal (fun x => (f x : Real>=0∞)) μ]
    rw [ENNReal
...
Depends on / 依赖: ENNReal, ENNReal.biSup_add, ENNReal.coe_le_coe, ENNReal.half_pos, ENNReal.lt_add_right, biSup_add, coe_le_coe, conv_rhs, convert, fs_le_f, half_pos, int_f, int_fs, lintegral_eq_nnreal, lt_add_right, lt_iSup_iff
-/
theorem exists_upperSemicontinuous_le_lintegral_le (f : α -> Real>=0) (int_f : (∫⁻ x, f x ∂μ) != ∞)
    {ε : Real>=0∞} (ε0 : ε != 0) :
    exists g : α -> Real>=0, (forall x, g x <= f x) ∧ UpperSemicontinuous g ∧
      (∫⁻ x, f x ∂μ) <= (∫⁻ x, g x ∂μ) + ε := by
  obtain ⟨fs, fs_le_f, int_fs⟩ :
  ...
  obtain ⟨g, g_le_fs, gcont, gint⟩ :
    exists g : α -> Real>=0,
      (forall x, g x <= fs x) ∧ UpperSemicontinuous g ∧ (∫⁻ x, fs x ∂μ) <= (∫⁻ x, g x ∂μ) + ε / 2 :=
...
/--
theorem `exists_upperSemicontinuous_le_integral_le` / 定理 `exists_upperSemicontinuous_le_integral_le`
...
theorem exists_upperSemicontinuous_le_integral_le
  ...
  proof: by
  lift ε to Real>=0 using εpos.le
  rw [NNReal.coe_pos]; rw [← ENNReal.coe_pos] at εpos
...
Depends on / 依赖: ENNReal, ENNReal.coe_pos, If.ne, Integrable, Integrable.mono, NNReal, NNReal.coe_pos, aemeasurable, aestronglyMeasurable, coe_nnreal_real, coe_pos, exists_upperSemicontinuous_le_lintegral_le, fint.hasFiniteIntegral, gcont.measurable.coe_nnreal_real.aemeasurable.aestronglyMeasurable, hasFiniteIntegral, hasFiniteIntegral_iff_ofNNReal, lintegral_mono, lt_of_le_of_lt, measurable, pos.le
-/
theorem exists_upperSemicontinuous_le_integral_le (f : α -> Real>=0)
    (fint : Integrable (fun x => (f x : Real)) μ) {ε : Real} (εpos : 0 < ε) :
    exists g : α -> Real>=0,
      (forall x, g x <= f x) ∧
      UpperSemicontinuous g ∧
      Integrable (fun x => (g x : Real)) μ ∧ (∫ x, (f x : Real) ∂μ) - ε <= ∫ x, ↑(g x) ∂μ := by
  lift ε to Real>=0 using εpos.le
  rw [NNReal.coe_pos]; rw [← ENNReal.coe_pos] at εpos
  have If : (∫⁻ x, f x ∂μ) < ∞ := hasFiniteIntegral_iff_ofNNReal.1 fint.hasFiniteIntegral
  rcases exists_upperSemicontinuous_le_lintegral_le f If.ne εpos.ne' with ⟨g, gf, gcont, gint⟩
...
/--
theorem `exists_lt_lowerSemicontinuous_integral_lt` / 定理 `exists_lt_lowerSemicontinuous_integral_lt`

English:
theorem exists_lt_lowerSemicontinuous_integral_lt
  statement: [SigmaFinite μ] (f : α -> Real) (hf : Integrable f μ)
  proof: by
  let δ : Real>=0 := .mk (ε / 2) (half_pos εpos).le
  have δpos : 0 < δ := half_pos εpos
  let fp : α -> Real>=0 := fun x => Real.toNNReal (f x)
  have int_fp : Integrable (fun x => (fp x : Real)) μ := hf.real_toNNReal
...
Depends on / 依赖: Integrable, Real.toNNReal, exists_lt_lowerSemicontinuous_integral_gt_nnreal, exists_upperSemi, fp_lt_gp, gp_integrable, gp_lt_top, gpcont, half_pos, hf.neg.real_toNNReal, hf.real_toNNReal, int_fm, int_fp, real_toNNReal, toNNReal
-/
theorem exists_lt_lowerSemicontinuous_integral_lt [SigmaFinite μ] (f : α -> Real) (hf : Integrable f μ)
    {ε : Real} (εpos : 0 < ε) :
    exists g : α -> EReal,
      (forall x, (f x : EReal) < g x) ∧
      LowerSemicontinuous g ∧
      Integrable (fun x => EReal.toReal (g x)) μ ∧
      (forallᵐ x ∂μ, g x < ⊤) ∧ (∫ x, EReal.toReal (g x) ∂μ) < (∫ x, f x ∂μ) + ε := by
  let δ : Real>=0 := .mk (ε / 2) (half_pos εpos).le
  have δpos : 0 < δ := half_pos εpos
  let fp : α -> Real>=0 := fun x => Real.toNNReal (f x)
  have int_fp : Integrable (fun x => (fp x : Real)) μ := hf.real_toNNReal
  ...
  let fm : α -> Real>=0 := fun x => Real.toNNReal (-f x)
  have int_fm : Integrable (fun x => (fm x : Real)) μ := hf.neg.real_toNNReal
  ...
  have ae_g : forallᵐ x ∂μ, (g x).toReal = (gp x : EReal).toReal - (gm x : EReal).toReal := by
...
/--
theorem `exists_upperSemicontinuous_lt_integral_gt` / 定理 `exists_upperSemicontinuous_lt_integral_gt`

English:
theorem exists_upperSemicontinuous_lt_integral_gt
  statement: [SigmaFinite μ] (f : α -> Real) (hf : Integrable f μ)
  proof: by
...
Depends on / 依赖: EReal.coe_neg, EReal.neg_le_neg_iff, EReal.neg_lt_comm, bot_lt_iff_ne_bot, coe_neg, comp_lowerSemicontinuous_antitone, continuous_neg, continuous_neg.comp_lowerSemicontinuous_antitone, convert, exists_lt_lowerSemicontinuous_integral_lt, g_integrable, g_integrable.neg, g_lt_f, g_lt_top, hf.neg, integral_neg, lt_neg_add_iff, lt_top_iff_ne_top, neg_le_neg_iff, neg_lt_comm
-/
theorem exists_upperSemicontinuous_lt_integral_gt [SigmaFinite μ] (f : α -> Real) (hf : Integrable f μ)
    {ε : Real} (εpos : 0 < ε) :
    exists g : α -> EReal,
      (forall x, (g x : EReal) < f x) ∧
      UpperSemicontinuous g ∧
      Integrable (fun x => EReal.toReal (g x)) μ ∧
      (forallᵐ x ∂μ, ⊥ < g x) ∧ (∫ x, f x ∂μ) < (∫ x, EReal.toReal (g x) ∂μ) + ε := by
  rcases exists_lt_lowerSemicontinuous_integral_lt (fun x => -f x) hf.neg εpos with
    ⟨g, g_lt_f, gcont, g_integrable, g_lt_top, gint⟩
  refine ⟨fun x => -g x, ?_, ?_, ?_, ?_, ?_⟩
  · exact fun x => EReal.neg_lt_comm.1 (by simpa only [EReal.coe_neg] using g_lt_f x)
