...
variable {p : Nat -> Real} {r : Real} (k : Nat)

/--
lemma `tendsto_zero_of_tendsto_mul_atTop` / 引理 `tendsto_zero_of_tendsto_mul_atTop`

English:
lemma tendsto_zero_of_tendsto_mul_atTop
  given: (hr : Tendsto (fun n => n * p n) atTop (𝓝 r))
  proof: by
...
Depends on / 依赖: eventually_ge_atTop, filter_upwards, hr.mul, tendsto_one_div_atTop_nhds_zero_nat
-/
...
open Asymptotics in
/--
lemma `tendsto_choose_mul_pow_atTop` / 引理 `tendsto_choose_mul_pow_atTop`

English:
lemma tendsto_choose_mul_pow_atTop
  given: (hr : Tendsto (fun n => n * p n) atTop (𝓝 r))
  proof: by
...
/--
theorem `tendsto_choose_mul_pow_of_tendsto_mul_atTop` / 定理 `tendsto_choose_mul_pow_of_tendsto_mul_atTop`

English:
theorem tendsto_choose_mul_pow_of_tendsto_mul_atTop
  given: (hr : Tendsto (fun n => n * p n) atTop (𝓝 r))
  proof: by
...
/--
lemma `binomial_tendsto_poissonPMFReal_atTop` / 引理 `binomial_tendsto_poissonPMFReal_atTop`

English:
lemma binomial_tendsto_poissonPMFReal_atTop
  statement: {r : Real>=0} {p : Nat -> unitInterval}
  proof: by
...
-/
lemma binomial_tendsto_poissonPMFReal_atTop {r : Real>=0} {p : Nat -> unitInterval}
    (hr : Tendsto (fun n => n * (p n : Real)) atTop (𝓝 r)) :
