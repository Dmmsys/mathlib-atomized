...
@[simps]
/--
Definition of `pointBot` / `pointBot` 的定义

English:
definition pointBot
  signature: (X : C)
  body: shrinkYoneda.flip.obj (op X)
...
定义 pointBot
  签名: (X : C)
  定义体: shrinkYoneda.flip.obj (op X)
  jointly_surjective {U} R hR x := by
    obtain rfl : R = ⊤ := by simpa using hR
    exact ⟨U, 𝟙 _, by simp, x, by simp⟩

Depends on / 依赖: shrinkYoneda, shrinkYoneda.flip.obj
-/
...
@[simps]
/--
Definition of `pointBotFunctor` / `pointBotFunctor` 的定义

English:
definition pointBotFunctor
  signature: :
  body: pointBot
...
@[simps! inv]
/--
Definition of `pointBotPresheafFiberIso` / `pointBotPresheafFiberIso` 的定义

English:
definition pointBotPresheafFiberIso
  signature: :
  body: (asIso ((pointBot X).toPresheafFiberNatTrans X
      (shrinkYonedaObjObjEquiv.symm (𝟙 X)))).symm

中文:
定义 pointBotPresheafFiberIso
  签名: :
  定义体: (asIso ((pointBot X).toPresheafFiberNatTrans X
      (shrinkYonedaObjObjEquiv.symm (𝟙 X)))).symm
-/
...
variable (C) in
/--
Definition of `pointsBot` / `pointsBot` 的定义

English:
definition pointsBot
  signature: :
  body: .ofObj pointBot
...
English:
instance [Small.{w}
  signature: C] : ObjectProperty.Small.{w} (pointsBot C)
  body: by
  dsimp [pointsBot]
  infer_instance
...
Depends on / 依赖: infer_instance, pointsBot
-/
...
variable (C) in
/--
lemma `isConservative_pointsBot` / 引理 `isConservative_pointsBot`

English:
lemma isConservative_pointsBot
  proof: .mk' (fun X S hS => by
