...
variable {𝕜 : Type*} [NontriviallyNormedField 𝕜] {m n : WithTop Nat∞}
...
/--
theorem `ContMDiffWithinAt.mfderivWithin` / 定理 `ContMDiffWithinAt.mfderivWithin`

English:
theorem ContMDiffWithinAt.mfderivWithin
  statement: {x₀ : N} {f : N -> M -> M'} {g : N -> M}
  proof: by
  -- first localize the result to a smaller set, to make sure everything happens in chart domains
  let t' := t inter g ⁻¹' ((extChartAt I (g x₀)).source)
  have ht't : t' subseteq t := inter_subset_left
  suffices CMDiffAt[t'] m (inTangentCoordinates I I' g (fun x => f x (g x))
      (fun x => m

中文:
定理 ContMDiffWithinAt.mfderivWithin
  结论: {x₀ : N} {f : N -> M -> M'} {g : N -> M}
  证明: by
  -- first localize the result to a smaller set, to make sure everything happens in chart domains
  let t' := t inter g ⁻¹' ((extChartAt I (g x₀)).source)
  have ht't : t' subseteq t := inter_subset_left
...
protected theorem ContMDiffWithinAt.mfderivWithin {x₀ : N} {f : N -> M -> M'} {g : N -> M}
  ...
  -- by continuity
  have hx₀gx₀ : (x₀, g x₀) in t ×ˢ u := by simp [hx₀, hu hx₀]
  ...
  have h2f : forallᶠ x₂ in 𝓝[t] x₀, CMDiffAt[u] 1 (f x₂) (g x₂) := by
...
.tendsto_nhdsWithin this).eventually h3f, self_mem_nhdsWithin] with x hx h'x
  ...
  have h2g : g ⁻¹' (extChartAt I (g x₀)).source in 𝓝[t] x₀ :=
  ...
  have : ContDiffWithinAt 𝕜 m (fun x => fderivWithin 𝕜
        ...
        ((extChartAt I (g x₀)).target inter (extChartAt I (g x₀)).symm ⁻¹' u)
        (extChartAt I (g x₀) (g ((extChartAt J x₀).symm x))))
      ((extChartAt J x₀).symm ⁻¹' t' inter range J) (extChartAt J x₀ x₀) := by
    have hf' := hf.mono (prod_mono_left ht't)
    have hg' := hg.mono (show t' subseteq t from inter_subset_left)
    ...
    apply ContDiffWithinAt.fderivWithin _ _ _ (show (m : WithTop Nat∞) + 1 <= n from mod_cast hmn)
    · simp [hx₀, t']
    · apply inter_subset_left.trans
      rw [preimage_subset_iff]
      intro a ha
      refine ⟨PartialEquiv.map_source _ (inter_subset_right ha :), ?_⟩
      rw [mem_preimage]; rw [PartialEquiv.left_inv (extChartAt I (g x₀))]
    ...
    · have : ((fun p => ((extChartAt J x₀).symm p.1, (extChartAt I (g x₀)).symm p.2)) ⁻¹' t' ×ˢ u
            inter range J ×ˢ (extChartAt I (g x₀)).target)
          subseteq ((fun p => ((extChartAt J x₀).symm p.1, (extChartAt I (g x₀)).symm p.2)) ⁻¹' t' ×ˢ u
            inter range J ×ˢ range I) := by
  ...
  have h1 : g x in u := hu hxt
  have h3 : UniqueMDiffAt[(extChartAt I (g x₀)).target inter (extChartAt I (g x₀)).symm ⁻¹' u]
  ...
  have A : mfderiv[range I] ((extChartAt I (g x₀)).symm) ((extChartAt I (g x₀)) (g x))
      = mfderiv[(extChartAt I (g x₀)).target inter (extChartAt I (g x₀)).symm ⁻¹' u]
  ...
  rw [inTangentCoordinates_eq_mfderiv_comp]; rw [A]; rw [← mfderivWithin_comp_of_eq]; rw [← mfderiv_comp_mfderivWithin_of_eq]
...
/--
theorem `ContMDiffWithinAt.mfderivWithin_const` / 定理 `ContMDiffWithinAt.mfderivWithin_const`

English:
theorem ContMDiffWithinAt.mfderivWithin_const
  statement: {x₀ : M} {f : M -> M'}
  proof: by
...
中文:
定理 ContMDiffWithinAt.mfderivWithin_const
  结论: {x₀ : M} {f : M -> M'}
  证明: by
  have : CMDiffAt[s ×ˢ s] n (fun x : M × M => f x.2) (x₀, x₀) :=
    hf.comp (x₀, x₀) contMDiffWithinAt_snd mapsTo_snd_prod
...
Depends on / 依赖: CMDiffAt, contMDiffWithinAt_id, contMDiffWithinAt_snd, hf.comp, mapsTo_id, mapsTo_snd_prod, mfderivWithin, this.mfderivWithin
-/
theorem ContMDiffWithinAt.mfderivWithin_const {x₀ : M} {f : M -> M'}
    (hf : CMDiffAt[s] n f x₀) (hmn : m + 1 <= n) (hx : x₀ in s) (hs : UniqueMDiff[s]) :
...
/--
theorem `ContMDiffWithinAt.mfderivWithin_apply` / 定理 `ContMDiffWithinAt.mfderivWithin_apply`

English:
theorem ContMDiffWithinAt.mfderivWithin_apply
  statement: {x₀ : N'}
  proof: ((hf.mfderivWithin hg hg₁x₀ h'g hmn hu).comp_of_eq hg₁ h'g₁ rfl).clm_apply hg₂

中文:
定理 ContMDiffWithinAt.mfderivWithin_apply
  结论: {x₀ : N'}
  证明: ((hf.mfderivWithin hg hg₁x₀ h'g hmn hu).comp_of_eq hg₁ h'g₁ rfl).clm_apply hg₂

Depends on / 依赖: clm_apply, comp_of_eq, hf.mfderivWithin, mfderivWithin
-/
theorem ContMDiffWithinAt.mfderivWithin_apply {x₀ : N'}
    ...
    (hg₁x₀ : g₁ x₀ in t) (h'g : MapsTo g t u) (hu : UniqueMDiff[u]) :
...
/--
theorem `ContMDiffAt.mfderiv` / 定理 `ContMDiffAt.mfderiv`

English:
theorem ContMDiffAt.mfderiv
  statement: {x₀ : N} (f : N -> M -> M') (g : N -> M)
  proof: by
...
/--
theorem `ContMDiffAt.mfderiv_const` / 定理 `ContMDiffAt.mfderiv_const`

English:
theorem ContMDiffAt.mfderiv_const
  statement: {x₀ : M} {f : M -> M'} (hf : CMDiffAt n f x₀)
  proof: haveI : CMDiffAt n (fun x : M × M => f x.2) (x₀, x₀) :=
    ContMDiffAt.comp (x₀, x₀) hf contMDiffAt_snd
  this.mfderiv (fun _ => f) id contMDiffAt_id hmn

中文:
定理 ContMDiffAt.mfderiv_const
  结论: {x₀ : M} {f : M -> M'} (hf : CMDiffAt n f x₀)
  证明: haveI : CMDiffAt n (fun x : M × M => f x.2) (x₀, x₀) :=
    ContMDiffAt.comp (x₀, x₀) hf contMDiffAt_snd
  this.mfderiv (fun _ => f) id contMDiffAt_id hmn

Depends on / 依赖: CMDiffAt, ContMDiffAt, ContMDiffAt.comp, contMDiffAt_id, contMDiffAt_snd, mfderiv, this.mfderiv
-/
...
/--
theorem `ContMDiffAt.mfderiv_apply` / 定理 `ContMDiffAt.mfderiv_apply`

English:
theorem ContMDiffAt.mfderiv_apply
  statement: {x₀ : N'} (f : N -> M -> M') (g : N -> M) (g₁ : N' -> N) (g₂ : N' -> E)
  proof: ((hf.mfderiv f g hg hmn).comp_of_eq hg₁ rfl).clm_apply hg₂

中文:
定理 ContMDiffAt.mfderiv_apply
  结论: {x₀ : N'} (f : N -> M -> M') (g : N -> M) (g₁ : N' -> N) (g₂ : N' -> E)
  证明: ((hf.mfderiv f g hg hmn).comp_of_eq hg₁ rfl).clm_apply hg₂

Depends on / 依赖: clm_apply, comp_of_eq, hf.mfderiv, mfderiv
-/
...
/--
theorem `ContMDiffOn.contMDiffOn_tangentMapWithin` / 定理 `ContMDiffOn.contMDiffOn_tangentMapWithin`
...
中文:
定理 ContMDiffOn.contMDiffOn_tangentMapWithin
  证明: by
  intro x₀ hx₀
  let s' : Set (TangentBundle I M) := (π E (TangentSpace I) ⁻¹' s)
  let b₁ : TangentBundle I M -> M := fun p => p.1
...
Depends on / 依赖: CMDiffAt, TangentBundle, TangentSpace, contMDiffWithinAt_id, contMDiffWithinAt_proj, le_self_add, le_self_add.trans, of_le
-/
theorem ContMDiffOn.contMDiffOn_tangentMapWithin
...
/--
theorem `ContMDiffOn.continuousOn_tangentMapWithin` / 定理 `ContMDiffOn.continuousOn_tangentMapWithin`

English:
theorem ContMDiffOn.continuousOn_tangentMapWithin
  statement: (hf : CMDiff[s] n f) (hmn : 1 <= n)
  proof: by
...
中文:
定理 ContMDiffOn.continuousOn_tangentMapWithin
  结论: (hf : CMDiff[s] n f) (hmn : 1 <= n)
  证明: by
  have : CMDiff[π E (TangentSpace I) ⁻¹' s] 0 (tangentMap[s] f) :=
    hf.contMDiffOn_tangentMapWithin hmn hs
  exact this.continuousOn

Depends on / 依赖: CMDiff, TangentSpace, contMDiffOn_tangentMapWithin, continuousOn, hf.contMDiffOn_tangentMapWithin, tangentMap, this.continuousOn
-/
...
/--
theorem `ContMDiff.contMDiff_tangentMap` / 定理 `ContMDiff.contMDiff_tangentMap`

English:
theorem ContMDiff.contMDiff_tangentMap
  given: (hf : CMDiff n f) (hmn : m + 1 <= n)
  proof: by
...
中文:
定理 ContMDiff.contMDiff_tangentMap
  条件: (hf : CMDiff n f) (hmn : m + 1 <= n)
  证明: by
  rw [← contMDiffOn_univ] at hf ⊢
  convert! hf.contMDiffOn_tangentMapWithin hmn uniqueMDiffOn_univ
  rw [tangentMapWithin_univ]

Depends on / 依赖: contMDiffOn_tangentMapWithin, contMDiffOn_univ, convert, hf.contMDiffOn_tangentMapWithin, tangentMapWithin_univ, uniqueMDiffOn_univ
-/
...
/--
theorem `ContMDiff.continuous_tangentMap` / 定理 `ContMDiff.continuous_tangentMap`

English:
theorem ContMDiff.continuous_tangentMap
  given: (hf : CMDiff n f) (hmn : 1 <= n)
  proof: by
  rw [← contMDiffOn_univ] at hf
...
中文:
定理 ContMDiff.continuous_tangentMap
  条件: (hf : CMDiff n f) (hmn : 1 <= n)
  证明: by
  rw [← contMDiffOn_univ] at hf
  rw [← continuousOn_univ]
  convert! hf.continuousOn_tangentMapWithin hmn uniqueMDiffOn_univ
...
Depends on / 依赖: contMDiffOn_univ, continuousOn_tangentMapWithin, continuousOn_univ, convert, hf.continuousOn_tangentMapWithin, tangentMapWithin_univ, uniqueMDiffOn_univ
-/
...
/--
theorem `tangentMap_tangentBundle_pure` / 定理 `tangentMap_tangentBundle_pure`

English:
theorem tangentMap_tangentBundle_pure
  statement: [Is : IsManifold I 1 M]
  proof: by
  rcases p with ⟨x, v⟩
  ...
  have A : MDiffAt (fun x => TotalSpace.mk' E (x : M) (0 : Tange
...
Depends on / 依赖: Bundle, Bundle.contMDiff_zeroSection, CMDiff, I.continuous_invFun, I.symm, IsOpen, IsOpen.mem_nhds, MDiffAt, OpenPartialHomeomorph, OpenPartialHomeomorph.open_target, TangentSpace, TotalSpace, TotalSpace.mk, chartAt, contMDiff_zeroSection, continuous_invFun, mdifferenti, mem_nhds, mfld_simps, open_target
-/
theorem tangentMap_tangentBundle_pure [Is : IsManifold I 1 M]
  ...
  have B : fderivWithin 𝕜 (fun x' : E => (x', (0 : E))) (Set.range I) (I ((chartAt H x) x)) v
      = (v, 0) := by
    rw [fderivWithin_eq_fderiv]; rw [DifferentiableAt.fderiv_prodMk]
    · simp
...
theorem mdifferentiable'
  given: (f : C^n⟮I, M; I', M'⟯) (hn : n != 0)
  statement: MDiff f
  proof: f.contMDiff.mdifferentiable hn

中文:
定理 mdifferentiable'
  条件: (f : C^n⟮I, M; I', M'⟯) (hn : n != 0)
  结论: MDiff f
  证明: f.contMDiff.mdifferentiable hn
-/
...
theorem mdifferentiable
  given: (f : C^∞⟮I, M; I', M'⟯)
  statement: MDiff f
  proof: f.mdifferentiable' (by simp)

中文:
定理 mdifferentiable
  条件: (f : C^∞⟮I, M; I', M'⟯)
  结论: MDiff f
  证明: f.mdifferentiable' (by simp)
-/
...
theorem mdifferentiableAt
  ...
  proof: f.mdifferentiable x

中文:
定理 mdifferentiableAt
  条件: (f : C^∞⟮I, M; I', M'⟯) {x}
  结论: MDiffAt f x
  证明: f.mdifferentiable x
-/
...
variable (I I' M M') in
/--
Definition of `equivTangentBundleProd` / `equivTangentBundleProd` 的定义

English:
definition equivTangentBundleProd
  signature: :
  body: (⟨p.1.1, p.2.1⟩, ⟨p.1.2, p.2.2⟩)
  invFun p := ⟨(p.1.1, p.2.1), (p.1.2, p.2.2)⟩

中文:
定义 equivTangentBundleProd
  签名: :
  定义体: (⟨p.1.1, p.2.1⟩, ⟨p.1.2, p.2.2⟩)
  invFun p := ⟨(p.1.1, p.2.1), (p.1.2, p.2.2)⟩
-/
...
/--
lemma `equivTangentBundleProd_eq_tangentMap_prod_tangentMap` / 引理 `equivTangentBundleProd_eq_tangentMap_prod_tangentMap`
...
中文:
引理 equivTangentBundleProd_eq_tangentMap_prod_tangentMap
  证明: by
  simp only [tangentMap_prodFst, tangentMap_prodSnd]; rfl

Depends on / 依赖: tangentMap_prodFst, tangentMap_prodSnd
-/
lemma equivTangentBundleProd_eq_tangentMap_prod_tangentMap :
...
/--
lemma `contMDiff_equivTangentBundleProd` / 引理 `contMDiff_equivTangentBundleProd`
...
中文:
引理 contMDiff_equivTangentBundleProd
  证明: by
  rw [equivTangentBundleProd_eq_tangentMap_prod_tangentMap]
  exact (contMDiff_fst.contMDiff_tangentMap le_rfl).prodMk
    (contMDiff_snd.contMDiff_tangentMap le_rfl)

Depends on / 依赖: contMDiff_fst, contMDiff_fst.contMDiff_tangentMap, contMDiff_snd, contMDiff_snd.contMDiff_tangentMap, contMDiff_tangentMap, equivTangentBundleProd_eq_tangentMap_prod_tangentMap, le_rfl, prodMk
-/
lemma contMDiff_equivTangentBundleProd :
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `contMDiff_equivTangentBundleProd_symm` / 引理 `contMDiff_equivTangentBundleProd_symm`
...
中文:
引理 contMDiff_equivTangentBundleProd_symm
  证明: by
  /- Contrary to what one might expect, this proof is nontrivial. It is not a formalization issue:
  even on paper, I don't have a simple proof of the statement. The reason is that there is no nice
  functorial expression for the map from `TM × T'M` to `T (M × M')`, so I need to come back to
