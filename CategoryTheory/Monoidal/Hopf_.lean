...
theorem one_antipode
  ...
  conv at this =>
    rhs
    rw [antipode_left]
  rw [Bimon.one_comul_assoc]; rw [tensorHom_def_assoc]; rw [unitors_inv_equal]; rw [← rightUnitor_inv_naturality_assoc]; rw [whisker_exchange_assoc]; rw [← rightUnitor_inv_naturality_assoc]; rw 
