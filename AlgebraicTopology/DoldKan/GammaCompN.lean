...
--- 原说明 ---
The isomorphism `(Γ₀.splitting K).nondegComplex ≅ K` for all `K : ChainComplex C
 ℕ`.
-/
...
--- 原说明 ---
The natural isomorphism `(Γ₀.splitting K).nondegComplex ≅ K` for `K : ChainCompl
ex C ℕ`.
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**AlgebraicTopology.DoldKan.N** 是 Mathlib 中的一个定理，位于命名空间 `AlgebraicTopology.DoldK
an`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**AlgebraicTopology.DoldKan.N** 是 Mathlib 中的一个引理，位于命名空间 `AlgebraicTopology.DoldK
an`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**AlgebraicTopology.DoldKan.whiskerLeft_toKaroubi_N** 是 Mathlib 中的一个引理，位于命名空间 `A
