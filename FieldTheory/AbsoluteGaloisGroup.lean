...
/--
Definition of `absoluteGaloisGroup` / `absoluteGaloisGroup` 的定义

English:
definition absoluteGaloisGroup
  body: AlgebraicClosure K ≃ₐ[K] AlgebraicClosure K
deriving Group, TopologicalSpace, IsTopologicalGroup

中文:
定义 absoluteGaloisGroup
  定义体: AlgebraicClosure K ≃ₐ[K] AlgebraicClosure K
deriving Group, TopologicalSpace, IsTopologicalGroup
...
/--
Instance `absoluteGaloisGroup.commutator_closure_isNormal` / 实例 `absoluteGaloisGroup.commutator_closure_isNormal`

English:
instance absoluteGaloisGroup.commutator_closure_isNormal
  signature: :
  body: Subgroup.is_normal_topologicalClosure (commutator (G_K K))

中文:
实例 absoluteGaloisGroup.commutator_closure_isNormal
  签名: :
  定义体: Subgroup.is_normal_topologicalClosure (commutator (G_K K))
...
/--
Definition of `absoluteGaloisGroupAbelianization` / `absoluteGaloisGroupAbelianization` 的定义

English:
abbreviation absoluteGaloisGroupAbelianization
  body: TopologicalAbelianization (G_K K)

local notation "G_K_ab" => absoluteGaloisGroupAbelianization

中文:
缩写 absoluteGaloisGroupAbelianization
  定义体: TopologicalAbelianization (G_K K)

local notation "G_K_ab" => absoluteGaloisGroupAbelianization

Depends on / 依赖: TopologicalAbelianization
-/
