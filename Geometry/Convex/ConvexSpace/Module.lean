/-
Copyright (c) 2026 Yaël Dillies. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yaël Dillies
-/
module

public import Mathlib.Algebra.Group.Pointwise.Set.Basic
public import Mathlib.Geometry.Convex.Star
public import Mathlib.LinearAlgebra.AffineSpace.Combination
public import Mathlib.LinearAlgebra.AffineSpace.AffineMap

/-!
# Modules are convex spaces

This file shows that every module over ordered coefficients is a convex space.

## Main declarations

* `ConvexSpace.ofModule`: A semimodule space over a semiring is a convex space.
* `convexSpaceSelf`: A semiring is a convex space over itself.
* `IsModuleConvexSpace`: Predicate for a convex space and module structures to be compatible.
-/

open scoped Pointwise

public noncomputable section

namespace Convexity
variable {F R M N I : Type*} [Semiring R] [PartialOrder R] [IsStrictOrderedRing R]

section AddCommMonoid
variable [AddCommMonoid M] [Module R M] [AddCommMonoid N] [Module R N] [SetLike F M]
  [AddSubmonoidClass F M] [SMulMemClass F R M] {f g : M -> N}

/-- Any semimodule over an ordered semiring is a convex space.

This is not an instance because it creates a diamond with structural instances such as
`ConvexSpace R X → ConvexSpace R Y → ConvexSpace R (X × Y)` because
`(∑ i, f i).fst = ∑ i, (f i).fst` isn't defeq, ultimately because `Finset.sum` isn't a field of
`AddCommMonoid` but derived from them through recursion. -/
@[expose, implicit_reducible]
/--
Definition of `ConvexSpace.ofModule` / `ConvexSpace.ofModule` 的定义

English:
definition ConvexSpace.ofModule
  signature: : ConvexSpace R M where
  body: w.weights.sum fun m r => r • m
  sConvexComb_single := by simp
  assoc := by
    simp [Finsupp.sum_mapDomain_index, add_smul, Finsupp.sum_sum_index, Finsupp.sum_smul_index,
      mul_smul, Finsupp.smul_sum]

中文:
定义 凸空间.ofModule
  签名: : 凸空间 R M where
  定义体: w.weights.sum fun m r => r • m
  sConvexComb_single := by simp
  assoc := by
    simp [Finsupp.sum_mapDomain_index, add_smul, Finsupp.sum_sum_index, Finsupp.sum_smul_index,
      mul_smul, Finsupp.smul_sum]

Depends on / 依赖: w.weights.sum, weights
-/
def ConvexSpace.ofModule : ConvexSpace R M where
  sConvexComb w := w.weights.sum fun m r => r • m
  sConvexComb_single := by simp
  assoc := by
    simp [Finsupp.sum_mapDomain_index, add_smul, Finsupp.sum_sum_index, Finsupp.sum_smul_index,
      mul_smul, Finsupp.smul_sum]

/--
Instance `convexSpaceSelf` / 实例 `convexSpaceSelf`

English:
instance convexSpaceSelf
  signature: : ConvexSpace R R
  body: .ofModule

中文:
实例 convexSpaceSelf
  签名: : 凸空间 R R
  定义体: .ofModule

Depends on / 依赖: ofModule
-/
instance convexSpaceSelf : ConvexSpace R R := .ofModule

variable (R M) [ConvexSpace R M] in
/--
Definition of `IsModuleConvexSpace` / `IsModuleConvexSpace` 的定义

English:
class IsModuleConvexSpace
  parameters: : Prop where
  axioms and operations (1):
    - sConvexComb_eq_sum((w : StdSimplex R M)) : w.sConvexComb = w.weights.sum fun m r => r • m

中文:
类 是ModuleConvex空间
  参数: : 命题 where
  公理与运算 (1 个):
    - sConvexComb_eq_sum((w : 标准单纯形 R M)) : w.sConvexComb = w.weights.求和 fun m r => r • m
-/
class IsModuleConvexSpace : Prop where
  sConvexComb_eq_sum (w : StdSimplex R M) : w.sConvexComb = w.weights.sum fun m r => r • m

export IsModuleConvexSpace (sConvexComb_eq_sum)
attribute [simp] sConvexComb_eq_sum

@[deprecated (since := "2026-04-03")]
alias _root_.convexCombination_eq_sum := sConvexComb_eq_sum

attribute [local instance] ConvexSpace.ofModule in
/--
lemma `IsModuleConvexSpace.ofModule` / 引理 `IsModuleConvexSpace.ofModule`

English:
lemma IsModuleConvexSpace.ofModule
  statement: IsModuleConvexSpace R M where
  proof: rfl

中文:
引理 是ModuleConvex空间.ofModule
  结论: 是ModuleConvex空间 R M where
  证明: rfl
-/
protected lemma IsModuleConvexSpace.ofModule : IsModuleConvexSpace R M where
  sConvexComb_eq_sum _ := rfl

/--
Instance `isModuleConvexSpace_self` / 实例 `isModuleConvexSpace_self`

English:
instance isModuleConvexSpace_self
  signature: : IsModuleConvexSpace R R
  body: .ofModule

中文:
实例 isModuleConvexSpace_self
  签名: : 是ModuleConvex空间 R R
  定义体: .ofModule

Depends on / 依赖: ofModule
-/
instance isModuleConvexSpace_self : IsModuleConvexSpace R R := .ofModule

section IsModuleConvexSpace
variable [ConvexSpace R M] [IsModuleConvexSpace R M] [ConvexSpace R N] [IsModuleConvexSpace R N]
  {x y : M} {s t : Set M} {a b : R}

/-- `iConvexComb` in a module can be expressed as a sum. -/
@[simp]
/--
lemma `iConvexComb_eq_sum` / 引理 `iConvexComb_eq_sum`

English:
lemma iConvexComb_eq_sum
  given: (w : StdSimplex R I) (f : I -> M)
  proof: by
  simp [iConvexComb, sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]

中文:
引理 iConvexComb_eq_sum
  条件: (w : 标准单纯形 R I) (f : I -> M)
  证明: by
  simp [iConvexComb, sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]

Depends on / 依赖: Finsupp, Finsupp.sum_mapDomain_index, add_smul, iConvexComb, sConvexComb_eq_sum, sum_mapDomain_index
-/
lemma iConvexComb_eq_sum (w : StdSimplex R I) (f : I -> M) :
    w.iConvexComb f = w.weights.sum fun i r => r • f i := by
  simp [iConvexComb, sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]

/-- `convexCombPair` in a module can be expressed as a sum. -/
@[simp]
/--
lemma `convexCombPair_eq_sum` / 引理 `convexCombPair_eq_sum`

English:
lemma convexCombPair_eq_sum
  given: (a b : R) (ha hb hab) (x y : M)
  proof: by
  classical simp [convexCombPair, sConvexComb_eq_sum, Finsupp.sum_add_index, add_smul]

中文:
引理 convexCombPair_eq_sum
  条件: (a b : R) (ha hb hab) (x y : M)
  证明: by
  classical simp [convexCombPair, sConvexComb_eq_sum, Finsupp.sum_add_index, add_smul]

Depends on / 依赖: Finsupp, Finsupp.sum_add_index, add_smul, classical, convexCombPair, sConvexComb_eq_sum, sum_add_index
-/
lemma convexCombPair_eq_sum (a b : R) (ha hb hab) (x y : M) :
    convexCombPair a b ha hb hab x y = a • x + b • y := by
  classical simp [convexCombPair, sConvexComb_eq_sum, Finsupp.sum_add_index, add_smul]

/--
lemma `IsAffineMap.map_sum_weights` / 引理 `IsAffineMap.map_sum_weights`

English:
lemma IsAffineMap.map_sum_weights
  given: (hf : IsAffineMap R f) (w : StdSimplex R I) (g : I -> M)
  proof: by
  simpa using hf.map_iConvexComb w g

中文:
引理 是仿射映射.map_sum_weights
  条件: (hf : 是仿射映射 R f) (w : 标准单纯形 R I) (g : I -> M)
  证明: by
  simpa using hf.map_iConvexComb w g

Depends on / 依赖: hf.map_iConvexComb, map_iConvexComb
-/
lemma IsAffineMap.map_sum_weights (hf : IsAffineMap R f) (w : StdSimplex R I) (g : I -> M) :
    f (w.weights.sum fun i r => r • g i) = w.weights.sum fun i r => r • f (g i) := by
  simpa using hf.map_iConvexComb w g

/--
lemma `IsAffineMap.map_smul_add_smul` / 引理 `IsAffineMap.map_smul_add_smul`

English:
lemma IsAffineMap.map_smul_add_smul
  statement: (hf : IsAffineMap R f) (ha : 0 <= a) (hb : 0 <= b)
  proof: by
  simpa using hf.map_convexCombPair ha hb hab x y

中文:
引理 是仿射映射.map_smul_add_smul
  结论: (hf : 是仿射映射 R f) (ha : 0 <= a) (hb : 0 <= b)
  证明: by
  simpa using hf.map_convexCombPair ha hb hab x y

Depends on / 依赖: hf.map_convexCombPair, map_convexCombPair
-/
lemma IsAffineMap.map_smul_add_smul (hf : IsAffineMap R f) (ha : 0 <= a) (hb : 0 <= b)
    (hab : a + b = 1) (x y : M) : f (a • x + b • y) = a • f x + b • f y := by
  simpa using hf.map_convexCombPair ha hb hab x y

/--
lemma `isConvexSet_coe` / 引理 `isConvexSet_coe`

English:
lemma isConvexSet_coe
  given: (S : F)
  statement: IsConvexSet R (S : Set M)
  proof: by
  refine .of_sConvexComb_mem fun w hw => ?_
  rw [sConvexComb_eq_sum]
exact AddSubmonoidClass.finsuppSum_mem _ _ _ fun m hm => SMulMemClass.smul_mem _ hw by simpa

中文:
引理 isConvexSet_coe
  条件: (S : F)
  结论: IsConvexSet R (S : 集合 M)
  证明: by
  refine .of_sConvexComb_mem fun w hw => ?_
  rw [sConvexComb_eq_sum]
exact AddSubmonoidClass.finsuppSum_mem _ _ _ fun m hm => SMulMemClass.smul_mem _ hw by simpa
-/
@[simp] lemma isConvexSet_coe (S : F) : IsConvexSet R (S : Set M) := by
  refine .of_sConvexComb_mem fun w hw => ?_
  rw [sConvexComb_eq_sum]
exact AddSubmonoidClass.finsuppSum_mem _ _ _ fun m hm => SMulMemClass.smul_mem _ hw by simpa

instance (S : F) : ConvexSpace R S := .subtype _ isConvexSet_coe _

@[simp]
/--
lemma `subtypeVal_submodule_sConvexComb` / 引理 `subtypeVal_submodule_sConvexComb`

English:
lemma subtypeVal_submodule_sConvexComb
  given: (S : F) (w : StdSimplex R S)
  proof: rfl

中文:
引理 subtypeVal_submodule_sConvexComb
  条件: (S : F) (w : 标准单纯形 R S)
  证明: rfl
-/
lemma subtypeVal_submodule_sConvexComb (S : F) (w : StdSimplex R S) :
    (w.sConvexComb : M) = w.iConvexComb (↑) := rfl

/--
lemma `subtypeVal_submodule_iConvexComb` / 引理 `subtypeVal_submodule_iConvexComb`

English:
lemma subtypeVal_submodule_iConvexComb
  given: (S : F) (w : StdSimplex R I) (f : I -> S)
  proof: subtypeVal_iConvexComb ..

中文:
引理 subtypeVal_submodule_iConvexComb
  条件: (S : F) (w : 标准单纯形 R I) (f : I -> S)
  证明: subtypeVal_iConvexComb ..

Depends on / 依赖: subtypeVal_iConvexComb
-/
lemma subtypeVal_submodule_iConvexComb (S : F) (w : StdSimplex R I) (f : I -> S) :
    (↑(w.iConvexComb f) : M) = w.iConvexComb (fun i => (f i).val) := subtypeVal_iConvexComb ..

/--
lemma `subtypeVal_submodule_convexCombPair` / 引理 `subtypeVal_submodule_convexCombPair`

English:
lemma subtypeVal_submodule_convexCombPair
  given: (S : F) (a b : R) (ha hb hab) (x y : S)
  proof: subtypeVal_convexCombPair ..

中文:
引理 subtypeVal_submodule_convexCombPair
  条件: (S : F) (a b : R) (ha hb hab) (x y : S)
  证明: subtypeVal_convexCombPair ..

Depends on / 依赖: subtypeVal_convexCombPair
-/
lemma subtypeVal_submodule_convexCombPair (S : F) (a b : R) (ha hb hab) (x y : S) :
    (↑(convexCombPair a b ha hb hab x y) : M) = convexCombPair a b ha hb hab x.val y.val :=
  subtypeVal_convexCombPair ..

instance (S : F) : IsModuleConvexSpace R S where sConvexComb_eq_sum w := by ext; simp [Finsupp.sum]

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: IsModuleConvexSpace R (M × N)
  body: by ext <;> simp [Finsupp.sum, Prod.fst_sum, Prod.snd_sum]

中文:
实例 :
  签名: 是ModuleConvex空间 R (M × N)
  定义体: by ext <;> simp [Finsupp.sum, Prod.fst_sum, Prod.snd_sum]

Depends on / 依赖: Finsupp, Finsupp.sum, Prod.fst_sum, Prod.snd_sum, fst_sum, snd_sum
-/
instance : IsModuleConvexSpace R (M × N) where
  sConvexComb_eq_sum w := by ext <;> simp [Finsupp.sum, Prod.fst_sum, Prod.snd_sum]

instance {ι : Type*} {M : ι -> Type*} [forall i, AddCommMonoid (M i)] [forall i, Module R (M i)]
    [forall i, ConvexSpace R (M i)] [forall i, IsModuleConvexSpace R (M i)] :
    IsModuleConvexSpace R (forall i, M i) where
  sConvexComb_eq_sum w := by ext; simp [Finsupp.sum]

instance {ι : Type*} : IsModuleConvexSpace R (ι ->₀ M) where
  sConvexComb_eq_sum w := by ext; simp [Finsupp.sum]

@[to_fun (attr := fun_prop)]
/--
lemma `IsAffineMap.add` / 引理 `IsAffineMap.add`

English:
lemma IsAffineMap.add
  given: (hf : IsAffineMap R f) (hg : IsAffineMap R g)
  statement: IsAffineMap R (f + g) where
  proof: by
    simp [hf.map_sum_weights, hg.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

中文:
引理 是仿射映射.add
  条件: (hf : 是仿射映射 R f) (hg : 是仿射映射 R g)
  结论: 是仿射映射 R (f + g) where
  证明: by
    simp [hf.map_sum_weights, hg.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

Depends on / 依赖: Finsupp, Finsupp.sum_mapDomain_index, add_smul, hf.map_sum_weights, hg.map_sum_weights, map_sum_weights, sum_mapDomain_index
-/
lemma IsAffineMap.add (hf : IsAffineMap R f) (hg : IsAffineMap R g) : IsAffineMap R (f + g) where
  map_sConvexComb w := by
    simp [hf.map_sum_weights, hg.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

/--
lemma `IsStarConvexSet.add` / 引理 `IsStarConvexSet.add`

English:
lemma IsStarConvexSet.add
  given: (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t)
  proof: by
  rw [← Set.add_image_prod]; exact (hs.prod ht).image (by fun_prop)

中文:
引理 IsStarConvexSet.add
  条件: (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t)
  证明: by
  rw [← Set.add_image_prod]; exact (hs.prod ht).image (by fun_prop)

Depends on / 依赖: Set.add_image_prod, add_image_prod, fun_prop, hs.prod
-/
lemma IsStarConvexSet.add (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t) :
    IsStarConvexSet R (x + y) (s + t) := by
  rw [← Set.add_image_prod]; exact (hs.prod ht).image (by fun_prop)

end IsModuleConvexSpace

variable (R I) in
/--
lemma `StdSimplex.isAffineMap_weights` / 引理 `StdSimplex.isAffineMap_weights`

English:
lemma StdSimplex.isAffineMap_weights
  statement: IsAffineMap R (weights (R := R) (M := I)) where
  proof: by simp [sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]

中文:
引理 标准单纯形.isAffineMap_weights
  结论: 是仿射映射 R (weights (R := R) (M := I)) where
  证明: by simp [sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]
-/
lemma StdSimplex.isAffineMap_weights : IsAffineMap R (weights (R := R) (M := I)) where
  map_sConvexComb s := by simp [sConvexComb_eq_sum, Finsupp.sum_mapDomain_index, add_smul]

end AddCommMonoid

section AddCommGroup
variable [AddCommGroup M] [Module R M] [AddCommGroup N] [Module R N]
  [ConvexSpace R M] [IsModuleConvexSpace R M] [ConvexSpace R N] [IsModuleConvexSpace R N]
  {x y : M} {s t : Set M} {f g : M -> N}

@[to_fun (attr := fun_prop)]
/--
lemma `IsAffineMap.neg` / 引理 `IsAffineMap.neg`

English:
lemma IsAffineMap.neg
  given: (hf : IsAffineMap R f)
  statement: IsAffineMap R (-f) where
  proof: by simp [hf.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

@[to_fun (attr := fun_prop)]

中文:
引理 是仿射映射.neg
  条件: (hf : 是仿射映射 R f)
  结论: 是仿射映射 R (-f) where
  证明: by simp [hf.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

@[to_fun (attr := fun_prop)]

Depends on / 依赖: Finsupp, Finsupp.sum_mapDomain_index, add_smul, hf.map_sum_weights, map_sum_weights, sum_mapDomain_index
-/
lemma IsAffineMap.neg (hf : IsAffineMap R f) : IsAffineMap R (-f) where
  map_sConvexComb w := by simp [hf.map_sum_weights, Finsupp.sum_mapDomain_index, add_smul]

@[to_fun (attr := fun_prop)]
/--
lemma `IsAffineMap.sub` / 引理 `IsAffineMap.sub`

English:
lemma IsAffineMap.sub
  given: (hf : IsAffineMap R f) (hg : IsAffineMap R g)
  statement: IsAffineMap R (f - g)
  proof: by
  simpa [sub_eq_add_neg] using hf.add hg.neg

中文:
引理 是仿射映射.sub
  条件: (hf : 是仿射映射 R f) (hg : 是仿射映射 R g)
  结论: 是仿射映射 R (f - g)
  证明: by
  simpa [sub_eq_add_neg] using hf.add hg.neg

Depends on / 依赖: hf.add, hg.neg, sub_eq_add_neg
-/
lemma IsAffineMap.sub (hf : IsAffineMap R f) (hg : IsAffineMap R g) : IsAffineMap R (f - g) := by
  simpa [sub_eq_add_neg] using hf.add hg.neg

/--
lemma `IsStarConvexSet.neg` / 引理 `IsStarConvexSet.neg`

English:
lemma IsStarConvexSet.neg
  given: (hs : IsStarConvexSet R x s)
  statement: IsStarConvexSet R (-x) (-s)
  proof: by
  rw [← Set.image_neg_eq_neg]; exact hs.image (by fun_prop)

中文:
引理 IsStarConvexSet.neg
  条件: (hs : IsStarConvexSet R x s)
  结论: IsStarConvexSet R (-x) (-s)
  证明: by
  rw [← Set.image_neg_eq_neg]; exact hs.image (by fun_prop)

Depends on / 依赖: Set.image_neg_eq_neg, fun_prop, hs.image, image_neg_eq_neg
-/
lemma IsStarConvexSet.neg (hs : IsStarConvexSet R x s) : IsStarConvexSet R (-x) (-s) := by
  rw [← Set.image_neg_eq_neg]; exact hs.image (by fun_prop)

/--
lemma `IsStarConvexSet.sub` / 引理 `IsStarConvexSet.sub`

English:
lemma IsStarConvexSet.sub
  given: (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t)
  proof: by
  rw [← Set.sub_image_prod]; exact (hs.prod ht).image (by fun_prop)

中文:
引理 IsStarConvexSet.sub
  条件: (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t)
  证明: by
  rw [← Set.sub_image_prod]; exact (hs.prod ht).image (by fun_prop)

Depends on / 依赖: Set.sub_image_prod, fun_prop, hs.prod, sub_image_prod
-/
lemma IsStarConvexSet.sub (hs : IsStarConvexSet R x s) (ht : IsStarConvexSet R y t) :
    IsStarConvexSet R (x - y) (s - t) := by
  rw [← Set.sub_image_prod]; exact (hs.prod ht).image (by fun_prop)

end AddCommGroup
end Convexity
