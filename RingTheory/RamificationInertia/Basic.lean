...
/--
theorem `sum_ramification_inertia_eq_finrank_fiber` / 定理 `sum_ramification_inertia_eq_finrank_fiber`

English:
theorem sum_ramification_inertia_eq_finrank_fiber
  proof: by
  let := Fintype.ofFinite (PrimeSpectrum (p.Fiber S))
  rw [IsArtinianRing.finrank_eq_sum_primeSpectrum]; rw [← (primesOverOrderIsoFiber R S p).symm.sum_comp]
...
/--
theorem `sum_ramification_inertia_eq_finrank` / 定理 `sum_ramification_inertia_eq_finrank`

English:
theorem sum_ramification_inertia_eq_finrank
  proof: by
  rw [sum_ramification_inertia_eq_finrank_fiber]; rw [finrank_fiber_eq_finrank]

中文:
定理 sum_ramification_inertia_eq_finrank
  证明: by
  rw [sum_ramification_inertia_eq_finrank_fiber]; rw [finrank_fiber_eq_finrank]

Depends on / 依赖: finrank_fiber_eq_finrank, sum_ramification_inertia_eq_finrank_fiber
-/
theorem sum_ramification_inertia_eq_finrank
...
/--
theorem `sum_ramification_inertia_eq_card` / 定理 `sum_ramification_inertia_eq_card`

English:
theorem sum_ramification_inertia_eq_card
  proof: by
  let := IsGaloisGroup.finite G R S
  rw [sum_ramification_inertia_eq_finrank]; rw [IsGaloisGroup.card_eq_finrank' G R S]

中文:
定理 sum_ramification_inertia_eq_card
  证明: by
  let := IsGaloisGroup.finite G R S
  rw [sum_ramification_inertia_eq_finrank]; rw [IsGaloisGroup.card_eq_finrank' G R S]

Depends on / 依赖: IsGaloisGroup, IsGaloisGroup.card_eq_finrank, IsGaloisGroup.finite, card_eq_finrank, finite, sum_ramification_inertia_eq_finrank
-/
theorem sum_ramification_inertia_eq_card
