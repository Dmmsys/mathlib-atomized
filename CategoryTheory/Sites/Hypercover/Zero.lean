/-
Copyright (c) 2025 Christian Merten. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Christian Merten
-/
module

public import Mathlib.CategoryTheory.Sites.Precoverage
public import Mathlib.CategoryTheory.Limits.Shapes.Products

/-!
# 0-hypercovers

Given a coverage `J` on a category `C`, we define the type
of `0`-hypercovers of an object `S : C`. They consist of a covering family
of morphisms `X i ⟶ S` indexed by a type `I₀` such that the induced presieve is in `J`.

We define this with respect to a coverage and not to a Grothendieck topology, because this
yields more control over the components of the cover.
-/

@[expose] public section

universe w'' w' w v u

namespace CategoryTheory

open Category Limits

variable {C : Type u} [Category.{v} C]

/-- The categorical data that is involved in a `0`-hypercover of an object `S`. This
consists of a family of morphisms `f i : X i ⟶ S` for `i : I₀`. -/
@[ext]
/-
**CategoryTheory.PreZeroHypercover** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTheory`。
形式化陈述：{C : Type u} → [CategoryTheory.Category.{v, u} C] → C → Type (max (max u v
) (w + 1))
参数：max (max u v) (w + 1)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The categorical data that is involved in a `0`-hypercover of an object `S`. This
consists of a family of morphisms `f i : X i ⟶ S` for `i : I₀`.
-/
structure PreZeroHypercover (S : C) where
  /-- the index type of the covering of `S` -/
  I₀ : Type w
  /-- the objects in the covering of `S` -/
  X (i : I₀) : C
  /-- the morphisms in the covering of `S` -/
  f (i : I₀) : X i ⟶ S

namespace PreZeroHypercover

variable {S T : C} (E : PreZeroHypercover.{w} S) (F : PreZeroHypercover.{w'} S)

/-- The assumption that the pullback of `X i₁` and `X i₂` over `S` exists
for any `i₁` and `i₂`. -/
/-
**CategoryTheory.PreZeroHypercover.HasPullbacks** 是 Mathlib 中的一个缩写定义，位于命名空间 `Cat
egoryTheory.PreZeroHypercover`。
形式化陈述：HasPullbacks
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The assumption that the pullback of `X i₁` and `X i₂` over `S` exists
for any `i₁` and `i₂`.
-/
abbrev HasPullbacks := ∀ (i₁ i₂ : E.I₀), HasPullback (E.f i₁) (E.f i₂)

/-- The presieve of `S` that is generated by the morphisms `f i : X i ⟶ S`. -/
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个缩写定义，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The presieve of `S` that is generated by the morphisms `f i : X i ⟶ S`.
-/
abbrev presieve₀ : Presieve S := .ofArrows _ E.f

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_f (i : E.I₀) : E.presieve₀ (E.f i) := ⟨i⟩

grind_pattern presieve₀_f => E.presieve₀ (E.f i)

/-- The sieve of `S` that is generated by the morphisms `f i : X i ⟶ S`. -/
/-
**CategoryTheory.PreZeroHypercover.sieve** 是 Mathlib 中的一个缩写定义，位于命名空间 `CategoryTh
eory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The sieve of `S` that is generated by the morphisms `f i : X i ⟶ S`.
-/
abbrev sieve₀ : Sieve S := .ofArrows _ E.f
/-
**CategoryTheory.PreZeroHypercover.sieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma sieve₀_f (i : E.I₀) : E.sieve₀ (E.f i) := ⟨_, 𝟙 _, E.f i, ⟨i⟩, by simp⟩

grind_pattern sieve₀_f => E.sieve₀ (E.f i)

/-- The pre-`0`-hypercover defined by a single morphism. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.singleton** 是 Mathlib 中的一个定义，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：singleton (f : S ⟶ T) : PreZeroHypercover.{w} T where I₀
参数：f : S ⟶ T。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The pre-`0`-hypercover defined by a single morphism.
-/
def singleton (f : S ⟶ T) : PreZeroHypercover.{w} T where
  I₀ := PUnit
  X _ := S
  f _ := f

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_singleton (f : S ⟶ T) : (singleton f).presieve₀ = .singleton f := by
  simp [singleton, presieve₀, Presieve.ofArrows_pUnit]
/-
**CategoryTheory.PreZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Pr
eZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (f : S ⟶ T) : Unique (PreZeroHypercover.singleton f).I₀ :=
  inferInstanceAs <| Unique PUnit

variable (S) in
/-- The empty pre-`0`-hypercover. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.empty** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
形式化陈述：empty : PreZeroHypercover.{w} S where I₀
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The empty pre-`0`-hypercover.
-/
def empty : PreZeroHypercover.{w} S where
  I₀ := PEmpty
  X := PEmpty.elim
  f i := i.elim
/-
**CategoryTheory.PreZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Pr
eZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : IsEmpty (empty S).I₀ := inferInstanceAs <| IsEmpty PEmpty

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_empty : (empty.{w} S).presieve₀ = ⊥ := by
  grind

/-- Pullback of a pre-`0`-hypercover along a morphism. The components are `pullback f (E.f i)`. -/
@[simps]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.pullback** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullback₁ (f : S ⟶ T) (E : PreZeroHypercover.{w} T) [∀ i, HasPullback f (E.f i)] :
    PreZeroHypercover.{w} S where
  I₀ := E.I₀
  X i := pullback f (E.f i)
  f _ := pullback.fst _ _

/-- Pullback of a pre-`0`-hypercover along a morphism. The components are `pullback (E.f i) f`. -/
@[simps]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.pullback** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullback₂ (f : S ⟶ T) (E : PreZeroHypercover.{w} T) [∀ i, HasPullback (E.f i) f] :
    PreZeroHypercover.{w} S where
  I₀ := E.I₀
  X i := pullback (E.f i) f
  f _ := pullback.snd _ _
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_pullback₁ (f : S ⟶ T) (E : PreZeroHypercover.{w} T) [∀ i, HasPullback (E.f i) f] :
    presieve₀ (E.pullback₂ f) = E.presieve₀.pullbackArrows f := by
  refine le_antisymm ?_ ?_
  · rintro - - ⟨i⟩
    use _, _, i
  · rintro W g ⟨-, -, ⟨i⟩⟩
    use i

/-- If `{Uᵢ}` covers `X`, this is the pre-`0`-hypercover of `X ×[Z] Y` given by `{Uᵢ ×[Z] Y}`. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.pullbackCoverOfLeft** 是 Mathlib 中的一个定义，位于命名空间
 `CategoryTheory.PreZeroHypercover`。
形式化陈述：pullbackCoverOfLeft {X : C} (E : PreZeroHypercover X) {Y Z : C} (f : X ⟶ Z
) (g : Y ⟶ Z) [HasPullback f g] [forall i, HasPullback (E.f i ≫ f) g] : PreZeroH
ypercover (pullback f g) where I₀
参数：E : PreZeroHypercover X；f : X ⟶ Z；g : Y ⟶ Z；E.f i ≫ f。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `{Uᵢ}` covers `X`, this is the pre-`0`-hypercover of `X ×[Z] Y` given by `{Uᵢ
 ×[Z] Y}`.
-/
noncomputable def pullbackCoverOfLeft {X : C} (E : PreZeroHypercover X) {Y Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ i, HasPullback (E.f i ≫ f) g] :
    PreZeroHypercover (pullback f g) where
  I₀ := E.I₀
  X i := pullback (E.f i ≫ f) g
  f i := pullback.map _ _ _ _ (E.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp)

/-- If `{Uᵢ}` covers `Y`, this is the pre-`0`-hypercover of `X ×[Z] Y` given by `{X ×[Z] Uᵢ}`. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.pullbackCoverOfRight** 是 Mathlib 中的一个定义，位于命名空
间 `CategoryTheory.PreZeroHypercover`。
形式化陈述：pullbackCoverOfRight {Y : C} (E : PreZeroHypercover.{w} Y) {X Z : C} (f : 
X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [forall i, HasPullback f (E.f i ≫ g)] : Pre
ZeroHypercover.{w} (pullback f g) where I₀
参数：E : PreZeroHypercover.{w} Y；f : X ⟶ Z；g : Y ⟶ Z；E.f i ≫ g。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `{Uᵢ}` covers `Y`, this is the pre-`0`-hypercover of `X ×[Z] Y` given by `{X 
×[Z] Uᵢ}`.
-/
noncomputable def pullbackCoverOfRight {Y : C} (E : PreZeroHypercover.{w} Y) {X Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ i, HasPullback f (E.f i ≫ g)] :
    PreZeroHypercover.{w} (pullback f g) where
  I₀ := E.I₀
  X i := pullback f (E.f i ≫ g)
  f i := pullback.map _ _ _ _ (𝟙 X) (E.f i) (𝟙 Z) (by simp) (by simp)

/-- Refining each component of a pre-`0`-hypercover yields a refined pre-`0`-hypercover of the
base. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.bind** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheor
y.PreZeroHypercover`。
形式化陈述：bind (E : PreZeroHypercover.{w} T) (F : forall i, PreZeroHypercover.{w'} (
E.X i)) : PreZeroHypercover.{max w w'} T where I₀
参数：E : PreZeroHypercover.{w} T；F : forall i, PreZeroHypercover.{w'} (E.X i)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Refining each component of a pre-`0`-hypercover yields a refined pre-`0`-hyperco
ver of the
base.
-/
def bind (E : PreZeroHypercover.{w} T) (F : ∀ i, PreZeroHypercover.{w'} (E.X i)) :
    PreZeroHypercover.{max w w'} T where
  I₀ := Σ (i : E.I₀), (F i).I₀
  X ij := (F ij.1).X ij.2
  f ij := (F ij.1).f ij.2 ≫ E.f ij.1

/-- Restrict the indexing type to `ι` by precomposing with a function `ι → E.I₀`. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.restrictIndex** 是 Mathlib 中的一个定义，位于命名空间 `Cate
goryTheory.PreZeroHypercover`。
形式化陈述：restrictIndex (E : PreZeroHypercover.{w} T) {ι : Type w'} (f : ι -> E.I₀) 
: PreZeroHypercover.{w'} T where I₀
参数：E : PreZeroHypercover.{w} T；f : ι -> E.I₀。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Restrict the indexing type to `ι` by precomposing with a function `ι → E.I₀`.
-/
def restrictIndex (E : PreZeroHypercover.{w} T) {ι : Type w'} (f : ι → E.I₀) :
    PreZeroHypercover.{w'} T where
  I₀ := ι
  X := E.X ∘ f
  f i := E.f (f i)

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_restrictIndex_equiv {ι : Type w'} (e : ι ≃ E.I₀) :
    (E.restrictIndex e).presieve₀ = E.presieve₀ := by
  refine le_antisymm (fun Y g ⟨i⟩ ↦ ⟨e i⟩) fun Y g ⟨i⟩ ↦ ?_
  obtain ⟨i, rfl⟩ := e.surjective i
  exact ⟨i⟩

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_restrictIndex_le {ι : Type*} (f : ι → E.I₀) :
    (E.restrictIndex f).presieve₀ ≤ E.presieve₀ := by
  rw [Presieve.ofArrows_le_iff]
  intro i
  exact .mk _

/-- Replace the indexing type of a pre-`0`-hypercover. -/
@[simps!]
/-
**CategoryTheory.PreZeroHypercover.reindex** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTh
eory.PreZeroHypercover`。
形式化陈述：reindex (E : PreZeroHypercover.{w} T) {ι : Type w'} (e : ι ≃ E.I₀) : PreZe
roHypercover.{w'} T
参数：E : PreZeroHypercover.{w} T；e : ι ≃ E.I₀。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Replace the indexing type of a pre-`0`-hypercover.
-/
def reindex (E : PreZeroHypercover.{w} T) {ι : Type w'} (e : ι ≃ E.I₀) :
    PreZeroHypercover.{w'} T :=
  E.restrictIndex e

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_reindex {ι : Type w'} (e : ι ≃ E.I₀) : (E.reindex e).presieve₀ = E.presieve₀ := by
  simp [reindex]

#adaptation_note
/-- `respectTransparency.types true` changes the auto-generated lemmas' signature -/
set_option backward.isDefEq.respectTransparency.types false in
/-- Pairwise intersection of two pre-`0`-hypercovers. -/
@[simps!]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.inter** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
形式化陈述：inter (E : PreZeroHypercover.{w} T) (F : PreZeroHypercover.{w'} T) [forall
 i j, HasPullback (E.f i) (F.f j)] : PreZeroHypercover.{max w w'} T
参数：E : PreZeroHypercover.{w} T；F : PreZeroHypercover.{w'} T；E.f i；F.f j。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
-/
def inter (E : PreZeroHypercover.{w} T) (F : PreZeroHypercover.{w'} T)
    [∀ i j, HasPullback (E.f i) (F.f j)] :
    PreZeroHypercover.{max w w'} T :=
  (E.bind (fun i ↦ F.pullback₁ (E.f i))).reindex (Equiv.sigmaEquivProd _ _).symm
/-
**CategoryTheory.PreZeroHypercover.inter_def** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：inter_def [forall i j, HasPullback (E.f i) (F.f j)] : E.inter F = (E.bind 
(fun i => F.pullback₁ (E.f i))).reindex (Equiv.sigmaEquivProd _ _).symm
参数：E.f i；F.f j。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inter_def [∀ i j, HasPullback (E.f i) (F.f j)] :
    E.inter F = (E.bind (fun i ↦ F.pullback₁ (E.f i))).reindex (Equiv.sigmaEquivProd _ _).symm :=
  rfl

/-- Disjoint union of two pre-`0`-hypercovers. -/
@[simps I₀, simps -isSimp X f]
/-
**CategoryTheory.PreZeroHypercover.sum** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory
.PreZeroHypercover`。
形式化陈述：sum {X : C} (E : PreZeroHypercover.{w} X) (F : PreZeroHypercover.{w'} X) :
 PreZeroHypercover.{max w w'} X where I₀
参数：E : PreZeroHypercover.{w} X；F : PreZeroHypercover.{w'} X。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Disjoint union of two pre-`0`-hypercovers.
-/
def sum {X : C} (E : PreZeroHypercover.{w} X) (F : PreZeroHypercover.{w'} X) :
    PreZeroHypercover.{max w w'} X where
  I₀ := E.I₀ ⊕ F.I₀
  X := Sum.elim E.X F.X
  f
    | .inl i => E.f i
    | .inr i => F.f i
/-
**CategoryTheory.PreZeroHypercover.sum_X_inl** 是 Mathlib 中的一个定理，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S)   (F : CategoryTheory.PreZeroHypercover S) (i : 
E.I₀), (E.sum F).X (Sum.inl i) = E.X i
参数：E : CategoryTheory.PreZeroHypercover S；F : CategoryTheory.PreZeroHypercover S
；i : E.I₀；E.sum F；Sum.inl i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma sum_X_inl (i : E.I₀) : (E.sum F).X (.inl i) = E.X i := rfl
/-
**CategoryTheory.PreZeroHypercover.sum_X_inr** 是 Mathlib 中的一个定理，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S)   (F : CategoryTheory.PreZeroHypercover S) (i : 
F.I₀), (E.sum F).X (Sum.inr i) = F.X i
参数：E : CategoryTheory.PreZeroHypercover S；F : CategoryTheory.PreZeroHypercover S
；i : F.I₀；E.sum F；Sum.inr i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma sum_X_inr (i : F.I₀) : (E.sum F).X (.inr i) = F.X i := rfl
/-
**CategoryTheory.PreZeroHypercover.sum_f_inl** 是 Mathlib 中的一个定理，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S)   (F : CategoryTheory.PreZeroHypercover S) (i : 
E.I₀), (E.sum F).f (Sum.inl i) = E.f i
参数：E : CategoryTheory.PreZeroHypercover S；F : CategoryTheory.PreZeroHypercover S
；i : E.I₀；E.sum F；Sum.inl i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma sum_f_inl (i : E.I₀) : (E.sum F).f (.inl i) = E.f i := rfl
/-
**CategoryTheory.PreZeroHypercover.sum_f_inr** 是 Mathlib 中的一个定理，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S)   (F : CategoryTheory.PreZeroHypercover S) (i : 
F.I₀), (E.sum F).f (Sum.inr i) = F.f i
参数：E : CategoryTheory.PreZeroHypercover S；F : CategoryTheory.PreZeroHypercover S
；i : F.I₀；E.sum F；Sum.inr i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma sum_f_inr (i : F.I₀) : (E.sum F).f (.inr i) = F.f i := rfl

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_sum : (E.sum F).presieve₀ = E.presieve₀ ⊔ F.presieve₀ := by
  rw [presieve₀, presieve₀, presieve₀]
  apply le_antisymm
  · intro Z g ⟨i⟩
    cases i
    · exact Or.inl (.mk _)
    · exact Or.inr (.mk _)
  · rintro Z g (⟨⟨i⟩⟩ | ⟨⟨i⟩⟩)
    · exact ⟨Sum.inl i⟩
    · exact ⟨Sum.inr i⟩

/-- Add a morphism to a pre-`0`-hypercover. -/
@[simps! I₀]
/-
**CategoryTheory.PreZeroHypercover.add** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory
.PreZeroHypercover`。
形式化陈述：add (E : PreZeroHypercover.{w} S) {T : C} (f : T ⟶ S) : PreZeroHypercover.
{w} S
参数：E : PreZeroHypercover.{w} S；f : T ⟶ S。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Add a morphism to a pre-`0`-hypercover.
-/
def add (E : PreZeroHypercover.{w} S) {T : C} (f : T ⟶ S) : PreZeroHypercover.{w} S :=
  (E.sum (.singleton f)).reindex (Equiv.optionEquivSumPUnit.{0} E.I₀)
/-
**CategoryTheory.PreZeroHypercover.add_X_some** 是 Mathlib 中的一个定理，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S) {T : C}   (f : T ⟶ S) (i : E.I₀), (E.add f).X (s
ome i) = E.X i
参数：E : CategoryTheory.PreZeroHypercover S；f : T ⟶ S；i : E.I₀；E.add f；some i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma add_X_some {T : C} (f : T ⟶ S) (i : E.I₀) : (E.add f).X (some i) = E.X i := rfl
/-
**CategoryTheory.PreZeroHypercover.add_X_none** 是 Mathlib 中的一个定理，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S) {T : C}   (f : T ⟶ S), (E.add f).X none = T
参数：E : CategoryTheory.PreZeroHypercover S；f : T ⟶ S；E.add f。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma add_X_none {T : C} (f : T ⟶ S) : (E.add f).X none = T := rfl
/-
**CategoryTheory.PreZeroHypercover.add_f_some** 是 Mathlib 中的一个定理，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S) {T : C}   (f : T ⟶ S) (i : E.I₀), (E.add f).f (s
ome i) = E.f i
参数：E : CategoryTheory.PreZeroHypercover S；f : T ⟶ S；i : E.I₀；E.add f；some i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma add_f_some {T : C} (f : T ⟶ S) (i : E.I₀) : (E.add f).f (some i) = E.f i := rfl
/-
**CategoryTheory.PreZeroHypercover.add_f_nome** 是 Mathlib 中的一个定理，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (E : Cate
goryTheory.PreZeroHypercover S) {T : C}   (f : T ⟶ S), (E.add f).f none = f
参数：E : CategoryTheory.PreZeroHypercover S；f : T ⟶ S；E.add f。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma add_f_nome {T : C} (f : T ⟶ S) : (E.add f).f none = f := rfl

set_option backward.isDefEq.respectTransparency.types false in
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] lemma presieve₀_add {T : C} (f : T ⟶ S) :
    (E.add f).presieve₀ = E.presieve₀ ⊔ .singleton f := by
  simp [add, presieve₀_reindex, presieve₀_sum]

/-- The single object pre-`0`-hypercover obtained from taking the coproduct of the components. -/
@[simps I₀ X, simps -isSimp f, implicit_reducible]
/-
**CategoryTheory.PreZeroHypercover.sigmaOfIsColimit** 是 Mathlib 中的一个定义，位于命名空间 `C
ategoryTheory.PreZeroHypercover`。
形式化陈述：sigmaOfIsColimit (E : PreZeroHypercover.{w} S) {c : Cofan E.X} (hc : IsCol
imit c) : PreZeroHypercover.{w} S where I₀
参数：E : PreZeroHypercover.{w} S；hc : IsColimit c。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The single object pre-`0`-hypercover obtained from taking the coproduct of the c
omponents.
-/
def sigmaOfIsColimit (E : PreZeroHypercover.{w} S) {c : Cofan E.X} (hc : IsColimit c) :
    PreZeroHypercover.{w} S where
  I₀ := PUnit
  X _ := c.pt
  f _ := Cofan.IsColimit.desc hc E.f

@[reassoc (attr := simp)]
/-
**CategoryTheory.PreZeroHypercover.inj_sigmaOfIsColimit_f** 是 Mathlib 中的一个引理，位于命
名空间 `CategoryTheory.PreZeroHypercover`。
形式化陈述：inj_sigmaOfIsColimit_f (E : PreZeroHypercover.{w} S) {c : Cofan E.X} (hc :
 IsColimit c) (i : E.I₀) (r : PUnit) : c.inj i ≫ (E.sigmaOfIsColimit hc).f r = E
.f i
参数：E : PreZeroHypercover.{w} S；hc : IsColimit c；i : E.I₀；r : PUnit。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `CategoryTheory.Limits.Cofan.IsColimit.fac`：∀ {β : Type w} {C : Type u} [
inst : CategoryTheory.Category.{v, u} C] {F : β → C} {c : CategoryTheory.Limits.
Cofan F}   (hc : CategoryTheory…
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
lemma inj_sigmaOfIsColimit_f (E : PreZeroHypercover.{w} S) {c : Cofan E.X} (hc : IsColimit c)
    (i : E.I₀) (r : PUnit) :
    c.inj i ≫ (E.sigmaOfIsColimit hc).f r = E.f i := by
  simp [PreZeroHypercover.sigmaOfIsColimit]

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_sigmaOfIsColimit (E : PreZeroHypercover.{w} S) {c : Cofan E.X} (hc : IsColimit c) :
    (E.sigmaOfIsColimit hc).presieve₀ = Presieve.singleton (Cofan.IsColimit.desc hc E.f) :=
  Presieve.ofArrows_pUnit _

section Category

variable {E : PreZeroHypercover.{w} S} {F : PreZeroHypercover.{w'} S}

/-- A morphism of pre-`0`-hypercovers of `S` is a family of refinement morphisms commuting
with the structure morphisms of `E` and `F`. -/
@[ext]
/-
**CategoryTheory.PreZeroHypercover.Hom** 是 Mathlib 中的一个结构，位于命名空间 `CategoryTheory
.PreZeroHypercover`。
形式化陈述：Hom (E : PreZeroHypercover.{w} S) (F : PreZeroHypercover.{w'} S) where /--
 The map between indexing types of the coverings of `S` -/ s₀ (i : E.I₀) : F.I₀ 
/-- The refinement morphisms between objects in the coverings of `S`. -/ h₀ (i :
 E.I₀) : E.X i ⟶ F.X (s₀ i) w₀ (i : E.I₀) : h₀ i ≫ F.f (s₀ i) = E.f i
参数：E : PreZeroHypercover.{w} S；F : PreZeroHypercover.{w'} S；i : E.I₀。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A morphism of pre-`0`-hypercovers of `S` is a family of refinement morphisms com
muting
with the structure morphisms of `E` and `F`.
-/
structure Hom (E : PreZeroHypercover.{w} S) (F : PreZeroHypercover.{w'} S) where
  /-- The map between indexing types of the coverings of `S` -/
  s₀ (i : E.I₀) : F.I₀
  /-- The refinement morphisms between objects in the coverings of `S`. -/
  h₀ (i : E.I₀) : E.X i ⟶ F.X (s₀ i)
  w₀ (i : E.I₀) : h₀ i ≫ F.f (s₀ i) = E.f i := by cat_disch

attribute [reassoc (attr := simp)] Hom.w₀

/-- The identity refinement of a pre-`0`-hypercover. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.Hom.id** 是 Mathlib 中的一个定义，位于命名空间 `CategoryThe
ory.PreZeroHypercover.Hom`。
形式化陈述：{C : Type u} → [inst : CategoryTheory.Category.{v, u} C] → {S : C} → (E : 
CategoryTheory.PreZeroHypercover S) → E.Hom E
参数：E : CategoryTheory.PreZeroHypercover S。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The identity refinement of a pre-`0`-hypercover.
-/
def Hom.id (E : PreZeroHypercover S) : Hom E E where
  s₀ := _root_.id
  h₀ _ := 𝟙 _

variable {G : PreZeroHypercover S}

/-- Composition of refinement morphisms of pre-`0`-hypercovers. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.Hom.comp** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover.Hom`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     {S : C} →
       {E : CategoryTheory.PreZeroHypercover S} →         {F : CategoryTheory.Pr
eZeroHypercover S} →           {G : CategoryTheory.PreZeroHypercover S} → E.Hom 
F → F.Hom G → E.Hom G
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Composition of refinement morphisms of pre-`0`-hypercovers.
-/
def Hom.comp (f : E.Hom F) (g : F.Hom G) : E.Hom G where
  s₀ := g.s₀ ∘ f.s₀
  h₀ i := f.h₀ i ≫ g.h₀ _

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
@[simps! id_s₀ id_h₀ comp_s₀ comp_h₀]
/-
**CategoryTheory.PreZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Pr
eZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : Category (PreZeroHypercover S) where
  Hom := Hom
  id E := Hom.id E
  comp f g := f.comp g
/-
**CategoryTheory.PreZeroHypercover.Hom.ext'** 是 Mathlib 中的一个定理，位于命名空间 `CategoryT
heory.PreZeroHypercover.Hom`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} {E : Cate
goryTheory.PreZeroHypercover S}   {F : CategoryTheory.PreZeroHypercover S} {f g 
: E.Hom F} (hs : f.s₀ = g.s₀),   (∀ (i : E.I₀), f.h₀ i = CategoryTheory.Category
Struct.comp (g.h₀ i) (CategoryTheory.eqToHom ⋯)) → f = g
参数：hs : f.s₀ = g.s₀；∀ (i : E.I₀), f.h₀ i = CategoryTheory.CategoryStruct.comp (g
.h₀ i) (CategoryTheory.eqToHom ⋯)。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `CategoryTheory.PreZeroHypercover.Hom.mk.injEq`：∀ {C : Type u} [inst : Ca
tegoryTheory.Category.{v, u} C] {S : C} {E : CategoryTheory.PreZeroHypercover S}
   {F : CategoryTheory.PreZeroHyper…
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `heq_eq_eq`：∀ {α : Sort u_1} (a b : α), (a ≍ b) = (a = b)
· 使用定理 `true_and`：∀ (p : Prop), (True ∧ p) = p
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `CategoryTheory.Category.comp_id`：∀ {obj : Type u} [self : CategoryTheory
.Category.{v, u} obj] {X Y : obj} (f : X ⟶ Y),   CategoryTheory.CategoryStruct.c
omp f (CategoryTheory…
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
-/
lemma Hom.ext' {E : PreZeroHypercover.{w} S} {F : PreZeroHypercover.{w'} S}
    {f g : E.Hom F} (hs : f.s₀ = g.s₀) (hh : ∀ i, f.h₀ i = g.h₀ i ≫ eqToHom (by rw [hs])) :
    f = g := by
  cases f
  cases g
  simp only at hs
  cat_disch
/-
**CategoryTheory.PreZeroHypercover.Hom.ext'_iff** 是 Mathlib 中的一个定理，位于命名空间 `Categ
oryTheory.PreZeroHypercover.Hom`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} {E : Cate
goryTheory.PreZeroHypercover S}   {F : CategoryTheory.PreZeroHypercover S} {f g 
: E.Hom F},   f = g ↔     ∃ (hs : f.s₀ = g.s₀), ∀ (i : E.I₀), f.h₀ i = CategoryT
heory.CategoryStruct.comp (g.h₀ i) (CategoryTheory.eqToHom ⋯)
参数：hs : f.s₀ = g.s₀；i : E.I₀；g.h₀ i；CategoryTheory.eqToHom ⋯。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `exists_prop_congr`：∀ {p p' : Prop} {q q' : p → Prop}, (∀ (h : p), q h ↔ 
q' h) → ∀ (hp : p ↔ p'), Exists q ↔ ∃ (h : p'), q' ⋯
· 使用定理 `Iff.of_eq`：∀ {a b : Prop}, a = b → (a ↔ b)
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `CategoryTheory.Category.comp_id`：∀ {obj : Type u} [self : CategoryTheory
.Category.{v, u} obj] {X Y : obj} (f : X ⟶ Y),   CategoryTheory.CategoryStruct.c
omp f (CategoryTheory…
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `implies_true`：∀ (α : Sort u), (∀ (a : α), True) = True
· 使用定理 `instNonemptyOfInhabited`：∀ {α : Sort u} [Inhabited α], Nonempty α
· 使用定理 `CategoryTheory.PreZeroHypercover.Hom.ext'`：∀ {C : Type u} [inst : Catego
ryTheory.Category.{v, u} C] {S : C} {E : CategoryTheory.PreZeroHypercover S}   {
F : CategoryTheory.PreZeroHyper…
-/
lemma Hom.ext'_iff {E : PreZeroHypercover.{w} S} {F : PreZeroHypercover.{w'} S}
    {f g : E.Hom F} :
    f = g ↔ ∃ (hs : f.s₀ = g.s₀), ∀ i, f.h₀ i = g.h₀ i ≫ eqToHom (by rw [hs]) :=
  ⟨fun h ↦ h ▸ by simp, fun ⟨hs, hh⟩ ↦ Hom.ext' hs hh⟩

set_option backward.defeqAttrib.useBackward true in
/-- Constructor for isomorphisms of pre-`0`-hypercovers. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.isoMk** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
形式化陈述：isoMk {S : C} {E F : PreZeroHypercover.{w} S} (s₀ : E.I₀ ≃ F.I₀) (h₀ : for
all i, E.X i ≅ F.X (s₀ i)) (w₀ : forall i, (h₀ i).hom ≫ F.f _ = E.f _
参数：s₀ : E.I₀ ≃ F.I₀；h₀ : forall i, E.X i ≅ F.X (s₀ i)。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s

--- 原说明 ---
Constructor for isomorphisms of pre-`0`-hypercovers.
-/
def isoMk {S : C} {E F : PreZeroHypercover.{w} S}
    (s₀ : E.I₀ ≃ F.I₀) (h₀ : ∀ i, E.X i ≅ F.X (s₀ i))
    (w₀ : ∀ i, (h₀ i).hom ≫ F.f _ = E.f _ := by cat_disch) :
    E ≅ F where
  hom.s₀ := s₀
  hom.h₀ i := (h₀ i).hom
  inv.s₀ := s₀.symm
  inv.h₀ i := eqToHom (by simp) ≫ (h₀ _).inv
  inv.w₀ i := by
    obtain ⟨i, rfl⟩ := s₀.surjective i
    simp only [← cancel_epi (h₀ i).hom, w₀, Category.assoc, Equiv.symm_apply_apply,
      eqToHom_iso_hom_naturality_assoc, Iso.hom_inv_id_assoc]
    rw [← CategoryTheory.eqToHom_naturality _ (by simp)]
    simp
  hom_inv_id := Hom.ext' (by ext; simp) (fun i ↦ by simp)
  inv_hom_id := Hom.ext' (by ext; simp) (fun i ↦ by simp)

@[simp]
/-
**CategoryTheory.PreZeroHypercover.hom_inv_s** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma hom_inv_s₀_apply {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : E.I₀) :
    e.inv.s₀ (e.hom.s₀ i) = i :=
  congr($(e.hom_inv_id).s₀ i)

@[simp]
/-
**CategoryTheory.PreZeroHypercover.inv_hom_s** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inv_hom_s₀_apply {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : F.I₀) :
    e.hom.s₀ (e.inv.s₀ i) = i :=
  congr($(e.inv_hom_id).s₀ i)

set_option backward.defeqAttrib.useBackward true in
@[reassoc (attr := simp)]
/-
**CategoryTheory.PreZeroHypercover.hom_inv_h** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma hom_inv_h₀ {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : E.I₀) :
    e.hom.h₀ i ≫ e.inv.h₀ (e.hom.s₀ i) = eqToHom (by simp) := by
  obtain ⟨hs, hh⟩ := Hom.ext'_iff.mp e.hom_inv_id
  simpa using hh i

set_option backward.defeqAttrib.useBackward true in
@[reassoc (attr := simp)]
/-
**CategoryTheory.PreZeroHypercover.inv_hom_h** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inv_hom_h₀ {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : F.I₀) :
    e.inv.h₀ i ≫ e.hom.h₀ (e.inv.s₀ i) = eqToHom (by simp) := by
  obtain ⟨hs, hh⟩ := Hom.ext'_iff.mp e.inv_hom_id
  simpa using hh i
/-
**CategoryTheory.PreZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Pr
eZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : E.I₀) :
    IsIso (e.hom.h₀ i) := by
  use e.inv.h₀ (e.hom.s₀ i) ≫ eqToHom (by simp)
  rw [hom_inv_h₀_assoc, eqToHom_trans, eqToHom_refl, Category.assoc,
    ← eqToHom_naturality _ (by simp), inv_hom_h₀_assoc]
  simp
/-
**CategoryTheory.PreZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Pr
eZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : F.I₀) :
    IsIso (e.inv.h₀ i) :=
  .of_isIso_fac_right (inv_hom_h₀ e i)

@[reassoc (attr := simp)]
/-
**CategoryTheory.PreZeroHypercover.inv_hom_h** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inv_hom_h₀_comp_f {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : E.I₀) :
    inv (e.hom.h₀ i) ≫ E.f i = F.f _ := by simp

@[reassoc (attr := simp)]
/-
**CategoryTheory.PreZeroHypercover.inv_inv_h** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inv_inv_h₀_comp_f {E F : PreZeroHypercover.{w} S} (e : E ≅ F) (i : F.I₀) :
    inv (e.inv.h₀ i) ≫ F.f i = E.f _ := by simp
/-
**CategoryTheory.PreZeroHypercover.Hom.sieve** 是 Mathlib 中的一个引理，位于命名空间 `Category
Theory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma Hom.sieve₀_le_sieve₀ {E F : PreZeroHypercover S} (f : E.Hom F) : E.sieve₀ ≤ F.sieve₀ := by
  rw [Sieve.generate_le_iff, Presieve.ofArrows_le_iff]
  intro i
  rw [← f.w₀ i]
  apply Sieve.downward_closed
  exact Sieve.le_generate _ _ _ ⟨f.s₀ i⟩
/-
**CategoryTheory.PreZeroHypercover.sieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma sieve₀_eq_of_iso {E F : PreZeroHypercover S} (e : E ≅ F) : E.sieve₀ = F.sieve₀ :=
  le_antisymm e.hom.sieve₀_le_sieve₀ e.inv.sieve₀_le_sieve₀

/-- The equivalence on index types induced by an isomorphism of pre-`0`-hypercovers. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.equivOfIso** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：equivOfIso {E F : PreZeroHypercover.{w} S} (e : E ≅ F) : E.I₀ ≃ F.I₀ where
 toFun
参数：e : E ≅ F。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The equivalence on index types induced by an isomorphism of pre-`0`-hypercovers.
-/
def equivOfIso {E F : PreZeroHypercover.{w} S} (e : E ≅ F) : E.I₀ ≃ F.I₀ where
  toFun := e.hom.s₀
  invFun := e.inv.s₀
  left_inv _ := by simp
  right_inv _ := by simp
/-
**CategoryTheory.PreZeroHypercover.mem_of_iso** 是 Mathlib 中的一个引理，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：mem_of_iso {K : Precoverage C} [K.IsStableUnderComposition] [K.HasIsos] {X
 : C} {E F : PreZeroHypercover.{w} X} (e : E ≅ F) (hE : E.presieve₀ in K X) : F.
presieve₀ in K X
参数：e : E ≅ F；hE : E.presieve₀ in K X。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `CategoryTheory.PreZeroHypercover.Hom.w₀`：∀ {C : Type u} [inst : Category
Theory.Category.{v, u} C] {S : C} {E : CategoryTheory.PreZeroHypercover S}   {F 
: CategoryTheory.PreZeroHyper…
· 使用引理 `le_antisymm`：le_antisymm : a <= b -> b <= a -> a = b
· 使用引理 `CategoryTheory.Presieve.ofArrows_le_iff`：ofArrows_le_iff {X : C} {ι : Ty
pe*} {Y : ι -> C} {f : forall i, Y i ⟶ X} {R : Presieve X} : Presieve.ofArrows Y
 f <= R ↔ forall i, R (f i)
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `implies_true`：∀ (α : Sort u), (∀ (a : α), True) = True
· 使用引理 `CategoryTheory.Precoverage.comp_mem_coverings`：comp_mem_coverings {J : P
recoverage C} [IsStableUnderComposition J] {ι : Type w} {S : C} {X : ι -> C} (f 
: forall i, X i ⟶ S) (hf : Presieve…
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用引理 `CategoryTheory.PreZeroHypercover.presieve₀_reindex`：presieve₀_reindex {ι
 : Type w'} (e : ι ≃ E.I₀) : (E.reindex e).presieve₀ = E.presieve₀
· 使用定理 `CategoryTheory.Presieve.ofArrows_pUnit`：ofArrows_pUnit : (ofArrows _ fun
 _ : PUnit.{w + 1} => f) = singleton f
· 使用定理 `CategoryTheory.Precoverage.mem_coverings_of_isIso`：∀ {C : Type u} {inst 
: CategoryTheory.Category.{v, u} C} {J : CategoryTheory.Precoverage C} [self : J
.HasIsos] {S T : C}   (f : S ⟶ T) [Cate…
· 使用定理 `CategoryTheory.PreZeroHypercover.instIsIsoH₀Inv`：∀ {C : Type u} [inst : 
CategoryTheory.Category.{v, u} C] {S : C} {E F : CategoryTheory.PreZeroHypercove
r S} (e : E ≅ F)   (i : F.I₀), Catego…
-/
lemma mem_of_iso {K : Precoverage C} [K.IsStableUnderComposition] [K.HasIsos] {X : C}
    {E F : PreZeroHypercover.{w} X} (e : E ≅ F) (hE : E.presieve₀ ∈ K X) :
    F.presieve₀ ∈ K X := by
  have : F.presieve₀ =
      Presieve.ofArrows (fun (i : Σ (_ : F.I₀), Unit) ↦ _) (fun i ↦ e.inv.h₀ i.1 ≫ E.f _) := by
    simp only [Hom.w₀]
    refine le_antisymm ?_ ?_
    · rw [Presieve.ofArrows_le_iff]
      intro i
      exact .mk (⟨i, ⟨⟩⟩ : Σ (_ : F.I₀), Unit)
    · simp [Presieve.ofArrows_le_iff]
  rw [this]
  refine K.comp_mem_coverings (fun i ↦ E.f (e.inv.s₀ i)) ?_ (fun i (k : Unit) ↦ e.inv.h₀ i) ?_
  · rwa [← E.presieve₀_reindex (PreZeroHypercover.equivOfIso e.symm)] at hE
  · intro i
    rw [Presieve.ofArrows_pUnit]
    exact K.mem_coverings_of_isIso _
/-
**CategoryTheory.PreZeroHypercover.mem_iff_of_iso** 是 Mathlib 中的一个引理，位于命名空间 `Cat
egoryTheory.PreZeroHypercover`。
形式化陈述：mem_iff_of_iso {K : Precoverage C} [K.IsStableUnderComposition] [K.HasIsos
] {X : C} {E F : PreZeroHypercover.{w} X} (e : E ≅ F) : E.presieve₀ in K X ↔ F.p
resieve₀ in K X
参数：e : E ≅ F。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.PreZeroHypercover.mem_of_iso`：mem_of_iso {K : Precoverage
 C} [K.IsStableUnderComposition] [K.HasIsos] {X : C} {E F : PreZeroHypercover.{w
} X} (e : E ≅ F) (hE : E.presieve…
-/
lemma mem_iff_of_iso {K : Precoverage C} [K.IsStableUnderComposition] [K.HasIsos] {X : C}
    {E F : PreZeroHypercover.{w} X} (e : E ≅ F) :
    E.presieve₀ ∈ K X ↔ F.presieve₀ ∈ K X :=
  ⟨fun h ↦ PreZeroHypercover.mem_of_iso e h, fun h ↦ PreZeroHypercover.mem_of_iso e.symm h⟩

/-- Compose a pre-`0`-hypercover with a morphism on the right. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.pushforward** 是 Mathlib 中的一个定义，位于命名空间 `Catego
ryTheory.PreZeroHypercover`。
形式化陈述：pushforward {X Y : C} (f : X ⟶ Y) (E : PreZeroHypercover.{w} X) : PreZeroH
ypercover.{w} Y where I₀
参数：f : X ⟶ Y；E : PreZeroHypercover.{w} X。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Compose a pre-`0`-hypercover with a morphism on the right.
-/
def pushforward {X Y : C} (f : X ⟶ Y) (E : PreZeroHypercover.{w} X) :
    PreZeroHypercover.{w} Y where
  I₀ := E.I₀
  X := E.X
  f i := E.f i ≫ f
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_pushforward {X Y : C} (f : X ⟶ Y) (E : PreZeroHypercover.{w} X) :
    (E.pushforward f).presieve₀ = E.presieve₀.pushforward f := by
  simp [presieve₀, Presieve.pushforward_ofArrows, pushforward]

set_option backward.isDefEq.respectTransparency false in
/-- Pushforward along a morphism is the same as refining the singleton pre-`0`-hypercover. -/
@[simps!]
/-
**CategoryTheory.PreZeroHypercover.pushforwardIsoBind** 是 Mathlib 中的一个定义，位于命名空间 
`CategoryTheory.PreZeroHypercover`。
形式化陈述：pushforwardIsoBind {X Y : C} (f : X ⟶ Y) (E : PreZeroHypercover.{w} X) : E
.pushforward f ≅ (singleton f).bind fun _ => E
参数：f : X ⟶ Y；E : PreZeroHypercover.{w} X。
该定义给出了一等式。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s

--- 原说明 ---
Pushforward along a morphism is the same as refining the singleton pre-`0`-hyper
cover.
-/
def pushforwardIsoBind {X Y : C} (f : X ⟶ Y) (E : PreZeroHypercover.{w} X) :
    E.pushforward f ≅ (singleton f).bind fun _ ↦ E :=
  isoMk ((Equiv.uniqueSigma fun i ↦ E.I₀).symm) (fun _ ↦ Iso.refl _)

end Category

section Functoriality

variable {D : Type*} [Category* D] {F : C ⥤ D}

/-- The image of a pre-`0`-hypercover under a functor. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.map** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory
.PreZeroHypercover`。
形式化陈述：map (F : C ⥤ D) (E : PreZeroHypercover.{w} S) : PreZeroHypercover.{w} (F.o
bj S) where I₀
参数：F : C ⥤ D；E : PreZeroHypercover.{w} S。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The image of a pre-`0`-hypercover under a functor.
-/
def map (F : C ⥤ D) (E : PreZeroHypercover.{w} S) : PreZeroHypercover.{w} (F.obj S) where
  I₀ := E.I₀
  X i := F.obj (E.X i)
  f i := F.map (E.f i)
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_map : (E.map F).presieve₀ = E.presieve₀.map F :=
  (Presieve.map_ofArrows _).symm
/-
**CategoryTheory.PreZeroHypercover.sieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheo
ry.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma sieve₀_map : (E.map F).sieve₀ = E.sieve₀.functorPushforward F := by
  simp [← Sieve.generate_map_eq_functorPushforward, Presieve.map_ofArrows, map]

end Functoriality

set_option backward.isDefEq.respectTransparency false in
/-- Pullback symmetry isomorphism. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.pullbackIso** 是 Mathlib 中的一个定义，位于命名空间 `Catego
ryTheory.PreZeroHypercover`。
形式化陈述：pullbackIso {S T : C} (f : S ⟶ T) (E : PreZeroHypercover.{w} T) [forall (i
 : E.I₀), HasPullback f (E.f i)] [forall (i : E.I₀), HasPullback (E.f i) f] : E.
pullback₁ f ≅ E.pullback₂ f where hom.s₀
参数：f : S ⟶ T；E : PreZeroHypercover.{w} T；i : E.I₀；E.f i；i : E.I₀；E.f i。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Pullback symmetry isomorphism.
-/
noncomputable def pullbackIso {S T : C} (f : S ⟶ T) (E : PreZeroHypercover.{w} T)
    [∀ (i : E.I₀), HasPullback f (E.f i)] [∀ (i : E.I₀), HasPullback (E.f i) f] :
    E.pullback₁ f ≅ E.pullback₂ f where
  hom.s₀ := id
  hom.h₀ i := (pullbackSymmetry _ _).hom
  inv.s₀ := id
  inv.h₀ i := (pullbackSymmetry _ _).inv
  hom_inv_id := by
    apply Hom.ext (by rfl)
    simp only [heq_eq_eq]
    ext i
    simp
  inv_hom_id := by
    apply Hom.ext (by rfl)
    simp only [heq_eq_eq]
    ext i
    simp

section

variable (F : PreZeroHypercover.{w'} S) {G : PreZeroHypercover.{w''} S}

/-- The left inclusion into the disjoint union. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.sumInl** 是 Mathlib 中的一个定义，位于命名空间 `CategoryThe
ory.PreZeroHypercover`。
形式化陈述：sumInl : E.Hom (E.sum F) where s₀
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The left inclusion into the disjoint union.
-/
def sumInl : E.Hom (E.sum F) where
  s₀ := Sum.inl
  h₀ _ := 𝟙 _

/-- The right inclusion into the disjoint union. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.sumInr** 是 Mathlib 中的一个定义，位于命名空间 `CategoryThe
ory.PreZeroHypercover`。
形式化陈述：sumInr : F.Hom (E.sum F) where s₀
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The right inclusion into the disjoint union.
-/
def sumInr : F.Hom (E.sum F) where
  s₀ := Sum.inr
  h₀ _ := 𝟙 _

variable {E F} in
/-- To give a refinement of the disjoint union, it suffices to give refinements of both
components. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.sumLift** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTh
eory.PreZeroHypercover`。
形式化陈述：sumLift (f : E.Hom G) (g : F.Hom G) : (E.sum F).Hom G where s₀
参数：f : E.Hom G；g : F.Hom G。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
To give a refinement of the disjoint union, it suffices to give refinements of b
oth
components.
-/
def sumLift (f : E.Hom G) (g : F.Hom G) : (E.sum F).Hom G where
  s₀ := Sum.elim f.s₀ g.s₀
  h₀
    | .inl i => f.h₀ i
    | .inr i => g.h₀ i

variable [∀ (i : E.I₀) (j : F.I₀), HasPullback (E.f i) (F.f j)]

set_option backward.isDefEq.respectTransparency.types false in
/-- First projection from the intersection of two pre-`0`-hypercovers. -/
@[simps]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.interFst** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
形式化陈述：interFst : Hom (inter E F) E where s₀ i
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
-/
def interFst : Hom (inter E F) E where
  s₀ i := i.1
  h₀ _ := pullback.fst _ _

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- Second projection from the intersection of two pre-`0`-hypercovers. -/
@[simps]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.interSnd** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
形式化陈述：interSnd : Hom (inter E F) F where s₀ i
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
-/
def interSnd : Hom (inter E F) F where
  s₀ i := i.2
  h₀ _ := pullback.snd _ _
  w₀ i := by simp [← pullback.condition]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
variable {E F} in
/-- Universal property of the intersection of two pre-`0`-hypercovers. -/
@[simps]
noncomputable
/-
**CategoryTheory.PreZeroHypercover.interLift** 是 Mathlib 中的一个定义，位于命名空间 `Category
Theory.PreZeroHypercover`。
形式化陈述：interLift (f : G.Hom E) (g : G.Hom F) : G.Hom (E.inter F) where s₀ i
参数：f : G.Hom E；g : G.Hom F。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
-/
def interLift (f : G.Hom E) (g : G.Hom F) :
    G.Hom (E.inter F) where
  s₀ i := ⟨f.s₀ i, g.s₀ i⟩
  h₀ i := pullback.lift (f.h₀ i) (g.h₀ i) (by simp)

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/-- The refinement given by restricting the indexing type. -/
@[simps]
/-
**CategoryTheory.PreZeroHypercover.restrictIndexHom** 是 Mathlib 中的一个定义，位于命名空间 `C
ategoryTheory.PreZeroHypercover`。
形式化陈述：restrictIndexHom {ι : Type w'} (f : ι -> E.I₀) : (E.restrictIndex f).Hom E
 where s₀
参数：f : ι -> E.I₀。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The refinement given by restricting the indexing type.
-/
def restrictIndexHom {ι : Type w'} (f : ι → E.I₀) : (E.restrictIndex f).Hom E where
  s₀ := f
  h₀ _ := 𝟙 _

end

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- If `{Uᵢ}` covers `X`, the pre-`0`-hypercover `{Uᵢ ×[Z] Y}` of `X ×[Z] Y` is isomorphic
to the pullback of `{Uᵢ}` along the first projection. -/
noncomputable
/-
**CategoryTheory.PreZeroHypercover.pullbackCoverOfLeftIsoPullback** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullbackCoverOfLeftIsoPullback₁ {X : C} (E : PreZeroHypercover X) {Y Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ i, HasPullback (pullback.fst f g) (E.f i)]
    [∀ i, HasPullback (E.f i) (pullback.fst f g)] :
    E.pullbackCoverOfLeft f g ≅ pullback₁ (pullback.fst f g) E :=
  PreZeroHypercover.isoMk (.refl _)
    (fun _ ↦ (pullbackRightPullbackFstIso _ _ _).symm ≪≫ pullbackSymmetry _ _)

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- If `{Uᵢ}` covers `Y`, the pre-`0`-hypercover `{X ×[Z] Uᵢ}` of `X ×[Z] Y` is isomorphic
to the pullback of `{Uᵢ}` along the second projection. -/
noncomputable
/-
**CategoryTheory.PreZeroHypercover.pullbackCoverOfRightIsoPullback** 是 Mathlib 中
的一个定义，位于命名空间 `CategoryTheory.PreZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullbackCoverOfRightIsoPullback₂ {Y : C} (E : PreZeroHypercover Y) {X Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ (i : E.I₀), HasPullback (E.f i) (pullback.snd f g)]
    [∀ i, HasPullback (pullback.snd f g) (E.f i)] :
    E.pullbackCoverOfRight f g ≅ pullback₂ (pullback.snd f g) E :=
  PreZeroHypercover.isoMk (.refl _)
    (fun _ ↦ (pullbackLeftPullbackSndIso _ _ _).symm ≪≫ pullbackSymmetry _ _)

end PreZeroHypercover

/-- The pre-`0`-hypercover associated to a presieve `R`. It is indexed by the morphisms in `R`. -/
@[simps -isSimp]
/-
**CategoryTheory.Presieve.preZeroHypercover** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.Presieve`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] → {S : C} → Cat
egoryTheory.Presieve S → CategoryTheory.PreZeroHypercover S
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The pre-`0`-hypercover associated to a presieve `R`. It is indexed by the morphi
sms in `R`.
-/
def Presieve.preZeroHypercover {S : C} (R : Presieve S) : PreZeroHypercover.{max u v} S where
  I₀ := R.uncurry
  X i := i.1.1
  f i := i.1.2

@[simp]
/-
**CategoryTheory.Presieve.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma Presieve.presieve₀_preZeroHypercover {S : C} (R : Presieve S) :
    R.preZeroHypercover.presieve₀ = R := by
  refine le_antisymm ?_ ?_
  · rintro - - ⟨i⟩
    exact i.2
  · intro Y f h
    let i : R.uncurry := ⟨⟨Y, f⟩, h⟩
    exact .mk i
/-
**CategoryTheory.Presieve.exists_eq_preZeroHypercover** 是 Mathlib 中的一个定理，位于命名空间 
`CategoryTheory.Presieve`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {S : C} (R : Cate
goryTheory.Presieve S), ∃ E, R = E.presieve₀
参数：R : CategoryTheory.Presieve S。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `CategoryTheory.Presieve.presieve₀_preZeroHypercover`：∀ {C : Type u} [ins
t : CategoryTheory.Category.{v, u} C] {S : C} (R : CategoryTheory.Presieve S),  
 R.preZeroHypercover.presieve₀ = R
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
lemma Presieve.exists_eq_preZeroHypercover {S : C} (R : Presieve S) :
    ∃ (E : PreZeroHypercover.{max u v} S), R = E.presieve₀ :=
  ⟨R.preZeroHypercover, by simp⟩

/-- The deduplication of a pre-`0`-hypercover `E` in universe `w` to a pre-`0`-hypercover in
universe `max u v`. This is indexed by the morphisms of `E`. -/
@[simps! -isSimp]
/-
**CategoryTheory.PreZeroHypercover.shrink** 是 Mathlib 中的一个定义，位于命名空间 `CategoryThe
ory.PreZeroHypercover`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     {S : C} →
 CategoryTheory.PreZeroHypercover S → CategoryTheory.PreZeroHypercover S
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The deduplication of a pre-`0`-hypercover `E` in universe `w` to a pre-`0`-hyper
cover in
universe `max u v`. This is indexed by the morphisms of `E`.
-/
def PreZeroHypercover.shrink {S : C} (E : PreZeroHypercover.{w} S) :
    PreZeroHypercover.{max u v} S :=
  E.presieve₀.preZeroHypercover

@[simp]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma PreZeroHypercover.presieve₀_shrink {S : C} (E : PreZeroHypercover.{w} S) :
    E.shrink.presieve₀ = E.presieve₀ :=
  E.presieve₀.presieve₀_preZeroHypercover
/-
**CategoryTheory.PreZeroHypercover.shrink_eq_shrink_of_presieve** 是 Mathlib 中的一个
引理，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma PreZeroHypercover.shrink_eq_shrink_of_presieve₀_eq_presieve₀ {S : C}
    {E F : PreZeroHypercover.{w} S} (h : E.presieve₀ = F.presieve₀) :
    E.shrink = F.shrink := by
  rw [shrink, shrink, h]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma PreZeroHypercover.presieve₀_eq_presieve₀_iff {S : C} {E F : PreZeroHypercover.{w} S} :
    E.presieve₀ = F.presieve₀ ↔ E.shrink = F.shrink := by
  refine ⟨fun h ↦ shrink_eq_shrink_of_presieve₀_eq_presieve₀ h, fun h ↦ ?_⟩
  rw [← E.presieve₀_shrink, ← F.presieve₀_shrink, h]

set_option backward.isDefEq.respectTransparency false in
/-- `E` refines its deduplication. -/
/-
**CategoryTheory.PreZeroHypercover.toShrink** 是 Mathlib 中的一个定义，位于命名空间 `CategoryT
heory.PreZeroHypercover`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] → {S : C} → (E 
: CategoryTheory.PreZeroHypercover S) → E.Hom E.shrink
参数：E : CategoryTheory.PreZeroHypercover S。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`E` refines its deduplication.
-/
def PreZeroHypercover.toShrink {S : C} (E : PreZeroHypercover.{w} S) : E.Hom E.shrink where
  s₀ i := ⟨⟨_, E.f i⟩, .mk i⟩
  h₀ i := 𝟙 _

/-- The deduplication of `E` refines `E`. -/
noncomputable
/-
**CategoryTheory.PreZeroHypercover.fromShrink** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.PreZeroHypercover`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] → {S : C} → (E 
: CategoryTheory.PreZeroHypercover S) → E.shrink.Hom E
参数：E : CategoryTheory.PreZeroHypercover S。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def PreZeroHypercover.fromShrink {S : C} (E : PreZeroHypercover.{w} S) : E.shrink.Hom E where
  s₀ i := (Presieve.ofArrows_surj _ _ i.2).choose
  h₀ i := eqToHom (Presieve.ofArrows_surj _ _ i.2).choose_spec.1.symm
  w₀ i := (Presieve.ofArrows_surj _ _ i.2).choose_spec.2.symm

section

/-- A precoverage respects isomorphisms if the property of being covering
is stable under isomorphism.
Use `PreZeroHypercover.presieve₀_mem_of_iso` for no universe restrictions. -/
/-
**CategoryTheory.Precoverage.RespectsIso** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTh
eory.Precoverage`。
形式化陈述：{C : Type u} → [inst : CategoryTheory.Category.{v, u} C] → CategoryTheory.
Precoverage C → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A precoverage respects isomorphisms if the property of being covering
is stable under isomorphism.
Use `PreZeroHypercover.presieve₀_mem_of_iso` for no universe restrictions.
-/
class Precoverage.RespectsIso (J : Precoverage C) : Prop where
  of_iso {S : C} {E F : PreZeroHypercover.{max u v} S} (e : E ≅ F) :
    E.presieve₀ ∈ J S → F.presieve₀ ∈ J S

variable {J : Precoverage C}
/-
**CategoryTheory.Precoverage.RespectsIso.of_forall_exists_iso** 是 Mathlib 中的一个定理
，位于命名空间 `CategoryTheory.Precoverage.RespectsIso`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {J : CategoryTheo
ry.Precoverage C} [J.RespectsIso] {S : C}   {R T : CategoryTheory.Presieve S},  
 (∀ ⦃Z : C⦄ (g : Z ⟶ S), R g → ∃ Y e, T (CategoryTheory.CategoryStruct.comp e.ho
m g)) →     (∀ ⦃Z : C⦄ (g : Z ⟶ S), T g → ∃ Y e, R (CategoryTheory.CategoryStruc
t.comp e.hom g)) →       R ∈ J.coverings S → T ∈ J.coverings S
参数：∀ ⦃Z : C⦄ (g : Z ⟶ S), R g → ∃ Y e, T (CategoryTheory.CategoryStruct.comp e.h
om g)；∀ ⦃Z : C⦄ (g : Z ⟶ S), T g → ∃ Y e, R (CategoryTheory.CategoryStruct.comp 
e.hom g)。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Subtype.property`：∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑s
elf
· 使用定理 `Equiv.refl`：Equiv.refl (s : Computation α) : s ~ s
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `CategoryTheory.Iso.inv_hom_id_assoc`：∀ {C : Type u} [inst : CategoryTheo
ry.Category.{v, u} C] {X Y : C} (self : X ≅ Y) {Z : C} (h : Y ⟶ Z),   CategoryTh
eory.CategoryStruct.comp …
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用引理 `le_antisymm`：le_antisymm : a <= b -> b <= a -> a = b
· 使用定理 `eq_of_heq`：∀ {α : Sort u} {a a' : α}, a ≍ a' → a = a'
· 使用定理 `CategoryTheory.Precoverage.RespectsIso.of_iso`：∀ {C : Type u} {inst : Ca
tegoryTheory.Category.{v, u} C} {J : CategoryTheory.Precoverage C} [self : J.Res
pectsIso]   {S : C} {E F : Category…
· 使用定理 `Classical.choose_spec`：∀ {α : Sort u} {p : α → Prop} (h : ∃ x, p x), p (
Classical.choose h)
-/
lemma Precoverage.RespectsIso.of_forall_exists_iso [J.RespectsIso] {S : C} {R T : Presieve S}
    (hRT : ∀ ⦃Z : C⦄ (g : Z ⟶ S), R g → ∃ (Y : C) (e : Y ≅ Z), T (e.hom ≫ g))
    (hTR : ∀ ⦃Z : C⦄ (g : Z ⟶ S), T g → ∃ (Y : C) (e : Y ≅ Z), R (e.hom ≫ g))
    (hR : R ∈ J S) :
    T ∈ J S := by
  choose YR eR hTeg using hRT
  choose YT eT hReg using hTR
  let E : PreZeroHypercover S :=
    { I₀ := R.uncurry ⊕ T.uncurry
      X i := i.elim (fun j ↦ j.1.1) (fun j ↦ YT _ j.2)
      f i :=
        match i with
        | .inl i => by exact i.1.2
        | .inr i => by exact (eT _ i.2).hom ≫ i.1.2 }
  let F : PreZeroHypercover S :=
    { I₀ := R.uncurry ⊕ T.uncurry
      X i := i.elim (fun j ↦ YR _ j.2) (fun j ↦ j.1.1)
      f i :=
        match i with
        | .inl i => by exact (eR _ i.2).hom ≫ i.1.2
        | .inr i => by exact i.1.2 }
  let e : E ≅ F := by
    refine PreZeroHypercover.isoMk (Equiv.refl _) (fun i ↦ ?_) (fun i ↦ ?_)
    · match i with
      | .inl i => dsimp [E, F]; symm; exact eR _ _
      | .inr i => dsimp [E, F]; apply eT
    · cases i <;> simp [E, F]
  have hER : E.presieve₀ = R := by
    refine le_antisymm ?_ fun Y g hg ↦ .mk (Sum.inl (⟨⟨Y, g⟩, hg⟩ : R.uncurry))
    rintro - - ⟨i⟩
    match i with
    | .inl i => exact i.2
    | .inr i => apply hReg
  have hFT : F.presieve₀ = T := by
    refine le_antisymm ?_ fun Y g hg ↦ .mk (Sum.inr (⟨⟨Y, g⟩, hg⟩ : T.uncurry))
    rintro - - ⟨i⟩
    match i with
    | .inl i => apply hTeg
    | .inr i => exact i.2
  rw [← hFT]
  apply RespectsIso.of_iso e
  rwa [hER]
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma PreZeroHypercover.presieve₀_mem_of_iso [J.RespectsIso] {S : C} {E F : PreZeroHypercover.{w} S}
    (e : E ≅ F) (hE : E.presieve₀ ∈ J S) : F.presieve₀ ∈ J S := by
  refine Precoverage.RespectsIso.of_forall_exists_iso ?_ ?_ hE
  · intro Z _ ⟨i⟩
    use F.X (e.hom.s₀ i), (asIso (e.hom.h₀ i)).symm
    simp
  · intro Z _ ⟨i⟩
    use E.X (e.inv.s₀ i), (asIso (e.inv.h₀ i)).symm
    simp
/-
**CategoryTheory.PreZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma PreZeroHypercover.presieve₀_mem_iff_of_iso [J.RespectsIso] {S : C}
    {E F : PreZeroHypercover.{w} S} (e : E ≅ F) :
    E.presieve₀ ∈ J S ↔ F.presieve₀ ∈ J S :=
  ⟨fun h ↦ E.presieve₀_mem_of_iso e h, fun h ↦ F.presieve₀_mem_of_iso e.symm h⟩

end

namespace Precoverage

variable {J : Precoverage C}

/-- The type of `0`-hypercovers of an object `S : C` in a category equipped with a
coverage `J`. This can be constructed from a covering of `S`. -/
/-
**CategoryTheory.Precoverage.ZeroHypercover** 是 Mathlib 中的一个归纳类型，位于命名空间 `Categor
yTheory.Precoverage`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] → CategoryTheor
y.Precoverage C → C → Type (max (max u v) (w + 1))
参数：max (max u v) (w + 1)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The type of `0`-hypercovers of an object `S : C` in a category equipped with a
coverage `J`. This can be constructed from a covering of `S`.
-/
structure ZeroHypercover (J : Precoverage C) (S : C) extends PreZeroHypercover.{w} S where
  mem₀ : toPreZeroHypercover.presieve₀ ∈ J S

namespace ZeroHypercover

variable {S T : C}

/-- The `0`-hypercover defined by a single covering morphism. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.singleton** 是 Mathlib 中的一个定义，位于命名空间 
`CategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：singleton (f : S ⟶ T) (hf : Presieve.singleton f in J T) : J.ZeroHypercove
r T where __
参数：f : S ⟶ T；hf : Presieve.singleton f in J T。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The `0`-hypercover defined by a single covering morphism.
-/
def singleton (f : S ⟶ T) (hf : Presieve.singleton f ∈ J T) :
    J.ZeroHypercover T where
  __ := PreZeroHypercover.singleton f
  mem₀ := by
    simpa [PreZeroHypercover.presieve₀, PreZeroHypercover.singleton, Presieve.ofArrows_pUnit]

/-- Pullback of a `0`-hypercover along a morphism. The components are `pullback f (E.f i)`. -/
@[simps toPreZeroHypercover]
noncomputable
/-
**CategoryTheory.Precoverage.ZeroHypercover.pullback** 是 Mathlib 中的一个定义，位于命名空间 `
CategoryTheory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullback₁ [J.IsStableUnderBaseChange] (f : S ⟶ T) (E : ZeroHypercover.{w} J T)
    [∀ i, HasPullback f (E.f i)] : J.ZeroHypercover S where
  __ := E.toPreZeroHypercover.pullback₁ f
  mem₀ := J.mem_coverings_of_isPullback E.f E.mem₀ f _
    (fun _ ↦ pullback.snd _ _) fun i ↦ IsPullback.of_hasPullback f (E.f i)

/-- Pullback of a `0`-hypercover along a morphism. The components are `pullback (E.f i) f`. -/
@[simps toPreZeroHypercover]
noncomputable
/-
**CategoryTheory.Precoverage.ZeroHypercover.pullback** 是 Mathlib 中的一个定义，位于命名空间 `
CategoryTheory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def pullback₂ [J.IsStableUnderBaseChange] (f : S ⟶ T) (E : ZeroHypercover.{w} J T)
    [∀ i, HasPullback (E.f i) f] : J.ZeroHypercover S where
  __ := E.toPreZeroHypercover.pullback₂ f
  mem₀ := J.mem_coverings_of_isPullback E.f E.mem₀ f _
    (fun _ ↦ pullback.fst _ _) fun i ↦ (IsPullback.of_hasPullback (E.f i) f).flip

/-- Refining each component of a `0`-hypercover yields a refined `0`-hypercover of the base. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.bind** 是 Mathlib 中的一个定义，位于命名空间 `Cate
goryTheory.Precoverage.ZeroHypercover`。
形式化陈述：bind [J.IsStableUnderComposition] (E : ZeroHypercover.{w} J T) (F : forall
 i, ZeroHypercover.{w'} J (E.X i)) : ZeroHypercover.{max w w'} J T where __
参数：E : ZeroHypercover.{w} J T；F : forall i, ZeroHypercover.{w'} J (E.X i)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Refining each component of a `0`-hypercover yields a refined `0`-hypercover of t
he base.
-/
def bind [J.IsStableUnderComposition] (E : ZeroHypercover.{w} J T)
    (F : ∀ i, ZeroHypercover.{w'} J (E.X i)) :
    ZeroHypercover.{max w w'} J T where
  __ := E.toPreZeroHypercover.bind (fun i ↦ (F i).toPreZeroHypercover)
  mem₀ :=
    comp_mem_coverings (f := E.f) (g := fun i j ↦ (F i).f j) E.mem₀ (fun i ↦ (F i).mem₀)

set_option backward.isDefEq.respectTransparency.types false in
/-- Pairwise intersection of two `0`-hypercovers. -/
@[simps toPreZeroHypercover]
noncomputable
/-
**CategoryTheory.Precoverage.ZeroHypercover.inter** 是 Mathlib 中的一个定义，位于命名空间 `Cat
egoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：inter [J.IsStableUnderBaseChange] [J.IsStableUnderComposition] (E : ZeroHy
percover.{w} J T) (F : ZeroHypercover.{w'} J T) [forall i j, HasPullback (E.f i)
 (F.f j)] : ZeroHypercover.{max w w'} J T where __
参数：E : ZeroHypercover.{w} J T；F : ZeroHypercover.{w'} J T；E.f i；F.f j。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
def inter [J.IsStableUnderBaseChange] [J.IsStableUnderComposition] (E : ZeroHypercover.{w} J T)
    (F : ZeroHypercover.{w'} J T) [∀ i j, HasPullback (E.f i) (F.f j)] :
    ZeroHypercover.{max w w'} J T where
  __ := E.toPreZeroHypercover.inter F.toPreZeroHypercover
  mem₀ := by
    rw [PreZeroHypercover.inter_def, PreZeroHypercover.presieve₀_reindex]
    exact (E.bind (fun i ↦ F.pullback₁ (E.f i))).mem₀

/-- Replace the indexing type of a `0`-hypercover. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.reindex** 是 Mathlib 中的一个定义，位于命名空间 `C
ategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：reindex (E : ZeroHypercover.{w} J T) {ι : Type w'} (e : ι ≃ E.I₀) : ZeroHy
percover.{w'} J T where __
参数：E : ZeroHypercover.{w} J T；e : ι ≃ E.I₀。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Replace the indexing type of a `0`-hypercover.
-/
def reindex (E : ZeroHypercover.{w} J T) {ι : Type w'} (e : ι ≃ E.I₀) :
    ZeroHypercover.{w'} J T where
  __ := E.toPreZeroHypercover.reindex e
  mem₀ := by simp [E.mem₀]

/-- Disjoint union of two `0`-hypercovers. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.sum** 是 Mathlib 中的一个定义，位于命名空间 `Categ
oryTheory.Precoverage.ZeroHypercover`。
形式化陈述：sum [J.IsStableUnderSup] (E : ZeroHypercover.{w} J S) (F : ZeroHypercover.
{w'} J S) : ZeroHypercover.{max w w'} J S where __
参数：E : ZeroHypercover.{w} J S；F : ZeroHypercover.{w'} J S。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Disjoint union of two `0`-hypercovers.
-/
def sum [J.IsStableUnderSup] (E : ZeroHypercover.{w} J S) (F : ZeroHypercover.{w'} J S) :
    ZeroHypercover.{max w w'} J S where
  __ := E.toPreZeroHypercover.sum F.toPreZeroHypercover
  mem₀ := by
    rw [PreZeroHypercover.presieve₀_sum]
    exact J.sup_mem_coverings E.mem₀ F.mem₀

/-- Add a single morphism to a `0`-hypercover. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.add** 是 Mathlib 中的一个定义，位于命名空间 `Categ
oryTheory.Precoverage.ZeroHypercover`。
形式化陈述：add (E : ZeroHypercover.{w} J S) {T : C} (f : T ⟶ S) (hf : E.presieve₀ ⊔ P
resieve.singleton f in J S) : ZeroHypercover.{w} J S where __
参数：E : ZeroHypercover.{w} J S；f : T ⟶ S；hf : E.presieve₀ ⊔ Presieve.singleton f 
in J S。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Add a single morphism to a `0`-hypercover.
-/
def add (E : ZeroHypercover.{w} J S) {T : C} (f : T ⟶ S)
    (hf : E.presieve₀ ⊔ Presieve.singleton f ∈ J S) :
    ZeroHypercover.{w} J S where
  __ := E.toPreZeroHypercover.add f
  mem₀ := by rwa [PreZeroHypercover.presieve₀_add]

/-- If `L` is a finer precoverage than `K`, any `0`-hypercover wrt. `K` is in particular
a `0`-hypercover wrt. to `L`. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.weaken** 是 Mathlib 中的一个定义，位于命名空间 `Ca
tegoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：weaken {K L : Precoverage C} {X : C} (E : Precoverage.ZeroHypercover K X) 
(h : K <= L) : Precoverage.ZeroHypercover L X where __
参数：E : Precoverage.ZeroHypercover K X；h : K <= L。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `L` is a finer precoverage than `K`, any `0`-hypercover wrt. `K` is in partic
ular
a `0`-hypercover wrt. to `L`.
-/
def weaken {K L : Precoverage C} {X : C} (E : Precoverage.ZeroHypercover K X) (h : K ≤ L) :
    Precoverage.ZeroHypercover L X where
  __ := E
  mem₀ := h _ E.mem₀

/-- Compose a `0`-hypercover with a morphism on the right. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.pushforward** 是 Mathlib 中的一个定义，位于命名空
间 `CategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：pushforward [J.IsStableUnderComposition] [J.HasIsos] {X Y : C} (f : X ⟶ Y)
 (hf : .singleton f in J _) (E : ZeroHypercover.{w} J X) : ZeroHypercover.{w} J 
Y where __
参数：f : X ⟶ Y；hf : .singleton f in J _；E : ZeroHypercover.{w} J X。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Compose a `0`-hypercover with a morphism on the right.
-/
def pushforward [J.IsStableUnderComposition] [J.HasIsos] {X Y : C} (f : X ⟶ Y)
    (hf : .singleton f ∈ J _) (E : ZeroHypercover.{w} J X) :
    ZeroHypercover.{w} J Y where
  __ := E.toPreZeroHypercover.pushforward f
  mem₀ := by
    rw [PreZeroHypercover.mem_iff_of_iso (E.pushforwardIsoBind _)]
    exact ((ZeroHypercover.singleton f hf).bind _).mem₀
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (K : Precoverage C) [K.HasPullbacks] {X Y : C} (E : K.ZeroHypercover X) (f : Y ⟶ X) :
    E.presieve₀.HasPullbacks f :=
  K.hasPullbacks_of_mem _ E.mem₀
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance {X Y : C} (E : PreZeroHypercover X) (f : Y ⟶ X) [E.presieve₀.HasPullbacks f]
    (i : E.I₀) : HasPullback (E.f i) f :=
  E.presieve₀.hasPullback f ⟨i⟩
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance {X Y : C} (E : PreZeroHypercover X) (f : Y ⟶ X) [E.presieve₀.HasPullbacks f]
    (i : E.I₀) : HasPullback f (E.f i) :=
  hasPullback_symmetry (E.f i) f

variable (J) in
/-- A morphism of `0`-hypercovers is a morphism of the underlying pre-`0`-hypercovers. -/
/-
**CategoryTheory.Precoverage.ZeroHypercover.Hom** 是 Mathlib 中的一个缩写定义，位于命名空间 `Cat
egoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：Hom (E : ZeroHypercover.{w} J S) (F : ZeroHypercover.{w'} J S)
参数：E : ZeroHypercover.{w} J S；F : ZeroHypercover.{w'} J S。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A morphism of `0`-hypercovers is a morphism of the underlying pre-`0`-hypercover
s.
-/
abbrev Hom (E : ZeroHypercover.{w} J S) (F : ZeroHypercover.{w'} J S) :=
  E.toPreZeroHypercover.Hom F.toPreZeroHypercover

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
@[simps! id_s₀ id_h₀ comp_s₀ comp_h₀]
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance : Category (ZeroHypercover.{w} J S) where
  Hom := Hom J
  id _ := PreZeroHypercover.Hom.id _
  comp := PreZeroHypercover.Hom.comp

set_option backward.isDefEq.respectTransparency.types false in
/-- An isomorphism in `0`-hypercovers is an isomorphism of the underlying pre-`0`-hypercovers. -/
@[simps]
/-
**CategoryTheory.Precoverage.ZeroHypercover.isoMk** 是 Mathlib 中的一个定义，位于命名空间 `Cat
egoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：isoMk {E F : ZeroHypercover.{w} J S} (e : E.toPreZeroHypercover ≅ F.toPreZ
eroHypercover) : E ≅ F where hom
参数：e : E.toPreZeroHypercover ≅ F.toPreZeroHypercover。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
An isomorphism in `0`-hypercovers is an isomorphism of the underlying pre-`0`-hy
percovers.
-/
def isoMk {E F : ZeroHypercover.{w} J S} (e : E.toPreZeroHypercover ≅ F.toPreZeroHypercover) :
    E ≅ F where
  hom := e.hom
  inv := e.inv
  hom_inv_id := e.hom_inv_id
  inv_hom_id := e.inv_hom_id

section Functoriality

variable {D : Type*} [Category* D] {F : C ⥤ D} {K : Precoverage D}

/-- The image of a `0`-hypercover under a functor. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.map** 是 Mathlib 中的一个定义，位于命名空间 `Categ
oryTheory.Precoverage.ZeroHypercover`。
形式化陈述：map (F : C ⥤ D) (E : ZeroHypercover.{w} J S) (h : J <= K.comap F) : ZeroHy
percover.{w} K (F.obj S) where __
参数：F : C ⥤ D；E : ZeroHypercover.{w} J S；h : J <= K.comap F。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The image of a `0`-hypercover under a functor.
-/
def map (F : C ⥤ D) (E : ZeroHypercover.{w} J S) (h : J ≤ K.comap F) :
    ZeroHypercover.{w} K (F.obj S) where
  __ := E.toPreZeroHypercover.map F
  mem₀ := by
    rw [PreZeroHypercover.presieve₀_map, ← mem_comap_iff]
    exact h _ E.mem₀

end Functoriality

/-
**CategoryTheory.Precoverage.ZeroHypercover.presieve** 是 Mathlib 中的一个引理，位于命名空间 `
CategoryTheory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma presieve₀_mem_of_iso [J.RespectsIso] {S : C} {E : J.ZeroHypercover S}
    {F : PreZeroHypercover.{w} S} (e : E.toPreZeroHypercover ≅ F) :
    F.presieve₀ ∈ J S :=
  E.toPreZeroHypercover.presieve₀_mem_of_iso e E.mem₀

/--
A `w`-`0`-hypercover `E` is `w'`-small if there exists an indexing type `ι` in `Type w'` and a
restriction map `ι → E.I₀` such that the restriction of `E` to `ι` is still covering.

Note: This is weaker than `E.I₀` being `w'`-small. For example, every Zariski cover of
`X : Scheme.{u}` is `u`-small, because `X` itself suffices as indexing type.
-/
/-
**CategoryTheory.Precoverage.ZeroHypercover.Small** 是 Mathlib 中的一个归纳类型，位于命名空间 `C
ategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] → {J : Category
Theory.Precoverage C} → {S : C} → J.ZeroHypercover S → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A `w`-`0`-hypercover `E` is `w'`-small if there exists an indexing type `ι` in `
Type w'` and a
restriction map `ι → E.I₀` such that the restriction of `E` to `ι` is still cove
ring.

Note: This is weaker than `E.I₀` being `w'`-small. For example, every Zariski co
ver of
`X : Scheme.{u}` is `u`-small, because `X` itself suffices as indexing type.
-/
protected class Small (E : ZeroHypercover.{w} J S) where
  exists_restrictIndex_mem (E) : ∃ (ι : Type w') (f : ι → E.I₀), (E.restrictIndex f).presieve₀ ∈ J S
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (E : ZeroHypercover.{w} J S) [Small.{w'} E.I₀] : ZeroHypercover.Small.{w'} E where
  exists_restrictIndex_mem := ⟨_, (equivShrink E.I₀).symm, by simp [E.mem₀]⟩

/-- The `w'`-index type of a `w'`-small `0`-hypercover. -/
/-
**CategoryTheory.Precoverage.ZeroHypercover.Small.Index** 是 Mathlib 中的一个定义，位于命名空
间 `CategoryTheory.Precoverage.ZeroHypercover.Small`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     {J : Cate
goryTheory.Precoverage C} → {S : C} → (E : J.ZeroHypercover S) → [E.Small] → Typ
e w'
参数：E : J.ZeroHypercover S。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Precoverage.ZeroHypercover.Small.exists_restrictIndex_mem
`：∀ {C : Type u} {inst : CategoryTheory.Category.{v, u} C} {J : CategoryTheory.P
recoverage C} {S : C}   (E : J.ZeroHypercover S) [self : E.Sma…

--- 原说明 ---
The `w'`-index type of a `w'`-small `0`-hypercover.
-/
def Small.Index (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w'} E] : Type w' :=
  (Small.exists_restrictIndex_mem E).choose

/-- The index restriction function of a small `0`-hypercover. -/
/-
**CategoryTheory.Precoverage.ZeroHypercover.Small.restrictFun** 是 Mathlib 中的一个定义
，位于命名空间 `CategoryTheory.Precoverage.ZeroHypercover.Small`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     {J : Cate
goryTheory.Precoverage C} →       {S : C} →         (E : J.ZeroHypercover S) → [
inst_1 : E.Small] → CategoryTheory.Precoverage.ZeroHypercover.Small.Index E → E.
I₀
参数：E : J.ZeroHypercover S。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Precoverage.ZeroHypercover.Small.exists_restrictIndex_mem
`：∀ {C : Type u} {inst : CategoryTheory.Category.{v, u} C} {J : CategoryTheory.P
recoverage C} {S : C}   (E : J.ZeroHypercover S) [self : E.Sma…

--- 原说明 ---
The index restriction function of a small `0`-hypercover.
-/
noncomputable def Small.restrictFun (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w'} E] :
    Index E → E.I₀ :=
  (Small.exists_restrictIndex_mem E).choose_spec.choose
/-
**CategoryTheory.Precoverage.ZeroHypercover.Small.mem** 是 Mathlib 中的一个引理，位于命名空间 
`CategoryTheory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma Small.mem₀ (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w'} E] :
    (E.restrictIndex <| Small.restrictFun E).presieve₀ ∈ J S :=
  (Small.exists_restrictIndex_mem E).choose_spec.choose_spec
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (E : ZeroHypercover.{w} J S) : ZeroHypercover.Small.{max u v} E where
  exists_restrictIndex_mem := by
    obtain ⟨ι, Y, f, h⟩ := E.presieve₀.exists_eq_ofArrows
    have (Z : C) (g : Z ⟶ S) (hg : Presieve.ofArrows Y f g) :
        ∃ (j : E.I₀) (h : Z = E.X j), g = eqToHom h ≫ E.f j := by
      obtain ⟨j⟩ : E.presieve₀ g := by rwa [h]
      use j, rfl
      simp
    choose j h₁ h₂ using this
    refine ⟨ι, fun i ↦ j _ _ (.mk i), ?_⟩
    convert! E.mem₀
    exact le_antisymm (fun Z g ⟨i⟩ ↦ ⟨_⟩) (h ▸ fun Z g ⟨i⟩ ↦ .mk' i (h₁ _ _ _) (h₂ _ _ _))

/-- Restrict a `w'`-small `0`-hypercover to a `w'`-`0`-hypercover. -/
@[simps toPreZeroHypercover]
noncomputable
/-
**CategoryTheory.Precoverage.ZeroHypercover.restrictIndexOfSmall** 是 Mathlib 中的一
个定义，位于命名空间 `CategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：restrictIndexOfSmall (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w
'} E] : ZeroHypercover.{w'} J S where __
参数：E : ZeroHypercover.{w} J S。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Precoverage.ZeroHypercover.Small.mem₀`：∀ {C : Type u} [in
st : CategoryTheory.Category.{v, u} C] {J : CategoryTheory.Precoverage C} {S : C
}   (E : J.ZeroHypercover S) [inst_1 : E.S…
-/
def restrictIndexOfSmall (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w'} E] :
    ZeroHypercover.{w'} J S where
  __ := E.toPreZeroHypercover.restrictIndex (Small.restrictFun E)
  mem₀ := Small.mem₀ E

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/-
**CategoryTheory.Precoverage.ZeroHypercover.** 是 Mathlib 中的一个实例，位于命名空间 `Category
Theory.Precoverage.ZeroHypercover`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (E : ZeroHypercover.{w} J S) [ZeroHypercover.Small.{w'} E] {T : C} (f : T ⟶ S)
    [IsStableUnderBaseChange J] [∀ (i : E.I₀), HasPullback f (E.f i)] :
    ZeroHypercover.Small.{w'} (E.pullback₁ f) := by
  use Small.Index E, Small.restrictFun E
  have _ (i) : HasPullback f (E.restrictIndexOfSmall.f i) := by dsimp; infer_instance
  exact ((restrictIndexOfSmall.{w'} E).pullback₁ f).mem₀

end ZeroHypercover

/-
**CategoryTheory.Precoverage.mem_iff_exists_zeroHypercover** 是 Mathlib 中的一个引理，位于
命名空间 `CategoryTheory.Precoverage`。
形式化陈述：mem_iff_exists_zeroHypercover {X : C} {R : Presieve X} : R in J X ↔ exists
 (𝒰 : ZeroHypercover.{max u v} J X), R = Presieve.ofArrows 𝒰.X 𝒰.f
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Presieve.exists_eq_ofArrows`：exists_eq_ofArrows (R : Pres
ieve X) : exists (ι : Type (max u₁ v₁)) (Y : ι -> C) (f : forall i, Y i ⟶ X), R 
= .ofArrows Y f
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `CategoryTheory.Precoverage.ZeroHypercover.mem₀`：∀ {C : Type u} [inst : C
ategoryTheory.Category.{v, u} C] {J : CategoryTheory.Precoverage C} {S : C}   (s
elf : J.ZeroHypercover S), self.pres…
-/
lemma mem_iff_exists_zeroHypercover {X : C} {R : Presieve X} :
    R ∈ J X ↔ ∃ (𝒰 : ZeroHypercover.{max u v} J X), R = Presieve.ofArrows 𝒰.X 𝒰.f := by
  refine ⟨fun hR ↦ ?_, fun ⟨𝒰, hR⟩ ↦ hR ▸ 𝒰.mem₀⟩
  obtain ⟨ι, Y, f, rfl⟩ := R.exists_eq_ofArrows
  use ⟨⟨ι, Y, f⟩, hR⟩
/-
**CategoryTheory.Precoverage.le_of_zeroHypercover** 是 Mathlib 中的一个引理，位于命名空间 `Cat
egoryTheory.Precoverage`。
形式化陈述：le_of_zeroHypercover {J K : Precoverage C} (h : forall ⦃X : C⦄ ⦃E : ZeroHy
percover.{max u v} J X⦄, E.presieve₀ in K X) : J <= K
参数：h : forall ⦃X : C⦄ ⦃E : ZeroHypercover.{max u v} J X⦄, E.presieve₀ in K X。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Presieve.exists_eq_preZeroHypercover`：∀ {C : Type u} [ins
t : CategoryTheory.Category.{v, u} C] {S : C} (R : CategoryTheory.Presieve S), ∃
 E, R = E.presieve₀
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
-/
lemma le_of_zeroHypercover {J K : Precoverage C}
    (h : ∀ ⦃X : C⦄ ⦃E : ZeroHypercover.{max u v} J X⦄, E.presieve₀ ∈ K X) :
    J ≤ K := by
  intro X R hR
  obtain ⟨E, rfl⟩ := R.exists_eq_preZeroHypercover
  exact h (E := ⟨E, hR⟩)

/-- A precoverage is `w`-small, if every `0`-hypercover is `w`-small. -/
/-
**CategoryTheory.Precoverage.Small** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTheory.P
recoverage`。
形式化陈述：{C : Type u} → [inst : CategoryTheory.Category.{v, u} C] → CategoryTheory.
Precoverage C → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A precoverage is `w`-small, if every `0`-hypercover is `w`-small.
-/
class Small (J : Precoverage C) : Prop where
  zeroHypercoverSmall : ∀ {S : C} (E : ZeroHypercover.{max u v} J S), ZeroHypercover.Small.{w'} E
/-
**CategoryTheory.Precoverage.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Precover
age`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (K : Precoverage C) : Small.{max u v} K where
  zeroHypercoverSmall := inferInstance
/-
**CategoryTheory.Precoverage.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Precover
age`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance (J : Precoverage C) [Small.{w} J] {S : C} (E : ZeroHypercover.{w'} J S) :
    ZeroHypercover.Small.{w} E := by
  have : ZeroHypercover.Small.{w} (ZeroHypercover.restrictIndexOfSmall.{max u v} E) :=
    Small.zeroHypercoverSmall _
  let E' := ZeroHypercover.restrictIndexOfSmall.{w}
    (ZeroHypercover.restrictIndexOfSmall.{max u v} E)
  use E'.I₀, ZeroHypercover.Small.restrictFun _ ∘ ZeroHypercover.Small.restrictFun _
  exact E'.mem₀
/-
**CategoryTheory.Precoverage.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Precover
age`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance {D : Type*} [Category* D] {F : C ⥤ D} (J : Precoverage D) [Small.{w} J] :
    Small.{w} (J.comap F) where
  zeroHypercoverSmall {X} E := by
    refine ⟨(E.map F le_rfl).restrictIndexOfSmall.I₀, ZeroHypercover.Small.restrictFun _, ?_⟩
    simpa using! (E.map F le_rfl).restrictIndexOfSmall.mem₀

set_option backward.isDefEq.respectTransparency.types false in
/-
**CategoryTheory.Precoverage.Small.inf** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheory
.Precoverage.Small`。
形式化陈述：∀ {C : Type u} [inst : CategoryTheory.Category.{v, u} C] {J K : CategoryTh
eory.Precoverage C} [J.Small],   (∀ ⦃X : C⦄ ⦃R S : CategoryTheory.Presieve X⦄, R
 ≤ S → S ∈ K.coverings X → R ∈ K.coverings X) → (J ⊓ K).Small
参数：∀ ⦃X : C⦄ ⦃R S : CategoryTheory.Presieve X⦄, R ≤ S → S ∈ K.coverings X → R ∈ 
K.coverings X；J ⊓ K。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `inf_le_left`：∀ {α : Type u} [inst : SemilatticeInf α] {a b : α}, a ⊓ b ≤
 a
· 使用定理 `CategoryTheory.Precoverage.instSmallOfSmall`：∀ {C : Type u} [inst : Cate
goryTheory.Category.{v, u} C] (J : CategoryTheory.Precoverage C) [J.Small] {S : 
C}   (E : J.ZeroHypercover S), E.…
· 使用定理 `CategoryTheory.Precoverage.ZeroHypercover.mem₀`：∀ {C : Type u} [inst : C
ategoryTheory.Category.{v, u} C] {J : CategoryTheory.Precoverage C} {S : C}   (s
elf : J.ZeroHypercover S), self.pres…
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `And.right`：∀ {a b : Prop}, a ∧ b → b
-/
lemma Small.inf {J K : Precoverage C} [Small.{w} J]
    (of_le : ∀ ⦃X : C⦄ ⦃R S : Presieve X⦄, R ≤ S → S ∈ K X → R ∈ K X) :
    Small.{w} (J ⊓ K) where
  zeroHypercoverSmall {S} E := by
    refine ⟨(E.weaken (inf_le_left)).restrictIndexOfSmall.I₀,
        ZeroHypercover.Small.restrictFun _, ⟨?_, ?_⟩⟩
    · exact (E.weaken (inf_le_left)).restrictIndexOfSmall.mem₀
    · exact of_le (by simp) E.mem₀.2
/-
**CategoryTheory.Precoverage.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.Precover
age`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance [IsStableUnderBaseChange J] : RespectsIso J where
  of_iso {S E F} e h := by
    refine J.mem_coverings_of_isPullback (fun i ↦ E.f (e.inv.s₀ i)) ?_ (𝟙 S) _ (fun i ↦ ?_) ?_
    · convert! h
      exact Presieve.ofArrows_comp_eq_of_surjective _ (fun i ↦ ⟨e.hom.s₀ i, by simp⟩)
    · exact e.inv.h₀ i
    · intro i
      exact CategoryTheory.IsPullback.of_vert_isIso (by simp)

namespace ZeroHypercover

variable [J.IsStableUnderBaseChange]

/-- If `{Uᵢ}` covers `X`, this is the `0`-hypercover of `X ×[Z] Y` given by `{Uᵢ ×[Z] Y}`. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.pullbackCoverOfLeft** 是 Mathlib 中的一个
定义，位于命名空间 `CategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：pullbackCoverOfLeft {X : C} (E : J.ZeroHypercover X) {Y Z : C} (f : X ⟶ Z)
 (g : Y ⟶ Z) [HasPullback f g] [forall i, HasPullback (E.f i) (pullback.fst f g)
] : J.ZeroHypercover (pullback f g) where __
参数：E : J.ZeroHypercover X；f : X ⟶ Z；g : Y ⟶ Z；E.f i；pullback.fst f g。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `{Uᵢ}` covers `X`, this is the `0`-hypercover of `X ×[Z] Y` given by `{Uᵢ ×[Z
] Y}`.
-/
noncomputable def pullbackCoverOfLeft {X : C} (E : J.ZeroHypercover X) {Y Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ i, HasPullback (E.f i) (pullback.fst f g)] :
    J.ZeroHypercover (pullback f g) where
  __ := E.toPreZeroHypercover.pullbackCoverOfLeft f g
  mem₀ := (E.pullback₁ (pullback.fst f g)).presieve₀_mem_of_iso
    (E.pullbackCoverOfLeftIsoPullback₁ _ _).symm

/-- If `{Uᵢ}` covers `Y`, this is the `0`-hypercover of `X ×[Z] Y` given by `{X ×[Z] Uᵢ}`. -/
@[simps toPreZeroHypercover]
/-
**CategoryTheory.Precoverage.ZeroHypercover.pullbackCoverOfRight** 是 Mathlib 中的一
个定义，位于命名空间 `CategoryTheory.Precoverage.ZeroHypercover`。
形式化陈述：pullbackCoverOfRight {Y : C} (E : J.ZeroHypercover Y) {X Z : C} (f : X ⟶ Z
) (g : Y ⟶ Z) [HasPullback f g] [forall i, HasPullback (E.f i) (pullback.snd f g
)] : J.ZeroHypercover (pullback f g) where __
参数：E : J.ZeroHypercover Y；f : X ⟶ Z；g : Y ⟶ Z；E.f i；pullback.snd f g。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `{Uᵢ}` covers `Y`, this is the `0`-hypercover of `X ×[Z] Y` given by `{X ×[Z]
 Uᵢ}`.
-/
noncomputable def pullbackCoverOfRight {Y : C} (E : J.ZeroHypercover Y) {X Z : C}
    (f : X ⟶ Z) (g : Y ⟶ Z) [HasPullback f g] [∀ i, HasPullback (E.f i) (pullback.snd f g)] :
    J.ZeroHypercover (pullback f g) where
  __ := E.toPreZeroHypercover.pullbackCoverOfRight f g
  mem₀ := (E.pullback₂ (pullback.snd f g)).presieve₀_mem_of_iso
    (E.pullbackCoverOfRightIsoPullback₂ _ _).symm

end ZeroHypercover

end Precoverage

end CategoryTheory

