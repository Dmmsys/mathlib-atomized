...
归纳类型 Relation
  ...
  构造子 (26 个):
    ...
    - trans: 对任意 (x y z) (_ : Relation x y) (_ : Relation y z), Relation x z
    - map: 对任意 (j j' : J) (f : j ⟶ j') (x : F.obj j), Relation (Prequotient.of j' (F.map f x)) (Prequotient.of j x)
    - zero: 对任意 j, Relation (Prequotient.of j 0) zero
    - one: 对任意 j, Relation (Prequotient.of j 1) one
    - neg: 对任意 (j) (x : F.obj j), Relation (Prequotient.of j (-x)) (neg (Prequotient.of j x))
    - add: 对任意 (j) (x y : F.obj j), Relation (Prequotient.of j (x + y)) (add (Prequotient.of j x) (Prequotient.of j y))
    - mul: 对任意 (j) (x y : F.obj j), Relation (Prequotient.of j (x * y)) (mul (Prequotient.of j x) (Prequotient.of j y))
    - neg_1: 对任意 (x x') (_ : Relation x x'), Relation (neg x) (neg x')
    - add_1: 对任意 (x x' y) (_ : Relation x x'), Relation (add x y) (add x' y)
    - add_2: 对任意 (x y y') (_ : Relation y y'), Relation (add x y) (add x y')
    - mul_1: 对任意 (x x' y) (_ : Relation x x'), Relation (mul x y) (mul x' y)
    - mul_2: 对任意 (x y y') (_ : Relation y y'), Relation (mul x y) (mul x y')
    - zero_add: 对任意 x, Relation (add zero x) x
    - add_zero: 对任意 x, Relation (add x zero) x
    - one_mul: 对任意 x, Relation (mul one x) x
    - mul_one: 对任意 x, Relation (mul x one) x
    - neg_add_cancel: 对任意 x, Relation (add (neg x) x) zero
    - add_comm: 对任意 x y, Relation (add x y) (add y x)
    - add_assoc: 对任意 x y z, Relation (add (add x y) z) (add x (add y z))
    - mul_assoc: 对任意 x y z, Relation (mul (mul x y) z) (mul x (mul y z))
    - left_distrib: 对任意 x y z, Relation (mul x (add y z)) (add (mul x y) (mul x z))
    - right_distrib: 对任意 x y z, Relation (mul (add x y) z) (add (mul x z) (mul y z))
    - zero_mul: 对任意 x, Relation (mul zero x) zero
    - mul_zero: 对任意 x, Relation (mul x zero) zero
-/
...
实例 ColimitType.instZero
  签名: : Zero (ColimitType F) where zero
...
中文:
实例 ColimitType.AddGroup
  签名: : AddGroup (ColimitType F) where
  定义体: Quotient.map neg Relation.neg_1
zero_add := Quotient.ind fun _ => Quotient.sound Relation.zero_add _
add_zero := Quotient.ind fun _ => Quotient.sound Relation.add_zero _
neg_add_cancel := Quotient.ind fun _ => Quotient.sound Relation.neg_add_cancel _
...
实例 InhabitedColimitType
  签名: : Inhabited ColimitType F where
  定义体: 0
-/
...
中文:
实例 ColimitType.AddGroupWithOne
  签名: : AddGroupWithOne (ColimitType F)
...
中文:
定理 quot_zero
  结论: Quot.mk Setoid.r zero = (0 : ColimitType F)
  证明: rfl
...
归纳类型 Relation
  ...
  构造子 (27 个):
    ...
    - neg_add_cancel: 对任意 x, Relation (add (neg x) x) zero
    - add_comm: 对任意 x y, Relation (add x y) (add y x)
    - mul_comm: 对任意 x y, Relation (mul x y) (mul y x)
    - add_assoc: 对任意 x y z, Relation (add (add x y) z) (add x (add y z))
    - mul_assoc: 对任意 x y z, Relation (mul (mul x y) z) (mul x (mul y z))
    - left_distrib: 对任意 x y z, Relation (mul x (add y z)) (add (mul x y) (mul x z))
    - right_distrib: 对任意 x y z, Relation (mul (add x y) z) (add (mul x z) (mul y z))
...
实例 hasColimits_commRingCat
  签名: : HasColimits CommRingCat where
  定义体: { has_colimit := fun F =>
        HasColimit.mk
          { cocone := colimitCocone F
            isColimit := colimitIsColimit F } }
