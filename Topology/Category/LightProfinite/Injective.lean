...
/--
lemma `exists_lift_of_finite_of_injective_of_surjective` / 引理 `exists_lift_of_finite_of_injective_of_surjective`

English:
lemma exists_lift_of_finite_of_injective_of_surjective
  statement: {X Y S T : Type*}
  proof: by
...
lemma exists_lift_of_finite_of_injective_of_surjective {X Y S T : Type*}
    ...
    (h_comm : g' ∘ f = f' ∘ g) : exists k : Y -> S, (Continuous k) ∧ (f' ∘ k = g') ∧ (k ∘ f = g) := by
  -- `T` is finite because it admits a surjection from a finite set
  have : Finite T := Finite.of_surjective f' f'_surj
  -- define the closed partition `Z` so `Z i` is the image under `f` of the fiber of `g` at `i`
  let Z : S -> Set Y := fun i => f '' g ⁻¹' {i}
...
/--
lemma `exists_lift_of_finite_of_mono_of_epi` / 引理 `exists_lift_of_finite_of_mono_of_epi`

English:
lemma exists_lift_of_finite_of_mono_of_epi
  statement: {X Y S T : Profinite.{u}} [Finite S]
  proof: by
...
/--
Instance `injective_of_finite` / 实例 `injective_of_finite`

English:
instance injective_of_finite
  signature: (S : Profinite.{u}) [Nonempty S] [Finite S]
  body: by
...
/--
Instance `injective_of_light` / 实例 `injective_of_light`

English:
instance injective_of_light
  signature: (S : LightProfinite.{u}) [Nonempty S]
  body: by
...
/--
Instance `LightProfinite.injective` / 实例 `LightProfinite.injective`

English:
instance LightProfinite.injective
  signature: (S : LightProfinite.{u}) [Nonempty S]
  body: by
...
Depends on / 依赖: Injective, Injective.factors, cat_disch, factors, lightToProfinite, lightToProfinite.map, lightToProfiniteFullyFaithful, lightToProfiniteFullyFaithful.map_injective, lightToProfiniteFullyFaithful.preimage, map_injective, preimage
-/
