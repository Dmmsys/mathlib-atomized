...
/--
lemma `hasSum_mellin` / 引理 `hasSum_mellin`

English:
lemma hasSum_mellin
  statement: {a : ι -> Complex} {p : ι -> Real} {F : Real -> Complex} {s : Complex}
  proof: by
...
-/
lemma hasSum_mellin {a : ι -> Complex} {p : ι -> Real} {F : Real -> Complex} {s : Complex}
    (hp : forall i, a i = 0 ∨ 0 < p i) (hs : 0 < s.re)
    (hF : forall t in Ioi 0, HasSum (fun i => a i * rexp (-p i * t)) (F t))
...
/--
lemma `hasSum_mellin_pi_mul` / 引理 `hasSum_mellin_pi_mul`

English:
lemma hasSum_mellin_pi_mul
  statement: {a : ι -> Complex} {q : ι -> Real} {F : Real -> Complex} {s : Complex}
  proof: by
...
-/
lemma hasSum_mellin_pi_mul {a : ι -> Complex} {q : ι -> Real} {F : Real -> Complex} {s : Complex}
    (hq : forall i, a i = 0 ∨ 0 < q i) (hs : 0 < s.re)
    (hF : forall t in Ioi 0, HasSum (fun i => a i * rexp (-π * q i * t)) (F t))
...
-/
lemma hasSum_mellin_pi_mul₀ {a : ι -> Complex} {p : ι -> Real} {F : Real -> Complex} {s : Complex}
    (hp : forall i, 0 <= p i) (hs : 0 < s.re)
    (hF : forall t in Ioi 0, HasSum (fun i => if p i = 0 then 0 else a i * rexp (-π * p i * t)) (F t))
    (h_sum : Summable fun i => ‖a i‖ / (p i) ^ s.re) :
...
/--
lemma `hasSum_mellin_pi_mul_sq` / 引理 `hasSum_mellin_pi_mul_sq`

English:
lemma hasSum_mellin_pi_mul_sq
  statement: {a : ι -> Complex} {r : ι -> Real} {F : Real -> Complex} {s : Complex} (hs : 0 < s.re)
  proof: by
...
-/
lemma hasSum_mellin_pi_mul_sq {a : ι -> Complex} {r : ι -> Real} {F : Real -> Complex} {s : Complex} (hs : 0 < s.re)
    (hF : forall t in Ioi 0, HasSum (fun i => if r i = 0 then 0 else a i * rexp (-π * r i ^ 2 * t)) (F t))
    (h_sum : Summable fun i => ‖a i‖ / |r i| ^ s.re) :
    HasSum (fun i => GammaReal s * a i / |r i| ^ s) (mellin F (s / 2)) := by
  have hs' : 0 < (s / 2).re := by rw [div_ofNat_re]; positivity
  simp_rw [← sq_eq_zero_iff (a := r _)] at hF
  convert! hasSum_mellin_pi_mul₀ (fun i => sq_nonneg (r i)) hs' hF ?_ using 3 with i
  · rw [← neg_div, GammaReal_def]
...
-/
lemma hasSum_mellin_pi_mul_sq' {a : ι -> Complex} {r : ι -> Real} {F : Real -> Complex} {s : Complex} (hs : 0 < s.re)
    (hF : forall t in Ioi 0, HasSum (fun i => a i * r i * rexp (-π * r i ^ 2 * t)) (F t))
    (h_sum : Summable fun i => ‖a i‖ / |r i| ^ s.re) :
    HasSum (fun i => GammaReal (s + 1) * a i * SignType.sign (r i) / |r i| ^ s)
    (mellin F ((s + 1) / 2)) := by
  have hs₁ : s != 0 := fun h => lt_irrefl _ (zero_re ▸ h ▸ hs)
  have hs₂ : 0 < (s + 1).re := by rw [add_re, one_re]; positivity
  have hs₃ : s + 1 != 0 := fun h => lt_irrefl _ (zero_re ▸ h ▸ hs₂)
