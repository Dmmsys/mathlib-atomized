...
-/
/-
**Mathlib.Linter.CommandStart.endPos** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`CommandStart.endPos stx` returns the position up until the `whitespace` linter 
checks the
formatting.
This is every declaration until the type-specification, if there is one, or the 
value,
as well as all `variable` commands.
-/
...
/-
**Mathlib.Linter.FormatError** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Linter`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
structure FormatError where
  ...
  deriving Inhabited
/-
**Mathlib.Linter.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Linter`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Linter.mkFormatError** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：mkFormatError (ls ms : String) (msg : String) (length : Nat
参数：ls ms : String；msg : String。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
* `ls` to be a "user-typed" string;
* `ms` to be a "pretty-printed" string;
* `msg` to be a custom error message, such as `extra space` or `remove line brea
k`;
* `length` (optional with default `1`), how many characters the error spans.

In particular, it extracts the position information within the string, both as n
umber of characters
and as `String.Pos`.
-/
...
-/
/-
**Mathlib.Linter.pushFormatError** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：pushFormatError (fs : Array FormatError) (f : FormatError) : Array FormatE
rror
参数：fs : Array FormatError；f : FormatError。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Add a new `FormatError` `f` to the array `fs`, trying, as much as possible, to m
erge the new
`FormatError` with the last entry of `fs`.
-/
...
partial
/-
**Mathlib.Linter.parallelScanAux** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：parallelScanAux (as : Array FormatError) (L M : String.Slice) : Array Form
atError
参数：as : Array FormatError；L M : String.Slice。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.parallelScan** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：parallelScan (src fmt : String) : Array FormatError
参数：src fmt : String。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Linter.Style.Whitespace.unlintedNodes** 是 Mathlib 中的一个缩写定义，位于命名空间 `Mat
hlib.Linter.Style.Whitespace`。
形式化陈述：unlintedNodes
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`unlintedNodes` contains the `SyntaxNodeKind`s for which there is no clear forma
tting preference:
if they appear in surface syntax, the linter will ignore formatting.

Currently, the unlined nodes are mostly related to `Subtype`, `Set` and `Finset`
 notation and
list notation.
-/
...
-/
/-
**Mathlib.Linter.Style.Whitespace.getUnlintedRanges** 是 Mathlib 中的一个定义，位于命名空间 `M
athlib.Linter.Style.Whitespace`。
形式化陈述：getUnlintedRanges (a : Array SyntaxNodeKind) : Std.HashSet Lean.Syntax.Ran
ge -> Syntax -> Std.HashSet Lean.Syntax.Range | curr, s@(.node _ kind args) => l
et new
参数：a : Array SyntaxNodeKind。
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given an array `a` of `SyntaxNodeKind`s, we accumulate the ranges of the syntax 
nodes of the
input syntax whose kind is in `a`.

The linter uses this information to avoid emitting a warning for nodes with kind
 contained in
`unlintedNodes`.
-/
...
-/
/-
**Mathlib.Linter.Style.Whitespace.isOutside** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.Style.Whitespace`。
形式化陈述：isOutside (rgs : Std.HashSet Lean.Syntax.Range) (rg : Lean.Syntax.Range) :
 Bool
参数：rgs : Std.HashSet Lean.Syntax.Range；rg : Lean.Syntax.Range。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a `HashSet` of `Lean.Syntax.Range`s `rgs` and a further `Lean.Syntax.Range
` `rg`,
`isOutside rgs rg` returns `false` if and only if `rgs` contains a range that co
mpletely contains
`rg`.
...
/-
**Mathlib.Linter.Style.Whitespace.whitespaceLinter** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Linter.Style.Whitespace`。
形式化陈述：whitespaceLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
