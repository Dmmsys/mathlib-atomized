...
/--
Definition of `cones` / `cones` 的定义

English:
definition cones
  signature: : Cᵒᵖ ⥤ Type (max u₁ v₃)
  body: (const J).op ⋙ yoneda.obj F
...
@[implicit_reducible, simps! obj map]
/--
Definition of `cocones` / `cocones` 的定义

English:
definition cocones
  signature: : C ⥤ Type (max u₁ v₃)
  body: const J ⋙ coyoneda.obj (op F)
...
English:
definition cones
  signature: : (J ⥤ C) ⥤ Cᵒᵖ ⥤ Type (max u₁ v₃) where
  body: Functor.cones
...
Depends on / 依赖: Functor, Functor.cones
-/
...
English:
definition cocones
  signature: : (J ⥤ C)ᵒᵖ ⥤ C ⥤ Type (max u₁ v₃) where
  body: Functor.cocones (unop F)
...
Depends on / 依赖: Functor, Functor.cocones, cocones
-/
...
@[to_dual]
/--
Instance `inhabitedCone` / 实例 `inhabitedCone`

English:
instance inhabitedCone
  signature: (F : Discrete PUnit ⥤ C)
  body: ⟨{ pt := F.obj ⟨⟨⟩⟩
      π := { app := fun ⟨⟨⟩⟩ => 𝟙 _
             naturality := by
              intro X Y f
...
Depends on / 依赖: F.obj, naturality
-/
...
@[to_dual (attr := reassoc), elementwise]
/--
theorem `Cone.w` / 定理 `Cone.w`

English:
theorem Cone.w
  given: {F : J ⥤ C} (c : Cone F) {j j' : J} (f : j ⟶ j')
  proof: by
...
#adaptation_note
/--
theorem `Cocone.w_apply.` / 定理 `Cocone.w_apply.`

English:
theorem Cocone.w_apply.{uF,
  given: w} {F
  statement: J ⥤ C} (c : Cocone F) {j j' : J} (f : j' ⟶ j)
  proof: by
  simp
...
@[simps]
/--
Definition of `extensions` / `extensions` 的定义

English:
definition extensions
  signature: (c : Cone F)
  body: ↾fun f => (const J).map f.down ≫ c.π
...
English:
definition extend
  signature: (c : Cone F) {X : C} (f : X ⟶ c.pt)
  body: X
  π := (const J).map f ≫ c.π
...
definition extensions
  ...
  body: ↾fun f => c.ι ≫ (const J).map f.down
...
/--
Definition of `ConeMorphism` / `ConeMorphism` 的定义

English:
structure ConeMorphism
  parameters: (A B : Cone F)
  axioms and operations (2):
    - hom : A.pt ⟶ B.pt
...
English:
structure CoconeMorphism
  parameters: (A B : Cocone F)
  axioms and operations (2):
    - hom : A.pt ⟶ B.pt
...
@[to_dual]
/--
Instance `inhabitedConeMorphism` / 实例 `inhabitedConeMorphism`

English:
instance inhabitedConeMorphism
  signature: (A : Cone F)
  body: ⟨{ hom := 𝟙 _ }⟩
...
/--
Instance `Cone.category` / 实例 `Cone.category`

English:
instance Cone.category
  signature: : Category (Cone F) where
  body: ConeMorphism A B
...
/--
theorem `ConeMorphism.ext` / 定理 `ConeMorphism.ext`

English:
theorem ConeMorphism.ext
  given: {c c' : Cone F} (f g : c ⟶ c') (w : f.hom = g.hom)
  statement: f = g
  proof: by
  cases f
  cases g
...
@[to_dual (attr := reassoc (attr := simp))]
/--
lemma `ConeMorphism.hom_inv_id` / 引理 `ConeMorphism.hom_inv_id`

English:
lemma ConeMorphism.hom_inv_id
  given: {c d : Cone F} (f : c ≅ d)
  statement: f.hom.hom ≫ f.inv.hom = 𝟙 _
  proof: by
...
Depends on / 依赖: Cone.category_comp_hom, category_comp_hom
-/
...
@[to_dual (attr := reassoc (attr := simp))]
/--
lemma `ConeMorphism.inv_hom_id` / 引理 `ConeMorphism.inv_hom_id`

English:
lemma ConeMorphism.inv_hom_id
  given: {c d : Cone F} (f : c ≅ d)
  statement: f.inv.hom ≫ f.hom.hom = 𝟙 _
  proof: by
  simp [← Cone.category_comp_hom]
...
@[to_dual (attr := reassoc (attr := simp))]
/--
lemma `ConeMorphism.map_w` / 引理 `ConeMorphism.map_w`

English:
lemma ConeMorphism.map_w
  given: {c c' : Cone F} (f : c ⟶ c') (G : C ⥤ D) (j : J)
  proof: by
  simp [← map_comp]
...
English:
definition ext
  signature: {c c' : Cone F} (φ : c.pt ≅ c'.pt)
  body: { hom := φ.hom }
  inv :=
    { hom := φ.inv
...
Depends on / 依赖: cat_disch, inv_comp_eq, inv_comp_eq.mpr
-/
def ext {c c' : Cone F} (φ : c.pt ≅ c'.pt)
    (w : forall j, c.π.app j = φ.hom ≫ c'.π.app j := by cat_disch) : c ≅ c' where
  hom := { hom := φ.hom }
  inv :=
    { hom := φ.inv
      w := fun j => φ.inv_comp_eq.mpr (w j) }
...
/--
Definition of `extInv` / `extInv` 的定义

English:
definition extInv
  signature: {c c' : Cone F} (φ : c.pt ≅ c'.pt)
  body: ext φ fun j => (Iso.inv_comp_eq φ).mp (w j)
...
def extInv {c c' : Cone F} (φ : c.pt ≅ c'.pt)
    (w : forall j, φ.inv ≫ c.π.app j = c'.π.app j := by cat_disch) : c ≅ c' :=
...
-/]
/--
theorem `cone_iso_of_hom_iso` / 定理 `cone_iso_of_hom_iso`

English:
theorem cone_iso_of_hom_iso
  given: {K : J ⥤ C} {c d : Cone K} (f : c ⟶ d) [i : IsIso f.hom]
  statement: IsIso f
  proof: ⟨⟨{ hom := inv f.hom
...
Depends on / 依赖: cat_disch, f.hom, inv_comp_eq
-/
...
/--
Definition of `extendHom` / `extendHom` 的定义

English:
definition extendHom
  signature: (s : Cone F) {X : C} (f : X ⟶ s.pt)
  body: f
...
/--
Definition of `extendId` / `extendId` 的定义
...
/--
Definition of `extendComp` / `extendComp` 的定义

English:
definition extendComp
  signature: (s : Cone F) {X Y : C} (f : X ⟶ Y) (g : Y ⟶ s.pt)
  body: ext (Iso.refl _)
...
definition extendIso
  signature: (s : Cone F) {X : C} (f : s.pt ≅ X)
  body: { hom := f.hom }
  inv := { hom := f.inv }
...
/--
Definition of `postcomposeComp` / `postcomposeComp` 的定义

English:
definition postcomposeComp
  signature: {G H : J ⥤ C} (α : F ⟶ G) (β : G ⟶ H)
  body: NatIso.ofComponents fun s => ext (Iso.refl _)

中文:
定义 postcomposeComp
  签名: {G H : J ⥤ C} (α : F ⟶ G) (β : G ⟶ H)
...
/--
Definition of `postcomposeId` / `postcomposeId` 的定义

English:
definition postcomposeId
  signature: : postcompose (𝟙 F) ≅ 𝟭 (Cone F)
  body: NatIso.ofComponents fun s => ext (Iso.refl _)
...
-/]
/--
Definition of `postcomposeEquivalence` / `postcomposeEquivalence` 的定义

English:
definition postcomposeEquivalence
  signature: {G : J ⥤ C} (α : F ≅ G)
  body: postcompose α.hom
...
中文:
定义 postcomposeEquivalence
  签名: {G : J ⥤ C} (α : F ≅ G)
  定义体: postcompose α.hom
  inverse := postcompose α.inv
  unitIso := NatIso.ofComponents fun s => ext (Iso.refl _)
  counitIso := NatIso.ofComponents fun s => ext (Iso.refl _)

Depends on / 依赖: postcompose
-/
...
Depends on / 依赖: c.whisker, whisker
-/
...
-/]
/--
Definition of `whiskeringEquivalence` / `whiskeringEquivalence` 的定义

English:
definition whiskeringEquivalence
  signature: (e : K ≌ J)
  body: whiskering e.functor
...
-/]
/--
Definition of `equivalenceOfReindexing` / `equivalenceOfReindexing` 的定义

English:
definition equivalenceOfReindexing
  signature: {G : K ⥤ C} (e : K ≌ J) (α : e.functor ⋙ F ≅ G)
  body: (whiskeringEquivalence e).trans (postcomposeEquivalence α)

中文:
定义 equivalenceOfReindexing
  签名: {G : K ⥤ C} (e : K ≌ J) (α : e.functor ⋙ F ≅ G)
  定义体: (whiskeringEquivalence e).trans (postcomposeEquivalence α)

Depends on / 依赖: postcomposeEquivalence, whiskeringEquivalence
-/
...
definition functoriality
  signature: : Cone F ⥤ Cone (F ⋙ G) where
  body: { pt := G.obj A.pt
      π :=
...
Depends on / 依赖: A.pt, ConeMorphism, ConeMorphism.map_w, G.map, G.map_comp, G.obj, f.hom, map_comp, map_w, naturality
-/
...
/--
Definition of `functorialityCompFunctoriality` / `functorialityCompFunctoriality` 的定义

English:
definition functorialityCompFunctoriality
  signature: (H : D ⥤ E)
...
中文:
定义 functorialityCompFunctoriality
  签名: (H : D ⥤ E)
...
instance functoriality_faithful
  signature: [G.Faithful]
  body: ConeMorphism.ext f g G.map_injective congr_arg ConeMorphism.hom h
...
-/]
/--
Definition of `functorialityEquivalence` / `functorialityEquivalence` 的定义

English:
definition functorialityEquivalence
  signature: (e : C ≌ D)
  body: let f : (F ⋙ e.functor) ⋙ e.inverse ≅ F :=
...
定义 functorialityEquivalence
  签名: (e : C ≌ D)
  定义体: let f : (F ⋙ e.functor) ⋙ e.inverse ≅ F :=
    Functor.associator _ _ _ ≪≫ isoWhiskerLeft _ e.unitIso.symm ≪≫ Functor.rightUnitor _
  { functor := functoriality F e.functor
    inverse := functoriality (F ⋙ e.functor) e.inverse ⋙ (postcomposeEquivalence f).functor
    unitIso := NatIso.ofComponents 
...
-/]
/--
Instance `reflects_cone_isomorphism` / 实例 `reflects_cone_isomorphism`

English:
instance reflects_cone_isomorphism
  signature: (F : C ⥤ D) [F.ReflectsIsomorphisms] (K : J ⥤ C)
  body: by
  constructor
  intro A B f _
...
Depends on / 依赖: F.map, ReflectsIsomorphisms, ReflectsIsomorphisms.reflects, cone_iso_of_hom_iso, f.hom, forget, functoriality, map_isIso, reflects
-/
...
/--
Definition of `mapCone` / `mapCone` 的定义

English:
definition mapCone
  signature: (c : Cone F)
  body: (Cone.functoriality F H).obj c
...
/--
Definition of `mapConeMapCone` / `mapConeMapCone` 的定义

English:
definition mapConeMapCone
  signature: {F : J ⥤ C} {H : C ⥤ D} {H' : D ⥤ E} (c : Cone F)
...
functorially. -/]
/--
Definition of `mapConeMorphism` / `mapConeMorphism` 的定义

English:
definition mapConeMorphism
  signature: {c c' : Cone F} (f : c ⟶ c')
  body: (Cone.functoriality F H).map f
...
for `F ⋙ H`. -/]
/--
Definition of `mapConeInv` / `mapConeInv` 的定义

English:
definition mapConeInv
  signature: [IsEquivalence H] (c : Cone (F ⋙ H))
  body: (Limits.Cone.functorialityEquivalence F (asEquivalence H)).inverse.obj c
...
/--
Definition of `mapConeMapConeInv` / `mapConeMapConeInv` 的定义

English:
definition mapConeMapConeInv
  signature: {F : J ⥤ D} (H : D ⥤ C) [IsEquivalence H]
  body: (Limits.Cone.functorialityEquivalence F (asEquivalence H)).counitIso.app c
...
/--
Definition of `mapConeInvMapCone` / `mapConeInvMapCone` 的定义

English:
definition mapConeInvMapCone
  signature: {F : J ⥤ D} (H : D ⥤ C) [IsEquivalence H] (c : Cone F)
  body: (Limits.Cone.functorialityEquivalence F (asEquivalence H)).unitIso.symm.app c
...
/--
Definition of `functorialityCompPostcompose` / `functorialityCompPostcompose` 的定义

English:
definition functorialityCompPostcompose
  signature: {H H' : C ⥤ D} (α : H ≅ H')
  body: NatIso.ofComponents fun c => Cone.ext (α.app _)

中文:
定义 functorialityCompPostcompose
  签名: {H H' : C ⥤ D} (α : H ≅ H')
  定义体: NatIso.ofComponents fun c => Cone.ext (α.app _)

Depends on / 依赖: Cone.ext, NatIso, NatIso.ofComponents, ofComponents
-/
...
-/]
/--
Definition of `postcomposeWhiskerLeftMapCone` / `postcomposeWhiskerLeftMapCone` 的定义

English:
definition postcomposeWhiskerLeftMapCone
  signature: {H H' : C ⥤ D} (α : H ≅ H') (c : Cone F)
  body: (functorialityCompPostcompose α).app c
...
-/]
/--
Definition of `mapConePostcompose` / `mapConePostcompose` 的定义

English:
definition mapConePostcompose
  signature: {α : F ⟶ G} {c}
  body: Cone.ext (Iso.refl _)

中文:
定义 mapConePostcompose
  签名: {α : F ⟶ G} {c}
  定义体: Cone.ext (Iso.refl _)
...
/--
Definition of `mapConePostcomposeEquivalenceFunctor` / `mapConePostcomposeEquivalenceFunctor` 的定义

English:
definition mapConePostcomposeEquivalenceFunctor
  signature: {α : F ≅ G} {c}
  body: Cone.ext (Iso.refl _)

中文:
定义 mapConePostcomposeEquivalenceFunctor
  签名: {α : F ≅ G} {c}
  定义体: Cone.ext (Iso.refl _)
...
/--
Definition of `mapConeWhisker` / `mapConeWhisker` 的定义

English:
definition mapConeWhisker
  signature: {E : K ⥤ J} {c : Cone F}
  body: Cone.ext (Iso.refl _)
...
/--
Definition of `Cone.op` / `Cone.op` 的定义

English:
definition Cone.op
  signature: (c : Cone F)
  body: Opposite.op c.pt
  ι := NatTrans.op c.π
...
Depends on / 依赖: Opposite, Opposite.op, c.pt
-/
...
/--
Definition of `Cone.unop` / `Cone.unop` 的定义

English:
definition Cone.unop
  signature: (c : Cone F.op)
  body: Opposite.unop c.pt
...
Depends on / 依赖: Opposite, Opposite.unop, c.pt
-/
...
-/]
/--
Definition of `coconeEquivalenceOpConeOp` / `coconeEquivalenceOpConeOp` 的定义

English:
definition coconeEquivalenceOpConeOp
  signature: : Cocone F ≌ (Cone F.op)ᵒᵖ where
  body: { obj := fun c => op (Cocone.op c)
      map := fun {X} {Y} f =>
        Quiver.Hom.op
          { hom := f.hom.op
            w := fun j => by
...
/--
Definition of `coneOpEquiv` / `coneOpEquiv` 的定义

English:
definition coneOpEquiv
  signature: {F : J ⥤ C}
  body: c.unop.op
...
/--
Definition of `coneOfCoconeLeftOp` / `coneOfCoconeLeftOp` 的定义

English:
definition coneOfCoconeLeftOp
  signature: (c : Cocone F.leftOp)
  body: op c.pt
...
/--
Definition of `coconeLeftOpOfCone` / `coconeLeftOpOfCone` 的定义

English:
definition coconeLeftOpOfCone
  signature: (c : Cone F)
  body: unop c.pt
...
/--
Definition of `coconeLeftOpOfConeEquiv` / `coconeLeftOpOfConeEquiv` 的定义

English:
definition coconeLeftOpOfConeEquiv
  signature: {F : J ⥤ Cᵒᵖ}
  body: coconeLeftOpOfCone c.unop
...
中文:
定义 coconeLeftOpOfConeEquiv
  签名: {F : J ⥤ Cᵒᵖ}
  定义体: coconeLeftOpOfCone c.unop
  functor.map f := { hom := f.unop.hom.unop, w j := congr($(f.unop.w j.unop).unop) }
inverse.obj c := .op coneOfCoconeLeftOp c
  inverse.map f := ⟨{ hom := f.hom.op, w j := congr($(f.w (.op j)).op) }⟩
  unitIso := Iso.refl _
...
Depends on / 依赖: c.unop, coconeLeftOpOfCone
-/
...
/--
Definition of `coneOfCoconeRightOp` / `coneOfCoconeRightOp` 的定义

English:
definition coneOfCoconeRightOp
  signature: (c : Cocone F.rightOp)
  body: unop c.pt
...
/--
Definition of `coconeRightOpOfCone` / `coconeRightOpOfCone` 的定义

English:
definition coconeRightOpOfCone
  signature: (c : Cone F)
  body: op c.pt
...
/--
Definition of `coconeRightOpOfConeEquiv` / `coconeRightOpOfConeEquiv` 的定义

English:
definition coconeRightOpOfConeEquiv
  signature: {F : Jᵒᵖ ⥤ C}
  body: coconeRightOpOfCone c.unop
...
中文:
定义 coconeRightOpOfConeEquiv
  签名: {F : Jᵒᵖ ⥤ C}
  定义体: coconeRightOpOfCone c.unop
  functor.map f := { hom := f.unop.hom.op, w j := congr($(f.unop.w (.op j)).op) }
inverse.obj c := .op coneOfCoconeRightOp c
  inverse.map f := ⟨{ hom := f.hom.unop, w j := congr($(f.w j.unop).unop) }⟩
  unitIso := Iso.refl _
...
Depends on / 依赖: c.unop, coconeRightOpOfCone
-/
...
/--
Definition of `coneOfCoconeUnop` / `coneOfCoconeUnop` 的定义

English:
definition coneOfCoconeUnop
  signature: (c : Cocone F.unop)
  body: op c.pt
...
/--
Definition of `coconeUnopOfCone` / `coconeUnopOfCone` 的定义

English:
definition coconeUnopOfCone
  signature: (c : Cone F)
  body: unop c.pt
  ι := NatTrans.unop c.π
...
/--
Definition of `coconeUnopOfConeEquiv` / `coconeUnopOfConeEquiv` 的定义

English:
definition coconeUnopOfConeEquiv
  signature: {F : Jᵒᵖ ⥤ Cᵒᵖ}
  body: coconeUnopOfCone c.unop
...
中文:
定义 coconeUnopOfConeEquiv
  签名: {F : Jᵒᵖ ⥤ Cᵒᵖ}
  定义体: coconeUnopOfCone c.unop
  functor.map f := { hom := f.unop.hom.unop, w j := congr($(f.unop.w (.op j)).unop) }
inverse.obj c := .op coneOfCoconeUnop c
  inverse.map f := ⟨{ hom := f.hom.op, w j := congr($(f.w j.unop).op) }⟩
  unitIso := Iso.refl _
...
Depends on / 依赖: c.unop, coconeUnopOfCone
-/
...
/--
Definition of `mapConeOp` / `mapConeOp` 的定义

English:
definition mapConeOp
  signature: (t : Cone F)
