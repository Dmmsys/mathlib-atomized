...
/--
theorem `charP_zero_or_prime_power` / 定理 `charP_zero_or_prime_power`

English:
theorem charP_zero_or_prime_power
  statement: (R : Type*) [CommRing R] [IsLocalRing R] (q : Nat)
  proof: by
  ...
  rcases CharP.char_is_prime_or_zero K r w
...
定理 charP_zero_or_prime_power
  ...
  rcases CharP.char_is_prime_or_zero K r w
-/
theorem charP_zero_or_prime_power (R : Type*) [CommRing R] [IsLocalRing R] (q : Nat)
  ...
  · let a := q / r ^ n
    ...
    have rn_cast_zero : ↑(r ^ n) = (0 : R) := by
      rw [← one_mul (↑(r ^ n) : R)]; rw [← a_unit.val_inv_mul]; rw [mul_assoc]; rw [← Nat.cast_mul]; rw [← q_eq_a_mul_rn]; rw [CharP.cast_eq_zero R q]; rw [mul_zero]
