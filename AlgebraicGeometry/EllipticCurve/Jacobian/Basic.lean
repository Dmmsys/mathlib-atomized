...
/--
Definition of `toJacobian` / `toJacobian` 的定义

English:
abbreviation toJacobian
...
lemma smul_fin3
  given: (P : Fin 3 -> R) (u : R)
  statement: u • P = ![u ^ 2 * P x, u ^ 3 * P y, u * P z]
  proof: rfl
...
Depends on / 依赖: comp_fin3, fin_cases, map_simp, smul_fin3
-/
...
Depends on / 依赖: Quotient, Quotient.eq.mpr, smul_equiv
-/
...
Depends on / 依赖: Units.smul_def, smul_def, smul_fin3_ext
-/
...
lemma not_equiv_of_Y_ne
  given: {P Q : Fin 3 -> R} (hy : P y * Q z ^ 3 != Q y * P z ^ 3)
  statement: ¬P ≈ Q
...
Depends on / 依赖: Units.mk0, Units.smul_def, Units.val_div_eq_div_val, Units.val_mk0, div_pow, fin3_def, mul_comm, mul_div, pow_ne_zero, smul_def, smul_fin3, val_div_eq_div_val, val_mk0
-/
...
Depends on / 依赖: div_self, equiv_of_X_eq_of_Y_eq, linear_combination, matrix_simp, one_ne_zero, pow_ne_zero
-/
...
lemma X_eq_iff
  ...
  proof: (div_eq_div_iff (pow_ne_zero 2 hPz) (pow_ne_zero 2 hQz)).symm
...
lemma Y_eq_iff
  given: {P Q : Fin 3 -> F} (hPz : P z != 0) (hQz : Q z != 0)
  proof: (div_eq_div_iff (pow_ne_zero 3 hPz) (pow_ne_zero 3 hQz)).symm
...
definition polynomial
  ...
  body: Y ^ 2 + C W'.a₁ * X * Y * Z + C W'.a₃ * Y * Z ^ 3
...
lemma eval_polynomial_of_Z_ne_zero
  given: {P : Fin 3 -> F} (hPz : P z != 0)
  statement: eval P W.polynomial / P z ^ 6 =
  proof: by
...
Depends on / 依赖: Equation, eval_polynomial
-/
...
Depends on / 依赖: OfNat.ofNat_ne_zero, add_zero, equation_iff, mul_zero, ofNat_ne_zero, split_ifs, sub_eq_zero, zero_comp, zero_pow
-/
...
Depends on / 依赖: Before, Mathlib, adaptation_note, canonicalizer, closed, directed, equation_of_Z_eq_zero, fin3_def_ext, github, github.com, leanprover, minimization, normalizer, one_pow, original, problem, replacing, whether
-/
...
Depends on / 依赖: Affine, Affine.equation_iff, comp_id, eqToHom_refl, equation_iff, fin3_def_ext, mul_one, one_pow
-/
...
lemma polynomialY_eq
  statement: W'.polynomialY = C 2 * Y + C W'.a₁ * X * Z + C W'.a₃ * Z ^ 3
  proof: by
...
中文:
引理 polynomialY_eq
  结论: W'.polynomialY = C 2 * Y + C W'.a₁ * X * Z + C W'.a₃ * Z ^ 3
  证明: by
  rw [polynomialY]; rw [polynomial]
  pderiv_simp
  ring1
...
lemma polynomialZ_eq
  statement: W'.polynomialZ = C W'.a₁ * X * Y + C (3 * W'.a₃) * Y * Z ^ 2 -
  proof: by
...
中文:
引理 polynomialZ_eq
  结论: W'.polynomialZ = C W'.a₁ * X * Y + C (3 * W'.a₃) * Y * Z ^ 2 -
  证明: by
  rw [polynomialZ]; rw [polynomial]
  pderiv_simp
  ring1
...
theorem polynomial_relation
  given: (P : Fin 3 -> R)
  statement: 6 * eval P W'.polynomial =
  proof: by
...
Depends on / 依赖: OfNat.ofNat_ne_zero, add_zero, mul_zero, neg_ne_zero, nonsingular_iff, ofNat_ne_zero, sub_zero, zero_pow, zero_sub
-/
...
Depends on / 依赖: equation_zero, fin3_def_ext, linear_combination, nonsingular_of_Z_eq_zero, not_and_or, one_ne_zero, true_and
-/
...
/--
lemma `X_ne_zero_of_Z_eq_zero` / 引理 `X_ne_zero_of_Z_eq_zero`

English:
lemma X_ne_zero_of_Z_eq_zero
...
Depends on / 依赖: OfNat.ofNat_ne_zero, eq_self, equation_of_Z_eq_zero, false_or, hP.left, mul_zero, ne_self_iff_false, nonsingular_of_Z_eq_zero, ofNat_ne_zero, or_false, pow_eq_zero_iff, true_and, two_ne_zero, zero_mul, zero_pow
-/
...
/--
lemma `isUnit_X_of_Z_eq_zero` / 引理 `isUnit_X_of_Z_eq_zero`

English:
lemma isUnit_X_of_Z_eq_zero
  given: {P : Fin 3 -> F} (hP : W.Nonsingular P) (hPz : P z = 0)
  statement: IsUnit (P x)
  proof: (X_ne_zero_of_Z_eq_zero hP hPz).isUnit
...
lemma Y_ne_zero_of_Z_eq_zero
  ...
  intro hPy
  rw [nonsingular_of_Z_eq_zero hPz]; rw [equation_of_Z_eq_zero hPz]; rw [hPy]; rw [zero_pow two_ne_zero] at hP
...
Depends on / 依赖: X_ne_zero_of_Z_eq_zero, eq_zero_of_pow_eq_zero, equation_of_Z_eq_zero, hP.left.symm, nonsingular_of_Z_eq_zero, two_ne_zero, zero_pow
-/
...
lemma nonsingularLift_zero
  given: [Nontrivial R]
  statement: W'.NonsingularLift ⟦![1, 1, 0]⟧
...
Depends on / 依赖: nonsingular_some
-/
...
Depends on / 依赖: Equation, eval_map, map_eq_zero_iff, map_polynomial
-/
