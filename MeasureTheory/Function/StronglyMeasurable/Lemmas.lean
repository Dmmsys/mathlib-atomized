...
@[fun_prop]
/--
lemma `aestronglyMeasurable_dirac` / 引理 `aestronglyMeasurable_dirac`

English:
lemma aestronglyMeasurable_dirac
  given: [MeasurableSingletonClass α] {a : α} {f : α -> β}
  proof: ⟨fun _ => f a, stronglyMeasurable_const, ae_eq_dirac f⟩
...
/--
theorem `MeasureTheory.AEStronglyMeasurable.comp_measurePreserving` / 定理 `MeasureTheory.AEStronglyMeasurable.comp_measurePreserving`
...
/--
theorem `MeasureTheory.MeasurePreserving.aestronglyMeasurable_comp_iff` / 定理 `MeasureTheory.MeasurePreserving.aestronglyMeasurable_comp_iff`

English:
theorem MeasureTheory.MeasurePreserving.aestronglyMeasurable_comp_iff
  statement: {β : Type*}
  proof: by
...
Depends on / 依赖: aestronglyMeasurable_map_iff, hf.map_eq, map_eq
-/
...
/--
theorem `aestronglyMeasurable_smul_const_iff` / 定理 `aestronglyMeasurable_smul_const_iff`

English:
theorem aestronglyMeasurable_smul_const_iff
  given: {f : α -> 𝕜} {c : E} (hc : c != 0)
  proof: (isClosedEmbedding_smul_left hc).isEmbedding.aestronglyMeasurable_comp_iff

中文:
定理 aestronglyMeasurable_smul_const_iff
  条件: {f : α -> 𝕜} {c : E} (hc : c != 0)
  证明: (isClosedEmbedding_smul_left hc).isEmbedding.aestronglyMeasurable_comp_iff

Depends on / 依赖: aestronglyMeasurable_comp_iff, isClosedEmbedding_smul_left, isEmbedding, isEmbedding.aestronglyMeasurable_comp_iff
-/
...
/--
theorem `StronglyMeasurable.apply_continuousLinearMap` / 定理 `StronglyMeasurable.apply_continuousLinearMap`

English:
theorem StronglyMeasurable.apply_continuousLinearMap
  proof: (ContinuousLinearMap.apply 𝕜 E v).continuous.comp_stronglyMeasurable hφ

@[fun_prop]

中文:
定理 StronglyMeasurable.apply_continuousLinearMap
  证明: (ContinuousLinearMap.apply 𝕜 E v).continuous.comp_stronglyMeasurable hφ

@[fun_prop]

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.apply, comp_stronglyMeasurable, continuous, continuous.comp_stronglyMeasurable
-/
theorem StronglyMeasurable.apply_continuousLinearMap
...
@[fun_prop]
/--
theorem `MeasureTheory.AEStronglyMeasurable.apply_continuousLinearMap` / 定理 `MeasureTheory.AEStronglyMeasurable.apply_continuousLinearMap`

English:
theorem MeasureTheory.AEStronglyMeasurable.apply_continuousLinearMap
  statement: {φ : α -> F ->L[𝕜] E}
...
Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.apply, comp_aestronglyMeasurable, continuous, continuous.comp_aestronglyMeasurable
-/
...
/--
theorem `ContinuousLinearMap.aestronglyMeasurable_comp₂` / 定理 `ContinuousLinearMap.aestronglyMeasurable_comp₂`

English:
theorem ContinuousLinearMap.aestronglyMeasurable_comp₂
  statement: (L : E ->L[𝕜] F ->L[𝕜] G) {f : α -> E}
...
/--
theorem `aestronglyMeasurable_withDensity_iff` / 定理 `aestronglyMeasurable_withDensity_iff`

English:
theorem aestronglyMeasurable_withDensity_iff
  statement: {E : Type*} [NormedAddCommGroup E]
  proof: by
  constructor
  · rintro ⟨g', g'meas, hg'⟩
    have A : MeasurableSet { x : α | f x != 0 } := (hf (measurableSet_singleton 0)).compl
    refine ⟨fun x => (f x : Real) • g' x, hf.coe_nnreal_real.stronglyMeasurable.smul g'meas, ?_⟩
    apply @ae_of_ae_restrict_of_ae_restrict_compl _ _ _ { x | f x !
...
Depends on / 依赖: ENNReal, ENNReal.coe_eq_zero, EventuallyEq, MeasurableSet, ae_of_ae_restrict_of_ae_restrict_compl, ae_restrict_iff, ae_withDensity_iff, coe_eq_zero, coe_nnreal_ennreal, coe_nnreal_real, filter_upwards, hf.coe_nnreal_ennreal, hf.coe_nnreal_real.stronglyMeasurable.smul, measurableSet_singleton, stronglyMeasurable
-/
theorem aestronglyMeasurable_withDensity_iff {E : Type*} [NormedAddCommGroup E]
    [NormedSpace Real E] {f : α -> Real>=0} (hf : Measurable f) {g : α -> E} :
    AEStronglyMeasurable g (μ.withDensity fun x => (f x : Real>=0∞)) ↔
      AEStronglyMeasurable (fun x => (f x : Real) • g x) μ := by
  constructor
  · rintro ⟨g', g'meas, hg'⟩
    have A : MeasurableSet { x : α | f x != 0 } := (hf (measurableSet_singleton 0)).compl
    refine ⟨fun x => (f x : Real) • g' x, hf.coe_nnreal_real.stronglyMeasurable.smul g'meas, ?_⟩
    ...
    · filter_upwards [ae_restrict_mem A.compl] with x hx
      ...
      simp [hx]
  · rintro ⟨g', g'meas, hg'⟩
    refine ⟨fun x => (f x : Real)⁻¹ • g' x, hf.coe_nnreal_real.inv.stronglyMeasurable.smul g'meas, ?_⟩
