...
-/
/-
**CategoryTheory.FreeMonoidalCategory** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTheor
y`。
形式化陈述：Type u → Type u
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a type `C`, the free monoidal category over `C` has as objects formal expr
essions built from
(formal) tensor products of terms of `C` and a formal unit. Its morphisms are co
mpositions and
tensor products of identities, unitors and associators.
-/
...
/-
**CategoryTheory.FreeMonoidalCategory.Hom** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryT
heory.FreeMonoidalCategory`。
形式化陈述：{C : Type u} → CategoryTheory.FreeMonoidalCategory C → CategoryTheory.Free
MonoidalCategory C → Type u
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Formal compositions and tensor products of identities, unitors and associators. 
The morphisms
of the free monoidal category are obtained as a quotient of these formal morphis
ms by the
relations defining a monoidal category.
-/
...
/-
**CategoryTheory.FreeMonoidalCategory.HomEquiv** 是 Mathlib 中的一个归纳类型，位于命名空间 `Cate
goryTheory.FreeMonoidalCategory`。
形式化陈述：{C : Type u} → {X Y : CategoryTheory.FreeMonoidalCategory C} → X.Hom Y → X
.Hom Y → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The morphisms of the free monoidal category satisfy 21 relations ensuring that t
he resulting
category is in fact a category and that it is monoidal.
-/
...
/-
**CategoryTheory.FreeMonoidalCategory.setoidHom** 是 Mathlib 中的一个实例，位于命名空间 `Categ
oryTheory.FreeMonoidalCategory`。
形式化陈述：setoidHom (X Y : F C) : Setoid (X ⟶ᵐ Y)
参数：X Y : F C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
We say that two formal morphisms in the free monoidal category are equivalent if
 they become
equal if we apply the relations that are true in a monoidal category. Note that 
we will prove
that there is only one equivalence class -- this is the monoidal coherence theor
em.
-/
...
/-
**CategoryTheory.FreeMonoidalCategory.categoryFreeMonoidalCategory** 是 Mathlib 中
的一个实例，位于命名空间 `CategoryTheory.FreeMonoidalCategory`。
形式化陈述：categoryFreeMonoidalCategory : Category.{u} (F C) where Hom X Y
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_comp** 是 Mathlib 中的一个定理，位于命名空间 `Categor
yTheory.FreeMonoidalCategory`。
形式化陈述：mk_comp {X Y Z : F C} (f : X ⟶ᵐ Y) (g : Y ⟶ᵐ Z) : ⟦f.comp g⟧ = @CategorySt
ruct.comp (F C) _ _ _ _ ⟦f⟧ ⟦g⟧
参数：f : X ⟶ᵐ Y；g : Y ⟶ᵐ Z。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_tensor** 是 Mathlib 中的一个定理，位于命名空间 `Categ
oryTheory.FreeMonoidalCategory`。
形式化陈述：mk_tensor {X₁ Y₁ X₂ Y₂ : F C} (f : X₁ ⟶ᵐ Y₁) (g : X₂ ⟶ᵐ Y₂) : ⟦f.tensor g⟧
 = @MonoidalCategory.tensorHom (F C) _ _ _ _ _ _ ⟦f⟧ ⟦g⟧
参数：f : X₁ ⟶ᵐ Y₁；g : X₂ ⟶ᵐ Y₂。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_whiskerLeft** 是 Mathlib 中的一个定理，位于命名空间 `
CategoryTheory.FreeMonoidalCategory`。
形式化陈述：mk_whiskerLeft (X : F C) {Y₁ Y₂ : F C} (f : Y₁ ⟶ᵐ Y₂) : ⟦f.whiskerLeft X⟧ 
= MonoidalCategory.whiskerLeft (C
参数：X : F C；f : Y₁ ⟶ᵐ Y₂。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_whiskerRight** 是 Mathlib 中的一个定理，位于命名空间 
`CategoryTheory.FreeMonoidalCategory`。
形式化陈述：mk_whiskerRight {X₁ X₂ : F C} (f : X₁ ⟶ᵐ X₂) (Y : F C) : ⟦f.whiskerRight Y
⟧ = MonoidalCategory.whiskerRight (C
参数：f : X₁ ⟶ᵐ X₂；Y : F C。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_id** 是 Mathlib 中的一个定理，位于命名空间 `CategoryT
heory.FreeMonoidalCategory`。
形式化陈述：mk_id {X : F C} : ⟦Hom.id X⟧ = 𝟙 X
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_** 是 Mathlib 中的一个定理，位于命名空间 `CategoryThe
ory.FreeMonoidalCategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_l_hom** 是 Mathlib 中的一个定理，位于命名空间 `Catego
ryTheory.FreeMonoidalCategory`。
形式化陈述：mk_l_hom {X : F C} : ⟦Hom.l_hom X⟧ = (fun_ X).hom
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.mk_l_inv** 是 Mathlib 中的一个定理，位于命名空间 `Catego
ryTheory.FreeMonoidalCategory`。
形式化陈述：mk_l_inv {X : F C} : ⟦Hom.l_inv X⟧ = (fun_ X).inv
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.tensor_eq_tensor** 是 Mathlib 中的一个定理，位于命名空间
 `CategoryTheory.FreeMonoidalCategory`。
形式化陈述：tensor_eq_tensor {X Y : F C} : X.tensor Y = X otimes Y
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.unit_eq_unit** 是 Mathlib 中的一个定理，位于命名空间 `Ca
tegoryTheory.FreeMonoidalCategory`。
形式化陈述：unit_eq_unit : FreeMonoidalCategory.unit = 𝟙_ (F C)
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.FreeMonoidalCategory.homMk** 是 Mathlib 中的一个缩写定义，位于命名空间 `Categor
yTheory.FreeMonoidalCategory`。
形式化陈述：homMk {X Y : F C} (f : X ⟶ᵐ Y) : X ⟶ Y
参数：f : X ⟶ᵐ Y。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
This is useful since the notation `⟦f⟧` often behaves like an element of the quo
tient set,
but not like a morphism. This is why we need weird `@CategoryStruct.comp (F C) .
..` in the
statement in `mk_comp` above.
-/
abbrev homMk {X Y : F C} (f : X ⟶ᵐ Y) : X ⟶ Y := ⟦f⟧
/-
**CategoryTheory.FreeMonoidalCategory.Hom.inductionOn** 是 Mathlib 中的一个定理，位于命名空间 
`CategoryTheory.FreeMonoidalCategory.Hom`。
形式化陈述：∀ {C : Type u} {motive : {X Y : CategoryTheory.FreeMonoidalCategory C} → (
X ⟶ Y) → Prop}   {X Y : CategoryTheory.FreeMonoidalCategory C} (t : X ⟶ Y),   (∀
 (X : CategoryTheory.FreeMonoidalCategory C), motive (CategoryTheory.CategoryStr
uct.id X)) →     (∀ (X Y Z : CategoryTheory.FreeMonoidalCategory C),         mot
ive (CategoryTheory.MonoidalCategoryStruct.associator X Y Z).hom) →       (∀ (X 
Y Z : CategoryTheory.FreeMonoidalCategory C),           motive (CategoryTheory.M
onoidalCategoryStruct.associator X Y Z).inv) →         (∀ (X : CategoryTheory.Fr
eeMonoidalCategory C),             motive (CategoryTheory.MonoidalCategoryStruct
.leftUnitor X).hom) →           (∀ (X : CategoryTheory.FreeMonoidalCategory C), 
              motive (CategoryTheory.MonoidalCategoryStruct.leftUnitor X).inv) →
             (∀ (X : CategoryTheory.FreeMonoidalCategory C),                 mot
ive (CategoryTheory.MonoidalCategoryStruct.rightUnitor X).hom) →               (
∀ (X : CategoryTheory.FreeMonoidalCategory C),                   motive (Categor
yTheory.MonoidalCategoryStruct.rightUnitor X).inv) →                 (∀ {X Y Z :
 CategoryTheory.FreeMonoidalCategory C} (f : X ⟶ Y) (g : Y ⟶ Z),                
     motive f → motive g → motive (CategoryTheory.CategoryStruct.comp f g)) →   
                (∀ (X : CategoryTheory.FreeMonoidalCategory C) {Y Z : CategoryTh
eory.FreeMonoidalCategory C}                       (f : Y ⟶ Z), motive f → motiv
e (CategoryTheory.MonoidalCategoryStruct.whiskerLeft X f)) →                    
 (∀ {X Y : CategoryTheory.FreeMonoidalCategory C} (f : X ⟶ Y)                   
      (Z : CategoryTheory.FreeMonoidalCategory C),                         motiv
e f → motive (CategoryTheory.MonoidalCategoryStruct.whiskerRight f Z)) →        
               motive t
参数：X ⟶ Y；t : X ⟶ Y；∀ (X : CategoryTheory.FreeMonoidalCategory C), motive (Catego
ryTheory.CategoryStruct.id X)；∀ (X Y Z : CategoryTheory.FreeMonoidalCategory C),
         motive (CategoryTheory.MonoidalCategoryStruct.associator X Y Z).hom；∀ (
X Y Z : CategoryTheory.FreeMonoidalCategory C),           motive (CategoryTheory
.MonoidalCategoryStruct.associator X Y Z).inv；∀ (X : CategoryTheory.FreeMonoidal
Category C),             motive (CategoryTheory.MonoidalCategoryStruct.leftUnito
r X).hom；∀ (X : CategoryTheory.FreeMonoidalCategory C),               motive (Ca
tegoryTheory.MonoidalCategoryStruct.leftUnitor X).inv；∀ (X : CategoryTheory.Free
MonoidalCategory C),                 motive (CategoryTheory.MonoidalCategoryStru
ct.rightUnitor X).hom；∀ (X : CategoryTheory.FreeMonoidalCategory C),            
       motive (CategoryTheory.MonoidalCategoryStruct.rightUnitor X).inv；∀ {X Y Z
 : CategoryTheory.FreeMonoidalCategory C} (f : X ⟶ Y) (g : Y ⟶ Z),              
       motive f → motive g → motive (CategoryTheory.CategoryStruct.comp f g)；∀ (
X : CategoryTheory.FreeMonoidalCategory C) {Y Z : CategoryTheory.FreeMonoidalCat
egory C}                       (f : Y ⟶ Z), motive f → motive (CategoryTheory.Mo
noidalCategoryStruct.whiskerLeft X f)；∀ {X Y : CategoryTheory.FreeMonoidalCatego
ry C} (f : X ⟶ Y)                         (Z : CategoryTheory.FreeMonoidalCatego
ry C),                         motive f → motive (CategoryTheory.MonoidalCategor
yStruct.whiskerRight f Z)。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.FreeMonoidalCategory.projectObj** 是 Mathlib 中的一个定义，位于命名空间 `Cate
goryTheory.FreeMonoidalCategory`。
形式化陈述：{C : Type u} →   {D : Type u'} →     [inst : CategoryTheory.Category.{v', 
u'} D] →       [CategoryTheory.MonoidalCategory D] → (C → D) → CategoryTheory.Fr
eeMonoidalCategory C → D
参数：C → D。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.projectMapAux** 是 Mathlib 中的一个定义，位于命名空间 `C
ategoryTheory.FreeMonoidalCategory`。
形式化陈述：{C : Type u} →   {D : Type u'} →     [inst : CategoryTheory.Category.{v', 
u'} D] →       [inst_1 : CategoryTheory.MonoidalCategory D] →         (f : C → D
) →           {X Y : CategoryTheory.FreeMonoidalCategory C} →             X.Hom 
Y →               (CategoryTheory.FreeMonoidalCategory.projectObj f X ⟶ Category
Theory.FreeMonoidalCategory.projectObj f Y)
参数：f : C → D；CategoryTheory.FreeMonoidalCategory.projectObj f X ⟶ CategoryTheory
.FreeMonoidalCategory.projectObj f Y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**CategoryTheory.FreeMonoidalCategory.projectMap** 是 Mathlib 中的一个定义，位于命名空间 `Cate
goryTheory.FreeMonoidalCategory`。
形式化陈述：projectMap (X Y : F C) : (X ⟶ Y) -> (projectObj f X ⟶ projectObj f Y)
参数：X Y : F C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CategoryTheory.FreeMonoidalCategory.project** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.FreeMonoidalCategory`。
形式化陈述：project : F C ⥤ D where obj
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
