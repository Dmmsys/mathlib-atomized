/-
Copyright (c) 2025 Junyan Xu. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Junyan Xu
-/
module

public import Mathlib.Algebra.Algebra.Defs
public import Mathlib.Topology.Covering.Quotient
public import Mathlib.Topology.Instances.AddCircle.Defs

/-!
# Covering maps involving `AddCircle`

-/

public section

namespace AddCircle

section AddCommGroup

open AddSubgroup

variable {𝕜 : Type*} [AddCommGroup 𝕜] (p : 𝕜) [TopologicalSpace 𝕜] [IsTopologicalAddGroup 𝕜]
  [DiscreteTopology (zmultiples p)]

/--
theorem `isAddQuotientCoveringMap_coe` / 定理 `isAddQuotientCoveringMap_coe`

English:
theorem isAddQuotientCoveringMap_coe
  proof: isAddQuotientCoveringMap_of_comm _ DiscreteTopology.isDiscrete

中文:
定理 isAddQuotientCoveringMap_coe
  证明: isAddQuotientCoveringMap_of_comm _ DiscreteTopology.isDiscrete

Depends on / 依赖: DiscreteTopology, DiscreteTopology.isDiscrete, isAddQuotientCoveringMap_of_comm, isDiscrete
-/
theorem isAddQuotientCoveringMap_coe :
    IsAddQuotientCoveringMap ((↑) : 𝕜 -> AddCircle p) (zmultiples p) :=
  isAddQuotientCoveringMap_of_comm _ DiscreteTopology.isDiscrete

/--
theorem `isCoveringMap_coe` / 定理 `isCoveringMap_coe`

English:
theorem isCoveringMap_coe
  statement: IsCoveringMap ((↑) : 𝕜 -> AddCircle p)
  proof: (isAddQuotientCoveringMap_coe p).isCoveringMap

中文:
定理 isCoveringMap_coe
  结论: IsCoveringMap ((↑) : 𝕜 -> AddCircle p)
  证明: (isAddQuotientCoveringMap_coe p).isCoveringMap

Depends on / 依赖: isAddQuotientCoveringMap_coe, isCoveringMap
-/
theorem isCoveringMap_coe : IsCoveringMap ((↑) : 𝕜 -> AddCircle p) :=
  (isAddQuotientCoveringMap_coe p).isCoveringMap

/--
theorem `isLocalHomeomorph_coe` / 定理 `isLocalHomeomorph_coe`

English:
theorem isLocalHomeomorph_coe
  statement: IsLocalHomeomorph ((↑) : 𝕜 -> AddCircle p)
  proof: (isCoveringMap_coe p).isLocalHomeomorph

中文:
定理 isLocalHomeomorph_coe
  结论: IsLocalHomeomorph ((↑) : 𝕜 -> AddCircle p)
  证明: (isCoveringMap_coe p).isLocalHomeomorph

Depends on / 依赖: isCoveringMap_coe, isLocalHomeomorph
-/
theorem isLocalHomeomorph_coe : IsLocalHomeomorph ((↑) : 𝕜 -> AddCircle p) :=
  (isCoveringMap_coe p).isLocalHomeomorph

end AddCommGroup

section Field

open Topology

variable {𝕜 : Type*} [TopologicalSpace 𝕜] [Ring 𝕜] [IsTopologicalRing 𝕜]
variable (p : 𝕜) [T0Space (AddCircle p)]

-- TODO: this comment seems outdated?

/--
theorem `isAddQuotientCoveringMap_zsmul` / 定理 `isAddQuotientCoveringMap_zsmul`

English:
theorem isAddQuotientCoveringMap_zsmul
  given: {n : Int} (hn : IsUnit (n : 𝕜))
  proof: by
  refine hn.isQuotientMap_zsmul (QuotientAddGroup.mk' _) isQuotientMap_quotient_mk'
.isAddQuotientCoveringMap_of_isDiscrete_ker_addMonoidHom
    (f := zsmulAddGroupHom (α := AddCircle p) n)
    (Set.Finite.isDiscrete <| finite_torsion_of_isSMulRegular_int _ _ fun _ => ?_)
  simp_rw [zsmul_eq_mul]

中文:
定理 isAddQuotientCoveringMap_zsmul
  条件: {n : 整数} (hn : IsUnit (n : 𝕜))
  证明: by
  refine hn.isQuotientMap_zsmul (QuotientAddGroup.mk' _) isQuotientMap_quotient_mk'
.isAddQuotientCoveringMap_of_isDiscrete_ker_addMonoidHom
    (f := zsmulAddGroupHom (α := AddCircle p) n)
    (Set.Finite.isDiscrete <| finite_torsion_of_isSMulRegular_int _ _ fun _ => ?_)
  simp_rw [zsmul_eq_mul]

Depends on / 依赖: AddCircle, Finite, QuotientAddGroup, QuotientAddGroup.mk, Set.Finite.isDiscrete, finite_torsion_of_isSMulRegular_int, hn.isQuotientMap_zsmul, hn.isSMulRegular, isAddQuotientCoveringMap_of_isDiscrete_ker_addMonoidHom, isDiscrete, isQuotientMap_quotient_mk, isQuotientMap_zsmul, isSMulRegular, simp_rw, zsmulAddGroupHom, zsmul_eq_mul
-/
theorem isAddQuotientCoveringMap_zsmul {n : Int} (hn : IsUnit (n : 𝕜)) :
    IsAddQuotientCoveringMap (n • · : AddCircle p -> _)
      (zsmulAddGroupHom (α := AddCircle p) n).ker := by
  refine hn.isQuotientMap_zsmul (QuotientAddGroup.mk' _) isQuotientMap_quotient_mk'
.isAddQuotientCoveringMap_of_isDiscrete_ker_addMonoidHom
    (f := zsmulAddGroupHom (α := AddCircle p) n)
    (Set.Finite.isDiscrete <| finite_torsion_of_isSMulRegular_int _ _ fun _ => ?_)
  simp_rw [zsmul_eq_mul]
  apply hn.isSMulRegular 𝕜

/--
theorem `isAddQuotientCoveringMap_nsmul` / 定理 `isAddQuotientCoveringMap_nsmul`

English:
theorem isAddQuotientCoveringMap_nsmul
  given: {n : Nat} (hn : IsUnit (n : 𝕜))
  proof: by
  convert! isAddQuotientCoveringMap_zsmul p (n := n) (mod_cast hn)
  all_goals ext; simp

中文:
定理 isAddQuotientCoveringMap_nsmul
  条件: {n : 自然数} (hn : IsUnit (n : 𝕜))
  证明: by
  convert! isAddQuotientCoveringMap_zsmul p (n := n) (mod_cast hn)
  all_goals ext; simp

Depends on / 依赖: AddCircle, all_goals, convert, isAddQuotientCoveringMap_zsmul, mod_cast
-/
theorem isAddQuotientCoveringMap_nsmul {n : Nat} (hn : IsUnit (n : 𝕜)) :
    IsAddQuotientCoveringMap (n • · : AddCircle p -> _)
      (nsmulAddMonoidHom (α := AddCircle p) n).ker := by
  convert! isAddQuotientCoveringMap_zsmul p (n := n) (mod_cast hn)
  all_goals ext; simp

/--
theorem `isAddQuotientCoveringMap_zsmul_of_ne_zero` / 定理 `isAddQuotientCoveringMap_zsmul_of_ne_zero`

English:
theorem isAddQuotientCoveringMap_zsmul_of_ne_zero
  given: [Algebra Rat 𝕜] (n : Int) [NeZero n]
  proof: isAddQuotientCoveringMap_zsmul p (n := n) by
    convert! (Int.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

中文:
定理 isAddQuotientCoveringMap_zsmul_of_ne_zero
  条件: [Algebra Rat 𝕜] (n : 整数) [NeZero n]
  证明: isAddQuotientCoveringMap_zsmul p (n := n) by
    convert! (Int.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

Depends on / 依赖: AddCircle
-/
theorem isAddQuotientCoveringMap_zsmul_of_ne_zero [Algebra Rat 𝕜] (n : Int) [NeZero n] :
    IsAddQuotientCoveringMap (n • · : AddCircle p -> _)
      (zsmulAddGroupHom (α := AddCircle p) n).ker :=
isAddQuotientCoveringMap_zsmul p (n := n) by
    convert! (Int.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

/--
theorem `isAddQuotientCoveringMap_nsmul_of_ne_zero` / 定理 `isAddQuotientCoveringMap_nsmul_of_ne_zero`

English:
theorem isAddQuotientCoveringMap_nsmul_of_ne_zero
  given: [Algebra Rat 𝕜] (n : Nat) [NeZero n]
  proof: isAddQuotientCoveringMap_nsmul p (n := n) by
    convert! (Nat.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

中文:
定理 isAddQuotientCoveringMap_nsmul_of_ne_zero
  条件: [Algebra Rat 𝕜] (n : 自然数) [NeZero n]
  证明: isAddQuotientCoveringMap_nsmul p (n := n) by
    convert! (Nat.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

Depends on / 依赖: AddCircle
-/
theorem isAddQuotientCoveringMap_nsmul_of_ne_zero [Algebra Rat 𝕜] (n : Nat) [NeZero n] :
    IsAddQuotientCoveringMap (n • · : AddCircle p -> _)
      (nsmulAddMonoidHom (α := AddCircle p) n).ker :=
isAddQuotientCoveringMap_nsmul p (n := n) by
    convert! (Nat.cast_ne_zero.mpr <| NeZero.ne n).isUnit.map (algebraMap Rat 𝕜); simp

end Field

end AddCircle
