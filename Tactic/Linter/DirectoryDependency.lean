...
/-
**Lean.Name.findPrefix** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Name.findPrefix {α} (f : Name -> Option α) (n : Name) : Option α
参数：f : Name -> Option α；n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Find the longest prefix of `n` such that `f` returns `some` (or return `none` ot
herwise).
-/
...
/-
**Lean.Name.prefixes** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Name.prefixes (n : Name) : NameSet
参数：n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Lean.Name.prefix** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Name.prefix? (n : Name) : Option Name
参数：n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Lean.Name.collectPrefixes** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Name.collectPrefixes (ns : Array Name) : NameSet
参数：ns : Array Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Lean.Name.prefixToName** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Lean.Name.prefixToName (p : Name) (ns : Array Name) : Option Name
参数：p : Name；ns : Array Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Linter.DirectoryDependency`。
形式化陈述：NamePrefixRel
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
That is, `R : NamePrefixRel` is supposed to answer given names `(n₁, n₂)` whethe
r there are any
prefixes `n₁'` of `n₁` and `n₂'` of `n₂` such that `n₁' R n₂'`.

The current implementation is a `NameMap` of `NameSet`s, testing each prefix of 
`n₁` and `n₂` in
turn. This can probably be optimized.
-/
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.** 是 Mathlib 中的一个实例，位于命名空间 `M
athlib.Linter.DirectoryDependency.NamePrefixRel`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.insert** 是 Mathlib 中的一个定义，位于命
名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：insert (r : NamePrefixRel) (n₁ n₂ : Name) : NamePrefixRel
参数：r : NamePrefixRel；n₁ n₂ : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.ofArray** 是 Mathlib 中的一个定义，位于
命名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：ofArray (xs : Array (Name × Name)) : NamePrefixRel
参数：xs : Array (Name × Name)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.find** 是 Mathlib 中的一个定义，位于命名空
间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：find (r : NamePrefixRel) (n₁ n₂ : Name) : Option (Name × Name)
参数：r : NamePrefixRel；n₁ n₂ : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.findAny** 是 Mathlib 中的一个定义，位于
命名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：findAny (r : NamePrefixRel) (n₁ : Name) (ns : Array Name) : Option (Name ×
 Name)
参数：r : NamePrefixRel；n₁ : Name；ns : Array Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Get a prefix of `n₁` that is related to any prefix of the names in `ns`; return 
the prefixes.

This should be more efficient than iterating over all names in `ns` and calling 
`find`,
since it doesn't need to worry about overlapping prefixes.
-/
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.containsKey** 是 Mathlib 中的一个定
义，位于命名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：containsKey (r : NamePrefixRel) (n : Name) : Bool
参数：r : NamePrefixRel；n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.contains** 是 Mathlib 中的一个定义，位
于命名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：contains (r : NamePrefixRel) (n₁ n₂ : Name) : Bool
参数：r : NamePrefixRel；n₁ n₂ : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.DirectoryDependency.NamePrefixRel.getAllLeft** 是 Mathlib 中的一个定义
，位于命名空间 `Mathlib.Linter.DirectoryDependency.NamePrefixRel`。
形式化陈述：getAllLeft (r : NamePrefixRel) (n : Name) : NameSet
参数：r : NamePrefixRel；n : Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.DirectoryDependency.allowedImportDirs** 是 Mathlib 中的一个定义，位于命名空间
 `Mathlib.Linter.DirectoryDependency`。
形式化陈述：allowedImportDirs : NamePrefixRel
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`allowedImportDirs` relates module prefixes, specifying that modules with the fi
rst prefix
are only allowed to import modules in the second directory.
For directories which are low in the import hierarchy, this opt-out approach is 
both more ergonomic
(fewer updates needed) and needs less configuration.
...
-/
/-
**Mathlib.Linter.DirectoryDependency.forbiddenImportDirs** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Linter.DirectoryDependency`。
形式化陈述：forbiddenImportDirs : NamePrefixRel
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`forbiddenImportDirs` relates module prefixes, specifying that modules with the 
first prefix
should not import modules with the second prefix (except if specifically allowed
 in
`overrideAllowedImportDirs`).

For example, ``(`Mathlib.Algebra.Notation, `Mathlib.Algebra)`` is in `forbiddenI
mportDirs` and
``(`Mathlib.Algebra.Notation, `Mathlib.Algebra.Notation)`` is in `overrideAllowe
dImportDirs`
because modules in `Mathlib/Algebra/Notation.lean` cannot import modules in `Mat
hlib.Algebra` that are
outside `Mathlib/Algebra/Notation.lean`.
-/
...
-/
/-
**Mathlib.Linter.DirectoryDependency.overrideAllowedImportDirs** 是 Mathlib 中的一个定
义，位于命名空间 `Mathlib.Linter.DirectoryDependency`。
形式化陈述：overrideAllowedImportDirs : NamePrefixRel
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`overrideAllowedImportDirs` relates module prefixes, specifying that modules wit
h the first
prefix are allowed to import modules with the second prefix, even if disallowed 
in
`forbiddenImportDirs`.
...
/-
**Mathlib.Linter.checkBlocklist** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：checkBlocklist (env : Environment) (mainModule : Name) (imports : Array Na
me) : Option MessageData
参数：env : Environment；mainModule : Name；imports : Array Name。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Check if one of the imports `imports` to `mainModule` is forbidden by `forbidden
ImportDirs`;
if so, return an error describing how the import transitively arises.
-/
