/-
Copyright (c) 2024 Robin Carlier. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Robin Carlier
-/
module

public import Mathlib.CategoryTheory.Monoidal.FunctorCategory
public import Mathlib.CategoryTheory.Monoidal.ExternalProduct.Basic
public import Mathlib.CategoryTheory.Monoidal.Closed.Types
public import Mathlib.CategoryTheory.Monoidal.Limits.Preserves
public import Mathlib.CategoryTheory.Limits.Preserves.Bifunctor
public import Mathlib.CategoryTheory.Limits.Preserves.FunctorCategory
public import Mathlib.CategoryTheory.Limits.IsConnected
public import Mathlib.CategoryTheory.Products.Associator
/-!
# Sifted categories

A category `C` is sifted if `C` is nonempty and the diagonal functor `C ⥤ C × C` is final.
Sifted categories can be characterized as those such that the colimit functor `(C ⥤ Type) ⥤ Type `
preserves finite products. We achieve this characterization in this file.

## Main results
- `isSifted_of_hasBinaryCoproducts_and_nonempty`: A nonempty category with binary coproducts is
  sifted.
- `IsSifted.colimPreservesFiniteProductsOfIsSifted`: The `Type`-valued colimit functor for sifted
  diagrams preserves finite products.
- `IsSifted.of_colimit_preservesFiniteProducts`: The converse: if the `Type`-valued colimit functor
  preserves finite products, the category is sifted.
- `IsSifted.of_final_functor_from_sifted`: A category admitting a final functor from a sifted
  category is itself sifted.

## References
- [nLab, *Sifted category*](https://ncatlab.org/nlab/show/sifted+category)
- [*Algebraic Theories*, Chapter 2.][Adamek_Rosicky_Vitale_2010]
-/

public section

universe w v v₁ v₂ u u₁ u₂

namespace CategoryTheory

open Limits CategoryTheory.Functor

section

variable (C : Type u) [Category.{v} C]

/--
Definition of `IsSiftedOrEmpty` / `IsSiftedOrEmpty` 的定义

English:
abbreviation IsSiftedOrEmpty
  signature: : Prop
  body: Final (diag C)

中文:
缩写 是SiftedOrEmpty
  签名: : 命题
  定义体: Final (diag C)
-/
abbrev IsSiftedOrEmpty : Prop := Final (diag C)

/--
Definition of `IsSifted` / `IsSifted` 的定义

English:
class IsSifted
  parameters: : Prop extends IsSiftedOrEmpty C where
  extends: IsSiftedOrEmpty C
  axioms and operations (1):
    - [nonempty : Nonempty C]

中文:
类 是Sifted
  参数: : 命题 extends 是SiftedOrEmpty C where
  继承: 是SiftedOrEmpty C
  公理与运算 (1 个):
    - [nonempty : 非空 C]
-/
class IsSifted : Prop extends IsSiftedOrEmpty C where
  [nonempty : Nonempty C]

/- This instance is scoped since
- it applies unconditionally (which can be a performance drain),
- infers a *very* generic typeclass,
- and does so from a *very* specialised class. -/
attribute [scoped instance] IsSifted.nonempty

namespace IsSifted

variable {C}

set_option backward.defeqAttrib.useBackward true in
/--
lemma `isSifted_of_equiv` / 引理 `isSifted_of_equiv`

English:
lemma isSifted_of_equiv
  given: [IsSifted C] {D : Type u₁} [Category.{v₁} D] (e : D ≌ C)
  statement: IsSifted D
  proof: letI : Final (diag D) := by
    let : D × D ≌ C × C := Equivalence.prod e e
    have sq : (e.inverse ⋙ diag D ⋙ this.functor ≅ diag C) :=
        NatIso.ofComponents (fun c => by dsimp [this]
                                        exact Iso.prod (e.counitIso.app c) (e.counitIso.app c))
    apply_rules [final_iff_comp_equivalence _ this.functor |>.mpr,
.mpr, final_of_natIso sq.symm] final_iff_final_comp e.inverse _
  letI : _root_.Nonempty D := ⟨e.inverse.obj (_root_.Nonempty.some IsSifted.nonempty)⟩
  ⟨⟩

中文:
引理 isSifted_of_equiv
  条件: [是Sifted C] {D : 类型u₁} [范畴.{v₁} D] (e : D ≌ C)
  结论: 是Sifted D
  证明: letI : Final (diag D) := by
    let : D × D ≌ C × C := Equivalence.prod e e
    have sq : (e.inverse ⋙ diag D ⋙ this.functor ≅ diag C) :=
        NatIso.ofComponents (fun c => by dsimp [this]
                                        exact Iso.prod (e.counitIso.app c) (e.counitIso.app c))
    apply_rules [final_iff_comp_equivalence _ this.functor |>.mpr,
.mpr, final_of_natIso sq.symm] final_iff_final_comp e.inverse _
  letI : _root_.Nonempty D := ⟨e.inverse.obj (_root_.Nonempty.some IsSifted.nonempty)⟩
  ⟨⟩

Depends on / 依赖: Equivalence, Equivalence.prod, IsSifted, IsSifted.nonempty, Iso.prod, NatIso, NatIso.ofComponents, Nonempty, _root_, _root_.Nonempty, _root_.Nonempty.some, apply_rules, counitIso, e.counitIso.app, e.inverse, e.inverse.obj, final_iff_comp_equivalence, final_iff_final_comp, final_of_natIso, functor
-/
lemma isSifted_of_equiv [IsSifted C] {D : Type u₁} [Category.{v₁} D] (e : D ≌ C) : IsSifted D :=
  letI : Final (diag D) := by
    let : D × D ≌ C × C := Equivalence.prod e e
    have sq : (e.inverse ⋙ diag D ⋙ this.functor ≅ diag C) :=
        NatIso.ofComponents (fun c => by dsimp [this]
                                        exact Iso.prod (e.counitIso.app c) (e.counitIso.app c))
    apply_rules [final_iff_comp_equivalence _ this.functor |>.mpr,
.mpr, final_of_natIso sq.symm] final_iff_final_comp e.inverse _
  letI : _root_.Nonempty D := ⟨e.inverse.obj (_root_.Nonempty.some IsSifted.nonempty)⟩
  ⟨⟩

/--
lemma `isSifted_iff_asSmallIsSifted` / 引理 `isSifted_iff_asSmallIsSifted`

English:
lemma isSifted_iff_asSmallIsSifted
  statement: IsSifted C ↔ IsSifted (AsSmall.{w} C) where
  proof: isSifted_of_equiv AsSmall.equiv.symm
  mpr _ := isSifted_of_equiv AsSmall.equiv

中文:
引理 isSifted_iff_asSmallIsSifted
  结论: 是Sifted C ↔ 是Sifted (AsSmall.{w} C) where
  证明: isSifted_of_equiv AsSmall.equiv.symm
  mpr _ := isSifted_of_equiv AsSmall.equiv

Depends on / 依赖: AsSmall, AsSmall.equiv.symm, isSifted_of_equiv
-/
lemma isSifted_iff_asSmallIsSifted : IsSifted C ↔ IsSifted (AsSmall.{w} C) where
  mp _ := isSifted_of_equiv AsSmall.equiv.symm
  mpr _ := isSifted_of_equiv AsSmall.equiv

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [IsSifted
  signature: C] : IsConnected C
  body: isConnected_of_zigzag
    (by intro c₁ c₂
        have X : StructuredArrow (c₁, c₂) (diag C) :=
          letI S : Final (diag C) := by infer_instance
          Nonempty.some (S.out (c₁, c₂)).is_nonempty
        use [X.right, c₂]
        constructor
        · constructor
          · exact Zag.of_hom X.hom.fst
          · simpa using Zag.of_inv X.hom.snd
        · rfl)

中文:
实例 [是Sifted
  签名: C] : 是连通 C
  定义体: isConnected_of_zigzag
    (by intro c₁ c₂
        have X : StructuredArrow (c₁, c₂) (diag C) :=
          letI S : Final (diag C) := by infer_instance
          Nonempty.some (S.out (c₁, c₂)).is_nonempty
        use [X.right, c₂]
        constructor
        · constructor
          · exact Zag.of_hom X.hom.fst
          · simpa using Zag.of_inv X.hom.snd
        · rfl)

Depends on / 依赖: Nonempty, Nonempty.some, S.out, StructuredArrow, X.hom.fst, X.hom.snd, X.right, Zag.of_hom, Zag.of_inv, infer_instance, isConnected_of_zigzag, is_nonempty, of_hom, of_inv
-/
instance [IsSifted C] : IsConnected C :=
  isConnected_of_zigzag
    (by intro c₁ c₂
        have X : StructuredArrow (c₁, c₂) (diag C) :=
          letI S : Final (diag C) := by infer_instance
          Nonempty.some (S.out (c₁, c₂)).is_nonempty
        use [X.right, c₂]
        constructor
        · constructor
          · exact Zag.of_hom X.hom.fst
          · simpa using Zag.of_inv X.hom.snd
        · rfl)

set_option backward.isDefEq.respectTransparency false in
set_option backward.defeqAttrib.useBackward true in
/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [HasBinaryCoproducts
  signature: C] : IsSiftedOrEmpty C
  body: by
    constructor
    rintro ⟨c₁, c₂⟩
have : _root_.Nonempty StructuredArrow (c₁, c₂) (diag C) :=
      ⟨.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂))⟩
    apply isConnected_of_zigzag
    rintro ⟨_, c, f⟩ ⟨_, c', g⟩
    dsimp only [const_obj_obj, diag_obj] at f g
    use [.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂)), .mk (g.fst, g.snd)]
    simp only [colimit.cocone_x, diag_obj, Prod.mk.eta, List.isChain_cons_cons,
      List.isChain_singleton, and_true, ne_eq, reduceCtorEq, not_false_eq_true,
      List.getLast_cons, List.cons_ne_self, List.getLast_singleton]
exact ⟨⟨Zag.of_inv StructuredArrow.homMk coprod.desc f.fst f.snd,
Zag.of_hom StructuredArrow.homMk coprod.desc g.fst g.snd⟩, rfl⟩

中文:
实例 [HasBinaryCoproducts
  签名: C] : 是SiftedOrEmpty C
  定义体: by
    constructor
    rintro ⟨c₁, c₂⟩
have : _root_.Nonempty StructuredArrow (c₁, c₂) (diag C) :=
      ⟨.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂))⟩
    apply isConnected_of_zigzag
    rintro ⟨_, c, f⟩ ⟨_, c', g⟩
    dsimp only [const_obj_obj, diag_obj] at f g
    use [.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂)), .mk (g.fst, g.snd)]
    simp only [colimit.cocone_x, diag_obj, Prod.mk.eta, List.isChain_cons_cons,
      List.isChain_singleton, and_true, ne_eq, reduceCtorEq, not_false_eq_true,
      List.getLast_cons, List.cons_ne_self, List.getLast_singleton]
exact ⟨⟨Zag.of_inv StructuredArrow.homMk coprod.desc f.fst f.snd,
Zag.of_hom StructuredArrow.homMk coprod.desc g.fst g.snd⟩, rfl⟩

Depends on / 依赖: List.isChain_cons_cons, List.isChain_singleton, Nonempty, Prod.mk.eta, StructuredArrow, _root_, _root_.Nonempty, and_true, cocone_x, colimit, colimit.cocone_x, const_obj_obj, coprod, coprod.inl, coprod.inr, diag_obj, g.fst, g.snd, isChain_cons_cons, isChain_singleton
-/
instance [HasBinaryCoproducts C] : IsSiftedOrEmpty C := by
    constructor
    rintro ⟨c₁, c₂⟩
have : _root_.Nonempty StructuredArrow (c₁, c₂) (diag C) :=
      ⟨.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂))⟩
    apply isConnected_of_zigzag
    rintro ⟨_, c, f⟩ ⟨_, c', g⟩
    dsimp only [const_obj_obj, diag_obj] at f g
    use [.mk ((coprod.inl : c₁ ⟶ c₁ ⨿ c₂), (coprod.inr : c₂ ⟶ c₁ ⨿ c₂)), .mk (g.fst, g.snd)]
    simp only [colimit.cocone_x, diag_obj, Prod.mk.eta, List.isChain_cons_cons,
      List.isChain_singleton, and_true, ne_eq, reduceCtorEq, not_false_eq_true,
      List.getLast_cons, List.cons_ne_self, List.getLast_singleton]
exact ⟨⟨Zag.of_inv StructuredArrow.homMk coprod.desc f.fst f.snd,
Zag.of_hom StructuredArrow.homMk coprod.desc g.fst g.snd⟩, rfl⟩

/--
Instance `isSifted_of_hasBinaryCoproducts_and_nonempty` / 实例 `isSifted_of_hasBinaryCoproducts_and_nonempty`

English:
instance isSifted_of_hasBinaryCoproducts_and_nonempty
  signature: [_root_.Nonempty C] [HasBinaryCoproducts C]

中文:
实例 isSifted_of_hasBinaryCoproducts_and_nonempty
  签名: [_root_.非空 C] [HasBinaryCoproducts C]
-/
instance isSifted_of_hasBinaryCoproducts_and_nonempty [_root_.Nonempty C] [HasBinaryCoproducts C] :
    IsSifted C where

section Prod

variable {D : Type u₁} [Category.{v₁} D]

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [IsSiftedOrEmpty
  signature: C] [IsSiftedOrEmpty D] :
  body: let e : (C × C) × (D × D) ≌ (C × D) × (C × D) := prod.prodμ ..
  final_of_natIso (Iso.refl ((Functor.diag C).prod (Functor.diag D) ⋙ e.functor))

中文:
实例 [是SiftedOrEmpty
  签名: C] [是SiftedOrEmpty D] :
  定义体: let e : (C × C) × (D × D) ≌ (C × D) × (C × D) := prod.prodμ ..
  final_of_natIso (Iso.refl ((Functor.diag C).prod (Functor.diag D) ⋙ e.functor))

Depends on / 依赖: Functor, Functor.diag, Iso.refl, e.functor, final_of_natIso, functor, prod.prod
-/
instance [IsSiftedOrEmpty C] [IsSiftedOrEmpty D] :
    IsSiftedOrEmpty (C × D) :=
  let e : (C × C) × (D × D) ≌ (C × D) × (C × D) := prod.prodμ ..
  final_of_natIso (Iso.refl ((Functor.diag C).prod (Functor.diag D) ⋙ e.functor))

/--
Instance `prod_isSifted` / 实例 `prod_isSifted`

English:
instance prod_isSifted
  signature: [IsSifted C] [IsSifted D]

中文:
实例 prod_isSifted
  签名: [是Sifted C] [是Sifted D]
-/
instance prod_isSifted [IsSifted C] [IsSifted D] : IsSifted (C × D) where

end Prod

end IsSifted

end

section

variable {C : Type u} [Category.{v} C] [IsSiftedOrEmpty C] {D : Type u₁} [Category.{v₁} D]
  {D' : Type u₂} [Category.{v₂} D'] (F : C ⥤ D) (G : C ⥤ D')

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [F.Final]
  signature: [G.Final]
  body: show (diag C ⋙ F.prod G).Final from final_comp _ _

中文:
实例 [F.终]
  签名: [G.终]
  定义体: show (diag C ⋙ F.prod G).Final from final_comp _ _

Depends on / 依赖: F.prod, final_comp
-/
instance [F.Final] [G.Final] : (F.prod' G).Final :=
  show (diag C ⋙ F.prod G).Final from final_comp _ _

end

noncomputable section SmallCategory

open MonoidalCategory CartesianMonoidalCategory

namespace IsSifted

variable {C : Type u} [SmallCategory C]

section

open scoped MonoidalCategory.ExternalProduct

variable (X Y : C ⥤ Type u)

set_option backward.isDefEq.respectTransparency false in
set_option backward.defeqAttrib.useBackward true in
/--
lemma `factorization_prodComparison_colim` / 引理 `factorization_prodComparison_colim`

English:
lemma factorization_prodComparison_colim
  proof: by
  apply colimit.hom_ext
  intro j
  dsimp [externalProductBifunctor, CartesianMonoidalCategory.prodComparison,
    externalProductBifunctorCurried, externalProduct]
  cat_disch

中文:
引理 factorization_prodComparison_colim
  证明: by
  apply colimit.hom_ext
  intro j
  dsimp [externalProductBifunctor, CartesianMonoidalCategory.prodComparison,
    externalProductBifunctorCurried, externalProduct]
  cat_disch

Depends on / 依赖: CartesianMonoidalCategory, CartesianMonoidalCategory.prodComparison, cat_disch, colimit, colimit.hom_ext, externalProduct, externalProductBifunctor, externalProductBifunctorCurried, hom_ext, prodComparison
-/
lemma factorization_prodComparison_colim :
    (HasColimit.isoOfNatIso ((externalProductCompDiagIso _ _).app (X, Y)).symm).hom ≫
      colimit.pre (X ⊠ Y) (diag C) ≫
        (PreservesColimit₂.isoColimitUncurryWhiskeringLeft₂ X Y <|
curriedTensor Type u).hom =
    CartesianMonoidalCategory.prodComparison colim X Y := by
  apply colimit.hom_ext
  intro j
  dsimp [externalProductBifunctor, CartesianMonoidalCategory.prodComparison,
    externalProductBifunctorCurried, externalProduct]
  cat_disch

variable [IsSifted C]

set_option backward.isDefEq.respectTransparency false in
/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: IsIso (CartesianMonoidalCategory.prodComparison colim X Y)
  body: by
  rw [← factorization_prodComparison_colim]
  infer_instance

中文:
实例 :
  签名: 是同构 (CartesianMonoidal范畴.prodComparison colim X Y)
  定义体: by
  rw [← factorization_prodComparison_colim]
  infer_instance

Depends on / 依赖: factorization_prodComparison_colim, infer_instance
-/
instance : IsIso (CartesianMonoidalCategory.prodComparison colim X Y) := by
  rw [← factorization_prodComparison_colim]
  infer_instance

/--
Instance `colim_preservesLimits_pair_of_sSifted` / 实例 `colim_preservesLimits_pair_of_sSifted`

English:
instance colim_preservesLimits_pair_of_sSifted
  signature: {X Y : C ⥤ Type u}
  body: preservesLimit_pair_of_isIso_prodComparison _ _ _

中文:
实例 colim_preservesLimits_pair_of_sSifted
  签名: {X Y : C ⥤ 类型u}
  定义体: preservesLimit_pair_of_isIso_prodComparison _ _ _

Depends on / 依赖: preservesLimit_pair_of_isIso_prodComparison
-/
instance colim_preservesLimits_pair_of_sSifted {X Y : C ⥤ Type u} :
    PreservesLimit (pair X Y) colim :=
  preservesLimit_pair_of_isIso_prodComparison _ _ _

/--
Instance `colim_preservesBinaryProducts_of_isSifted` / 实例 `colim_preservesBinaryProducts_of_isSifted`

English:
instance colim_preservesBinaryProducts_of_isSifted
  signature: :
  body: by
  constructor
  intro F
  apply preservesLimit_of_iso_diagram colim (diagramIsoPair F).symm

中文:
实例 colim_preservesBinaryProducts_of_isSifted
  签名: :
  定义体: by
  constructor
  intro F
  apply preservesLimit_of_iso_diagram colim (diagramIsoPair F).symm

Depends on / 依赖: diagramIsoPair, preservesLimit_of_iso_diagram
-/
instance colim_preservesBinaryProducts_of_isSifted :
    PreservesLimitsOfShape (Discrete WalkingPair) (colim : (C ⥤ _) ⥤ Type u) := by
  constructor
  intro F
  apply preservesLimit_of_iso_diagram colim (diagramIsoPair F).symm

/--
Instance `colim_preservesTerminal_of_isSifted` / 实例 `colim_preservesTerminal_of_isSifted`

English:
instance colim_preservesTerminal_of_isSifted
  signature: :
  body: by
  apply preservesTerminal_of_iso
  symm
  apply (_ : ⊤_ (Type u) ≅ PUnit.{u + 1}).trans
  · apply_rules [(Types.colimitConstPUnitIsoPUnit C).symm.trans, HasColimit.isoOfNatIso,
      IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
exact fun _ => isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _
.toFun terminalIsTerminal · exact Types.isTerminalEquivIsoPUnit (⊤_ (Type u))

中文:
实例 colim_preservesTerminal_of_isSifted
  签名: :
  定义体: by
  apply preservesTerminal_of_iso
  symm
  apply (_ : ⊤_ (Type u) ≅ PUnit.{u + 1}).trans
  · apply_rules [(Types.colimitConstPUnitIsoPUnit C).symm.trans, HasColimit.isoOfNatIso,
      IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
exact fun _ => isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _
.toFun terminalIsTerminal · exact Types.isTerminalEquivIsoPUnit (⊤_ (Type u))

Depends on / 依赖: HasColimit, HasColimit.isoOfNatIso, IsTerminal, IsTerminal.uniqueUpToIso, Iso.refl, Types.colimitConstPUnitIsoPUnit, Types.isTerminalEquivIsoPUnit, Types.isTerminalPUnit, apply_rules, colimitConstPUnitIsoPUnit, evaluationJointlyReflectsLimits, isLimitChangeEmptyCone, isTerminalEquivIsoPUnit, isTerminalPUnit, isoOfNatIso, preservesTerminal_of_iso, symm.trans, terminalIsTerminal, uniqueUpToIso
-/
instance colim_preservesTerminal_of_isSifted :
    PreservesLimit (Functor.empty.{0} (C ⥤ Type u)) colim := by
  apply preservesTerminal_of_iso
  symm
  apply (_ : ⊤_ (Type u) ≅ PUnit.{u + 1}).trans
  · apply_rules [(Types.colimitConstPUnitIsoPUnit C).symm.trans, HasColimit.isoOfNatIso,
      IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
exact fun _ => isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _
.toFun terminalIsTerminal · exact Types.isTerminalEquivIsoPUnit (⊤_ (Type u))

/--
Instance `colim_preservesLimitsOfShape_pempty_of_isSifted` / 实例 `colim_preservesLimitsOfShape_pempty_of_isSifted`

English:
instance colim_preservesLimitsOfShape_pempty_of_isSifted
  signature: :
  body: preservesLimitsOfShape_pempty_of_preservesTerminal _

中文:
实例 colim_preservesLimitsOfShape_pempty_of_isSifted
  签名: :
  定义体: preservesLimitsOfShape_pempty_of_preservesTerminal _

Depends on / 依赖: preservesLimitsOfShape_pempty_of_preservesTerminal
-/
instance colim_preservesLimitsOfShape_pempty_of_isSifted :
    PreservesLimitsOfShape (Discrete PEmpty.{1}) (colim : (C ⥤ _) ⥤ Type u) :=
  preservesLimitsOfShape_pempty_of_preservesTerminal _

/--
Instance `colim_preservesFiniteProducts_of_isSifted` / 实例 `colim_preservesFiniteProducts_of_isSifted`

English:
instance colim_preservesFiniteProducts_of_isSifted
  signature: :
  body: PreservesFiniteProducts.of_preserves_binary_and_terminal colim

中文:
实例 colim_preservesFiniteProducts_of_isSifted
  签名: :
  定义体: PreservesFiniteProducts.of_preserves_binary_and_terminal colim

Depends on / 依赖: PreservesFiniteProducts, PreservesFiniteProducts.of_preserves_binary_and_terminal, of_preserves_binary_and_terminal
-/
instance colim_preservesFiniteProducts_of_isSifted :
    PreservesFiniteProducts (colim : (C ⥤ _) ⥤ Type u) :=
  PreservesFiniteProducts.of_preserves_binary_and_terminal colim

end

section

variable (C)

open Opposite in
open scoped MonoidalCategory.ExternalProduct in
/--
theorem `isSiftedOrEmpty_of_colim_preservesBinaryProducts` / 定理 `isSiftedOrEmpty_of_colim_preservesBinaryProducts`

English:
theorem isSiftedOrEmpty_of_colim_preservesBinaryProducts
  proof: by
  apply final_of_colimit_comp_coyoneda_iso_pUnit
  rintro ⟨c₁, c₂⟩
calc colimit diag C ⋙ coyoneda.obj (op (c₁, c₂))
_ ≅ colimit _ ⋙ (coyoneda.obj _) ⊠ (coyoneda.obj _) :=
HasColimit.isoOfNatIso isoWhiskerLeft _ .refl _
_ ≅ colimit (_ otimes _) := HasColimit.isoOfNatIso .refl _
    _ ≅ (colimit _) otimes (colimit _) := CartesianMonoidalCategory.prodComparisonIso colim _ _
    _ ≅ PUnit otimes PUnit :=
      (Coyoneda.colimitCoyonedaIso _) otimesᵢ (Coyoneda.colimitCoyonedaIso _)
    _ ≅ PUnit := fun_ _

中文:
定理 isSiftedOrEmpty_of_colim_preservesBinaryProducts
  证明: by
  apply final_of_colimit_comp_coyoneda_iso_pUnit
  rintro ⟨c₁, c₂⟩
calc colimit diag C ⋙ coyoneda.obj (op (c₁, c₂))
_ ≅ colimit _ ⋙ (coyoneda.obj _) ⊠ (coyoneda.obj _) :=
HasColimit.isoOfNatIso isoWhiskerLeft _ .refl _
_ ≅ colimit (_ otimes _) := HasColimit.isoOfNatIso .refl _
    _ ≅ (colimit _) otimes (colimit _) := CartesianMonoidalCategory.prodComparisonIso colim _ _
    _ ≅ PUnit otimes PUnit :=
      (Coyoneda.colimitCoyonedaIso _) otimesᵢ (Coyoneda.colimitCoyonedaIso _)
    _ ≅ PUnit := fun_ _

Depends on / 依赖: CartesianMonoidalCategory, CartesianMonoidalCategory.prodComparisonIso, Coyoneda, Coyoneda.colimitCoyonedaIso, HasColimit, HasColimit.isoOfNatIso, colimit, colimitCoyonedaIso, coyoneda, coyoneda.obj, final_of_colimit_comp_coyoneda_iso_pUnit, fun_, isoOfNatIso, isoWhiskerLeft, otimes, prodComparisonIso
-/
theorem isSiftedOrEmpty_of_colim_preservesBinaryProducts
    [PreservesLimitsOfShape (Discrete WalkingPair) (colim : (C ⥤ Type u) ⥤ Type u)] :
    IsSiftedOrEmpty C := by
  apply final_of_colimit_comp_coyoneda_iso_pUnit
  rintro ⟨c₁, c₂⟩
calc colimit diag C ⋙ coyoneda.obj (op (c₁, c₂))
_ ≅ colimit _ ⋙ (coyoneda.obj _) ⊠ (coyoneda.obj _) :=
HasColimit.isoOfNatIso isoWhiskerLeft _ .refl _
_ ≅ colimit (_ otimes _) := HasColimit.isoOfNatIso .refl _
    _ ≅ (colimit _) otimes (colimit _) := CartesianMonoidalCategory.prodComparisonIso colim _ _
    _ ≅ PUnit otimes PUnit :=
      (Coyoneda.colimitCoyonedaIso _) otimesᵢ (Coyoneda.colimitCoyonedaIso _)
    _ ≅ PUnit := fun_ _

/--
lemma `isSiftedOrEmpty_of_colim_preservesFiniteProducts` / 引理 `isSiftedOrEmpty_of_colim_preservesFiniteProducts`

English:
lemma isSiftedOrEmpty_of_colim_preservesFiniteProducts
  proof: isSiftedOrEmpty_of_colim_preservesBinaryProducts C

中文:
引理 isSiftedOrEmpty_of_colim_preservesFiniteProducts
  证明: isSiftedOrEmpty_of_colim_preservesBinaryProducts C

Depends on / 依赖: isSiftedOrEmpty_of_colim_preservesBinaryProducts
-/
lemma isSiftedOrEmpty_of_colim_preservesFiniteProducts
    [h : PreservesFiniteProducts (colim : (C ⥤ Type u) ⥤ Type u)] :
    IsSiftedOrEmpty C :=
  isSiftedOrEmpty_of_colim_preservesBinaryProducts C

/--
lemma `nonempty_of_colim_preservesLimitsOfShapeFinZero` / 引理 `nonempty_of_colim_preservesLimitsOfShapeFinZero`

English:
lemma nonempty_of_colim_preservesLimitsOfShapeFinZero
  proof: by
  suffices connected : IsConnected C by infer_instance
  rw [Types.isConnected_iff_colimit_constPUnitFunctor_iso_pUnit]
  constructor
  haveI : PreservesLimitsOfShape (Discrete PEmpty) (colim : (C ⥤ _) ⥤ Type u) :=
    preservesLimitsOfShape_of_equiv (Discrete.equivalence finZeroEquiv') _
.trans apply HasColimit.isoOfNatIso (_ : Types.constPUnitFunctor C ≅ (⊤_ (C ⥤ Type u)))
.trans · apply PreservesTerminal.iso colim
    exact Types.terminalIso
  · apply_rules [IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
    intro _
exact isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _

中文:
引理 nonempty_of_colim_preservesLimitsOfShapeFinZero
  证明: by
  suffices connected : IsConnected C by infer_instance
  rw [Types.isConnected_iff_colimit_constPUnitFunctor_iso_pUnit]
  constructor
  haveI : PreservesLimitsOfShape (Discrete PEmpty) (colim : (C ⥤ _) ⥤ Type u) :=
    preservesLimitsOfShape_of_equiv (Discrete.equivalence finZeroEquiv') _
.trans apply HasColimit.isoOfNatIso (_ : Types.constPUnitFunctor C ≅ (⊤_ (C ⥤ Type u)))
.trans · apply PreservesTerminal.iso colim
    exact Types.terminalIso
  · apply_rules [IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
    intro _
exact isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _

Depends on / 依赖: Discrete, Discrete.equivalence, HasColimit, HasColimit.isoOfNatIso, IsConnected, IsTerminal, IsTerminal.uniqueUpToIso, PEmpty, PreservesLimitsOfShape, PreservesTerminal, PreservesTerminal.iso, Types.constPUnitFunctor, Types.isConnected_iff_colimit_constPUnitFunctor_iso_pUnit, Types.terminalIso, apply_rules, connected, constPUnitFunctor, equivalence, finZeroEquiv, infer_instance
-/
lemma nonempty_of_colim_preservesLimitsOfShapeFinZero
    [PreservesLimitsOfShape (Discrete (Fin 0)) (colim : (C ⥤ Type u) ⥤ Type u)] :
    Nonempty C := by
  suffices connected : IsConnected C by infer_instance
  rw [Types.isConnected_iff_colimit_constPUnitFunctor_iso_pUnit]
  constructor
  haveI : PreservesLimitsOfShape (Discrete PEmpty) (colim : (C ⥤ _) ⥤ Type u) :=
    preservesLimitsOfShape_of_equiv (Discrete.equivalence finZeroEquiv') _
.trans apply HasColimit.isoOfNatIso (_ : Types.constPUnitFunctor C ≅ (⊤_ (C ⥤ Type u)))
.trans · apply PreservesTerminal.iso colim
    exact Types.terminalIso
  · apply_rules [IsTerminal.uniqueUpToIso _ terminalIsTerminal, evaluationJointlyReflectsLimits]
    intro _
exact isLimitChangeEmptyCone _ Types.isTerminalPUnit _ Iso.refl _

/--
theorem `of_colim_preservesFiniteProducts` / 定理 `of_colim_preservesFiniteProducts`

English:
theorem of_colim_preservesFiniteProducts
  proof: by
  have := isSiftedOrEmpty_of_colim_preservesFiniteProducts C
  have := nonempty_of_colim_preservesLimitsOfShapeFinZero C
  constructor

中文:
定理 of_colim_preservesFiniteProducts
  证明: by
  have := isSiftedOrEmpty_of_colim_preservesFiniteProducts C
  have := nonempty_of_colim_preservesLimitsOfShapeFinZero C
  constructor

Depends on / 依赖: isSiftedOrEmpty_of_colim_preservesFiniteProducts, nonempty_of_colim_preservesLimitsOfShapeFinZero
-/
theorem of_colim_preservesFiniteProducts
    [h : PreservesFiniteProducts (colim : (C ⥤ Type u) ⥤ Type u)] :
    IsSifted C := by
  have := isSiftedOrEmpty_of_colim_preservesFiniteProducts C
  have := nonempty_of_colim_preservesLimitsOfShapeFinZero C
  constructor

variable {C}

/--
theorem `of_final_functor_from_sifted'` / 定理 `of_final_functor_from_sifted'`

English:
theorem of_final_functor_from_sifted'
  proof: by
  have : PreservesFiniteProducts (colim : (D ⥤ Type u) ⥤ _) :=
    ⟨fun n => preservesLimitsOfShape_of_natIso (Final.colimIso F)⟩
  exact of_colim_preservesFiniteProducts D

中文:
定理 of_final_functor_from_sifted'
  证明: by
  have : PreservesFiniteProducts (colim : (D ⥤ Type u) ⥤ _) :=
    ⟨fun n => preservesLimitsOfShape_of_natIso (Final.colimIso F)⟩
  exact of_colim_preservesFiniteProducts D

Depends on / 依赖: Final.colimIso, PreservesFiniteProducts, colimIso, of_colim_preservesFiniteProducts, preservesLimitsOfShape_of_natIso
-/
theorem of_final_functor_from_sifted'
    {D : Type u} [SmallCategory D] [IsSifted C] (F : C ⥤ D) [Final F] : IsSifted D := by
  have : PreservesFiniteProducts (colim : (D ⥤ Type u) ⥤ _) :=
    ⟨fun n => preservesLimitsOfShape_of_natIso (Final.colimIso F)⟩
  exact of_colim_preservesFiniteProducts D

end

end IsSifted

end SmallCategory

variable {C : Type u} [Category.{v} C]

/--
theorem `IsSifted.of_final_functor_from_sifted` / 定理 `IsSifted.of_final_functor_from_sifted`

English:
theorem IsSifted.of_final_functor_from_sifted
  statement: {D : Type u₁} [Category.{v₁} D] [h₁ : IsSifted C]
  proof: by
  rw [isSifted_iff_asSmallIsSifted] at h₁ ⊢
exact of_final_functor_from_sifted'
    AsSmall.equiv.{_, _, max u₁ v₁}.inverse ⋙ F ⋙ AsSmall.equiv.{_, _, max u v}.functor

中文:
定理 是Sifted.of_final_functor_from_sifted
  结论: {D : 类型u₁} [范畴.{v₁} D] [h₁ : 是Sifted C]
  证明: by
  rw [isSifted_iff_asSmallIsSifted] at h₁ ⊢
exact of_final_functor_from_sifted'
    AsSmall.equiv.{_, _, max u₁ v₁}.inverse ⋙ F ⋙ AsSmall.equiv.{_, _, max u v}.functor

Depends on / 依赖: AsSmall, AsSmall.equiv, functor, inverse, isSifted_iff_asSmallIsSifted, of_final_functor_from_sifted
-/
theorem IsSifted.of_final_functor_from_sifted {D : Type u₁} [Category.{v₁} D] [h₁ : IsSifted C]
    (F : C ⥤ D) [Final F] : IsSifted D := by
  rw [isSifted_iff_asSmallIsSifted] at h₁ ⊢
exact of_final_functor_from_sifted'
    AsSmall.equiv.{_, _, max u₁ v₁}.inverse ⋙ F ⋙ AsSmall.equiv.{_, _, max u v}.functor

end CategoryTheory
