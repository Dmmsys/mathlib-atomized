...
/-
**TopCat.uliftFunctor** 是 Mathlib 中的一个定义，位于命名空间 `TopCat`。
形式化陈述：uliftFunctor : TopCat.{u} ⥤ TopCat.{max u v} where obj X
该定义给出了上述对象。
...
/-
**TopCat.uliftFunctorObjHomeo** 是 Mathlib 中的一个定义，位于命名空间 `TopCat`。
形式化陈述：uliftFunctorObjHomeo (X : TopCat.{u}) : X ≃ₜ uliftFunctor.{v}.obj X
参数：X : TopCat.{u}。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**TopCat.uliftFunctorObjHomeo_naturality_apply** 是 Mathlib 中的一个引理，位于命名空间 `TopCat
`。
形式化陈述：uliftFunctorObjHomeo_naturality_apply {X Y : TopCat.{u}} (f : X ⟶ Y) (x : 
X) : uliftFunctor.{v}.map f (X.uliftFunctorObjHomeo x) = Y.uliftFunctorObjHomeo 
(f x)
参数：f : X ⟶ Y；x : X。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**TopCat.uliftFunctorObjHomeo_symm_naturality_apply** 是 Mathlib 中的一个引理，位于命名空间 `T
opCat`。
形式化陈述：uliftFunctorObjHomeo_symm_naturality_apply {X Y : TopCat.{u}} (f : X ⟶ Y) 
(x : uliftFunctor.{v}.obj X) : Y.uliftFunctorObjHomeo.symm (uliftFunctor.{v}.map
 f x) = f (X.uliftFunctorObjHomeo.symm x)
参数：f : X ⟶ Y；x : uliftFunctor.{v}.obj X。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps!]
/-
**TopCat.uliftFunctorCompForgetIso** 是 Mathlib 中的一个定义，位于命名空间 `TopCat`。
形式化陈述：uliftFunctorCompForgetIso : uliftFunctor.{v, u} ⋙ forget TopCat.{max u v} 
≅ forget TopCat.{u} ⋙ CategoryTheory.uliftFunctor.{v, u}
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**TopCat.uliftFunctorFullyFaithful** 是 Mathlib 中的一个定义，位于命名空间 `TopCat`。
形式化陈述：uliftFunctorFullyFaithful : uliftFunctor.{v, u}.FullyFaithful where preima
ge f
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
