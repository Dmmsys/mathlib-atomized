...
/--
lemma `opNorm_mul_flip_apply` / 引理 `opNorm_mul_flip_apply`

English:
lemma opNorm_mul_flip_apply
  given: (a : E)
  statement: ‖(mul 𝕜 E).flip a‖ = ‖a‖
  proof: by
  refine le_antisymm
  ...
  refine opNorm_le_bound _ (norm_nonneg _) fun b => ?

中文:
引理 opNorm_mul_flip_apply
  条件: (a : E)
  结论: ‖(mul 𝕜 E).flip a‖ = ‖a‖
  证明: by
  refine le_antisymm
...
Depends on / 依赖: flip_apply, ge_iff_le, le_antisymm, mul_apply, mul_comm, norm_mul_le, norm_nonneg, norm_star, opNorm_le_bound, opNorm_mul_apply, star_mul, star_star
-/
...
/--
lemma `opNNNorm_mul_flip_apply` / 引理 `opNNNorm_mul_flip_apply`

English:
lemma opNNNorm_mul_flip_apply
  given: (a : E)
  statement: ‖(mul 𝕜 E).flip a‖₊ = ‖a‖₊
  proof: Subtype.ext (opNorm_mul_flip_apply 𝕜 a)

中文:
引理 opNNNorm_mul_flip_apply
  条件: (a : E)
  结论: ‖(mul 𝕜 E).flip a‖₊ = ‖a‖₊
  证明: Subtype.ext (opNorm_mul_flip_apply 𝕜 a)

Depends on / 依赖: Subtype, Subtype.ext, opNorm_mul_flip_apply
-/
...
/--
lemma `isometry_mul_flip` / 引理 `isometry_mul_flip`

English:
lemma isometry_mul_flip
  statement: Isometry (mul 𝕜 E).flip
  proof: AddMonoidHomClass.isometry_of_norm _ (opNorm_mul_flip_apply 𝕜)
...
/--
Instance `CStarRing.instRegularNormedAlgebra` / 实例 `CStarRing.instRegularNormedAlgebra`

English:
instance CStarRing.instRegularNormedAlgebra
  signature: : RegularNormedAlgebra 𝕜 E where
  body: AddMonoidHomClass.isometry_of_norm (mul 𝕜 E) fun a => NNReal.eq_iff.mp
    show ‖mul 𝕜 E a‖₊ = ‖a‖₊ by
...
Depends on / 依赖: AddMonoidHomClass, AddMonoidHomClass.isometry_of_norm, NNReal, NNReal.eq_iff.mp, eq_iff, isometry_of_norm
-/
...
isometry_mul' := AddMonoidHomClass.isometry_of_norm (mul 𝕜 E) fun a => NNReal.eq_iff.mp
    ...
    · have ha : 0 < ‖a‖₊ := hr.pos
      rw [← inv_inv ‖a‖₊]; rw [NNReal.lt_inv_iff_mul_lt (inv_ne_zero ha.ne')] at hr
      obtain ⟨k, hk₁, hk₂⟩ :=
...
/--
theorem `Unitization.norm_splitMul_snd_sq` / 定理 `Unitization.norm_splitMul_snd_sq`

English:
theorem Unitization.norm_splitMul_snd_sq
  given: (x : Unitization 𝕜 E)
  proof: by
  ...
  refine (Real.le_sqrt (norm_nonneg _) (nor

中文:
定理 Unitization.norm_splitMul_snd_sq
  条件: (x : Unitization 𝕜 E)
  证明: by
  /- The key idea is that we can use `sSup_unitClosedBall_eq_norm` to make this about
  applying this linear map to elements of norm at most one. There is a bit of `sqrt` and `sq`
  shuffling that needs to occur, which is primarily just an annoyance. -/
...
theorem Unitization.norm_splitMul_snd_sq (x : Unitization 𝕜 E) :
  ...
  rw [← Real.sqrt_sq (norm_nonneg _)]; rw [Real.sqrt_le_sqrt_iff (norm_nonneg _)]; rw [sq]; rw [← CStarRing.norm_star_mul_self]; rw [add_apply]; rw [star_add]; rw [mul_apply']; rw [Algebra.algebraMap_eq_smul_one]; rw [smul_apply]; rw [one_apply_eq_self]; rw [star_mul]; rw [star_smul]; rw [add_mul]; rw [smul_mul_assoc]; rw [← mul_smul_comm]; rw [mul_assoc]; rw [← mul_add]; rw [← sSup_unitClosedBall_eq_norm]
...
/--
Instance `Unitization.instCStarRing` / 实例 `Unitization.instCStarRing`

English:
instance Unitization.instCStarRing
  signature: : CStarRing (Unitization 𝕜 E) where
  body: by
    ...
    have h₁ : forall x : Unitization 𝕜 E,
        ‖(Unitization.splitMul 𝕜 E x).snd‖ <= ‖(Unitization.splitMul 𝕜 E (
...
unital, `A⁺¹ ≃⋆ₐ[ℂ] (ℂ × A)`. -/
scoped[CStarAlgebra] postfix:max "⁺¹" => Unitization Complex

/--
Instance `Unitization.instCStarAlgebra` / 实例 `Unitization.instCStarAlgebra`

English:
instance Unitization.instCStarAlgebra
  signature: {A : Type*} [NonUnitalCStarAlgebra A]
...
/--
Instance `Unitization.instCommCStarAlgebra` / 实例 `Unitization.instCommCStarAlgebra`

English:
instance Unitization.instCommCStarAlgebra
  signature: {A : Type*} [NonUnitalCommCStarAlgebra A]
