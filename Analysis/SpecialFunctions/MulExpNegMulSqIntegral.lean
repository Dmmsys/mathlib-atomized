...
variable {E : Type*} [TopologicalSpace E] [MeasurableSpace E] [BorelSpace E]
    {P : Measure E} [IsFiniteMeasure P] {ε : Real}

/--
theorem `integrable_mulExpNegMulSq_comp` / 定理 `integrable_mulExpNegMulSq_comp`

English:
theorem integrable_mulExpNegMulSq_comp
  given: (f : C(E, Real)) (hε : 0 < ε)
  proof: by
...
中文:
定理 integrable_mulExpNegMulSq_comp
  条件: (f : C(E, 实数)) (hε : 0 < ε)
  证明: by
  apply integrable P ⟨⟨fun x => mulExpNegMulSq ε (f x), by fun_prop⟩, ⟨2 * (√ε)⁻¹, _⟩⟩
  exact fun x y => dist_mulExpNegMulSq_le_two_mul_sqrt hε (f x) (f y)

Depends on / 依赖: dist_mulExpNegMulSq_le_two_mul_sqrt, fun_prop, integrable, mulExpNegMulSq
-/
theorem integrable_mulExpNegMulSq_comp (f : C(E, Real)) (hε : 0 < ε) :
    Integrable (fun x => mulExpNegMulSq ε (f x)) P := by
  apply integrable P ⟨⟨fun x => mulExpNegMulSq ε (f x), by fun_prop⟩, ⟨2 * (√ε)⁻¹, _⟩⟩
  exact fun x y => dist_mulExpNegMulSq_le_two_mul_sqrt hε (f x) (f y)

/--
theorem `integrable_mulExpNegMulSq_comp_restrict_of_isCompact` / 定理 `integrable_mulExpNegMulSq_comp_restrict_of_isCompact`

English:
theorem integrable_mulExpNegMulSq_comp_restrict_of_isCompact
  statement: {K : Set E} (hK : IsCompact K)
  proof: g.continuous.mulExpNegMulSq.continuousOn.integrableOn_compact' hK hKmeas
...
theorem integrable_mulExpNegMulSq_comp_restrict_of_isCompact {K : Set E} (hK : IsCompact K)
    (hKmeas : MeasurableSet K) (g : C(E, Real)) :
...
/--
theorem `tendsto_integral_mulExpNegMulSq_comp` / 定理 `tendsto_integral_mulExpNegMulSq_comp`

English:
theorem tendsto_integral_mulExpNegMulSq_comp
  given: (g : E ->ᵇ Real)
  proof: by
...
中文:
定理 tendsto_integral_mulExpNegMulSq_comp
  条件: (g : E ->ᵇ 实数)
  证明: by
  apply tendsto_of_seq_tendsto
  intro u hu
...
Depends on / 依赖: Eventually, Eventually.of_forall, Set.mem_Ioi.mp, abs_mulExpNegMulSq_comp_le_norm, aestronglyMeasurable, continuous, eventually_atTop, eventually_atTop.mp, g.continuous.mulExpNegMulSq.aestronglyMeasurable, h_bound, h_lim, h_meas, le_of_lt, mem_Ioi, mulExpNegMulSq, of_forall, tendsto_integral_filter_of_norm_le_const, tendsto_nhdsWithin, tendsto_nhdsWithin_iff, tendsto_nhdsWithin_iff.mp
-/
theorem tendsto_integral_mulExpNegMulSq_comp (g : E ->ᵇ Real) :
...
/--
theorem `tendsto_integral_mul_one_add_inv_smul_sq_pow` / 定理 `tendsto_integral_mul_one_add_inv_smul_sq_pow`

English:
theorem tendsto_integral_mul_one_add_inv_smul_sq_pow
  given: (g : E ->ᵇ Real) (hε : 0 < ε)
  proof: by
  apply tendsto_integral_filter_of_norm_le_const ?h_meas ?h_bound ?h_lim
  · apply Eventually.of_forall
    exact fun n => StronglyMeasurable.aestronglyMeasurable (Continuous.stronglyMeasurable
        (Continuous.mul g.continuous ((1 + ((n : Real)⁻¹ • -(ε • g * g))) ^ n).continuous))
  · obtain 

中文:
定理 tendsto_integral_mul_one_add_inv_smul_sq_pow
  条件: (g : E ->ᵇ 实数) (hε : 0 < ε)
  证明: by
  apply tendsto_integral_filter_of_norm_le_const ?h_meas ?h_bound ?h_lim
  · apply Eventually.of_forall
...
Depends on / 依赖: Continuous, Continuous.mul, Continuous.stronglyMeasurable, Eventually, Eventually.of_forall, Nat.cast_le.mpr, StronglyMeasurable, StronglyMeasurable.aestronglyMeasurable, aestronglyMeasurable, cast_le, continuous, eventually_atTop, exists_nat_gt, g.continuous, h_bound, h_lim, h_meas, lt_of_le_of_lt, lt_of_lt_of_le, mul_nonneg
-/
theorem tendsto_integral_mul_one_add_inv_smul_sq_pow (g : E ->ᵇ Real) (hε : 0 < ε) :
    Tendsto (fun (n : Nat) => ∫ x, (g * (1 + (n : Real)⁻¹ • -(ε • g * g)) ^ n) x ∂P)
 ...
 mul_le_of_le_one_right (norm_nonneg _) ?_
    ...
    rw [mul_assoc]; rw [inv_mul_eq_div]; rw [abs_le]
    ...
    rw [← abs_le_iff_mul_self_le]; rw [abs_norm]
...
/--
theorem `integral_mulExpNegMulSq_comp_eq` / 定理 `integral_mulExpNegMulSq_comp_eq`

English:
theorem integral_mulExpNegMulSq_comp_eq
  statement: {P' : Measure E} [IsFiniteMeasure P']
  proof: by
  have one_add_inv_mul_mem (n : Nat) : g * (1 + (n : Real)⁻¹ • -(ε • g * g)) ^ n in A := by
    ...
    exact Subalgebra.neg_mem A (Subalgebra.mul_mem A (Subal
...
Depends on / 依赖: Subalgebra, Subalgebra.add_mem, Subalgebra.mul_mem, Subalgebra.neg_mem, Subalgebra.one_mem, Subalgebra.pow_mem, Subalgebra.smul_mem, Tendsto, add_mem, mulExpNegMulSq, mul_mem, neg_mem, one_add_inv_mul_m, one_add_inv_mul_mem, one_mem, pow_mem, smul_mem
-/
theorem integral_mulExpNegMulSq_comp_eq {P' : Measure E} [IsFiniteMeasure P']
    {A : Subalgebra Real (E ->ᵇ Real)} (hε : 0 < ε)
    (heq : forall g in A, ∫ x, (g : E -> Real) x ∂P = ∫ x, (g : E -> Real) x ∂P') {g : E ->ᵇ Real} (hgA : g in A) :
  ...
  have limP : Tendsto (fun n : Nat => ∫ x, (g * (1 + (n : Real)⁻¹ • -(ε • g * g)) ^ n) x ∂P) atTop
...
/--
theorem `abs_integral_sub_setIntegral_mulExpNegMulSq_comp_lt` / 定理 `abs_integral_sub_setIntegral_mulExpNegMulSq_comp_lt`

English:
theorem abs_integral_sub_setIntegral_mulExpNegMulSq_comp_lt
  statement: (f : C(E, Real))
  proof: by
  ...
  rw [mul_inv_lt_iff₀ (sqrt_pos_of_pos hε)]; rw [mul_self_sqrt (le_of_lt hε)]
...
Depends on / 依赖: Eventually, Eventually.of_forall, abs_mulExpNegMulSq_le, integrable_mulExpNegMulSq_comp, le_of_lt, lt_of_le_of_lt, mul_self_sqrt, norm_integral_sub_setIntegral_le, of_forall, sqrt_pos_of_pos, toReal_lt_of_lt_ofReal
-/
theorem abs_integral_sub_setIntegral_mulExpNegMulSq_comp_lt (f : C(E, Real))
...
/--
theorem `abs_setIntegral_mulExpNegMulSq_comp_sub_le_mul_measure` / 定理 `abs_setIntegral_mulExpNegMulSq_comp_sub_le_mul_measure`

English:
theorem abs_setIntegral_mulExpNegMulSq_comp_sub_le_mul_measure
  statement: {K : Set E} (hK : IsCompact K)
  proof: by
...
Depends on / 依赖: dist_mulExpNegMulSq_le_dist, hK.measure_lt_top, hKmeas, integrable_mulExpNegMulSq_comp_restrict_of_isCompact, integral_sub, le_trans, measure_lt_top, norm_eq_abs, norm_setIntegral_le_of_norm_le_const
-/
theorem abs_setIntegral_mulExpNegMulSq_comp_sub_le_mul_measure {K : Set E} (hK : IsCompact K)
    (hKmeas : MeasurableSet K) (f g : C(E, Real)) {δ : Real} (hε : 0 < ε)
    (hfg : forall x in K, |g x - f x| < δ) :
...
/--
theorem `dist_integral_mulExpNegMulSq_comp_le` / 定理 `dist_integral_mulExpNegMulSq_comp_le`

English:
theorem dist_integral_mulExpNegMulSq_comp_le
  statement: (f : E ->ᵇ Real)
  proof: by
  ...
  let const : Real := (max (P.real Set.univ) (P'.real Set.univ))
 

中文:
定理 dist_integral_mulExpNegMulSq_comp_le
  结论: (f : E ->ᵇ 实数)
  证明: by
  -- if both measures are zero, the result is trivial
  by_cases hPP' : P = 0 ∧ P' = 0
...
-/
theorem dist_integral_mulExpNegMulSq_comp_le (f : E ->ᵇ Real)
    {A : Subalgebra Real (E ->ᵇ Real)} (hA : (A.map (toContinuousMapₐ Real)).SeparatesPoints)
    (heq : forall g in A, ∫ x, (g : E -> Real) x ∂P = ∫ x, (g : E -> Real) x ∂P') (hε : 0 < ε) :
  ...
  have hKPbound : P (KP union KP')ᶜ < ε.toNNReal := lt_of_le_of_lt
        (measure_mono (Set.compl_subset_compl_of_subset (Set.subset_union_left))) hKP
  have hKP'bound : P' (KP union KP')ᶜ < ε.toNNReal := lt_of_le_of_lt
  ...
  let g := hg'A.choose
  have hgA : g in A := hg'A.choose_spec.1
  have hgapprox : forall x in K, ‖g x - f x‖ < √ε * const⁻¹ := by
    rw [← coe_toContinuousMapₐ Real g]; rw [hg'A.choose_spec.2]
    exact hg'approx
  ...
  calc
    ...
    <= |∫ x, mulExpNegMulSq ε (f x) ∂P - ∫ x in K, mulExpNegMulSq ε (f x) ∂P|
      ...
      + |∫ x in K, mulExpNegMulSq ε (f x) ∂P' - ∫ x, mulExpNegMulSq ε (f x) ∂P'| :=
        @dist_triangle8 Real _ _ _ _ _ _ _ _ _
  _ <= 6 * √ε := by linarith
