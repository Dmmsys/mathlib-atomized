...
/--
theorem `smul_singleton_mem_nhds_of_sigmaCompact` / 定理 `smul_singleton_mem_nhds_of_sigmaCompact`
...
/--
theorem `isOpenMap_smul_of_sigmaCompact` / 定理 `isOpenMap_smul_of_sigmaCompact`

English:
theorem isOpenMap_smul_of_sigmaCompact
  given: (x : X)
  statement: IsOpenMap (fun (g : G) => g • x)
  proof: by
...
@[to_additive]
/--
theorem `MonoidHom.isOpenMap_of_sigmaCompact` / 定理 `MonoidHom.isOpenMap_of_sigmaCompact`
