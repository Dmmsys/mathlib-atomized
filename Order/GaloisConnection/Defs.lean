...
/-
**GaloisConnection** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：GaloisConnection [Preorder α] [Preorder β] (l : α -> β) (u : β -> α)
参数：l : α -> β；u : β -> α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A Galois connection is a pair of functions `l` and `u` satisfying
`l a ≤ b ↔ a ≤ u b`. They are special cases of adjoint functors in category theo
ry,
but do not depend on the category theory library in mathlib.
-/
...
/-
**GaloisConnection.monotone_intro** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：monotone_intro (hu : Monotone u) (hl : Monotone l) (h_u_l : forall a, a <=
 u (l a)) (h_l_u : forall a, l (u a) <= a) : GaloisConnection l u
参数：hu : Monotone u；hl : Monotone l；h_u_l : forall a, a <= u (l a)；h_l_u : forall
 a, l (u a) <= a。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual self]
/-
**GaloisConnection.dual** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：∀ {α : Type u} {β : Type v} [inst : Preorder α] [inst_1 : Preorder β] {l :
 α → β} {u : β → α},   GaloisConnection l u →     GaloisConnection (⇑OrderDual.t
oDual ∘ u ∘ ⇑OrderDual.ofDual) (⇑OrderDual.toDual ∘ l ∘ ⇑OrderDual.ofDual)
参数：⇑OrderDual.toDual ∘ u ∘ ⇑OrderDual.ofDual；⇑OrderDual.toDual ∘ l ∘ ⇑OrderDual.
ofDual。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual none]
/-
**GaloisConnection.le_iff_le** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：le_iff_le {a : α} {b : β} : l a <= b ↔ a <= u b
该定理/引理刻画了左右两侧的等价关系。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[to_dual le_u]
/-
**GaloisConnection.l_le** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：l_le {a : α} {b : β} : a <= u b -> l a <= b
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual l_u_le]
/-
**GaloisConnection.le_u_l** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：le_u_l (a) : a <= u (l a)
参数：a。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[to_dual]
/-
**GaloisConnection.monotone_u** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection`。
形式化陈述：monotone_u : Monotone u
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `GaloisConnection.le_u`：∀ {α : Type u} {β : Type v} [inst : Preorder α] [
inst_1 : Preorder β] {u : α → β} {l : β → α},   GaloisConnection l u → ∀ {a : α}
...
@[to_dual]
/-
**GaloisConnection.monotone_l_comp_u** 是 Mathlib 中的一个定理，位于命名空间 `GaloisConnection
`。
形式化陈述：monotone_l_comp_u : Monotone (l ∘ u)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
