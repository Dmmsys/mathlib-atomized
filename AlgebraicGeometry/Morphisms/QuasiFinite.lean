...
@[mk_iff]
/--
Definition of `LocallyQuasiFinite` / `LocallyQuasiFinite` 的定义

English:
class LocallyQuasiFinite
...
English:
instance :
  signature: HasRingHomProperty @LocallyQuasiFinite RingHom.QuasiFinite
  body: RingHom.QuasiFinite.propertyIsLocal
  eq_affineLocally' := by
    ext X Y f
...
Depends on / 依赖: QuasiFinite, RingHom, RingHom.QuasiFinite.propertyIsLocal, propertyIsLocal
-/
...
English:
instance :
  signature: MorphismProperty.IsStableUnderComposition @LocallyQuasiFinite
  body: HasRingHomProperty.stableUnderComposition RingHom.QuasiFinite.stableUnderComposition
...
/--
theorem `LocallyQuasiFinite.of_comp` / 定理 `LocallyQuasiFinite.of_comp`

English:
theorem LocallyQuasiFinite.of_comp
...
/--
theorem `LocallyQuasiFinite.comp_iff` / 定理 `LocallyQuasiFinite.comp_iff`

English:
theorem LocallyQuasiFinite.comp_iff
  statement: {X Y Z : Scheme} (f : X ⟶ Y) (g : Y ⟶ Z)
...
English:
instance :
  signature: MorphismProperty.IsMultiplicative @LocallyQuasiFinite
  body: inferInstance
...
English:
instance :
  signature: MorphismProperty.IsStableUnderBaseChange @LocallyQuasiFinite
  body: HasRingHomProperty.isStableUnderBaseChange RingHom.QuasiFinite.isStableUnderBaseChange
...
English:
instance :
  signature: MorphismProperty.Respects @LocallyQuasiFinite @IsOpenImmersion
...
Depends on / 依赖: HasRingHomProperty, HasRingHomProperty.respects_isOpenImmersion, QuasiFinite, RingHom, RingHom.QuasiFinite.holdsForLocalizationAway, RingHom.QuasiFinite.stableUnderComposition.stableUnderCompositionWithLocalizationAway, holdsForLocalizationAway, respects_isOpenImmersion, stableUnderComposition, stableUnderCompositionWithLocalizationAway
-/
...
/--
lemma `Scheme.Hom.isDiscrete_preimage_singleton` / 引理 `Scheme.Hom.isDiscrete_preimage_singleton`

English:
lemma Scheme.Hom.isDiscrete_preimage_singleton
  given: [LocallyQuasiFinite f] (y : Y)
  proof: by
...
Depends on / 依赖: Scheme, Scheme.Hom.range_fiber, f.fiber, isDiscrete_univ_iff, isDiscrete_univ_iff.mpr, isEmbedding, isEmbedding.toIsInducing, toIsInducing
-/
...
/--
lemma `Scheme.Hom.isDiscrete_preimage` / 引理 `Scheme.Hom.isDiscrete_preimage`

English:
lemma Scheme.Hom.isDiscrete_preimage
  given: [LocallyQuasiFinite f] {s : Set Y} (hs : IsDiscrete s)
...
/--
lemma `Scheme.Hom.finite_preimage_singleton` / 引理 `Scheme.Hom.finite_preimage_singleton`

English:
lemma Scheme.Hom.finite_preimage_singleton
  given: [LocallyQuasiFinite f] [QuasiCompact f] (y : Y)
  proof: by
...
Depends on / 依赖: Scheme, Scheme.Hom.range_fiber, Set.finite_univ.image, f.fiber, finite_univ
-/
...
/--
lemma `Scheme.Hom.finite_preimage` / 引理 `Scheme.Hom.finite_preimage`

English:
lemma Scheme.Hom.finite_preimage
  statement: [LocallyQuasiFinite f] [QuasiCompact f]
...
/--
lemma `Scheme.Hom.tendsto_cofinite_cofinite` / 引理 `Scheme.Hom.tendsto_cofinite_cofinite`

English:
lemma Scheme.Hom.tendsto_cofinite_cofinite
  given: [LocallyQuasiFinite f] [QuasiCompact f]
...
nonrec lemma LocallyQuasiFinite.of_fiberToSpecResidueField
    (hf : forall x, LocallyQuasiFinite (f.fiberToSpecResidueField x)) : LocallyQuasiFinite f := by
  change id _ -- avoid typeclass synthesis from getting stuck on the wlog hypothesis.
  wlog hY : exists R, Y = Spec R
...
CommRingCat.ofHom algebraMap P.ResidueField (P.Fiber S)) by
    ...
    exact .of_quasiFinite
  obtain ⟨x, rfl⟩ : exists x : Spec R, P = x.asIdeal := ⟨⟨P, hP⟩, rfl⟩
...
/--
lemma `locallyQuasiFinite_iff_isFinite_fiber` / 引理 `locallyQuasiFinite_iff_isFinite_fiber`

English:
lemma locallyQuasiFinite_iff_isFinite_fiber
  given: {f : X ⟶ Y} [QuasiCompact f]
  proof: ⟨fun _ => inferInstance, fun _ => .of_fiberToSpecResidueField f fun _ => inferInstance⟩
...
lemma locallyQuasiFinite_iff_isFinite_fiber {f : X ⟶ Y} [QuasiCompact f] :
    LocallyQuasiFinite f ↔ forall x, IsFinite (f.fiberToSpecResidueField x) :=
...
nonrec lemma locallyQuasiFinite_iff_isDiscrete_preimage_singleton
    ...
    LocallyQuasiFinite f ↔ forall x, IsDiscrete (f ⁻¹' {x}) := by
...
nonrec lemma LocallyQuasiFinite.of_finite_preimage_singleton
    [LocallyOfFiniteType f] (hf : forall x, (f ⁻¹' {x}).Finite) : LocallyQuasiFinite f := by
  change id _ -- avoid typeclass synthesis from getting stuck on the wlog hypothesis.
  wlog hY : exists R, Y = Spec R
  · refine (IsZariskiLocalAtTarget.iff_of_openCover Y.affineCover).mpr fun i =>
      this (f := pullback.snd _ _) (fun x => ?_) ⟨_, rfl⟩
...
/--
lemma `locallyQuasiFinite_iff_finite_preimage_singleton` / 引理 `locallyQuasiFinite_iff_finite_preimage_singleton`

English:
lemma locallyQuasiFinite_iff_finite_preimage_singleton
  proof: ⟨fun _ => f.finite_preimage_singleton, .of_finite_preimage_singleton f⟩

中文:
引理 locallyQuasiFinite_iff_finite_preimage_singleton
  证明: ⟨fun _ => f.finite_preimage_singleton, .of_finite_preimage_singleton f⟩

Depends on / 依赖: f.finite_preimage_singleton, finite_preimage_singleton, of_finite_preimage_singleton
-/
lemma locallyQuasiFinite_iff_finite_preimage_singleton
    {f : X ⟶ Y} [LocallyOfFiniteType f] [QuasiCompact f] :
    LocallyQuasiFinite f ↔ forall x, (f ⁻¹' {x}).Finite :=
  ⟨fun _ => f.finite_preimage_singleton, .of_finite_preimage_singleton f⟩

/--
lemma `LocallyQuasiFinite.of_injective` / 引理 `LocallyQuasiFinite.of_injective`

English:
lemma LocallyQuasiFinite.of_injective
  statement: {f : X ⟶ Y} [LocallyOfFiniteType f]
...
/--
Definition of `Scheme.Hom.QuasiFiniteAt` / `Scheme.Hom.QuasiFiniteAt` 的定义

English:
definition Scheme.Hom.QuasiFiniteAt
  signature: (x : X)
...
Depends on / 依赖: QuasiFinite, f.stalkMap, hom.QuasiFinite, stalkMap
-/
...
set_option backward.isDefEq.respectTransparency.types false in
/--
lemma `Scheme.Hom.QuasiFiniteAt.quasiFiniteAt` / 引理 `Scheme.Hom.QuasiFiniteAt.quasiFiniteAt`
...
lemma Scheme.Hom.QuasiFiniteAt.quasiFiniteAt
    ...
    (hU : IsAffineOpen U) (hVU : V <= f ⁻¹ᵁ U) (hxV : x in V.1) :
...
/--
lemma `Scheme.Hom.quasiFiniteAt` / 引理 `Scheme.Hom.quasiFiniteAt`

English:
lemma Scheme.Hom.quasiFiniteAt
  given: [LocallyQuasiFinite f] (x : X)
  proof: by
...
Depends on / 依赖: Algebra, Algebra.QuasiFinite, AtPrime, HasRingHomProperty, HasRingHomProperty.stalkMap, Localization, Localization.AtPrime, QuasiFinite, RingHom, RingHom.quasiFinite_algebraMap.mpr, algebraMap, algebraize, convert, introv, of_comp, quasiFinite_algebraMap, stalkMap
-/
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `Scheme.Hom.quasiFiniteAt_comp_iff_of_isOpenImmersion` / 引理 `Scheme.Hom.quasiFiniteAt_comp_iff_of_isOpenImmersion`
...
Depends on / 依赖: CommRingCat, CommRingCat.hom_comp, QuasiFinite, QuasiFiniteAt, RingHom, RingHom.QuasiFinite.respectsIso.cancel_right_isIso, cancel_right_isIso, hom_comp, respectsIso, stalkMap_comp
-/
lemma Scheme.Hom.quasiFiniteAt_comp_iff_of_isOpenImmersion
...
/--
lemma `Scheme.Hom.quasiFiniteAt_comp_iff` / 引理 `Scheme.Hom.quasiFiniteAt_comp_iff`

English:
lemma Scheme.Hom.quasiFiniteAt_comp_iff
  statement: {Z : Scheme} {f : X ⟶ Y} {g : Y ⟶ Z} {x : X}
  proof: by
...
Depends on / 依赖: QuasiFinite, QuasiFiniteAt, RingHom, RingHom.QuasiFinite.comp_iff, comp_iff, g.quasiFiniteAt, quasiFiniteAt, stalkMap_comp
-/
...
/--
lemma `Scheme.Hom.quasiFiniteAt_iff` / 引理 `Scheme.Hom.quasiFiniteAt_iff`

English:
lemma Scheme.Hom.quasiFiniteAt_iff
  given: {f : X ⟶ Y} {x : X}
  proof: by
...
Depends on / 依赖: HasRingHomProperty, HasRingHomProperty.Spec_iff, LocallyQuasiFinite, LocallyQuasiFinite.comp_iff, QuasiFiniteAt, SpecMap_stalkMap_fromSpecStalk, Spec_iff, comp_iff
-/
...
nonrec lemma Scheme.Hom.quasiFiniteAt_iff_isOpen_singleton_asFiber
  ...
  · obtain ⟨i, y, hy⟩ := Y.affineCover.exists_eq (f x)
    ...
    have H : pullback.snd f ι ⁻¹' {pullback.snd f ι x} =
      ...
      rw [← Set.preimage_comp]; rw [← TopCat.coe_comp]; rw [← Scheme.Hom.comp_base]; rw [← Scheme.Hom.comp_apply]; rw [pullback.condition]
    ...
    have : Topology.IsOpenEmbedding f' := by
      ...
      congr!
    rw [this.isOpen_iff_image_isOpen]; rw [Set.image_singleton]; rfl
  obtain ⟨R, rfl⟩ := hY
  wlog hX : exists S, X = Spec S
  · obtain ⟨i, x, rfl⟩ := X.affineCover.exists_eq x
    ...
    rw [((f ⁻¹' {f (ι x)}).restrictPreimage_isOpenEmbedding
      ι.isOpenEmbedding).isOpen_iff_image_isOpen]; rw [Set.image_singleton]; rfl
  obtain ⟨S, rfl⟩ := hX
  obtain ⟨φ, rfl⟩ := Spec.map_surjective f
