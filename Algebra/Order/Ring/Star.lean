...
example {R : Type*} [Semiring R] [PartialOrder R] [IsOrderedRing R]
    ...
    (hy : 0 <= y) : x * y = y * x := by
  rw [← IsSelfAdjoint.of_nonneg (mul_nonneg hy hx)]; rw [star_mul]; rw [IsSelfAdjoint.of_nonneg hx]; rw [IsSelfAdjoint.of_nonneg hy]
...
instance toIsOrderedRing
  signature: (R : Type*) [CommSemiring R] [PartialOrder R]
  body: smul_le_smul_of_nonneg_left hbc ha
