...
/-
**MulOpposite.comul_def** 是 Mathlib 中的一个引理，位于命名空间 `MulOpposite`。
形式化陈述：comul_def [CoalgebraStruct R A] : comul (R
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**MulOpposite.counit_def** 是 Mathlib 中的一个引理，位于命名空间 `MulOpposite`。
形式化陈述：counit_def [CoalgebraStruct R A] : counit (R
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
