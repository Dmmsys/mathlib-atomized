...
/-
**SeparationQuotient.liftContinuousMonoidHom** 是 Mathlib 中的一个定义，位于命名空间 `Separati
onQuotient`。
形式化陈述：liftContinuousMonoidHom [CommMonoid M] [ContinuousMul M] [CommMonoid N] (f
 : ContinuousMonoidHom M N) (hf : forall x y, Inseparable x y -> f x = f y) : Co
ntinuousMonoidHom (SeparationQuotient M) N where toFun
参数：f : ContinuousMonoidHom M N；hf : forall x y, Inseparable x y -> f x = f y。
该定义给出了上述对象。
...
/-
**SeparationQuotient.liftContinuousCommMonoidHom_mk** 是 Mathlib 中的一个定理，位于命名空间 `S
eparationQuotient`。
形式化陈述：liftContinuousCommMonoidHom_mk [CommMonoid M] [ContinuousMul M] [CommMonoi
d N] (f : ContinuousMonoidHom M N) (hf : forall x y, Inseparable x y -> f x = f 
y) (x : M) : liftContinuousMonoidHom f hf (mk x) = f x
参数：f : ContinuousMonoidHom M N；hf : forall x y, Inseparable x y -> f x = f y；x :
 M。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
