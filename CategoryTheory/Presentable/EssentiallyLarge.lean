...
include hP in
/--
lemma `essentiallyLarge_top` / 引理 `essentiallyLarge_top`
...
variable (C κ) in
/--
lemma `HasCardinalFilteredGenerator.exists_equivalence` / 引理 `HasCardinalFilteredGenerator.exists_equivalence`
...
Depends on / 依赖: HasCardinalFilteredGenerator, HasCardinalFilteredGenerator.exists_generator, essentiallyLarge_top, exists_equivalence_iff_of_locallySmall, exists_generator, hP.essentiallyLarge_top
-/
lemma HasCardinalFilteredGenerator.exists_equivalence
    [HasCardinalFilteredGenerator C κ] :
    exists (J : Type (w + 1)) (_ : Category.{w} J), Nonempty (C ≌ J) := by
  rw [exists_equivalence_iff_of_locallySmall]
  obtain ⟨P, _, hP⟩ := HasCardinalFilteredGenerator.exists_generator C κ
  exact hP.essentiallyLarge_top
