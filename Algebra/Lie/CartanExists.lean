...
/--
Definition of `lieCharpoly` / `lieCharpoly` 的定义

English:
definition lieCharpoly
  signature: : Polynomial R[X]
  body: letI bL := chooseBasis R L
...
Depends on / 依赖: LieHom, LieHom.toLinearMap, MvPolynomial, MvPolynomial.aeval, RingHomClass, RingHomClass.toRingHom, bL.repr, chooseBasis, polyCharpoly, toLinearMap, toRingHom
-/
...
/--
lemma `lieCharpoly_monic` / 引理 `lieCharpoly_monic`

English:
lemma lieCharpoly_monic
  statement: (lieCharpoly R M x y).Monic
  proof: (polyCharpoly_monic _ _).map _

中文:
引理 lieCharpoly_monic
  结论: (lieCharpoly R M x y).Monic
  证明: (polyCharpoly_monic _ _).map _

Depends on / 依赖: polyCharpoly_monic
-/
...
set_option backward.privateInPublic.warn false in
/--
lemma `lieCharpoly_natDegree` / 引理 `lieCharpoly_natDegree`

English:
lemma lieCharpoly_natDegree
  given: [Nontrivial R]
  statement: (lieCharpoly R M x y).natDegree = finrank R M
  proof: by
...
Depends on / 依赖: lieCharpoly, natDegree_map, polyCharpoly_monic, polyCharpoly_natDegree
-/
...
variable {R} in
/--
lemma `lieCharpoly_map_eval` / 引理 `lieCharpoly_map_eval`

English:
lemma lieCharpoly_map_eval
  given: (r : R)
  proof: by
  rw [lieCharpoly]; rw [map_map]
  ...
  simp_rw [← coe_aeval_eq_evalRingHom, ← AlgHom.comp_toRingHom, MvPolynomial.comp_aeval,
    map_add, map_mul, aeval_C, Algeb

中文:
引理 lieCharpoly_map_eval
  条件: (r : R)
  证明: by
  rw [lieCharpoly]; rw [map_map]
  set b := chooseBasis R L
...
Depends on / 依赖: AlgHom, AlgHom.comp_toRingHom, Algebra, Algebra.algebraMap_self, LieHom, LieHom.coe_toLinearMap, MvPolynomial, MvPolynomial.coe_aeval_eq_eval, MvPolynomial.comp_aeval, RingHom, RingHom.id_apply, aeval_C, aeval_X, algebraMap_self, b.repr, chooseBasis, coe_aeval_eq_eval, coe_aeval_eq_evalRingHom, coe_toLinearMap, comp_aeval
-/
...
set_option backward.privateInPublic.warn false in
/--
lemma `lieCharpoly_coeff_natDegree` / 引理 `lieCharpoly_coeff_natDegree`

English:
lemma lieCharpoly_coeff_natDegree
  given: [Nontrivial R] (i j : Nat) (hij : i + j = finrank R M)
  proof: by
...
Depends on / 依赖: MvPolynomial, MvPolynomial.aeval_natDegree_le, aeval_natDegree_le, chooseBasis, coeff_map, lieCharpoly, mul_one, natDegree_linear_le, polyCharpoly_coeff_isHomogeneous, totalDegree_le
-/
lemma lieCharpoly_coeff_natDegree [Nontrivial R] (i j : Nat) (hij : i + j = finrank R M) :
...
/--
lemma `engel_isBot_of_isMin` / 引理 `engel_isBot_of_isMin`

English:
lemma engel_isBot_of_isMin
  statement: (hLK : finrank K L <= #K) (U : LieSubalgebra K L)
  proof: by
  rcases E with ⟨_, x, hxU, rfl⟩
  rintro ⟨_, y, hyU, rfl⟩
  -- It will be useful to repackage the Engel subalgebras
  set Ex : {engel K x | x in U} := ⟨engel K x, x, hxU, rfl⟩
  set Ey : {engel K y | y in U} := ⟨engel K y, y, hyU, rfl⟩
  replace hUle : U <= Ex := hUle
  replace hmin : forall E, 
...
lemma engel_isBot_of_isMin (hLK : finrank K L <= #K) (U : LieSubalgebra K L)
    (E : {engel K x | x in U}) (hUle : U <= E) (hmin : IsMin E) :
    IsBot E := by
  rcases E with ⟨_, x, hxU, rfl⟩
  rintro ⟨_, y, hyU, rfl⟩
  -- It will be useful to repackage the Engel subalgebras
  ...
  set Ey : {engel K y | y in U} := ⟨engel K y, y, hyU, rfl⟩
  replace hUle : U <= Ex := hUle
  replace hmin : forall E, E <= Ex -> Ex <= E := @hmin
  ...
  suffices forall i < r, χ.coeff i = 0 by
  ...
  · -- `The polynomial `coeff χ 0` is zero if it evaluates to zero on all elements of `K`,
    ...
    rw [← coe_evalRingHom]; rw [← coeff_map]; rw [lieCharpoly_map_eval]; rw [← constantCoeff_apply]; rw [LinearMap.charpoly_constantCoeff_eq_zero_iff]
  ...
  have hψ : constantCoeff ψ != 0 := by
    ...
    obtain ⟨z, hz0, hxz⟩ : exists z : Q, z != 0 ∧ ⁅x', z⁆ = 0 := by
      ...
      rw [constantCoeff_apply]; rw [← coeff_map]; rw [lieCharpoly_map_eval]; rw [← constantCoeff_apply]; rw [map_zero]; rw [LinearMap.charpoly_constantCoeff_eq_zero_iff] at H
    ...
    have : ⁅x, z⁆ in E := by rwa [← LieSubmodule.Quotient.mk_eq_zero']
  ...
  obtain ⟨s, hs, hsψ⟩ : exists s : Finset K, r <= s.card ∧ forall α in s, (constantCoeff ψ).eval α != 0 := by
    classical
  ...
  rw [← coe_evalRingHom]; rw [← coeff_map]; rw [lieCharpoly_map_eval]; rw [(LinearMap.charpoly_eq_X_pow_iff _).mpr]; rw [coeff_X_pow]; rw [if_neg hi.ne]
  ...
  have hz' : exists n : Nat, (toEnd K U Q v ^ n) z' = 0 := by
  ...
  obtain hn₀ | ⟨k, hk⟩ : n = 0 ∨ exists k, n = k + 1 := by cases n <;> simp
  ...
  rw [← coe_evalRingHom]; rw [constantCoeff_apply]; rw [← coeff_map]; rw [lieCharpoly_map_eval]; rw [← constantCoeff_apply]; rw [ne_eq]; rw [LinearMap.charpoly_constantCoeff_eq_zero_iff] at hsψ
...
/--
lemma `exists_isCartanSubalgebra_engel_of_finrank_le_card` / 引理 `exists_isCartanSubalgebra_engel_of_finrank_le_card`

English:
lemma exists_isCartanSubalgebra_engel_of_finrank_le_card
  given: (h : finrank K L <= #K)
  proof: by
  ...
  intro y hy
  set Ex : {engel K z | z in engel K x} := ⟨engel K x, x, self_mem_engel _ _, rfl⟩
...
Depends on / 依赖: LieSubalgebra, LieSubalgebra.toSubmodule_in, engel_isBot_of_isMin, exists_isRegular_of_finrank_le_card, finrank, isNilpotent_of_forall_le_engel, le_rfl, normalizer_engel, self_mem_engel, this.ge, toSubmodule_in
-/
lemma exists_isCartanSubalgebra_engel_of_finrank_le_card (h : finrank K L <= #K) :
    exists x : L, IsCartanSubalgebra (engel K x) := by
  obtain ⟨x, hx⟩ := exists_isRegular_of_finrank_le_card K L h
  use x
  refine ⟨?_, normalizer_engel _ _⟩
  apply isNilpotent_of_forall_le_engel
...
/--
lemma `exists_isCartanSubalgebra_engel` / 引理 `exists_isCartanSubalgebra_engel`

English:
lemma exists_isCartanSubalgebra_engel
  given: [Infinite K]
  proof: by
...
Depends on / 依赖: Cardinal, Cardinal.infinite_iff.mp, Infinite, exists_isCartanSubalgebra_engel_of_finrank_le_card, infinite_iff, natCast_le_aleph0, natCast_le_aleph0.trans
-/
