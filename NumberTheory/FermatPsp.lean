...
/--
Definition of `ProbablePrime` / `ProbablePrime` 的定义

English:
definition ProbablePrime
  signature: (n b : Nat)
  body: n ∣ b ^ (n - 1) - 1
...
定义 ProbablePrime
  ...
  定义体: n ∣ b ^ (n - 1) - 1
-/
def ProbablePrime (n b : Nat) : Prop :=
  n ∣ b ^ (n - 1) - 1

/--
Definition of `FermatPsp` / `FermatPsp` 的定义

English:
definition FermatPsp
  signature: (n b : Nat)
  body: ProbablePrime n b ∧ ¬n.Prime ∧ 1 < n

中文:
定义 FermatPsp
  签名: (n b : 自然数)
  定义体: ProbablePrime n b ∧ ¬n.Prime ∧ 1 < n

Depends on / 依赖: ProbablePrime, n.Prime
-/
def FermatPsp (n b : Nat) : Prop :=
  ProbablePrime n b ∧ ¬n.Prime ∧ 1 < n

/--
Instance `decidableProbablePrime` / 实例 `decidableProbablePrime`

English:
instance decidableProbablePrime
  signature: (n b : Nat)
  body: Nat.decidable_dvd _ _

中文:
实例 decidableProbablePrime
  签名: (n b : 自然数)
  定义体: Nat.decidable_dvd _ _

Depends on / 依赖: Nat.decidable_dvd, decidable_dvd
-/
instance decidableProbablePrime (n b : Nat) : Decidable (ProbablePrime n b) :=
  Nat.decidable_dvd _ _

/--
Instance `decidablePsp` / 实例 `decidablePsp`

English:
instance decidablePsp
  signature: (n b : Nat)
...
-/
instance decidablePsp (n b : Nat) : Decidable (FermatPsp n b) :=
  inferInstanceAs (Decidable (_ ∧ _))

/--
theorem `coprime_of_probablePrime` / 定理 `coprime_of_probablePrime`

English:
theorem coprime_of_probablePrime
  given: {n b : Nat} (h : ProbablePrime n b) (h₁ : 1 <= n) (h₂ : 1 <= b)
  proof: by
  by_cases h₃ : 2 <= n
  · -- To prove that `n` is coprime with `b`, we need to show that for all prime factors of `n`,
    ...
    -- If `k` is a prime number that divides both `n` and `b`, then we know that `n = m * k

中文:
定理 coprime_of_probablePrime
  条件: {n b : 自然数} (h : ProbablePrime n b) (h₁ : 1 <= n) (h₂ : 1 <= b)
  证明: by
  by_cases h₃ : 2 <= n
  · -- To prove that `n` is coprime with `b`, we need to show that for all prime factors of `n`,
    -- we can derive a contradiction if `n` divides `b`.
...
Depends on / 依赖: coprime, factors
-/
theorem coprime_of_probablePrime {n b : Nat} (h : ProbablePrime n b) (h₁ : 1 <= n) (h₂ : 1 <= b) :
    Nat.Coprime n b := by
  by_cases h₃ : 2 <= n
  · -- To prove that `n` is coprime with `b`, we need to show that for all prime factors of `n`,
    -- we can derive a contradiction if `n` divides `b`.
...
/--
theorem `probablePrime_iff_modEq` / 定理 `probablePrime_iff_modEq`

English:
theorem probablePrime_iff_modEq
  given: (n : Nat) {b : Nat} (h : 1 <= b)
  proof: by
...
中文:
定理 probablePrime_iff_modEq
  条件: (n : 自然数) {b : 自然数} (h : 1 <= b)
  证明: by
  have : 1 <= b ^ (n - 1) := one_le_pow₀ h
  -- For exact mod_cast
  rw [Nat.ModEq.comm]
  ...
  · intro h₁
    exact mod_cast Nat.ModEq.dvd h₁
-/
theorem probablePrime_iff_modEq (n : Nat) {b : Nat} (h : 1 <= b) :
...
/--
theorem `coprime_of_fermatPsp` / 定理 `coprime_of_fermatPsp`

English:
theorem coprime_of_fermatPsp
  given: {n b : Nat} (h : FermatPsp n b) (h₁ : 1 <= b)
  statement: Nat.Coprime n b
  proof: by
  rcases h with ⟨hp, _, hn₂⟩
  exact coprime_of_probablePrime hp (by lia) h₁

中文:
定理 coprime_of_fermatPsp
  条件: {n b : 自然数} (h : FermatPsp n b) (h₁ : 1 <= b)
  结论: 自然数.Coprime n b
  证明: by
  rcases h with ⟨hp, _, hn₂⟩
  exact coprime_of_probablePrime hp (by lia) h₁

Depends on / 依赖: coprime_of_probablePrime
-/
theorem coprime_of_fermatPsp {n b : Nat} (h : FermatPsp n b) (h₁ : 1 <= b) : Nat.Coprime n b := by
  rcases h with ⟨hp, _, hn₂⟩
  exact coprime_of_probablePrime hp (by lia) h₁

/--
theorem `fermatPsp_base_one` / 定理 `fermatPsp_base_one`

English:
theorem fermatPsp_base_one
  given: {n : Nat} (h₁ : 1 < n) (h₂ : ¬n.Prime)
  statement: FermatPsp n 1
  proof: by
...
-/
theorem fermatPsp_base_one {n : Nat} (h₁ : 1 < n) (h₂ : ¬n.Prime) : FermatPsp n 1 := by
  refine ⟨show n ∣ 1 ^ (n - 1) - 1 from ?_, h₂, h₁⟩
  exact show 0 = 1 ^ (n - 1) - 1 by simp ▸ dvd_zero n
...
/--
theorem `a_id_helper` / 定理 `a_id_helper`

English:
theorem a_id_helper
  given: {a b : Nat} (ha : 2 <= a) (hb : 2 < b)
  statement: b < (a ^ b - 1) / (a - 1)
  proof: by
  rw [← Nat.geomSum_eq ha]
  calc
    b = ∑ _ in Finset.range b, (1 : Nat) := by simp
    _ < _ := by
...
定理 a_id_helper
  ...
  calc
    ...
    _ < _ := by
      ...
      exact ⟨1, Finset.mem_range.mpr (by lia), by simpa using! ha⟩
-/
private theorem a_id_helper {a b : Nat} (ha : 2 <= a) (hb : 2 < b) : b < (a ^ b - 1) / (a - 1) := by
  rw [← Nat.geomSum_eq ha]
  calc
    b = ∑ _ in Finset.range b, (1 : Nat) := by simp
    _ < _ := by
...
/--
theorem `b_id_helper` / 定理 `b_id_helper`

English:
theorem b_id_helper
  given: {a b : Nat} (ha : 2 <= a) (hb : 2 < b)
  statement: 2 <= (a ^ b + 1) / (a + 1)
  proof: by
...
定理 b_id_helper
  ...
  calc
    ...
    _ <= a ^ b := pow_right_mono₀ (Nat.le_of_succ_le ha) hb
-/
private theorem b_id_helper {a b : Nat} (ha : 2 <= a) (hb : 2 < b) : 2 <= (a ^ b + 1) / (a + 1) := by
  rw [Nat.le_div_iff_mul_le (Nat.zero_lt_succ _)]
  apply Nat.succ_le_succ
  calc
    2 * a + 1 <= a ^ 2 * a := by nlinarith
...
/--
theorem `AB_id_helper` / 定理 `AB_id_helper`

English:
theorem AB_id_helper
  given: (b p : Nat) (_ : 2 <= b) (hp : Odd p)
  proof: by
...
定理 AB_id_helper
  ...
  ring_nf
-/
private theorem AB_id_helper (b p : Nat) (_ : 2 <= b) (hp : Odd p) :
...
/--
theorem `bp_helper` / 定理 `bp_helper`

English:
theorem bp_helper
  given: {b p : Nat} (hb : 0 < b) (hp : 1 <= p)
  proof: have hi_bsquared : 1 <= b ^ 2 := Nat.one_le_pow _ _ hb
  calc
...
中文:
定理 bp_helper
  条件: {b p : 自然数} (hb : 0 < b) (hp : 1 <= p)
  证明: have hi_bsquared : 1 <= b ^ 2 := Nat.one_le_pow _ _ hb
  calc
    b ^ (2 * p) - 1 - (b ^ 2 - 1) = b ^ (2 * p) - (1 + (b ^ 2 - 1)) := by rw [Nat.sub_sub]
    _ = b ^ (2 * p) - (1 + b ^ 2 - 1) := by rw [Nat.add_sub_assoc hi_bsquared]
    _ = b ^ (2 * p) - b ^ 2 := by rw [Nat.add_sub_cancel_left]
    _
-/
private theorem bp_helper {b p : Nat} (hb : 0 < b) (hp : 1 <= p) :
...
/--
Definition of `psp_from_prime` / `psp_from_prime` 的定义

English:
definition psp_from_prime
  signature: (b : Nat) (p : Nat)
  body: (b ^ p - 1) / (b - 1) * ((b ^ p + 1) / (b + 1))

中文:
定义 psp_from_prime
  签名: (b : 自然数) (p : 自然数)
  定义体: (b ^ p - 1) / (b - 1) * ((b ^ p + 1) / (b + 1))
-/
private def psp_from_prime (b : Nat) (p : Nat) : Nat :=
  (b ^ p - 1) / (b - 1) * ((b ^ p + 1) / (b + 1))

/--
theorem `psp_from_prime_psp` / 定理 `psp_from_prime_psp`

English:
theorem psp_from_prime_psp
  statement: {b : Nat} (b_ge_two : 2 <= b) {p : Nat} (p_prime : p.Prime)
  proof: by
...
定理 psp_from_prime_psp
  ...
  have hi_bsquared : 0 < 
-/
private theorem psp_from_prime_psp {b : Nat} (b_ge_two : 2 <= b) {p : Nat} (p_prime : p.Prime)
  ...
  have ha₁ : (b ^ 2 - 1) * (A * B - 1) = b * (b ^ (p - 1) - 1) * (b ^ p + b) := by
    ...
    rw [← Nat.mul_sub_right_distrib]; rw [mul_comm] at AB_id
    rw [AB_id]
  ...
  have ha₃ : p ∣ b ^ (p - 1) - 1 := by
    ...
    have : IsCoprime (b : Int) ↑p := this.symm.isCoprime
  ...
  have ha₇ : A * B ∣ b ^ (2 * p) - 1 := by
    ...
    have : A * B * (b ^ 2 - 1) = (b ^ (2 * p) - 1) / (b ^ 2 - 1) * (b ^ 2 - 1) :=
      congr_arg (fun x : Nat => x * (b ^ 2 - 1)) AB_id
...
/--
theorem `psp_from_prime_gt_p` / 定理 `psp_from_prime_gt_p`

English:
theorem psp_from_prime_gt_p
  given: {b : Nat} (b_ge_two : 2 <= b) {p : Nat} (p_gt_two : 2 < p)
  proof: by
  unfold psp_from_prime
  set A := (b ^ p - 1) / (b - 1)
  set B := (b ^ p + 1) / (b + 1)
...
中文:
定理 psp_from_prime_gt_p
  条件: {b : 自然数} (b_ge_two : 2 <= b) {p : 自然数} (p_gt_two : 2 < p)
  证明: by
  unfold psp_from_prime
  set A := (b ^ p - 1) / (b - 1)
  set B := (b ^ p + 1) / (b + 1)
  ...
  exact hA.trans_le (Nat.le_mul_of_pos_right _ hB)
-/
private theorem psp_from_prime_gt_p {b : Nat} (b_ge_two : 2 <= b) {p : Nat} (p_gt_two : 2 < p) :
...
/--
theorem `exists_infinite_pseudoprimes` / 定理 `exists_infinite_pseudoprimes`

English:
theorem exists_infinite_pseudoprimes
  given: {b : Nat} (h : 1 <= b) (m : Nat)
  proof: by
...
Depends on / 依赖: b_ge_two
-/
theorem exists_infinite_pseudoprimes {b : Nat} (h : 1 <= b) (m : Nat) :
    exists n : Nat, FermatPsp n b ∧ m <= n := by
  by_cases b_ge_two : 2 <= b
  -- If `2 ≤ b`, then because there exist infinite prime numbers, there is a prime number p with
  -- `m ≤ p` and `¬p ∣ b*(b^2 - 1)`. We pick a prime number `b*(b^2 - 1) + 1 + m ≤ p` because we
...
/--
theorem `frequently_atTop_fermatPsp` / 定理 `frequently_atTop_fermatPsp`

English:
theorem frequently_atTop_fermatPsp
  given: {b : Nat} (h : 1 <= b)
  statement: existsᶠ n in Filter.atTop, FermatPsp n b
  proof: by
...
定理 frequently_atTop_fermatPsp
  ...
  exact ⟨p, hp.2, hp.1⟩
-/
theorem frequently_atTop_fermatPsp {b : Nat} (h : 1 <= b) : existsᶠ n in Filter.atTop, FermatPsp n b := by
  -- Based on the proof of `Nat.frequently_atTop_modEq_one`
  refine Filter.frequently_atTop.2 fun n => ?_
  obtain ⟨p, hp⟩ := exists_infinite_pseudoprimes h n
  exact ⟨p, hp.2, hp.1⟩
...
/--
theorem `infinite_setOfPred_pseudoprimes` / 定理 `infinite_setOfPred_pseudoprimes`

English:
theorem infinite_setOfPred_pseudoprimes
  given: {b : Nat} (h : 1 <= b)
  proof: Nat.frequently_atTop_iff_infinite.mp (frequently_atTop_fermatPsp h)

@[deprecated (since := "2026-07-09")]
alias infinite_setOf_pseudoprimes := infinite_setOfPred_pseudoprimes

中文:
定理 infinite_setOfPred_pseudoprimes
  条件: {b : 自然数} (h : 1 <= b)
  证明: Nat.frequently_atTop_iff_infinite.mp (frequently_atTop_fermatPsp h)

@[deprecated (since := "2026-07-09")]
alias infinite_setOf_pseudoprimes := infinite_setOfPred_pseudoprimes

Depends on / 依赖: Nat.frequently_atTop_iff_infinite.mp, frequently_atTop_fermatPsp, frequently_atTop_iff_infinite
-/
theorem infinite_setOfPred_pseudoprimes {b : Nat} (h : 1 <= b) :
    Set.Infinite { n : Nat | FermatPsp n b } :=
