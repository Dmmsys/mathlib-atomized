...
public theorem ContinuousAlgEquiv.eq_continuousLinearEquivConjContinuousAlgEquiv
    ...
    exists U : V ≃L[𝕜] W, f = U.conjContinuousAlgEquiv := by
...
· exact ⟨{ toLinearEquiv := 0 }, ext Subsingleton.allEq _ _⟩
  ...
  obtain ⟨z, hz⟩ : exists z : W, ¬ f (smulRight v u) z = (0 : W ->L[𝕜] W) z := by
...
/--
Definition of `auxContinuousLinearEquiv` / `auxContinuousLinearEquiv` 的定义

English:
abbreviation auxContinuousLinearEquiv
  signature: :
  body: (α' • e.toContinuousLinearMap).toLinearMap
...
缩写 auxContinuousLinearEquiv
  签名: :
  定义体: (α' • e.toContinuousLinearMap).toLinearMap
  invFun := (α' • e.toContinuousLinearMap.adjoint).toLinearMap
  left_inv x := by
    have := by simpa using congr($he x)
    simp [smul_smul, hα2, this, hα, ← mul_assoc]
...
/--
theorem `coe_auxContinuousLinearEquiv` / 定理 `coe_auxContinuousLinearEquiv`

English:
theorem coe_auxContinuousLinearEquiv
  proof: rfl

中文:
定理 coe_auxContinuousLinearEquiv
  证明: rfl
-/
...
/--
Definition of `auxIsometry` / `auxIsometry` 的定义

English:
abbreviation auxIsometry
  signature: (hαa : starRingEnd 𝕜 α' = α')
...
noncomputable abbrev auxIsometry (hαa : starRingEnd 𝕜 α' = α') :
    V ≃ₗᵢ[𝕜] W where
.toLinearEquiv __ := auxContinuousLinearEquiv e hα hα2 he he'
  norm_map' := by
    rw [ContinuousLinearEquiv.coe_toLinearEquiv]; rw [← ContinuousLinearEquiv.coe_coe]; rw [norm_map_iff_adjoint_comp_self]; rw [coe_auxContinuousLinearEquiv]; rw [MulActionSemiHomClass.map_smulₛₗ]
...
/--
theorem `coe_auxIsometry` / 定理 `coe_auxIsometry`

English:
theorem coe_auxIsometry
  given: (hαa : starRingEnd 𝕜 α' = α')
  proof: rfl

中文:
定理 coe_auxIsometry
  条件: (hαa : starRingEnd 𝕜 α' = α')
  证明: rfl
-/
...
/--
theorem `coe_symm_auxIsometry` / 定理 `coe_symm_auxIsometry`

English:
theorem coe_symm_auxIsometry
  given: (hαa : starRingEnd 𝕜 α' = α')
  proof: by
  ext y
...
public theorem StarAlgEquiv.eq_linearIsometryEquivConjStarAlgEquiv
    ...
    exists U : V ≃ₗᵢ[𝕜] W, f = U.conjStarAlgEquiv := by
...
.symm have (x : V ->L[𝕜] V) : adjoint (f x) = f (adjoint x) := map_star _ _
...
have hα_pos := RCLike.ofReal_pos.mp hα_re ▸ (lt_of_le_of_ne' hα_nonneg hα_ne_zero)
  ...
  set β := (((RCLike.re α : Real) ^ (-(1 / 2 : Real)) : Real) : 𝕜)
  have hβ : β * β = α⁻¹ := by
    rw [hα_re]
