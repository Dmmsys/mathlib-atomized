...
/--
theorem `tendsto_div_of_monotone_of_exists_subseq_tendsto_div` / 定理 `tendsto_div_of_monotone_of_exists_subseq_tendsto_div`

English:
theorem tendsto_div_of_monotone_of_exists_subseq_tendsto_div
  statement: (u : Nat -> Real) (l : Real)
  proof: by
...
定理 tendsto_div_of_monotone_of_存在_subseq_tendsto_div
  ...
  /- To check the result up to some `ε > 0`, we use a sequence `c` for which the ratio
    ...
    and fr
-/
theorem tendsto_div_of_monotone_of_exists_subseq_tendsto_div (u : Nat -> Real) (l : Real)
    (hmono : Monotone u)
    (hlim : forall a : Real, 1 < a -> exists c : Nat -> Nat, (forallᶠ n in atTop, (c (n + 1) : Real) <= a * c n) ∧
  ...
  have lnonneg : 0 <= l := by
    ...
    gcongr
    exact hmono zero_le
  have A : forall ε : Real, 0 < ε -> forallᶠ n in atTop, u n - n * l <= ε * (1 + ε + l) * n := by
    intro ε εpos
    rcases hlim (1 + ε) ((lt_add_iff_pos_right _).2 εpos) with ⟨c, cgrowth, ctop, clim⟩
    have L : forallᶠ n in atTop, u (c n) - c n * l <= ε * c n := by
      rw [← tendsto_sub_nhds_zero_iff]; rw [← Asymptotics.isLittleO_one_iff Real]; rw [Asymptotics.isLittleO_iff] at clim
      ...
      calc
        ...
        _ <= ε * c n := by
          ...
          simpa using hn
    obtain ⟨a, ha⟩ :
      exists a : Nat, forall b : Nat, a <= b -> (c (b + 1) : Real) <= (1 + ε) * c b ∧ u (c b) - c b * l <= ε * c b :=
    ...
    have IcN : (c N : Real) <= (1 + ε) * c (N - 1) := by
  ...
  have B : forall ε : Real, 0 < ε -> forallᶠ n : Nat in atTop, (n : Real) * l - u n <= ε * (1 + l) * n := by
    intro ε εpos
    rcases hlim (1 + ε) ((lt_add_iff_pos_right _).2 εpos) with ⟨c, cgrowth, ctop, clim⟩
    have L : forallᶠ n : Nat in atTop, (c n : Real) * l - u (c n) <= ε * c n := by
      rw [← tendsto_sub_nhds_zero_iff]; rw [← Asymptotics.isLittleO_one_iff Real]; rw [Asymptotics.isLittleO_iff] at clim
      filter_upwards [clim εpos, ctop (Ioi_mem_atTop 0)] with n hn cnpos'
      have cnpos : 0 < c n := cnpos'
      calc
        (c n : Real) * l - u (c n) = -(u (c n) / c n - l) * c n := by field
        _ <= ε * c n := by
          gcongr
    ...
    obtain ⟨a, ha⟩ :
      exists a : Nat,
        forall b : Nat, a <= b -> (c (b + 1) : Real) <= (1 + ε) * c b ∧ (c b : Real) * l - u (c b) <= ε * c b :=
      eventually_atTop.1 (cgrowth.and L)
    let M := ((Finset.range (a + 1)).image fun i => c i).max' (by simp)
    filter_upwards [Ici_mem_atTop M] with n hn
    have exN : exists N, n < c N := by
  ...
  · obtain ⟨ε, hε, εpos⟩ : exists ε : Real, d + ε * (1 + l) < l ∧ 0 < ε := by
    ...
    calc
      d < (n : Real)⁻¹ * n * (l - ε * (1 + l)) := by
        rw [inv_mul_cancel₀]; rw [one_mul]
        · linarith only [hε]
        · exact Nat.cast_ne_zero.2 (ne_of_gt npos)
      _ = (n : Real)⁻¹ * (n * l - ε * (1 + l) * n) := by ring
      _ <= (n : Real)⁻¹ * u n := by gcongr; linarith only [hn]
  · obtain ⟨ε, hε, εpos⟩ : exists ε : Real, l + ε * (1 + ε + l) < d ∧ 0 < ε := by
...
/--
theorem `tendsto_div_of_monotone_of_tendsto_div_floor_pow` / 定理 `tendsto_div_of_monotone_of_tendsto_div_floor_pow`

English:
theorem tendsto_div_of_monotone_of_tendsto_div_floor_pow
  statement: (u : Nat -> Real) (l : Real) (hmono : Monotone u)
  proof: by
  apply tendsto_div_of_monotone_of_exists_subseq_tendsto_div u l hmono
  intro a ha
  obtain ⟨k, hk⟩ : exists k, c k < a := ((tendsto_order.1 clim).2 a ha).exists
  refine
    ⟨fun n => ⌊c k ^ n⌋₊, ?_,
      (tendsto_nat_floor_atTop (α := Real)).comp (tendsto_pow_atTop_atTop_of_one_lt (cone k)), 
...
Depends on / 依赖: Nat.one_le_cast, Nat.one_le_floor_iff, Tendsto, one_le_cast, one_le_floor_iff, tendsto_div_of_monotone_of_exists_subseq_tendsto_div, tendsto_nat_floor_atTop, tendsto_order, tendsto_pow_atTop_atTop_of_one_lt, trans_le, zero_lt_one, zero_lt_one.trans_le
-/
theorem tendsto_div_of_monotone_of_tendsto_div_floor_pow (u : Nat -> Real) (l : Real) (hmono : Monotone u)
    (c : Nat -> Real) (cone : forall k, 1 < c k) (clim : Tendsto c atTop (𝓝 1))
    (hc : forall k, Tendsto (fun n : Nat => u ⌊c k ^ n⌋₊ / ⌊c k ^ n⌋₊) atTop (𝓝 l)) :
    Tendsto (fun n => u n / n) atTop (𝓝 l) := by
  apply tendsto_div_of_monotone_of_exists_subseq_tendsto_div u l hmono
  intro a ha
  obtain ⟨k, hk⟩ : exists k, c k < a := ((tendsto_order.1 clim).2 a ha).exists
  refine
    ⟨fun n => ⌊c k ^ n⌋₊, ?_,
      (tendsto_nat_floor_atTop (α := Real)).comp (tendsto_pow_atTop_atTop_of_one_lt (cone k)), hc k⟩
  have H : forall n : Nat, (0 : Real) < ⌊c k ^ n⌋₊ := by
    intro n
    refine zero_lt_one.trans_le ?_
    simp only [Nat.one_le_cast, Nat.one_le_floor_iff, one_le_pow₀ (cone k).le]
  have A :
    Tendsto (fun n : Nat => (⌊c k ^ (n + 1)⌋₊ : Real) / c k ^ (n + 1) * c k / (⌊c k ^ n⌋₊ / c k ^ n))
      atTop (𝓝 (1 * c k / 1)) := by
  ...
  have B : Tendsto (fun n : Nat => (⌊c k ^ (n + 1)⌋₊ : Real) / ⌊c k ^ n⌋₊) atTop (𝓝 (c k)) := by
...
/--
theorem `sum_div_pow_sq_le_div_sq` / 定理 `sum_div_pow_sq_le_div_sq`

English:
theorem sum_div_pow_sq_le_div_sq
  given: (N : Nat) {j : Real} (hj : 0 < j) {c : Real} (hc : 1 < c)
  proof: by
  have cpos : 0 < c := zero_lt_one.trans hc
  have A : (0 : Real) < c⁻¹ ^ 2 := sq_pos_of_pos (inv_pos.2 cpos)
  have B : c ^ 2 * ((1 : Real) - c⁻¹ ^ 2)⁻¹ <= c ^ 3 * (c - 1)⁻¹ := by
    rw [← div_eq_mul_inv]; rw [← div_eq_mul_inv]; rw [div_le_div_iff₀ _ (sub_pos.2 hc)]
    swap
    · exact sub_pos

中文:
定理 sum_div_pow_sq_le_div_sq
  条件: (N : 自然数) {j : 实数} (hj : 0 < j) {c : 实数} (hc : 1 < c)
  证明: by
  have cpos : 0 < c := zero_lt_one.trans hc
  have A : (0 : Real) < c⁻¹ ^ 2 := sq_pos_of_pos (inv_pos.2 cpos)
  have B : c ^ 2 * ((1 : Real) - c⁻¹ ^ 2)⁻¹ <= c ^ 3 * (c - 1)⁻¹ := by
...
Depends on / 依赖: cpos.le, div_eq_mul_inv, inv_nonneg, inv_pos, inv_pow, mul_assoc, mul_comm, mul_one, mul_sub, sq_pos_of_pos, sub_le_sub_iff_left, sub_pos, two_ne_zero, zero_lt_one, zero_lt_one.trans
-/
theorem sum_div_pow_sq_le_div_sq (N : Nat) {j : Real} (hj : 0 < j) {c : Real} (hc : 1 < c) :
    (∑ i in range N with j < c ^ i, (1 : Real) / (c ^ i) ^ 2) <= c ^ 3 * (c - 1)⁻¹ / j ^ 2 := by
  have cpos : 0 < c := zero_lt_one.trans hc
  have A : (0 : Real) < c⁻¹ ^ 2 := sq_pos_of_pos (inv_pos.2 cpos)
  have B : c ^ 2 * ((1 : Real) - c⁻¹ ^ 2)⁻¹ <= c ^ 3 * (c - 1)⁻¹ := by
    rw [← div_eq_mul_inv]; rw [← div_eq_mul_inv]; rw [div_le_div_iff₀ _ (sub_pos.2 hc)]
  ...
  calc
    (∑ i in range N with j < c ^ i, (1 : Real) / (c ^ i) ^ 2) <=
        ∑ i in Ico ⌊Real.log j / Real.log c⌋₊ N, (1 : Real) / (c ^ i) ^ 2 := by
      gcongr
      intro i hi
    ...
    _ <= (c⁻¹ ^ 2) ^ ⌊Real.log j / Real.log c⌋₊ / ((1 : Real) - c⁻¹ ^ 2) :=
      geom_sum_Ico_le_of_lt_one (sq_nonneg _) C
    _ <= (c⁻¹ ^ 2) ^ (Real.log j / Real.log c - 1) / ((1 : Real) - c⁻¹ ^ 2) := by
      gcongr
      rw [← Real.rpow_natCast]
      exact Real.rpow_le_rpow_of_exponent_ge A C.le (Nat.sub_one_lt_floor _).le
    _ = c ^ 2 * ((1 : Real) - c⁻¹ ^ 2)⁻¹ / j ^ 2 := by
      have I : (c⁻¹ ^ 2) ^ (Real.log j / Real.log c) = (1 : Real) / j ^ 2 := by
...
/--
theorem `mul_pow_le_nat_floor_pow` / 定理 `mul_pow_le_nat_floor_pow`

English:
theorem mul_pow_le_nat_floor_pow
  given: {c : Real} (hc : 1 < c) (i : Nat)
  statement: (1 - c⁻¹) * c ^ i <= ⌊c ^ i⌋₊
  proof: by
  have cpos : 0 < c := zero_lt_one.trans hc
...
中文:
定理 mul_pow_le_nat_floor_pow
  条件: {c : 实数} (hc : 1 < c) (i : 自然数)
  结论: (1 - c⁻¹) * c ^ i <= ⌊c ^ i⌋₊
  证明: by
  have cpos : 0 < c := zero_lt_one.trans hc
  rcases eq_or_ne i 0 with (rfl | hi)
...
Depends on / 依赖: Nat.cast_one, Nat.floor_one, Nat.sub_one_lt_floor, cast_one, cpos.le, div_eq_mul_inv, eq_or_ne, floor_one, hc.le, inv_nonneg, mul_one, one_le_div, pow_one, pow_zero, sub_le_self_iff, sub_one_lt_floor, zero_lt_one, zero_lt_one.trans
-/
theorem mul_pow_le_nat_floor_pow {c : Real} (hc : 1 < c) (i : Nat) : (1 - c⁻¹) * c ^ i <= ⌊c ^ i⌋₊ := by
  have cpos : 0 < c := zero_lt_one.trans hc
  rcases eq_or_ne i 0 with (rfl | hi)
  · simp only [pow_zero, Nat.floor_one, Nat.cast_one, mul_one, sub_le_self_iff, inv_nonneg, cpos.le]
  calc
...
/--
theorem `sum_div_nat_floor_pow_sq_le_div_sq` / 定理 `sum_div_nat_floor_pow_sq_le_div_sq`
...
theorem sum_div_nat_floor_pow_sq_le_div_sq
  ...
  calc
    (∑ i in range N with j < ⌊c ^ i⌋₊, (1 : Real) / (⌊c ^ i⌋₊ : Real) ^ 2) <=
        ∑ i in range N with j < c ^ i, (1 : Real) / (⌊c ^ i⌋₊ : Real) ^ 2 := by
      gcongr with k hk; e

中文:
定理 sum_div_nat_floor_pow_sq_le_div_sq
  条件: (N : 自然数) {j : 实数} (hj : 0 < j) {c : 实数} (hc : 1 < c)
  证明: by
  have cpos : 0 < c := zero_lt_one.trans hc
  have A : 0 < 1 - c⁻¹ := sub_pos.2 (inv_lt_one_of_one_lt₀ hc)
...
Depends on / 依赖: Nat.floor_le, floor_le, mul_div_assoc, mul_one, rotate_left, sq_pos_of_po, sub_pos, zero_lt_one, zero_lt_one.trans
-/
theorem sum_div_nat_floor_pow_sq_le_div_sq (N : Nat) {j : Real} (hj : 0 < j) {c : Real} (hc : 1 < c) :
    (∑ i in range N with j < ⌊c ^ i⌋₊, (1 : Real) / (⌊c ^ i⌋₊ : Real) ^ 2) <=
      c ^ 5 * (c - 1)⁻¹ ^ 3 / j ^ 2 := by
  have cpos : 0 < c := zero_lt_one.trans hc
  have A : 0 < 1 - c⁻¹ := sub_pos.2 (inv_lt_one_of_one_lt₀ hc)
  calc
    ...
    _ <= ∑ i in range N with j < c ^ i, (1 - c⁻¹)⁻¹ ^ 2 * ((1 : Real) / (c ^ i) ^ 2) := by
      gcongr with i
