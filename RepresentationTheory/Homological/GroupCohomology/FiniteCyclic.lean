...
@[simps!]
/-
**Rep.FiniteCyclicGroup.homResolutionIso** 是 Mathlib 中的一个定义，位于命名空间 `Rep.FiniteCy
clicGroup`。
形式化陈述：homResolutionIso : (resolution k g hg).complex.linearYonedaObj k A ≅ modul
eCatCochainComplex A g
该定义给出了上述对象。
...
/-
**Rep.FiniteCyclicGroup.groupCohomologyIso** 是 Mathlib 中的一个缩写定义，位于命名空间 `Rep.Fini
teCyclicGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
nonzero and even. -/
/-
**Rep.FiniteCyclicGroup.groupCohomologyIsoEven** 是 Mathlib 中的一个定义，位于命名空间 `Rep.Fi
niteCyclicGroup`。
形式化陈述：groupCohomologyIsoEven (i : Nat) [h₀ : NeZero i] (hi : Even i) : groupCoho
mology A i ≅ (normHomCompSub A g).homology
参数：i : Nat；hi : Even i。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Rep.FiniteCyclicGroup.groupCohomology** 是 Mathlib 中的一个缩写定义，位于命名空间 `Rep.FiniteC
yclicGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
set_option backward.isDefEq.respectTransparency false in
/-
**Rep.FiniteCyclicGroup.groupCohomology** 是 Mathlib 中的一个引理，位于命名空间 `Rep.FiniteCyc
licGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
odd. -/
/-
**Rep.FiniteCyclicGroup.groupCohomologyIsoOdd** 是 Mathlib 中的一个定义，位于命名空间 `Rep.Fin
iteCyclicGroup`。
形式化陈述：groupCohomologyIsoOdd (i : Nat) (hi : Odd i) : groupCohomology A i ≅ (subC
ompNormHom A g).homology
参数：i : Nat；hi : Odd i。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
