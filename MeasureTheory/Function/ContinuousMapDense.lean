...
variable {E : Type*} [NormedAddCommGroup E] {μ : Measure α} {p : Real>=0∞}
...
/--
theorem `exists_continuous_eLpNorm_sub_le_of_closed` / 定理 `exists_continuous_eLpNorm_sub_le_of_closed`

English:
theorem exists_continuous_eLpNorm_sub_le_of_closed
  statement: [μ.OuterRegular] (hp : p != ∞) {s u : Set α}
  proof: by
  obtain ⟨η, η_pos, hη⟩ :
      exists η : Real>=0, 0 < η ∧ forall s : Set α, μ s <= η -> eLpNorm (s.indicator fun _x => c) p μ <= ε :=
    exists_eLpNorm_indicator_le hp c hε
  have ηpos : (0 : Real>=0∞) < η := ENNReal.coe_lt_coe.2 η_pos
...
Depends on / 依赖: ENNReal, ENNReal.coe_lt_coe, IsOpen, V_open, coe_lt_coe, eLpNorm, exists_eLpNorm_indicator_le, exists_isOpen_sdiff_lt, indicator, inter_subset, measurableSet, measure_mono, pos.ne, s.indicator, s_closed, s_closed.measurableSet.exists_isOpen_sdiff_lt, subset_inter, subseteq
-/
theorem exists_continuous_eLpNorm_sub_le_of_closed [μ.OuterRegular] (hp : p != ∞) {s u : Set α}
    (s_closed : IsClosed s) (u_open : IsOpen u) (hsu : s subseteq u) (hs : μ s != ∞) (c : E) {ε : Real>=0∞}
    (hε : ε != 0) :
    exists f : α -> E,
      Continuous f ∧
        eLpNorm (fun x => f x - s.indicator (fun _y => c) x) p μ <= ε ∧
          (forall x, ‖f x‖ <= ‖c‖) ∧ Function.support f subseteq u ∧ MemLp f p μ := by
  obtain ⟨η, η_pos, hη⟩ :
      exists η : Real>=0, 0 < η ∧ forall s : Set α, μ s <= η -> eLpNorm (s.indicator fun _x => c) p μ <= ε :=
    exists_eLpNorm_indicator_le hp c hε
  have ηpos : (0 : Real>=0∞) < η := ENNReal.coe_lt_coe.2 η_pos
  obtain ⟨V, sV, V_open, h'V, hV⟩ : exists (V : Set α), V ⊇ s ∧ IsOpen V ∧ μ V < ∞ ∧ μ (V \ s) < η :=
    s_closed.measurableSet.exists_isOpen_sdiff_lt hs ηpos.ne'
  let v := u inter V
  have hsv : s subseteq v := subset_inter hsu sV
  ...
  have g_norm : forall x, ‖g x‖ = g x := fun x => by rw [Real.norm_eq_abs, abs_of_nonneg (hg_range x).1]
  have gc_bd0 : forall x, ‖g x • c‖ <= ‖c‖ := by
    intro x
    simp only [norm_smul, g_norm x]
    apply mul_le_of_le_one_left (norm_nonneg _)
    exact (hg_range x).2
  have gc_bd :
      forall x, ‖g x • c - s.indicator (fun _x => c) x‖ <= ‖(v \ s).indicator (fun _x => c) x‖ := by
    intro x
    by_cases hv : x in v
  ...
  have gc_support : (Function.support fun x : α => g x • c) subseteq v := by
  ...
  have gc_mem : MemLp (fun x => g x • c) p μ := by
    ...
    have : eLpNorm (v.indicator fun _x => (1 : Real)) p μ < ⊤ :=
...
/--
theorem `MemLp.exists_hasCompactSupport_eLpNorm_sub_le` / 定理 `MemLp.exists_hasCompactSupport_eLpNorm_sub_le`
...
Depends on / 依赖: Continuous, HasCompactSupport, eLpNorm, g_cont, g_mem, g_support
-/
theorem MemLp.exists_hasCompactSupport_eLpNorm_sub_le
    [R1Space α] [WeaklyLocallyCompactSpace α] [μ.Regular]
    (hp : p != ∞) {f : α -> E} (hf : MemLp f p μ) {ε : Real>=0∞} (hε : ε != 0) :
    exists g : α -> E, HasCompactSupport g ∧ eLpNorm (f - g) p μ <= ε ∧ Continuous g ∧ MemLp g p μ := by
  suffices H :
      exists g : α -> E, eLpNorm (f - g) p μ <= ε ∧ Continuous g ∧ MemLp g p μ ∧ HasCompactSupport g by
    rcases H with ⟨g, hg, g_cont, g_mem, g_support⟩
    exact ⟨g, g_support, hg, g_cont, g_mem⟩
  ...
  obtain ⟨η, ηpos, hη⟩ :
      exists η : Real>=0, 0 < η ∧ forall s : Set α, μ s <= η -> eLpNorm (s.indicator fun _x => c) p μ <= δ :=
    exists_eLpNorm_indicator_le hp c δpos.ne'
  have hη_pos' : (0 : Real>=0∞) < η := ENNReal.coe_pos.2 ηpos
  obtain ⟨s, st, s_compact, s_closed, μs⟩ :
      exists s, s subseteq t ∧ IsCompact s ∧ IsClosed s ∧ μ (t \ s) < η :=
  ...
  have I1 : eLpNorm ((s.indicator fun _y => c) - t.indicator fun _y => c) p μ <= δ := by
    rw [← eLpNorm_neg]; rw [neg_sub]; rw [← indicator_sdiff st]
    exact hη _ μs.le
  obtain ⟨k, k_compact, sk⟩ : exists k : Set α, IsCompact k ∧ s subseteq interior k :=
...
/--
theorem `MemLp.exists_hasCompactSupport_integral_rpow_sub_le` / 定理 `MemLp.exists_hasCompactSupport_integral_rpow_sub_le`
...
Depends on / 依赖: ENNReal, ENNReal.coe_ne_top, ENNReal.ofReal, ENNReal.ofReal_eq_zero, Real.rpow_pos_of_pos, coe_ne_top, eLpNorm, eLpNorm_eq_integral_rpow_norm, exists_hasCompactSupport_eLpNorm_sub_le, g_cont, g_mem, g_support, hf.exists_hasCompactSupport_eLpNorm_sub_le, hf.sub, not_le, ofReal, ofReal_eq_zero, rpow_pos_of_pos
-/
theorem MemLp.exists_hasCompactSupport_integral_rpow_sub_le
    [R1Space α] [WeaklyLocallyCompactSpace α] [μ.Regular]
    {p : Real} (hp : 0 < p) {f : α -> E} (hf : MemLp f (ENNReal.ofReal p) μ) {ε : Real} (hε : 0 < ε) :
    exists g : α -> E,
      HasCompactSupport g ∧
...
/--
theorem `Integrable.exists_hasCompactSupport_lintegral_sub_le` / 定理 `Integrable.exists_hasCompactSupport_lintegral_sub_le`
...
Depends on / 依赖: ENNReal, ENNReal.one_ne_top, eLpNorm_one_eq_lintegral_enorm, exists_hasCompactSupport_eLpNorm_sub_le, hf.exists_hasCompactSupport_eLpNorm_sub_le, memLp_one_iff_integrable, one_ne_top
-/
theorem Integrable.exists_hasCompactSupport_lintegral_sub_le
    [R1Space α] [WeaklyLocallyCompactSpace α] [μ.Regular]
    {f : α -> E} (hf : Integrable f μ) {ε : Real>=0∞} (hε : ε != 0) :
    exists g : α -> E,
...
/--
theorem `Integrable.exists_hasCompactSupport_integral_sub_le` / 定理 `Integrable.exists_hasCompactSupport_integral_sub_le`
...
Depends on / 依赖: ENNReal, ENNReal.ofReal_one, exists_hasCompactSupport_integral_rpow_sub_le, hf.exists_hasCompactSupport_integral_rpow_sub_le, memLp_one_iff_integrable, ofReal_one, zero_lt_one
-/
theorem Integrable.exists_hasCompactSupport_integral_sub_le
    [R1Space α] [WeaklyLocallyCompactSpace α] [μ.Regular]
    {f : α -> E} (hf : Integrable f μ) {ε : Real} (hε : 0 < ε) :
    exists g : α -> E, HasCompactSupport g ∧ (∫ x, ‖f x - g x‖ ∂μ) <= ε ∧
...
/--
theorem `MemLp.exists_boundedContinuous_eLpNorm_sub_le` / 定理 `MemLp.exists_boundedContinuous_eLpNorm_sub_le`

English:
theorem MemLp.exists_boundedContinuous_eLpNorm_sub_le
  statement: [μ.WeaklyRegular] (hp : p != ∞) {f : α -> E}
  proof: by
  suffices H :
      exists g : α -> E, eLpNorm (f - g) p μ <= ε ∧ Continuous g ∧ MemLp g p μ ∧ IsBounded (range g) by
...
Depends on / 依赖: Continuous, IsBounded, Metric, Metric.isBounded_range_iff, eLpNorm, g_bd, g_cont, g_mem, isBounded_range_iff
-/
theorem MemLp.exists_boundedContinuous_eLpNorm_sub_le [μ.WeaklyRegular] (hp : p != ∞) {f : α -> E}
    (hf : MemLp f p μ) {ε : Real>=0∞} (hε : ε != 0) :
    exists g : α ->ᵇ E, eLpNorm (f - (g : α -> E)) p μ <= ε ∧ MemLp g p μ := by
  suffices H :
      exists g : α -> E, eLpNorm (f - g) p μ <= ε ∧ Continuous g ∧ MemLp g p μ ∧ IsBounded (range g) by
    rcases H with ⟨g, hg, g_cont, g_mem, g_bd⟩
    exact ⟨⟨⟨g, g_cont⟩, Metric.isBounded_range_iff.1 g_bd⟩, hg, g_mem⟩
  ...
  obtain ⟨η, ηpos, hη⟩ :
    ...
    exists_eLpNorm_indicator_le hp c δpos.ne'
  have hη_pos' : (0 : Real>=0∞) < η := ENNReal.coe_pos.2 ηpos
  obtain ⟨s, st, s_closed, μs⟩ : exists s, s subseteq t ∧ IsClosed s ∧ μ (t \ s) < η :=
...
/--
theorem `MemLp.exists_boundedContinuous_integral_rpow_sub_le` / 定理 `MemLp.exists_boundedContinuous_integral_rpow_sub_le`

English:
theorem MemLp.exists_boundedContinuous_integral_rpow_sub_le
  statement: [μ.WeaklyRegular] {p : Real} (hp : 0 < p)
  proof: by
  have I : 0 < ε ^ (1 / p) := Real.rpow_pos_of_pos hε _
  have A : ENNReal.ofReal (ε ^ (1 / p)) != 0 := by
    simp only [Ne, ENNReal.ofReal_eq_zero, not_le, I]
...
Depends on / 依赖: ENNReal, ENNReal.coe_ne_top, ENNReal.ofReal, ENNReal.ofReal_eq_zero, ENNReal.ofReal_l, Real.rpow_pos_of_pos, coe_ne_top, eLpNorm, eLpNorm_eq_integral_rpow_norm, exists_boundedContinuous_eLpNorm_sub_le, g_mem, hf.exists_boundedContinuous_eLpNorm_sub_le, hf.sub, not_le, ofReal, ofReal_eq_zero, ofReal_l, rpow_pos_of_pos
-/
theorem MemLp.exists_boundedContinuous_integral_rpow_sub_le [μ.WeaklyRegular] {p : Real} (hp : 0 < p)
    {f : α -> E} (hf : MemLp f (ENNReal.ofReal p) μ) {ε : Real} (hε : 0 < ε) :
    exists g : α ->ᵇ E, (∫ x, ‖f x - g x‖ ^ p ∂μ) <= ε ∧ MemLp g (ENNReal.ofReal p) μ := by
  have I : 0 < ε ^ (1 / p) := Real.rpow_pos_of_pos hε _
  have A : ENNReal.ofReal (ε ^ (1 / p)) != 0 := by
    simp only [Ne, ENNReal.ofReal_eq_zero, not_le, I]
  have B : ENNReal.ofReal p != 0 := by simpa only [Ne, ENNReal.ofReal_eq_zero, not_le] using hp
...
/--
theorem `Integrable.exists_boundedContinuous_lintegral_sub_le` / 定理 `Integrable.exists_boundedContinuous_lintegral_sub_le`

English:
theorem Integrable.exists_boundedContinuous_lintegral_sub_le
  statement: [μ.WeaklyRegular] {f : α -> E}
  proof: by
  simp only [← memLp_one_iff_integrable, ← eLpNorm_one_eq_lintegral_enorm] at hf ⊢
...
Depends on / 依赖: ENNReal, ENNReal.one_ne_top, eLpNorm_one_eq_lintegral_enorm, exists_boundedContinuous_eLpNorm_sub_le, hf.exists_boundedContinuous_eLpNorm_sub_le, memLp_one_iff_integrable, one_ne_top
-/
theorem Integrable.exists_boundedContinuous_lintegral_sub_le [μ.WeaklyRegular] {f : α -> E}
    (hf : Integrable f μ) {ε : Real>=0∞} (hε : ε != 0) :
    exists g : α ->ᵇ E, ∫⁻ x, ‖f x - g x‖ₑ ∂μ <= ε ∧ Integrable g μ := by
  simp only [← memLp_one_iff_integrable, ← eLpNorm_one_eq_lintegral_enorm] at hf ⊢
  exact hf.exists_boundedContinuous_eLpNorm_sub_le ENNReal.one_ne_top hε

/--
theorem `Integrable.exists_boundedContinuous_integral_sub_le` / 定理 `Integrable.exists_boundedContinuous_integral_sub_le`

English:
theorem Integrable.exists_boundedContinuous_integral_sub_le
  statement: [μ.WeaklyRegular] {f : α -> E}
  proof: by
  simp only [← memLp_one_iff_integrable, ← ENNReal.ofReal_one]
    at hf ⊢
...
Depends on / 依赖: ENNReal, ENNReal.ofReal_one, exists_boundedContinuous_integral_rpow_sub_le, hf.exists_boundedContinuous_integral_rpow_sub_le, memLp_one_iff_integrable, ofReal_one, zero_lt_one
-/
theorem Integrable.exists_boundedContinuous_integral_sub_le [μ.WeaklyRegular] {f : α -> E}
    (hf : Integrable f μ) {ε : Real} (hε : 0 < ε) :
    exists g : α ->ᵇ E, (∫ x, ‖f x - g x‖ ∂μ) <= ε ∧ Integrable g μ := by
  simp only [← memLp_one_iff_integrable, ← ENNReal.ofReal_one]
    at hf ⊢
  simpa using hf.exists_boundedContinuous_integral_rpow_sub_le zero_lt_one hε
...
/--
theorem `boundedContinuousFunction_dense` / 定理 `boundedContinuousFunction_dense`

English:
theorem boundedContinuousFunction_dense
  statement: [SecondCountableTopologyEither α E] [Fact (1 <= p)]
  proof: by
  intro f
  refine (mem_closure_iff_nhds_basis Metric.nhds_basis_closedEBall).2 fun ε hε => ?_
  obtain ⟨g, hg, g_mem⟩ :
      exists g : α ->ᵇ E, eLpNorm ((f : α -> E) - (g : α -> E)) p μ <= ε ∧ MemLp g p μ :=
    (Lp.memLp f).exists_boundedContinuous_eLpNorm_sub_le hp hε.ne'
  refine ⟨g_mem.toL

中文:
定理 boundedContinuousFunction_dense
  结论: [SecondCountableTopologyEither α E] [Fact (1 <= p)]
  证明: by
  intro f
  refine (mem_closure_iff_nhds_basis Metric.nhds_basis_closedEBall).2 fun ε hε => ?_
  obtain ⟨g, hg, g_mem⟩ :
...
Depends on / 依赖: Lp.edist_toLp_toLp, Lp.memLp, Lp.toLp_coeFn, Metric, Metric.mem_closedEBall, Metric.nhds_basis_closedEBall, eLpNorm, edist_toLp_toLp, exists_boundedContinuous_eLpNorm_sub_le, g_mem, g_mem.toLp, mem_closedEBall, mem_closure_iff_nhds_basis, nhds_basis_closedEBall, toLp_coeFn
-/
...
/--
theorem `boundedContinuousFunction_topologicalClosure` / 定理 `boundedContinuousFunction_topologicalClosure`

English:
theorem boundedContinuousFunction_topologicalClosure
  statement: [SecondCountableTopologyEither α E]
  proof: SetLike.ext' (boundedContinuousFunction_dense E μ hp).closure_eq

中文:
定理 boundedContinuousFunction_topologicalClosure
  结论: [SecondCountableTopologyEither α E]
  证明: SetLike.ext' (boundedContinuousFunction_dense E μ hp).closure_eq

Depends on / 依赖: SetLike, SetLike.ext, boundedContinuousFunction_dense, closure_eq
-/
...
variable (𝕜 : Type*) [NormedRing 𝕜] [Module 𝕜 E] [IsBoundedSMul 𝕜 E] [NormedSpace Real E]
...
/--
theorem `toLp_denseRange` / 定理 `toLp_denseRange`

English:
theorem toLp_denseRange
  given: [μ.WeaklyRegular] [IsFiniteMeasure μ] (hp : p != ∞)
  proof: by
...
Depends on / 依赖: MeasureTheory, MeasureTheory.Lp.boundedContinuousFunction_dense, boundedContinuousFunction_dense, range_toLp
-/
...
English:
theorem toLp_denseRange
  given: [CompactSpace α] [μ.WeaklyRegular] [IsFiniteMeasure μ] (hp : p != ∞)
  proof: by
...
Depends on / 依赖: BoundedContinuousFunction, BoundedContinuousFunction.toLp_denseRange, f.toContinuousMap, range_subset_iff, toContinuousMap, toLp_denseRange
-/
