...
/--
theorem `tendsto_sum_pi_div_four` / 定理 `tendsto_sum_pi_div_four`
...
theorem tendsto_sum_pi_div_four
  ...
  obtain ⟨l, h⟩ :
      exists l, Tendsto (fun n => ∑ i in range n, (-1 : Real) ^ i / (2 * i + 1)) atTop (𝓝 l) := by
  ...
  have m : 𝓝[<] (1 : Real) <= 𝓝 1 := tendsto_nhdsWithin_of_tendsto_nhds fun _ a => a
  have q : Tendsto (fun x : Real => x ^ 2) (𝓝[<] 1) (𝓝[<] 1) := by
...
exact ⟨Ioo_mem_nhdsLT by simp,
  ...
  replace abel : Tendsto arctan (𝓝[<] 1) (𝓝 l) := by
    apply abel.congr'
    rw [eventuallyEq_nhdsWithin_iff]; rw [Metric.eventually_nhds_iff]
    use 1, zero_lt_one
    intro y hy1 hy2
    rw [dist_eq]; rw [abs_sub_lt_iff] at hy1
    rw [Set.mem_Iio] at hy2
    have ny : ‖y‖ < 1 := by rw [norm_eq_abs, abs_lt]; constructor <;> linarith
    rw [← (hasSum_arctan ny).tsum_eq]; rw [Function.comp_apply]; rw [← tsum_mul_right]
...
-/
theorem tendsto_sum_pi_div_four :
    Tendsto (fun k => ∑ i in range k, (-1 : Real) ^ i / (2 * i + 1)) atTop (𝓝 (π / 4)) := by
  -- The series is alternating with terms of decreasing magnitude, so it converges to some limit
  obtain ⟨l, h⟩ :
      exists l, Tendsto (fun n => ∑ i in range n, (-1 : Real) ^ i / (2 * i + 1)) atTop (𝓝 l) := by
    apply Antitone.tendsto_alternating_series_of_tendsto_zero
