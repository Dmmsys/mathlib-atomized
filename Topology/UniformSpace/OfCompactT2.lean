...
@[instance_reducible]
/--
Definition of `uniformSpaceOfCompactR1` / `uniformSpaceOfCompactR1` 的定义

English:
definition uniformSpaceOfCompactR1
  signature: [TopologicalSpace γ] [CompactSpace γ] [R1Space γ]
  body: 𝓝ˢ (diagonal γ)
...
def uniformSpaceOfCompactR1 [TopologicalSpace γ] [CompactSpace γ] [R1Space γ] : UniformSpace γ where
  ...
  comp := by
    ...
    obtain ⟨⟨x, y⟩, hxy⟩ : exists p : γ × γ, ClusterPt p (F ⊓ 𝓟 Vᶜ) := exists_clusterPt_of_compactSpace _
    ...
    have ninsep_xy : ¬Inseparable x y := by
      intro h
      rw [← mem_closure_iff_clusterPt]; rw [(h.prod .rfl).mem_closed_iff isClosed_closure]; rw [closure_compl] at clV
      apply clV
    ...
    let W := U₁ ×ˢ U₁ union U₂ ×ˢ U₂ union U₃ ×ˢ U₃
    have W_in : W in 𝓝Δ := by
    ...
    have : W ○ W in F := mem_lift' W_in
    -- And V₁ ×ˢ V₂ ∈ 𝓝 (x, y)
    have hV₁₂ : V₁ ×ˢ V₂ in 𝓝 (x, y) := prod_mem_nhds V₁_in V₂_in
  ...
  nhds_eq_comap_uniformity x := by
    ...
    conv_rhs =>
      congr
      · enter [1, i, 1, hi]
        rw [← hi.nhds_eq]; rw [comap_const_of_mem fun V => mem_of_mem_nhds]
      · enter [1, i, 1, hi]
    ...
    rw [iSup_subtype']; rw [@iSup_const _ _ _ _ ⟨⟨x]; rw [.rfl⟩⟩]
    simp
