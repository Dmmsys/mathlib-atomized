...
/-
**ContMDiffVectorBundle.pullback** 是 Mathlib 中的一个实例，位于命名空间 ``。
形式化陈述：ContMDiffVectorBundle.pullback : ContMDiffVectorBundle n F (f *ᵖ E) IB' wh
ere contMDiffOn_coordChangeL
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
...
--- 原说明 ---
For a `C^n` vector bundle `E` over a manifold `B` and a `C^n` map `f : B' → B`, 
the pullback
vector bundle `f *ᵖ E` is a `C^n` vector bundle.
-/
