...
/-
**WType.Nat** 是 Mathlib 中的一个归纳类型，位于命名空间 `WType`。
...
/-
**WType.Nat** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The arity of the constructors for the naturals, `zero` takes no arguments, `succ
` takes one
-/
def Natβ : Natα → Type
  | Natα.zero => Empty
...
@[simp]
/-
**WType.ofNat** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：ℕ → WType WType.Natβ
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**WType.toNat** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：WType WType.Natβ → ℕ
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**WType.leftInverse_nat** 是 Mathlib 中的一个定理，位于命名空间 `WType`。
形式化陈述：Function.LeftInverse WType.ofNat WType.toNat
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem leftInverse_nat : Function.LeftInverse ofNat toNat
  ...
  | WType.mk Natα.succ f => by
    ...
    rfl
/-
**WType.rightInverse_nat** 是 Mathlib 中的一个定理，位于命名空间 `WType`。
形式化陈述：Function.RightInverse WType.ofNat WType.toNat
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**WType.equivNat** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：equivNat : WType Natβ ≃ Nat where toFun
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `WType.leftInverse_nat`：Function.LeftInverse WType.ofNat WType.toNat
· 使用定理 `WType.rightInverse_nat`：Function.RightInverse WType.ofNat WType.toNat
...
-/
/-
**WType.List** 是 Mathlib 中的一个归纳类型，位于命名空间 `WType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**WType.List** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The arities of each constructor for lists, `nil` takes no arguments, `cons hd` t
akes one
-/
...
@[simp]
/-
**WType.ofList** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：(γ : Type u) → List γ → WType (WType.Listβ γ)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**WType.toList** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：(γ : Type u) → WType (WType.Listβ γ) → List γ
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**WType.leftInverse_list** 是 Mathlib 中的一个定理，位于命名空间 `WType`。
形式化陈述：∀ (γ : Type u), Function.LeftInverse (WType.ofList γ) (WType.toList γ)
参数：γ : Type u；WType.ofList γ；WType.toList γ。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem leftInverse_list : Function.LeftInverse (ofList γ) (toList _)
  ...
  | WType.mk (Listα.cons x) f => by
    ...
    rfl
/-
**WType.rightInverse_list** 是 Mathlib 中的一个定理，位于命名空间 `WType`。
形式化陈述：∀ (γ : Type u), Function.RightInverse (WType.ofList γ) (WType.toList γ)
参数：γ : Type u；WType.ofList γ；WType.toList γ。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**WType.equivList** 是 Mathlib 中的一个定义，位于命名空间 `WType`。
形式化陈述：equivList : WType (Listβ γ) ≃ List γ where toFun
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `WType.leftInverse_list`：∀ (γ : Type u), Function.LeftInverse (WType.ofLi
st γ) (WType.toList γ)
· 使用定理 `WType.rightInverse_list`：∀ (γ : Type u), Function.RightInverse (WType.of
List γ) (WType.toList γ)
