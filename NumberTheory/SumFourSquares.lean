...
/--
theorem `euler_four_squares` / 定理 `euler_four_squares`

English:
theorem euler_four_squares
  given: {R : Type*} [CommRing R] (a b c d x y z w : R)
  proof: by ring
...
/--
theorem `Nat.euler_four_squares` / 定理 `Nat.euler_four_squares`

English:
theorem Nat.euler_four_squares
  given: (a b c d x y z w : Nat)
  proof: by
  rw [← Int.natCast_inj]
  push_cast
...
Depends on / 依赖: Int.natCast_inj, _root_, _root_.euler_four_squares, euler_four_squares, natCast_inj, sq_abs
-/
theorem Nat.euler_four_squares (a b c d x y z w : Nat) :
    ((a : Int) * x - b * y - c * z - d * w).natAbs ^ 2 +
      ((a : Int) * y + b * x + c * w - d * z).natAbs ^ 2 +
      ((a : Int) * z - b * w + c * x + d * y).natAbs ^ 2 +
      ((a : Int) * w + b * z - c * y + d * x).natAbs ^ 2 =
...
/--
theorem `sq_add_sq_of_two_mul_sq_add_sq` / 定理 `sq_add_sq_of_two_mul_sq_add_sq`

English:
theorem sq_add_sq_of_two_mul_sq_add_sq
  given: {m x y : Int} (h : 2 * m = x ^ 2 + y ^ 2)
  proof: have : Even (x ^ 2 + y ^ 2) := by simp [← h]
mul_right_injective₀ (show (2 * 2 : Int) != 0 by decide)
    calc
      2 * 2 * m = (x - y) ^ 2 + (x + y) ^ 2 := by rw [mul_assoc, h]; ring
      _ = (2 * ((x - y) / 2)) ^ 2 + (2 * ((x + y) / 2)) ^ 2 := by
        rw [Int.mul_ediv_cancel' _]; rw [Int.mul_

中文:
定理 sq_add_sq_of_two_mul_sq_add_sq
  条件: {m x y : 整数} (h : 2 * m = x ^ 2 + y ^ 2)
  证明: have : Even (x ^ 2 + y ^ 2) := by simp [← h]
mul_right_injective₀ (show (2 * 2 : Int) != 0 by decide)
    calc
      2 * 2 * m = (x - y) ^ 2 + (x + y) ^ 2 := by rw [mul_assoc, h]; ring
      _ = (2 * ((x - y) / 2)) ^ 2 + (2 * ((x + y) / 2)) ^ 2 := by
...
Depends on / 依赖: Int.mul_ediv_cancel, even_iff_two_dvd, mul_assoc, mul_ediv_cancel, parity_simps
-/
theorem sq_add_sq_of_two_mul_sq_add_sq {m x y : Int} (h : 2 * m = x ^ 2 + y ^ 2) :
    m = ((x - y) / 2) ^ 2 + ((x + y) / 2) ^ 2 :=
...
mul_right_injective₀ (show (2 * 2 : Int) != 0 by decide)
    calc
      2 * 2 * m = (x - y) ^ 2 + (x + y) ^ 2 := by rw [mul_assoc, h]; ring
      _ = (2 * ((x - y) / 2)) ^ 2 + (2 * ((x + y) / 2)) ^ 2 := by
        rw [Int.mul_ediv_cancel' _]; rw [Int.mul_ediv_cancel' _] <;>
          simpa [sq, parity_simps, ← even_iff_two_dvd]
      _ = 2 * 2 * (((x - y) / 2) ^ 2 + ((x + y) / 2) ^ 2) := by nlinarith

/--
theorem `lt_of_sum_four_squares_eq_mul` / 定理 `lt_of_sum_four_squares_eq_mul`

English:
theorem lt_of_sum_four_squares_eq_mul
  statement: {a b c d k m : Nat}
  proof: by nlinarith

中文:
定理 lt_of_sum_four_squares_eq_mul
  结论: {a b c d k m : 自然数}
  证明: by nlinarith
-/
theorem lt_of_sum_four_squares_eq_mul {a b c d k m : Nat}
    (h : a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = k * m)
    (ha : 2 * a < m) (hb : 2 * b < m) (hc : 2 * c < m) (hd : 2 * d < m) :
    k < m := by nlinarith

/--
theorem `exists_sq_add_sq_add_one_eq_mul` / 定理 `exists_sq_add_sq_add_one_eq_mul`

English:
theorem exists_sq_add_sq_add_one_eq_mul
  ...
  rw [sub_neg_eq_add]; rw [mul_comm] at hk
  have hk₀ : 0 < k := by
...
Depends on / 依赖: Int.modEq_iff_dvd, Nat.cast_nonneg, Nat.cast_pos, Nat.sq_add_sq_zmodEq, cast_nonneg, cast_pos, eq_two_or_odd, hab.symm, lt_of_sum_four_squares_eq_mul, modEq_iff_dvd, mod_cast, mul_comm, pos_of_mul_pos_left, replace, sq_add_sq_zmodEq, sub_neg_eq_add
-/
theorem exists_sq_add_sq_add_one_eq_mul (p : Nat) [hp : Fact p.Prime] :
    exists (a b k : Nat), 0 < k ∧ k < p ∧ a ^ 2 + b ^ 2 + 1 = k * p := by
  rcases hp.1.eq_two_or_odd' with (rfl | hodd)
  · use 1, 0, 1; simp
  rcases Nat.sq_add_sq_zmodEq p (-1) with ⟨a, b, ha, hb, hab⟩
  rcases Int.modEq_iff_dvd.1 hab.symm with ⟨k, hk⟩
...
/--
theorem `sum_four_squares_of_two_mul_sum_four_squares` / 定理 `sum_four_squares_of_two_mul_sum_four_squares`

English:
theorem sum_four_squares_of_two_mul_sum_four_squares
  statement: {m a b c d : Int}
  proof: by
  have : forall f : Fin 4 -> ZMod 2, f 0 ^ 2 + f 1 ^ 2 + f 2 ^ 2 + f 3 ^ 2 = 0 -> exists i : Fin 4,
      f i ^ 2 + f (swap i 0 1) ^ 2 = 0 ∧ f (swap i 0 2) ^ 2 + f (swap i 0 3) ^ 2 = 0 := by
    decide
  set f : Fin 4 -> Int := ![a, b, c, d]
obtain ⟨i, hσ⟩ := this (fun x => ↑(f x)) by
    rw [← @

中文:
定理 sum_four_squares_of_two_mul_sum_four_squares
  结论: {m a b c d : 整数}
  证明: by
  have : forall f : Fin 4 -> ZMod 2, f 0 ^ 2 + f 1 ^ 2 + f 2 ^ 2 + f 3 ^ 2 = 0 -> exists i : Fin 4,
      f i ^ 2 + f (swap i 0 1) ^ 2 = 0 ∧ f (swap i 0 2) ^ 2 + f (swap i 0 3) ^ 2 = 0 := by
...
obtain ⟨i, hσ⟩ := this (fun x => ↑(f x)) by
    rw [← @
-/
private theorem sum_four_squares_of_two_mul_sum_four_squares {m a b c d : Int}
    (h : a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = 2 * m) :
    exists w x y z : Int, w ^ 2 + x ^ 2 + y ^ 2 + z ^ 2 = m := by
  have : forall f : Fin 4 -> ZMod 2, f 0 ^ 2 + f 1 ^ 2 + f 2 ^ 2 + f 3 ^ 2 = 0 -> exists i : Fin 4,
      f i ^ 2 + f (swap i 0 1) ^ 2 = 0 ∧ f (swap i 0 2) ^ 2 + f (swap i 0 3) ^ 2 = 0 := by
    decide
  set f : Fin 4 -> Int := ![a, b, c, d]
obtain ⟨i, hσ⟩ := this (fun x => ↑(f x)) by
    rw [← @zero_mul (ZMod 2) _ m]; rw [← show ((2 : Int) : ZMod 2) = 0 from rfl]; rw [← Int.cast_mul]; rw [← h]
    simp only [Int.cast_add, Int.cast_pow]
    rfl
  set σ := swap i 0
  obtain ⟨x, hx⟩ : (2 : Int) ∣ f (σ 0) ^ 2 + f (σ 1) ^ 2 :=
(CharP.intCast_eq_zero_iff (ZMod 2) 2 _).1 by
  ...
  obtain ⟨y, hy⟩ : (2 : Int) ∣ f (σ 2) ^ 2 + f (σ 3) ^ 2 :=
(CharP.intCast_eq_zero_iff (ZMod 2) 2 _).1 by
...
/--
theorem `Prime.sum_four_squares` / 定理 `Prime.sum_four_squares`

English:
theorem Prime.sum_four_squares
  given: {p : Nat} (hp : p.Prime)
  proof: by
  classical
  have := Fact.mk hp
  -- Find `a`, `b`, `c`, `d`, `0 < m < p` such that `a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = m * p`
  have natAbs_iff {a b c d : Int} {k : Nat} :
...
定理 Prime.sum_four_squares
  ...
  have natAbs_iff {a b c d : Int} {k : Nat} :
    ...
    rw [
-/
protected theorem Prime.sum_four_squares {p : Nat} (hp : p.Prime) :
    exists a b c d : Nat, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = p := by
  classical
  have := Fact.mk hp
  -- Find `a`, `b`, `c`, `d`, `0 < m < p` such that `a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = m * p`
  have natAbs_iff {a b c d : Int} {k : Nat} :
      a.natAbs ^ 2 + b.natAbs ^ 2 + c.natAbs ^ 2 + d.natAbs ^ 2 = k ↔
        a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = k := by
    rw [← @Nat.cast_inj Int]; push_cast [sq_abs]; rfl
  have hm : exists m < p, 0 < m ∧ exists a b c d : Nat, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = m * p := by
  ...
  · -- If `m` is an even number, then `(m / 2) * p` can be represented as a sum of four squares
    ...
    apply_fun (Nat.cast : Nat -> Int) at habcd
  ...
  · -- For each `x` in `a`, `b`, `c`, `d`, take a number `f x ≡ x [ZMOD m]` with least possible
    ...
    obtain ⟨f, hf_lt, hf_mod⟩ :
        exists f : Nat -> Int, (forall x, 2 * (f x).natAbs < m) ∧ forall x, (f x : ZMod m) = x := by
    ...
    rsuffices ⟨w, x, y, z, hw, hx, hy, hz, h⟩ : exists w x y z : Int, ↑m ∣ w ∧ ↑m ∣ x ∧ ↑m ∣ y ∧ ↑m ∣ z ∧
    ...
    replace hr : (f b) ^ 2 + (f a) ^ 2 + (f d) ^ 2 + (-f c) ^ 2 = ↑(m * r) := by
      rw [← natAbs_iff]; rw [natAbs_neg]; rw [← hr]
      ac_rfl
...
/--
theorem `sum_four_squares` / 定理 `sum_four_squares`

English:
theorem sum_four_squares
  given: (n : Nat)
  statement: exists a b c d : Nat, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = n
  proof: by
  ...
  | prime p hp => exact hp.sum

中文:
定理 sum_four_squares
  条件: (n : 自然数)
  结论: 存在 a b c d : 自然数, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = n
  证明: by
  -- The proof is by induction on prime factorization. The case of prime `n` was proved above,
  -- the inductive step follows from `Nat.euler_four_squares`.
  induction n using Nat.recOnMul with
  ...
  | prime p hp => exact hp.sum
-/
theorem sum_four_squares (n : Nat) : exists a b c d : Nat, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = n := by
  -- The proof is by induction on prime factorization. The case of prime `n` was proved above,
  -- the inductive step follows from `Nat.euler_four_squares`.
  induction n using Nat.recOnMul with
  | zero => exact ⟨0, 0, 0, 0, rfl⟩
