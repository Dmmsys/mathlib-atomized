/-
Copyright (c) 2021 Yury Kudryashov. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yury Kudryashov
-/
module

public import Mathlib.Analysis.BoxIntegral.Box.SubboxInduction
public import Mathlib.Analysis.BoxIntegral.Partition.Tagged

/-!
# Induction on subboxes

In this file we prove (see
`BoxIntegral.Box.exists_taggedPartition_isHenstock_isSubordinate_homothetic`) that for every box `I`
in `ℝⁿ` and a function `r : ℝⁿ → ℝ` positive on `I` there exists a tagged partition `π` of `I` such
that

* `π` is a Henstock partition;
* `π` is subordinate to `r`;
* each box in `π` is homothetic to `I` with coefficient of the form `1 / 2 ^ n`.

Later we will use this lemma to prove that the Henstock filter is nontrivial, hence the Henstock
integral is well-defined.

## Tags

partition, tagged partition, Henstock integral
-/

@[expose] public section


namespace BoxIntegral

open Set Metric

open Topology

noncomputable section

variable {ι : Type*} [Fintype ι] {I J : Box ι}

namespace Prepartition

/--
Definition of `splitCenter` / `splitCenter` 的定义

English:
definition splitCenter
  signature: (I : Box ι)
  body: Finset.univ.map (Box.splitCenterBoxEmb I)
  le_of_mem' := by simp [I.splitCenterBox_le]
  pairwiseDisjoint := by
    rw [Finset.coe_map]; rw [Finset.coe_univ]; rw [image_univ]
    rintro _ ⟨s, rfl⟩ _ ⟨t, rfl⟩ Hne
    exact I.disjoint_splitCenterBox (mt (congr_arg _) Hne)

@[simp]

中文:
定义 splitCenter
  签名: (I : Box ι)
  定义体: Finset.univ.map (Box.splitCenterBoxEmb I)
  le_of_mem' := by simp [I.splitCenterBox_le]
  pairwiseDisjoint := by
    rw [Finset.coe_map]; rw [Finset.coe_univ]; rw [image_univ]
    rintro _ ⟨s, rfl⟩ _ ⟨t, rfl⟩ Hne
    exact I.disjoint_splitCenterBox (mt (congr_arg _) Hne)

@[simp]

Depends on / 依赖: Box.splitCenterBoxEmb, Finset, Finset.univ.map, splitCenterBoxEmb
-/
def splitCenter (I : Box ι) : Prepartition I where
  boxes := Finset.univ.map (Box.splitCenterBoxEmb I)
  le_of_mem' := by simp [I.splitCenterBox_le]
  pairwiseDisjoint := by
    rw [Finset.coe_map]; rw [Finset.coe_univ]; rw [image_univ]
    rintro _ ⟨s, rfl⟩ _ ⟨t, rfl⟩ Hne
    exact I.disjoint_splitCenterBox (mt (congr_arg _) Hne)

@[simp]
/--
theorem `mem_splitCenter` / 定理 `mem_splitCenter`

English:
theorem mem_splitCenter
  statement: J in splitCenter I ↔ exists s, I.splitCenterBox s = J
  proof: by simp [splitCenter]

中文:
定理 mem_splitCenter
  结论: J in splitCenter I ↔ 存在 s, I.splitCenterBox s = J
  证明: by simp [splitCenter]

Depends on / 依赖: splitCenter
-/
theorem mem_splitCenter : J in splitCenter I ↔ exists s, I.splitCenterBox s = J := by simp [splitCenter]

/--
theorem `isPartition_splitCenter` / 定理 `isPartition_splitCenter`

English:
theorem isPartition_splitCenter
  given: (I : Box ι)
  statement: IsPartition (splitCenter I)
  proof: fun x hx => by
  simp [hx]

中文:
定理 isPartition_splitCenter
  条件: (I : Box ι)
  结论: IsPartition (splitCenter I)
  证明: fun x hx => by
  simp [hx]
-/
theorem isPartition_splitCenter (I : Box ι) : IsPartition (splitCenter I) := fun x hx => by
  simp [hx]

/--
theorem `upper_sub_lower_of_mem_splitCenter` / 定理 `upper_sub_lower_of_mem_splitCenter`

English:
theorem upper_sub_lower_of_mem_splitCenter
  given: (h : J in splitCenter I) (i : ι)
  proof: let ⟨s, hs⟩ := mem_splitCenter.1 h
  hs ▸ I.upper_sub_lower_splitCenterBox s i

中文:
定理 upper_sub_lower_of_mem_splitCenter
  条件: (h : J in splitCenter I) (i : ι)
  证明: let ⟨s, hs⟩ := mem_splitCenter.1 h
  hs ▸ I.upper_sub_lower_splitCenterBox s i

Depends on / 依赖: I.upper_sub_lower_splitCenterBox, mem_splitCenter, upper_sub_lower_splitCenterBox
-/
theorem upper_sub_lower_of_mem_splitCenter (h : J in splitCenter I) (i : ι) :
    J.upper i - J.lower i = (I.upper i - I.lower i) / 2 :=
  let ⟨s, hs⟩ := mem_splitCenter.1 h
  hs ▸ I.upper_sub_lower_splitCenterBox s i

end Prepartition

namespace Box

open Prepartition TaggedPrepartition

/-- Let `p` be a predicate on `Box ι`, let `I` be a box. Suppose that the following two properties
hold true.

* Consider a smaller box `J ≤ I`. The hyperplanes passing through the center of `J` split it into
  `2 ^ n` boxes. If `p` holds true on each of these boxes, then it true on `J`.
* For each `z` in the closed box `I.Icc` there exists a neighborhood `U` of `z` within `I.Icc` such
  that for every box `J ≤ I` such that `z ∈ J.Icc ⊆ U`, if `J` is homothetic to `I` with a
  coefficient of the form `1 / 2 ^ m`, then `p` is true on `J`.

Then `p I` is true. See also `BoxIntegral.Box.subbox_induction_on'` for a version using
`BoxIntegral.Box.splitCenterBox` instead of `BoxIntegral.Prepartition.splitCenter`. -/
@[elab_as_elim]
/--
theorem `subbox_induction_on` / 定理 `subbox_induction_on`

English:
theorem subbox_induction_on
  statement: {p : Box ι -> Prop} (I : Box ι)
  proof: by
  refine subbox_induction_on' I (fun J hle hs => H_ind J hle fun J' h' => ?_) H_nhds
  rcases mem_splitCenter.1 h' with ⟨s, rfl⟩
  exact hs s

中文:
定理 subbox_induction_on
  结论: {p : Box ι -> 命题} (I : Box ι)
  证明: by
  refine subbox_induction_on' I (fun J hle hs => H_ind J hle fun J' h' => ?_) H_nhds
  rcases mem_splitCenter.1 h' with ⟨s, rfl⟩
  exact hs s

Depends on / 依赖: H_ind, H_nhds, mem_splitCenter, subbox_induction_on
-/
theorem subbox_induction_on {p : Box ι -> Prop} (I : Box ι)
    (H_ind : forall J <= I, (forall J' in splitCenter J, p J') -> p J)
    (H_nhds : forall z in Box.Icc I, exists U in 𝓝[Box.Icc I] z, forall J <= I, forall (m : Nat),
      z in Box.Icc J -> Box.Icc J subseteq U ->
        (forall i, J.upper i - J.lower i = (I.upper i - I.lower i) / 2 ^ m) -> p J) :
    p I := by
  refine subbox_induction_on' I (fun J hle hs => H_ind J hle fun J' h' => ?_) H_nhds
  rcases mem_splitCenter.1 h' with ⟨s, rfl⟩
  exact hs s

/--
theorem `exists_taggedPartition_isHenstock_isSubordinate_homothetic` / 定理 `exists_taggedPartition_isHenstock_isSubordinate_homothetic`

English:
theorem exists_taggedPartition_isHenstock_isSubordinate_homothetic
  statement: (I : Box ι)
  proof: by
  refine subbox_induction_on I (fun J _ hJ => ?_) fun z _ => ?_
  · choose! πi hP hHen hr Hn _ using hJ
    choose! n hn using Hn
    have hP : ((splitCenter J).biUnionTagged πi).IsPartition :=
      (isPartition_splitCenter _).biUnionTagged hP
    have hsub : forall J' in (splitCenter J).biUnion

中文:
定理 存在_taggedPartition_isHenstock_isSubordinate_homothetic
  结论: (I : Box ι)
  证明: by
  refine subbox_induction_on I (fun J _ hJ => ?_) fun z _ => ?_
  · choose! πi hP hHen hr Hn _ using hJ
    choose! n hn using Hn
    have hP : ((splitCenter J).biUnionTagged πi).IsPartition :=
      (isPartition_splitCenter _).biUnionTagged hP
    have hsub : forall J' in (splitCenter J).biUnion

Depends on / 依赖: IsPartition, J.lower, J.upper, biUnionTagged, isPartition_splitCenter, mem_biUnionTagged, splitCenter, subbox_induction_on
-/
theorem exists_taggedPartition_isHenstock_isSubordinate_homothetic (I : Box ι)
    (r : (ι -> Real) -> Ioi (0 : Real)) :
    exists π : TaggedPrepartition I, π.IsPartition ∧ π.IsHenstock ∧ π.IsSubordinate r ∧
      (forall J in π, exists m : Nat, forall i, (J :).upper i - J.lower i = (I.upper i - I.lower i) / 2 ^ m) ∧
        π.distortion = I.distortion := by
  refine subbox_induction_on I (fun J _ hJ => ?_) fun z _ => ?_
  · choose! πi hP hHen hr Hn _ using hJ
    choose! n hn using Hn
    have hP : ((splitCenter J).biUnionTagged πi).IsPartition :=
      (isPartition_splitCenter _).biUnionTagged hP
    have hsub : forall J' in (splitCenter J).biUnionTagged πi, exists n : Nat, forall i,
        (J' :).upper i - J'.lower i = (J.upper i - J.lower i) / 2 ^ n := by
      intro J' hJ'
      rcases (splitCenter J).mem_biUnionTagged.1 hJ' with ⟨J₁, h₁, h₂⟩
      refine ⟨n J₁ J' + 1, fun i => ?_⟩
      simp only [hn J₁ h₁ J' h₂, upper_sub_lower_of_mem_splitCenter h₁, pow_succ', div_div]
    refine ⟨_, hP, isHenstock_biUnionTagged.2 hHen, isSubordinate_biUnionTagged.2 hr, hsub, ?_⟩
    refine TaggedPrepartition.distortion_of_const _ hP.nonempty_boxes fun J' h' => ?_
    rcases hsub J' h' with ⟨n, hn⟩
    exact Box.distortion_eq_of_sub_eq_div hn
  · refine ⟨Box.Icc I inter closedBall z (r z),
      inter_mem_nhdsWithin _ (closedBall_mem_nhds _ (r z).coe_prop), ?_⟩
    intro J _ n Hmem HIcc Hsub
    rw [Set.subset_inter_iff] at HIcc
    refine ⟨single _ _ le_rfl _ Hmem, isPartition_single _, isHenstock_single _,
      (isSubordinate_single _ _).2 HIcc.2, ?_, distortion_single _ _⟩
    simp only [TaggedPrepartition.mem_single, forall_eq]
    refine ⟨0, fun i => ?_⟩
    simp

end Box

namespace Prepartition

open TaggedPrepartition Finset Function

/--
theorem `exists_tagged_le_isHenstock_isSubordinate_iUnion_eq` / 定理 `exists_tagged_le_isHenstock_isSubordinate_iUnion_eq`

English:
theorem exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
  statement: {I : Box ι} (r : (ι -> Real) -> Ioi (0 : Real))
  proof: by
  have := fun J => Box.exists_taggedPartition_isHenstock_isSubordinate_homothetic J r
  choose! πi πip πiH πir _ πid using this
  refine ⟨π.biUnionTagged πi, biUnion_le _ _, isHenstock_biUnionTagged.2 fun J _ => πiH J,
    isSubordinate_biUnionTagged.2 fun J _ => πir J, ?_, π.iUnion_biUnion_parti

中文:
定理 存在_tagged_le_isHenstock_isSubordinate_iUnion_eq
  结论: {I : Box ι} (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: by
  have := fun J => Box.exists_taggedPartition_isHenstock_isSubordinate_homothetic J r
  choose! πi πip πiH πir _ πid using this
  refine ⟨π.biUnionTagged πi, biUnion_le _ _, isHenstock_biUnionTagged.2 fun J _ => πiH J,
    isSubordinate_biUnionTagged.2 fun J _ => πir J, ?_, π.iUnion_biUnion_parti

Depends on / 依赖: Box.exists_taggedPartition_isHenstock_isSubordinate_homothetic, biUnionTagged, biUnion_le, distortion_biUnionTagged, exists_taggedPartition_isHenstock_isSubordinate_homothetic, iUnion_biUnion_partition, isHenstock_biUnionTagged, isSubordinate_biUnionTagged, sup_congr
-/
theorem exists_tagged_le_isHenstock_isSubordinate_iUnion_eq {I : Box ι} (r : (ι -> Real) -> Ioi (0 : Real))
    (π : Prepartition I) :
    exists π' : TaggedPrepartition I, π'.toPrepartition <= π ∧ π'.IsHenstock ∧ π'.IsSubordinate r ∧
      π'.distortion = π.distortion ∧ π'.iUnion = π.iUnion := by
  have := fun J => Box.exists_taggedPartition_isHenstock_isSubordinate_homothetic J r
  choose! πi πip πiH πir _ πid using this
  refine ⟨π.biUnionTagged πi, biUnion_le _ _, isHenstock_biUnionTagged.2 fun J _ => πiH J,
    isSubordinate_biUnionTagged.2 fun J _ => πir J, ?_, π.iUnion_biUnion_partition fun J _ => πip J⟩
  rw [distortion_biUnionTagged]
  exact sup_congr rfl fun J _ => πid J

/--
Definition of `toSubordinate` / `toSubordinate` 的定义

English:
definition toSubordinate
  signature: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  body: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose

中文:
定义 toSubordinate
  签名: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  定义体: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose

Depends on / 依赖: exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
def toSubordinate (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) : TaggedPrepartition I :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose

/--
theorem `toSubordinate_toPrepartition_le` / 定理 `toSubordinate_toPrepartition_le`

English:
theorem toSubordinate_toPrepartition_le
  given: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  proof: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.1

中文:
定理 toSubordinate_toPrepartition_le
  条件: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.1

Depends on / 依赖: choose_spec, exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
theorem toSubordinate_toPrepartition_le (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π.toSubordinate r).toPrepartition <= π :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.1

/--
theorem `isHenstock_toSubordinate` / 定理 `isHenstock_toSubordinate`

English:
theorem isHenstock_toSubordinate
  given: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  proof: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.1

中文:
定理 isHenstock_toSubordinate
  条件: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.1

Depends on / 依赖: choose_spec, exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
theorem isHenstock_toSubordinate (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π.toSubordinate r).IsHenstock :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.1

/--
theorem `isSubordinate_toSubordinate` / 定理 `isSubordinate_toSubordinate`

English:
theorem isSubordinate_toSubordinate
  given: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  proof: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.1

@[simp]

中文:
定理 isSubordinate_toSubordinate
  条件: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.1

@[simp]

Depends on / 依赖: choose_spec, exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
theorem isSubordinate_toSubordinate (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π.toSubordinate r).IsSubordinate r :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.1

@[simp]
/--
theorem `distortion_toSubordinate` / 定理 `distortion_toSubordinate`

English:
theorem distortion_toSubordinate
  given: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  proof: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.1

@[simp]

中文:
定理 distortion_toSubordinate
  条件: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.1

@[simp]

Depends on / 依赖: choose_spec, exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
theorem distortion_toSubordinate (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π.toSubordinate r).distortion = π.distortion :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.1

@[simp]
/--
theorem `iUnion_toSubordinate` / 定理 `iUnion_toSubordinate`

English:
theorem iUnion_toSubordinate
  given: (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real))
  proof: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.2

中文:
定理 iUnion_toSubordinate
  条件: (π : 预分拆 I) (r : (ι -> 实数) -> 左开右无界区间 (0 : 实数))
  证明: (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.2

Depends on / 依赖: choose_spec, exists_tagged_le_isHenstock_isSubordinate_iUnion_eq
-/
theorem iUnion_toSubordinate (π : Prepartition I) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π.toSubordinate r).iUnion = π.iUnion :=
  (π.exists_tagged_le_isHenstock_isSubordinate_iUnion_eq r).choose_spec.2.2.2.2

end Prepartition

namespace TaggedPrepartition

/--
Definition of `unionComplToSubordinate` / `unionComplToSubordinate` 的定义

English:
definition unionComplToSubordinate
  signature: (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
  body: π₁.disjUnion (π₂.toSubordinate r)
    (((π₂.iUnion_toSubordinate r).trans hU).symm ▸ disjoint_sdiff_self_right)

中文:
定义 unionComplToSubordinate
  签名: (π₁ : 标记预分拆 I) (π₂ : 预分拆 I)
  定义体: π₁.disjUnion (π₂.toSubordinate r)
    (((π₂.iUnion_toSubordinate r).trans hU).symm ▸ disjoint_sdiff_self_right)

Depends on / 依赖: disjUnion, disjoint_sdiff_self_right, iUnion_toSubordinate, toSubordinate
-/
def unionComplToSubordinate (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
    (hU : π₂.iUnion = ↑I \ π₁.iUnion) (r : (ι -> Real) -> Ioi (0 : Real)) : TaggedPrepartition I :=
  π₁.disjUnion (π₂.toSubordinate r)
    (((π₂.iUnion_toSubordinate r).trans hU).symm ▸ disjoint_sdiff_self_right)

/--
theorem `isPartition_unionComplToSubordinate` / 定理 `isPartition_unionComplToSubordinate`

English:
theorem isPartition_unionComplToSubordinate
  statement: (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
  proof: Prepartition.isPartitionDisjUnionOfEqDiff ((π₂.iUnion_toSubordinate r).trans hU)

中文:
定理 isPartition_unionComplToSubordinate
  结论: (π₁ : 标记预分拆 I) (π₂ : 预分拆 I)
  证明: Prepartition.isPartitionDisjUnionOfEqDiff ((π₂.iUnion_toSubordinate r).trans hU)

Depends on / 依赖: Prepartition, Prepartition.isPartitionDisjUnionOfEqDiff, iUnion_toSubordinate, isPartitionDisjUnionOfEqDiff
-/
theorem isPartition_unionComplToSubordinate (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
    (hU : π₂.iUnion = ↑I \ π₁.iUnion) (r : (ι -> Real) -> Ioi (0 : Real)) :
    IsPartition (π₁.unionComplToSubordinate π₂ hU r) :=
  Prepartition.isPartitionDisjUnionOfEqDiff ((π₂.iUnion_toSubordinate r).trans hU)

open scoped Classical in
@[simp]
/--
theorem `unionComplToSubordinate_boxes` / 定理 `unionComplToSubordinate_boxes`

English:
theorem unionComplToSubordinate_boxes
  statement: (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
  proof: rfl

@[simp]

中文:
定理 unionComplToSubordinate_boxes
  结论: (π₁ : 标记预分拆 I) (π₂ : 预分拆 I)
  证明: rfl

@[simp]
-/
theorem unionComplToSubordinate_boxes (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
    (hU : π₂.iUnion = ↑I \ π₁.iUnion) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π₁.unionComplToSubordinate π₂ hU r).boxes = π₁.boxes union (π₂.toSubordinate r).boxes := rfl

@[simp]
/--
theorem `iUnion_unionComplToSubordinate_boxes` / 定理 `iUnion_unionComplToSubordinate_boxes`

English:
theorem iUnion_unionComplToSubordinate_boxes
  statement: (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
  proof: (isPartition_unionComplToSubordinate _ _ _ _).iUnion_eq

@[simp]

中文:
定理 iUnion_unionComplToSubordinate_boxes
  结论: (π₁ : 标记预分拆 I) (π₂ : 预分拆 I)
  证明: (isPartition_unionComplToSubordinate _ _ _ _).iUnion_eq

@[simp]

Depends on / 依赖: iUnion_eq, isPartition_unionComplToSubordinate
-/
theorem iUnion_unionComplToSubordinate_boxes (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
    (hU : π₂.iUnion = ↑I \ π₁.iUnion) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π₁.unionComplToSubordinate π₂ hU r).iUnion = I :=
  (isPartition_unionComplToSubordinate _ _ _ _).iUnion_eq

@[simp]
/--
theorem `distortion_unionComplToSubordinate` / 定理 `distortion_unionComplToSubordinate`

English:
theorem distortion_unionComplToSubordinate
  statement: (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
  proof: by
  simp [unionComplToSubordinate]

中文:
定理 distortion_unionComplToSubordinate
  结论: (π₁ : 标记预分拆 I) (π₂ : 预分拆 I)
  证明: by
  simp [unionComplToSubordinate]

Depends on / 依赖: unionComplToSubordinate
-/
theorem distortion_unionComplToSubordinate (π₁ : TaggedPrepartition I) (π₂ : Prepartition I)
    (hU : π₂.iUnion = ↑I \ π₁.iUnion) (r : (ι -> Real) -> Ioi (0 : Real)) :
    (π₁.unionComplToSubordinate π₂ hU r).distortion = max π₁.distortion π₂.distortion := by
  simp [unionComplToSubordinate]

end TaggedPrepartition

end

end BoxIntegral
