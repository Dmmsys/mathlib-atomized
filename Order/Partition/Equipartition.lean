/-
Copyright (c) 2022 Yaël Dillies, Bhavik Mehta. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yaël Dillies, Bhavik Mehta
-/
module

public import Mathlib.Algebra.Order.Ring.Nat
public import Mathlib.Data.Set.Equitable
public import Mathlib.Logic.Equiv.Fin.Basic
public import Mathlib.Order.Partition.Finpartition
public import Mathlib.Tactic.ApplyFun

/-!
# Finite equipartitions

This file defines finite equipartitions, the partitions whose parts all are the same size up to a
difference of `1`.

## Main declarations

* `Finpartition.IsEquipartition`: Predicate for a `Finpartition` to be an equipartition.
* `Finpartition.IsEquipartition.exists_partPreservingEquiv`: part-preserving enumeration of a finset
  equipped with an equipartition. Indices of elements in the same part are congruent modulo
  the number of parts.
-/

@[expose] public section


open Finset Fintype

namespace Finpartition

variable {α : Type*} [DecidableEq α] {s t : Finset α} (P : Finpartition s)

/--
Definition of `IsEquipartition` / `IsEquipartition` 的定义

English:
definition IsEquipartition
  signature: : Prop
  body: (P.parts : Set (Finset α)).EquitableOn card

中文:
定义 IsEquipartition
  签名: : 命题
  定义体: (P.parts : Set (Finset α)).EquitableOn card

Depends on / 依赖: EquitableOn, Finset, P.parts
-/
def IsEquipartition : Prop :=
  (P.parts : Set (Finset α)).EquitableOn card

/--
theorem `isEquipartition_iff_card_parts_eq_average` / 定理 `isEquipartition_iff_card_parts_eq_average`

English:
theorem isEquipartition_iff_card_parts_eq_average
  proof: by
  simp_rw [IsEquipartition, Finset.equitableOn_iff, P.sum_card_parts]

中文:
定理 isEquipartition_iff_card_parts_eq_average
  证明: by
  simp_rw [IsEquipartition, Finset.equitableOn_iff, P.sum_card_parts]

Depends on / 依赖: Finset, Finset.equitableOn_iff, IsEquipartition, P.sum_card_parts, equitableOn_iff, simp_rw, sum_card_parts
-/
theorem isEquipartition_iff_card_parts_eq_average :
    P.IsEquipartition ↔
      forall a : Finset α, a in P.parts -> #a = #s / #P.parts ∨ #a = #s / #P.parts + 1 := by
  simp_rw [IsEquipartition, Finset.equitableOn_iff, P.sum_card_parts]

variable {P}

/--
lemma `not_isEquipartition` / 引理 `not_isEquipartition`

English:
lemma not_isEquipartition
  proof: Set.not_equitableOn

中文:
引理 not_isEquipartition
  证明: Set.not_equitableOn

Depends on / 依赖: Set.not_equitableOn, not_equitableOn
-/
lemma not_isEquipartition :
    ¬P.IsEquipartition ↔ exists a in P.parts, exists b in P.parts, #b + 1 < #a := Set.not_equitableOn

/--
theorem `_root_.Set.Subsingleton.isEquipartition` / 定理 `_root_.Set.Subsingleton.isEquipartition`

English:
theorem _root_.Set.Subsingleton.isEquipartition
  given: (h : (P.parts : Set (Finset α)).Subsingleton)
  proof: Set.Subsingleton.equitableOn h _

中文:
定理 _root_.Set.Subsingleton.isEquipartition
  条件: (h : (P.parts : Set (Finset α)).Subsingleton)
  证明: Set.Subsingleton.equitableOn h _

Depends on / 依赖: Set.Subsingleton.equitableOn, Subsingleton, equitableOn
-/
theorem _root_.Set.Subsingleton.isEquipartition (h : (P.parts : Set (Finset α)).Subsingleton) :
    P.IsEquipartition :=
  Set.Subsingleton.equitableOn h _

/--
theorem `IsEquipartition.card_parts_eq_average` / 定理 `IsEquipartition.card_parts_eq_average`

English:
theorem IsEquipartition.card_parts_eq_average
  given: (hP : P.IsEquipartition) (ht : t in P.parts)
  proof: P.isEquipartition_iff_card_parts_eq_average.1 hP _ ht

中文:
定理 IsEquipartition.card_parts_eq_average
  条件: (hP : P.IsEquipartition) (ht : t in P.parts)
  证明: P.isEquipartition_iff_card_parts_eq_average.1 hP _ ht

Depends on / 依赖: P.isEquipartition_iff_card_parts_eq_average, isEquipartition_iff_card_parts_eq_average
-/
theorem IsEquipartition.card_parts_eq_average (hP : P.IsEquipartition) (ht : t in P.parts) :
    #t = #s / #P.parts ∨ #t = #s / #P.parts + 1 :=
  P.isEquipartition_iff_card_parts_eq_average.1 hP _ ht

/--
theorem `IsEquipartition.card_part_eq_average_iff` / 定理 `IsEquipartition.card_part_eq_average_iff`

English:
theorem IsEquipartition.card_part_eq_average_iff
  given: (hP : P.IsEquipartition) (ht : t in P.parts)
  proof: by
  have a := hP.card_parts_eq_average ht
  lia

中文:
定理 IsEquipartition.card_part_eq_average_iff
  条件: (hP : P.IsEquipartition) (ht : t in P.parts)
  证明: by
  have a := hP.card_parts_eq_average ht
  lia

Depends on / 依赖: card_parts_eq_average, hP.card_parts_eq_average
-/
theorem IsEquipartition.card_part_eq_average_iff (hP : P.IsEquipartition) (ht : t in P.parts) :
    #t = #s / #P.parts ↔ #t != #s / #P.parts + 1 := by
  have a := hP.card_parts_eq_average ht
  lia

/--
theorem `IsEquipartition.average_le_card_part` / 定理 `IsEquipartition.average_le_card_part`

English:
theorem IsEquipartition.average_le_card_part
  given: (hP : P.IsEquipartition) (ht : t in P.parts)
  proof: by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le hP ht

中文:
定理 IsEquipartition.average_le_card_part
  条件: (hP : P.IsEquipartition) (ht : t in P.parts)
  证明: by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le hP ht

Depends on / 依赖: EquitableOn, Finset, Finset.EquitableOn.le, P.sum_card_parts, sum_card_parts
-/
theorem IsEquipartition.average_le_card_part (hP : P.IsEquipartition) (ht : t in P.parts) :
    #s / #P.parts <= #t := by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le hP ht

/--
theorem `IsEquipartition.card_part_le_average_add_one` / 定理 `IsEquipartition.card_part_le_average_add_one`

English:
theorem IsEquipartition.card_part_le_average_add_one
  given: (hP : P.IsEquipartition) (ht : t in P.parts)
  proof: by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le_add_one hP ht

中文:
定理 IsEquipartition.card_part_le_average_add_one
  条件: (hP : P.IsEquipartition) (ht : t in P.parts)
  证明: by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le_add_one hP ht

Depends on / 依赖: EquitableOn, Finset, Finset.EquitableOn.le_add_one, P.sum_card_parts, le_add_one, sum_card_parts
-/
theorem IsEquipartition.card_part_le_average_add_one (hP : P.IsEquipartition) (ht : t in P.parts) :
    #t <= #s / #P.parts + 1 := by
  rw [← P.sum_card_parts]
  exact Finset.EquitableOn.le_add_one hP ht

/--
theorem `IsEquipartition.filter_ne_average_add_one_eq_average` / 定理 `IsEquipartition.filter_ne_average_add_one_eq_average`

English:
theorem IsEquipartition.filter_ne_average_add_one_eq_average
  given: (hP : P.IsEquipartition)
  proof: by
  ext p
  simp only [mem_filter, and_congr_right_iff]
  exact fun hp => (hP.card_part_eq_average_iff hp).symm

中文:
定理 IsEquipartition.filter_ne_average_add_one_eq_average
  条件: (hP : P.IsEquipartition)
  证明: by
  ext p
  simp only [mem_filter, and_congr_right_iff]
  exact fun hp => (hP.card_part_eq_average_iff hp).symm

Depends on / 依赖: and_congr_right_iff, card_part_eq_average_iff, hP.card_part_eq_average_iff, mem_filter
-/
theorem IsEquipartition.filter_ne_average_add_one_eq_average (hP : P.IsEquipartition) :
    {p in P.parts | ¬#p = #s / #P.parts + 1} = {p in P.parts | #p = #s / #P.parts} := by
  ext p
  simp only [mem_filter, and_congr_right_iff]
  exact fun hp => (hP.card_part_eq_average_iff hp).symm

/--
theorem `IsEquipartition.card_large_parts_eq_mod` / 定理 `IsEquipartition.card_large_parts_eq_mod`

English:
theorem IsEquipartition.card_large_parts_eq_mod
  given: (hP : P.IsEquipartition)
  proof: by
  have z := P.sum_card_parts
  rw [← sum_filter_add_sum_filter_not (s := P.parts) (p := fun x => #x = #s / #P.parts + 1)]; rw [hP.filter_ne_average_add_one_eq_average]; rw [sum_const_nat (m := #s / #P.parts + 1) (by simp)]; rw [sum_const_nat (m := #s / #P.parts) (by simp)]; rw [← hP.filter_ne_ave

中文:
定理 IsEquipartition.card_large_parts_eq_mod
  条件: (hP : P.IsEquipartition)
  证明: by
  have z := P.sum_card_parts
  rw [← sum_filter_add_sum_filter_not (s := P.parts) (p := fun x => #x = #s / #P.parts + 1)]; rw [hP.filter_ne_average_add_one_eq_average]; rw [sum_const_nat (m := #s / #P.parts + 1) (by simp)]; rw [sum_const_nat (m := #s / #P.parts) (by simp)]; rw [← hP.filter_ne_ave

Depends on / 依赖: P.parts, P.sum_card_parts, add_assoc, add_comm, add_left_inj, add_mul, card_filter_add_card_filter_not, filter_ne_average_add_one_eq_average, hP.filter_ne_average_add_one_eq_average, mul_add, mul_one, sum_card_parts, sum_const_nat, sum_filter_add_sum_filter_not
-/
theorem IsEquipartition.card_large_parts_eq_mod (hP : P.IsEquipartition) :
    #{p in P.parts | #p = #s / #P.parts + 1} = #s % #P.parts := by
  have z := P.sum_card_parts
  rw [← sum_filter_add_sum_filter_not (s := P.parts) (p := fun x => #x = #s / #P.parts + 1)]; rw [hP.filter_ne_average_add_one_eq_average]; rw [sum_const_nat (m := #s / #P.parts + 1) (by simp)]; rw [sum_const_nat (m := #s / #P.parts) (by simp)]; rw [← hP.filter_ne_average_add_one_eq_average]; rw [mul_add]; rw [add_comm]; rw [← add_assoc]; rw [← add_mul]; rw [mul_one]; rw [add_comm #_]; rw [card_filter_add_card_filter_not]; rw [add_comm] at z
  rw [← add_left_inj]; rw [Nat.mod_add_div]; rw [z]

/--
theorem `IsEquipartition.card_small_parts_eq_mod` / 定理 `IsEquipartition.card_small_parts_eq_mod`

English:
theorem IsEquipartition.card_small_parts_eq_mod
  given: (hP : P.IsEquipartition)
  proof: by
  conv_rhs =>
    arg 1
    rw [← card_filter_add_card_filter_not (p := fun p => #p = #s / #P.parts + 1)]
  rw [hP.card_large_parts_eq_mod]; rw [add_tsub_cancel_left]; rw [hP.filter_ne_average_add_one_eq_average]

中文:
定理 IsEquipartition.card_small_parts_eq_mod
  条件: (hP : P.IsEquipartition)
  证明: by
  conv_rhs =>
    arg 1
    rw [← card_filter_add_card_filter_not (p := fun p => #p = #s / #P.parts + 1)]
  rw [hP.card_large_parts_eq_mod]; rw [add_tsub_cancel_left]; rw [hP.filter_ne_average_add_one_eq_average]

Depends on / 依赖: P.parts, add_tsub_cancel_left, card_filter_add_card_filter_not, card_large_parts_eq_mod, conv_rhs, filter_ne_average_add_one_eq_average, hP.card_large_parts_eq_mod, hP.filter_ne_average_add_one_eq_average
-/
theorem IsEquipartition.card_small_parts_eq_mod (hP : P.IsEquipartition) :
    #{p in P.parts | #p = #s / #P.parts} = #P.parts - #s % #P.parts := by
  conv_rhs =>
    arg 1
    rw [← card_filter_add_card_filter_not (p := fun p => #p = #s / #P.parts + 1)]
  rw [hP.card_large_parts_eq_mod]; rw [add_tsub_cancel_left]; rw [hP.filter_ne_average_add_one_eq_average]

/--
theorem `IsEquipartition.exists_partsEquiv` / 定理 `IsEquipartition.exists_partsEquiv`

English:
theorem IsEquipartition.exists_partsEquiv
  given: (hP : P.IsEquipartition)
  proof: by
  let el := {p in P.parts | #p = #s / #P.parts + 1}.equivFin
  let es := {p in P.parts | #p = #s / #P.parts}.equivFin
  simp_rw [mem_filter, hP.card_large_parts_eq_mod] at el
  simp_rw [mem_filter, hP.card_small_parts_eq_mod] at es
  let sneg :
      {x // x in P.parts ∧ ¬#x = #s / #P.parts + 1} 

中文:
定理 IsEquipartition.exists_partsEquiv
  条件: (hP : P.IsEquipartition)
  证明: by
  let el := {p in P.parts | #p = #s / #P.parts + 1}.equivFin
  let es := {p in P.parts | #p = #s / #P.parts}.equivFin
  simp_rw [mem_filter, hP.card_large_parts_eq_mod] at el
  simp_rw [mem_filter, hP.card_small_parts_eq_mod] at es
  let sneg :
      {x // x in P.parts ∧ ¬#x = #s / #P.parts + 1} 

Depends on / 依赖: Equiv.refl, Equiv.refl_apply, P.parts, and_congr_right_iff, card_large_parts_eq_mod, card_part_eq_average_iff, card_small_parts_eq_mod, equivFin, hP.card_large_parts_eq_mod, hP.card_part_eq_average_iff, hP.card_small_parts_eq_mod, mem_filter, ne_eq, refl_apply, replace, simp_rw, subtypeEquiv
-/
theorem IsEquipartition.exists_partsEquiv (hP : P.IsEquipartition) :
    exists f : P.parts ≃ Fin #P.parts, forall t, #t.1 = #s / #P.parts + 1 ↔ f t < #s % #P.parts := by
  let el := {p in P.parts | #p = #s / #P.parts + 1}.equivFin
  let es := {p in P.parts | #p = #s / #P.parts}.equivFin
  simp_rw [mem_filter, hP.card_large_parts_eq_mod] at el
  simp_rw [mem_filter, hP.card_small_parts_eq_mod] at es
  let sneg :
      {x // x in P.parts ∧ ¬#x = #s / #P.parts + 1} ≃ {x // x in P.parts ∧ #x = #s / #P.parts} := by
    apply (Equiv.refl _).subtypeEquiv
    simp only [Equiv.refl_apply, and_congr_right_iff]
    exact fun _ ha => by rw [hP.card_part_eq_average_iff ha, ne_eq]
  replace el : { x : P.parts // #x.1 = #s / #P.parts + 1 } ≃
      Fin (#s % #P.parts) := (Equiv.Set.sep ..).symm.trans el
  replace es : { x : P.parts // ¬#x.1 = #s / #P.parts + 1 } ≃
      Fin (#P.parts - #s % #P.parts) := (Equiv.Set.sep ..).symm.trans (sneg.trans es)
  let f := (Equiv.sumCompl _).symm.trans ((el.sumCongr es).trans finSumFinEquiv)
  use f.trans (finCongr (Nat.add_sub_of_le P.card_mod_card_parts_le))
  intro ⟨p, _⟩
  simp_rw [f, Equiv.trans_apply, Equiv.sumCongr_apply, finCongr_apply, Fin.val_cast]
  by_cases hc : #p = #s / #P.parts + 1 <;> simp [hc]

/--
theorem `IsEquipartition.exists_partPreservingEquiv` / 定理 `IsEquipartition.exists_partPreservingEquiv`

English:
theorem IsEquipartition.exists_partPreservingEquiv
  given: (hP : P.IsEquipartition)
  statement: exists f : s ≃ Fin #s,
  proof: by
  obtain ⟨f, hf⟩ := P.exists_enumeration
  obtain ⟨g, hg⟩ := hP.exists_partsEquiv
  let z := fun a => #P.parts * (f a).2 + g (f a).1
  have gl := fun a => (g (f a).1).2
  have less : forall a, z a < #s := fun a => by
    rcases hP.card_parts_eq_average (f a).1.2 with (c | c)
    · calc
        _ 

中文:
定理 IsEquipartition.exists_partPreservingEquiv
  条件: (hP : P.IsEquipartition)
  结论: 存在 f : s ≃ Fin #s,
  证明: by
  obtain ⟨f, hf⟩ := P.exists_enumeration
  obtain ⟨g, hg⟩ := hP.exists_partsEquiv
  let z := fun a => #P.parts * (f a).2 + g (f a).1
  have gl := fun a => (g (f a).1).2
  have less : forall a, z a < #s := fun a => by
    rcases hP.card_parts_eq_average (f a).1.2 with (c | c)
    · calc
        _ 

Depends on / 依赖: Nat.le_add_right, P.exists_enumeration, P.parts, card_parts_eq_average, exists_enumeration, exists_partsEquiv, hP.card_parts_eq_average, hP.exists_partsEquiv, le_add_right, mul_add_one
-/
theorem IsEquipartition.exists_partPreservingEquiv (hP : P.IsEquipartition) : exists f : s ≃ Fin #s,
    forall a b : s, P.part a = P.part b ↔ f a % #P.parts = f b % #P.parts := by
  obtain ⟨f, hf⟩ := P.exists_enumeration
  obtain ⟨g, hg⟩ := hP.exists_partsEquiv
  let z := fun a => #P.parts * (f a).2 + g (f a).1
  have gl := fun a => (g (f a).1).2
  have less : forall a, z a < #s := fun a => by
    rcases hP.card_parts_eq_average (f a).1.2 with (c | c)
    · calc
        _ < #P.parts * ((f a).2 + 1) := by simp only [z, mul_add_one]; gcongr; exact gl a
        _ <= #P.parts * (#s / #P.parts) := by gcongr; exact c ▸ (f a).2.2
        _ <= #P.parts * (#s / #P.parts) + #s % #P.parts := Nat.le_add_right ..
        _ = _ := Nat.div_add_mod ..
    · rw [← Nat.div_add_mod #s #P.parts]
      exact add_lt_add_of_le_of_lt (mul_le_mul_right (by lia) _) ((hg (f a).1).mp c)
  let z' : s -> Fin #s := fun a => ⟨z a, less a⟩
  have bij : z'.Bijective := by
    refine (bijective_iff_injective_and_card z').mpr ⟨fun a b e => ?_, by simp⟩
    simp_rw [z', z, Fin.mk.injEq, mul_comm #P.parts] at e
    have : NeZero #P.parts := ⟨((Nat.zero_le _).trans_lt (gl a)).ne'⟩
    change (#P.parts).divModEquiv.symm (_, _) = (#P.parts).divModEquiv.symm (_, _) at e
    simp only [Equiv.apply_eq_iff_eq, Prod.mk.injEq] at e
    apply_fun f
exact Sigma.ext e.2 (Fin.heq_ext_iff (by rw [e.2])).mpr e.1
  use Equiv.ofBijective _ bij
  intro a b
  simp_rw [z', z, Equiv.ofBijective_apply, hf a b, Nat.mul_add_mod,
    Nat.mod_eq_of_lt (gl a), Nat.mod_eq_of_lt (gl b), Fin.val_eq_val, g.apply_eq_iff_eq]

/-! ### Discrete and indiscrete finpartitions -/


variable (s)

/--
theorem `bot_isEquipartition` / 定理 `bot_isEquipartition`

English:
theorem bot_isEquipartition
  statement: (⊥ : Finpartition s).IsEquipartition
  proof: Set.equitableOn_iff_exists_eq_eq_add_one.2 ⟨1, by simp⟩

中文:
定理 bot_isEquipartition
  结论: (⊥ : Finpartition s).IsEquipartition
  证明: Set.equitableOn_iff_exists_eq_eq_add_one.2 ⟨1, by simp⟩

Depends on / 依赖: Set.equitableOn_iff_exists_eq_eq_add_one, equitableOn_iff_exists_eq_eq_add_one
-/
theorem bot_isEquipartition : (⊥ : Finpartition s).IsEquipartition :=
  Set.equitableOn_iff_exists_eq_eq_add_one.2 ⟨1, by simp⟩

/--
theorem `top_isEquipartition` / 定理 `top_isEquipartition`

English:
theorem top_isEquipartition
  given: [Decidable (s = ∅)]
  statement: (⊤ : Finpartition s).IsEquipartition
  proof: Set.Subsingleton.isEquipartition (parts_top_subsingleton _)

中文:
定理 top_isEquipartition
  条件: [Decidable (s = ∅)]
  结论: (⊤ : Finpartition s).IsEquipartition
  证明: Set.Subsingleton.isEquipartition (parts_top_subsingleton _)

Depends on / 依赖: Set.Subsingleton.isEquipartition, Subsingleton, isEquipartition, parts_top_subsingleton
-/
theorem top_isEquipartition [Decidable (s = ∅)] : (⊤ : Finpartition s).IsEquipartition :=
  Set.Subsingleton.isEquipartition (parts_top_subsingleton _)

/--
theorem `indiscrete_isEquipartition` / 定理 `indiscrete_isEquipartition`

English:
theorem indiscrete_isEquipartition
  given: {hs : s != ∅}
  statement: (indiscrete hs).IsEquipartition
  proof: by
  rw [IsEquipartition]; rw [indiscrete_parts]; rw [coe_singleton]
  exact Set.equitableOn_singleton s _

中文:
定理 indiscrete_isEquipartition
  条件: {hs : s != ∅}
  结论: (indiscrete hs).IsEquipartition
  证明: by
  rw [IsEquipartition]; rw [indiscrete_parts]; rw [coe_singleton]
  exact Set.equitableOn_singleton s _

Depends on / 依赖: IsEquipartition, Set.equitableOn_singleton, coe_singleton, equitableOn_singleton, indiscrete_parts
-/
theorem indiscrete_isEquipartition {hs : s != ∅} : (indiscrete hs).IsEquipartition := by
  rw [IsEquipartition]; rw [indiscrete_parts]; rw [coe_singleton]
  exact Set.equitableOn_singleton s _

end Finpartition
