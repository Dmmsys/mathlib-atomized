...
noncomputable
/--
Definition of `isLimitOfIsPullbackOfIsConnected` / `isLimitOfIsPullbackOfIsConnected` 的定义

English:
definition isLimitOfIsPullbackOfIsConnected
  body: (hf (Classical.arbitrary _)).lift
...
定义 isLimitOfIsPullbackOfIsConnected
  定义体: (hf (Classical.arbitrary _)).lift
    (s.π.app (Classical.arbitrary _)) (hcG.lift ((Cone.postcompose α).obj s)) (by simp)
  fac s j := by
    let f (i : _) : s.pt ⟶ cF.pt :=
      (hf i).lift (s.π.app i) (hcG.lift ((Cone.postcompose α).obj s)) (by simp)
...
def isLimitOfIsPullbackOfIsConnected
    ...
    (hf : forall i, IsPullback (cF.π.app i) f.hom (α.app i) (cG.π.app i))
...
noncomputable
/--
Definition of `isColimitOfIsPushoutOfIsConnected` / `isColimitOfIsPushoutOfIsConnected` 的定义

English:
definition isColimitOfIsPushoutOfIsConnected
  body: (hf (Classical.arbitrary _)).desc
...
定义 isColimitOfIsPushoutOfIsConnected
  定义体: (hf (Classical.arbitrary _)).desc
    (hcF.desc ((Cocone.precompose α).obj s)) (s.ι.app (Classical.arbitrary _)) (by simp)
  fac s j := by
    let f (i : _) : cG.pt ⟶ s.pt :=
      (hf i).desc (hcF.desc ((Cocone.precompose α).obj s)) (s.ι.app i) (by simp)
...
def isColimitOfIsPushoutOfIsConnected
    ...
    (hf : forall i, IsPushout (cF.ι.app i) (α.app i) f.hom (cG.ι.app i))
