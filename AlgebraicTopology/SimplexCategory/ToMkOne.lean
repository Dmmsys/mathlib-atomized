...
/-
**SimplexCategory.toMk** 是 Mathlib 中的一个定义，位于命名空间 `SimplexCategory`。
...
@[local grind =]
/-
**SimplexCategory.toMk** 是 Mathlib 中的一个引理，位于命名空间 `SimplexCategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
(replacing grind's canonicalizer with a type-directed normalizer), `grind` close
d the goals in
the four lemmas below. It is not yet clear whether this is due to defeq abuse in
 Mathlib or a
...
--- 原说明 ---
The bijection `Fin (n + 2) ≃ (⦋n⦌ ⟶ ⦋1⦌)` which sends `i : Fin (n + 2)` to the
morphism `⦋n⦌ ⟶ ⦋1⦌` in the simplex category which corresponds to the monotone m
ap
`Fin (n + 1) → Fin 2` which takes `i` times the value `0`.
-/
