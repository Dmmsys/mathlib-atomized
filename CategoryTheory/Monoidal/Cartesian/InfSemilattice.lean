...
English:
lemma tensorObj
  given: {C : Type u} [SemilatticeInf C] [OrderTop C] {X Y : C}
...
引理 tensorObj
  ...
  证明: rfl
-/
lemma tensorObj {C : Type u} [SemilatticeInf C] [OrderTop C] {X Y : C} : X otimes Y = X ⊓ Y := rfl
...
English:
lemma tensorUnit
  given: {C : Type u} [SemilatticeInf C] [OrderTop C]
  proof: rfl
