...
English:
definition tensor
  signature: : (relations₁.tensor relations₂).Solution (M₁ otimes[A] M₂) where
  body: fun ⟨g₁, g₂⟩ => solution₁.var g₁ otimesₜ solution₂.var g₂
...
set_option backward.isDefEq.respectTransparency false in
/--
Definition of `isPresentationCoreTensor` / `isPresentationCoreTensor` 的定义

English:
definition isPresentationCoreTensor
  signature: :
  body: uncurry _ _ _ _ (h₁.desc
    { var := fun g₁ => h₂.desc
        { var := fun g₂ => s.var ⟨g₁, g₂⟩
...
include h₁ h₂ in
/--
lemma `IsPresentation.tensor` / 引理 `IsPresentation.tensor`

English:
lemma IsPresentation.tensor
  statement: (solution₁.tensor solution₂).IsPresentation
...
English:
definition tensor
  signature: : Presentation A (M₁ otimes[A] M₂) where
  body: _
  R := _
  relation := _
...
定义 tensor
  ...
  toIsPresentation := pres₁.toIsPresentation.tensor pres₂.toIsPresentation
-/
noncomputable def tensor : Presentation A (M₁ otimes[A] M₂) where
  G := _
  R := _
  relation := _
  toSolution := pres₁.toSolution.tensor pres₂.toSolution
