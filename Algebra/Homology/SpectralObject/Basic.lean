...
/-
**CategoryTheory.Abelian.SpectralObject** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryThe
ory.Abelian`。
形式化陈述：(C : Type u_1) →   (ι : Type u_2) →     [inst : CategoryTheory.Category.{u
_3, u_1} C] →       [CategoryTheory.Category.{u_4, u_2} ι] → [CategoryTheory.Abe
lian C] → Type (max (max (max u_1 u_2) u_3) u_4)
参数：max (max u_1 u_2) u_3。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A spectral object in an abelian category category `C` indexed by a category `ι`
consists of a family of functors `H n : ComposableArrows ι 1 ⥤ C` for all `n : ℤ
`, and a
functorial long exact sequence
`⋯ ⟶ (H n₀).obj (mk₁ f) ⟶ (H n₀).obj (mk₁ (f ≫ g)) ⟶ (H n₀).obj (mk₁ g) ⟶ (H n₁)
.obj (mk₁ f) ⟶ ⋯`
when `n₀ + 1 = n₁` and `f` and `g` are composable morphisms in `ι`. (This will b
e
shortened as `H^n₀(f) ⟶ H^n₀(f ≫ g) ⟶ H^n₀(g) ⟶ H^n₁(f)` in the documentation.)
-/
...
@[reassoc (attr := simp)]
/-
**CategoryTheory.Abelian.SpectralObject.zero** 是 Mathlib 中的一个引理，位于命名空间 `Category
...
@[simps]
/-
**CategoryTheory.Abelian.SpectralObject.sc** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTh
...
def sc₃ (n₀ n₁ : ℤ) (hn₁ : n₀ + 1 = n₁ := by lia) : ShortComplex C :=
  ShortComplex.mk _ _ (X.zero₃ f g fg h n₀ n₁ hn₁)
/-
**CategoryTheory.Abelian.SpectralObject.exact** 是 Mathlib 中的一个引理，位于命名空间 `Categor
...
of a spectral object, when `f ≫ g = fg` and `n₀ + 1 = n₁`. -/
/-
**CategoryTheory.Abelian.SpectralObject.composableArrows** 是 Mathlib 中的一个缩写定义，位于
...
/-
**CategoryTheory.Abelian.SpectralObject.composableArrows** 是 Mathlib 中的一个引理，位于命名
...
@[ext]
/-
**CategoryTheory.Abelian.SpectralObject.Hom** 是 Mathlib 中的一个结构，位于命名空间 `CategoryT
heory.Abelian.SpectralObject`。
形式化陈述：Hom (X' : SpectralObject C ι) where /-- The natural transformation that is
 part of a morphism between spectral objects. -/ hom (n : Int) : X.H n ⟶ X'.H n 
comm (n₀ n₁ : Int) (hn₁ : n₀ + 1 = n₁) {i j k : ι} (f : i ⟶ j) (g : j ⟶ k) : X.δ
 f g n₀ n₁ hn₁ ≫ (hom n₁).app (mk₁ f) = (hom n₀).app (mk₁ g) ≫ X'.δ f g n₀ n₁ hn
₁
参数：X' : SpectralObject C ι；n : Int。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
set_option backward.defeqAttrib.useBackward true in
/-
**CategoryTheory.Abelian.SpectralObject.isZero_H_map_mk** 是 Mathlib 中的一个引理，位于命名空
...
include h₁ in
/-
**CategoryTheory.Abelian.SpectralObject.mono_H_map_two** 是 Mathlib 中的一个引理，位于命名空间
...
include h₂ hn₁ in
/-
**CategoryTheory.Abelian.SpectralObject.epi_H_map_two** 是 Mathlib 中的一个引理，位于命名空间 
`CategoryTheory.Abelian.SpectralObject`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include h₁ h₂ hn₁ in
/-
**CategoryTheory.Abelian.SpectralObject.isIso_H_map_two** 是 Mathlib 中的一个引理，位于命名空
间 `CategoryTheory.Abelian.SpectralObject`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
