...
lemma cpow_mul_div_cpow_eq_div_div_cpow
  ...
  rw [show (n + 1 : Complex)⁻¹ = (n + 1 : Real)⁻¹ by simp]; rw [show (n + 1 : Complex) = (n + 1 : Real) by norm_cast]; rw
...
lemma LSeries.tendsto_cpow_mul_atTop
  ...
  let F := fun (x : Real) => {m | n + 1 < m}.indicator (fun m => f m / (m / (n + 1) : Complex)
...
lemma LSeries.tendsto_atTop
  ...
  convert! LSeries.tendsto_cpow_mul_atTop (n
...
Eventually.of_forall by simp [LSeries_eq_zero_of_abscissaOfAbsConv_eq_top h]
  · simp only [ne_eq, h, or_false]
    ...
    · let F (n : Nat) : Complex := if n = 0 then 0 else f
...
lemma LSeries_sub_eventuallyEq_zero_of_LSeries_eventually_eq
  ...
  lift yg to Real using ⟨hyg₂.ne, ((OrderBot.bot_le _)
