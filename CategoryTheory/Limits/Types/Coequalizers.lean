/-
Copyright (c) 2020 Kim Morrison. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Kim Morrison
-/
module

public import Mathlib.CategoryTheory.Limits.Shapes.Equalizers
public import Mathlib.CategoryTheory.Limits.Types.Colimits
public import Mathlib.Logic.Function.Coequalizer
public import Mathlib.Tactic.CategoryTheory.Elementwise

/-!
# Coequalizers in Type

The coequalizer of a pair of maps `(f, g)` from `X` to `Y`
is the quotient of `Y` by `∀ x : Y, f x ~ g x`

-/

@[expose] public section

universe v u

open CategoryTheory Limits ConcreteCategory

namespace CategoryTheory.Limits.Types

variable {X Y Z : Type u} (f g : X ⟶ Y)

/--
Definition of `coequalizerColimit` / `coequalizerColimit` 的定义

English:
definition coequalizerColimit
  signature: : Limits.ColimitCocone (parallelPair f g) where
  body: Cofork.ofπ (↾(Function.Coequalizer.mk f g))
      (by ext x; exact Function.Coequalizer.condition f g x)
  isColimit :=
    Cofork.IsColimit.mk _
      (fun s => ↾(Function.Coequalizer.desc f g s.π
        (by ext x; exact ConcreteCategory.congr_hom s.condition x)))
      (fun _ => rfl)
      (fun _

中文:
定义 coequalizerColimit
  签名: : Limits.ColimitCocone (parallelPair f g) where
  定义体: Cofork.ofπ (↾(Function.Coequalizer.mk f g))
      (by ext x; exact Function.Coequalizer.condition f g x)
  isColimit :=
    Cofork.IsColimit.mk _
      (fun s => ↾(Function.Coequalizer.desc f g s.π
        (by ext x; exact ConcreteCategory.congr_hom s.condition x)))
      (fun _ => rfl)
      (fun _

Depends on / 依赖: Coequalizer, Cofork, Cofork.IsColimit.mk, Cofork.of, ConcreteCategory, ConcreteCategory.congr_hom, Function, Function.Coequalizer.condition, Function.Coequalizer.desc, Function.Coequalizer.mk, IsColimit, Quot.inductionOn, condition, congr_hom, inductionOn, isColimit, s.condition
-/
def coequalizerColimit : Limits.ColimitCocone (parallelPair f g) where
  cocone :=
    Cofork.ofπ (↾(Function.Coequalizer.mk f g))
      (by ext x; exact Function.Coequalizer.condition f g x)
  isColimit :=
    Cofork.IsColimit.mk _
      (fun s => ↾(Function.Coequalizer.desc f g s.π
        (by ext x; exact ConcreteCategory.congr_hom s.condition x)))
      (fun _ => rfl)
      (fun _ _ hm => by ext x; exact Quot.inductionOn x (congr_hom hm))

/--
theorem `coequalizer_preimage_image_eq_of_preimage_eq` / 定理 `coequalizer_preimage_image_eq_of_preimage_eq`

English:
theorem coequalizer_preimage_image_eq_of_preimage_eq
  statement: (π : Y ⟶ Z) (e : f ≫ π = g ≫ π)
  proof: by
  have lem : forall x y, Function.Coequalizer.Rel f g x y -> (x in U ↔ y in U) := by
    rintro _ _ ⟨x⟩
    change x in f ⁻¹' U ↔ x in g ⁻¹' U
    rw [H]
  have eqv : _root_.Equivalence fun x y => x in U ↔ y in U := by
    aesop (add safe constructors _root_.Equivalence)
  ext
  constructor
  · r

中文:
定理 coequalizer_preimage_image_eq_of_preimage_eq
  结论: (π : Y ⟶ Z) (e : f ≫ π = g ≫ π)
  证明: by
  have lem : forall x y, Function.Coequalizer.Rel f g x y -> (x in U ↔ y in U) := by
    rintro _ _ ⟨x⟩
    change x in f ⁻¹' U ↔ x in g ⁻¹' U
    rw [H]
  have eqv : _root_.Equivalence fun x y => x in U ↔ y in U := by
    aesop (add safe constructors _root_.Equivalence)
  ext
  constructor
  · r

Depends on / 依赖: Coequalizer, Equivalence, Function, Function.Coequalizer.Rel, WalkingParallelPair, WalkingParallelPair.one, _root_, _root_.Equivalence, coconePointUniqueUpToIso, coequalizerColimit, comp_coconePointUniqueUpToIso_inv, constructors, h.coconePointUniqueUpToIso, h.comp_coconePointUniqueUpToIso_inv, mono_iff_injective
-/
theorem coequalizer_preimage_image_eq_of_preimage_eq (π : Y ⟶ Z) (e : f ≫ π = g ≫ π)
    (h : IsColimit (Cofork.ofπ π e)) (U : Set Y) (H : f ⁻¹' U = g ⁻¹' U) : π ⁻¹' π '' U = U := by
  have lem : forall x y, Function.Coequalizer.Rel f g x y -> (x in U ↔ y in U) := by
    rintro _ _ ⟨x⟩
    change x in f ⁻¹' U ↔ x in g ⁻¹' U
    rw [H]
  have eqv : _root_.Equivalence fun x y => x in U ↔ y in U := by
    aesop (add safe constructors _root_.Equivalence)
  ext
  constructor
  · rw [←
      show _ = π from
        h.comp_coconePointUniqueUpToIso_inv (coequalizerColimit f g).2
          WalkingParallelPair.one]
    rintro ⟨y, hy, e'⟩
    dsimp at e'
    have e'' :=
      (mono_iff_injective
            (h.coconePointUniqueUpToIso (coequalizerColimit f g).isColimit).inv).mp
        inferInstance
    refine (eqv.eqvGen_iff.mp (Relation.EqvGen.mono lem y _ (Quot.eqvGen_exact ?_))).mp hy
    apply e''
    convert! e'
  · exact fun hx => ⟨_, hx, rfl⟩

/--
Definition of `coequalizerIso` / `coequalizerIso` 的定义

English:
definition coequalizerIso
  signature: : coequalizer f g ≅ (Function.Coequalizer f g)
  body: colimit.isoColimitCocone (coequalizerColimit f g)

@[elementwise (attr := simp)]

中文:
定义 coequalizerIso
  签名: : coequalizer f g ≅ (Function.Coequalizer f g)
  定义体: colimit.isoColimitCocone (coequalizerColimit f g)

@[elementwise (attr := simp)]

Depends on / 依赖: coequalizerColimit, colimit, colimit.isoColimitCocone, isoColimitCocone
-/
noncomputable def coequalizerIso : coequalizer f g ≅ (Function.Coequalizer f g) :=
  colimit.isoColimitCocone (coequalizerColimit f g)

@[elementwise (attr := simp)]
/--
theorem `coequalizerIso_π_comp_hom` / 定理 `coequalizerIso_π_comp_hom`

English:
theorem coequalizerIso_π_comp_hom
  proof: colimit.isoColimitCocone_ι_hom (coequalizerColimit f g) WalkingParallelPair.one

@[elementwise (attr := simp)]

中文:
定理 coequalizerIso_π_comp_hom
  证明: colimit.isoColimitCocone_ι_hom (coequalizerColimit f g) WalkingParallelPair.one

@[elementwise (attr := simp)]

Depends on / 依赖: WalkingParallelPair, WalkingParallelPair.one, coequalizerColimit, colimit, colimit.isoColimitCocone_
-/
theorem coequalizerIso_π_comp_hom :
    coequalizer.π f g ≫ (coequalizerIso f g).hom = ↾(Function.Coequalizer.mk f g) :=
  colimit.isoColimitCocone_ι_hom (coequalizerColimit f g) WalkingParallelPair.one

@[elementwise (attr := simp)]
/--
theorem `coequalizerIso_quot_comp_inv` / 定理 `coequalizerIso_quot_comp_inv`

English:
theorem coequalizerIso_quot_comp_inv
  proof: rfl

中文:
定理 coequalizerIso_quot_comp_inv
  证明: rfl
-/
theorem coequalizerIso_quot_comp_inv :
    ↾(Function.Coequalizer.mk f g) ≫ (coequalizerIso f g).inv = coequalizer.π f g :=
  rfl

end CategoryTheory.Limits.Types
