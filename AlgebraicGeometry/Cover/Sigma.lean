...
English:
definition sigma
  signature: (𝒰 : Cover.{v} (precoverage P) S)
  body: PUnit.{v + 1}
  X _ := ∐ 𝒰.X
  f _ := Sigma.desc 𝒰.f
  mem₀ := by
...
@[simp]
/--
lemma `presieve₀_sigma` / 引理 `presieve₀_sigma`

English:
lemma presieve₀_sigma
  given: {S : Scheme.{u}} (𝒰 : Cover.{v} (precoverage P) S)
  proof: by
...
Depends on / 依赖: Presieve, Presieve.ofArrows_le_iff, Presieve.singleton_self, le_antisymm, ofArrows_le_iff, singleton_self
-/
...
English:
instance :
  signature: Unique 𝒰.sigma.I₀
  body: inferInstanceAs Unique PUnit.{v + 1}
...
@[simps]
/--
Definition of `Hom.sigma` / `Hom.sigma` 的定义

English:
definition Hom.sigma
  signature: (f : 𝒰 ⟶ 𝒱)
  body: default
...
definition sigmaFunctor
  signature: : S.Cover (precoverage P) ⥤ S.Cover (precoverage P) where
  body: 𝒰.sigma
...
中文:
定义 sigmaFunctor
  签名: : S.Cover (precoverage P) ⥤ S.Cover (precoverage P) where
  定义体: 𝒰.sigma
  map f := Scheme.Cover.Hom.sigma f
map_id 𝒰 := PreZeroHypercover.Hom.ext rfl by
    simp only [sigma_I₀, sigma_X, heq_eq_eq]
