...
/--
theorem `tendsto_apply_add_mul_sq_div_sub` / 定理 `tendsto_apply_add_mul_sq_div_sub`

English:
theorem tendsto_apply_add_mul_sq_div_sub
  statement: {f : Real -> Real} {x a c d : Real} {l : Filter Real} (hl : l <= 𝓝[!=] x)
  proof: by
...
Depends on / 依赖: Eventually, Eventually.filter_mono, Tendsto, Tendsto.congr, Tendsto.mono_left, _root_, _root_.sub_self, add_zero, const_add, const_mul, filter_mono, filter_upwards, hf.comp, hl.trans, mono_left, mul_one, mul_zero, nhdsWithin_le_nhds, self_mem_nhdsWithin, sub_const
-/
theorem tendsto_apply_add_mul_sq_div_sub {f : Real -> Real} {x a c d : Real} {l : Filter Real} (hl : l <= 𝓝[!=] x)
  ...
  have : forallᶠ y in l, y + c * (y - x) ^ 2 != x := by apply Tendsto.mono_right h' hl self_mem_nhdsWithin
  filter_upwards [this] with y hy
  simp [field]

/--
theorem `StieltjesFunction.ae_hasDerivAt` / 定理 `StieltjesFunction.ae_hasDerivAt`

English:
theorem StieltjesFunction.ae_hasDerivAt
  given: (f : StieltjesFunction Real)
  proof: by
...
定理 StieltjesFunction.ae_hasDerivAt
  ...
  /- Denote by `μ` the Stieltjes measure associated to `f`.
    ...
    and `f (x^-) = f x` almost everywh
-/
theorem StieltjesFunction.ae_hasDerivAt (f : StieltjesFunction Real) :
    forallᵐ x, HasDerivAt f (rnDeriv f.measure volume x).toReal x := by
  /- Denote by `μ` the Stieltjes measure associated to `f`.
    The general theorem `VitaliFamily.ae_tendsto_rnDeriv` ensures that `μ [x, y] / (y - x)` tends
    to the Radon-Nikodym derivative as `y` tends to `x` from the right. As `μ [x,y] = f y - f (x^-)`
  ...
  filter_upwards [VitaliFamily.ae_tendsto_rnDeriv (vitaliFamily (volume : Measure Real) 1) f.measure,
  ...
  have L3 : Tendsto (fun y => (leftLim f (y + 1 * (y - x) ^ 2) - f x) / (y - x)) (𝓝[<] x)
    ...
    · apply Tendsto.mono_left _ nhdsWithin_le_nhds
      have : Tendsto (fun y : Real => y + ↑1 * (y - x) ^ 2) (𝓝 x) (𝓝 (x + ↑1 * (x - x) ^ 2)) :=
  ...
  rw [hasDerivAt_iff_tendsto_slope]; rw [slope_fun_def_field]; rw [← nhdsLT_sup_nhdsGT]; rw [tendsto_sup]
  exact ⟨L4, L1⟩

/--
theorem `Monotone.ae_hasDerivAt` / 定理 `Monotone.ae_hasDerivAt`

English:
theorem Monotone.ae_hasDerivAt
  given: {f : Real -> Real} (hf : Monotone f)
  proof: by
...
定理 Monotone.ae_hasDerivAt
  ...
  /- We already know that the Stieltjes function associated to `f` (i.e., `g : x ↦ f (x^+)`) is
    ...
    scale `y 
-/
theorem Monotone.ae_hasDerivAt {f : Real -> Real} (hf : Monotone f) :
    forallᵐ x, HasDerivAt f (rnDeriv hf.stieltjesFunction.measure volume x).toReal x := by
  /- We already know that the Stieltjes function associated to `f` (i.e., `g : x ↦ f (x^+)`) is
    differentiable almost everywhere. We reduce to this statement by sandwiching values of `f` with
    values of `g`, by shifting with `(y - x)^2` (which has no influence on the relevant
    scale `y - x`.) -/
  ...
  have A : hf.stieltjesFunction x = f x := by
    rw [Classical.not_not]; rw [hf.continuousAt_iff_leftLim_eq_rightLim] at h'x
    apply le_antisymm _ (hf.le_rightLim (le_refl _))
    rw [← h'x]
    exact hf.leftLim_le (le_refl _)
  rw [hasDerivAt_iff_tendsto_slope]; rw [(nhdsLT_sup_nhdsGT x).symm]; rw [tendsto_sup]; rw [slope_fun_def_field]; rw [A] at hx
...
/--
theorem `Monotone.ae_differentiableAt` / 定理 `Monotone.ae_differentiableAt`
...
Depends on / 依赖: ae_hasDerivAt, differentiableAt, filter_upwards, hf.ae_hasDerivAt, hx.differentiableAt
-/
theorem Monotone.ae_differentiableAt {f : Real -> Real} (hf : Monotone f) :
    forallᵐ x, DifferentiableAt Real f x := by
  filter_upwards [hf.ae_hasDerivAt] with x hx using hx.differentiableAt

/--
theorem `MonotoneOn.ae_differentiableWithinAt_of_mem` / 定理 `MonotoneOn.ae_differentiableWithinAt_of_mem`

English:
theorem MonotoneOn.ae_differentiableWithinAt_of_mem
  given: {f : Real -> Real} {s : Set Real} (hf : MonotoneOn f s)
  proof: by
...
定理 MonotoneOn.ae_differentiableWithinAt_of_mem
  ...
  apply ae_of_mem_of_ae_o
-/
theorem MonotoneOn.ae_differentiableWithinAt_of_mem {f : Real -> Real} {s : Set Real} (hf : MonotoneOn f s) :
    forallᵐ x, x in s -> DifferentiableWithinAt Real f s x := by
  /- We use a global monotone extension of `f`, and argue that this extension is differentiable
    almost everywhere. Such an extension need not exist (think of `1/x` on `(0, +∞)`), but it exists
    if one restricts first the function to a compact interval `[a, b]`. -/
  apply ae_of_mem_of_ae_of_mem_inter_Ioo
  intro a b as bs _
  obtain ⟨g, hg, gf⟩ : exists g : Real -> Real, Monotone g ∧ EqOn f g (s inter Icc a b) :=
  ...
  have : Ioo a b in 𝓝[s] x := nhdsWithin_le_nhds (Ioo_mem_nhds h'x.2.1 h'x.2.2)
  filter_upwards [self_mem_nhdsWithin, this] with y hy h'y
  exact gf ⟨hy, h'y.1.le, h'y.2.le⟩

/--
theorem `MonotoneOn.ae_differentiableWithinAt` / 定理 `MonotoneOn.ae_differentiableWithinAt`

English:
theorem MonotoneOn.ae_differentiableWithinAt
  statement: {f : Real -> Real} {s : Set Real} (hf : MonotoneOn f s)
  proof: by
...
-/
theorem MonotoneOn.ae_differentiableWithinAt {f : Real -> Real} {s : Set Real} (hf : MonotoneOn f s)
