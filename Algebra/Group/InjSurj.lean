...
/-
**Function.Injective.semigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: Semigroup M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (x y : M₁), 
f (x * y) = f x * f y) → Semigroup M₁
...
/-
**Function.Injective.commMagma** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: CommMagma M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (x y : M₁), 
f (x * y) = f x * f y) → CommMagma M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Function.Injective.commSemigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective
`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: CommSemigroup M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (x y : M
₁), f (x * y) = f x * f y) → CommSemigroup M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
...
/-
**Function.Injective.isLeftCancelMul** 是 Mathlib 中的一个定理，位于命名空间 `Function.Injecti
ve`。
形式化陈述：∀ {M₁ : Type u_1} {M₂ : Type u_2} [inst : Mul M₁] [inst_1 : Mul M₂] [IsLef
tCancelMul M₂] (f : M₁ → M₂),   Function.Injective f → (∀ (x y : M₁), f (x * y) 
= f x * f y) → IsLeftCancelMul M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Function.Injective.isRightCancelMul** 是 Mathlib 中的一个定理，位于命名空间 `Function.Inject
ive`。
形式化陈述：∀ {M₁ : Type u_1} {M₂ : Type u_2} [inst : Mul M₁] [inst_1 : Mul M₂] [IsRig
htCancelMul M₂] (f : M₁ → M₂),   Function.Injective f → (∀ (x y : M₁), f (x * y)
 = f x * f y) → IsRightCancelMul M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Function.Injective.isCancelMul** 是 Mathlib 中的一个定理，位于命名空间 `Function.Injective`。
形式化陈述：∀ {M₁ : Type u_1} {M₂ : Type u_2} [inst : Mul M₁] [inst_1 : Mul M₂] [IsCan
celMul M₂] (f : M₁ → M₂),   Function.Injective f → (∀ (x y : M₁), f (x * y) = f 
x * f y) → IsCancelMul M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Function.Injective.leftCancelSemigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Inj
ective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: LeftCancelSemigroup M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (x
 y : M₁), f (x * y) = f x * f y) → LeftCancelSemigroup M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
semigroup. -/]
/-
**Function.Injective.rightCancelSemigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.In
jective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: RightCancelSemigroup M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (
x y : M₁), f (x * y) = f x * f y) → RightCancelSemigroup M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Function.Injective.mulOneClass** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : MulOneClass M₂] →           (f : M₁ → M₂) → Functi
on.Injective f → f 1 = 1 → (∀ (x y : M₁), f (x * y) = f x * f y) → MulOneClass M
₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Function.Injective.monoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Monoid M₂] →      
       (f : M₁ → M₂) →               Function.Injective f →                 f 1 
= 1 → (∀ (x y : M₁), f (x * y) = f x * f y) → (∀ (x : M₁) (n : ℕ), f (x ^ n) = f
 x ^ n) → Monoid M₁
...
/-
**Function.Injective.leftCancelMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Inject
ive`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : LeftCancelMonoid M
₂] →             (f : M₁ → M₂) →               Function.Injective f →           
...
admits an injective map that preserves `0` and `+` to an additive left cancel monoid. -/]
/-
**Function.Injective.rightCancelMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injec
tive`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : RightCancelMonoid 
M₂] →             (f : M₁ → M₂) →               Function.Injective f →          
       f 1 = 1 →                   (∀ (x y : M₁), f (x * y) = f x * f y) →      
...
admits an injective map that preserves `0` and `+` to an additive left cancel monoid. -/]
/-
**Function.Injective.cancelMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`
。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : CancelMonoid M₂] →
             (f : M₁ → M₂) →               Function.Injective f →               
  f 1 = 1 →                   (∀ (x y : M₁), f (x * y) = f x * f y) → (∀ (x : M₁
) (n : ℕ), f (x ^ n) = f x ^ n) → CancelMonoid M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁) (n : ℕ), f (x ^ n)
 = f x ^ n。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `RightCancelMonoid.toIsRightCancelMul`：∀ {M : Type u} [self : RightCancel
Monoid M], IsRightCancelMul M
-/
...
/-
**Function.Injective.commMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : CommMonoid M₂] →  
           (f : M₁ → M₂) →               Function.Injective f →                 
f 1 = 1 →                   (∀ (x y : M₁), f (x * y) = f x * f y) → (∀ (x : M₁) 
(n : ℕ), f (x ^ n) = f x ^ n) → CommMonoid M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁) (n : ℕ), f (x ^ n)
 = f x ^ n。
本定义的构造引用了以下数学事实（定理与引理）：
...
/-
**Function.Injective.cancelCommMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Inject
ive`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : CancelCommMonoid M
₂] →             (f : M₁ → M₂) →               Function.Injective f →           
      f 1 = 1 →                   (∀ (x y : M₁), f (x * y) = f x * f y) →       
...
/-
**Function.Injective.involutiveInv** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective
`。
形式化陈述：{M₂ : Type u_2} →   {M₁ : Type u_3} →     [inst : Inv M₁] →       [inst_1 
: InvolutiveInv M₂] →         (f : M₁ → M₂) → Function.Injective f → (∀ (x : M₁)
, f x⁻¹ = (f x)⁻¹) → InvolutiveInv M₁
参数：f : M₁ → M₂；∀ (x : M₁), f x⁻¹ = (f x)⁻¹。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Function.Injective.invOneClass** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : One M₁] →       [inst_1 
: Inv M₁] →         [inst_2 : InvOneClass M₂] →           (f : M₁ → M₂) → Functi
on.Injective f → f 1 = 1 → (∀ (x : M₁), f x⁻¹ = (f x)⁻¹) → InvOneClass M₁
参数：f : M₁ → M₂；∀ (x : M₁), f x⁻¹ = (f x)⁻¹。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
arguments. -/]
/-
**Function.Injective.divInvMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`
。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : DivInvMonoid M₂] →                   (f : M₁ → M₂) →                     F
unction.Injective f →                       f 1 = 1 →                         (∀
 (x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f x
⁻¹ = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f y
) →                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →   
                              (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → DivInv
Monoid M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
`[SMul ℤ M₁]` arguments. -/]
/-
**Function.Injective.divInvOneMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injecti
ve`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : DivInvOneMonoid M₂] →                   (f : M₁ → M₂) →                   
  Function.Injective f →                       f 1 = 1 →                        
 (∀ (x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), 
f x⁻¹ = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / 
f y) →                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →
                                 (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → Div
InvOneMonoid M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
...
/-
**Function.Injective.divisionMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injectiv
e`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : DivisionMonoid M₂] →                   (f : M₁ → M₂) →                    
 Function.Injective f →                       f 1 = 1 →                         
(∀ (x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f
 x⁻¹ = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f
 y) →                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) → 
                                (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → Divi
sionMonoid M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
...
/-
**Function.Injective.divisionCommMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Inje
ctive`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : DivisionCommMonoid M₂] →                   (f : M₁ → M₂) →                
     Function.Injective f →                       f 1 = 1 →                     
    (∀ (x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁
), f x⁻¹ = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x
 / f y) →                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n
...
/-
**Function.Injective.group** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : Group M₂] →                   (f : M₁ → M₂) →                     Function
.Injective f →                       f 1 = 1 →                         (∀ (x y :
 M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f x⁻¹ = (f
 x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f y) →    
                           (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →          
...
/-
**Function.Injective.commGroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Injective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₁] →       [inst_1 
: One M₁] →         [inst_2 : Pow M₁ ℕ] →           [inst_3 : Inv M₁] →         
    [inst_4 : Div M₁] →               [inst_5 : Pow M₁ ℤ] →                 [ins
t_6 : CommGroup M₂] →                   (f : M₁ → M₂) →                     Func
tion.Injective f →                       f 1 = 1 →                         (∀ (x
 y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f x⁻¹ 
= (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f y) →
                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →      
                           (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → CommGroup
 M₁
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
...
/-
**Function.Surjective.semigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: Semigroup M₁] →         (f : M₁ → M₂) → Function.Surjective f → (∀ (x y : M₁),
 f (x * y) = f x * f y) → Semigroup M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
a surjective map that preserves `+` from an additive commutative semigroup. -/]
/-
**Function.Surjective.commMagma** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: CommMagma M₁] →         (f : M₁ → M₂) → Function.Surjective f → (∀ (x y : M₁),
 f (x * y) = f x * f y) → CommMagma M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
a surjective map that preserves `+` from an additive commutative semigroup. -/]
/-
**Function.Surjective.commSemigroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjecti
ve`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: CommSemigroup M₁] →         (f : M₁ → M₂) → Function.Surjective f → (∀ (x y : 
M₁), f (x * y) = f x * f y) → CommSemigroup M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `CommMagma.mul_comm`：∀ {G : Type u} [self : CommMagma G] (a b : G), a * b
 = b * a
...
/-
**Function.Surjective.mulOneClass** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective
`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : MulOneClass M₁] →           (f : M₁ → M₂) → Functi
on.Surjective f → f 1 = 1 → (∀ (x y : M₁), f (x * y) = f x * f y) → MulOneClass 
M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Function.Surjective.monoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : Pow M₂ ℕ] →           [inst_3 : Monoid M₁] →      
       (f : M₁ → M₂) →               Function.Surjective f →                 f 1
 = 1 → (∀ (x y : M₁), f (x * y) = f x * f y) → (∀ (x : M₁) (n : ℕ), f (x ^ n) = 
f x ^ n) → Monoid M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁) (n : ℕ), f (x ^ n)
 = f x ^ n。
本定义的构造引用了以下数学事实（定理与引理）：
...
/-
**Function.Surjective.commMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`
。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : Pow M₂ ℕ] →           [inst_3 : CommMonoid M₁] →  
           (f : M₁ → M₂) →               Function.Surjective f →                
 f 1 = 1 →                   (∀ (x y : M₁), f (x * y) = f x * f y) → (∀ (x : M₁)
 (n : ℕ), f (x ^ n) = f x ^ n) → CommMonoid M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁) (n : ℕ), f (x ^ n)
 = f x ^ n。
...
preserves `-` to a type which has an involutive negation. -/]
/-
**Function.Surjective.involutiveInv** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjecti
ve`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_3} →     [inst : Inv M₂] →       [inst_1 
: InvolutiveInv M₁] →         (f : M₁ → M₂) → Function.Surjective f → (∀ (x : M₁
), f x⁻¹ = (f x)⁻¹) → InvolutiveInv M₂
参数：f : M₁ → M₂；∀ (x : M₁), f x⁻¹ = (f x)⁻¹。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
a `SubNegMonoid`. -/]
/-
**Function.Surjective.divInvMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjectiv
e`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : Pow M₂ ℕ] →           [inst_3 : Inv M₂] →         
    [inst_4 : Div M₂] →               [inst_5 : Pow M₂ ℤ] →                 [ins
t_6 : DivInvMonoid M₁] →                   (f : M₁ → M₂) →                     F
unction.Surjective f →                       f 1 = 1 →                         (
∀ (x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f 
x⁻¹ = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f 
y) →                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →  
                               (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → DivIn
vMonoid M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
...
/-
**Function.Surjective.group** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : Pow M₂ ℕ] →           [inst_3 : Inv M₂] →         
    [inst_4 : Div M₂] →               [inst_5 : Pow M₂ ℤ] →                 [ins
t_6 : Group M₁] →                   (f : M₁ → M₂) →                     Function
.Surjective f →                       f 1 = 1 →                         (∀ (x y 
: M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f x⁻¹ = (
f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f y) →   
                            (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →         
                        (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → Group M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
...
/-
**Function.Surjective.commGroup** 是 Mathlib 中的一个定义，位于命名空间 `Function.Surjective`。
形式化陈述：{M₁ : Type u_1} →   {M₂ : Type u_2} →     [inst : Mul M₂] →       [inst_1 
: One M₂] →         [inst_2 : Pow M₂ ℕ] →           [inst_3 : Inv M₂] →         
    [inst_4 : Div M₂] →               [inst_5 : Pow M₂ ℤ] →                 [ins
t_6 : CommGroup M₁] →                   (f : M₁ → M₂) →                     Func
tion.Surjective f →                       f 1 = 1 →                         (∀ (
x y : M₁), f (x * y) = f x * f y) →                           (∀ (x : M₁), f x⁻¹
 = (f x)⁻¹) →                             (∀ (x y : M₁), f (x / y) = f x / f y) 
→                               (∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n) →     
                            (∀ (x : M₁) (n : ℤ), f (x ^ n) = f x ^ n) → CommGrou
p M₂
参数：f : M₁ → M₂；∀ (x y : M₁), f (x * y) = f x * f y；∀ (x : M₁), f x⁻¹ = (f x)⁻¹；∀
 (x y : M₁), f (x / y) = f x / f y；∀ (x : M₁) (n : ℕ), f (x ^ n) = f x ^ n；∀ (x 
: M₁) (n : ℤ), f (x ^ n) = f x ^ n。
