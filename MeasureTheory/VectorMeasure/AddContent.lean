...
/--
Definition of `of_additive_of_le_measure` / `of_additive_of_le_measure` 的定义

English:
definition of_additive_of_le_measure
  body: m
...
def of_additive_of_le_measure
    (m : Set α -> E) (hm : forall s, ‖m s‖ₑ <= μ s) [IsFiniteMeasure μ]
    (h'm : forall s t, MeasurableSet s -> MeasurableSet t -> Disjoint s t -> m (s union t) = m s + m t)
    (h''m : forall s, ¬ MeasurableSet s -> m s = 0) : VectorMeasure α E where
  measureOf' := m
  empty' := by simpa using h'm ∅ ∅ MeasurableSet.empty MeasurableSet.empty (by simp)
  not_measurable' := h''m
  m_iUnion' f f_meas f_disj := by
    ...
    have A n : m (⋃ i in Finset.range n, f i) = ∑ i in Finset.range n, m (f i) := by
      induction n with
      ...
      | succ n ih =>
        ...
        rw [h'm _ _ (f_meas n)]; rw [ih]
        · exact Finset.measurableSet_biUnion _ (fun i hi => f_meas i)
        · simp only [Finset.mem_range, Set.disjoint_iUnion_right]
          intro i hi
          exact f_disj hi.ne'
    have B n : m (⋃ i, f i) = m (⋃ i in Finset.range n, f i) + m (⋃ i in Set.Ici n, f i) := by
      have : ⋃ i, f i = (⋃ i in Finset.range n, f i) union (⋃ i in Set.Ici n, f i) := by
        ext; simp; grind
      rw [this]
      apply h'm
      · exact Finset.measurableSet_biUnion _ (fun i hi => f_meas i)
    ...
    have C n : m (⋃ i, f i) - ∑ i in Finset.range n, m (f i) = m (⋃ i in Set.Ici n, f i) := by
      rw [B n]; rw [A]; simp
    simp only [C]
    apply tendsto_of_tendsto_of_tendsto_of_le_of_le tendsto_const_nhds
      (h := fun n => μ (⋃ i in Set.Ici n, f i)) ?_ (fun i => bot_le) (fun i => hm _)
...
/--
lemma `exists_extension_of_isSetRing_of_le_measure_of_dense` / 引理 `exists_extension_of_isSetRing_of_le_measure_of_dense`

English:
lemma exists_extension_of_isSetRing_of_le_measure_of_dense
...
中文:
引理 exists_extension_of_isSetRing_of_le_measure_of_dense
...
lemma exists_extension_of_isSetRing_of_le_measure_of_dense [IsFiniteMeasure μ]
    ...
    (hCmeas : forall s in C, MeasurableSet s) (hm : forall s in C, ‖m s‖ₑ <= μ s)
    (h'C : forall t ε, MeasurableSet t -> 0 < ε -> exists s in C, μ (s ∆ t) < ε) :
    exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ s := by
  /- We will extend by continuity the function `m` from the class `C` to all measurable sets,
  thanks to the fact that `C` is dense. To implement this properly, we work in the space
  `MeasuredSets μ` with the distance `edist s t = μ (s ∆ t)`. The assumptions guarantee that
  ...
  let C' : Set (MeasuredSets μ) := {s | exists c in C, s = c}
  have C'C (s : MeasuredSets μ) (hs : s in C') : (s : Set α) in C := by
  ...
  have lip : LipschitzWith 1 m₀ := by
    ...
    have Is : ((s : Set α) inter t) union (s \ t) = (s : Set α) := Set.inter_union_sdiff _ _
    have It : ((t : Set α) inter s) union (t \ s) = (t : Set α) := Set.inter_union_sdiff _ _
    nth_rewrite 1 [← Is]
    nth_rewrite 3 [← It]
    rw [addContent_union hC (hC.inter_mem (C'C _ t.2) (C'C _ s.2))
        (hC.sdiff_mem (C'C _ t.2) (C'C _ s.2)) Set.disjoint_sdiff_inter.symm]; rw [addContent_union hC (hC.inter_mem (C'C _ s.2) (C'C _ t.2))
        (hC.sdiff_mem (C'C _ s.2) (C'C _ t.2)) Set.disjoint_sdiff_inter.symm]; rw [Set.inter_comm]
  ...
  have hBound : forall s, ‖m₁ s‖ₑ <= μ s := by
  ...
  have hAddit (s t : MeasuredSets μ) (h : Disjoint (s : Set α) t) :
      m₁ ⟨s union t, s.2.union t.2⟩ = m₁ s + m₁ t := by
    suffices forall ε > 0, ‖m₁ (⟨s union t, s.2.union t.2⟩) - m₁ s - m₁ t‖ₑ < ε by
      rw [← sub_eq_zero]; rw [← enorm_eq_zero]; rw [sub_add_eq_sub_sub]
      exact eq_bot_iff.2 (le_of_forall_gt this)
    intro ε εpos
    obtain ⟨δ, δpos, hδ⟩ : exists δ, 0 < δ ∧ 8 * δ = ε :=
    ...
    obtain ⟨s', s'C, hs'⟩ : exists s' in C, μ (s' ∆ s) < δ := h'C _ _ s.2 δpos
    obtain ⟨t', t'C, ht'⟩ : exists t' in C, μ (t' ∆ t) < δ := h'C _ _ t.2 δpos
    have It : ‖m t' - m₁ t‖ₑ < δ := by
      have : m₁ ⟨t', hCmeas _ t'C⟩ = m t' :=
        C'_dense.extend_eq lip.continuous ⟨⟨t', hCmeas _ t'C⟩, ⟨t', t'C, rfl⟩⟩
      rw [← this]; rw [← edist_eq_enorm_sub]
    ...
    have hμ' : μ (s' inter t') < 2 * δ := calc
      μ (s' inter t')
      _ <= μ (s inter t union (s' ∆ s) union (t' ∆ t)) := measure_mono (by grind)
      _ = μ ((s' ∆ s) union (t' ∆ t)) := by simp [Set.disjoint_iff_inter_eq_empty.mp h]
    ...
    let s'' := s' \ t'
    have s''C : s'' in C := hC.sdiff_mem s'C t'C
    ...
    have Ist : ‖m (s'' union t') - m₁ ⟨s union t, s.2.union t.2⟩‖ₑ < 4 * δ := by
      have s''t'C : s'' union t' in C := hC.union_mem s''C t'C
      have : m₁ ⟨s'' union t', hCmeas _ s''t'C⟩ = m (s'' union t') :=
        C'_dense.extend_eq lip.continuous ⟨⟨s'' union t', hCmeas _ s''t'C⟩, ⟨s'' union t', s''t'C, rfl⟩⟩
      rw [← this]; rw [← edist_eq_enorm_sub]
      apply (m₁_lip _ _).trans_lt
      simp only [ENNReal.coe_one, MeasuredSets.edist_def, one_mul]
      change μ ((s'' union t') ∆ (s union t)) < 4 * δ
      calc μ ((s'' union t') ∆ (s union t))
      _ <= μ (s'' ∆ s union t' ∆ t) := measure_mono (Set.union_symmDiff_union_subset ..)
      _ <= μ (s'' ∆ s) + μ (t' ∆ t) := measure_union_le _ _
      _ < 3 * δ + δ := by gcongr
      _ = 4 * δ := by ring
    ...
    calc ‖m₁ (⟨s union t, s.2.union t.2⟩) - m₁ s - m₁ t‖ₑ
    _ = ‖(m (s'' union t') - m s'' - m t') + (m₁ ⟨s union t, s.2.union t.2⟩ - m (s'' union t'))
          + (m s'' - m₁ s) + (m t' - m₁ t)‖ₑ := by abel_nf
    _ <= ‖m (s'' union t') - m s'' - m t'‖ₑ + ‖m₁ ⟨s union t, s.2.union t.2⟩ - m (s'' union t')‖ₑ
          + ‖m s'' - m₁ s‖ₑ + ‖m t' - m₁ t‖ₑ := enorm_add₄_le
    _ = ‖m₁ ⟨s union t, s.2.union t.2⟩ - m (s'' union t')‖ₑ + ‖m s'' - m₁ s‖ₑ + ‖m t' - m₁ t‖ₑ := by
...
/--
lemma `exists_extension_of_isSetSemiring_of_le_measure_of_dense` / 引理 `exists_extension_of_isSetSemiring_of_le_measure_of_dense`

English:
lemma exists_extension_of_isSetSemiring_of_le_measure_of_dense
  statement: [IsFiniteMeasure μ]
  proof: by
  set m₀ : AddContent E (supClosure C) := m.supClosure hC with hm₀
  have A (s) (hs : s in supClosure C) : ‖m₀ s‖ₑ <= μ s := by
    rw [hC.mem_supClosure_iff] at hs
    rcases hs with ⟨P, PC⟩
    nth_rewrite 2 [← P.sup_parts]
    rw [hm₀]; rw [AddContent.supClosure_apply_finpartition hC _ PC]; rw

中文:
引理 exists_extension_of_isSetSemiring_of_le_measure_of_dense
  结论: [IsFiniteMeasure μ]
  证明: by
  set m₀ : AddContent E (supClosure C) := m.supClosure hC with hm₀
  have A (s) (hs : s in supClosure C) : ‖m₀ s‖ₑ <= μ s := by
...
Depends on / 依赖: AddContent, AddContent.supClosure_apply_finpartition, Finset, Finset.sup_set_eq_biUnion, MeasurableSet, P.disjoint, P.sup_parts, disjoint, enorm_sum_le, hC.mem_supClosure_iff, hCmeas, m.supClosure, measure_biUnion_finset, mem_supClosure_iff, nth_rewrite, supClosure, supClosure_apply_finpartition, sup_parts, sup_set_eq_biUnion
-/
lemma exists_extension_of_isSetSemiring_of_le_measure_of_dense [IsFiniteMeasure μ]
    ...
    (hCmeas : forall s in C, MeasurableSet s) (hm : forall s in C, ‖m s‖ₑ <= μ s)
    (h'C : forall t ε, MeasurableSet t -> 0 < ε -> exists s in supClosure C, μ (s ∆ t) < ε) :
    exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ s := by
  set m₀ : AddContent E (supClosure C) := m.supClosure hC with hm₀
  have A (s) (hs : s in supClosure C) : ‖m₀ s‖ₑ <= μ s := by
    rw [hC.mem_supClosure_iff] at hs
    rcases hs with ⟨P, PC⟩
    nth_rewrite 2 [← P.sup_parts]
    rw [hm₀]; rw [AddContent.supClosure_apply_finpartition hC _ PC]; rw [Finset.sup_set_eq_biUnion]; rw [measure_biUnion_finset P.disjoint (fun b hb => hCmeas _ (PC hb))]
    apply (enorm_sum_le _ _).trans
    gcongr with t ht
    exact hm _ (PC ht)
  have B (s) (hs : s in supClosure C) : MeasurableSet s := by
    rw [hC.mem_supClosure_iff] at hs
    rcases hs with ⟨P, PC⟩
    rw [← P.sup_parts]; rw [Finset.sup_set_eq_biUnion]
...
/--
lemma `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover` / 引理 `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover`

English:
lemma exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover
  proof: by
...
中文:
引理 exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover
  证明: by
  apply VectorMeasure.exists_extension_of_isSetSemiring_of_le_measure_of_dense hC ?_ hm ?_
  · intro s hs
    rw [h'C]
...
private lemma exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover
    ...
    (hm : forall s in C, ‖m s‖ₑ <= μ s)
    (h'C : hα = generateFrom C) (h''C : exists D : Set (Set α), D.Countable ∧ D subseteq C ∧ μ (⋃₀ D)ᶜ = 0) :
    exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ s := by
  apply VectorMeasure.exists_extension_of_isSetSemiring_of_le_measure_of_dense hC ?_ hm ?_
  · intro s hs
    rw [h'C]
...
/--
theorem `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom` / 定理 `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom`

English:
theorem exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom
  proof: by
  have M (s) (hs : s in C) : MeasurableSet s := by
...
中文:
定理 exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom
  证明: by
  have M (s) (hs : s in C) : MeasurableSet s := by
    rw [h'C]; exact measurableSet_generateFrom hs
  rcases Measure.exists_ae_subset_biUnion_countable μ M with ⟨D, DC, D_count, hD⟩
...
Depends on / 依赖: D_count, MeasurableSet, MeasurableSet.sUnion, Measure, Measure.exists_ae_subset_biUnion_countable, VectorMeasure, exists_ae_subset_biUnion_countable, exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom_of_cover, measurableSet_generateFrom, restrict, sUnion
-/
theorem exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom
    [IsFiniteMeasure μ] {C : Set (Set α)} {m : AddContent E C} (hC : IsSetSemiring C)
    (hm : forall s in C, ‖m s‖ₑ <= μ s) (h'C : hα = generateFrom C) :
    exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ s := by
  have M (s) (hs : s in C) : MeasurableSet s := by
    rw [h'C]; exact measurableSet_generateFrom hs
  rcases Measure.exists_ae_subset_biUnion_countable μ M with ⟨D, DC, D_count, hD⟩
  have MD : MeasurableSet (⋃₀ D) := MeasurableSet.sUnion D_count (fun t ht => M _ (DC ht))
  let μ' := μ.restrict (⋃₀ D)
  obtain ⟨m', h, h'⟩ : exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ' s := by
...
/--
theorem `exists_extension_of_isSetSemiring_of_le_measure` / 定理 `exists_extension_of_isSetSemiring_of_le_measure`
...
中文:
定理 exists_extension_of_isSetSemiring_of_le_measure
  结论: [NormedSpace 实数 E]
  证明: by
  /- On the sigma-algebra `M` generated by `C`, the desired vector measure is provided by
  `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom`. We extend it to the whole
  ...
  This extens
-/
theorem exists_extension_of_isSetSemiring_of_le_measure [NormedSpace Real E]
    [IsFiniteMeasure μ] {C : Set (Set α)} {m : AddContent E C} (hC : IsSetSemiring C)
    (hm : forall s in C, ‖m s‖ₑ <= μ s) (h'C : forall s in C, MeasurableSet s) :
    exists m' : VectorMeasure α E, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ s := by
  /- On the sigma-algebra `M` generated by `C`, the desired vector measure is provided by
  `exists_extension_of_isSetSemiring_of_le_measure_of_generateFrom`. We extend it to the whole
  sigma-algebra of measurable sets by integrating the conditional expectation with respect to `M`.
  ...
  obtain ⟨m', m'C, hm'⟩ :
      exists m' : @VectorMeasure α M E _ _, (forall s in C, m' s = m s) ∧ forall s, ‖m' s‖ₑ <= μ' s := by
  ...
  { measureOf' s := if MeasurableSet s then ∫ᵛ x, μ[s.indicator 1 | M] x ∂• m' else 0
    ...
    m_iUnion' f f_meas hf := by
      have : ∫ᵛ (x : α), μ[fun x => ∑' (d : Nat), (f d).indicator 1 x | M] x ∂•m'
      ...
      have I : ∑' (i : Nat), ∫⁻ (a : α), ‖μ[(f i).indicator (1 : α -> Real) | M] a‖ₑ ∂μ < ∞ := by
        have A i : ∫⁻ a, ‖μ[(f i).indicator (1 : α -> Real) | M] a‖ₑ ∂μ = μ (f i) :=
      ...
      · apply ne_of_lt
        grw [m'_le, hμ']
        have A i : Measurable[M] (fun x => ‖μ[(f i).indicator (1 : α -> Real) | M] x‖ₑ) := by fun_prop
  ...
  · simp only [coe_mk, h'C s hs, ↓reduceIte, m'']
    ...
    rw [this]; rw [integral_indicator (measurableSet_generateFrom hs)]
