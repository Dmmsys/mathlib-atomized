...
lemma le_rieszMeasure_tsupport_subset
  ...
  rw [← Compacts.coe_mk (tsupport f) f.hasCompactSupport]; rw [rieszMeasure]; rw [this]; 
...
lemma rieszMeasure_le_of_eq_one
  ...
  intro p hp
  rw [← ENNReal.ofReal_coe_nnreal]; rw [ENNReal.ofReal_eq_ofReal_iff (Λ.map_nonneg hf) NNReal.zero_le_coe] at
...
lemma range_cut_partition
  ...
  have hy {n m : Fin N} (h : n < m) : y n + ε <= y m := calc
    _ <= a + ε * m + ε := by
      exact add_le_add_three (by rfl) ((mul_le_mul_iff_of_pos_left h
...
lemma integral_riesz_aux
  ...
  obtain ⟨a, b, hab⟩ : exists a b : Real, a < b ∧ range f subse
