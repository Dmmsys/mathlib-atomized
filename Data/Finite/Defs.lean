...
中文:
定理 finite_iff_存在_equiv_fin
  条件: {α : 类型层*}
  结论: 有限 α ↔ 存在 n, 非空 (α ≃ 有限集 n)
...
中文:
定理 等价.set_finite_iff
  条件: {s : 集合 α} {t : 集合 β} (hst : s ≃ t)
  结论: s.有限 ↔ t.有限
  证明: by
  simp_rw [← Set.finite_coe_iff, hst.finite_iff]
