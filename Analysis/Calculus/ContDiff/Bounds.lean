...
/--
theorem `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_aux` / 定理 `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_aux`

English:
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_aux
  statement: {Du Eu Fu Gu : Type u}
  proof: by
...
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_aux {Du Eu Fu Gu : Type u}
    ...
    (B : Eu ->L[𝕜] Fu ->L[𝕜] Gu) {f : Du -> Eu} {g : Du -> Fu} {n : Nat} {s : Set Du} {x : Du}
    (hf : ContDiffOn 𝕜 n f s) (hg : ContDiffOn 𝕜 n g s) (hs : UniqueDiffOn 𝕜 s) (hx : x in s) :
    ‖iteratedFDerivWithin 𝕜 n (fun y => B (f y) (g y)) s x‖ <=
      ‖B‖ * ∑ i in Finset.range (n + 1), (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
  ...
  | zero =>
    ...
    apply B.le_opNorm₂
  | succ n IH =>
    have In : (n : Nat∞ω) + 1 <= n.succ := by simp only [Nat.cast_succ, le_refl]
    have I1 :
        ‖iteratedFDerivWithin 𝕜 n (fun y : Du => B.precompR Du (f y) (fderivWithin 𝕜 g s y)) s x‖ <=
          ‖B‖ * ∑ i in Finset.range (n + 1), n.choose i * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
            ‖iteratedFDerivWithin 𝕜 (n + 1 - i) g s x‖ := by
      calc
        ‖iteratedFDerivWithin 𝕜 n (fun y : Du => B.precompR Du (f y) (fderivWithin 𝕜 g s y)) s x‖ <=
            ‖B.precompR Du‖ * ∑ i in Finset.range (n + 1),
        ...
        _ <= ‖B‖ * ∑ i in Finset.range (n + 1), n.choose i * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
    ...
    have I2 :
        ...
        ‖B‖ * ∑ i in Finset.range (n + 1), n.choose i * ‖iteratedFDerivWithin 𝕜 (i + 1) f s x‖ *
          ‖iteratedFDerivWithin 𝕜 (n - i) g s x‖ :=
      calc
        ‖iteratedFDerivWithin 𝕜 n (fun y : Du => B.precompL Du (fderivWithin 𝕜 f s y) (g y)) s x‖ <=
            ‖B.precompL Du‖ * ∑ i in Finset.range (n + 1),
...
/--
theorem `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear` / 定理 `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear`

English:
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear
  statement: (B : E ->L[𝕜] F ->L[𝕜] G)
  proof: by
...
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear (B : E ->L[𝕜] F ->L[𝕜] G)
    {f : D -> E} {g : D -> F} {N : Nat∞ω} {s : Set D} {x : D} (hf : ContDiffOn 𝕜 N f s)
    (hg : ContDiffOn 𝕜 N g s) (hs : UniqueDiffOn 𝕜 s) (hx : x in s) {n : Nat} (hn : n <= N) :
    ‖iteratedFDerivWithin 𝕜 n (fun y => B (f y) (g y)) s x‖ <=
      ‖B‖ * ∑ i in Finset.range (n + 1), (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
        ‖iteratedFDerivWithin 𝕜 (n - i) g s x‖ := by
  /- We reduce the bound to the case where all spaces live in the same universe (in which we
  ...
  have Nfu : forall i, ‖iteratedFDerivWithin 𝕜 i fu su xu‖ = ‖iteratedFDerivWithin 𝕜 i f s x‖ := by
    intro i
    ...
    rwa [← xu_x] at hx
  have Ngu : forall i, ‖iteratedFDerivWithin 𝕜 i gu su xu‖ = ‖iteratedFDerivWithin 𝕜 i g s x‖ := by
    intro i
    rw [LinearIsometryEquiv.norm_iteratedFDerivWithin_comp_left _ _ hsu hxu]
    rw [LinearIsometryEquiv.norm_iteratedFDerivWithin_comp_right _ _ hs]; rw [xu_x]
    rwa [← xu_x] at hx
  ...
  have : ‖iteratedFDerivWithin 𝕜 n (fun y => Bu (fu y) (gu y)) su xu‖ <=
      ‖Bu‖ * ∑ i in Finset.range (n + 1), (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i fu su xu‖ *
...
/--
theorem `ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear` / 定理 `ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear`

English:
theorem ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear
  statement: (B : E ->L[𝕜] F ->L[𝕜] G) {f : D -> E}
  proof: by
...
Depends on / 依赖: B.norm_iteratedFDerivWithin_le_of_bilinear, contDiffOn, hf.contDiffOn, hg.contDiffOn, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_le_of_bilinear, simp_rw, uniqueDiffOn_univ
-/
theorem ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear (B : E ->L[𝕜] F ->L[𝕜] G) {f : D -> E}
    {g : D -> F} {N : Nat∞ω} (hf : ContDiff 𝕜 N f) (hg : ContDiff 𝕜 N g) (x : D) {n : Nat}
    (hn : n <= N) :
    ‖iteratedFDeriv 𝕜 n (fun y => B (f y) (g y)) x‖ <= ‖B‖ * ∑ i in Finset.range (n + 1),
      (n.choose i : Real) * ‖iteratedFDeriv 𝕜 i f x‖ * ‖iteratedFDeriv 𝕜 (n - i) g x‖ := by
  simp_rw [← iteratedFDerivWithin_univ]
  exact B.norm_iteratedFDerivWithin_le_of_bilinear hf.contDiffOn hg.contDiffOn uniqueDiffOn_univ
    (mem_univ x) hn

/--
theorem `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_of_le_one` / 定理 `ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_of_le_one`
...
Depends on / 依赖: B.norm_iteratedFDerivWithin_le_of_bilinear, mul_le_of_le_one_left, norm_iteratedFDerivWithin_le_of_bilinear
-/
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_le_of_bilinear_of_le_one
    (B : E ->L[𝕜] F ->L[𝕜] G) {f : D -> E} {g : D -> F} {N : Nat∞ω} {s : Set D} {x : D}
    (hf : ContDiffOn 𝕜 N f s) (hg : ContDiffOn 𝕜 N g s) (hs : UniqueDiffOn 𝕜 s) (hx : x in s) {n : Nat}
    (hn : n <= N) (hB : ‖B‖ <= 1) : ‖iteratedFDerivWithin 𝕜 n (fun y => B (f y) (g y)) s x‖ <=
      ∑ i in Finset.range (n + 1), (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
        ‖iteratedFDerivWithin 𝕜 (n - i) g s x‖ := by
  apply (B.norm_iteratedFDerivWithin_le_of_bilinear hf hg hs hx hn).trans
  exact mul_le_of_le_one_left (by positivity) hB

/--
theorem `ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear_of_le_one` / 定理 `ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear_of_le_one`
...
Depends on / 依赖: B.norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, contDiffOn, hf.contDiffOn, hg.contDiffOn, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, simp_rw, uniqueDiffOn_univ
-/
theorem ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear_of_le_one (B : E ->L[𝕜] F ->L[𝕜] G)
    {f : D -> E} {g : D -> F} {N : Nat∞ω} (hf : ContDiff 𝕜 N f) (hg : ContDiff 𝕜 N g)
    (x : D) {n : Nat} (hn : n <= N) (hB : ‖B‖ <= 1) :
...
/--
theorem `norm_iteratedFDerivWithin_smul_le` / 定理 `norm_iteratedFDerivWithin_smul_le`

English:
theorem norm_iteratedFDerivWithin_smul_le
  statement: {f : E -> 𝕜'} {g : E -> F} {N : Nat∞ω}
  proof: (ContinuousLinearMap.lsmul 𝕜 𝕜' :
    𝕜' ->L[𝕜] F ->L[𝕜] F).norm_iteratedFDerivWithin_le_of_bilinear_of_le_one
      hf hg hs hx hn ContinuousLinearMap.opNorm_lsmul_le

中文:
定理 norm_iteratedFDerivWithin_smul_le
  结论: {f : E -> 𝕜'} {g : E -> F} {N : 自然数∞ω}
  证明: (ContinuousLinearMap.lsmul 𝕜 𝕜' :
    𝕜' ->L[𝕜] F ->L[𝕜] F).norm_iteratedFDerivWithin_le_of_bilinear_of_le_one
      hf hg hs hx hn ContinuousLinearMap.opNorm_lsmul_le

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.lsmul, ContinuousLinearMap.opNorm_lsmul_le, norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, opNorm_lsmul_le
-/
theorem norm_iteratedFDerivWithin_smul_le {f : E -> 𝕜'} {g : E -> F} {N : Nat∞ω}
    (hf : ContDiffOn 𝕜 N f s) (hg : ContDiffOn 𝕜 N g s) (hs : UniqueDiffOn 𝕜 s) {x : E} (hx : x in s)
    {n : Nat} (hn : n <= N) : ‖iteratedFDerivWithin 𝕜 n (fun y => f y • g y) s x‖ <=
      ∑ i in Finset.range (n + 1), (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i f s x‖ *
        ‖iteratedFDerivWithin 𝕜 (n - i) g s x‖ :=
...
/--
theorem `norm_iteratedFDeriv_smul_le` / 定理 `norm_iteratedFDeriv_smul_le`

English:
theorem norm_iteratedFDeriv_smul_le
  statement: {f : E -> 𝕜'} {g : E -> F} {N : Nat∞ω} (hf : ContDiff 𝕜 N f)
  proof: (ContinuousLinearMap.lsmul 𝕜 𝕜' : 𝕜' ->L[𝕜] F ->L[𝕜] F).norm_iteratedFDeriv_le_of_bilinear_of_le_one
...
Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.lsmul, ContinuousLinearMap.opNorm_lsmul_le, norm_iteratedFDeriv_le_of_bilinear_of_le_one, opNorm_lsmul_le
-/
theorem norm_iteratedFDeriv_smul_le {f : E -> 𝕜'} {g : E -> F} {N : Nat∞ω} (hf : ContDiff 𝕜 N f)
    (hg : ContDiff 𝕜 N g) (x : E) {n : Nat} (hn : n <= N) :
    ‖iteratedFDeriv 𝕜 n (fun y => f y • g y) x‖ <= ∑ i in Finset.range (n + 1),
      (n.choose i : Real) * ‖iteratedFDeriv 𝕜 i f x‖ * ‖iteratedFDeriv 𝕜 (n - i) g x‖ :=
...
/--
theorem `norm_iteratedFDerivWithin_mul_le` / 定理 `norm_iteratedFDerivWithin_mul_le`

English:
theorem norm_iteratedFDerivWithin_mul_le
  statement: {f : E -> A} {g : E -> A} {N : Nat∞ω}
  proof: (ContinuousLinearMap.mul 𝕜 A :
    A ->L[𝕜] A ->L[𝕜] A).norm_iteratedFDerivWithin_le_of_bilinear_of_le_one
      hf hg hs hx hn (ContinuousLinearMap.opNorm_mul_le _ _)

中文:
定理 norm_iteratedFDerivWithin_mul_le
  结论: {f : E -> A} {g : E -> A} {N : 自然数∞ω}
  证明: (ContinuousLinearMap.mul 𝕜 A :
    A ->L[𝕜] A ->L[𝕜] A).norm_iteratedFDerivWithin_le_of_bilinear_of_le_one
      hf hg hs hx hn (ContinuousLinearMap.opNorm_mul_le _ _)

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.mul, ContinuousLinearMap.opNorm_mul_le, norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, opNorm_mul_le
-/
theorem norm_iteratedFDerivWithin_mul_le {f : E -> A} {g : E -> A} {N : Nat∞ω}
    (hf : ContDiffOn 𝕜 N f s) (hg : ContDiffOn 𝕜 N g s) (hs : UniqueDiffOn 𝕜 s)
    {x : E} (hx : x in s) {n : Nat} (hn : n <= N) :
...
/--
theorem `norm_iteratedFDeriv_mul_le` / 定理 `norm_iteratedFDeriv_mul_le`

English:
theorem norm_iteratedFDeriv_mul_le
  statement: {f : E -> A} {g : E -> A} {N : Nat∞ω} (hf : ContDiff 𝕜 N f)
  proof: by
  simp_rw [← iteratedFDerivWithin_univ]
...
Depends on / 依赖: contDiffOn, hf.contDiffOn, hg.contDiffOn, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_mul_le, simp_rw, uniqueDiffOn_univ
-/
theorem norm_iteratedFDeriv_mul_le {f : E -> A} {g : E -> A} {N : Nat∞ω} (hf : ContDiff 𝕜 N f)
    (hg : ContDiff 𝕜 N g) (x : E) {n : Nat} (hn : n <= N) :
    ‖iteratedFDeriv 𝕜 n (fun y => f y * g y) x‖ <= ∑ i in Finset.range (n + 1),
      (n.choose i : Real) * ‖iteratedFDeriv 𝕜 i f x‖ * ‖iteratedFDeriv 𝕜 (n - i) g x‖ := by
  simp_rw [← iteratedFDerivWithin_univ]
...
/--
theorem `norm_iteratedFDerivWithin_prod_le` / 定理 `norm_iteratedFDerivWithin_prod_le`

English:
theorem norm_iteratedFDerivWithin_prod_le
  statement: [DecidableEq ι] [NormOneClass A'] {u : Finset ι}
  proof: by
...
Depends on / 依赖: Finset, Finset.induction, Finset.mem_insert, Finset.prod_insert, Finset.sum_coe_sort, Finset.sum_equiv, Finset.sym, Finset.symInsertEquiv, Finset.univ, Sym.eq_nil_of_card_zero, contDiffOn_prod, eq_nil_of_card_zero, forall_eq_or_imp, generalizing, insert, iteratedFDerivWithin_succ_const, le_trans, mem_insert, norm_iteratedFDerivWithin_mul_le, prod_insert
-/
theorem norm_iteratedFDerivWithin_prod_le [DecidableEq ι] [NormOneClass A'] {u : Finset ι}
    {f : ι -> E -> A'} {N : Nat∞ω} (hf : forall i in u, ContDiffOn 𝕜 N (f i) s)
    (hs : UniqueDiffOn 𝕜 s) {x : E} (hx : x in s) {n : Nat} (hn : n <= N) :
    ‖iteratedFDerivWithin 𝕜 n (∏ j in u, f j ·) s x‖ <=
      ∑ p in u.sym n, (p : Multiset ι).countPerms *
  ...
  | insert i u hi IH =>
    ...
    rw [Finset.sum_equiv (Finset.symInsertEquiv hi) (t := Finset.univ)
      (g := (fun v => v.countPerms *
          ∏ j in insert i u]; rw [‖iteratedFDerivWithin 𝕜 (v.count j) (f j) s x‖) ∘
        Sym.toMultiset ∘ Subtype.val ∘ (Finset.symInsertEquiv hi).symm)
      (by simp) (by simp only [← comp_apply (g := Finset.symInsertEquiv hi)]; rw [comp_assoc]; simp)]
    rw [← Finset.univ_sigma_univ]; rw [Finset.sum_sigma]; rw [Finset.sum_range]
    ...
    grw [IH]
    rw [Finset.mul_sum]; rw [← Finset.sum_coe_sort]
    ...
    rw [Sym.count_coe_fill_self_of_notMem hip]; rw [Sym.countPerms_coe_fill_of_notMem hip]
...
/--
theorem `norm_iteratedFDeriv_prod_le` / 定理 `norm_iteratedFDeriv_prod_le`
...
Depends on / 依赖: contDiffOn, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_prod_le, uniqueDiffOn_univ
-/
theorem norm_iteratedFDeriv_prod_le [DecidableEq ι] [NormOneClass A'] {u : Finset ι}
    {f : ι -> E -> A'} {N : Nat∞ω} (hf : forall i in u, ContDiff 𝕜 N (f i)) {x : E} {n : Nat}
    (hn : n <= N) :
...
/--
theorem `norm_iteratedFDerivWithin_comp_le_aux` / 定理 `norm_iteratedFDerivWithin_comp_le_aux`

English:
theorem norm_iteratedFDerivWithin_comp_le_aux
  statement: {Fu Gu : Type u} [NormedAddCommGroup Fu]
  proof: by
...
theorem norm_iteratedFDerivWithin_comp_le_aux {Fu Gu : Type u} [NormedAddCommGroup Fu]
    [NormedSpace 𝕜 Fu] [NormedAddCommGroup Gu] [NormedSpace 𝕜 Gu] {g : Fu -> Gu} {f : E -> Fu} {n : Nat}
    {s : Set E} {t : Set Fu} {x : E} (hg : ContDiffOn 𝕜 n g t) (hf : ContDiffOn 𝕜 n f s)
    (ht : UniqueDiffOn 𝕜 t) (hs : UniqueDiffOn 𝕜 s) (hst : MapsTo f s t) (hx : x in s) {C : Real}
    {D : Real} (hC : forall i, i <= n -> ‖iteratedFDerivWithin 𝕜 i g t (f x)‖ <= C)
    (hD : forall i, 1 <= i -> i <= n -> ‖iteratedFDerivWithin 𝕜 i f s x‖ <= D ^ i) :
  ...
  | hi n IH =>
  have M : (n : Nat∞ω) < n.succ := Nat.cast_lt.2 n.lt_succ_self
  ...
  have I : forall i in Finset.range (n + 1),
    ...
    · intro j hj
      have : ‖iteratedFDerivWithin 𝕜 j (fderivWithin 𝕜 g t) t (f x)‖ =
        ...
        rw [iteratedFDerivWithin_succ_eq_comp_right ht (hst hx)]; rw [comp_apply]; rw [LinearIsometryEquiv.norm_map]
      rw [this]
  ...
  have J : forall i, ‖iteratedFDerivWithin 𝕜 (n - i) (fderivWithin 𝕜 f s) s x‖ <= D ^ (n - i + 1) := by
    intro i
  ...
  calc
    ...
    _ <= ∑ i in Finset.range (n + 1),
        (n.choose i : Real) * ‖iteratedFDerivWithin 𝕜 i (fderivWithin 𝕜 g t ∘ f) s x‖ *
    ...
    -- assumption for `g' ∘ f`).
    _ <= ∑ i in Finset.range (n + 1), (n.choose i : Real) * (i ! * C * D ^ i) * D ^ (n - i + 1) := by
      gcongr with i hi
      · exact I i hi
      · exact J i
    ...
    -- the claimed bound.
    _ = ∑ i in Finset.range (n + 1),
        (n ! : Real) * ((i ! : Real)⁻¹ * i !) * C * (D ^ i * D ^ (n - i + 1)) * ((n - i)! : Real)⁻¹ := by
      congr! 1 with i hi
      simp only [Nat.cast_choose Real (Finset.mem_range_succ_iff.1 hi), div_eq_inv_mul, mul_inv]
      ring
    _ = ∑ i in Finset.range (n + 1), (n ! : Real) * 1 * C * D ^ (n + 1) * ((n - i)! : Real)⁻¹ := by
      congr! with i hi
    ...
    _ <= ∑ i in Finset.range (n + 1), (n ! : Real) * 1 * C * D ^ (n + 1) * 1 := by
      gcongr with i
...
/--
theorem `norm_iteratedFDerivWithin_comp_le` / 定理 `norm_iteratedFDerivWithin_comp_le`

English:
theorem norm_iteratedFDerivWithin_comp_le
  statement: {g : F -> G} {f : E -> F} {n : Nat} {s : Set E} {t : Set F}
  proof: by
  /- We reduce the bound to the case where all spaces live in the same universe (in which we
    already have proved the result), by using linear isometries between the spaces and their `ULift`
    to a common universe. These linear isometries preserve the norm of the iterated derivative. -/
...
定理 norm_iteratedFDerivWithin_comp_le
  ...
  le
-/
theorem norm_iteratedFDerivWithin_comp_le {g : F -> G} {f : E -> F} {n : Nat} {s : Set E} {t : Set F}
    {x : E} {N : Nat∞ω} (hg : ContDiffOn 𝕜 N g t) (hf : ContDiffOn 𝕜 N f s) (hn : n <= N)
    (ht : UniqueDiffOn 𝕜 t) (hs : UniqueDiffOn 𝕜 s) (hst : MapsTo f s t) (hx : x in s) {C : Real}
    {D : Real} (hC : forall i, i <= n -> ‖iteratedFDerivWithin 𝕜 i g t (f x)‖ <= C)
    (hD : forall i, 1 <= i -> i <= n -> ‖iteratedFDerivWithin 𝕜 i f s x‖ <= D ^ i) :
    ‖iteratedFDerivWithin 𝕜 n (g ∘ f) s x‖ <= n ! * C * D ^ n := by
  ...
  have Nfu : forall i, ‖iteratedFDerivWithin 𝕜 i fu s x‖ = ‖iteratedFDerivWithin 𝕜 i f s x‖ := fun i => by
...
/--
theorem `norm_iteratedFDeriv_comp_le'` / 定理 `norm_iteratedFDeriv_comp_le'`

English:
theorem norm_iteratedFDeriv_comp_le'
  statement: {g : F -> G} {f : E -> F} {n : Nat} {N : Nat∞ω}
  proof: by
...
中文:
定理 norm_iteratedFDeriv_comp_le'
  结论: {g : F -> G} {f : E -> F} {n : 自然数} {N : 自然数∞ω}
  证明: by
  simp_rw [← iteratedFDerivWithin_univ] at hD ⊢
  exact norm_iteratedFDerivWithin_comp_le hg hf.contDiffOn hn ht' uniqueDiffOn_univ
    (by simp [mapsTo_iff_subset_preimage, ht]) (mem_univ x) hC hD

Depends on / 依赖: contDiffOn, hf.contDiffOn, iteratedFDerivWithin_univ, mapsTo_iff_subset_preimage, mem_univ, norm_iteratedFDerivWithin_comp_le, simp_rw, uniqueDiffOn_univ
-/
theorem norm_iteratedFDeriv_comp_le' {g : F -> G} {f : E -> F} {n : Nat} {N : Nat∞ω}
    {t : Set F} (ht : Set.range f subseteq t) (ht' : UniqueDiffOn 𝕜 t)
    (hg : ContDiffOn 𝕜 N g t) (hf : ContDiff 𝕜 N f) (hn : n <= N) (x : E) {C : Real} {D : Real}
    (hC : forall i, i <= n -> ‖iteratedFDerivWithin 𝕜 i g t (f x)‖ <= C)
    (hD : forall i, 1 <= i -> i <= n -> ‖iteratedFDeriv 𝕜 i f x‖ <= D ^ i) :
...
Depends on / 依赖: contDiffOn, hg.contDiffOn, iteratedFDerivWithin_univ, norm_iteratedFDeriv_comp_le, simp_rw, subset_univ, uniqueDiffOn_univ
-/
theorem norm_iteratedFDeriv_comp_le {g : F -> G} {f : E -> F} {n : Nat} {N : Nat∞ω}
    (hg : ContDiff 𝕜 N g) (hf : ContDiff 𝕜 N f) (hn : n <= N) (x : E) {C : Real} {D : Real}
    (hC : forall i, i <= n -> ‖iteratedFDeriv 𝕜 i g (f x)‖ <= C)
    (hD : forall i, 1 <= i -> i <= n -> ‖iteratedFDeriv 𝕜 i f x‖ <= D ^ i) :
    ‖iteratedFDeriv 𝕜 n (g ∘ f) x‖ <= n ! * C * D ^ n := by
  simp_rw [← iteratedFDerivWithin_univ] at hC
  exact norm_iteratedFDeriv_comp_le' (subset_univ _) uniqueDiffOn_univ hg.contDiffOn hf hn x hC hD
...
/--
theorem `norm_iteratedFDerivWithin_clm_apply` / 定理 `norm_iteratedFDerivWithin_clm_apply`

English:
theorem norm_iteratedFDerivWithin_clm_apply
  statement: {f : E -> F ->L[𝕜] G} {g : E -> F} {s : Set E} {x : E}
  proof: by
...
Depends on / 依赖: B.norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, ContinuousLinearMap, ContinuousLinearMap.apply, ContinuousLinearMap.coe_id, ContinuousLinearMap.flip, ContinuousLinearMap.opNorm_flip, ContinuousLinearMap.opNorm_le_bound, coe_id, norm_iteratedFDerivWithin_le_of_bilinear_of_le_one, one_mul, opNorm_flip, opNorm_le_bound, zero_le_one
-/
theorem norm_iteratedFDerivWithin_clm_apply {f : E -> F ->L[𝕜] G} {g : E -> F} {s : Set E} {x : E}
    {N : Nat∞ω} {n : Nat} (hf : ContDiffOn 𝕜 N f s) (hg : ContDiffOn 𝕜 N g s)
    (hs : UniqueDiffOn 𝕜 s) (hx : x in s) (hn : n <= N) :
    ‖iteratedFDerivWithin 𝕜 n (fun y => (f y) (g y)) s x‖ <=
...
/--
theorem `norm_iteratedFDeriv_clm_apply` / 定理 `norm_iteratedFDeriv_clm_apply`

English:
theorem norm_iteratedFDeriv_clm_apply
  statement: {f : E -> F ->L[𝕜] G} {g : E -> F} {N : Nat∞ω} {n : Nat}
  proof: by
...
中文:
定理 norm_iteratedFDeriv_clm_apply
  结论: {f : E -> F ->L[𝕜] G} {g : E -> F} {N : 自然数∞ω} {n : 自然数}
  证明: by
  simp only [← iteratedFDerivWithin_univ]
  exact norm_iteratedFDerivWithin_clm_apply hf.contDiffOn hg.contDiffOn uniqueDiffOn_univ
    (Set.mem_univ x) hn

Depends on / 依赖: Set.mem_univ, contDiffOn, hf.contDiffOn, hg.contDiffOn, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_clm_apply, uniqueDiffOn_univ
-/
theorem norm_iteratedFDeriv_clm_apply {f : E -> F ->L[𝕜] G} {g : E -> F} {N : Nat∞ω} {n : Nat}
...
/--
theorem `ContinuousLinearMap.norm_iteratedFDerivWithin_comp_left` / 定理 `ContinuousLinearMap.norm_iteratedFDerivWithin_comp_left`

English:
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_comp_left
  statement: (L : F ->L[𝕜] G) {f : E -> F}
  proof: by
...
Depends on / 依赖: L.iteratedFDerivWithin_comp_left, L.norm_compContinuousMultilinearMap_le, iteratedFDerivWithin, iteratedFDerivWithin_comp_left, norm_compContinuousMultilinearMap_le
-/
theorem ContinuousLinearMap.norm_iteratedFDerivWithin_comp_left (L : F ->L[𝕜] G) {f : E -> F}
    {s : Set E} {x : E} {N : Nat∞ω} {n : Nat} (hf : ContDiffWithinAt 𝕜 N f s x)
    (hs : UniqueDiffOn 𝕜 s) (hx : x in s) (hn : n <= N) :
...
/--
theorem `ContinuousLinearMap.norm_iteratedFDeriv_comp_left` / 定理 `ContinuousLinearMap.norm_iteratedFDeriv_comp_left`

English:
theorem ContinuousLinearMap.norm_iteratedFDeriv_comp_left
  statement: (L : F ->L[𝕜] G) {f : E -> F} {x : E}
  proof: by
  simp only [← iteratedFDerivWithin_univ]
...
Depends on / 依赖: L.norm_iteratedFDerivWithin_comp_left, Set.mem_univ, contDiffWithinAt, hf.contDiffWithinAt, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_comp_left, uniqueDiffOn_univ
-/
theorem ContinuousLinearMap.norm_iteratedFDeriv_comp_left (L : F ->L[𝕜] G) {f : E -> F} {x : E}
    {N : Nat∞ω} {n : Nat} (hf : ContDiffAt 𝕜 N f x) (hn : n <= N) :
...
/--
theorem `norm_iteratedFDerivWithin_clm_apply_const` / 定理 `norm_iteratedFDerivWithin_clm_apply_const`

English:
theorem norm_iteratedFDerivWithin_clm_apply_const
  statement: {f : E -> F ->L[𝕜] G} {c : F} {s : Set E} {x : E}
  proof: by
...
Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.apply, ContinuousLinearMap.apply_apply, apply_apply, f.le_opNorm, le_opNorm, mul_comm, norm_iteratedFDerivWithin_comp_left, norm_nonneg, opNorm_le_bound
-/
theorem norm_iteratedFDerivWithin_clm_apply_const {f : E -> F ->L[𝕜] G} {c : F} {s : Set E} {x : E}
    {N : Nat∞ω} {n : Nat} (hf : ContDiffWithinAt 𝕜 N f s x) (hs : UniqueDiffOn 𝕜 s)
    (hx : x in s) (hn : n <= N) :
...
/--
theorem `norm_iteratedFDeriv_clm_apply_const` / 定理 `norm_iteratedFDeriv_clm_apply_const`

English:
theorem norm_iteratedFDeriv_clm_apply_const
  statement: {f : E -> F ->L[𝕜] G} {c : F} {x : E}
  proof: by
  simp only [← iteratedFDerivWithin_univ]
...
中文:
定理 norm_iteratedFDeriv_clm_apply_const
  结论: {f : E -> F ->L[𝕜] G} {c : F} {x : E}
  证明: by
  simp only [← iteratedFDerivWithin_univ]
  exact norm_iteratedFDerivWithin_clm_apply_const hf.contDiffWithinAt uniqueDiffOn_univ
    (Set.mem_univ x) hn

Depends on / 依赖: Set.mem_univ, contDiffWithinAt, hf.contDiffWithinAt, iteratedFDerivWithin_univ, mem_univ, norm_iteratedFDerivWithin_clm_apply_const, uniqueDiffOn_univ
-/
