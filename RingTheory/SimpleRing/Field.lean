...
/--
lemma `isField_center` / 引理 `isField_center`

English:
lemma isField_center
  given: (A : Type*) [Ring A] [IsSimpleRing A]
  statement: IsField (Subring.center A) where
...
lemma isField_center (A : Type*) [Ring A] [IsSimpleRing A] : IsField (Subring.center A) where
  ...
  mul_inv_cancel := by
    ...
    have mem : 1 in I := one_mem_of_ne_zero_mem I hx2 (by simpa [I, mem_mk'] using ⟨1, by simp⟩)
...
/--
lemma `isSimpleRing_iff_isField` / 引理 `isSimpleRing_iff_isField`

English:
lemma isSimpleRing_iff_isField
  given: (A : Type*) [CommRing A]
  statement: IsSimpleRing A ↔ IsField A
  proof: ⟨fun _ => Subring.topEquiv.symm.toMulEquiv.isField by
...
Depends on / 依赖: IsSimpleRing, IsSimpleRing.isField_center, Subring, Subring.center_eq_top, Subring.topEquiv.symm.toMulEquiv.isField, center_eq_top, h.toField, isField, isField_center, toField, toMulEquiv, topEquiv
-/
