...
/-
**Complex.eq_mul_div_one_add_of_eq_div_sub** 是 Mathlib 中的一个引理，位于命名空间 `Complex`。
形式化陈述：eq_mul_div_one_add_of_eq_div_sub (_ : M != 0) (_ : 2 * M - z != 0) (h : w 
= z / (2 * M - z)) : z = 2 * M * w / (1 + w)
参数：_ : M != 0；_ : 2 * M - z != 0；h : w = z / (2 * M - z)。
该定理/引理给出了一组等式。
...
`‖2M * w / (1 + w)‖ ≤ 2M * ‖w‖ / (1 - ‖w‖)`. -/
/-
**Complex.norm_two_mul_div_one_add_le** 是 Mathlib 中的一个引理，位于命名空间 `Complex`。
形式化陈述：norm_two_mul_div_one_add_le (hM : 0 < M) (hw : ‖w‖ < 1) : ‖2 * ↑M * w / (1
 + w)‖ <= 2 * M * ‖w‖ / (1 - ‖w‖)
参数：hM : 0 < M；hw : ‖w‖ < 1。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Complex.norm_le_norm_two_mul_sub** 是 Mathlib 中的一个引理，位于命名空间 `Complex`。
形式化陈述：norm_le_norm_two_mul_sub (_ : 0 < M) (_ : z.re <= M) : ‖z‖ <= ‖2 * M - z‖
参数：_ : 0 < M；_ : z.re <= M。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
If `z.re ≤ M`, then `‖z‖ ≤ ‖2M - z‖`. This shows that the Schwarz transform
`z ↦ z / (2M - z)` has numerator smaller than denominator when the real part is 
bounded by M
-/
...
/-
**Complex.schwarz_applied** 是 Mathlib 中的一个引理，位于命名空间 `Complex`。
形式化陈述：schwarz_applied (hM : 0 < M) (hf : DifferentiableOn Complex f (ball 0 R)) 
(hf₁ : Set.MapsTo f (ball 0 R) {z | z.re <= M}) (hz : z in ball 0 R) (hf₂ : f 0 
= 0) : ‖f z / (2 * M - f z)‖ <= (1 / R) * ‖z‖
参数：hM : 0 < M；hf : DifferentiableOn Complex f (ball 0 R)；hf₁ : Set.MapsTo f (bal
l 0 R) {z | z.re <= M}；hz : z in ball 0 R；hf₂ : f 0 = 0。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Application of the Schwarz lemma to the transformed function. If `f` is differen
tiable on
the ball, maps into `{z | z.re ≤ M}`, and satisfies `f 0 = 0`, then the Schwarz 
transform
satisfies the bound from the Schwarz lemma.
-/
