/-
Copyright (c) 2018 Johannes Hölzl. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Johannes Hölzl, Jens Wagemaker, Aaron Anderson
-/
module

public import Mathlib.RingTheory.Ideal.Operations
public import Mathlib.RingTheory.UniqueFactorizationDomain.Defs

/-!
# Unique factorization and ascending chain condition on ideals

## Main results
* `Ideal.setOfPred_isPrincipal_wellFoundedOn_gt`,
  `WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt`
  in a domain, well-foundedness of the strict version of ∣ is equivalent to the ascending
  chain condition on principal ideals.
-/

public section

variable {α : Type*}

open UniqueFactorizationMonoid in
/--
theorem `Ideal.IsPrime.exists_mem_prime_of_ne_bot` / 定理 `Ideal.IsPrime.exists_mem_prime_of_ne_bot`

English:
theorem Ideal.IsPrime.exists_mem_prime_of_ne_bot
  statement: {R : Type*} [CommSemiring R]
  proof: by
  obtain ⟨a : R, ha₁ : a in I, ha₂ : a != 0⟩ := Submodule.exists_mem_ne_zero_of_ne_bot hI
  replace ha₁ : (factors a).prod in I := by
    obtain ⟨u : Rˣ, hu : (factors a).prod * u = a⟩ := factors_prod ha₂
    rwa [← hu, mul_unit_mem_iff_mem _ u.isUnit] at ha₁
  obtain ⟨p : R, hp₁ : p in factors a

中文:
定理 Ideal.IsPrime.exists_mem_prime_of_ne_bot
  结论: {R : 类型} [CommSemiring R]
  证明: by
  obtain ⟨a : R, ha₁ : a in I, ha₂ : a != 0⟩ := Submodule.exists_mem_ne_zero_of_ne_bot hI
  replace ha₁ : (factors a).prod in I := by
    obtain ⟨u : Rˣ, hu : (factors a).prod * u = a⟩ := factors_prod ha₂
    rwa [← hu, mul_unit_mem_iff_mem _ u.isUnit] at ha₁
  obtain ⟨p : R, hp₁ : p in factors a

Depends on / 依赖: Submodule, Submodule.exists_mem_ne_zero_of_ne_bot, exists_mem_ne_zero_of_ne_bot, factors, factors_prod, isUnit, mul_unit_mem_iff_mem, multiset_prod_mem_iff_exists_mem, prime_of_factor, replace, u.isUnit
-/
theorem Ideal.IsPrime.exists_mem_prime_of_ne_bot {R : Type*} [CommSemiring R]
    [UniqueFactorizationMonoid R] {I : Ideal R} (hI₂ : I.IsPrime) (hI : I != ⊥) :
    exists x in I, Prime x := by
  obtain ⟨a : R, ha₁ : a in I, ha₂ : a != 0⟩ := Submodule.exists_mem_ne_zero_of_ne_bot hI
  replace ha₁ : (factors a).prod in I := by
    obtain ⟨u : Rˣ, hu : (factors a).prod * u = a⟩ := factors_prod ha₂
    rwa [← hu, mul_unit_mem_iff_mem _ u.isUnit] at ha₁
  obtain ⟨p : R, hp₁ : p in factors a, hp₂ : p in I⟩ :=
    (hI₂.multiset_prod_mem_iff_exists_mem <| factors a).1 ha₁
  exact ⟨p, hp₂, prime_of_factor p hp₁⟩

section Ideal

/--
lemma `Ideal.setOfPred_isPrincipal_wellFoundedOn_gt` / 引理 `Ideal.setOfPred_isPrincipal_wellFoundedOn_gt`

English:
lemma Ideal.setOfPred_isPrincipal_wellFoundedOn_gt
  given: [CommSemiring α] [WfDvdMonoid α] [IsDomain α]
  proof: by
  have : {I : Ideal α | I.IsPrincipal} = ((fun a => Ideal.span {a}) '' Set.univ) := by
    ext
    simp [Submodule.isPrincipal_iff, eq_comm]
  rw [this]; rw [Set.wellFoundedOn_image]; rw [Set.wellFoundedOn_univ]
  convert! wellFounded_dvdNotUnit (α := α)
  ext
  exact Ideal.span_singleton_lt_span

中文:
引理 Ideal.setOfPred_isPrincipal_wellFoundedOn_gt
  条件: [CommSemiring α] [WfDvdMonoid α] [IsDomain α]
  证明: by
  have : {I : Ideal α | I.IsPrincipal} = ((fun a => Ideal.span {a}) '' Set.univ) := by
    ext
    simp [Submodule.isPrincipal_iff, eq_comm]
  rw [this]; rw [Set.wellFoundedOn_image]; rw [Set.wellFoundedOn_univ]
  convert! wellFounded_dvdNotUnit (α := α)
  ext
  exact Ideal.span_singleton_lt_span

Depends on / 依赖: I.IsPrincipal, Ideal.span, Ideal.span_singleton_lt_span_singleton, IsPrincipal, Set.univ, Set.wellFoundedOn_image, Set.wellFoundedOn_univ, Submodule, Submodule.isPrincipal_iff, convert, eq_comm, isPrincipal_iff, span_singleton_lt_span_singleton, wellFoundedOn_image, wellFoundedOn_univ, wellFounded_dvdNotUnit
-/
lemma Ideal.setOfPred_isPrincipal_wellFoundedOn_gt [CommSemiring α] [WfDvdMonoid α] [IsDomain α] :
    {I : Ideal α | I.IsPrincipal}.WellFoundedOn (· > ·) := by
  have : {I : Ideal α | I.IsPrincipal} = ((fun a => Ideal.span {a}) '' Set.univ) := by
    ext
    simp [Submodule.isPrincipal_iff, eq_comm]
  rw [this]; rw [Set.wellFoundedOn_image]; rw [Set.wellFoundedOn_univ]
  convert! wellFounded_dvdNotUnit (α := α)
  ext
  exact Ideal.span_singleton_lt_span_singleton

@[deprecated (since := "2026-07-09")]
alias Ideal.setOf_isPrincipal_wellFoundedOn_gt := Ideal.setOfPred_isPrincipal_wellFoundedOn_gt

/--
lemma `WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt` / 引理 `WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt`

English:
lemma WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt
  statement: [CommSemiring α] [IsDomain α]
  proof: by
  have : WellFounded (α := {I : Ideal α // I.IsPrincipal}) (· > ·) := h
  constructor
  convert! InvImage.wf (fun a => ⟨Ideal.span ({ a } : Set α), _, rfl⟩) this
  ext
  exact Ideal.span_singleton_lt_span_singleton.symm

@[deprecated (since := "2026-07-09")]
alias WfDvdMonoid.of_setOf_isPrincipal

中文:
引理 WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt
  结论: [CommSemiring α] [IsDomain α]
  证明: by
  have : WellFounded (α := {I : Ideal α // I.IsPrincipal}) (· > ·) := h
  constructor
  convert! InvImage.wf (fun a => ⟨Ideal.span ({ a } : Set α), _, rfl⟩) this
  ext
  exact Ideal.span_singleton_lt_span_singleton.symm

@[deprecated (since := "2026-07-09")]
alias WfDvdMonoid.of_setOf_isPrincipal

Depends on / 依赖: I.IsPrincipal, Ideal.span, Ideal.span_singleton_lt_span_singleton.symm, InvImage, InvImage.wf, IsPrincipal, WellFounded, convert, span_singleton_lt_span_singleton
-/
lemma WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt [CommSemiring α] [IsDomain α]
    (h : {I : Ideal α | I.IsPrincipal}.WellFoundedOn (· > ·)) :
    WfDvdMonoid α := by
  have : WellFounded (α := {I : Ideal α // I.IsPrincipal}) (· > ·) := h
  constructor
  convert! InvImage.wf (fun a => ⟨Ideal.span ({ a } : Set α), _, rfl⟩) this
  ext
  exact Ideal.span_singleton_lt_span_singleton.symm

@[deprecated (since := "2026-07-09")]
alias WfDvdMonoid.of_setOf_isPrincipal_wellFoundedOn_gt :=
  WfDvdMonoid.of_setOfPred_isPrincipal_wellFoundedOn_gt

end Ideal
