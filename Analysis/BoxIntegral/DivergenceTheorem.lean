...
variable {E : Type u} [NormedAddCommGroup E] [NormedSpace Real E] {n : Nat}
...
/--
theorem `norm_volume_sub_integral_face_upper_sub_lower_smul_le` / 定理 `norm_volume_sub_integral_face_upper_sub_lower_smul_le`

English:
theorem norm_volume_sub_integral_face_upper_sub_lower_smul_le
  statement: {f : (Fin (n + 1) -> Real) -> E}
  proof: by
...
-/
theorem norm_volume_sub_integral_face_upper_sub_lower_smul_le {f : (Fin (n + 1) -> Real) -> E}
    {f' : (Fin (n + 1) -> Real) ->L[Real] E} (hfc : ContinuousOn f (Box.Icc I)) {x : Fin (n + 1) -> Real}
    (hxI : x in (Box.Icc I)) {a : E} {ε : Real} (h0 : 0 < ε)
    (hε : forall y in (Box.Icc I), ‖f y - a - f' (y - x)‖ <= ε * ‖y - x‖) {c : Real>=0}
    (hc : I.distortion <= c) :
    ‖(∏ j, (I.upper j - I.lower j)) • f' (Pi.single i 1) -
      (integral (I.face i) ⊥ (f ∘ i.insertNth (α := fun _ => Real) (I.upper i)) BoxAdditiveMap.volume -
        integral (I.face i) ⊥ (f ∘ i.insertNth (α := fun _ => Real) (I.lower i))
...
/--
theorem `hasIntegral_GP_pderiv` / 定理 `hasIntegral_GP_pderiv`

English:
theorem hasIntegral_GP_pderiv
  statement: (f : (Fin (n + 1) -> Real) -> E)
  proof: by
...
theorem hasIntegral_GP_pderiv (f : (Fin (n + 1) -> Real) -> E)
    ...
    (hs : s.Countable) (Hs : forall x in s, ContinuousWithinAt f (Box.Icc I) x)
    (Hd : forall x in (Box.Icc I) \ s, HasFDerivWithinAt f (f' x) (Box.Icc I) x) (i : Fin (n + 1)) :
...
/--
theorem `hasIntegral_GP_divergence_of_forall_hasDerivWithinAt` / 定理 `hasIntegral_GP_divergence_of_forall_hasDerivWithinAt`
...
Depends on / 依赖: HasIntegral, HasIntegral.sum, continuousWithinAt_pi, hasFDerivWithinAt_pi, hasIntegral_GP_pderiv
-/
theorem hasIntegral_GP_divergence_of_forall_hasDerivWithinAt
    (f : (Fin (n + 1) -> Real) -> Fin (n + 1) -> E)
    (f' : (Fin (n + 1) -> Real) -> (Fin (n + 1) -> Real) ->L[Real] (Fin (n + 1) -> E))
    (s : Set (Fin (n + 1) -> Real)) (hs : s.Countable)
    (Hs : forall x in s, ContinuousWithinAt f (Box.Icc I) x)
