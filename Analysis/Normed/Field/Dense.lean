...
/--
theorem `IsAlgClosed.of_denseRange` / 定理 `IsAlgClosed.of_denseRange`

English:
theorem IsAlgClosed.of_denseRange
  statement: {K L : Type*} [Field K] [NontriviallyNormedField L]
  proof: by
...
theorem IsAlgClosed.of_denseRange {K L : Type*} [Field K] [NontriviallyNormedField L]
  ...
  classical
    let S : Finset F := {x in (f.rootSet F).toFinset | x != a}
  let δ : Real := if hS : S.Nonempty then Finset.min' (S.image fun x => ‖a - x‖)
      (Finset.image_nonempty.mpr hS) else 1
  have norm_sub_le : forall a' : F, IsConjRoot L a a' -> a != a' -> δ <= ‖a - a'‖ := by
    intro a' conj ne
    ...
    · apply Finset.min'_le (S.image fun x => ‖a - x‖) (‖a - a'‖)
      ...
      simp only [minpoly.eq_of_irreducible_of_monic firr fa0 fmon, Finset.mem_filter,
        Set.mem_toFinset, S]
      rw [← (isConjRoot_iff_mem_minpoly_rootSet ⟨f]; rw [fmon]; rw [fa0⟩)]
      exact ⟨conj, ne.symm⟩
    · simp only [ne_eq, Finset.not_nonempty_iff_eq_empty, Finset.filter_eq_empty_iff,
      Set.mem_toFinset, not_not, S] at hS
      rw [isConjRoot_iff_mem_minpoly_rootSet ⟨f]; rw [fmon]; rw [fa0⟩]; rw [← minpoly.eq_of_irreducible_of_monic firr fa0 fmon] at conj
  ...
  have δpos : δ > 0 := by
    ...
    · simp only [gt_iff_lt, Finset.lt_min'_iff, Finset.mem_image, forall_exists_index, and_imp,
      ...
      rw [norm_pos_iff]; rw [sub_ne_zero]
      exact ha'.2.symm
    · linarith
  ...
  have hab : ‖a - b‖ < δ := by
    rw [← Real.rpow_natCast]; rw [← mul_comm_div]; rw [div_self]; rw [one_mul]; rw [← Real.rpow_mul (div_pos δpos (by positivity)).le]; rw [mul_inv_cancel₀] at hab
    · simpa [mul_assoc, div_mul_cancel₀ _ (by positivity : (max ‖a‖ 1) > 0).ne'] using hab
    · simp [fnatdeg0]
    · positivity
  have bbot : b in (⊥ : IntermediateField L F) := by
    rw [Polynomial.aroots_def]; rw [Splits.roots_map ((IsAlgClosed.splits g).map _)]; rw [Multiset.mem_map] at hb
  ...
  have abot : a in (⊥ : IntermediateField L F) := by
