...
/--
lemma `comp_lim_obj_ext` / 引理 `comp_lim_obj_ext`

English:
lemma comp_lim_obj_ext
  statement: {j : J} {G : J ⥤ K ⥤ Type v} (x y : (G ⋙ lim).obj j)
  proof: limit_ext _ x y w

中文:
引理 comp_lim_obj_ext
  结论: {j : J} {G : J ⥤ K ⥤ 类型v} (x y : (G ⋙ lim).obj j)
  证明: limit_ext _ x y w
-/
@[ext] lemma comp_lim_obj_ext {j : J} {G : J ⥤ K ⥤ Type v} (x y : (G ⋙ lim).obj j)
    (w : forall (k : K), limit.π (G.obj j) k x = limit.π (G.obj j) k y) : x = y :=
  limit_ext _ x y w
...
/--
theorem `colimitLimitToLimitColimit_injective` / 定理 `colimitLimitToLimitColimit_injective`
...
中文:
定理 colimitLimitToLimitColimit_injective
  证明: by
  classical
    cases nonempty_fintype J
    -- Suppose we have two terms `x y` in the colimit (over `K`) of the limits (over `J`),
...
theorem colimitLimitToLimitColimit_injective :
  ...
  classical
    ...
    let k j := (h j).choose
    let f : forall j, kx ⟶ k j := fun j => (h j).choose_spec.choose
    let g : forall j, ky ⟶ k j := fun j => (h j).choose_spec.choose_spec.choose
    -- where the images of the components of the representatives become equal:
    have w :
      forall j, F.map (𝟙 j ×ₘ f j) (limit.π ((curry.obj (swap K J ⋙ F)).obj kx) j x) =
    ...
    let O : Finset K := Finset.univ.image k union {kx, ky}
    have kxO : kx in O := Finset.mem_union.mpr (Or.inr (by simp))
    have kyO : ky in O := Finset.mem_union.mpr (Or.inr (by simp))
    have kjO : forall j, k j in O := fun j => Finset.mem_union.mpr (Or.inl (by simp))
    let H : Finset (Σ' (X Y : K) (_ : X in O) (_ : Y in O), X ⟶ Y) :=
      (Finset.univ.image fun j : J =>
          ⟨kx, k j, kxO, Finset.mem_union.mpr (Or.inl (by simp)), f j⟩) union
        Finset.univ.image fun j : J => ⟨ky, k j, kyO, Finset.mem_union.mpr (Or.inl (by simp)), g j⟩
    obtain ⟨S, T, W⟩ := IsFiltered.sup_exists O H
    have fH : forall j, (⟨kx, k j, kxO, kjO j, f j⟩ : Σ' (X Y : K) (_ : X in O) (_ : Y in O), X ⟶ Y) in H :=
      fun j =>
      Finset.mem_union.mpr
        (Or.inl
          (by
            ...
            simp only))
    have gH :
      forall j, (⟨ky, k j, kyO, kjO j, g j⟩ : Σ' (X Y : K) (_ : X in O) (_ : Y in O), X ⟶ Y) in H :=
      fun j =>
      Finset.mem_union.mpr
        (Or.inr
          (by
...
/--
theorem `colimitLimitToLimitColimit_surjective` / 定理 `colimitLimitToLimitColimit_surjective`
...
中文:
定理 colimitLimitToLimitColimit_surjective
  证明: by
  classical
    -- We begin with some element `x` in the limit (over J) over the colimits (over K),
    intro x
...
theorem colimitLimitToLimitColimit_surjective :
  ...
  classical
    ...
    let y : forall j, F.obj (j, k j) := fun j => (z j).choose_spec.choose
    ...
    have g : forall j, k j ⟶ k' := fun j => IsFiltered.toSup (Finset.univ.image k) ∅ (by simp)
    clear_value k'
    ...
    have w :
      ...
      rw [e]; rw [← limit.w_apply _ f]; rw [← e]
    ...
    let kf : forall {j j'} (_ : j ⟶ j'), K := fun f => (w f).choose
    let gf : forall {j j'} (f : j ⟶ j'), k' ⟶ kf f := fun f => (w f).choose_spec.choose
    let hf : forall {j j'} (f : j ⟶ j'), k' ⟶ kf f := fun f =>
    ...
    let O :=
      (Finset.univ.biUnion fun j => Finset.univ.biUnion fun j' => Finset.univ.image
        (@kf j j')) union {k'}
    have kfO : forall {j j'} (f : j ⟶ j'), kf f in O := fun {j} {j'} f =>
      Finset.mem_union.mpr
        (Or.inl
          (Finset.mem_biUnion.mpr ⟨j, Finset.mem_univ j,
            Finset.mem_biUnion.mpr ⟨j', Finset.mem_univ j',
              Finset.mem_image.mpr ⟨f, Finset.mem_univ _, rfl⟩⟩⟩))
    have k'O : k' in O := Finset.mem_union.mpr (Or.inr (Finset.mem_singleton.mpr rfl))
    let H : Finset (Σ' (X Y : K) (_ : X in O) (_ : Y in O), X ⟶ Y) :=
    ...
    let i : forall {j j'} (f : j ⟶ j'), kf f ⟶ k'' := fun {j} {j'} f => i' (kfO f)
    have s : forall {j₁ j₂ j₃ j₄} (f : j₁ ⟶ j₂) (f' : j₃ ⟶ j₄), gf f ≫ i f = hf f' ≫ i f' := by
      intro j₁ j₂ j₃ j₄ f f'
      rw [s']; rw [s']
      · exact k'O
      · exact Finset.mem_biUnion.mpr ⟨j₃, Finset.mem_univ _,
          Finset.mem_biUnion.mpr ⟨j₄, Finset.mem_univ _,
            Finset.mem_biUnion.mpr ⟨f', Finset.mem_univ _, by
              ...
              rw [Finset.mem_insert]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [Finset.mem_singleton]; rw [eq_self]; rw [or_true]
              trivial⟩⟩⟩
      · exact Finset.mem_biUnion.mpr ⟨j₁, Finset.mem_univ _,
          Finset.mem_biUnion.mpr ⟨j₂, Finset.mem_univ _,
            Finset.mem_biUnion.mpr ⟨f, Finset.mem_univ _, by
              -- This works by `simp`, but has very high variation in heartbeats.
              rw [Finset.mem_insert]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [PSigma.mk.injEq]; rw [heq_eq_eq]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [eq_self]; rw [true_and]; rw [Finset.mem_singleton]; rw [eq_self]; rw [true_or]
              trivial⟩⟩⟩
    clear_value i
      ...
      · -- After which it's just a calculation, using `s` and `wf`, to see they are coherent.
        ...
        calc
          ...
          _ =
            ...
            rw [← comp_apply]; rw [← Functor.map_comp]; rw [prod_comp]; rw [comp_id]; rw [assoc]
          _ =
              F.map (𝟙 j' ×ₘ i f) (F.map (𝟙 j' ×ₘ (g j' ≫ gf f)) (y j')) := by
            rw [← wf f]
          _ = F.map (𝟙 j' ×ₘ (g j' ≫ gf f ≫ i f)) (y j') := by
            rw [← comp_apply]; rw [← Functor.map_comp]; rw [prod_comp]; rw [id_comp]; rw [assoc]
    ...
    · -- We can do this componentwise:
      ...
      dsimp
      rw [← dsimp% e j]; rw [dsimp% Limit.π_mk _ _ h]
...
/--
Instance `colimitLimitToLimitColimit_isIso` / 实例 `colimitLimitToLimitColimit_isIso`

English:
instance colimitLimitToLimitColimit_isIso
  signature: : IsIso (colimitLimitToLimitColimit F)
  body: (isIso_iff_bijective _).mpr
...
Depends on / 依赖: colimitLimitToLimitColimit_injective, colimitLimitToLimitColimit_surjective, isIso_iff_bijective
-/
...
set_option backward.isDefEq.respectTransparency false in
/--
Instance `colimitLimitToLimitColimitCone_iso` / 实例 `colimitLimitToLimitColimitCone_iso`

English:
instance colimitLimitToLimitColimitCone_iso
  signature: (F : J ⥤ K ⥤ Type v)
  body: by
...
中文:
实例 colimitLimitToLimitColimitCone_iso
  签名: (F : J ⥤ K ⥤ 类型v)
  定义体: by
  have : IsIso (colimitLimitToLimitColimitCone F).hom := by
    suffices IsIso (colimitLimitToLimitColimit (uncurry.obj F) ≫
...
Depends on / 依赖: Cone.cone_iso_of_hom_iso, IsIso.comp_isIso, colimitLimitToLimitColimit, colimitLimitToLimitColimitCone, comp_isIso, cone_iso_of_hom_iso, currying, currying.unitIso.app, infer_instance, lim.map, uncurry, uncurry.obj, unitIso, whiskerRight
-/
...
/--
Instance `filtered_colim_preservesFiniteLimits_of_types` / 实例 `filtered_colim_preservesFiniteLimits_of_types`
...
instance filtered_colim_preservesFiniteLimits_of_types
  ...
  · exact asIso (colimitLimit

中文:
实例 filtered_colim_preservesFiniteLimits_of_types
  签名: :
  定义体: by
  apply preservesFiniteLimits_of_preservesFiniteLimitsOfSize.{v₂}
  intro J _ _
...
Depends on / 依赖: Functor, Functor.mapIso, IsLimit, IsLimit.ofIsoLimit, colim.mapCone, colimitLimitToLimitColimitCone, hc.uniqueUpToIso, isLimit, limit.cone, limit.isLimit, mapCone, mapIso, ofIsoLimit, preservesFiniteLimits_of_preservesFiniteLimitsOfSize, uniqueUpToIso
-/
...
variable {C : Type u} [Category.{v} C] {FC : C -> C -> Type*} {CC : C -> Type v}
    [forall X Y, FunLike (FC X Y) (CC X) (CC Y)] [ConcreteCategory.{v} C FC]
...
/--
Instance `filtered_colim_preservesFiniteLimits` / 实例 `filtered_colim_preservesFiniteLimits`

English:
instance filtered_colim_preservesFiniteLimits
  signature: :
  body: haveI : PreservesLimitsOfShape J ((colim : (K ⥤ C) ⥤ _) ⋙ forget C) :=
    preservesLimitsOfShape_of_natIso (preservesColimitNatIso _).symm
  preservesLimitsOfShape_of_reflects_of_preserves _ (forget C)

中文:
实例 filtered_colim_preservesFiniteLimits
  签名: :
  定义体: haveI : PreservesLimitsOfShape J ((colim : (K ⥤ C) ⥤ _) ⋙ forget C) :=
    preservesLimitsOfShape_of_natIso (preservesColimitNatIso _).symm
  preservesLimitsOfShape_of_reflects_of_preserves _ (forget C)

Depends on / 依赖: PreservesLimitsOfShape, forget, preservesColimitNatIso, preservesLimitsOfShape_of_natIso, preservesLimitsOfShape_of_reflects_of_preserves
-/
...
instance [PreservesFiniteLimits
  signature: (forget C)] [PreservesColimitsOfShape K (forget C)]
  body: by
  apply preservesFiniteLimits_of_preservesFiniteLimitsOfSize.{v}
  intro J _ _
  infer_instance
...
Depends on / 依赖: infer_instance, preservesFiniteLimits_of_preservesFiniteLimitsOfSize
-/
...
/--
Definition of `colimitLimitIso` / `colimitLimitIso` 的定义

English:
definition colimitLimitIso
  signature: (F : J ⥤ K ⥤ C)
  body: (isLimitOfPreserves colim (limit.isLimit _)).conePointUniqueUpToIso (limit.isLimit _) ≪≫
    HasLimit.isoOfNatIso (colimitFlipIsoCompColim _).symm

中文:
定义 colimitLimitIso
  签名: (F : J ⥤ K ⥤ C)
  定义体: (isLimitOfPreserves colim (limit.isLimit _)).conePointUniqueUpToIso (limit.isLimit _) ≪≫
    HasLimit.isoOfNatIso (colimitFlipIsoCompColim _).symm

Depends on / 依赖: HasLimit, HasLimit.isoOfNatIso, colimitFlipIsoCompColim, conePointUniqueUpToIso, isLimit, isLimitOfPreserves, isoOfNatIso, limit.isLimit
-/
...
@[reassoc (attr := simp)]
/--
theorem `ι_colimitLimitIso_limit_π` / 定理 `ι_colimitLimitIso_limit_π`

English:
theorem ι_colimitLimitIso_limit_π
  given: (F : J ⥤ K ⥤ C) (a) (b)
  proof: by
  simp [colimitLimitIso]

中文:
定理 ι_colimitLimitIso_limit_π
  条件: (F : J ⥤ K ⥤ C) (a) (b)
  证明: by
  simp [colimitLimitIso]

Depends on / 依赖: colimitLimitIso
-/
