/-
Copyright (c) 2019 Chris Hughes. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Chris Hughes
-/
module

public import Mathlib.NumberTheory.Zsqrtd.GaussianInt
public import Mathlib.NumberTheory.LegendreSymbol.Basic

/-!
# Facts about the Gaussian integers relying on quadratic reciprocity.

## Main statements

`prime_iff_mod_four_eq_three_of_nat_prime`
A prime natural number is prime in `ℤ[i]` if and only if it is `3` mod `4`

-/

public section


open Zsqrtd Complex

open scoped ComplexConjugate

local notation "Int[i]" => GaussianInt

namespace GaussianInt

open PrincipalIdealRing

/--
theorem `mod_four_eq_three_of_nat_prime_of_prime` / 定理 `mod_four_eq_three_of_nat_prime_of_prime`

English:
theorem mod_four_eq_three_of_nat_prime_of_prime
  statement: (p : Nat) [hp : Fact p.Prime]
  proof: hp.1.eq_two_or_odd.elim
    (fun hp2 => by
      have := hpi.irreducible.isUnit_or_isUnit (a := ⟨1, 1⟩) (b := ⟨1, -1⟩)
      simp [hp2, Zsqrtd.ext_iff, ← norm_eq_one_iff, norm_def] at this)
    fun hp1 =>
    by_contradiction fun hp3 : p % 4 != 3 => by
      let ⟨k, hk⟩ := (ZMod.exists_sq_eq_neg_one_iff (p := p)).2 hp3
      obtain ⟨k, k_lt_p, rfl⟩ : exists (k' : Nat) (_ : k' < p), (k' : ZMod p) = k := by
        exact ⟨k.val, k.val_lt, ZMod.natCast_zmod_val k⟩
      have hpk : p ∣ k ^ 2 + 1 := by
        rw [pow_two]; rw [← CharP.cast_eq_zero_iff (ZMod p) p]; rw [Nat.cast_add]; rw [Nat.cast_mul]; rw [Nat.cast_one]; rw [← hk]; rw [neg_add_cancel]
      have hkmul : (k ^ 2 + 1 : Int[i]) = ⟨k, 1⟩ * ⟨k, -1⟩ := by ext <;> simp [sq]
      have hk₀ : k != 0 := by rintro rfl; simp at hk
      have hkltp := calc
          1 + k * k <= k * (k + 1) := by lia
          _ < p * p := mul_lt_mul k_lt_p k_lt_p (Nat.succ_pos _) (Nat.zero_le _)
      have hpk₁ : ¬(p : Int[i]) ∣ ⟨k, -1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, -1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (-1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      have hpk₂ : ¬(p : Int[i]) ∣ ⟨k, 1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, 1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      obtain ⟨y, hy⟩ := hpk
      have := hpi.2.2 ⟨k, 1⟩ ⟨k, -1⟩ ⟨y, by rw [← hkmul, ← Nat.cast_mul p, ← hy]; simp⟩
      tauto

中文:
定理 mod_four_eq_three_of_nat_prime_of_prime
  结论: (p : 自然数) [hp : Fact p.素]
  证明: hp.1.eq_two_or_odd.elim
    (fun hp2 => by
      have := hpi.irreducible.isUnit_or_isUnit (a := ⟨1, 1⟩) (b := ⟨1, -1⟩)
      simp [hp2, Zsqrtd.ext_iff, ← norm_eq_one_iff, norm_def] at this)
    fun hp1 =>
    by_contradiction fun hp3 : p % 4 != 3 => by
      let ⟨k, hk⟩ := (ZMod.exists_sq_eq_neg_one_iff (p := p)).2 hp3
      obtain ⟨k, k_lt_p, rfl⟩ : exists (k' : Nat) (_ : k' < p), (k' : ZMod p) = k := by
        exact ⟨k.val, k.val_lt, ZMod.natCast_zmod_val k⟩
      have hpk : p ∣ k ^ 2 + 1 := by
        rw [pow_two]; rw [← CharP.cast_eq_zero_iff (ZMod p) p]; rw [Nat.cast_add]; rw [Nat.cast_mul]; rw [Nat.cast_one]; rw [← hk]; rw [neg_add_cancel]
      have hkmul : (k ^ 2 + 1 : Int[i]) = ⟨k, 1⟩ * ⟨k, -1⟩ := by ext <;> simp [sq]
      have hk₀ : k != 0 := by rintro rfl; simp at hk
      have hkltp := calc
          1 + k * k <= k * (k + 1) := by lia
          _ < p * p := mul_lt_mul k_lt_p k_lt_p (Nat.succ_pos _) (Nat.zero_le _)
      have hpk₁ : ¬(p : Int[i]) ∣ ⟨k, -1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, -1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (-1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      have hpk₂ : ¬(p : Int[i]) ∣ ⟨k, 1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, 1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      obtain ⟨y, hy⟩ := hpk
      have := hpi.2.2 ⟨k, 1⟩ ⟨k, -1⟩ ⟨y, by rw [← hkmul, ← Nat.cast_mul p, ← hy]; simp⟩
      tauto

Depends on / 依赖: CharP.cast_eq_zero_iff, ZMod.exists_sq_eq_neg_one_iff, ZMod.natCast_zmod_val, Zsqrtd, Zsqrtd.ext_iff, by_contradiction, cast_eq_zero_iff, eq_two_or_odd, eq_two_or_odd.elim, exists_sq_eq_neg_one_iff, ext_iff, hpi.irreducible.isUnit_or_isUnit, irreducible, isUnit_or_isUnit, k.val, k.val_lt, k_lt_p, natCast_zmod_val, norm_def, norm_eq_one_iff
-/
theorem mod_four_eq_three_of_nat_prime_of_prime (p : Nat) [hp : Fact p.Prime]
    (hpi : Prime (p : Int[i])) : p % 4 = 3 :=
  hp.1.eq_two_or_odd.elim
    (fun hp2 => by
      have := hpi.irreducible.isUnit_or_isUnit (a := ⟨1, 1⟩) (b := ⟨1, -1⟩)
      simp [hp2, Zsqrtd.ext_iff, ← norm_eq_one_iff, norm_def] at this)
    fun hp1 =>
    by_contradiction fun hp3 : p % 4 != 3 => by
      let ⟨k, hk⟩ := (ZMod.exists_sq_eq_neg_one_iff (p := p)).2 hp3
      obtain ⟨k, k_lt_p, rfl⟩ : exists (k' : Nat) (_ : k' < p), (k' : ZMod p) = k := by
        exact ⟨k.val, k.val_lt, ZMod.natCast_zmod_val k⟩
      have hpk : p ∣ k ^ 2 + 1 := by
        rw [pow_two]; rw [← CharP.cast_eq_zero_iff (ZMod p) p]; rw [Nat.cast_add]; rw [Nat.cast_mul]; rw [Nat.cast_one]; rw [← hk]; rw [neg_add_cancel]
      have hkmul : (k ^ 2 + 1 : Int[i]) = ⟨k, 1⟩ * ⟨k, -1⟩ := by ext <;> simp [sq]
      have hk₀ : k != 0 := by rintro rfl; simp at hk
      have hkltp := calc
          1 + k * k <= k * (k + 1) := by lia
          _ < p * p := mul_lt_mul k_lt_p k_lt_p (Nat.succ_pos _) (Nat.zero_le _)
      have hpk₁ : ¬(p : Int[i]) ∣ ⟨k, -1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, -1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (-1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      have hpk₂ : ¬(p : Int[i]) ∣ ⟨k, 1⟩ := fun ⟨x, hx⟩ =>
lt_irrefl (p * x : Int[i]).norm.natAbs
          calc
            (norm (p * x : Int[i])).natAbs = (Zsqrtd.norm ⟨k, 1⟩).natAbs := by rw [hx]
            _ < (norm (p : Int[i])).natAbs := by simpa [add_comm, Zsqrtd.norm] using! hkltp
            _ <= (norm (p * x : Int[i])).natAbs :=
              norm_le_norm_mul_left _ fun hx0 =>
show (1 : Int) != 0 by decide by simpa [hx0] using! congr_arg Zsqrtd.im hx
      obtain ⟨y, hy⟩ := hpk
      have := hpi.2.2 ⟨k, 1⟩ ⟨k, -1⟩ ⟨y, by rw [← hkmul, ← Nat.cast_mul p, ← hy]; simp⟩
      tauto

/--
theorem `prime_of_nat_prime_of_mod_four_eq_three` / 定理 `prime_of_nat_prime_of_mod_four_eq_three`

English:
theorem prime_of_nat_prime_of_mod_four_eq_three
  given: (p : Nat) [Fact p.Prime] (hp3 : p % 4 = 3)
  proof: irreducible_iff_prime.1
    by_contradiction fun hpi =>
      let ⟨a, b, hab⟩ := sq_add_sq_of_nat_prime_of_not_irreducible p hpi
      have : forall a b : ZMod 4, a ^ 2 + b ^ 2 != (p : ZMod 4) := by
        rw [← ZMod.natCast_mod p 4]; rw [hp3]; decide
      this a b (hab ▸ by simp)

中文:
定理 prime_of_nat_prime_of_mod_four_eq_three
  条件: (p : 自然数) [Fact p.素] (hp3 : p % 4 = 3)
  证明: irreducible_iff_prime.1
    by_contradiction fun hpi =>
      let ⟨a, b, hab⟩ := sq_add_sq_of_nat_prime_of_not_irreducible p hpi
      have : forall a b : ZMod 4, a ^ 2 + b ^ 2 != (p : ZMod 4) := by
        rw [← ZMod.natCast_mod p 4]; rw [hp3]; decide
      this a b (hab ▸ by simp)

Depends on / 依赖: ZMod.natCast_mod, by_contradiction, irreducible_iff_prime, natCast_mod, sq_add_sq_of_nat_prime_of_not_irreducible
-/
theorem prime_of_nat_prime_of_mod_four_eq_three (p : Nat) [Fact p.Prime] (hp3 : p % 4 = 3) :
    Prime (p : Int[i]) :=
irreducible_iff_prime.1
    by_contradiction fun hpi =>
      let ⟨a, b, hab⟩ := sq_add_sq_of_nat_prime_of_not_irreducible p hpi
      have : forall a b : ZMod 4, a ^ 2 + b ^ 2 != (p : ZMod 4) := by
        rw [← ZMod.natCast_mod p 4]; rw [hp3]; decide
      this a b (hab ▸ by simp)

/--
theorem `prime_iff_mod_four_eq_three_of_nat_prime` / 定理 `prime_iff_mod_four_eq_three_of_nat_prime`

English:
theorem prime_iff_mod_four_eq_three_of_nat_prime
  given: (p : Nat) [Fact p.Prime]
  proof: ⟨mod_four_eq_three_of_nat_prime_of_prime p, prime_of_nat_prime_of_mod_four_eq_three p⟩

中文:
定理 prime_iff_mod_four_eq_three_of_nat_prime
  条件: (p : 自然数) [Fact p.素]
  证明: ⟨mod_four_eq_three_of_nat_prime_of_prime p, prime_of_nat_prime_of_mod_four_eq_three p⟩

Depends on / 依赖: mod_four_eq_three_of_nat_prime_of_prime, prime_of_nat_prime_of_mod_four_eq_three
-/
theorem prime_iff_mod_four_eq_three_of_nat_prime (p : Nat) [Fact p.Prime] :
    Prime (p : Int[i]) ↔ p % 4 = 3 :=
  ⟨mod_four_eq_three_of_nat_prime_of_prime p, prime_of_nat_prime_of_mod_four_eq_three p⟩

end GaussianInt
