...
/-
**Quiver.Arborescence** 是 Mathlib 中的一个归纳类型，位于命名空间 `Quiver`。
...
/-
**Quiver.root** 是 Mathlib 中的一个定义，位于命名空间 `Quiver`。
形式化陈述：root (V : Type u) [Quiver V] [Arborescence V] : V
参数：V : Type u。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[instance_reducible]
/-
**Quiver.arborescenceMk** 是 Mathlib 中的一个定义，位于命名空间 `Quiver`。
形式化陈述：arborescenceMk {V : Type u} [Quiver V] (r : V) (height : V -> Nat) (height
_lt : forall ⦃a b⦄, (a ⟶ b) -> height a < height b) (unique_arrow : forall ⦃a b 
c : V⦄ (e : a ⟶ c) (f : b ⟶ c), a = b ∧ e ≍ f) (root_or_arrow : forall b, b = r 
∨ exists a, Nonempty (a ⟶ b)) : Arborescence V where root
参数：r : V；height : V -> Nat；height_lt : forall ⦃a b⦄, (a ⟶ b) -> height a < heigh
t b；unique_arrow : forall ⦃a b c : V⦄ (e : a ⟶ c) (f : b ⟶ c), a = b ∧ e ≍ f；roo
t_or_arrow : forall b, b = r ∨ exists a, Nonempty (a ⟶ b)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Quiver.RootedConnected** 是 Mathlib 中的一个归纳类型，位于命名空间 `Quiver`。
形式化陈述：{V : Type u} → [Quiver V] → V → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Quiver.shortestPath** 是 Mathlib 中的一个定义，位于命名空间 `Quiver`。
形式化陈述：shortestPath (b : V) : Path r b
参数：b : V。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Quiver.shortest_path_spec** 是 Mathlib 中的一个定理，位于命名空间 `Quiver`。
形式化陈述：shortest_path_spec {a : V} (p : Path r a) : (shortestPath r a).length <= p
.length
参数：p : Path r a。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Quiver.geodesicSubtree** 是 Mathlib 中的一个定义，位于命名空间 `Quiver`。
形式化陈述：geodesicSubtree : WideSubquiver V
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Quiver.geodesicArborescence** 是 Mathlib 中的一个实例，位于命名空间 `Quiver`。
形式化陈述：geodesicArborescence : Arborescence (geodesicSubtree r)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
