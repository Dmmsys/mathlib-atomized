...
/-
**FiberwiseLinear.openPartialHomeomorph** 是 Mathlib 中的一个定义，位于命名空间 `FiberwiseLine
ar`。
形式化陈述：openPartialHomeomorph (φ : B -> F ≃L[𝕜] F) (hU : IsOpen U) (hφ : Continuou
sOn (fun x => φ x : B -> F ->L[𝕜] F) U) (h2φ : ContinuousOn (fun x => (φ x).symm
 : B -> F ->L[𝕜] F) U) : OpenPartialHomeomorph (B × F) (B × F) where toFun x
参数：φ : B -> F ≃L[𝕜] F；hU : IsOpen U；hφ : ContinuousOn (fun x => φ x : B -> F ->L
[𝕜] F) U；h2φ : ContinuousOn (fun x => (φ x).symm : B -> F ->L[𝕜] F) U。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
For `B` a topological space and `F` a `𝕜`-normed space, a map from `U : Set B` t
o `F ≃L[𝕜] F`
determines an open partial homeomorphism from `B × F` to itself by its action fi
berwise.
-/
...
equivalences. -/
/-
**FiberwiseLinear.trans_openPartialHomeomorph_apply** 是 Mathlib 中的一个定理，位于命名空间 `F
iberwiseLinear`。
形式化陈述：trans_openPartialHomeomorph_apply (hU : IsOpen U) (hφ : ContinuousOn (fun 
x => φ x : B -> F ->L[𝕜] F) U) (h2φ : ContinuousOn (fun x => (φ x).symm : B -> F
 ->L[𝕜] F) U) (hU' : IsOpen U') (hφ' : ContinuousOn (fun x => φ' x : B -> F ->L[
𝕜] F) U') (h2φ' : ContinuousOn (fun x => (φ' x).symm : B -> F ->L[𝕜] F) U') (b :
 B) (v : F) : (FiberwiseLinear.openPartialHomeomorph φ hU hφ h2φ ≫ₕ FiberwiseLin
ear.openPartialHomeomorph φ' hU' hφ' h2φ') ⟨b, v⟩ = ⟨b, φ' b (φ b v)⟩
参数：hU : IsOpen U；hφ : ContinuousOn (fun x => φ x : B -> F ->L[𝕜] F) U；h2φ : Cont
inuousOn (fun x => (φ x).symm : B -> F ->L[𝕜] F) U；hU' : IsOpen U'；hφ' : Continu
ousOn (fun x => φ' x : B -> F ->L[𝕜] F) U'；h2φ' : ContinuousOn (fun x => (φ' x).
symm : B -> F ->L[𝕜] F) U'；b : B；v : F。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
--- 原说明 ---
Compute the composition of two open partial homeomorphisms induced by fiberwise 
linear
equivalences.
-/
...
linear equivalences. -/
/-
**FiberwiseLinear.source_trans_openPartialHomeomorph** 是 Mathlib 中的一个定理，位于命名空间 `
FiberwiseLinear`。
形式化陈述：source_trans_openPartialHomeomorph (hU : IsOpen U) (hφ : ContinuousOn (fun
 x => φ x : B -> F ->L[𝕜] F) U) (h2φ : ContinuousOn (fun x => (φ x).symm : B -> 
F ->L[𝕜] F) U) (hU' : IsOpen U') (hφ' : ContinuousOn (fun x => φ' x : B -> F ->L
[𝕜] F) U') (h2φ' : ContinuousOn (fun x => (φ' x).symm : B -> F ->L[𝕜] F) U') : (
FiberwiseLinear.openPartialHomeomorph φ hU hφ h2φ ≫ₕ FiberwiseLinear.openPartial
Homeomorph φ' hU' hφ' h2φ').source = (U inter U') ×ˢ univ
参数：hU : IsOpen U；hφ : ContinuousOn (fun x => φ x : B -> F ->L[𝕜] F) U；h2φ : Cont
inuousOn (fun x => (φ x).symm : B -> F ->L[𝕜] F) U；hU' : IsOpen U'；hφ' : Continu
ousOn (fun x => φ' x : B -> F ->L[𝕜] F) U'；h2φ' : ContinuousOn (fun x => (φ' x).
symm : B -> F ->L[𝕜] F) U'。
...
--- 原说明 ---
Compute the source of the composition of two open partial homeomorphisms induced
 by fiberwise
linear equivalences.
-/
...
linear equivalences. -/
/-
**FiberwiseLinear.target_trans_openPartialHomeomorph** 是 Mathlib 中的一个定理，位于命名空间 `
FiberwiseLinear`。
形式化陈述：target_trans_openPartialHomeomorph (hU : IsOpen U) (hφ : ContinuousOn (fun
 x => φ x : B -> F ->L[𝕜] F) U) (h2φ : ContinuousOn (fun x => (φ x).symm : B -> 
F ->L[𝕜] F) U) (hU' : IsOpen U') (hφ' : ContinuousOn (fun x => φ' x : B -> F ->L
[𝕜] F) U') (h2φ' : ContinuousOn (fun x => (φ' x).symm : B -> F ->L[𝕜] F) U') : (
FiberwiseLinear.openPartialHomeomorph φ hU hφ h2φ ≫ₕ FiberwiseLinear.openPartial
Homeomorph φ' hU' hφ' h2φ').target = (U inter U') ×ˢ univ
参数：hU : IsOpen U；hφ : ContinuousOn (fun x => φ x : B -> F ->L[𝕜] F) U；h2φ : Cont
inuousOn (fun x => (φ x).symm : B -> F ->L[𝕜] F) U；hU' : IsOpen U'；hφ' : Continu
ousOn (fun x => φ' x : B -> F ->L[𝕜] F) U'；h2φ' : ContinuousOn (fun x => (φ' x).
symm : B -> F ->L[𝕜] F) U'。
...
--- 原说明 ---
Compute the target of the composition of two open partial homeomorphisms induced
 by fiberwise
linear equivalences.
-/
...
/-
**ContMDiffFiberwiseLinear.locality_aux** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Let `e` be an open partial homeomorphism of `B × F`.  Suppose that at every poin
t `p` in the
source of `e`, there is some neighbourhood `s` of `p` on which `e` is equal to a
 bi-`C^n` fiberwise
linear open partial homeomorphism.
Then the source of `e` is of the form `U ×ˢ univ`, for some set `U` in `B`, and,
 at any point `x` in
`U`, admits a neighbourhood `u` of `x` such that `e` is equal on `u ×ˢ univ` to 
some bi-`C^n`
fiberwise linear open partial homeomorphism.
-/
...
--- 原说明 ---
Let `e` be an open partial homeomorphism of `B × F` whose source is `U ×ˢ univ`,
 for some set
`U` in `B`, and which, at any point `x` in `U`, admits a neighbourhood `u` of `x
` such that `e` is
equal on `u ×ˢ univ` to some bi-`C^n` fiberwise linear open partial homeomorphis
m.  Then `e` itself
is equal to some bi-`C^n` fiberwise linear open partial homeomorphism.

This is the key mathematical point of the `locality` condition in the constructi
on of the
`StructureGroupoid` of bi-`C^n` fiberwise linear open partial homeomorphisms.  T
he proof is by
gluing together the various bi-`C^n` fiberwise linear open partial homeomorphism
 which exist
locally.

The `U` in the conclusion is the same `U` as in the hypothesis. We state it like
 this, because this
is exactly what we need for `contMDiffFiberwiseLinear`.
-/
theorem ContMDiffFiberwiseLinear.locality_aux₂
...
/-
**mem_aux** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**contMDiffFiberwiseLinear** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：contMDiffFiberwiseLinear (n : Nat∞ω) : StructureGroupoid (B × F) where mem
bers
参数：n : Nat∞ω。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
For `B` a manifold and `F` a normed space, the groupoid on `B × F` consisting of
 local
homeomorphisms which are bi-`C^n` and fiberwise linear, and induce the identity 
on `B`.
When a (topological) vector bundle is `C^n`, then the composition of charts asso
ciated
to the vector bundle belong to this groupoid.
-/
...
@[simp]
/-
**mem_contMDiffFiberwiseLinear_iff** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：mem_contMDiffFiberwiseLinear_iff {n : Nat∞ω} (e : OpenPartialHomeomorph (B
 × F) (B × F)) : e in contMDiffFiberwiseLinear B F IB n ↔ exists (φ : B -> F ≃L[
𝕜] F) (U : Set B) (hU : IsOpen U) (hφ : ContMDiffOn IB 𝓘(𝕜, F ->L[𝕜] F) n (fun x
 => φ x : B -> F ->L[𝕜] F) U) (h2φ : ContMDiffOn IB 𝓘(𝕜, F ->L[𝕜] F) n (fun x =>
 (φ x).symm : B -> F ->L[𝕜] F) U), e.EqOnSource (FiberwiseLinear.openPartialHome
omorph φ hU hφ.continuousOn h2φ.continuousOn)
参数：e : OpenPartialHomeomorph (B × F) (B × F)。
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `_private.Mathlib.Geometry.Manifold.VectorBundle.FiberwiseLinear.0.mem_au
x`：∀ {𝕜 : Type u_1} {B : Type u_2} {F : Type u_3} [inst : TopologicalSpace B] [i
nst_1 : NontriviallyNormedField 𝕜]   [inst_2 : NormedAddCommGro…
-/
