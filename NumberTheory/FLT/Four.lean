/-
Copyright (c) 2020 Paul van Wamelen. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Paul van Wamelen
-/
module

public import Mathlib.Data.Nat.Factors
public import Mathlib.NumberTheory.FLT.Basic
public import Mathlib.NumberTheory.PythagoreanTriples
public import Mathlib.RingTheory.Coprime.Lemmas
public import Mathlib.Tactic.LinearCombination

/-!
# Fermat's Last Theorem for the case n = 4

There are no non-zero integers `a`, `b` and `c` such that `a ^ 4 + b ^ 4 = c ^ 4`.
-/

@[expose] public section

assert_not_exists TwoSidedIdeal

noncomputable section

/--
Definition of `Fermat42` / `Fermat42` 的定义

English:
definition Fermat42
  signature: (a b c : Int)
  body: a != 0 ∧ b != 0 ∧ a ^ 4 + b ^ 4 = c ^ 2

中文:
定义 Fermat42
  签名: (a b c : 整数)
  定义体: a != 0 ∧ b != 0 ∧ a ^ 4 + b ^ 4 = c ^ 2
-/
def Fermat42 (a b c : Int) : Prop :=
  a != 0 ∧ b != 0 ∧ a ^ 4 + b ^ 4 = c ^ 2

namespace Fermat42

/--
theorem `comm` / 定理 `comm`

English:
theorem comm
  given: {a b c : Int}
  statement: Fermat42 a b c ↔ Fermat42 b a c
  proof: by
  delta Fermat42
  rw [add_comm]
  tauto

中文:
定理 comm
  条件: {a b c : 整数}
  结论: Fermat42 a b c ↔ Fermat42 b a c
  证明: by
  delta Fermat42
  rw [add_comm]
  tauto

Depends on / 依赖: Fermat42, add_comm
-/
theorem comm {a b c : Int} : Fermat42 a b c ↔ Fermat42 b a c := by
  delta Fermat42
  rw [add_comm]
  tauto

/--
theorem `mul` / 定理 `mul`

English:
theorem mul
  given: {a b c k : Int} (hk0 : k != 0)
  proof: by
  delta Fermat42
  constructor
  · grind [mul_eq_zero]
  · intro f42
    constructor
    · exact right_ne_zero_of_mul f42.1
    constructor
    · exact right_ne_zero_of_mul f42.2.1
    apply (mul_right_inj' (pow_ne_zero 4 hk0)).mp
    linear_combination f42.2.2

中文:
定理 mul
  条件: {a b c k : 整数} (hk0 : k != 0)
  证明: by
  delta Fermat42
  constructor
  · grind [mul_eq_zero]
  · intro f42
    constructor
    · exact right_ne_zero_of_mul f42.1
    constructor
    · exact right_ne_zero_of_mul f42.2.1
    apply (mul_right_inj' (pow_ne_zero 4 hk0)).mp
    linear_combination f42.2.2

Depends on / 依赖: Fermat42, linear_combination, mul_eq_zero, mul_right_inj, pow_ne_zero, right_ne_zero_of_mul
-/
theorem mul {a b c k : Int} (hk0 : k != 0) :
    Fermat42 a b c ↔ Fermat42 (k * a) (k * b) (k ^ 2 * c) := by
  delta Fermat42
  constructor
  · grind [mul_eq_zero]
  · intro f42
    constructor
    · exact right_ne_zero_of_mul f42.1
    constructor
    · exact right_ne_zero_of_mul f42.2.1
    apply (mul_right_inj' (pow_ne_zero 4 hk0)).mp
    linear_combination f42.2.2

/--
theorem `ne_zero` / 定理 `ne_zero`

English:
theorem ne_zero
  given: {a b c : Int} (h : Fermat42 a b c)
  statement: c != 0
  proof: by
  apply ne_zero_pow two_ne_zero _; apply ne_of_gt
  rw [← h.2.2]; rw [(by ring : a ^ 4 + b ^ 4 = (a ^ 2) ^ 2 + (b ^ 2) ^ 2)]
  exact
    add_pos (sq_pos_of_ne_zero (pow_ne_zero 2 h.1)) (sq_pos_of_ne_zero (pow_ne_zero 2 h.2.1))

中文:
定理 ne_zero
  条件: {a b c : 整数} (h : Fermat42 a b c)
  结论: c != 0
  证明: by
  apply ne_zero_pow two_ne_zero _; apply ne_of_gt
  rw [← h.2.2]; rw [(by ring : a ^ 4 + b ^ 4 = (a ^ 2) ^ 2 + (b ^ 2) ^ 2)]
  exact
    add_pos (sq_pos_of_ne_zero (pow_ne_zero 2 h.1)) (sq_pos_of_ne_zero (pow_ne_zero 2 h.2.1))

Depends on / 依赖: add_pos, ne_of_gt, ne_zero_pow, pow_ne_zero, sq_pos_of_ne_zero, two_ne_zero
-/
theorem ne_zero {a b c : Int} (h : Fermat42 a b c) : c != 0 := by
  apply ne_zero_pow two_ne_zero _; apply ne_of_gt
  rw [← h.2.2]; rw [(by ring : a ^ 4 + b ^ 4 = (a ^ 2) ^ 2 + (b ^ 2) ^ 2)]
  exact
    add_pos (sq_pos_of_ne_zero (pow_ne_zero 2 h.1)) (sq_pos_of_ne_zero (pow_ne_zero 2 h.2.1))

/--
Definition of `Minimal` / `Minimal` 的定义

English:
definition Minimal
  signature: (a b c : Int)
  body: Fermat42 a b c ∧ forall a1 b1 c1 : Int, Fermat42 a1 b1 c1 -> Int.natAbs c <= Int.natAbs c1

中文:
定义 极小
  签名: (a b c : 整数)
  定义体: Fermat42 a b c ∧ forall a1 b1 c1 : Int, Fermat42 a1 b1 c1 -> Int.natAbs c <= Int.natAbs c1

Depends on / 依赖: Fermat42, Int.natAbs, natAbs
-/
def Minimal (a b c : Int) : Prop :=
  Fermat42 a b c ∧ forall a1 b1 c1 : Int, Fermat42 a1 b1 c1 -> Int.natAbs c <= Int.natAbs c1

/--
theorem `exists_minimal` / 定理 `exists_minimal`

English:
theorem exists_minimal
  given: {a b c : Int} (h : Fermat42 a b c)
  statement: exists a0 b0 c0, Minimal a0 b0 c0
  proof: by
  classical
  let S : Set Nat := { n | exists s : Int × Int × Int, Fermat42 s.1 s.2.1 s.2.2 ∧ n = Int.natAbs s.2.2 }
  have S_nonempty : S.Nonempty := by
    use Int.natAbs c
    rw [Set.mem_ofPred_eq]
    use ⟨a, ⟨b, c⟩⟩
  let m : Nat := Nat.find S_nonempty
  have m_mem : m in S := Nat.find_spec S_nonempty
  rcases m_mem with ⟨s0, hs0, hs1⟩
  use s0.1, s0.2.1, s0.2.2, hs0
  intro a1 b1 c1 h1
  rw [← hs1]
  apply Nat.find_min'
  use ⟨a1, ⟨b1, c1⟩⟩

中文:
定理 存在_minimal
  条件: {a b c : 整数} (h : Fermat42 a b c)
  结论: 存在 a0 b0 c0, 极小 a0 b0 c0
  证明: by
  classical
  let S : Set Nat := { n | exists s : Int × Int × Int, Fermat42 s.1 s.2.1 s.2.2 ∧ n = Int.natAbs s.2.2 }
  have S_nonempty : S.Nonempty := by
    use Int.natAbs c
    rw [Set.mem_ofPred_eq]
    use ⟨a, ⟨b, c⟩⟩
  let m : Nat := Nat.find S_nonempty
  have m_mem : m in S := Nat.find_spec S_nonempty
  rcases m_mem with ⟨s0, hs0, hs1⟩
  use s0.1, s0.2.1, s0.2.2, hs0
  intro a1 b1 c1 h1
  rw [← hs1]
  apply Nat.find_min'
  use ⟨a1, ⟨b1, c1⟩⟩

Depends on / 依赖: Fermat42, Int.natAbs, Nat.find, Nat.find_min, Nat.find_spec, Nonempty, S.Nonempty, S_nonempty, Set.mem_ofPred_eq, classical, find_min, find_spec, m_mem, mem_ofPred_eq, natAbs
-/
theorem exists_minimal {a b c : Int} (h : Fermat42 a b c) : exists a0 b0 c0, Minimal a0 b0 c0 := by
  classical
  let S : Set Nat := { n | exists s : Int × Int × Int, Fermat42 s.1 s.2.1 s.2.2 ∧ n = Int.natAbs s.2.2 }
  have S_nonempty : S.Nonempty := by
    use Int.natAbs c
    rw [Set.mem_ofPred_eq]
    use ⟨a, ⟨b, c⟩⟩
  let m : Nat := Nat.find S_nonempty
  have m_mem : m in S := Nat.find_spec S_nonempty
  rcases m_mem with ⟨s0, hs0, hs1⟩
  use s0.1, s0.2.1, s0.2.2, hs0
  intro a1 b1 c1 h1
  rw [← hs1]
  apply Nat.find_min'
  use ⟨a1, ⟨b1, c1⟩⟩

/--
theorem `coprime_of_minimal` / 定理 `coprime_of_minimal`

English:
theorem coprime_of_minimal
  given: {a b c : Int} (h : Minimal a b c)
  statement: IsCoprime a b
  proof: by
  apply Int.isCoprime_iff_gcd_eq_one.mpr
  by_contra hab
  obtain ⟨p, hp, hpa, hpb⟩ := Nat.Prime.not_coprime_iff_dvd.mp hab
  obtain ⟨a1, rfl⟩ := Int.natCast_dvd.mpr hpa
  obtain ⟨b1, rfl⟩ := Int.natCast_dvd.mpr hpb
  have hpc : (p : Int) ^ 2 ∣ c := by
    rw [← Int.pow_dvd_pow_iff two_ne_zero]; rw [← h.1.2.2]
    apply Dvd.intro (a1 ^ 4 + b1 ^ 4)
    ring
  obtain ⟨c1, rfl⟩ := hpc
  have hf : Fermat42 a1 b1 c1 :=
    (Fermat42.mul (Int.natCast_ne_zero.mpr (Nat.Prime.ne_zero hp))).mpr h.1
  apply Nat.le_lt_asymm (h.2 _ _ _ hf)
  rw [Int.natAbs_mul]; rw [lt_mul_iff_one_lt_left]; rw [Int.natAbs_pow]; rw [Int.natAbs_natCast]
  · exact Nat.one_lt_pow two_ne_zero (Nat.Prime.one_lt hp)
  · exact Nat.pos_of_ne_zero (Int.natAbs_ne_zero.2 (ne_zero hf))

中文:
定理 coprime_of_minimal
  条件: {a b c : 整数} (h : 极小 a b c)
  结论: IsCoprime a b
  证明: by
  apply Int.isCoprime_iff_gcd_eq_one.mpr
  by_contra hab
  obtain ⟨p, hp, hpa, hpb⟩ := Nat.Prime.not_coprime_iff_dvd.mp hab
  obtain ⟨a1, rfl⟩ := Int.natCast_dvd.mpr hpa
  obtain ⟨b1, rfl⟩ := Int.natCast_dvd.mpr hpb
  have hpc : (p : Int) ^ 2 ∣ c := by
    rw [← Int.pow_dvd_pow_iff two_ne_zero]; rw [← h.1.2.2]
    apply Dvd.intro (a1 ^ 4 + b1 ^ 4)
    ring
  obtain ⟨c1, rfl⟩ := hpc
  have hf : Fermat42 a1 b1 c1 :=
    (Fermat42.mul (Int.natCast_ne_zero.mpr (Nat.Prime.ne_zero hp))).mpr h.1
  apply Nat.le_lt_asymm (h.2 _ _ _ hf)
  rw [Int.natAbs_mul]; rw [lt_mul_iff_one_lt_left]; rw [Int.natAbs_pow]; rw [Int.natAbs_natCast]
  · exact Nat.one_lt_pow two_ne_zero (Nat.Prime.one_lt hp)
  · exact Nat.pos_of_ne_zero (Int.natAbs_ne_zero.2 (ne_zero hf))

Depends on / 依赖: Dvd.intro, Equiv.Set.univ, Fermat42, Fermat42.mul, Int.isCoprime_iff_gcd_eq_one.mpr, Int.natCast_dvd.mpr, Int.natCast_ne_zero.mpr, Int.pow_dvd_pow_iff, Nat.Prime.ne_zero, Nat.Prime.not_coprime_iff_dvd.mp, Nat.le_lt_asymm, isCoprime_iff_gcd_eq_one, le_lt_asymm, natCast_dvd, natCast_ne_zero, ne_zero, not_coprime_iff_dvd, pow_dvd_pow_iff, two_ne_zero
-/
theorem coprime_of_minimal {a b c : Int} (h : Minimal a b c) : IsCoprime a b := by
  apply Int.isCoprime_iff_gcd_eq_one.mpr
  by_contra hab
  obtain ⟨p, hp, hpa, hpb⟩ := Nat.Prime.not_coprime_iff_dvd.mp hab
  obtain ⟨a1, rfl⟩ := Int.natCast_dvd.mpr hpa
  obtain ⟨b1, rfl⟩ := Int.natCast_dvd.mpr hpb
  have hpc : (p : Int) ^ 2 ∣ c := by
    rw [← Int.pow_dvd_pow_iff two_ne_zero]; rw [← h.1.2.2]
    apply Dvd.intro (a1 ^ 4 + b1 ^ 4)
    ring
  obtain ⟨c1, rfl⟩ := hpc
  have hf : Fermat42 a1 b1 c1 :=
    (Fermat42.mul (Int.natCast_ne_zero.mpr (Nat.Prime.ne_zero hp))).mpr h.1
  apply Nat.le_lt_asymm (h.2 _ _ _ hf)
  rw [Int.natAbs_mul]; rw [lt_mul_iff_one_lt_left]; rw [Int.natAbs_pow]; rw [Int.natAbs_natCast]
  · exact Nat.one_lt_pow two_ne_zero (Nat.Prime.one_lt hp)
  · exact Nat.pos_of_ne_zero (Int.natAbs_ne_zero.2 (ne_zero hf))

/--
theorem `minimal_comm` / 定理 `minimal_comm`

English:
theorem minimal_comm
  given: {a b c : Int}
  statement: Minimal a b c -> Minimal b a c
  proof: fun ⟨h1, h2⟩ =>
  ⟨Fermat42.comm.mp h1, h2⟩

中文:
定理 minimal_comm
  条件: {a b c : 整数}
  结论: 极小 a b c -> 极小 b a c
  证明: fun ⟨h1, h2⟩ =>
  ⟨Fermat42.comm.mp h1, h2⟩
-/
theorem minimal_comm {a b c : Int} : Minimal a b c -> Minimal b a c := fun ⟨h1, h2⟩ =>
  ⟨Fermat42.comm.mp h1, h2⟩

/--
theorem `neg_of_minimal` / 定理 `neg_of_minimal`

English:
theorem neg_of_minimal
  given: {a b c : Int}
  statement: Minimal a b c -> Minimal a b (-c)
  proof: by
  rintro ⟨⟨ha, hb, heq⟩, h2⟩
  constructor
  · apply And.intro ha (And.intro hb _)
    rw [heq]
    exact (neg_sq c).symm
  rwa [Int.natAbs_neg c]

中文:
定理 neg_of_minimal
  条件: {a b c : 整数}
  结论: 极小 a b c -> 极小 a b (-c)
  证明: by
  rintro ⟨⟨ha, hb, heq⟩, h2⟩
  constructor
  · apply And.intro ha (And.intro hb _)
    rw [heq]
    exact (neg_sq c).symm
  rwa [Int.natAbs_neg c]

Depends on / 依赖: And.intro, Int.natAbs_neg, natAbs_neg, neg_sq
-/
theorem neg_of_minimal {a b c : Int} : Minimal a b c -> Minimal a b (-c) := by
  rintro ⟨⟨ha, hb, heq⟩, h2⟩
  constructor
  · apply And.intro ha (And.intro hb _)
    rw [heq]
    exact (neg_sq c).symm
  rwa [Int.natAbs_neg c]

/--
theorem `exists_odd_minimal` / 定理 `exists_odd_minimal`

English:
theorem exists_odd_minimal
  given: {a b c : Int} (h : Fermat42 a b c)
  proof: by
  obtain ⟨a0, b0, c0, hf⟩ := exists_minimal h
  rcases Int.emod_two_eq_zero_or_one a0 with hap | hap
  · rcases Int.emod_two_eq_zero_or_one b0 with hbp | hbp
    · exfalso
      have h1 : 2 ∣ (Int.gcd a0 b0 : Int) :=
        Int.dvd_coe_gcd (Int.dvd_of_emod_eq_zero hap) (Int.dvd_of_emod_eq_zero hbp)
      rw [Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal hf)] at h1
      revert h1
      decide
    · exact ⟨b0, ⟨a0, ⟨c0, minimal_comm hf, hbp⟩⟩⟩
  exact ⟨a0, ⟨b0, ⟨c0, hf, hap⟩⟩⟩

中文:
定理 存在_odd_minimal
  条件: {a b c : 整数} (h : Fermat42 a b c)
  证明: by
  obtain ⟨a0, b0, c0, hf⟩ := exists_minimal h
  rcases Int.emod_two_eq_zero_or_one a0 with hap | hap
  · rcases Int.emod_two_eq_zero_or_one b0 with hbp | hbp
    · exfalso
      have h1 : 2 ∣ (Int.gcd a0 b0 : Int) :=
        Int.dvd_coe_gcd (Int.dvd_of_emod_eq_zero hap) (Int.dvd_of_emod_eq_zero hbp)
      rw [Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal hf)] at h1
      revert h1
      decide
    · exact ⟨b0, ⟨a0, ⟨c0, minimal_comm hf, hbp⟩⟩⟩
  exact ⟨a0, ⟨b0, ⟨c0, hf, hap⟩⟩⟩

Depends on / 依赖: Int.dvd_coe_gcd, Int.dvd_of_emod_eq_zero, Int.emod_two_eq_zero_or_one, Int.gcd, Int.isCoprime_iff_gcd_eq_one.mp, coprime_of_minimal, dvd_coe_gcd, dvd_of_emod_eq_zero, emod_two_eq_zero_or_one, exists_minimal, isCoprime_iff_gcd_eq_one, minimal_comm, revert
-/
theorem exists_odd_minimal {a b c : Int} (h : Fermat42 a b c) :
    exists a0 b0 c0, Minimal a0 b0 c0 ∧ a0 % 2 = 1 := by
  obtain ⟨a0, b0, c0, hf⟩ := exists_minimal h
  rcases Int.emod_two_eq_zero_or_one a0 with hap | hap
  · rcases Int.emod_two_eq_zero_or_one b0 with hbp | hbp
    · exfalso
      have h1 : 2 ∣ (Int.gcd a0 b0 : Int) :=
        Int.dvd_coe_gcd (Int.dvd_of_emod_eq_zero hap) (Int.dvd_of_emod_eq_zero hbp)
      rw [Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal hf)] at h1
      revert h1
      decide
    · exact ⟨b0, ⟨a0, ⟨c0, minimal_comm hf, hbp⟩⟩⟩
  exact ⟨a0, ⟨b0, ⟨c0, hf, hap⟩⟩⟩

/--
theorem `exists_pos_odd_minimal` / 定理 `exists_pos_odd_minimal`

English:
theorem exists_pos_odd_minimal
  given: {a b c : Int} (h : Fermat42 a b c)
  proof: by
  obtain ⟨a0, b0, c0, hf, hc⟩ := exists_odd_minimal h
  rcases lt_trichotomy 0 c0 with (h1 | h1 | h1)
  · use a0, b0, c0
  · exfalso
    exact ne_zero hf.1 h1.symm
  · use a0, b0, -c0, neg_of_minimal hf, hc
    exact neg_pos.mpr h1

中文:
定理 存在_pos_odd_minimal
  条件: {a b c : 整数} (h : Fermat42 a b c)
  证明: by
  obtain ⟨a0, b0, c0, hf, hc⟩ := exists_odd_minimal h
  rcases lt_trichotomy 0 c0 with (h1 | h1 | h1)
  · use a0, b0, c0
  · exfalso
    exact ne_zero hf.1 h1.symm
  · use a0, b0, -c0, neg_of_minimal hf, hc
    exact neg_pos.mpr h1

Depends on / 依赖: exists_odd_minimal, h1.symm, lt_trichotomy, ne_zero, neg_of_minimal, neg_pos, neg_pos.mpr
-/
theorem exists_pos_odd_minimal {a b c : Int} (h : Fermat42 a b c) :
    exists a0 b0 c0, Minimal a0 b0 c0 ∧ a0 % 2 = 1 ∧ 0 < c0 := by
  obtain ⟨a0, b0, c0, hf, hc⟩ := exists_odd_minimal h
  rcases lt_trichotomy 0 c0 with (h1 | h1 | h1)
  · use a0, b0, c0
  · exfalso
    exact ne_zero hf.1 h1.symm
  · use a0, b0, -c0, neg_of_minimal hf, hc
    exact neg_pos.mpr h1

end Fermat42

/--
theorem `Int.isCoprime_of_sq_sum` / 定理 `Int.isCoprime_of_sq_sum`

English:
theorem Int.isCoprime_of_sq_sum
  given: {r s : Int} (h2 : IsCoprime s r)
  statement: IsCoprime (r ^ 2 + s ^ 2) r
  proof: by
  rw [sq]; rw [sq]
  exact (IsCoprime.mul_left h2 h2).mul_add_left_left r

中文:
定理 整数.isCoprime_of_sq_sum
  条件: {r s : 整数} (h2 : IsCoprime s r)
  结论: IsCoprime (r ^ 2 + s ^ 2) r
  证明: by
  rw [sq]; rw [sq]
  exact (IsCoprime.mul_left h2 h2).mul_add_left_left r

Depends on / 依赖: IsCoprime, IsCoprime.mul_left, mul_add_left_left, mul_left
-/
theorem Int.isCoprime_of_sq_sum {r s : Int} (h2 : IsCoprime s r) : IsCoprime (r ^ 2 + s ^ 2) r := by
  rw [sq]; rw [sq]
  exact (IsCoprime.mul_left h2 h2).mul_add_left_left r

/--
theorem `Int.isCoprime_of_sq_sum'` / 定理 `Int.isCoprime_of_sq_sum'`

English:
theorem Int.isCoprime_of_sq_sum'
  given: {r s : Int} (h : IsCoprime r s)
  proof: by
  apply IsCoprime.mul_right (Int.isCoprime_of_sq_sum (isCoprime_comm.mp h))
  rw [add_comm]; apply Int.isCoprime_of_sq_sum h

中文:
定理 整数.isCoprime_of_sq_sum'
  条件: {r s : 整数} (h : IsCoprime r s)
  证明: by
  apply IsCoprime.mul_right (Int.isCoprime_of_sq_sum (isCoprime_comm.mp h))
  rw [add_comm]; apply Int.isCoprime_of_sq_sum h

Depends on / 依赖: Int.isCoprime_of_sq_sum, IsCoprime, IsCoprime.mul_right, add_comm, isCoprime_comm, isCoprime_comm.mp, isCoprime_of_sq_sum, mul_right
-/
theorem Int.isCoprime_of_sq_sum' {r s : Int} (h : IsCoprime r s) :
    IsCoprime (r ^ 2 + s ^ 2) (r * s) := by
  apply IsCoprime.mul_right (Int.isCoprime_of_sq_sum (isCoprime_comm.mp h))
  rw [add_comm]; apply Int.isCoprime_of_sq_sum h

namespace Fermat42

-- If we have a solution to a ^ 4 + b ^ 4 = c ^ 2, we can construct a smaller one. This
-- implies there can't be a smallest solution.
/--
theorem `not_minimal` / 定理 `not_minimal`

English:
theorem not_minimal
  given: {a b c : Int} (h : Minimal a b c) (ha2 : a % 2 = 1) (hc : 0 < c)
  statement: False
  proof: by
  -- Use the fact that a ^ 2, b ^ 2, c form a pythagorean triple to obtain m and n such that
  -- a ^ 2 = m ^ 2 - n ^ 2, b ^ 2 = 2 * m * n and c = m ^ 2 + n ^ 2
  -- first the formula:
  have ht : PythagoreanTriple (a ^ 2) (b ^ 2) c := by
    delta PythagoreanTriple
    linear_combination h.1.2.2
  -- coprime requirement:
  have h2 : Int.gcd (a ^ 2) (b ^ 2) = 1 :=
    Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal h).pow
  -- in order to reduce the possibilities we get from the classification of pythagorean triples
  -- it helps if we know the parity of a ^ 2 (and the sign of c):
  have ha22 : a ^ 2 % 2 = 1 := by
    rw [sq]; rw [Int.mul_emod]; rw [ha2]
    decide
  obtain ⟨m, n, ht1, ht2, ht3, ht4, ht5, ht6⟩ := ht.coprime_classification' h2 ha22 hc
  -- Now a, n, m form a pythagorean triple and so we can obtain r and s such that
  -- a = r ^ 2 - s ^ 2, n = 2 * r * s and m = r ^ 2 + s ^ 2
  -- formula:
  have htt : PythagoreanTriple a n m := by
    delta PythagoreanTriple
    linear_combination ht1
  -- a and n are coprime, because a ^ 2 = m ^ 2 - n ^ 2 and m and n are coprime.
  have h3 : Int.gcd a n = 1 := by
    apply Int.isCoprime_iff_gcd_eq_one.mp
    apply @IsCoprime.of_mul_left_left _ _ _ a
    rw [← sq]; rw [ht1]; rw [(by ring : m ^ 2 - n ^ 2 = m ^ 2 + -n * n)]
    exact (Int.isCoprime_iff_gcd_eq_one.mpr ht4).pow_left.add_mul_right_left (-n)
  -- m is positive because b is non-zero and b ^ 2 = 2 * m * n and we already have 0 ≤ m.
  have hb20 : b ^ 2 != 0 := pow_ne_zero _ h.1.2.1
  have h4 : 0 < m := by
    apply lt_of_le_of_ne ht6
    rintro rfl
    lia
  obtain ⟨r, s, _, htt2, htt3, htt4, htt5, htt6⟩ := htt.coprime_classification' h3 ha2 h4
  -- Now use the fact that (b / 2) ^ 2 = m * r * s, and m, r and s are pairwise coprime to obtain
  -- i, j and k such that m = i ^ 2, r = j ^ 2 and s = k ^ 2.
  -- m and r * s are coprime because m = r ^ 2 + s ^ 2 and r and s are coprime.
  have hcp : Int.gcd m (r * s) = 1 := by
    rw [htt3]
    exact Int.isCoprime_iff_gcd_eq_one.mp
      (Int.isCoprime_of_sq_sum' (Int.isCoprime_iff_gcd_eq_one.mpr htt4))
  -- b is even because b ^ 2 = 2 * m * n.
  have hb2 : 2 ∣ b := by
    apply @Int.Prime.dvd_pow' _ 2 _ Nat.prime_two
    rw [ht2]; rw [mul_assoc]
    exact dvd_mul_right 2 (m * n)
  obtain ⟨b', hb2'⟩ := hb2
  have hs : b' ^ 2 = m * (r * s) := by
    apply (mul_right_inj' (by simp : (4 : Int) != 0)).mp
    linear_combination (-b - 2 * b') * hb2' + ht2 + 2 * m * htt2
  have hrsz : r * s != 0 := by grind
  have h2b0 : b' != 0 := by grind
  obtain ⟨i, hi⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- use m is positive to exclude m = - i ^ 2
  have hi' : ¬m = -i ^ 2 := by
    by_contra h1
    have hit : -i ^ 2 <= 0 := neg_nonpos.mpr (sq_nonneg i)
    rw [← h1] at hit
    apply absurd h4 (not_lt.mpr hit)
  replace hi : m = i ^ 2 := Or.resolve_right hi hi'
  rw [mul_comm] at hs
  rw [Int.gcd_comm] at hcp
  -- obtain d such that r * s = d ^ 2
  obtain ⟨d, hd⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- (b / 2) ^ 2 and m are positive so r * s is positive
  have hd' : ¬r * s = -d ^ 2 := by
    by_contra h1
    rw [h1] at hs
    have h2 : b' ^ 2 <= 0 := by
      rw [hs]; rw [(by ring : -d ^ 2 * m = -(d ^ 2 * m))]
      exact neg_nonpos.mpr ((mul_nonneg_iff_of_pos_right h4).mpr (sq_nonneg d))
    have h2' : 0 <= b' ^ 2 := by apply sq_nonneg b'
    exact absurd (lt_of_le_of_ne h2' (Ne.symm (pow_ne_zero _ h2b0))) (not_lt.mpr h2)
  replace hd : r * s = d ^ 2 := Or.resolve_right hd hd'
  -- r = +/- j ^ 2
  obtain ⟨j, hj⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hj0 : j ≠ 0 := by grind
  rw [mul_comm] at hd
  rw [Int.gcd_comm] at htt4
  -- s = +/- k ^ 2
  obtain ⟨k, hk⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hk0 : k ≠ 0 := by grind
  have hj2 : r ^ 2 = j ^ 4 := by grind
  have hk2 : s ^ 2 = k ^ 4 := by grind
  -- from m = r ^ 2 + s ^ 2 we now get a new solution to a ^ 4 + b ^ 4 = c ^ 2:
  have hh : i ^ 2 = j ^ 4 + k ^ 4 := by grind
  have hn : n ≠ 0 := by grind
  -- and it has a smaller c: from c = m ^ 2 + n ^ 2 we see that m is smaller than c, and i ^ 2 = m.
  have hic : Int.natAbs i < Int.natAbs c := by
    apply Int.ofNat_lt.mp
    rw [← Int.eq_natAbs_of_nonneg (le_of_lt hc)]
    apply lt_of_le_of_lt (Int.natAbs_le_self_sq i)
    rw [← hi]; rw [ht3]
    apply lt_of_le_of_lt (Int.le_self_sq m)
    exact lt_add_of_pos_right (m ^ 2) (sq_pos_of_ne_zero hn)
  have hic' : Int.natAbs c ≤ Int.natAbs i := by
    apply h.2 j k i
    exact ⟨hj0, hk0, hh.symm⟩
  apply absurd (not_le_of_gt hic) (not_not.mpr hic')

中文:
定理 not_minimal
  条件: {a b c : 整数} (h : 极小 a b c) (ha2 : a % 2 = 1) (hc : 0 < c)
  结论: 假
  证明: by
  -- Use the fact that a ^ 2, b ^ 2, c form a pythagorean triple to obtain m and n such that
  -- a ^ 2 = m ^ 2 - n ^ 2, b ^ 2 = 2 * m * n and c = m ^ 2 + n ^ 2
  -- first the formula:
  have ht : PythagoreanTriple (a ^ 2) (b ^ 2) c := by
    delta PythagoreanTriple
    linear_combination h.1.2.2
  -- coprime requirement:
  have h2 : Int.gcd (a ^ 2) (b ^ 2) = 1 :=
    Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal h).pow
  -- in order to reduce the possibilities we get from the classification of pythagorean triples
  -- it helps if we know the parity of a ^ 2 (and the sign of c):
  have ha22 : a ^ 2 % 2 = 1 := by
    rw [sq]; rw [Int.mul_emod]; rw [ha2]
    decide
  obtain ⟨m, n, ht1, ht2, ht3, ht4, ht5, ht6⟩ := ht.coprime_classification' h2 ha22 hc
  -- Now a, n, m form a pythagorean triple and so we can obtain r and s such that
  -- a = r ^ 2 - s ^ 2, n = 2 * r * s and m = r ^ 2 + s ^ 2
  -- formula:
  have htt : PythagoreanTriple a n m := by
    delta PythagoreanTriple
    linear_combination ht1
  -- a and n are coprime, because a ^ 2 = m ^ 2 - n ^ 2 and m and n are coprime.
  have h3 : Int.gcd a n = 1 := by
    apply Int.isCoprime_iff_gcd_eq_one.mp
    apply @IsCoprime.of_mul_left_left _ _ _ a
    rw [← sq]; rw [ht1]; rw [(by ring : m ^ 2 - n ^ 2 = m ^ 2 + -n * n)]
    exact (Int.isCoprime_iff_gcd_eq_one.mpr ht4).pow_left.add_mul_right_left (-n)
  -- m is positive because b is non-zero and b ^ 2 = 2 * m * n and we already have 0 ≤ m.
  have hb20 : b ^ 2 != 0 := pow_ne_zero _ h.1.2.1
  have h4 : 0 < m := by
    apply lt_of_le_of_ne ht6
    rintro rfl
    lia
  obtain ⟨r, s, _, htt2, htt3, htt4, htt5, htt6⟩ := htt.coprime_classification' h3 ha2 h4
  -- Now use the fact that (b / 2) ^ 2 = m * r * s, and m, r and s are pairwise coprime to obtain
  -- i, j and k such that m = i ^ 2, r = j ^ 2 and s = k ^ 2.
  -- m and r * s are coprime because m = r ^ 2 + s ^ 2 and r and s are coprime.
  have hcp : Int.gcd m (r * s) = 1 := by
    rw [htt3]
    exact Int.isCoprime_iff_gcd_eq_one.mp
      (Int.isCoprime_of_sq_sum' (Int.isCoprime_iff_gcd_eq_one.mpr htt4))
  -- b is even because b ^ 2 = 2 * m * n.
  have hb2 : 2 ∣ b := by
    apply @Int.Prime.dvd_pow' _ 2 _ Nat.prime_two
    rw [ht2]; rw [mul_assoc]
    exact dvd_mul_right 2 (m * n)
  obtain ⟨b', hb2'⟩ := hb2
  have hs : b' ^ 2 = m * (r * s) := by
    apply (mul_right_inj' (by simp : (4 : Int) != 0)).mp
    linear_combination (-b - 2 * b') * hb2' + ht2 + 2 * m * htt2
  have hrsz : r * s != 0 := by grind
  have h2b0 : b' != 0 := by grind
  obtain ⟨i, hi⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- use m is positive to exclude m = - i ^ 2
  have hi' : ¬m = -i ^ 2 := by
    by_contra h1
    have hit : -i ^ 2 <= 0 := neg_nonpos.mpr (sq_nonneg i)
    rw [← h1] at hit
    apply absurd h4 (not_lt.mpr hit)
  replace hi : m = i ^ 2 := Or.resolve_right hi hi'
  rw [mul_comm] at hs
  rw [Int.gcd_comm] at hcp
  -- obtain d such that r * s = d ^ 2
  obtain ⟨d, hd⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- (b / 2) ^ 2 and m are positive so r * s is positive
  have hd' : ¬r * s = -d ^ 2 := by
    by_contra h1
    rw [h1] at hs
    have h2 : b' ^ 2 <= 0 := by
      rw [hs]; rw [(by ring : -d ^ 2 * m = -(d ^ 2 * m))]
      exact neg_nonpos.mpr ((mul_nonneg_iff_of_pos_right h4).mpr (sq_nonneg d))
    have h2' : 0 <= b' ^ 2 := by apply sq_nonneg b'
    exact absurd (lt_of_le_of_ne h2' (Ne.symm (pow_ne_zero _ h2b0))) (not_lt.mpr h2)
  replace hd : r * s = d ^ 2 := Or.resolve_right hd hd'
  -- r = +/- j ^ 2
  obtain ⟨j, hj⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hj0 : j ≠ 0 := by grind
  rw [mul_comm] at hd
  rw [Int.gcd_comm] at htt4
  -- s = +/- k ^ 2
  obtain ⟨k, hk⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hk0 : k ≠ 0 := by grind
  have hj2 : r ^ 2 = j ^ 4 := by grind
  have hk2 : s ^ 2 = k ^ 4 := by grind
  -- from m = r ^ 2 + s ^ 2 we now get a new solution to a ^ 4 + b ^ 4 = c ^ 2:
  have hh : i ^ 2 = j ^ 4 + k ^ 4 := by grind
  have hn : n ≠ 0 := by grind
  -- and it has a smaller c: from c = m ^ 2 + n ^ 2 we see that m is smaller than c, and i ^ 2 = m.
  have hic : Int.natAbs i < Int.natAbs c := by
    apply Int.ofNat_lt.mp
    rw [← Int.eq_natAbs_of_nonneg (le_of_lt hc)]
    apply lt_of_le_of_lt (Int.natAbs_le_self_sq i)
    rw [← hi]; rw [ht3]
    apply lt_of_le_of_lt (Int.le_self_sq m)
    exact lt_add_of_pos_right (m ^ 2) (sq_pos_of_ne_zero hn)
  have hic' : Int.natAbs c ≤ Int.natAbs i := by
    apply h.2 j k i
    exact ⟨hj0, hk0, hh.symm⟩
  apply absurd (not_le_of_gt hic) (not_not.mpr hic')
-/
theorem not_minimal {a b c : Int} (h : Minimal a b c) (ha2 : a % 2 = 1) (hc : 0 < c) : False := by
  -- Use the fact that a ^ 2, b ^ 2, c form a pythagorean triple to obtain m and n such that
  -- a ^ 2 = m ^ 2 - n ^ 2, b ^ 2 = 2 * m * n and c = m ^ 2 + n ^ 2
  -- first the formula:
  have ht : PythagoreanTriple (a ^ 2) (b ^ 2) c := by
    delta PythagoreanTriple
    linear_combination h.1.2.2
  -- coprime requirement:
  have h2 : Int.gcd (a ^ 2) (b ^ 2) = 1 :=
    Int.isCoprime_iff_gcd_eq_one.mp (coprime_of_minimal h).pow
  -- in order to reduce the possibilities we get from the classification of pythagorean triples
  -- it helps if we know the parity of a ^ 2 (and the sign of c):
  have ha22 : a ^ 2 % 2 = 1 := by
    rw [sq]; rw [Int.mul_emod]; rw [ha2]
    decide
  obtain ⟨m, n, ht1, ht2, ht3, ht4, ht5, ht6⟩ := ht.coprime_classification' h2 ha22 hc
  -- Now a, n, m form a pythagorean triple and so we can obtain r and s such that
  -- a = r ^ 2 - s ^ 2, n = 2 * r * s and m = r ^ 2 + s ^ 2
  -- formula:
  have htt : PythagoreanTriple a n m := by
    delta PythagoreanTriple
    linear_combination ht1
  -- a and n are coprime, because a ^ 2 = m ^ 2 - n ^ 2 and m and n are coprime.
  have h3 : Int.gcd a n = 1 := by
    apply Int.isCoprime_iff_gcd_eq_one.mp
    apply @IsCoprime.of_mul_left_left _ _ _ a
    rw [← sq]; rw [ht1]; rw [(by ring : m ^ 2 - n ^ 2 = m ^ 2 + -n * n)]
    exact (Int.isCoprime_iff_gcd_eq_one.mpr ht4).pow_left.add_mul_right_left (-n)
  -- m is positive because b is non-zero and b ^ 2 = 2 * m * n and we already have 0 ≤ m.
  have hb20 : b ^ 2 != 0 := pow_ne_zero _ h.1.2.1
  have h4 : 0 < m := by
    apply lt_of_le_of_ne ht6
    rintro rfl
    lia
  obtain ⟨r, s, _, htt2, htt3, htt4, htt5, htt6⟩ := htt.coprime_classification' h3 ha2 h4
  -- Now use the fact that (b / 2) ^ 2 = m * r * s, and m, r and s are pairwise coprime to obtain
  -- i, j and k such that m = i ^ 2, r = j ^ 2 and s = k ^ 2.
  -- m and r * s are coprime because m = r ^ 2 + s ^ 2 and r and s are coprime.
  have hcp : Int.gcd m (r * s) = 1 := by
    rw [htt3]
    exact Int.isCoprime_iff_gcd_eq_one.mp
      (Int.isCoprime_of_sq_sum' (Int.isCoprime_iff_gcd_eq_one.mpr htt4))
  -- b is even because b ^ 2 = 2 * m * n.
  have hb2 : 2 ∣ b := by
    apply @Int.Prime.dvd_pow' _ 2 _ Nat.prime_two
    rw [ht2]; rw [mul_assoc]
    exact dvd_mul_right 2 (m * n)
  obtain ⟨b', hb2'⟩ := hb2
  have hs : b' ^ 2 = m * (r * s) := by
    apply (mul_right_inj' (by simp : (4 : Int) != 0)).mp
    linear_combination (-b - 2 * b') * hb2' + ht2 + 2 * m * htt2
  have hrsz : r * s != 0 := by grind
  have h2b0 : b' != 0 := by grind
  obtain ⟨i, hi⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- use m is positive to exclude m = - i ^ 2
  have hi' : ¬m = -i ^ 2 := by
    by_contra h1
    have hit : -i ^ 2 <= 0 := neg_nonpos.mpr (sq_nonneg i)
    rw [← h1] at hit
    apply absurd h4 (not_lt.mpr hit)
  replace hi : m = i ^ 2 := Or.resolve_right hi hi'
  rw [mul_comm] at hs
  rw [Int.gcd_comm] at hcp
  -- obtain d such that r * s = d ^ 2
  obtain ⟨d, hd⟩ := Int.sq_of_gcd_eq_one hcp hs.symm
  -- (b / 2) ^ 2 and m are positive so r * s is positive
  have hd' : ¬r * s = -d ^ 2 := by
    by_contra h1
    rw [h1] at hs
    have h2 : b' ^ 2 <= 0 := by
      rw [hs]; rw [(by ring : -d ^ 2 * m = -(d ^ 2 * m))]
      exact neg_nonpos.mpr ((mul_nonneg_iff_of_pos_right h4).mpr (sq_nonneg d))
    have h2' : 0 <= b' ^ 2 := by apply sq_nonneg b'
    exact absurd (lt_of_le_of_ne h2' (Ne.symm (pow_ne_zero _ h2b0))) (not_lt.mpr h2)
  replace hd : r * s = d ^ 2 := Or.resolve_right hd hd'
  -- r = +/- j ^ 2
  obtain ⟨j, hj⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hj0 : j ≠ 0 := by grind
  rw [mul_comm] at hd
  rw [Int.gcd_comm] at htt4
  -- s = +/- k ^ 2
  obtain ⟨k, hk⟩ := Int.sq_of_gcd_eq_one htt4 hd
  have hk0 : k ≠ 0 := by grind
  have hj2 : r ^ 2 = j ^ 4 := by grind
  have hk2 : s ^ 2 = k ^ 4 := by grind
  -- from m = r ^ 2 + s ^ 2 we now get a new solution to a ^ 4 + b ^ 4 = c ^ 2:
  have hh : i ^ 2 = j ^ 4 + k ^ 4 := by grind
  have hn : n ≠ 0 := by grind
  -- and it has a smaller c: from c = m ^ 2 + n ^ 2 we see that m is smaller than c, and i ^ 2 = m.
  have hic : Int.natAbs i < Int.natAbs c := by
    apply Int.ofNat_lt.mp
    rw [← Int.eq_natAbs_of_nonneg (le_of_lt hc)]
    apply lt_of_le_of_lt (Int.natAbs_le_self_sq i)
    rw [← hi]; rw [ht3]
    apply lt_of_le_of_lt (Int.le_self_sq m)
    exact lt_add_of_pos_right (m ^ 2) (sq_pos_of_ne_zero hn)
  have hic' : Int.natAbs c ≤ Int.natAbs i := by
    apply h.2 j k i
    exact ⟨hj0, hk0, hh.symm⟩
  apply absurd (not_le_of_gt hic) (not_not.mpr hic')

end Fermat42

/--
theorem `not_fermat_42` / 定理 `not_fermat_42`

English:
theorem not_fermat_42
  given: {a b c : ℤ} (ha : a ≠ 0) (hb : b ≠ 0)
  statement: a ^ 4 + b ^ 4 ≠ c ^ 2
  proof: by
  intro h
  obtain ⟨a0, b0, c0, ⟨hf, h2, hp⟩⟩ :=
    Fermat42.exists_pos_odd_minimal (And.intro ha (And.intro hb h))
  apply Fermat42.not_minimal hf h2 hp

中文:
定理 not_fermat_42
  条件: {a b c : ℤ} (ha : a ≠ 0) (hb : b ≠ 0)
  结论: a ^ 4 + b ^ 4 ≠ c ^ 2
  证明: by
  intro h
  obtain ⟨a0, b0, c0, ⟨hf, h2, hp⟩⟩ :=
    Fermat42.exists_pos_odd_minimal (And.intro ha (And.intro hb h))
  apply Fermat42.not_minimal hf h2 hp

Depends on / 依赖: And.intro, Fermat42, Fermat42.exists_pos_odd_minimal, Fermat42.not_minimal, exists_pos_odd_minimal, not_minimal
-/
theorem not_fermat_42 {a b c : ℤ} (ha : a ≠ 0) (hb : b ≠ 0) : a ^ 4 + b ^ 4 ≠ c ^ 2 := by
  intro h
  obtain ⟨a0, b0, c0, ⟨hf, h2, hp⟩⟩ :=
    Fermat42.exists_pos_odd_minimal (And.intro ha (And.intro hb h))
  apply Fermat42.not_minimal hf h2 hp

/--
theorem `fermatLastTheoremFour` / 定理 `fermatLastTheoremFour`

English:
theorem fermatLastTheoremFour
  statement: FermatLastTheoremFor 4
  proof: by
  rw [fermatLastTheoremFor_iff_int]
  intro a b c ha hb _ heq
  apply @not_fermat_42 _ _ (c ^ 2) ha hb
  rw [heq]; ring

中文:
定理 fermatLastTheoremFour
  结论: FermatLastTheoremFor 4
  证明: by
  rw [fermatLastTheoremFor_iff_int]
  intro a b c ha hb _ heq
  apply @not_fermat_42 _ _ (c ^ 2) ha hb
  rw [heq]; ring

Depends on / 依赖: fermatLastTheoremFor_iff_int, not_fermat_42
-/
theorem fermatLastTheoremFour : FermatLastTheoremFor 4 := by
  rw [fermatLastTheoremFor_iff_int]
  intro a b c ha hb _ heq
  apply @not_fermat_42 _ _ (c ^ 2) ha hb
  rw [heq]; ring

/--
theorem `FermatLastTheorem.of_odd_primes` / 定理 `FermatLastTheorem.of_odd_primes`

English:
theorem FermatLastTheorem.of_odd_primes
  proof: by
  intro n h
  obtain hdvd | ⟨p, hpprime, hdvd, hpodd⟩ := Nat.four_dvd_or_exists_odd_prime_and_dvd_of_two_lt h
    <;> apply FermatLastTheoremWith.mono hdvd
  · exact fermatLastTheoremFour
  · exact hprimes p hpprime hpodd

中文:
定理 FermatLastTheorem.of_odd_primes
  证明: by
  intro n h
  obtain hdvd | ⟨p, hpprime, hdvd, hpodd⟩ := Nat.four_dvd_or_exists_odd_prime_and_dvd_of_two_lt h
    <;> apply FermatLastTheoremWith.mono hdvd
  · exact fermatLastTheoremFour
  · exact hprimes p hpprime hpodd

Depends on / 依赖: FermatLastTheoremWith, FermatLastTheoremWith.mono, Nat.four_dvd_or_exists_odd_prime_and_dvd_of_two_lt, fermatLastTheoremFour, four_dvd_or_exists_odd_prime_and_dvd_of_two_lt, hpprime, hprimes
-/
theorem FermatLastTheorem.of_odd_primes
    (hprimes : forall p : Nat, Nat.Prime p -> Odd p -> FermatLastTheoremFor p) : FermatLastTheorem := by
  intro n h
  obtain hdvd | ⟨p, hpprime, hdvd, hpodd⟩ := Nat.four_dvd_or_exists_odd_prime_and_dvd_of_two_lt h
    <;> apply FermatLastTheoremWith.mono hdvd
  · exact fermatLastTheoremFour
  · exact hprimes p hpprime hpodd
