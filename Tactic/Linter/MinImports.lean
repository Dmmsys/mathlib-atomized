...
-/
/-
**Mathlib.Linter.ImportState** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Linter`。
形式化陈述：ImportState where /-- The transitive closure of the import graph of the cu
rrent file. The value is `none` only at initialization time, as the linter immed
iately sets it to its value for the current file. -/ transClosure : Option (Name
Map NameSet)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`ImportState` is the structure keeping track of the data that the `minImports` l
inter uses.
* `transClosure` is the import graph of the current file.
* `minImports` is the `NameSet` of minimal imports to build the file up to the c
urrent command.
* `importSize` is the number of transitive imports to build the file up to the c
urrent command.
-/
...
-/
/-
**Mathlib.Linter.MinImports.importsBelow** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Lint
er.MinImports`。
形式化陈述：importsBelow (tc : NameMap NameSet) (ms : NameSet) : NameSet
参数：tc : NameMap NameSet；ms : NameSet。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`importsBelow tc ms` takes as input a `NameMap NameSet` `tc`, representing the
`transitiveClosure` of the imports of the current module, and a `NameSet` of mod
ule names `ms`.
It returns the modules that are transitively imported by `ms`, using the data in
 `tc`.
-/
...
@[inherit_doc Mathlib.Linter.linter.minImports]
/-
**Mathlib.Linter.MinImports.minImportsLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Linter.MinImports`。
形式化陈述：minImportsLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
