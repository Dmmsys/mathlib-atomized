...
/--
theorem `Equicontinuous.comap_uniformFun_eq` / 定理 `Equicontinuous.comap_uniformFun_eq`

English:
theorem Equicontinuous.comap_uniformFun_eq
  given: [CompactSpace X] (F_eqcont : Equicontinuous F)
  proof: by
...
theorem Equicontinuous.comap_uniformFun_eq [CompactSpace X] (F_eqcont : Equicontinuous F) :
  ...
  let Ω x : Set X := {y | forall i, (F i x, F i y) in V}
  ...
  have : (⋂ a in A, {ij : ι × ι | (F ij.1 a, F ij.2 a) in V}) subseteq
...
/--
lemma `Equicontinuous.isUniformInducing_uniformFun_iff_pi` / 引理 `Equicontinuous.isUniformInducing_uniformFun_iff_pi`

English:
lemma Equicontinuous.isUniformInducing_uniformFun_iff_pi
  statement: [UniformSpace ι] [CompactSpace X]
  proof: by
...
Depends on / 依赖: F_eqcont, F_eqcont.comap_uniformFun_eq, comap_uniformFun_eq, isUniformInducing_iff_uniformSpace
-/
...
/--
lemma `Equicontinuous.inducing_uniformFun_iff_pi` / 引理 `Equicontinuous.inducing_uniformFun_iff_pi`

English:
lemma Equicontinuous.inducing_uniformFun_iff_pi
  statement: [TopologicalSpace ι] [CompactSpace X]
  proof: by
...
Depends on / 依赖: F_eqcont, F_eqcont.comap_uniformFun_eq, Pi.uniformSpace, UniformFun, UniformFun.uniformSpace, comap_uniformFun_eq, isInducing_iff, toTopologicalSpace, uniformSpace
-/
...
/--
theorem `Equicontinuous.tendsto_uniformFun_iff_pi` / 定理 `Equicontinuous.tendsto_uniformFun_iff_pi`

English:
theorem Equicontinuous.tendsto_uniformFun_iff_pi
...
.comp H · exact UniformFun.uniformContinuous_toFun.continuous.tendsto _
  -- To prove it, assume that `F` tend
...
.comp H · exact UniformFun.uniformContinuous_toFun.continuous.tendsto _
  ...
  · set S : Set (X -> α) := closure (range F)
    ...
    have f_mem : f in S := mem_closure_of_tendsto H range_mem_map
...
/--
theorem `EquicontinuousOn.comap_uniformOnFun_eq` / 定理 `EquicontinuousOn.comap_uniformOnFun_eq`

English:
theorem EquicontinuousOn.comap_uniformOnFun_eq
  statement: {𝔖 : Set (Set X)} (𝔖_compact : forall K in 𝔖, IsCompact K)
  proof: by
...
定理 EquicontinuousOn.comap_uniformOnFun_eq
  ...
  -- `K.domRestrict ∘ F : ι → (K →ᵤ α)` 
-/
theorem EquicontinuousOn.comap_uniformOnFun_eq {𝔖 : Set (Set X)} (𝔖_compact : forall K in 𝔖, IsCompact K)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K) :
  ...
  have H1 : (UniformOnFun.uniformSpace X α 𝔖).comap F =
      ⨅ (K in 𝔖), (UniformFun.uniformSpace _ _).comap (K.domRestrict ∘ F) := by
  ...
  have H2 : (Pi.uniformSpace _).comap ((⋃₀ 𝔖).domRestrict ∘ F) =
      ⨅ (K in 𝔖), (Pi.uniformSpace _).comap (K.domRestrict ∘ F) := by
  ...
  have H3 : forall K in 𝔖, (UniformFun.uniformSpace K α).comap (K.domRestrict ∘ F) =
...
/--
lemma `EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi'` / 引理 `EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi'`

English:
lemma EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi'
  statement: [UniformSpace ι]
  proof: by
...
Depends on / 依赖: EquicontinuousOn, EquicontinuousOn.comap_uniformOnFun_eq, F_eqcont, comap_uniformOnFun_eq, isUniformInducing_iff_uniformSpace
-/
lemma EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi' [UniformSpace ι]
    {𝔖 : Set (Set X)} (𝔖_compact : forall K in 𝔖, IsCompact K)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K) :
...
-/
lemma EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi [UniformSpace ι]
    {𝔖 : Set (Set X)} (𝔖_covers : ⋃₀ 𝔖 = univ) (𝔖_compact : forall K in 𝔖, IsCompact K)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K) :
    IsUniformInducing (UniformOnFun.ofFun 𝔖 ∘ F) ↔
  ...
  rw [EquicontinuousOn.isUniformInducing_uniformOnFun_iff_pi' 𝔖_compact F_eqcont]; rw [show domRestrict (⋃₀ 𝔖) ∘ F = φ.symm ∘ F by rfl]
  exact ⟨fun H => φ.isUniformInducing.comp H, fun H => φ.symm.isUniformInducing.comp H⟩

/--
lemma `EquicontinuousOn.inducing_uniformOnFun_iff_pi'` / 引理 `EquicontinuousOn.inducing_uniformOnFun_iff_pi'`

English:
lemma EquicontinuousOn.inducing_uniformOnFun_iff_pi'
...
Depends on / 依赖: EquicontinuousOn, EquicontinuousOn.comap_uniformOnFun_eq, F_eqcont, Pi.uniformSpace, UniformOnFun, UniformOnFun.uniformSpace, comap_uniformOnFun_eq, domRestrict, isInducing_iff, toTopologicalSpace, uniformSpace
-/
...
/--
lemma `EquicontinuousOn.isInducing_uniformOnFun_iff_pi` / 引理 `EquicontinuousOn.isInducing_uniformOnFun_iff_pi`

English:
lemma EquicontinuousOn.isInducing_uniformOnFun_iff_pi
  statement: [TopologicalSpace ι]
  proof: by
  rw [eq_univ_iff_forall] at 𝔖_covers
  -- This obviously follows from the previous lemma, we formalize it by going through the
...
lemma EquicontinuousOn.isInducing_uniformOnFun_iff_pi [TopologicalSpace ι]
  ...
  let φ : ((⋃₀ 𝔖) -> α) ≃ₜ (X -> α) := Homeomorph.piCongrLeft (Y := fun _ => α)
    (Equiv.subtypeUnivEquiv 𝔖_covers)
  rw [EquicontinuousOn.inducing_uniformOnFun_iff_pi' 𝔖_compact F_eqcont]; rw [show domRestrict (⋃₀ 𝔖) ∘ F = φ.symm ∘ F by rfl]
...
/--
theorem `EquicontinuousOn.tendsto_uniformOnFun_iff_pi'` / 定理 `EquicontinuousOn.tendsto_uniformOnFun_iff_pi'`
...
中文:
定理 EquicontinuousOn.tendsto_uniformOnFun_iff_pi'
  证明: by
  -- Recall that the uniform structure on `X →ᵤ[𝔖] α` is the one induced by all the maps
  -- `K.domRestrict : (X →ᵤ[𝔖] α) → (K →ᵤ α)` for `K ∈ 𝔖`.
  -- Similarly, the uniform structure on `X → α` induced by the map
...
theorem EquicontinuousOn.tendsto_uniformOnFun_iff_pi'
    {𝔖 : Set (Set X)} (𝔖_compact : forall K in 𝔖, IsCompact K)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K) (ℱ : Filter ι) (f : X -> α) :
  ...
  rw [← Filter.tendsto_comap_iff (g := (⋃₀ 𝔖).domRestrict)]; rw [← nhds_induced]
...
theorem EquicontinuousOn.tendsto_uniformOnFun_iff_pi
  ...
  let φ : ((⋃₀ 𝔖) -> α) ≃ₜ (X -> α) := Homeomorph.piCongrLeft (Y := fun _ => α)
    (Equiv.subtypeUnivEquiv 𝔖_covers)
  rw [EquicontinuousOn.tendsto_uniformOnFun_iff_pi' 𝔖_compact F_eqcont]; rw [show domRestrict (⋃₀ 𝔖) ∘ F = φ.symm ∘ F by rfl]; rw [show domRe
...
Depends on / 依赖: EquicontinuousOn, EquicontinuousOn.tendsto_uniformOnFun_iff_pi, Equiv.subtypeUnivEquiv, F_eqcont, Homeomorph, Homeomorph.piCongrLeft, domRestrict, eq_univ_iff_forall, isInducing, piCongrLeft, subtypeUnivEquiv, symm.isInducing.tendsto_nhds_iff, tendsto_nhds_iff, tendsto_uniformOnFun_iff_pi
-/
theorem EquicontinuousOn.tendsto_uniformOnFun_iff_pi
    {𝔖 : Set (Set X)} (𝔖_compact : forall K in 𝔖, IsCompact K) (𝔖_covers : ⋃₀ 𝔖 = univ)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K) (ℱ : Filter ι) (f : X -> α) :
    Tendsto (UniformOnFun.ofFun 𝔖 ∘ F) ℱ (𝓝 <| UniformOnFun.ofFun 𝔖 f) ↔
    Tendsto F ℱ (𝓝 f) := by
  rw [eq_univ_iff_forall] at 𝔖_covers
  let φ : ((⋃₀ 𝔖) -> α) ≃ₜ (X -> α) := Homeomorph.piCongrLeft (Y := fun _ => α)
    (Equiv.subtypeUnivEquiv 𝔖_covers)
  rw [EquicontinuousOn.tendsto_uniformOnFun_iff_pi' 𝔖_compact F_eqcont]; rw [show domRestrict (⋃₀ 𝔖) ∘ F = φ.symm ∘ F by rfl]; rw [show domRestrict (⋃₀ 𝔖) f = φ.symm f by rfl]; rw [φ.symm.isInducing.tendsto_nhds_iff]

/--
theorem `EquicontinuousOn.isClosed_range_pi_of_uniformOnFun'` / 定理 `EquicontinuousOn.isClosed_range_pi_of_uniformOnFun'`
...
中文:
定理 EquicontinuousOn.isClosed_range_pi_of_uniformOnFun'
  证明: by
  -- Do we have no equivalent of `nontriviality`?
  rcases isEmpty_or_nonempty α with _ | _
  · simp [isClosed_discrete]
...
/--
theorem `EquicontinuousOn.isClosed_range_uniformOnFun_iff_pi` / 定理 `EquicontinuousOn.isClosed_range_uniformOnFun_iff_pi`
...
theorem EquicontinuousOn.isClosed_range_uniformOnFun_iff_pi
  ...
  simp_rw [isClosed_iff_clusterPt, ← Filter.map_top, ← mapClusterPt_def,
    ...
    ← EquicontinuousOn.tendsto_uniform

中文:
定理 EquicontinuousOn.isClosed_range_uniformOnFun_iff_pi
  证明: by
  -- This follows from the previous lemmas and the characterization of the closure using filters.
  simp_rw [isClosed_iff_clusterPt, ← Filter.map_top, ← mapClusterPt_def,
    mapClusterPt_iff_ultrafilter, range_comp, (UniformOnFun.ofFun 𝔖).surjective.forall,
...
/--
theorem `ArzelaAscoli.compactSpace_of_closed_inducing'` / 定理 `ArzelaAscoli.compactSpace_of_closed_inducing'`

English:
theorem ArzelaAscoli.compactSpace_of_closed_inducing'
  statement: [TopologicalSpace ι] {𝔖 : Set (Set X)}
  proof: by
  ...
  -- Thus, we just have to check that the rang
...
theorem ArzelaAscoli.compactSpace_of_closed_inducing' [TopologicalSpace ι] {𝔖 : Set (Set X)}
    (𝔖_compact : forall K in 𝔖, IsCompact K) (F_ind : IsInducing (UniformOnFun.ofFun 𝔖 ∘ F))
    (F_cl : IsClosed <| range <| UniformOnFun.ofFun 𝔖 ∘ F)
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K)
    (F_pointwiseCompact : forall K in 𝔖, forall x in K, exists Q, IsCompact Q ∧ forall i, F i x in Q) :
    CompactSpace ι := by
  -- By equicontinuity, we know that the topology on `ι` is also the one induced by
  -- `domRestrict (⋃₀ 𝔖) ∘ F`.
  have : IsInducing (domRestrict (⋃₀ 𝔖) ∘ F) := by
  ...
  rw [← isCompact_univ_iff]; rw [this.isCompact_iff]; rw [image_univ]
...
/--
theorem `ArzelaAscoli.compactSpace_of_isClosedEmbedding` / 定理 `ArzelaAscoli.compactSpace_of_isClosedEmbedding`

English:
theorem ArzelaAscoli.compactSpace_of_isClosedEmbedding
  statement: [TopologicalSpace ι] {𝔖 : Set (Set X)}
  proof: compactSpace_of_closed_inducing' 𝔖_compact F_clemb.isInducing F_clemb.isClosed_range
...
Depends on / 依赖: F_clemb, F_clemb.isClosed_range, F_clemb.isInducing, F_eqcont, F_pointwiseCompact, compactSpace_of_closed_inducing, isClosed_range, isInducing
-/
theorem ArzelaAscoli.compactSpace_of_isClosedEmbedding [TopologicalSpace ι] {𝔖 : Set (Set X)}
    (𝔖_compact : forall K in 𝔖, IsCompact K) (F_clemb : IsClosedEmbedding (UniformOnFun.ofFun 𝔖 ∘ F))
    (F_eqcont : forall K in 𝔖, EquicontinuousOn F K)
    (F_pointwiseCompact : forall K in 𝔖, forall x in K, exists Q, IsCompact Q ∧ forall i, F i x in Q) :
    CompactSpace ι :=
...
/--
theorem `ArzelaAscoli.isCompact_closure_of_isClosedEmbedding` / 定理 `ArzelaAscoli.isCompact_closure_of_isClosedEmbedding`

English:
theorem ArzelaAscoli.isCompact_closure_of_isClosedEmbedding
  statement: [TopologicalSpace ι] [T2Space α]
  proof: by
  ...
  have : forall K in 𝔖, forall x in K, Continuous (eval x ∘ F) := fun K hK x hx =>
.continuous.comp F
...
theorem ArzelaAscoli.isCompact_closure_of_isClosedEmbedding [TopologicalSpace ι] [T2Space α]
    ...
    {s : Set ι} (s_eqcont : forall K in 𝔖, EquicontinuousOn (F ∘ ((↑) : s -> ι)) K)
    (s_pointwiseCompact : forall K in 𝔖, forall x in K, exists Q, IsCompact Q ∧ forall i in s, F i x in Q) :
    IsCompact (closure s) := by
  -- We apply `ArzelaAscoli.compactSpace_of_isClosedEmbedding` to the map
  -- `F ∘ (↑) : closure s → (X → α)`, for which all the hypotheses are easily verified.
  rw [isCompact_iff_compactSpace]
  have : forall K in 𝔖, forall x in K, Continuous (eval x ∘ F) := fun K hK x hx =>
.continuous.comp F_clemb.continuous UniformOnFun.uniformContinuous_eval_of_mem _ _ hx hK
  have cls_eqcont : forall K in 𝔖, EquicontinuousOn (F ∘ ((↑) : closure s -> ι)) K :=
fun K hK => (s_eqcont K hK).closure' show Continuous (K.domRestrict ∘ F) from
      continuous_pi fun ⟨x, hx⟩ => this K hK x hx
  have cls_pointwiseCompact : forall K in 𝔖, forall x in K, exists Q, IsCompact Q ∧ closure s subseteq {i | F i x in Q} :=
...
/--
theorem `ArzelaAscoli.isCompact_of_equicontinuous` / 定理 `ArzelaAscoli.isCompact_of_equicontinuous`
...
theorem ArzelaAscoli.isCompact_of_equicontinuous
  ...
  rw [← IsInducing.subtypeVal.of_comp_iff]; rw [← EquicontinuousOn.isInducing_uniformOnFun_iff_pi]
  · exact Cont

中文:
定理 ArzelaAscoli.isCompact_of_equicontinuous
  证明: by
  suffices h : IsInducing (Equiv.Set.image _ S DFunLike.coe_injective) by
    rw [isCompact_iff_compactSpace] at hS1 ⊢
    exact (Equiv.toHomeomorphOfIsInducing _ h).symm.compactSpace
...
Depends on / 依赖: ContinuousMap, ContinuousMap.isUniformEmbedding_toUniformOnFunIsCompact.isInducing.comp, DFunLike, DFunLike.coe_injective, EquicontinuousOn, EquicontinuousOn.isInducing_uniformOnFun_iff_pi, Equiv.Set.image, Equiv.toHomeomorphOfIsInducing, IsInducing, IsInducing.subtypeVal.of_comp_iff, coe_injective, compactSpace, eq_univ_iff_forall, eq_univ_iff_forall.mpr, isCompact_iff_compactSpace, isCompact_singleton, isInducing, isInducing_uniformOnFun_iff_pi, isUniformEmbedding_toUniformOnFunIsCompact, mem_sUnion_of_mem
-/
theorem ArzelaAscoli.isCompact_of_equicontinuous
