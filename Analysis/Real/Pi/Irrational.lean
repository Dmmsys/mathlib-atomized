...
variable {n : ℕ} {θ : ℝ}
/-
**I_zero** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**recursion'** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
While it is most natural to give the recursive formula for `I (n + 2) θ`, as wel
l as give the second
base case of `I 1 θ`, it is in fact more convenient to give the recursive formul
a for `I (n + 1) θ`
in terms of `I n θ` and `I (n - 1) θ` (note the natural subtraction!).
Despite the usually inconvenient subtraction, this in fact allows deducing both 
of the above facts
with significantly fewer analysis computations.
In addition, note the `0 ^ n` on the right-hand side - this is intentional, and 
again allows
combining the proof of the "usual" recursion formula and the base case `I 1 θ`.
-/
...
--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The recursive formula for `I (n + 2) θ * θ ^ 2` in terms of `I n θ` and `I (n + 
1) θ`.
-/
private lemma recursion (n : ℕ) :
    I (n + 2) θ * θ ^ 2 =
...
-/
/-
**I_one** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The second base case for the induction on `n`, giving an explicit formula for `I
 1 θ`.
-/
...
-/
/-
**sinPoly** 是 Mathlib 中的一个定义，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The first of the two integer-coefficient polynomials that describe the behaviour
 of the
sequence of integrals `I`.
While not given in the informal proof, these are easy to deduce from the recursi
on formulae.
-/
...
-/
/-
**cosPoly** 是 Mathlib 中的一个定义，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The second of the two integer-coefficient polynomials that describe the behaviou
r of the
sequence of integrals `I`.
While not given in the informal proof, these are easy to deduce from the recursi
on formulae.
...
-/
/-
**sinPoly_natDegree_le** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
Prove a degree bound for `sinPoly n` by induction. Note this is where we find th
e value in an
explicit description of `sinPoly`.
-/
...
-/
/-
**cosPoly_natDegree_le** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
Prove a degree bound for `cosPoly n` by induction. Note this is where we find th
e value in an
explicit description of `cosPoly`.
-/
...
-/
/-
**sinPoly_add_cosPoly_eval** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The key lemma: the sequence of integrals `I` can be written as a linear combinat
ion of `sin` and
`cos`, with coefficients given by the polynomials `sinPoly` and `cosPoly`.
-/
...
-/
/-
**is_integer** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
For a polynomial `p` with natural degree `≤ k` and integer coefficients, evaluat
ing `p` at a
rational `a / b` gives a rational of the form `z / b ^ k`.
TODO: should this be moved elsewhere? It uses none of the pi-specific definition
s.
-/
...
-/
/-
**I_pos** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The integrand in the definition of `I` is nonnegative and takes a positive value
 at least one point,
so the integral is positive.
-/
...
-/
/-
**I_le** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
The integrand in the definition of `I` is bounded by 1 and the interval has leng
th 2, so the
integral is bounded above by `2`.
-/
...
-/
/-
**tendsto_pow_div_factorial_at_top_aux** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Auxiliary for the proof that `π` is irrational.
For any real `a`, we have that `a ^ (2n+1) / n!` tends to `0` as `n → ∞`.  This 
is just a
reformulation of tendsto_pow_div_factorial_atTop, which asserts the same for `a 
^ n / n!`
-/
...
/-
**not_irrational_exists_rep** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `x` is rational, it can be written as `a / b` with `a : ℤ` and `b : ℕ` satisf
ying `b > 0`.
-/
...
/-
**irrational_pi** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：Irrational Real.pi
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Irrational.of_div_natCast`：of_div_natCast (m : Nat) (h : Irrational (x /
 m)) : Irrational x
...
· 使用定理 `_private.Mathlib.Analysis.Real.Pi.Irrational.0.not_irrational_exists_rep
`：∀ {x : ℝ}, ¬Irrational x → ∃ a b, 0 < b ∧ x = ↑a / ↑b
