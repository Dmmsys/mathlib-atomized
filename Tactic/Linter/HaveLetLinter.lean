...
/-
**Mathlib.Linter.haveLet.isHave** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.haveLe
t`。
...
/-
**Mathlib.Linter.haveLet.InfoTree.foldInfoM** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.haveLet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.haveLet.toFormat_propTypes** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.haveLet`。
形式化陈述：toFormat_propTypes (ctx : ContextInfo) (lc : LocalContext) (es : Array (Ex
pr × Name)) : CommandElabM (Array (Format × Name))
参数：ctx : ContextInfo；lc : LocalContext；es : Array (Expr × Name)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
given a `ContextInfo`, a `LocalContext` and an `Array` of `Expr`essions `es` wit
h a `Name`,
`toFormat_propTypes` creates a `MetaM` context, and returns an array of
the pretty-printed `Format` of `e`, together with the (unchanged) name
for each `Expr`ession `e` in `es` whose type is a `Prop`.
...
Concretely, `toFormat_propTypes` runs `inferType` in `CommandElabM`.
This is the kind of monadic lift that `nonPropHaves` uses to decide whether the 
Type of a `have`
is in `Prop` or not.
The output `Format` is just so that the linter displays a better message.
-/
...
public partial
/-
**Mathlib.Linter.haveLet.nonPropHaves** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.
haveLet`。
形式化陈述：nonPropHaves : InfoTree -> CommandElabM (Array (Syntax × Format))
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.haveLet.haveLetLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter
.haveLet`。
形式化陈述：haveLetLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
