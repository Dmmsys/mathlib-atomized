...
/-
**CategoryTheory.ComposableArrows.four** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory
.ComposableArrows`。
...
--- 原说明 ---
The morphism `mk₃ f₁ f₂ f₃₄ ⟶ mk₃ f₁ f₂₃ f₄` when `f₂ ≫ f₂ = f₂₃` and `f₃ ≫ f₃ =
 f₃₄`.
-/
...
@[simp]
/-
**CategoryTheory.ComposableArrows.four** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory
.ComposableArrows`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.ComposableArrows.four** 是 Mathlib 中的一个缩写定义，位于命名空间 `CategoryTheo
ry.ComposableArrows`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
