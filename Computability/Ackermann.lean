/-
Copyright (c) 2022 Violeta Hernández Palacios. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Violeta Hernández Palacios
-/
module

public import Mathlib.Computability.PartrecCode
public import Mathlib.Tactic.Ring
public import Mathlib.Tactic.NormNum

/-!
# Ackermann function

In this file, we define the two-argument Ackermann function `ack`. Despite having a recursive
definition, we show that this isn't a primitive recursive function.

## Main results

- `exists_lt_ack_of_nat_primrec`: any primitive recursive function is pointwise bounded above by
  `ack m` for some `m`.
- `not_primrec₂_ack`: the two-argument Ackermann function is not primitive recursive.
- `computable₂_ack`: the two-argument Ackermann function is computable.

## Proof approach

We very broadly adapt the proof idea from
https://www.planetmath.org/ackermannfunctionisnotprimitiverecursive. Namely, we prove that for any
primitive recursive `f : ℕ → ℕ`, there exists `m` such that `f n < ack m n` for all `n`. This then
implies that `fun n => ack n n` can't be primitive recursive, and so neither can `ack`. We aren't
able to use the same bounds as in that proof though, since our approach of using pairing functions
differs from their approach of using multivariate functions.

The important bounds we show during the main inductive proof (`exists_lt_ack_of_nat_primrec`)
are the following. Assuming `∀ n, f n < ack a n` and `∀ n, g n < ack b n`, we have:

- `∀ n, pair (f n) (g n) < ack (max a b + 3) n`.
- `∀ n, g (f n) < ack (max a b + 2) n`.
- `∀ n, Nat.rec (f n.unpair.1) (fun (y IH : ℕ) => g (pair n.unpair.1 (pair y IH)))
  n.unpair.2 < ack (max a b + 9) n`.

The last one is evidently the hardest. Using `unpair_add_le`, we reduce it to the more manageable

- `∀ m n, rec (f m) (fun (y IH : ℕ) => g (pair m (pair y IH))) n <
  ack (max a b + 9) (m + n)`.

We then prove this by induction on `n`. Our proof crucially depends on `ack_pair_lt`, which is
applied twice, giving us a constant of `4 + 4`. The rest of the proof consists of simpler bounds
which bump up our constant to `9`.
-/

@[expose] public section


open Nat

/--
Definition of `ack` / `ack` 的定义

English:
definition ack
  signature: : Nat -> Nat -> Nat

中文:
定义 ack
  签名: : 自然数 -> 自然数 -> 自然数
-/
def ack : Nat -> Nat -> Nat
  | 0, n => n + 1
  | m + 1, 0 => ack m 1
  | m + 1, n + 1 => ack m (ack (m + 1) n)

@[simp]
/--
theorem `ack_zero` / 定理 `ack_zero`

English:
theorem ack_zero
  given: (n : Nat)
  statement: ack 0 n = n + 1
  proof: by rw [ack]

@[simp]

中文:
定理 ack_zero
  条件: (n : 自然数)
  结论: ack 0 n = n + 1
  证明: by rw [ack]

@[simp]
-/
theorem ack_zero (n : Nat) : ack 0 n = n + 1 := by rw [ack]

@[simp]
/--
theorem `ack_succ_zero` / 定理 `ack_succ_zero`

English:
theorem ack_succ_zero
  given: (m : Nat)
  statement: ack (m + 1) 0 = ack m 1
  proof: by rw [ack]

@[simp]

中文:
定理 ack_succ_zero
  条件: (m : 自然数)
  结论: ack (m + 1) 0 = ack m 1
  证明: by rw [ack]

@[simp]
-/
theorem ack_succ_zero (m : Nat) : ack (m + 1) 0 = ack m 1 := by rw [ack]

@[simp]
/--
theorem `ack_succ_succ` / 定理 `ack_succ_succ`

English:
theorem ack_succ_succ
  given: (m n : Nat)
  statement: ack (m + 1) (n + 1) = ack m (ack (m + 1) n)
  proof: by rw [ack]

@[simp]

中文:
定理 ack_succ_succ
  条件: (m n : 自然数)
  结论: ack (m + 1) (n + 1) = ack m (ack (m + 1) n)
  证明: by rw [ack]

@[simp]
-/
theorem ack_succ_succ (m n : Nat) : ack (m + 1) (n + 1) = ack m (ack (m + 1) n) := by rw [ack]

@[simp]
/--
theorem `ack_one` / 定理 `ack_one`

English:
theorem ack_one
  given: (n : Nat)
  statement: ack 1 n = n + 2
  proof: by
  induction n with
  | zero => simp
  | succ n IH => simp [IH]

@[simp]

中文:
定理 ack_one
  条件: (n : 自然数)
  结论: ack 1 n = n + 2
  证明: by
  induction n with
  | zero => simp
  | succ n IH => simp [IH]

@[simp]
-/
theorem ack_one (n : Nat) : ack 1 n = n + 2 := by
  induction n with
  | zero => simp
  | succ n IH => simp [IH]

@[simp]
/--
theorem `ack_two` / 定理 `ack_two`

English:
theorem ack_two
  given: (n : Nat)
  statement: ack 2 n = 2 * n + 3
  proof: by
  induction n with
  | zero => simp
  | succ n IH => simpa [mul_succ]

@[simp]

中文:
定理 ack_two
  条件: (n : 自然数)
  结论: ack 2 n = 2 * n + 3
  证明: by
  induction n with
  | zero => simp
  | succ n IH => simpa [mul_succ]

@[simp]

Depends on / 依赖: mul_succ
-/
theorem ack_two (n : Nat) : ack 2 n = 2 * n + 3 := by
  induction n with
  | zero => simp
  | succ n IH => simpa [mul_succ]

@[simp]
/--
theorem `ack_three` / 定理 `ack_three`

English:
theorem ack_three
  given: (n : Nat)
  statement: ack 3 n = 2 ^ (n + 3) - 3
  proof: by
  induction n with
  | zero => simp
  | succ n IH =>
    rw [ack_succ_succ]; rw [IH]; rw [ack_two]; rw [Nat.succ_add]; rw [Nat.pow_succ 2 (n + 3)]; rw [mul_comm _ 2]; rw [Nat.mul_sub_left_distrib]; rw [← Nat.sub_add_comm]; rw [two_mul 3]; rw [Nat.add_sub_add_right]
    calc 2 * 3
      _ <= 2 * 2 ^ 3 := by simp
      _ <= 2 * 2 ^ (n + 3) := by gcongr <;> lia

中文:
定理 ack_three
  条件: (n : 自然数)
  结论: ack 3 n = 2 ^ (n + 3) - 3
  证明: by
  induction n with
  | zero => simp
  | succ n IH =>
    rw [ack_succ_succ]; rw [IH]; rw [ack_two]; rw [Nat.succ_add]; rw [Nat.pow_succ 2 (n + 3)]; rw [mul_comm _ 2]; rw [Nat.mul_sub_left_distrib]; rw [← Nat.sub_add_comm]; rw [two_mul 3]; rw [Nat.add_sub_add_right]
    calc 2 * 3
      _ <= 2 * 2 ^ 3 := by simp
      _ <= 2 * 2 ^ (n + 3) := by gcongr <;> lia

Depends on / 依赖: Nat.add_sub_add_right, Nat.mul_sub_left_distrib, Nat.pow_succ, Nat.sub_add_comm, Nat.succ_add, ack_succ_succ, ack_two, add_sub_add_right, mul_comm, mul_sub_left_distrib, pow_succ, sub_add_comm, succ_add, two_mul
-/
theorem ack_three (n : Nat) : ack 3 n = 2 ^ (n + 3) - 3 := by
  induction n with
  | zero => simp
  | succ n IH =>
    rw [ack_succ_succ]; rw [IH]; rw [ack_two]; rw [Nat.succ_add]; rw [Nat.pow_succ 2 (n + 3)]; rw [mul_comm _ 2]; rw [Nat.mul_sub_left_distrib]; rw [← Nat.sub_add_comm]; rw [two_mul 3]; rw [Nat.add_sub_add_right]
    calc 2 * 3
      _ <= 2 * 2 ^ 3 := by simp
      _ <= 2 * 2 ^ (n + 3) := by gcongr <;> lia

/--
theorem `ack_pos` / 定理 `ack_pos`

English:
theorem ack_pos
  statement: forall m n, 0 < ack m n

中文:
定理 ack_pos
  结论: 对任意 m n, 0 < ack m n
-/
theorem ack_pos : forall m n, 0 < ack m n
  | 0, n => by simp
  | m + 1, 0 => by
    rw [ack_succ_zero]
    apply ack_pos
  | m + 1, n + 1 => by
    rw [ack_succ_succ]
    apply ack_pos

/--
theorem `one_lt_ack_succ_left` / 定理 `one_lt_ack_succ_left`

English:
theorem one_lt_ack_succ_left
  statement: forall m n, 1 < ack (m + 1) n

中文:
定理 one_lt_ack_succ_left
  结论: 对任意 m n, 1 < ack (m + 1) n
-/
theorem one_lt_ack_succ_left : forall m n, 1 < ack (m + 1) n
  | 0, n => by simp
  | m + 1, 0 => by
    rw [ack_succ_zero]
    apply one_lt_ack_succ_left
  | m + 1, n + 1 => by
    rw [ack_succ_succ]
    apply one_lt_ack_succ_left

/--
theorem `one_lt_ack_succ_right` / 定理 `one_lt_ack_succ_right`

English:
theorem one_lt_ack_succ_right
  statement: forall m n, 1 < ack m (n + 1)

中文:
定理 one_lt_ack_succ_right
  结论: 对任意 m n, 1 < ack m (n + 1)
-/
theorem one_lt_ack_succ_right : forall m n, 1 < ack m (n + 1)
  | 0, n => by simp
  | m + 1, n => one_lt_ack_succ_left m (n + 1)

/--
theorem `ack_strictMono_right` / 定理 `ack_strictMono_right`

English:
theorem ack_strictMono_right
  statement: forall m, StrictMono (ack m)

中文:
定理 ack_strictMono_right
  结论: 对任意 m, 严格递增 (ack m)
-/
theorem ack_strictMono_right : forall m, StrictMono (ack m)
  | 0, n₁, n₂, h => by simpa using h
  | m + 1, 0, n + 1, _h => by
    rw [ack_succ_zero]; rw [ack_succ_succ]
    exact ack_strictMono_right _ (one_lt_ack_succ_left m n)
  | m + 1, n₁ + 1, n₂ + 1, h => by
    rw [ack_succ_succ]; rw [ack_succ_succ]
    apply ack_strictMono_right _ (ack_strictMono_right _ _)
    rwa [add_lt_add_iff_right] at h

/--
theorem `ack_mono_right` / 定理 `ack_mono_right`

English:
theorem ack_mono_right
  given: (m : Nat)
  statement: Monotone (ack m)
  proof: (ack_strictMono_right m).monotone

中文:
定理 ack_mono_right
  条件: (m : 自然数)
  结论: 递增 (ack m)
  证明: (ack_strictMono_right m).monotone

Depends on / 依赖: ack_strictMono_right, monotone
-/
theorem ack_mono_right (m : Nat) : Monotone (ack m) :=
  (ack_strictMono_right m).monotone

/--
theorem `ack_injective_right` / 定理 `ack_injective_right`

English:
theorem ack_injective_right
  given: (m : Nat)
  statement: Function.Injective (ack m)
  proof: (ack_strictMono_right m).injective

@[simp]

中文:
定理 ack_injective_right
  条件: (m : 自然数)
  结论: 函数.单射 (ack m)
  证明: (ack_strictMono_right m).injective

@[simp]

Depends on / 依赖: ack_strictMono_right, injective
-/
theorem ack_injective_right (m : Nat) : Function.Injective (ack m) :=
  (ack_strictMono_right m).injective

@[simp]
/--
theorem `ack_lt_iff_right` / 定理 `ack_lt_iff_right`

English:
theorem ack_lt_iff_right
  given: {m n₁ n₂ : Nat}
  statement: ack m n₁ < ack m n₂ ↔ n₁ < n₂
  proof: (ack_strictMono_right m).lt_iff_lt

@[simp]

中文:
定理 ack_lt_iff_right
  条件: {m n₁ n₂ : 自然数}
  结论: ack m n₁ < ack m n₂ ↔ n₁ < n₂
  证明: (ack_strictMono_right m).lt_iff_lt

@[simp]

Depends on / 依赖: ack_strictMono_right, lt_iff_lt
-/
theorem ack_lt_iff_right {m n₁ n₂ : Nat} : ack m n₁ < ack m n₂ ↔ n₁ < n₂ :=
  (ack_strictMono_right m).lt_iff_lt

@[simp]
/--
theorem `ack_le_iff_right` / 定理 `ack_le_iff_right`

English:
theorem ack_le_iff_right
  given: {m n₁ n₂ : Nat}
  statement: ack m n₁ <= ack m n₂ ↔ n₁ <= n₂
  proof: (ack_strictMono_right m).le_iff_le

@[simp]

中文:
定理 ack_le_iff_right
  条件: {m n₁ n₂ : 自然数}
  结论: ack m n₁ <= ack m n₂ ↔ n₁ <= n₂
  证明: (ack_strictMono_right m).le_iff_le

@[simp]

Depends on / 依赖: ack_strictMono_right, le_iff_le
-/
theorem ack_le_iff_right {m n₁ n₂ : Nat} : ack m n₁ <= ack m n₂ ↔ n₁ <= n₂ :=
  (ack_strictMono_right m).le_iff_le

@[simp]
/--
theorem `ack_inj_right` / 定理 `ack_inj_right`

English:
theorem ack_inj_right
  given: {m n₁ n₂ : Nat}
  statement: ack m n₁ = ack m n₂ ↔ n₁ = n₂
  proof: (ack_injective_right m).eq_iff

中文:
定理 ack_inj_right
  条件: {m n₁ n₂ : 自然数}
  结论: ack m n₁ = ack m n₂ ↔ n₁ = n₂
  证明: (ack_injective_right m).eq_iff

Depends on / 依赖: ack_injective_right, eq_iff
-/
theorem ack_inj_right {m n₁ n₂ : Nat} : ack m n₁ = ack m n₂ ↔ n₁ = n₂ :=
  (ack_injective_right m).eq_iff

/--
theorem `max_ack_right` / 定理 `max_ack_right`

English:
theorem max_ack_right
  given: (m n₁ n₂ : Nat)
  statement: ack m (max n₁ n₂) = max (ack m n₁) (ack m n₂)
  proof: (ack_mono_right m).map_max

中文:
定理 max_ack_right
  条件: (m n₁ n₂ : 自然数)
  结论: ack m (最大值 n₁ n₂) = 最大值 (ack m n₁) (ack m n₂)
  证明: (ack_mono_right m).map_max

Depends on / 依赖: ack_mono_right, map_max
-/
theorem max_ack_right (m n₁ n₂ : Nat) : ack m (max n₁ n₂) = max (ack m n₁) (ack m n₂) :=
  (ack_mono_right m).map_max

/--
theorem `add_lt_ack` / 定理 `add_lt_ack`

English:
theorem add_lt_ack
  statement: forall m n, m + n < ack m n
  proof: by lia
      _ < ack m (m + n + 2) := add_lt_ack _ _
      _ <= ack m (ack (m + 1) n) :=
ack_mono_right m le_of_eq_of_le (by rw [succ_eq_add_one]; ring_nf)
 succ_le_of_lt add_lt_ack (m + 1) n
      _ = ack (m + 1) (n + 1) := (ack_succ_succ m n).symm

中文:
定理 add_lt_ack
  结论: 对任意 m n, m + n < ack m n
  证明: by lia
      _ < ack m (m + n + 2) := add_lt_ack _ _
      _ <= ack m (ack (m + 1) n) :=
ack_mono_right m le_of_eq_of_le (by rw [succ_eq_add_one]; ring_nf)
 succ_le_of_lt add_lt_ack (m + 1) n
      _ = ack (m + 1) (n + 1) := (ack_succ_succ m n).symm

Depends on / 依赖: ack_mono_right, ack_succ_succ, add_lt_ack, le_of_eq_of_le, ring_nf, succ_eq_add_one, succ_le_of_lt
-/
theorem add_lt_ack : forall m n, m + n < ack m n
  | 0, n => by simp
  | m + 1, 0 => by simpa using add_lt_ack m 1
  | m + 1, n + 1 =>
    calc
      m + 1 + n + 1 <= m + (m + n + 2) := by lia
      _ < ack m (m + n + 2) := add_lt_ack _ _
      _ <= ack m (ack (m + 1) n) :=
ack_mono_right m le_of_eq_of_le (by rw [succ_eq_add_one]; ring_nf)
 succ_le_of_lt add_lt_ack (m + 1) n
      _ = ack (m + 1) (n + 1) := (ack_succ_succ m n).symm

/--
theorem `add_add_one_le_ack` / 定理 `add_add_one_le_ack`

English:
theorem add_add_one_le_ack
  given: (m n : Nat)
  statement: m + n + 1 <= ack m n
  proof: succ_le_of_lt (add_lt_ack m n)

中文:
定理 add_add_one_le_ack
  条件: (m n : 自然数)
  结论: m + n + 1 <= ack m n
  证明: succ_le_of_lt (add_lt_ack m n)

Depends on / 依赖: add_lt_ack, succ_le_of_lt
-/
theorem add_add_one_le_ack (m n : Nat) : m + n + 1 <= ack m n :=
  succ_le_of_lt (add_lt_ack m n)

/--
theorem `lt_ack_left` / 定理 `lt_ack_left`

English:
theorem lt_ack_left
  given: (m n : Nat)
  statement: m < ack m n
  proof: (self_le_add_right m n).trans_lt add_lt_ack m n

中文:
定理 lt_ack_left
  条件: (m n : 自然数)
  结论: m < ack m n
  证明: (self_le_add_right m n).trans_lt add_lt_ack m n

Depends on / 依赖: add_lt_ack, self_le_add_right, trans_lt
-/
theorem lt_ack_left (m n : Nat) : m < ack m n :=
(self_le_add_right m n).trans_lt add_lt_ack m n

/--
theorem `lt_ack_right` / 定理 `lt_ack_right`

English:
theorem lt_ack_right
  given: (m n : Nat)
  statement: n < ack m n
  proof: (self_le_add_left n m).trans_lt add_lt_ack m n

中文:
定理 lt_ack_right
  条件: (m n : 自然数)
  结论: n < ack m n
  证明: (self_le_add_left n m).trans_lt add_lt_ack m n

Depends on / 依赖: add_lt_ack, self_le_add_left, trans_lt
-/
theorem lt_ack_right (m n : Nat) : n < ack m n :=
(self_le_add_left n m).trans_lt add_lt_ack m n

-- we reorder the arguments to appease the equation compiler
/--
theorem `ack_strict_mono_left'` / 定理 `ack_strict_mono_left'`

English:
theorem ack_strict_mono_left'
  statement: forall {m₁ m₂} (n), m₁ < m₂ -> ack m₁ n < ack m₂ n
  proof: by lia
      _ <= m + ack (m + 1) n := by gcongr; exact add_add_one_le_ack ..
      _ < ack m (ack (m + 1) n) := add_lt_ack ..
  | m₁ + 1, m₂ + 1, 0 => fun h => by
    simpa using ack_strict_mono_left' 1 ((add_lt_add_iff_right 1).1 h)
  | m₁ + 1, m₂ + 1, n + 1 => fun h => by
    rw [ack_succ_succ]; rw [ack_succ_succ]
    exact
      (ack_strict_mono_left' _ <| (add_lt_add_iff_right 1).1 h).trans
        (ack_strictMono_right _ <| ack_strict_mono_left' n h)

中文:
定理 ack_strict_mono_left'
  结论: 对任意 {m₁ m₂} (n), m₁ < m₂ -> ack m₁ n < ack m₂ n
  证明: by lia
      _ <= m + ack (m + 1) n := by gcongr; exact add_add_one_le_ack ..
      _ < ack m (ack (m + 1) n) := add_lt_ack ..
  | m₁ + 1, m₂ + 1, 0 => fun h => by
    simpa using ack_strict_mono_left' 1 ((add_lt_add_iff_right 1).1 h)
  | m₁ + 1, m₂ + 1, n + 1 => fun h => by
    rw [ack_succ_succ]; rw [ack_succ_succ]
    exact
      (ack_strict_mono_left' _ <| (add_lt_add_iff_right 1).1 h).trans
        (ack_strictMono_right _ <| ack_strict_mono_left' n h)
-/
private theorem ack_strict_mono_left' : forall {m₁ m₂} (n), m₁ < m₂ -> ack m₁ n < ack m₂ n
  | m, 0, _ => fun h => (not_lt_zero m h).elim
  | 0, m + 1, 0 => fun _h => by simpa using one_lt_ack_succ_right m 0
  | 0, m + 1, n + 1 => fun h => by
    rw [ack_zero]; rw [ack_succ_succ]
    calc
      n + 1 + 1 <= m + (m + 1 + n + 1) := by lia
      _ <= m + ack (m + 1) n := by gcongr; exact add_add_one_le_ack ..
      _ < ack m (ack (m + 1) n) := add_lt_ack ..
  | m₁ + 1, m₂ + 1, 0 => fun h => by
    simpa using ack_strict_mono_left' 1 ((add_lt_add_iff_right 1).1 h)
  | m₁ + 1, m₂ + 1, n + 1 => fun h => by
    rw [ack_succ_succ]; rw [ack_succ_succ]
    exact
      (ack_strict_mono_left' _ <| (add_lt_add_iff_right 1).1 h).trans
        (ack_strictMono_right _ <| ack_strict_mono_left' n h)

/--
theorem `ack_strictMono_left` / 定理 `ack_strictMono_left`

English:
theorem ack_strictMono_left
  given: (n : Nat)
  statement: StrictMono fun m => ack m n
  proof: fun _m₁ _m₂ =>
  ack_strict_mono_left' n

中文:
定理 ack_strictMono_left
  条件: (n : 自然数)
  结论: 严格递增 fun m => ack m n
  证明: fun _m₁ _m₂ =>
  ack_strict_mono_left' n
-/
theorem ack_strictMono_left (n : Nat) : StrictMono fun m => ack m n := fun _m₁ _m₂ =>
  ack_strict_mono_left' n

/--
theorem `ack_mono_left` / 定理 `ack_mono_left`

English:
theorem ack_mono_left
  given: (n : Nat)
  statement: Monotone fun m => ack m n
  proof: (ack_strictMono_left n).monotone

中文:
定理 ack_mono_left
  条件: (n : 自然数)
  结论: 递增 fun m => ack m n
  证明: (ack_strictMono_left n).monotone

Depends on / 依赖: ack_strictMono_left, monotone
-/
theorem ack_mono_left (n : Nat) : Monotone fun m => ack m n :=
  (ack_strictMono_left n).monotone

/--
theorem `ack_injective_left` / 定理 `ack_injective_left`

English:
theorem ack_injective_left
  given: (n : Nat)
  statement: Function.Injective fun m => ack m n
  proof: (ack_strictMono_left n).injective

@[simp]

中文:
定理 ack_injective_left
  条件: (n : 自然数)
  结论: 函数.单射 fun m => ack m n
  证明: (ack_strictMono_left n).injective

@[simp]

Depends on / 依赖: ack_strictMono_left, injective
-/
theorem ack_injective_left (n : Nat) : Function.Injective fun m => ack m n :=
  (ack_strictMono_left n).injective

@[simp]
/--
theorem `ack_lt_iff_left` / 定理 `ack_lt_iff_left`

English:
theorem ack_lt_iff_left
  given: {m₁ m₂ n : Nat}
  statement: ack m₁ n < ack m₂ n ↔ m₁ < m₂
  proof: (ack_strictMono_left n).lt_iff_lt

@[simp]

中文:
定理 ack_lt_iff_left
  条件: {m₁ m₂ n : 自然数}
  结论: ack m₁ n < ack m₂ n ↔ m₁ < m₂
  证明: (ack_strictMono_left n).lt_iff_lt

@[simp]

Depends on / 依赖: ack_strictMono_left, lt_iff_lt
-/
theorem ack_lt_iff_left {m₁ m₂ n : Nat} : ack m₁ n < ack m₂ n ↔ m₁ < m₂ :=
  (ack_strictMono_left n).lt_iff_lt

@[simp]
/--
theorem `ack_le_iff_left` / 定理 `ack_le_iff_left`

English:
theorem ack_le_iff_left
  given: {m₁ m₂ n : Nat}
  statement: ack m₁ n <= ack m₂ n ↔ m₁ <= m₂
  proof: (ack_strictMono_left n).le_iff_le

@[simp]

中文:
定理 ack_le_iff_left
  条件: {m₁ m₂ n : 自然数}
  结论: ack m₁ n <= ack m₂ n ↔ m₁ <= m₂
  证明: (ack_strictMono_left n).le_iff_le

@[simp]

Depends on / 依赖: ack_strictMono_left, le_iff_le
-/
theorem ack_le_iff_left {m₁ m₂ n : Nat} : ack m₁ n <= ack m₂ n ↔ m₁ <= m₂ :=
  (ack_strictMono_left n).le_iff_le

@[simp]
/--
theorem `ack_inj_left` / 定理 `ack_inj_left`

English:
theorem ack_inj_left
  given: {m₁ m₂ n : Nat}
  statement: ack m₁ n = ack m₂ n ↔ m₁ = m₂
  proof: (ack_injective_left n).eq_iff

中文:
定理 ack_inj_left
  条件: {m₁ m₂ n : 自然数}
  结论: ack m₁ n = ack m₂ n ↔ m₁ = m₂
  证明: (ack_injective_left n).eq_iff

Depends on / 依赖: ack_injective_left, eq_iff
-/
theorem ack_inj_left {m₁ m₂ n : Nat} : ack m₁ n = ack m₂ n ↔ m₁ = m₂ :=
  (ack_injective_left n).eq_iff

/--
theorem `max_ack_left` / 定理 `max_ack_left`

English:
theorem max_ack_left
  given: (m₁ m₂ n : Nat)
  statement: ack (max m₁ m₂) n = max (ack m₁ n) (ack m₂ n)
  proof: (ack_mono_left n).map_max

@[gcongr]

中文:
定理 max_ack_left
  条件: (m₁ m₂ n : 自然数)
  结论: ack (最大值 m₁ m₂) n = 最大值 (ack m₁ n) (ack m₂ n)
  证明: (ack_mono_left n).map_max

@[gcongr]

Depends on / 依赖: ack_mono_left, map_max
-/
theorem max_ack_left (m₁ m₂ n : Nat) : ack (max m₁ m₂) n = max (ack m₁ n) (ack m₂ n) :=
  (ack_mono_left n).map_max

@[gcongr]
/--
theorem `ack_le_ack` / 定理 `ack_le_ack`

English:
theorem ack_le_ack
  given: {m₁ m₂ n₁ n₂ : Nat} (hm : m₁ <= m₂) (hn : n₁ <= n₂)
  statement: ack m₁ n₁ <= ack m₂ n₂
  proof: (ack_mono_left n₁ hm).trans ack_mono_right m₂ hn

中文:
定理 ack_le_ack
  条件: {m₁ m₂ n₁ n₂ : 自然数} (hm : m₁ <= m₂) (hn : n₁ <= n₂)
  结论: ack m₁ n₁ <= ack m₂ n₂
  证明: (ack_mono_left n₁ hm).trans ack_mono_right m₂ hn

Depends on / 依赖: ack_mono_left, ack_mono_right
-/
theorem ack_le_ack {m₁ m₂ n₁ n₂ : Nat} (hm : m₁ <= m₂) (hn : n₁ <= n₂) : ack m₁ n₁ <= ack m₂ n₂ :=
(ack_mono_left n₁ hm).trans ack_mono_right m₂ hn

/--
theorem `ack_succ_right_le_ack_succ_left` / 定理 `ack_succ_right_le_ack_succ_left`

English:
theorem ack_succ_right_le_ack_succ_left
  given: (m n : Nat)
  statement: ack m (n + 1) <= ack (m + 1) n
  proof: by
  rcases n with - | n
  · simp
  · rw [ack_succ_succ]
    apply ack_mono_right m (le_trans _ <| add_add_one_le_ack _ n)
    lia

中文:
定理 ack_succ_right_le_ack_succ_left
  条件: (m n : 自然数)
  结论: ack m (n + 1) <= ack (m + 1) n
  证明: by
  rcases n with - | n
  · simp
  · rw [ack_succ_succ]
    apply ack_mono_right m (le_trans _ <| add_add_one_le_ack _ n)
    lia

Depends on / 依赖: ack_mono_right, ack_succ_succ, add_add_one_le_ack, le_trans
-/
theorem ack_succ_right_le_ack_succ_left (m n : Nat) : ack m (n + 1) <= ack (m + 1) n := by
  rcases n with - | n
  · simp
  · rw [ack_succ_succ]
    apply ack_mono_right m (le_trans _ <| add_add_one_le_ack _ n)
    lia

-- All the inequalities from this point onwards are specific to the main proof.
/--
theorem `sq_le_two_pow_add_one_minus_three` / 定理 `sq_le_two_pow_add_one_minus_three`

English:
theorem sq_le_two_pow_add_one_minus_three
  given: (n : Nat)
  statement: n ^ 2 <= 2 ^ (n + 1) - 3
  proof: by
  induction n with
  | zero => simp
  | succ k => cases k <;> lia

中文:
定理 sq_le_two_pow_add_one_minus_three
  条件: (n : 自然数)
  结论: n ^ 2 <= 2 ^ (n + 1) - 3
  证明: by
  induction n with
  | zero => simp
  | succ k => cases k <;> lia
-/
private theorem sq_le_two_pow_add_one_minus_three (n : Nat) : n ^ 2 <= 2 ^ (n + 1) - 3 := by
  induction n with
  | zero => simp
  | succ k => cases k <;> lia

/--
theorem `ack_add_one_sq_lt_ack_add_three` / 定理 `ack_add_one_sq_lt_ack_add_three`

English:
theorem ack_add_one_sq_lt_ack_add_three
  statement: forall m n, (ack m n + 1) ^ 2 <= ack (m + 3) n

中文:
定理 ack_add_one_sq_lt_ack_add_three
  结论: 对任意 m n, (ack m n + 1) ^ 2 <= ack (m + 3) n
-/
theorem ack_add_one_sq_lt_ack_add_three : forall m n, (ack m n + 1) ^ 2 <= ack (m + 3) n
  | 0, n => by simpa using sq_le_two_pow_add_one_minus_three (n + 2)
  | m + 1, 0 => by
    rw [ack_succ_zero]; rw [ack_succ_zero]
    apply ack_add_one_sq_lt_ack_add_three
  | m + 1, n + 1 => by
    rw [ack_succ_succ]; rw [ack_succ_succ]
    apply (ack_add_one_sq_lt_ack_add_three _ _).trans (ack_mono_right _ <| ack_mono_left _ _)
    lia

/--
theorem `ack_ack_lt_ack_max_add_two` / 定理 `ack_ack_lt_ack_max_add_two`

English:
theorem ack_ack_lt_ack_max_add_two
  given: (m n k : Nat)
  statement: ack m (ack n k) < ack (max m n + 2) k
  proof: calc
    ack m (ack n k) <= ack (max m n) (ack n k) := ack_mono_left _ (le_max_left _ _)
    _ < ack (max m n) (ack (max m n + 1) k) :=
ack_strictMono_right _ ack_strictMono_left k lt_succ_of_le le_max_right m n
    _ = ack (max m n + 1) (k + 1) := (ack_succ_succ _ _).symm
    _ <= ack (max m n + 2) k := ack_succ_right_le_ack_succ_left _ _

中文:
定理 ack_ack_lt_ack_max_add_two
  条件: (m n k : 自然数)
  结论: ack m (ack n k) < ack (最大值 m n + 2) k
  证明: calc
    ack m (ack n k) <= ack (max m n) (ack n k) := ack_mono_left _ (le_max_left _ _)
    _ < ack (max m n) (ack (max m n + 1) k) :=
ack_strictMono_right _ ack_strictMono_left k lt_succ_of_le le_max_right m n
    _ = ack (max m n + 1) (k + 1) := (ack_succ_succ _ _).symm
    _ <= ack (max m n + 2) k := ack_succ_right_le_ack_succ_left _ _

Depends on / 依赖: ack_mono_left, ack_strictMono_left, ack_strictMono_right, ack_succ_right_le_ack_succ_left, ack_succ_succ, le_max_left, le_max_right, lt_succ_of_le
-/
theorem ack_ack_lt_ack_max_add_two (m n k : Nat) : ack m (ack n k) < ack (max m n + 2) k :=
  calc
    ack m (ack n k) <= ack (max m n) (ack n k) := ack_mono_left _ (le_max_left _ _)
    _ < ack (max m n) (ack (max m n + 1) k) :=
ack_strictMono_right _ ack_strictMono_left k lt_succ_of_le le_max_right m n
    _ = ack (max m n + 1) (k + 1) := (ack_succ_succ _ _).symm
    _ <= ack (max m n + 2) k := ack_succ_right_le_ack_succ_left _ _

/--
theorem `ack_add_one_sq_lt_ack_add_four` / 定理 `ack_add_one_sq_lt_ack_add_four`

English:
theorem ack_add_one_sq_lt_ack_add_four
  given: (m n : Nat)
  statement: ack m ((n + 1) ^ 2) < ack (m + 4) n
  proof: calc
    ack m ((n + 1) ^ 2) < ack m ((ack m n + 1) ^ 2) :=
ack_strictMono_right m Nat.pow_lt_pow_left (succ_lt_succ <| lt_ack_right m n) two_ne_zero
_ <= ack m (ack (m + 3) n) := ack_mono_right m ack_add_one_sq_lt_ack_add_three m n
_ <= ack (m + 2) (ack (m + 3) n) := ack_mono_left _ by lia
    _ = ack (m + 3) (n + 1) := (ack_succ_succ _ n).symm
    _ <= ack (m + 4) n := ack_succ_right_le_ack_succ_left _ n

中文:
定理 ack_add_one_sq_lt_ack_add_four
  条件: (m n : 自然数)
  结论: ack m ((n + 1) ^ 2) < ack (m + 4) n
  证明: calc
    ack m ((n + 1) ^ 2) < ack m ((ack m n + 1) ^ 2) :=
ack_strictMono_right m Nat.pow_lt_pow_left (succ_lt_succ <| lt_ack_right m n) two_ne_zero
_ <= ack m (ack (m + 3) n) := ack_mono_right m ack_add_one_sq_lt_ack_add_three m n
_ <= ack (m + 2) (ack (m + 3) n) := ack_mono_left _ by lia
    _ = ack (m + 3) (n + 1) := (ack_succ_succ _ n).symm
    _ <= ack (m + 4) n := ack_succ_right_le_ack_succ_left _ n

Depends on / 依赖: Nat.pow_lt_pow_left, ack_add_one_sq_lt_ack_add_three, ack_mono_left, ack_mono_right, ack_strictMono_right, ack_succ_right_le_ack_succ_left, ack_succ_succ, lt_ack_right, pow_lt_pow_left, succ_lt_succ, two_ne_zero
-/
theorem ack_add_one_sq_lt_ack_add_four (m n : Nat) : ack m ((n + 1) ^ 2) < ack (m + 4) n :=
  calc
    ack m ((n + 1) ^ 2) < ack m ((ack m n + 1) ^ 2) :=
ack_strictMono_right m Nat.pow_lt_pow_left (succ_lt_succ <| lt_ack_right m n) two_ne_zero
_ <= ack m (ack (m + 3) n) := ack_mono_right m ack_add_one_sq_lt_ack_add_three m n
_ <= ack (m + 2) (ack (m + 3) n) := ack_mono_left _ by lia
    _ = ack (m + 3) (n + 1) := (ack_succ_succ _ n).symm
    _ <= ack (m + 4) n := ack_succ_right_le_ack_succ_left _ n

/--
theorem `ack_pair_lt` / 定理 `ack_pair_lt`

English:
theorem ack_pair_lt
  given: (m n k : Nat)
  statement: ack m (pair n k) < ack (m + 4) (max n k)
  proof: (ack_strictMono_right m <| pair_lt_max_add_one_sq n k).trans
    ack_add_one_sq_lt_ack_add_four _ _

中文:
定理 ack_pair_lt
  条件: (m n k : 自然数)
  结论: ack m (pair n k) < ack (m + 4) (最大值 n k)
  证明: (ack_strictMono_right m <| pair_lt_max_add_one_sq n k).trans
    ack_add_one_sq_lt_ack_add_four _ _

Depends on / 依赖: ack_add_one_sq_lt_ack_add_four, ack_strictMono_right, pair_lt_max_add_one_sq
-/
theorem ack_pair_lt (m n k : Nat) : ack m (pair n k) < ack (m + 4) (max n k) :=
(ack_strictMono_right m <| pair_lt_max_add_one_sq n k).trans
    ack_add_one_sq_lt_ack_add_four _ _

/--
theorem `exists_lt_ack_of_nat_primrec` / 定理 `exists_lt_ack_of_nat_primrec`

English:
theorem exists_lt_ack_of_nat_primrec
  given: {f : Nat -> Nat} (hf : Nat.Primrec f)
  proof: by
  induction hf with
  | zero => exact ⟨0, ack_pos 0⟩
  | succ =>
    refine ⟨1, fun n => ?_⟩
    rw [succ_eq_one_add]
    apply add_lt_ack
  | left =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_left_le n
  | right =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_right_le n
  | @pair f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    refine ⟨max a b + 3, fun n => ?_⟩
    calc
      pair (f n) (g n) < (max (f n) (g n) + 1) ^ 2 := pair_lt_max_add_one_sq ..
      _ <= (ack (max a b) n + 1) ^ 2 := by rw [max_ack_left]; gcongr; exacts [(ha n).le, (hb n).le]
      _ <= ack (max a b + 3) n := ack_add_one_sq_lt_ack_add_three ..
  | comp hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    exact
      ⟨max a b + 2, fun n =>
(ha _).trans (ack_strictMono_right a <| hb n).trans ack_ack_lt_ack_max_add_two a b n⟩
  | @prec f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    -- We prove this simpler inequality first.
    have :
      forall {m n},
        rec (f m) (fun y IH => g <| pair m <| pair y IH) n < ack (max a b + 9) (m + n) := by
      intro m n
      -- We induct on n.
      induction n with
      | zero => -- The base case is easy.
        apply (ha m).trans (ack_strictMono_left m <| (le_max_left a b).trans_lt _)
        lia
      | succ n IH => -- We get rid of the first `pair`.
        simp only
        apply (hb _).trans ((ack_pair_lt _ _ _).trans_le _)
        -- If m is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ m with h₁ | h₁
        · rw [max_eq_left h₁.le]
          gcongr <;> omega
        rw [max_eq_right h₁]
        -- We get rid of the second `pair`.
        apply (ack_pair_lt _ _ _).le.trans
        -- If n is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ n with h₂ | h₂
        · rw [max_eq_left h₂.le, add_assoc]
          exact
            ack_le_ack (Nat.add_le_add (le_max_right a b) <| by simp)
              ((le_succ n).trans <| self_le_add_left _ _)
        rw [max_eq_right h₂]
        -- We now use the inductive hypothesis, and some simple algebraic manipulation.
        apply (ack_strictMono_right _ IH).le.trans
        rw [add_succ m]; rw [add_succ _ 8]; rw [succ_eq_add_one]; rw [succ_eq_add_one]; rw [ack_succ_succ (_ + 8)]; rw [add_assoc]
        exact ack_mono_left _ (Nat.add_le_add (le_max_right a b) le_rfl)
    -- The proof is now simple.
exact ⟨max a b + 9, fun n => this.trans_le ack_mono_right _ unpair_add_le n⟩

中文:
定理 存在_lt_ack_of_nat_primrec
  条件: {f : 自然数 -> 自然数} (hf : 自然数.Primrec f)
  证明: by
  induction hf with
  | zero => exact ⟨0, ack_pos 0⟩
  | succ =>
    refine ⟨1, fun n => ?_⟩
    rw [succ_eq_one_add]
    apply add_lt_ack
  | left =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_left_le n
  | right =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_right_le n
  | @pair f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    refine ⟨max a b + 3, fun n => ?_⟩
    calc
      pair (f n) (g n) < (max (f n) (g n) + 1) ^ 2 := pair_lt_max_add_one_sq ..
      _ <= (ack (max a b) n + 1) ^ 2 := by rw [max_ack_left]; gcongr; exacts [(ha n).le, (hb n).le]
      _ <= ack (max a b + 3) n := ack_add_one_sq_lt_ack_add_three ..
  | comp hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    exact
      ⟨max a b + 2, fun n =>
(ha _).trans (ack_strictMono_right a <| hb n).trans ack_ack_lt_ack_max_add_two a b n⟩
  | @prec f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    -- We prove this simpler inequality first.
    have :
      forall {m n},
        rec (f m) (fun y IH => g <| pair m <| pair y IH) n < ack (max a b + 9) (m + n) := by
      intro m n
      -- We induct on n.
      induction n with
      | zero => -- The base case is easy.
        apply (ha m).trans (ack_strictMono_left m <| (le_max_left a b).trans_lt _)
        lia
      | succ n IH => -- We get rid of the first `pair`.
        simp only
        apply (hb _).trans ((ack_pair_lt _ _ _).trans_le _)
        -- If m is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ m with h₁ | h₁
        · rw [max_eq_left h₁.le]
          gcongr <;> omega
        rw [max_eq_right h₁]
        -- We get rid of the second `pair`.
        apply (ack_pair_lt _ _ _).le.trans
        -- If n is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ n with h₂ | h₂
        · rw [max_eq_left h₂.le, add_assoc]
          exact
            ack_le_ack (Nat.add_le_add (le_max_right a b) <| by simp)
              ((le_succ n).trans <| self_le_add_left _ _)
        rw [max_eq_right h₂]
        -- We now use the inductive hypothesis, and some simple algebraic manipulation.
        apply (ack_strictMono_right _ IH).le.trans
        rw [add_succ m]; rw [add_succ _ 8]; rw [succ_eq_add_one]; rw [succ_eq_add_one]; rw [ack_succ_succ (_ + 8)]; rw [add_assoc]
        exact ack_mono_left _ (Nat.add_le_add (le_max_right a b) le_rfl)
    -- The proof is now simple.
exact ⟨max a b + 9, fun n => this.trans_le ack_mono_right _ unpair_add_le n⟩

Depends on / 依赖: Nat.lt_succ_iff, ack_pos, ack_zero, add_lt_ack, lt_succ_iff, pair_lt_max_add_one_sq, succ_eq_one_add, unpair_left_le, unpair_right_le
-/
theorem exists_lt_ack_of_nat_primrec {f : Nat -> Nat} (hf : Nat.Primrec f) :
    exists m, forall n, f n < ack m n := by
  induction hf with
  | zero => exact ⟨0, ack_pos 0⟩
  | succ =>
    refine ⟨1, fun n => ?_⟩
    rw [succ_eq_one_add]
    apply add_lt_ack
  | left =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_left_le n
  | right =>
    refine ⟨0, fun n => ?_⟩
    rw [ack_zero]; rw [Nat.lt_succ_iff]
    exact unpair_right_le n
  | @pair f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    refine ⟨max a b + 3, fun n => ?_⟩
    calc
      pair (f n) (g n) < (max (f n) (g n) + 1) ^ 2 := pair_lt_max_add_one_sq ..
      _ <= (ack (max a b) n + 1) ^ 2 := by rw [max_ack_left]; gcongr; exacts [(ha n).le, (hb n).le]
      _ <= ack (max a b + 3) n := ack_add_one_sq_lt_ack_add_three ..
  | comp hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    exact
      ⟨max a b + 2, fun n =>
(ha _).trans (ack_strictMono_right a <| hb n).trans ack_ack_lt_ack_max_add_two a b n⟩
  | @prec f g hf hg IHf IHg =>
    obtain ⟨a, ha⟩ := IHf; obtain ⟨b, hb⟩ := IHg
    -- We prove this simpler inequality first.
    have :
      forall {m n},
        rec (f m) (fun y IH => g <| pair m <| pair y IH) n < ack (max a b + 9) (m + n) := by
      intro m n
      -- We induct on n.
      induction n with
      | zero => -- The base case is easy.
        apply (ha m).trans (ack_strictMono_left m <| (le_max_left a b).trans_lt _)
        lia
      | succ n IH => -- We get rid of the first `pair`.
        simp only
        apply (hb _).trans ((ack_pair_lt _ _ _).trans_le _)
        -- If m is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ m with h₁ | h₁
        · rw [max_eq_left h₁.le]
          gcongr <;> omega
        rw [max_eq_right h₁]
        -- We get rid of the second `pair`.
        apply (ack_pair_lt _ _ _).le.trans
        -- If n is the maximum, we get a very weak inequality.
        rcases lt_or_ge _ n with h₂ | h₂
        · rw [max_eq_left h₂.le, add_assoc]
          exact
            ack_le_ack (Nat.add_le_add (le_max_right a b) <| by simp)
              ((le_succ n).trans <| self_le_add_left _ _)
        rw [max_eq_right h₂]
        -- We now use the inductive hypothesis, and some simple algebraic manipulation.
        apply (ack_strictMono_right _ IH).le.trans
        rw [add_succ m]; rw [add_succ _ 8]; rw [succ_eq_add_one]; rw [succ_eq_add_one]; rw [ack_succ_succ (_ + 8)]; rw [add_assoc]
        exact ack_mono_left _ (Nat.add_le_add (le_max_right a b) le_rfl)
    -- The proof is now simple.
exact ⟨max a b + 9, fun n => this.trans_le ack_mono_right _ unpair_add_le n⟩

/--
theorem `not_nat_primrec_ack_self` / 定理 `not_nat_primrec_ack_self`

English:
theorem not_nat_primrec_ack_self
  statement: ¬Nat.Primrec fun n => ack n n
  proof: fun h => by
  obtain ⟨m, hm⟩ := exists_lt_ack_of_nat_primrec h
  exact (hm m).false

中文:
定理 not_nat_primrec_ack_self
  结论: ¬自然数.Primrec fun n => ack n n
  证明: fun h => by
  obtain ⟨m, hm⟩ := exists_lt_ack_of_nat_primrec h
  exact (hm m).false

Depends on / 依赖: exists_lt_ack_of_nat_primrec
-/
theorem not_nat_primrec_ack_self : ¬Nat.Primrec fun n => ack n n := fun h => by
  obtain ⟨m, hm⟩ := exists_lt_ack_of_nat_primrec h
  exact (hm m).false

/--
theorem `not_primrec_ack_self` / 定理 `not_primrec_ack_self`

English:
theorem not_primrec_ack_self
  statement: ¬Primrec fun n => ack n n
  proof: by
  rw [Primrec.nat_iff]
  exact not_nat_primrec_ack_self

中文:
定理 not_primrec_ack_self
  结论: ¬Primrec fun n => ack n n
  证明: by
  rw [Primrec.nat_iff]
  exact not_nat_primrec_ack_self

Depends on / 依赖: Primrec, Primrec.nat_iff, nat_iff, not_nat_primrec_ack_self
-/
theorem not_primrec_ack_self : ¬Primrec fun n => ack n n := by
  rw [Primrec.nat_iff]
  exact not_nat_primrec_ack_self

/--
theorem `not_primrec₂_ack` / 定理 `not_primrec₂_ack`

English:
theorem not_primrec₂_ack
  statement: ¬Primrec₂ ack
  proof: fun h =>
not_primrec_ack_self h.comp Primrec.id Primrec.id

中文:
定理 not_primrec₂_ack
  结论: ¬Primrec₂ ack
  证明: fun h =>
not_primrec_ack_self h.comp Primrec.id Primrec.id
-/
theorem not_primrec₂_ack : ¬Primrec₂ ack := fun h =>
not_primrec_ack_self h.comp Primrec.id Primrec.id

namespace Nat.Partrec.Code

/--
Definition of `pappAck` / `pappAck` 的定义

English:
definition pappAck
  signature: : Nat -> Code

中文:
定义 pappAck
  签名: : 自然数 -> 余de
-/
def pappAck : Nat -> Code
  | 0 => .succ
  | n + 1 => step (pappAck n)
where
  /-- Yields single recursion step on `pappAck`. -/
  step (c : Code) : Code :=
    .curry (.prec (.comp c (.const 1)) (.comp c (.comp .right .right))) 0

/--
lemma `primrec_pappAck_step` / 引理 `primrec_pappAck_step`

English:
lemma primrec_pappAck_step
  statement: Primrec pappAck.step
  proof: by
  apply_rules
    [Code.primrec₂_curry.comp, Code.primrec₂_prec.comp, Code.primrec₂_comp.comp,
      _root_.Primrec.id, Primrec.const]

中文:
引理 primrec_pappAck_step
  结论: Primrec pappAck.step
  证明: by
  apply_rules
    [Code.primrec₂_curry.comp, Code.primrec₂_prec.comp, Code.primrec₂_comp.comp,
      _root_.Primrec.id, Primrec.const]

Depends on / 依赖: Code.primrec, Primrec, Primrec.const, _comp.comp, _curry.comp, _prec.comp, _root_, _root_.Primrec.id, apply_rules
-/
lemma primrec_pappAck_step : Primrec pappAck.step := by
  apply_rules
    [Code.primrec₂_curry.comp, Code.primrec₂_prec.comp, Code.primrec₂_comp.comp,
      _root_.Primrec.id, Primrec.const]

set_option backward.isDefEq.respectTransparency false in
@[simp]
/--
lemma `eval_pappAck_step_zero` / 引理 `eval_pappAck_step_zero`

English:
lemma eval_pappAck_step_zero
  given: (c : Code)
  statement: (pappAck.step c).eval 0 = c.eval 1
  proof: by
  simp [pappAck.step, Code.eval]

中文:
引理 eval_pappAck_step_zero
  条件: (c : 余de)
  结论: (pappAck.step c).eval 0 = c.eval 1
  证明: by
  simp [pappAck.step, Code.eval]

Depends on / 依赖: Code.eval, pappAck, pappAck.step
-/
lemma eval_pappAck_step_zero (c : Code) : (pappAck.step c).eval 0 = c.eval 1 := by
  simp [pappAck.step, Code.eval]

set_option backward.isDefEq.respectTransparency false in
@[simp]
/--
lemma `eval_pappAck_step_succ` / 引理 `eval_pappAck_step_succ`

English:
lemma eval_pappAck_step_succ
  given: (c : Code) (n)
  proof: by
  simp [pappAck.step, Code.eval]

中文:
引理 eval_pappAck_step_succ
  条件: (c : 余de) (n)
  证明: by
  simp [pappAck.step, Code.eval]

Depends on / 依赖: Code.eval, pappAck, pappAck.step
-/
lemma eval_pappAck_step_succ (c : Code) (n) :
    (pappAck.step c).eval (n + 1) = ((pappAck.step c).eval n).bind c.eval := by
  simp [pappAck.step, Code.eval]

/--
lemma `primrec_pappAck` / 引理 `primrec_pappAck`

English:
lemma primrec_pappAck
  statement: Primrec pappAck
  proof: by
  suffices Primrec (Nat.rec Code.succ (fun _ c => pappAck.step c)) by
    convert! this using 2 with n; induction n <;> simp [pappAck, *]
  apply_rules [Primrec.nat_rec₁, primrec_pappAck_step.comp, Primrec.snd]

中文:
引理 primrec_pappAck
  结论: Primrec pappAck
  证明: by
  suffices Primrec (Nat.rec Code.succ (fun _ c => pappAck.step c)) by
    convert! this using 2 with n; induction n <;> simp [pappAck, *]
  apply_rules [Primrec.nat_rec₁, primrec_pappAck_step.comp, Primrec.snd]

Depends on / 依赖: Code.succ, Nat.rec, Primrec, Primrec.nat_rec, Primrec.snd, apply_rules, convert, pappAck, pappAck.step, primrec_pappAck_step, primrec_pappAck_step.comp
-/
lemma primrec_pappAck : Primrec pappAck := by
  suffices Primrec (Nat.rec Code.succ (fun _ c => pappAck.step c)) by
    convert! this using 2 with n; induction n <;> simp [pappAck, *]
  apply_rules [Primrec.nat_rec₁, primrec_pappAck_step.comp, Primrec.snd]

set_option backward.isDefEq.respectTransparency false in
@[simp]
/--
lemma `eval_pappAck` / 引理 `eval_pappAck`

English:
lemma eval_pappAck
  given: (m n)
  statement: (pappAck m).eval n = Part.some (ack m n)
  proof: by
  induction m, n using ack.induct with
    | case1 n => simp [Code.eval, pappAck]
    | case2 m hm => simp [pappAck, hm]
    | case3 m n hmn₁ hmn₂ => dsimp only [pappAck] at *; simp [hmn₁, hmn₂]

中文:
引理 eval_pappAck
  条件: (m n)
  结论: (pappAck m).eval n = Part.some (ack m n)
  证明: by
  induction m, n using ack.induct with
    | case1 n => simp [Code.eval, pappAck]
    | case2 m hm => simp [pappAck, hm]
    | case3 m n hmn₁ hmn₂ => dsimp only [pappAck] at *; simp [hmn₁, hmn₂]

Depends on / 依赖: Code.eval, ack.induct, induct, pappAck
-/
lemma eval_pappAck (m n) : (pappAck m).eval n = Part.some (ack m n) := by
  induction m, n using ack.induct with
    | case1 n => simp [Code.eval, pappAck]
    | case2 m hm => simp [pappAck, hm]
    | case3 m n hmn₁ hmn₂ => dsimp only [pappAck] at *; simp [hmn₁, hmn₂]

/--
theorem `_root_.computable₂_ack` / 定理 `_root_.computable₂_ack`

English:
theorem _root_.computable₂_ack
  statement: Computable₂ ack
  proof: by
  apply _root_.Partrec.of_eq_tot
    (f := fun p : Nat × Nat => (pappAck p.1).eval p.2) (g := fun p : Nat × Nat => ack p.1 p.2)
  · change Partrec₂ (fun m n => (pappAck m).eval n)
    apply_rules only
      [Code.eval_part.comp₂, Computable.fst, Computable.snd, primrec_pappAck.to_comp.comp]
  · simp

中文:
定理 _root_.computable₂_ack
  结论: Computable₂ ack
  证明: by
  apply _root_.Partrec.of_eq_tot
    (f := fun p : Nat × Nat => (pappAck p.1).eval p.2) (g := fun p : Nat × Nat => ack p.1 p.2)
  · change Partrec₂ (fun m n => (pappAck m).eval n)
    apply_rules only
      [Code.eval_part.comp₂, Computable.fst, Computable.snd, primrec_pappAck.to_comp.comp]
  · simp

Depends on / 依赖: Code.eval_part.comp, Computable, Computable.fst, Computable.snd, Partrec, _root_, _root_.Partrec.of_eq_tot, apply_rules, eval_part, of_eq_tot, pappAck, primrec_pappAck, primrec_pappAck.to_comp.comp, to_comp
-/
theorem _root_.computable₂_ack : Computable₂ ack := by
  apply _root_.Partrec.of_eq_tot
    (f := fun p : Nat × Nat => (pappAck p.1).eval p.2) (g := fun p : Nat × Nat => ack p.1 p.2)
  · change Partrec₂ (fun m n => (pappAck m).eval n)
    apply_rules only
      [Code.eval_part.comp₂, Computable.fst, Computable.snd, primrec_pappAck.to_comp.comp]
  · simp

end Nat.Partrec.Code
