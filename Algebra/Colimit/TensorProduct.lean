...
/--
theorem `Submodule.FG.exists_rTensor_fg_inclusion_eq` / 定理 `Submodule.FG.exists_rTensor_fg_inclusion_eq`

English:
theorem Submodule.FG.exists_rTensor_fg_inclusion_eq
  statement: {N : Submodule R P} (hN : N.FG)
  proof: by
...
theorem Submodule.FG.exists_rTensor_fg_inclusion_eq {N : Submodule R P} (hN : N.FG)
    {x y : N otimes[R] M} (eq : N.subtype.rTensor M x = N.subtype.rTensor M y) :
    exists N', N'.FG ∧ exists h : N <= N', (N.inclusion h).rTensor M x = (N.inclusion h).rTensor M y := by
  lift N to {N : Submodule R P // N.FG} using hN
  apply_fun (Module.fgSystem.equiv R P).symm.toLinearMap.rTensor M at eq
  apply_fun directLimitLeft _ _ at eq
  simp_rw [← LinearMap.rTensor_comp_apply, ← (LinearEquiv.eq_toLinearMap_symm_comp _ _).mpr
