...
/-
**List.modifyLast.go_concat** 是 Mathlib 中的一个定理，位于命名空间 `List`。
...
private theorem modifyLast.go_concat (f : α → α) (a : α) (tl : List α) (r : Array α) :
  ...
  | cons hd tl =>
    ...
    simp only [Array.toListAppend_eq, Array.toList_push, nil_append,
      append_assoc]
/-
**List.modifyLast_concat** 是 Mathlib 中的一个定理，位于命名空间 `List`。
形式化陈述：modifyLast_concat (f : α -> α) (a : α) (l : List α) : modifyLast f (l ++ [
a]) = l ++ [f a]
参数：f : α -> α；a : α；l : List α。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**List.modifyLast_append_of_right_ne_nil** 是 Mathlib 中的一个定理，位于命名空间 `List`。
形式化陈述：modifyLast_append_of_right_ne_nil (f : α -> α) (l₁ l₂ : List α) (_ : l₂ !=
 []) : modifyLast f (l₁ ++ l₂) = l₁ ++ modifyLast f l₂
参数：f : α -> α；l₁ l₂ : List α；_ : l₂ != []。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
