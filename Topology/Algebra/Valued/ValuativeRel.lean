...
English:
theorem of_zero
  statement: [ContinuousConstVAdd R R]
  proof: by
...
Depends on / 依赖: image_subset_iff, image_vadd, vadd_mem_nhds_vadd_iff
-/
theorem of_zero [ContinuousConstVAdd R R]
    (h₀ : forall s : Set R, s in 𝓝 0 ↔ exists γ : (ValueGroupWithZero R)ˣ, { z | v z < γ } subseteq s) :
...
/--
lemma `v_eq_valuation` / 引理 `v_eq_valuation`

English:
lemma v_eq_valuation
  statement: {R : Type*} [Ring R] [ValuativeRel R] [UniformSpace R]
  proof: rfl
...
/--
lemma `continuous_valuation` / 引理 `continuous_valuation`

English:
lemma continuous_valuation
  statement: Continuous v
  proof: by
