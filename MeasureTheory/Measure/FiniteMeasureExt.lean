...
/--
theorem `ext_of_forall_mem_subalgebra_integral_eq_of_pseudoEMetric_complete_countable` / 定理 `ext_of_forall_mem_subalgebra_integral_eq_of_pseudoEMetric_complete_countable`
...
theorem ext_of_forall_mem_subalgebra_integral_eq_of_pseudoEMetric_complete_countable
    ...
    (heq : forall g in A, ∫ x, (g : E -> 𝕜) x ∂P = ∫ x, (g : E -> 𝕜) x ∂P') : P = P' := by
  --consider the real subalgebra of the purely real-valued elements of A
  let A_toReal := (A.restrictScalars Real).comap
    (ofRealAm.compLeftContinuousBounded Real lipschitzWith_ofReal)
  --the real subalgebra separates points
...
/--
theorem `ext_of_forall_mem_subalgebra_integral_eq_of_polish` / 定理 `ext_of_forall_mem_subalgebra_integral_eq_of_polish`

English:
theorem ext_of_forall_mem_subalgebra_integral_eq_of_polish
  statement: [TopologicalSpace E] [PolishSpace E]
  proof: by
...
Depends on / 依赖: TopologicalSpace, TopologicalSpace.upgradeIsCompletelyMetrizable, ext_of_forall_mem_subalgebra_integral_eq_of_pseudoEMetric_complete_countable, upgradeIsCompletelyMetrizable
-/
