...
English:
definition over
  signature: : Point.{w} (J.over X) where
  body: FunctorToTypes.fromOverFunctor Φ.fiber x
  initiallySmall :=
...
中文:
定义 over
  签名: : Point.{w} (J.over X) where
  定义体: FunctorToTypes.fromOverFunctor Φ.fiber x
  initiallySmall :=
    initiallySmall_of_initial_of_initiallySmall
      (FunctorToTypes.fromOverFunctorElementsEquivalence Φ.fiber x).inverse
  jointly_surjective := by
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `IsConservativeFamilyOfPoints.over` / 引理 `IsConservativeFamilyOfPoints.over`

English:
lemma IsConservativeFamilyOfPoints.over
  proof: mk' (fun Y S hS => by
    obtain ⟨Y, f, rfl⟩ := Over.mk_surjective Y
    obtain ⟨S, rfl⟩ := (Sieve.overEquiv _).symm.surjective S
    rw [mem_over_iff]; rw [OrderIso.apply_symm_apply]
...
English:
instance [HasEnoughPoints.{w}
  signature: J] [J.WEqualsLocallyBijective (Type w)]
  body: by
...
Depends on / 依赖: HasEnoughPoints, HasEnoughPoints.exists_objectProperty, exists_objectProperty, hP.over
-/
