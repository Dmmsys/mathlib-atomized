...
@[stacks 01Z2]
/--
lemma `Scheme.nonempty_of_isLimit` / 引理 `Scheme.nonempty_of_isLimit`

English:
lemma Scheme.nonempty_of_isLimit
  statement: [IsCofilteredOrEmpty I]
  proof: by
  classical
...
Depends on / 依赖: D.map, D.obj, IsAffine, IsCofiltered, Nonempty, Nonempty.map, Nonempty.some, affineCover, affineCover.finiteSubcover, classical, e.inv, finiteSubcover, isEmpty_or_nonempty, isLimitEquivIsTerminalOfIsEmpty, specULiftZIsTerminal, uniqueUpToIso
-/
lemma Scheme.nonempty_of_isLimit [IsCofilteredOrEmpty I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] [forall i, Nonempty (D.obj i)]
    [forall i, CompactSpace (D.obj i)] :
    Nonempty c.pt := by
  classical
  cases isEmpty_or_nonempty I
  · have e := (isLimitEquivIsTerminalOfIsEmpty _ _ hc).uniqueUpToIso specULiftZIsTerminal
  ...
  · have i := Nonempty.some ‹Nonempty I›
    ...
    obtain ⟨j, H⟩ :
        exists j : 𝒰.I₀, forall {i'} (f : i' ⟶ i), Nonempty ((𝒰.pullback₁ (D.map f)).X j) := by
      by_contra! H
      ...
      have (j : 𝒰.I₀) : IsEmpty ((𝒰.pullback₁ (D.map (g i (by simp)))).X j) := by
        let F : (𝒰.pullback₁ (D.map (g i (by simp)))).X j ⟶
          ...
          pullback.map _ _ _ _ (D.map (g _ (by simp))) (𝟙 _) (𝟙 _) (by
            rw [← D.map_comp]; rw [IsCofiltered.infTo_commutes]
            · simp [g]
            · simp
...
/--
lemma `exists_mem_of_isClosed_of_nonempty` / 引理 `exists_mem_of_isClosed_of_nonempty`
...
lemma exists_mem_of_isClosed_of_nonempty
  ...
  { obj i := (vanishingIdeal ⟨Z i, hZc i⟩).subscheme
    map {X Y} f := subschemeMap _ _ (D.map f) (by
      rw [map_vanishingIdeal]; rw [← le_support_iff_le_vanishingIdeal]
...
Depends on / 依赖: D.map, IsAffineHom, Scheme, cancel_mono, closure_subset_iff, hmapsTo, le_support_iff_le_vanishingIdeal, map_comp, map_id, map_vanishingIdeal, naturality, subscheme, subschemeMap, subset_preimage, vanishingIdeal
-/
lemma exists_mem_of_isClosed_of_nonempty
    [IsCofilteredOrEmpty I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    (Z : forall (i : I), Set (D.obj i))
    (hZc : forall (i : I), IsClosed (Z i))
    (hZne : forall i, (Z i).Nonempty)
    (hZcpt : forall i, IsCompact (Z i))
    (hmapsTo : forall {i i' : I} (f : i ⟶ i'), Set.MapsTo (D.map f) (Z i) (Z i')) :
    exists (s : c.pt), forall i, c.π.app i s in Z i := by
  let D' : I ⥤ Scheme :=
  { obj i := (vanishingIdeal ⟨Z i, hZc i⟩).subscheme
    map {X Y} f := subschemeMap _ _ (D.map f) (by
      rw [map_vanishingIdeal]; rw [← le_support_iff_le_vanishingIdeal]
  ...
  { lift s := IsClosedImmersion.lift (subschemeι _) (hc.lift ((Cone.postcompose ι).obj s)) (by
      suffices forall i, vanishingIdeal ⟨Z i, hZc i⟩ <= (s.π.app i ≫ ι.app i).ker by
...
Depends on / 依赖: Functor, Functor.Initial.isLimitWhiskerEquiv, Initial, IsAffineHom, Over.forall_iff, Over.forget, Over.w, exists_mem_of_isClosed_of_nonempty, forall_iff, forget, i.hom, i.left, infer_instance, isLimitWhiskerEquiv
-/
lemma exists_mem_of_isClosed_of_nonempty'
    [IsCofilteredOrEmpty I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    {j : I}
    (Z : forall (i : I), (i ⟶ j) -> Set (D.obj i))
    (hZc : forall i hij, IsClosed (Z i hij))
    (hZne : forall i hij, (Z i hij).Nonempty)
    (hZcpt : forall i hij, IsCompact (Z i hij))
    (hstab : forall (i i' : I) (hi'i : i' ⟶ i) (hij : i ⟶ j),
      Set.MapsTo (D.map hi'i) (Z i' (hi'i ≫ hij)) (Z i hij)) :
    exists (s : c.pt), forall i hij, c.π.app i s in Z i hij := by
  have {i₁ i₂ : Over j} (f : i₁ ⟶ i₂) : IsAffineHom ((Over.forget j ⋙ D).map f) := by
    dsimp; infer_instance
  simpa [Over.forall_iff] using! exists_mem_of_isClosed_of_nonempty (Over.forget j ⋙ D) _
    ((Functor.Initial.isLimitWhiskerEquiv (Over.forget j) c).symm hc)
...
include hc in
/--
lemma `exists_map_eq_top` / 引理 `exists_map_eq_top`
...
Depends on / 依赖: D.map, Functor, Functor.map_comp, Set.mem_univ, Set.nonempty_compl, SetLike, SetLike.coe_injective.ne, TopologicalSpace, TopologicalSpace.Opens.map_coe, absurd, coe_injective, exists_mem_of_isClosed_of_nonempty, hU.ge, isClosed_compl, isClosed_compl.isCompact, isCompact, map_coe, map_comp, mem_univ, ne_eq
-/
lemma exists_map_eq_top
    [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    ...
    exists (j : I) (fji : j ⟶ i), D.map fji ⁻¹ᵁ U = ⊤ := by
  by_contra! H
  obtain ⟨s, hs⟩ := exists_mem_of_isClosed_of_nonempty' D c hc (fun j f => (D.map f ⁻¹ᵁ U)ᶜ)
    (fun j f => (D.map f ⁻¹ᵁ U).2.isClosed_compl) (fun j f => by
      simp only [TopologicalSpace.Opens.map_coe, Set.nonempty_compl, ne_eq]
...
@[simps] noncomputable
/--
Definition of `opensDiagram` / `opensDiagram` 的定义

English:
definition opensDiagram
  signature: (i : I) (U : (D.obj i).Opens)
  body: D.map j.hom ⁻¹ᵁ U
  map {j k} f := (D.map f.left).resLE _ _
    (by rw [← Scheme.Hom.comp_preimage, ← D.map_comp, Over.w f])

中文:
定义 opensDiagram
  签名: (i : I) (U : (D.obj i).Opens)
  定义体: D.map j.hom ⁻¹ᵁ U
  map {j k} f := (D.map f.left).resLE _ _
    (by rw [← Scheme.Hom.comp_preimage, ← D.map_comp, Over.w f])

Depends on / 依赖: D.map, j.hom
-/
...
@[simps] noncomputable
/--
Definition of `opensDiagramι` / `opensDiagramι` 的定义

English:
definition opensDiagramι
  signature: (i : I) (U : (D.obj i).Opens)
...
@[simps] noncomputable
/--
Definition of `opensCone` / `opensCone` 的定义

English:
definition opensCone
  signature: (i : I) (U : (D.obj i).Opens)
  body: c.π.app i ⁻¹ᵁ U
...
noncomputable
/--
Definition of `isLimitOpensCone` / `isLimitOpensCone` 的定义

English:
definition isLimitOpensCone
  signature: [IsCofiltered I] (i : I) (U : (D.obj i).Opens)
  body: isLimitOfIsPullbackOfIsConnected (opensDiagramι D i U) _ _
...
Depends on / 依赖: Functor, Functor.Initial.isLimitWhiskerEquiv, Initial, IsOpenImmersion, IsOpenImmersion.isPullback, Over.forget, Scheme, Scheme.Hom.comp_preimage, comp_preimage, forget, isLimitOfIsPullbackOfIsConnected, isLimitWhiskerEquiv, isPullback
-/
...
English:
instance [forall
  signature: {i j} (f : i ⟶ j), IsAffineHom (D.map f)] {i
  body: by
  refine ⟨fun V hV => ?_⟩
  convert!
...
Depends on / 依赖: D.map, D.map_comp, Over.w, Scheme, Scheme.Op, Subtype, Subtype.ext, convert, f.left, hV.image_of_isOpenImmersion, image_of_isOpenImmersion, j.hom, k.hom, map_comp, opensDiagram_obj, preimage, preimage_of_isOpenImmersion
-/
instance [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] {i : I}
  ...
  · ext x
    change _ in V ↔ _
    refine ⟨fun h => ⟨⟨(D.map f.left).base x.1, ?_⟩, ?_, rfl⟩, ?_⟩
    · change (D.map f.left ≫ D.map k.hom).base x.1 in U
      rw [← D.map_comp]; rw [Over.w f]
      exact x.2
    · convert! h
  ...
  · simp only [opensDiagram_obj, Scheme.Opens.opensRange_ι]
    rintro x ⟨⟨y, h₁ : (D.map k.hom).base y in U⟩, h₂, e⟩
...
include hc in
/--
lemma `exists_map_preimage_le_map_preimage` / 引理 `exists_map_preimage_le_map_preimage`
...
lemma exists_map_preimage_le_map_preimage
  ...
  have H : ((c.π.app i ⁻¹ᵁ U).ι ≫ c.π.app i) ⁻¹ᵁ V = ⊤ := by
    rw [Scheme.Hom.comp_preimage]; rw [← top_le_iff]
    exact .trans 
...
Depends on / 依赖: CompactSpace, D.map, QuasiCompact, QuasiCompact.isCompact_preimage, Scheme, Scheme.Hom.comp_preim, Scheme.Hom.comp_preimage, Scheme.Hom.preimage_mono, Scheme.isoOfEq, comp_preim, comp_preimage, exists_map_eq_top, isCompact_iff_compactSpace, isCompact_iff_compactSpace.mp, isCompact_preimage, isLimitOpensCone, isoOfEq, j.hom, opensDiagram, preimage_mono
-/
lemma exists_map_preimage_le_map_preimage
    [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    ...
    exists (j : I) (fji : j ⟶ i), D.map fji ⁻¹ᵁ U <= D.map fji ⁻¹ᵁ V := by
  have (j : Over i) : CompactSpace ↥((opensDiagram D i U).obj j) :=
    isCompact_iff_compactSpace.mp (QuasiCompact.isCompact_preimage (f := (D.map j.hom)) _ U.2 hU)
  have H : ((c.π.app i ⁻¹ᵁ U).ι ≫ c.π.app i) ⁻¹ᵁ V = ⊤ := by
    rw [Scheme.Hom.comp_preimage]; rw [← top_le_iff]
...
@[stacks 01Z4 "(2)"]
/--
lemma `exists_map_preimage_eq_map_preimage` / 引理 `exists_map_preimage_eq_map_preimage`
...
Depends on / 依赖: Functor, Functor.map_comp, H.ge, H.le, IsCofiltered, IsCofiltered.cospan, Scheme, Scheme.Hom.comp_preimage, Scheme.Hom.preimage_mono, comp_preimage, cospan, exists_map_preimage_le_map_preimage, le_antisymm, map_comp, preimage_mono
-/
lemma exists_map_preimage_eq_map_preimage
    [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
...
include hc in
/--
lemma `isBasis_preimage_isAffineOpen` / 引理 `isBasis_preimage_isAffineOpen`

English:
lemma isBasis_preimage_isAffineOpen
  given: [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
  proof: by
...
Depends on / 依赖: D.obj, IsAffine, IsAffineOpen, IsAffineOpen.exists_basicOpen_le, IsCofiltered, IsCofiltered.nonempty, Scheme, Scheme.isAffine_of_isLimi, Set.mem_univ, TopologicalSpace, TopologicalSpace.Opens.isBasis_iff_nbhd.mpr, exists_basicOpen_le, exists_subset_of_mem_open, hV.preimage, isAffine_of_isLimi, isBasis_affineOpens, isBasis_affineOpens.exists_subset_of_mem_open, isBasis_iff_nbhd, isOpen_univ, mem_univ
-/
lemma isBasis_preimage_isAffineOpen [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] :
  ...
  rw [← c.pt.basicOpen_res_eq _ (eqToHom h.symm).op]; rw [← CommRingCat.comp_apply]; rw [Scheme.Hom.app_eq_appLE]; rw [Scheme.Hom.appLE_map]; rw [← this]
  exact ⟨hxr, hrU⟩
...
@[stacks 01Z4 "(1)"]
/--
lemma `exists_preimage_eq` / 引理 `exists_preimage_eq`
...
Depends on / 依赖: D.map, Finite, IsCofiltered, IsCofiltered.inf_objs_exists, IsDominant, Set.Finite.mem_toFinset, Surjective, TopologicalSpac, attach, classical, denseRange, exists_finite_of_isCompact, f.surjective.denseRange, hsf.toFinset.attach.image, inf_objs_exists, isBasis_preimage_isAffineOpen, mem_toFinset, replace, surjective, toFinset
-/
lemma exists_preimage_eq
    [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    (U : c.pt.Opens) (hU : IsCompact (U : Set c.pt)) :
    exists (i : I) (V : (D.obj i).Opens), IsCompact (V : Set (D.obj i)) ∧ c.π.app i ⁻¹ᵁ V = U := by
  classical
  obtain ⟨s, hs, hsf, rfl⟩ := (isBasis_preimage_isAffineOpen D c hc).exists_finite_of_isCompact hU
  have : Finite s := hsf
  choose i V hV hVi using fun x : s => hs x.2
  ...
  replace fj : forall b : s, j ⟶ i b := fun b => (@fj (i b) (by simpa using ⟨b.1, b.2, rfl⟩)).some
...
include hc in
/--
lemma `isAffineHom_π_app` / 引理 `isAffineHom_π_app`

English:
lemma isAffineHom_π_app
  given: [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] (i : I)
  proof: have (j : _) : IsAffine ((opensDiagram D i U).obj j) := hU.preimage _
...
Depends on / 依赖: IsAffine, hU.preimage, opensDiagram, preimage
-/
lemma isAffineHom_π_app [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] (i : I) :
...
include hc in
/--
lemma `Scheme.compactSpace_of_isLimit` / 引理 `Scheme.compactSpace_of_isLimit`

English:
lemma Scheme.compactSpace_of_isLimit
  statement: [IsCofiltered I]
  proof: by
  obtain ⟨i⟩ := IsCofiltered.nonempty (C := I)
...
Depends on / 依赖: IsCofiltered, IsCofiltered.nonempty, QuasiCompact, QuasiCompact.compactSpace_of_compactSpace, compactSpace_of_compactSpace, nonempty
-/
lemma Scheme.compactSpace_of_isLimit [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] [forall i, CompactSpace (D.obj i)] :
...
variable [forall i, CompactSpace (D.obj i)] [LocallyOfFiniteType f] [IsCofiltered I]
...
private nonrec lemma Scheme.exists_hom_hom_comp_eq_comp_of_isAffine_of_locallyOfFiniteType
    [IsAffine S] [IsAffine X] [forall i, IsAffine (D.obj i)] [IsAffine c.pt]
    {i : I} (a : D.obj i ⟶ X) (ha : t.app i = a ≫ f)
    {j : I} (b : D.obj j ⟶ X) (hb : t.app j = b ≫ f)
    (hab : c.π.app i ≫ a = c.π.app j ≫ b) :
    exists (k : I) (hik : k ⟶ i) (hjk : k ⟶ j),
      D.map hik ≫ a = D.map hjk ≫ b := by
  wlog hS : exists R, S = Spec R generalizing S
  ...
  wlog hD : exists D' : I ⥤ CommRingCatᵒᵖ, D = D' ⋙ Scheme.Spec generalizing D
  ...
  obtain ⟨t, rfl⟩ : exists t' : (Functor.const Iᵒᵖ).obj R ⟶ D.leftOp,
...
/--
Definition of `ExistsHomHomCompEqCompAux` / `ExistsHomHomCompEqCompAux` 的定义

English:
structure ExistsHomHomCompEqCompAux
  parameters: where
  axioms and operations (12):
    ...
    - [h𝒰S : forall i, IsAffine (𝒰S.X i)]
    - 𝒰X((i : (𝒰S.pullback₁ f).I₀)) : Scheme.OpenCover.{u} ((𝒰S.pullback₁ f).X i)
    - [h𝒰X : forall i j, IsAffine ((𝒰X i).X j)]
...
variable {D t f c} [forall {i j : I} (f : i ⟶ j), IsAffineHom (D.map f)]
variable (A : ExistsHomHomCompEqCompAux D t f)

set_option backward.isDefEq.respectTransparency false in
omit [LocallyOfFiniteType f] in
/--
lemma `exists_index` / 引理 `exists_index`

English:
lemma exists_index
  statement: exists (i' : I) (hii' : i' ⟶ A.i),
  proof: by
...
Depends on / 依赖: A.ha.symm.trans, A.hb, A.hc, D.map, IsClosed, MapsTo, Pullback, Scheme, Scheme.Hom.continuous, Scheme.Pullback.diagonalCoverDiagonalRange, Scheme.Pullback.range_diago, Set.MapsTo, W.isOpen.isClosed_compl, continuous, diagonalCoverDiagonalRange, exists_mem_of_isClosed_of_nonempty, isClosed_compl, isCompact, isOpen, preimage
-/
lemma exists_index : exists (i' : I) (hii' : i' ⟶ A.i),
...
definition i'
  signature: : I
  body: A.exists_index.choose

中文:
定义 i'
  签名: : I
  定义体: A.exists_index.choose

Depends on / 依赖: A.exists_index.choose, exists_index
-/
def i' : I := A.exists_index.choose

/--
Definition of `hii'` / `hii'` 的定义

English:
definition hii'
  signature: : A.i' ⟶ A.i
  body: A.exists_index.choose_spec.choose

中文:
定义 hii'
  签名: : A.i' ⟶ A.i
  定义体: A.exists_index.choose_spec.choose

Depends on / 依赖: A.exists_index.choose_spec.choose, choose_spec, exists_index
-/
...
English:
definition g
  signature: : D.obj A.i' ⟶ pullback f f
  body: (D.map A.hii' ≫ pullback.lift A.a A.b (A.ha.symm.trans A.hb))

中文:
定义 g
  签名: : D.obj A.i' ⟶ pullback f f
  定义体: (D.map A.hii' ≫ pullback.lift A.a A.b (A.ha.symm.trans A.hb))

Depends on / 依赖: A.ha.symm.trans, A.hb, A.hii, D.map, pullback, pullback.lift
-/
...
omit [LocallyOfFiniteType f] in
/--
lemma `range_g_subset` / 引理 `range_g_subset`
...
Depends on / 依赖: A.exists_index.choose_spec.choose_spec, ExistsHomHomCompEqCompAux, ExistsHomHomCompEqCompAux.hii, choose_spec, exists_index
-/
lemma range_g_subset :
    Set.range A.g subseteq Scheme.Pullback.diagonalCoverDiagonalRange f A.𝒰S A.𝒰X := by
  simpa [ExistsHomHomCompEqCompAux.hii', g] using! A.exists_index.choose_spec.choose_spec

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
Definition of `𝒰D₀` / `𝒰D₀` 的定义

English:
definition 𝒰D₀
  signature: : Scheme.OpenCover.{u} (D.obj A.i')
  body: Scheme.Cover.mkOfCovers (Σ i : A.𝒰S.I₀, (A.𝒰X i).I₀) _
...
Depends on / 依赖: A.range_g_subset, Pullback, Scheme, Scheme.Cover.mkOfCovers, Scheme.Pullback.diagonalCover, Scheme.Pullback.diagonalCoverDiagonalRange, Scheme.Pullback.range_fst, Set.mem_range, diagonalCover, diagonalCoverDiagonalRange, mem_range, mkOfCovers, range_fst, range_g_subset
-/
...
definition 𝒰D
  signature: : Scheme.OpenCover.{u} (D.obj A.i')
  body: A.𝒰D₀.bind fun _ => Scheme.affineCover _
...
definition D'
  signature: (j : A.𝒰D.I₀)
  body: Over.post D ⋙ Over.pullback (A.𝒰D.f j) ⋙ Over.forget _

中文:
定义 D'
  签名: (j : A.𝒰D.I₀)
  定义体: Over.post D ⋙ Over.pullback (A.𝒰D.f j) ⋙ Over.forget _

Depends on / 依赖: Over.forget, Over.post, Over.pullback, forget, pullback
-/
...
definition c'
  signature: (j : A.𝒰D.I₀)
  body: (Over.pullback (A.𝒰D.f j) ⋙ Over.forget _).mapCone ((Over.conePost _ _).obj A.c)

中文:
定义 c'
  签名: (j : A.𝒰D.I₀)
  定义体: (Over.pullback (A.𝒰D.f j) ⋙ Over.forget _).mapCone ((Over.conePost _ _).obj A.c)

Depends on / 依赖: Over.conePost, Over.forget, Over.pullback, conePost, forget, mapCone, pullback
-/
...
/--
Definition of `hc'` / `hc'` 的定义

English:
definition hc'
  signature: (j : A.𝒰D.I₀)
  body: isLimitOfPreserves (Over.pullback (A.𝒰D.f j) ⋙ Over.forget _) (Over.isLimitConePost _ A.hc)

中文:
定义 hc'
  签名: (j : A.𝒰D.I₀)
  定义体: isLimitOfPreserves (Over.pullback (A.𝒰D.f j) ⋙ Over.forget _) (Over.isLimitConePost _ A.hc)

Depends on / 依赖: A.hc, Over.forget, Over.isLimitConePost, Over.pullback, forget, isLimitConePost, isLimitOfPreserves, pullback
-/
def hc' (j : A.𝒰D.I₀) : IsLimit (A.c' j) :=
  isLimitOfPreserves (Over.pullback (A.𝒰D.f j) ⋙ Over.forget _) (Over.isLimitConePost _ A.hc)

variable [forall i, IsAffineHom (A.c.π.app i)]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
lemma `exists_eq` / 引理 `exists_eq`

English:
lemma exists_eq
  given: (j : A.𝒰D.I₀)
  statement: exists (k : I) (hki' : k ⟶ A.i'),
  proof: by
...
Depends on / 依赖: IsAffine, LocallyOfFiniteType, Over.conePost, Over.forget, Over.post, Over.pullback, S.pullbackHom, Scheme, Scheme.Cover.pullbackHom, condit, conePost, forget, infer_instance, j.fst.fst, j.fst.snd, mapCone, pullback, pullback.condit, pullbackHom
-/
lemma exists_eq (j : A.𝒰D.I₀) : exists (k : I) (hki' : k ⟶ A.i'),
  ...
  obtain ⟨k, hik, hjk, H⟩ := Scheme.exists_hom_hom_comp_eq_comp_of_isAffine_of_locallyOfFiniteType
    ...
    (by
      rw [← cancel_mono ((A.𝒰X j.1.1).f j.1.2)]; rw [← cancel_mono (pullback.fst f (A.𝒰S.f j.1.1))]
...
variable [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
include hc in
/--
lemma `Scheme.exists_hom_comp_eq_comp_of_locallyOfFiniteType` / 引理 `Scheme.exists_hom_comp_eq_comp_of_locallyOfFiniteType`
...
Depends on / 依赖: A.exists_eq, A.hii, D.finiteSubcover, D.obj, ExistsHomHomCompEqCompAux, Pullback, S.affineCover, Scheme, Scheme.Pullback.diagonalCover, Scheme.Pullback.diagonalCoverDiagonalRange, Scheme.affineCover, affineCover, classical, diagonalCover, diagonalCoverDiagonalRange, exists_eq, finiteSubcover, hom_ext, isEmpty_or_nonempty, isInitialOfIsEmpty
-/
lemma Scheme.exists_hom_comp_eq_comp_of_locallyOfFiniteType
    {i : I} (a b : D.obj i ⟶ X) (ha : t.app i = a ≫ f) (hb : t.app i = b ≫ f)
    (hab : c.π.app i ≫ a = c.π.app i ≫ b) :
    exists (k : I) (hik : k ⟶ i), D.map hik ≫ a = D.map hik ≫ b := by
  classical
  have := isAffineHom_π_app _ _ hc
  let A : ExistsHomHomCompEqCompAux D t f :=
    { c := c, hc := hc, i := i, a := a, ha := ha, b := b, hb := hb, hab := hab
  ...
  let O : Finset I := {A.i'} union Finset.univ.image (fun i : 𝒰Df.I₀ => k <| A.𝒰D.idx i.1)
  let o := Nonempty.some (inferInstance : Nonempty 𝒰Df.I₀)
  have ho : k (A.𝒰D.idx o.1) in O := by
    simp [O]
...
⟨k A.𝒰D.idx i.1, A.i', by simp [O], by simp [O], hki' A.𝒰D.idx i.1⟩))
  have (w v : 𝒰Df.I₀) :
    ...
    trans hl1 (show A.i' in O by simp [O])
...
· exact .symm hl2 _ _ (Finset.mem_image_of_mem _ (by simp))
  ...
  rw [← reassoc_of% hF]; rw [← reassoc_of% hF]; rw [heq]
...
/--
lemma `Scheme.exists_hom_hom_comp_eq_comp_of_locallyOfFiniteType` / 引理 `Scheme.exists_hom_hom_comp_eq_comp_of_locallyOfFiniteType`
...
Depends on / 依赖: D.map, IsCofiltered, IsCofiltered.min, IsCofiltered.minToLeft, IsCofiltered.minToRight, Scheme, Scheme.exists_hom_comp_eq_comp_of_locallyOfFiniteType, exists_hom_comp_eq_comp_of_locallyOfFiniteType, minToLeft, minToRight
-/
lemma Scheme.exists_hom_hom_comp_eq_comp_of_locallyOfFiniteType
    {i : I} (a : D.obj i ⟶ X) (ha : t.app i = a ≫ f)
    {j : I} (b : D.obj j ⟶ X) (hb : t.app j = b ≫ f)
...
include hc in
/--
lemma `exists_appTop_map_eq_zero_of_isAffine_of_isLimit` / 引理 `exists_appTop_map_eq_zero_of_isAffine_of_isLimit`
...
Depends on / 依赖: D.obj, D.op.obj, FilteredColimit, IsAffine, Scheme, Types.FilteredColimit.isColimit_eq_iff, f.unop, forget, hc.op, infer_instance, isColimitOfPreserves, isColimit_eq_iff, j.unop
-/
lemma exists_appTop_map_eq_zero_of_isAffine_of_isLimit
    [forall i, IsAffine (D.obj i)]
    (i : I) (s : Γ(D.obj i, ⊤)) (hs : (c.π.app i).appTop s = 0) :
    exists (j : I) (f : j ⟶ i), (D.map f).appTop s = 0 := by
  have : forall i, IsAffine (D.op.obj i).unop := by dsimp; infer_instance
  obtain ⟨j, f, hj⟩ := (Types.FilteredColimit.isColimit_eq_iff'
    (isColimitOfPreserves (Scheme.Γ ⋙ forget _) hc.op) s (0 : Γ(D.obj i, ⊤))).mp
    (by dsimp at hs ⊢; simpa)
...
include hc in
/--
lemma `exists_appTop_map_eq_zero_of_isLimit` / 引理 `exists_appTop_map_eq_zero_of_isLimit`

English:
lemma exists_appTop_map_eq_zero_of_isLimit
  statement: [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
  proof: by
  classical
  have (x : D.obj i) : exists (U : (D.obj i).Opens) (hU : IsAffineOpen U)
      (hU : x in U) (j : I) (f : j ⟶ i), (D.map f).app U (s |_ U) = 0 := by
...
Depends on / 依赖: D.map, D.obj, IsAffine, IsAffineOpen, IsClosedImmersion, MorphismProperty, MorphismProperty.of_isPullback, Set.mem_univ, UniversallyClosed, classical, exists_appTop_map_eq_zero_of_isAffine_of_isLimit, exists_subset_of_mem_open, hU.preimage, hf.flip, isBasis_affineOpens, isBasis_affineOpens.exists_subset_of_mem_open, isClosedEmbedding, isClosedEmbedding.isClosedMap, isClosedMap, isLimitOpensCone
-/
lemma exists_appTop_map_eq_zero_of_isLimit [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
  ...
  have h₂ : D.map (fk (Finset.mem_insert_self _ _)) ⁻¹ᵁ U l <= D.map (fk (Finset.mem_insert_of_mem
    ...
    rw [← Scheme.Hom.comp_preimage]; rw [← D.map_comp]; rw [h₁]
...
include hc in
/--
lemma `exists_app_map_eq_zero_of_isLimit` / 引理 `exists_app_map_eq_zero_of_isLimit`
...
Depends on / 依赖: D.obj
-/
lemma exists_app_map_eq_zero_of_isLimit [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    {i : I} {U : (D.obj i).Opens} (hU : IsCompact (X := D.obj i) U) (s : Γ(D.obj i, U))
    (hs : (c.π.app i).app U s = 0) :
    exists (j : I) (f : j ⟶ i), (D.map f).app U s = 0 := by
  have : CompactSpace ↥((opensDiagram D i U).obj (Over.mk (𝟙 i))) :=
    isCompact_iff_compactSpace.mp (by simpa)
  have H : (D.map (𝟙 _) ⁻¹ᵁ U).ι ''ᵁ ⊤ <= U := by simp
  obtain ⟨j, f, hf⟩ := exists_appTop_map_eq_zero_of_isLimit _ _
    ...
    ((D.obj i).presheaf.map (homOfLE (show (D.map (𝟙 _) ⁻¹ᵁ U).ι ''ᵁ ⊤ <= U by simp)).op s) (by
      rw [← map_zero (c.pt.presheaf.map (homOfLE
        (show (c.π.app i ⁻¹ᵁ U).ι ''ᵁ ⊤ <= c.π.app i ⁻¹ᵁ U by simp)).op).hom]; rw [← hs]
...
include hc in
/--
lemma `exists_app_map_eq_map_of_isLimit` / 引理 `exists_app_map_eq_map_of_isLimit`
...
Depends on / 依赖: D.obj
-/
lemma exists_app_map_eq_map_of_isLimit [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    {i : I} {U : (D.obj i).Opens} (hU : IsCompact (X := D.obj i) U) (s t : Γ(D.obj i, U))
    (hs : (c.π.app i).app U s = (c.π.app i).app U t) :
    exists (j : I) (f : j ⟶ i), (D.map f).app U s = (D.map f).app U t := by
  simpa [sub_eq_zero] using exists_app_map_eq_zero_of_isLimit _ _ hc hU (s - t)
    (by simpa +instances [map_sub, sub_eq_zero])
...
include hc in
/--
lemma `exists_appTop_π_eq_of_isAffine_of_isLimit` / 引理 `exists_appTop_π_eq_of_isAffine_of_isLimit`
...
Depends on / 依赖: D.op.obj, IsAffine, Scheme, Types.jointly_surjective_of_isColimit, choose_spec, forget, hc.op, infer_instance, isColimitOfPreserves, jointly_surjective_of_isColimit
-/
lemma exists_appTop_π_eq_of_isAffine_of_isLimit
    [forall i, IsAffine (D.obj i)] (s : Γ(c.pt, ⊤)) :
    exists (i : I) (t : Γ(D.obj i, ⊤)), (c.π.app i).appTop t = s := by
  have : forall i, IsAffine (D.op.obj i).unop := by dsimp; infer_instance
  exact ⟨_, (Types.jointly_surjective_of_isColimit
    (isColimitOfPreserves (Scheme.Γ ⋙ forget _) hc.op) s).choose_spec⟩
...
include hc in
/--
lemma `exists_appTop_π_eq_of_isLimit` / 引理 `exists_appTop_π_eq_of_isLimit`

English:
lemma exists_appTop_π_eq_of_isLimit
  statement: [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
  proof: by
  classical
  have := Scheme.compactSpace_of_isLimit _ _ hc
  have (x : c.pt) : exists (i : I) (U : (D.obj i).Opens) (hU : IsAffineOpen U)
      (hU : (c.π.app i).base x in U) (t : Γ(D.obj i, U)), (c.π.app i).app U t = s |_ _ := by
...
Depends on / 依赖: D.obj, IsAffine, IsAffineOpen, IsCofiltered, IsCofiltered.nonempty, Scheme, Scheme.compactSpace_of_isLimit, Set.mem_univ, c.pt, classical, compactSpace_of_isLimit, exists_subset_of_mem_open, hU.preimage, isBasis_affineOpens, isBasis_affineOpens.exists_subset_of_mem_open, isOpen_univ, mem_univ, nonempty, opensDiagram, preimage
-/
lemma exists_appTop_π_eq_of_isLimit [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    (s : Γ(c.pt, ⊤)) [forall i, CompactSpace (D.obj i)] [forall i, QuasiSeparatedSpace (D.obj i)] :
    exists (i : I) (t : Γ(D.obj i, ⊤)), s = (c.π.app i).appTop t := by
  classical
  have := Scheme.compactSpace_of_isLimit _ _ hc
  have (x : c.pt) : exists (i : I) (U : (D.obj i).Opens) (hU : IsAffineOpen U)
      (hU : (c.π.app i).base x in U) (t : Γ(D.obj i, U)), (c.π.app i).app U t = s |_ _ := by
    ...
    · suffices (c.π.app j.1 ≫ D.map j.hom).base x in U by simpa [-Cone.w] using this
      rwa [Cone.w]
  ...
  dsimp at ht
  have (x y : c.pt) : exists (j : I) (fjx : j ⟶ i x) (fjy : j ⟶ i y),
  ...
  let S : Finset _ := σ.image i union Finset.image₂ j σ σ
  have hiS {x} (hx : x in σ) : i x in S := Finset.subset_union_left (Finset.mem_image_of_mem i hx)
  have hjS {x y} (hx : x in σ) (hy : y in σ) : j x y in S :=
    Finset.subset_union_right (Finset.mem_image₂_of_mem hx hy)
  obtain ⟨k, fk, hk⟩ := IsCofiltered.inf_exists S
    (σ.attach.image₂ (fun (x y : σ) => ⟨j x.1 y.1, i x.1, hjS x.2 y.2, hiS x.2, fjx x y⟩) σ.attach union
    σ.attach.image₂ (fun (x y : σ) => ⟨j x.1 y.1, i y.1, hjS x.2 y.2, hiS y.2, fjy x y⟩) σ.attach)
  have hk₁ {x y} (hx : x in σ) (hy : y in σ) := hk (hjS hx hy) (hiS hx) (f := fjx x y)
    (Finset.subset_union_left (Finset.mem_image₂.mpr ⟨⟨x, hx⟩, by simp, ⟨y, hy⟩, by simp, rfl⟩))
  have hk₂ {x y} (hx : x in σ) (hy : y in σ) := hk (hjS hx hy) (hiS hy) (f := fjy x y)
    (Finset.subset_union_right (Finset.mem_image₂.mpr ⟨⟨x, hx⟩, by simp, ⟨y, hy⟩, by simp, rfl⟩))
  obtain ⟨k', fk'k, hk'⟩ := exists_map_eq_top D c hc
    (⨆ (x : _) (hx : x in σ), D.map (fk (hiS hx)) ⁻¹ᵁ U x) (by
  ...
  have := ((Presheaf.isSheaf_iff_isSheaf_forget _ _ (forget _)).mp (D.obj k').IsSheaf).isSheafFor
    ...
    (fun x _ => by
      obtain ⟨ix, hix, h⟩ : exists ix, exists (h : ix in σ), (D.map (fk'k ≫ fk (hiS h))).base x in U ix := by
        simpa using hk'.ge (Set.mem_univ x)
      refine ⟨D.map (fk'k ≫ fk (hiS hix)) ⁻¹ᵁ U ix, homOfLE le_top,
        Sieve.ofArrows_mk (I := σ) _ _ ⟨ix, hix⟩, h⟩)
  rw [← Presieve.isSheafFor_iff_generate]; rw [Presieve.isSheafFor_arrows_iff] at this
  obtain ⟨t₀, ht₀, -⟩ := this (fun x => (D.map _).app _ (t x)) fun x y V fVx fVy _ => by
    have H : V <= D.map (fk'k ≫ fk (hjS x.2 y.2)) ⁻¹ᵁ
      ...
      rw [← Functor.map_comp]; rw [← Functor.map_comp]; rw [Category.assoc]; rw [Category.assoc]; rw [hk₁ x.2 y.2]; rw [hk₂ x.2 y.2]; rw [le_inf_iff]
      exact ⟨fVx.le, fVy.le⟩
    convert!
 ...
 (hj x y)) using 1
  ...
  have H : c.π.app (i (σi y)) ⁻¹ᵁ U (σi y) <=
    ...
    rw [← Scheme.Hom.comp_preimage]; rw [Cone.w]
  ...
  · dsimp [Scheme.Opens.toScheme_presheaf_obj]
    rw [← ConcreteCategory.comp_apply]; rw [← ConcreteCategory.comp_apply]; rw [← ConcreteCategory.comp_apply]
    congr 2
    simp [Scheme.Hom.app_eq_appLE]

include hc in
/--
lemma `nonempty_isColimit_Γ_mapCocone` / 引理 `nonempty_isColimit_Γ_mapCocone`
...
Depends on / 依赖: CommRingCat, FilteredColimit, Quiver, Quiver.Hom.op, ReflectsColimit, ReflectsColimit.reflects, ReflectsFilteredColimits, Types.FilteredColimit.isColimitOf, choose_spec, choose_spec.choose_spec, exists_app_map_eq_map_of_isLimit, forget, isColimitOf, isCompact_univ, reflects, reflectsColimitsOfShape_of_reflectsIsomorphisms
-/
lemma nonempty_isColimit_Γ_mapCocone [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    [forall i, CompactSpace (D.obj i)] [forall i, QuasiSeparatedSpace (D.obj i)] :
...
English:
instance [forall
  signature: {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
  body: have : PreservesColimit D.op Scheme.Γ := ⟨fun hc => nonempty_isColimit_Γ_mapCocone D _ hc.unop⟩
...
Depends on / 依赖: D.op, PreservesColimit, Scheme, hc.unop, preservesLimit_rightOp
-/
instance [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    [forall i, CompactSpace (D.obj i)] [forall i, QuasiSeparatedSpace (D.obj i)] :
...
@[stacks 01Z5]
/--
lemma `Scheme.exists_isQuasiAffine_of_isLimit` / 引理 `Scheme.exists_isQuasiAffine_of_isLimit`

English:
lemma Scheme.exists_isQuasiAffine_of_isLimit
  statement: [IsCofiltered I]
  proof: by
  classical
  have (x : c.pt) : exists (i : I) (f : Γ(D.obj i, ⊤)),
      IsAffineOpen (Scheme.basicOpen _ f) ∧ c.π.app i x in (Scheme.basicOpen _ f) := by
    obtain ⟨i⟩ := IsCofiltered.nonempty (C := I)
...
Depends on / 依赖: D.obj, IsAffineOpen, IsCofiltered, IsCofiltered.nonempty, IsQuasiAffine, IsQuasiAffine.isBasis_basicOpen, Scheme, Scheme.basicOpen, Set.mem_univ, basicOpen, c.pt, classical, exists_ap, exists_subset_of_mem_open, isBasis_affineOpens, isBasis_affineOpens.exists_subset_of_mem_open, isBasis_basicOpen, isOpen, isOpen_univ, mem_univ
-/
lemma Scheme.exists_isQuasiAffine_of_isLimit [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    [forall (i : I), CompactSpace (D.obj i)]
    [forall (i : I), QuasiSeparatedSpace (D.obj i)]
    [IsQuasiAffine c.pt] :
    exists i, IsQuasiAffine (D.obj i) := by
  classical
  have (x : c.pt) : exists (i : I) (f : Γ(D.obj i, ⊤)),
      IsAffineOpen (Scheme.basicOpen _ f) ∧ c.π.app i x in (Scheme.basicOpen _ f) := by
    obtain ⟨i⟩ := IsCofiltered.nonempty (C := I)
    ...
    · change x in c.π.app l ⁻¹ᵁ (D.obj l).basicOpen _
  ...
  obtain ⟨k, fkj, hk⟩ := exists_map_eq_top D c hc
    (⨆ k, D.map (fj _ (Finset.mem_image_of_mem i (hσiσ k))) ⁻¹ᵁ (D.obj (i _)).basicOpen (f _)) (by
      ...
      change (c.π.app j ≫ D.map _).base x in (D.obj (i (σi x))).basicOpen (f (σi x))
      rw [Cone.w]
      exact hσi _)
  ...
  rw [← Scheme.preimage_basicOpen_top]; rw [← Scheme.preimage_basicOpen_top]
...
@[stacks 01Z6]
/--
lemma `Scheme.exists_isAffine_of_isLimit` / 引理 `Scheme.exists_isAffine_of_isLimit`

English:
lemma Scheme.exists_isAffine_of_isLimit
  statement: [IsCofiltered I]
  proof: by
  have := isAffineHom_π_app _ _ hc
  obtain ⟨i, hi⟩ := Scheme.exists_isQuasiAffine_of_isLimit D c hc
  have : forall {i j : I} (f : i ⟶ j), IsAffineHom ((D ⋙ Γ.rightOp ⋙ Scheme.Spec).map f) := by
    dsimp; infer_instance
...
Depends on / 依赖: CompactSpace, D.obj, IsAffineHom, Scheme, Scheme.Spec, Scheme.exists_isQuasiAffine_of_isLimit, c.pt, exists_isQuasiAffine_of_isLimit, exists_map_eq_top, infer_instance, isLimitOfPreserves, opensRange, rightOp
-/
...
@[stacks 01Z4 "(1)"]
/--
lemma `exists_isAffineOpen_preimage_eq` / 引理 `exists_isAffineOpen_preimage_eq`
...
Depends on / 依赖: CompactSpace, D.map, IsAffine, QuasiCompact, QuasiCompact.isCompact_preimage, QuasiSeparatedSpace, Scheme, Scheme.exists_isAffine_of_isLimit, exists_isAffine_of_isLimit, exists_preimage_eq, hU.isCompact, isCompact, isCompact_iff_compactSpace, isCompact_iff_compactSpace.mp, isCompact_preimage, isLimitOpensCone, isQuasiSeparated_iff_quasiSeparatedSpace, of_quasiSeparatedSpace, opensCone, opensDiagram
-/
lemma exists_isAffineOpen_preimage_eq
    [IsCofiltered I] [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    [forall i, QuasiSeparatedSpace (D.obj i)]
    (U : c.pt.Opens) (hU : IsAffineOpen U) :
    exists (i : I) (V : (D.obj i).Opens), IsAffineOpen V ∧ c.π.app i ⁻¹ᵁ V = U := by
  obtain ⟨i, U, hU', rfl⟩ := exists_preimage_eq D c hc U hU.isCompact
  have (j : Over i) : CompactSpace ((opensDiagram D i U).obj j) :=
    isCompact_iff_compactSpace.mp (QuasiCompact.isCompact_preimage _ U.2 hU')
  have (j : Over i) : QuasiSeparatedSpace ((opensDiagram D i U).obj j) :=
...
include hc in
/--
lemma `Scheme.exists_isOpenCover_and_isAffine_of_finite` / 引理 `Scheme.exists_isOpenCover_and_isAffine_of_finite`
...
lemma Scheme.exists_isOpenCover_and_isAffine_of_finite
  ...
  replace fi : forall k, i ⟶ j k := fun k => (fi (by simp)).some
...
Depends on / 依赖: D.map, Finset, Finset.univ.image, Hom.comp_preimage, Hom.preimage_iSup, IsCofiltered, IsCofiltered.inf_objs_exists, classical, comp_preimage, exists_isAffineOpen_preimage_eq, exists_map_eq_top, inf_objs_exists, nonempty_fintype, preimage, preimage_iSup, replace, simp_rw
-/
lemma Scheme.exists_isOpenCover_and_isAffine_of_finite [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] [forall (i : I), CompactSpace (D.obj i)]
    [forall (i : I), QuasiSeparatedSpace (D.obj i)]
    {J : Type*} [Finite J] (U : J -> c.pt.Opens) (hU : IsOpenCover U)
    (hU' : forall i, IsAffineOpen (U i)) :
    exists (i : I) (V : J -> (D.obj i).Opens),
      IsOpenCover V ∧ forall j, IsAffineOpen (V j) ∧ U j = c.π.app i ⁻¹ᵁ (V j) := by
  classical
  choose j V hV hVU using fun k => exists_isAffineOpen_preimage_eq D c hc (U k) (hU' k)
  cases nonempty_fintype J
  obtain ⟨i, fi⟩ := IsCofiltered.inf_objs_exists (Finset.univ.image j)
...
include hc in
/--
lemma `Scheme.exists_isOpenCover_and_isAffine` / 引理 `Scheme.exists_isOpenCover_and_isAffine`

English:
lemma Scheme.exists_isOpenCover_and_isAffine
  statement: [IsCofiltered I]
  proof: by
...
Depends on / 依赖: IsOpenCover, IsZariskiLocalAtTarget, IsZariskiLocalAtTarget.restrict, Opens.carrier_eq_coe, Opens.coe_mk, Opens.coe_top, Opens.iSup_mk, Scheme, Scheme.exists_isOpenCover_and_isAffine_of_finite, Set.iUnion_subtype, SetLike, SetLike.coe_subset_coe, carrier_eq_coe, coe_mk, coe_subset_coe, coe_top, compactSpace_of_isLimit, elim_finite_subcover, eq_top_iff, exists_isOpenCover_and_isAffine_of_finite
-/
lemma Scheme.exists_isOpenCover_and_isAffine [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)]
    [forall (i : I), CompactSpace (D.obj i)]
    [forall (i : I), QuasiSeparatedSpace (D.obj i)]
    {J : Type*} (U : J -> c.pt.Opens) (hU : IsOpenCover U) (hU' : forall i, IsAffineOpen (U i)) :
    exists (i : I) (s : Finset J) (V : s -> (D.obj i).Opens),
      IsOpenCover V ∧ forall j, IsAffineOpen (V j) ∧ U j = c.π.app i ⁻¹ᵁ (V j) := by
  have := compactSpace_of_isLimit D c hc
  obtain ⟨s, hs⟩ := isCompact_univ.elim_finite_subcover _
    (fun i => (U i).isOpen) hU.iSup_set_eq_univ.ge
...
include hc in
/--
lemma `Scheme.OpenCover.exists_of_isCofiltered_of_finite` / 引理 `Scheme.OpenCover.exists_of_isCofiltered_of_finite`
...
Depends on / 依赖: IsAffineOpen, IsAffineOpen.isoSpec_inv_, IsAffineOpen.range_fromSpec, IsOpenImmersion, IsOpenImmersion.lift, Scheme, Scheme.exists_isOpenCover_and_isAffine_of_finite, SetLike, SetLike.mem_coe, carrier, exists_isOpenCover_and_isAffine_of_finite, exists_mem, hV.exists_mem, isAffineOpen_opensRange, isOpenCover_opensRange, isoSpec, left.isoSpec.inv, mem_coe, ofArrows_mem_precoverage_iff, range_fromSpec
-/
lemma Scheme.OpenCover.exists_of_isCofiltered_of_finite [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f)] [forall (i : I), CompactSpace (D.obj i)]
    [forall (i : I), QuasiSeparatedSpace (D.obj i)]
    (𝒰 : OpenCover.{w} c.pt) [forall i, IsAffine (𝒰.X i)] [Finite 𝒰.I₀] :
    exists (i : I) (R : 𝒰.I₀ -> CommRingCat.{u}) (f : forall (a : 𝒰.I₀), Spec (R a) ⟶ (D.obj i))
      (_ : Presieve.ofArrows _ f in zariskiPrecoverage _) (g : forall (j : 𝒰.I₀), 𝒰.X j ⟶ Spec (R j)),
      forall (j : 𝒰.I₀), IsPullback (g j) (𝒰.f j) (f j) (c.π.app i) := by
  obtain ⟨i, V, hV, hV'⟩ := Scheme.exists_isOpenCover_and_isAffine_of_finite _ _ hc _
    𝒰.isOpenCover_opensRange fun k => isAffineOpen_opensRange (𝒰.f k)
  have hV'' (k) := dsimp% congr($((hV' k).right).carrier)
...
private nonrec lemma Scheme.exists_π_app_comp_eq_of_locallyOfFinitePresentation_of_isAffine
    ...
    [IsAffine S] [IsAffine X] [forall i, IsAffine (D.obj i)]
    (a : c.pt ⟶ X) (ha : c.π ≫ t = (Functor.const _).map (a ≫ f)) :
    exists (i : I) (g : D.obj i ⟶ X), c.π.app i ≫ g = a ∧ g ≫ f = t.app i := by
  ...
  obtain ⟨t, rfl⟩ : exists t' : (Functor.const Iᵒᵖ).obj R ⟶ D.leftOp,
    ...
    ⟨⟨fun i => Spec.preimage (t.app i.unop), fun _ _ f => Spec.map_injective
      (by simpa using! (t.naturality f.unop).symm)⟩, by ext : 2; simp [e]⟩
  wlog hc' : exists c' : Cocone D.leftOp, c = Scheme.Spec.mapCone (coneOfCoconeLeftOp c') generalizing c
  · have inst : IsAffine c.pt := isAffine_of_isLimit _ hc
    ...
    have inst : forall i, IsAffine ((D ⋙ Scheme.Spec).op.obj i).unop := by dsimp; infer_instance
...
include hc in
/--
lemma `Scheme.exists_π_app_comp_eq_of_locallyOfFinitePresentation` / 引理 `Scheme.exists_π_app_comp_eq_of_locallyOfFinitePresentation`
...
lemma Scheme.exists_π_app_comp_eq_of_locallyOfFinitePresentation
  ...
  obtain ⟨i', fi'i, hi'⟩ : exists (i' : I) (fi'i : i' ⟶ i),
      forall j, D.map fi'i ⁻¹ᵁ 𝒱 j <= t.app i' ⁻¹ᵁ j.1.1.2.1.2.1 := by
    choose k fk hk using fun j => exists_map_preimage_le_map_preimage D c hc (h𝒱𝒰 j).1.isCompact
      (V := t.app i ⁻¹ᵁ j.1.1.2.1.2.1) (by
      rw [← Hom.comp_preimage]; rw [← NatTrans.comp_app]; rw [ha]
    ...
    rw [← hfi']; rw [Functor.map_comp]; rw [Hom.comp_preimage]
  ...
  have : exists k, exists (fk : k ⟶ i), forall j, exists (ak : ↑(D.map fk ⁻¹ᵁ 𝒱 j) ⟶ X),
      ak ≫ f = Opens.ι _ ≫ t.app k ∧ c.π.app _ ∣_ _ ≫ ak = Opens.ι _ ≫ a := by
    let 𝒱' := (D.map fi'i ⁻¹ᵁ 𝒱 ·)
    have h𝒱'𝒰 (j : s) : c.π.app i' ⁻¹ᵁ 𝒱' j = j.1.1.1 := by
      rw [← Hom.comp_preimage]; rw [c.w fi'i]; exact (h𝒱𝒰 j).2.symm
...
Scheme.Hom.resLE_comp_resLE_assoc] using! congr(Scheme.homOfLE _ e ≫ (hk j) ≫ Opens.ι _)
  ...
  obtain ⟨l, flk, hl⟩ : exists (l : I) (flk : l ⟶ k), forall j₁ j₂, Scheme.homOfLE _ inf_le_left ≫
      D.map flk ∣_ _ ≫ ak j₁ = Scheme.homOfLE _ inf_le_right ≫ D.map flk ∣_ _ ≫ ak j₂ := by
    let 𝒱' := (D.map fki ⁻¹ᵁ 𝒱 ·)
    have (j₁ j₂ : s) : exists (l : I) (flk : l ⟶ k), Scheme.homOfLE _ inf_le_left ≫ D.map flk ∣_ _ ≫
...
set_option backward.isDefEq.respectTransparency false in
/--
Instance `Scheme.preservesColimit_yoneda` / 实例 `Scheme.preservesColimit_yoneda`

English:
instance Scheme.preservesColimit_yoneda
  signature: (D : I ⥤ Over S) [IsCofiltered I]
  body: by
...
Depends on / 依赖: CoconeTypes, CompactSpace, Functor, Functor.CoconeTypes.descColimitType_injective_iff_of_isFiltered, IsAffineHom, Limits, Limits.Types.isColimit_iff_coconeTypesIsColimit, Over.forget, QuasiCompact, QuasiSeparatedSpace, Scheme, Scheme.exists_hom_comp_eq_comp_of_locally, UniversallyClosed, descColimitType_injective_iff_of_isFiltered, exists_hom_comp_eq_comp_of_locally, forget, infer_instance, isColimit_iff_coconeTypesIsColimit
-/
instance Scheme.preservesColimit_yoneda (D : I ⥤ Over S) [IsCofiltered I]
    [forall {i j} (f : i ⟶ j), IsAffineHom (D.map f).left]
    [forall (i : I), CompactSpace (D.obj i).left] [forall (i : I), QuasiSeparatedSpace (D.obj i).left]
