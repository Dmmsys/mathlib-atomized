...
variable [SeminormedAddCommGroup E] [NormedSpace Real E] [PseudoMetricSpace P] [NormedAddTorsor E P]
variable {s : Set E}

/--
theorem `Wbtw.dist_add_dist` / 定理 `Wbtw.dist_add_dist`

English:
theorem Wbtw.dist_add_dist
  given: {x y z : P} (h : Wbtw Real x y z)
  proof: by
  obtain ⟨a, ⟨ha₀, ha₁⟩, rfl⟩ := h
  simp [abs_of_nonneg, ha₀, ha₁, sub_mul]

中文:
定理 Wbtw.dist_add_dist
  条件: {x y z : P} (h : Wbtw 实数 x y z)
  证明: by
  obtain ⟨a, ⟨ha₀, ha₁⟩, rfl⟩ := h
  simp [abs_of_nonneg, ha₀, ha₁, sub_mul]

Depends on / 依赖: abs_of_nonneg, sub_mul
-/
theorem Wbtw.dist_add_dist {x y z : P} (h : Wbtw Real x y z) :
    dist x y + dist y z = dist x z := by
  obtain ⟨a, ⟨ha₀, ha₁⟩, rfl⟩ := h
  simp [abs_of_nonneg, ha₀, ha₁, sub_mul]

/--
theorem `dist_add_dist_of_mem_segment` / 定理 `dist_add_dist_of_mem_segment`

English:
theorem dist_add_dist_of_mem_segment
  given: {x y z : E} (h : y in [x -[Real] z])
  proof: (mem_segment_iff_wbtw.1 h).dist_add_dist

中文:
定理 dist_add_dist_of_mem_segment
  条件: {x y z : E} (h : y in [x -[实数] z])
  证明: (mem_segment_iff_wbtw.1 h).dist_add_dist

Depends on / 依赖: dist_add_dist, mem_segment_iff_wbtw
-/
theorem dist_add_dist_of_mem_segment {x y z : E} (h : y in [x -[Real] z]) :
...
variable [NormedAddCommGroup E] [NormedSpace Real E] [FiniteDimensional Real E] {s t : Set E} {x : E}

/--
lemma `exists_mem_interior_convexHull_affineBasis` / 引理 `exists_mem_interior_convexHull_affineBasis`

English:
lemma exists_mem_interior_convexHull_affineBasis
  given: (hs : s in 𝓝 x)
  proof: by
...
lemma exists_mem_interior_convexHull_affineBasis (hs : s in 𝓝 x) :
    exists b : AffineBasis (Fin (finrank Real E + 1)) Real E,
      x in interior (convexHull Real (range b)) ∧ convexHull Real (range b) subseteq s := by
  -- By translating, WLOG `x` is the origin.
  wlog hx : x = 0
  · obtain ⟨b, hb⟩ := this (s := -x +ᵥ s) (by simpa using vadd_mem_nhds_vadd (-x) hs) rfl
    use x +ᵥ b
...
/--
theorem `Convex.exists_subset_interior_convexHull_finset_of_isCompact` / 定理 `Convex.exists_subset_interior_convexHull_finset_of_isCompact`
...
-/
theorem Convex.exists_subset_interior_convexHull_finset_of_isCompact
    (hs₁ : Convex Real s) (hs₂ : IsCompact s) (ht : t in 𝓝ˢ s) :
    exists u : Finset E, s subseteq interior (convexHull Real u) ∧ convexHull Real u subseteq t := by
  classical
  rcases mem_nhdsSet_iff_exists.1 ht with ⟨U, hU₁, hU₂, hU₃⟩
  rcases compact_open_separated_add_left hs₂ hU₁ hU₂ with ⟨V, hV₁, hV₂⟩
  rcases exists_mem_interior_convexHull_affineBasis hV₁ with ⟨b, hb₁, hb₂⟩
