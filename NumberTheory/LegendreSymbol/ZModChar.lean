...
@[simps]
/-
**ZMod.** 是 Mathlib 中的一个定义，位于命名空间 `ZMod`。
...
/-
**ZMod.isQuadratic_** 是 Mathlib 中的一个定理，位于命名空间 `ZMod`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**ZMod.** 是 Mathlib 中的一个定理，位于命名空间 `ZMod`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**ZMod.neg_one_pow_div_two_of_one_mod_four** 是 Mathlib 中的一个定理，位于命名空间 `ZMod`。
形式化陈述：neg_one_pow_div_two_of_one_mod_four {n : Nat} (hn : n % 4 = 1) : (-1 : Int
) ^ (n / 2) = 1
参数：hn : n % 4 = 1。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**ZMod.neg_one_pow_div_two_of_three_mod_four** 是 Mathlib 中的一个定理，位于命名空间 `ZMod`。
形式化陈述：neg_one_pow_div_two_of_three_mod_four {n : Nat} (hn : n % 4 = 3) : (-1 : I
nt) ^ (n / 2) = -1
参数：hn : n % 4 = 3。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
