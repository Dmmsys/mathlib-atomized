...
/--
theorem `exists_homeomorph_extension` / 定理 `exists_homeomorph_extension`
...
theorem exists_homeomorph_extension {E : Type*} [NormedAddCommGroup E] [NormedSpace Real E]
    ...
    {f : E -> F} {f' : E ≃L[Real] F} {c : Real>=0} (hf : ApproximatesLinearOn f (f' : E ->L[Real] F) s c)
    (hc : Subsingleton E ∨ lipschitzExtensionConstant F * c < ‖(f'.symm : F ->L[Real] E)‖₊⁻¹) :
    exists g : E ≃ₜ F, EqOn f g s := by
  -- the difference `f - f'` is Lipschitz on `s`. It can be extended to a Lipschitz function `u`
  -- on the whole space, with a slightly worse Lipschitz constant. Then `f' + u` will be the
  -- desired homeomorphism.
  obtain ⟨u, hu, uf⟩ :
