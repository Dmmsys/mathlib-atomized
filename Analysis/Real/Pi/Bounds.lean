...
/--
theorem `pi_gt_sqrtTwoAddSeries` / 定理 `pi_gt_sqrtTwoAddSeries`

English:
theorem pi_gt_sqrtTwoAddSeries
  given: (n : Nat)
  statement: 2 ^ (n + 1) * √(2 - sqrtTwoAddSeries 0 n) < π
  proof: by
  have : √(2 - sqrtTwoAddSeries 0 n) / 2 * 2 ^ (n + 2) < π := by
    rw [← lt_div_iff₀]; rw [← sin_pi_over_two_pow_succ]
    focus
      apply sin_lt
...
定理 pi_gt_sqrtTwoAddSeries
  条件: (n : 自然数)
  结论: 2 ^ (n + 1) * √(2 - sqrtTwoAddSeries 0 n) < π
  证明: by
  have : √(2 - sqrtTwoAddSeries 0 n) / 2 * 2 ^ (n + 2) < π := by
    rw [← lt_div_iff₀]; rw [← sin_pi_over_two_pow_succ]
    focus
...
-/
theorem pi_gt_sqrtTwoAddSeries (n : Nat) : 2 ^ (n + 1) * √(2 - sqrtTwoAddSeries 0 n) < π := by
  have : √(2 - sqrtTwoAddSeries 0 n) / 2 * 2 ^ (n + 2) < π := by
    rw [← lt_div_iff₀]; rw [← sin_pi_over_two_pow_succ]
    focus
      apply sin_lt
...
/--
theorem `pi_lt_sqrtTwoAddSeries` / 定理 `pi_lt_sqrtTwoAddSeries`

English:
theorem pi_lt_sqrtTwoAddSeries
  given: (n : Nat)
  proof: by
  have : π < (√(2 - sqrtTwoAddSeries 0 n) / 2 + 1 / (2 ^ n) ^ 3 / 4) * (2 : Real) ^ (n + 2) := by
...
-/
theorem pi_lt_sqrtTwoAddSeries (n : Nat) :
...
/--
theorem `pi_lower_bound_start` / 定理 `pi_lower_bound_start`
...
theorem pi_lower_bound_start
  ...
  rwa [le_sub_comm, show (0 : Real) = (0 : Nat) / (1 : Nat) by rw [Nat.cast_zero, zero_div]]

中文:
定理 pi_lower_bound_start
  结论: (n : 自然数) {a}
  证明: by
  refine lt_of_le_of_lt ?_ (pi_gt_sqrtTwoAddSeries n); rw [mul_comm]
  refine (div_le_iff₀ (pow_pos (by simp) _)).mp (le_sqrt_of_sq_le ?_)
...
Depends on / 依赖: Nat.cast_zero, cast_zero, le_sqrt_of_sq_le, le_sub_comm, lt_of_le_of_lt, mul_comm, pi_gt_sqrtTwoAddSeries, pow_pos, zero_div
-/
theorem pi_lower_bound_start (n : Nat) {a}
    (h : sqrtTwoAddSeries ((0 : Nat) / (1 : Nat)) n <= (2 : Real) - (a / (2 : Real) ^ (n + 1)) ^ 2) :
    a < π := by
  refine lt_of_le_of_lt ?_ (pi_gt_sqrtTwoAddSeries n); rw [mul_comm]
  refine (div_le_iff₀ (pow_pos (by simp) _)).mp (le_sqrt_of_sq_le ?_)
  rwa [le_sub_comm, show (0 : Real) = (0 : Nat) / (1 : Nat) by rw [Nat.cast_zero, zero_div]]
...
/--
theorem `sqrtTwoAddSeries_step_up` / 定理 `sqrtTwoAddSeries_step_up`

English:
theorem sqrtTwoAddSeries_step_up
  statement: (c d : Nat) {a b n : Nat} {z : Real} (hz : sqrtTwoAddSeries (c / d) n <= z)
  proof: by
...
定理 sqrtTwoAddSeries_step_up
  结论: (c d : 自然数) {a b n : 自然数} {z : 实数} (hz : sqrtTwoAddSeries (c / d) n <= z)
  证明: by
  refine le_trans ?_ hz; rw [sqrtTwoAddSeries_succ]; apply sqrtTwoAddSeries_monotone_left
  have hb' : 0 < (b : Real) := Nat.cast_pos.2 hb
  have hd' : 0 < (d : Real) := Nat.cast_pos.2 hd
...
-/
theorem sqrtTwoAddSeries_step_up (c d : Nat) {a b n : Nat} {z : Real} (hz : sqrtTwoAddSeries (c / d) n <= z)
...
/--
theorem `pi_upper_bound_start` / 定理 `pi_upper_bound_start`
...
Depends on / 依赖: Nat.cast_zero, cast_zero, div_nonneg, le_of_lt, le_sub_iff_add_le, lt_of_lt_of_le, pi_lt_sqrtTwoAddSeries, pow_nonneg, pow_pos, sqrt_le_left, sub_le_comm, sub_nonneg, zero_div, zero_lt_two
-/
theorem pi_upper_bound_start (n : Nat) {a}
    (h : (2 : Real) - ((a - 1 / (4 : Real) ^ n) / (2 : Real) ^ (n + 1)) ^ 2 <=
        sqrtTwoAddSeries ((0 : Nat) / (1 : Nat)) n)
    (h₂ : (1 : Real) / (4 : Real) ^ n <= a) : π < a := by
  refine lt_of_lt_of_le (pi_lt_sqrtTwoAddSeries n) ?_
  rw [← le_sub_iff_add_le]; rw [← le_div_iff₀']; rw [sqrt_le_left]; rw [sub_le_comm]
  · rwa [Nat.cast_zero, zero_div] at h
  · exact div_nonneg (sub_nonneg.2 h₂) (pow_nonneg (le_of_lt zero_lt_two) _)
...
/--
theorem `sqrtTwoAddSeries_step_down` / 定理 `sqrtTwoAddSeries_step_down`

English:
theorem sqrtTwoAddSeries_step_down
  statement: (a b : Nat) {c d n : Nat} {z : Real}
  proof: by
...
-/
theorem sqrtTwoAddSeries_step_down (a b : Nat) {c d n : Nat} {z : Real}
...
elab "pi_lower_bound " "[" l:term,* "]" : tactic => do
  ...
  for l in els do
    let {num, den, ..} ← unsafe Meta.evalExpr Rat q(Rat) (← Term.elabTermAndSynthesize l (some q(Rat)))
...
/--
theorem `pi_gt_three` / 定理 `pi_gt_three`
...
/--
theorem `pi_lt_four` / 定理 `pi_lt_four`
...
/--
theorem `pi_gt_d6` / 定理 `pi_gt_d6`

English:
theorem pi_gt_d6
  statement: 3.141592 < π
  proof: by
...
/--
theorem `pi_lt_d6` / 定理 `pi_lt_d6`

English:
theorem pi_lt_d6
  statement: π < 3.141593
  proof: by
...
/--
theorem `pi_gt_d20` / 定理 `pi_gt_d20`

English:
theorem pi_gt_d20
  statement: 3.14159265358979323846 < π
  proof: by
...
/--
theorem `pi_lt_d20` / 定理 `pi_lt_d20`

English:
theorem pi_lt_d20
  statement: π < 3.14159265358979323847
  proof: by
...
/--
theorem `floor_pi_eq_three` / 定理 `floor_pi_eq_three`

English:
theorem floor_pi_eq_three
  statement: ⌊π⌋ = 3
  proof: Int.floor_eq_iff.mpr ⟨pi_gt_three.le, by exact_mod_cast pi_lt_four⟩

中文:
定理 floor_pi_eq_three
  结论: ⌊π⌋ = 3
  证明: Int.floor_eq_iff.mpr ⟨pi_gt_three.le, by exact_mod_cast pi_lt_four⟩

Depends on / 依赖: Int.floor_eq_iff.mpr, floor_eq_iff, pi_gt_three, pi_gt_three.le, pi_lt_four
-/
theorem floor_pi_eq_three : ⌊π⌋ = 3 :=
  Int.floor_eq_iff.mpr ⟨pi_gt_three.le, by exact_mod_cast pi_lt_four⟩

/--
theorem `ceil_pi_eq_four` / 定理 `ceil_pi_eq_four`

English:
theorem ceil_pi_eq_four
  statement: ⌈π⌉ = 4
  proof: Int.ceil_eq_iff.mpr ⟨by exact_mod_cast pi_gt_three, pi_lt_four.le⟩

中文:
定理 ceil_pi_eq_four
  结论: ⌈π⌉ = 4
  证明: Int.ceil_eq_iff.mpr ⟨by exact_mod_cast pi_gt_three, pi_lt_four.le⟩

Depends on / 依赖: Int.ceil_eq_iff.mpr, ceil_eq_iff, pi_gt_three, pi_lt_four, pi_lt_four.le
-/
theorem ceil_pi_eq_four : ⌈π⌉ = 4 :=
  Int.ceil_eq_iff.mpr ⟨by exact_mod_cast pi_gt_three, pi_lt_four.le⟩

/--
theorem `round_pi_eq_three` / 定理 `round_pi_eq_three`

English:
theorem round_pi_eq_three
  statement: round π = 3
  proof: by
.trans Int.floor_eq_iff.mpr ⟨by grind [pi_gt_three], ?_⟩ refine round_eq _
  grw [pi_lt_d2]
  norm_num

中文:
定理 round_pi_eq_three
  结论: round π = 3
  证明: by
.trans Int.floor_eq_iff.mpr ⟨by grind [pi_gt_three], ?_⟩ refine round_eq _
  grw [pi_lt_d2]
...
Depends on / 依赖: Int.floor_eq_iff.mpr, floor_eq_iff, pi_gt_three, pi_lt_d2, round_eq
-/
