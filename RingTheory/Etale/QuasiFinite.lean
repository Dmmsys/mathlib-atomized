...
noncomputable
/-
**Ideal.fiberIsoOfBijectiveResidueField** 是 Mathlib 中的一个定义，位于命名空间 ``。
形式化陈述：Ideal.fiberIsoOfBijectiveResidueField (H : Function.Bijective (Ideal.Resid
ueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p))) : q.primesOver (R' otimes[R
] S) ≃o p.primesOver S
参数：H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.ove
r_def p))。
该定义给出了上述对象。
...
/-
**Ideal.comap_fiberIsoOfBijectiveResidueField_symm** 是 Mathlib 中的一个引理，位于命名空间 ``。
形式化陈述：Ideal.comap_fiberIsoOfBijectiveResidueField_symm (H : Function.Bijective (
Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p))) (Q : p.primesOve
r S) : ((Ideal.fiberIsoOfBijectiveResidueField H).symm Q).1.comap (RingHomClass.
toRingHom Algebra.TensorProduct.includeRight) = Q.1
参数：H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.ove
r_def p))；Q : p.primesOver S。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `PrimeSpectrum.comapEquiv_apply`：∀ {R : Type u} {S : Type v} [inst : Comm
Semiring R] [inst_1 : CommSemiring S] (e : R ≃+* S) (p : PrimeSpectrum R),   (Pr
imeSpectrum.comapEqu…
...
@[simp]
/-
**Ideal.comap_fiberIsoOfBijectiveResidueField_apply** 是 Mathlib 中的一个引理，位于命名空间 ``
。
形式化陈述：Ideal.comap_fiberIsoOfBijectiveResidueField_apply (H : Function.Bijective 
(Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p))) (Q : q.primesOv
er (R' otimes[R] S)) : (Ideal.fiberIsoOfBijectiveResidueField H Q).1 = Q.1.comap
 Algebra.TensorProduct.includeRight
参数：H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.ove
r_def p))；Q : q.primesOver (R' otimes[R] S)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Ideal.over_def`：over_def [P.LiesOver p] : p = P.under A
· 使用定理 `Algebra.to_smulCommClass`：∀ {R : Type u_4} {A : Type u_5} [inst : CommSe
...
· 使用引理 `Ideal.comap_fiberIsoOfBijectiveResidueField_symm`：Ideal.comap_fiberIsoOf
BijectiveResidueField_symm (H : Function.Bijective (Ideal.ResidueField.mapₐ p q 
(Algebra.ofId _ _) (q.over_def p))) (Q…
-/
...
/-
**Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap** 是 Mathlib 中的一个引理，位于命
名空间 ``。
形式化陈述：Ideal.eq_of_comap_eq_comap_of_bijective_residueFieldMap (H : Function.Bije
ctive (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.over_def p))) (P₁ P₂ : 
Ideal (R' otimes[R] S)) [P₁.IsPrime] [P₂.IsPrime] [P₁.LiesOver q] [P₂.LiesOver q
] (H₂ : P₁.comap Algebra.TensorProduct.includeRight.toRingHom = P₂.comap Algebra
.TensorProduct.includeRight.toRingHom) : P₁ = P₂
参数：H : Function.Bijective (Ideal.ResidueField.mapₐ p q (Algebra.ofId _ _) (q.ove
r_def p))；P₁ P₂ : Ideal (R' otimes[R] S)；H₂ : P₁.comap Algebra.TensorProduct.inc
ludeRight.toRingHom = P₂.comap Algebra.TensorProduct.includeRight.toRingHom。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Ideal.over_def`：over_def [P.LiesOver p] : p = P.under A
· 使用定理 `Algebra.to_smulCommClass`：∀ {R : Type u_4} {A : Type u_5} [inst : CommSe
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用引理 `Ideal.comap_fiberIsoOfBijectiveResidueField_apply`：Ideal.comap_fiberIsoO
fBijectiveResidueField_apply (H : Function.Bijective (Ideal.ResidueField.mapₐ p 
q (Algebra.ofId _ _) (q.over_def p))) (…
-/
...
@[stacks 00UI]
/-
**Localization.exists_finite_awayMap** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Suppose `f : S →ₐ[R] T` is an `R`-algebra homomorphism with `S` integral and `T`
 of finite type,
such that the induced map `S[1/g] → T[1/g]` is surjective for some `g : S`.
Then for any prime `p` of `R` such that `1 ⊗ₜ g` is invertible in `κ(p) ⊗ S`,
there exists `r ∉ p` such that `T[1/r]` is finite over `R[1/r]`.
...
/-
**Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver** 是 Mathlib 
中的一个引理，位于命名空间 ``。
形式化陈述：Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver {R S : T
ype*} [CommRing R] [CommRing S] [Algebra R S] (p : Ideal R) [p.IsPrime] (q : Ide
al S) [q.IsPrime] [q.LiesOver p] [Algebra.FiniteType R S] [Algebra.QuasiFiniteAt
 R q] : exists s ∉ q, exists hs : IsIntegral R s, (forall q' : Ideal S, q'.IsPri
me -> q' != q -> q'.LiesOver p -> s in q') ∧ forall (q' : Ideal (integralClosure
 R S)), q'.IsPrime -> q' != q.under _ -> q'.LiesOver p -> ⟨s, hs⟩ in q'
参数：p : Ideal R；q : Ideal S。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Ideal.exists_not_mem_forall_mem_of_ne_of_liesOver`：∀ {R : Type u_1} {S :
 Type u_2} [inst : CommRing R] [inst_1 : CommRing S] [inst_2 : Algebra R S] (p :
 Ideal R)   [p.IsPrime] (q : Ideal S) […
...
--- 原说明 ---
A variant of `Ideal.exists_not_mem_forall_mem_of_ne_of_liesOver` that also gives
 you
control on the primes in the integral closure.
-/
...
/-
**Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux** 是 Mathlib 中的一个引
理，位于命名空间 ``。
形式化陈述：Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux {R : Type u} 
{S : Type v} [CommRing R] [CommRing S] [Algebra R S] [Algebra.FiniteType R S] (p
 : Ideal R) [p.IsPrime] (q : Ideal S) [q.IsPrime] [q.LiesOver p] [Algebra.QuasiF
initeAt R q] : exists (R' : Type u) (_ : CommRing R') (_ : Algebra R R') (_ : Al
gebra.Etale R R') (P : Ideal R') (_ : P.IsPrime) (_ : P.LiesOver p) (e : R' otim
es[R] S) (_ : IsIdempotentElem e) (e₀ : R' otimes[R] integralClosure R S) (_ : I
sIdempotentElem e₀) (_ : A
参数：p : Ideal R；q : Ideal S。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Algebra.to_smulCommClass`：∀ {R : Type u_4} {A : Type u_5} [inst : CommSe
...
nst_3 : AddCommMonoi…
· 使用定理 `Ideal.over_def`：over_def [P.LiesOver p] : p = P.under A
· 使用引理 `Algebra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver`：Algeb
ra.exists_notMem_and_isIntegral_forall_mem_of_ne_of_liesOver {R S : Type*} [Comm
Ring R] [CommRing S] [Algebra R S] (p : Ideal R) [p.IsP…
...
@[stacks 00UJ]
/-
**Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq** 是 Mathlib 中的一个引理，位于
命名空间 ``。
形式化陈述：Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq {R : Type u} {S :
 Type v} [CommRing R] [CommRing S] [Algebra R S] [Algebra.FiniteType R S] (p : I
deal R) [p.IsPrime] (q : Ideal S) [q.IsPrime] [q.LiesOver p] [Algebra.QuasiFinit
eAt R q] : exists (R' : Type u) (_ : CommRing R') (_ : Algebra R R') (_ : Algebr
a.Etale R R') (P : Ideal R') (_ : P.IsPrime) (_ : P.LiesOver p) (e : R' otimes[R
] S) (_ : IsIdempotentElem e) (P' : Ideal (R' otimes[R] S)) (_ : P'.IsPrime) (_ 
: P'.LiesOver P), P'.comap
参数：p : Ideal R；q : Ideal S。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Algebra.to_smulCommClass`：∀ {R : Type u_4} {A : Type u_5} [inst : CommSe
...
nst_3 : AddCommMonoi…
· 使用定理 `Ideal.over_def`：over_def [P.LiesOver p] : p = P.under A
· 使用引理 `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux`：Algebra.ex
ists_etale_isIdempotentElem_forall_liesOver_eq_aux {R : Type u} {S : Type v} [Co
mmRing R] [CommRing S] [Algebra R S] [Algebra.Fini…
· 使用定理 `Algebra.ZariskisMainProperty.of_finiteType`：∀ {R : Type u} {S : Type v} 
[inst : CommRing R] [inst_1 : CommRing S] [inst_2 : Algebra R S] [Algebra.Finite
Type R S]   (p : Ideal S) [inst_…
...
· 使用引理 `Algebra.exists_etale_isIdempotentElem_forall_liesOver_eq_aux₂`：Algebra.e
xists_etale_isIdempotentElem_forall_liesOver_eq_aux₂ {R S R' R'' : Type*} [CommR
ing R] [CommRing S] [Algebra R S] [Algebra.FiniteTy…
· 使用定理 `And.right`：∀ {a b : Prop}, a ∧ b → b
...
· 使用定理 `IsLocalization.Away.eq_1`：∀ {R : Type u_1} [inst : CommSemiring R] (x : 
R) (S : Type u_4) [inst_1 : CommSemiring S] [inst_2 : Algebra R S],   IsLocaliza
tion.Away x S …
...
--- 原说明 ---
Let `S` be a finite type `R`-algebra, and `q` a prime lying over `p` such that `
S` is quasi-finite
at `q`.
We may construct an etale `R`-algebra `R'` and a prime `P` lying over `p` with `
κ(P) = κ(p)`,
such that `R' ⊗[R] S = A × B`, `A` is finite as an `R'`-module,
and there exists a unique prime in `A` lying over `P`. This prime will also lie 
over `q`.
...
/-
**Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq_aux** 是 
Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A key induction step of `exists_etale_completeOrthogonalIdempotents_forall_liesO
ver_eq`.
-/
...
/-
**Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq'** 是 Mat
hlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A less universe polymorphic version of
`exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq`. Use that instea
d.
-/
...
@[stacks 00UL]
/-
**Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq** 是 Math
lib 中的一个引理，位于命名空间 ``。
形式化陈述：Algebra.exists_etale_completeOrthogonalIdempotents_forall_liesOver_eq {R :
 Type u} {S : Type v} [CommRing R] [CommRing S] [Algebra R S] [Module.Finite R S
] (p : Ideal R) [p.IsPrime] : exists (R' : Type u) (_ : CommRing R') (_ : Algebr
a R R') (_ : Algebra.Etale R R') (P : Ideal R') (_ : P.IsPrime) (_ : P.LiesOver 
p) (n : Nat) (e : Fin (n + 1) -> R' otimes[R] S) (_ : CompleteOrthogonalIdempote
nts e) (P' : Fin n -> Ideal (R' otimes[R] S)) (_ : forall i, (P' i).IsPrime) (_ 
: forall i, (P' i).LiesOve
参数：p : Ideal R。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Algebra.to_smulCommClass`：∀ {R : Type u_4} {A : Type u_5} [inst : CommSe
miring R] [inst_1 : Semiring A] [inst_2 : Algebra R A],   SMulCommClass R A A
· 使用定理 `Ideal.over_def`：over_def [P.LiesOver p] : p = P.under A
· 使用定理 `_private.Mathlib.RingTheory.Etale.QuasiFinite.0.Algebra.exists_etale_com
pleteOrthogonalIdempotents_forall_liesOver_eq'`：∀ {R : Type u} {S : Type (max u 
v)} [inst : CommRing R] [inst_1 : CommRing S] [inst_2 : Algebra R S] [Module.Fin
ite R S]   (p : Ideal R) [in…
· 使用定理 `IsScalarTower.right`：∀ {R : Type u} {A : Type w} [inst : CommSemiring R]
 [inst_1 : Semiring A] [inst_2 : Algebra R A], IsScalarTower R A A
...
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `And.right`：∀ {a b : Prop}, a ∧ b → b
· 使用引理 `Ideal.comap_comapₐ`：comap_comapₐ {R A B C : Type*} [CommSemiring R] [Sem
iring A] [Algebra R A] [Semiring B] [Algebra R B] [Semiring C] [Algebra R C] {I 
: Ideal …
· 使用定理 `AlgEquiv.symm_comp`：symm_comp (e : A₁ ≃ₐ[R] A₂) : AlgHom.comp ↑e.symm (e
 : A₁ ->ₐ[R] A₂) = AlgHom.id R A₁
· 使用引理 `Ideal.comap_idₐ`：comap_idₐ {R S : Type*} [CommSemiring R] [Semiring S] [
Algebra R S] (I : Ideal S) : Ideal.comap (AlgHom.id R S) I = I
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True

--- 原说明 ---
If `S` is finite over `R`, and `p` is a prime of `R`, then there exists an étale
 neighborhood
`(R', P)` of `p` with `κ(p) = κ(P)` such that `R' ⊗[R] S ≃ₐ[R'] R₁ × ... × Rₙ × 
A`,
each `Rᵢ` has a unique prime `Pᵢ` lying over `P`, and no other prime in `R' ⊗[R]
 S` lies over `P`.

This is merely an iterated application of `exists_etale_isIdempotentElem_forall_
liesOver_eq`.
This is weaker than the corresponding statement of stacks project (in particular
 we asked for
`Module.Finite` instead of quasi finite when localized at `p`, so that we don't 
need to keep
track of this when passing to quotients and tensor products), and the only reaso
n is that
the corresponding stronger statement is even harder to state and even more annoy
ing to prove.
-/
