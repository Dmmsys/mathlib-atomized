...
/-- Push forward the action of `R` on `M` along a compatible surjective map `f : R →+ S`. -/]
/--
Definition of `Function.Surjective.mulActionLeft` / `Function.Surjective.mulActionLeft` 的定义

English:
abbreviation Function.Surjective.mulActionLeft
  signature: {R S M : Type*} [Monoid R] [MulAction R M] [Monoid S]
  body: by rw [← f.map_one, hsmul, one_smul]
...
Depends on / 依赖: f.map_mul, f.map_one, hf.forall, map_mul, map_one, mul_smul, one_smul
-/
abbrev Function.Surjective.mulActionLeft {R S M : Type*} [Monoid R] [MulAction R M] [Monoid S]
    [SMul S M] (f : R ->* S) (hf : Surjective f) (hsmul : forall (c) (x : M), f c • x = c • x) :
    MulAction S M where
...
@[to_additive]
/--
lemma `compHom_smul_def` / 引理 `compHom_smul_def`
...
@[to_additive]
/--
lemma `isPretransitive_compHom` / 引理 `isPretransitive_compHom`

English:
lemma isPretransitive_compHom
  statement: {E F G : Type*} [Monoid E] [Monoid F] [MulAction F G]
  proof: MulAction.compHom _ f
    IsPretransitive E G := by
  let _ : MulAction E G := MulAction.compHom _ f
  refine ⟨fun x y => ?_⟩
  obtain ⟨m, rfl⟩ : exists m : F, m • x = y := exists_smul_eq F x y
  obtain ⟨e, rfl⟩ : exists e, f e = m := hf m
  exact ⟨e, rfl⟩
...
@[to_additive]
/--
lemma `IsPretransitive.of_compHom` / 引理 `IsPretransitive.of_compHom`

English:
lemma IsPretransitive.of_compHom
  statement: {M N α : Type*} [Monoid M] [Monoid N] [MulAction N α]
...
/--
Definition of `MonoidHom.smulOneHom` / `MonoidHom.smulOneHom` 的定义

English:
definition MonoidHom.smulOneHom
  signature: {M N} [Monoid M] [MulOneClass N] [MulAction M N] [IsScalarTower M N N]
  body: x • (1 : N)
...
/--
Definition of `monoidHomEquivMulActionIsScalarTower` / `monoidHomEquivMulActionIsScalarTower` 的定义

English:
definition monoidHomEquivMulActionIsScalarTower
  signature: (M N) [Monoid M] [Monoid N]
  body: ⟨MulAction.compHom N f, SMul.comp.isScalarTower _⟩
...
Depends on / 依赖: MulAction, MulAction.compHom, SMul.comp.isScalarTower, compHom, isScalarTower
-/
