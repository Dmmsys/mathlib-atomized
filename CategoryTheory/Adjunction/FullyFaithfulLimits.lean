...
/--
lemma `preservesColimitsOfSize_iff` / 引理 `preservesColimitsOfSize_iff`
...
中文:
引理 preservesColimitsOfSize_iff
  证明: by
  have := adj.isLeftAdjoint
  refine ⟨fun _ => inferInstance, fun _ => ⟨fun {J _} => ?_⟩⟩
  rw [adj.preservesColimitsOfShape_iff]
...
Depends on / 依赖: adj.isLeftAdjoint, adj.preservesColimitsOfShape_iff, infer_instance, isLeftAdjoint, preservesColimitsOfShape_iff
-/
lemma preservesColimitsOfSize_iff
