...
/--
theorem `countable_meas_le_ne_meas_lt` / 定理 `countable_meas_le_ne_meas_lt`

English:
theorem countable_meas_le_ne_meas_lt
  given: (g : α -> R)
  proof: by
  ...
  let F : R -> Real>=0∞ := fun t => μ {a : α | t <= g a}
  apply (countable_image_gt_image_Ioi F).mono
  intro t ht


中文:
定理 countable_meas_le_ne_meas_lt
  条件: (g : α -> R)
  证明: by
  -- the target set is contained in the set of points where the function `t ↦ μ {a : α | t ≤ g a}`
  -- jumps down on the right of `t`. This jump set is countable for any function.
...
/--
theorem `meas_le_ae_eq_meas_lt` / 定理 `meas_le_ae_eq_meas_lt`

English:
theorem meas_le_ae_eq_meas_lt
  statement: {R : Type*} [LinearOrder R] [MeasurableSpace R]
  proof: Set.Countable.measure_zero (countable_meas_le_ne_meas_lt μ g) _
...
variable {α : Type*} [MeasurableSpace α] {f : α -> Real} {g : Real -> Real}

set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `lintegral_comp_eq_lintegral_meas_le_mul_of_measurable_of_sigmaFinite` / 定理 `lintegral_comp_eq_lintegral_meas_le_mul_of_measurable_of_sigmaFinite`

English:
theorem lintegral_comp_eq_lintegral_meas_le_mul_of_measurable_of_sigmaFinite
  proof: by
  have g_intble' : forall t : Real, 0 <= t -> IntervalIntegrable g volume 0 t := by
    intro t ht
    rcases eq_or_lt_of_le ht with h | h
    · simp [← h]
    · exact g_intble t h
  have integrand_eq : forall ω,
      ENNReal.ofReal (∫ t in 0..f ω, g t) = ∫⁻ t in Ioc 0 (f ω), ENNReal.ofReal (g t

中文:
定理 lintegral_comp_eq_lintegral_meas_le_mul_of_measurable_of_sigmaFinite
  证明: by
  have g_intble' : forall t : Real, 0 <= t -> IntervalIntegrable g volume 0 t := by
    intro t ht
    rcases eq_or_lt_of_le ht with h | h
...
Depends on / 依赖: ENNReal, ENNReal.ofReal, IntervalIntegrable, MeasurableSet, eq_or_lt_of_le, filter_upwards, g_ae_nn, g_intble, g_nn, integrand_eq, measurableSet_Ioc, ofReal, ofReal_integral_eq_lintegral, restrict, self_mem_ae_restrict, volume, volume.restrict
-/
theorem lintegral_comp_eq_lintegral_meas_le_mul_of_measurable_of_sigmaFinite
    (μ : Measure α) [SFinite μ]
    (f_nn : 0 <= f) (f_mble : Measurable f)
    (g_intble : forall t > 0, IntervalIntegrable g volume 0 t) (g_mble : Measurable g)
    (g_nn : forall t > 0, 0 <= g t) :
  ...
  rw [← lintegral_indicator measurableSet_Ioi]; rw [lintegral_lintegral_swap]
  · apply congr_arg
    funext s
    have aux₁ :
      (fun x => (Ioc 0 (f x)).indicator (fun t : Real => ENNReal.ofReal (g t)) s) = fun x =>
        ENNReal.ofReal (g s) * (Ioi (0 : Real)).indicator (fun _ => 1) s *
          (Ici s).indicator (fun _ : Real => (1 : Real>=0∞)) (f x) := by
      funext a
      by_cases h : s in Ioc (0 : Real) (f a)
      · simp only [h, show s in Ioi (0 : Real) from h.1, show f a in Ici s from h.2, indicator_of_mem,
          mul_one]
      · have h_copy := h
    ...
    · apply ENNReal.mul_ne_top ENNReal.ofReal_ne_top
      by_cases h : (0 : Real) < s <;> · simp [h]
    simp_rw [show
        (fun a => (Ici s).indicator (fun _ : Real => (1 : Real>=0∞)) (f a)) = fun a =>
    ...
    rw [lintegral_one]; rw [Measure.restrict_apply MeasurableSet.univ]; rw [univ_inter]; rw [indicator_mul_left]; rw [mul_assoc]; rw [show
        (Ioi 0).indicator (fun _x : Real => (1 : Real>=0∞)) s * μ {a : α | s <= f a} =
          (Ioi 0).indicator (fun _x : Real => 1 * μ {a : α | s <= f a}) s
        by by_cases h : 0 < s <;> simp [h]]
    simp_rw [mul_comm _ (ENNReal.ofReal _), one_mul]
    rfl
  have aux₂ :
    (Function.uncurry fun (x : α) (y : Real) =>
        (Ioc 0 (f x)).indicator (fun t : Real => ENNReal.ofReal (g t)) y) =
      {p : α × Real | p.2 in Ioc 0 (f p.1)}.indicator fun p => ENNReal.ofReal (g p.2) := by
    funext p
    cases p with | mk p_fst p_snd => ?_
    rw [Function.uncurry_apply_pair]
    by_cases h : p_snd in Ioc 0 (f p_fst)
    · have h' : (p_fst, p_snd) in {p : α × Real | p.snd in Ioc 0 (f p.fst)} := h
      rw [Set.indicator_of_mem h']; rw [Set.indicator_of_mem h]
    · have h' : (p_fst, p_snd) ∉ {p : α × Real | p.snd in Ioc 0 (f p.fst)} := h
      rw [Set.indicator_of_notMem h']; rw [Set.indicator_of_notMem h]
  rw [aux₂]
  have mble₀ : MeasurableSet {p : α × Real | p.snd in Ioc 0 (f p.fst)} := by
...
/--
theorem `lintegral_comp_eq_lintegral_meas_le_mul_of_measurable` / 定理 `lintegral_comp_eq_lintegral_meas_le_mul_of_measurable`

English:
theorem lintegral_comp_eq_lintegral_meas_le_mul_of_measurable
  ...
  have f_nonneg : forall ω, 0 <= f ω := fun ω => f_nn ω
  -- trivial case where `g` is ae zero. Then both integrals vanish.
  by_cases H1 : g =ᵐ[volume.restrict (Ioi (0 : Re
...
theorem lintegral_comp_eq_lintegral_meas_le_mul_of_measurable (μ : Measure α)
  ...
  have f_nonneg : forall ω, 0 <= f ω := fun ω => f_nn ω
  -- trivial case where `g` is ae zero. Then both integrals vanish.
  by_cases H1 : g =ᵐ[volume.restrict (Ioi (0 : Real))] 0
  · have A : ∫⁻ ω, ENNReal.ofReal (∫ t in 0..f ω, g t) ∂μ = 0 := by
      have : forall ω, ∫ t in 0..f ω, g t = ∫ t in 0..f ω, 0 := by
        intro ω
    ...
    have B : ∫⁻ t in Ioi 0, μ {a : α | t <= f a} * ENNReal.ofReal (g t) = 0 := by
      have : (fun t => μ {a : α | t <= f a} * ENNReal.ofReal (g t))
        =ᵐ[volume.restrict (Ioi (0 : Real))] 0 := by
  ...
  by_cases! H2 : exists s > 0, 0 < ∫ t in 0..s, g t ∧ μ {a : α | s < f a} = ∞
  · rcases H2 with ⟨s, s_pos, hs, h's⟩
    ...
    have A : ∫⁻ t in Ioi 0, μ {a : α | t <= f a} * ENNReal.ofReal (g t) = ∞ := by
      ...
      ∞ = ∫⁻ t in Ioc 0 s, ∞ * ENNReal.ofReal (g t) := by
          have I_pos : ∫⁻ (a : Real) in Ioc 0 s, ENNReal.ofReal (g a) != 0 := by
          ...
          rw [lintegral_const_mul]; rw [ENNReal.top_mul I_pos]
  ...
  have M_bdd : BddAbove {s : Real | g =ᵐ[volume.restrict (Ioc (0 : Real) s)] 0} := by
    contrapose H1
    have : forall (n : Nat), g =ᵐ[volume.restrict (Ioc (0 : Real) n)] 0 := by
      intro n
      rcases not_bddAbove_iff.1 H1 n with ⟨s, hs, ns⟩
      exact ae_restrict_of_ae_restrict_of_subset (Ioc_subset_Ioc_right ns.le) hs
    have Hg : g =ᵐ[volume.restrict (⋃ (n : Nat), (Ioc (0 : Real) n))] 0 :=
      (ae_restrict_iUnion_iff _ _).2 this
    have : (⋃ (n : Nat), (Ioc (0 : Real) n)) = Ioi 0 :=
      iUnion_Ioc_eq_Ioi_self_iff.2 (fun x _ => exists_nat_ge x)
    rwa [this] at Hg
  -- let `M` be the largest number such that `g` vanishes ae on `(0, M]`.
  let M : Real := sSup {s : Real | g =ᵐ[volume.restrict (Ioc (0 : Real) s)] 0}
  have zero_mem : 0 in {s : Real | g =ᵐ[volume.restrict (Ioc (0 : Real) s)] 0} := by simpa using! trivial
  have M_nonneg : 0 <= M := le_csSup M_bdd zero_mem
  -- Then the function `g` indeed vanishes ae on `(0, M]`.
  have hgM : g =ᵐ[volume.restrict (Ioc (0 : Real) M)] 0 := by
    rw [← restrict_Ioo_eq_restrict_Ioc]
    obtain ⟨u, -, uM, ulim⟩ : exists u, StrictMono u ∧ (forall (n : Nat), u n < M) ∧ Tendsto u atTop (𝓝 M) :=
      exists_seq_strictMono_tendsto M
    have I : forall n, g =ᵐ[volume.restrict (Ioc (0 : Real) (u n))] 0 := by
      intro n
      obtain ⟨s, hs, uns⟩ : exists s, g =ᵐ[Measure.restrict volume (Ioc 0 s)] 0 ∧ u n < s :=
    ...
    have : g =ᵐ[volume.restrict (⋃ n, Ioc (0 : Real) (u n))] 0 := (ae_restrict_iUnion_iff _ _).2 I
    apply ae_restrict_of_ae_restrict_of_subset _ this
    rintro x ⟨x_pos, xM⟩
    obtain ⟨n, hn⟩ : exists n, x < u n := ((tendsto_order.1 ulim).1 _ xM).exists
  ...
  have : SigmaFinite ν := by
    obtain ⟨u, -, uM, ulim⟩ : exists u, StrictAnti u ∧ (forall (n : Nat), M < u n) ∧ Tendsto u atTop (𝓝 M) :=
      exists_seq_strictAnti_tendsto M
    let s : ν.FiniteSpanningSetsIn univ :=
    { set := fun n => {a | f a <= M} union {a | u n < f a}
      ...
      finite := by
        ...
        have J : μ {a | u n < f a} < ∞ := by
          ...
          by_contra H3
          rw [not_lt]; rw [intervalIntegral.integral_of_le (M_nonneg.trans (uM n).le)] at H3
          have g_nn_ae : forallᵐ t ∂(volume.restrict (Ioc 0 (u n))), 0 <= g t := by
            filter_upwards [ae_restrict_mem measurableSet_Ioc] with s hs using g_nn _ hs.1
          have Ig : ∫ (t : Real) in Ioc 0 (u n), g t = 0 :=
            le_antisymm H3 (integral_nonneg_of_ae g_nn_ae)
          have J : forallᵐ t ∂(volume.restrict (Ioc 0 (u n))), g t = 0 :=
      ...
      spanning := by
        ...
        · obtain ⟨n, hn⟩ : exists n, u n < f a := ((tendsto_order.1 ulim).2 _ ha).exists
  ...
  have A : ∫⁻ ω, ENNReal.ofReal (∫ t in 0..f ω, g t) ∂μ
    ...
    have I : ∫⁻ ω in {a | M < f a}ᶜ, ENNReal.ofReal (∫ t in 0..f ω, g t) ∂μ
      ...
      have : ∫ (t : Real) in 0..f s, g t = ∫ (t : Real) in 0..f s, 0 := by
    ...
    rw [← lintegral_add_compl _ meas]; rw [I]; rw [add_zero]
  ...
  have B : ∫⁻ t in Ioi 0, μ {a : α | t <= f a} * ENNReal.ofReal (g t)
    ...
    have I : Ioi (0 : Real) = Ioc (0 : Real) M union Ioi M := (Ioc_union_Ioi_eq_Ioi M_nonneg).symm
    have J : Disjoint (Ioc 0 M) (Ioi M) := Ioc_disjoint_Ioi le_rfl
    rw [I]; rw [lintegral_union measurableSet_Ioi J]; rw [lintegral_union measurableSet_Ioi J]; rw [B1]; rw [B2]
...
/--
theorem `lintegral_comp_eq_lintegral_meas_le_mul` / 定理 `lintegral_comp_eq_lintegral_meas_le_mul`

English:
theorem lintegral_comp_eq_lintegral_meas_le_mul
  statement: (μ : Measure α) (f_nn : 0 <=ᵐ[μ] f)
  proof: by
...
Depends on / 依赖: AEMeasurable, AEMeasurable.exists_measurable_nonneg, G_intble, G_mble, G_nn, IntervalIntegrable, Ioc_subset_Ioi_self, Measurable, Measure, Measure.restrict_mono, ae_mono, aemeasurable, aemeasurable_Ioi_of_forall_Ioc, exists_measurable_nonneg, g_eq_G, g_eq_G_on, g_intble, g_nn, le_rfl, restrict
-/
theorem lintegral_comp_eq_lintegral_meas_le_mul (μ : Measure α) (f_nn : 0 <=ᵐ[μ] f)
    (f_mble : AEMeasurable f μ) (g_intble : forall t > 0, IntervalIntegrable g volume 0 t)
    (g_nn : forallᵐ t ∂volume.restrict (Ioi 0), 0 <= g t) :
    ∫⁻ ω, ENNReal.ofReal (∫ t in 0..f ω, g t) ∂μ =
      ∫⁻ t in Ioi 0, μ {a : α | t <= f a} * ENNReal.ofReal (g t) := by
  obtain ⟨G, G_mble, G_nn, g_eq_G⟩ : exists G : Real -> Real, Measurable G ∧ 0 <= G
      ∧ g =ᵐ[volume.restrict (Ioi 0)] G := by
    refine AEMeasurable.exists_measurable_nonneg ?_ g_nn
    exact aemeasurable_Ioi_of_forall_Ioc fun t ht => (g_intble t ht).1.1.aemeasurable
  have g_eq_G_on : forall t, g =ᵐ[volume.restrict (Ioc 0 t)] G := fun t =>
    ae_mono (Measure.restrict_mono Ioc_subset_Ioi_self le_rfl) g_eq_G
  have G_intble : forall t > 0, IntervalIntegrable G volume 0 t := by
    refine fun t t_pos => ⟨(g_intble t t_pos).1.congr_fun_ae (g_eq_G_on t), ?_⟩
    rw [Ioc_eq_empty_of_le t_pos.lt.le]
    exact integrableOn_empty
  obtain ⟨F, F_mble, F_nn, f_eq_F⟩ : exists F : α -> Real, Measurable F ∧ 0 <= F ∧ f =ᵐ[μ] F := by
  ...
  have eq₂ : forallᵐ ω ∂μ,
    ...
    rw [uIoc_of_le fω_nn]; rw [←
      ae_restrict_iff' (measurableSet_Ioc : MeasurableSet (Ioc (0 : Real) (F ω)))]
    exact g_eq_G_on (F ω)
...
/--
theorem `lintegral_eq_lintegral_meas_le` / 定理 `lintegral_eq_lintegral_meas_le`
...
Depends on / 依赖: ENNReal, ENNReal.ofReal_one, Eventually, Eventually.of_forall, IntervalIntegrable, cst_intble, f_mble, f_nn, integral_const, intervalIntegrable_const, intervalIntegral, intervalIntegral.integral_const, lintegral_comp_eq_lintegral_meas_le_mul, mul_one, ofReal_one, of_forall, simp_rw, smul_eq_mul, sub_zero, volume
-/
...
variable {f : α -> Real} {g : Real -> Real}

/--
theorem `lintegral_comp_eq_lintegral_meas_lt_mul` / 定理 `lintegral_comp_eq_lintegral_meas_lt_mul`

English:
theorem lintegral_comp_eq_lintegral_meas_lt_mul
  statement: (μ : Measure α) (f_nn : 0 <=ᵐ[μ] f)
  proof: by
...
Depends on / 依赖: f_mble, f_nn, filter_upwards, g_intble, g_nn, lintegral_comp_eq_lintegral_meas_le_mul, lintegral_congr_ae, meas_le_ae_eq_meas_lt, restrict, volume, volume.restrict
-/
...
/--
theorem `lintegral_eq_lintegral_meas_lt` / 定理 `lintegral_eq_lintegral_meas_lt`

English:
theorem lintegral_eq_lintegral_meas_lt
  statement: (μ : Measure α)
  proof: by
...
Depends on / 依赖: f_mble, f_nn, filter_upwards, lintegral_congr_ae, lintegral_eq_lintegral_meas_le, meas_le_ae_eq_meas_lt, restrict, volume, volume.restrict
-/
...
variable {α : Type*} [MeasurableSpace α] {μ : Measure α} {f : α -> Real}

/--
theorem `Integrable.integral_eq_integral_meas_lt` / 定理 `Integrable.integral_eq_integral_meas_lt`
...
theorem Integrable.integral_eq_integral_meas_lt
  ...
  have rhs_finite : ∫⁻ (t : Real) in Set.Ioi 0, μ {a | t < f a} < ∞ := by simp only [← key, lhs_finite]
  have rhs_integran
...
Depends on / 依赖: ENNReal, ENNReal.ofReal, ENNReal.toReal_eq_toReal_iff, Integrable, Integrable.lintegral_lt_top, Set.Ioi, aemeasurable, convert, f_intble, f_intble.aemeasurable, f_nn, integral_eq_lintegr, lhs_finite, lhs_finite.ne, lintegral_eq_lintegral_meas_lt, lintegral_lt_top, measure_gt_lt_top, ofReal, rhs_finite, rhs_finite.ne
-/
theorem Integrable.integral_eq_integral_meas_lt
  ...
  have lhs_finite : ∫⁻ (ω : α), ENNReal.ofReal (f ω) ∂μ < ∞ := Integrable.lintegral_lt_top f_intble
  have rhs_finite : ∫⁻ (t : Real) in Set.Ioi 0, μ {a | t < f a} < ∞ := by simp only [← key, lhs_finite]
  have rhs_integrand_finite : forall (t : Real), t > 0 -> μ {a | t < f a} < ∞ :=
  ...
  · have aux := @integral_eq_lintegral_of_nonneg_ae _ _ ((volume : Measure Real).restrict (Set.Ioi 0))
...
/--
theorem `Integrable.integral_eq_integral_meas_le` / 定理 `Integrable.integral_eq_integral_meas_le`
...
Depends on / 依赖: ENNReal, ENNReal.toReal, Integrable, Integrable.integral_eq_integral_meas_lt, f_intble, f_nn, filter_upwards, ht.symm, integral_congr_ae, integral_eq_integral_meas_lt, meas_le_ae_eq_meas_lt, restrict, toReal, volume, volume.restrict
-/
theorem Integrable.integral_eq_integral_meas_le
    (f_intble : Integrable f μ) (f_nn : 0 <=ᵐ[μ] f) :
    ∫ ω, f ω ∂μ = ∫ t in Set.Ioi 0, μ.real {a : α | t <= f a} := by
...
/--
lemma `Integrable.integral_eq_integral_Ioc_meas_le` / 引理 `Integrable.integral_eq_integral_Ioc_meas_le`

English:
lemma Integrable.integral_eq_integral_Ioc_meas_le
  statement: {f : α -> Real} {M : Real}
  proof: by
...
Depends on / 依赖: ENNReal, ENNReal.toReal_eq_zero_iff, Eventually, Eventually.of_forall, Ioc_subset_Ioi_self, Or.inl, Set.mem_sdiff, f_bdd, f_intble, f_intble.integral_eq_integral_meas_le, f_nn, filter_upwards, integral_eq_integral_meas_le, measureReal_def, measure_eq_zero_iff_ae_notMem, measure_mono_null, mem_Ioc, mem_Ioi, mem_sdiff, not_and
-/
lemma Integrable.integral_eq_integral_Ioc_meas_le {f : α -> Real} {M : Real}
  ...
  rw [measureReal_def]; rw [ENNReal.toReal_eq_zero_iff]
