...
/-
**Mathlib.Linter.Style.setOption.parseSetOption** 是 Mathlib 中的一个定义，位于命名空间 `Mathl
ib.Linter.Style.setOption`。
形式化陈述：parseSetOption : Syntax -> Option Name -- This handles all four possibilit
ies of `_val`: a string, number, `true` and `false`. | `(command|set_option $nam
e:ident $_val) => some name.getId | `(set_option $name:ident $_val in $_x) => so
me name.getId | `(tactic|set_option $name:ident $_val in $_x) => some name.getId
 | _ => none  /-- Whether a given piece of syntax is a `set_option` command, tac
tic or term. -/ public def isSetOption : Syntax -> Bool
该定义给出了一等式。
...
-/
/-
**Mathlib.Linter.Style.setOption.setOptionLinter** 是 Mathlib 中的一个定义，位于命名空间 `Math
lib.Linter.Style.setOption`。
形式化陈述：setOptionLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
The `setOption` linter: this lints any `set_option` command, term or tactic
which sets a `debug`, `pp`, `profiler` or `trace` option.
This also warns if an option containing `maxHeartbeats` (typically, the `maxHear
tbeats` or
`synthInstance.maxHeartbeats` option) or the `linter.flexible` or
`backward.inferInstanceAs.wrap.reuseSubInstances ` option is set.

**Why is this bad?** The `debug`, `pp`, `profiler` and `trace` options are good 
for debugging,
but should not be used in production code.

`maxHeartbeats` options should be scoped as `set_option opt in ...` (and be foll
owed by a comment
explaining the need for them; another linter enforces this).
The `linter.flexible` option should be scoped as `set_option opt in ...`.

**How to fix this?** The `maxHeartbeats` and `linter.flexible` option changes ca
n be scoped to
individual commands, if they are truly necessary.
New `backward.inferInstanceAs.wrap.reuseSubInstances` instances are technical de
bt,
and should not be introduced.

The `debug`, `pp`, `profiler` and `trace` are usually not necessary for producti
on code,
so you can simply remove them. (Some tests will intentionally use one of these o
ptions;
in this case, simply allow the linter.)
-/
...
/-
**Mathlib.Linter.Style.missingEnd.missingEndLinter** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Linter.Style.missingEnd`。
形式化陈述：missingEndLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
partial
/-
**Mathlib.Linter.findCDot** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：findCDot : Syntax -> Array Syntax | stx@(.node _ kind args) => let dargs
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Linter.unwanted_cdot** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
形式化陈述：unwanted_cdot (stx : Syntax) : Array Syntax
参数：stx : Syntax。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.Style.cdotLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Styl
e`。
形式化陈述：cdotLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
public partial
/-
**Mathlib.Linter.Style.dollarSyntax.findDollarSyntax** 是 Mathlib 中的一个定义，位于命名空间 `
Mathlib.Linter.Style.dollarSyntax`。
形式化陈述：findDollarSyntax : Syntax -> Array Syntax | stx@(.node _ kind args) => let
 dargs
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.dollarSyntax.dollarSyntaxLinter** 是 Mathlib 中的一个定义，位于命名空间
 `Mathlib.Linter.Style.dollarSyntax`。
形式化陈述：dollarSyntaxLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
public partial
/-
**Mathlib.Linter.Style.lambdaSyntax.findLambdaSyntax** 是 Mathlib 中的一个定义，位于命名空间 `
Mathlib.Linter.Style.lambdaSyntax`。
形式化陈述：findLambdaSyntax : Syntax -> Array Syntax | stx@(.node _ kind args) => let
 dargs
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.lambdaSyntax.lambdaSyntaxLinter** 是 Mathlib 中的一个定义，位于命名空间
 `Mathlib.Linter.Style.lambdaSyntax`。
形式化陈述：lambdaSyntaxLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.longFile.longFileLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Linter.Style.longFile`。
形式化陈述：longFileLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.longLine.isImport** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Lint
er.Style.longLine`。
形式化陈述：isImport (s : String) : Bool
参数：s : String。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.longLine.longLineLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Linter.Style.longLine`。
形式化陈述：longLineLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.nameCheck.doubleUnderscore** 是 Mathlib 中的一个定义，位于命名空间 `Mat
hlib.Linter.Style.nameCheck`。
形式化陈述：doubleUnderscore : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.openClassical.openClassicalLinter** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Linter.Style.openClassical`。
形式化陈述：openClassicalLinter : Linter where run stx
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Linter.Style.elabShow** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Style`
。
形式化陈述：elabShow (newType : Term) : TacticM Unit
参数：newType : Term。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Run the `show` tactic, with a linter warning when one should use `change` instea
d.
-/
