...
/-
**Mathlib.Tactic.elabOptAttrArg** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic`。
形式化陈述：elabOptAttrArg : TSyntax ``optAttrArg -> TermElabM (Array Attribute) | `(o
ptAttrArg| (attr
该定义给出了上述对象。
...
/-
**Mathlib.Tactic.checkImplicitTransparency** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Ta
ctic`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Re-implementation of the inner loop of `Lean.Linter.tacticCheckInstances` for us
e on
declarations synthesized by Mathlib attributes (`@[simps]`, `@[reassoc]`, `@[ele
mentwise]`, ...).
Returns the list of semireducible non-instance definitions that `Meta.check decl
Type .default`
had to unfold but `Meta.check declType .implicit` would not, or `none` if `declT
ype` already
passes the `.implicit` check.
-/
...
`linter.all`. -/
/-
**Mathlib.Tactic.warnIfImplicitIllTyped** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tacti
c`。
形式化陈述：warnIfImplicitIllTyped (ref : Syntax) (declName : Name) (declType : Expr) 
: MetaM Unit
参数：ref : Syntax；declName : Name；declType : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Extension of `linter.tacticCheckInstances` to lemmas produced by Mathlib attribu
tes such as
`@[simps]`, `@[reassoc]`, and `@[elementwise]`. Call sites pass the syntax of th
e user's
attribute (`ref`), the name of the generated lemma (`declName`), and the lemma's
 type
(`declType`); a warning is emitted at `ref` if `declType` is type-correct at `.d
efault` but
not at `.implicit`, listing the semireducible definitions that would need to be
marked `@[implicit_reducible]` to fix the mismatch.

The check is gated by the existing core option `linter.tacticCheckInstances` and
 is silent
otherwise; following the convention of the core linter, it does *not* participat
e in
`linter.all`.
-/
...
-/
/-
**Mathlib.Tactic.addRelatedDecl** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic`。
形式化陈述：addRelatedDecl (src tgt : Name) (ref : Syntax) (attrs : TSyntax ``optAttrA
rg) (construct : Expr -> List Name -> MetaM (Expr × List Name)) (docstringPrefix
? : Option String
参数：src tgt : Name；ref : Syntax；attrs : TSyntax ``optAttrArg；construct : Expr -> 
List Name -> MetaM (Expr × List Name)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Arguments:
* `src : Name` is the existing declaration that we are modifying.
* `prefix_ : String` will be prepended and `suffix : String` will be appended to
 `src`
  to form the name of the new declaration.
* `ref : Syntax` is the syntax where the user requested the related declaration.
* `construct value levels : MetaM (Expr × List Name)`
  given an `Expr.const` referring to the original declaration, and its universe 
variables,
  should construct the value of the new declaration,
  along with the names of its universe variables.
* `attrs` is the attributes that should be applied to both the new and the origi
nal declaration,
  e.g. in the usage `@[reassoc (attr := simp)]`.
  We apply it to both declarations, to have the same behavior as `to_additive`, 
and to shorten some
  attribute commands. Note that `@[elementwise (attr := simp), reassoc (attr := 
simp)]` will try
  to apply `simp` twice to the current declaration, but that causes no issues.
* `docstringPrefix?` is prepended to the doc-string of `src` to form the doc-str
ing of `tgt`.
  If it is `none`, only the doc-string of `src` is used.
* When `hoverInfo := true`, the generated constant will be shown as the hover in
formation on `ref`.
  Warning: As a result, the original doc-string of `ref` will not be visible,
  and go-to-def on `ref` will not go to the definition of `ref`.
-/
