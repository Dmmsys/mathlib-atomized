...
@[simps! hom left]
/--
Definition of `affineOverMk` / `affineOverMk` 的定义

English:
definition affineOverMk
  signature: {P : MorphismProperty Scheme.{u}} {R : CommRingCat.{u}}
  body: .mk ⊤ f hf
...
/--
Instance `isCoverDense_toOver_Spec` / 实例 `isCoverDense_toOver_Spec`
...
set_option backward.isDefEq.respectTransparency.types false in
/--
Instance `isOneHypercoverDense_toOver_Spec` / 实例 `isOneHypercoverDense_toOver_Spec`

English:
instance isOneHypercoverDense_toOver_Spec
  signature: :
  body: Functor.IsOneHypercoverDense.of_hasPullbacks fun X => by
...
实例 isOneHypercoverDense_toOver_Spec
  签名: :
  定义体: Functor.IsOneHypercoverDense.of_hasPullbacks fun X => by
    let 𝒰 := affineOpenCover X.left
    let 𝒱 : Cover (precoverage P) X.left :=
      𝒰.openCover.changeProp (fun _ => IsZariskiLocalAtSource.of_isOpenImmersion _)
    let _ (i : 𝒱.I₀) : (𝒱.X i).Over S := ⟨𝒰.f i ≫ X.hom⟩
