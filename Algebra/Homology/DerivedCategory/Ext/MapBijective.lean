...
/--
lemma `Functor.mapExt_bijective_of_preservesProjectiveObjects` / 引理 `Functor.mapExt_bijective_of_preservesProjectiveObjects`

English:
lemma Functor.mapExt_bijective_of_preservesProjectiveObjects
  statement: [F.Full] [F.Faithful] [HasExt.{w} C]
  proof: by
...
lemma Functor.mapExt_bijective_of_preservesProjectiveObjects [F.Full] [F.Faithful] [HasExt.{w} C]
    [HasExt.{w'} D] [EnoughProjectives C] [F.PreservesProjectiveObjects] (X Y : C) (n : Nat) :
...
/--
lemma `Functor.mapExt_bijective_of_preservesInjectiveObjects` / 引理 `Functor.mapExt_bijective_of_preservesInjectiveObjects`

English:
lemma Functor.mapExt_bijective_of_preservesInjectiveObjects
  statement: [F.Full] [F.Faithful] [HasExt.{w} C]
  proof: by
...
lemma Functor.mapExt_bijective_of_preservesInjectiveObjects [F.Full] [F.Faithful] [HasExt.{w} C]
    [HasExt.{w'} D] [EnoughInjectives C] [F.PreservesInjectiveObjects] (X Y : C) (n : Nat) :
    Function.Bijective (F.mapExtAddHom X Y n) := by
  induction n generalizing Y with
  | zero => simpa [Ext.mapExactFunctor₀] using ⟨Faithful.map_injective, Full.map_surjective⟩
  | succ n hn =>
