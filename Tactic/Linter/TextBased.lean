...
定义 ErrorContext.find?_comparable
  签名: (e : ErrorContext) (exceptions : Array ErrorContext)
...
definition parse?_errorContext
  ...
  match parts with
    | filename :: ":" :: "line" :: lineNumber :: ":" :: errorCode :: ":" :: errorMessage =>
      ...
      -- dealing with e.g. Windows or PO
...
中文:
定义 formatErrors
  签名: (errors : Array ErrorContext) (style : ErrorFormat)
  定义体: do
  for e in errors do
    IO.println (outputMessage e style)
-/
...
中文:
定义 lintFile
  签名: (opts : LinterOptions) (path : FilePath) (exceptions : Array ErrorContext)
  定义体: do
  let mut errors := #[]
  -- Whether any changes were made by auto-fixes.
  let mut changes_made := false
...
.addExtension "lean" let path := mkFilePath (module.components.map

中文:
定义 lintModules
  签名: (opts : LinterOptions) (nolints : Array String) (moduleNames : Array Lean.Name)
  定义体: do
  let styleExceptions := parseStyleExceptions nolints
  let mut numberErrorFiles : UInt32 := 0
  let mut allUnexpectedErrors := #[]
...
定义 modulesNotUpperCamelCase
  签名: (opts : LinterOptions) (modules : Array Lean.Name)
  定义体: do
  unless getLinterValue linter.modulesUpperCamelCase opts do return 0
