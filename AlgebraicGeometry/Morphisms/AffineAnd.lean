/-
Copyright (c) 2024 Christian Merten. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Christian Merten
-/
module

public import Mathlib.AlgebraicGeometry.Morphisms.Affine
public import Mathlib.AlgebraicGeometry.Morphisms.RingHomProperties

/-!
# Affine morphisms with additional ring hom property

In this file we define a constructor `affineAnd Q` for affine target morphism properties of schemes
from a property of ring homomorphisms `Q`: A morphism `f : X ⟶ Y` with affine target satisfies
`affineAnd Q` if it is an affine morphism (i.e. `X` is affine) and the induced ring map on global
sections satisfies `Q`.

`affineAnd Q` inherits most stability properties of `Q` and is local at the target if `Q` is local
at the (algebraic) source.

Typical examples of this are affine morphisms (where `Q` is trivial), finite morphisms
(where `Q` is module finite) or closed immersions (where `Q` is being surjective).

-/

@[expose] public section

universe v u

open CategoryTheory TopologicalSpace Opposite MorphismProperty

namespace AlgebraicGeometry

section

variable (Q : forall {R S : Type u} [CommRing R] [CommRing S], (R ->+* S) -> Prop)

/--
Definition of `affineAnd` / `affineAnd` 的定义

English:
definition affineAnd
  signature: : AffineTargetMorphismProperty
  body: fun X _ f => IsAffine X ∧ Q (f.appTop).hom

@[simp]

中文:
定义 affineAnd
  签名: : AffineTargetMorphism命题erty
  定义体: fun X _ f => IsAffine X ∧ Q (f.appTop).hom

@[simp]

Depends on / 依赖: IsAffine, appTop, f.appTop
-/
def affineAnd : AffineTargetMorphismProperty :=
  fun X _ f => IsAffine X ∧ Q (f.appTop).hom

@[simp]
/--
lemma `affineAnd_apply` / 引理 `affineAnd_apply`

English:
lemma affineAnd_apply
  given: {X Y : Scheme.{u}} (f : X ⟶ Y) [IsAffine Y]
  proof: Iff.rfl

中文:
引理 affineAnd_apply
  条件: {X Y : Scheme.{u}} (f : X ⟶ Y) [IsAffine Y]
  证明: Iff.rfl

Depends on / 依赖: A.mem_degenerate_iff, Iff.rfl, X.degenerate_eq_univ_of_hasDimensionLT, degenerate_eq_univ_of_hasDimensionLT, mem_degenerate_iff
-/
lemma affineAnd_apply {X Y : Scheme.{u}} (f : X ⟶ Y) [IsAffine Y] :
    affineAnd Q f ↔ IsAffine X ∧ Q (f.appTop).hom :=
  Iff.rfl

attribute [local simp] AffineTargetMorphismProperty.toProperty_apply

variable {Q}

set_option backward.isDefEq.respectTransparency false in
/--
lemma `affineAnd_respectsIso` / 引理 `affineAnd_respectsIso`

English:
lemma affineAnd_respectsIso
  given: (hP : RingHom.RespectsIso Q)
  proof: by
  refine RespectsIso.mk _ ?_ ?_
  · intro X Y Z e f ⟨hZ, ⟨hY, hf⟩⟩
    simpa [hP.cancel_right_isIso, IsAffine.of_isIso e.hom]
  · intro X Y Z e f ⟨hZ, hf⟩
    simpa [AffineTargetMorphismProperty.toProperty, IsAffine.of_isIso e.inv, hP.cancel_left_isIso]

中文:
引理 affineAnd_respectsIso
  条件: (hP : RingHom.RespectsIso Q)
  证明: by
  refine RespectsIso.mk _ ?_ ?_
  · intro X Y Z e f ⟨hZ, ⟨hY, hf⟩⟩
    simpa [hP.cancel_right_isIso, IsAffine.of_isIso e.hom]
  · intro X Y Z e f ⟨hZ, hf⟩
    simpa [AffineTargetMorphismProperty.toProperty, IsAffine.of_isIso e.inv, hP.cancel_left_isIso]

Depends on / 依赖: AffineTargetMorphismProperty, AffineTargetMorphismProperty.toProperty, IsAffine, IsAffine.of_isIso, RespectsIso, RespectsIso.mk, cancel_left_isIso, cancel_right_isIso, e.hom, e.inv, hP.cancel_left_isIso, hP.cancel_right_isIso, of_isIso, toProperty
-/
lemma affineAnd_respectsIso (hP : RingHom.RespectsIso Q) :
    (affineAnd Q).toProperty.RespectsIso := by
  refine RespectsIso.mk _ ?_ ?_
  · intro X Y Z e f ⟨hZ, ⟨hY, hf⟩⟩
    simpa [hP.cancel_right_isIso, IsAffine.of_isIso e.hom]
  · intro X Y Z e f ⟨hZ, hf⟩
    simpa [AffineTargetMorphismProperty.toProperty, IsAffine.of_isIso e.inv, hP.cancel_left_isIso]

set_option backward.isDefEq.respectTransparency false in
/--
lemma `affineAnd_isLocal` / 引理 `affineAnd_isLocal`

English:
lemma affineAnd_isLocal
  statement: (hPi : RingHom.RespectsIso Q) (hQl : RingHom.LocalizationAwayPreserves Q)
  proof: affineAnd_respectsIso hPi
  to_basicOpen {X Y _} f r := fun ⟨hX, hf⟩ => by
    simp only at hf
    constructor
    · simp only [Scheme.preimage_basicOpen, Opens.map_top]
      exact (isAffineOpen_top X).basicOpen _
    · dsimp only
      rw [morphismRestrict_appTop]; rw [CommRingCat.hom_comp]; rw [h

中文:
引理 affineAnd_isLocal
  结论: (hPi : RingHom.RespectsIso Q) (hQl : RingHom.LocalizationAwayPreserves Q)
  证明: affineAnd_respectsIso hPi
  to_basicOpen {X Y _} f r := fun ⟨hX, hf⟩ => by
    simp only at hf
    constructor
    · simp only [Scheme.preimage_basicOpen, Opens.map_top]
      exact (isAffineOpen_top X).basicOpen _
    · dsimp only
      rw [morphismRestrict_appTop]; rw [CommRingCat.hom_comp]; rw [h

Depends on / 依赖: affineAnd_respectsIso
-/
lemma affineAnd_isLocal (hPi : RingHom.RespectsIso Q) (hQl : RingHom.LocalizationAwayPreserves Q)
    (hQs : RingHom.OfLocalizationSpan Q) : (affineAnd Q).IsLocal where
  respectsIso := affineAnd_respectsIso hPi
  to_basicOpen {X Y _} f r := fun ⟨hX, hf⟩ => by
    simp only at hf
    constructor
    · simp only [Scheme.preimage_basicOpen, Opens.map_top]
      exact (isAffineOpen_top X).basicOpen _
    · dsimp only
      rw [morphismRestrict_appTop]; rw [CommRingCat.hom_comp]; rw [hPi.cancel_right_isIso]
      -- Not sure why the `show` fixes the following `rw` complaining about "motive is incorrect"
      change Q (Scheme.Hom.app f ((Y.basicOpen r).ι ''ᵁ ⊤)).hom
      rw [Scheme.Opens.ι_image_top]
      rw [(isAffineOpen_top Y).app_basicOpen_eq_away_map f (isAffineOpen_top X)]; rw [CommRingCat.hom_comp]; rw [hPi.cancel_right_isIso]; rw [← Scheme.Hom.appTop]
      dsimp only [Opens.map_top]
      apply hQl
      exact hf
  of_basicOpenCover {X Y _} f s hs hf := by
    dsimp [affineAnd] at hf
    have : IsAffine X := by
      apply isAffine_of_isAffineOpen_basicOpen (f.appTop '' s)
      · apply_fun Ideal.map (f.appTop).hom at hs
        rwa [Ideal.map_span, Ideal.map_top] at hs
      · rintro - ⟨r, hr, rfl⟩
        simp_rw [Scheme.preimage_basicOpen] at hf
        exact (hf ⟨r, hr⟩).left
    refine ⟨inferInstance, hQs.ofIsLocalization' hPi (f.appTop).hom s hs fun a => ?_⟩
    refine ⟨Γ(Y, Y.basicOpen a.val), Γ(X, X.basicOpen (f.appTop a.val)), inferInstance,
      inferInstance, inferInstance, inferInstance, inferInstance, ?_, ?_⟩
    · exact (isAffineOpen_top X).isLocalization_basicOpen (f.appTop a.val)
    · obtain ⟨_, hf⟩ := hf a
      rw [morphismRestrict_appTop]; rw [CommRingCat.hom_comp]; rw [hPi.cancel_right_isIso] at hf
      -- Not sure why the `show` fixes the following `rw` complaining about "motive is incorrect"
      have hf : Q (Scheme.Hom.app f ((Y.basicOpen a.1).ι ''ᵁ ⊤)).hom := hf
      rw [Scheme.Opens.ι_image_top] at hf
      rw [(isAffineOpen_top Y).app_basicOpen_eq_away_map _ (isAffineOpen_top X)] at hf
      rwa [CommRingCat.hom_comp, hPi.cancel_right_isIso] at hf

/--
lemma `affineAnd_isLocal_of_propertyIsLocal` / 引理 `affineAnd_isLocal_of_propertyIsLocal`

English:
lemma affineAnd_isLocal_of_propertyIsLocal
  proof: affineAnd_isLocal hPi.respectsIso hPi.localizationAwayPreserves hPi.ofLocalizationSpan

中文:
引理 affineAnd_isLocal_of_propertyIsLocal
  证明: affineAnd_isLocal hPi.respectsIso hPi.localizationAwayPreserves hPi.ofLocalizationSpan

Depends on / 依赖: affineAnd_isLocal, hPi.localizationAwayPreserves, hPi.ofLocalizationSpan, hPi.respectsIso, localizationAwayPreserves, ofLocalizationSpan, respectsIso
-/
lemma affineAnd_isLocal_of_propertyIsLocal
    (hPi : RingHom.PropertyIsLocal Q) : (affineAnd Q).IsLocal :=
  affineAnd_isLocal hPi.respectsIso hPi.localizationAwayPreserves hPi.ofLocalizationSpan

/--
lemma `affineAnd_isStableUnderBaseChange` / 引理 `affineAnd_isStableUnderBaseChange`

English:
lemma affineAnd_isStableUnderBaseChange
  statement: (hQi : RingHom.RespectsIso Q)
  proof: by
  have : (affineAnd Q).toProperty.RespectsIso := affineAnd_respectsIso hQi
  apply AffineTargetMorphismProperty.IsStableUnderBaseChange.mk
  intro X Y S _ _ f g ⟨hY, hg⟩
  exact ⟨inferInstance, hQb.pullback_fst_appTop _ hQi f _ hg⟩

中文:
引理 affineAnd_isStableUnderBaseChange
  结论: (hQi : RingHom.RespectsIso Q)
  证明: by
  have : (affineAnd Q).toProperty.RespectsIso := affineAnd_respectsIso hQi
  apply AffineTargetMorphismProperty.IsStableUnderBaseChange.mk
  intro X Y S _ _ f g ⟨hY, hg⟩
  exact ⟨inferInstance, hQb.pullback_fst_appTop _ hQi f _ hg⟩

Depends on / 依赖: AffineTargetMorphismProperty, AffineTargetMorphismProperty.IsStableUnderBaseChange.mk, IsStableUnderBaseChange, RespectsIso, affineAnd, affineAnd_respectsIso, hQb.pullback_fst_appTop, pullback_fst_appTop, toProperty, toProperty.RespectsIso
-/
lemma affineAnd_isStableUnderBaseChange (hQi : RingHom.RespectsIso Q)
    (hQb : RingHom.IsStableUnderBaseChange Q) :
    (affineAnd Q).IsStableUnderBaseChange := by
  have : (affineAnd Q).toProperty.RespectsIso := affineAnd_respectsIso hQi
  apply AffineTargetMorphismProperty.IsStableUnderBaseChange.mk
  intro X Y S _ _ f g ⟨hY, hg⟩
  exact ⟨inferInstance, hQb.pullback_fst_appTop _ hQi f _ hg⟩

set_option backward.isDefEq.respectTransparency false in
/--
lemma `targetAffineLocally_affineAnd_iff` / 引理 `targetAffineLocally_affineAnd_iff`

English:
lemma targetAffineLocally_affineAnd_iff
  statement: (hQi : RingHom.RespectsIso Q)
  proof: by
  simp only [targetAffineLocally, affineAnd_apply, morphismRestrict_app, CommRingCat.hom_comp,
    hQi.cancel_right_isIso]
  refine ⟨fun hf U hU => ?_, fun h U => ?_⟩
  · obtain ⟨hfU, hf⟩ := hf ⟨U, hU⟩
    use hfU
    have hf : Q (Scheme.Hom.app f (((⟨U, hU⟩ : Y.affineOpens) : Y.Opens).ι ''ᵁ ⊤)).

中文:
引理 targetAffineLocally_affineAnd_iff
  结论: (hQi : RingHom.RespectsIso Q)
  证明: by
  simp only [targetAffineLocally, affineAnd_apply, morphismRestrict_app, CommRingCat.hom_comp,
    hQi.cancel_right_isIso]
  refine ⟨fun hf U hU => ?_, fun h U => ?_⟩
  · obtain ⟨hfU, hf⟩ := hf ⟨U, hU⟩
    use hfU
    have hf : Q (Scheme.Hom.app f (((⟨U, hU⟩ : Y.affineOpens) : Y.Opens).ι ''ᵁ ⊤)).

Depends on / 依赖: CommRingCat, CommRingCat.hom_comp, Scheme, Scheme.Hom.app, Scheme.Opens, Y.Opens, Y.affineOpens, affineAnd_apply, affineOpens, cancel_right_isIso, hQi.cancel_right_isIso, hom_comp, morphismRestrict_app, targetAffineLocally
-/
lemma targetAffineLocally_affineAnd_iff (hQi : RingHom.RespectsIso Q)
    {X Y : Scheme.{u}} (f : X ⟶ Y) :
    targetAffineLocally (affineAnd Q) f ↔ forall U : Y.Opens, IsAffineOpen U ->
      IsAffineOpen (f ⁻¹ᵁ U) ∧ Q (f.app U).hom := by
  simp only [targetAffineLocally, affineAnd_apply, morphismRestrict_app, CommRingCat.hom_comp,
    hQi.cancel_right_isIso]
  refine ⟨fun hf U hU => ?_, fun h U => ?_⟩
  · obtain ⟨hfU, hf⟩ := hf ⟨U, hU⟩
    use hfU
    have hf : Q (Scheme.Hom.app f (((⟨U, hU⟩ : Y.affineOpens) : Y.Opens).ι ''ᵁ ⊤)).hom := hf
    rwa [Scheme.Opens.ι_image_top] at hf
  · refine ⟨(h U U.2).1, ?_⟩
    change Q (Scheme.Hom.app f ((U : Y.Opens).ι ''ᵁ ⊤)).hom
    rw [Scheme.Opens.ι_image_top]
    exact (h U U.2).2

/--
lemma `targetAffineLocally_affineAnd_iff'` / 引理 `targetAffineLocally_affineAnd_iff'`

English:
lemma targetAffineLocally_affineAnd_iff'
  statement: (hQi : RingHom.RespectsIso Q)
  proof: by
  rw [targetAffineLocally_affineAnd_iff hQi]; rw [isAffineHom_iff]
  aesop

中文:
引理 targetAffineLocally_affineAnd_iff'
  结论: (hQi : RingHom.RespectsIso Q)
  证明: by
  rw [targetAffineLocally_affineAnd_iff hQi]; rw [isAffineHom_iff]
  aesop

Depends on / 依赖: isAffineHom_iff, targetAffineLocally_affineAnd_iff
-/
lemma targetAffineLocally_affineAnd_iff' (hQi : RingHom.RespectsIso Q)
    {X Y : Scheme.{u}} (f : X ⟶ Y) :
    targetAffineLocally (affineAnd Q) f ↔
      IsAffineHom f ∧ forall U : Y.Opens, IsAffineOpen U -> Q (f.app U).hom := by
  rw [targetAffineLocally_affineAnd_iff hQi]; rw [isAffineHom_iff]
  aesop

set_option backward.isDefEq.respectTransparency false in
/--
lemma `targetAffineLocally_affineAnd_iff_affineLocally` / 引理 `targetAffineLocally_affineAnd_iff_affineLocally`

English:
lemma targetAffineLocally_affineAnd_iff_affineLocally
  statement: (hQ : RingHom.PropertyIsLocal Q)
  proof: by
  have : HasRingHomProperty (affineLocally Q) Q := ⟨hQ, rfl⟩
  rw [targetAffineLocally_affineAnd_iff' hQ.respectsIso]
  simp only [and_congr_right_iff]
  intro hf
  constructor
  · wlog hY : IsAffine Y
    · intro h
      rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := affineLocally Q)
      

中文:
引理 targetAffineLocally_affineAnd_iff_affineLocally
  结论: (hQ : RingHom.命题ertyIsLocal Q)
  证明: by
  have : HasRingHomProperty (affineLocally Q) Q := ⟨hQ, rfl⟩
  rw [targetAffineLocally_affineAnd_iff' hQ.respectsIso]
  simp only [and_congr_right_iff]
  intro hf
  constructor
  · wlog hY : IsAffine Y
    · intro h
      rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := affineLocally Q)
      

Depends on / 依赖: CommRingCat, CommRingCat.hom_comp, HasRingHomProperty, HasRingHomProperty.iff_of_isAffine, IsAffine, IsZariskiLocalAtTarget, IsZariskiLocalAtTarget.iff_of_iSup_eq_top, Subcomplex, Subcomplex.toRange, affineLocally, and_congr_right_iff, hQ.respe, hQ.respectsIso, hasDimensionLT_of_epi, hf.isAffine_preimage, hom_comp, iSup_affineOpens_eq_top, iff_of_iSup_eq_top, iff_of_isAffine, isAffine_preimage
-/
lemma targetAffineLocally_affineAnd_iff_affineLocally (hQ : RingHom.PropertyIsLocal Q)
    {X Y : Scheme.{u}} (f : X ⟶ Y) :
    targetAffineLocally (affineAnd Q) f ↔ IsAffineHom f ∧ affineLocally Q f := by
  have : HasRingHomProperty (affineLocally Q) Q := ⟨hQ, rfl⟩
  rw [targetAffineLocally_affineAnd_iff' hQ.respectsIso]
  simp only [and_congr_right_iff]
  intro hf
  constructor
  · wlog hY : IsAffine Y
    · intro h
      rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := affineLocally Q)
        _ (iSup_affineOpens_eq_top _)]
      intro U
      have : IsAffine (f ⁻¹ᵁ U) := hf.isAffine_preimage U U.2
      rw [HasRingHomProperty.iff_of_isAffine (P := affineLocally Q)]; rw [morphismRestrict_appTop]; rw [CommRingCat.hom_comp]; rw [hQ.respectsIso.cancel_right_isIso]
      apply h
      rw [Scheme.Opens.ι_image_top]
      exact U.2
    intro h
    have : IsAffine X := isAffine_of_isAffineHom f
    rw [HasRingHomProperty.iff_of_isAffine (P := affineLocally Q)]
    exact h ⊤ (isAffineOpen_top Y)
  · intro h U hU
    rw [affineLocally_iff_affineOpens_le] at h
    rw [f.app_eq_appLE]
    exact h ⟨U, hU⟩ ⟨f ⁻¹ᵁ U, hf.isAffine_preimage U hU⟩ (by simp)

/--
lemma `targetAffineLocally_affineAnd_eq_affineLocally` / 引理 `targetAffineLocally_affineAnd_eq_affineLocally`

English:
lemma targetAffineLocally_affineAnd_eq_affineLocally
  given: (hQ : RingHom.PropertyIsLocal Q)
  proof: by
  ext X Y f
  exact targetAffineLocally_affineAnd_iff_affineLocally hQ f

中文:
引理 targetAffineLocally_affineAnd_eq_affineLocally
  条件: (hQ : RingHom.命题ertyIsLocal Q)
  证明: by
  ext X Y f
  exact targetAffineLocally_affineAnd_iff_affineLocally hQ f

Depends on / 依赖: targetAffineLocally_affineAnd_iff_affineLocally
-/
lemma targetAffineLocally_affineAnd_eq_affineLocally (hQ : RingHom.PropertyIsLocal Q) :
    targetAffineLocally (affineAnd Q) =
      (@IsAffineHom ⊓ @affineLocally Q : MorphismProperty Scheme.{u}) := by
  ext X Y f
  exact targetAffineLocally_affineAnd_iff_affineLocally hQ f

variable {W : forall {R S : Type u} [CommRing R] [CommRing S], (R ->+* S) -> Prop}

/--
lemma `targetAffineLocally_affineAnd_le` / 引理 `targetAffineLocally_affineAnd_le`

English:
lemma targetAffineLocally_affineAnd_le
  proof: by
  intro X Y f h U
  exact ⟨(h U).1, hQW (h U).2⟩

中文:
引理 targetAffineLocally_affineAnd_le
  证明: by
  intro X Y f h U
  exact ⟨(h U).1, hQW (h U).2⟩
-/
lemma targetAffineLocally_affineAnd_le
    (hQW : forall {R S : Type u} [CommRing R] [CommRing S] {f : R ->+* S}, Q f -> W f) :
    targetAffineLocally (affineAnd Q) <= targetAffineLocally (affineAnd W) := by
  intro X Y f h U
  exact ⟨(h U).1, hQW (h U).2⟩

end

section

variable {Q : forall {R S : Type u} [CommRing R] [CommRing S], (R ->+* S) -> Prop}

/--
lemma `HasAffineProperty.affineAnd_isStableUnderComposition` / 引理 `HasAffineProperty.affineAnd_isStableUnderComposition`

English:
lemma HasAffineProperty.affineAnd_isStableUnderComposition
  statement: {P : MorphismProperty Scheme.{u}}
  proof: by
    wlog hZ : IsAffine Z
    · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := P) _ (iSup_affineOpens_eq_top _)]
      intro U
      rw [morphismRestrict_comp]
      exact this hA hQ _ _ (IsZariskiLocalAtTarget.restrict hf _)
        (IsZariskiLocalAtTarget.restrict hg _) U.2
    rw [HasAffin

中文:
引理 HasAffineProperty.affineAnd_isStableUnderComposition
  结论: {P : Morphism命题erty Scheme.{u}}
  证明: by
    wlog hZ : IsAffine Z
    · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := P) _ (iSup_affineOpens_eq_top _)]
      intro U
      rw [morphismRestrict_comp]
      exact this hA hQ _ _ (IsZariskiLocalAtTarget.restrict hf _)
        (IsZariskiLocalAtTarget.restrict hg _) U.2
    rw [HasAffin

Depends on / 依赖: HasAffineProperty, HasAffineProperty.iff_of_isAffine, IsAffine, IsZariskiLocalAtTarget, IsZariskiLocalAtTarget.iff_of_iSup_eq_top, IsZariskiLocalAtTarget.restrict, affineAnd, iSup_affineOpens_eq_top, iff_of_iSup_eq_top, iff_of_isAffine, morphismRestrict_comp, restrict
-/
lemma HasAffineProperty.affineAnd_isStableUnderComposition {P : MorphismProperty Scheme.{u}}
    (hA : HasAffineProperty P (affineAnd Q)) (hQ : RingHom.StableUnderComposition Q) :
    P.IsStableUnderComposition where
  comp_mem {X Y Z} f g hf hg := by
    wlog hZ : IsAffine Z
    · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := P) _ (iSup_affineOpens_eq_top _)]
      intro U
      rw [morphismRestrict_comp]
      exact this hA hQ _ _ (IsZariskiLocalAtTarget.restrict hf _)
        (IsZariskiLocalAtTarget.restrict hg _) U.2
    rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))] at hg
    obtain ⟨hY, hg⟩ := hg
    rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))] at hf
    obtain ⟨hX, hf⟩ := hf
    rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))]
    exact ⟨hX, hQ _ _ hg hf⟩

/--
lemma `HasAffineProperty.affineAnd_isStableUnderBaseChange` / 引理 `HasAffineProperty.affineAnd_isStableUnderBaseChange`

English:
lemma HasAffineProperty.affineAnd_isStableUnderBaseChange
  statement: {P : MorphismProperty Scheme.{u}}
  proof: HasAffineProperty.isStableUnderBaseChange
    (AlgebraicGeometry.affineAnd_isStableUnderBaseChange hQi hQb)

中文:
引理 HasAffineProperty.affineAnd_isStableUnderBaseChange
  结论: {P : Morphism命题erty Scheme.{u}}
  证明: HasAffineProperty.isStableUnderBaseChange
    (AlgebraicGeometry.affineAnd_isStableUnderBaseChange hQi hQb)

Depends on / 依赖: AlgebraicGeometry, AlgebraicGeometry.affineAnd_isStableUnderBaseChange, HasAffineProperty, HasAffineProperty.isStableUnderBaseChange, affineAnd_isStableUnderBaseChange, isStableUnderBaseChange
-/
lemma HasAffineProperty.affineAnd_isStableUnderBaseChange {P : MorphismProperty Scheme.{u}}
    (_ : HasAffineProperty P (affineAnd Q)) (hQi : RingHom.RespectsIso Q)
    (hQb : RingHom.IsStableUnderBaseChange Q) :
    P.IsStableUnderBaseChange :=
  HasAffineProperty.isStableUnderBaseChange
    (AlgebraicGeometry.affineAnd_isStableUnderBaseChange hQi hQb)

/--
lemma `HasAffineProperty.affineAnd_containsIdentities` / 引理 `HasAffineProperty.affineAnd_containsIdentities`

English:
lemma HasAffineProperty.affineAnd_containsIdentities
  statement: {P : MorphismProperty Scheme.{u}}
  proof: by
    rw [eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi]
    intro U hU
    exact ⟨hU, hQ _⟩

中文:
引理 HasAffineProperty.affineAnd_containsIdentities
  结论: {P : Morphism命题erty Scheme.{u}}
  证明: by
    rw [eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi]
    intro U hU
    exact ⟨hU, hQ _⟩

Depends on / 依赖: eq_targetAffineLocally, targetAffineLocally_affineAnd_iff
-/
lemma HasAffineProperty.affineAnd_containsIdentities {P : MorphismProperty Scheme.{u}}
    (hA : HasAffineProperty P (affineAnd Q)) (hQi : RingHom.RespectsIso Q)
    (hQ : RingHom.ContainsIdentities Q) :
    P.ContainsIdentities where
  id_mem X := by
    rw [eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi]
    intro U hU
    exact ⟨hU, hQ _⟩

/--
lemma `HasAffineProperty.affineAnd_iff` / 引理 `HasAffineProperty.affineAnd_iff`

English:
lemma HasAffineProperty.affineAnd_iff
  statement: (P : MorphismProperty Scheme.{u})
  proof: by
  simp_rw [isAffineHom_iff]
  refine ⟨fun h X Y f => ?_, fun h => ⟨affineAnd_isLocal hQi hQl hQs, ?_⟩⟩
  · rw [eq_targetAffineLocally P, targetAffineLocally_affineAnd_iff hQi]
    lia
  · ext X Y f
    rw [targetAffineLocally_affineAnd_iff hQi]; rw [h f]
    aesop

中文:
引理 HasAffineProperty.affineAnd_iff
  结论: (P : Morphism命题erty Scheme.{u})
  证明: by
  simp_rw [isAffineHom_iff]
  refine ⟨fun h X Y f => ?_, fun h => ⟨affineAnd_isLocal hQi hQl hQs, ?_⟩⟩
  · rw [eq_targetAffineLocally P, targetAffineLocally_affineAnd_iff hQi]
    lia
  · ext X Y f
    rw [targetAffineLocally_affineAnd_iff hQi]; rw [h f]
    aesop

Depends on / 依赖: affineAnd_isLocal, eq_targetAffineLocally, isAffineHom_iff, simp_rw, targetAffineLocally_affineAnd_iff
-/
lemma HasAffineProperty.affineAnd_iff (P : MorphismProperty Scheme.{u})
    (hQi : RingHom.RespectsIso Q) (hQl : RingHom.LocalizationAwayPreserves Q)
    (hQs : RingHom.OfLocalizationSpan Q) :
    HasAffineProperty P (affineAnd Q) ↔
      forall {X Y : Scheme.{u}} (f : X ⟶ Y), P f ↔
        (IsAffineHom f ∧ forall U : Y.Opens, IsAffineOpen U -> Q (f.app U).hom) := by
  simp_rw [isAffineHom_iff]
  refine ⟨fun h X Y f => ?_, fun h => ⟨affineAnd_isLocal hQi hQl hQs, ?_⟩⟩
  · rw [eq_targetAffineLocally P, targetAffineLocally_affineAnd_iff hQi]
    lia
  · ext X Y f
    rw [targetAffineLocally_affineAnd_iff hQi]; rw [h f]
    aesop

set_option backward.isDefEq.respectTransparency.types false in
/--
lemma `HasAffineProperty.affineAnd_le_isAffineHom` / 引理 `HasAffineProperty.affineAnd_le_isAffineHom`

English:
lemma HasAffineProperty.affineAnd_le_isAffineHom
  statement: (P : MorphismProperty Scheme.{u})
  proof: by
  intro X Y f hf
  wlog hY : IsAffine Y
  · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := @IsAffineHom) _ (iSup_affineOpens_eq_top _)]
    intro U
    exact this P hA _ (IsZariskiLocalAtTarget.restrict hf _) U.2
  rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))] at hf
  

中文:
引理 HasAffineProperty.affineAnd_le_isAffineHom
  结论: (P : Morphism命题erty Scheme.{u})
  证明: by
  intro X Y f hf
  wlog hY : IsAffine Y
  · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := @IsAffineHom) _ (iSup_affineOpens_eq_top _)]
    intro U
    exact this P hA _ (IsZariskiLocalAtTarget.restrict hf _) U.2
  rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))] at hf
  

Depends on / 依赖: HasAffineProperty, HasAffineProperty.iff_of_isAffine, IsAffine, IsAffineHom, IsZariskiLocalAtTarget, IsZariskiLocalAtTarget.iff_of_iSup_eq_top, IsZariskiLocalAtTarget.restrict, affineAnd, iSup_affineOpens_eq_top, iff_of_iSup_eq_top, iff_of_isAffine, restrict
-/
lemma HasAffineProperty.affineAnd_le_isAffineHom (P : MorphismProperty Scheme.{u})
    (hA : HasAffineProperty P (affineAnd Q)) : P <= @IsAffineHom := by
  intro X Y f hf
  wlog hY : IsAffine Y
  · rw [IsZariskiLocalAtTarget.iff_of_iSup_eq_top (P := @IsAffineHom) _ (iSup_affineOpens_eq_top _)]
    intro U
    exact this P hA _ (IsZariskiLocalAtTarget.restrict hf _) U.2
  rw [HasAffineProperty.iff_of_isAffine (P := P) (Q := (affineAnd Q))] at hf
  rw [HasAffineProperty.iff_of_isAffine (P := @IsAffineHom)]
  exact hf.1

/--
lemma `HasAffineProperty.affineAnd_eq_of_propertyIsLocal` / 引理 `HasAffineProperty.affineAnd_eq_of_propertyIsLocal`

English:
lemma HasAffineProperty.affineAnd_eq_of_propertyIsLocal
  statement: {P P' : MorphismProperty Scheme.{u}}
  proof: by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [targetAffineLocally_affineAnd_eq_affineLocally]; rw [HasRingHomProperty.eq_affineLocally (P := P')]
  exact HasRingHomProperty.isLocal_ringHomProperty P'

中文:
引理 HasAffineProperty.affineAnd_eq_of_propertyIsLocal
  结论: {P P' : Morphism命题erty Scheme.{u}}
  证明: by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [targetAffineLocally_affineAnd_eq_affineLocally]; rw [HasRingHomProperty.eq_affineLocally (P := P')]
  exact HasRingHomProperty.isLocal_ringHomProperty P'

Depends on / 依赖: HasAffineProperty, HasAffineProperty.eq_targetAffineLocally, HasRingHomProperty, HasRingHomProperty.eq_affineLocally, HasRingHomProperty.isLocal_ringHomProperty, eq_affineLocally, eq_targetAffineLocally, isLocal_ringHomProperty, targetAffineLocally_affineAnd_eq_affineLocally
-/
lemma HasAffineProperty.affineAnd_eq_of_propertyIsLocal {P P' : MorphismProperty Scheme.{u}}
    (hP : HasAffineProperty P (affineAnd Q)) [HasRingHomProperty P' Q] :
    P = (@IsAffineHom ⊓ P' : MorphismProperty Scheme.{u}) := by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [targetAffineLocally_affineAnd_eq_affineLocally]; rw [HasRingHomProperty.eq_affineLocally (P := P')]
  exact HasRingHomProperty.isLocal_ringHomProperty P'

/--
lemma `HasAffineProperty.SpecMap_iff_of_affineAnd` / 引理 `HasAffineProperty.SpecMap_iff_of_affineAnd`

English:
lemma HasAffineProperty.SpecMap_iff_of_affineAnd
  statement: {P : MorphismProperty Scheme.{u}}
  proof: by
  have := RingHom.toMorphismProperty_respectsIso_iff.mp hQi
  rw [HasAffineProperty.iff_of_isAffine (P := P)]; rw [affineAnd]; rw [and_iff_right]
  exacts [MorphismProperty.arrow_mk_iso_iff (RingHom.toMorphismProperty Q)
    (arrowIsoΓSpecOfIsAffine f).symm, inferInstance]

中文:
引理 HasAffineProperty.SpecMap_iff_of_affineAnd
  结论: {P : Morphism命题erty Scheme.{u}}
  证明: by
  have := RingHom.toMorphismProperty_respectsIso_iff.mp hQi
  rw [HasAffineProperty.iff_of_isAffine (P := P)]; rw [affineAnd]; rw [and_iff_right]
  exacts [MorphismProperty.arrow_mk_iso_iff (RingHom.toMorphismProperty Q)
    (arrowIsoΓSpecOfIsAffine f).symm, inferInstance]

Depends on / 依赖: CommRing, HasAffineProperty, HasAffineProperty.iff_of_isAffine, MorphismProperty, MorphismProperty.arrow_mk_iso_iff, RingHom, RingHom.toMorphismProperty, RingHom.toMorphismProperty_respectsIso_iff.mp, affineAnd, and_iff_right, arrow_mk_iso_iff, exacts, iff_of_isAffine, toMorphismProperty, toMorphismProperty_respectsIso_iff, variable
-/
lemma HasAffineProperty.SpecMap_iff_of_affineAnd {P : MorphismProperty Scheme.{u}}
    (hP : HasAffineProperty P (affineAnd Q)) (hQi : RingHom.RespectsIso Q)
    {R S : CommRingCat.{u}} (f : R ⟶ S) : P (Spec.map f) ↔ Q f.hom := by
  have := RingHom.toMorphismProperty_respectsIso_iff.mp hQi
  rw [HasAffineProperty.iff_of_isAffine (P := P)]; rw [affineAnd]; rw [and_iff_right]
  exacts [MorphismProperty.arrow_mk_iso_iff (RingHom.toMorphismProperty Q)
    (arrowIsoΓSpecOfIsAffine f).symm, inferInstance]
variable {Q' : forall {R S : Type u} [CommRing R] [CommRing S], (R ->+* S) -> Prop}

/--
lemma `HasAffineProperty.affineAnd_le_affineAnd` / 引理 `HasAffineProperty.affineAnd_le_affineAnd`

English:
lemma HasAffineProperty.affineAnd_le_affineAnd
  statement: {P P' : MorphismProperty Scheme.{u}}
  proof: by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [HasAffineProperty.eq_targetAffineLocally (P := P')]
  exact targetAffineLocally_affineAnd_le hQQ'

中文:
引理 HasAffineProperty.affineAnd_le_affineAnd
  结论: {P P' : Morphism命题erty Scheme.{u}}
  证明: by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [HasAffineProperty.eq_targetAffineLocally (P := P')]
  exact targetAffineLocally_affineAnd_le hQQ'

Depends on / 依赖: HasAffineProperty, HasAffineProperty.eq_targetAffineLocally, eq_targetAffineLocally, targetAffineLocally_affineAnd_le
-/
lemma HasAffineProperty.affineAnd_le_affineAnd {P P' : MorphismProperty Scheme.{u}}
    (hP : HasAffineProperty P (affineAnd Q)) (hP' : HasAffineProperty P' (affineAnd Q'))
    (hQQ' : forall {R S : Type u} [CommRing R] [CommRing S] {f : R ->+* S}, Q f -> Q' f) :
    P <= P' := by
  rw [HasAffineProperty.eq_targetAffineLocally (P := P)]; rw [HasAffineProperty.eq_targetAffineLocally (P := P')]
  exact targetAffineLocally_affineAnd_le hQQ'

set_option backward.isDefEq.respectTransparency false in
/--
lemma `HasAffineProperty.coprodDesc_affineAnd` / 引理 `HasAffineProperty.coprodDesc_affineAnd`

English:
lemma HasAffineProperty.coprodDesc_affineAnd
  statement: {P : MorphismProperty Scheme.{u}}
  proof: by
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP f hf
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP g hg
  rw [HasAffineProperty.eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi] at hf hg ⊢
  refine fun W hW => ⟨hW.preimage _, ?_⟩
  let e : Γ(U ⨿ V, Limits

中文:
引理 HasAffineProperty.coprodDesc_affineAnd
  结论: {P : Morphism命题erty Scheme.{u}}
  证明: by
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP f hf
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP g hg
  rw [HasAffineProperty.eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi] at hf hg ⊢
  refine fun W hW => ⟨hW.preimage _, ?_⟩
  let e : Γ(U ⨿ V, Limits

Depends on / 依赖: HasAffineProperty, HasAffineProperty.affineAnd_le_isAffineHom, HasAffineProperty.eq_targetAffineLocally, Limits, Limits.coprod.desc, Limits.prod.mapIso, Scheme, Scheme.Hom.comp_preimage, Scheme.coprodPresheafObjIso, U.presheaf.mapIso, V.presheaf.mapIso, affineAnd_le_isAffineHom, comp_preimage, coprod, coprodPresheafObjIso, eqToIso, eq_targetAffineLocally, hW.preimage, mapIso, preimage
-/
lemma HasAffineProperty.coprodDesc_affineAnd {P : MorphismProperty Scheme.{u}}
    (hP : HasAffineProperty P (affineAnd Q)) (hQi : RingHom.RespectsIso Q)
    (hQ : forall {R S T : Type u} [CommRing R] [CommRing S] [CommRing T] (f : R ->+* S) (g : R ->+* T),
      Q f -> Q g -> Q (f.prod g))
    {U V X : Scheme.{u}} (f : U ⟶ X) (g : V ⟶ X) (hf : P f) (hg : P g) :
    P (Limits.coprod.desc f g) := by
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP f hf
  have := HasAffineProperty.affineAnd_le_isAffineHom P hP g hg
  rw [HasAffineProperty.eq_targetAffineLocally P]; rw [targetAffineLocally_affineAnd_iff hQi] at hf hg ⊢
  refine fun W hW => ⟨hW.preimage _, ?_⟩
  let e : Γ(U ⨿ V, Limits.coprod.desc f g ⁻¹ᵁ W) ≅ Γ(U, f ⁻¹ᵁ W) ⨯ Γ(V, g ⁻¹ᵁ W) :=
    Scheme.coprodPresheafObjIso _ ≪≫ Limits.prod.mapIso
      (U.presheaf.mapIso (eqToIso (by simp [← Scheme.Hom.comp_preimage])).op)
      (V.presheaf.mapIso (eqToIso (by simp [← Scheme.Hom.comp_preimage])).op)
  rw [← hQi.cancel_right_isIso _ e.hom]; rw [← CommRingCat.hom_comp]; rw [← hQi.cancel_right_isIso _
    ((Limits.limit.isLimit _).conePointUniqueUpToIso (CommRingCat.prodFanIsLimit _ _)).hom]; rw [← CommRingCat.hom_comp]
  have {R S T : Type u} [CommRing R] [CommRing S] [CommRing T] (f : R ->+* S × T) :
      Q (.comp (.fst _ _) f) -> Q (.comp (.snd _ _) f) -> Q f :=
    hQ (.comp (.fst _ _) f) (.comp (.snd _ _) f)
  refine this _ ?_ ?_
  · have : (Limits.coprod.desc f g).app W ≫ e.hom ≫ Limits.prod.fst = f.app W := by
      simp [e, Scheme.Hom.app_eq_appLE, Scheme.Hom.appLE_comp_appLE]
    convert! (hf W hW).2
    exact congr(($this).1)
  · have : (Limits.coprod.desc f g).app W ≫ e.hom ≫ Limits.prod.snd = g.app W := by
      simp [e, Scheme.Hom.app_eq_appLE, Scheme.Hom.appLE_comp_appLE]
    convert! (hg W hW).2
    exact congr(($this).1)

end

end AlgebraicGeometry
