...
/-
**CategoryTheory.ObjectProperty.ContainsZero** 是 Mathlib 中的一个归纳类型，位于命名空间 `Catego
...
/-
**CategoryTheory.ObjectProperty.exists_prop_of_containsZero** 是 Mathlib 中的一个引理，位
于命名空间 `CategoryTheory.ObjectProperty`。
形式化陈述：exists_prop_of_containsZero [P.ContainsZero] : exists (Z : C), IsZero Z ∧ 
P Z
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.ObjectProperty.ContainsZero.exists_zero`：∀ {C : Type u} {
inst : CategoryTheory.Category.{v, u} C} {P : CategoryTheory.ObjectProperty C} [
self : P.ContainsZero],   ∃ Z, CategoryTheor…
-/
...
/-
**CategoryTheory.ObjectProperty.prop_of_isZero** 是 Mathlib 中的一个引理，位于命名空间 `Catego
ryTheory.ObjectProperty`。
形式化陈述：prop_of_isZero [P.ContainsZero] [P.IsClosedUnderIsomorphisms] {Z : C} (hZ 
: IsZero Z) : P Z
参数：hZ : IsZero Z。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.ObjectProperty.prop_zero** 是 Mathlib 中的一个引理，位于命名空间 `CategoryThe
ory.ObjectProperty`。
形式化陈述：prop_zero [P.ContainsZero] [P.IsClosedUnderIsomorphisms] [HasZeroObject C]
 : P 0
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.Functor.kernel** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Functo
r`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     {D : Type
 u'} →       [inst_1 : CategoryTheory.Category.{v', u'} D] → CategoryTheory.Func
tor C D → CategoryTheory.ObjectProperty C
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
