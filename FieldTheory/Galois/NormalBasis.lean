...
/--
theorem `exists_linearIndependent_algEquiv_apply_of_finite` / 定理 `exists_linearIndependent_algEquiv_apply_of_finite`

English:
theorem exists_linearIndependent_algEquiv_apply_of_finite
  given: [Finite L]
  proof: by
...
Depends on / 依赖: Finite, Finite.of_injective, Fintype, Fintype.ofFinite, algebraMap, injective, ofFinite, of_injective
-/
theorem exists_linearIndependent_algEquiv_apply_of_finite [Finite L] :
    exists x : L, LinearIndependent K fun σ : Gal(L/K) => σ x := by
  have := Finite.of_injective _ (algebraMap K L).injective
  have := Fintype.ofFinite K
  /- Since `K[X]` is a PID and `L` is a finitely generated `K[X]`-module (with `X` acting as
    the Frobenius automorphism), there exists `x : L` whose `K[X]`-annihilator is equal to the
  ...
  use x
  rw [← span_minpoly_eq_annihilator]; rw [minpoly_frobeniusAlgHom]; rw [eq_comm] at hx
...
.map' ((AEval'.of _).symm.toLinearMap ∘ₗ (liftQ _ _ hx.le).restrictScalars K) by
  ...
  rw [toSpanSingleton_apply]; rw [AEval'.X_pow_smul_of]; rw [End.smul_def]; rw [End.coe_pow]
  rfl
...
/--
theorem `exists_linearIndependent_algEquiv_apply_of_infinite` / 定理 `exists_linearIndependent_algEquiv_apply_of_infinite`

English:
theorem exists_linearIndependent_algEquiv_apply_of_infinite
  given: [Infinite K]
  proof: by
  classical
...
apply span_flip_eq_top_iff_linearIndependent.mpr
  ...
  obtain ⟨c, hc⟩ : exists c : _ -> L, M.det.eval c = 1 := by
  ...
  obtain ⟨b, hb⟩ : exists b : _ -> K, M.det.eval (algebraMap K L ∘ b) != 0 := by
    by_contra! h
...
funext_iff.mp (algebraMap K L).injective.comp_left ?_⟩
  ...
  rw [RingHom.map_det]; rw [RingHom.mapMatrix_apply] at hb
...
public theorem normalBasis_apply (e : Gal(L/K)) : normalBasis K L e = e (normalBasis K L 1) := by
  rw [normalBasis]; rw [coe_basisOfLinearIndependentOfCardEqFinrank]; rw [AlgEquiv.one_apply]
