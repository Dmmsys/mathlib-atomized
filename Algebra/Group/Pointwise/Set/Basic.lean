/-
Copyright (c) 2019 Johan Commelin. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Johan Commelin, Floris van Doorn, Yaël Dillies
-/
module

public import Mathlib.Algebra.Group.Equiv.Basic
public import Mathlib.Algebra.Group.Prod
public import Mathlib.Algebra.Order.Monoid.Unbundled.Pow
public import Mathlib.Data.Set.NAry

/-!
# Pointwise operations of sets

This file defines pointwise algebraic operations on sets.

## Main declarations

For sets `s` and `t` and scalar `a`:
* `s * t`: Multiplication, set of all `x * y` where `x ∈ s` and `y ∈ t`.
* `s + t`: Addition, set of all `x + y` where `x ∈ s` and `y ∈ t`.
* `s⁻¹`: Inversion, set of all `x⁻¹` where `x ∈ s`.
* `-s`: Negation, set of all `-x` where `x ∈ s`.
* `s / t`: Division, set of all `x / y` where `x ∈ s` and `y ∈ t`.
* `s - t`: Subtraction, set of all `x - y` where `x ∈ s` and `y ∈ t`.

For `α` a semigroup/monoid, `Set α` is a semigroup/monoid.
As an unfortunate side effect, this means that `n • s`, where `n : ℕ`, is ambiguous between
pointwise scaling and repeated pointwise addition; the former has `(2 : ℕ) • {1, 2} = {2, 4}`, while
the latter has `(2 : ℕ) • {1, 2} = {2, 3, 4}`. See note [pointwise nat action].

Appropriate definitions and results are also transported to the additive theory via `to_additive`.

## Implementation notes

* The following expressions are considered in simp-normal form in a group:
  `(fun h ↦ h * g) ⁻¹' s`, `(fun h ↦ g * h) ⁻¹' s`, `(fun h ↦ h * g⁻¹) ⁻¹' s`,
  `(fun h ↦ g⁻¹ * h) ⁻¹' s`, `s * t`, `s⁻¹`, `(1 : Set _)` (and similarly for additive variants).
  Expressions equal to one of these will be simplified.
* We put all instances in the scope `Pointwise`, so that these instances are not available by
  default. Note that we do not mark them as reducible (as argued by note [reducible non-instances])
  since we expect the scope to be open whenever the instances are actually used (and making the
  instances reducible changes the behavior of `simp`).

## Tags

set multiplication, set addition, pointwise addition, pointwise multiplication,
pointwise subtraction
-/

@[expose] public section

assert_not_exists Set.iUnion MulAction MonoidWithZero IsOrderedMonoid

library_note «pointwise nat action» /--
Pointwise monoids (`Set`, `Finset`, `Filter`) have derived pointwise actions of the form
`SMul α β → SMul α (Set β)`. When `α` is `ℕ` or `ℤ`, this action conflicts with the
nat or int action coming from `Set β` being a `Monoid` or `DivInvMonoid`. For example,
`2 • {a, b}` can both be `{2 • a, 2 • b}` (pointwise action, pointwise repeated addition,
`Set.smulSet`) and `{a + a, a + b, b + a, b + b}` (nat or int action, repeated pointwise
addition, `Set.NSMul`).

Because the pointwise action can easily be spelled out in such cases, we give higher priority to the
nat and int actions.
-/

open Function MulOpposite

variable {F α β γ : Type*}

namespace Set

/-! ### `0`/`1` as sets -/

section One

variable [One α] {s : Set α} {a : α}

/-- The set `1 : Set α` is defined as `{1}` in scope `Pointwise`. -/
@[to_additive (attr := instance_reducible)
  /-- The set `0 : Set α` is defined as `{0}` in scope `Pointwise`. -/]
/-
**Set.one** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [One α] → One (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def one : One (Set α) :=
  ⟨{1}⟩

scoped[Pointwise] attribute [instance] Set.one Set.zero

open scoped Pointwise

-- TODO: This would be a good simp lemma scoped to `Pointwise`, but it seems `@[simp]` can't be
-- scoped
@[to_additive]
/-
**Set.singleton_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singleton_one : ({1} : Set α) = 1
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem singleton_one : ({1} : Set α) = 1 :=
  rfl

@[to_additive (attr := simp, push)]
/-
**Set.mem_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mem_one : a in (1 : Set α) ↔ a = 1
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_one : a ∈ (1 : Set α) ↔ a = 1 :=
  Iff.rfl

@[to_additive]
/-
**Set.one_mem_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：one_mem_one : (1 : α) in (1 : Set α)
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem one_mem_one : (1 : α) ∈ (1 : Set α) :=
  Eq.refl _

@[to_additive (attr := simp)]
/-
**Set.one_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：one_subset : 1 subseteq s ↔ (1 : α) in s
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.singleton_subset_iff`：singleton_subset_iff {a : α} {s : Set α} : {a}
 subseteq s ↔ a in s
-/
theorem one_subset : 1 ⊆ s ↔ (1 : α) ∈ s :=
  singleton_subset_iff

@[to_additive (attr := simp)]
/-
**Set.one_nonempty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：one_nonempty : (1 : Set α).Nonempty
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem one_nonempty : (1 : Set α).Nonempty :=
  ⟨1, rfl⟩

@[to_additive (attr := simp)]
/-
**Set.image_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_one {f : α -> β} : f '' 1 = {f 1}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
theorem image_one {f : α → β} : f '' 1 = {f 1} :=
  image_singleton

@[to_additive]
/-
**Set.subset_one_iff_eq** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：subset_one_iff_eq : s subseteq 1 ↔ s = ∅ ∨ s = 1
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.subset_singleton_iff_eq`：subset_singleton_iff_eq {s : Set α} {x : α}
 : s subseteq {x} ↔ s = ∅ ∨ s = {x}
-/
theorem subset_one_iff_eq : s ⊆ 1 ↔ s = ∅ ∨ s = 1 :=
  subset_singleton_iff_eq

@[to_additive]
/-
**Set.Nonempty.subset_one_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : One α] {s : Set α}, s.Nonempty → (s ⊆ 1 ↔ s = 1)
参数：s ⊆ 1 ↔ s = 1。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.subset_singleton_iff`：∀ {α : Type u_1} {s : Set α} {a : α},
 s.Nonempty → (s ⊆ {a} ↔ s = {a})
-/
theorem Nonempty.subset_one_iff (h : s.Nonempty) : s ⊆ 1 ↔ s = 1 :=
  h.subset_singleton_iff

/-- The singleton operation as a `OneHom`. -/
@[to_additive /-- The singleton operation as a `ZeroHom`. -/]
/-
**Set.singletonOneHom** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：singletonOneHom : OneHom α (Set α) where toFun
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Set.singleton_one`：singleton_one : ({1} : Set α) = 1

--- 原说明 ---
The singleton operation as a `OneHom`.
-/
def singletonOneHom : OneHom α (Set α) where
  toFun := singleton; map_one' := singleton_one

@[to_additive (attr := simp)]
/-
**Set.coe_singletonOneHom** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：coe_singletonOneHom : (singletonOneHom : α -> Set α) = singleton
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_singletonOneHom : (singletonOneHom : α → Set α) = singleton :=
  rfl
/-
**Set.image_op_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : One α], MulOpposite.op '' 1 = 1
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
@[to_additive] lemma image_op_one : (1 : Set α).image op = 1 := image_singleton

@[to_additive (attr := simp) zero_prod_zero]
/-
**Set.one_prod_one** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：one_prod_one [One β] : (1 ×ˢ 1 : Set (α × β)) = 1
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.ext`：ext {a b : Set α} (h : forall (x : α), x in a ↔ x in b) : a = b
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
lemma one_prod_one [One β] : (1 ×ˢ 1 : Set (α × β)) = 1 := by ext; simp [Prod.ext_iff]

end One

/-! ### Set negation/inversion -/


section Inv

/-- The pointwise inversion of set `s⁻¹` is defined as `{x | x⁻¹ ∈ s}` in scope `Pointwise`. It is
equal to `{x⁻¹ | x ∈ s}`, see `Set.image_inv_eq_inv`. -/
@[to_additive (attr := instance_reducible)
      /-- The pointwise negation of set `-s` is defined as `{x | -x ∈ s}` in scope `Pointwise`.
      It is equal to `{-x | x ∈ s}`, see `Set.image_neg_eq_neg`. -/]
/-
**Set.inv** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [Inv α] → Inv (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def inv [Inv α] : Inv (Set α) :=
  ⟨preimage Inv.inv⟩

scoped[Pointwise] attribute [instance] Set.inv Set.neg

open scoped Pointwise

section Inv

variable {ι : Sort*} [Inv α] {s t : Set α} {a : α}

@[to_additive (attr := simp)]
/-
**Set.inv_ofPred** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_ofPred (p : α -> Prop) : {x | p x}⁻¹ = {x | p x⁻¹}
参数：p : α -> Prop。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem inv_ofPred (p : α → Prop) : {x | p x}⁻¹ = {x | p x⁻¹} :=
  rfl

@[deprecated (since := "2026-07-09")] alias inv_setOf := inv_ofPred
@[deprecated (since := "2026-07-09")] alias neg_setOf := neg_ofPred

@[to_additive (attr := simp, push)]
/-
**Set.mem_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mem_inv : a in s⁻¹ ↔ a⁻¹ in s
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_inv : a ∈ s⁻¹ ↔ a⁻¹ ∈ s :=
  Iff.rfl

@[to_additive (attr := simp)]
/-
**Set.inv_preimage** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_preimage : Inv.inv ⁻¹' s = s⁻¹
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem inv_preimage : Inv.inv ⁻¹' s = s⁻¹ :=
  rfl

@[to_additive (attr := simp)]
/-
**Set.inv_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_empty : (∅ : Set α)⁻¹ = ∅
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem inv_empty : (∅ : Set α)⁻¹ = ∅ :=
  rfl

@[to_additive (attr := simp)]
/-
**Set.inv_univ** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_univ : (univ : Set α)⁻¹ = univ
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem inv_univ : (univ : Set α)⁻¹ = univ :=
  rfl

@[to_additive (attr := simp)]
/-
**Set.inter_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_inv : (s inter t)⁻¹ = s⁻¹ inter t⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.preimage_inter`：preimage_inter {s t : Set β} : f ⁻¹' (s inter t) = f
 ⁻¹' s inter f ⁻¹' t
-/
theorem inter_inv : (s ∩ t)⁻¹ = s⁻¹ ∩ t⁻¹ :=
  preimage_inter

@[to_additive (attr := simp)]
/-
**Set.union_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_inv : (s union t)⁻¹ = s⁻¹ union t⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.preimage_union`：preimage_union {s t : Set β} : f ⁻¹' (s union t) = f
 ⁻¹' s union f ⁻¹' t
-/
theorem union_inv : (s ∪ t)⁻¹ = s⁻¹ ∪ t⁻¹ :=
  preimage_union

@[to_additive (attr := simp)]
/-
**Set.compl_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：compl_inv : sᶜ⁻¹ = s⁻¹ᶜ
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.preimage_compl`：preimage_compl {s : Set β} : f ⁻¹' sᶜ = (f ⁻¹' s)ᶜ
-/
theorem compl_inv : sᶜ⁻¹ = s⁻¹ᶜ :=
  preimage_compl

@[to_additive (attr := simp) neg_prod]
/-
**Set.inv_prod** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：inv_prod [Inv β] (s : Set α) (t : Set β) : (s ×ˢ t)⁻¹ = s⁻¹ ×ˢ t⁻¹
参数：s : Set α；t : Set β。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inv_prod [Inv β] (s : Set α) (t : Set β) : (s ×ˢ t)⁻¹ = s⁻¹ ×ˢ t⁻¹ := rfl

end Inv

section InvolutiveInv

variable [InvolutiveInv α] {s t : Set α} {a : α}

@[to_additive]
/-
**Set.inv_mem_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_mem_inv : a⁻¹ in s⁻¹ ↔ a in s
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
theorem inv_mem_inv : a⁻¹ ∈ s⁻¹ ↔ a ∈ s := by simp only [mem_inv, inv_inv]

@[to_additive (attr := simp)]
/-
**Set.nonempty_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：nonempty_inv : s⁻¹.Nonempty ↔ s.Nonempty
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Surjective.nonempty_preimage`：∀ {α : Type u_1} {β : Type u_2} {
f : α → β}, Function.Surjective f → ∀ {s : Set β}, (f ⁻¹' s).Nonempty ↔ s.Nonemp
ty
· 使用定理 `Function.Involutive.surjective`：∀ {α : Sort u} {f : α → α}, Function.Inv
olutive f → Function.Surjective f
· 使用定理 `inv_involutive`：inv_involutive : Function.Involutive (Inv.inv : G -> G)
-/
theorem nonempty_inv : s⁻¹.Nonempty ↔ s.Nonempty :=
  inv_involutive.surjective.nonempty_preimage

@[to_additive]
/-
**Set.Nonempty.inv** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : InvolutiveInv α] {s : Set α}, s.Nonempty → s⁻¹.No
nempty
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.nonempty_inv`：nonempty_inv : s⁻¹.Nonempty ↔ s.Nonempty
-/
theorem Nonempty.inv (h : s.Nonempty) : s⁻¹.Nonempty :=
  nonempty_inv.2 h

@[to_additive (attr := simp)]
/-
**Set.image_inv_eq_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congr_fun`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, f = g 
→ ∀ (a : α), f a = g a
· 使用定理 `Set.image_eq_preimage_of_inverse`：image_eq_preimage_of_inverse {f : α ->
 β} {g : β -> α} (h₁ : LeftInverse g f) (h₂ : RightInverse g f) : image f = prei
mage g
· 使用定理 `Function.Involutive.leftInverse`：∀ {α : Sort u} {f : α → α}, Function.In
volutive f → Function.LeftInverse f f
· 使用定理 `inv_involutive`：inv_involutive : Function.Involutive (Inv.inv : G -> G)
· 使用定理 `Function.Involutive.rightInverse`：∀ {α : Sort u} {f : α → α}, Function.I
nvolutive f → Function.RightInverse f f
-/
theorem image_inv_eq_inv : (·⁻¹) '' s = s⁻¹ :=
  congr_fun (image_eq_preimage_of_inverse inv_involutive.leftInverse inv_involutive.rightInverse) _

@[to_additive (attr := simp)]
/-
**Set.inv_eq_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_eq_empty : s⁻¹ = ∅ ↔ s = ∅
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.image_eq_empty`：image_eq_empty {α β} {f : α -> β} {s : Set α} : f ''
 s = ∅ ↔ s = ∅
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem inv_eq_empty : s⁻¹ = ∅ ↔ s = ∅ := by
  rw [← image_inv_eq_inv, image_eq_empty]

@[to_additive (attr := simp)]
/-
**Set.involutiveInv** 是 Mathlib 中的一个实例，位于命名空间 `Set`。
形式化陈述：involutiveInv : InvolutiveInv (Set α) where inv_inv s
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance involutiveInv : InvolutiveInv (Set α) where
  inv_inv s := by simp only [← inv_preimage, preimage_preimage, inv_inv, preimage_id']

@[to_additive (attr := simp)]
/-
**Set.inv_subset_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_subset_inv : s⁻¹ subseteq t⁻¹ ↔ s subseteq t
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Surjective.preimage_subset_preimage_iff`：∀ {α : Type u_1} {β : 
Type u_2} {f : α → β} {s t : Set β}, Function.Surjective f → (f ⁻¹' s ⊆ f ⁻¹' t 
↔ s ⊆ t)
· 使用定理 `Equiv.surjective`：∀ {α : Sort u} {β : Sort v} (e : α ≃ β), Function.Surj
ective ⇑e
-/
theorem inv_subset_inv : s⁻¹ ⊆ t⁻¹ ↔ s ⊆ t :=
  (Equiv.inv α).surjective.preimage_subset_preimage_iff

@[to_additive]
/-
**Set.inv_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_subset : s⁻¹ subseteq t ↔ s subseteq t⁻¹
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.inv_subset_inv`：inv_subset_inv : s⁻¹ subseteq t⁻¹ ↔ s subseteq t
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem inv_subset : s⁻¹ ⊆ t ↔ s ⊆ t⁻¹ := by rw [← inv_subset_inv, inv_inv]

@[to_additive]
/-
**Set.inv_eq_self_iff_inv_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_eq_self_iff_inv_subset : s⁻¹ = s ↔ s⁻¹ subseteq s
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `le_of_eq`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a = b → a ≤ b
· 使用引理 `antisymm`：antisymm [Std.Antisymm r] : a ≺ b -> b ≺ a -> a = b
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用定理 `Set.inv_subset`：inv_subset : s⁻¹ subseteq t ↔ s subseteq t⁻¹
-/
theorem inv_eq_self_iff_inv_subset : s⁻¹ = s ↔ s⁻¹ ⊆ s :=
  ⟨le_of_eq, fun h => antisymm h <| inv_subset.mp h⟩

@[to_additive (attr := simp)]
/-
**Set.inv_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_singleton (a : α) : ({a} : Set α)⁻¹ = {a⁻¹}
参数：a : α。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
theorem inv_singleton (a : α) : ({a} : Set α)⁻¹ = {a⁻¹} := by
  rw [← image_inv_eq_inv, image_singleton]

@[to_additive (attr := simp)]
/-
**Set.inv_insert** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_insert (a : α) (s : Set α) : (insert a s)⁻¹ = insert a⁻¹ s⁻¹
参数：a : α；s : Set α。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.insert_eq`：insert_eq (x : α) (s : Set α) : insert x s = ({x} : Set α
) union s
· 使用定理 `Set.union_inv`：union_inv : (s union t)⁻¹ = s⁻¹ union t⁻¹
· 使用定理 `Set.inv_singleton`：inv_singleton (a : α) : ({a} : Set α)⁻¹ = {a⁻¹}
-/
theorem inv_insert (a : α) (s : Set α) : (insert a s)⁻¹ = insert a⁻¹ s⁻¹ := by
  rw [insert_eq, union_inv, inv_singleton, insert_eq]

@[to_additive]
/-
**Set.inv_range** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inv_range {ι : Sort*} {f : ι -> α} : (range f)⁻¹ = range fun i => (f i)⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.range_comp`：range_comp (g : α -> β) (f : ι -> α) : range (g ∘ f) = g
 '' range f
-/
theorem inv_range {ι : Sort*} {f : ι → α} : (range f)⁻¹ = range fun i => (f i)⁻¹ := by
  rw [← image_inv_eq_inv]
  exact (range_comp ..).symm

@[to_additive]
/-
**Set.inv_range'** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：inv_range' {ι : Type*} {f : ι -> α} : (range f)⁻¹ = range f⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.inv_range`：inv_range {ι : Sort*} {f : ι -> α} : (range f)⁻¹ = range 
fun i => (f i)⁻¹
-/
lemma inv_range' {ι : Type*} {f : ι → α} : (range f)⁻¹ = range f⁻¹ := inv_range

@[to_additive]
/-
**Set.image_inv_of_apply_inv_eq** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_inv_of_apply_inv_eq {f g : α -> β} (H : forall x in s, f x⁻¹ = g x) 
: f '' (s⁻¹) = g '' s
参数：H : forall x in s, f x⁻¹ = g x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.image_image`：image_image (g : β -> γ) (f : α -> β) (s : Set α) : g '
' f '' s = (fun x => g (f x)) '' s
· 使用定理 `Set.image_congr`：image_congr {f g : α -> β} {s : Set α} (h : forall a in
 s, f a = g a) : f '' s = g '' s
-/
theorem image_inv_of_apply_inv_eq {f g : α → β} (H : ∀ x ∈ s, f x⁻¹ = g x) :
    f '' (s⁻¹) = g '' s := by
  rw [← Set.image_inv_eq_inv, Set.image_image]; exact Set.image_congr H

@[to_additive]
/-
**Set.image_inv_of_apply_inv_eq_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_inv_of_apply_inv_eq_inv [InvolutiveInv β] {f g : α -> β} (H : forall
 x in s, f x⁻¹ = (g x)⁻¹) : f '' s⁻¹ = (g '' s)⁻¹
参数：H : forall x in s, f x⁻¹ = (g x)⁻¹。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.image_image`：image_image (g : β -> γ) (f : α -> β) (s : Set α) : g '
' f '' s = (fun x => g (f x)) '' s
· 使用定理 `Set.image_inv_of_apply_inv_eq`：image_inv_of_apply_inv_eq {f g : α -> β} 
(H : forall x in s, f x⁻¹ = g x) : f '' (s⁻¹) = g '' s
-/
theorem image_inv_of_apply_inv_eq_inv [InvolutiveInv β] {f g : α → β}
    (H : ∀ x ∈ s, f x⁻¹ = (g x)⁻¹) : f '' s⁻¹ = (g '' s)⁻¹ := by
  conv_rhs => rw [← image_inv_eq_inv, image_image, ← image_inv_of_apply_inv_eq H]

@[to_additive (attr := simp)]
/-
**Set.forall_inv_mem** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：forall_inv_mem {p : α -> Prop} : (forall x, x⁻¹ in s -> p x) ↔ forall x in
 s, p x⁻¹
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Equiv.forall_congr_right`：∀ {α : Sort u} {β : Sort v} {q : β → Prop} (e 
: α ≃ β), (∀ (a : α), q (e a)) ↔ ∀ (b : β), q b
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `implies_congr`：∀ {p₁ p₂ : Sort u} {q₁ q₂ : Sort v}, p₁ = p₂ → q₁ = q₂ → 
(p₁ → q₁) = (p₂ → q₂)
· 使用定理 `congrFun`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, f = g →
 ∀ (a : α), f a = g a
· 使用定理 `Equiv.inv_apply`：∀ (G : Type u_14) [inst : InvolutiveInv G], ⇑(Equiv.inv
 G) = Inv.inv
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
theorem forall_inv_mem {p : α → Prop} : (∀ x, x⁻¹ ∈ s → p x) ↔ ∀ x ∈ s, p x⁻¹ := by
  rw [← (Equiv.inv _).forall_congr_right]
  simp

@[to_additive (attr := simp)]
/-
**Set.exists_inv_mem** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：exists_inv_mem {p : α -> Prop} : (exists x, x⁻¹ in s ∧ p x) ↔ exists x in 
s, p x⁻¹
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Equiv.exists_congr_right`：∀ {α : Sort u} {β : Sort v} {q : β → Prop} (e 
: α ≃ β), (∃ a, q (e a)) ↔ ∃ b, q b
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrFun`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, f = g →
 ∀ (a : α), f a = g a
· 使用定理 `Equiv.inv_apply`：∀ (G : Type u_14) [inst : InvolutiveInv G], ⇑(Equiv.inv
 G) = Inv.inv
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
theorem exists_inv_mem {p : α → Prop} : (∃ x, x⁻¹ ∈ s ∧ p x) ↔ ∃ x ∈ s, p x⁻¹ := by
  rw [← (Equiv.inv _).exists_congr_right]
  simp

open MulOpposite

@[to_additive]
/-
**Set.image_op_inv** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_op_inv : op '' s⁻¹ = (op '' s)⁻¹
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `Function.Semiconj.set_image`：∀ {α : Type u_1} {β : Type u_2} {f : α → β}
 {ga : α → α} {gb : β → β},   Function.Semiconj f ga gb → Function.Semiconj (Set
.image f) (Set.im…
· 使用定理 `MulOpposite.op_inv`：∀ {α : Type u_1} [inst : Inv α] (x : α), MulOpposite
.op x⁻¹ = (MulOpposite.op x)⁻¹
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
theorem image_op_inv : op '' s⁻¹ = (op '' s)⁻¹ := by
  simp_rw [← image_inv_eq_inv, Function.Semiconj.set_image op_inv s]

end InvolutiveInv

end Inv

open scoped Pointwise

/-! ### Set addition/multiplication -/


section Mul

variable {ι : Sort*} {κ : ι → Sort*} [Mul α] {s s₁ s₂ t t₁ t₂ u : Set α} {a b : α}

/-- The pointwise multiplication of sets `s * t` and `t` is defined as `{x * y | x ∈ s, y ∈ t}` in
scope `Pointwise`. -/
@[to_additive (attr := instance_reducible)
      /-- The pointwise addition of sets `s + t` is defined as `{x + y | x ∈ s, y ∈ t}` in locale
      `Pointwise`. -/]
/-
**Set.mul** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [Mul α] → Mul (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def mul : Mul (Set α) :=
  ⟨image2 (· * ·)⟩

scoped[Pointwise] attribute [instance] Set.mul Set.add

@[to_additive (attr := simp)]
/-
**Set.image2_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image2_mul : image2 (· * ·) s t = s * t
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem image2_mul : image2 (· * ·) s t = s * t :=
  rfl

@[to_additive (attr := push)]
/-
**Set.mem_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mem_mul : a in s * t ↔ exists x in s, exists y in t, x * y = a
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_mul : a ∈ s * t ↔ ∃ x ∈ s, ∃ y ∈ t, x * y = a :=
  Iff.rfl

@[to_additive]
/-
**Set.mul_mem_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_mem_mul : a in s -> b in t -> a * b in s * t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mem_image2_of_mem`：mem_image2_of_mem (ha : a in s) (hb : b in t) : f
 a b in image2 f s t
-/
theorem mul_mem_mul : a ∈ s → b ∈ t → a * b ∈ s * t :=
  mem_image2_of_mem

@[to_additive add_image_prod]
/-
**Set.image_mul_prod** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul_prod : (fun x : α × α => x.fst * x.snd) '' s ×ˢ t = s * t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `Set.image_prod`：image_prod : (fun x : α × β => f x.1 x.2) '' s ×ˢ t = im
age2 f s t
-/
theorem image_mul_prod : (fun x : α × α => x.fst * x.snd) '' s ×ˢ t = s * t :=
  image_prod _

@[to_additive (attr := simp)]
/-
**Set.empty_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：empty_mul : ∅ * s = ∅
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_empty_left`：image2_empty_left : image2 f ∅ t = ∅
-/
theorem empty_mul : ∅ * s = ∅ :=
  image2_empty_left

@[to_additive (attr := simp)]
/-
**Set.mul_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_empty : s * ∅ = ∅
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_empty_right`：image2_empty_right : image2 f s ∅ = ∅
-/
theorem mul_empty : s * ∅ = ∅ :=
  image2_empty_right

@[to_additive (attr := simp)]
/-
**Set.mul_eq_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_eq_empty : s * t = ∅ ↔ s = ∅ ∨ t = ∅
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_eq_empty_iff`：image2_eq_empty_iff : image2 f s t = ∅ ↔ s = ∅ 
∨ t = ∅
-/
theorem mul_eq_empty : s * t = ∅ ↔ s = ∅ ∨ t = ∅ :=
  image2_eq_empty_iff

@[to_additive (attr := simp)]
/-
**Set.mul_nonempty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_nonempty : (s * t).Nonempty ↔ s.Nonempty ∧ t.Nonempty
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_nonempty_iff`：image2_nonempty_iff : (image2 f s t).Nonempty ↔
 s.Nonempty ∧ t.Nonempty
-/
theorem mul_nonempty : (s * t).Nonempty ↔ s.Nonempty ∧ t.Nonempty :=
  image2_nonempty_iff

@[to_additive]
/-
**Set.Nonempty.mul** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] {s t : Set α}, s.Nonempty → t.Nonempty → (
s * t).Nonempty
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.image2`：∀ {α : Type u_1} {β : Type u_3} {γ : Type u_5} {f :
 α → β → γ} {s : Set α} {t : Set β},   s.Nonempty → t.Nonempty → (Set.image2 f s
 t).Nonem…
-/
theorem Nonempty.mul : s.Nonempty → t.Nonempty → (s * t).Nonempty :=
  Nonempty.image2

@[to_additive]
/-
**Set.Nonempty.of_mul_left** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] {s t : Set α}, (s * t).Nonempty → s.Nonemp
ty
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.of_image2_left`：∀ {α : Type u_1} {β : Type u_3} {γ : Type u
_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → s.N
onempty
-/
theorem Nonempty.of_mul_left : (s * t).Nonempty → s.Nonempty :=
  Nonempty.of_image2_left

@[to_additive]
/-
**Set.Nonempty.of_mul_right** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] {s t : Set α}, (s * t).Nonempty → t.Nonemp
ty
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.of_image2_right`：∀ {α : Type u_1} {β : Type u_3} {γ : Type 
u_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → t.
Nonempty
-/
theorem Nonempty.of_mul_right : (s * t).Nonempty → t.Nonempty :=
  Nonempty.of_image2_right

@[to_additive (attr := simp)]
/-
**Set.mul_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_singleton : s * {b} = (· * b) '' s
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton_right`：image2_singleton_right : image2 f s {b} = (f
un a => f a b) '' s
-/
theorem mul_singleton : s * {b} = (· * b) '' s :=
  image2_singleton_right

@[to_additive (attr := simp)]
/-
**Set.singleton_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singleton_mul : {a} * t = (a * ·) '' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton_left`：image2_singleton_left : image2 f {a} t = f a 
'' t
-/
theorem singleton_mul : {a} * t = (a * ·) '' t :=
  image2_singleton_left

@[to_additive]
/-
**Set.singleton_mul_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singleton_mul_singleton : ({a} : Set α) * {b} = {a * b}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton`：image2_singleton : image2 f {a} {b} = {f a b}
-/
theorem singleton_mul_singleton : ({a} : Set α) * {b} = {a * b} :=
  image2_singleton

@[to_additive]
/-
**Set.mul_subset_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_subset_mul : s₁ subseteq t₁ -> s₂ subseteq t₂ -> s₁ * s₂ subseteq t₁ *
 t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset`：image2_subset (hs : s subseteq s') (ht : t subseteq t
') : image2 f s t subseteq image2 f s' t'
-/
theorem mul_subset_mul : s₁ ⊆ t₁ → s₂ ⊆ t₂ → s₁ * s₂ ⊆ t₁ * t₂ :=
  image2_subset

@[to_additive]
/-
**Set.mul_subset_mul_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_subset_mul_left : t₁ subseteq t₂ -> s * t₁ subseteq s * t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_left`：image2_subset_left (ht : t subseteq t') : image2
 f s t subseteq image2 f s t'
-/
theorem mul_subset_mul_left : t₁ ⊆ t₂ → s * t₁ ⊆ s * t₂ :=
  image2_subset_left

@[to_additive]
/-
**Set.mul_subset_mul_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_subset_mul_right : s₁ subseteq s₂ -> s₁ * t subseteq s₂ * t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_right`：image2_subset_right (hs : s subseteq s') : imag
e2 f s t subseteq image2 f s' t
-/
theorem mul_subset_mul_right : s₁ ⊆ s₂ → s₁ * t ⊆ s₂ * t :=
  image2_subset_right
/-
**Set.** 是 Mathlib 中的一个实例，位于命名空间 `Set`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[to_additive] instance : MulLeftMono (Set α) where elim _s _t₁ _t₂ := mul_subset_mul_left
/-
**Set.** 是 Mathlib 中的一个实例，位于命名空间 `Set`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[to_additive] instance : MulRightMono (Set α) where elim _t _s₁ _s₂ := mul_subset_mul_right

@[to_additive]
/-
**Set.mul_subset_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_subset_iff : s * t subseteq u ↔ forall x in s, forall y in t, x * y in
 u
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_iff`：image2_subset_iff {u : Set γ} : image2 f s t subs
eteq u ↔ forall x in s, forall y in t, f x y in u
-/
theorem mul_subset_iff : s * t ⊆ u ↔ ∀ x ∈ s, ∀ y ∈ t, x * y ∈ u :=
  image2_subset_iff

@[to_additive]
/-
**Set.union_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_mul : (s₁ union s₂) * t = s₁ * t union s₂ * t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_left`：image2_union_left : image2 f (s union s') t = ima
ge2 f s t union image2 f s' t
-/
theorem union_mul : (s₁ ∪ s₂) * t = s₁ * t ∪ s₂ * t :=
  image2_union_left

@[to_additive]
/-
**Set.mul_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_union : s * (t₁ union t₂) = s * t₁ union s * t₂
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_right`：image2_union_right : image2 f s (t union t') = i
mage2 f s t union image2 f s t'
-/
theorem mul_union : s * (t₁ ∪ t₂) = s * t₁ ∪ s * t₂ :=
  image2_union_right

@[to_additive]
/-
**Set.inter_mul_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_mul_subset : s₁ inter s₂ * t subseteq s₁ * t inter (s₂ * t)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_subset_left`：image2_inter_subset_left : image2 f (s int
er s') t subseteq image2 f s t inter image2 f s' t
-/
theorem inter_mul_subset : s₁ ∩ s₂ * t ⊆ s₁ * t ∩ (s₂ * t) :=
  image2_inter_subset_left

@[to_additive]
/-
**Set.mul_inter_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_inter_subset : s * (t₁ inter t₂) subseteq s * t₁ inter (s * t₂)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_subset_right`：image2_inter_subset_right : image2 f s (t
 inter t') subseteq image2 f s t inter image2 f s t'
-/
theorem mul_inter_subset : s * (t₁ ∩ t₂) ⊆ s * t₁ ∩ (s * t₂) :=
  image2_inter_subset_right

@[to_additive]
/-
**Set.inter_mul_union_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_mul_union_subset_union : s₁ inter s₂ * (t₁ union t₂) subseteq s₁ * t
₁ union s₂ * t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_union_subset_union`：image2_inter_union_subset_union : i
mage2 f (s inter s') (t union t') subseteq image2 f s t union image2 f s' t'
-/
theorem inter_mul_union_subset_union : s₁ ∩ s₂ * (t₁ ∪ t₂) ⊆ s₁ * t₁ ∪ s₂ * t₂ :=
  image2_inter_union_subset_union

@[to_additive]
/-
**Set.union_mul_inter_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_mul_inter_subset_union : (s₁ union s₂) * (t₁ inter t₂) subseteq s₁ *
 t₁ union s₂ * t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_inter_subset_union`：image2_union_inter_subset_union : i
mage2 f (s union s') (t inter t') subseteq image2 f s t union image2 f s' t'
-/
theorem union_mul_inter_subset_union : (s₁ ∪ s₂) * (t₁ ∩ t₂) ⊆ s₁ * t₁ ∪ s₂ * t₂ :=
  image2_union_inter_subset_union

/-- The singleton operation as a `MulHom`. -/
@[to_additive /-- The singleton operation as an `AddHom`. -/]
/-
**Set.singletonMulHom** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：singletonMulHom : α ->ₙ* Set α where toFun
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The singleton operation as a `MulHom`.
-/
def singletonMulHom : α →ₙ* Set α where
  toFun := singleton
  map_mul' _ _ := singleton_mul_singleton.symm

@[to_additive (attr := simp)]
/-
**Set.coe_singletonMulHom** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：coe_singletonMulHom : (singletonMulHom : α -> Set α) = singleton
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_singletonMulHom : (singletonMulHom : α → Set α) = singleton :=
  rfl

@[to_additive (attr := simp)]
/-
**Set.singletonMulHom_apply** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singletonMulHom_apply (a : α) : singletonMulHom a = {a}
参数：a : α。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem singletonMulHom_apply (a : α) : singletonMulHom a = {a} :=
  rfl

open MulOpposite

@[to_additive (attr := simp)]
/-
**Set.image_op_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_op_mul : op '' (s * t) = op '' t * op '' s
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image_image2_antidistrib`：image_image2_antidistrib {g : γ -> δ} {f' 
: β' -> α' -> δ} {g₁ : β -> β'} {g₂ : α -> α'} (h_antidistrib : forall a b, g (f
 a b) = f' (g₁ b) …
· 使用定理 `MulOpposite.op_mul`：∀ {α : Type u_1} [inst : Mul α] (x y : α), MulOpposi
te.op (x * y) = MulOpposite.op y * MulOpposite.op x
-/
theorem image_op_mul : op '' (s * t) = op '' t * op '' s :=
  image_image2_antidistrib op_mul

@[to_additive (attr := simp) prod_add_prod_comm]
/-
**Set.prod_mul_prod_comm** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：prod_mul_prod_comm [Mul β] (s₁ s₂ : Set α) (t₁ t₂ : Set β) : (s₁ ×ˢ t₁) * 
(s₂ ×ˢ t₂) = (s₁ * s₂) ×ˢ (t₁ * t₂)
参数：s₁ s₂ : Set α；t₁ t₂ : Set β。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.ext`：ext {a b : Set α} (h : forall (x : α), x in a ↔ x in b) : a = b
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `Prod.mk.injEq`：∀ {α : Type u} {β : Type v} (fst : α) (snd : β) (fst_1 : 
α) (snd_1 : β),   ((fst, snd) = (fst_1, snd_1)) = (fst = fst_1 ∧ snd = snd_1)
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `true_and`：∀ (p : Prop), (True ∧ p) = p
-/
lemma prod_mul_prod_comm [Mul β] (s₁ s₂ : Set α) (t₁ t₂ : Set β) :
    (s₁ ×ˢ t₁) * (s₂ ×ˢ t₂) = (s₁ * s₂) ×ˢ (t₁ * t₂) := by ext; simp [mem_mul]; aesop

end Mul

/-! ### Set subtraction/division -/


section Div

variable {ι : Sort*} {κ : ι → Sort*} [Div α] {s s₁ s₂ t t₁ t₂ u : Set α} {a b : α}

/-- The pointwise division of sets `s / t` is defined as `{x / y | x ∈ s, y ∈ t}` in locale
`Pointwise`. -/
@[to_additive (attr := instance_reducible)
      /-- The pointwise subtraction of sets `s - t` is defined as `{x - y | x ∈ s, y ∈ t}` in locale
      `Pointwise`. -/]
/-
**Set.div** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [Div α] → Div (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def div : Div (Set α) :=
  ⟨image2 (· / ·)⟩

scoped[Pointwise] attribute [instance] Set.div Set.sub

@[to_additive (attr := simp)]
/-
**Set.image2_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image2_div : image2 (· / ·) s t = s / t
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem image2_div : image2 (· / ·) s t = s / t :=
  rfl

@[to_additive (attr := push)]
/-
**Set.mem_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mem_div : a in s / t ↔ exists x in s, exists y in t, x / y = a
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.rfl`：∀ {a : Prop}, a ↔ a
-/
theorem mem_div : a ∈ s / t ↔ ∃ x ∈ s, ∃ y ∈ t, x / y = a :=
  Iff.rfl

@[to_additive]
/-
**Set.div_mem_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_mem_div : a in s -> b in t -> a / b in s / t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mem_image2_of_mem`：mem_image2_of_mem (ha : a in s) (hb : b in t) : f
 a b in image2 f s t
-/
theorem div_mem_div : a ∈ s → b ∈ t → a / b ∈ s / t :=
  mem_image2_of_mem

@[to_additive sub_image_prod]
/-
**Set.image_div_prod** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_div_prod : (fun x : α × α => x.fst / x.snd) '' s ×ˢ t = s / t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `Set.image_prod`：image_prod : (fun x : α × β => f x.1 x.2) '' s ×ˢ t = im
age2 f s t
-/
theorem image_div_prod : (fun x : α × α => x.fst / x.snd) '' s ×ˢ t = s / t :=
  image_prod _

@[to_additive (attr := simp)]
/-
**Set.empty_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：empty_div : ∅ / s = ∅
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_empty_left`：image2_empty_left : image2 f ∅ t = ∅
-/
theorem empty_div : ∅ / s = ∅ :=
  image2_empty_left

@[to_additive (attr := simp)]
/-
**Set.div_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_empty : s / ∅ = ∅
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_empty_right`：image2_empty_right : image2 f s ∅ = ∅
-/
theorem div_empty : s / ∅ = ∅ :=
  image2_empty_right

@[to_additive (attr := simp)]
/-
**Set.div_eq_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_eq_empty : s / t = ∅ ↔ s = ∅ ∨ t = ∅
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_eq_empty_iff`：image2_eq_empty_iff : image2 f s t = ∅ ↔ s = ∅ 
∨ t = ∅
-/
theorem div_eq_empty : s / t = ∅ ↔ s = ∅ ∨ t = ∅ :=
  image2_eq_empty_iff

@[to_additive (attr := simp)]
/-
**Set.div_nonempty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_nonempty : (s / t).Nonempty ↔ s.Nonempty ∧ t.Nonempty
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_nonempty_iff`：image2_nonempty_iff : (image2 f s t).Nonempty ↔
 s.Nonempty ∧ t.Nonempty
-/
theorem div_nonempty : (s / t).Nonempty ↔ s.Nonempty ∧ t.Nonempty :=
  image2_nonempty_iff

@[to_additive]
/-
**Set.Nonempty.div** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Div α] {s t : Set α}, s.Nonempty → t.Nonempty → (
s / t).Nonempty
参数：s / t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.image2`：∀ {α : Type u_1} {β : Type u_3} {γ : Type u_5} {f :
 α → β → γ} {s : Set α} {t : Set β},   s.Nonempty → t.Nonempty → (Set.image2 f s
 t).Nonem…
-/
theorem Nonempty.div : s.Nonempty → t.Nonempty → (s / t).Nonempty :=
  Nonempty.image2

@[to_additive]
/-
**Set.Nonempty.of_div_left** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Div α] {s t : Set α}, (s / t).Nonempty → s.Nonemp
ty
参数：s / t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.of_image2_left`：∀ {α : Type u_1} {β : Type u_3} {γ : Type u
_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → s.N
onempty
-/
theorem Nonempty.of_div_left : (s / t).Nonempty → s.Nonempty :=
  Nonempty.of_image2_left

@[to_additive]
/-
**Set.Nonempty.of_div_right** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Div α] {s t : Set α}, (s / t).Nonempty → t.Nonemp
ty
参数：s / t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.of_image2_right`：∀ {α : Type u_1} {β : Type u_3} {γ : Type 
u_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → t.
Nonempty
-/
theorem Nonempty.of_div_right : (s / t).Nonempty → t.Nonempty :=
  Nonempty.of_image2_right

@[to_additive (attr := simp)]
/-
**Set.div_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_singleton : s / {b} = (· / b) '' s
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton_right`：image2_singleton_right : image2 f s {b} = (f
un a => f a b) '' s
-/
theorem div_singleton : s / {b} = (· / b) '' s :=
  image2_singleton_right

@[to_additive (attr := simp)]
/-
**Set.singleton_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singleton_div : {a} / t = (· / ·) a '' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton_left`：image2_singleton_left : image2 f {a} t = f a 
'' t
-/
theorem singleton_div : {a} / t = (· / ·) a '' t :=
  image2_singleton_left

@[to_additive]
/-
**Set.singleton_div_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singleton_div_singleton : ({a} : Set α) / {b} = {a / b}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_singleton`：image2_singleton : image2 f {a} {b} = {f a b}
-/
theorem singleton_div_singleton : ({a} : Set α) / {b} = {a / b} :=
  image2_singleton

@[to_additive (attr := mono, gcongr)]
/-
**Set.div_subset_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_subset_div : s₁ subseteq t₁ -> s₂ subseteq t₂ -> s₁ / s₂ subseteq t₁ /
 t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset`：image2_subset (hs : s subseteq s') (ht : t subseteq t
') : image2 f s t subseteq image2 f s' t'
-/
theorem div_subset_div : s₁ ⊆ t₁ → s₂ ⊆ t₂ → s₁ / s₂ ⊆ t₁ / t₂ :=
  image2_subset

@[to_additive]
/-
**Set.div_subset_div_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_subset_div_left : t₁ subseteq t₂ -> s / t₁ subseteq s / t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_left`：image2_subset_left (ht : t subseteq t') : image2
 f s t subseteq image2 f s t'
-/
theorem div_subset_div_left : t₁ ⊆ t₂ → s / t₁ ⊆ s / t₂ :=
  image2_subset_left

@[to_additive]
/-
**Set.div_subset_div_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_subset_div_right : s₁ subseteq s₂ -> s₁ / t subseteq s₂ / t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_right`：image2_subset_right (hs : s subseteq s') : imag
e2 f s t subseteq image2 f s' t
-/
theorem div_subset_div_right : s₁ ⊆ s₂ → s₁ / t ⊆ s₂ / t :=
  image2_subset_right

@[to_additive]
/-
**Set.div_subset_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_subset_iff : s / t subseteq u ↔ forall x in s, forall y in t, x / y in
 u
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_subset_iff`：image2_subset_iff {u : Set γ} : image2 f s t subs
eteq u ↔ forall x in s, forall y in t, f x y in u
-/
theorem div_subset_iff : s / t ⊆ u ↔ ∀ x ∈ s, ∀ y ∈ t, x / y ∈ u :=
  image2_subset_iff

@[to_additive]
/-
**Set.union_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_div : (s₁ union s₂) / t = s₁ / t union s₂ / t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_left`：image2_union_left : image2 f (s union s') t = ima
ge2 f s t union image2 f s' t
-/
theorem union_div : (s₁ ∪ s₂) / t = s₁ / t ∪ s₂ / t :=
  image2_union_left

@[to_additive]
/-
**Set.div_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_union : s / (t₁ union t₂) = s / t₁ union s / t₂
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_right`：image2_union_right : image2 f s (t union t') = i
mage2 f s t union image2 f s t'
-/
theorem div_union : s / (t₁ ∪ t₂) = s / t₁ ∪ s / t₂ :=
  image2_union_right

@[to_additive]
/-
**Set.inter_div_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_div_subset : s₁ inter s₂ / t subseteq s₁ / t inter (s₂ / t)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_subset_left`：image2_inter_subset_left : image2 f (s int
er s') t subseteq image2 f s t inter image2 f s' t
-/
theorem inter_div_subset : s₁ ∩ s₂ / t ⊆ s₁ / t ∩ (s₂ / t) :=
  image2_inter_subset_left

@[to_additive]
/-
**Set.div_inter_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：div_inter_subset : s / (t₁ inter t₂) subseteq s / t₁ inter (s / t₂)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_subset_right`：image2_inter_subset_right : image2 f s (t
 inter t') subseteq image2 f s t inter image2 f s t'
-/
theorem div_inter_subset : s / (t₁ ∩ t₂) ⊆ s / t₁ ∩ (s / t₂) :=
  image2_inter_subset_right

@[to_additive]
/-
**Set.inter_div_union_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_div_union_subset_union : s₁ inter s₂ / (t₁ union t₂) subseteq s₁ / t
₁ union s₂ / t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_union_subset_union`：image2_inter_union_subset_union : i
mage2 f (s inter s') (t union t') subseteq image2 f s t union image2 f s' t'
-/
theorem inter_div_union_subset_union : s₁ ∩ s₂ / (t₁ ∪ t₂) ⊆ s₁ / t₁ ∪ s₂ / t₂ :=
  image2_inter_union_subset_union

@[to_additive]
/-
**Set.union_div_inter_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_div_inter_subset_union : (s₁ union s₂) / (t₁ inter t₂) subseteq s₁ /
 t₁ union s₂ / t₂
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_inter_subset_union`：image2_union_inter_subset_union : i
mage2 f (s union s') (t inter t') subseteq image2 f s t union image2 f s' t'
-/
theorem union_div_inter_subset_union : (s₁ ∪ s₂) / (t₁ ∩ t₂) ⊆ s₁ / t₁ ∪ s₂ / t₂ :=
  image2_union_inter_subset_union

@[to_additive (attr := simp) prod_sub_prod_comm]
/-
**Set.prod_div_prod_comm** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：prod_div_prod_comm [Div β] (s₁ s₂ : Set α) (t₁ t₂ : Set β) : (s₁ ×ˢ t₁) / 
(s₂ ×ˢ t₂) = (s₁ / s₂) ×ˢ (t₁ / t₂)
参数：s₁ s₂ : Set α；t₁ t₂ : Set β。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.ext`：ext {a b : Set α} (h : forall (x : α), x in a ↔ x in b) : a = b
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `Prod.mk.injEq`：∀ {α : Type u} {β : Type v} (fst : α) (snd : β) (fst_1 : 
α) (snd_1 : β),   ((fst, snd) = (fst_1, snd_1)) = (fst = fst_1 ∧ snd = snd_1)
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `true_and`：∀ (p : Prop), (True ∧ p) = p
-/
lemma prod_div_prod_comm [Div β] (s₁ s₂ : Set α) (t₁ t₂ : Set β) :
    (s₁ ×ˢ t₁) / (s₂ ×ˢ t₂) = (s₁ / s₂) ×ˢ (t₁ / t₂) := by aesop (add simp mem_div)

end Div

-- TODO: rename `NPow` to `npow` and `ZPow` to `zpow`.
/-- Repeated pointwise multiplication (not the same as pointwise repeated multiplication!) of a
`Set`. See note [pointwise nat action]. -/
@[to_additive (attr := instance_reducible)
/-- Repeated pointwise addition (not the same as pointwise repeated addition!) of a `Set`. See
note [pointwise nat action]. -/]
/-
**Set.NPow** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [One α] → [Mul α] → Pow (Set α) ℕ
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def NPow [One α] [Mul α] : Pow (Set α) ℕ :=
  ⟨fun s n => npowRec n s⟩

/-- Repeated pointwise multiplication/division (not the same as pointwise repeated
multiplication/division!) of a `Set`. See note [pointwise nat action]. -/
@[to_additive (attr := instance_reducible)
/-- Repeated pointwise addition/subtraction (not the same as pointwise repeated
addition/subtraction!) of a `Set`. See note [pointwise nat action]. -/]
/-
**Set.ZPow** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [One α] → [Mul α] → [Inv α] → Pow (Set α) ℤ
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def ZPow [One α] [Mul α] [Inv α] : Pow (Set α) ℤ :=
  ⟨fun s n => zpowRec npowRec n s⟩

scoped[Pointwise] attribute [instance] Set.NSMul Set.NPow Set.ZSMul Set.ZPow

/-- `Set α` is a `Semigroup` under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
  /-- `Set α` is an `AddSemigroup` under pointwise operations if `α` is. -/]
/-
**Set.semigroup** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [Semigroup α] → Semigroup (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def semigroup [Semigroup α] : Semigroup (Set α) :=
  { Set.mul with mul_assoc := fun _ _ _ => image2_assoc mul_assoc }

section CommSemigroup

variable [CommSemigroup α] {s t : Set α}

/-- `Set α` is a `CommSemigroup` under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
  /-- `Set α` is an `AddCommSemigroup` under pointwise operations if `α` is. -/]
/-
**Set.commSemigroup** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [CommSemigroup α] → CommSemigroup (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def commSemigroup : CommSemigroup (Set α) :=
  { Set.semigroup with mul_comm := fun _ _ => image2_comm mul_comm }

@[to_additive]
/-
**Set.inter_mul_union_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：inter_mul_union_subset : s inter t * (s union t) subseteq s * t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_inter_union_subset`：image2_inter_union_subset {f : α -> α -> 
β} {s t : Set α} (hf : forall a b, f a b = f b a) : image2 f (s inter t) (s unio
n t) subseteq image…
· 使用定理 `mul_comm`：mul_comm : forall a b : G, a * b = b * a
-/
theorem inter_mul_union_subset : s ∩ t * (s ∪ t) ⊆ s * t :=
  image2_inter_union_subset mul_comm

@[to_additive]
/-
**Set.union_mul_inter_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：union_mul_inter_subset : (s union t) * (s inter t) subseteq s * t
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image2_union_inter_subset`：image2_union_inter_subset {f : α -> α -> 
β} {s t : Set α} (hf : forall a b, f a b = f b a) : image2 f (s union t) (s inte
r t) subseteq image…
· 使用定理 `mul_comm`：mul_comm : forall a b : G, a * b = b * a
-/
theorem union_mul_inter_subset : (s ∪ t) * (s ∩ t) ⊆ s * t :=
  image2_union_inter_subset mul_comm

end CommSemigroup

section MulOneClass

variable [MulOneClass α]

/-- `Set α` is a `MulOneClass` under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
  /-- `Set α` is an `AddZeroClass` under pointwise operations if `α` is. -/]
/-
**Set.mulOneClass** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [MulOneClass α] → MulOneClass (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def mulOneClass : MulOneClass (Set α) :=
  { Set.one, Set.mul with
    mul_one := image2_right_identity mul_one
    one_mul := image2_left_identity one_mul }

scoped[Pointwise]
  attribute [instance]
    Set.mulOneClass Set.addZeroClass Set.semigroup Set.addSemigroup Set.commSemigroup
    Set.addCommSemigroup

@[to_additive]
/-
**Set.subset_mul_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：subset_mul_left (s : Set α) {t : Set α} (ht : (1 : α) in t) : s subseteq s
 * t
参数：s : Set α；ht : (1 : α) in t。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `mul_one`：mul_one : forall a : M, a * 1 = a
-/
theorem subset_mul_left (s : Set α) {t : Set α} (ht : (1 : α) ∈ t) : s ⊆ s * t := fun x hx =>
  ⟨x, hx, 1, ht, mul_one _⟩

@[to_additive]
/-
**Set.subset_mul_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：subset_mul_right {s : Set α} (t : Set α) (hs : (1 : α) in s) : t subseteq 
s * t
参数：t : Set α；hs : (1 : α) in s。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `one_mul`：one_mul : forall a : M, 1 * a = a
-/
theorem subset_mul_right {s : Set α} (t : Set α) (hs : (1 : α) ∈ s) : t ⊆ s * t := fun x hx =>
  ⟨1, hs, x, hx, one_mul _⟩

/-- The singleton operation as a `MonoidHom`. -/
@[to_additive /-- The singleton operation as an `AddMonoidHom`. -/]
/-
**Set.singletonMonoidHom** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：singletonMonoidHom : α ->* Set α
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The singleton operation as a `MonoidHom`.
-/
def singletonMonoidHom : α →* Set α :=
  { singletonMulHom, singletonOneHom with }

@[to_additive (attr := simp)]
/-
**Set.coe_singletonMonoidHom** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：coe_singletonMonoidHom : (singletonMonoidHom : α -> Set α) = singleton
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem coe_singletonMonoidHom : (singletonMonoidHom : α → Set α) = singleton :=
  rfl

@[to_additive (attr := simp)]
/-
**Set.singletonMonoidHom_apply** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：singletonMonoidHom_apply (a : α) : singletonMonoidHom a = {a}
参数：a : α。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem singletonMonoidHom_apply (a : α) : singletonMonoidHom a = {a} :=
  rfl

end MulOneClass

section Monoid

variable [Monoid α] {s t : Set α} {a : α} {m n : ℕ}

/-- `Set α` is a `Monoid` under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
  /-- `Set α` is an `AddMonoid` under pointwise operations if `α` is. -/]
/-
**Set.monoid** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [Monoid α] → Monoid (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def monoid : Monoid (Set α) :=
  { Set.semigroup, Set.mulOneClass, @Set.NPow α _ _ with }

scoped[Pointwise] attribute [instance] Set.monoid Set.addMonoid

-- `Set.pow_left_monotone` doesn't exist since it would syntactically be a special case of
-- `pow_left_mono`

@[to_additive]
/-
**Set.pow_right_monotone** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {s : Set α}, 1 ∈ s → Monotone fun x => 
s ^ x
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `pow_right_monotone`：pow_right_monotone (ha : 1 <= a) : Monotone fun n : 
Nat => a ^ n
· 使用定理 `Set.instMulLeftMono`：∀ {α : Type u_2} [inst : Mul α], MulLeftMono (Set α
)
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.one_subset`：one_subset : 1 subseteq s ↔ (1 : α) in s
-/
protected lemma pow_right_monotone (hs : 1 ∈ s) : Monotone (s ^ ·) :=
  pow_right_monotone <| one_subset.2 hs

@[to_additive]
/-
**Set.pow_subset_pow_left** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：pow_subset_pow_left (hst : s subseteq t) : s ^ n subseteq t ^ n
参数：hst : s subseteq t。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `pow_left_mono`：pow_left_mono (n : Nat) : Monotone fun a : M => a ^ n
· 使用定理 `Set.instMulLeftMono`：∀ {α : Type u_2} [inst : Mul α], MulLeftMono (Set α
)
· 使用定理 `Set.instMulRightMono`：∀ {α : Type u_2} [inst : Mul α], MulRightMono (Set
 α)
-/
lemma pow_subset_pow_left (hst : s ⊆ t) : s ^ n ⊆ t ^ n := pow_left_mono _ hst

@[to_additive]
/-
**Set.pow_subset_pow_right** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：pow_subset_pow_right (hs : 1 in s) (hmn : m <= n) : s ^ m subseteq s ^ n
参数：hs : 1 in s；hmn : m <= n。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.pow_right_monotone`：∀ {α : Type u_2} [inst : Monoid α] {s : Set α}, 
1 ∈ s → Monotone fun x => s ^ x
-/
lemma pow_subset_pow_right (hs : 1 ∈ s) (hmn : m ≤ n) : s ^ m ⊆ s ^ n :=
  Set.pow_right_monotone hs hmn

@[to_additive (attr := gcongr)]
/-
**Set.pow_subset_pow** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：pow_subset_pow (hst : s subseteq t) (ht : 1 in t) (hmn : m <= n) : s ^ m s
ubseteq t ^ n
参数：hst : s subseteq t；ht : 1 in t；hmn : m <= n。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `LE.le.trans`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a ≤ b → b
 ≤ c → a ≤ c
· 使用引理 `Set.pow_subset_pow_left`：pow_subset_pow_left (hst : s subseteq t) : s ^ 
n subseteq t ^ n
· 使用引理 `Set.pow_subset_pow_right`：pow_subset_pow_right (hs : 1 in s) (hmn : m <=
 n) : s ^ m subseteq s ^ n
-/
lemma pow_subset_pow (hst : s ⊆ t) (ht : 1 ∈ t) (hmn : m ≤ n) : s ^ m ⊆ t ^ n :=
  (pow_subset_pow_left hst).trans (pow_subset_pow_right ht hmn)

@[to_additive]
/-
**Set.subset_pow** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：subset_pow (hs : 1 in s) (hn : n != 0) : s subseteq s ^ n
参数：hs : 1 in s；hn : n != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用引理 `pow_one`：pow_one (a : M) : a ^ 1 = a
· 使用引理 `Set.pow_subset_pow_right`：pow_subset_pow_right (hs : 1 in s) (hmn : m <=
 n) : s ^ m subseteq s ^ n
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Nat.one_le_iff_ne_zero`：∀ {n : ℕ}, 1 ≤ n ↔ n ≠ 0
-/
lemma subset_pow (hs : 1 ∈ s) (hn : n ≠ 0) : s ⊆ s ^ n := by
  simpa using pow_subset_pow_right hs <| Nat.one_le_iff_ne_zero.2 hn

@[to_additive]
/-
**Set.pow_subset_pow_mul_of_sq_subset_mul** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：pow_subset_pow_mul_of_sq_subset_mul (hst : s ^ 2 subseteq t * s) (hn : n !
= 0) : s ^ n subseteq t ^ (n - 1) * s
参数：hst : s ^ 2 subseteq t * s；hn : n != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `pow_le_pow_mul_of_sq_le_mul`：pow_le_pow_mul_of_sq_le_mul [MulLeftMono M]
 {a b : M} (hab : a ^ 2 <= b * a) : forall {n}, n != 0 -> a ^ n <= b ^ (n - 1) *
 a | 1, _ => by s…
· 使用定理 `Set.instMulRightMono`：∀ {α : Type u_2} [inst : Mul α], MulRightMono (Set
 α)
· 使用定理 `Set.instMulLeftMono`：∀ {α : Type u_2} [inst : Mul α], MulLeftMono (Set α
)
-/
lemma pow_subset_pow_mul_of_sq_subset_mul (hst : s ^ 2 ⊆ t * s) (hn : n ≠ 0) :
    s ^ n ⊆ t ^ (n - 1) * s := pow_le_pow_mul_of_sq_le_mul hst hn

@[to_additive (attr := simp) nsmul_empty]
/-
**Set.empty_pow** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：empty_pow (hn : n != 0) : (∅ : Set α) ^ n = ∅
参数：hn : n != 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `pow_succ`：pow_succ (a : M) (n : Nat) : a ^ (n + 1) = a ^ n * a
· 使用定理 `Set.mul_empty`：mul_empty : s * ∅ = ∅
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
lemma empty_pow (hn : n ≠ 0) : (∅ : Set α) ^ n = ∅ := match n with | n + 1 => by simp [pow_succ]

@[to_additive]
/-
**Set.Nonempty.pow** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {s : Set α}, s.Nonempty → ∀ {n : ℕ}, (s
 ^ n).Nonempty
参数：s ^ n。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma Nonempty.pow (hs : s.Nonempty) : ∀ {n}, (s ^ n).Nonempty
  | 0 => by simp
  | n + 1 => by rw [pow_succ]; exact hs.pow.mul hs
/-
**Set.pow_eq_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {s : Set α} {n : ℕ}, s ^ n = ∅ ↔ s = ∅ 
∧ n ≠ 0
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `Mathlib.Tactic.Contrapose.contrapose₁`：contrapose₁ {p q : Prop} : (¬ q -
> ¬ p) -> (p -> q)
· 使用定理 `implies_congr`：∀ {p₁ p₂ : Sort u} {q₁ q₂ : Sort v}, p₁ = p₂ → q₁ = q₂ → 
(p₁ → q₁) = (p₂ → q₂)
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `Mathlib.Tactic.Push.not_and_or_eq`：not_and_or_eq : (¬ (p ∧ q)) = (¬ p ∨ 
¬ q)
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.Nonempty.pow`：∀ {α : Type u_2} [inst : Monoid α] {s : Set α}, s.None
mpty → ∀ {n : ℕ}, (s ^ n).Nonempty
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `pow_zero`：pow_zero (a : M) : a ^ 0 = 1
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用引理 `Set.empty_pow`：empty_pow (hn : n != 0) : (∅ : Set α) ^ n = ∅
-/
@[to_additive (attr := simp)] lemma pow_eq_empty : s ^ n = ∅ ↔ s = ∅ ∧ n ≠ 0 := by
  constructor
  · contrapose! +distrib
    rintro (hs | rfl)
    · exact hs.pow
    · simp
  · rintro ⟨rfl, hn⟩
    exact empty_pow hn

@[to_additive (attr := simp) nsmul_singleton]
/-
**Set.singleton_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] (a : α) (n : ℕ), {a} ^ n = {a ^ n}
参数：a : α；n : ℕ。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma singleton_pow (a : α) : ∀ n, ({a} : Set α) ^ n = {a ^ n}
  | 0 => by simp [singleton_one]
  | n + 1 => by simp [pow_succ, singleton_pow _ n]
/-
**Set.pow_mem_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {s : Set α} {a : α} {n : ℕ}, a ∈ s → a 
^ n ∈ s ^ n
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.singleton_pow`：∀ {α : Type u_2} [inst : Monoid α] (a : α) (n : ℕ), {
a} ^ n = {a ^ n}
· 使用引理 `Set.pow_subset_pow_left`：pow_subset_pow_left (hst : s subseteq t) : s ^ 
n subseteq t ^ n
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.singleton_subset_iff`：singleton_subset_iff {a : α} {s : Set α} : {a}
 subseteq s ↔ a in s
-/
@[to_additive] lemma pow_mem_pow (ha : a ∈ s) : a ^ n ∈ s ^ n := by
  simpa using pow_subset_pow_left (singleton_subset_iff.2 ha)
/-
**Set.one_mem_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {s : Set α} {n : ℕ}, 1 ∈ s → 1 ∈ s ^ n
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `one_pow`：one_pow {a : R} (b : Nat) (ha : IsNat a 1) : a ^ b = a
· 使用定理 `Set.pow_mem_pow`：∀ {α : Type u_2} [inst : Monoid α] {s : Set α} {a : α} 
{n : ℕ}, a ∈ s → a ^ n ∈ s ^ n
-/
@[to_additive] lemma one_mem_pow (hs : 1 ∈ s) : 1 ∈ s ^ n := by simpa using pow_mem_pow hs

@[to_additive]
/-
**Set.inter_pow_subset** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：inter_pow_subset : (s inter t) ^ n subseteq s ^ n inter t ^ n
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.subset_inter`：subset_inter {s t r : Set α} (rs : r subseteq s) (rt :
 r subseteq t) : r subseteq s inter t
· 使用定理 `pow_le_pow_left'`：∀ {M : Type u_3} [inst : Monoid M] [inst_1 : Preorder 
M] [MulLeftMono M] [MulRightMono M] {a b : M},   a ≤ b → ∀ (i : ℕ), a ^ i ≤ b ^ 
i
· 使用定理 `Set.instMulLeftMono`：∀ {α : Type u_2} [inst : Mul α], MulLeftMono (Set α
)
· 使用定理 `Set.instMulRightMono`：∀ {α : Type u_2} [inst : Mul α], MulRightMono (Set
 α)
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
-/
lemma inter_pow_subset : (s ∩ t) ^ n ⊆ s ^ n ∩ t ^ n := by apply subset_inter <;> gcongr <;> simp

@[to_additive]
/-
**Set.mul_univ_of_one_mem** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_univ_of_one_mem (hs : (1 : α) in s) : s * univ = univ
参数：hs : (1 : α) in s。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.eq_univ_iff_forall`：eq_univ_iff_forall {s : Set α} : s = univ ↔ fora
ll x, x in s
· 使用定理 `Set.mem_mul`：mem_mul : a in s * t ↔ exists x in s, exists y in t, x * y 
= a
· 使用定理 `Set.mem_univ`：mem_univ (x : α) : x in @univ α
· 使用定理 `one_mul`：one_mul : forall a : M, 1 * a = a
-/
theorem mul_univ_of_one_mem (hs : (1 : α) ∈ s) : s * univ = univ :=
  eq_univ_iff_forall.2 fun _ => mem_mul.2 ⟨_, hs, _, mem_univ _, one_mul _⟩

@[to_additive]
/-
**Set.univ_mul_of_one_mem** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：univ_mul_of_one_mem (ht : (1 : α) in t) : univ * t = univ
参数：ht : (1 : α) in t。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.eq_univ_iff_forall`：eq_univ_iff_forall {s : Set α} : s = univ ↔ fora
ll x, x in s
· 使用定理 `Set.mem_mul`：mem_mul : a in s * t ↔ exists x in s, exists y in t, x * y 
= a
· 使用定理 `Set.mem_univ`：mem_univ (x : α) : x in @univ α
· 使用定理 `mul_one`：mul_one : forall a : M, a * 1 = a
-/
theorem univ_mul_of_one_mem (ht : (1 : α) ∈ t) : univ * t = univ :=
  eq_univ_iff_forall.2 fun _ => mem_mul.2 ⟨_, mem_univ _, _, ht, mul_one _⟩

@[to_additive (attr := simp)]
/-
**Set.univ_mul_univ** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：univ_mul_univ : (univ : Set α) * univ = univ
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mul_univ_of_one_mem`：mul_univ_of_one_mem (hs : (1 : α) in s) : s * u
niv = univ
· 使用定理 `Set.mem_univ`：mem_univ (x : α) : x in @univ α
-/
theorem univ_mul_univ : (univ : Set α) * univ = univ :=
  mul_univ_of_one_mem <| mem_univ _

@[to_additive (attr := simp) nsmul_univ]
/-
**Set.univ_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : Monoid α] {n : ℕ}, n ≠ 0 → Set.univ ^ n = Set.uni
v
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
theorem univ_pow : ∀ {n : ℕ}, n ≠ 0 → (univ : Set α) ^ n = univ
  | 0 => fun h => (h rfl).elim
  | 1 => fun _ => pow_one _
  | n + 2 => fun _ => by rw [pow_succ, univ_pow n.succ_ne_zero, univ_mul_univ]

@[to_additive]
/-
**Set._root_.IsUnit.set** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected theorem _root_.IsUnit.set : IsUnit a → IsUnit ({a} : Set α) :=
  IsUnit.map (singletonMonoidHom : α →* Set α)

@[to_additive nsmul_prod]
/-
**Set.prod_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} {β : Type u_3} [inst : Monoid α] [inst_1 : Monoid β] (s :
 Set α) (t : Set β) (n : ℕ),   s ×ˢ t ^ n = (s ^ n) ×ˢ (t ^ n)
参数：s : Set α；t : Set β；n : ℕ；s ^ n；t ^ n。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma prod_pow [Monoid β] (s : Set α) (t : Set β) : ∀ n, (s ×ˢ t) ^ n = (s ^ n) ×ˢ (t ^ n)
  | 0 => by simp
  | n + 1 => by simp [pow_succ, prod_pow _ _ n]

end Monoid

section IsLeftCancelMul
variable [Mul α] [IsLeftCancelMul α] {s t : Set α}

@[to_additive]
/-
**Set.Nontrivial.mul_left** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nontrivial`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] [IsLeftCancelMul α] {s t : Set α}, t.Nontr
ivial → s.Nonempty → (s * t).Nontrivial
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mul_mem_mul`：mul_mem_mul : a in s -> b in t -> a * b in s * t
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
-/
lemma Nontrivial.mul_left : t.Nontrivial → s.Nonempty → (s * t).Nontrivial := by
  rintro ⟨a, ha, b, hb, hab⟩ ⟨c, hc⟩
  exact ⟨c * a, mul_mem_mul hc ha, c * b, mul_mem_mul hc hb, by simpa⟩

@[to_additive]
/-
**Set.Nontrivial.mul** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nontrivial`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] [IsLeftCancelMul α] {s t : Set α}, s.Nontr
ivial → t.Nontrivial → (s * t).Nontrivial
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nontrivial.mul_left`：∀ {α : Type u_2} [inst : Mul α] [IsLeftCancelMu
l α] {s t : Set α}, t.Nontrivial → s.Nonempty → (s * t).Nontrivial
· 使用定理 `Set.Nontrivial.nonempty`：∀ {α : Type u} {s : Set α}, s.Nontrivial → s.No
nempty
-/
lemma Nontrivial.mul (hs : s.Nontrivial) (ht : t.Nontrivial) : (s * t).Nontrivial :=
  ht.mul_left hs.nonempty

end IsLeftCancelMul

section IsRightCancelMul
variable [Mul α] [IsRightCancelMul α] {s t : Set α}

@[to_additive]
/-
**Set.Nontrivial.mul_right** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nontrivial`。
形式化陈述：∀ {α : Type u_2} [inst : Mul α] [IsRightCancelMul α] {s t : Set α}, s.Nont
rivial → t.Nonempty → (s * t).Nontrivial
参数：s * t。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mul_mem_mul`：mul_mem_mul : a in s -> b in t -> a * b in s * t
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
-/
lemma Nontrivial.mul_right : s.Nontrivial → t.Nonempty → (s * t).Nontrivial := by
  rintro ⟨a, ha, b, hb, hab⟩ ⟨c, hc⟩
  exact ⟨a * c, mul_mem_mul ha hc, b * c, mul_mem_mul hb hc, by simpa⟩

end IsRightCancelMul

section CancelMonoid
variable [CancelMonoid α] {s t : Set α} {a : α} {n : ℕ}

@[to_additive]
/-
**Set.Nontrivial.pow** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nontrivial`。
形式化陈述：∀ {α : Type u_2} [inst : CancelMonoid α] {s : Set α}, s.Nontrivial → ∀ {n 
: ℕ}, n ≠ 0 → (s ^ n).Nontrivial
参数：s ^ n。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma Nontrivial.pow (hs : s.Nontrivial) : ∀ {n}, n ≠ 0 → (s ^ n).Nontrivial
  | 1, _ => by simpa
  | n + 2, _ => by simpa [pow_succ] using (hs.pow n.succ_ne_zero).mul hs

end CancelMonoid

/-- `Set α` is a `CommMonoid` under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
  /-- `Set α` is an `AddCommMonoid` under pointwise operations if `α` is. -/]
/-
**Set.commMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [CommMonoid α] → CommMonoid (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def commMonoid [CommMonoid α] : CommMonoid (Set α) :=
  { Set.monoid, Set.commSemigroup with }

scoped[Pointwise] attribute [instance] Set.commMonoid Set.addCommMonoid

open scoped Pointwise

section DivisionMonoid

variable [DivisionMonoid α] {s t : Set α} {n : ℤ}

@[to_additive]
/-
**Set.mul_eq_one_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Set α}, s * t = 1 ↔ ∃ a 
b, s = {a} ∧ t = {b} ∧ a * b = 1
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.subst`：∀ {α : Sort u} {motive : α → Prop} {a b : α}, a = b → motive a
 → motive b
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.one_nonempty`：one_nonempty : (1 : Set α).Nonempty
· 使用定理 `Set.Nonempty.of_image2_left`：∀ {α : Type u_1} {β : Type u_3} {γ : Type u
_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → s.N
onempty
· 使用定理 `Set.Nonempty.of_image2_right`：∀ {α : Type u_1} {β : Type u_3} {γ : Type 
u_5} {f : α → β → γ} {s : Set α} {t : Set β},   (Set.image2 f s t).Nonempty → t.
Nonempty
· 使用定理 `Eq.subset`：∀ {α : Type u_1} [UsesSetNotationForOrder α] [inst : Preorder
 α] {a b : α}, a = b → a ⊆ b
· 使用定理 `Set.mem_image2_of_mem`：mem_image2_of_mem (ha : a in s) (hb : b in t) : f
 a b in image2 f s t
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.eq_singleton_iff_unique_mem`：eq_singleton_iff_unique_mem : s = {a} ↔
 a in s ∧ forall x in s, x = a
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `eq_inv_of_mul_eq_one_left`：eq_inv_of_mul_eq_one_left (h : a * b = 1) : a
 = b⁻¹
· 使用定理 `inv_eq_of_mul_eq_one_left`：inv_eq_of_mul_eq_one_left (h : a * b = 1) : b
⁻¹ = a
· 使用定理 `eq_inv_of_mul_eq_one_right`：eq_inv_of_mul_eq_one_right (h : a * b = 1) :
 b = a⁻¹
· 使用定理 `inv_eq_of_mul_eq_one_right`：inv_eq_of_mul_eq_one_right : a * b = 1 -> a⁻
¹ = b
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.singleton_mul_singleton`：singleton_mul_singleton : ({a} : Set α) * {
b} = {a * b}
· 使用定理 `Set.singleton_one`：singleton_one : ({1} : Set α) = 1
-/
protected theorem mul_eq_one_iff : s * t = 1 ↔ ∃ a b, s = {a} ∧ t = {b} ∧ a * b = 1 := by
  refine ⟨fun h => ?_, ?_⟩
  · have hst : (s * t).Nonempty := h.symm.subst one_nonempty
    obtain ⟨a, ha⟩ := hst.of_image2_left
    obtain ⟨b, hb⟩ := hst.of_image2_right
    have H : ∀ {a b}, a ∈ s → b ∈ t → a * b = (1 : α) := fun {a b} ha hb =>
      h.subset <| mem_image2_of_mem ha hb
    refine ⟨a, b, ?_, ?_, H ha hb⟩ <;> refine eq_singleton_iff_unique_mem.2 ⟨‹_›, fun x hx => ?_⟩
    · exact (eq_inv_of_mul_eq_one_left <| H hx hb).trans (inv_eq_of_mul_eq_one_left <| H ha hb)
    · exact (eq_inv_of_mul_eq_one_right <| H ha hx).trans (inv_eq_of_mul_eq_one_right <| H ha hb)
  · rintro ⟨b, c, rfl, rfl, h⟩
    rw [singleton_mul_singleton, h, singleton_one]
/-
**Set.nonempty_image_mulLeft_inv_inter_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Set α} {a : α},   ((fun 
x => a⁻¹ * x) '' s ∩ t).Nonempty ↔ ((fun x => x * a) '' s⁻¹ ∩ t⁻¹).Nonempty
参数：(fun x => a⁻¹ * x) '' s ∩ t；(fun x => x * a) '' s⁻¹ ∩ t⁻¹。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.nonempty_inv`：nonempty_inv : s⁻¹.Nonempty ↔ s.Nonempty
· 使用定理 `Set.inter_inv`：inter_inv : (s inter t)⁻¹ = s⁻¹ inter t⁻¹
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `Set.image_image`：image_image (g : β -> γ) (f : α -> β) (s : Set α) : g '
' f '' s = (fun x => g (f x)) '' s
· 使用定理 `Set.image_congr`：image_congr {f g : α -> β} {s : Set α} (h : forall a in
 s, f a = g a) : f '' s = g '' s
· 使用定理 `mul_inv_rev`：mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
@[to_additive] theorem nonempty_image_mulLeft_inv_inter_iff {a : α} :
    ((a⁻¹ * ·) '' s ∩ t).Nonempty ↔ ((· * a) '' s⁻¹ ∩ t⁻¹).Nonempty := by
  rw [← nonempty_inv, inter_inv]; simp_rw [← image_inv_eq_inv, image_image, mul_inv_rev, inv_inv]
/-
**Set.nonempty_image_mulRight_inv_inter_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Set α} {a : α},   ((fun 
x => x * a⁻¹) '' s ∩ t).Nonempty ↔ ((fun x => a * x) '' s⁻¹ ∩ t⁻¹).Nonempty
参数：(fun x => x * a⁻¹) '' s ∩ t；(fun x => a * x) '' s⁻¹ ∩ t⁻¹。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.nonempty_inv`：nonempty_inv : s⁻¹.Nonempty ↔ s.Nonempty
· 使用定理 `Set.inter_inv`：inter_inv : (s inter t)⁻¹ = s⁻¹ inter t⁻¹
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `Set.image_image`：image_image (g : β -> γ) (f : α -> β) (s : Set α) : g '
' f '' s = (fun x => g (f x)) '' s
· 使用定理 `Set.image_congr`：image_congr {f g : α -> β} {s : Set α} (h : forall a in
 s, f a = g a) : f '' s = g '' s
· 使用定理 `mul_inv_rev`：mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
@[to_additive] theorem nonempty_image_mulRight_inv_inter_iff {a : α} :
    ((· * a⁻¹) '' s ∩ t).Nonempty ↔ ((a * ·) '' s⁻¹ ∩ t⁻¹).Nonempty := by
  rw [← nonempty_inv, inter_inv]; simp_rw [← image_inv_eq_inv, image_image, mul_inv_rev, inv_inv]

/-- `Set α` is a division monoid under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible)
    /-- `Set α` is a subtraction monoid under pointwise operations if `α` is. -/]
/-
**Set.divisionMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [DivisionMonoid α] → DivisionMonoid (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def divisionMonoid : DivisionMonoid (Set α) :=
  { Set.monoid, Set.involutiveInv, Set.div, @Set.ZPow α _ _ _ with
    mul_inv_rev := fun s t => by
      simp_rw [← image_inv_eq_inv]
      exact image_image2_antidistrib mul_inv_rev
    inv_eq_of_mul := fun s t h => by
      obtain ⟨a, b, rfl, rfl, hab⟩ := Set.mul_eq_one_iff.1 h
      rw [inv_singleton, inv_eq_of_mul_eq_one_right hab]
    div_eq_mul_inv := fun s t => by
      rw [← image_id (s / t), ← image_inv_eq_inv]
      exact image_image2_distrib_right div_eq_mul_inv }

scoped[Pointwise] attribute [instance] Set.divisionMonoid Set.subtractionMonoid

@[to_additive (attr := simp 500)]
/-
**Set.isUnit_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：isUnit_iff : IsUnit s ↔ exists a, s = {a} ∧ IsUnit a
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用定理 `Set.mul_eq_one_iff`：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Se
t α}, s * t = 1 ↔ ∃ a b, s = {a} ∧ t = {b} ∧ a * b = 1
· 使用定理 `Units.mul_inv`：mul_inv : (a * ↑a⁻¹ : α) = 1
· 使用定理 `Set.singleton_injective`：singleton_injective : Injective (singleton : α 
-> Set α)
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.singleton_mul_singleton`：singleton_mul_singleton : ({a} : Set α) * {
b} = {a * b}
· 使用定理 `Units.inv_mul`：inv_mul : (↑a⁻¹ * a : α) = 1
· 使用定理 `IsUnit.set`：∀ {α : Type u_2} [inst : Monoid α] {a : α}, IsUnit a → IsUni
t {a}
-/
theorem isUnit_iff : IsUnit s ↔ ∃ a, s = {a} ∧ IsUnit a := by
  constructor
  · rintro ⟨u, rfl⟩
    obtain ⟨a, b, ha, hb, h⟩ := Set.mul_eq_one_iff.1 u.mul_inv
    refine ⟨a, ha, ⟨a, b, h, singleton_injective ?_⟩, rfl⟩
    rw [← singleton_mul_singleton, ← ha, ← hb]
    exact u.inv_mul
  · rintro ⟨a, rfl, ha⟩
    exact ha.set

@[to_additive (attr := simp)]
/-
**Set.univ_div_univ** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：univ_div_univ : (univ / univ : Set α) = univ
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `div_eq_mul_inv`：div_eq_mul_inv (a b : G) : a / b = a * b⁻¹
· 使用定理 `Set.univ_mul_univ`：univ_mul_univ : (univ : Set α) * univ = univ
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
lemma univ_div_univ : (univ / univ : Set α) = univ := by simp [div_eq_mul_inv]
/-
**Set.subset_div_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Set α}, 1 ∈ t → s ⊆ s / 
t
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `div_eq_mul_inv`：div_eq_mul_inv (a b : G) : a / b = a * b⁻¹
· 使用定理 `Set.subset_mul_left`：subset_mul_left (s : Set α) {t : Set α} (ht : (1 : 
α) in t) : s subseteq s * t
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `inv_one`：inv_one : (1 : G)⁻¹ = 1
-/
@[to_additive] lemma subset_div_left (ht : 1 ∈ t) : s ⊆ s / t := by
  rw [div_eq_mul_inv]; exact subset_mul_left _ <| by simpa
/-
**Set.inv_subset_div_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s t : Set α}, 1 ∈ s → t⁻¹ ⊆ s 
/ t
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `div_eq_mul_inv`：div_eq_mul_inv (a b : G) : a / b = a * b⁻¹
· 使用定理 `Set.subset_mul_right`：subset_mul_right {s : Set α} (t : Set α) (hs : (1 
: α) in s) : t subseteq s * t
-/
@[to_additive] lemma inv_subset_div_right (hs : 1 ∈ s) : t⁻¹ ⊆ s / t := by
  rw [div_eq_mul_inv]; exact subset_mul_right _ hs

@[to_additive (attr := simp) zsmul_empty]
/-
**Set.empty_zpow** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：empty_zpow (hn : n != 0) : (∅ : Set α) ^ n = ∅
参数：hn : n != 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `zpow_natCast`：zpow_natCast (a : G) : forall n : Nat, a ^ (n : Int) = a ^
 n | 0 => (zpow_zero _).trans (pow_zero _).symm | n + 1 => calc a ^ (↑(n + 1) : 
In…
· 使用引理 `Set.empty_pow`：empty_pow (hn : n != 0) : (∅ : Set α) ^ n = ∅
· 使用定理 `eq_false`：∀ {p : Prop}, ¬p → p = False
· 使用定理 `not_false_eq_true`：(¬False) = True
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `zpow_negSucc`：zpow_negSucc (a : G) (n : Nat) : a ^ (Int.negSucc n) = (a 
^ (n + 1))⁻¹
· 使用定理 `and_false`：∀ (p : Prop), (p ∧ False) = False
-/
lemma empty_zpow (hn : n ≠ 0) : (∅ : Set α) ^ n = ∅ := by cases n <;> aesop

@[to_additive]
/-
**Set.Nonempty.zpow** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s : Set α}, s.Nonempty → ∀ {n 
: ℤ}, (s ^ n).Nonempty
参数：s ^ n。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.Nonempty.pow`：∀ {α : Type u_2} [inst : Monoid α] {s : Set α}, s.None
mpty → ∀ {n : ℕ}, (s ^ n).Nonempty
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `zpow_negSucc`：zpow_negSucc (a : G) (n : Nat) : a ^ (Int.negSucc n) = (a 
^ (n + 1))⁻¹
-/
lemma Nonempty.zpow (hs : s.Nonempty) : ∀ {n : ℤ}, (s ^ n).Nonempty
  | (n : ℕ) => hs.pow
  | .negSucc n => by simpa using hs.pow
/-
**Set.zpow_eq_empty** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {α : Type u_2} [inst : DivisionMonoid α] {s : Set α} {n : ℤ}, s ^ n = ∅ 
↔ s = ∅ ∧ n ≠ 0
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `Mathlib.Tactic.Contrapose.contrapose₁`：contrapose₁ {p q : Prop} : (¬ q -
> ¬ p) -> (p -> q)
· 使用定理 `implies_congr`：∀ {p₁ p₂ : Sort u} {q₁ q₂ : Sort v}, p₁ = p₂ → q₁ = q₂ → 
(p₁ → q₁) = (p₂ → q₂)
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `Mathlib.Tactic.Push.not_and_or_eq`：not_and_or_eq : (¬ (p ∧ q)) = (¬ p ∨ 
¬ q)
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.Nonempty.zpow`：∀ {α : Type u_2} [inst : DivisionMonoid α] {s : Set α
}, s.Nonempty → ∀ {n : ℤ}, (s ^ n).Nonempty
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用引理 `zpow_ofNat`：zpow_ofNat (a : G) (n : Nat) : a ^ (ofNat(n) : Int) = a ^ Of
Nat.ofNat n
· 使用定理 `pow_zero`：pow_zero (a : M) : a ^ 0 = 1
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用引理 `Set.empty_zpow`：empty_zpow (hn : n != 0) : (∅ : Set α) ^ n = ∅
-/
@[to_additive (attr := simp)] lemma zpow_eq_empty : s ^ n = ∅ ↔ s = ∅ ∧ n ≠ 0 := by
  constructor
  · contrapose! +distrib
    rintro (hs | rfl)
    · exact hs.zpow
    · simp
  · rintro ⟨rfl, hn⟩
    exact empty_zpow hn

@[to_additive (attr := simp) zsmul_singleton]
/-
**Set.singleton_zpow** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：singleton_zpow (a : α) (n : Int) : ({a} : Set α) ^ n = {a ^ n}
参数：a : α；n : Int。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `zpow_natCast`：zpow_natCast (a : G) : forall n : Nat, a ^ (n : Int) = a ^
 n | 0 => (zpow_zero _).trans (pow_zero _).symm | n + 1 => calc a ^ (↑(n + 1) : 
In…
· 使用定理 `Set.singleton_pow`：∀ {α : Type u_2} [inst : Monoid α] (a : α) (n : ℕ), {
a} ^ n = {a ^ n}
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `zpow_negSucc`：zpow_negSucc (a : G) (n : Nat) : a ^ (Int.negSucc n) = (a 
^ (n + 1))⁻¹
· 使用定理 `Set.inv_singleton`：inv_singleton (a : α) : ({a} : Set α)⁻¹ = {a⁻¹}
-/
lemma singleton_zpow (a : α) (n : ℤ) : ({a} : Set α) ^ n = {a ^ n} := by cases n <;> simp

end DivisionMonoid

/-- `Set α` is a commutative division monoid under pointwise operations if `α` is. -/
@[to_additive (attr := instance_reducible) subtractionCommMonoid
      /-- `Set α` is a commutative subtraction monoid under pointwise operations if `α` is. -/]
/-
**Set.divisionCommMonoid** 是 Mathlib 中的一个定义，位于命名空间 `Set`。
形式化陈述：{α : Type u_2} → [DivisionCommMonoid α] → DivisionCommMonoid (Set α)
参数：Set α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected def divisionCommMonoid [DivisionCommMonoid α] :
    DivisionCommMonoid (Set α) :=
  { Set.divisionMonoid, Set.commSemigroup with }

scoped[Pointwise] attribute [instance] Set.divisionCommMonoid Set.subtractionCommMonoid

section Group

variable [Group α] {s t : Set α} {a b : α}

/-! Note that `Set` is not a `Group` because `s / s ≠ 1` in general. -/


@[to_additive (attr := simp)]
/-
**Set.one_mem_div_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：one_mem_div_iff : (1 : α) in s / t ↔ ¬Disjoint s t
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True

--- 原说明 ---
Note that `Set` is not a `Group` because `s / s ≠ 1` in general.
-/
theorem one_mem_div_iff : (1 : α) ∈ s / t ↔ ¬Disjoint s t := by
  simp [not_disjoint_iff_nonempty_inter, mem_div, div_eq_one, Set.Nonempty]

@[to_additive (attr := simp)]
/-
**Set.one_mem_inv_mul_iff** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：one_mem_inv_mul_iff : (1 : α) in t⁻¹ * s ↔ ¬Disjoint s t
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
-/
lemma one_mem_inv_mul_iff : (1 : α) ∈ t⁻¹ * s ↔ ¬Disjoint s t := by
  aesop (add simp [not_disjoint_iff_nonempty_inter, mem_mul, mul_eq_one_iff_eq_inv, Set.Nonempty])

@[to_additive]
/-
**Set.one_notMem_div_iff** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：one_notMem_div_iff : (1 : α) ∉ s / t ↔ Disjoint s t
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.not_left`：Iff.not_left (h : a ↔ ¬b) : ¬a ↔ b
· 使用定理 `Set.one_mem_div_iff`：one_mem_div_iff : (1 : α) in s / t ↔ ¬Disjoint s t
-/
theorem one_notMem_div_iff : (1 : α) ∉ s / t ↔ Disjoint s t :=
  one_mem_div_iff.not_left

@[to_additive]
/-
**Set.one_notMem_inv_mul_iff** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：one_notMem_inv_mul_iff : (1 : α) ∉ t⁻¹ * s ↔ Disjoint s t
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.not_left`：Iff.not_left (h : a ↔ ¬b) : ¬a ↔ b
· 使用引理 `Set.one_mem_inv_mul_iff`：one_mem_inv_mul_iff : (1 : α) in t⁻¹ * s ↔ ¬Dis
joint s t
-/
lemma one_notMem_inv_mul_iff : (1 : α) ∉ t⁻¹ * s ↔ Disjoint s t := one_mem_inv_mul_iff.not_left

alias ⟨_, _root_.Disjoint.one_notMem_div_set⟩ := one_notMem_div_iff

attribute [to_additive] Disjoint.one_notMem_div_set

@[to_additive]
/-
**Set.Nonempty.one_mem_div** 是 Mathlib 中的一个定理，位于命名空间 `Set.Nonempty`。
形式化陈述：∀ {α : Type u_2} [inst : Group α] {s : Set α}, s.Nonempty → 1 ∈ s / s
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.mem_div`：mem_div : a in s / t ↔ exists x in s, exists y in t, x / y 
= a
· 使用定理 `div_self'`：div_self' (a : G) : a / a = 1
-/
theorem Nonempty.one_mem_div (h : s.Nonempty) : (1 : α) ∈ s / s :=
  let ⟨a, ha⟩ := h
  mem_div.2 ⟨a, ha, a, ha, div_self' _⟩

@[to_additive]
/-
**Set.isUnit_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：isUnit_singleton (a : α) : IsUnit ({a} : Set α)
参数：a : α。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `IsUnit.set`：∀ {α : Type u_2} [inst : Monoid α] {a : α}, IsUnit a → IsUni
t {a}
· 使用引理 `Group.isUnit`：Group.isUnit [Group α] (a : α) : IsUnit a
-/
theorem isUnit_singleton (a : α) : IsUnit ({a} : Set α) :=
  (Group.isUnit a).set

@[to_additive (attr := simp)]
/-
**Set.isUnit_iff_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：isUnit_iff_singleton : IsUnit s ↔ exists a, s = {a}
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `and_true`：∀ (p : Prop), (p ∧ True) = p
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
theorem isUnit_iff_singleton : IsUnit s ↔ ∃ a, s = {a} := by
  simp only [isUnit_iff, Group.isUnit, and_true]

@[to_additive (attr := simp)]
/-
**Set.image_mul_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul_left : (a * ·) '' t = (a⁻¹ * ·) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_eq_preimage_of_inverse`：image_eq_preimage_of_inverse {f : α ->
 β} {g : β -> α} (h₁ : LeftInverse g f) (h₂ : RightInverse g f) : image f = prei
mage g
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `inv_mul_cancel_left`：inv_mul_cancel_left (a b : G) : a⁻¹ * (a * b) = b
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `mul_inv_cancel_left`：mul_inv_cancel_left (a b : G) : a * (a⁻¹ * b) = b
-/
theorem image_mul_left : (a * ·) '' t = (a⁻¹ * ·) ⁻¹' t := by
  rw [image_eq_preimage_of_inverse] <;> intro c <;> simp

@[to_additive (attr := simp)]
/-
**Set.image_mul_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul_right : (· * b) '' t = (· * b⁻¹) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_eq_preimage_of_inverse`：image_eq_preimage_of_inverse {f : α ->
 β} {g : β -> α} (h₁ : LeftInverse g f) (h₂ : RightInverse g f) : image f = prei
mage g
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `mul_inv_cancel_right`：mul_inv_cancel_right (a b : G) : a * b * b⁻¹ = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `inv_mul_cancel_right`：inv_mul_cancel_right (a b : G) : a * b⁻¹ * b = a
-/
theorem image_mul_right : (· * b) '' t = (· * b⁻¹) ⁻¹' t := by
  rw [image_eq_preimage_of_inverse] <;> intro c <;> simp

@[to_additive]
/-
**Set.image_mul_left'** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul_left' : (a⁻¹ * ·) '' t = (a * ·) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_mul_left`：image_mul_left : (a * ·) '' t = (a⁻¹ * ·) ⁻¹' t
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
theorem image_mul_left' : (a⁻¹ * ·) '' t = (a * ·) ⁻¹' t := by simp

@[to_additive]
/-
**Set.image_mul_right'** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul_right' : (· * b⁻¹) '' t = (· * b) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_mul_right`：image_mul_right : (· * b) '' t = (· * b⁻¹) ⁻¹' t
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
theorem image_mul_right' : (· * b⁻¹) '' t = (· * b) ⁻¹' t := by simp

@[to_additive]
/-
**Set.image_div_left** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_div_left : (a / ·) '' t = (·⁻¹ * a) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_eq_preimage_of_inverse`：image_eq_preimage_of_inverse {f : α ->
 β} {g : β -> α} (h₁ : LeftInverse g f) (h₂ : RightInverse g f) : image f = prei
mage g
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `inv_div`：inv_div : (a / b)⁻¹ = b / a
· 使用定理 `div_mul_cancel`：div_mul_cancel (a b : G) : a / b * b = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用引理 `div_mul_cancel_right`：div_mul_cancel_right (a b : G) : a / (b * a) = b⁻¹
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
-/
theorem image_div_left : (a / ·) '' t = (·⁻¹ * a) ⁻¹' t := by
  rw [image_eq_preimage_of_inverse] <;> intro c <;> simp

@[to_additive]
/-
**Set.image_div_right** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_div_right : (· / b) '' t = (· * b) ⁻¹' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_eq_preimage_of_inverse`：image_eq_preimage_of_inverse {f : α ->
 β} {g : β -> α} (h₁ : LeftInverse g f) (h₂ : RightInverse g f) : image f = prei
mage g
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `div_mul_cancel`：div_mul_cancel (a b : G) : a / b * b = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `mul_div_cancel_right`：mul_div_cancel_right (a b : G) : a * b / b = a
-/
theorem image_div_right : (· / b) '' t = (· * b) ⁻¹' t := by
  rw [image_eq_preimage_of_inverse] <;> intro c <;> simp

@[to_additive (attr := simp)]
/-
**Set.preimage_mul_left_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_left_singleton : (a * ·) ⁻¹' {b} = {a⁻¹ * b}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_mul_left'`：image_mul_left' : (a⁻¹ * ·) '' t = (a * ·) ⁻¹' t
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
theorem preimage_mul_left_singleton : (a * ·) ⁻¹' {b} = {a⁻¹ * b} := by
  rw [← image_mul_left', image_singleton]

@[to_additive (attr := simp)]
/-
**Set.preimage_mul_right_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_right_singleton : (· * a) ⁻¹' {b} = {b * a⁻¹}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_mul_right'`：image_mul_right' : (· * b⁻¹) '' t = (· * b) ⁻¹' t
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
theorem preimage_mul_right_singleton : (· * a) ⁻¹' {b} = {b * a⁻¹} := by
  rw [← image_mul_right', image_singleton]

@[to_additive (attr := simp)]
/-
**Set.preimage_inv_mul_right_singleton** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_inv_mul_right_singleton : (·⁻¹ * a) ⁻¹' {b} = {a / b}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_div_left`：image_div_left : (a / ·) '' t = (·⁻¹ * a) ⁻¹' t
· 使用定理 `Set.image_singleton`：image_singleton {f : α -> β} {a : α} : f '' {a} = {
f a}
-/
theorem preimage_inv_mul_right_singleton : (·⁻¹ * a) ⁻¹' {b} = {a / b} := by
  rw [← image_div_left, image_singleton]

@[to_additive (attr := simp)]
/-
**Set.preimage_mul_left_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_left_one : (a * ·) ⁻¹' 1 = {a⁻¹}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_mul_left'`：image_mul_left' : (a⁻¹ * ·) '' t = (a * ·) ⁻¹' t
· 使用定理 `Set.image_one`：image_one {f : α -> β} : f '' 1 = {f 1}
· 使用定理 `mul_one`：mul_one : forall a : M, a * 1 = a
-/
theorem preimage_mul_left_one : (a * ·) ⁻¹' 1 = {a⁻¹} := by
  rw [← image_mul_left', image_one, mul_one]

@[to_additive (attr := simp)]
/-
**Set.preimage_mul_right_one** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_right_one : (· * b) ⁻¹' 1 = {b⁻¹}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_mul_right'`：image_mul_right' : (· * b⁻¹) '' t = (· * b) ⁻¹' t
· 使用定理 `Set.image_one`：image_one {f : α -> β} : f '' 1 = {f 1}
· 使用定理 `one_mul`：one_mul : forall a : M, 1 * a = a
-/
theorem preimage_mul_right_one : (· * b) ⁻¹' 1 = {b⁻¹} := by
  rw [← image_mul_right', image_one, one_mul]

@[to_additive]
/-
**Set.preimage_mul_left_one'** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_left_one' : (a⁻¹ * ·) ⁻¹' 1 = {a}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.preimage_mul_left_one`：preimage_mul_left_one : (a * ·) ⁻¹' 1 = {a⁻¹}
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
theorem preimage_mul_left_one' : (a⁻¹ * ·) ⁻¹' 1 = {a} := by simp

@[to_additive]
/-
**Set.preimage_mul_right_one'** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_right_one' : (· * b⁻¹) ⁻¹' 1 = {b}
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.preimage_mul_right_one`：preimage_mul_right_one : (· * b) ⁻¹' 1 = {b⁻
¹}
· 使用定理 `inv_inv`：inv_inv (a : G) : a⁻¹⁻¹ = a
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
theorem preimage_mul_right_one' : (· * b⁻¹) ⁻¹' 1 = {b} := by simp

@[to_additive (attr := simp)]
/-
**Set.mul_univ** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：mul_univ (hs : s.Nonempty) : s * (univ : Set α) = univ
参数：hs : s.Nonempty。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.eq_univ_of_forall`：eq_univ_of_forall {s : Set α} : (forall x, x in s
) -> s = univ
· 使用定理 `trivial`：True
· 使用定理 `mul_inv_cancel_left`：mul_inv_cancel_left (a b : G) : a * (a⁻¹ * b) = b
-/
theorem mul_univ (hs : s.Nonempty) : s * (univ : Set α) = univ :=
  let ⟨a, ha⟩ := hs
  eq_univ_of_forall fun b => ⟨a, ha, a⁻¹ * b, trivial, mul_inv_cancel_left ..⟩

@[to_additive (attr := simp)]
/-
**Set.univ_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：univ_mul (ht : t.Nonempty) : (univ : Set α) * t = univ
参数：ht : t.Nonempty。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.eq_univ_of_forall`：eq_univ_of_forall {s : Set α} : (forall x, x in s
) -> s = univ
· 使用定理 `trivial`：True
· 使用定理 `inv_mul_cancel_right`：inv_mul_cancel_right (a b : G) : a * b⁻¹ * b = a
-/
theorem univ_mul (ht : t.Nonempty) : (univ : Set α) * t = univ :=
  let ⟨a, ha⟩ := ht
  eq_univ_of_forall fun b => ⟨b * a⁻¹, trivial, a, ha, inv_mul_cancel_right ..⟩

@[to_additive]
/-
**Set.image_inv** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：image_inv [DivisionMonoid β] [FunLike F α β] [MonoidHomClass F α β] (f : F
) (s : Set α) : f '' s⁻¹ = (f '' s)⁻¹
参数：f : F；s : Set α。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `Set.image_inv_eq_inv`：image_inv_eq_inv : (·⁻¹) '' s = s⁻¹
· 使用定理 `Set.image_comm`：image_comm {β'} {f : β -> γ} {g : α -> β} {f' : α -> β'}
 {g' : β' -> γ} (h_comm : forall a, f (g a) = g' (f' a)) : (s.image g).image f =
 (s.…
· 使用定理 `map_inv`：map_inv [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f 
: F) (a : G) : f a⁻¹ = (f a)⁻¹
-/
lemma image_inv [DivisionMonoid β] [FunLike F α β] [MonoidHomClass F α β] (f : F) (s : Set α) :
    f '' s⁻¹ = (f '' s)⁻¹ := by
  rw [← image_inv_eq_inv, ← image_inv_eq_inv]; exact image_comm (map_inv _)

end Group

section Mul

variable [Mul α] [Mul β] [FunLike F α β] [MulHomClass F α β] (m : F) {s t : Set α}

@[to_additive]
/-
**Set.image_mul** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_mul : m '' (s * t) = m '' s * m '' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image_image2_distrib`：image_image2_distrib {g : γ -> δ} {f' : α' -> 
β' -> δ} {g₁ : α -> α'} {g₂ : β -> β'} (h_distrib : forall a b, g (f a b) = f' (
g₁ a) (g₂ b)) …
· 使用定理 `map_mul`：map_mul [MulHomClass F M N] (f : F) (x y : M) : f (x * y) = f x
 * f y
-/
theorem image_mul : m '' (s * t) = m '' s * m '' t :=
  image_image2_distrib <| map_mul m

@[to_additive]
/-
**Set.mul_subset_range** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：mul_subset_range {s t : Set β} (hs : s subseteq range m) (ht : t subseteq 
range m) : s * t subseteq range m
参数：hs : s subseteq range m；ht : t subseteq range m。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `map_mul`：map_mul [MulHomClass F M N] (f : F) (x y : M) : f (x * y) = f x
 * f y
-/
lemma mul_subset_range {s t : Set β} (hs : s ⊆ range m) (ht : t ⊆ range m) : s * t ⊆ range m := by
  rintro _ ⟨a, ha, b, hb, rfl⟩
  obtain ⟨a, rfl⟩ := hs ha
  obtain ⟨b, rfl⟩ := ht hb
  exact ⟨a * b, map_mul ..⟩

@[to_additive]
/-
**Set.preimage_mul_preimage_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_mul_preimage_subset {s t : Set β} : m ⁻¹' s * m ⁻¹' t subseteq m 
⁻¹' (s * t)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `map_mul`：map_mul [MulHomClass F M N] (f : F) (x y : M) : f (x * y) = f x
 * f y
-/
theorem preimage_mul_preimage_subset {s t : Set β} : m ⁻¹' s * m ⁻¹' t ⊆ m ⁻¹' (s * t) := by
  rintro _ ⟨_, _, _, _, rfl⟩
  exact ⟨_, ‹_›, _, ‹_›, (map_mul m ..).symm⟩

@[to_additive]
/-
**Set.preimage_mul** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：preimage_mul (hm : Injective m) {s t : Set β} (hs : s subseteq range m) (h
t : t subseteq range m) : m ⁻¹' (s * t) = m ⁻¹' s * m ⁻¹' t
参数：hm : Injective m；hs : s subseteq range m；ht : t subseteq range m。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Injective.image_injective`：∀ {α : Type u_1} {β : Type u_2} {f :
 α → β}, Function.Injective f → Function.Injective (Set.image f)
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_mul`：image_mul : m '' (s * t) = m '' s * m '' t
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.image_preimage_eq_iff`：image_preimage_eq_iff {f : α -> β} {s : Set β
} : f '' f ⁻¹' s = s ↔ s subseteq range f
· 使用引理 `Set.mul_subset_range`：mul_subset_range {s t : Set β} (hs : s subseteq ra
nge m) (ht : t subseteq range m) : s * t subseteq range m
-/
lemma preimage_mul (hm : Injective m) {s t : Set β} (hs : s ⊆ range m) (ht : t ⊆ range m) :
    m ⁻¹' (s * t) = m ⁻¹' s * m ⁻¹' t :=
  hm.image_injective <| by
    rw [image_mul, image_preimage_eq_iff.2 hs, image_preimage_eq_iff.2 ht,
      image_preimage_eq_iff.2 (mul_subset_range m hs ht)]

end Mul

section Monoid
variable [Monoid α] [Monoid β] [FunLike F α β]

@[to_additive]
/-
**Set.image_pow_of_ne_zero** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {F : Type u_1} {α : Type u_2} {β : Type u_3} [inst : Monoid α] [inst_1 :
 Monoid β] [inst_2 : FunLike F α β]   [MulHomClass F α β] {n : ℕ}, n ≠ 0 → ∀ (f 
: F) (s : Set α), ⇑f '' s ^ n = (⇑f '' s) ^ n
参数：f : F；s : Set α；⇑f '' s。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma image_pow_of_ne_zero [MulHomClass F α β] :
    ∀ {n}, n ≠ 0 → ∀ (f : F) (s : Set α), f '' (s ^ n) = (f '' s) ^ n
  | 1, _ => by simp
  | n + 2, _ => by simp [image_mul, pow_succ _ n.succ, image_pow_of_ne_zero]

@[to_additive]
/-
**Set.image_pow** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {F : Type u_1} {α : Type u_2} {β : Type u_3} [inst : Monoid α] [inst_1 :
 Monoid β] [inst_2 : FunLike F α β]   [MonoidHomClass F α β] (f : F) (s : Set α)
 (n : ℕ), ⇑f '' s ^ n = (⇑f '' s) ^ n
参数：f : F；s : Set α；n : ℕ；⇑f '' s。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `pow_zero`：pow_zero (a : M) : a ^ 0 = 1
· 使用定理 `Set.image_one`：image_one {f : α -> β} : f '' 1 = {f 1}
· 使用定理 `map_one`：map_one [OneHomClass F M N] (f : F) : f 1 = 1
· 使用定理 `MonoidHomClass.toOneHomClass`：∀ {F : Type u_10} {M : outParam (Type u_11
)} {N : outParam (Type u_12)} {inst : MulOne M} {inst_1 : MulOne N}   {inst_2 : 
FunLike F M N} [se…
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `Set.image_pow_of_ne_zero`：∀ {F : Type u_1} {α : Type u_2} {β : Type u_3}
 [inst : Monoid α] [inst_1 : Monoid β] [inst_2 : FunLike F α β]   [MulHomClass F
 α β] {n : ℕ},…
· 使用定理 `MonoidHomClass.toMulHomClass`：∀ {F : Type u_10} {M : outParam (Type u_11
)} {N : outParam (Type u_12)} {inst : MulOne M} {inst_1 : MulOne N}   {inst_2 : 
FunLike F M N} [se…
· 使用定理 `Nat.succ_ne_zero`：∀ (n : ℕ), n.succ ≠ 0
-/
lemma image_pow [MonoidHomClass F α β] (f : F) (s : Set α) : ∀ n, f '' (s ^ n) = (f '' s) ^ n
  | 0 => by simp [singleton_one]
  | n + 1 => image_pow_of_ne_zero n.succ_ne_zero ..

@[to_additive]
/-
**Set.preimage_pow_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：∀ {F : Type u_1} {α : Type u_2} {β : Type u_3} [inst : Monoid α] [inst_1 :
 Monoid β] [inst_2 : FunLike F α β]   [MonoidHomClass F α β] (f : F) (s : Set β)
 (n : ℕ), (⇑f ⁻¹' s) ^ n ⊆ ⇑f ⁻¹' s ^ n
参数：f : F；s : Set β；n : ℕ；⇑f ⁻¹' s。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma preimage_pow_subset [MonoidHomClass F α β] (f : F) (s : Set β) :
    ∀ n, (f ⁻¹' s) ^ n ⊆ f ⁻¹' (s ^ n)
  | 0 => by simp [Set.subset_def]
  | n + 1 => by simpa [pow_succ] using Subset.trans (mul_subset_mul_right
    (preimage_pow_subset f s n)) (preimage_mul_preimage_subset f)

end Monoid

section Group

variable [Group α] [DivisionMonoid β] [FunLike F α β] [MonoidHomClass F α β] (m : F) {s t : Set α}

@[to_additive]
/-
**Set.image_div** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：image_div : m '' (s / t) = m '' s / m '' t
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.image_image2_distrib`：image_image2_distrib {g : γ -> δ} {f' : α' -> 
β' -> δ} {g₁ : α -> α'} {g₂ : β -> β'} (h_distrib : forall a b, g (f a b) = f' (
g₁ a) (g₂ b)) …
· 使用定理 `map_div`：map_div [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f 
: F) : forall a b, f (a / b) = f a / f b
-/
theorem image_div : m '' (s / t) = m '' s / m '' t :=
  image_image2_distrib <| map_div m

@[to_additive]
/-
**Set.div_subset_range** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：div_subset_range {s t : Set β} (hs : s subseteq range m) (ht : t subseteq 
range m) : s / t subseteq range m
参数：hs : s subseteq range m；ht : t subseteq range m。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `map_div`：map_div [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f 
: F) : forall a b, f (a / b) = f a / f b
-/
lemma div_subset_range {s t : Set β} (hs : s ⊆ range m) (ht : t ⊆ range m) : s / t ⊆ range m := by
  rintro _ ⟨a, ha, b, hb, rfl⟩
  obtain ⟨a, rfl⟩ := hs ha
  obtain ⟨b, rfl⟩ := ht hb
  exact ⟨a / b, map_div ..⟩

@[to_additive]
/-
**Set.preimage_div_preimage_subset** 是 Mathlib 中的一个定理，位于命名空间 `Set`。
形式化陈述：preimage_div_preimage_subset {s t : Set β} : m ⁻¹' s / m ⁻¹' t subseteq m 
⁻¹' (s / t)
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `map_div`：map_div [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f 
: F) : forall a b, f (a / b) = f a / f b
-/
theorem preimage_div_preimage_subset {s t : Set β} : m ⁻¹' s / m ⁻¹' t ⊆ m ⁻¹' (s / t) := by
  rintro _ ⟨_, _, _, _, rfl⟩
  exact ⟨_, ‹_›, _, ‹_›, (map_div m ..).symm⟩

@[to_additive]
/-
**Set.preimage_div** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：preimage_div (hm : Injective m) {s t : Set β} (hs : s subseteq range m) (h
t : t subseteq range m) : m ⁻¹' (s / t) = m ⁻¹' s / m ⁻¹' t
参数：hm : Injective m；hs : s subseteq range m；ht : t subseteq range m。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Injective.image_injective`：∀ {α : Type u_1} {β : Type u_2} {f :
 α → β}, Function.Injective f → Function.Injective (Set.image f)
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Set.image_div`：image_div : m '' (s / t) = m '' s / m '' t
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.image_preimage_eq_iff`：image_preimage_eq_iff {f : α -> β} {s : Set β
} : f '' f ⁻¹' s = s ↔ s subseteq range f
· 使用引理 `Set.div_subset_range`：div_subset_range {s t : Set β} (hs : s subseteq ra
nge m) (ht : t subseteq range m) : s / t subseteq range m
-/
lemma preimage_div (hm : Injective m) {s t : Set β} (hs : s ⊆ range m) (ht : t ⊆ range m) :
    m ⁻¹' (s / t) = m ⁻¹' s / m ⁻¹' t :=
  hm.image_injective <| by
    rw [image_div, image_preimage_eq_iff.2 hs, image_preimage_eq_iff.2 ht,
      image_preimage_eq_iff.2 (div_subset_range m hs ht)]

end Group

section Pi

variable {ι : Type*} {α : ι → Type*} [∀ i, Inv (α i)]

@[to_additive (attr := simp)]
/-
**Set.inv_pi** 是 Mathlib 中的一个引理，位于命名空间 `Set`。
形式化陈述：inv_pi (s : Set ι) (t : forall i, Set (α i)) : (s.pi t)⁻¹ = s.pi fun i => 
(t i)⁻¹
参数：s : Set ι；t : forall i, Set (α i)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.ext`：ext {a b : Set α} (h : forall (x : α), x in a ↔ x in b) : a = b
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
→ a₁ = a₂ → f₁ a₁ = f₂ a₂
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `forall_congr`：∀ {α : Sort u} {p q : α → Prop}, (∀ (a : α), p a = q a) → 
(∀ (a : α), p a) = ∀ (a : α), q a
· 使用定理 `implies_congr`：∀ {p₁ p₂ : Sort u} {q₁ q₂ : Sort v}, p₁ = p₂ → q₁ = q₂ → 
(p₁ → q₁) = (p₂ → q₂)
· 使用定理 `iff_self`：∀ (p : Prop), (p ↔ p) = True
-/
lemma inv_pi (s : Set ι) (t : ∀ i, Set (α i)) : (s.pi t)⁻¹ = s.pi fun i ↦ (t i)⁻¹ := by ext x; simp

end Pi

section Pointwise

open scoped Pointwise

@[to_additive]
/-
**Set.MapsTo.mul** 是 Mathlib 中的一个定理，位于命名空间 `Set.MapsTo`。
形式化陈述：∀ {α : Type u_2} {β : Type u_3} [inst : Mul β] {A : Set α} {B₁ B₂ : Set β}
 {f₁ f₂ : α → β},   Set.MapsTo f₁ A B₁ → Set.MapsTo f₂ A B₂ → Set.MapsTo (f₁ * f
₂) A (B₁ * B₂)
参数：f₁ * f₂；B₁ * B₂。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.mul_mem_mul`：mul_mem_mul : a in s -> b in t -> a * b in s * t
-/
lemma MapsTo.mul [Mul β] {A : Set α} {B₁ B₂ : Set β} {f₁ f₂ : α → β}
    (h₁ : MapsTo f₁ A B₁) (h₂ : MapsTo f₂ A B₂) : MapsTo (f₁ * f₂) A (B₁ * B₂) :=
  fun _ h => mul_mem_mul (h₁ h) (h₂ h)

@[to_additive]
/-
**Set.MapsTo.inv** 是 Mathlib 中的一个定理，位于命名空间 `Set.MapsTo`。
形式化陈述：∀ {α : Type u_2} {β : Type u_3} [inst : InvolutiveInv β] {A : Set α} {B : 
Set β} {f : α → β},   Set.MapsTo f A B → Set.MapsTo f⁻¹ A B⁻¹
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `Set.inv_mem_inv`：inv_mem_inv : a⁻¹ in s⁻¹ ↔ a in s
-/
lemma MapsTo.inv [InvolutiveInv β] {A : Set α} {B : Set β} {f : α → β} (h : MapsTo f A B) :
    MapsTo (f⁻¹) A (B⁻¹) :=
  fun _ ha => inv_mem_inv.2 (h ha)


@[to_additive]
/-
**Set.MapsTo.div** 是 Mathlib 中的一个定理，位于命名空间 `Set.MapsTo`。
形式化陈述：∀ {α : Type u_2} {β : Type u_3} [inst : Div β] {A : Set α} {B₁ B₂ : Set β}
 {f₁ f₂ : α → β},   Set.MapsTo f₁ A B₁ → Set.MapsTo f₂ A B₂ → Set.MapsTo (f₁ / f
₂) A (B₁ / B₂)
参数：f₁ / f₂；B₁ / B₂。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Set.div_mem_div`：div_mem_div : a in s -> b in t -> a / b in s / t
-/
lemma MapsTo.div [Div β] {A : Set α} {B₁ B₂ : Set β} {f₁ f₂ : α → β}
    (h₁ : MapsTo f₁ A B₁) (h₂ : MapsTo f₂ A B₂) : MapsTo (f₁ / f₂) A (B₁ / B₂) :=
  fun _ ha => div_mem_div (h₁ ha) (h₂ ha)

end Pointwise

end Set

