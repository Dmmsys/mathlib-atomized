...
/-
**CategoryTheory.CommShift** 是 Mathlib 中的一个结构，位于命名空间 `CategoryTheory`。
...
`
and `z_zero₂`. We also require an additional data of `ε m n : (CatCenter D)ˣ`
for `m` and `n`: even though this is determined by the `z` field of `TwistShiftD
ata`,
we make it a separate field so as to have control on its definitional properties
.
-/
...
@[simps]
/-
**CategoryTheory.CommShift** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
A bifunctor `G : C₁ ⥤ C₂ ⥤ D` commutes with the shifts by `M` if all functors
`G.obj X₁` and `G.flip X₂` are equipped with `Functor.CommShift` structures, in 
a way
that is natural in `X₁` and `X₂`, and that these isomorphisms commute up to
the multiplication with an element in `(CatCenter D)ˣ` which is determined by
a `CommShift₂Setup D M` structure. (In most cases, one should use the
...
`G.obj X₁` and `G.flip X₂` are equipped with `Functor.CommShift` structures, in 
a way
that is natural in `X₁` and `X₂`, and that these isomorphisms for the shift by `
p`
on the first variable and the shift by `q` on the second variable commute up
to the sign `(-1) ^ (p * q)`.
-/
...
/-
**CategoryTheory.Functor.CommShift₂.precomp** 是 Mathlib 中的一个实例，位于命名空间 `CategoryT
...
--- 原说明 ---
If `τ : G₁ ⟶ G₂` is a natural transformation between two bifunctors
which commute shifts on both variables, this typeclass asserts a compatibility o
f `τ`
with these shifts.
-/
...
set_option backward.defeqAttrib.useBackward true in
/-
**CategoryTheory.NatTrans.CommShift₂.** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheory.
NatTrans.CommShift₂`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
