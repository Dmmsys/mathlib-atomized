/-
Copyright (c) 2022 Andrew Yang. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Andrew Yang
-/
module

public import Mathlib.AlgebraicGeometry.AffineScheme
public import Mathlib.AlgebraicGeometry.Gluing
public import Mathlib.CategoryTheory.Limits.Opposites
public import Mathlib.CategoryTheory.Limits.Shapes.Diagonal
public import Mathlib.CategoryTheory.Monoidal.Cartesian.Over

/-!
# Fibred products of schemes

In this file we construct the fibred product of schemes via gluing.
We roughly follow [har77] Theorem 3.3.

In particular, the main construction is to show that for an open cover `{ Uᵢ }` of `X`, if there
exist fibred products `Uᵢ ×[Z] Y` for each `i`, then there exists a fibred product `X ×[Z] Y`.

Then, for constructing the fibred product for arbitrary schemes `X, Y, Z`, we can use the
construction to reduce to the case where `X, Y, Z` are all affine, where fibred products are
constructed via tensor products.

-/

@[expose] public section


universe u v w

noncomputable section

open CategoryTheory Functor CartesianMonoidalCategory Limits AlgebraicGeometry

namespace AlgebraicGeometry.Scheme

namespace Pullback

variable {X Y Z : Scheme.{u}} (𝒰 : OpenCover.{u} X) (f : X ⟶ Z) (g : Y ⟶ Z)
variable [forall i, HasPullback (𝒰.f i ≫ f) g]

/-- The intersection of `Uᵢ ×[Z] Y` and `Uⱼ ×[Z] Y` is given by (Uᵢ ×[Z] Y) ×[X] Uⱼ -/
@[instance_reducible]
/--
Definition of `v` / `v` 的定义

English:
definition v
  signature: (i j : 𝒰.I₀)
  body: pullback ((pullback.fst (𝒰.f i ≫ f) g) ≫ 𝒰.f i) (𝒰.f j)

中文:
定义 v
  签名: (i j : 𝒰.I₀)
  定义体: pullback ((pullback.fst (𝒰.f i ≫ f) g) ≫ 𝒰.f i) (𝒰.f j)

Depends on / 依赖: pullback, pullback.fst
-/
def v (i j : 𝒰.I₀) : Scheme :=
  pullback ((pullback.fst (𝒰.f i ≫ f) g) ≫ 𝒰.f i) (𝒰.f j)

/--
Definition of `t` / `t` 的定义

English:
definition t
  signature: (i j : 𝒰.I₀)
  body: by
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f i ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f j) (𝒰.f i) (𝒰.f i ≫ f) g
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f j ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f i) (𝒰.f j) (𝒰.f j ≫ f) g
  refine (pullbackSymmetry ..).hom ≫ (pullbackAssoc ..).inv ≫ ?_
  re

中文:
定义 t
  签名: (i j : 𝒰.I₀)
  定义体: by
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f i ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f j) (𝒰.f i) (𝒰.f i ≫ f) g
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f j ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f i) (𝒰.f j) (𝒰.f j ≫ f) g
  refine (pullbackSymmetry ..).hom ≫ (pullbackAssoc ..).inv ≫ ?_
  re

Depends on / 依赖: Category, Category.comp, HasPullback, condition_assoc, hasPullback_assoc_symm, pullback, pullback.condition_assoc, pullback.map, pullback.snd, pullbackAssoc, pullbackSymmetry, pullbackSymmetry_hom_comp_snd_assoc
-/
def t (i j : 𝒰.I₀) : v 𝒰 f g i j ⟶ v 𝒰 f g j i := by
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f i ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f j) (𝒰.f i) (𝒰.f i ≫ f) g
  have : HasPullback (pullback.snd _ _ ≫ 𝒰.f j ≫ f) g :=
    hasPullback_assoc_symm (𝒰.f i) (𝒰.f j) (𝒰.f j ≫ f) g
  refine (pullbackSymmetry ..).hom ≫ (pullbackAssoc ..).inv ≫ ?_
  refine ?_ ≫ (pullbackAssoc ..).hom ≫ (pullbackSymmetry ..).hom
  refine pullback.map _ _ _ _ (pullbackSymmetry _ _).hom (𝟙 _) (𝟙 _) ?_ ?_
  · rw [pullbackSymmetry_hom_comp_snd_assoc, pullback.condition_assoc, Category.comp_id]
  · rw [Category.comp_id, Category.id_comp]

@[simp, reassoc]
/--
theorem `t_fst_fst` / 定理 `t_fst_fst`

English:
theorem t_fst_fst
  given: (i j : 𝒰.I₀)
  statement: t 𝒰 f g i j ≫ pullback.fst _ _ ≫ pullback.fst _ _ =
  proof: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_inv_fst_fst,
    pullbackSymmetry_hom_comp_fst]

@[simp, reassoc]

中文:
定理 t_fst_fst
  条件: (i j : 𝒰.I₀)
  结论: t 𝒰 f g i j ≫ pullback.fst _ _ ≫ pullback.fst _ _ =
  证明: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_inv_fst_fst,
    pullbackSymmetry_hom_comp_fst]

@[simp, reassoc]

Depends on / 依赖: Category, Category.assoc, lift_fst_assoc, pullback, pullback.lift_fst_assoc, pullbackAssoc_hom_snd_fst, pullbackAssoc_inv_fst_fst, pullbackSymmetry_hom_comp_fst, pullbackSymmetry_hom_comp_fst_assoc, pullbackSymmetry_hom_comp_snd
-/
theorem t_fst_fst (i j : 𝒰.I₀) : t 𝒰 f g i j ≫ pullback.fst _ _ ≫ pullback.fst _ _ =
    pullback.snd _ _ := by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_inv_fst_fst,
    pullbackSymmetry_hom_comp_fst]

@[simp, reassoc]
/--
theorem `t_fst_snd` / 定理 `t_fst_snd`

English:
theorem t_fst_snd
  given: (i j : 𝒰.I₀)
  proof: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_snd,
    pullback.lift_snd, Category.comp_id, pullbackAssoc_inv_snd, pullbackSymmetry_hom_comp_snd_assoc]

@[simp, reassoc]

中文:
定理 t_fst_snd
  条件: (i j : 𝒰.I₀)
  证明: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_snd,
    pullback.lift_snd, Category.comp_id, pullbackAssoc_inv_snd, pullbackSymmetry_hom_comp_snd_assoc]

@[simp, reassoc]

Depends on / 依赖: Category, Category.assoc, Category.comp_id, comp_id, lift_snd, pullback, pullback.lift_snd, pullbackAssoc_hom_snd_snd, pullbackAssoc_inv_snd, pullbackSymmetry_hom_comp_fst_assoc, pullbackSymmetry_hom_comp_snd_assoc
-/
theorem t_fst_snd (i j : 𝒰.I₀) :
    t 𝒰 f g i j ≫ pullback.fst _ _ ≫ pullback.snd _ _ = pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_fst_assoc, pullbackAssoc_hom_snd_snd,
    pullback.lift_snd, Category.comp_id, pullbackAssoc_inv_snd, pullbackSymmetry_hom_comp_snd_assoc]

@[simp, reassoc]
/--
theorem `t_snd` / 定理 `t_snd`

English:
theorem t_snd
  given: (i j : 𝒰.I₀)
  statement: t 𝒰 f g i j ≫ pullback.snd _ _ =
  proof: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_hom_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_fst, pullbackAssoc_inv_fst_snd,
    pullbackSymmetry_hom_comp_snd_assoc]

中文:
定理 t_snd
  条件: (i j : 𝒰.I₀)
  结论: t 𝒰 f g i j ≫ pullback.snd _ _ =
  证明: by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_hom_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_fst, pullbackAssoc_inv_fst_snd,
    pullbackSymmetry_hom_comp_snd_assoc]

Depends on / 依赖: Category, Category.assoc, lift_fst_assoc, pullback, pullback.lift_fst_assoc, pullbackAssoc_hom_fst, pullbackAssoc_inv_fst_snd, pullbackSymmetry_hom_comp_fst, pullbackSymmetry_hom_comp_snd, pullbackSymmetry_hom_comp_snd_assoc
-/
theorem t_snd (i j : 𝒰.I₀) : t 𝒰 f g i j ≫ pullback.snd _ _ =
    pullback.fst _ _ ≫ pullback.fst _ _ := by
  simp only [t, Category.assoc, pullbackSymmetry_hom_comp_snd, pullbackAssoc_hom_fst,
    pullback.lift_fst_assoc, pullbackSymmetry_hom_comp_fst, pullbackAssoc_inv_fst_snd,
    pullbackSymmetry_hom_comp_snd_assoc]

/--
theorem `t_id` / 定理 `t_id`

English:
theorem t_id
  given: (i : 𝒰.I₀)
  statement: t 𝒰 f g i i = 𝟙 _
  proof: by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, Category.assoc, t_fst_fst]
    · simp only [Category.assoc, t_fst_snd]
  · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, t_snd, Category.asso

中文:
定理 t_id
  条件: (i : 𝒰.I₀)
  结论: t 𝒰 f g i i = 𝟙 _
  证明: by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, Category.assoc, t_fst_fst]
    · simp only [Category.assoc, t_fst_snd]
  · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, t_snd, Category.asso

Depends on / 依赖: Category, Category.assoc, Category.id_comp, cancel_mono, condition, hom_ext, id_comp, pullback, pullback.condition, pullback.hom_ext, t_fst_fst, t_fst_snd, t_snd
-/
theorem t_id (i : 𝒰.I₀) : t 𝒰 f g i i = 𝟙 _ := by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, Category.assoc, t_fst_fst]
    · simp only [Category.assoc, t_fst_snd]
  · rw [← cancel_mono (𝒰.f i)]; simp only [pullback.condition, t_snd, Category.assoc]

/--
Definition of `fV` / `fV` 的定义

English:
abbreviation fV
  signature: (i j : 𝒰.I₀)
  body: pullback.fst _ _

中文:
缩写 fV
  签名: (i j : 𝒰.I₀)
  定义体: pullback.fst _ _

Depends on / 依赖: pullback, pullback.fst
-/
abbrev fV (i j : 𝒰.I₀) : v 𝒰 f g i j ⟶ pullback (𝒰.f i ≫ f) g :=
  pullback.fst _ _

/--
Definition of `t'` / `t'` 的定义

English:
definition t'
  signature: (i j k : 𝒰.I₀)
  body: by
  refine (pullbackRightPullbackFstIso ..).hom ≫ ?_
  refine ?_ ≫ (pullbackSymmetry _ _).hom
  refine ?_ ≫ (pullbackRightPullbackFstIso ..).inv
  refine pullback.map _ _ _ _ (t 𝒰 f g i j) (𝟙 _) (𝟙 _) ?_ ?_
  · simp_rw [Category.comp_id, t_fst_fst_assoc, ← pullback.condition]
  · rw [Category.comp_

中文:
定义 t'
  签名: (i j k : 𝒰.I₀)
  定义体: by
  refine (pullbackRightPullbackFstIso ..).hom ≫ ?_
  refine ?_ ≫ (pullbackSymmetry _ _).hom
  refine ?_ ≫ (pullbackRightPullbackFstIso ..).inv
  refine pullback.map _ _ _ _ (t 𝒰 f g i j) (𝟙 _) (𝟙 _) ?_ ?_
  · simp_rw [Category.comp_id, t_fst_fst_assoc, ← pullback.condition]
  · rw [Category.comp_

Depends on / 依赖: Category, Category.comp_id, Category.id_comp, comp_id, condition, id_comp, pullback, pullback.condition, pullback.map, pullbackRightPullbackFstIso, pullbackSymmetry, simp_rw, t_fst_fst_assoc
-/
def t' (i j k : 𝒰.I₀) :
    pullback (fV 𝒰 f g i j) (fV 𝒰 f g i k) ⟶ pullback (fV 𝒰 f g j k) (fV 𝒰 f g j i) := by
  refine (pullbackRightPullbackFstIso ..).hom ≫ ?_
  refine ?_ ≫ (pullbackSymmetry _ _).hom
  refine ?_ ≫ (pullbackRightPullbackFstIso ..).inv
  refine pullback.map _ _ _ _ (t 𝒰 f g i j) (𝟙 _) (𝟙 _) ?_ ?_
  · simp_rw [Category.comp_id, t_fst_fst_assoc, ← pullback.condition]
  · rw [Category.comp_id, Category.id_comp]

@[simp, reassoc]
/--
theorem `t'_fst_fst_fst` / 定理 `t'_fst_fst_fst`

English:
theorem t'_fst_fst_fst
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]

中文:
定理 t'_fst_fst_fst
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
-/
theorem t'_fst_fst_fst (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.fst _ _ =
      pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
/--
theorem `t'_fst_fst_snd` / 定理 `t'_fst_fst_snd`

English:
theorem t'_fst_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]

中文:
定理 t'_fst_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
-/
theorem t'_fst_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ =
      pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
/--
theorem `t'_fst_snd` / 定理 `t'_fst_snd`

English:
theorem t'_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_snd, pullback.lift_snd, Category.comp_id,
    pullbackRightPullbackFstIso_hom_snd]

@[simp, reassoc]

中文:
定理 t'_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_snd, pullback.lift_snd, Category.comp_id,
    pullbackRightPullbackFstIso_hom_snd]

@[simp, reassoc]
-/
theorem t'_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.fst _ _ ≫ pullback.snd _ _ =
      pullback.snd _ _ ≫ pullback.snd _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_fst_assoc,
    pullbackRightPullbackFstIso_inv_snd_snd, pullback.lift_snd, Category.comp_id,
    pullbackRightPullbackFstIso_hom_snd]

@[simp, reassoc]
/--
theorem `t'_snd_fst_fst` / 定理 `t'_snd_fst_fst`

English:
theorem t'_snd_fst_fst
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]

中文:
定理 t'_snd_fst_fst
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
-/
theorem t'_snd_fst_fst (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.snd _ _ ≫ pullback.fst _ _ ≫ pullback.fst _ _ =
      pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_fst,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
/--
theorem `t'_snd_fst_snd` / 定理 `t'_snd_fst_snd`

English:
theorem t'_snd_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]

中文:
定理 t'_snd_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
-/
theorem t'_snd_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.snd _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ =
      pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_fst_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

@[simp, reassoc]
/--
theorem `t'_snd_snd` / 定理 `t'_snd_snd`

English:
theorem t'_snd_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

中文:
定理 t'_snd_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]
-/
theorem t'_snd_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ pullback.snd _ _ ≫ pullback.snd _ _ =
      pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.fst _ _ := by
  simp only [t', Category.assoc, pullbackSymmetry_hom_comp_snd_assoc,
    pullbackRightPullbackFstIso_inv_fst_assoc, pullback.lift_fst_assoc, t_snd,
    pullbackRightPullbackFstIso_hom_fst_assoc]

/--
theorem `cocycle_fst_fst_fst` / 定理 `cocycle_fst_fst_fst`

English:
theorem cocycle_fst_fst_fst
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t'_fst_fst_fst, t'_fst_snd, t'_snd_snd]

中文:
定理 cocycle_fst_fst_fst
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t'_fst_fst_fst, t'_fst_snd, t'_snd_snd]

Depends on / 依赖: Functor, Functor.PullbackObjObj.ofIsInitial, MonoidalClosed, MonoidalClosed.internalHom, PullbackObjObj, _fst_fst_fst, _fst_snd, _snd_snd, initial, initial.to, initialIsInitial, internalHom, ofIsInitial
-/
theorem cocycle_fst_fst_fst (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.fst _ _ ≫ pullback.fst _ _ ≫
      pullback.fst _ _ = pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.fst _ _ := by
  simp only [t'_fst_fst_fst, t'_fst_snd, t'_snd_snd]

/--
theorem `cocycle_fst_fst_snd` / 定理 `cocycle_fst_fst_snd`

English:
theorem cocycle_fst_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t'_fst_fst_snd]

中文:
定理 cocycle_fst_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t'_fst_fst_snd]

Depends on / 依赖: Functor, Functor.PullbackObjObj.ofIsTerminal, MonoidalClosed, MonoidalClosed.internalHom, PullbackObjObj, _fst_fst_snd, internalHom, ofIsTerminal, terminal, terminal.from, terminalIsTerminal
-/
theorem cocycle_fst_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.fst _ _ ≫ pullback.fst _ _ ≫
      pullback.snd _ _ = pullback.fst _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t'_fst_fst_snd]

/--
theorem `cocycle_fst_snd` / 定理 `cocycle_fst_snd`

English:
theorem cocycle_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t'_fst_snd, t'_snd_snd, t'_fst_fst_fst]

中文:
定理 cocycle_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t'_fst_snd, t'_snd_snd, t'_fst_fst_fst]

Depends on / 依赖: IsTerminal, IsTerminal.isTerminalObj, _fst_fst_fst, _fst_snd, _snd_snd, infer_instance, isIso_of_isTerminal, isTerminalObj, terminal, terminal.from, terminalIsTerminal
-/
theorem cocycle_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.fst _ _ ≫ pullback.snd _ _ =
      pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [t'_fst_snd, t'_snd_snd, t'_fst_fst_fst]

/--
theorem `cocycle_snd_fst_fst` / 定理 `cocycle_snd_fst_fst`

English:
theorem cocycle_snd_fst_fst
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [pullback.condition_assoc, t'_snd_fst_fst, t'_fst_snd, t'_snd_snd]

中文:
定理 cocycle_snd_fst_fst
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [pullback.condition_assoc, t'_snd_fst_fst, t'_fst_snd, t'_snd_snd]

Depends on / 依赖: _fst_snd, _snd_fst_fst, _snd_snd, condition_assoc, pullback, pullback.condition_assoc, quasicategory_of_innerFibration, terminal, terminal.from
-/
theorem cocycle_snd_fst_fst (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.snd _ _ ≫ pullback.fst _ _ ≫
      pullback.fst _ _ = pullback.snd _ _ ≫ pullback.fst _ _ ≫ pullback.fst _ _ := by
  simp only [pullback.condition_assoc, t'_snd_fst_fst, t'_fst_snd, t'_snd_snd]

/--
theorem `cocycle_snd_fst_snd` / 定理 `cocycle_snd_fst_snd`

English:
theorem cocycle_snd_fst_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [pullback.condition_assoc, t'_snd_fst_snd]

中文:
定理 cocycle_snd_fst_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [pullback.condition_assoc, t'_snd_fst_snd]

Depends on / 依赖: _snd_fst_snd, condition_assoc, pullback, pullback.condition_assoc
-/
theorem cocycle_snd_fst_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.snd _ _ ≫ pullback.fst _ _ ≫
      pullback.snd _ _ = pullback.snd _ _ ≫ pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp only [pullback.condition_assoc, t'_snd_fst_snd]

/--
theorem `cocycle_snd_snd` / 定理 `cocycle_snd_snd`

English:
theorem cocycle_snd_snd
  given: (i j k : 𝒰.I₀)
  proof: by
  simp only [t'_snd_snd, t'_fst_fst_fst, t'_fst_snd]

中文:
定理 cocycle_snd_snd
  条件: (i j k : 𝒰.I₀)
  证明: by
  simp only [t'_snd_snd, t'_fst_fst_fst, t'_fst_snd]

Depends on / 依赖: _fst_fst_fst, _fst_snd, _snd_snd
-/
theorem cocycle_snd_snd (i j k : 𝒰.I₀) :
    t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j ≫ pullback.snd _ _ ≫ pullback.snd _ _ =
      pullback.snd _ _ ≫ pullback.snd _ _ := by
  simp only [t'_snd_snd, t'_fst_fst_fst, t'_fst_snd]

-- `by tidy` should solve it, but it times out.
/--
theorem `cocycle` / 定理 `cocycle`

English:
theorem cocycle
  given: (i j k : 𝒰.I₀)
  statement: t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j = 𝟙 _
  proof: by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · apply pullback.hom_ext
      · simp_rw [Category.assoc, cocycle_fst_fst_fst 𝒰 f g i j k]
      · simp_rw [Category.assoc, cocycle_fst_fst_snd 𝒰 f g i j k]
    · simp_rw [Category.assoc, cocycle_fst_snd 𝒰 f g i j k

中文:
定理 cocycle
  条件: (i j k : 𝒰.I₀)
  结论: t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j = 𝟙 _
  证明: by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · apply pullback.hom_ext
      · simp_rw [Category.assoc, cocycle_fst_fst_fst 𝒰 f g i j k]
      · simp_rw [Category.assoc, cocycle_fst_fst_snd 𝒰 f g i j k]
    · simp_rw [Category.assoc, cocycle_fst_snd 𝒰 f g i j k

Depends on / 依赖: Category, Category.assoc, Category.id_comp, cocycle_fst_fst_fst, cocycle_fst_fst_snd, cocycle_fst_snd, cocycle_snd_fst_fst, cocycle_snd_fst_snd, cocycle_snd_snd, hom_ext, id_comp, pullback, pullback.hom_ext, simp_rw
-/
theorem cocycle (i j k : 𝒰.I₀) : t' 𝒰 f g i j k ≫ t' 𝒰 f g j k i ≫ t' 𝒰 f g k i j = 𝟙 _ := by
  apply pullback.hom_ext <;> rw [Category.id_comp]
  · apply pullback.hom_ext
    · apply pullback.hom_ext
      · simp_rw [Category.assoc, cocycle_fst_fst_fst 𝒰 f g i j k]
      · simp_rw [Category.assoc, cocycle_fst_fst_snd 𝒰 f g i j k]
    · simp_rw [Category.assoc, cocycle_fst_snd 𝒰 f g i j k]
  · apply pullback.hom_ext
    · apply pullback.hom_ext
      · simp_rw [Category.assoc, cocycle_snd_fst_fst 𝒰 f g i j k]
      · simp_rw [Category.assoc, cocycle_snd_fst_snd 𝒰 f g i j k]
    · simp_rw [Category.assoc, cocycle_snd_snd 𝒰 f g i j k]

/-- Given `Uᵢ ×[Z] Y`, this is the glued fibred product `X ×[Z] Y`. -/
@[simps U V f t t', simps -isSimp J]
/--
Definition of `gluing` / `gluing` 的定义

English:
definition gluing
  signature: : Scheme.GlueData.{u} where
  body: 𝒰.I₀
  U i := pullback (𝒰.f i ≫ f) g
  V := fun ⟨i, j⟩ => v 𝒰 f g i j
  -- `p⁻¹(Uᵢ ∩ Uⱼ)` where `p : Uᵢ ×[Z] Y ⟶ Uᵢ ⟶ X`.
  f _ _ := pullback.fst _ _
  f_id _ := inferInstance
  f_open := inferInstance
  t i j := t 𝒰 f g i j
  t_id i := t_id 𝒰 f g i
  t' i j k := t' 𝒰 f g i j k
  t_fac i j k := by
 

中文:
定义 gluing
  签名: : 概形.粘合数据.{u} where
  定义体: 𝒰.I₀
  U i := pullback (𝒰.f i ≫ f) g
  V := fun ⟨i, j⟩ => v 𝒰 f g i j
  -- `p⁻¹(Uᵢ ∩ Uⱼ)` where `p : Uᵢ ×[Z] Y ⟶ Uᵢ ⟶ X`.
  f _ _ := pullback.fst _ _
  f_id _ := inferInstance
  f_open := inferInstance
  t i j := t 𝒰 f g i j
  t_id i := t_id 𝒰 f g i
  t' i j k := t' 𝒰 f g i j k
  t_fac i j k := by
 
-/
def gluing : Scheme.GlueData.{u} where
  J := 𝒰.I₀
  U i := pullback (𝒰.f i ≫ f) g
  V := fun ⟨i, j⟩ => v 𝒰 f g i j
  -- `p⁻¹(Uᵢ ∩ Uⱼ)` where `p : Uᵢ ×[Z] Y ⟶ Uᵢ ⟶ X`.
  f _ _ := pullback.fst _ _
  f_id _ := inferInstance
  f_open := inferInstance
  t i j := t 𝒰 f g i j
  t_id i := t_id 𝒰 f g i
  t' i j k := t' 𝒰 f g i j k
  t_fac i j k := by
    apply pullback.hom_ext
    on_goal 1 => apply pullback.hom_ext
    all_goals
      simp only [t'_snd_fst_fst, t'_snd_fst_snd, t'_snd_snd, t_fst_fst, t_fst_snd, t_snd,
        Category.assoc]
  cocycle i j k := cocycle 𝒰 f g i j k

@[simp]
/--
lemma `gluing_ι` / 引理 `gluing_ι`

English:
lemma gluing_ι
  given: (j : 𝒰.I₀)
  proof: rfl

中文:
引理 gluing_ι
  条件: (j : 𝒰.I₀)
  证明: rfl
-/
lemma gluing_ι (j : 𝒰.I₀) :
    (gluing 𝒰 f g).ι j = Multicoequalizer.π (gluing 𝒰 f g).diagram j := rfl

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `p1` / `p1` 的定义

English:
definition p1
  signature: : (gluing 𝒰 f g).glued ⟶ X
  body: by
  apply Multicoequalizer.desc (gluing 𝒰 f g).diagram _ fun i => pullback.fst _ _ ≫ 𝒰.f i
  simp [t_fst_fst_assoc, ← pullback.condition]

中文:
定义 p1
  签名: : (gluing 𝒰 f g).glued ⟶ X
  定义体: by
  apply Multicoequalizer.desc (gluing 𝒰 f g).diagram _ fun i => pullback.fst _ _ ≫ 𝒰.f i
  simp [t_fst_fst_assoc, ← pullback.condition]

Depends on / 依赖: Multicoequalizer, Multicoequalizer.desc, condition, diagram, gluing, pullback, pullback.condition, pullback.fst, t_fst_fst_assoc
-/
def p1 : (gluing 𝒰 f g).glued ⟶ X := by
  apply Multicoequalizer.desc (gluing 𝒰 f g).diagram _ fun i => pullback.fst _ _ ≫ 𝒰.f i
  simp [t_fst_fst_assoc, ← pullback.condition]

set_option backward.isDefEq.respectTransparency.types false in
set_option backward.defeqAttrib.useBackward true in
/--
Definition of `p2` / `p2` 的定义

English:
definition p2
  signature: : (gluing 𝒰 f g).glued ⟶ Y
  body: by
  apply Multicoequalizer.desc _ _ fun i => pullback.snd _ _
  simp [t_fst_snd]

中文:
定义 p2
  签名: : (gluing 𝒰 f g).glued ⟶ Y
  定义体: by
  apply Multicoequalizer.desc _ _ fun i => pullback.snd _ _
  simp [t_fst_snd]

Depends on / 依赖: Multicoequalizer, Multicoequalizer.desc, pullback, pullback.snd, t_fst_snd
-/
def p2 : (gluing 𝒰 f g).glued ⟶ Y := by
  apply Multicoequalizer.desc _ _ fun i => pullback.snd _ _
  simp [t_fst_snd]

set_option backward.isDefEq.respectTransparency false in
/--
theorem `p_comm` / 定理 `p_comm`

English:
theorem p_comm
  statement: p1 𝒰 f g ≫ f = p2 𝒰 f g ≫ g
  proof: by
  apply Multicoequalizer.hom_ext
  simp [p1, p2, pullback.condition]

中文:
定理 p_comm
  结论: p1 𝒰 f g ≫ f = p2 𝒰 f g ≫ g
  证明: by
  apply Multicoequalizer.hom_ext
  simp [p1, p2, pullback.condition]

Depends on / 依赖: Multicoequalizer, Multicoequalizer.hom_ext, condition, hom_ext, pullback, pullback.condition
-/
theorem p_comm : p1 𝒰 f g ≫ f = p2 𝒰 f g ≫ g := by
  apply Multicoequalizer.hom_ext
  simp [p1, p2, pullback.condition]

variable (s : PullbackCone f g)

set_option backward.isDefEq.respectTransparency false in
/--
Definition of `gluedLiftPullbackMap` / `gluedLiftPullbackMap` 的定义

English:
definition gluedLiftPullbackMap
  signature: (i j : 𝒰.I₀)
  body: by
  refine (pullbackRightPullbackFstIso _ _ _).hom ≫ ?_
  refine pullback.map _ _ _ _ ?_ (𝟙 _) (𝟙 _) ?_ ?_
  · exact (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition
  · simpa using! pullback.condition
  · simp only [Category.comp_id, Categ

中文:
定义 gluedLiftPullbackMap
  签名: (i j : 𝒰.I₀)
  定义体: by
  refine (pullbackRightPullbackFstIso _ _ _).hom ≫ ?_
  refine pullback.map _ _ _ _ ?_ (𝟙 _) (𝟙 _) ?_ ?_
  · exact (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition
  · simpa using! pullback.condition
  · simp only [Category.comp_id, Categ

Depends on / 依赖: Category, Category.comp_id, Category.id_comp, comp_id, condition, id_comp, pullback, pullback.condition, pullback.map, pullbackRightPullbackFstIso, pullbackSymmetry, s.condition, s.snd
-/
def gluedLiftPullbackMap (i j : 𝒰.I₀) :
    pullback ((𝒰.pullback₁ s.fst).f i) ((𝒰.pullback₁ s.fst).f j) ⟶
      (gluing 𝒰 f g).V ⟨i, j⟩ := by
  refine (pullbackRightPullbackFstIso _ _ _).hom ≫ ?_
  refine pullback.map _ _ _ _ ?_ (𝟙 _) (𝟙 _) ?_ ?_
  · exact (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition
  · simpa using! pullback.condition
  · simp only [Category.comp_id, Category.id_comp]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
@[reassoc]
/--
theorem `gluedLiftPullbackMap_fst` / 定理 `gluedLiftPullbackMap_fst`

English:
theorem gluedLiftPullbackMap_fst
  given: (i j : 𝒰.I₀)
  proof: by
  simp [gluedLiftPullbackMap]

中文:
定理 gluedLiftPullbackMap_fst
  条件: (i j : 𝒰.I₀)
  证明: by
  simp [gluedLiftPullbackMap]

Depends on / 依赖: gluedLiftPullbackMap
-/
theorem gluedLiftPullbackMap_fst (i j : 𝒰.I₀) :
    gluedLiftPullbackMap 𝒰 f g s i j ≫ pullback.fst _ _ =
      pullback.fst _ _ ≫
        (pullbackSymmetry _ _).hom ≫
          pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition := by
  simp [gluedLiftPullbackMap]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
@[reassoc]
/--
theorem `gluedLiftPullbackMap_snd` / 定理 `gluedLiftPullbackMap_snd`

English:
theorem gluedLiftPullbackMap_snd
  given: (i j : 𝒰.I₀)
  proof: by
  simp [gluedLiftPullbackMap]

中文:
定理 gluedLiftPullbackMap_snd
  条件: (i j : 𝒰.I₀)
  证明: by
  simp [gluedLiftPullbackMap]

Depends on / 依赖: gluedLiftPullbackMap
-/
theorem gluedLiftPullbackMap_snd (i j : 𝒰.I₀) :
    gluedLiftPullbackMap 𝒰 f g s i j ≫ pullback.snd _ _ = pullback.snd _ _ ≫ pullback.snd _ _ := by
  simp [gluedLiftPullbackMap]

set_option backward.isDefEq.respectTransparency false in
/--
Definition of `gluedLift` / `gluedLift` 的定义

English:
definition gluedLift
  signature: : s.pt ⟶ (gluing 𝒰 f g).glued
  body: by
  fapply Cover.glueMorphisms (𝒰.pullback₁ s.fst)
  · exact fun i => (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition ≫ (gluing 𝒰 f g).ι i
  intro i j
  rw [← gluedLiftPullbackMap_fst_assoc]; rw [← gluing_f]; rw [← (gluing 𝒰 f g).glue_cond

中文:
定义 gluedLift
  签名: : s.pt ⟶ (gluing 𝒰 f g).glued
  定义体: by
  fapply Cover.glueMorphisms (𝒰.pullback₁ s.fst)
  · exact fun i => (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition ≫ (gluing 𝒰 f g).ι i
  intro i j
  rw [← gluedLiftPullbackMap_fst_assoc]; rw [← gluing_f]; rw [← (gluing 𝒰 f g).glue_cond

Depends on / 依赖: Category, Category.assoc, Category.id_comp, Cover.glueMorphisms, Iso.inv_comp_eq, condition, fapply, glueMorphisms, glue_condition, gluedLiftPullbackMap_fst_assoc, gluedLiftPullbackMap_snd, gluing, gluing_f, gluing_t, hom_ext, id_comp, inv_comp_eq, pullbac, pullback, pullback.hom_ext
-/
def gluedLift : s.pt ⟶ (gluing 𝒰 f g).glued := by
  fapply Cover.glueMorphisms (𝒰.pullback₁ s.fst)
  · exact fun i => (pullbackSymmetry _ _).hom ≫
      pullback.map _ _ _ _ (𝟙 _) s.snd f (Category.id_comp _).symm s.condition ≫ (gluing 𝒰 f g).ι i
  intro i j
  rw [← gluedLiftPullbackMap_fst_assoc]; rw [← gluing_f]; rw [← (gluing 𝒰 f g).glue_condition i j]; rw [gluing_t]; rw [gluing_f]
  simp_rw [← Category.assoc]
  congr 1
  apply pullback.hom_ext <;> simp_rw [Category.assoc]
  · rw [t_fst_fst, gluedLiftPullbackMap_snd]
    congr 1
    rw [← Iso.inv_comp_eq]; rw [pullbackSymmetry_inv_comp_snd]; rw [pullback.lift_fst]; rw [Category.comp_id]
  · rw [t_fst_snd, gluedLiftPullbackMap_fst_assoc, pullback.lift_snd, pullback.lift_snd]
    simp_rw [pullbackSymmetry_hom_comp_snd_assoc]
    exact pullback.condition_assoc _

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `gluedLift_p1` / 定理 `gluedLift_p1`

English:
theorem gluedLift_p1
  statement: gluedLift 𝒰 f g s ≫ p1 𝒰 f g = s.fst
  proof: by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [Cover.ι_glueMorphisms (𝒰.pullback₁ s.fst)]
  simp [p1, pullback.condition]

中文:
定理 gluedLift_p1
  结论: gluedLift 𝒰 f g s ≫ p1 𝒰 f g = s.fst
  证明: by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [Cover.ι_glueMorphisms (𝒰.pullback₁ s.fst)]
  simp [p1, pullback.condition]

Depends on / 依赖: Category, Category.assoc, Cover.fromGlued, Multicoequalizer, Multicoequalizer.hom_ext, cancel_epi, condition, fromGlued, gluedLift, hom_ext, pullback, pullback.condition, s.fst, simp_rw
-/
theorem gluedLift_p1 : gluedLift 𝒰 f g s ≫ p1 𝒰 f g = s.fst := by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [Cover.ι_glueMorphisms (𝒰.pullback₁ s.fst)]
  simp [p1, pullback.condition]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `gluedLift_p2` / 定理 `gluedLift_p2`

English:
theorem gluedLift_p2
  statement: gluedLift 𝒰 f g s ≫ p2 𝒰 f g = s.snd
  proof: by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [(Cover.ι_glueMorphisms <| 𝒰.pullback₁ s.fst)]
  simp [p2]

中文:
定理 gluedLift_p2
  结论: gluedLift 𝒰 f g s ≫ p2 𝒰 f g = s.snd
  证明: by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [(Cover.ι_glueMorphisms <| 𝒰.pullback₁ s.fst)]
  simp [p2]

Depends on / 依赖: Category, Category.assoc, Cover.fromGlued, Multicoequalizer, Multicoequalizer.hom_ext, cancel_epi, fromGlued, gluedLift, hom_ext, s.fst, simp_rw
-/
theorem gluedLift_p2 : gluedLift 𝒰 f g s ≫ p2 𝒰 f g = s.snd := by
  rw [← cancel_epi (Cover.fromGlued <| 𝒰.pullback₁ s.fst)]
  apply Multicoequalizer.hom_ext
  intro b
  simp_rw [Cover.fromGlued, Multicoequalizer.π_desc_assoc, gluedLift, ← Category.assoc]
  simp_rw [(Cover.ι_glueMorphisms <| 𝒰.pullback₁ s.fst)]
  simp [p2]

set_option backward.isDefEq.respectTransparency.types false in
/--
Definition of `pullbackFstιToV` / `pullbackFstιToV` 的定义

English:
definition pullbackFstιToV
  signature: (i j : 𝒰.I₀)
  body: (pullbackSymmetry _ _ ≪≫ pullbackRightPullbackFstIso (p1 𝒰 f g) (𝒰.f i) _).hom ≫
    (pullback.congrHom (Multicoequalizer.π_desc ..) rfl).hom

中文:
定义 pullbackFstιToV
  签名: (i j : 𝒰.I₀)
  定义体: (pullbackSymmetry _ _ ≪≫ pullbackRightPullbackFstIso (p1 𝒰 f g) (𝒰.f i) _).hom ≫
    (pullback.congrHom (Multicoequalizer.π_desc ..) rfl).hom

Depends on / 依赖: Multicoequalizer, congrHom, pullback, pullback.congrHom, pullbackRightPullbackFstIso, pullbackSymmetry
-/
def pullbackFstιToV (i j : 𝒰.I₀) :
    pullback (pullback.fst (p1 𝒰 f g) (𝒰.f i)) ((gluing 𝒰 f g).ι j) ⟶
      v 𝒰 f g j i :=
  (pullbackSymmetry _ _ ≪≫ pullbackRightPullbackFstIso (p1 𝒰 f g) (𝒰.f i) _).hom ≫
    (pullback.congrHom (Multicoequalizer.π_desc ..) rfl).hom

set_option backward.isDefEq.respectTransparency false in
@[simp, reassoc]
/--
theorem `pullbackFstιToV_fst` / 定理 `pullbackFstιToV_fst`

English:
theorem pullbackFstιToV_fst
  given: (i j : 𝒰.I₀)
  proof: by
  simp [pullbackFstιToV, p1]

中文:
定理 pullbackFstιToV_fst
  条件: (i j : 𝒰.I₀)
  证明: by
  simp [pullbackFstιToV, p1]
-/
theorem pullbackFstιToV_fst (i j : 𝒰.I₀) :
    pullbackFstιToV 𝒰 f g i j ≫ pullback.fst _ _ = pullback.snd _ _ := by
  simp [pullbackFstιToV, p1]

set_option backward.isDefEq.respectTransparency false in
@[simp, reassoc]
/--
theorem `pullbackFstιToV_snd` / 定理 `pullbackFstιToV_snd`

English:
theorem pullbackFstιToV_snd
  given: (i j : 𝒰.I₀)
  proof: by
  simp [pullbackFstιToV, p1]

中文:
定理 pullbackFstιToV_snd
  条件: (i j : 𝒰.I₀)
  证明: by
  simp [pullbackFstιToV, p1]
-/
theorem pullbackFstιToV_snd (i j : 𝒰.I₀) :
    pullbackFstιToV 𝒰 f g i j ≫ pullback.snd _ _ = pullback.fst _ _ ≫ pullback.snd _ _ := by
  simp [pullbackFstιToV, p1]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `lift_comp_ι` / 定理 `lift_comp_ι`

English:
theorem lift_comp_ι
  given: (i : 𝒰.I₀)
  proof: by
  apply Cover.hom_ext ((gluing 𝒰 f g).openCover.pullback₁ (pullback.fst _ _))
  intro j
  dsimp only [Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover,
    PreZeroHypercover.pullback₁_X, PreZeroHypercover.pullback₁_f]
  trans pullbackFstιToV 𝒰 f g i j ≫ fV 𝒰 f g j i ≫ (gluing 𝒰 f g).ι _
 

中文:
定理 lift_comp_ι
  条件: (i : 𝒰.I₀)
  证明: by
  apply Cover.hom_ext ((gluing 𝒰 f g).openCover.pullback₁ (pullback.fst _ _))
  intro j
  dsimp only [Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover,
    PreZeroHypercover.pullback₁_X, PreZeroHypercover.pullback₁_f]
  trans pullbackFstιToV 𝒰 f g i j ≫ fV 𝒰 f g j i ≫ (gluing 𝒰 f g).ι _
 

Depends on / 依赖: Category, Category.assoc, Cover.hom_ext, PreZeroHypercover, PreZeroHypercover.pullback, Precoverage, Precoverage.ZeroHypercover.pullback, ZeroHypercover, glue_condition, gluing, gluing_f, gluing_t, hom_ext, openCover, openCover.pullback, pullback, pullback.fst, pullback.hom_ext, simp_rw, t_fst_fst
-/
theorem lift_comp_ι (i : 𝒰.I₀) :
    pullback.lift (pullback.snd _ _) (pullback.fst _ _ ≫ p2 𝒰 f g)
          (by rw [← pullback.condition_assoc, Category.assoc, p_comm]) ≫
        (gluing 𝒰 f g).ι i =
      (pullback.fst _ _ : pullback (p1 𝒰 f g) (𝒰.f i) ⟶ _) := by
  apply Cover.hom_ext ((gluing 𝒰 f g).openCover.pullback₁ (pullback.fst _ _))
  intro j
  dsimp only [Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover,
    PreZeroHypercover.pullback₁_X, PreZeroHypercover.pullback₁_f]
  trans pullbackFstιToV 𝒰 f g i j ≫ fV 𝒰 f g j i ≫ (gluing 𝒰 f g).ι _
  · rw [← show _ = fV 𝒰 f g j i ≫ _ from (gluing 𝒰 f g).glue_condition j i]
    simp_rw [← Category.assoc]
    congr 1
    rw [gluing_f]; rw [gluing_t]
    apply pullback.hom_ext <;> simp_rw [Category.assoc]
    · simp_rw [t_fst_fst, pullback.lift_fst, pullbackFstιToV_snd, GlueData.openCover_f]
    · simp_rw [t_fst_snd, pullback.lift_snd, pullbackFstιToV_fst_assoc, pullback.condition_assoc,
        GlueData.openCover_f, p2]
      simp
  · rw [pullback.condition, ← Category.assoc]
    simp_rw [pullbackFstιToV_fst, GlueData.openCover_f]

set_option backward.isDefEq.respectTransparency false in
/--
Definition of `pullbackP1Iso` / `pullbackP1Iso` 的定义

English:
definition pullbackP1Iso
  signature: (i : 𝒰.I₀)
  body: by
  fconstructor
  · exact
      pullback.lift (pullback.snd _ _) (pullback.fst _ _ ≫ p2 𝒰 f g)
        (by rw [← pullback.condition_assoc, Category.assoc, p_comm])
  · exact pullback.lift ((gluing 𝒰 f g).ι i) (pullback.fst _ _)
      (by rw [gluing_ι, p1, Multicoequalizer.π_desc])
  · apply pullba

中文:
定义 pullbackP1Iso
  签名: (i : 𝒰.I₀)
  定义体: by
  fconstructor
  · exact
      pullback.lift (pullback.snd _ _) (pullback.fst _ _ ≫ p2 𝒰 f g)
        (by rw [← pullback.condition_assoc, Category.assoc, p_comm])
  · exact pullback.lift ((gluing 𝒰 f g).ι i) (pullback.fst _ _)
      (by rw [gluing_ι, p1, Multicoequalizer.π_desc])
  · apply pullba

Depends on / 依赖: Category, Category.assoc, Category.i, Category.id_comp, Multicoequalizer, condition_assoc, fconstructor, gluing, hom_ext, id_comp, lift_fst, lift_snd, p_comm, pullback, pullback.condition_assoc, pullback.fst, pullback.hom_ext, pullback.lift, pullback.lift_fst, pullback.lift_snd
-/
def pullbackP1Iso (i : 𝒰.I₀) : pullback (p1 𝒰 f g) (𝒰.f i) ≅ pullback (𝒰.f i ≫ f) g := by
  fconstructor
  · exact
      pullback.lift (pullback.snd _ _) (pullback.fst _ _ ≫ p2 𝒰 f g)
        (by rw [← pullback.condition_assoc, Category.assoc, p_comm])
  · exact pullback.lift ((gluing 𝒰 f g).ι i) (pullback.fst _ _)
      (by rw [gluing_ι, p1, Multicoequalizer.π_desc])
  · apply pullback.hom_ext
    · simpa using lift_comp_ι 𝒰 f g i
    · simp_rw [Category.assoc, pullback.lift_snd, pullback.lift_fst, Category.id_comp]
  · apply pullback.hom_ext
    · simp_rw [Category.assoc, pullback.lift_fst, pullback.lift_snd, Category.id_comp]
    · simp [p2]

@[simp, reassoc]
/--
theorem `pullbackP1Iso_hom_fst` / 定理 `pullbackP1Iso_hom_fst`

English:
theorem pullbackP1Iso_hom_fst
  given: (i : 𝒰.I₀)
  proof: by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

@[simp, reassoc]

中文:
定理 pullbackP1Iso_hom_fst
  条件: (i : 𝒰.I₀)
  证明: by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

@[simp, reassoc]

Depends on / 依赖: lift_fst, pullback, pullback.lift_fst, pullbackP1Iso, simp_rw
-/
theorem pullbackP1Iso_hom_fst (i : 𝒰.I₀) :
    (pullbackP1Iso 𝒰 f g i).hom ≫ pullback.fst _ _ = pullback.snd _ _ := by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

@[simp, reassoc]
/--
theorem `pullbackP1Iso_hom_snd` / 定理 `pullbackP1Iso_hom_snd`

English:
theorem pullbackP1Iso_hom_snd
  given: (i : 𝒰.I₀)
  proof: by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

中文:
定理 pullbackP1Iso_hom_snd
  条件: (i : 𝒰.I₀)
  证明: by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

Depends on / 依赖: lift_snd, pullback, pullback.lift_snd, pullbackP1Iso, simp_rw
-/
theorem pullbackP1Iso_hom_snd (i : 𝒰.I₀) :
    (pullbackP1Iso 𝒰 f g i).hom ≫ pullback.snd _ _ = pullback.fst _ _ ≫ p2 𝒰 f g := by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

set_option backward.isDefEq.respectTransparency.types false in
@[simp, reassoc]
/--
theorem `pullbackP1Iso_inv_fst` / 定理 `pullbackP1Iso_inv_fst`

English:
theorem pullbackP1Iso_inv_fst
  given: (i : 𝒰.I₀)
  proof: by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

中文:
定理 pullbackP1Iso_inv_fst
  条件: (i : 𝒰.I₀)
  证明: by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

Depends on / 依赖: lift_fst, pullback, pullback.lift_fst, pullbackP1Iso, simp_rw
-/
theorem pullbackP1Iso_inv_fst (i : 𝒰.I₀) :
    (pullbackP1Iso 𝒰 f g i).inv ≫ pullback.fst _ _ = (gluing 𝒰 f g).ι i := by
  simp_rw [pullbackP1Iso, pullback.lift_fst]

set_option backward.isDefEq.respectTransparency.types false in
@[simp, reassoc]
/--
theorem `pullbackP1Iso_inv_snd` / 定理 `pullbackP1Iso_inv_snd`

English:
theorem pullbackP1Iso_inv_snd
  given: (i : 𝒰.I₀)
  proof: by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

中文:
定理 pullbackP1Iso_inv_snd
  条件: (i : 𝒰.I₀)
  证明: by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

Depends on / 依赖: lift_snd, pullback, pullback.lift_snd, pullbackP1Iso, simp_rw
-/
theorem pullbackP1Iso_inv_snd (i : 𝒰.I₀) :
    (pullbackP1Iso 𝒰 f g i).inv ≫ pullback.snd _ _ = pullback.fst _ _ := by
  simp_rw [pullbackP1Iso, pullback.lift_snd]

set_option backward.isDefEq.respectTransparency false in
@[simp, reassoc]
/--
theorem `pullbackP1Iso_hom_ι` / 定理 `pullbackP1Iso_hom_ι`

English:
theorem pullbackP1Iso_hom_ι
  given: (i : 𝒰.I₀)
  proof: by
  rw [← gluing_ι]; rw [← pullbackP1Iso_inv_fst]; rw [Iso.hom_inv_id_assoc]

中文:
定理 pullbackP1Iso_hom_ι
  条件: (i : 𝒰.I₀)
  证明: by
  rw [← gluing_ι]; rw [← pullbackP1Iso_inv_fst]; rw [Iso.hom_inv_id_assoc]

Depends on / 依赖: Iso.hom_inv_id_assoc, hom_inv_id_assoc, pullbackP1Iso_inv_fst
-/
theorem pullbackP1Iso_hom_ι (i : 𝒰.I₀) :
    (pullbackP1Iso 𝒰 f g i).hom ≫ Multicoequalizer.π (gluing 𝒰 f g).diagram i =
    pullback.fst _ _ := by
  rw [← gluing_ι]; rw [← pullbackP1Iso_inv_fst]; rw [Iso.hom_inv_id_assoc]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
Definition of `gluedIsLimit` / `gluedIsLimit` 的定义

English:
definition gluedIsLimit
  signature: : IsLimit (PullbackCone.mk _ _ (p_comm 𝒰 f g))
  body: by
  apply PullbackCone.isLimitAux'
  intro s
  refine ⟨gluedLift 𝒰 f g s, gluedLift_p1 𝒰 f g s, gluedLift_p2 𝒰 f g s, ?_⟩
  intro m h₁ h₂
  simp_rw [PullbackCone.mk_pt, PullbackCone.mk_π_app] at h₁ h₂
apply Cover.hom_ext 𝒰.pullback₁ s.fst
  intro i
  rw [gluedLift]; rw [(Cover.ι_glueMorphisms <| 𝒰.

中文:
定义 gluedIsLimit
  签名: : 是极限 (PullbackCone.mk _ _ (p_comm 𝒰 f g))
  定义体: by
  apply PullbackCone.isLimitAux'
  intro s
  refine ⟨gluedLift 𝒰 f g s, gluedLift_p1 𝒰 f g s, gluedLift_p2 𝒰 f g s, ?_⟩
  intro m h₁ h₂
  simp_rw [PullbackCone.mk_pt, PullbackCone.mk_π_app] at h₁ h₂
apply Cover.hom_ext 𝒰.pullback₁ s.fst
  intro i
  rw [gluedLift]; rw [(Cover.ι_glueMorphisms <| 𝒰.

Depends on / 依赖: Cover.hom_ext, PreZeroHypercover, PreZeroHypercover.pullback, Precoverage, Precoverage.ZeroHypercover.pullback, PullbackCone, PullbackCone.isLimitAux, PullbackCone.mk_, PullbackCone.mk_pt, ZeroHypercover, cancel_epi, gluedLift, gluedLift_p1, gluedLift_p2, hom_ext, isLimitAux, mk_pt, pullbackRightPul, s.fst, simp_rw
-/
def gluedIsLimit : IsLimit (PullbackCone.mk _ _ (p_comm 𝒰 f g)) := by
  apply PullbackCone.isLimitAux'
  intro s
  refine ⟨gluedLift 𝒰 f g s, gluedLift_p1 𝒰 f g s, gluedLift_p2 𝒰 f g s, ?_⟩
  intro m h₁ h₂
  simp_rw [PullbackCone.mk_pt, PullbackCone.mk_π_app] at h₁ h₂
apply Cover.hom_ext 𝒰.pullback₁ s.fst
  intro i
  rw [gluedLift]; rw [(Cover.ι_glueMorphisms <| 𝒰.pullback₁ s.fst)]
  dsimp only [Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover,
    PreZeroHypercover.pullback₁_X, PullbackCone.mk_pt, PreZeroHypercover.pullback₁_f, gluing_ι]
  rw [← cancel_epi
    (pullbackRightPullbackFstIso (p1 𝒰 f g) (𝒰.f i) m ≪≫ pullback.congrHom h₁ rfl).hom]; rw [Iso.trans_hom]; rw [Category.assoc]; rw [pullback.congrHom_hom]; rw [pullback.lift_fst_assoc]; rw [Category.comp_id]; rw [pullbackRightPullbackFstIso_hom_fst_assoc]; rw [pullback.condition]
  conv_lhs => rhs; rw [← pullbackP1Iso_hom_ι]
  simp_rw [← Category.assoc]
  congr 1
  apply pullback.hom_ext
  · simp_rw [Category.assoc, pullbackP1Iso_hom_fst, pullback.lift_fst, Category.comp_id,
      pullbackSymmetry_hom_comp_fst, pullback.lift_snd, Category.comp_id,
      pullbackRightPullbackFstIso_hom_snd]
  · simp_rw [Category.assoc, pullbackP1Iso_hom_snd, pullback.lift_snd,
      pullbackSymmetry_hom_comp_snd_assoc, pullback.lift_fst_assoc, Category.comp_id,
      pullbackRightPullbackFstIso_hom_fst_assoc, ← pullback.condition_assoc, h₂]

include 𝒰 in
/--
theorem `hasPullback_of_cover` / 定理 `hasPullback_of_cover`

English:
theorem hasPullback_of_cover
  statement: HasPullback f g
  proof: ⟨⟨⟨_, gluedIsLimit 𝒰 f g⟩⟩⟩

中文:
定理 hasPullback_of_cover
  结论: HasPullback f g
  证明: ⟨⟨⟨_, gluedIsLimit 𝒰 f g⟩⟩⟩

Depends on / 依赖: gluedIsLimit
-/
theorem hasPullback_of_cover : HasPullback f g :=
  ⟨⟨⟨_, gluedIsLimit 𝒰 f g⟩⟩⟩

/--
Instance `affine_hasPullback` / 实例 `affine_hasPullback`

English:
instance affine_hasPullback
  signature: {A B C : CommRingCat}
  body: by
  rw [← Scheme.Spec.map_preimage f]; rw [← Scheme.Spec.map_preimage g]
  exact ⟨⟨⟨_, isLimitOfHasPullbackOfPreservesLimit
    Scheme.Spec (Scheme.Spec.preimage f) (Scheme.Spec.preimage g)⟩⟩⟩

中文:
实例 affine_hasPullback
  签名: {A B C : 交换环范畴}
  定义体: by
  rw [← Scheme.Spec.map_preimage f]; rw [← Scheme.Spec.map_preimage g]
  exact ⟨⟨⟨_, isLimitOfHasPullbackOfPreservesLimit
    Scheme.Spec (Scheme.Spec.preimage f) (Scheme.Spec.preimage g)⟩⟩⟩

Depends on / 依赖: Scheme, Scheme.Spec, Scheme.Spec.map_preimage, Scheme.Spec.preimage, isLimitOfHasPullbackOfPreservesLimit, map_preimage, preimage
-/
instance affine_hasPullback {A B C : CommRingCat}
    (f : Spec A ⟶ Spec C)
    (g : Spec B ⟶ Spec C) : HasPullback f g := by
  rw [← Scheme.Spec.map_preimage f]; rw [← Scheme.Spec.map_preimage g]
  exact ⟨⟨⟨_, isLimitOfHasPullbackOfPreservesLimit
    Scheme.Spec (Scheme.Spec.preimage f) (Scheme.Spec.preimage g)⟩⟩⟩

set_option backward.isDefEq.respectTransparency false in
/--
theorem `affine_affine_hasPullback` / 定理 `affine_affine_hasPullback`

English:
theorem affine_affine_hasPullback
  statement: {B C : CommRingCat} {X : Scheme}
  proof: hasPullback_of_cover X.affineCover f g

中文:
定理 affine_affine_hasPullback
  结论: {B C : 交换环范畴} {X : 概形}
  证明: hasPullback_of_cover X.affineCover f g

Depends on / 依赖: X.affineCover, affineCover, hasPullback_of_cover
-/
theorem affine_affine_hasPullback {B C : CommRingCat} {X : Scheme}
    (f : X ⟶ Spec C) (g : Spec B ⟶ Spec C) :
    HasPullback f g :=
  hasPullback_of_cover X.affineCover f g

/--
Instance `base_affine_hasPullback` / 实例 `base_affine_hasPullback`

English:
instance base_affine_hasPullback
  signature: {C : CommRingCat} {X Y : Scheme} (f : X ⟶ Spec C)
  body: @hasPullback_symmetry _ _ _ _ _ _ _
    (@hasPullback_of_cover _ _ _ Y.affineCover g f fun _ =>
@hasPullback_symmetry _ _ _ _ _ _ _ affine_affine_hasPullback _ _)

中文:
实例 base_affine_hasPullback
  签名: {C : 交换环范畴} {X Y : 概形} (f : X ⟶ Spec C)
  定义体: @hasPullback_symmetry _ _ _ _ _ _ _
    (@hasPullback_of_cover _ _ _ Y.affineCover g f fun _ =>
@hasPullback_symmetry _ _ _ _ _ _ _ affine_affine_hasPullback _ _)

Depends on / 依赖: Y.affineCover, affineCover, affine_affine_hasPullback, hasPullback_of_cover, hasPullback_symmetry
-/
instance base_affine_hasPullback {C : CommRingCat} {X Y : Scheme} (f : X ⟶ Spec C)
    (g : Y ⟶ Spec C) : HasPullback f g :=
  @hasPullback_symmetry _ _ _ _ _ _ _
    (@hasPullback_of_cover _ _ _ Y.affineCover g f fun _ =>
@hasPullback_symmetry _ _ _ _ _ _ _ affine_affine_hasPullback _ _)

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
Instance `left_affine_comp_pullback_hasPullback` / 实例 `left_affine_comp_pullback_hasPullback`

English:
instance left_affine_comp_pullback_hasPullback
  signature: {X Y Z : Scheme} (f : X ⟶ Z) (g : Y ⟶ Z)
  body: by
  simpa [pullback.condition] using
    hasPullback_assoc_symm f (Z.affineCover.f i) (Z.affineCover.f i) g

中文:
实例 left_affine_comp_pullback_hasPullback
  签名: {X Y Z : 概形} (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: by
  simpa [pullback.condition] using
    hasPullback_assoc_symm f (Z.affineCover.f i) (Z.affineCover.f i) g

Depends on / 依赖: Z.affineCover.f, affineCover, condition, hasPullback_assoc_symm, pullback, pullback.condition
-/
instance left_affine_comp_pullback_hasPullback {X Y Z : Scheme} (f : X ⟶ Z) (g : Y ⟶ Z)
    (i : Z.affineCover.I₀) : HasPullback ((Z.affineCover.pullback₁ f).f i ≫ f) g := by
  simpa [pullback.condition] using
    hasPullback_assoc_symm f (Z.affineCover.f i) (Z.affineCover.f i) g

set_option backward.isDefEq.respectTransparency.types false in
instance {X Y Z : Scheme} (f : X ⟶ Z) (g : Y ⟶ Z) : HasPullback f g :=
  hasPullback_of_cover (Z.affineCover.pullback₁ f) f g

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: HasPullbacks Scheme
  body: hasPullbacks_of_hasLimit_cospan _

中文:
实例 :
  签名: 有Pullbacks 概形
  定义体: hasPullbacks_of_hasLimit_cospan _

Depends on / 依赖: hasPullbacks_of_hasLimit_cospan
-/
instance : HasPullbacks Scheme :=
  hasPullbacks_of_hasLimit_cospan _

set_option backward.isDefEq.respectTransparency false in
/--
Instance `isAffine_of_isAffine_isAffine_isAffine` / 实例 `isAffine_of_isAffine_isAffine_isAffine`

English:
instance isAffine_of_isAffine_isAffine_isAffine
  signature: {X Y Z : Scheme}
  body: .of_isIso
    (pullback.map f g (Spec.map (Γ.map f.op)) (Spec.map (Γ.map g.op))
        X.toSpecΓ Y.toSpecΓ Z.toSpecΓ
        (Scheme.toSpecΓ_naturality f) (Scheme.toSpecΓ_naturality g) ≫
      (PreservesPullback.iso Scheme.Spec _ _).inv)

中文:
实例 isAffine_of_isAffine_isAffine_isAffine
  签名: {X Y Z : 概形}
  定义体: .of_isIso
    (pullback.map f g (Spec.map (Γ.map f.op)) (Spec.map (Γ.map g.op))
        X.toSpecΓ Y.toSpecΓ Z.toSpecΓ
        (Scheme.toSpecΓ_naturality f) (Scheme.toSpecΓ_naturality g) ≫
      (PreservesPullback.iso Scheme.Spec _ _).inv)

Depends on / 依赖: PreservesPullback, PreservesPullback.iso, Scheme, Scheme.Spec, Scheme.toSpec, Spec.map, X.toSpec, Y.toSpec, Z.toSpec, f.op, g.op, of_isIso, pullback, pullback.map
-/
instance isAffine_of_isAffine_isAffine_isAffine {X Y Z : Scheme}
    (f : X ⟶ Z) (g : Y ⟶ Z) [IsAffine X] [IsAffine Y] [IsAffine Z] :
    IsAffine (pullback f g) :=
  .of_isIso
    (pullback.map f g (Spec.map (Γ.map f.op)) (Spec.map (Γ.map g.op))
        X.toSpecΓ Y.toSpecΓ Z.toSpecΓ
        (Scheme.toSpecΓ_naturality f) (Scheme.toSpecΓ_naturality g) ≫
      (PreservesPullback.iso Scheme.Spec _ _).inv)

-- The converse is also true. See `Scheme.isEmpty_pullback_iff`.
/--
theorem `_root_.AlgebraicGeometry.Scheme.isEmpty_pullback` / 定理 `_root_.AlgebraicGeometry.Scheme.isEmpty_pullback`

English:
theorem _root_.AlgebraicGeometry.Scheme.isEmpty_pullback
  proof: isEmpty_of_commSq (IsPullback.of_hasPullback f g).toCommSq H

中文:
定理 _root_.AlgebraicGeometry.概形.isEmpty_pullback
  证明: isEmpty_of_commSq (IsPullback.of_hasPullback f g).toCommSq H

Depends on / 依赖: IsPullback, IsPullback.of_hasPullback, isEmpty_of_commSq, of_hasPullback, toCommSq
-/
theorem _root_.AlgebraicGeometry.Scheme.isEmpty_pullback
    {X Y S : Scheme.{u}} (f : X ⟶ S) (g : Y ⟶ S)
    (H : Disjoint (Set.range f) (Set.range g)) : IsEmpty ↑(Limits.pullback f g) :=
  isEmpty_of_commSq (IsPullback.of_hasPullback f g).toCommSq H

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- Given an open cover `{ Xᵢ }` of `X`, then `X ×[Z] Y` is covered by `Xᵢ ×[Z] Y`. -/
@[simps! I₀ X f]
/--
Definition of `openCoverOfLeft` / `openCoverOfLeft` 的定义

English:
definition openCoverOfLeft
  signature: (𝒰 : OpenCover.{v} X) (f : X ⟶ Z) (g : Y ⟶ Z)
  body: 𝒰.I₀
  X i := pullback (𝒰.f i ≫ f) g
  f i := pullback.map (𝒰.f i ≫ f) g f g (𝒰.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp)
  mem₀ := by
    rw [ofArrows_mem_precoverage_iff]
    refine ⟨fun x => ?_, fun i => ?_⟩
    · let 𝒱 := ((gluing 𝒰.ulift f g).openCover.pushforwardIso
              (limit.isoLimitCon

中文:
定义 openCoverOfLeft
  签名: (𝒰 : OpenCover.{v} X) (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: 𝒰.I₀
  X i := pullback (𝒰.f i ≫ f) g
  f i := pullback.map (𝒰.f i ≫ f) g f g (𝒰.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp)
  mem₀ := by
    rw [ofArrows_mem_precoverage_iff]
    refine ⟨fun x => ?_, fun i => ?_⟩
    · let 𝒱 := ((gluing 𝒰.ulift f g).openCover.pushforwardIso
              (limit.isoLimitCon
-/
def openCoverOfLeft (𝒰 : OpenCover.{v} X) (f : X ⟶ Z) (g : Y ⟶ Z) :
    OpenCover (pullback f g) where
  I₀ := 𝒰.I₀
  X i := pullback (𝒰.f i ≫ f) g
  f i := pullback.map (𝒰.f i ≫ f) g f g (𝒰.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp)
  mem₀ := by
    rw [ofArrows_mem_precoverage_iff]
    refine ⟨fun x => ?_, fun i => ?_⟩
    · let 𝒱 := ((gluing 𝒰.ulift f g).openCover.pushforwardIso
              (limit.isoLimitCone ⟨_, gluedIsLimit 𝒰.ulift f g⟩).inv).copy
          𝒰.ulift.I₀ (fun i => pullback (𝒰.ulift.f i ≫ f) g)
          (fun i => pullback.map _ _ _ _ (𝒰.ulift.f i) (𝟙 _) (𝟙 _) (Category.comp_id _) (by simp))
          (Equiv.refl 𝒰.ulift.I₀) (fun _ => Iso.refl _) fun i => by
        simp_rw [Cover.pushforwardIso_I₀, Cover.pushforwardIso_f, GlueData.openCover_f,
          GlueData.openCover_I₀, gluing_J]
        exact pullback.hom_ext (by simp [p1]) (by simp [p2])
      obtain ⟨i, x, rfl⟩ := 𝒱.exists_eq x
      exact ⟨_, x, rfl⟩
    · dsimp
      have : pullback.map (𝒰.f i ≫ f) g f g (𝒰.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp) =
        (pullbackSymmetry _ _).hom ≫ (pullbackLeftPullbackSndIso _ _ _).inv ≫
          pullback.fst _ _ ≫ (pullbackSymmetry _ _).hom := by aesop
      rw [this]
      infer_instance

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- Given an open cover `{ Yᵢ }` of `Y`, then `X ×[Z] Y` is covered by `X ×[Z] Yᵢ`. -/
@[simps! I₀ X f]
/--
Definition of `openCoverOfRight` / `openCoverOfRight` 的定义

English:
definition openCoverOfRight
  signature: (𝒰 : OpenCover.{v} Y) (f : X ⟶ Z) (g : Y ⟶ Z)
  body: by
  fapply
    ((openCoverOfLeft 𝒰 g f).pushforwardIso (pullbackSymmetry _ _).hom).copy 𝒰.I₀
      (fun i => pullback f (𝒰.f i ≫ g))
      (fun i => pullback.map _ _ _ _ (𝟙 _) (𝒰.f i) (𝟙 _) (by simp) (Category.comp_id _))
      (Equiv.refl _) fun i => pullbackSymmetry _ _
  intro i
  dsimp
  apply 

中文:
定义 openCoverOfRight
  签名: (𝒰 : OpenCover.{v} Y) (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: by
  fapply
    ((openCoverOfLeft 𝒰 g f).pushforwardIso (pullbackSymmetry _ _).hom).copy 𝒰.I₀
      (fun i => pullback f (𝒰.f i ≫ g))
      (fun i => pullback.map _ _ _ _ (𝟙 _) (𝒰.f i) (𝟙 _) (by simp) (Category.comp_id _))
      (Equiv.refl _) fun i => pullbackSymmetry _ _
  intro i
  dsimp
  apply 

Depends on / 依赖: Category, Category.comp_id, Equiv.refl, comp_id, fapply, hom_ext, openCoverOfLeft, pullback, pullback.hom_ext, pullback.map, pullbackSymmetry, pushforwardIso
-/
def openCoverOfRight (𝒰 : OpenCover.{v} Y) (f : X ⟶ Z) (g : Y ⟶ Z) :
    OpenCover.{v} (pullback f g) := by
  fapply
    ((openCoverOfLeft 𝒰 g f).pushforwardIso (pullbackSymmetry _ _).hom).copy 𝒰.I₀
      (fun i => pullback f (𝒰.f i ≫ g))
      (fun i => pullback.map _ _ _ _ (𝟙 _) (𝒰.f i) (𝟙 _) (by simp) (Category.comp_id _))
      (Equiv.refl _) fun i => pullbackSymmetry _ _
  intro i
  dsimp
  apply pullback.hom_ext <;> simp

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- Given an open cover `{ Xᵢ }` of `X` and an open cover `{ Yⱼ }` of `Y`, then
`X ×[Z] Y` is covered by `Xᵢ ×[Z] Yⱼ`. -/
@[simps! I₀ X f]
/--
Definition of `openCoverOfLeftRight` / `openCoverOfLeftRight` 的定义

English:
definition openCoverOfLeftRight
  signature: (𝒰X : OpenCover.{v} X) (𝒰Y : OpenCover.{w} Y) (f : X ⟶ Z) (g : Y ⟶ Z)
  body: by
  fapply
    Cover.copy ((openCoverOfLeft 𝒰X f g).bind fun x => openCoverOfRight 𝒰Y (𝒰X.f x ≫ f) g)
      (𝒰X.I₀ × 𝒰Y.I₀) (fun ij => pullback (𝒰X.f ij.1 ≫ f) (𝒰Y.f ij.2 ≫ g))
      (fun ij =>
        pullback.map _ _ _ _ (𝒰X.f ij.1) (𝒰Y.f ij.2) (𝟙 _) (Category.comp_id _)
          (Category.comp_

中文:
定义 openCoverOfLeftRight
  签名: (𝒰X : OpenCover.{v} X) (𝒰Y : OpenCover.{w} Y) (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: by
  fapply
    Cover.copy ((openCoverOfLeft 𝒰X f g).bind fun x => openCoverOfRight 𝒰Y (𝒰X.f x ≫ f) g)
      (𝒰X.I₀ × 𝒰Y.I₀) (fun ij => pullback (𝒰X.f ij.1 ≫ f) (𝒰Y.f ij.2 ≫ g))
      (fun ij =>
        pullback.map _ _ _ _ (𝒰X.f ij.1) (𝒰Y.f ij.2) (𝟙 _) (Category.comp_id _)
          (Category.comp_

Depends on / 依赖: Category, Category.comp_id, Cover.copy, Equiv.sigmaEquivProd, Iso.refl, comp_id, fapply, hom_ext, openCoverOfLeft, openCoverOfRight, pullback, pullback.hom_ext, pullback.map, sigmaEquivProd
-/
def openCoverOfLeftRight (𝒰X : OpenCover.{v} X) (𝒰Y : OpenCover.{w} Y) (f : X ⟶ Z) (g : Y ⟶ Z) :
    OpenCover.{max v w} (pullback f g) := by
  fapply
    Cover.copy ((openCoverOfLeft 𝒰X f g).bind fun x => openCoverOfRight 𝒰Y (𝒰X.f x ≫ f) g)
      (𝒰X.I₀ × 𝒰Y.I₀) (fun ij => pullback (𝒰X.f ij.1 ≫ f) (𝒰Y.f ij.2 ≫ g))
      (fun ij =>
        pullback.map _ _ _ _ (𝒰X.f ij.1) (𝒰Y.f ij.2) (𝟙 _) (Category.comp_id _)
          (Category.comp_id _))
      (Equiv.sigmaEquivProd _ _).symm fun _ => Iso.refl _
  rintro ⟨i, j⟩
  apply pullback.hom_ext <;> simp

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- (Implementation). Use `openCoverOfBase` instead. -/
@[simps! f]
/--
Definition of `openCoverOfBase'` / `openCoverOfBase'` 的定义

English:
definition openCoverOfBase'
  signature: (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z)
  body: by
  apply (openCoverOfLeft (𝒰.pullback₁ f) f g).bind
  intro i
  haveI := ((IsPullback.of_hasPullback (pullback.snd g (𝒰.f i))
    (pullback.snd f (𝒰.f i))).paste_horiz (IsPullback.of_hasPullback _ _)).flip
  refine
    @coverOfIsIso _ _ _ _ _
      (f := (pullbackSymmetry (pullback.snd f (𝒰.f i)) 

中文:
定义 openCoverOfBase'
  签名: (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: by
  apply (openCoverOfLeft (𝒰.pullback₁ f) f g).bind
  intro i
  haveI := ((IsPullback.of_hasPullback (pullback.snd g (𝒰.f i))
    (pullback.snd f (𝒰.f i))).paste_horiz (IsPullback.of_hasPullback _ _)).flip
  refine
    @coverOfIsIso _ _ _ _ _
      (f := (pullbackSymmetry (pullback.snd f (𝒰.f i)) 

Depends on / 依赖: Category, Category.comp_id, Category.id_comp, IsPullback, IsPullback.of_hasPullback, comp_id, condition, coverOfIsIso, id_comp, isLimit, isoLimitCone, limit.isoLimitCone, of_hasPullback, openCoverOfLeft, paste_horiz, pullback, pullback.condition, pullback.map, pullback.snd, pullbackSymmetry
-/
def openCoverOfBase' (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z) :
    OpenCover.{v} (pullback f g) := by
  apply (openCoverOfLeft (𝒰.pullback₁ f) f g).bind
  intro i
  haveI := ((IsPullback.of_hasPullback (pullback.snd g (𝒰.f i))
    (pullback.snd f (𝒰.f i))).paste_horiz (IsPullback.of_hasPullback _ _)).flip
  refine
    @coverOfIsIso _ _ _ _ _
      (f := (pullbackSymmetry (pullback.snd f (𝒰.f i)) (pullback.snd g (𝒰.f i))).hom ≫
        (limit.isoLimitCone ⟨_, this.isLimit⟩).inv ≫
        pullback.map _ _ _ _ (𝟙 _) (𝟙 _) (𝟙 _) ?_ ?_) inferInstance
  · simp [← pullback.condition]
  · simp only [Category.comp_id, Category.id_comp]

set_option backward.isDefEq.respectTransparency false in
/-- Given an open cover `{ Zᵢ }` of `Z`, then `X ×[Z] Y` is covered by `Xᵢ ×[Zᵢ] Yᵢ`, where
  `Xᵢ = X ×[Z] Zᵢ` and `Yᵢ = Y ×[Z] Zᵢ` is the preimage of `Zᵢ` in `X` and `Y`. -/
@[simps! I₀ X f]
/--
Definition of `openCoverOfBase` / `openCoverOfBase` 的定义

English:
definition openCoverOfBase
  signature: (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z)
  body: by
  apply
    (openCoverOfBase' 𝒰 f g).copy 𝒰.I₀
      (fun i =>
        pullback (pullback.snd _ _ : pullback f (𝒰.f i) ⟶ _)
          (pullback.snd _ _ : pullback g (𝒰.f i) ⟶ _))
      (fun i =>
        pullback.map _ _ _ _ (pullback.fst _ _) (pullback.fst _ _) (𝒰.f i)
          pullback.conditio

中文:
定义 openCoverOfBase
  签名: (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z)
  定义体: by
  apply
    (openCoverOfBase' 𝒰 f g).copy 𝒰.I₀
      (fun i =>
        pullback (pullback.snd _ _ : pullback f (𝒰.f i) ⟶ _)
          (pullback.snd _ _ : pullback g (𝒰.f i) ⟶ _))
      (fun i =>
        pullback.map _ _ _ _ (pullback.fst _ _) (pullback.fst _ _) (𝒰.f i)
          pullback.conditio

Depends on / 依赖: Category, Category.id_comp, Equiv.prodPUnit, Equiv.sigmaEquivProd, Iso.refl, Iso.refl_hom, PullbackCone, PullbackCone.m, condition, id_comp, limit.lift_, openCoverOfBase, prodPUnit, pullback, pullback.condition.symm, pullback.fst, pullback.map, pullback.snd, refl_hom, sigmaEquivProd
-/
def openCoverOfBase (𝒰 : OpenCover.{v} Z) (f : X ⟶ Z) (g : Y ⟶ Z) :
    OpenCover.{v} (pullback f g) := by
  apply
    (openCoverOfBase' 𝒰 f g).copy 𝒰.I₀
      (fun i =>
        pullback (pullback.snd _ _ : pullback f (𝒰.f i) ⟶ _)
          (pullback.snd _ _ : pullback g (𝒰.f i) ⟶ _))
      (fun i =>
        pullback.map _ _ _ _ (pullback.fst _ _) (pullback.fst _ _) (𝒰.f i)
          pullback.condition.symm pullback.condition.symm)
      ((Equiv.prodPUnit 𝒰.I₀).symm.trans (Equiv.sigmaEquivProd 𝒰.I₀ PUnit).symm) fun _ => Iso.refl _
  intro i
  rw [Iso.refl_hom]; rw [Category.id_comp]; rw [openCoverOfBase'_f]
  ext : 1 <;>
  · simp only [limit.lift_π, PullbackCone.mk_π_app, Equiv.trans_apply, Category.assoc,
    limit.lift_π_assoc, cospan_left, Category.comp_id,
      limit.isoLimitCone_inv_π_assoc, PullbackCone.π_app_left, IsPullback.cone_fst,
      pullbackSymmetry_hom_comp_snd_assoc, limit.isoLimitCone_inv_π,
      PullbackCone.π_app_right, IsPullback.cone_snd, pullbackSymmetry_hom_comp_fst_assoc]
    rfl

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
-- TODO: generalize to covers in subcanonical topologies
open pullback in
attribute [local simp] condition condition_assoc in
/--
lemma `_root_.AlgebraicGeometry.Scheme.isPullback_of_openCover` / 引理 `_root_.AlgebraicGeometry.Scheme.isPullback_of_openCover`

English:
lemma _root_.AlgebraicGeometry.Scheme.isPullback_of_openCover
  proof: by
  have h : fWX ≫ fXZ = fWY ≫ fYZ :=
    Scheme.Cover.hom_ext (𝒰.pullback₁ fWX) _ _ fun i => by simpa using (H i).w
  suffices IsIso (lift fWX fWY h) from .of_iso_pullback ⟨h⟩ (asIso (lift _ _ h)) (by simp) (by simp)
  have H₁ (i : _) : IsIso ((openCoverOfLeft 𝒰 fXZ fYZ).pullbackHom (lift fWX fWY 

中文:
引理 _root_.AlgebraicGeometry.概形.isPullback_of_openCover
  证明: by
  have h : fWX ≫ fXZ = fWY ≫ fYZ :=
    Scheme.Cover.hom_ext (𝒰.pullback₁ fWX) _ _ fun i => by simpa using (H i).w
  suffices IsIso (lift fWX fWY h) from .of_iso_pullback ⟨h⟩ (asIso (lift _ _ h)) (by simp) (by simp)
  have H₁ (i : _) : IsIso ((openCoverOfLeft 𝒰 fXZ fYZ).pullbackHom (lift fWX fWY 

Depends on / 依赖: IsPullback, IsPullback.paste_vert_iff, Scheme, Scheme.Cover.hom_ext, hom_ext, of_hasPullback, of_iso_pullback, openCoverOfLeft, paste_vert_iff, pullbackHom
-/
lemma _root_.AlgebraicGeometry.Scheme.isPullback_of_openCover
    {W : Scheme.{u}} (fWX : W ⟶ X) (fWY : W ⟶ Y) (fXZ : X ⟶ Z) (fYZ : Y ⟶ Z) (𝒰 : X.OpenCover)
    (H : forall i, IsPullback (𝒰.pullbackHom fWX i) ((𝒰.pullback₁ fWX).f i ≫ fWY) (𝒰.f i ≫ fXZ) fYZ) :
    IsPullback fWX fWY fXZ fYZ := by
  have h : fWX ≫ fXZ = fWY ≫ fYZ :=
    Scheme.Cover.hom_ext (𝒰.pullback₁ fWX) _ _ fun i => by simpa using (H i).w
  suffices IsIso (lift fWX fWY h) from .of_iso_pullback ⟨h⟩ (asIso (lift _ _ h)) (by simp) (by simp)
  have H₁ (i : _) : IsIso ((openCoverOfLeft 𝒰 fXZ fYZ).pullbackHom (lift fWX fWY h) i) := by
    let f := map (𝒰.f i ≫ fXZ) fYZ fXZ fYZ (𝒰.f i) (𝟙 Y) (𝟙 Z) (by simp) (by simp)
    have : IsPullback (fst (𝒰.f i ≫ fXZ) fYZ) f (𝒰.f i) (fst _ _) := by
      simpa [← IsPullback.paste_vert_iff (.of_hasPullback _ _), f] using .of_hasPullback _ _
    have H' : IsPullback (fst fWX (𝒰.f i)) (lift (snd _ _) (fst _ _ ≫ fWY) (by simp [← h]))
        (lift fWX fWY h) f := by
      rw [← IsPullback.paste_vert_iff this.flip (by ext <;> simp [f])]
      simpa using .of_hasPullback _ _
    convert! (inferInstance : IsIso (H'.isoPullback.inv ≫ (H i).isoPullback.hom))
    aesop (add simp [Iso.eq_inv_comp, Scheme.Cover.pullbackHom])
  exact MorphismProperty.of_zeroHypercover_target (P := .isomorphisms Scheme)
    (Scheme.Pullback.openCoverOfLeft 𝒰 fXZ fYZ) H₁

variable (f : X ⟶ Y) (𝒰 : OpenCover.{u} Y) (𝒱 : forall i, OpenCover.{w} ((𝒰.pullback₁ f).X i))

/--
Given `𝒰 i` covering `Y` and `𝒱 i j` covering `𝒰 i`, this is the open cover
`𝒱 i j₁ ×[𝒰 i] 𝒱 i j₂` ranging over all `i`, `j₁`, `j₂`.
-/
noncomputable
/--
Definition of `diagonalCover` / `diagonalCover` 的定义

English:
definition diagonalCover
  signature: : (pullback.diagonalObj f).OpenCover
  body: (openCoverOfBase 𝒰 f f).bind
    fun i => openCoverOfLeftRight (𝒱 i) (𝒱 i) (𝒰.pullbackHom _ _) (𝒰.pullbackHom _ _)

中文:
定义 diagonalCover
  签名: : (pullback.diagonalObj f).OpenCover
  定义体: (openCoverOfBase 𝒰 f f).bind
    fun i => openCoverOfLeftRight (𝒱 i) (𝒱 i) (𝒰.pullbackHom _ _) (𝒰.pullbackHom _ _)

Depends on / 依赖: openCoverOfBase, openCoverOfLeftRight, pullbackHom
-/
def diagonalCover : (pullback.diagonalObj f).OpenCover :=
  (openCoverOfBase 𝒰 f f).bind
    fun i => openCoverOfLeftRight (𝒱 i) (𝒱 i) (𝒰.pullbackHom _ _) (𝒰.pullbackHom _ _)

set_option backward.isDefEq.respectTransparency.types false in
/-- The image of `𝒱 i j₁ ×[𝒰 i] 𝒱 i j₂` in `diagonalCover` with `j₁ = j₂` -/
noncomputable
/--
Definition of `diagonalCoverDiagonalRange` / `diagonalCoverDiagonalRange` 的定义

English:
definition diagonalCoverDiagonalRange
  signature: : (pullback.diagonalObj f).Opens
  body: ⨆ i : Σ i, (𝒱 i).I₀, ((diagonalCover f 𝒰 𝒱).f ⟨i.1, i.2, i.2⟩).opensRange

中文:
定义 diagonalCoverDiagonalRange
  签名: : (pullback.diagonalObj f).Opens
  定义体: ⨆ i : Σ i, (𝒱 i).I₀, ((diagonalCover f 𝒰 𝒱).f ⟨i.1, i.2, i.2⟩).opensRange

Depends on / 依赖: diagonalCover, opensRange
-/
def diagonalCoverDiagonalRange : (pullback.diagonalObj f).Opens :=
  ⨆ i : Σ i, (𝒱 i).I₀, ((diagonalCover f 𝒰 𝒱).f ⟨i.1, i.2, i.2⟩).opensRange

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
lemma `diagonalCover_map` / 引理 `diagonalCover_map`

English:
lemma diagonalCover_map
  given: (I)
  statement: (diagonalCover f 𝒰 𝒱).f I =
  proof: by
  cases I
  ext1 <;> simp [diagonalCover, Cover.pullbackHom]

中文:
引理 diagonalCover_map
  条件: (I)
  结论: (diagonalCover f 𝒰 𝒱).f I =
  证明: by
  cases I
  ext1 <;> simp [diagonalCover, Cover.pullbackHom]

Depends on / 依赖: Cover.pullbackHom, diagonalCover, pullbackHom
-/
lemma diagonalCover_map (I) : (diagonalCover f 𝒰 𝒱).f I =
    pullback.map _ _ _ _
    ((𝒱 I.fst).f _ ≫ pullback.fst _ _) ((𝒱 I.fst).f _ ≫ pullback.fst _ _) (𝒰.f _)
    (by simp)
    (by simp) := by
  cases I
  ext1 <;> simp [diagonalCover, Cover.pullbackHom]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/-- The restriction of the diagonal `X ⟶ X ×ₛ X` to `𝒱 i j ×[𝒰 i] 𝒱 i j` is the diagonal
`𝒱 i j ⟶ 𝒱 i j ×[𝒰 i] 𝒱 i j`. -/
noncomputable
/--
Definition of `diagonalRestrictIsoDiagonal` / `diagonalRestrictIsoDiagonal` 的定义

English:
definition diagonalRestrictIsoDiagonal
  signature: (i j)
  body: by
  refine (morphismRestrictOpensRange _ _).trans ?_
  refine Arrow.isoMk ?_ (Iso.refl _) ?_
  · exact pullback.congrHom rfl (diagonalCover_map _ _ _ _) ≪≫
      pullbackDiagonalMapIso _ _ _ _ ≪≫ (asIso (pullback.diagonal _)).symm
  have H : pullback.snd (pullback.diagonal f) ((diagonalCover f 𝒰 𝒱)

中文:
定义 diagonalRestrictIsoDiagonal
  签名: (i j)
  定义体: by
  refine (morphismRestrictOpensRange _ _).trans ?_
  refine Arrow.isoMk ?_ (Iso.refl _) ?_
  · exact pullback.congrHom rfl (diagonalCover_map _ _ _ _) ≪≫
      pullbackDiagonalMapIso _ _ _ _ ≪≫ (asIso (pullback.diagonal _)).symm
  have H : pullback.snd (pullback.diagonal f) ((diagonalCover f 𝒰 𝒱)

Depends on / 依赖: Arrow.isoMk, Iso.refl, cancel_mono, congrHom, diagonal, diagonalCover, diagonalCover_map, hom_ext, morphismRestrictOpensRange, pullback, pullback.congrHom, pullback.diagonal, pullback.fst, pullback.hom_ext, pullback.snd, pullbackDiagonalMapIso
-/
def diagonalRestrictIsoDiagonal (i j) :
    Arrow.mk (pullback.diagonal f ∣_ ((diagonalCover f 𝒰 𝒱).f ⟨i, j, j⟩).opensRange) ≅
    Arrow.mk (pullback.diagonal ((𝒱 i).f j ≫ pullback.snd _ _)) := by
  refine (morphismRestrictOpensRange _ _).trans ?_
  refine Arrow.isoMk ?_ (Iso.refl _) ?_
  · exact pullback.congrHom rfl (diagonalCover_map _ _ _ _) ≪≫
      pullbackDiagonalMapIso _ _ _ _ ≪≫ (asIso (pullback.diagonal _)).symm
  have H : pullback.snd (pullback.diagonal f) ((diagonalCover f 𝒰 𝒱).f ⟨i, (j, j)⟩) ≫
      pullback.snd _ _ = pullback.snd _ _ ≫ pullback.fst _ _ := by
    rw [← cancel_mono ((𝒱 i).f _)]
    apply pullback.hom_ext
    · trans pullback.snd (pullback.diagonal f) ((diagonalCover f 𝒰 𝒱).f ⟨i, (j, j)⟩) ≫
        (diagonalCover f 𝒰 𝒱).f _ ≫ pullback.snd _ _
      · simp [diagonalCover_map]
      symm
      trans pullback.snd (pullback.diagonal f) ((diagonalCover f 𝒰 𝒱).f ⟨i, (j, j)⟩) ≫
        (diagonalCover f 𝒰 𝒱).f _ ≫ pullback.fst _ _
      · simp [diagonalCover_map]
      · rw [← pullback.condition_assoc, ← pullback.condition_assoc]
        simp
    · simp [pullback.condition, Cover.pullbackHom]
  dsimp [Cover.pullbackHom] at H ⊢
  apply pullback.hom_ext
  · simp only [Category.assoc, pullback.diagonal_fst, Category.comp_id]
    simp only [← Category.assoc, IsIso.comp_inv_eq]
    apply pullback.hom_ext <;> simp [H]
  · simp only [Category.assoc, pullback.diagonal_snd, Category.comp_id]
    simp only [← Category.assoc, IsIso.comp_inv_eq]
    apply pullback.hom_ext <;> simp [H]

end Pullback

end AlgebraicGeometry.Scheme

namespace AlgebraicGeometry

/--
Instance `Scheme.pullback_map_isOpenImmersion` / 实例 `Scheme.pullback_map_isOpenImmersion`

English:
instance Scheme.pullback_map_isOpenImmersion
  signature: {X Y S X' Y' S' : Scheme}
  body: by
  rw [pullback_map_eq_pullbackFstFstIso_inv]
  infer_instance

中文:
实例 概形.pullback_map_isOpenImmersion
  签名: {X Y S X' Y' S' : 概形}
  定义体: by
  rw [pullback_map_eq_pullbackFstFstIso_inv]
  infer_instance

Depends on / 依赖: infer_instance, pullback_map_eq_pullbackFstFstIso_inv
-/
instance Scheme.pullback_map_isOpenImmersion {X Y S X' Y' S' : Scheme}
    (f : X ⟶ S) (g : Y ⟶ S) (f' : X' ⟶ S') (g' : Y' ⟶ S')
    (i₁ : X ⟶ X') (i₂ : Y ⟶ Y') (i₃ : S ⟶ S') (e₁ : f ≫ i₃ = i₁ ≫ f') (e₂ : g ≫ i₃ = i₂ ≫ g')
    [IsOpenImmersion i₁] [IsOpenImmersion i₂] [Mono i₃] :
    IsOpenImmersion (pullback.map f g f' g' i₁ i₂ i₃ e₁ e₂) := by
  rw [pullback_map_eq_pullbackFstFstIso_inv]
  infer_instance

section CartesianMonoidalCategory
variable {S : Scheme}

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: CartesianMonoidalCategory (Over S)
  body: Over.cartesianMonoidalCategory _

中文:
实例 :
  签名: CartesianMonoidal范畴 (Over S)
  定义体: Over.cartesianMonoidalCategory _

Depends on / 依赖: Over.cartesianMonoidalCategory, cartesianMonoidalCategory
-/
instance : CartesianMonoidalCategory (Over S) := Over.cartesianMonoidalCategory _
/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: BraidedCategory (Over S)
  body: .ofCartesianMonoidalCategory

中文:
实例 :
  签名: 辫范畴 (Over S)
  定义体: .ofCartesianMonoidalCategory

Depends on / 依赖: ofCartesianMonoidalCategory
-/
instance : BraidedCategory (Over S) := .ofCartesianMonoidalCategory

end CartesianMonoidalCategory

section Spec

variable (R S T : Type u) [CommRing R] [CommRing S] [CommRing T] [Algebra R S] [Algebra R T]

open TensorProduct Algebra.TensorProduct CommRingCat RingHomClass

/-- The isomorphism between the fibred product of two schemes `Spec S` and `Spec T`
over a scheme `Spec R` and the `Spec` of the tensor product `S ⊗[R] T`. -/
noncomputable
/--
Definition of `pullbackSpecIso` / `pullbackSpecIso` 的定义

English:
definition pullbackSpecIso
  signature: :
  body: letI H := IsLimit.equivIsoLimit (PullbackCone.eta _)
    (PushoutCocone.isColimitEquivIsLimitOp _ (CommRingCat.pushoutCoconeIsColimit R S T))
  limit.isoLimitCone ⟨_, isLimitPullbackConeMapOfIsLimit Scheme.Spec _ H⟩

中文:
定义 pullbackSpecIso
  签名: :
  定义体: letI H := IsLimit.equivIsoLimit (PullbackCone.eta _)
    (PushoutCocone.isColimitEquivIsLimitOp _ (CommRingCat.pushoutCoconeIsColimit R S T))
  limit.isoLimitCone ⟨_, isLimitPullbackConeMapOfIsLimit Scheme.Spec _ H⟩

Depends on / 依赖: CommRingCat, CommRingCat.pushoutCoconeIsColimit, IsLimit, IsLimit.equivIsoLimit, PullbackCone, PullbackCone.eta, PushoutCocone, PushoutCocone.isColimitEquivIsLimitOp, Scheme, Scheme.Spec, equivIsoLimit, isColimitEquivIsLimitOp, isLimitPullbackConeMapOfIsLimit, isoLimitCone, limit.isoLimitCone, pushoutCoconeIsColimit
-/
def pullbackSpecIso :
    pullback (Spec.map (CommRingCat.ofHom (algebraMap R S)))
      (Spec.map (CommRingCat.ofHom (algebraMap R T))) ≅ Spec (.of <| S otimes[R] T) :=
  letI H := IsLimit.equivIsoLimit (PullbackCone.eta _)
    (PushoutCocone.isColimitEquivIsLimitOp _ (CommRingCat.pushoutCoconeIsColimit R S T))
  limit.isoLimitCone ⟨_, isLimitPullbackConeMapOfIsLimit Scheme.Spec _ H⟩

/--
The composition of the inverse of the isomorphism `pullbackSpecIso R S T` (from the pullback of
`Spec S ⟶ Spec R` and `Spec T ⟶ Spec R` to `Spec (S ⊗[R] T)`) with the first projection is
the morphism `Spec (S ⊗[R] T) ⟶ Spec S` obtained by applying `Spec.map` to the ring morphism
`s ↦ s ⊗ₜ[R] 1`.
-/
@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_inv_fst` / 引理 `pullbackSpecIso_inv_fst`

English:
lemma pullbackSpecIso_inv_fst
  proof: limit.isoLimitCone_inv_π _ _

@[reassoc]

中文:
引理 pullbackSpecIso_inv_fst
  证明: limit.isoLimitCone_inv_π _ _

@[reassoc]

Depends on / 依赖: limit.isoLimitCone_inv_
-/
lemma pullbackSpecIso_inv_fst :
    (pullbackSpecIso R S T).inv ≫ pullback.fst _ _ = Spec.map (ofHom includeLeftRingHom) :=
  limit.isoLimitCone_inv_π _ _

@[reassoc]
/--
lemma `pullbackSpecIso_inv_fst'` / 引理 `pullbackSpecIso_inv_fst'`

English:
lemma pullbackSpecIso_inv_fst'
  proof: pullbackSpecIso_inv_fst ..

中文:
引理 pullbackSpecIso_inv_fst'
  证明: pullbackSpecIso_inv_fst ..

Depends on / 依赖: pullbackSpecIso_inv_fst
-/
lemma pullbackSpecIso_inv_fst' :
    (pullbackSpecIso R S T).inv ≫ pullback.fst _ _ = Spec.map (ofHom (algebraMap S _)) :=
  pullbackSpecIso_inv_fst ..

/--
The composition of the inverse of the isomorphism `pullbackSpecIso R S T` (from the pullback of
`Spec S ⟶ Spec R` and `Spec T ⟶ Spec R` to `Spec (S ⊗[R] T)`) with the second projection is
the morphism `Spec (S ⊗[R] T) ⟶ Spec T` obtained by applying `Spec.map` to the ring morphism
`t ↦ 1 ⊗ₜ[R] t`.
-/
@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_inv_snd` / 引理 `pullbackSpecIso_inv_snd`

English:
lemma pullbackSpecIso_inv_snd
  proof: limit.isoLimitCone_inv_π _ _

中文:
引理 pullbackSpecIso_inv_snd
  证明: limit.isoLimitCone_inv_π _ _

Depends on / 依赖: includeRight, otimes, toRingHom
-/
lemma pullbackSpecIso_inv_snd :
    (pullbackSpecIso R S T).inv ≫ pullback.snd _ _ =
      Spec.map (ofHom (R := T) (S := S otimes[R] T) (toRingHom includeRight)) :=
  limit.isoLimitCone_inv_π _ _

/--
The composition of the isomorphism `pullbackSpecIso R S T` (from the pullback of
`Spec S ⟶ Spec R` and `Spec T ⟶ Spec R` to `Spec (S ⊗[R] T)`) with the morphism
`Spec (S ⊗[R] T) ⟶ Spec S` obtained by applying `Spec.map` to the ring morphism `s ↦ s ⊗ₜ[R] 1`
is the first projection.
-/
@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_hom_fst` / 引理 `pullbackSpecIso_hom_fst`

English:
lemma pullbackSpecIso_hom_fst
  proof: by
  rw [← pullbackSpecIso_inv_fst]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]

中文:
引理 pullbackSpecIso_hom_fst
  证明: by
  rw [← pullbackSpecIso_inv_fst]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]

Depends on / 依赖: Iso.hom_inv_id_assoc, hom_inv_id_assoc, pullbackSpecIso_inv_fst
-/
lemma pullbackSpecIso_hom_fst :
    (pullbackSpecIso R S T).hom ≫ Spec.map (ofHom includeLeftRingHom) = pullback.fst _ _ := by
  rw [← pullbackSpecIso_inv_fst]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_hom_fst'` / 引理 `pullbackSpecIso_hom_fst'`

English:
lemma pullbackSpecIso_hom_fst'
  proof: pullbackSpecIso_hom_fst ..

中文:
引理 pullbackSpecIso_hom_fst'
  证明: pullbackSpecIso_hom_fst ..

Depends on / 依赖: pullbackSpecIso_hom_fst
-/
lemma pullbackSpecIso_hom_fst' :
    (pullbackSpecIso R S T).hom ≫ Spec.map (ofHom (algebraMap S _)) = pullback.fst _ _ :=
  pullbackSpecIso_hom_fst ..

/--
The composition of the isomorphism `pullbackSpecIso R S T` (from the pullback of
`Spec S ⟶ Spec R` and `Spec T ⟶ Spec R` to `Spec (S ⊗[R] T)`) with the morphism
`Spec (S ⊗[R] T) ⟶ Spec T` obtained by applying `Spec.map` to the ring morphism `t ↦ 1 ⊗ₜ[R] t`
is the second projection.
-/
@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_hom_snd` / 引理 `pullbackSpecIso_hom_snd`

English:
lemma pullbackSpecIso_hom_snd
  proof: by
  rw [← pullbackSpecIso_inv_snd]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]

中文:
引理 pullbackSpecIso_hom_snd
  证明: by
  rw [← pullbackSpecIso_inv_snd]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]

Depends on / 依赖: Iso.hom_inv_id_assoc, hom_inv_id_assoc, pullbackSpecIso_inv_snd
-/
lemma pullbackSpecIso_hom_snd :
    (pullbackSpecIso R S T).hom ≫ Spec.map (ofHom (toRingHom includeRight)) = pullback.snd _ _ := by
  rw [← pullbackSpecIso_inv_snd]; rw [Iso.hom_inv_id_assoc]

@[reassoc (attr := simp)]
/--
lemma `pullbackSpecIso_hom_base` / 引理 `pullbackSpecIso_hom_base`

English:
lemma pullbackSpecIso_hom_base
  proof: by
  simp [Algebra.TensorProduct.algebraMap_def]

中文:
引理 pullbackSpecIso_hom_base
  证明: by
  simp [Algebra.TensorProduct.algebraMap_def]

Depends on / 依赖: Algebra, Algebra.TensorProduct.algebraMap_def, TensorProduct, algebraMap_def
-/
lemma pullbackSpecIso_hom_base :
    (pullbackSpecIso R S T).hom ≫ Spec.map (ofHom (algebraMap R _)) =
      pullback.fst _ _ ≫ Spec.map (ofHom (algebraMap _ _)) := by
  simp [Algebra.TensorProduct.algebraMap_def]

/--
lemma `isPullback_SpecMap_of_isPushout` / 引理 `isPullback_SpecMap_of_isPushout`

English:
lemma isPullback_SpecMap_of_isPushout
  statement: {A B C P : CommRingCat} (f : A ⟶ B) (g : A ⟶ C)
  proof: IsPullback.map Scheme.Spec h.op.flip

中文:
引理 isPullback_SpecMap_of_isPushout
  结论: {A B C P : 交换环范畴} (f : A ⟶ B) (g : A ⟶ C)
  证明: IsPullback.map Scheme.Spec h.op.flip

Depends on / 依赖: IsPullback, IsPullback.map, Scheme, Scheme.Spec, h.op.flip
-/
lemma isPullback_SpecMap_of_isPushout {A B C P : CommRingCat} (f : A ⟶ B) (g : A ⟶ C)
    (inl : B ⟶ P) (inr : C ⟶ P) (h : IsPushout f g inl inr) :
    IsPullback (Spec.map inl) (Spec.map inr) (Spec.map f) (Spec.map g) :=
  IsPullback.map Scheme.Spec h.op.flip

/--
lemma `isPullback_SpecMap_pushout` / 引理 `isPullback_SpecMap_pushout`

English:
lemma isPullback_SpecMap_pushout
  given: {A B C : CommRingCat} (f : A ⟶ B) (g : A ⟶ C)
  proof: by
  apply isPullback_SpecMap_of_isPushout
  exact IsPushout.of_hasPushout f g

中文:
引理 isPullback_SpecMap_pushout
  条件: {A B C : 交换环范畴} (f : A ⟶ B) (g : A ⟶ C)
  证明: by
  apply isPullback_SpecMap_of_isPushout
  exact IsPushout.of_hasPushout f g

Depends on / 依赖: IsPushout, IsPushout.of_hasPushout, isPullback_SpecMap_of_isPushout, of_hasPushout
-/
lemma isPullback_SpecMap_pushout {A B C : CommRingCat} (f : A ⟶ B) (g : A ⟶ C) :
    IsPullback (Spec.map (pushout.inl f g))
      (Spec.map (pushout.inr f g)) (Spec.map f) (Spec.map g) := by
  apply isPullback_SpecMap_of_isPushout
  exact IsPushout.of_hasPushout f g

/--
lemma `diagonal_SpecMap` / 引理 `diagonal_SpecMap`

English:
lemma diagonal_SpecMap
  proof: by
  ext1 <;> simp only [pullback.diagonal_fst, pullback.diagonal_snd, ← Spec.map_comp, ← Spec.map_id,
    AlgHom.toRingHom_eq_coe, Category.assoc, pullbackSpecIso_inv_fst, pullbackSpecIso_inv_snd]
  · congr 1; ext x; change x = Algebra.TensorProduct.lmul' R (S := S) (x otimesₜ[R] 1); simp
  · congr

中文:
引理 diagonal_SpecMap
  证明: by
  ext1 <;> simp only [pullback.diagonal_fst, pullback.diagonal_snd, ← Spec.map_comp, ← Spec.map_id,
    AlgHom.toRingHom_eq_coe, Category.assoc, pullbackSpecIso_inv_fst, pullbackSpecIso_inv_snd]
  · congr 1; ext x; change x = Algebra.TensorProduct.lmul' R (S := S) (x otimesₜ[R] 1); simp
  · congr

Depends on / 依赖: AlgHom, AlgHom.toRingHom_eq_coe, Algebra, Algebra.TensorProduct.lmul, Category, Category.assoc, Spec.map_comp, Spec.map_id, TensorProduct, diagonal_fst, diagonal_snd, map_comp, map_id, pullback, pullback.diagonal_fst, pullback.diagonal_snd, pullbackSpecIso_inv_fst, pullbackSpecIso_inv_snd, toRingHom_eq_coe
-/
lemma diagonal_SpecMap :
    pullback.diagonal (Spec.map (CommRingCat.ofHom (algebraMap R S))) =
      Spec.map (CommRingCat.ofHom (Algebra.TensorProduct.lmul' R : S otimes[R] S ->ₐ[R] S).toRingHom) ≫
        (pullbackSpecIso R S S).inv := by
  ext1 <;> simp only [pullback.diagonal_fst, pullback.diagonal_snd, ← Spec.map_comp, ← Spec.map_id,
    AlgHom.toRingHom_eq_coe, Category.assoc, pullbackSpecIso_inv_fst, pullbackSpecIso_inv_snd]
  · congr 1; ext x; change x = Algebra.TensorProduct.lmul' R (S := S) (x otimesₜ[R] 1); simp
  · congr 1; ext x; change x = Algebra.TensorProduct.lmul' R (S := S) (1 otimesₜ[R] x); simp

end Spec

namespace Scheme
variable {M S T : Scheme.{u}} [M.Over S] {f : T ⟶ S}

@[simps]
/--
Instance `canonicallyOverPullback` / 实例 `canonicallyOverPullback`

English:
instance canonicallyOverPullback
  signature: : (pullback (M ↘ S) f).CanonicallyOver T where
  body: pullback.snd (M ↘ S) f

@[simps! -isSimp mul one]

中文:
实例 canonicallyOverPullback
  签名: : (pullback (M ↘ S) f).CanonicallyOver T where
  定义体: pullback.snd (M ↘ S) f

@[simps! -isSimp mul one]

Depends on / 依赖: pullback, pullback.snd
-/
instance canonicallyOverPullback : (pullback (M ↘ S) f).CanonicallyOver T where
  hom := pullback.snd (M ↘ S) f

@[simps! -isSimp mul one]
/--
Instance `monObjAsOverPullback` / 实例 `monObjAsOverPullback`

English:
instance monObjAsOverPullback
  signature: [MonObj (asOver M S)]
  body: by
  unfold asOver OverClass.asOver at *; exact Over.monObjMkPullbackSnd

中文:
实例 monObjAsOverPullback
  签名: [MonObj (asOver M S)]
  定义体: by
  unfold asOver OverClass.asOver at *; exact Over.monObjMkPullbackSnd

Depends on / 依赖: Over.monObjMkPullbackSnd, OverClass, OverClass.asOver, asOver, monObjMkPullbackSnd
-/
instance monObjAsOverPullback [MonObj (asOver M S)] : MonObj (asOver (pullback (M ↘ S) f) T) := by
  unfold asOver OverClass.asOver at *; exact Over.monObjMkPullbackSnd

/--
Instance `isCommMonObj_asOver_pullback` / 实例 `isCommMonObj_asOver_pullback`

English:
instance isCommMonObj_asOver_pullback
  signature: [MonObj (asOver M S)] [IsCommMonObj (asOver M S)]
  body: by
  unfold asOver OverClass.asOver at *; exact Over.isCommMonObj_mk_pullbackSnd

中文:
实例 isCommMonObj_asOver_pullback
  签名: [MonObj (asOver M S)] [是交换MonObj (asOver M S)]
  定义体: by
  unfold asOver OverClass.asOver at *; exact Over.isCommMonObj_mk_pullbackSnd

Depends on / 依赖: Over.isCommMonObj_mk_pullbackSnd, OverClass, OverClass.asOver, asOver, isCommMonObj_mk_pullbackSnd
-/
instance isCommMonObj_asOver_pullback [MonObj (asOver M S)] [IsCommMonObj (asOver M S)] :
    IsCommMonObj (asOver (pullback (M ↘ S) f) T) := by
  unfold asOver OverClass.asOver at *; exact Over.isCommMonObj_mk_pullbackSnd

/--
Instance `GrpObjAsOverPullback` / 实例 `GrpObjAsOverPullback`

English:
instance GrpObjAsOverPullback
  signature: [GrpObj (asOver M S)]
  body: by
  unfold asOver OverClass.asOver at *; exact Over.grpObjMkPullbackSnd

中文:
实例 GrpObjAsOverPullback
  签名: [GrpObj (asOver M S)]
  定义体: by
  unfold asOver OverClass.asOver at *; exact Over.grpObjMkPullbackSnd

Depends on / 依赖: Over.grpObjMkPullbackSnd, OverClass, OverClass.asOver, asOver, grpObjMkPullbackSnd
-/
instance GrpObjAsOverPullback [GrpObj (asOver M S)] : GrpObj (asOver (pullback (M ↘ S) f) T) := by
  unfold asOver OverClass.asOver at *; exact Over.grpObjMkPullbackSnd

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: (pullback.fst (M ↘ S) (𝟙 S)).IsOver S
  body: ⟨pullback.condition.trans (by simp)⟩

中文:
实例 :
  签名: (pullback.fst (M ↘ S) (𝟙 S)).是Over S
  定义体: ⟨pullback.condition.trans (by simp)⟩

Depends on / 依赖: condition, pullback, pullback.condition.trans
-/
instance : (pullback.fst (M ↘ S) (𝟙 S)).IsOver S := ⟨pullback.condition.trans (by simp)⟩

/--
Instance `isMonHom_fst_id_right` / 实例 `isMonHom_fst_id_right`

English:
instance isMonHom_fst_id_right
  signature: [MonObj (asOver M S)]
  body: by
  unfold asOver OverClass.asOver at *; exact Over.isMonHom_pullbackFst_id_right

中文:
实例 isMonHom_fst_id_right
  签名: [MonObj (asOver M S)]
  定义体: by
  unfold asOver OverClass.asOver at *; exact Over.isMonHom_pullbackFst_id_right

Depends on / 依赖: Over.isMonHom_pullbackFst_id_right, OverClass, OverClass.asOver, asOver, isMonHom_pullbackFst_id_right
-/
instance isMonHom_fst_id_right [MonObj (asOver M S)] :
    IsMonHom ((pullback.fst (M ↘ S) (𝟙 S)).asOver S) := by
  unfold asOver OverClass.asOver at *; exact Over.isMonHom_pullbackFst_id_right

end AlgebraicGeometry.Scheme
