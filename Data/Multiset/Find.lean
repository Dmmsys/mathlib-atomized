...
/-
**Multiset.find** 是 Mathlib 中的一个定理，位于命名空间 `Multiset`。
形式化陈述：find?_coe (l : List α) (hp) : (l : Multiset α).find? p hp = l.find? (fun a
 => p a)
参数：l : List α；hp。
该定理/引理给出了一组等式。
...
--- 原说明 ---
`s.find? p ⋯` finds the subsingleton element of `s` satisfying the condition `p`
, if one exists.
