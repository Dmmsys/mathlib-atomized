...
/-
**auxContinuousLinearEquiv** 是 Mathlib 中的一个缩写定义，位于命名空间 ``。
形式化陈述：auxContinuousLinearEquiv : V ≃L[𝕜] W where toLinearMap
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Scalar multiple of a continuous linear equivalence (given certain properties are
 satisfied).
-/
...
/-
**coe_auxContinuousLinearEquiv** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**auxIsometry** 是 Mathlib 中的一个缩写定义，位于命名空间 ``。
形式化陈述：auxIsometry (hαa : starRingEnd 𝕜 α' = α') : V ≃ₗᵢ[𝕜] W where .toLinearEqui
v __
参数：hαa : starRingEnd 𝕜 α' = α'。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**coe_auxIsometry** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
@[simp] theorem coe_auxIsometry (hαa : starRingEnd 𝕜 α' = α') :
    (auxIsometry e hα hα2 he he' hαa).toContinuousLinearEquiv.toContinuousLinearMap =
      α' • e.toContinuousLinearMap := rfl
/-
**coe_symm_auxIsometry** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
