...
/--
theorem `summable_norm_of_tsum_eLpNorm_ne_top` / 定理 `summable_norm_of_tsum_eLpNorm_ne_top`

English:
theorem summable_norm_of_tsum_eLpNorm_ne_top
  ...
  · have : forallᵐ x ∂μ, forall n, ‖f n x‖ₑ <= eLpNorm (f n) ∞ μ := ae_all_
...
Depends on / 依赖: filter_upwards, hx.ne, tsum_enorm_ne_top_iff_summable_norm
-/
theorem summable_norm_of_tsum_eLpNorm_ne_top {ι : Type*} [Countable ι]
    {p : Real>=0∞} (hp : 1 <= p) {f : ι -> X -> E} (hf : forall n, AEStronglyMeasurable (f n) μ)
    (h'f : ∑' n, eLpNorm (f n) p μ != ∞) :
    forallᵐ a ∂μ, Summable (fun n => ‖f n a‖) := by
  suffices H : forallᵐ a ∂μ, ∑' n, ‖f n a‖ₑ < ∞ by
    filter_upwards [H] with x hx using tsum_enorm_ne_top_iff_summable_norm.1 hx.ne
  -- the result is straightforward in `L^∞`.
  rcases eq_top_or_lt_top p with rfl | h'p
  · have : forallᵐ x ∂μ, forall n, ‖f n x‖ₑ <= eLpNorm (f n) ∞ μ := ae_all_iff.2 (fun n => ae_le_eLpNormEssSup)
  ...
  have A (s : Set X) (hs : MeasurableSet s) (h's : μ s != ∞) :
      forallᵐ x ∂μ, x in s -> ∑' n, ‖f n x‖ₑ < ∞ := by
  ...
  have B n : exists s, MeasurableSet s ∧ (f n =ᵐ[μ.restrict sᶜ] 0) ∧ SigmaFinite (μ.restrict s) := by
...
/--
theorem `hasSum_coeFn_tsum_nat` / 定理 `hasSum_coeFn_tsum_nat`

English:
theorem hasSum_coeFn_tsum_nat
  statement: {p : Real>=0∞} [hp : Fact (1 <= p)]
  proof: by
  have A : forallᵐ a ∂μ, Summable (fun n => ‖f n a‖) := by
    apply summable_norm_of_tsum_eLpNorm_ne_top hp.out (fun n => Lp.aestronglyMeasurable (f n))
    simpa [enorm_def] using hf
  have B : forallᵐ x ∂μ, forall n, ⇑(∑ i in range n, f i) x = ∑ i in range n, f i x := by
    rw [ae_all_iff]
  

中文:
定理 hasSum_coeFn_tsum_nat
  结论: {p : 实数>=0∞} [hp : Fact (1 <= p)]
  证明: by
  have A : forallᵐ a ∂μ, Summable (fun n => ‖f n a‖) := by
    apply summable_norm_of_tsum_eLpNorm_ne_top hp.out (fun n => Lp.aestronglyMeasurable (f n))
    simpa [enorm_def] using hf
...
-/
private theorem hasSum_coeFn_tsum_nat {p : Real>=0∞} [hp : Fact (1 <= p)]
    [CompleteSpace E] {f : Nat -> Lp E p μ} (hf : ∑' n, ‖f n‖ₑ != ∞) :
    forallᵐ a ∂μ, HasSum (fun n => f n a) (⇑(∑' n, f n) a) := by
  have A : forallᵐ a ∂μ, Summable (fun n => ‖f n a‖) := by
    apply summable_norm_of_tsum_eLpNorm_ne_top hp.out (fun n => Lp.aestronglyMeasurable (f n))
    simpa [enorm_def] using hf
  have B : forallᵐ x ∂μ, forall n, ⇑(∑ i in range n, f i) x = ∑ i in range n, f i x := by
  ...
  obtain ⟨ns, hns, nslim⟩ : exists ns : Nat -> Nat, StrictMono ns ∧ forallᵐ x ∂μ,
      Tendsto (fun i => (∑ j in range (ns i), f j : Lp E p μ) x) atTop (𝓝 ((∑' n, f n) x)) := by
    have : Tendsto (fun i => (∑ j in range i, f j)) atTop (𝓝 (∑' n, f n)) :=
...
/--
theorem `hasSum_coeFn_tsum` / 定理 `hasSum_coeFn_tsum`

English:
theorem hasSum_coeFn_tsum
  statement: {p : Real>=0∞} [hp : Fact (1 <= p)] {ι : Type*} [Countable ι]
  proof: by
  rcases finite_or_infinite ι with hι | hι
  · let : Fintype ι := Fintype.ofFinite ι
    filter_upwards [coeFn_fun_finsetSum univ f] with x hx
    rw [tsum_fintype]; rw [hx]
    exact hasSum_fintype _
  · obtain ⟨e⟩ := nonempty_equiv_of_countable (α := Nat) (β := ι)
    have : forallᵐ a ∂μ, HasSu
...
Depends on / 依赖: Fintype, Fintype.ofFinite, HasSum, Lp.hasSum_coeFn_tsum_nat, coeFn_fun_finsetSum, convert, e.hasSum_iff, e.tsum_eq, filter_upwards, finite_or_infinite, hasSum_coeFn_tsum_nat, hasSum_fintype, hasSum_iff, nonempty_equiv_of_countable, ofFinite, tsum_eq, tsum_fintype
-/
theorem hasSum_coeFn_tsum {p : Real>=0∞} [hp : Fact (1 <= p)] {ι : Type*} [Countable ι]
  ...
  · let : Fintype ι := Fintype.ofFinite ι
    ...
    exact hasSum_fintype _
  · obtain ⟨e⟩ := nonempty_equiv_of_countable (α := Nat) (β := ι)
    have : forallᵐ a ∂μ, HasSum (fun n => f (e n) a) (⇑(∑' n, f (e n)) a) := by
...
/--
theorem `coeFn_tsum` / 定理 `coeFn_tsum`

English:
theorem coeFn_tsum
  statement: {ι : Type*} [Countable ι] {p : Real>=0∞} [hp : Fact (1 <= p)]
  proof: by
...
Depends on / 依赖: Lp.hasSum_coeFn_tsum, filter_upwards, hasSum_coeFn_tsum, hx.tsum_eq.symm, tsum_eq
-/
theorem coeFn_tsum {ι : Type*} [Countable ι] {p : Real>=0∞} [hp : Fact (1 <= p)]
    [CompleteSpace E] {f : ι -> Lp E p μ} (hf : ∑' n, ‖f n‖ₑ != ∞) :
