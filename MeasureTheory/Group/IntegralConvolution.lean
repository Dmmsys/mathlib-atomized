...
@[to_additive]
/--
lemma `integrable_mconv_iff` / 引理 `integrable_mconv_iff`

English:
lemma integrable_mconv_iff
  given: [SFinite ν] (hf : AEStronglyMeasurable f (μ ∗ₘ ν))
  proof: by
...
中文:
引理 integrable_mconv_iff
  条件: [SFinite ν] (hf : AEStronglyMeasurable f (μ ∗ₘ ν))
  证明: by
  simp [Measure.mconv, integrable_map_measure hf (by fun_prop),
    integrable_prod_iff (hf.comp_measurable (by fun_prop))]
...
Depends on / 依赖: Measure, Measure.mconv, comp_measurable, fun_prop, hf.comp_measurable, integrable_map_measure, integrable_prod_iff
-/
...
@[to_additive]
/--
lemma `integral_mconv` / 引理 `integral_mconv`

English:
lemma integral_mconv
  given: [NormedSpace Real F] [SFinite μ] [SFinite ν] (hf : Integrable f (μ ∗ₘ ν))
  proof: by
...
Depends on / 依赖: Measure, Measure.mconv, fun_prop, integrable_map_measure, integral_map, integral_prod
-/
lemma integral_mconv [NormedSpace Real F] [SFinite μ] [SFinite ν] (hf : Integrable f (μ ∗ₘ ν)) :
