...
definition composableArrowsFunctor
  signature: :
  body: CochainComplex.mappingCone (f.map' 0 1)
...
定义 composableArrowsFunctor
  签名: :
  定义体: CochainComplex.mappingCone (f.map' 0 1)
  map φ := map _ _ (φ.app 0) (φ.app 1) (ComposableArrows.naturality' φ 0 1)
  map_id _ := map_id _
  map_comp _ _ := map_comp _ _ _ _ _ _ _ _ _

Depends on / 依赖: CochainComplex, CochainComplex.mappingCone, f.map, mappingCone
-/
noncomputable def composableArrowsFunctor :
    ComposableArrows (CochainComplex C Int) 1 ⥤ CochainComplex C Int where
...
@[simps]
/--
Definition of `spectralObjectMappingCone` / `spectralObjectMappingCone` 的定义

English:
definition spectralObjectMappingCone
  signature: :
  body: composableArrowsFunctor C ⋙ HomotopyCategory.quotient _ _
...
定义 spectralObjectMappingCone
  签名: :
  定义体: composableArrowsFunctor C ⋙ HomotopyCategory.quotient _ _
  δ'.app D := ((HomotopyCategory.quotient C (ComplexShape.up Int)).mapTriangle.obj
    (CochainComplex.mappingConeCompTriangle (D.map' 0 1) (D.map' 1 2))).mor₃
  δ'.naturality D₁ D₂ φ := by
    obtain ⟨_, _, _, f, g, rfl⟩ := ComposableArrows.mk₂_surjective D₁
...
noncomputable def spectralObjectMappingCone :
    SpectralObject (HomotopyCategory C (ComplexShape.up Int)) (CochainComplex C Int) where
