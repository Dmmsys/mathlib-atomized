...
/-
**Mathlib.Tactic.GCongr.GCongrKey** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Tactic.GCon
gr`。
形式化陈述：GCongrKey where /-- The name of the relation. For example, `a + b ≤ a + c`
 has ``relName
...
/-
**Mathlib.Tactic.GCongr.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Tactic.GCongr`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Tactic.GCongr.GCongrHyp** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathlib.Tactic.GC
ongr`。
形式化陈述：Type
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
The data for a "main" hypothesis of a `gcongr` lemma.
That is a hypothesis of the form `∀ xs, a xs' ~ b xs'` where `xs'` is a subset o
f `xs`,
and `a` and `b` form a "varying pair" in the conclusion of the `gcongr` lemma.
-/
...
/-
**Mathlib.Tactic.GCongr.GCongrLemma** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Tactic.GC
ongr`。
形式化陈述：GCongrLemma where /-- The keys under which the lemma is stored. This is us
ually one key, but for `gcongr strict` lemmas, this stores two keys, for the LHS
 and RHS respectively. -/ keys : List GCongrKey /-- The name of the lemma. -/ de
clName : Name /-- `mainSubgoals` are the subgoals on which `gcongr` will be recu
rsively called. -/ mainSubgoals : Array GCongrHyp /-- The number of arguments th
at `declName` takes when applying it. -/ numHyps : Nat /-- The given priority of
 the lemma, for example as
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.GCongrLemmas** 是 Mathlib 中的一个缩写定义，位于命名空间 `Mathlib.Tactic
.GCongr`。
形式化陈述：GCongrLemmas
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.GCongrLemma.prioLE** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Ta
ctic.GCongr.GCongrLemma`。
形式化陈述：Mathlib.Tactic.GCongr.GCongrLemma → Mathlib.Tactic.GCongr.GCongrLemma → Bo
ol
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.addGCongrLemmaEntry** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.T
actic.GCongr`。
形式化陈述：addGCongrLemmaEntry (m : GCongrLemmas) (l : GCongrLemma) : GCongrLemmas
参数：m : GCongrLemmas；l : GCongrLemma。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Insert a `GCongrLemma` in a collection of lemmas, making sure they are sorted by
 priority.
-/
...
/-
**Mathlib.Tactic.GCongr.findGCongrLemmas** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tact
ic.GCongr`。
形式化陈述：findGCongrLemmas? (relName lhs rhs : Name) (arity : Nat) : CoreM (List GCo
ngrLemma)
参数：relName lhs rhs : Name；arity : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Return the `gcongr` lemmas whose conclusion has the shape `relName (lhs ..) (rhs
 ..)`,
where `arity` is the number of arguments in `..`. This is used by the `gcongr` t
actic.
-/
...
--- 原说明 ---
Get the `gcongr` lemmas whose conclusion has the shape
`relName (head ..) _`, if `forward := true`, or `rel _ (head ..)`, if `forward :
= false`.
`arity` is the number of arguments in `..`. This is used by the `grw` tactic.
-/
...
/-
**Mathlib.Tactic.GCongr.getCongrAppFnArgs** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.GCongr`。
形式化陈述：getCongrAppFnArgs (e : Expr) : Option (Name × Array Expr)
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given an application `f a₁ .. aₙ`, return the name of `f`, and the array of argu
ments `aᵢ`.
-/
...
/-
**Mathlib.Tactic.GCongr.getRel** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GCongr`
。
形式化陈述：getRel (e : Expr) : Option (Name × Expr × Expr)
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.updateRel** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GCon
gr`。
形式化陈述：updateRel (r e : Expr) (isLhs : Bool) : Expr
参数：r e : Expr；isLhs : Bool。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
conclusion `target`. -/
/-
**Mathlib.Tactic.GCongr.makeGCongrLemma** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tacti
c.GCongr`。
形式化陈述：makeGCongrLemma (hyps : Array Expr) (target : Expr) (declName : Name) (pri
o : Nat) (strict forGrw : Bool) : MetaM GCongrLemma
参数：hyps : Array Expr；target : Expr；declName : Name；prio : Nat；strict forGrw : Bo
ol。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Std.Rxi.Iterator.instProductive`：∀ {α : Type u} [inst : Std.PRange.Upwar
dEnumerable α] [Std.PRange.LawfulUpwardEnumerable α],   Std.Iterators.Productive
 (Std.Rxi.Iterator α)…
...
/-
**Mathlib.Tactic.GCongr.addGCongrLemma** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.GCongr`。
形式化陈述：addGCongrLemma (declName : Name) (stx : Syntax) (kind : AttributeKind) : M
etaM Unit
参数：declName : Name；stx : Syntax；kind : AttributeKind。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr._root_.Lean.MVarId.applyRflOrId** 是 Mathlib 中的一个定义，位于命名空
间 `Mathlib.Tactic.GCongr`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.GCongr.gcongrDischarger** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tact
ic.GCongr`。
形式化陈述：gcongrDischarger (goal : MVarId) : MetaM Unit
参数：goal : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.exactRefl** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GCon
gr`。
形式化陈述：Mathlib.Tactic.GCongr.ForwardExt
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
See if the term is `a = b` and the goal is `a ∼ b` or `b ∼ a`, with `∼` reflexiv
e.
-/
@[gcongr_forward] def exactRefl : ForwardExt where
  eval h goal := do
    ...
    goal.applyRfl
/-
**Mathlib.Tactic.GCongr.exact** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GCongr`。
形式化陈述：Mathlib.Tactic.GCongr.ForwardExt
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
these hypotheses. -/
/-
**Mathlib.Tactic.GCongr._root_.Lean.MVarId.gcongrForward** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Tactic.GCongr`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Attempt to resolve an (implicitly) relational goal by one of a provided list of 
hypotheses,
either with such a hypothesis directly or by a limited palette of relational for
ward-reasoning from
these hypotheses.
-/
...
-/
/-
**Mathlib.Tactic.GCongr.gcongrForwardDischarger** 是 Mathlib 中的一个定义，位于命名空间 `Mathl
ib.Tactic.GCongr`。
形式化陈述：gcongrForwardDischarger (goal : MVarId) : MetaM Bool
参数：goal : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
This is used as the default main-goal discharger,
consisting of running `Lean.MVarId.gcongrForward` (trying a term together with l
imited
forward-reasoning on that term) on each nontrivial hypothesis.
-/
...
/-
**Mathlib.Tactic.GCongr.mkHoleAnnotation** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tact
ic.GCongr`。
形式化陈述：mkHoleAnnotation (e : Expr) : Expr
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.hasHoleAnnotation** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.GCongr`。
形式化陈述：hasHoleAnnotation (e : Expr) : Bool
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.containsHoleAnnotation** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Tactic.GCongr`。
形式化陈述：containsHoleAnnotation (e : Expr) : Bool
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Determine whether `e` contains a `gcongrHole` mdata annotation in any subexpress
ion.
This tells `gcongr` whether to continue applying `gcongr` lemmas.
-/
...
/-
**Mathlib.Tactic.GCongr.elabCHoleExpand** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tacti
c.GCongr`。
...
/-
**Mathlib.Tactic.GCongr.rel_imp_rel** 是 Mathlib 中的一个引理，位于命名空间 `Mathlib.Tactic.GC
ongr`。
形式化陈述：rel_imp_rel (h₁ : r c a) (h₂ : r b d) : r a b -> r c d
参数：h₁ : r c a；h₂ : r b d。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
-/
/-
**Mathlib.Tactic.GCongr.relImpRelLemma** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.GCongr`。
形式化陈述：relImpRelLemma (arity : Nat) : List GCongrLemma
参数：arity : Nat。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Construct a `GCongrLemma` for `gcongr` goals of the form `a ≺ b → c ≺ d`.
This will be tried if there is no other available `@[gcongr]` lemma.
For example, the relation `a ≡ b [ZMOD n]` has an instance of `IsTrans`, so a co
ngruence of the form
`a ≡ b [ZMOD n] → c ≡ d [ZMOD n]` can be solved with `rel_imp_rel`.
-/
...
/-
**Mathlib.Tactic.GCongr.State** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Tactic.GCongr`。
形式化陈述：State where /-- The new goals produced by `gcongr`. This includes side-goa
ls and main goals. TODO?: split this into separate side and main goals, so that 
the side goals can be discharged using a nested `gcongr by ...` syntax. -/ newGo
als : Array MVarId
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.Context** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathlib.Tactic.GCon
gr`。
形式化陈述：Type
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.GCongrM** 是 Mathlib 中的一个缩写定义，位于命名空间 `Mathlib.Tactic.GCon
gr`。
形式化陈述：GCongrM
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.GCongrM.run** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GC
ongr.GCongrM`。
形式化陈述：{α : Type} →   Mathlib.Tactic.GCongr.GCongrM α →     optParam (List (TSynt
ax `rintroPat)) [] →       optParam (MVarId → MetaM Bool) Mathlib.Tactic.GCongr.
gcongrForwardDischarger →         optParam (MVarId → MetaM Unit) Mathlib.Tactic.
GCongr.gcongrDischarger → MetaM (α × Array MVarId)
参数：List (TSyntax `rintroPat)；MVarId → MetaM Bool；MVarId → MetaM Unit；α × Array M
VarId。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.pushNewGoal** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GC
ongr`。
形式化陈述：pushNewGoal (g : MVarId) : GCongrM Unit
参数：g : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.introN** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.GCongr`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.dischargeMain** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.
GCongr`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.GCongr.dischargeSide** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.
GCongr`。
形式化陈述：dischargeSide (mvarId : MVarId) : GCongrM Unit
参数：mvarId : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.GCongr.applyGCongrLemma** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tact
ic.GCongr`。
形式化陈述：applyGCongrLemma (g : MVarId) (lem : GCongr.GCongrLemma) : GCongrM (Array 
(MVarId × Bool) × Array MVarId)
参数：g : MVarId；lem : GCongr.GCongrLemma。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Std.Rxi.Iterator.instProductive`：∀ {α : Type u} [inst : Std.PRange.Upwar
dEnumerable α] [Std.PRange.LawfulUpwardEnumerable α],   Std.Iterators.Productive
...
--- 原说明 ---
Apply generalized congruence lemma `lem` to goal `g`, throwing an error if it fa
ils.
Return an array of main subgoals and an array of side goals.
For each main subgoal, also return whether the sides of the relation are swapped
(i.e. whether it is contravariant).
...
This function is used by both the `gcongr` and `grw` tactic.
In the case of `grw`, one of the two sides of the goal is a metavariable that is
 filled in
by this function.

In the main subgoals, the forall binders are introduced. They are named using th
e `with` clause of
`gcongr`, or otherwise using binder names in the goal. `grw` relies on these nam
es to ensure that
binder names are preserved in the goal.
-/
...
match. -/
/-
**Mathlib.Tactic.GCongr._root_.Lean.MVarId.gcongr** 是 Mathlib 中的一个定义，位于命名空间 `Mat
hlib.Tactic.GCongr`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The core of the `gcongr` tactic.  Parse a goal into the form `(f _ ... _) ∼ (f _
 ... _)`,
look up any relevant `@[gcongr]` lemmas, try to apply them, recursively run the 
tactic itself on
"main" goals which are generated, and run the discharger on side goals which are
 generated. If there
is a user-provided template, first check that the template asks us to descend th
is far into the
match.
-/
