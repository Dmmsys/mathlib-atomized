...
@[simps]
/--
Definition of `Matrix.dualNumberEquiv` / `Matrix.dualNumberEquiv` 的定义

English:
definition Matrix.dualNumberEquiv
  signature: : Matrix n n (DualNumber R) ≃ₐ[R] DualNumber (Matrix n n R) where
  body: ⟨of fun i j => (A i j).fst, of fun i j => (A i j).snd⟩
