...
/--
Definition of `singleFunctorCompHomologyFunctorIso` / `singleFunctorCompHomologyFunctorIso` 的定义

English:
definition singleFunctorCompHomologyFunctorIso
  signature: (n : Int)
  body: Functor.isoWhiskerRight ((SingleFunctors.evaluation _ _ n).mapIso
...
中文:
定义 singleFunctorCompHomologyFunctorIso
  签名: (n : 整数)
  定义体: Functor.isoWhiskerRight ((SingleFunctors.evaluation _ _ n).mapIso
    (singleFunctorsPostcompQIso C)) _ ≪≫ Functor.associator _ _ _ ≪≫
    Functor.isoWhiskerLeft _ (homologyFunctorFactors C n) ≪≫
      (HomologicalComplex.homologyFunctorSingleIso _ _ _)

Depends on / 依赖: Functor, Functor.associator, Functor.isoWhiskerLeft, Functor.isoWhiskerRight, HomologicalComplex, HomologicalComplex.homologyFunctorSingleIso, SingleFunctors, SingleFunctors.evaluation, associator, evaluation, homologyFunctorFactors, homologyFunctorSingleIso, isoWhiskerLeft, isoWhiskerRight, mapIso, singleFunctorsPostcompQIso
-/
noncomputable def singleFunctorCompHomologyFunctorIso (n : Int) :
...
instance (n : Int) : (singleFunctor C n).Faithful where
...
instance (n : Int) : (singleFunctor C n).Full where
  map_surjective {A B} f := by
    ...
    suffices exists f', f = Q.map f' by
      obtain ⟨f', rfl⟩ := this
