...
中文:
定义 Adapted
  签名: (f : Filtration ι m) (u : (i : ι) -> Ω -> β i)
...
中文:
定理 mul
  结论: [对任意 i, Mul (β i)] [对任意 i, MeasurableMul₂ (β i)]
...
中文:
定理 div
  结论: [对任意 i, Div (β i)] [对任意 i, MeasurableDiv₂ (β i)]
...
中文:
定理 inv
  条件: [对任意 i, Group (β i)] [对任意 i, MeasurableInv (β i)] (hu : Adapted f u)
  证明: fun i => (hu i).inv
-/
...
中文:
定理 adapted_const
  条件: {β : 类型} [MeasurableSpace β] (f : Filtration ι m) (x : β)
...
中文:
定理 mul
  结论: [对任意 i, Mul (β i)] [对任意 i, ContinuousMul (β i)]
  证明: fun i => (hu i).mul (hv i)
...
中文:
定理 div'
  结论: [对任意 i, Div (β i)] [对任意 i, ContinuousDiv (β i)]
  证明: fun i => (hu i).div' (hv i)
...
中文:
定理 inv
  条件: [对任意 i, Group (β i)] [对任意 i, ContinuousInv (β i)] (hu : StronglyAdapted f u)
  证明: fun i => (hu i).inv
-/
...
中文:
定理 smul
  结论: [对任意 i, SMul 实数 (β i)] [对任意 i, ContinuousConstSMul 实数 (β i)]
  证明: fun i => (hu i).const_smul c
-/
...
中文:
引理 norm
  结论: {β : ι -> 类型} {u : (i : ι) -> Ω -> β i} [对任意 i, SeminormedAddCommGroup (β i)]
  证明: fun t => (hu t).norm
-/
...
定理 StronglyAdapted.adapted
  结论: [mΒ : 对任意 i, MeasurableSpace (β i)] [对任意 i, BorelSpace (β i)]
...
定理 Adapted.stronglyAdapted
  结论: [mΒ : 对任意 i, MeasurableSpace (β i)]
...
中文:
定理 stronglyAdapted_const
  条件: {β : 类型} [TopologicalSpace β] (f : Filtration ι m) (x : β)
...
定理 stronglyAdapted_zero'
  条件: [对任意 i, Zero (β i)] (f : Filtration ι m)
...
中文:
定理 stronglyAdapted_zero
  条件: (β : 类型) [TopologicalSpace β] [Zero β] (f : Filtration ι m)
...
中文:
定理 Filtration.stronglyAdapted_natural
  结论: [对任意 i, MetrizableSpace (β i)]
  证明: by
  intro i
  refine StronglyMeasurable.mono ?_ (le_iSup₂_of_le i (le_refl i) le_rfl)
  rw [stronglyMeasurable_iff_measurable_separable]
...
中文:
定义 IsProgressive
  签名: [MeasurableSpace ι] [MeasurableSpace β] (f : Filtration ι m)
...
定理 isProgressive_const
  结论: {mi : MeasurableSpace ι} {mβ : MeasurableSpace β} (f : Filtration ι m)
...
中文:
定理 mul
  结论: [Mul β] [MeasurableMul₂ β] (hu : IsProgressive f u)
...
中文:
定理 finsetProd
  结论: {γ} [CommMonoid β] [MeasurableMul₂ β] {U : γ -> ι -> Ω -> β}
...
中文:
定理 inv
  条件: [Group β] [MeasurableInv β] (hu : IsProgressive f u)
  证明: fun i => (hu i).inv
...
中文:
定理 div
  结论: [Group β] [MeasurableDiv₂ β] (hu : IsProgressive f u)
...
中文:
引理 norm
  条件: [NormedAddCommGroup β] [OpensMeasurableSpace β] (hu : IsProgressive f u)
...
定义 IsStronglyProgressive
  签名: [MeasurableSpace ι] (f : Filtration ι m) (u : ι -> Ω -> β)
...
定理 isStronglyProgressive_const
  条件: [MeasurableSpace ι] (f : Filtration ι m) (b : β)
  证明: fun i =>
  @stronglyMeasurable_const _ _ (Subtype.instMeasurableSpace.prod (f i)) _ _
-/
...
中文:
定理 comp
  结论: {t : ι -> Ω -> ι} [TopologicalSpace ι] [BorelSpace ι] [PseudoMetrizableSpace ι]
  证明: by
  intro i
  have : (fun p : ↥(Set.Iic i) × Ω => u (t (p.fst : ι) p.snd) p.snd) =
    (fun p : ↥(Set.Iic i) × Ω => u (p.fst : ι) p.snd) ∘ fun p : ↥(Set.Iic i) × Ω =>
...
中文:
定理 mul
  结论: [Mul β] [ContinuousMul β] (hu : IsStronglyProgressive f u)
  证明: fun i =>
  (hu i).mul (hv i)
...
中文:
定理 finsetProd'
  结论: {γ} [CommMonoid β] [ContinuousMul β] {U : γ -> ι -> Ω -> β}
...
中文:
定理 inv
  条件: [Group β] [ContinuousInv β] (hu : IsStronglyProgressive f u)
  证明: fun i => (hu i).inv
...
中文:
定理 div'
  结论: [Group β] [ContinuousDiv β] (hu : IsStronglyProgressive f u)
  证明: fun i =>
  (hu i).div' (hv i)
-/
...
中文:
引理 norm
  结论: {β : 类型} {u : ι -> Ω -> β} [SeminormedAddCommGroup β]
  证明: fun t => (hu t).norm
-/
...
引理 IsProgressive.isStronglyProgressive
  结论: {mi : MeasurableSpace ι} {mβ : MeasurableSpace β}
...
定理 isStronglyProgressive_of_tendsto'
  结论: {γ} [MeasurableSpace ι] [PseudoMetrizableSpace β]
  证明: by
  intro i
  apply @stronglyMeasurable_of_tendsto (Set.Iic i × Ω) β γ
    (MeasurableSpace.prod _ (f i)) _ _ fltr _ _ _ _ fun l => h l i
...
中文:
定理 isStronglyProgressive_of_tendsto
  结论: [MeasurableSpace ι] [PseudoMetrizableSpace β]
...
定理 StronglyAdapted.isStronglyProgressive_of_continuous
  结论: [TopologicalSpace ι] [MetrizableSpace ι]
  证明: fun i =>
  @stronglyMeasurable_uncurry_of_continuous_of_stronglyMeasurable _ _ (Set.Iic i) _ _ _ _ _ _ _
    (f i) _ (fun ω => (hu_cont ω).comp continuous_induced_dom) fun j => (h j).mono (f.mono j.prop)
-/
...
定理 StronglyAdapted.isStronglyProgressive_of_discrete
  结论: [TopologicalSpace ι] [DiscreteTopology ι]
