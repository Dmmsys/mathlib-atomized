...
/--
theorem `exist_mem_one_of_mem_maximal_ideal` / 定理 `exist_mem_one_of_mem_maximal_ideal`

English:
theorem exist_mem_one_of_mem_maximal_ideal
  statement: [IsLocalRing R] {p₁ p₀ : PrimeSpectrum R}
  proof: by
...
Depends on / 依赖: IsLocalRing, IsLocalRing.le_maximalIdeal_of_isPrime, IsMaximal, IsMaximal.isPrime, IsPrime, PrimeSpectrum, isPrime, le_maximalIdeal_of_isPrime, le_sup_left, le_sup_right, le_sup_right.trans, lt_of_le_not_ge, lt_top, mem_span_singleton_self, nonempty_minimalPrimes, primeSpectrumQuotientOrderIsoZeroLocus, primeSpectrumQuotientOrderIsoZeroLocus.symm, span_singleton_le_iff_mem, sup_le, trans_lt
-/
theorem exist_mem_one_of_mem_maximal_ideal [IsLocalRing R] {p₁ p₀ : PrimeSpectrum R}
    (h₀ : p₀ < p₁) (h₁ : p₁ < closedPoint R) {x : R} (hx : x in 𝔪) :
    exists q : PrimeSpectrum R, x in q.asIdeal ∧ p₀ < q ∧ q.asIdeal < 𝔪 := by
  by_cases hn : x in p₀.1
  · exact ⟨p₁, h₀.le hn, h₀, h₁⟩
  let e := p₀.1.primeSpectrumQuotientOrderIsoZeroLocus.symm
obtain ⟨q, hq⟩ := (p₀.1 + span {x}).nonempty_minimalPrimes
    sup_le (IsLocalRing.le_maximalIdeal_of_isPrime p₀.1) ((span_singleton_le_iff_mem 𝔪).mpr hx)
.ne .trans_lt (IsMaximal.isPrime' 𝔪).1.lt_top
  let q : PrimeSpectrum R := ⟨q, hq.1.1⟩
  have : q.1.IsPrime := q.2
  have hxq : x in q.1 := le_sup_right.trans hq.1.2 (mem_span_singleton_self x)
...
set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `exist_mem_one_of_mem_two` / 定理 `exist_mem_one_of_mem_two`

English:
theorem exist_mem_one_of_mem_two
  statement: {p₁ p₀ p₂ : PrimeSpectrum R}
  proof: by
...
Depends on / 依赖: AtPrime, IsLocalization, IsLocalization.AtPrime.primeSpectrumOrderIso, Localization, Localization.AtP, Localization.AtPrime, Localization.AtPrime.map_eq_maximalIdeal, PrimeSpectrum, PrimeSpectrum.ext, algebraMap, closedPoint, e.symm, e.symm.lt_iff_lt.mpr, exist_mem_one_of_mem_maximal_ideal, le_refl, lt_iff_lt, map_eq_maximalIdeal, primeSpectrumOrderIso
-/
theorem exist_mem_one_of_mem_two {p₁ p₀ p₂ : PrimeSpectrum R}
    (h₀ : p₀ < p₁) (h₁ : p₁ < p₂) {x : R} (hx : x in p₂.asIdeal) :
    exists q : (PrimeSpectrum R), x in q.asIdeal ∧ p₀ < q ∧ q < p₂ := by
  let e := IsLocalization.AtPrime.primeSpectrumOrderIso (Localization.AtPrime p₂.1) p₂.1
  have hm : closedPoint (Localization.AtPrime p₂.1) =
    e.symm ⟨p₂, le_refl p₂⟩ := (PrimeSpectrum.ext Localization.AtPrime.map_eq_maximalIdeal).symm
  obtain ⟨q, hxq, h₀, h₁⟩ :=
...
/--
theorem `exist_ltSeries_mem_one_of_mem_last` / 定理 `exist_ltSeries_mem_one_of_mem_last`

English:
theorem exist_ltSeries_mem_one_of_mem_last
  statement: (p : LTSeries (PrimeSpectrum R))
  proof: by
  ...
  | zero =>
    ...
    rw [show 0 = Fin.last p.length from F
...
Depends on / 依赖: Fin.last, Fin.natCast_eq_mk, Fin.zero_eq_mk.mpr, Nat.one_lt_succ_succ, RelSeries, RelSeries.head, RelSeries.last, RelSeries.last_singleton, RelSeries.singleton, RelSeries.singleton_length, RelSeries.singleton_toFun, and_true, generalize, generalizing, last_singleton, length, natCast_eq_mk, one_lt_succ_succ, p.last, p.length
-/
theorem exist_ltSeries_mem_one_of_mem_last (p : LTSeries (PrimeSpectrum R))
    {x : R} (hx : x in p.last.asIdeal) : exists q : LTSeries (PrimeSpectrum R),
    x in (q 1).asIdeal ∧ p.length = q.length ∧ p.head = q.head ∧ p.last = q.last := by
  generalize hp : p.length = n
  induction n generalizing p with
  | zero =>
    use RelSeries.singleton _ p.last
    simp only [RelSeries.singleton_toFun, hx, RelSeries.singleton_length, RelSeries.head,
      RelSeries.last_singleton, and_true, true_and]
    rw [show 0 = Fin.last p.length from Fin.zero_eq_mk.mpr hp]; rw [RelSeries.last]
  | succ n hn => ?_
  by_cases h0 : n = 0
  · use p
    have h1 : 1 = Fin.last p.length := by
      rw [Fin.last]; rw [hp]; rw [h0]; rw [zero_add]
      exact Fin.natCast_eq_mk (Nat.one_lt_succ_succ 0)
    simpa [h1, hp] using! hx
  obtain ⟨q, hxq, h2, hq⟩ : exists q : PrimeSpectrum R, x in q.1 ∧ p ⟨p.length - 2, _⟩ < q ∧ q < p.last :=
