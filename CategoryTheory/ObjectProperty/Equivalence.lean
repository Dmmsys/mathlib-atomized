...
/-
**CategoryTheory.ObjectProperty.essSurj_** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheo
ry.ObjectProperty`。
...
/-
**CategoryTheory.ObjectProperty.isEquivalence_** 是 Mathlib 中的一个引理，位于命名空间 `Catego
...
@[simps]
/-
**CategoryTheory.ObjectProperty.topEquivalence** 是 Mathlib 中的一个定义，位于命名空间 `Catego
ryTheory.ObjectProperty`。
形式化陈述：topEquivalence : ObjectProperty.FullSubcategory (C
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The equivalence between the full subcategory `⊤` of a category `C` and `C` itsel
f.
-/
...
@[simps]
/-
**CategoryTheory.Equivalence.congrFullSubcategory** 是 Mathlib 中的一个定义，位于命名空间 `Cat
egoryTheory.Equivalence`。
形式化陈述：congrFullSubcategory [Q.IsClosedUnderIsomorphisms] (h : Q.inverseImage e.f
unctor = P) : P.FullSubcategory ≌ Q.FullSubcategory where functor
参数：h : Q.inverseImage e.functor = P。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
