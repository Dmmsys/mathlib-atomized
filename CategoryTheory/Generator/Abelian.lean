...
/--
theorem `has_injective_coseparator` / 定理 `has_injective_coseparator`

English:
theorem has_injective_coseparator
  given: [HasLimits C] [EnoughInjectives C] (G : C) (hG : IsSeparator G)
  proof: by
...
theorem has_injective_coseparator [HasLimits C] [EnoughInjectives C] (G : C) (hG : IsSeparator G) :
    exists G : C, Injective G ∧ IsCoseparator G := by
  have : WellPowered.{v} C := wellPowered_of_isDetector G hG.isDetector
  have : HasProductsOfShape (Subobject (op G)) C := hasProductsOfShape_of_small.{v} _ _
  let T : C := Injective.under (piObj fun P : Subobject (op G) => unop P)
...
/--
theorem `has_projective_separator` / 定理 `has_projective_separator`

English:
theorem has_projective_separator
  statement: [HasColimits C] [EnoughProjectives C] (G : C)
  proof: by
...
Depends on / 依赖: has_injective_coseparator, isSeparator_op_iff, isSeparator_unop_iff
-/
theorem has_projective_separator [HasColimits C] [EnoughProjectives C] (G : C)
    (hG : IsCoseparator G) : exists G : C, Projective G ∧ IsSeparator G := by
  obtain ⟨T, hT₁, hT₂⟩ := has_injective_coseparator (op G) ((isSeparator_op_iff _).2 hG)
  exact ⟨unop T, inferInstance, (isSeparator_unop_iff _).2 hT₂⟩
