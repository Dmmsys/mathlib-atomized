...
/--
lemma `lift_awayMapₐ_awayMapₐ_surjective` / 引理 `lift_awayMapₐ_awayMapₐ_surjective`

English:
lemma lift_awayMapₐ_awayMapₐ_surjective
  statement: {d e : Nat} {f : A} (hf : f in 𝒜 d)
  proof: by
  intro z
...
中文:
引理 lift_awayMapₐ_awayMapₐ_surjective
  结论: {d e : 自然数} {f : A} (hf : f in 𝒜 d)
  证明: by
  intro z
  obtain ⟨⟨n, ⟨a, ha⟩, ⟨b, hb'⟩, ⟨j, (rfl : _ = b)⟩⟩, rfl⟩ := mk_surjective z
  by_cases hfg : (f * g) ^ j = 0
...
Depends on / 依赖: DirectSum, DirectSum.degree_eq_of_mem_mem, HomogeneousLocalization, HomogeneousLocalization.subsingleton, NumDenSameDeg, SetLike, SetLike.mul_mem_graded, SetLike.pow_mem_graded, Submonoid, Submonoid.powers, convert, degree_eq_of_mem_mem, infer_instance, mk_surjective, mul_mem_graded, pow_mem_graded, powers, subsingleton, this.elim
-/
lemma lift_awayMapₐ_awayMapₐ_surjective {d e : Nat} {f : A} (hf : f in 𝒜 d)
    {g : A} (hg : g in 𝒜 e) {x : A} (hx : x = f * g) (hd : 0 < d) :
    Function.Surjective
...
/--
Instance `isSeparated` / 实例 `isSeparated`

English:
instance isSeparated
  signature: : IsSeparated (toSpecZero 𝒜)
  body: by
...
Depends on / 依赖: Cover.pullbackHom, IsClosedImmersion, IsZariskiLocalAtTarget, IsZariskiLocalAtTarget.of_openCover, MorphismProperty, MorphismProperty.cancel_left_of_respectsIso, Pullback, Pullback.openCoverOfLeftRight, Scheme, TensorProduct, affineOpenCover, cancel_left_of_respectsIso, of_openCover, openCover, openCoverOfLeftRight, pullback, pullbackDiagonalMapIdIso, pullbackHom, toSpecZero
-/
instance isSeparated : IsSeparated (toSpecZero 𝒜) := by
  ...
  rw [← MorphismProperty.cancel_right_of_respectsIso (P := @IsClosedImmersion) _ e₁.hom]; rw [← MorphismProperty.cancel_left_of_respectsIso (P := @IsClosedImmersion) e₂.inv]
  let F : Away 𝒜 i.2.1 otimes[𝒜 0] Away 𝒜 j.2.1 ->+* Away 𝒜 (i.2.1 * j.2.1) :=
  ...
  convert!
    IsClosedImmersion.spec_of_surjective
      (CommRingCat.ofHom (R := Away 𝒜 i.2.1 otimes[𝒜 0] Away 𝒜 j.2.1) F) this using 1
...
English:
instance :
  signature: Scheme.IsSeparated (Proj 𝒜)
  body: (HasAffineProperty.iff_of_isAffine (P := @IsSeparated)).mp (isSeparated 𝒜)
...
English:
instance [Algebra.FiniteType
  signature: (𝒜 0) A] : LocallyOfFiniteType (Proj.toSpecZero 𝒜)
  body: by
...
Depends on / 依赖: Category, Category.assoc, GradedAlgebra, GradedAlgebra.exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, IsZariskiLocalAtSource, IsZariskiLocalAtSource.iff_of_iSup_eq_top, LocallyOfFiniteType, MorphismProperty, MorphismProperty.cancel_left_of_respectsIso, Proj.away, Proj.basicOpenIsoSpec, Proj.iSup_basicOpen_eq_top, basicOpenIsoSpec, bot_lt, cancel_left_of_respectsIso, exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, iSup_basicOpen_eq_top, iff_of_iSup_eq_top
-/
instance [Algebra.FiniteType (𝒜 0) A] : LocallyOfFiniteType (Proj.toSpecZero 𝒜) := by
  ...
  rw [← MorphismProperty.cancel_left_of_respectsIso (P := @LocallyOfFiniteType)
    (Proj.basicOpenIsoSpec 𝒜 (i : A) (hxd _ i.2) (hd _ i.2).bot_lt).inv]; rw [← Category.assoc]; rw [← Proj.awayι]; rw [Proj.awayι_toSpecZero]; rw [HasRingHomProperty.Spec_iff (P := @LocallyOfFiniteType)]
...
English:
instance [Algebra.FiniteType
  signature: (𝒜 0) A] : QuasiCompact (Proj.toSpecZero 𝒜)
  body: by
...
Depends on / 依赖: GradedAlgebra, GradedAlgebra.exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, HasAffineProperty, HasAffineProperty.iff_of_isAffine, IsCompact, Proj.away, Proj.basicOpen, Proj.iSup_basicOpen_eq_top, Proj.opensRange_away, QuasiCompact, basicOpen, bot_lt, continuous, exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, iSup_basicOpen_eq_top, iff_of_isAffine, isCompact_range
-/
...
/--
theorem `valuativeCriterion_existence_aux` / 定理 `valuativeCriterion_existence_aux`
...
theorem valuativeCriterion_existence_aux
  ...
  let ψ (i : ι) : ValueGroup O K :=
    valuation O K ((φ (Away.isLocalizationElem (hxdi j) (hxdi i))) ^ ∏ k in Finset.univ.erase i, d k)
  have : Nonempty ι := ⟨j⟩
  let Kmax := (Finset.univ.image ψ).max' (by simp)
  have ⟨i₀, hi1⟩ : exists a, ψ a = Kmax :=

中文:
定理 valuativeCriterion_existence_aux
  证明: by
  classical
  cases nonempty_fintype ι
  let ψ (i : ι) : ValueGroup O K :=
...
Depends on / 依赖: Away.isLocalizationElem, Finset, Finset.max, Finset.univ.erase, Finset.univ.image, Nonempty, ValueGroup, _mem, classical, isLocalizationElem, le_max, nonempty_fintype, valuation, zero_lt_iff, zero_lt_iff.mpr
-/
theorem valuativeCriterion_existence_aux
    ...
    (d : ι -> Nat) (hdi : forall i, 0 < d i) (hxdi : forall i, x i in 𝒜 (d i)) :
    exists (j₀ : ι) (φ' : Away 𝒜 (x j * x j₀) ->+* K), φ'.comp (awayMap 𝒜 (hxdi j₀) rfl) = φ ∧
  ...
  have : Nonempty ι := ⟨j⟩
  let Kmax := (Finset.univ.image ψ).max' (by simp)
  have ⟨i₀, hi1⟩ : exists a, ψ a = Kmax := by simpa using Finset.max'_mem (Finset.univ.image ψ)
  ...
  rintro _ ⟨sx, rfl⟩
  rw [RingHom.mem_range]; rw [← ValuationRing.mem_integer_iff]; rw [Valuation.mem_integer_iff]
...
exact ((valuation O K).map_add _ _).trans sup_le_iff.mpr ⟨hhx, hhy⟩
  | smul a x₀ hx hx1 =>
    ...
    rw [RingHom.algebraMap_toAlgebra]; rw [awayMap_fromZeroRingHom 𝒜 (hxdi j) (mul_comm (x j) (x i₀))]; rw [← awayMap_fromZeroRingHom 𝒜 (hxdi i₀) rfl a]; rw [hφ'1]
  ...
  | mem x1 h =>
    ...
    have H : (∏ i, x i ^ ai i) * x i₀ ^ (a * (d j - 1)) in 𝒜 ((a * d i₀) • d j) := by
      convert!
    ...
    suffices valuation O K (φ (Away.mk 𝒜 (hxdi j) _ _ H) /
      ...
      convert! this
      rw [eq_div_iff (pow_ne_zero _ hunit.ne_zero)]; rw [← hφ'1]; rw [← hφ'1]; rw [RingHom.comp_apply]; rw [← map_pow]; rw [← map_mul]
      congr
      ext
    ...
    rw [map_div₀]; rw [div_le_iff₀ ((pow_pos ((Valuation.pos_iff _).mpr hunit.ne_zero) _).trans_eq
      (Valuation.map_pow _ _ _).symm)]; rw [one_mul]; rw [← pow_le_pow_iff_left₀ zero_le zero_le
    ...
    calc
      ...
      _ = ψ i₀ ^ (d i₀ * a * d j) := by
          rw [Finset.prod_pow_eq_pow_sum]; rw [← pow_add]
      ...
      _ = valuation O K ((φ _) ^ a) ^ (d j * ∏ i, d i) := by
          · simp only [ψ, ← map_pow]
            congr 2
            rw [← pow_mul]; rw [← pow_mul]; rw [← mul_assoc]; rw [← mul_assoc]; rw [← mul_assoc]; rw [Finset.univ.prod_erase_mul d (h := Finset.mem_univ _)]; rw [mul_comm _ a]; rw [mul_right_comm]

set_option backward.isDefEq.respectTransparency.types false in
@[stacks 01MF]
/--
lemma `valuativeCriterion_existence` / 引理 `valuativeCriterion_existence`

English:
lemma valuativeCriterion_existence
  given: [Algebra.FiniteType (𝒜 0) A]
  proof: by
  rintro ⟨O, K, i₁, i₂, w⟩
  obtain ⟨x, hx, hx'⟩ := GradedAlgebra.exists_finset_adjoin_eq_top_and_homogeneous_ne_zero 𝒜
  choose d hd hxd using hx'
  have : i₁ (IsLocalRing.closedPoint K) in (⊤ : (Proj 𝒜).Opens) := trivial
  rw [← Proj.iSup_basicOpen_eq_top' 𝒜 (ι := x) (↑) (fun i => ⟨_]; rw [hxd 
...
Depends on / 依赖: GradedAlgebra, GradedAlgebra.exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, IsLocalRing, IsLocalRing.closedPoint, Proj.away, Proj.iSup_basicOpen_eq_top, Proj.opensRange, Set.range, TopologicalSpace, TopologicalSpace.Opens.mem_iSup, bot_lt, closedPoint, exists_finset_adjoin_eq_top_and_homogeneous_ne_zero, iSup_basicOpen_eq_top, mem_iSup, opensRange, subseteq
-/
lemma valuativeCriterion_existence [Algebra.FiniteType (𝒜 0) A] :
  ...
  choose d hd hxd using hx'
  have : i₁ (IsLocalRing.closedPoint K) in (⊤ : (Proj 𝒜).Opens) := trivial
  rw [← Proj.iSup_basicOpen_eq_top' 𝒜 (ι := x) (↑) (fun i => ⟨_]; rw [hxd _ i.2⟩) (by simpa using hx)]; rw [TopologicalSpace.Opens.mem_iSup] at this
  obtain ⟨i, hi⟩ := this
  have : Set.range i₁ subseteq (Proj.awayι 𝒜 _ (hxd i i.2) (hd i i.2).bot_lt).opensRange := by
  ...
  have H : Spec.preimage i₂ ≫ CommRingCat.ofHom (algebraMap O K) =
    ...
    rw [← Proj.awayι_toSpecZero _ _ (hxd i i.2)]; rw [IsOpenImmersion.lift_fac_assoc]
  ...
  · rw [← Spec.map_comp_assoc]
    ...
    change _ = φ ≫ _
    rw [← Spec.map_preimage φ]; rw [← CommRingCat.ofHom_hom (Spec.preimage φ)]; rw [← hφ]; rw [← CommRingCat.ofHom_comp]
  ...
  · simp only [Category.assoc, Proj.awayι_toSpecZero, ← Spec.map_comp]
    ...
    rw [awayMap_fromZeroRingHom]; rw [← awayMap_fromZeroRingHom 𝒜 (hxd i₀ i₀.2) rfl]; rw [← RingHom.comp_apply]; rw [hφ]
...
English:
instance [Algebra.FiniteType
  signature: (𝒜 0) A] : UniversallyClosed (Proj.toSpecZero 𝒜)
  body: by
...
Depends on / 依赖: UniversallyClosed, UniversallyClosed.eq_valuativeCriterion, eq_valuativeCriterion, valuativeCriterion_existence
-/
