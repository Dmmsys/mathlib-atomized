/-
Copyright (c) 2025 Junyan Xu. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Junyan Xu
-/
module

public import Mathlib.GroupTheory.Coset.Defs
public import Mathlib.Topology.Algebra.ConstMulAction
public import Mathlib.Topology.Algebra.Group.Quotient
public import Mathlib.Topology.Covering.Basic

/-!
# Covering maps to quotients by free and properly discontinuous group actions
-/

@[expose] public section

variable {E X : Type*} [TopologicalSpace E] [TopologicalSpace X] (f : E -> X)
variable (G : Type*) [Group G] [MulAction G E]

open Topology

/--
Definition of `IsAddQuotientCoveringMap` / `IsAddQuotientCoveringMap` 的定义

English:
structure IsAddQuotientCoveringMap
  parameters: (G) [AddGroup G] [AddAction G E]
  extends: IsQuotientMap f, ContinuousConstVAdd G E
  axioms and operations (2):
    - apply_eq_iff_mem_orbit({e₁ e₂}) : f e₁ = f e₂ ↔ e₁ in AddAction.orbit G e₂
    - disjoint((e : E)) : exists U in 𝓝 e, forall g : G, ((g +ᵥ ·) '' U inter U).Nonempty -> g = 0

中文:
结构 是加法QuotientCovering映射
  参数: (G) [加法群 G] [加法作用 G E]
  继承: 是商映射 f, 连续常数向量加法 G E
  公理与运算 (2 个):
    - apply_eq_iff_mem_orbit({e₁ e₂}) : f e₁ = f e₂ ↔ e₁ in 加法作用.orbit G e₂
    - disjoint((e : E)) : 存在 U in 𝓝 e, 对任意 g : G, ((g +ᵥ ·) '' U inter U).非空 -> g = 0
-/
structure IsAddQuotientCoveringMap (G) [AddGroup G] [AddAction G E] : Prop
    extends IsQuotientMap f, ContinuousConstVAdd G E where
  apply_eq_iff_mem_orbit {e₁ e₂} : f e₁ = f e₂ ↔ e₁ in AddAction.orbit G e₂
  disjoint (e : E) : exists U in 𝓝 e, forall g : G, ((g +ᵥ ·) '' U inter U).Nonempty -> g = 0

/-- A function from a topological space `E` with an action by a discrete group to another
topological space `X` is a quotient covering map if it is a quotient map, the action is
continuous and transitive on fibers, and every point of `E` has a neighborhood whose translates
by the group elements are pairwise disjoint. -/
@[mk_iff, to_additive]
/--
Definition of `IsQuotientCoveringMap` / `IsQuotientCoveringMap` 的定义

English:
structure IsQuotientCoveringMap
  parameters: : Prop extends IsQuotientMap f, ContinuousConstSMul G E where
  extends: IsQuotientMap f, ContinuousConstSMul G E
  axioms and operations (2):
    - apply_eq_iff_mem_orbit({e₁ e₂}) : f e₁ = f e₂ ↔ e₁ in MulAction.orbit G e₂
    - disjoint((e : E)) : exists U in 𝓝 e, forall g : G, ((g • ·) '' U inter U).Nonempty -> g = 1

中文:
结构 是QuotientCovering映射
  参数: : 命题 extends 是商映射 f, 连续常数标量乘法 G E where
  继承: 是商映射 f, 连续常数标量乘法 G E
  公理与运算 (2 个):
    - apply_eq_iff_mem_orbit({e₁ e₂}) : f e₁ = f e₂ ↔ e₁ in 乘法作用.orbit G e₂
    - disjoint((e : E)) : 存在 U in 𝓝 e, 对任意 g : G, ((g • ·) '' U inter U).非空 -> g = 1
-/
structure IsQuotientCoveringMap : Prop extends IsQuotientMap f, ContinuousConstSMul G E where
  apply_eq_iff_mem_orbit {e₁ e₂} : f e₁ = f e₂ ↔ e₁ in MulAction.orbit G e₂
  disjoint (e : E) : exists U in 𝓝 e, forall g : G, ((g • ·) '' U inter U).Nonempty -> g = 1

attribute [to_additive] isQuotientCoveringMap_iff

/--
lemma `IsAddQuotientCoveringMap.toMultiplicative` / 引理 `IsAddQuotientCoveringMap.toMultiplicative`

English:
lemma IsAddQuotientCoveringMap.toMultiplicative
  statement: (G) [AddGroup G] [AddAction G E]
  proof: hf.toIsQuotientMap
  continuous_const_smul g := by simpa using hf.continuous_const_vadd (Multiplicative.ofAdd.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Multiplicative.ofAdd.symm g)⟩

中文:
引理 是加法QuotientCovering映射.toMultiplicative
  结论: (G) [加法群 G] [加法作用 G E]
  证明: hf.toIsQuotientMap
  continuous_const_smul g := by simpa using hf.continuous_const_vadd (Multiplicative.ofAdd.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Multiplicative.ofAdd.symm g)⟩

Depends on / 依赖: hf.toIsQuotientMap, toIsQuotientMap
-/
lemma IsAddQuotientCoveringMap.toMultiplicative (G) [AddGroup G] [AddAction G E]
    (hf : IsAddQuotientCoveringMap f G) :
    IsQuotientCoveringMap f (Multiplicative G) where
  __ := hf.toIsQuotientMap
  continuous_const_smul g := by simpa using hf.continuous_const_vadd (Multiplicative.ofAdd.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Multiplicative.ofAdd.symm g)⟩

/--
lemma `IsQuotientCoveringMap.toAdditive` / 引理 `IsQuotientCoveringMap.toAdditive`

English:
lemma IsQuotientCoveringMap.toAdditive
  statement: (G) [Group G] [MulAction G E]
  proof: hf.toIsQuotientMap
  continuous_const_vadd g := by simpa using hf.continuous_const_smul (Additive.ofMul.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Additive.ofMul.symm g)⟩

中文:
引理 是QuotientCovering映射.toAdditive
  结论: (G) [群 G] [乘法作用 G E]
  证明: hf.toIsQuotientMap
  continuous_const_vadd g := by simpa using hf.continuous_const_smul (Additive.ofMul.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Additive.ofMul.symm g)⟩

Depends on / 依赖: hf.toIsQuotientMap, toIsQuotientMap
-/
lemma IsQuotientCoveringMap.toAdditive (G) [Group G] [MulAction G E]
    (hf : IsQuotientCoveringMap f G) :
    IsAddQuotientCoveringMap f (Additive G) where
  __ := hf.toIsQuotientMap
  continuous_const_vadd g := by simpa using hf.continuous_const_smul (Additive.ofMul.symm g)
  apply_eq_iff_mem_orbit {e₁ e₂} := by simp [hf.apply_eq_iff_mem_orbit]
  disjoint e := by
    obtain ⟨U, hU, hU'⟩ := hf.disjoint e
    exact ⟨U, hU, fun g => by simpa using hU' (Additive.ofMul.symm g)⟩

namespace IsQuotientCoveringMap

/--
theorem `subgroup_congr` / 定理 `subgroup_congr`

English:
theorem subgroup_congr
  given: (S S' : Subgroup G) (eq : S = S')
  proof: by rw [eq]

中文:
定理 subgroup_congr
  条件: (S S' : 子群 G) (eq : S = S')
  证明: by rw [eq]
-/
@[to_additive] theorem subgroup_congr (S S' : Subgroup G) (eq : S = S') :
    IsQuotientCoveringMap f S ↔ IsQuotientCoveringMap f S' := by rw [eq]

variable {f G} (hf : IsQuotientCoveringMap f G)
include hf

/--
theorem `map_smul` / 定理 `map_smul`

English:
theorem map_smul
  given: (g : G) {e : E}
  statement: f (g • e) = f e
  proof: hf.apply_eq_iff_mem_orbit.mpr ⟨g, rfl⟩

中文:
定理 map_smul
  条件: (g : G) {e : E}
  结论: f (g • e) = f e
  证明: hf.apply_eq_iff_mem_orbit.mpr ⟨g, rfl⟩
-/
@[to_additive] protected theorem map_smul (g : G) {e : E} : f (g • e) = f e :=
  hf.apply_eq_iff_mem_orbit.mpr ⟨g, rfl⟩

/--
theorem `isCancelSMul` / 定理 `isCancelSMul`

English:
theorem isCancelSMul
  statement: IsCancelSMul G E where
  proof: by
    have ⟨U, heU, hU⟩ := hf.disjoint e
    simpa [inv_mul_eq_one, eq_comm] using hU (g'⁻¹ * g)
      ⟨e, ⟨e, mem_of_mem_nhds heU, by simpa [mul_smul, inv_smul_eq_iff]⟩, mem_of_mem_nhds heU⟩

中文:
定理 isCancelSMul
  结论: 是消去标量乘法 G E where
  证明: by
    have ⟨U, heU, hU⟩ := hf.disjoint e
    simpa [inv_mul_eq_one, eq_comm] using hU (g'⁻¹ * g)
      ⟨e, ⟨e, mem_of_mem_nhds heU, by simpa [mul_smul, inv_smul_eq_iff]⟩, mem_of_mem_nhds heU⟩
-/
@[to_additive] theorem isCancelSMul : IsCancelSMul G E where
  right_cancel' g g' e eq := by
    have ⟨U, heU, hU⟩ := hf.disjoint e
    simpa [inv_mul_eq_one, eq_comm] using hU (g'⁻¹ * g)
      ⟨e, ⟨e, mem_of_mem_nhds heU, by simpa [mul_smul, inv_smul_eq_iff]⟩, mem_of_mem_nhds heU⟩

/--
theorem `homeomorph_comp` / 定理 `homeomorph_comp`

English:
theorem homeomorph_comp
  statement: {Y} [TopologicalSpace Y]
  proof: φ.isQuotientMap.comp hf.toIsQuotientMap
  continuous_const_smul := hf.continuous_const_smul
  apply_eq_iff_mem_orbit := by simpa using @hf.apply_eq_iff_mem_orbit
  disjoint := hf.disjoint

omit hf in

中文:
定理 homeomorph_comp
  结论: {Y} [拓扑空间 Y]
  证明: φ.isQuotientMap.comp hf.toIsQuotientMap
  continuous_const_smul := hf.continuous_const_smul
  apply_eq_iff_mem_orbit := by simpa using @hf.apply_eq_iff_mem_orbit
  disjoint := hf.disjoint

omit hf in
-/
@[to_additive] theorem homeomorph_comp {Y} [TopologicalSpace Y]
    (φ : X ≃ₜ Y) : IsQuotientCoveringMap (φ ∘ f) G where
  __ := φ.isQuotientMap.comp hf.toIsQuotientMap
  continuous_const_smul := hf.continuous_const_smul
  apply_eq_iff_mem_orbit := by simpa using @hf.apply_eq_iff_mem_orbit
  disjoint := hf.disjoint

omit hf in
/--
theorem `homeomorph_comp_iff` / 定理 `homeomorph_comp_iff`

English:
theorem homeomorph_comp_iff
  statement: {Y} [TopologicalSpace Y]
  proof: by convert! h.homeomorph_comp φ.symm; ext; simp
  mpr h := h.homeomorph_comp φ

中文:
定理 homeomorph_comp_iff
  结论: {Y} [拓扑空间 Y]
  证明: by convert! h.homeomorph_comp φ.symm; ext; simp
  mpr h := h.homeomorph_comp φ
-/
@[to_additive (attr := simp)] theorem homeomorph_comp_iff {Y} [TopologicalSpace Y]
    (φ : X ≃ₜ Y) : IsQuotientCoveringMap (φ ∘ f) G ↔ IsQuotientCoveringMap f G where
  mp h := by convert! h.homeomorph_comp φ.symm; ext; simp
  mpr h := h.homeomorph_comp φ

/-- Fibers of a quotient covering map by a group G is a G-torsor. -/
@[to_additive /-- Fibers of a quotient covering map by an additive group G is a G-torsor. -/]
/--
Definition of `fiberEquivGroup` / `fiberEquivGroup` 的定义

English:
definition fiberEquivGroup
  signature: {x : X} (e : f ⁻¹' {x})
  body: have := hf.isCancelSMul
.symm .ofBijective (fun g => ⟨g • e, (hf.map_smul g).trans e.2⟩)
    ⟨fun _ _ eq => IsCancelSMul.right_cancel _ _ e.1 congr($eq), fun e' =>
      have ⟨g, eq⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm); ⟨g, Subtype.ext eq⟩⟩

中文:
定义 fiberEquivGroup
  签名: {x : X} (e : f ⁻¹' {x})
  定义体: have := hf.isCancelSMul
.symm .ofBijective (fun g => ⟨g • e, (hf.map_smul g).trans e.2⟩)
    ⟨fun _ _ eq => IsCancelSMul.right_cancel _ _ e.1 congr($eq), fun e' =>
      have ⟨g, eq⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm); ⟨g, Subtype.ext eq⟩⟩

Depends on / 依赖: IsCancelSMul, IsCancelSMul.right_cancel, Subtype, Subtype.ext, apply_eq_iff_mem_orbit, hf.apply_eq_iff_mem_orbit.mp, hf.isCancelSMul, hf.map_smul, isCancelSMul, map_smul, ofBijective, right_cancel
-/
noncomputable def fiberEquivGroup {x : X} (e : f ⁻¹' {x}) : f ⁻¹' {x} ≃ G :=
  have := hf.isCancelSMul
.symm .ofBijective (fun g => ⟨g • e, (hf.map_smul g).trans e.2⟩)
    ⟨fun _ _ eq => IsCancelSMul.right_cancel _ _ e.1 congr($eq), fun e' =>
      have ⟨g, eq⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm); ⟨g, Subtype.ext eq⟩⟩

/--
theorem `fiberEquivGroup_self` / 定理 `fiberEquivGroup_self`

English:
theorem fiberEquivGroup_self
  given: {x : X} (e : f ⁻¹' {x})
  statement: hf.fiberEquivGroup e e = 1
  proof: (Equiv.eq_symm_apply _).mp Subtype.ext (one_smul ..).symm

中文:
定理 fiberEquivGroup_self
  条件: {x : X} (e : f ⁻¹' {x})
  结论: hf.fiberEquivGroup e e = 1
  证明: (Equiv.eq_symm_apply _).mp Subtype.ext (one_smul ..).symm
-/
@[simp] theorem fiberEquivGroup_self {x : X} (e : f ⁻¹' {x}) : hf.fiberEquivGroup e e = 1 :=
(Equiv.eq_symm_apply _).mp Subtype.ext (one_smul ..).symm

set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `fiberEquivGroup_eq_iff` / 定理 `fiberEquivGroup_eq_iff`

English:
theorem fiberEquivGroup_eq_iff
  given: {x : X} (e e' : f ⁻¹' {x}) (g : G)
  proof: by
  rw [fiberEquivGroup]; rw [Equiv.symm_apply_eq]; rw [Equiv.ofBijective_apply]; rw [Subtype.mk.injEq]

中文:
定理 fiberEquivGroup_eq_iff
  条件: {x : X} (e e' : f ⁻¹' {x}) (g : G)
  证明: by
  rw [fiberEquivGroup]; rw [Equiv.symm_apply_eq]; rw [Equiv.ofBijective_apply]; rw [Subtype.mk.injEq]
-/
@[simp] theorem fiberEquivGroup_eq_iff {x : X} (e e' : f ⁻¹' {x}) (g : G) :
    hf.fiberEquivGroup e e' = g ↔ e' = g • (e : E) := by
  rw [fiberEquivGroup]; rw [Equiv.symm_apply_eq]; rw [Equiv.ofBijective_apply]; rw [Subtype.mk.injEq]

/--
theorem `fiberEquivGroup_smul_self` / 定理 `fiberEquivGroup_smul_self`

English:
theorem fiberEquivGroup_smul_self
  given: {x : X} (e : f ⁻¹' {x}) {e' : f ⁻¹' {x}}
  proof: congr($((hf.fiberEquivGroup e).symm_apply_apply e'))

中文:
定理 fiberEquivGroup_smul_self
  条件: {x : X} (e : f ⁻¹' {x}) {e' : f ⁻¹' {x}}
  证明: congr($((hf.fiberEquivGroup e).symm_apply_apply e'))
-/
@[simp] theorem fiberEquivGroup_smul_self {x : X} (e : f ⁻¹' {x}) {e' : f ⁻¹' {x}} :
    hf.fiberEquivGroup e e' • (e : E) = (e' : E) :=
  congr($((hf.fiberEquivGroup e).symm_apply_apply e'))

/--
Definition of `mulActionFiber` / `mulActionFiber` 的定义

English:
definition mulActionFiber
  signature: (x : X)
  body: SubMulAction.mulAction ⟨f ⁻¹' {x}, fun g _ h => (hf.map_smul g).trans h⟩

中文:
定义 mulActionFiber
  签名: (x : X)
  定义体: SubMulAction.mulAction ⟨f ⁻¹' {x}, fun g _ h => (hf.map_smul g).trans h⟩
-/
@[implicit_reducible] def mulActionFiber (x : X) : MulAction G (f ⁻¹' {x}) :=
  SubMulAction.mulAction ⟨f ⁻¹' {x}, fun g _ h => (hf.map_smul g).trans h⟩

/--
lemma `coe_mulActionFiber_smul` / 引理 `coe_mulActionFiber_smul`

English:
lemma coe_mulActionFiber_smul
  given: (x : X) (g : G) (e : f ⁻¹' {x})
  proof: hf.mulActionFiber x
    (↑(g • e) : E) = g • (e : E) :=
  rfl

中文:
引理 coe_mulActionFiber_smul
  条件: (x : X) (g : G) (e : f ⁻¹' {x})
  证明: hf.mulActionFiber x
    (↑(g • e) : E) = g • (e : E) :=
  rfl
-/
@[simp] lemma coe_mulActionFiber_smul (x : X) (g : G) (e : f ⁻¹' {x}) :
    letI := hf.mulActionFiber x
    (↑(g • e) : E) = g • (e : E) :=
  rfl

/--
lemma `mulActionFiber_isPretransitive` / 引理 `mulActionFiber_isPretransitive`

English:
lemma mulActionFiber_isPretransitive
  given: (x : X)
  proof: hf.mulActionFiber x
    MulAction.IsPretransitive G (f ⁻¹' {x}) := by
  let := hf.mulActionFiber x
  constructor
  intro e e'
  obtain ⟨g, hg⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm)
  exact ⟨g, Subtype.ext hg⟩

中文:
引理 mulActionFiber_isPretransitive
  条件: (x : X)
  证明: hf.mulActionFiber x
    MulAction.IsPretransitive G (f ⁻¹' {x}) := by
  let := hf.mulActionFiber x
  constructor
  intro e e'
  obtain ⟨g, hg⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm)
  exact ⟨g, Subtype.ext hg⟩

Depends on / 依赖: hf.mulActionFiber, mulActionFiber
-/
lemma mulActionFiber_isPretransitive (x : X) :
    letI := hf.mulActionFiber x
    MulAction.IsPretransitive G (f ⁻¹' {x}) := by
  let := hf.mulActionFiber x
  constructor
  intro e e'
  obtain ⟨g, hg⟩ := hf.apply_eq_iff_mem_orbit.mp (e'.2.trans e.2.symm)
  exact ⟨g, Subtype.ext hg⟩

/--
Definition of `toPermFiber` / `toPermFiber` 的定义

English:
definition toPermFiber
  signature: (x : X)
  body: (hf.mulActionFiber x).toPermHom

中文:
定义 toPermFiber
  签名: (x : X)
  定义体: (hf.mulActionFiber x).toPermHom
-/
@[simps!] def toPermFiber (x : X) : G ->* Equiv.Perm (f ⁻¹' {x}) :=
  (hf.mulActionFiber x).toPermHom

/--
theorem `toPermFiber_ext` / 定理 `toPermFiber_ext`

English:
theorem toPermFiber_ext
  statement: (x : X) (e : f ⁻¹' {x}) {g g' : G}
  proof: have := hf.isCancelSMul
  IsCancelSMul.right_cancel _ _ e.1 congr($eq)

中文:
定理 toPermFiber_ext
  结论: (x : X) (e : f ⁻¹' {x}) {g g' : G}
  证明: have := hf.isCancelSMul
  IsCancelSMul.right_cancel _ _ e.1 congr($eq)

Depends on / 依赖: IsCancelSMul, IsCancelSMul.right_cancel, hf.isCancelSMul, isCancelSMul, right_cancel
-/
theorem toPermFiber_ext (x : X) (e : f ⁻¹' {x}) {g g' : G}
    (eq : hf.toPermFiber x g e = hf.toPermFiber x g' e) : g = g' :=
  have := hf.isCancelSMul
  IsCancelSMul.right_cancel _ _ e.1 congr($eq)

/--
theorem `toPermFiber_injective` / 定理 `toPermFiber_injective`

English:
theorem toPermFiber_injective
  given: (x : X)
  statement: Function.Injective (hf.toPermFiber x)
  proof: have ⟨e, he⟩ := hf.surjective x
  fun _ _ eq => hf.toPermFiber_ext x ⟨e, he⟩ congr($eq _)

中文:
定理 toPermFiber_injective
  条件: (x : X)
  结论: 函数.单射 (hf.toPermFiber x)
  证明: have ⟨e, he⟩ := hf.surjective x
  fun _ _ eq => hf.toPermFiber_ext x ⟨e, he⟩ congr($eq _)

Depends on / 依赖: hf.surjective, hf.toPermFiber_ext, surjective, toPermFiber_ext
-/
theorem toPermFiber_injective (x : X) : Function.Injective (hf.toPermFiber x) :=
  have ⟨e, he⟩ := hf.surjective x
  fun _ _ eq => hf.toPermFiber_ext x ⟨e, he⟩ congr($eq _)

/--
theorem `exists_toPermFiber_eq` / 定理 `exists_toPermFiber_eq`

English:
theorem exists_toPermFiber_eq
  given: {x : X} (e e' : f ⁻¹' {x})
  statement: exists g, hf.toPermFiber x g e = e'
  proof: by
  let := hf.mulActionFiber x
  have := hf.mulActionFiber_isPretransitive x
  obtain ⟨g, rfl⟩ := MulAction.IsPretransitive.exists_smul_eq e e' (M := G)
  use g
  ext
  simp

中文:
定理 存在_toPermFiber_eq
  条件: {x : X} (e e' : f ⁻¹' {x})
  结论: 存在 g, hf.toPermFiber x g e = e'
  证明: by
  let := hf.mulActionFiber x
  have := hf.mulActionFiber_isPretransitive x
  obtain ⟨g, rfl⟩ := MulAction.IsPretransitive.exists_smul_eq e e' (M := G)
  use g
  ext
  simp

Depends on / 依赖: IsPretransitive, MulAction, MulAction.IsPretransitive.exists_smul_eq, exists_smul_eq, hf.mulActionFiber, hf.mulActionFiber_isPretransitive, mulActionFiber, mulActionFiber_isPretransitive
-/
theorem exists_toPermFiber_eq {x : X} (e e' : f ⁻¹' {x}) : exists g, hf.toPermFiber x g e = e' := by
  let := hf.mulActionFiber x
  have := hf.mulActionFiber_isPretransitive x
  obtain ⟨g, rfl⟩ := MulAction.IsPretransitive.exists_smul_eq e e' (M := G)
  use g
  ext
  simp

end IsQuotientCoveringMap

namespace Topology.IsQuotientMap

variable {f G} (hf : IsQuotientMap f)
include hf

section MulAction

open Bundle

variable [ContinuousConstSMul G E]
variable (hfG : forall {e₁ e₂}, f e₁ = f e₂ ↔ e₁ in MulAction.orbit G e₂)
include hfG

/-- If a group `G` acts on a space `E` and `U` is an open subset disjoint from all other
`G`-translates of itself, and `p` is a quotient map by this action, then `p` admits a
`Bundle.Trivialization` over the base set `p(U)`. -/
@[to_additive (attr := simps! source target baseSet)
/-- If a group `G` acts on a space `E` and `U` is an open subset disjoint from all
other `G`-translates of itself, and `p` is a quotient map by this action, then `p` admits a
`Bundle.Trivialization` over the base set `p(U)`. -/]
/--
Definition of `trivializationOfSMulDisjoint` / `trivializationOfSMulDisjoint` 的定义

English:
definition trivializationOfSMulDisjoint
  signature: [TopologicalSpace G] [DiscreteTopology G]
  body: by
  have pGE (g : G) e : f (g • e) = f e := hfG.mpr ⟨g, rfl⟩
  have preim_im : f ⁻¹' f '' U = ⋃ g : G, (g • ·) ⁻¹' U := by
    ext e; refine ⟨fun ⟨e', heU, he⟩ => ?_, ?_⟩
    · obtain ⟨g, rfl⟩ := hfG.mp he; exact ⟨_, ⟨g, rfl⟩, heU⟩
    · intro ⟨_, ⟨g, rfl⟩, hg⟩; exact ⟨_, hg, pGE g e⟩
  have : Nonempty (X -> E) := ⟨Function.surjInv hf.surjective⟩
  refine IsOpen.trivializationDiscrete (fun g => (g • ·) ⁻¹' U) (f '' U)
    ?_ (fun g W hWU => ⟨fun hoW => (hoW.preimage hf.continuous).inter (open_U.preimage <|
      continuous_const_smul g), fun isOpen => hf.isOpen_preimage.mp ?_⟩) (fun g e₁ h₁ e₂ h₂ he => ?_)
    ?_ (fun {g₁ g₂} hne => disjoint_iff_inf_le.mpr fun e ⟨h₁, h₂⟩ => hne <|
      mul_inv_eq_one.mp (disjoint _ ⟨_, ⟨_, h₂, ?_⟩, h₁⟩)) preim_im.subset
  · rw [← hf.isOpen_preimage, preim_im]
    exact isOpen_iUnion fun g => open_U.preimage (continuous_const_smul g)
  · convert! isOpen_iUnion fun g : G => isOpen.preimage (continuous_const_smul g)
    ext e; refine ⟨fun hW => ?_, ?_⟩
    · have ⟨e', he', hfe⟩ := hWU hW
      obtain ⟨g', rfl⟩ := hfG.mp hfe
      refine ⟨_, ⟨g⁻¹ * g', rfl⟩, ?_, ?_⟩
      · apply Set.mem_of_eq_of_mem (pGE _ e) hW
      · apply Set.mem_of_eq_of_mem _ he'; simp_rw [mul_smul, smul_inv_smul]
    · rintro ⟨_, ⟨g, rfl⟩, hW, -⟩; apply Set.mem_of_eq_of_mem (pGE _ e).symm hW
  · rw [← pGE g, ← pGE g e₂] at he
    have ⟨g', he⟩ := hfG.mp he
    rw [← smul_left_cancel_iff g]; rw [← he]; rw [disjoint g' ⟨_]; rw [⟨_]; rw [h₂]; rw [he⟩]; rw [h₁⟩]
    apply one_smul
  · rintro g x ⟨e, hU, rfl⟩; exact ⟨g⁻¹ • e, by apply (smul_inv_smul g e).symm ▸ hU, pGE _ e⟩
  · simp_rw [mul_smul, inv_smul_smul]

中文:
定义 trivializationOfSMulDisjoint
  签名: [拓扑空间 G] [离散拓扑 G]
  定义体: by
  have pGE (g : G) e : f (g • e) = f e := hfG.mpr ⟨g, rfl⟩
  have preim_im : f ⁻¹' f '' U = ⋃ g : G, (g • ·) ⁻¹' U := by
    ext e; refine ⟨fun ⟨e', heU, he⟩ => ?_, ?_⟩
    · obtain ⟨g, rfl⟩ := hfG.mp he; exact ⟨_, ⟨g, rfl⟩, heU⟩
    · intro ⟨_, ⟨g, rfl⟩, hg⟩; exact ⟨_, hg, pGE g e⟩
  have : Nonempty (X -> E) := ⟨Function.surjInv hf.surjective⟩
  refine IsOpen.trivializationDiscrete (fun g => (g • ·) ⁻¹' U) (f '' U)
    ?_ (fun g W hWU => ⟨fun hoW => (hoW.preimage hf.continuous).inter (open_U.preimage <|
      continuous_const_smul g), fun isOpen => hf.isOpen_preimage.mp ?_⟩) (fun g e₁ h₁ e₂ h₂ he => ?_)
    ?_ (fun {g₁ g₂} hne => disjoint_iff_inf_le.mpr fun e ⟨h₁, h₂⟩ => hne <|
      mul_inv_eq_one.mp (disjoint _ ⟨_, ⟨_, h₂, ?_⟩, h₁⟩)) preim_im.subset
  · rw [← hf.isOpen_preimage, preim_im]
    exact isOpen_iUnion fun g => open_U.preimage (continuous_const_smul g)
  · convert! isOpen_iUnion fun g : G => isOpen.preimage (continuous_const_smul g)
    ext e; refine ⟨fun hW => ?_, ?_⟩
    · have ⟨e', he', hfe⟩ := hWU hW
      obtain ⟨g', rfl⟩ := hfG.mp hfe
      refine ⟨_, ⟨g⁻¹ * g', rfl⟩, ?_, ?_⟩
      · apply Set.mem_of_eq_of_mem (pGE _ e) hW
      · apply Set.mem_of_eq_of_mem _ he'; simp_rw [mul_smul, smul_inv_smul]
    · rintro ⟨_, ⟨g, rfl⟩, hW, -⟩; apply Set.mem_of_eq_of_mem (pGE _ e).symm hW
  · rw [← pGE g, ← pGE g e₂] at he
    have ⟨g', he⟩ := hfG.mp he
    rw [← smul_left_cancel_iff g]; rw [← he]; rw [disjoint g' ⟨_]; rw [⟨_]; rw [h₂]; rw [he⟩]; rw [h₁⟩]
    apply one_smul
  · rintro g x ⟨e, hU, rfl⟩; exact ⟨g⁻¹ • e, by apply (smul_inv_smul g e).symm ▸ hU, pGE _ e⟩
  · simp_rw [mul_smul, inv_smul_smul]

Depends on / 依赖: Function, Function.surjInv, IsOpen, IsOpen.trivializationDiscrete, Nonempty, continuous, continuous_, hf.continuous, hf.surjective, hfG.mp, hfG.mpr, hoW.preimage, open_U, open_U.preimage, preim_im, preimage, surjInv, surjective, trivializationDiscrete
-/
noncomputable def trivializationOfSMulDisjoint [TopologicalSpace G] [DiscreteTopology G]
    (U : Set E) (open_U : IsOpen U) (disjoint : forall g : G, ((g • ·) '' U inter U).Nonempty -> g = 1) :
    Trivialization G f := by
  have pGE (g : G) e : f (g • e) = f e := hfG.mpr ⟨g, rfl⟩
  have preim_im : f ⁻¹' f '' U = ⋃ g : G, (g • ·) ⁻¹' U := by
    ext e; refine ⟨fun ⟨e', heU, he⟩ => ?_, ?_⟩
    · obtain ⟨g, rfl⟩ := hfG.mp he; exact ⟨_, ⟨g, rfl⟩, heU⟩
    · intro ⟨_, ⟨g, rfl⟩, hg⟩; exact ⟨_, hg, pGE g e⟩
  have : Nonempty (X -> E) := ⟨Function.surjInv hf.surjective⟩
  refine IsOpen.trivializationDiscrete (fun g => (g • ·) ⁻¹' U) (f '' U)
    ?_ (fun g W hWU => ⟨fun hoW => (hoW.preimage hf.continuous).inter (open_U.preimage <|
      continuous_const_smul g), fun isOpen => hf.isOpen_preimage.mp ?_⟩) (fun g e₁ h₁ e₂ h₂ he => ?_)
    ?_ (fun {g₁ g₂} hne => disjoint_iff_inf_le.mpr fun e ⟨h₁, h₂⟩ => hne <|
      mul_inv_eq_one.mp (disjoint _ ⟨_, ⟨_, h₂, ?_⟩, h₁⟩)) preim_im.subset
  · rw [← hf.isOpen_preimage, preim_im]
    exact isOpen_iUnion fun g => open_U.preimage (continuous_const_smul g)
  · convert! isOpen_iUnion fun g : G => isOpen.preimage (continuous_const_smul g)
    ext e; refine ⟨fun hW => ?_, ?_⟩
    · have ⟨e', he', hfe⟩ := hWU hW
      obtain ⟨g', rfl⟩ := hfG.mp hfe
      refine ⟨_, ⟨g⁻¹ * g', rfl⟩, ?_, ?_⟩
      · apply Set.mem_of_eq_of_mem (pGE _ e) hW
      · apply Set.mem_of_eq_of_mem _ he'; simp_rw [mul_smul, smul_inv_smul]
    · rintro ⟨_, ⟨g, rfl⟩, hW, -⟩; apply Set.mem_of_eq_of_mem (pGE _ e).symm hW
  · rw [← pGE g, ← pGE g e₂] at he
    have ⟨g', he⟩ := hfG.mp he
    rw [← smul_left_cancel_iff g]; rw [← he]; rw [disjoint g' ⟨_]; rw [⟨_]; rw [h₂]; rw [he⟩]; rw [h₁⟩]
    apply one_smul
  · rintro g x ⟨e, hU, rfl⟩; exact ⟨g⁻¹ • e, by apply (smul_inv_smul g e).symm ▸ hU, pGE _ e⟩
  · simp_rw [mul_smul, inv_smul_smul]

/--
lemma `isCoveringMapOn_of_smul_disjoint` / 引理 `isCoveringMapOn_of_smul_disjoint`

English:
lemma isCoveringMapOn_of_smul_disjoint
  proof: by
  let : TopologicalSpace G := ⊥; have : DiscreteTopology G := ⟨rfl⟩
  suffices forall x in f '' {e | MulAction.stabilizer G e = ⊥}, exists t : Trivialization G f, x in t.baseSet by
    choose t ht using this; exact IsCoveringMapOn.mk _ _ _ _ fun x => ht x x.2
  rintro x ⟨e, he, rfl⟩
  have ⟨U, heU, hU⟩ := disjoint e
  refine ⟨hf.trivializationOfSMulDisjoint hfG (interior U) isOpen_interior
    fun g hg => ?_, e, mem_interior_iff_mem_nhds.mpr heU, rfl⟩
  rw [← Subgroup.mem_bot]; rw [← he]
  exact hU _ (hg.mono (by grw [interior_subset]))

中文:
引理 isCoveringMapOn_of_smul_disjoint
  证明: by
  let : TopologicalSpace G := ⊥; have : DiscreteTopology G := ⟨rfl⟩
  suffices forall x in f '' {e | MulAction.stabilizer G e = ⊥}, exists t : Trivialization G f, x in t.baseSet by
    choose t ht using this; exact IsCoveringMapOn.mk _ _ _ _ fun x => ht x x.2
  rintro x ⟨e, he, rfl⟩
  have ⟨U, heU, hU⟩ := disjoint e
  refine ⟨hf.trivializationOfSMulDisjoint hfG (interior U) isOpen_interior
    fun g hg => ?_, e, mem_interior_iff_mem_nhds.mpr heU, rfl⟩
  rw [← Subgroup.mem_bot]; rw [← he]
  exact hU _ (hg.mono (by grw [interior_subset]))
-/
@[to_additive] lemma isCoveringMapOn_of_smul_disjoint
    (disjoint : forall e : E, exists U in 𝓝 e, forall g : G, ((g • ·) '' U inter U).Nonempty -> g • e = e) :
    IsCoveringMapOn f (f '' {e | MulAction.stabilizer G e = ⊥}) := by
  let : TopologicalSpace G := ⊥; have : DiscreteTopology G := ⟨rfl⟩
  suffices forall x in f '' {e | MulAction.stabilizer G e = ⊥}, exists t : Trivialization G f, x in t.baseSet by
    choose t ht using this; exact IsCoveringMapOn.mk _ _ _ _ fun x => ht x x.2
  rintro x ⟨e, he, rfl⟩
  have ⟨U, heU, hU⟩ := disjoint e
  refine ⟨hf.trivializationOfSMulDisjoint hfG (interior U) isOpen_interior
    fun g hg => ?_, e, mem_interior_iff_mem_nhds.mpr heU, rfl⟩
  rw [← Subgroup.mem_bot]; rw [← he]
  exact hU _ (hg.mono (by grw [interior_subset]))

section ProperlyDiscontinuousSMul

variable [ProperlyDiscontinuousSMul G E] [LocallyCompactSpace E] [T2Space E]

/--
lemma `isCoveringMapOn_of_properlyDiscontinuousSMul` / 引理 `isCoveringMapOn_of_properlyDiscontinuousSMul`

English:
lemma isCoveringMapOn_of_properlyDiscontinuousSMul
  proof: hf.isCoveringMapOn_of_smul_disjoint hfG
    (ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G)

中文:
引理 isCoveringMapOn_of_properlyDiscontinuousSMul
  证明: hf.isCoveringMapOn_of_smul_disjoint hfG
    (ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G)
-/
@[to_additive] lemma isCoveringMapOn_of_properlyDiscontinuousSMul :
    IsCoveringMapOn f (f '' {e | MulAction.stabilizer G e = ⊥}) :=
  hf.isCoveringMapOn_of_smul_disjoint hfG
    (ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G)

/--
lemma `isQuotientCoveringMap_of_properlyDiscontinuousSMul` / 引理 `isQuotientCoveringMap_of_properlyDiscontinuousSMul`

English:
lemma isQuotientCoveringMap_of_properlyDiscontinuousSMul
  given: [IsCancelSMul G E]
  proof: hf
  apply_eq_iff_mem_orbit := hfG
  disjoint e :=
    have ⟨U, heU, hU⟩ := ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G e
    ⟨U, heU, fun g hg => isCancelSMul_iff_eq_one_of_smul_eq.mp ‹_› _ _ (hU g hg)⟩

omit hf hfG

中文:
引理 isQuotientCoveringMap_of_properlyDiscontinuousSMul
  条件: [是消去标量乘法 G E]
  证明: hf
  apply_eq_iff_mem_orbit := hfG
  disjoint e :=
    have ⟨U, heU, hU⟩ := ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G e
    ⟨U, heU, fun g hg => isCancelSMul_iff_eq_one_of_smul_eq.mp ‹_› _ _ (hU g hg)⟩

omit hf hfG
-/
@[to_additive] lemma isQuotientCoveringMap_of_properlyDiscontinuousSMul [IsCancelSMul G E] :
    IsQuotientCoveringMap f G where
  __ := hf
  apply_eq_iff_mem_orbit := hfG
  disjoint e :=
    have ⟨U, heU, hU⟩ := ProperlyDiscontinuousSMul.exists_nhds_image_smul_eq_self G e
    ⟨U, heU, fun g hg => isCancelSMul_iff_eq_one_of_smul_eq.mp ‹_› _ _ (hU g hg)⟩

omit hf hfG

/--
lemma `_root_.isCoveringMapOn_quotientMk_of_properlyDiscontinuousSMul` / 引理 `_root_.isCoveringMapOn_quotientMk_of_properlyDiscontinuousSMul`

English:
lemma _root_.isCoveringMapOn_quotientMk_of_properlyDiscontinuousSMul
  proof: isQuotientMap_quotient_mk'.isCoveringMapOn_of_properlyDiscontinuousSMul Quotient.eq''

中文:
引理 _root_.isCoveringMapOn_quotientMk_of_properlyDiscontinuousSMul
  证明: isQuotientMap_quotient_mk'.isCoveringMapOn_of_properlyDiscontinuousSMul Quotient.eq''
-/
@[to_additive] lemma _root_.isCoveringMapOn_quotientMk_of_properlyDiscontinuousSMul :
IsCoveringMapOn (Quotient.mk _)
      (Quotient.mk <| MulAction.orbitRel G E) '' {e | MulAction.stabilizer G e = ⊥} :=
  isQuotientMap_quotient_mk'.isCoveringMapOn_of_properlyDiscontinuousSMul Quotient.eq''

/--
lemma `_root_.isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul` / 引理 `_root_.isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul`

English:
lemma _root_.isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul
  proof: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_properlyDiscontinuousSMul Quotient.eq''

中文:
引理 _root_.isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul
  证明: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_properlyDiscontinuousSMul Quotient.eq''
-/
@[to_additive] lemma _root_.isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul
    [IsCancelSMul G E] : IsQuotientCoveringMap (Quotient.mk <| MulAction.orbitRel G E) G :=
  isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_properlyDiscontinuousSMul Quotient.eq''

end ProperlyDiscontinuousSMul

end MulAction

/--
lemma `isQuotientCoveringMap_of_subgroup` / 引理 `isQuotientCoveringMap_of_subgroup`

English:
lemma isQuotientCoveringMap_of_subgroup
  statement: [Group E] [IsTopologicalGroup E]
  proof: hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.rightRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulLeft_inter_nonempty
⟨_, mul_singleton_mem_nhds_of_nhds_one e hU, fun s hs => Subtype.ext disj _ s.2 by
      obtain ⟨_, ⟨_, ⟨x, hx, _, rfl, rfl⟩, rfl⟩, y, hy, g, rfl, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_right_cancel ((mul_assoc ..).trans he.symm)⟩, hy⟩⟩

中文:
引理 isQuotientCoveringMap_of_subgroup
  结论: [群 E] [是拓扑群 E]
  证明: hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.rightRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulLeft_inter_nonempty
⟨_, mul_singleton_mem_nhds_of_nhds_one e hU, fun s hs => Subtype.ext disj _ s.2 by
      obtain ⟨_, ⟨_, ⟨x, hx, _, rfl, rfl⟩, rfl⟩, y, hy, g, rfl, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_right_cancel ((mul_assoc ..).trans he.symm)⟩, hy⟩⟩
-/
@[to_additive] lemma isQuotientCoveringMap_of_subgroup [Group E] [IsTopologicalGroup E]
    (G : Subgroup E) (hG : IsDiscrete (G : Set E)) (hfG : forall {e₁ e₂}, f e₁ = f e₂ ↔ e₂ * e₁⁻¹ in G) :
    IsQuotientCoveringMap f G where
  __ := hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.rightRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulLeft_inter_nonempty
⟨_, mul_singleton_mem_nhds_of_nhds_one e hU, fun s hs => Subtype.ext disj _ s.2 by
      obtain ⟨_, ⟨_, ⟨x, hx, _, rfl, rfl⟩, rfl⟩, y, hy, g, rfl, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_right_cancel ((mul_assoc ..).trans he.symm)⟩, hy⟩⟩

/--
lemma `isQuotientCoveringMap_of_subgroupOp` / 引理 `isQuotientCoveringMap_of_subgroupOp`

English:
lemma isQuotientCoveringMap_of_subgroupOp
  statement: [Group E] [IsTopologicalGroup E]
  proof: hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.leftRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulRight_inter_nonempty
⟨_, singleton_mul_mem_nhds_of_nhds_one e hU, fun ⟨⟨s⟩, hS⟩ hs => Subtype.ext
MulOpposite.unop_injective disj _ hS by
      obtain ⟨_, ⟨_, ⟨_, rfl, x, hx, rfl⟩, rfl⟩, g, rfl, y, hy, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_left_cancel (he.trans <| mul_assoc ..).symm⟩, hy⟩⟩

omit hf in

中文:
引理 isQuotientCoveringMap_of_subgroupOp
  结论: [群 E] [是拓扑群 E]
  证明: hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.leftRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulRight_inter_nonempty
⟨_, singleton_mul_mem_nhds_of_nhds_one e hU, fun ⟨⟨s⟩, hS⟩ hs => Subtype.ext
MulOpposite.unop_injective disj _ hS by
      obtain ⟨_, ⟨_, ⟨_, rfl, x, hx, rfl⟩, rfl⟩, g, rfl, y, hy, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_left_cancel (he.trans <| mul_assoc ..).symm⟩, hy⟩⟩

omit hf in
-/
@[to_additive] lemma isQuotientCoveringMap_of_subgroupOp [Group E] [IsTopologicalGroup E]
    (G : Subgroup E) (hG : IsDiscrete (G : Set E)) (hfG : forall {e₁ e₂}, f e₁ = f e₂ ↔ e₁⁻¹ * e₂ in G) :
    IsQuotientCoveringMap f G.op where
  __ := hf
  apply_eq_iff_mem_orbit := hfG.trans QuotientGroup.leftRel_apply.symm
  disjoint e := have ⟨U, hU, _, disj⟩ := hG.exists_nhds_eq_one_of_image_mulRight_inter_nonempty
⟨_, singleton_mul_mem_nhds_of_nhds_one e hU, fun ⟨⟨s⟩, hS⟩ hs => Subtype.ext
MulOpposite.unop_injective disj _ hS by
      obtain ⟨_, ⟨_, ⟨_, rfl, x, hx, rfl⟩, rfl⟩, g, rfl, y, hy, he⟩ := hs
      exact ⟨y, ⟨x, hx, mul_left_cancel (he.trans <| mul_assoc ..).symm⟩, hy⟩⟩

omit hf in
/--
lemma `isQuotientCoveringMap_of_isDiscrete_ker_monoidHom` / 引理 `isQuotientCoveringMap_of_isDiscrete_ker_monoidHom`

English:
lemma isQuotientCoveringMap_of_isDiscrete_ker_monoidHom
  statement: [Group E]
  proof: hf.isQuotientCoveringMap_of_subgroup f.ker disc fun {_ _} => by
    rw [eq_comm]; rw [← mul_inv_eq_one]; rw [← map_inv]; rw [← map_mul]; rfl

中文:
引理 isQuotientCoveringMap_of_isDiscrete_ker_monoidHom
  结论: [群 E]
  证明: hf.isQuotientCoveringMap_of_subgroup f.ker disc fun {_ _} => by
    rw [eq_comm]; rw [← mul_inv_eq_one]; rw [← map_inv]; rw [← map_mul]; rfl
-/
@[to_additive] lemma isQuotientCoveringMap_of_isDiscrete_ker_monoidHom [Group E]
    [IsTopologicalGroup E] [Group X] {f : E ->* X} (hf : IsQuotientMap f)
    (disc : IsDiscrete (f.ker : Set E)) :
    IsQuotientCoveringMap f f.ker :=
  hf.isQuotientCoveringMap_of_subgroup f.ker disc fun {_ _} => by
    rw [eq_comm]; rw [← mul_inv_eq_one]; rw [← map_inv]; rw [← map_mul]; rfl

end Topology.IsQuotientMap

/--
lemma `Subgroup.isQuotientCoveringMap` / 引理 `Subgroup.isQuotientCoveringMap`

English:
lemma Subgroup.isQuotientCoveringMap
  statement: {G} [Group G] [TopologicalSpace G]
  proof: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroupOp S hS
    Quotient.eq''.trans QuotientGroup.leftRel_apply

中文:
引理 子群.isQuotientCoveringMap
  结论: {G} [群 G] [拓扑空间 G]
  证明: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroupOp S hS
    Quotient.eq''.trans QuotientGroup.leftRel_apply
-/
@[to_additive] lemma Subgroup.isQuotientCoveringMap {G} [Group G] [TopologicalSpace G]
    [IsTopologicalGroup G] (S : Subgroup G) (hS : IsDiscrete (S : Set G)) :
    IsQuotientCoveringMap (QuotientGroup.mk (s := S)) S.op :=
isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroupOp S hS
    Quotient.eq''.trans QuotientGroup.leftRel_apply

/--
lemma `Subgroup.isQuotientCoveringMap_of_comm` / 引理 `Subgroup.isQuotientCoveringMap_of_comm`

English:
lemma Subgroup.isQuotientCoveringMap_of_comm
  statement: {G} [CommGroup G] [TopologicalSpace G]
  proof: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroup S hS Quotient.eq''.trans
QuotientGroup.leftRel_apply.trans by rw [mul_comm]

中文:
引理 子群.isQuotientCoveringMap_of_comm
  结论: {G} [交换群 G] [拓扑空间 G]
  证明: isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroup S hS Quotient.eq''.trans
QuotientGroup.leftRel_apply.trans by rw [mul_comm]
-/
@[to_additive] lemma Subgroup.isQuotientCoveringMap_of_comm {G} [CommGroup G] [TopologicalSpace G]
    [IsTopologicalGroup G] (S : Subgroup G) (hS : IsDiscrete (S : Set G)) :
    IsQuotientCoveringMap (QuotientGroup.mk (s := S)) S :=
isQuotientMap_quotient_mk'.isQuotientCoveringMap_of_subgroup S hS Quotient.eq''.trans
QuotientGroup.leftRel_apply.trans by rw [mul_comm]

namespace IsQuotientCoveringMap

/--
lemma `isCoveringMap` / 引理 `isCoveringMap`

English:
lemma isCoveringMap
  given: (h : IsQuotientCoveringMap f G)
  proof: isCoveringMap_iff_isCoveringMapOn_univ.mpr by
    have := h.toContinuousConstSMul
    convert! ← h.isCoveringMapOn_of_smul_disjoint h.apply_eq_iff_mem_orbit fun e => ?_
    · refine Set.eq_univ_of_forall fun x => ?_
      obtain ⟨e, rfl⟩ := h.surjective x
      have ⟨U, hU, hGU⟩ := h.disjoint e
      replace hU := mem_of_mem_nhds hU
      exact ⟨e, (Subgroup.eq_bot_iff_forall _).mpr fun g hg => hGU g (⟨e, ⟨e, hU, hg⟩, hU⟩), rfl⟩
    · have ⟨U, hU, hGU⟩ := h.disjoint e
      exact ⟨U, hU, fun g hg => by rw [hGU g hg, one_smul]⟩

中文:
引理 isCoveringMap
  条件: (h : 是QuotientCovering映射 f G)
  证明: isCoveringMap_iff_isCoveringMapOn_univ.mpr by
    have := h.toContinuousConstSMul
    convert! ← h.isCoveringMapOn_of_smul_disjoint h.apply_eq_iff_mem_orbit fun e => ?_
    · refine Set.eq_univ_of_forall fun x => ?_
      obtain ⟨e, rfl⟩ := h.surjective x
      have ⟨U, hU, hGU⟩ := h.disjoint e
      replace hU := mem_of_mem_nhds hU
      exact ⟨e, (Subgroup.eq_bot_iff_forall _).mpr fun g hg => hGU g (⟨e, ⟨e, hU, hg⟩, hU⟩), rfl⟩
    · have ⟨U, hU, hGU⟩ := h.disjoint e
      exact ⟨U, hU, fun g hg => by rw [hGU g hg, one_smul]⟩
-/
@[to_additive] lemma isCoveringMap (h : IsQuotientCoveringMap f G) :
    IsCoveringMap f :=
isCoveringMap_iff_isCoveringMapOn_univ.mpr by
    have := h.toContinuousConstSMul
    convert! ← h.isCoveringMapOn_of_smul_disjoint h.apply_eq_iff_mem_orbit fun e => ?_
    · refine Set.eq_univ_of_forall fun x => ?_
      obtain ⟨e, rfl⟩ := h.surjective x
      have ⟨U, hU, hGU⟩ := h.disjoint e
      replace hU := mem_of_mem_nhds hU
      exact ⟨e, (Subgroup.eq_bot_iff_forall _).mpr fun g hg => hGU g (⟨e, ⟨e, hU, hg⟩, hU⟩), rfl⟩
    · have ⟨U, hU, hGU⟩ := h.disjoint e
      exact ⟨U, hU, fun g hg => by rw [hGU g hg, one_smul]⟩

/--
theorem `isOpenQuotientMap` / 定理 `isOpenQuotientMap`

English:
theorem isOpenQuotientMap
  given: (h : IsQuotientCoveringMap f G)
  proof: h.surjective
  continuous := h.isCoveringMap.continuous
  isOpenMap := h.isCoveringMap.isOpenMap

中文:
定理 isOpenQuotientMap
  条件: (h : 是QuotientCovering映射 f G)
  证明: h.surjective
  continuous := h.isCoveringMap.continuous
  isOpenMap := h.isCoveringMap.isOpenMap
-/
@[to_additive] theorem isOpenQuotientMap (h : IsQuotientCoveringMap f G) :
    IsOpenQuotientMap f where
  surjective := h.surjective
  continuous := h.isCoveringMap.continuous
  isOpenMap := h.isCoveringMap.isOpenMap

end IsQuotientCoveringMap

/--
theorem `isQuotientCoveringMap_iff_isCoveringMap_and` / 定理 `isQuotientCoveringMap_iff_isCoveringMap_and`

English:
theorem isQuotientCoveringMap_iff_isCoveringMap_and
  proof: have := h.toContinuousConstSMul
    ⟨h.isCoveringMap, h.surjective, this, h.isCancelSMul, h.apply_eq_iff_mem_orbit⟩
  mpr h := (isQuotientCoveringMap_iff ..).mpr ⟨h.1.isQuotientMap h.2.1, h.2.2.1, h.2.2.2.2, fun e =>
    have ⟨_, U, heU, hU, hfU, H, hH⟩ := h.1 (f e)
    ⟨Subtype.val '' Prod.snd ∘ H ⁻¹' {(H ⟨e, heU⟩).2}, (hfU.isOpenMap_subtype_val _ <|
(isOpen_discrete _).preimage by fun_prop).mem_nhds ⟨⟨e, heU⟩, rfl, rfl⟩, fun g => by
      rintro ⟨_, ⟨_, ⟨x, hx, rfl⟩, rfl⟩, y, hy, eq⟩
      have := h.2.2.2.1
      apply IsCancelSMul.right_cancel _ _ x.1
      simp_rw [← eq, one_smul]
      refine congr($(H.injective <| Prod.ext (Subtype.ext ?_) <| hy.trans hx.symm))
      simp_rw [hH]
      exact h.2.2.2.2.mpr ⟨_, eq.symm⟩⟩⟩

中文:
定理 isQuotientCoveringMap_iff_isCoveringMap_and
  证明: have := h.toContinuousConstSMul
    ⟨h.isCoveringMap, h.surjective, this, h.isCancelSMul, h.apply_eq_iff_mem_orbit⟩
  mpr h := (isQuotientCoveringMap_iff ..).mpr ⟨h.1.isQuotientMap h.2.1, h.2.2.1, h.2.2.2.2, fun e =>
    have ⟨_, U, heU, hU, hfU, H, hH⟩ := h.1 (f e)
    ⟨Subtype.val '' Prod.snd ∘ H ⁻¹' {(H ⟨e, heU⟩).2}, (hfU.isOpenMap_subtype_val _ <|
(isOpen_discrete _).preimage by fun_prop).mem_nhds ⟨⟨e, heU⟩, rfl, rfl⟩, fun g => by
      rintro ⟨_, ⟨_, ⟨x, hx, rfl⟩, rfl⟩, y, hy, eq⟩
      have := h.2.2.2.1
      apply IsCancelSMul.right_cancel _ _ x.1
      simp_rw [← eq, one_smul]
      refine congr($(H.injective <| Prod.ext (Subtype.ext ?_) <| hy.trans hx.symm))
      simp_rw [hH]
      exact h.2.2.2.2.mpr ⟨_, eq.symm⟩⟩⟩
-/
@[to_additive] theorem isQuotientCoveringMap_iff_isCoveringMap_and :
    IsQuotientCoveringMap f G ↔ IsCoveringMap f ∧ f.Surjective ∧ ContinuousConstSMul G E ∧
      IsCancelSMul G E ∧ forall {e₁ e₂}, f e₁ = f e₂ ↔ e₁ in MulAction.orbit G e₂ where
  mp h := have := h.toContinuousConstSMul
    ⟨h.isCoveringMap, h.surjective, this, h.isCancelSMul, h.apply_eq_iff_mem_orbit⟩
  mpr h := (isQuotientCoveringMap_iff ..).mpr ⟨h.1.isQuotientMap h.2.1, h.2.2.1, h.2.2.2.2, fun e =>
    have ⟨_, U, heU, hU, hfU, H, hH⟩ := h.1 (f e)
    ⟨Subtype.val '' Prod.snd ∘ H ⁻¹' {(H ⟨e, heU⟩).2}, (hfU.isOpenMap_subtype_val _ <|
(isOpen_discrete _).preimage by fun_prop).mem_nhds ⟨⟨e, heU⟩, rfl, rfl⟩, fun g => by
      rintro ⟨_, ⟨_, ⟨x, hx, rfl⟩, rfl⟩, y, hy, eq⟩
      have := h.2.2.2.1
      apply IsCancelSMul.right_cancel _ _ x.1
      simp_rw [← eq, one_smul]
      refine congr($(H.injective <| Prod.ext (Subtype.ext ?_) <| hy.trans hx.symm))
      simp_rw [hH]
      exact h.2.2.2.2.mpr ⟨_, eq.symm⟩⟩⟩
