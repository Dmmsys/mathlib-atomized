...
set_option backward.isDefEq.respectTransparency false in
/--
theorem `isCommMonObj_of_isProper_of_isIntegral_tensorObj_of_isAlgClosed` / 定理 `isCommMonObj_of_isProper_of_isIntegral_tensorObj_of_isAlgClosed`

English:
theorem isCommMonObj_of_isProper_of_isIntegral_tensorObj_of_isAlgClosed
  statement: [IsAlgClosed K]
  proof: by
  let S := Spec (.of K)
  let point : S := IsLocalRing.closedPoint K
  have hpoint : IsClosed {point} := isClosed_discrete _
  have : Nonempty G.left := ⟨η[G].left point⟩
  have : IsProper (G otimes G).hom := by dsimp; infer_instance
...
Depends on / 依赖: Function, Function.surjective_to_subsingleton, G.hom, G.left, IsClosed, IsLocalRing, IsLocalRing.closedPoint, IsProper, JacobsonSpace, LocallyOfFiniteType, LocallyOfFiniteType.jacobsonSpace, Nonempty, Surjective, closedPoint, hpoint, infer_instance, isClosed_discrete, jacobsonSpace, otimes, surjective_to_subsingleton
-/
theorem isCommMonObj_of_isProper_of_isIntegral_tensorObj_of_isAlgClosed [IsAlgClosed K]
    (G : Over (Spec (.of K))) [IsProper G.hom] [IsIntegral (G otimes G).left] [GrpObj G] :
    IsCommMonObj G := by
  let S := Spec (.of K)
  let point : S := IsLocalRing.closedPoint K
  have hpoint : IsClosed {point} := isClosed_discrete _
  have : Nonempty G.left := ⟨η[G].left point⟩
  have : IsProper (G otimes G).hom := by dsimp; infer_instance
  have : JacobsonSpace (G otimes G).left := LocallyOfFiniteType.jacobsonSpace (Y := Spec _) (G otimes G).hom
  ...
  let γ : G otimes G ⟶ G otimes G := lift (fst _ _) (GrpObj.commutator _)
  ...
  ext1
  have H : γ.left '' ((fst G G).left ⁻¹' {η[G].left point}) subseteq {(lift η[G] η[G]).left point} := by
    rw [Set.image_subset_iff]; rw [← Set.sdiff_eq_empty]; rw [← Set.not_nonempty_iff_eq_empty]
    intro H
obtain ⟨c₀, ⟨hc₁, hc₂⟩, hc₃⟩ := nonempty_inter_closedPoints H by
      rw [Set.sdiff_eq_compl_inter]; rw [← Set.image_singleton]; rw [← Set.image_singleton];
    ...
    obtain ⟨⟨c, hc⟩, e⟩ := (pointEquivClosedPoint (G otimes G).hom).surjective ⟨c₀, hc₃⟩
    obtain rfl : c point = c₀ := congr(($e).1)
    let fc : 𝟙_ (Over S) ⟶ 𝟙_ (Over S) otimes G := lift (𝟙 _) (Over.homMk c hc ≫ snd G G)
...
exact hc₂ by simp [H₁, H₂, ← Scheme.Hom.comp_apply, Category.assoc, ← Over.comp_left]
  ...
  have H (x : U) : ((pullback.fst G.hom G.hom) ⁻¹' {x.1} inter Set.range ⇑γ.left).Finite := by
    ...
    rw [← this]; rw [← Set.preimage_comp]; rw [← TopCat.coe_comp]; rw [← Scheme.Hom.comp_base]; rw [morphismRestrict_ι]; rw [← Category.assoc]; rw [Scheme.Hom.comp_base (_ ≫ _) (fst G G).left]; rw [TopCat.coe_comp]; rw [Set.preimage_comp]; rw [Set.image_preimage_eq_inter_range]
    simp only [Scheme.Hom.comp_base, TopCat.coe_comp, Set.range_comp, Scheme.Opens.range_ι]
    dsimp
    rw [Set.image_preimage_eq_inter_range]; rw [Scheme.IdealSheafData.range_subschemeι]; rw [Scheme.Hom.support_ker]; rw [← Set.inter_assoc]; rw [← Set.preimage_inter]; rw [Set.singleton_inter_of_mem x.2]; rw [IsClosed.closure_eq
  ...
  · intro y hyU hy
    ...
    let xe : (G otimes G).left := (fst G G ≫ (ρ_ _).inv ≫ G ◁ η[G]).left y
    have : γ.left y = xe := by
      ...
      · let α : G otimes G ⟶ G otimes G := toUnit _ ≫ x otimesₘ 𝟙 _
        convert!
...
@[stacks 0BFD]
/--
theorem `isCommMonObj_of_isProper_of_geometricallyIntegral` / 定理 `isCommMonObj_of_isProper_of_geometricallyIntegral`
...
theorem isCommMonObj_of_isProper_of_geometricallyIntegral
  ...
  have : IsIntegral (G' otimes G').left := by dsimp [G']; infer_instance
...
Depends on / 依赖: AlgebraicClosure, CommRingCat, CommRingCat.ofHom, Functor, Functor.Monoidal, Functor.grpObjObj, GrpObj, IsIntegral, IsProper, Monoidal, Over.pullback, Spec.map, algebraMap, cancel_epi, grpObjObj, infer_instance, isCommMonObj_of_isProper_of_isIntegral_tensorObj_of_isAlgClosed, map_injective, otimes, pullback
-/
theorem isCommMonObj_of_isProper_of_geometricallyIntegral
