...
/--
theorem `isProperMap_iff_isClosedMap_filter` / 定理 `isProperMap_iff_isClosedMap_filter`

English:
theorem isProperMap_iff_isClosedMap_filter
  statement: {X : Type u} {Y : Type v} [TopologicalSpace X]
  proof: by
  constructor <;> intro H
...
theorem isProperMap_iff_isClosedMap_filter {X : Type u} {Y : Type v} [TopologicalSpace X]
  ...
  -- This is a contradiction, which completes the proof.
    rcases hx (U ×ˢ {𝒢 | Uᶜ in 𝒢}) (prod_mem_nhds hU (isOpen_setOfPred_mem.mem_nhds hUc)) with
      ⟨⟨z, 𝒢⟩, ⟨⟨hz : z in U, hz' : Uᶜ in 𝒢⟩, rfl : 𝒢 = pure z⟩⟩
    exact hz' hz

/--
theorem `isProperMap_iff_isClosedMap_ultrafilter` / 定理 `isProperMap_iff_isClosedMap_ultrafilter`
...
theorem isProperMap_iff_isClosedMap_ultrafilter {X : Type u} {Y : Type v} [TopologicalSpace X]
  ...
  · rw [isProperMap_iff_ultrafilter]
    ...
    refine ⟨x, rfl, fun U hU => Ultrafilter.compl_notMem_iff.mp fun hUc => ?_⟩
    rw [mem_closure_iff_nhds] at hx
    rcases hx (U ×ˢ {𝒢 | Uᶜ in 𝒢}) (prod_mem_nhds hU ((ultrafilter_isOpen_basic _).mem_nhds hUc))
      with ⟨⟨y, 𝒢⟩, ⟨⟨hy : y in U, hy' : Uᶜ in 𝒢⟩, rfl : 𝒢 = pure y⟩⟩
    exact hy' hy

/--
theorem `isProperMap_iff_universally_closed` / 定理 `isProperMap_iff_universally_closed`
...
Depends on / 依赖: H.continuous, H.universally_closed, continuous, isProperMap_iff_isClosedMap_ultrafilter, universally_closed
-/
theorem isProperMap_iff_universally_closed {X : Type u} {Y : Type v} [TopologicalSpace X]
    [TopologicalSpace Y] {f : X -> Y} :
    IsProperMap f ↔ Continuous f ∧ forall (Z : Type u) [TopologicalSpace Z],
