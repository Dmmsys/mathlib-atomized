...
/--
theorem `perfectlyNormalSpace_iff_forall_isClosed_preimage_zero` / 定理 `perfectlyNormalSpace_iff_forall_isClosed_preimage_zero`
...
theorem perfectlyNormalSpace_iff_forall_isClosed_preimage_zero
  proof: by
    ...
    have (n : Nat) : Disjoint s (U n)ᶜ := by
...
-/
theorem perfectlyNormalSpace_iff_forall_isClosed_preimage_zero :
    PerfectlyNormalSpace X ↔ forall s, IsClosed s -> exists f : C(X, Real), s = f ⁻¹' {0} ∧
      forall x, f x in Icc (0 : Real) 1 where
  mp h s hs := by
    -- write `s` as the intersection of a sequence of open sets `U n`
    obtain ⟨U, ho, hu⟩ := isGδ_iff_eq_iInter_nat.1 hs.isGδ
    have (n : Nat) : Disjoint s (U n)ᶜ := by
    ...
    have hsb (x : X) (n : Nat) : ‖(f n) x * (1 / 2 / 2 ^ n)‖ <= 1 / 2 / 2 ^ n := by
    ...
    · ext x
      ...
      · suffices forall n, f n x = 0 from by simp [h, this]
...
obtain ⟨i, hi⟩ := mem_iUnion.1 compl_iInter _ ▸ mem_compl (hu ▸ h)
  ...
  mpr h :=
    { normal s t hs ht hst := by
        ...
        have hsn : SeparatedNhds {(0 : Real)} {1} :=
      ...
      closed_gdelta s hs := by
        ...
        · simp only [hf, ← preimage_iInter,
            ← preimage_range_inter (s := ⋂ (n : Nat), Iio (1 / (n + 1) : Real)), inter_iInter]
          congr
          rw [eq_comm]; rw [eq_singleton_iff_unique_mem]
          refine ⟨mem_iInter_of_mem fun n => ?_, fun x h => ?_⟩
          · exact ⟨(hf ▸ hse : (f ⁻¹' {0}).Nonempty), show 0 < (1 / (n + 1) : Real) by positivity⟩
          · apply le_antisymm
