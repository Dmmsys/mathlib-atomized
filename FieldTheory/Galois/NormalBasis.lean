...
/-
**exists_linearIndependent_algEquiv_apply_of_finite** 是 Mathlib 中的一个定理，位于命名空间 ``
。
形式化陈述：exists_linearIndependent_algEquiv_apply_of_finite [Finite L] : exists x : 
L, LinearIndependent K fun σ : Gal(L/K) => σ x
该定理/引理给出了一组等式。
...
/-
**exists_linearIndependent_algEquiv_apply_of_infinite** 是 Mathlib 中的一个定理，位于命名空间 
``。
形式化陈述：exists_linearIndependent_algEquiv_apply_of_infinite [Infinite K] : exists 
x : L, LinearIndependent K fun σ : Gal(L/K) => σ x
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
