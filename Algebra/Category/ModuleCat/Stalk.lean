...
variable {C : Type*} [SmallCategory C] [IsFiltered C] (R : C ⥤ RingCat) (M : C ⥤ Ab)
    [forall i, Module (R.obj i) (M.obj i)]
    (H : forall {i j} (f : i ⟶ j) r m, M.map f (r • m) = R.map f r • M.map f m)

/-- (Implementation). The scalar multiplication function on `ColimitType`. -/
protected noncomputable
/--
Definition of `colimit.smul` / `colimit.smul` 的定义

English:
definition colimit.smul
  signature: (r : (R ⋙ forget _).ColimitType) (m : (M ⋙ forget _).ColimitType)
  body: by
...
中文:
定义 colimit.smul
  签名: (r : (R ⋙ forget _).ColimitType) (m : (M ⋙ forget _).ColimitType)
  定义体: by
  refine Quot.liftOn₂ r m (fun Ua Vb => Functor.ιColimitType _ (max Ua.1 Vb.1) <|
    letI a : R.obj (max Ua.1 Vb.1) := R.map (leftToMax Ua.1 Vb.1) Ua.2
    letI b : M.obj (max Ua.1 Vb.1) := M.map (rightToMax Ua.1 Vb.1) Vb.2
...
/--
Definition of `filteredColimitsModule` / `filteredColimitsModule` 的定义

English:
abbreviation filteredColimitsModule
  signature: : Module (RingCat.FilteredColimits.colimit R)
  body: colimit.smul R M H
...
/--
Definition of `IsColimit.module` / `IsColimit.module` 的定义

English:
abbreviation IsColimit.module
  signature: {cR : Cocone R} (hcR : IsColimit cR) {cM : Cocone M}
  body: letI := filteredColimitsModule R M H
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `IsColimit.ι_smul` / 引理 `IsColimit.ι_smul`

English:
lemma IsColimit.ι_smul
  statement: {cR : Cocone R} (hcR : IsColimit cR) {cM : Cocone M}
  proof: IsColimit.module R M H hcR hcM
    cM.ι.app i (r • m) =
...
English:
instance :
  signature: Module (R.stalk x) ↑(TopCat.Presheaf.stalk M.presheaf x)
  body: letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R).obj i)
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `germ_ringCat_smul` / 引理 `germ_ringCat_smul`

English:
lemma germ_ringCat_smul
  given: (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U))
  proof: letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R).obj i)
      (((OpenNhds.inclusion x).op ⋙ M.presheaf).obj i) := by
    dsimp; infer_instance
...
引理 germ_ringCat_smul
  条件: (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U))
  证明: letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R).obj i)
      (((OpenNhds.inclusion x).op ⋙ M.presheaf).obj i) := by
    dsimp; infer_instance
  Limits.IsColimit.ι_smul ((OpenNhds.inclusion x).op ⋙ R) ((OpenNhds.inclusion x).op ⋙ M.presheaf)
    (fun f r m => M.map_smul _ _ _)
...
-/
lemma germ_ringCat_smul (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U)) :
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `germ_smul` / 引理 `germ_smul`

English:
lemma germ_smul
  given: (x : X) (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U))
  proof: letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R ⋙ forget₂ _ RingCat).obj i)
      (((OpenNhds.inclusion x).op ⋙ M.presheaf).obj i) := by
    dsimp; infer_instance
...
中文:
引理 germ_smul
  条件: (x : X) (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U))
  证明: letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R ⋙ forget₂ _ RingCat).obj i)
      (((OpenNhds.inclusion x).op ⋙ M.presheaf).obj i) := by
    dsimp; infer_instance
  Limits.IsColimit.ι_smul ((OpenNhds.inclusion x).op ⋙ R ⋙ forget₂ _ _)
    ((OpenNhds.inclusion x).op ⋙ M.presheaf)
...
-/
lemma germ_smul (x : X) (U : Opens X) (hx : x in U) (r : R.obj (op U)) (m : M.obj (op U)) :
    TopCat.Presheaf.germ M.presheaf U x hx (r • m) =
      R.germ U x hx r • TopCat.Presheaf.germ M.presheaf U x hx m :=
  letI (i : (OpenNhds x)ᵒᵖ) : Module (((OpenNhds.inclusion x).op ⋙ R ⋙ forget₂ _ RingCat).obj i)
      (((OpenNhds.inclusion x).op ⋙ M.presheaf).obj i) := by
