...
/--
lemma `hasProjectiveDimensionLE_of_semiLinearEquiv` / 引理 `hasProjectiveDimensionLE_of_semiLinearEquiv`

English:
lemma hasProjectiveDimensionLE_of_semiLinearEquiv
  statement: (e' : M ≃ₛₗ[RingHomClass.toRingHom e] N)
  proof: by
...
lemma hasProjectiveDimensionLE_of_semiLinearEquiv (e' : M ≃ₛₗ[RingHomClass.toRingHom e] N)
    (n : Nat) [HasProjectiveDimensionLE M n] : HasProjectiveDimensionLE N n := by
  induction n generalizing M N e' with
  | zero =>
    have : HasProjectiveDimensionLE M 0 := ‹_›
    simp only [HasProjectiveDimensionLE, zero_add] at this ⊢
...
attribute [local instance] RingHomInvPair.of_ringEquiv in
/--
lemma `projectiveDimension_eq_of_semiLinearEquiv` / 引理 `projectiveDimension_eq_of_semiLinearEquiv`

English:
lemma projectiveDimension_eq_of_semiLinearEquiv
  given: (e' : M ≃ₛₗ[RingHomClass.toRingHom e] N)
  proof: by
...
/--
lemma `hasProjectiveDimensionLE_of_linearEquiv` / 引理 `hasProjectiveDimensionLE_of_linearEquiv`

English:
lemma hasProjectiveDimensionLE_of_linearEquiv
  statement: (e : M ≃ₗ[R] N)
...
lemma hasProjectiveDimensionLE_of_linearEquiv (e : M ≃ₗ[R] N)
    (n : Nat) [HasProjectiveDimensionLE M n] : HasProjectiveDimensionLE N n :=
...
/--
lemma `projectiveDimension_eq_of_linearEquiv` / 引理 `projectiveDimension_eq_of_linearEquiv`

English:
lemma projectiveDimension_eq_of_linearEquiv
  given: (e : M ≃ₗ[R] N)
  proof: #adaptation_note /-- 2026-05-20 (kmill) #13807, instances are more eager to apply, but the
  `univ_out_params` attribute for `Small` doesn't seem to restrict local instances, so the
  wrong universe levels are inferred. Added `.{v, v'}`. -/
...
/--
lemma `projectiveDimension_eq_zero_of_projective` / 引理 `projectiveDimension_eq_zero_of_projective`

English:
lemma projectiveDimension_eq_zero_of_projective
  statement: (M : ModuleCat.{v} R) [Nontrivial M]
  proof: by
...
Depends on / 依赖: ModuleCat, ModuleCat.isZero_iff_subsingleton, isZero_iff_subsingleton, not_subsingleton_iff_nontrivial, projectiveDimension_eq_zero_iff
-/
