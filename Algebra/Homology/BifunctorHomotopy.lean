...
variable {K₁ L₁ : HomologicalComplex C₁ c₁} {f₁ f₁' : K₁ ⟶ L₁} (h₁ : Homotopy f₁ f₁')
  ...
  (F : C₁ ⥤ C₂ ⥤ D) [F.Additive] [forall X₁, (F.obj X₁).Additive]
...
definition hom₁
  signature: (j j' : J)
  body: HomologicalComplex₂.totalDesc _
...
定义 hom₁
  签名: (j j' : J)
  定义体: HomologicalComplex₂.totalDesc _
    (fun i₁ i₂ _ => ComplexShape.ε₁ c₁ c₂ c (c₁.prev i₁, i₂) •
      (F.map (h₁.hom i₁ (c₁.prev i₁))).app (K₂.X i₂) ≫
      (F.obj (L₁.X (c₁.prev i₁))).map (f₂.f i₂) ≫ ιMapBifunctorOrZero L₁ L₂ F c _ _ j')

Depends on / 依赖: ComplexShape, F.map, F.obj, totalDesc
-/
...
@[reassoc]
/--
lemma `ιMapBifunctor_hom₁` / 引理 `ιMapBifunctor_hom₁`

English:
lemma ιMapBifunctor_hom₁
  statement: (i₁ i₁' : I₁) (i₂ : I₂) (j j' : J)
  proof: by
  subst h'
  simp [hom₁]

中文:
引理 ιMapBifunctor_hom₁
  结论: (i₁ i₁' : I₁) (i₂ : I₂) (j j' : J)
  证明: by
  subst h'
  simp [hom₁]
...
Depends on / 依赖: ComplexShape, F.map, F.obj, NatIso, NatIso.naturality_1, naturality_1, singleFunctorCompHomologyFunctorIso, totalDesc
-/
noncomputable def hom₂ (j j' : J) :
    (mapBifunctor K₁ K₂ F c).X j ⟶ (mapBifunctor L₁ L₂ F c).X j' :=
  HomologicalComplex₂.totalDesc _
...
Depends on / 依赖: CochainComplex, CochainComplex.singleFunctor, Q.map, Q.obj, X.exists_iso_single, e.inv, exists_iso_single, map_surjective, right_fac_of_isStrictlyLE_of_isStrictlyGE, singleFunctor
-/
lemma ιMapBifunctor_hom₂ (i₁ : I₁) (i₂ i₂' : I₂) (j j' : J)
    (h : ComplexShape.π c₁ c₂ c (i₁, i₂') = j) (h' : c₂.prev i₂' = i₂) :
...
English:
lemma zero₁
  given: (j j' : J) (h : ¬ c.Rel j' j)
  proof: by
  ext i₁ i₂ h'
  dsimp [hom₁]
  rw [comp_zero]; rw [HomologicalComplex₂.ι_totalDesc]
...
Depends on / 依赖: ComplexShape, ComplexShape.rel_, Functor, Functor.map_zero, comp_zero, map_zero, smul_zero, zero_app, zero_comp
-/
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `comm₁_aux` / 引理 `comm₁_aux`

English:
lemma comm₁_aux
  statement: {i₁ i₁' : I₁} (hi₁ : c₁.Rel i₁ i₁') {i₂ i₂' : I₂} (hi₂ : c₂.Rel i₂ i₂') (j : J)
  proof: by
  have hj' : ComplexShape.π c₁ c₂ c ⟨i₁, i₂'⟩ = j := by
    rw [← hj]; rw [← ComplexShape.next_π₂ c₁ c i₁ hi₂]; rw [ComplexShape.next_π₁ c₂ c hi₁ i₂]
  rw [HomologicalComplex₂.d₂_eq _ _ _ hi₂ _ hj']; rw [HomologicalComplex₂.d₂_eq _ _ _ hi₂ _
        (by rw [← c.next_eq' (ComplexShape.rel_π₂ c₁ c 
...
Depends on / 依赖: ComplexShape, ComplexShape.next_, ComplexShape.rel_, Linear, Linear.comp_unit, Linear.comp_units_smul, Linear.units_smul_comp, c.next_eq, comp_unit, comp_units_smul, next_eq, prev_eq, units_smul_comp
-/
lemma comm₁_aux {i₁ i₁' : I₁} (hi₁ : c₁.Rel i₁ i₁') {i₂ i₂' : I₂} (hi₂ : c₂.Rel i₂ i₂') (j : J)
  ...
  have hj' : ComplexShape.π c₁ c₂ c ⟨i₁, i₂'⟩ = j := by
    rw [← hj]; rw [← ComplexShape.next_π₂ c₁ c i₁ hi₂]; rw [ComplexShape.next_π₁ c₂ c hi₁ i₂]
  rw [HomologicalComplex₂.d₂_eq _ _ _ hi₂ _ hj']; rw [HomologicalComplex₂.d₂_eq _ _ _ hi₂ _
        (by rw [← c.next_eq' (ComplexShape.rel_π₂ c₁ c i₁' hi₂)]; rw [hj]),
...
Depends on / 依赖: AddMonoidHom, AddMonoidHom.mk, Functor, Functor.mapBifunctorHomologicalComplex_obj_obj_toGradedObject, Functor.map_add, Functor.map_comp, HomologicalCo, NatTrans, NatTrans.app_add, NatTrans.comp_app, Preadditive, Preadditive.add_comp, Preadditive.comp_add, _apply, add_comp, app_add, comp_add, comp_app, dNext_eq_dFrom_fromNext, fromNext
-/
lemma comm₁ (j : J) :
  ...
  have : forall {X Y : D} (a b c d e f : X ⟶ Y), a = c -> b = e -> f = -d ->
...
/--
Definition of `mapBifunctorMapHomotopy₁` / `mapBifunctorMapHomotopy₁` 的定义

English:
definition mapBifunctorMapHomotopy₁
  signature: :
  body: hom₁ h₁ f₂ F c
  zero := zero₁ h₁ f₂ F c
  comm := comm₁ h₁ f₂ F c
...
definition mapBifunctorMapHomotopy₂
  signature: :
  body: letI : TotalComplexShape c₂ c₁ c := TotalComplexShape.symm c₁ c₂ c
  ...
  letI H : Homotopy (mapBifunctorMap f₁ f₂ F c) (mapBifunctorMa

中文:
定义 mapBifunctorMapHomotopy₂
  签名: :
  定义体: letI : TotalComplexShape c₂ c₁ c := TotalComplexShape.symm c₁ c₂ c
  letI : TotalComplexShapeSymmetry c₁ c₂ c := TotalComplexShape.symmSymmetry c₁ c₂ c
  haveI : F.flip.Additive := { }
  haveI (X₁ : C₂) : (F.flip.obj X₁).Additive := { }
  letI H : Homotopy (mapBifunctorMap f₁ f₂ F c) (mapBifunctorMa

Depends on / 依赖: Additive, F.flip, F.flip.Additive, F.flip.obj, Homotopy, Homotopy.ofE, Homotopy.ofEq, TotalComplexShape, TotalComplexShape.symm, TotalComplexShape.symmSymmetry, TotalComplexShapeSymmetry, compLeft, compRight, mapBifunctorFlipIso, mapBifunctorMap, symmSymmetry
-/
noncomputable def mapBifunctorMapHomotopy₂ :
  ...
  haveI hom₂_eq : hom₂ f₁ h₂ F c = H.hom := by
    ...
    rw [add_zero]; rw [zero_add]; rw [ι_mapBifunctorFlipIso_inv_assoc]; rw [Linear.units_smul_comp]; rw [ιMapBifunctor_hom₁_assoc h₂ f₁ F.flip c _ i₂ i₁ j j'
        (by rw [ComplexShape.π_symm c₁ c₂ c i₁ i₂]; rw [hj]) rfl,
    ...
    · rw [ιMapBifunctorOrZero_eq _ _ _ _ _ _ _ hj',
      ...
      · congr 1
        nth_rw 2 [mul_comm]
        rw [← ComplexShape.σ_ε₂ c₁ c i₁ hi₂]; rw [mul_comm]; rw [← mul_assoc]; rw [Int.units_mul_self]; rw [one_mul]
