...
/-
**Real.sin_lt** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：sin_lt (h : 0 < x) : sin x < x
参数：h : 0 < x。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
theorem sin_lt (h : 0 < x) : sin x < x := by
  ...
  simp
/-
**Real.sin_le** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：sin_le (hx : 0 <= x) : sin x <= x
参数：hx : 0 <= x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
lemma sin_le (hx : 0 ≤ x) : sin x ≤ x := by
  ...
  · exact (sin_lt hx).le
/-
**Real.lt_sin** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：lt_sin (hx : x < 0) : x < sin x
参数：hx : x < 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
...
/-
**Real.le_sin** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：le_sin (hx : x <= 0) : x <= sin x
参数：hx : x <= 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
...
· 使用定理 `covariant_swap_add_of_covariant_add`：∀ (N : Type u_2) (r : N → N → Prop)
 [inst : AddCommSemigroup N] [CovariantClass N N (fun x1 x2 => x1 + x2) r],   Co
variantClass N N (Functio…
· 使用引理 `Real.sin_le`：sin_le (hx : 0 <= x) : sin x <= x
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
...
/-
**Real.lt_sin_mul** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：lt_sin_mul {x : Real} (hx : 0 < x) (hx' : x < 1) : x < sin (π / 2 * x)
参数：hx : 0 < x；hx' : x < 1。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `strictConcaveOn_sin_Icc`：strictConcaveOn_sin_Icc : StrictConcaveOn Real 
(Icc 0 π) sin
· 使用引理 `le_rfl`：le_rfl : a <= a
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `Real.pi_pos`：pi_pos : 0 < π
· 使用定理 `Real.pi_div_two_pos`：pi_div_two_pos : 0 < π / 2
· 使用定理 `half_le_self`：∀ {α : Type u_2} [inst : Semifield α] [inst_1 : PartialOrd
...
/-
**Real.le_sin_mul** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：le_sin_mul {x : Real} (hx : 0 <= x) (hx' : x <= 1) : x <= sin (π / 2 * x)
参数：hx : 0 <= x；hx' : x <= 1。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.mul_lt_sin** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：mul_lt_sin {x : Real} (hx : 0 < x) (hx' : x < π / 2) : 2 / π * x < sin x
参数：hx : 0 < x；hx' : x < π / 2。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `Real.lt_sin_mul`：lt_sin_mul {x : Real} (hx : 0 < x) (hx' : x < 1) : x < 
sin (π / 2 * x)
...
-/
/-
**Real.mul_le_sin** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：mul_le_sin {x : Real} (hx : 0 <= x) (hx' : x <= π / 2) : 2 / π * x <= sin 
x
参数：hx : 0 <= x；hx' : x <= π / 2。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `or_self`：∀ (p : Prop), (p ∨ p) = p
· 使用定理 `not_false_eq_true`：(¬False) = True
· 使用定理 `Real.le_sin_mul`：le_sin_mul {x : Real} (hx : 0 <= x) (hx' : x <= 1) : x 
<= sin (π / 2 * x)
...
In the range `[0, π / 2]`, we have a linear lower bound on `sin`. The other half
 is given by
`Real.sin_le`.
-/
...
/-
**Real.sin_le_mul** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：sin_le_mul (hx : -(π / 2) <= x) (hx₀ : x <= 0) : sin x <= 2 / π * x
参数：hx : -(π / 2) <= x；hx₀ : x <= 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `covariant_swap_add_of_covariant_add`：∀ (N : Type u_2) (r : N → N → Prop)
 [inst : AddCommSemigroup N] [CovariantClass N N (fun x1 x2 => x1 + x2) r],   Co
variantClass N N (Functio…
· 使用定理 `Real.mul_le_sin`：mul_le_sin {x : Real} (hx : 0 <= x) (hx' : x <= π / 2) 
: 2 / π * x <= sin x
· 使用定理 `Iff.mpr`：∀ {a b : Prop}, (a ↔ b) → b → a
· 使用定理 `neg_nonneg`：∀ {α : Type u} [inst : AddGroup α] [inst_1 : LE α] [AddLeftM
ono α] {a : α}, 0 ≤ -a ↔ a ≤ 0
...
/-
**Real.mul_abs_le_abs_sin** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：mul_abs_le_abs_sin (hx : |x| <= π / 2) : 2 / π * |x| <= |sin x|
参数：hx : |x| <= π / 2。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.sin_sq_lt_sq** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：sin_sq_lt_sq (hx : x != 0) : sin x ^ 2 < x ^ 2
参数：hx : x != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Classical.em`：∀ (p : Prop), p ∨ ¬p
...
/-
**Real.sin_sq_le_sq** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：sin_sq_le_sq : sin x ^ 2 <= x ^ 2
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用引理 `Real.sin_sq_lt_sq`：sin_sq_lt_sq (hx : x != 0) : sin x ^ 2 < x ^ 2
-/
...
/-
**Real.abs_sin_lt_abs** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：abs_sin_lt_abs (hx : x != 0) : |sin x| < |x|
参数：hx : x != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Real.abs_sin_le_abs** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：abs_sin_le_abs : |sin x| <= |x|
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用引理 `sq_le_sq`：sq_le_sq : a ^ 2 <= b ^ 2 ↔ |a| <= |b|
· 使用引理 `Real.sin_sq_le_sq`：sin_sq_le_sq : sin x ^ 2 <= x ^ 2
-/
lemma abs_sin_le_abs : |sin x| ≤ |x| := sq_le_sq.1 sin_sq_le_sq
/-
**Real.one_sub_sq_div_two_lt_cos** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：one_sub_sq_div_two_lt_cos (hx : x != 0) : 1 - x ^ 2 / 2 < cos x
参数：hx : x != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `LT.lt.trans_eq'`：∀ {α : Type u_1} {a b c : α} [inst : LT α], b < a → b =
 c → c < a
· 使用引理 `Real.sin_sq_lt_sq`：sin_sq_lt_sq (hx : x != 0) : sin x ^ 2 < x ^ 2
· 使用引理 `Mathlib.Meta.Positivity.div_ne_zero_of_ne_zero_of_pos`：div_ne_zero_of_ne
_zero_of_pos (ha : a != 0) (hb : 0 < b) : a / b != 0
...
lemma one_sub_sq_div_two_lt_cos (hx : x ≠ 0) : 1 - x ^ 2 / 2 < cos x := by
  ...
  linarith
/-
**Real.one_sub_sq_div_two_le_cos** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：one_sub_sq_div_two_le_cos : 1 - x ^ 2 / 2 <= cos x
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
· 使用定理 `Nat.instNeZeroSucc`：∀ {n : ℕ}, NeZero (n + 1)
...
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用引理 `Real.one_sub_sq_div_two_lt_cos`：one_sub_sq_div_two_lt_cos (hx : x != 0) 
: 1 - x ^ 2 / 2 < cos x
-/
...
/-
**Real.one_sub_mul_le_cos** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：one_sub_mul_le_cos (hx₀ : 0 <= x) (hx : x <= π / 2) : 1 - 2 / π * x <= cos
 x
参数：hx₀ : 0 <= x；hx : x <= π / 2。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `div_mul_div_cancel₀'`：∀ {G₀ : Type u_3} [inst : CommGroupWithZero G₀] {a
 : G₀}, a ≠ 0 → ∀ (b c : G₀), a / b * (c / a) = c / b
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `FloorSemiring.instCharZero`：∀ {α : Type u_2} [inst : Semiring α] [inst_1
 : PartialOrder α] [FloorSemiring α], CharZero α
· 使用定理 `not_false_eq_true`：(¬False) = True
...
/-
**Real.one_add_mul_le_cos** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：one_add_mul_le_cos (hx₀ : -(π / 2) <= x) (hx : x <= 0) : 1 + 2 / π * x <= 
cos x
参数：hx₀ : -(π / 2) <= x；hx : x <= 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用引理 `Real.one_sub_mul_le_cos`：one_sub_mul_le_cos (hx₀ : 0 <= x) (hx : x <= π 
/ 2) : 1 - 2 / π * x <= cos x
· 使用引理 `le_of_not_gt`：le_of_not_gt (h : ¬b < a) : a <= b
· 使用定理 `Mathlib.Tactic.Linarith.lt_irrefl`：lt_irrefl {α : Type u} [Preorder α] {
a : α} : ¬a < a
...
/-
**Real.cos_le_one_sub_mul_cos_sq** 是 Mathlib 中的一个引理，位于命名空间 `Real`。
形式化陈述：cos_le_one_sub_mul_cos_sq (hx : |x| <= π) : cos x <= 1 - 2 / π ^ 2 * x ^ 2
参数：hx : |x| <= π。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.sin_gt_sub_cube** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：sin_gt_sub_cube {x : Real} (hx : 0 < x) : x - x ^ 3 / 6 < Real.sin x
参数：hx : 0 < x。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
This inequality is tight, in that the constant 6 is best possible. -/
/-
**Real.sin_ge_sub_cube** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：sin_ge_sub_cube {x : Real} (hx : 0 <= x) : x - x ^ 3 / 6 <= Real.sin x
参数：hx : 0 <= x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.abs_sub_sin_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：abs_sub_sin_le (x : Real) : |x - Real.sin x| <= |x| ^ 3 / 6
参数：x : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.deriv_tan_sub_id** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：deriv_tan_sub_id (x : Real) (h : cos x != 0) : deriv (fun y : Real => tan 
y - y) x = 1 / cos x ^ 2 - 1
参数：x : Real；h : cos x != 0。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Real.lt_tan** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：lt_tan {x : Real} (h1 : 0 < x) (h2 : x < π / 2) : x < tan x
参数：h1 : 0 < x；h2 : x < π / 2。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
/-
**Real.le_tan** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：le_tan {x : Real} (h1 : 0 <= x) (h2 : x < π / 2) : x <= tan x
参数：h1 : 0 <= x；h2 : x < π / 2。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `Real.lt_tan`：lt_tan {x : Real} (h1 : 0 < x) (h2 : x < π / 2) : x < tan x
-/
...
/-
**Real.cos_lt_one_div_sqrt_sq_add_one** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：cos_lt_one_div_sqrt_sq_add_one {x : Real} (hx1 : -(3 * π / 2) <= x) (hx2 :
 x <= 3 * π / 2) (hx3 : x != 0) : cos x < (1 / √(x ^ 2 + 1) : Real)
参数：hx1 : -(3 * π / 2) <= x；hx2 : x <= 3 * π / 2；hx3 : x != 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
· 使用定理 `Nat.instNeZeroSucc`：∀ {n : ℕ}, NeZero (n + 1)
...
/-
**Real.cos_le_one_div_sqrt_sq_add_one** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：cos_le_one_div_sqrt_sq_add_one {x : Real} (hx1 : -(3 * π / 2) <= x) (hx2 :
 x <= 3 * π / 2) : cos x <= (1 : Real) / √(x ^ 2 + 1)
参数：hx1 : -(3 * π / 2) <= x；hx2 : x <= 3 * π / 2。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
· 使用定理 `Nat.instNeZeroSucc`：∀ {n : ℕ}, NeZero (n + 1)
...
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `Real.cos_lt_one_div_sqrt_sq_add_one`：cos_lt_one_div_sqrt_sq_add_one {x :
 Real} (hx1 : -(3 * π / 2) <= x) (hx2 : x <= 3 * π / 2) (hx3 : x != 0) : cos x <
 (1 / √(x ^ 2 + 1) : Real…
-/
...
/-
**Real.lipschitzWith_sin** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：lipschitzWith_sin : LipschitzWith 1 sin
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Real.lipschitzWith_cos** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：lipschitzWith_cos : LipschitzWith 1 cos
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `lipschitzWith_of_nnnorm_deriv_le`：∀ {𝕜 : Type u_3} {G : Type u_4} [inst 
: RCLike 𝕜] [inst_1 : NormedAddCommGroup G] [inst_2 : NormedSpace 𝕜 G] {f : 𝕜 → 
...
/-
**Real.abs_sin_sub_sin_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：abs_sin_sub_sin_le (x y : Real) : |sin x - sin y| <= |x - y|
参数：x y : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
...
· 使用定理 `one_mul`：one_mul : forall a : M, 1 * a = a
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Real.lipschitzWith_sin`：lipschitzWith_sin : LipschitzWith 1 sin
-/
theorem abs_sin_sub_sin_le (x y : ℝ) : |sin x - sin y| ≤ |x - y| := by
  simpa [edist_dist] using! lipschitzWith_sin x y
/-
**Real.abs_cos_sub_cos_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：abs_cos_sub_cos_le (x y : Real) : |cos x - cos y| <= |x - y|
参数：x y : Real。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
...
· 使用定理 `one_mul`：one_mul : forall a : M, 1 * a = a
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Real.lipschitzWith_cos`：lipschitzWith_cos : LipschitzWith 1 cos
-/
theorem abs_cos_sub_cos_le (x y : ℝ) : |cos x - cos y| ≤ |x - y| := by
  simpa [edist_dist] using! lipschitzWith_cos x y
/-
**Real.norm_exp_I_mul_ofReal_sub_one_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：norm_exp_I_mul_ofReal_sub_one_le {x : Real} : ‖.exp (.I * x) - (1 : Comple
x)‖ <= ‖x‖
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Nat.instAtLeastTwoHAddOfNat`：∀ (n : ℕ) [NeZero n], (n + 1).AtLeastTwo
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Complex.norm_exp_I_mul_ofReal_sub_one`：norm_exp_I_mul_ofReal_sub_one (x 
: Real) : ‖exp (I * x) - 1‖ = ‖2 * Real.sin (x / 2)‖
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
...
/-
**Real.enorm_exp_I_mul_ofReal_sub_one_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：enorm_exp_I_mul_ofReal_sub_one_le {x : Real} : ‖.exp (.I * x) - (1 : Compl
ex)‖ₑ <= ‖x‖ₑ
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
...
· 使用定理 `Real.norm_exp_I_mul_ofReal_sub_one_le`：norm_exp_I_mul_ofReal_sub_one_le 
{x : Real} : ‖.exp (.I * x) - (1 : Complex)‖ <= ‖x‖
-/
...
/-
**Real.nnnorm_exp_I_mul_ofReal_sub_one_le** 是 Mathlib 中的一个定理，位于命名空间 `Real`。
形式化陈述：nnnorm_exp_I_mul_ofReal_sub_one_le {x : Real} : ‖.exp (.I * x) - (1 : Comp
lex)‖₊ <= ‖x‖₊
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
...
· 使用定理 `Real.enorm_exp_I_mul_ofReal_sub_one_le`：enorm_exp_I_mul_ofReal_sub_one_l
e {x : Real} : ‖.exp (.I * x) - (1 : Complex)‖ₑ <= ‖x‖ₑ
-/
