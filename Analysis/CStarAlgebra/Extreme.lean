...
/--
theorem `isStarProjection_iff_mem_extremePoints_setOfPred_nonneg_inter_unitClosedBall` / 定理 `isStarProjection_iff_mem_extremePoints_setOfPred_nonneg_inter_unitClosedBall`
...
theorem isStarProjection_iff_mem_extremePoints_setOfPred_nonneg_inter_unitClosedBall
  ...
  · /- Suppose `e` is a star projection, and `a` and `

中文:
定理 isStarProjection_iff_mem_extremePoints_setOfPred_nonneg_inter_unitClosedBall
  证明: by
  simp only [mem_closedBall_zero_iff, mem_extremePoints_iff_left, mem_inter_iff,
    mem_ofPred_eq, and_imp]
  refine ⟨fun he => ⟨⟨he.nonneg, he.norm_le⟩,
...
Depends on / 依赖: Suppose, and_imp, closed, he.nonneg, he.norm_le, mem_closedBall_zero_iff, mem_extremePoints_iff_left, mem_inter_iff, mem_ofPred_eq, nonneg, nonnegative, norm_le, positive, projection, unitization
-/
theorem isStarProjection_iff_mem_extremePoints_setOfPred_nonneg_inter_unitClosedBall
    [PartialOrder A] [StarOrderedRing A] {e : A} :
    IsStarProjection e ↔ e in extremePoints Real ({x : A | 0 <= x} inter closedBall 0 1) := by
  simp only [mem_closedBall_zero_iff, mem_extremePoints_iff_left, mem_inter_iff,
    mem_ofPred_eq, and_imp]
  refine ⟨fun he => ⟨⟨he.nonneg, he.norm_le⟩,
    fun a ha ha1 b hb hb1 ⟨t, s, h0t, h0s, hts, hlin⟩ => ?_⟩, fun ⟨⟨h1, h2⟩, h3⟩ => ?_⟩
...
.mp (by simpa [norm_inr]) norm_le_one_iff_of_nonneg (b : A⁺¹) (by simpa)
      _ = e - e * (t • a + s • b) * e := by
  ...
  · /- Now suppose `e` is an extreme point of the nonnegative closed unit ball.
    ...
    have := calc
      0 <= (e : A⁺¹) * (2 - e) := by
.mp (by simpa [norm_inr]) have : (e : A⁺¹) <= 1 := norm_le_one_iff_of_nonneg _ (by simpa)
        apply Commute.mul_nonneg (by simpa) (by grw [sub_nonneg, this, one_le_two])
        simp [commute_iff_eq, mul_sub, sub_mul, mul_two, two_mul]
      _ = (((2 : Real) • e - e * e : A) : A⁺¹) := by simp [mul_sub, two_smul, mul_two]
    refine ⟨h3 _ (Commute.mul_nonneg h1 h1 rfl) ?_ ((2 : Real) • e - e * e) this.of_inr ?_
      ⟨2⁻¹, 2⁻¹, by simp [smul_sub, ← one_div, smul_smul]⟩, h1.isSelfAdjoint⟩
    · grw [norm_mul_le, h2, h2, one_mul]
    · rw [← norm_inr (𝕜 := Complex), norm_le_one_iff_of_nonneg _ this, ← sub_nonneg]
