...
/--
lemma `Lattice.mem_ideal_sup_principal` / 引理 `Lattice.mem_ideal_sup_principal`

English:
lemma Lattice.mem_ideal_sup_principal
  given: [Lattice α] (a b : α) (J : Ideal α)
  proof: ⟨fun ⟨j, ⟨jJ, _, ha', bja'⟩⟩ => ⟨j, jJ, le_trans bja' (sup_le_sup_left ha' j)⟩,
...
Depends on / 依赖: le_refl, le_trans, sup_le_sup_left
-/
lemma Lattice.mem_ideal_sup_principal [Lattice α] (a b : α) (J : Ideal α) :
    b in J ⊔ principal a ↔ exists j in J, b <= j ⊔ a :=
...
/--
theorem `DistribLattice.prime_ideal_of_disjoint_filter_ideal` / 定理 `DistribLattice.prime_ideal_of_disjoint_filter_ideal`

English:
theorem DistribLattice.prime_ideal_of_disjoint_filter_ideal
  statement: [DistribLattice α]
  proof: by
  -- Let S be the set of ideals containing I and disjoint from F.
  set S : Set (Set α) := { J : Set α | IsIdeal J ∧ I <= J ∧ Disjoint (F : Set α) J }
  -- Then I is in S...
  have IinS : ↑I in S := ⟨Order.Ideal.isIdeal I, by trivial⟩
  -- ...and S contains upper bounds for any non-empty chains.
  have chainub : forall c subseteq S, IsChain (· subseteq ·) c -> c.Nonempty -> exists ub in S, forall s in c, s subseteq ub := by
  ...
  have a₁J₁ : a₁ in J₁ := mem_of_subset_of_mem (le_sup_right : _ <= J ⊔ _) mem_principal_self
  have a₂J₂ : a₂ in J₂ := mem_of_subset_of_mem (le_sup_right : _ <= J ⊔ _) mem_principal_self
  ...
  let b := b₁ ⊔ b₂
  have bJ : b in J := sup_mem b₁J b₂J
  ...
  have ba₁a₂F : b ⊔ (a₁ ⊓ a₂) in F := PFilter.mem_of_le ineq (PFilter.inf_mem c₁F c₂F)
...
theorem DistribLattice.prime_ideal_of_disjoint_filter_ideal [DistribLattice α]
    ...
    exists J : Ideal α, (IsPrime J) ∧ I <= J ∧ Disjoint (F : Set α) J := by
  -- Let S be the set of ideals containing I and disjoint from F.
  set S : Set (Set α) := { J : Set α | IsIdeal J ∧ I <= J ∧ Disjoint (F : Set α) J }
  -- Then I is in S...
  have IinS : ↑I in S := ⟨Order.Ideal.isIdeal I, by trivial⟩
