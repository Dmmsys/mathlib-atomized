...
/-
**CategoryTheory.Idempotents.natTrans_eq** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheo
ry.Idempotents`。
形式化陈述：natTrans_eq {F G : Karoubi C ⥤ D} (φ : F ⟶ G) (P : Karoubi C) : φ.app P = 
F.map (decompId_i P) ≫ φ.app P.X ≫ G.map (decompId_p P)
参数：φ : F ⟶ G；P : Karoubi C。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
tegory.{v₁, u₁} C] {D : Type u₂} [inst_1 : CategoryTheory.Category.{v₂, u₂} D]  
 (self : CategoryTh…
· 使用定理 `CategoryTheory.Idempotents.Karoubi.decompId`：decompId (P : Karoubi C) : 
...
@[simps]
/-
**CategoryTheory.Idempotents.FunctorExtension₁.obj** 是 Mathlib 中的一个定义，位于命名空间 `Ca
tegoryTheory.Idempotents.FunctorExtension₁`。
形式化陈述：obj (F : C ⥤ Karoubi D) : Karoubi C ⥤ Karoubi D where obj P
参数：F : C ⥤ Karoubi D。
该定义给出了上述对象。
...
@[simps]
/-
**CategoryTheory.Idempotents.FunctorExtension₁.map** 是 Mathlib 中的一个定义，位于命名空间 `Ca
tegoryTheory.Idempotents.FunctorExtension₁`。
形式化陈述：map {F G : C ⥤ Karoubi D} (φ : F ⟶ G) : obj F ⟶ obj G where app P
参数：φ : F ⟶ G。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Idempotents.functorExtension** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.Idempotents`。
形式化陈述：functorExtension : (C ⥤ D) ⥤ Karoubi C ⥤ D
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
The natural isomorphism expressing that functors `Karoubi C ⥤ Karoubi D` obtaine
d
using `functorExtension₁` actually extend the original functors `C ⥤ Karoubi D`.
-/
...
/-
**CategoryTheory.Idempotents.KaroubiUniversal** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.Idempotents`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The counit isomorphism of the equivalence `(C ⥤ Karoubi D) ≌ (Karoubi C ⥤ Karoub
i D)`.
-/
...
**CategoryTheory.Idempotents.karoubiUniversal** 是 Mathlib 中的一个定义，位于命名空间 `Categor
yTheory.Idempotents`。
形式化陈述：karoubiUniversal : C ⥤ D ≌ Karoubi C ⥤ D
该定义给出了上述对象。
...
/-
**CategoryTheory.Idempotents.karoubiUniversal_functor_eq** 是 Mathlib 中的一个定理，位于命名
空间 `CategoryTheory.Idempotents`。
形式化陈述：karoubiUniversal_functor_eq : (karoubiUniversal C D).functor = functorExte
nsion C D
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.defeqAttrib.useBackward true in
/-
**CategoryTheory.Idempotents.whiskeringLeft_obj_preimage_app** 是 Mathlib 中的一个定理，
位于命名空间 `CategoryTheory.Idempotents`。
形式化陈述：whiskeringLeft_obj_preimage_app {F G : Karoubi C ⥤ D} (τ : toKaroubi _ ⋙ F
 ⟶ toKaroubi _ ⋙ G) (P : Karoubi C) : (((whiskeringLeft _ _ _).obj (toKaroubi _)
).preimage τ).app P = F.map P.decompId_i ≫ τ.app P.X ≫ G.map P.decompId_p
参数：τ : toKaroubi _ ⋙ F ⟶ toKaroubi _ ⋙ G；P : Karoubi C。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Functor.IsEquivalence.full`：∀ {C : Type u₁} {inst : Categ
oryTheory.Category.{v₁, u₁} C} {D : Type u₂} {inst_1 : CategoryTheory.Category.{
v₂, u₂} D}   {F : CategoryTheor…
· 使用定理 `CategoryTheory.Idempotents.instIsEquivalenceFunctorKaroubiObjWhiskeringL
eftToKaroubi`：∀ (C : Type u_1) (D : Type u_2) [inst : CategoryTheory.Category.{v
_1, u_1} C]   [inst_1 : CategoryTheory.Category.{v_2, u_2} D] [CategoryThe…
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
...
/-
**CategoryTheory.Idempotents.whiskeringLeftObjToKaroubiFullyFaithful** 是 Mathlib
 中的一个定义，位于命名空间 `CategoryTheory.Idempotents`。
形式化陈述：whiskeringLeftObjToKaroubiFullyFaithful : ((Functor.whiskeringLeft C (Karo
ubi C) D).obj (toKaroubi C)).FullyFaithful where preimage {F G} τ
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
