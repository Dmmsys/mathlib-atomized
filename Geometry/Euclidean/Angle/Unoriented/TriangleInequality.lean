...
/-
**InnerProductGeometry.ortho** 是 Mathlib 中的一个定义，位于命名空间 `InnerProductGeometry`。
形式化陈述：ortho (x y : V) : V
参数：x y : V。
该定义给出了上述对象。
...
/-
**InnerProductGeometry.ortho_eq_sub_inner** 是 Mathlib 中的一个引理，位于命名空间 `InnerProduc
tGeometry`。
形式化陈述：ortho_eq_sub_inner (x : V) {y : V} (hy : ‖y‖ = 1) : ortho y x = x - inner 
Real y x • y
参数：x : V；hy : ‖y‖ = 1。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InnerProductGeometry.inner_ortho_nonneg** 是 Mathlib 中的一个引理，位于命名空间 `InnerProduc
tGeometry`。
形式化陈述：inner_ortho_nonneg {x y : V} (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) : 0 <= ⟪x, orth
o y x⟫
参数：hx : ‖x‖ = 1；hy : ‖y‖ = 1。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InnerProductGeometry.inner_normalize_ortho** 是 Mathlib 中的一个引理，位于命名空间 `InnerPro
ductGeometry`。
形式化陈述：inner_normalize_ortho (x y : V) : ⟪y, normalize (ortho y x)⟫ = 0
参数：x y : V。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include hx hy
/-
**InnerProductGeometry.inner_normalized_ortho_sq_add_inner_sq_eq_one** 是 Mathlib
 中的一个引理，位于命名空间 `InnerProductGeometry`。
形式化陈述：inner_normalized_ortho_sq_add_inner_sq_eq_one : ⟪x, normalize (ortho y x)⟫
 ^ 2 + ⟪x, y⟫ ^ 2 = 1
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma inner_normalized_ortho_sq_add_inner_sq_eq_one :
  ...
  field_simp; exact H2
/-
**InnerProductGeometry.inner_ortho_right_eq_sin_angle** 是 Mathlib 中的一个引理，位于命名空间 
`InnerProductGeometry`。
形式化陈述：inner_ortho_right_eq_sin_angle : ⟪x, normalize (ortho y x)⟫ = Real.sin (an
gle x y)
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**InnerProductGeometry.angle_le_angle_add_angle_aux** 是 Mathlib 中的一个引理，位于命名空间 `I
nnerProductGeometry`。
形式化陈述：angle_le_angle_add_angle_aux : x = Real.cos (angle x y) • y + Real.sin (an
gle x y) • normalize (ortho y x)
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include hz in
/-
**InnerProductGeometry.angle_le_angle_add_angle_of_norm_eq_one** 是 Mathlib 中的一个引
理，位于命名空间 `InnerProductGeometry`。
形式化陈述：angle_le_angle_add_angle_of_norm_eq_one : angle x z <= angle x y + angle y
 z
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma angle_le_angle_add_angle_of_norm_eq_one : angle x z ≤ angle x y + angle y z := by
  ...
  ring
/-
**InnerProductGeometry.ortho_ne_zero_of_not_collinear** 是 Mathlib 中的一个引理，位于命名空间 
`InnerProductGeometry`。
形式化陈述：ortho_ne_zero_of_not_collinear (hxy1 : angle x y != 0) (hxy2 : angle x y !
= π) : ortho x y != 0
参数：hxy1 : angle x y != 0；hxy2 : angle x y != π。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**InnerProductGeometry.eq_of_angle_eq_zero** 是 Mathlib 中的一个引理，位于命名空间 `InnerProdu
ctGeometry`。
形式化陈述：eq_of_angle_eq_zero (hxy : angle x y = 0) : x = y
参数：hxy : angle x y = 0。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
include hz in
/-
**InnerProductGeometry.angle_expression_of_angle_eq_angle_sum** 是 Mathlib 中的一个引理
，位于命名空间 `InnerProductGeometry`。
形式化陈述：angle_expression_of_angle_eq_angle_sum : angle x z != π -> angle x z = ang
le x y + angle y z -> Real.sin (angle x z) • y = Real.sin (angle y z) • x + Real
.sin (angle x y) • z
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**InnerProductGeometry.mem_span_of_linear_combination** 是 Mathlib 中的一个引理，位于命名空间 
`InnerProductGeometry`。
形式化陈述：mem_span_of_linear_combination {x y z : V} {kx ky kz : Real>=0} (hy : ky !
= 0) (hlincomb : ky • y = kx • x + kz • z) : y in Submodule.span Real>=0 {x, z}
参数：hy : ky != 0；hlincomb : ky • y = kx • x + kz • z。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
