...
Depends on / 依赖: Set.subset_def, le_principal_iff, mem_iInf_of_mem, mem_lift, mem_ofPred, mem_of_superset, mem_principal_self, ofCore, principal_mono, subset_def, tendsto_iInf_iInf, tendsto_principal_principal, trans_lt, triangle, uniformity
-/
def ofFun [AddCommMonoid M] [PartialOrder M]
    (d : X -> X -> M) (refl : forall x, d x x = 0)
    (symm : forall x y, d x y = d y x) (triangle : forall x y z, d x z <= d x y + d y z)
    (half : forall ε > (0 : M), exists δ > (0 : M), forall x < δ, forall y < δ, x + y < ε) :
    UniformSpace X :=
  .ofCore
    { uniformity := ⨅ r > 0, 𝓟 { x | d x.1 x.2 < r }
refl := le_iInf₂ fun r hr => principal_mono.2 by simp [Set.subset_def, *]
...
/--
theorem `hasBasis_ofFun` / 定理 `hasBasis_ofFun`

English:
theorem hasBasis_ofFun
  statement: [AddCommMonoid M] [LinearOrder M]
...
Depends on / 依赖: hasBasis_biInf_principal, lt_min, lt_of_lt_of_le, min_le_left, min_le_right
-/
theorem hasBasis_ofFun [AddCommMonoid M] [LinearOrder M]
    (h₀ : exists x : M, 0 < x) (d : X -> X -> M) (refl : forall x, d x x = 0) (symm : forall x y, d x y = d y x)
...
@[instance_reducible]
/--
Definition of `ofFunOfHasBasis` / `ofFunOfHasBasis` 的定义

English:
definition ofFunOfHasBasis
  signature: [t : TopologicalSpace X] [AddCommMonoid M] [LinearOrder M]
  body: t
...
def ofFunOfHasBasis [t : TopologicalSpace X] [AddCommMonoid M] [LinearOrder M]
    ...
    (symm : forall x y, d x y = d y x) (triangle : forall x y z, d x z <= d x y + d y z)
    (half : forall ε > (0 : M), exists δ > (0 : M), forall x < δ, forall y < δ, x + y < ε)
    (basis : forall x, (𝓝 x).HasBasis (fun ε => 0 < ε) (fun ε => {y | d x y < ε})) :
