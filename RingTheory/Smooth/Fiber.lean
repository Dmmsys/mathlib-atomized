...
variable [IsLocalRing R] [IsLocalRing S] [IsLocalHom (algebraMap R S)]
  [Algebra.FormallySmooth 𝓀[R] (𝓀[R] otimes[R] S)]

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
attribute [local instance] TensorProduct.rightAlgebra in
/--
lemma `FormallySmooth.of_formallySmooth_residueField_tensor_aux` / 引理 `FormallySmooth.of_formallySmooth_residueField_tensor_aux`

English:
lemma FormallySmooth.of_formallySmooth_residueField_tensor_aux
  proof: by
  /-
...
/--
lemma `FormallySmooth.of_formallySmooth_residueField_tensor` / 引理 `FormallySmooth.of_formallySmooth_residueField_tensor`

English:
lemma FormallySmooth.of_formallySmooth_residueField_tensor
  statement: (M : Submonoid P)
  proof: by
  /-
...
/--
lemma `IsSmoothAt.of_formallySmooth_fiber` / 引理 `IsSmoothAt.of_formallySmooth_fiber`
...
/--
lemma `Smooth.of_formallySmooth_fiber` / 引理 `Smooth.of_formallySmooth_fiber`

English:
lemma Smooth.of_formallySmooth_fiber
  statement: [Algebra.FinitePresentation R S]
  proof: by
...
Depends on / 依赖: Set.eq_univ_iff_forall.mpr, asIdeal, eq_univ_iff_forall, of_formallySmooth_fiber, q.asIdeal.under, smoothLocus_eq_univ_iff, smoothLocus_eq_univ_iff.mp
-/
lemma Smooth.of_formallySmooth_fiber [Algebra.FinitePresentation R S]
    (H : forall (I : Ideal R) [I.IsPrime], FormallySmooth I.ResidueField (I.Fiber S)) :
...
@[stacks 08WD "(3) => (1)"]
/--
lemma `Etale.of_formallyUnramified_of_flat` / 引理 `Etale.of_formallyUnramified_of_flat`

English:
lemma Etale.of_formallyUnramified_of_flat
  statement: {R S : Type*} [CommRing R] [CommRing S] [Algebra R S]
  proof: have : Smooth R S := .of_formallySmooth_fiber inferInstance
...
Depends on / 依赖: CompactSpace, ParacompactSpace, Smooth, of_formallySmooth_fiber
-/
...
/--
lemma `IsEtaleAt.of_isUnramifiedAt_of_flat` / 引理 `IsEtaleAt.of_isUnramifiedAt_of_flat`

English:
lemma IsEtaleAt.of_isUnramifiedAt_of_flat
  proof: by
  obtain ⟨f, hfp, H⟩ := exists_unramified_of_isUnramifiedAt (R := R) q
  change ⟨q, ‹_›⟩ in etaleLocus R S
...
中文:
引理 IsEtaleAt.of_isUnramifiedAt_of_flat
  证明: by
  obtain ⟨f, hfp, H⟩ := exists_unramified_of_isUnramifiedAt (R := R) q
  change ⟨q, ‹_›⟩ in etaleLocus R S
  suffices Algebra.Etale R (Localization.Away f) from
...
Depends on / 依赖: Algebra, Algebra.Etale, CompactSpace, Localization, Localization.Away, ParacompactSpace, basicOpen_subset_etaleLocus_iff, etaleLocus, exists_unramified_of_isUnramifiedAt, of_formallyUnramified_of_flat
-/
lemma IsEtaleAt.of_isUnramifiedAt_of_flat
