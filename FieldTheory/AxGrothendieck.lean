...
theorem ax_grothendieck_of_locally_finite
  ...
  /- `s` is the set of all coefficients of the polynomial, as well as all of
    the coordinates of `v`, the point I am trying to find
...
theorem realize_genericPolyMapSurjOnOfInjOn
  ...
  have injOnAlt : forall {S : Set (ι -> K)} (f : (ι -> K) -> (ι -> K)),
      S.InjOn f ↔ forall x y, x in S -> y in S -> f x = f y -> x = y :=
...
theorem ACF_models_genericPolyMapSurjOnOfInjOn_of_prime
  ...
  rintro v ⟨f, _⟩
  exact ax_grothendieck_of_locally_finite (K := ZMod p) 
...
theorem ACF_models_genericPolyMapSurjOnOfInjOn_of_prime_or_zero
  ...
  · rw [ACF_zero_realize_iff_infinite_ACF_prime_realize]
    ...
    intro ⟨p, hp⟩
    exact ACF_models_genericPolyMapSurjOnOfInjO
...
theorem ax_grothendieck_zeroLocus
  ...
  exact ax_grothendieck_of_definable S (mvPolynomial_zeroL
