...
/-
**IsSlice** 是 Mathlib 中的一个定义，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
### Semilinearity from slice property

A set `s` is said to have slice property if `a + b + c ∈ s` whenever `a ∈ s`, `a
 + b ∈ s` and
`a + c ∈ s`. In `ℕ ^ k`, any set that has slice property is a semilinear set. Th
is is an important
lemma to prove the semilinearity of linear equation solutions.
-/
...
/-
**IsSlice.sep_apply_eq** 是 Mathlib 中的一个定理，位于命名空间 `IsSlice`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsSlice.exists_isSemilinearSet_setOfPred_le** 是 Mathlib 中的一个定理，位于命名空间 `IsSlice
`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_of_isSlice** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_setOfPred_eq** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_preimage_of_isLinearSet** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_preimage** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_inter_of_isLinearSet** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Nat.isSemilinearSet_inter** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**toRatVec** 是 Mathlib 中的一个定义，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
### Semilinearity of complement and set difference

We first show that the complement of a proper linear set `s` in `ℕ ^ k` is semil
inear, through
several private definitions:

1. `base`, `periods`: the base vector and the set of periods of the proper linea
r set `s`.
2. `basisSet`, `basis`: the linearly independent periods of `s` can be extended 
to a basis of
  `ℚ ^ k`.
3. `fundamentalDomain`: the set of vectors in `ℕ ^ k`, starting from `base`, wit
h coordinates under
  `basis` in `[0, 1) ^ k`.
4. `floor`, `fract`: every vector in `ℕ ^ k` can be decomposed into a `ℤ`-linear
 combination of
  `basisSet` and a vector in `fundamentalDomain`.
5. `setOfFractNe`, `setOfFloorNeg`, `setOfFloorPos`: the complement of `s` is de
composed into three
  semilinear sets.
...
/-
**toRatVec_inj** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**toRatVec_mono** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem toRatVec_mono (x y : ι → ℕ) : toRatVec x ≤ toRatVec y ↔ x ≤ y := by
  ...
  simp [toRatVec]
/-
**toRatVec_nonneg** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem toRatVec_nonneg (x : ι → ℕ) : 0 ≤ toRatVec x := by
  ...
  simp
/-
**linearIndepOn_toRatVec** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.base** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def base : ι → ℕ := hs.choose
/-
**IsProperLinearSet.periods** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def periods : Set (ι → ℕ) := hs.choose_spec.choose
/-
**IsProperLinearSet.finite_periods** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem finite_periods : hs.periods.Finite := hs.choose_spec.choose_spec.1
/-
**IsProperLinearSet.linearIndepOn_periods** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLin
earSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.eq_base_vadd_closure_periods** 是 Mathlib 中的一个定理，位于命名空间 `IsPr
operLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
variable [Finite ι]
/-
**IsProperLinearSet.basisSet** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.periods_subset_basisSet** 是 Mathlib 中的一个定理，位于命名空间 `IsProperL
inearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.basisSet_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLin
earSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.finite_basisSet** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet
`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.linearIndepOn_basisSet** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLi
nearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.span_basisSet** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.basis** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**IsProperLinearSet.basis_apply** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem basis_apply (i) : hs.basis i = toRatVec i.1 := by
  simp [basis]
/-
**IsProperLinearSet.fundamentalDomain** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearS
et`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.finite_fundamentalDomain** 是 Mathlib 中的一个定理，位于命名空间 `IsProper
LinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem finite_fundamentalDomain : hs.fundamentalDomain.Finite := by
  ...
  · exact (hx i).2.le
/-
**IsProperLinearSet.floor** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def floor (x i) :=
  ⌊hs.basis.repr (toRatVec x - toRatVec hs.base) i⌋
/-
**IsProperLinearSet.fract** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def fract (x) :=
  x + ∑ i, (-hs.floor x i).toNat • i.1 - ∑ i, (hs.floor x i).toNat • i.1
/-
**IsProperLinearSet.floor_base** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem floor_base (i) : hs.floor hs.base i = 0 := by simp [floor]
/-
**IsProperLinearSet.fract_base** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem fract_base : hs.fract hs.base = hs.base := by simp [fract, hs.floor_base]
/-
**IsProperLinearSet.floor_add_nsmul_self** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLine
arSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.floor_add_sum** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.floor_le_floor_add_of_mem_closure** 是 Mathlib 中的一个定理，位于命名空间 
`IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.floor_add_of_mem_closure** 是 Mathlib 中的一个定理，位于命名空间 `IsProper
LinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.floor_toNat_sum_le** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinear
Set`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.add_floor_neg_toNat_sum_eq** 是 Mathlib 中的一个定理，位于命名空间 `IsProp
erLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.toRatVec_fract_eq** 是 Mathlib 中的一个定理，位于命名空间 `IsProperLinearS
et`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.fract_add_of_mem_closure** 是 Mathlib 中的一个定理，位于命名空间 `IsProper
LinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**IsProperLinearSet.fract_mem_fundamentalDomain** 是 Mathlib 中的一个定理，位于命名空间 `IsPro
perLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.fract_eq_self_of_mem_fundamentalDomain** 是 Mathlib 中的一个定理，位于
命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private theorem fract_eq_self_of_mem_fundamentalDomain {x} (hx : x ∈ hs.fundamentalDomain) :
  ...
  simp
/-
**IsProperLinearSet.mem_iff_fract_eq_and_floor_nonneg** 是 Mathlib 中的一个定理，位于命名空间 
`IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.setOfFractNe** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def setOfFractNe : Set (ι → ℕ) := { x | hs.fract x ≠ hs.base }
/-
**IsProperLinearSet.isSemilinearSet_setOfFractNe** 是 Mathlib 中的一个定理，位于命名空间 `IsPr
operLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.setOfFloorNeg** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def setOfFloorNeg : Set (ι → ℕ) :=
  { x | hs.fract x = hs.base ∧ ∃ i, hs.floor x i < 0 }
/-
**IsProperLinearSet.isSemilinearSet_setOfFloorNeg** 是 Mathlib 中的一个定理，位于命名空间 `IsP
roperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**IsProperLinearSet.setOfFloorPos** 是 Mathlib 中的一个定义，位于命名空间 `IsProperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private noncomputable def setOfFloorPos : Set (ι → ℕ) :=
  { x | hs.fract x = hs.base ∧ ∃ i, i.1 ∉ hs.periods ∧ 0 < hs.floor x i }
/-
**IsProperLinearSet.isSemilinearSet_setOfFloorPos** 是 Mathlib 中的一个定理，位于命名空间 `IsP
roperLinearSet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_compl_of_isProperLinearSet** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private lemma Nat.isSemilinearSet_compl_of_isProperLinearSet [Finite ι] {s : Set (ι → ℕ)}
  ...
  grind
/-
**Nat.isSemilinearSet_compl** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Nat.isSemilinearSet_sdiff** 是 Mathlib 中的一个定理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
