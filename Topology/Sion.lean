...
/-
**Sion.sublevelLeft** 是 Mathlib 中的一个定义，位于命名空间 `Sion`。
形式化陈述：sublevelLeft (b : β) (z : F) : Set X
参数：b : β；z : F。
该定义给出了上述对象。
...
variable {X Y f}
/-
**Sion.mem_sublevelLeft_iff** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：mem_sublevelLeft_iff {b : β} {y : Y} {x : X} : x in sublevelLeft X f b y ↔
 f x y <= b
该定理/引理刻画了左右两侧的等价关系。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Sion.monotone_sublevelLeft** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：monotone_sublevelLeft {u v : β} (y : Y) (h : u <= v) : sublevelLeft X f u 
y subseteq sublevelLeft X f v y
参数：y : Y；h : u <= v。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Sion.disjoint_sublevelLeft** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：disjoint_sublevelLeft {a : E} {b : β} {y y' : Y} (ha : forall x in X, f a 
y ⊔ f a y' <= f x y ⊔ f x y') (hb : b < f a y ⊔ f a y') : Disjoint (sublevelLeft
 X f b y) (sublevelLeft X f b y')
参数：ha : forall x in X, f a y ⊔ f a y' <= f x y ⊔ f x y'；hb : b < f a y ⊔ f a y'。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Disjointness criterion for two `sublevelLeft` at different values of the paramet
er
-/
...
/-
**Sion.nonempty_sublevelLeft** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：nonempty_sublevelLeft [TopologicalSpace E] (ne_X : X.Nonempty) (kX : IsCom
pact X) (hfy : forall y in Y, LowerSemicontinuousOn (fun x : E => f x y) X) (y :
 Y) {b : β} (h : forall y in Y, exists x in X, f x y <= b) : (sublevelLeft X f b
 y).Nonempty
参数：ne_X : X.Nonempty；kX : IsCompact X；hfy : forall y in Y, LowerSemicontinuousOn
 (fun x : E => f x y) X；y : Y；h : forall y in Y, exists x in X, f x y <= b。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Sion.isClosed_sublevelLeft** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：isClosed_sublevelLeft [TopologicalSpace E] (hfy : forall y in Y, LowerSemi
continuousOn (fun x : E => f x y) X) (b : β) (y : Y) : IsClosed (sublevelLeft X 
f b y)
参数：hfy : forall y in Y, LowerSemicontinuousOn (fun x : E => f x y) X；b : β；y : Y
。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
From lower semicontinuity of `f(·, y)`, deduce that `sublevelLeft` sets are clos
ed
-/
...
/-
**Sion.isPreconnected_sublevelLeft** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：isPreconnected_sublevelLeft [TopologicalSpace E] [AddCommGroup E] [Module 
Real E] [IsTopologicalAddGroup E] [ContinuousSMul Real E] (hfy' : forall y in Y,
 QuasiconvexOn Real X (fun x => f x y)) (b : β) (y : Y) : IsPreconnected (sublev
elLeft X f b y)
参数：hfy' : forall y in Y, QuasiconvexOn Real X (fun x => f x y)；b : β；y : Y。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Sion.sublevelLeft_subset_union** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：sublevelLeft_subset_union [AddCommGroup F] [Module Real F] (hfx' : forall 
x in X, QuasiconcaveOn Real Y fun y => f x y) (b : β) (y y' : Y) (z : segment Re
al y.val y'.val) : sublevelLeft X f b z subseteq sublevelLeft X f b y union subl
evelLeft X f b y'
参数：hfx' : forall x in X, QuasiconcaveOn Real Y fun y => f x y；b : β；y y' : Y；z :
 segment Real y.val y'.val。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
From the quasi-concavity of `f (x, ·)`, deduce an inclusion of `sublevelLeft` se
ts
-/
...
/-
**Sion.sublevelLeft_subset_or** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：sublevelLeft_subset_or [TopologicalSpace E] [AddCommGroup E] [IsTopologica
lAddGroup E] [Module Real E] [ContinuousSMul Real E] [AddCommGroup F] [Module Re
al F] (hfx' : forall x in X, QuasiconcaveOn Real Y fun y => f x y) (hfy : forall
 y in Y, LowerSemicontinuousOn (fun x : E => f x y) X) (hfy' : forall y in Y, Qu
asiconvexOn Real X fun x => f x y) (cY : Convex Real Y) (a : E) (b : β) (y y' : 
Y) (ha : forall x in X, f a y ⊔ f a y' <= f x y ⊔ f x y') (hb : b < f a y ⊔ f a 
y') (z : segment Real y.va
参数：hfx' : forall x in X, QuasiconcaveOn Real Y fun y => f x y；hfy : forall y in 
Y, LowerSemicontinuousOn (fun x : E => f x y) X；hfy' : forall y in Y, Quasiconve
xOn Real X fun x => f x y；cY : Convex Real Y；a : E；b : β；y y' : Y；ha : forall x 
in X, f a y ⊔ f a y' <= f x y ⊔ f x y'；hb : b < f a y ⊔ f a y'。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Sion.setOfPred_sublevelLeft_subset** 是 Mathlib 中的一个定义，位于命名空间 `Sion`。
形式化陈述：setOfPred_sublevelLeft_subset (b b' : β) (y y' : Y) : Set (segment Real y.
val y'.val)
参数：b b' : β；y y' : Y。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Sion.isClosed_setOfPred_sublevelLeft_subset** 是 Mathlib 中的一个定理，位于命名空间 `Sion`。
形式化陈述：isClosed_setOfPred_sublevelLeft_subset (a : E) (b b' : β) (y y' : Y) (ha :
 forall x in X, f a y ⊔ f a y' <= f x y ⊔ f x y') (hb : forall y in Y, exists x 
in X, f x y <= b) (hb' : b' < f a y ⊔ f a y') (hbb' : b < b') : IsClosed (setOfP
red_sublevelLeft_subset X Y f b b' y y')
参数：a : E；b b' : β；y y' : Y；ha : forall x in X, f a y ⊔ f a y' <= f x y ⊔ f x y'；
hb : forall y in Y, exists x in X, f x y <= b；hb' : b' < f a y ⊔ f a y'；hbb' : b
 < b'。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
