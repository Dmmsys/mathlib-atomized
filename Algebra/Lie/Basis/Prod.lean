...
sets. -/
/-
**LieAlgebra.Basis.prod** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A distinguished subalgebra of the product of two based Lie algebras with equival
ent indexing
sets.
-/
protected def prod :
...
/-
**LieAlgebra.Basis.prodSymmEquiv** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prodSymmEquiv : prod eι.symm b₂ b₁ ≃ₗ⁅R⁆ b₁.prod eι b₂
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The equivalence obtained by interchanging the two bases consumed by `LieAlgebra.
Basis.prod`.
-/
...
@[simp]
/-
**LieAlgebra.Basis.prodSymmEquiv_symm_apply** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebr
a.Basis`。
形式化陈述：prodSymmEquiv_symm_apply {y : b₁.prod eι b₂} : (prodSymmEquiv eι b₁ b₂).sy
mm y = ⟨((y : L₁ × L₂).snd, (y : L₁ × L₂).fst), ((prodSymmEquiv eι b₁ b₂).symm y
).property⟩
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma prodSymmEquiv_symm_apply {y : b₁.prod eι b₂} :
  ...
  rfl
/-
**LieAlgebra.Basis.h_mem_prod** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：h_mem_prod (i : ι₁) : (b₁.h i, b₂.h (eι i)) in b₁.prod eι b₂
参数：i : ι₁。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.prodCartan** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prodCartan : LieSubalgebra R (prod eι b₁ b₂)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Subject to the assumtions demanded by `LieAlgebra.Basis.isCartanSubalgebra` this
 will be a Cartan
subalgebra of `LieAlgebra.Basis.prod`.
-/
def prodCartan :
...
open Finsupp in
/-
**LieAlgebra.Basis.prodCartanEquiv_aux** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Bas
is`。
形式化陈述：prodCartanEquiv_aux : InjOn (LinearMap.fst R L₁ L₂) (span R {(b₁.h i, b₂.h
 (eι i)) | i : ι₁})
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.prodCartanEquiv** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prodCartanEquiv : prodCartan eι b₁ b₂ ≃ₗ[R] H₁
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The Lie subalgebra `LieAlgebra.Basis.prod` is a copy of the Cartan subalgebra of
 the input.
-/
def prodCartanEquiv :
...
/-
**LieAlgebra.Basis.prodH** 是 Mathlib 中的一个缩写定义，位于命名空间 `LieAlgebra.Basis`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected abbrev prodH (i : ι₁) : b₁.prod eι b₂ :=
  ⟨(b₁.h i, b₂.h (eι i)), subset_lieSpan <| by aesop⟩
/-
**LieAlgebra.Basis.prodE** 是 Mathlib 中的一个缩写定义，位于命名空间 `LieAlgebra.Basis`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected abbrev prodE (i : ι₁) : b₁.prod eι b₂ :=
  ⟨(b₁.e i, b₂.e (eι i)), subset_lieSpan <| by aesop⟩
/-
**LieAlgebra.Basis.prodF** 是 Mathlib 中的一个缩写定义，位于命名空间 `LieAlgebra.Basis`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
protected abbrev prodF (i : ι₁) : b₁.prod eι b₂ :=
  ⟨(b₁.f i, b₂.f (eι i)), subset_lieSpan <| by aesop⟩
/-
**LieAlgebra.Basis.basisProd_aux** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：basisProd_aux : lieSpan R (prod eι b₁ b₂) (range (prodE eι b₁ b₂) union ra
nge (prodF eι b₁ b₂)) = ⊤
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.basisProd** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：basisProd (hA : b₁.A.reindex eι eι = b₂.A) : Basis ι₁ (prodCartan eι b₁ b₂
) where A
参数：hA : b₁.A.reindex eι eι = b₂.A。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**LieAlgebra.Basis.surjective_fst_prod** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Bas
is`。
形式化陈述：surjective_fst_prod : Surjective ((LieHom.fst R L₁ L₂).comp (prod eι b₁ b₂
).incl)
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma surjective_fst_prod :
  ...
  simp [range_comp]
/-
**LieAlgebra.Basis.surjective_snd_prod** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Bas
is`。
形式化陈述：surjective_snd_prod : Surjective ((LieHom.snd R L₁ L₂).comp (prod eι b₁ b₂
).incl)
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
lemma surjective_snd_prod :
  ...
  simp [range_comp]
/-
**LieAlgebra.Basis.lie_fst_eq_zero_of_mem_prodCartan** 是 Mathlib 中的一个引理，位于命名空间 `
LieAlgebra.Basis`。
形式化陈述：lie_fst_eq_zero_of_mem_prodCartan {y : b₁.prod eι b₂} (hy : y in prodCarta
n eι b₁ b₂) (x : H₁) : ⁅(y : L₁ × L₂).fst, (x : L₁)⁆ = 0
参数：hy : y in prodCartan eι b₁ b₂；x : H₁。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.lie_snd_eq_zero_of_mem_prodCartan** 是 Mathlib 中的一个引理，位于命名空间 `
LieAlgebra.Basis`。
形式化陈述：lie_snd_eq_zero_of_mem_prodCartan {y : b₁.prod eι b₂} (hy : y in prodCarta
n eι b₁ b₂) (x : H₂) : ⁅(y : L₁ × L₂).snd, (x : L₂)⁆ = 0
参数：hy : y in prodCartan eι b₁ b₂；x : H₂。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.prod_lt_top** 是 Mathlib 中的一个引理，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prod_lt_top [Nontrivial L₂] : b₁.prod eι b₂ < ⊤
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LieAlgebra.Basis.prodEquivLeft** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prodEquivLeft : b₁.prod eι b₂ ≃ₗ⁅K⁆ L₁
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**LieAlgebra.Basis.prodEquivRight** 是 Mathlib 中的一个定义，位于命名空间 `LieAlgebra.Basis`。
形式化陈述：prodEquivRight : b₁.prod eι b₂ ≃ₗ⁅K⁆ L₂
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
