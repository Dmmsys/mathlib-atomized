...
variable {p n : Nat} [NeZero n] (A : Type w) (B : Type z) (K : Type u) {L : Type v} (C : Type w)
...
definition zeta
  signature: : B
  body: (exists_isPrimitiveRoot A B (Set.mem_singleton n) (NeZero.ne _) :
    exists r : B, IsPrimitiveRoot r n).choose

中文:
定义 zeta
  签名: : B
  定义体: (exists_isPrimitiveRoot A B (Set.mem_singleton n) (NeZero.ne _) :
    exists r : B, IsPrimitiveRoot r n).choose

Depends on / 依赖: IsPrimitiveRoot, NeZero, NeZero.ne, Set.mem_singleton, exists_isPrimitiveRoot, mem_singleton
-/
...
@[simp]
/--
theorem `zeta_spec` / 定理 `zeta_spec`

English:
theorem zeta_spec
  statement: IsPrimitiveRoot (zeta n A B) n
  proof: (exists_isPrimitiveRoot A B (Set.mem_singleton n) (NeZero.ne _) :
...
Depends on / 依赖: IsPrimitiveRoot, NeZero, NeZero.ne, Set.mem_singleton, choose_spec, exists_isPrimitiveRoot, mem_singleton
-/
...
/--
theorem `aeval_zeta` / 定理 `aeval_zeta`

English:
theorem aeval_zeta
  given: [IsDomain B] [NeZero (n : B)]
  proof: by
...
Depends on / 依赖: IsRoot, IsRoot.def, eval_map_algebraMap, isRoot_cyclotomic_iff, map_cyclotomic, zeta_spec
-/
...
/--
theorem `zeta_isRoot` / 定理 `zeta_isRoot`

English:
theorem zeta_isRoot
  given: [IsDomain B] [NeZero (n : B)]
  statement: IsRoot (cyclotomic n B) (zeta n A B)
  proof: by
...
Depends on / 依赖: IsRoot, IsRoot.def, aeval_def, aeval_zeta, convert, map_cyclotomic
-/
...
/--
theorem `zeta_pow` / 定理 `zeta_pow`

English:
theorem zeta_pow
  statement: zeta n A B ^ n = 1
  proof: (zeta_spec n A B).pow_eq_one

中文:
定理 zeta_pow
  结论: zeta n A B ^ n = 1
  证明: (zeta_spec n A B).pow_eq_one

Depends on / 依赖: pow_eq_one, zeta_spec
-/
...
definition noncomputable
  signature: def powerBasis
  body: -- this is purely an optimization
...
定义 noncomputable
  签名: def powerBasis
  定义体: -- this is purely an optimization
letI pb := Algebra.adjoin.powerBasis (integral {n} K L).isIntegral ζ
pb.map (Subalgebra.equivOfEq _ _ (IsCyclotomicExtension.adjoin_primitive_root_eq_top hζ)).trans
    Subalgebra.topEquiv
-/
...
/--
theorem `powerBasis_gen_mem_adjoin_zeta_sub_one` / 定理 `powerBasis_gen_mem_adjoin_zeta_sub_one`
...
中文:
定理 powerBasis_gen_mem_adjoin_zeta_sub_one
  证明: by
  rw [powerBasis_gen]; rw [adjoin_singleton_eq_range_aeval]; rw [AlgHom.mem_range]
  exact ⟨X + 1, by simp⟩

Depends on / 依赖: AlgHom, AlgHom.mem_range, adjoin_singleton_eq_range_aeval, mem_range, powerBasis_gen
-/
theorem powerBasis_gen_mem_adjoin_zeta_sub_one :
    (hζ.powerBasis K).gen in adjoin K ({ζ - 1} : Set L) := by
  rw [powerBasis_gen]; rw [adjoin_singleton_eq_range_aeval]; rw [AlgHom.mem_range]
  exact ⟨X + 1, by simp⟩
...
@[simps!]
/--
Definition of `subOnePowerBasis` / `subOnePowerBasis` 的定义

English:
definition subOnePowerBasis
  signature: : PowerBasis K L
  body: by
...
Depends on / 依赖: PowerBasis, PowerBasis.adjoin_eq_top_of_gen_mem_adjoin, PowerBasis.ofAdjoinEqTop, adjoin_eq_top_of_gen_mem_adjoin, integral, isIntegral, isIntegral_one, ofAdjoinEqTop, powerBasis_gen_mem_adjoin_zeta_sub_one
-/
...
/--
Definition of `embeddingsEquivPrimitiveRoots` / `embeddingsEquivPrimitiveRoots` 的定义

English:
definition embeddingsEquivPrimitiveRoots
  signature: (C : Type*) [CommRing C] [IsDomain C] [Algebra K C]
  body: (hζ.powerBasis K).liftEquiv.trans
    { toFun := fun x => by
...
@[simp]
/--
theorem `embeddingsEquivPrimitiveRoots_apply_coe` / 定理 `embeddingsEquivPrimitiveRoots_apply_coe`

English:
theorem embeddingsEquivPrimitiveRoots_apply_coe
  statement: (C : Type*) [CommRing C] [IsDomain C] [Algebra K C]
  proof: rfl
...
English:
theorem finrank
  given: (hirr : Irreducible (cyclotomic n K))
  statement: finrank K L = n.totient
  proof: by
...
Depends on / 依赖: IsCyclotomicExtension, IsCyclotomicExtension.neZero, IsPrimitiveRoot, IsPrimitiveRoot.powerBasis_dim, finrank, minpoly_eq_cyclotomic_of_irreducible, natDegree_cyclotomic, neZero, powerBasis, powerBasis_dim, zeta_spec
-/
...
variable {L} in
/--
theorem `_root_.IsPrimitiveRoot.lcm_totient_le_finrank` / 定理 `_root_.IsPrimitiveRoot.lcm_totient_le_finrank`

English:
theorem _root_.IsPrimitiveRoot.lcm_totient_le_finrank
  statement: [FiniteDimensional K L] {p q : Nat} {x y : L}
  proof: by
...
-/
theorem _root_.IsPrimitiveRoot.lcm_totient_le_finrank [FiniteDimensional K L] {p q : Nat} {x y : L}
...
variable (n) in
/--
theorem `dvd_of_isCyclotomicExtension` / 定理 `dvd_of_isCyclotomicExtension`

English:
theorem dvd_of_isCyclotomicExtension
  statement: [IsCyclotomicExtension {n} Rat K] {ζ : K}
  proof: by
  have hl : NeZero l := ⟨hl⟩
...
-/
theorem dvd_of_isCyclotomicExtension [IsCyclotomicExtension {n} Rat K] {ζ : K}
    {l : Nat} (hζ : IsPrimitiveRoot ζ l) (hl : l != 0) : l ∣ 2 * n := by
  have hl : NeZero l := ⟨hl⟩
  have hroot := IsCyclotomicExtension.zeta_spec n Rat K
  have key := IsPrimitiveRoot.lcm_totient_le_finrank hζ hroot
    (cyclotomic.irreducible_rat <| Nat.lcm_pos (Nat.pos_of_ne_zero hl.1) (NeZero.pos n))
...
/--
theorem `exists_neg_pow_of_isOfFinOrder` / 定理 `exists_neg_pow_of_isOfFinOrder`

English:
theorem exists_neg_pow_of_isOfFinOrder
...
-/
theorem exists_neg_pow_of_isOfFinOrder [IsCyclotomicExtension {n} Rat K]
    (hno : Odd n) {ζ x : K} (hζ : IsPrimitiveRoot ζ n) (hx : IsOfFinOrder x) :
    exists r : Nat, x = (-ζ) ^ r := by
  have hnegζ : IsPrimitiveRoot (-ζ) (2 * n) := by
    convert! IsPrimitiveRoot.orderOf (-ζ)
    rw [neg_eq_neg_one_mul]; rw [(Commute.all _ _).orderOf_mul_eq_mul_orderOf_of_coprime]
    · simp [hζ.eq_orderOf]
...
/--
theorem `exists_pow_or_neg_mul_pow_of_isOfFinOrder` / 定理 `exists_pow_or_neg_mul_pow_of_isOfFinOrder`

English:
theorem exists_pow_or_neg_mul_pow_of_isOfFinOrder
  statement: [IsCyclotomicExtension {n} Rat K]
  proof: by
  obtain ⟨r, hr⟩ := hζ.exists_neg_pow_of_isOfFinOrder hno hx
  refine ⟨r % n, Nat.mod_lt _ (NeZero.pos _), ?_⟩
  rw [show ζ ^ (r % n) = ζ ^ r from (IsPrimitiveRoot.eq_orderOf hζ).symm ▸ pow_mod_orderOf ..]; rw [hr]
...
Depends on / 依赖: IsPrimitiveRoot, IsPrimitiveRoot.eq_orderOf, Nat.even_or_odd, Nat.mod_lt, NeZero, NeZero.pos, eq_orderOf, even_or_odd, exists_neg_pow_of_isOfFinOrder, h.neg_one_pow, mod_lt, neg_one_pow, neg_pow, pow_mod_orderOf
-/
theorem exists_pow_or_neg_mul_pow_of_isOfFinOrder [IsCyclotomicExtension {n} Rat K]
    (hno : Odd n) {ζ x : K} (hζ : IsPrimitiveRoot ζ n) (hx : IsOfFinOrder x) :
    exists r : Nat, r < n ∧ (x = ζ ^ r ∨ x = -ζ ^ r) := by
  obtain ⟨r, hr⟩ := hζ.exists_neg_pow_of_isOfFinOrder hno hx
  refine ⟨r % n, Nat.mod_lt _ (NeZero.pos _), ?_⟩
  rw [show ζ ^ (r % n) = ζ ^ r from (IsPrimitiveRoot.eq_orderOf hζ).symm ▸ pow_mod_orderOf ..]; rw [hr]
  rcases Nat.even_or_odd r with (h | h) <;> simp [neg_pow, h.neg_one_pow]
...
/--
theorem `norm_eq_neg_one_pow` / 定理 `norm_eq_neg_one_pow`

English:
theorem norm_eq_neg_one_pow
  given: (hζ : IsPrimitiveRoot ζ 2) [IsDomain L]
  proof: by
...
Depends on / 依赖: Algebra, Algebra.norm_algebraMap, algebraMap, eq_neg_one_of_two_right, norm_algebraMap
-/
...
/--
theorem `norm_eq_one` / 定理 `norm_eq_one`

English:
theorem norm_eq_one
  statement: [IsDomain L] [IsCyclotomicExtension {n} K L] (hn : n != 2)
  proof: by
  have := IsCyclotomicExtension.neZero' n K L
  by_cases h1 : n = 1
...
omit [NeZero n] in
/--
theorem `norm_eq_one_of_linearly_ordered` / 定理 `norm_eq_one_of_linearly_ordered`

English:
theorem norm_eq_one_of_linearly_ordered
  statement: {K : Type*}
  proof: by
  have hz := congr_arg (norm K) ((IsPrimitiveRoot.iff_def _ n).1 hζ).1
  rw [← (algebraMap K L).map_one]; rw [Algebra.norm_algebraMap]; rw [one_pow]; rw [map_pow]; rw [← one_pow n] at hz
  exact StrictMono.injective hodd.strictMono_pow hz

中文:
定理 norm_eq_one_of_linearly_ordered
  结论: {K : 类型}
  证明: by
  have hz := congr_arg (norm K) ((IsPrimitiveRoot.iff_def _ n).1 hζ).1
  rw [← (algebraMap K L).map_one]; rw [Algebra.norm_algebraMap]; rw [one_pow]; rw [map_pow]; rw [← one_pow n] at hz
...
Depends on / 依赖: Algebra, Algebra.norm_algebraMap, IsPrimitiveRoot, IsPrimitiveRoot.iff_def, StrictMono, StrictMono.injective, algebraMap, congr_arg, hodd.strictMono_pow, iff_def, injective, map_one, map_pow, norm_algebraMap, one_pow, strictMono_pow
-/
...
/--
theorem `norm_of_cyclotomic_irreducible` / 定理 `norm_of_cyclotomic_irreducible`

English:
theorem norm_of_cyclotomic_irreducible
  statement: [IsDomain L] [IsCyclotomicExtension {n} K L]
  proof: by
  split_ifs with hn
  · subst hn
    rw [norm_eq_neg_one_pow (K := K) hζ]; rw [IsCyclotomicExtension.finrank _ hirr]
    norm_cast
...
Depends on / 依赖: IsCyclotomicExtension, IsCyclotomicExtension.finrank, finrank, norm_eq_neg_one_pow, norm_eq_one, split_ifs
-/
...
/--
theorem `sub_one_norm_eq_eval_cyclotomic` / 定理 `sub_one_norm_eq_eval_cyclotomic`

English:
theorem sub_one_norm_eq_eval_cyclotomic
  statement: [IsCyclotomicExtension {n} K L] (h : 2 < n)
  proof: by
  have := IsCyclotomicExtension.neZero' n K L
...
theorem sub_one_norm_eq_eval_cyclotomic [IsCyclotomicExtension {n} K L] (h : 2 < n)
    (hirr : Irreducible (cyclotomic n K)) : norm K (ζ - 1) = ↑(eval 1 (cyclotomic n Int)) := by
  have := IsCyclotomicExtension.neZero' n K L
  let E := AlgebraicClosure L
  obtain ⟨z, hz⟩ := IsAlgClosed.exists_root _ (degree_cyclotomic_pos n E (NeZero.pos _)).ne.symm
  apply (algebraMap K E).injective
...
/--
theorem `sub_one_norm_isPrimePow` / 定理 `sub_one_norm_isPrimePow`

English:
theorem sub_one_norm_isPrimePow
  statement: (hn : IsPrimePow n) [IsCyclotomicExtension {n} K L]
  proof: by
...
/--
theorem `minpoly_sub_one_eq_cyclotomic_comp` / 定理 `minpoly_sub_one_eq_cyclotomic_comp`

English:
theorem minpoly_sub_one_eq_cyclotomic_comp
  statement: [Algebra K A] [IsDomain A] {ζ : A}
  proof: by
...
Depends on / 依赖: IsCyclotomicExtension, IsCyclotomicExtension.neZero, add_algebraMap, algebraMap, minpoly, minpoly.add_algebraMap, minpoly_eq_cyclotomic_of_irreducible, neZero, sub_eq_add_neg
-/
...
/--
theorem `norm_pow_sub_one_of_prime_pow_ne_two` / 定理 `norm_pow_sub_one_of_prime_pow_ne_two`

English:
theorem norm_pow_sub_one_of_prime_pow_ne_two
  statement: {k s : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
  proof: by
...
-/
theorem norm_pow_sub_one_of_prime_pow_ne_two {k s : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
...
/--
theorem `norm_pow_sub_one_of_prime_ne_two` / 定理 `norm_pow_sub_one_of_prime_ne_two`

English:
theorem norm_pow_sub_one_of_prime_ne_two
  statement: {k : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
  proof: by
...
Depends on / 依赖: Nat.prime_two, eq_of_prime_pow_eq, hpri.out, norm_pow_sub_one_of_prime_pow_ne_two, pow_one, prime_iff, prime_two, replace, succ_pos
-/
theorem norm_pow_sub_one_of_prime_ne_two {k : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
    [hpri : Fact p.Prime] [IsCyclotomicExtension {p ^ (k + 1)} K L]
    (hirr : Irreducible (cyclotomic (p ^ (k + 1)) K)) {s : Nat} (hs : s <= k) (hodd : p != 2) :
...
/--
theorem `norm_sub_one_of_prime_ne_two` / 定理 `norm_sub_one_of_prime_ne_two`

English:
theorem norm_sub_one_of_prime_ne_two
  statement: {k : Nat} (hζ : IsPrimitiveRoot ζ ↑(p ^ (k + 1)))
  proof: by
...
Depends on / 依赖: k.zero_le, norm_pow_sub_one_of_prime_ne_two, zero_le
-/
theorem norm_sub_one_of_prime_ne_two {k : Nat} (hζ : IsPrimitiveRoot ζ ↑(p ^ (k + 1)))
    [hpri : Fact p.Prime] [IsCyclotomicExtension {p ^ (k + 1)} K L]
...
Depends on / 依赖: Irreducible, IsCyclotomicExtension, IsPrimitiveRoot, cyclotomic, norm_sub_one_of_prime_ne_two, replace
-/
...
/--
theorem `norm_pow_sub_one_two` / 定理 `norm_pow_sub_one_two`

English:
theorem norm_pow_sub_one_two
  statement: {k : Nat} (hζ : IsPrimitiveRoot ζ (2 ^ (k + 1)))
  proof: by
  have := hζ.pow_of_dvd
...
-/
theorem norm_pow_sub_one_two {k : Nat} (hζ : IsPrimitiveRoot ζ (2 ^ (k + 1)))
...
/--
theorem `norm_sub_one_two` / 定理 `norm_sub_one_two`

English:
theorem norm_sub_one_two
  statement: {k : Nat} (hζ : IsPrimitiveRoot ζ (2 ^ k)) (hk : 2 <= k)
  proof: by
  have : 2 < 2 ^ k := by
    nth_rw 1 [← pow_one 2]
...
-/
theorem norm_sub_one_two {k : Nat} (hζ : IsPrimitiveRoot ζ (2 ^ k)) (hk : 2 <= k)
...
/--
theorem `norm_pow_sub_one_eq_prime_pow_of_ne_zero` / 定理 `norm_pow_sub_one_eq_prime_pow_of_ne_zero`

English:
theorem norm_pow_sub_one_eq_prime_pow_of_ne_zero
  statement: {k s : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
  proof: by
...
-/
theorem norm_pow_sub_one_eq_prime_pow_of_ne_zero {k s : Nat} (hζ : IsPrimitiveRoot ζ (p ^ (k + 1)))
...
/--
theorem `norm_zeta_eq_one` / 定理 `norm_zeta_eq_one`

English:
theorem norm_zeta_eq_one
  statement: [IsCyclotomicExtension {n} K L] (hn : n != 2)
  proof: (zeta_spec n K L).norm_eq_one hn hirr
...
/--
theorem `norm_zeta_sub_one_of_isPrimePow` / 定理 `norm_zeta_sub_one_of_isPrimePow`

English:
theorem norm_zeta_sub_one_of_isPrimePow
  statement: (hn : IsPrimePow n) [IsCyclotomicExtension {n} K L]
  proof: (zeta_spec n K L).sub_one_norm_isPrimePow hn hirr h
...
/--
theorem `norm_zeta_pow_sub_one_of_prime_pow_ne_two` / 定理 `norm_zeta_pow_sub_one_of_prime_pow_ne_two`

English:
theorem norm_zeta_pow_sub_one_of_prime_pow_ne_two
  statement: {k : Nat} [Fact p.Prime]
  proof: (zeta_spec _ K L).norm_pow_sub_one_of_prime_pow_ne_two hirr hs htwo
...
-/
theorem norm_zeta_pow_sub_one_of_prime_pow_ne_two {k : Nat} [Fact p.Prime]
    [IsCyclotomicExtension {p ^ (k + 1)} K L]
    (hirr : Irreducible (cyclotomic (p ^ (k + 1)) K)) {s : Nat} (hs : s <= k)
    (htwo : p ^ (k - s + 1) != 2) :
    norm K (zeta (p ^ (k + 1)) K L ^ p ^ s - 1) = (p : K) ^ p ^ s :=
  (zeta_spec _ K L).norm_pow_sub_one_of_prime_pow_ne_two hirr hs htwo

/--
theorem `norm_zeta_pow_sub_one_of_prime_ne_two` / 定理 `norm_zeta_pow_sub_one_of_prime_ne_two`

English:
theorem norm_zeta_pow_sub_one_of_prime_ne_two
  statement: {k : Nat} [Fact p.Prime]
  proof: (zeta_spec _ K L).norm_sub_one_of_prime_ne_two hirr h
...
-/
theorem norm_zeta_pow_sub_one_of_prime_ne_two {k : Nat} [Fact p.Prime]
    [IsCyclotomicExtension {p ^ (k + 1)} K L]
...
/--
theorem `norm_zeta_sub_one_of_prime_ne_two` / 定理 `norm_zeta_sub_one_of_prime_ne_two`

English:
theorem norm_zeta_sub_one_of_prime_ne_two
...
/--
theorem `norm_zeta_pow_sub_one_two` / 定理 `norm_zeta_pow_sub_one_two`

English:
theorem norm_zeta_pow_sub_one_two
  statement: {k : Nat} (hk : 2 <= k)
  proof: norm_sub_one_two (zeta_spec (2 ^ k) K L) hk hirr

中文:
定理 norm_zeta_pow_sub_one_two
  结论: {k : 自然数} (hk : 2 <= k)
  证明: norm_sub_one_two (zeta_spec (2 ^ k) K L) hk hirr

Depends on / 依赖: norm_sub_one_two, zeta_spec
-/
theorem norm_zeta_pow_sub_one_two {k : Nat} (hk : 2 <= k)
