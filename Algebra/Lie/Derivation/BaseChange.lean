...
/-
**Lie.Derivation.ofDerivation** 是 Mathlib 中的一个定义，位于命名空间 `Lie.Derivation`。
形式化陈述：ofDerivation : Derivation R A A ->ₗ⁅R⁆ LieDerivation R (A otimes[R] L) (A 
otimes[R] L) where toFun d
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A derivation of an associative `R`-algebra `A`, induces a Lie derivation of `A ⊗
[R] L` for any
Lie algebra `L` over `R`.
-/
...
@[simp]
/-
**Lie.Derivation.ofDerivation_apply** 是 Mathlib 中的一个引理，位于命名空间 `Lie.Derivation`。
形式化陈述：ofDerivation_apply (d : Derivation R A A) (x : A otimes[R] L) : ofDerivati
on L d x = d.toLinearMap.rTensor L x
参数：d : Derivation R A A；x : A otimes[R] L。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
Algebra `A` over `R`. -/
/-
**Lie.Derivation.ofLieDerivation** 是 Mathlib 中的一个定义，位于命名空间 `Lie.Derivation`。
形式化陈述：ofLieDerivation : (LieDerivation R L L) ->ₗ⁅R⁆ (LieDerivation R (A otimes[
R] L) (A otimes[R] L)) where toFun d
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A Lie derivation of an `R-`Lie algebra `L`, induces a Lie derivation of `A ⊗[R] 
L` for any
Algebra `A` over `R`.
-/
...
@[simp]
/-
**Lie.Derivation.ofLieDerivation_apply** 是 Mathlib 中的一个引理，位于命名空间 `Lie.Derivation
`。
形式化陈述：ofLieDerivation_apply (d : LieDerivation R L L) (x : A otimes[R] L) : ofLi
eDerivation A d x = d.toLinearMap.lTensor A x
参数：d : LieDerivation R L L；x : A otimes[R] L。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
