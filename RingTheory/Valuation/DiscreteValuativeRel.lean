...
/--
lemma `nonempty_orderIso_withZeroMul_int_iff` / 引理 `nonempty_orderIso_withZeroMul_int_iff`

English:
lemma nonempty_orderIso_withZeroMul_int_iff
  given: [Semiring R] [ValuativeRel R]
  proof: by
  constructor
  · rintro ⟨e⟩
    let x := e.symm (exp (-1))
...
lemma nonempty_orderIso_withZeroMul_int_iff [Semiring R] [ValuativeRel R] :
    Nonempty (ValueGroupWithZero R ≃*o Intᵐ⁰) ↔
...
/--
lemma `IsDiscrete.of_compatible_withZeroMulInt` / 引理 `IsDiscrete.of_compatible_withZeroMulInt`

English:
lemma IsDiscrete.of_compatible_withZeroMulInt
  statement: [Ring R] [ValuativeRel R]
  proof: by
...
lemma IsDiscrete.of_compatible_withZeroMulInt [Ring R] [ValuativeRel R]
    (v : Valuation R Intᵐ⁰) [v.Compatible] : IsDiscrete R := by
  have : IsRankLeOne R := .of_compatible_mulArchimedean v
  by_cases h : IsNontrivial R
  · by_cases H : DenselyOrdered (ValueGroupWithZero R)
    · classical
