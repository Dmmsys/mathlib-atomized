...
/-
**MultilinearMap.free_and_finite_fin** 是 Mathlib 中的一个定理，位于命名空间 `MultilinearMap`。
...
/-
**MultilinearMap.free_and_finite** 是 Mathlib 中的一个定理，位于命名空间 `MultilinearMap`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**MultilinearMap._root_.Module.Finite.multilinearMap** 是 Mathlib 中的一个实例，位于命名空间 `
MultilinearMap`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
instance _root_.Module.Finite.multilinearMap : Module.Finite R (MultilinearMap R M₁ M₂) :=
  free_and_finite.2
/-
**MultilinearMap._root_.Module.Free.multilinearMap** 是 Mathlib 中的一个实例，位于命名空间 `Mu
ltilinearMap`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
