...
`Fintype.piFinset t` is a `Finset (Π a, t a)`. -/]
/--
lemma `prod_univ_pi` / 引理 `prod_univ_pi`

English:
lemma prod_univ_pi
  statement: [DecidableEq ι] [Fintype ι] {κ : ι -> Type*} (t : forall i, Finset (κ i))
  proof: by
...
Depends on / 依赖: mem_univ, prod_nbij
-/
lemma prod_univ_pi [DecidableEq ι] [Fintype ι] {κ : ι -> Type*} (t : forall i, Finset (κ i))
    (f : (forall i in (univ : Finset ι), κ i) -> β) :
