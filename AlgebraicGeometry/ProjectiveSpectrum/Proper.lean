...
/-
**AlgebraicGeometry.Proj.lift_awayMap** 是 Mathlib 中的一个引理，位于命名空间 `AlgebraicGeomet
ry.Proj`。
...
/-
**AlgebraicGeometry.Proj.isSeparated** 是 Mathlib 中的一个实例，位于命名空间 `AlgebraicGeometr
y.Proj`。
形式化陈述：isSeparated : IsSeparated (toSpecZero 𝒜)
该定义给出了上述对象。
本声明引用了以下数学事实（定理与引理）：
...
b
· 使用引理 `AlgebraicGeometry.Proj.lift_awayMapₐ_awayMapₐ_surjective`：lift_awayMapₐ_
awayMapₐ_surjective {d e : Nat} {f : A} (hf : f in 𝒜 d) {g : A} (hg : g in 𝒜 e) 
{x : A} (hx : x = f * g) (hd : 0 < d) : Functi…
...
/-
**AlgebraicGeometry.Proj.valuativeCriterion_existence_aux** 是 Mathlib 中的一个定理，位于命
名空间 `AlgebraicGeometry.Proj`。
形式化陈述：valuativeCriterion_existence_aux {O : Type*} [CommRing O] [IsDomain O] [Va
luationRing O] {K : Type*} [Field K] [Algebra O K] [IsFractionRing O K] (φ₀ : (𝒜
 0) ->+* O) (ι : Type*) [Finite ι] (x : ι -> A) (h2 : Algebra.adjoin (𝒜 0) (Set.
range x) = ⊤) (j : ι) (φ : Away 𝒜 (x j) ->+* K) (hcomm : (algebraMap O K).comp φ
₀ = φ.comp (fromZeroRingHom 𝒜 _)) (d : ι -> Nat) (hdi : forall i, 0 < d i) (hxdi
 : forall i, x i in 𝒜 (d i)) : exists (j₀ : ι) (φ' : Away 𝒜 (x j * x j₀) ->+* K)
, φ'.comp (awayMap 𝒜 (hxdi
参数：φ₀ : (𝒜 0) ->+* O；ι : Type*；x : ι -> A；h2 : Algebra.adjoin (𝒜 0) (Set.range x
) = ⊤；j : ι；φ : Away 𝒜 (x j) ->+* K；hcomm : (algebraMap O K).comp φ₀ = φ.comp (f
romZeroRingHom 𝒜 _)；d : ι -> Nat；hdi : forall i, 0 < d i；hxdi : forall i, x i in
 𝒜 (d i)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `AddSubgroupClass.toAddSubmonoidClass`：∀ {S : Type u_3} {G : outParam (Ty
...
--- 原说明 ---
Let `𝒜` be a graded ring generated over `𝒜₀` by finitely many homogeneous elemen
ts.
Suppose we have the following diagram for some homogeneous `x`
with `O` a valuation ring and `K = Frac(O)`.
```
...
O ←---- 𝒜_{(x₀)}                𝒜_{(x)}
    φₗ
```
This is the underlying algebraic statement of the valuative criterion for `Proj 
𝒜`.
-/
...
@[stacks 01MF]
/-
**AlgebraicGeometry.Proj.valuativeCriterion_existence** 是 Mathlib 中的一个引理，位于命名空间 
`AlgebraicGeometry.Proj`。
形式化陈述：valuativeCriterion_existence [Algebra.FiniteType (𝒜 0) A] : ValuativeCrite
rion.Existence (Proj.toSpecZero 𝒜)
参数：𝒜 0。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `AddSubgroupClass.toAddSubmonoidClass`：∀ {S : Type u_3} {G : outParam (Ty
...
· 使用引理 `AlgebraicGeometry.Proj.iSup_basicOpen_eq_top'`：iSup_basicOpen_eq_top' {ι
 : Type*} (f : ι -> A) (hfn : forall i, exists n, f i in 𝒜 n) (hf : Algebra.adjo
in (𝒜 0) (Set.range f) = ⊤) : ⨆ i, …
· 使用定理 `Subtype.property`：∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑s
elf
