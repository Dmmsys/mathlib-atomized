...
definition restrictScalars
  signature: :
  body: { val := (PresheafOfModules.restrictScalars α.hom).obj M'.val
      isSheaf := M'.isSheaf }
...
定义 restrictScalars
  签名: :
  定义体: { val := (PresheafOfModules.restrictScalars α.hom).obj M'.val
      isSheaf := M'.isSheaf }
  map φ := { val := (PresheafOfModules.restrictScalars α.hom).map φ.val }

Depends on / 依赖: PresheafOfModules, PresheafOfModules.restrictScalars, isSheaf, restrictScalars
-/
...
English:
instance :
  signature: (restrictScalars.{v} α).Additive
...
/--
Definition of `restrictHomEquivOfIsLocallySurjective` / `restrictHomEquivOfIsLocallySurjective` 的定义

English:
definition restrictHomEquivOfIsLocallySurjective
  body: (restrictScalars α).map f
