...
/-
**Mathlib.Deriving.Traversable.nestedMap** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.D
eriving.Traversable`。
形式化陈述：Expr → Expr → Expr → Elab.TermElabM Expr
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`nestedMap f α (List (Array (List α)))` synthesizes the expression
`Functor.map (Functor.map (Functor.map f))`. `nestedMap` assumes that `α` appear
s in
`(List (Array (List α)))`.
...
/-
**Mathlib.Deriving.Traversable.mapField** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Deriv
ing.Traversable`。
形式化陈述：mapField (n : Name) (cl f α β e : Expr) : TermElabM Expr
参数：n : Name；cl f α β e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.getAuxDefOfDeclName** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Deriving.Traversable`。
形式化陈述：getAuxDefOfDeclName : TermElabM FVarId
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Get the auxiliary local declaration corresponding to the current declaration. If
 there are
multiple declarations it will throw.
-/
...
/-
**Mathlib.Deriving.Traversable.mapConstructor** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Deriving.Traversable`。
形式化陈述：mapConstructor (c n : Name) (f α β : Expr) (args₀ : List Expr) (args₁ : Li
st (Bool × Expr)) (m : MVarId) : TermElabM Unit
参数：c n : Name；f α β : Expr；args₀ : List Expr；args₁ : List (Bool × Expr)；m : MVar
Id。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.mkCasesOnMatch** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Deriving.Traversable`。
形式化陈述：mkCasesOnMatch (type : Name) (levels : List Level) (params : List Expr) (m
otive : Expr) (indices : List Expr) (val : Expr) (rhss : (ctor : Name) -> (field
s : List FVarId) -> TermElabM Expr) : TermElabM Expr
参数：type : Name；levels : List Level；params : List Expr；motive : Expr；indices : Li
st Expr；val : Expr；rhss : (ctor : Name) -> (fields : List FVarId) -> TermElabM E
xpr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Makes a `match` expression corresponding to the application of `casesOn` like:
```lean
match (motive := motive) indices₁, indices₂, .., (val : type.{univs} params₁ par
ams₂ ..) with
| _, _, .., ctor₁ fields₁₁ fields₁₂ .. => rhss ctor₁ [fields₁₁, fields₁₂, ..]
| _, _, .., ctor₂ fields₂₁ fields₂₂ .. => rhss ctor₂ [fields₂₁, fields₂₂, ..]
```
...
/-
**Mathlib.Deriving.Traversable.getFVarIdsNotImplementationDetails** 是 Mathlib 中的
一个定义，位于命名空间 `Mathlib.Deriving.Traversable`。
形式化陈述：getFVarIdsNotImplementationDetails : MetaM (List FVarId)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.getFVarsNotImplementationDetails** 是 Mathlib 中的一个
定义，位于命名空间 `Mathlib.Deriving.Traversable`。
形式化陈述：getFVarsNotImplementationDetails : MetaM (List Expr)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Get `Expr`s of `FVarId`s which is not implementation details in the current cont
ext.
-/
...
/-
**Mathlib.Deriving.Traversable.mkMap** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Deriving
.Traversable`。
形式化陈述：mkMap (type : Name) (m : MVarId) : TermElabM Unit
参数：type : Name；m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.deriveFunctor** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Deriving.Traversable`。
形式化陈述：deriveFunctor (m : MVarId) : TermElabM Unit
参数：m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.mkInstanceNameForTypeExpr** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Deriving.Traversable`。
形式化陈述：mkInstanceNameForTypeExpr (type : Expr) : TermElabM Name
参数：type : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.mkOneInstance** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Deriving.Traversable`。
形式化陈述：mkOneInstance (n cls : Name) (tac : MVarId -> TermElabM Unit) (mkInst : Na
me -> Expr -> TermElabM Expr
参数：n cls : Name；tac : MVarId -> TermElabM Unit。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Derive the `cls` instance for the inductive type constructor `n` using the `tac`
 tactic.
-/
...
/-
**Mathlib.Deriving.Traversable.higherOrderDeriveHandler** 是 Mathlib 中的一个定义，位于命名空
间 `Mathlib.Deriving.Traversable`。
形式化陈述：higherOrderDeriveHandler (cls : Name) (tac : MVarId -> TermElabM Unit) (de
ps : List DerivingHandler
参数：cls : Name；tac : MVarId -> TermElabM Unit。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.functorDeriveHandler** 是 Mathlib 中的一个定义，位于命名空间 `M
athlib.Deriving.Traversable`。
形式化陈述：functorDeriveHandler : DerivingHandler
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.deriveLawfulFunctor** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Deriving.Traversable`。
形式化陈述：deriveLawfulFunctor (m : MVarId) : TermElabM Unit
参数：m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.lawfulFunctorDeriveHandler** 是 Mathlib 中的一个定义，位于命
名空间 `Mathlib.Deriving.Traversable`。
形式化陈述：lawfulFunctorDeriveHandler : DerivingHandler
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`(List (Array (List α)))` -/
/-
**Mathlib.Deriving.Traversable.nestedTraverse** 是 Mathlib 中的一个不透明定义，位于命名空间 `Math
lib.Deriving.Traversable`。
形式化陈述：Expr → Expr → Expr → Elab.TermElabM Expr
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.traverseField** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Deriving.Traversable`。
形式化陈述：traverseField (n : Name) (cl f v e : Expr) : TermElabM (Bool × Expr)
参数：n : Name；cl f v e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.traverseConstructor** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Deriving.Traversable`。
形式化陈述：traverseConstructor (c n : Name) (applInst f α β : Expr) (args₀ : List Exp
r) (args₁ : List (Bool × Expr)) (m : MVarId) : TermElabM Unit
参数：c n : Name；applInst f α β : Expr；args₀ : List Expr；args₁ : List (Bool × Expr)
；m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.mkTraverse** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Der
iving.Traversable`。
形式化陈述：mkTraverse (type : Name) (m : MVarId) : TermElabM Unit
参数：type : Name；m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.deriveTraversable** 是 Mathlib 中的一个定义，位于命名空间 `Math
lib.Deriving.Traversable`。
形式化陈述：deriveTraversable (m : MVarId) : TermElabM Unit
参数：m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.traversableDeriveHandler** 是 Mathlib 中的一个定义，位于命名空
间 `Mathlib.Deriving.Traversable`。
形式化陈述：traversableDeriveHandler : DerivingHandler
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.simpFunctorGoal** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Deriving.Traversable`。
形式化陈述：simpFunctorGoal (m : MVarId) (s : Simp.Context) (simprocs : Simp.SimprocsA
rray
参数：m : MVarId；s : Simp.Context。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Deriving.Traversable.traversableLawStarter** 是 Mathlib 中的一个定义，位于命名空间 `
Mathlib.Deriving.Traversable`。
形式化陈述：traversableLawStarter (m : MVarId) (n : Name) (s : MetaM Simp.Context) (ta
c : Array FVarId -> InductionSubgoal -> MVarId -> MetaM Unit) : MetaM Unit
参数：m : MVarId；n : Name；s : MetaM Simp.Context；tac : Array FVarId -> InductionSub
...
/-
**Mathlib.Deriving.Traversable.deriveLawfulTraversable** 是 Mathlib 中的一个定义，位于命名空间
 `Mathlib.Deriving.Traversable`。
形式化陈述：deriveLawfulTraversable (m : MVarId) : TermElabM Unit
参数：m : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Traversable.lawfulTraversableDeriveHandler** 是 Mathlib 中的一个定义
，位于命名空间 `Mathlib.Deriving.Traversable`。
形式化陈述：lawfulTraversableDeriveHandler : DerivingHandler
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
