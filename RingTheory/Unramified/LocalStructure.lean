...
variable (R) in
/--
Definition of `HasStandardEtaleSurjectionOn` / `HasStandardEtaleSurjectionOn` 的定义

English:
definition HasStandardEtaleSurjectionOn
  signature: (f : S)
  body: exists (P : StandardEtalePair R) (φ : P.Ring ->ₐ[R] Localization.Away f), Function.Surjective φ

中文:
定义 HasStandardEtaleSurjectionOn
  签名: (f : S)
  定义体: exists (P : StandardEtalePair R) (φ : P.Ring ->ₐ[R] Localization.Away f), Function.Surjective φ

Depends on / 依赖: Function, Function.Surjective, Localization, Localization.Away, P.Ring, StandardEtalePair, Surjective
-/
def HasStandardEtaleSurjectionOn (f : S) : Prop :=
  exists (P : StandardEtalePair R) (φ : P.Ring ->ₐ[R] Localization.Away f), Function.Surjective φ

/--
lemma `HasStandardEtaleSurjectionOn.mk` / 引理 `HasStandardEtaleSurjectionOn.mk`

English:
lemma HasStandardEtaleSurjectionOn.mk
  statement: [IsStandardEtale R A]
  proof: let P : StandardEtalePresentation R A := Nonempty.some inferInstance
...
Depends on / 依赖: IsLocalization, IsLocalization.algEquiv, Localization, Localization.Away, Nonempty, Nonempty.some, P.equivRing.symm.toAlgHom, StandardEtalePresentation, algEquiv, equivRing, powers, restrictScalars, symm.toAlgHom, toAlgHom
-/
...
/--
lemma `HasStandardEtaleSurjectionOn.of_dvd` / 引理 `HasStandardEtaleSurjectionOn.of_dvd`
...
/--
lemma `HasStandardEtaleSurjectionOn.isStandardEtale` / 引理 `HasStandardEtaleSurjectionOn.isStandardEtale`

English:
lemma HasStandardEtaleSurjectionOn.isStandardEtale
  proof: .of_surjective _ H.choose_spec.choose_spec

中文:
引理 HasStandardEtaleSurjectionOn.isStandardEtale
  证明: .of_surjective _ H.choose_spec.choose_spec

Depends on / 依赖: H.choose_spec.choose_spec, choose_spec, of_surjective
-/
lemma HasStandardEtaleSurjectionOn.isStandardEtale
...
/--
theorem `exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top_aux₁` / 定理 `exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top_aux₁`

English:
theorem exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top_aux₁
  proof: by
...
private theorem exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top_aux₁
    ...
    (RingHom.ker (aeval (R := R) x).toRingHom).map (mapRingHom (algebraMap R P.ResidueField)) =
      RingHom.ker (aeval (1 otimesₜ x : P.Fiber S)).toRingHom := by
  have hx' : Function.Surjective (aeval (R := R) x) :=
    (AlgHom.range_eq_top _).mp ((adjoin_singleton_eq_range_aeval R x).symm.trans hx)
  let I := RingHom.ker (aeval (R := R) x).toRingHom
  let e : P.Fiber S ≃ₐ[P.ResidueField]
...
private theorem exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top_aux₂
    ...
    (hp₁ : Ideal.span {p.map (algebraMap R P.ResidueField)} =
      RingHom.ker (aeval ((1 : P.ResidueField) otimesₜ[R] x)).toRingHom)
    (hp₂ : R[x] = ⊤) :
...
/--
lemma `exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top` / 引理 `exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top`
...
private lemma exists_hasStandardEtaleSurjectionOn_of_exists_adjoin_singleton_eq_top
    ...
    exists f ∉ Q, HasStandardEtaleSurjectionOn R f := by
  cases subsingleton_or_nontrivial S
  · cases Ideal.IsPrime.ne_top' (Subsingleton.elim Q ⊤)
  have := (algebraMap R S).domain_nontrivial
  -- Suppose `S = R[X]/I` is finite (as an `R`-module), and `R`-unramified at a prime `Q`,
...
/--
lemma `exists_notMem_forall_ne_mem_and_adjoin_eq_top` / 引理 `exists_notMem_forall_ne_mem_and_adjoin_eq_top`
...
lemma exists_notMem_forall_ne_mem_and_adjoin_eq_top
    ...
    exists t ∉ Q, (forall Q' in (Q.under R).primesOver S, Q' != Q -> t in Q') ∧
...
/--
lemma `exists_primesOver_under_adjoin_eq_singleton_and_residueField_bijective` / 引理 `exists_primesOver_under_adjoin_eq_singleton_and_residueField_bijective`

English:
lemma exists_primesOver_under_adjoin_eq_singleton_and_residueField_bijective
  proof: Localization.AtPrime.algebraOfLiesOver (Q.under R[x]) Q
...
lemma exists_primesOver_under_adjoin_eq_singleton_and_residueField_bijective
    ...
    exists x : S, (Q.under (R[x])).primesOver S = {Q} ∧
...
-- Subsumed by `Algebra.IsUnramifiedAt.exists_hasStandardEtaleSurjectionOn`.
/--
lemma `exists_hasStandardEtaleSurjectionOn_of_finite` / 引理 `exists_hasStandardEtaleSurjectionOn_of_finite`

English:
lemma exists_hasStandardEtaleSurjectionOn_of_finite
  proof: by
  obtain ⟨x, hQ', hQ'Q⟩ :=
...
/--
lemma `exists_hasStandardEtaleSurjectionOn` / 引理 `exists_hasStandardEtaleSurjectionOn`
...
@[stacks 00UE]
/--
lemma `IsEtaleAt.exists_isStandardEtale` / 引理 `IsEtaleAt.exists_isStandardEtale`
...
lemma IsEtaleAt.exists_isStandardEtale
    ...
    exists f, f ∉ Q ∧ IsStandardEtale R (Localization.Away f) := by
  obtain ⟨f, hfQ, h⟩ := exists_etale_of_isEtaleAt (R := R) Q
  obtain ⟨g, hgQ, hg⟩ := IsUnramifiedAt.exists_hasStandardEtaleSurjectionOn (R := R) Q
  have : Etale R (Localization.Away (f * g)) := by
    rw [← basicOpen_subset_etaleLocus_iff_etale] at h ⊢
...
set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `IsSmoothAt.exists_isStandardEtale_mvPolynomial` / 定理 `IsSmoothAt.exists_isStandardEtale_mvPolynomial`
...
theorem IsSmoothAt.exists_isStandardEtale_mvPolynomial
    ...
    exists f ∉ p, exists (n : Nat) (_ : Algebra (MvPolynomial (Fin n) R) (Localization.Away f)),
