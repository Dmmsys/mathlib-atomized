...
/--
lemma `integrable_llr_of_integrable_llr_compProd` / 引理 `integrable_llr_of_integrable_llr_compProd`
...
-/
lemma integrable_llr_of_integrable_llr_compProd
    (h_ac : μ otimesₘ κ ≪ ν otimesₘ η) (h_int : Integrable (llr (μ otimesₘ κ) (ν otimesₘ η)) (μ otimesₘ κ)) :
...
/--
lemma `rnDeriv_compProd_mul_log_eq_mul_add` / 引理 `rnDeriv_compProd_mul_log_eq_mul_add`

English:
lemma rnDeriv_compProd_mul_log_eq_mul_add
  given: (h_ac : μ otimesₘ κ ≪ μ otimesₘ η)
  proof: by
  filter_upwards [rnDeriv_compProd h_ac ν] with p hp
  simp_rw [hp, ENNReal.toReal_mul]
  by_cases h_zero1 : ((∂μ/∂ν) p.1).toReal = 0
  · simp [h_zero1]
  by_cases h_zero2 : ((∂μ otimesₘ κ/∂μ otimesₘ η) p).toReal = 0
  · simp [h_zero2]
  simp [log_mul h_zero1 h_zero2]

中文:
引理 rnDeriv_compProd_mul_log_eq_mul_add
  条件: (h_ac : μ otimesₘ κ ≪ μ otimesₘ η)
  证明: by
  filter_upwards [rnDeriv_compProd h_ac ν] with p hp
  simp_rw [hp, ENNReal.toReal_mul]
  by_cases h_zero1 : ((∂μ/∂ν) p.1).toReal = 0
...
Depends on / 依赖: ENNReal, ENNReal.toReal_mul, e.symm, filter_upwards, h_ac, h_zero1, h_zero2, log_mul, rnDeriv_compProd, simp_rw, toReal, toReal_mul
-/
lemma rnDeriv_compProd_mul_log_eq_mul_add (h_ac : μ otimesₘ κ ≪ μ otimesₘ η) :
    forallᵐ p ∂(ν otimesₘ η), ((∂μ otimesₘ κ/∂ν otimesₘ η) p).toReal * log ((∂μ otimesₘ κ/∂ν otimesₘ η) p).toReal =
      (((∂μ otimesₘ κ/∂ν otimesₘ η) p).toReal * (log ((∂μ/∂ν) p.1).toReal +
        log ((∂(μ otimesₘ κ)/∂(μ otimesₘ η)) p).toReal)) := by
  filter_upwards [rnDeriv_compProd h_ac ν] with p hp
  simp_rw [hp, ENNReal.toReal_mul]
  by_cases h_zero1 : ((∂μ/∂ν) p.1).toReal = 0
  · simp [h_zero1]
...
/--
lemma `integrable_llr_compProd_iff` / 引理 `integrable_llr_compProd_iff`

English:
lemma integrable_llr_compProd_iff
  given: (h_ac : μ otimesₘ κ ≪ ν otimesₘ η)
  proof: by
...
引理 integrable_llr_compProd_iff
  条件: (h_ac : μ otimesₘ κ ≪ ν otimesₘ η)
  证明: by
  have ⟨h_ac_μν, h_ac_κη⟩ := Measure.absolutelyContinuous_compProd_iff.mp h_ac
  rw [← integrable_rnDeriv_mul_log_iff h_ac]; rw [integrable_congr (rnDeriv_compProd_mul_log_eq_mul_add h_ac_κη)]; rw [integrable_toReal_rnDeriv_mul_iff h_ac]
  have h_iff_κ : Integrable (llr μ ν) μ ↔ Integrable (fun x => llr μ ν x.1) (μ otimesₘ κ) := by
...
-/
lemma integrable_llr_compProd_iff (h_ac : μ otimesₘ κ ≪ ν otimesₘ η) :
    Integrable (llr (μ otimesₘ κ) (ν otimesₘ η)) (μ otimesₘ κ) ↔
      Integrable (llr μ ν) μ ∧ Integrable (llr (μ otimesₘ κ) (μ otimesₘ η)) (μ otimesₘ κ) := by
  have ⟨h_ac_μν, h_ac_κη⟩ := Measure.absolutelyContinuous_compProd_iff.mp h_ac
  rw [← integrable_rnDeriv_mul_log_iff h_ac]; rw [integrable_congr (rnDeriv_compProd_mul_log_eq_mul_add h_ac_κη)]; rw [integrable_toReal_rnDeriv_mul_iff h_ac]
  have h_iff_κ : Integrable (llr μ ν) μ ↔ Integrable (fun x => llr μ ν x.1) (μ otimesₘ κ) := by
    rw [Measure.integrable_compProd_iff]
...
/--
lemma `integral_llr_compProd_eq_add` / 引理 `integral_llr_compProd_eq_add`

English:
lemma integral_llr_compProd_eq_add
  statement: (h_ac : μ otimesₘ κ ≪ ν otimesₘ η)
  proof: by
  have ⟨h_ac_μν, h_ac_κη⟩ := Measure.absolutelyContinuous_compProd_iff.mp h_ac
...
引理 integral_llr_compProd_eq_add
  结论: (h_ac : μ otimesₘ κ ≪ ν otimesₘ η)
  证明: by
  have ⟨h_ac_μν, h_ac_κη⟩ := Measure.absolutelyContinuous_compProd_iff.mp h_ac
  have ⟨h_int_μν, h_int_κη⟩ := (integrable_llr_compProd_iff h_ac).mp h_int
  have h_int1 : Integrable (fun p => log ((∂μ/∂ν) p.1).toReal) (μ otimesₘ κ) := by
...
-/
lemma integral_llr_compProd_eq_add (h_ac : μ otimesₘ κ ≪ ν otimesₘ η)
    (h_int : Integrable (llr (μ otimesₘ κ) (ν otimesₘ η)) (μ otimesₘ κ)) :
    ∫ p, llr (μ otimesₘ κ) (ν otimesₘ η) p ∂μ otimesₘ κ =
      ∫ a, llr μ ν a ∂μ + ∫ p, llr (μ otimesₘ κ) (μ otimesₘ η) p ∂(μ otimesₘ κ) := by
  have ⟨h_ac_μν, h_ac_κη⟩ := Measure.absolutelyContinuous_compProd_iff.mp h_ac
  have ⟨h_int_μν, h_int_κη⟩ := (integrable_llr_compProd_iff h_ac).mp h_int
  have h_int1 : Integrable (fun p => log ((∂μ/∂ν) p.1).toReal) (μ otimesₘ κ) := by
    rw [Measure.integrable_compProd_iff]
...
@[simp]
/--
lemma `klDiv_compProd_left` / 引理 `klDiv_compProd_left`

English:
lemma klDiv_compProd_left
  statement: klDiv (μ otimesₘ κ) (ν otimesₘ κ) = klDiv μ ν
  proof: by
  -- first, if we don't have absolute continuity, both sides are `∞`
  by_cases h_ac : μ otimesₘ κ ≪ ν otimesₘ κ
  swap
...
引理 klDiv_compProd_left
  结论: klDiv (μ otimesₘ κ) (ν otimesₘ κ) = klDiv μ ν
  证明: by
  -- first, if we don't have absolute continuity, both sides are `∞`
  by_cases h_ac : μ otimesₘ κ ≪ ν otimesₘ κ
  swap
...
variable (μ ν κ η) in
/--
theorem `klDiv_compProd_eq_add` / 定理 `klDiv_compProd_eq_add`

English:
theorem klDiv_compProd_eq_add
  statement: klDiv (μ otimesₘ κ) (ν otimesₘ η) = klDiv μ ν + klDiv (μ otimesₘ κ) (μ otimesₘ η)
  proof: by
  -- first, if we don't have absolute continuity, both sides are `∞`
  by_cases h_ac : μ otimesₘ κ ≪ ν otimesₘ η
  swap
...
定理 klDiv_compProd_eq_add
  结论: klDiv (μ otimesₘ κ) (ν otimesₘ η) = klDiv μ ν + klDiv (μ otimesₘ κ) (μ otimesₘ η)
  证明: by
  -- first, if we don't have absolute continuity, both sides are `∞`
  by_cases h_ac : μ otimesₘ κ ≪ ν otimesₘ η
  swap
