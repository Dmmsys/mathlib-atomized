...
English:
class IsIrreducible
  parameters: (φ : Measure α) (κ : Kernel α α)
...
instance {c : Real>=0∞} {φ : Measure α} {κ : Kernel α α} [hκ : IsIrreducible φ κ] :
    IsIrreducible (c • φ) κ where
  irreducible s hs hsp := by
simpa using hκ.irreducible hs by simp_all

/--
lemma `isIrreducible_of_le_measure` / 引理 `isIrreducible_of_le_measure`

English:
lemma isIrreducible_of_le_measure
  statement: {φ₁ φ₂ : Measure α} (hφ : φ₁ <= φ₂)
  proof: by
...
Depends on / 依赖: Std.lt_of_lt_of_le, irreducible, lt_of_lt_of_le
-/
