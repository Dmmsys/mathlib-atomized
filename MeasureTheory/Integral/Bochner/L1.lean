/-
Copyright (c) 2019 Zhouhang Zhou. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Zhouhang Zhou, Yury Kudryashov, Sébastien Gouëzel, Rémy Degenne
-/
module

public import Mathlib.MeasureTheory.Integral.SetToL1

/-!
# Bochner integral

The Bochner integral extends the definition of the Lebesgue integral to functions that map from a
measure space into a Banach space (complete normed vector space). It is constructed here
for L1 functions by extending the integral on simple functions. See the file
`Mathlib/MeasureTheory/Integral/Bochner/Basic.lean` for the integral of functions
and corresponding API.

## Main definitions

The Bochner integral is defined through the extension process described in the file
`Mathlib/MeasureTheory/Integral/SetToL1.lean`, which follows these steps:

1. Define the integral of the indicator of a set. This is `weightedSMul μ s x = μ.real s • x`.
  `weightedSMul μ` is shown to be linear in the value `x` and `DominatedFinMeasAdditive`
  (defined in the file `Mathlib/MeasureTheory/Integral/SetToL1.lean`) with respect to the set `s`.

2. Define the integral on simple functions of the type `SimpleFunc α E` (notation : `α →ₛ E`)
  where `E` is a real normed space. (See `SimpleFunc.integral` for details.)

3. Transfer this definition to define the integral on `L1.simpleFunc α E` (notation :
  `α →₁ₛ[μ] E`), see `L1.simpleFunc.integral`. Show that this integral is a continuous linear
  map from `α →₁ₛ[μ] E` to `E`.

4. Define the Bochner integral on L1 functions by extending the integral on integrable simple
  functions `α →₁ₛ[μ] E` using `ContinuousLinearMap.extend` and the fact that the embedding of
  `α →₁ₛ[μ] E` into `α →₁[μ] E` is dense.

## Notation

* `α →ₛ E` : simple functions (defined in `Mathlib/MeasureTheory/Function/SimpleFunc.lean`)
* `α →₁[μ] E` : functions in L1 space, i.e., equivalence classes of integrable functions (defined in
                `Mathlib/MeasureTheory/Function/LpSpace/Basic.lean`)
* `α →₁ₛ[μ] E` : simple functions in L1 space, i.e., equivalence classes of integrable simple
                 functions (defined in `Mathlib/MeasureTheory/Function/SimpleFuncDenseLp.lean`)

Note: `ₛ` is typed using `\_s`. Sometimes it shows as a box if the font is missing.

## Tags

Bochner integral, simple function, function space, Lebesgue dominated convergence theorem

-/

@[expose] public section


assert_not_exists Differentiable

noncomputable section

open Filter ENNReal Set
open scoped NNReal ENNReal MeasureTheory

namespace MeasureTheory

variable {α E F 𝕜 : Type*}

section WeightedSMul

open ContinuousLinearMap

variable [NormedAddCommGroup F] [NormedSpace Real F] {m : MeasurableSpace α} {μ : Measure α}

/--
Definition of `weightedSMul` / `weightedSMul` 的定义

English:
definition weightedSMul
  signature: {_ : MeasurableSpace α} (μ : Measure α) (s : Set α)
  body: μ.real s • ContinuousLinearMap.id Real F

中文:
定义 weightedSMul
  签名: {_ : 可测空间 α} (μ : 测度 α) (s : 集合 α)
  定义体: μ.real s • ContinuousLinearMap.id Real F

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.id
-/
def weightedSMul {_ : MeasurableSpace α} (μ : Measure α) (s : Set α) : F ->L[Real] F :=
  μ.real s • ContinuousLinearMap.id Real F

/--
theorem `weightedSMul_apply` / 定理 `weightedSMul_apply`

English:
theorem weightedSMul_apply
  given: {m : MeasurableSpace α} (μ : Measure α) (s : Set α) (x : F)
  proof: by simp [weightedSMul]

@[simp]

中文:
定理 weightedSMul_apply
  条件: {m : 可测空间 α} (μ : 测度 α) (s : 集合 α) (x : F)
  证明: by simp [weightedSMul]

@[simp]

Depends on / 依赖: weightedSMul
-/
theorem weightedSMul_apply {m : MeasurableSpace α} (μ : Measure α) (s : Set α) (x : F) :
    weightedSMul μ s x = μ.real s • x := by simp [weightedSMul]

@[simp]
/--
theorem `weightedSMul_zero_measure` / 定理 `weightedSMul_zero_measure`

English:
theorem weightedSMul_zero_measure
  given: {m : MeasurableSpace α}
  proof: by ext1; simp [weightedSMul]

@[simp]

中文:
定理 weightedSMul_zero_measure
  条件: {m : 可测空间 α}
  证明: by ext1; simp [weightedSMul]

@[simp]

Depends on / 依赖: weightedSMul
-/
theorem weightedSMul_zero_measure {m : MeasurableSpace α} :
    weightedSMul (0 : Measure α) = (0 : Set α -> F ->L[Real] F) := by ext1; simp [weightedSMul]

@[simp]
/--
theorem `weightedSMul_empty` / 定理 `weightedSMul_empty`

English:
theorem weightedSMul_empty
  given: {m : MeasurableSpace α} (μ : Measure α)
  proof: by ext1 x; rw [weightedSMul_apply]; simp

中文:
定理 weightedSMul_empty
  条件: {m : 可测空间 α} (μ : 测度 α)
  证明: by ext1 x; rw [weightedSMul_apply]; simp

Depends on / 依赖: weightedSMul_apply
-/
theorem weightedSMul_empty {m : MeasurableSpace α} (μ : Measure α) :
    weightedSMul μ ∅ = (0 : F ->L[Real] F) := by ext1 x; rw [weightedSMul_apply]; simp

/--
theorem `weightedSMul_add_measure` / 定理 `weightedSMul_add_measure`

English:
theorem weightedSMul_add_measure
  statement: {m : MeasurableSpace α} (μ ν : Measure α) {s : Set α}
  proof: by
  ext1 x
  push_cast
  simp_rw [Pi.add_apply, weightedSMul_apply]
  rw [measureReal_add_apply]; rw [add_smul]

中文:
定理 weightedSMul_add_measure
  结论: {m : 可测空间 α} (μ ν : 测度 α) {s : 集合 α}
  证明: by
  ext1 x
  push_cast
  simp_rw [Pi.add_apply, weightedSMul_apply]
  rw [measureReal_add_apply]; rw [add_smul]

Depends on / 依赖: Pi.add_apply, add_apply, add_smul, measureReal_add_apply, simp_rw, weightedSMul_apply
-/
theorem weightedSMul_add_measure {m : MeasurableSpace α} (μ ν : Measure α) {s : Set α}
    (hμs : μ s != ∞) (hνs : ν s != ∞) :
    (weightedSMul (μ + ν) s : F ->L[Real] F) = weightedSMul μ s + weightedSMul ν s := by
  ext1 x
  push_cast
  simp_rw [Pi.add_apply, weightedSMul_apply]
  rw [measureReal_add_apply]; rw [add_smul]

/--
theorem `weightedSMul_smul_measure` / 定理 `weightedSMul_smul_measure`

English:
theorem weightedSMul_smul_measure
  given: {m : MeasurableSpace α} (μ : Measure α) (c : Real>=0∞) {s : Set α}
  proof: by
  ext1 x
  simp [weightedSMul_apply, smul_smul]

中文:
定理 weightedSMul_smul_measure
  条件: {m : 可测空间 α} (μ : 测度 α) (c : 实数>=0∞) {s : 集合 α}
  证明: by
  ext1 x
  simp [weightedSMul_apply, smul_smul]

Depends on / 依赖: smul_smul, weightedSMul_apply
-/
theorem weightedSMul_smul_measure {m : MeasurableSpace α} (μ : Measure α) (c : Real>=0∞) {s : Set α} :
    (weightedSMul (c • μ) s : F ->L[Real] F) = c.toReal • weightedSMul μ s := by
  ext1 x
  simp [weightedSMul_apply, smul_smul]

/--
theorem `weightedSMul_congr` / 定理 `weightedSMul_congr`

English:
theorem weightedSMul_congr
  given: (s t : Set α) (hst : μ s = μ t)
  proof: by
  ext1 x; simp_rw [weightedSMul_apply, measureReal_def]; congr 2

中文:
定理 weightedSMul_congr
  条件: (s t : 集合 α) (hst : μ s = μ t)
  证明: by
  ext1 x; simp_rw [weightedSMul_apply, measureReal_def]; congr 2

Depends on / 依赖: measureReal_def, simp_rw, weightedSMul_apply
-/
theorem weightedSMul_congr (s t : Set α) (hst : μ s = μ t) :
    (weightedSMul μ s : F ->L[Real] F) = weightedSMul μ t := by
  ext1 x; simp_rw [weightedSMul_apply, measureReal_def]; congr 2

/--
theorem `weightedSMul_null` / 定理 `weightedSMul_null`

English:
theorem weightedSMul_null
  given: {s : Set α} (h_zero : μ s = 0)
  statement: (weightedSMul μ s : F ->L[Real] F) = 0
  proof: by
  ext1 x; rw [weightedSMul_apply, measureReal_def, h_zero]; simp

中文:
定理 weightedSMul_null
  条件: {s : 集合 α} (h_zero : μ s = 0)
  结论: (weightedSMul μ s : F ->L[实数] F) = 0
  证明: by
  ext1 x; rw [weightedSMul_apply, measureReal_def, h_zero]; simp

Depends on / 依赖: h_zero, measureReal_def, weightedSMul_apply
-/
theorem weightedSMul_null {s : Set α} (h_zero : μ s = 0) : (weightedSMul μ s : F ->L[Real] F) = 0 := by
  ext1 x; rw [weightedSMul_apply, measureReal_def, h_zero]; simp

/--
theorem `weightedSMul_union'` / 定理 `weightedSMul_union'`

English:
theorem weightedSMul_union'
  statement: (s t : Set α) (ht : MeasurableSet t) (hs_finite : μ s != ∞)
  proof: by
  ext1 x
  simp_rw [add_apply, weightedSMul_apply, measureReal_union hdisj ht, add_smul]

@[nolint unusedArguments]

中文:
定理 weightedSMul_union'
  结论: (s t : 集合 α) (ht : 可测集 t) (hs_finite : μ s != ∞)
  证明: by
  ext1 x
  simp_rw [add_apply, weightedSMul_apply, measureReal_union hdisj ht, add_smul]

@[nolint unusedArguments]

Depends on / 依赖: add_apply, add_smul, measureReal_union, simp_rw, weightedSMul_apply
-/
theorem weightedSMul_union' (s t : Set α) (ht : MeasurableSet t) (hs_finite : μ s != ∞)
    (ht_finite : μ t != ∞) (hdisj : Disjoint s t) :
    (weightedSMul μ (s union t) : F ->L[Real] F) = weightedSMul μ s + weightedSMul μ t := by
  ext1 x
  simp_rw [add_apply, weightedSMul_apply, measureReal_union hdisj ht, add_smul]

@[nolint unusedArguments]
/--
theorem `weightedSMul_union` / 定理 `weightedSMul_union`

English:
theorem weightedSMul_union
  statement: (s t : Set α) (_hs : MeasurableSet s) (ht : MeasurableSet t)
  proof: weightedSMul_union' s t ht hs_finite ht_finite hdisj

中文:
定理 weightedSMul_union
  结论: (s t : 集合 α) (_hs : 可测集 s) (ht : 可测集 t)
  证明: weightedSMul_union' s t ht hs_finite ht_finite hdisj

Depends on / 依赖: hs_finite, ht_finite, weightedSMul_union
-/
theorem weightedSMul_union (s t : Set α) (_hs : MeasurableSet s) (ht : MeasurableSet t)
    (hs_finite : μ s != ∞) (ht_finite : μ t != ∞) (hdisj : Disjoint s t) :
    (weightedSMul μ (s union t) : F ->L[Real] F) = weightedSMul μ s + weightedSMul μ t :=
  weightedSMul_union' s t ht hs_finite ht_finite hdisj

/--
theorem `weightedSMul_smul` / 定理 `weightedSMul_smul`

English:
theorem weightedSMul_smul
  statement: [SMul 𝕜 F] [SMulCommClass Real 𝕜 F] (c : 𝕜)
  proof: by
  simp_rw [weightedSMul_apply, smul_comm]

中文:
定理 weightedSMul_smul
  结论: [标量乘法 𝕜 F] [标量交换类 实数 𝕜 F] (c : 𝕜)
  证明: by
  simp_rw [weightedSMul_apply, smul_comm]

Depends on / 依赖: simp_rw, smul_comm, weightedSMul_apply
-/
theorem weightedSMul_smul [SMul 𝕜 F] [SMulCommClass Real 𝕜 F] (c : 𝕜)
    (s : Set α) (x : F) : weightedSMul μ s (c • x) = c • weightedSMul μ s x := by
  simp_rw [weightedSMul_apply, smul_comm]

/--
theorem `norm_weightedSMul_le` / 定理 `norm_weightedSMul_le`

English:
theorem norm_weightedSMul_le
  given: (s : Set α)
  statement: ‖(weightedSMul μ s : F ->L[Real] F)‖ <= μ.real s
  proof: calc
    ‖(weightedSMul μ s : F ->L[Real] F)‖ = ‖μ.real s‖ * ‖ContinuousLinearMap.id Real F‖ :=
      norm_smul (μ.real s) (ContinuousLinearMap.id Real F)
    _ <= ‖μ.real s‖ :=
      ((mul_le_mul_of_nonneg_left norm_id_le (norm_nonneg _)).trans (mul_one _).le)
    _ = abs μ.real s := Real.norm_eq_a

中文:
定理 norm_weightedSMul_le
  条件: (s : 集合 α)
  结论: ‖(weightedSMul μ s : F ->L[实数] F)‖ <= μ.real s
  证明: calc
    ‖(weightedSMul μ s : F ->L[Real] F)‖ = ‖μ.real s‖ * ‖ContinuousLinearMap.id Real F‖ :=
      norm_smul (μ.real s) (ContinuousLinearMap.id Real F)
    _ <= ‖μ.real s‖ :=
      ((mul_le_mul_of_nonneg_left norm_id_le (norm_nonneg _)).trans (mul_one _).le)
    _ = abs μ.real s := Real.norm_eq_a

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.id, ENNReal, ENNReal.toReal_nonneg, Real.norm_eq_abs, abs_eq_self, abs_eq_self.mpr, mul_le_mul_of_nonneg_left, mul_one, norm_eq_abs, norm_id_le, norm_nonneg, norm_smul, toReal_nonneg, weightedSMul
-/
theorem norm_weightedSMul_le (s : Set α) : ‖(weightedSMul μ s : F ->L[Real] F)‖ <= μ.real s :=
  calc
    ‖(weightedSMul μ s : F ->L[Real] F)‖ = ‖μ.real s‖ * ‖ContinuousLinearMap.id Real F‖ :=
      norm_smul (μ.real s) (ContinuousLinearMap.id Real F)
    _ <= ‖μ.real s‖ :=
      ((mul_le_mul_of_nonneg_left norm_id_le (norm_nonneg _)).trans (mul_one _).le)
    _ = abs μ.real s := Real.norm_eq_abs _
    _ = μ.real s := abs_eq_self.mpr ENNReal.toReal_nonneg

/--
theorem `dominatedFinMeasAdditive_weightedSMul` / 定理 `dominatedFinMeasAdditive_weightedSMul`

English:
theorem dominatedFinMeasAdditive_weightedSMul
  given: {_ : MeasurableSpace α} (μ : Measure α)
  proof: ⟨weightedSMul_union, fun s _ _ => (norm_weightedSMul_le s).trans (one_mul _).symm.le⟩

中文:
定理 dominatedFinMeasAdditive_weightedSMul
  条件: {_ : 可测空间 α} (μ : 测度 α)
  证明: ⟨weightedSMul_union, fun s _ _ => (norm_weightedSMul_le s).trans (one_mul _).symm.le⟩

Depends on / 依赖: norm_weightedSMul_le, one_mul, symm.le, weightedSMul_union
-/
theorem dominatedFinMeasAdditive_weightedSMul {_ : MeasurableSpace α} (μ : Measure α) :
    DominatedFinMeasAdditive μ (weightedSMul μ : Set α -> F ->L[Real] F) 1 :=
  ⟨weightedSMul_union, fun s _ _ => (norm_weightedSMul_le s).trans (one_mul _).symm.le⟩

/--
theorem `weightedSMul_nonneg` / 定理 `weightedSMul_nonneg`

English:
theorem weightedSMul_nonneg
  statement: [PartialOrder F] [IsOrderedModule Real F]
  proof: by
  simp only [weightedSMul, _root_.id, coe_id', smul_apply]
  exact smul_nonneg toReal_nonneg hx

中文:
定理 weightedSMul_nonneg
  结论: [偏序 F] [是Ordered模 实数 F]
  证明: by
  simp only [weightedSMul, _root_.id, coe_id', smul_apply]
  exact smul_nonneg toReal_nonneg hx

Depends on / 依赖: _root_, _root_.id, coe_id, smul_apply, smul_nonneg, toReal_nonneg, weightedSMul
-/
theorem weightedSMul_nonneg [PartialOrder F] [IsOrderedModule Real F]
    (s : Set α) (x : F) (hx : 0 <= x) : 0 <= weightedSMul μ s x := by
  simp only [weightedSMul, _root_.id, coe_id', smul_apply]
  exact smul_nonneg toReal_nonneg hx

end WeightedSMul

local infixr:25 " ->ₛ " => SimpleFunc

namespace SimpleFunc

section PosPart

variable [LinearOrder E] [Zero E] [MeasurableSpace α]

/--
Definition of `posPart` / `posPart` 的定义

English:
definition posPart
  signature: (f : α ->ₛ E)
  body: f.map fun b => max b 0

中文:
定义 posPart
  签名: (f : α ->ₛ E)
  定义体: f.map fun b => max b 0

Depends on / 依赖: f.map
-/
def posPart (f : α ->ₛ E) : α ->ₛ E :=
  f.map fun b => max b 0

/--
Definition of `negPart` / `negPart` 的定义

English:
definition negPart
  signature: [Neg E] (f : α ->ₛ E)
  body: posPart (-f)

中文:
定义 negPart
  签名: [取负 E] (f : α ->ₛ E)
  定义体: posPart (-f)

Depends on / 依赖: posPart
-/
def negPart [Neg E] (f : α ->ₛ E) : α ->ₛ E :=
  posPart (-f)

/--
theorem `posPart_map_norm` / 定理 `posPart_map_norm`

English:
theorem posPart_map_norm
  given: (f : α ->ₛ Real)
  statement: (posPart f).map norm = posPart f
  proof: by
  ext; rw [map_apply, Real.norm_eq_abs, abs_of_nonneg]; exact le_max_right _ _

中文:
定理 posPart_map_norm
  条件: (f : α ->ₛ 实数)
  结论: (posPart f).map norm = posPart f
  证明: by
  ext; rw [map_apply, Real.norm_eq_abs, abs_of_nonneg]; exact le_max_right _ _

Depends on / 依赖: Real.norm_eq_abs, abs_of_nonneg, le_max_right, map_apply, norm_eq_abs
-/
theorem posPart_map_norm (f : α ->ₛ Real) : (posPart f).map norm = posPart f := by
  ext; rw [map_apply, Real.norm_eq_abs, abs_of_nonneg]; exact le_max_right _ _

/--
theorem `negPart_map_norm` / 定理 `negPart_map_norm`

English:
theorem negPart_map_norm
  given: (f : α ->ₛ Real)
  statement: (negPart f).map norm = negPart f
  proof: by
  rw [negPart]; exact posPart_map_norm _

中文:
定理 negPart_map_norm
  条件: (f : α ->ₛ 实数)
  结论: (negPart f).map norm = negPart f
  证明: by
  rw [negPart]; exact posPart_map_norm _

Depends on / 依赖: negPart, posPart_map_norm
-/
theorem negPart_map_norm (f : α ->ₛ Real) : (negPart f).map norm = negPart f := by
  rw [negPart]; exact posPart_map_norm _

/--
theorem `posPart_sub_negPart` / 定理 `posPart_sub_negPart`

English:
theorem posPart_sub_negPart
  given: (f : α ->ₛ Real)
  statement: f.posPart - f.negPart = f
  proof: by
  simp only [posPart, negPart]
  ext a
  rw [coe_sub]
  exact max_zero_sub_eq_self (f a)

中文:
定理 posPart_sub_negPart
  条件: (f : α ->ₛ 实数)
  结论: f.posPart - f.negPart = f
  证明: by
  simp only [posPart, negPart]
  ext a
  rw [coe_sub]
  exact max_zero_sub_eq_self (f a)

Depends on / 依赖: coe_sub, max_zero_sub_eq_self, negPart, posPart
-/
theorem posPart_sub_negPart (f : α ->ₛ Real) : f.posPart - f.negPart = f := by
  simp only [posPart, negPart]
  ext a
  rw [coe_sub]
  exact max_zero_sub_eq_self (f a)

end PosPart

section Integral

/-!
### The Bochner integral of simple functions

Define the Bochner integral of simple functions of the type `α →ₛ β` where `β` is a normed group,
and prove basic properties of this integral.
-/


open Finset

variable [NormedAddCommGroup E] [NormedAddCommGroup F] [NormedSpace Real F]
  {m : MeasurableSpace α} {μ : Measure α}

/--
Definition of `integral` / `integral` 的定义

English:
definition integral
  signature: {_ : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F)
  body: f.setToSimpleFunc (weightedSMul μ)

中文:
定义 integral
  签名: {_ : 可测空间 α} (μ : 测度 α) (f : α ->ₛ F)
  定义体: f.setToSimpleFunc (weightedSMul μ)

Depends on / 依赖: f.setToSimpleFunc, setToSimpleFunc, weightedSMul
-/
def integral {_ : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F) : F :=
  f.setToSimpleFunc (weightedSMul μ)

/--
theorem `integral_def` / 定理 `integral_def`

English:
theorem integral_def
  given: {_ : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F)
  proof: rfl

中文:
定理 integral_def
  条件: {_ : 可测空间 α} (μ : 测度 α) (f : α ->ₛ F)
  证明: rfl
-/
theorem integral_def {_ : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F) :
    f.integral μ = f.setToSimpleFunc (weightedSMul μ) := rfl

/--
theorem `integral_eq` / 定理 `integral_eq`

English:
theorem integral_eq
  given: {m : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F)
  proof: by
  simp [integral, setToSimpleFunc, weightedSMul_apply]

中文:
定理 integral_eq
  条件: {m : 可测空间 α} (μ : 测度 α) (f : α ->ₛ F)
  证明: by
  simp [integral, setToSimpleFunc, weightedSMul_apply]

Depends on / 依赖: integral, setToSimpleFunc, weightedSMul_apply
-/
theorem integral_eq {m : MeasurableSpace α} (μ : Measure α) (f : α ->ₛ F) :
    f.integral μ = ∑ x in f.range, μ.real (f ⁻¹' {x}) • x := by
  simp [integral, setToSimpleFunc, weightedSMul_apply]

/--
theorem `integral_eq_sum_filter` / 定理 `integral_eq_sum_filter`

English:
theorem integral_eq_sum_filter
  statement: [DecidablePred fun x : F => x != 0] {m : MeasurableSpace α}
  proof: by
  simp_rw [integral_def, setToSimpleFunc_eq_sum_filter, weightedSMul_apply]

中文:
定理 integral_eq_sum_filter
  结论: [DecidablePred fun x : F => x != 0] {m : 可测空间 α}
  证明: by
  simp_rw [integral_def, setToSimpleFunc_eq_sum_filter, weightedSMul_apply]

Depends on / 依赖: integral_def, setToSimpleFunc_eq_sum_filter, simp_rw, weightedSMul_apply
-/
theorem integral_eq_sum_filter [DecidablePred fun x : F => x != 0] {m : MeasurableSpace α}
    (f : α ->ₛ F) (μ : Measure α) :
    f.integral μ = ∑ x in {x in f.range | x != 0}, μ.real (f ⁻¹' {x}) • x := by
  simp_rw [integral_def, setToSimpleFunc_eq_sum_filter, weightedSMul_apply]

/--
theorem `integral_eq_sum_of_subset` / 定理 `integral_eq_sum_of_subset`

English:
theorem integral_eq_sum_of_subset
  statement: [DecidablePred fun x : F => x != 0] {f : α ->ₛ F} {s : Finset F}
  proof: by
  rw [SimpleFunc.integral_eq_sum_filter]; rw [Finset.sum_subset hs]
  rintro x - hx; rw [Finset.mem_filter, not_and_or, Ne, Classical.not_not] at hx
  rcases hx.symm with (rfl | hx)
  · simp
  rw [SimpleFunc.mem_range] at hx
  rw [preimage_eq_empty] <;> simp [Set.disjoint_singleton_left, hx]

@[s

中文:
定理 integral_eq_sum_of_subset
  结论: [DecidablePred fun x : F => x != 0] {f : α ->ₛ F} {s : 有限集 F}
  证明: by
  rw [SimpleFunc.integral_eq_sum_filter]; rw [Finset.sum_subset hs]
  rintro x - hx; rw [Finset.mem_filter, not_and_or, Ne, Classical.not_not] at hx
  rcases hx.symm with (rfl | hx)
  · simp
  rw [SimpleFunc.mem_range] at hx
  rw [preimage_eq_empty] <;> simp [Set.disjoint_singleton_left, hx]

@[s

Depends on / 依赖: Classical, Classical.not_not, Finset, Finset.mem_filter, Finset.sum_subset, Set.disjoint_singleton_left, SimpleFunc, SimpleFunc.integral_eq_sum_filter, SimpleFunc.mem_range, disjoint_singleton_left, hx.symm, integral_eq_sum_filter, mem_filter, mem_range, not_and_or, not_not, preimage_eq_empty, sum_subset
-/
theorem integral_eq_sum_of_subset [DecidablePred fun x : F => x != 0] {f : α ->ₛ F} {s : Finset F}
    (hs : {x in f.range | x != 0} subseteq s) :
    f.integral μ = ∑ x in s, μ.real (f ⁻¹' {x}) • x := by
  rw [SimpleFunc.integral_eq_sum_filter]; rw [Finset.sum_subset hs]
  rintro x - hx; rw [Finset.mem_filter, not_and_or, Ne, Classical.not_not] at hx
  rcases hx.symm with (rfl | hx)
  · simp
  rw [SimpleFunc.mem_range] at hx
  rw [preimage_eq_empty] <;> simp [Set.disjoint_singleton_left, hx]

@[simp]
/--
theorem `integral_const` / 定理 `integral_const`

English:
theorem integral_const
  given: {m : MeasurableSpace α} (μ : Measure α) (y : F)
  proof: by
  classical
  calc
    (const α y).integral μ = ∑ z in {y}, μ.real (const α y ⁻¹' {z}) • z :=
integral_eq_sum_of_subset (filter_subset _ _).trans (range_const_subset _ _)
    _ = μ.real univ • y := by simp [Set.preimage]

中文:
定理 integral_const
  条件: {m : 可测空间 α} (μ : 测度 α) (y : F)
  证明: by
  classical
  calc
    (const α y).integral μ = ∑ z in {y}, μ.real (const α y ⁻¹' {z}) • z :=
integral_eq_sum_of_subset (filter_subset _ _).trans (range_const_subset _ _)
    _ = μ.real univ • y := by simp [Set.preimage]

Depends on / 依赖: Set.preimage, classical, filter_subset, integral, integral_eq_sum_of_subset, preimage, range_const_subset
-/
theorem integral_const {m : MeasurableSpace α} (μ : Measure α) (y : F) :
    (const α y).integral μ = μ.real univ • y := by
  classical
  calc
    (const α y).integral μ = ∑ z in {y}, μ.real (const α y ⁻¹' {z}) • z :=
integral_eq_sum_of_subset (filter_subset _ _).trans (range_const_subset _ _)
    _ = μ.real univ • y := by simp [Set.preimage]

set_option backward.defeqAttrib.useBackward true in
@[simp]
/--
theorem `integral_piecewise_zero` / 定理 `integral_piecewise_zero`

English:
theorem integral_piecewise_zero
  statement: {m : MeasurableSpace α} (f : α ->ₛ F) (μ : Measure α) {s : Set α}
  proof: by
  classical
  refine (integral_eq_sum_of_subset ?_).trans
      ((sum_congr rfl fun y hy => ?_).trans (integral_eq_sum_filter _ _).symm)
  · intro y hy
    simp only [mem_filter, mem_range, coe_piecewise, coe_zero, piecewise_eq_indicator,
      mem_range_indicator] at *
    rcases hy with ⟨⟨rfl, 

中文:
定理 integral_piecewise_zero
  结论: {m : 可测空间 α} (f : α ->ₛ F) (μ : 测度 α) {s : 集合 α}
  证明: by
  classical
  refine (integral_eq_sum_of_subset ?_).trans
      ((sum_congr rfl fun y hy => ?_).trans (integral_eq_sum_filter _ _).symm)
  · intro y hy
    simp only [mem_filter, mem_range, coe_piecewise, coe_zero, piecewise_eq_indicator,
      mem_range_indicator] at *
    rcases hy with ⟨⟨rfl, 

Depends on / 依赖: Set.mem_range_self, Set.piecewise_eq_indicator, classical, coe_piecewise, coe_zero, exacts, f.measurableSet_preimage, indicator_preimage_of_notMem, integral_eq_sum_filter, integral_eq_sum_of_subset, measurableSet_preimage, measureReal_restrict_apply, mem_filter, mem_range, mem_range_indicator, mem_range_self, piecewise_eq_indicator, sum_congr
-/
theorem integral_piecewise_zero {m : MeasurableSpace α} (f : α ->ₛ F) (μ : Measure α) {s : Set α}
    (hs : MeasurableSet s) : (piecewise s hs f 0).integral μ = f.integral (μ.restrict s) := by
  classical
  refine (integral_eq_sum_of_subset ?_).trans
      ((sum_congr rfl fun y hy => ?_).trans (integral_eq_sum_filter _ _).symm)
  · intro y hy
    simp only [mem_filter, mem_range, coe_piecewise, coe_zero, piecewise_eq_indicator,
      mem_range_indicator] at *
    rcases hy with ⟨⟨rfl, -⟩ | ⟨x, -, rfl⟩, h₀⟩
    exacts [(h₀ rfl).elim, ⟨Set.mem_range_self _, h₀⟩]
  · dsimp
    rw [Set.piecewise_eq_indicator]; rw [indicator_preimage_of_notMem]; rw [measureReal_restrict_apply (f.measurableSet_preimage _)]
    exact fun h₀ => (mem_filter.1 hy).2 (Eq.symm h₀)

/--
theorem `map_integral` / 定理 `map_integral`

English:
theorem map_integral
  given: (f : α ->ₛ E) (g : E -> F) (hf : Integrable f μ) (hg : g 0 = 0)
  proof: map_setToSimpleFunc _ weightedSMul_union hf hg

中文:
定理 map_integral
  条件: (f : α ->ₛ E) (g : E -> F) (hf : 可积 f μ) (hg : g 0 = 0)
  证明: map_setToSimpleFunc _ weightedSMul_union hf hg

Depends on / 依赖: map_setToSimpleFunc, weightedSMul_union
-/
theorem map_integral (f : α ->ₛ E) (g : E -> F) (hf : Integrable f μ) (hg : g 0 = 0) :
    (f.map g).integral μ = ∑ x in f.range, (μ.real (f ⁻¹' {x})) • g x :=
  map_setToSimpleFunc _ weightedSMul_union hf hg

/--
theorem `integral_eq_lintegral'` / 定理 `integral_eq_lintegral'`

English:
theorem integral_eq_lintegral'
  statement: {f : α ->ₛ E} {g : E -> Real>=0∞} (hf : Integrable f μ) (hg0 : g 0 = 0)
  proof: by
  have hf' : f.FinMeasSupp μ := integrable_iff_finMeasSupp.1 hf
  simp only [← map_apply g f, lintegral_eq_lintegral]
  rw [map_integral f _ hf]; rw [map_lintegral]; rw [ENNReal.toReal_sum]
  · refine Finset.sum_congr rfl fun b _ => ?_
    rw [smul_eq_mul]; rw [toReal_mul]; rw [mul_comm]; rw [Fun

中文:
定理 integral_eq_lintegral'
  结论: {f : α ->ₛ E} {g : E -> 实数>=0∞} (hf : 可积 f μ) (hg0 : g 0 = 0)
  证明: by
  have hf' : f.FinMeasSupp μ := integrable_iff_finMeasSupp.1 hf
  simp only [← map_apply g f, lintegral_eq_lintegral]
  rw [map_integral f _ hf]; rw [map_lintegral]; rw [ENNReal.toReal_sum]
  · refine Finset.sum_congr rfl fun b _ => ?_
    rw [smul_eq_mul]; rw [toReal_mul]; rw [mul_comm]; rw [Fun

Depends on / 依赖: ENNReal, ENNReal.toReal_sum, FinMeasSupp, Finset, Finset.sum_congr, Function, Function.comp_apply, WithTop, WithTop.zero_ne_top, comp_apply, f.FinMeasSupp, integrable_iff_finMeasSupp, lintegral_eq_lintegral, map_apply, map_integral, map_lintegral, meas_preimage_singleton_ne_zero, measureReal_def, mul_comm, mul_ne_top
-/
theorem integral_eq_lintegral' {f : α ->ₛ E} {g : E -> Real>=0∞} (hf : Integrable f μ) (hg0 : g 0 = 0)
    (ht : forall b, g b != ∞) :
    (f.map (ENNReal.toReal ∘ g)).integral μ = ENNReal.toReal (∫⁻ a, g (f a) ∂μ) := by
  have hf' : f.FinMeasSupp μ := integrable_iff_finMeasSupp.1 hf
  simp only [← map_apply g f, lintegral_eq_lintegral]
  rw [map_integral f _ hf]; rw [map_lintegral]; rw [ENNReal.toReal_sum]
  · refine Finset.sum_congr rfl fun b _ => ?_
    rw [smul_eq_mul]; rw [toReal_mul]; rw [mul_comm]; rw [Function.comp_apply]; rw [measureReal_def]
  · rintro a -
    by_cases a0 : a = 0
    · rw [a0, hg0, zero_mul]; exact WithTop.zero_ne_top
    · apply mul_ne_top (ht a) (hf'.meas_preimage_singleton_ne_zero a0).ne
  · simp [hg0]

variable [NormedSpace Real E]

/--
theorem `integral_congr` / 定理 `integral_congr`

English:
theorem integral_congr
  given: {f g : α ->ₛ E} (hf : Integrable f μ) (h : f =ᵐ[μ] g)
  proof: setToSimpleFunc_congr (weightedSMul μ) (fun _ _ => weightedSMul_null) weightedSMul_union hf h

中文:
定理 integral_congr
  条件: {f g : α ->ₛ E} (hf : 可积 f μ) (h : f =ᵐ[μ] g)
  证明: setToSimpleFunc_congr (weightedSMul μ) (fun _ _ => weightedSMul_null) weightedSMul_union hf h

Depends on / 依赖: setToSimpleFunc_congr, weightedSMul, weightedSMul_null, weightedSMul_union
-/
theorem integral_congr {f g : α ->ₛ E} (hf : Integrable f μ) (h : f =ᵐ[μ] g) :
    f.integral μ = g.integral μ :=
  setToSimpleFunc_congr (weightedSMul μ) (fun _ _ => weightedSMul_null) weightedSMul_union hf h

/--
theorem `integral_eq_lintegral` / 定理 `integral_eq_lintegral`

English:
theorem integral_eq_lintegral
  given: {f : α ->ₛ Real} (hf : Integrable f μ) (h_pos : 0 <=ᵐ[μ] f)
  proof: by
  have : f =ᵐ[μ] f.map (ENNReal.toReal ∘ ENNReal.ofReal) :=
    h_pos.mono fun a h => (ENNReal.toReal_ofReal h).symm
  rw [← integral_eq_lintegral' hf]
  exacts [integral_congr hf this, ENNReal.ofReal_zero, fun b => ENNReal.ofReal_ne_top]

中文:
定理 integral_eq_lintegral
  条件: {f : α ->ₛ 实数} (hf : 可积 f μ) (h_pos : 0 <=ᵐ[μ] f)
  证明: by
  have : f =ᵐ[μ] f.map (ENNReal.toReal ∘ ENNReal.ofReal) :=
    h_pos.mono fun a h => (ENNReal.toReal_ofReal h).symm
  rw [← integral_eq_lintegral' hf]
  exacts [integral_congr hf this, ENNReal.ofReal_zero, fun b => ENNReal.ofReal_ne_top]

Depends on / 依赖: ENNReal, ENNReal.ofReal, ENNReal.ofReal_ne_top, ENNReal.ofReal_zero, ENNReal.toReal, ENNReal.toReal_ofReal, exacts, f.map, h_pos, h_pos.mono, integral_congr, integral_eq_lintegral, ofReal, ofReal_ne_top, ofReal_zero, toReal, toReal_ofReal
-/
theorem integral_eq_lintegral {f : α ->ₛ Real} (hf : Integrable f μ) (h_pos : 0 <=ᵐ[μ] f) :
    f.integral μ = ENNReal.toReal (∫⁻ a, ENNReal.ofReal (f a) ∂μ) := by
  have : f =ᵐ[μ] f.map (ENNReal.toReal ∘ ENNReal.ofReal) :=
    h_pos.mono fun a h => (ENNReal.toReal_ofReal h).symm
  rw [← integral_eq_lintegral' hf]
  exacts [integral_congr hf this, ENNReal.ofReal_zero, fun b => ENNReal.ofReal_ne_top]

/--
theorem `integral_add` / 定理 `integral_add`

English:
theorem integral_add
  given: {f g : α ->ₛ E} (hf : Integrable f μ) (hg : Integrable g μ)
  proof: setToSimpleFunc_add _ weightedSMul_union hf hg

中文:
定理 integral_add
  条件: {f g : α ->ₛ E} (hf : 可积 f μ) (hg : 可积 g μ)
  证明: setToSimpleFunc_add _ weightedSMul_union hf hg

Depends on / 依赖: setToSimpleFunc_add, weightedSMul_union
-/
theorem integral_add {f g : α ->ₛ E} (hf : Integrable f μ) (hg : Integrable g μ) :
    integral μ (f + g) = integral μ f + integral μ g :=
  setToSimpleFunc_add _ weightedSMul_union hf hg

/--
theorem `integral_neg` / 定理 `integral_neg`

English:
theorem integral_neg
  given: {f : α ->ₛ E} (hf : Integrable f μ)
  statement: integral μ (-f) = -integral μ f
  proof: setToSimpleFunc_neg _ weightedSMul_union hf

中文:
定理 integral_neg
  条件: {f : α ->ₛ E} (hf : 可积 f μ)
  结论: integral μ (-f) = -integral μ f
  证明: setToSimpleFunc_neg _ weightedSMul_union hf

Depends on / 依赖: setToSimpleFunc_neg, weightedSMul_union
-/
theorem integral_neg {f : α ->ₛ E} (hf : Integrable f μ) : integral μ (-f) = -integral μ f :=
  setToSimpleFunc_neg _ weightedSMul_union hf

/--
theorem `integral_sub` / 定理 `integral_sub`

English:
theorem integral_sub
  given: {f g : α ->ₛ E} (hf : Integrable f μ) (hg : Integrable g μ)
  proof: setToSimpleFunc_sub _ weightedSMul_union hf hg

中文:
定理 integral_sub
  条件: {f g : α ->ₛ E} (hf : 可积 f μ) (hg : 可积 g μ)
  证明: setToSimpleFunc_sub _ weightedSMul_union hf hg

Depends on / 依赖: setToSimpleFunc_sub, weightedSMul_union
-/
theorem integral_sub {f g : α ->ₛ E} (hf : Integrable f μ) (hg : Integrable g μ) :
    integral μ (f - g) = integral μ f - integral μ g :=
  setToSimpleFunc_sub _ weightedSMul_union hf hg

/--
theorem `integral_smul` / 定理 `integral_smul`

English:
theorem integral_smul
  statement: [DistribSMul 𝕜 E] [SMulCommClass Real 𝕜 E]
  proof: setToSimpleFunc_smul _ weightedSMul_union weightedSMul_smul c hf

中文:
定理 integral_smul
  结论: [分配标量乘法 𝕜 E] [标量交换类 实数 𝕜 E]
  证明: setToSimpleFunc_smul _ weightedSMul_union weightedSMul_smul c hf

Depends on / 依赖: setToSimpleFunc_smul, weightedSMul_smul, weightedSMul_union
-/
theorem integral_smul [DistribSMul 𝕜 E] [SMulCommClass Real 𝕜 E]
    (c : 𝕜) {f : α ->ₛ E} (hf : Integrable f μ) :
    integral μ (c • f) = c • integral μ f :=
  setToSimpleFunc_smul _ weightedSMul_union weightedSMul_smul c hf

/--
theorem `norm_setToSimpleFunc_le_integral_norm` / 定理 `norm_setToSimpleFunc_le_integral_norm`

English:
theorem norm_setToSimpleFunc_le_integral_norm
  statement: (T : Set α -> E ->L[Real] F) {C : Real}
  proof: calc
    ‖f.setToSimpleFunc T‖ <= C * ∑ x in f.range, μ.real (f ⁻¹' {x}) * ‖x‖ :=
      norm_setToSimpleFunc_le_sum_mul_norm_of_integrable T hT_norm f hf
    _ = C * (f.map norm).integral μ := by
      rw [map_integral f norm hf norm_zero]; simp_rw [smul_eq_mul]

中文:
定理 norm_setToSimpleFunc_le_integral_norm
  结论: (T : 集合 α -> E ->L[实数] F) {C : 实数}
  证明: calc
    ‖f.setToSimpleFunc T‖ <= C * ∑ x in f.range, μ.real (f ⁻¹' {x}) * ‖x‖ :=
      norm_setToSimpleFunc_le_sum_mul_norm_of_integrable T hT_norm f hf
    _ = C * (f.map norm).integral μ := by
      rw [map_integral f norm hf norm_zero]; simp_rw [smul_eq_mul]

Depends on / 依赖: f.map, f.range, f.setToSimpleFunc, hT_norm, integral, map_integral, norm_setToSimpleFunc_le_sum_mul_norm_of_integrable, norm_zero, setToSimpleFunc, simp_rw, smul_eq_mul
-/
theorem norm_setToSimpleFunc_le_integral_norm (T : Set α -> E ->L[Real] F) {C : Real}
    (hT_norm : forall s, MeasurableSet s -> μ s < ∞ -> ‖T s‖ <= C * μ.real s) {f : α ->ₛ E}
    (hf : Integrable f μ) : ‖f.setToSimpleFunc T‖ <= C * (f.map norm).integral μ :=
  calc
    ‖f.setToSimpleFunc T‖ <= C * ∑ x in f.range, μ.real (f ⁻¹' {x}) * ‖x‖ :=
      norm_setToSimpleFunc_le_sum_mul_norm_of_integrable T hT_norm f hf
    _ = C * (f.map norm).integral μ := by
      rw [map_integral f norm hf norm_zero]; simp_rw [smul_eq_mul]

/--
theorem `norm_integral_le_integral_norm` / 定理 `norm_integral_le_integral_norm`

English:
theorem norm_integral_le_integral_norm
  given: (f : α ->ₛ E) (hf : Integrable f μ)
  proof: by
  refine (norm_setToSimpleFunc_le_integral_norm _ (fun s _ _ => ?_) hf).trans (one_mul _).le
  exact (norm_weightedSMul_le s).trans (one_mul _).symm.le

中文:
定理 norm_integral_le_integral_norm
  条件: (f : α ->ₛ E) (hf : 可积 f μ)
  证明: by
  refine (norm_setToSimpleFunc_le_integral_norm _ (fun s _ _ => ?_) hf).trans (one_mul _).le
  exact (norm_weightedSMul_le s).trans (one_mul _).symm.le

Depends on / 依赖: norm_setToSimpleFunc_le_integral_norm, norm_weightedSMul_le, one_mul, symm.le
-/
theorem norm_integral_le_integral_norm (f : α ->ₛ E) (hf : Integrable f μ) :
    ‖f.integral μ‖ <= (f.map norm).integral μ := by
  refine (norm_setToSimpleFunc_le_integral_norm _ (fun s _ _ => ?_) hf).trans (one_mul _).le
  exact (norm_weightedSMul_le s).trans (one_mul _).symm.le

/--
theorem `integral_add_measure` / 定理 `integral_add_measure`

English:
theorem integral_add_measure
  given: {ν} (f : α ->ₛ E) (hf : Integrable f (μ + ν))
  proof: by
  simp_rw [integral_def]
  refine setToSimpleFunc_add_left'
    (weightedSMul μ) (weightedSMul ν) (weightedSMul (μ + ν)) (fun s _ hμνs => ?_) hf
  rw [lt_top_iff_ne_top]; rw [Measure.coe_add]; rw [Pi.add_apply]; rw [ENNReal.add_ne_top] at hμνs
  rw [weightedSMul_add_measure _ _ hμνs.1 hμνs.2]

中文:
定理 integral_add_measure
  条件: {ν} (f : α ->ₛ E) (hf : 可积 f (μ + ν))
  证明: by
  simp_rw [integral_def]
  refine setToSimpleFunc_add_left'
    (weightedSMul μ) (weightedSMul ν) (weightedSMul (μ + ν)) (fun s _ hμνs => ?_) hf
  rw [lt_top_iff_ne_top]; rw [Measure.coe_add]; rw [Pi.add_apply]; rw [ENNReal.add_ne_top] at hμνs
  rw [weightedSMul_add_measure _ _ hμνs.1 hμνs.2]

Depends on / 依赖: ENNReal, ENNReal.add_ne_top, Measure, Measure.coe_add, Pi.add_apply, add_apply, add_ne_top, coe_add, integral_def, lt_top_iff_ne_top, setToSimpleFunc_add_left, simp_rw, weightedSMul, weightedSMul_add_measure
-/
theorem integral_add_measure {ν} (f : α ->ₛ E) (hf : Integrable f (μ + ν)) :
    f.integral (μ + ν) = f.integral μ + f.integral ν := by
  simp_rw [integral_def]
  refine setToSimpleFunc_add_left'
    (weightedSMul μ) (weightedSMul ν) (weightedSMul (μ + ν)) (fun s _ hμνs => ?_) hf
  rw [lt_top_iff_ne_top]; rw [Measure.coe_add]; rw [Pi.add_apply]; rw [ENNReal.add_ne_top] at hμνs
  rw [weightedSMul_add_measure _ _ hμνs.1 hμνs.2]

section Order

variable [PartialOrder F] [IsOrderedAddMonoid F] [IsOrderedModule Real F]

/--
lemma `integral_nonneg` / 引理 `integral_nonneg`

English:
lemma integral_nonneg
  given: {f : α ->ₛ F} (hf : 0 <=ᵐ[μ] f)
  proof: by
  rw [integral_eq]
  apply Finset.sum_nonneg
  rw [forall_mem_range]
  intro y
  by_cases hy : 0 <= f y
  · positivity
  · suffices μ (f ⁻¹' {f y}) = 0 by simp [this, measureReal_def]
    rw [← nonpos_iff_eq_zero]
    refine le_of_le_of_eq (measure_mono fun x hx => ?_) (ae_iff.mp hf)
    simp onl

中文:
引理 integral_nonneg
  条件: {f : α ->ₛ F} (hf : 0 <=ᵐ[μ] f)
  证明: by
  rw [integral_eq]
  apply Finset.sum_nonneg
  rw [forall_mem_range]
  intro y
  by_cases hy : 0 <= f y
  · positivity
  · suffices μ (f ⁻¹' {f y}) = 0 by simp [this, measureReal_def]
    rw [← nonpos_iff_eq_zero]
    refine le_of_le_of_eq (measure_mono fun x hx => ?_) (ae_iff.mp hf)
    simp onl

Depends on / 依赖: Finset, Finset.sum_nonneg, Set.mem_preimage, ae_iff, ae_iff.mp, forall_mem_range, integral_eq, le_of_le_of_eq, measureReal_def, measure_mono, mem_ofPred_eq, mem_preimage, mem_singleton_iff, nonpos_iff_eq_zero, sum_nonneg
-/
lemma integral_nonneg {f : α ->ₛ F} (hf : 0 <=ᵐ[μ] f) :
    0 <= f.integral μ := by
  rw [integral_eq]
  apply Finset.sum_nonneg
  rw [forall_mem_range]
  intro y
  by_cases hy : 0 <= f y
  · positivity
  · suffices μ (f ⁻¹' {f y}) = 0 by simp [this, measureReal_def]
    rw [← nonpos_iff_eq_zero]
    refine le_of_le_of_eq (measure_mono fun x hx => ?_) (ae_iff.mp hf)
    simp only [Set.mem_preimage, mem_singleton_iff, mem_ofPred_eq] at hx ⊢
    exact hx ▸ hy

/--
lemma `integral_mono` / 引理 `integral_mono`

English:
lemma integral_mono
  given: {f g : α ->ₛ F} (h : f <=ᵐ[μ] g) (hf : Integrable f μ) (hg : Integrable g μ)
  proof: by
  rw [← sub_nonneg]; rw [← integral_sub hg hf]
  rw [← sub_nonneg_ae] at h
  exact integral_nonneg h

中文:
引理 integral_mono
  条件: {f g : α ->ₛ F} (h : f <=ᵐ[μ] g) (hf : 可积 f μ) (hg : 可积 g μ)
  证明: by
  rw [← sub_nonneg]; rw [← integral_sub hg hf]
  rw [← sub_nonneg_ae] at h
  exact integral_nonneg h

Depends on / 依赖: integral_nonneg, integral_sub, sub_nonneg, sub_nonneg_ae
-/
lemma integral_mono {f g : α ->ₛ F} (h : f <=ᵐ[μ] g) (hf : Integrable f μ) (hg : Integrable g μ) :
    f.integral μ <= g.integral μ := by
  rw [← sub_nonneg]; rw [← integral_sub hg hf]
  rw [← sub_nonneg_ae] at h
  exact integral_nonneg h

/--
lemma `integral_mono_measure` / 引理 `integral_mono_measure`

English:
lemma integral_mono_measure
  given: {ν} {f : α ->ₛ F} (hf : 0 <=ᵐ[ν] f) (hμν : μ <= ν) (hfν : Integrable f ν)
  proof: by
  simp only [integral_eq]
  apply Finset.sum_le_sum
  simp only [forall_mem_range]
  intro x
  by_cases hx : 0 <= f x
  · obtain (hx | hx) := hx.eq_or_lt
    · simp [← hx]
    simp only [measureReal_def]
    gcongr
.ne exact integrable_iff.mp hfν (f x) hx.ne'
  · suffices ν (f ⁻¹' {f x}) = 0 by
 

中文:
引理 integral_mono_measure
  条件: {ν} {f : α ->ₛ F} (hf : 0 <=ᵐ[ν] f) (hμν : μ <= ν) (hfν : 可积 f ν)
  证明: by
  simp only [integral_eq]
  apply Finset.sum_le_sum
  simp only [forall_mem_range]
  intro x
  by_cases hx : 0 <= f x
  · obtain (hx | hx) := hx.eq_or_lt
    · simp [← hx]
    simp only [measureReal_def]
    gcongr
.ne exact integrable_iff.mp hfν (f x) hx.ne'
  · suffices ν (f ⁻¹' {f x}) = 0 by
 

Depends on / 依赖: Finset, Finset.sum_le_sum, ae_iff, ae_iff.mp, eq_or_lt, forall_mem_range, hx.eq_or_lt, hx.ne, integrable_iff, integrable_iff.mp, integral_eq, measureReal_def, measure_mono, nonpos_iff_eq_zero, sum_le_sum, trans_eq
-/
lemma integral_mono_measure {ν} {f : α ->ₛ F} (hf : 0 <=ᵐ[ν] f) (hμν : μ <= ν) (hfν : Integrable f ν) :
    f.integral μ <= f.integral ν := by
  simp only [integral_eq]
  apply Finset.sum_le_sum
  simp only [forall_mem_range]
  intro x
  by_cases hx : 0 <= f x
  · obtain (hx | hx) := hx.eq_or_lt
    · simp [← hx]
    simp only [measureReal_def]
    gcongr
.ne exact integrable_iff.mp hfν (f x) hx.ne'
  · suffices ν (f ⁻¹' {f x}) = 0 by
      have A : μ (f ⁻¹' {f x}) = 0 := by simpa using (hμν _ |>.trans_eq this)
      simp [measureReal_def, A, this]
    rw [← nonpos_iff_eq_zero]; rw [← ae_iff.mp hf]
    refine measure_mono fun y hy => ?_
    simp_all

end Order

end Integral

end SimpleFunc

namespace L1

open AEEqFun Lp.simpleFunc Lp

variable [NormedAddCommGroup E] {m : MeasurableSpace α} {μ : Measure α}

namespace SimpleFunc

/--
theorem `norm_eq_integral` / 定理 `norm_eq_integral`

English:
theorem norm_eq_integral
  given: (f : α ->₁ₛ[μ] E)
  statement: ‖f‖ = ((toSimpleFunc f).map norm).integral μ
  proof: by
  rw [norm_eq_sum_mul f]; rw [(toSimpleFunc f).map_integral norm (SimpleFunc.integrable f) norm_zero]
  simp_rw [smul_eq_mul]

中文:
定理 norm_eq_integral
  条件: (f : α ->₁ₛ[μ] E)
  结论: ‖f‖ = ((toSimpleFunc f).map norm).integral μ
  证明: by
  rw [norm_eq_sum_mul f]; rw [(toSimpleFunc f).map_integral norm (SimpleFunc.integrable f) norm_zero]
  simp_rw [smul_eq_mul]

Depends on / 依赖: SimpleFunc, SimpleFunc.integrable, integrable, map_integral, norm_eq_sum_mul, norm_zero, simp_rw, smul_eq_mul, toSimpleFunc
-/
theorem norm_eq_integral (f : α ->₁ₛ[μ] E) : ‖f‖ = ((toSimpleFunc f).map norm).integral μ := by
  rw [norm_eq_sum_mul f]; rw [(toSimpleFunc f).map_integral norm (SimpleFunc.integrable f) norm_zero]
  simp_rw [smul_eq_mul]

section PosPart

/-- Positive part of a simple function in L1 space. -/
nonrec def posPart (f : α ->₁ₛ[μ] Real) : α ->₁ₛ[μ] Real :=
  ⟨Lp.posPart (f : α ->₁[μ] Real), by
    rcases f with ⟨f, s, hsf⟩
    use s.posPart
    simp only [SimpleFunc.posPart, SimpleFunc.coe_map, Function.comp_def, coe_posPart, ← hsf,
      posPart_mk] ⟩

/--
Definition of `negPart` / `negPart` 的定义

English:
definition negPart
  signature: (f : α ->₁ₛ[μ] Real)
  body: posPart (-f)

@[norm_cast]

中文:
定义 negPart
  签名: (f : α ->₁ₛ[μ] 实数)
  定义体: posPart (-f)

@[norm_cast]

Depends on / 依赖: posPart
-/
def negPart (f : α ->₁ₛ[μ] Real) : α ->₁ₛ[μ] Real :=
  posPart (-f)

@[norm_cast]
/--
theorem `coe_posPart` / 定理 `coe_posPart`

English:
theorem coe_posPart
  given: (f : α ->₁ₛ[μ] Real)
  statement: (posPart f : α ->₁[μ] Real) = Lp.posPart (f : α ->₁[μ] Real)
  proof: rfl

@[norm_cast]

中文:
定理 coe_posPart
  条件: (f : α ->₁ₛ[μ] 实数)
  结论: (posPart f : α ->₁[μ] 实数) = Lp.posPart (f : α ->₁[μ] 实数)
  证明: rfl

@[norm_cast]
-/
theorem coe_posPart (f : α ->₁ₛ[μ] Real) : (posPart f : α ->₁[μ] Real) = Lp.posPart (f : α ->₁[μ] Real) := rfl

@[norm_cast]
/--
theorem `coe_negPart` / 定理 `coe_negPart`

English:
theorem coe_negPart
  given: (f : α ->₁ₛ[μ] Real)
  statement: (negPart f : α ->₁[μ] Real) = Lp.negPart (f : α ->₁[μ] Real)
  proof: rfl

中文:
定理 coe_negPart
  条件: (f : α ->₁ₛ[μ] 实数)
  结论: (negPart f : α ->₁[μ] 实数) = Lp.negPart (f : α ->₁[μ] 实数)
  证明: rfl
-/
theorem coe_negPart (f : α ->₁ₛ[μ] Real) : (negPart f : α ->₁[μ] Real) = Lp.negPart (f : α ->₁[μ] Real) := rfl

end PosPart

section SimpleFuncIntegral

/-!
### The Bochner integral of `L1`

Define the Bochner integral on `α →₁ₛ[μ] E` by extension from the simple functions `α →₁ₛ[μ] E`,
and prove basic properties of this integral. -/

variable [NormedRing 𝕜] [Module 𝕜 E] [IsBoundedSMul 𝕜 E] [NormedSpace Real E] [SMulCommClass Real 𝕜 E]

attribute [local instance] simpleFunc.isBoundedSMul simpleFunc.module simpleFunc.normedSpace

/--
Definition of `integral` / `integral` 的定义

English:
definition integral
  signature: (f : α ->₁ₛ[μ] E)
  body: (toSimpleFunc f).integral μ

中文:
定义 integral
  签名: (f : α ->₁ₛ[μ] E)
  定义体: (toSimpleFunc f).integral μ

Depends on / 依赖: integral, toSimpleFunc
-/
def integral (f : α ->₁ₛ[μ] E) : E :=
  (toSimpleFunc f).integral μ

/--
theorem `integral_eq_integral` / 定理 `integral_eq_integral`

English:
theorem integral_eq_integral
  given: (f : α ->₁ₛ[μ] E)
  statement: integral f = (toSimpleFunc f).integral μ
  proof: rfl

nonrec theorem integral_eq_lintegral {f : α ->₁ₛ[μ] Real} (h_pos : 0 <=ᵐ[μ] toSimpleFunc f) :
    integral f = ENNReal.toReal (∫⁻ a, ENNReal.ofReal ((toSimpleFunc f) a) ∂μ) := by
  rw [integral]; rw [SimpleFunc.integral_eq_lintegral (SimpleFunc.integrable f) h_pos]

中文:
定理 integral_eq_integral
  条件: (f : α ->₁ₛ[μ] E)
  结论: integral f = (toSimpleFunc f).integral μ
  证明: rfl

nonrec theorem integral_eq_lintegral {f : α ->₁ₛ[μ] Real} (h_pos : 0 <=ᵐ[μ] toSimpleFunc f) :
    integral f = ENNReal.toReal (∫⁻ a, ENNReal.ofReal ((toSimpleFunc f) a) ∂μ) := by
  rw [integral]; rw [SimpleFunc.integral_eq_lintegral (SimpleFunc.integrable f) h_pos]
-/
theorem integral_eq_integral (f : α ->₁ₛ[μ] E) : integral f = (toSimpleFunc f).integral μ := rfl

nonrec theorem integral_eq_lintegral {f : α ->₁ₛ[μ] Real} (h_pos : 0 <=ᵐ[μ] toSimpleFunc f) :
    integral f = ENNReal.toReal (∫⁻ a, ENNReal.ofReal ((toSimpleFunc f) a) ∂μ) := by
  rw [integral]; rw [SimpleFunc.integral_eq_lintegral (SimpleFunc.integrable f) h_pos]

/--
theorem `integral_eq_setToL1S` / 定理 `integral_eq_setToL1S`

English:
theorem integral_eq_setToL1S
  given: (f : α ->₁ₛ[μ] E)
  statement: integral f = setToL1S (weightedSMul μ) f
  proof: rfl

nonrec theorem integral_congr {f g : α ->₁ₛ[μ] E} (h : toSimpleFunc f =ᵐ[μ] toSimpleFunc g) :
    integral f = integral g :=
  SimpleFunc.integral_congr (SimpleFunc.integrable f) h

中文:
定理 integral_eq_setToL1S
  条件: (f : α ->₁ₛ[μ] E)
  结论: integral f = setToL1S (weightedSMul μ) f
  证明: rfl

nonrec theorem integral_congr {f g : α ->₁ₛ[μ] E} (h : toSimpleFunc f =ᵐ[μ] toSimpleFunc g) :
    integral f = integral g :=
  SimpleFunc.integral_congr (SimpleFunc.integrable f) h
-/
theorem integral_eq_setToL1S (f : α ->₁ₛ[μ] E) : integral f = setToL1S (weightedSMul μ) f := rfl

nonrec theorem integral_congr {f g : α ->₁ₛ[μ] E} (h : toSimpleFunc f =ᵐ[μ] toSimpleFunc g) :
    integral f = integral g :=
  SimpleFunc.integral_congr (SimpleFunc.integrable f) h

/--
theorem `integral_add` / 定理 `integral_add`

English:
theorem integral_add
  given: (f g : α ->₁ₛ[μ] E)
  statement: integral (f + g) = integral f + integral g
  proof: setToL1S_add _ (fun _ _ => weightedSMul_null) weightedSMul_union _ _

中文:
定理 integral_add
  条件: (f g : α ->₁ₛ[μ] E)
  结论: integral (f + g) = integral f + integral g
  证明: setToL1S_add _ (fun _ _ => weightedSMul_null) weightedSMul_union _ _

Depends on / 依赖: setToL1S_add, weightedSMul_null, weightedSMul_union
-/
theorem integral_add (f g : α ->₁ₛ[μ] E) : integral (f + g) = integral f + integral g :=
  setToL1S_add _ (fun _ _ => weightedSMul_null) weightedSMul_union _ _

/--
theorem `integral_smul` / 定理 `integral_smul`

English:
theorem integral_smul
  given: (c : 𝕜) (f : α ->₁ₛ[μ] E)
  statement: integral (c • f) = c • integral f
  proof: setToL1S_smul _ (fun _ _ => weightedSMul_null) weightedSMul_union weightedSMul_smul c f

中文:
定理 integral_smul
  条件: (c : 𝕜) (f : α ->₁ₛ[μ] E)
  结论: integral (c • f) = c • integral f
  证明: setToL1S_smul _ (fun _ _ => weightedSMul_null) weightedSMul_union weightedSMul_smul c f

Depends on / 依赖: setToL1S_smul, weightedSMul_null, weightedSMul_smul, weightedSMul_union
-/
theorem integral_smul (c : 𝕜) (f : α ->₁ₛ[μ] E) : integral (c • f) = c • integral f :=
  setToL1S_smul _ (fun _ _ => weightedSMul_null) weightedSMul_union weightedSMul_smul c f

/--
theorem `norm_integral_le_norm` / 定理 `norm_integral_le_norm`

English:
theorem norm_integral_le_norm
  given: (f : α ->₁ₛ[μ] E)
  statement: ‖integral f‖ <= ‖f‖
  proof: by
  rw [integral]; rw [norm_eq_integral]
  exact (toSimpleFunc f).norm_integral_le_integral_norm (SimpleFunc.integrable f)

中文:
定理 norm_integral_le_norm
  条件: (f : α ->₁ₛ[μ] E)
  结论: ‖integral f‖ <= ‖f‖
  证明: by
  rw [integral]; rw [norm_eq_integral]
  exact (toSimpleFunc f).norm_integral_le_integral_norm (SimpleFunc.integrable f)

Depends on / 依赖: SimpleFunc, SimpleFunc.integrable, integrable, integral, norm_eq_integral, norm_integral_le_integral_norm, toSimpleFunc
-/
theorem norm_integral_le_norm (f : α ->₁ₛ[μ] E) : ‖integral f‖ <= ‖f‖ := by
  rw [integral]; rw [norm_eq_integral]
  exact (toSimpleFunc f).norm_integral_le_integral_norm (SimpleFunc.integrable f)

variable (α E μ 𝕜)

/--
Definition of `integralCLM'` / `integralCLM'` 的定义

English:
definition integralCLM'
  signature: : (α ->₁ₛ[μ] E) ->L[𝕜] E
  body: LinearMap.mkContinuous ⟨⟨integral, integral_add⟩, integral_smul⟩ 1 fun f =>
le_trans (norm_integral_le_norm _) by rw [one_mul]

中文:
定义 integralCLM'
  签名: : (α ->₁ₛ[μ] E) ->L[𝕜] E
  定义体: LinearMap.mkContinuous ⟨⟨integral, integral_add⟩, integral_smul⟩ 1 fun f =>
le_trans (norm_integral_le_norm _) by rw [one_mul]

Depends on / 依赖: LinearMap, LinearMap.mkContinuous, integral, integral_add, integral_smul, le_trans, mkContinuous, norm_integral_le_norm, one_mul
-/
def integralCLM' : (α ->₁ₛ[μ] E) ->L[𝕜] E :=
  LinearMap.mkContinuous ⟨⟨integral, integral_add⟩, integral_smul⟩ 1 fun f =>
le_trans (norm_integral_le_norm _) by rw [one_mul]

/--
Definition of `integralCLM` / `integralCLM` 的定义

English:
definition integralCLM
  signature: : (α ->₁ₛ[μ] E) ->L[Real] E
  body: integralCLM' α E Real μ

中文:
定义 integralCLM
  签名: : (α ->₁ₛ[μ] E) ->L[实数] E
  定义体: integralCLM' α E Real μ

Depends on / 依赖: integralCLM
-/
def integralCLM : (α ->₁ₛ[μ] E) ->L[Real] E :=
  integralCLM' α E Real μ

variable {α E μ 𝕜}

local notation "Integral" => integralCLM α E μ

open ContinuousLinearMap

/--
theorem `norm_Integral_le_one` / 定理 `norm_Integral_le_one`

English:
theorem norm_Integral_le_one
  statement: ‖Integral‖ <= 1
  proof: LinearMap.mkContinuous_norm_le _ zero_le_one fun f => by
    simpa [one_mul] using norm_integral_le_norm f

中文:
定理 norm_整数egral_le_one
  结论: ‖积分‖ <= 1
  证明: LinearMap.mkContinuous_norm_le _ zero_le_one fun f => by
    simpa [one_mul] using norm_integral_le_norm f

Depends on / 依赖: LinearMap, LinearMap.mkContinuous_norm_le, mkContinuous_norm_le, norm_integral_le_norm, one_mul, zero_le_one
-/
theorem norm_Integral_le_one : ‖Integral‖ <= 1 :=
  LinearMap.mkContinuous_norm_le _ zero_le_one fun f => by
    simpa [one_mul] using norm_integral_le_norm f

section PosPart

/--
theorem `posPart_toSimpleFunc` / 定理 `posPart_toSimpleFunc`

English:
theorem posPart_toSimpleFunc
  given: (f : α ->₁ₛ[μ] Real)
  proof: by
  have eq : forall a, (toSimpleFunc f).posPart a = max ((toSimpleFunc f) a) 0 := fun a => rfl
  have ae_eq : forallᵐ a ∂μ, toSimpleFunc (posPart f) a = max ((toSimpleFunc f) a) 0 := by
    filter_upwards [toSimpleFunc_eq_toFun (posPart f), Lp.coeFn_posPart (f : α ->₁[μ] Real),
      toSimpleFunc_

中文:
定理 posPart_toSimpleFunc
  条件: (f : α ->₁ₛ[μ] 实数)
  证明: by
  have eq : forall a, (toSimpleFunc f).posPart a = max ((toSimpleFunc f) a) 0 := fun a => rfl
  have ae_eq : forallᵐ a ∂μ, toSimpleFunc (posPart f) a = max ((toSimpleFunc f) a) 0 := by
    filter_upwards [toSimpleFunc_eq_toFun (posPart f), Lp.coeFn_posPart (f : α ->₁[μ] Real),
      toSimpleFunc_

Depends on / 依赖: Lp.coeFn_posPart, ae_eq, ae_eq.mono, coeFn_posPart, convert, filter_upwards, posPart, toSimpleFunc, toSimpleFunc_eq_toFun
-/
theorem posPart_toSimpleFunc (f : α ->₁ₛ[μ] Real) :
    toSimpleFunc (posPart f) =ᵐ[μ] (toSimpleFunc f).posPart := by
  have eq : forall a, (toSimpleFunc f).posPart a = max ((toSimpleFunc f) a) 0 := fun a => rfl
  have ae_eq : forallᵐ a ∂μ, toSimpleFunc (posPart f) a = max ((toSimpleFunc f) a) 0 := by
    filter_upwards [toSimpleFunc_eq_toFun (posPart f), Lp.coeFn_posPart (f : α ->₁[μ] Real),
      toSimpleFunc_eq_toFun f] with _ _ h₂ h₃
    convert! h₂ using 1
    rw [h₃]
  refine ae_eq.mono fun a h => ?_
  rw [h]; rw [eq]

/--
theorem `negPart_toSimpleFunc` / 定理 `negPart_toSimpleFunc`

English:
theorem negPart_toSimpleFunc
  given: (f : α ->₁ₛ[μ] Real)
  proof: by
  rw [SimpleFunc.negPart]; rw [MeasureTheory.SimpleFunc.negPart]
  filter_upwards [posPart_toSimpleFunc (-f), neg_toSimpleFunc f]
  intro a h₁ h₂
  rw [h₁]
  change max _ _ = max _ _
  rw [h₂]
  simp

中文:
定理 negPart_toSimpleFunc
  条件: (f : α ->₁ₛ[μ] 实数)
  证明: by
  rw [SimpleFunc.negPart]; rw [MeasureTheory.SimpleFunc.negPart]
  filter_upwards [posPart_toSimpleFunc (-f), neg_toSimpleFunc f]
  intro a h₁ h₂
  rw [h₁]
  change max _ _ = max _ _
  rw [h₂]
  simp

Depends on / 依赖: MeasureTheory, MeasureTheory.SimpleFunc.negPart, SimpleFunc, SimpleFunc.negPart, filter_upwards, negPart, neg_toSimpleFunc, posPart_toSimpleFunc
-/
theorem negPart_toSimpleFunc (f : α ->₁ₛ[μ] Real) :
    toSimpleFunc (negPart f) =ᵐ[μ] (toSimpleFunc f).negPart := by
  rw [SimpleFunc.negPart]; rw [MeasureTheory.SimpleFunc.negPart]
  filter_upwards [posPart_toSimpleFunc (-f), neg_toSimpleFunc f]
  intro a h₁ h₂
  rw [h₁]
  change max _ _ = max _ _
  rw [h₂]
  simp

/--
theorem `integral_eq_norm_posPart_sub` / 定理 `integral_eq_norm_posPart_sub`

English:
theorem integral_eq_norm_posPart_sub
  given: (f : α ->₁ₛ[μ] Real)
  statement: integral f = ‖posPart f‖ - ‖negPart f‖
  proof: by
  -- Convert things in `L¹` to their `SimpleFunc` counterpart
  have ae_eq₁ : (toSimpleFunc f).posPart =ᵐ[μ] (toSimpleFunc (posPart f)).map norm := by
    filter_upwards [posPart_toSimpleFunc f] with _ h
    rw [SimpleFunc.map_apply]; rw [h]
    conv_lhs => rw [← SimpleFunc.posPart_map_norm, Simp

中文:
定理 integral_eq_norm_posPart_sub
  条件: (f : α ->₁ₛ[μ] 实数)
  结论: integral f = ‖posPart f‖ - ‖negPart f‖
  证明: by
  -- Convert things in `L¹` to their `SimpleFunc` counterpart
  have ae_eq₁ : (toSimpleFunc f).posPart =ᵐ[μ] (toSimpleFunc (posPart f)).map norm := by
    filter_upwards [posPart_toSimpleFunc f] with _ h
    rw [SimpleFunc.map_apply]; rw [h]
    conv_lhs => rw [← SimpleFunc.posPart_map_norm, Simp
-/
theorem integral_eq_norm_posPart_sub (f : α ->₁ₛ[μ] Real) : integral f = ‖posPart f‖ - ‖negPart f‖ := by
  -- Convert things in `L¹` to their `SimpleFunc` counterpart
  have ae_eq₁ : (toSimpleFunc f).posPart =ᵐ[μ] (toSimpleFunc (posPart f)).map norm := by
    filter_upwards [posPart_toSimpleFunc f] with _ h
    rw [SimpleFunc.map_apply]; rw [h]
    conv_lhs => rw [← SimpleFunc.posPart_map_norm, SimpleFunc.map_apply]
  -- Convert things in `L¹` to their `SimpleFunc` counterpart
  have ae_eq₂ : (toSimpleFunc f).negPart =ᵐ[μ] (toSimpleFunc (negPart f)).map norm := by
    filter_upwards [negPart_toSimpleFunc f] with _ h
    rw [SimpleFunc.map_apply]; rw [h]
    conv_lhs => rw [← SimpleFunc.negPart_map_norm, SimpleFunc.map_apply]
  rw [integral]; rw [norm_eq_integral]; rw [norm_eq_integral]; rw [← SimpleFunc.integral_sub]
  · change (toSimpleFunc f).integral μ =
      ((toSimpleFunc (posPart f)).map norm - (toSimpleFunc (negPart f)).map norm).integral μ
    apply MeasureTheory.SimpleFunc.integral_congr (SimpleFunc.integrable f)
    filter_upwards [ae_eq₁, ae_eq₂] with _ h₁ h₂
    rw [SimpleFunc.sub_apply]; rw [← h₁]; rw [← h₂]
    exact DFunLike.congr_fun (toSimpleFunc f).posPart_sub_negPart.symm _
  · exact (SimpleFunc.integrable f).pos_part.congr ae_eq₁
  · exact (SimpleFunc.integrable f).neg_part.congr ae_eq₂

end PosPart

end SimpleFuncIntegral

end SimpleFunc

open L1.SimpleFunc

local notation "Integral" => @integralCLM α E _ _ _ _ _ μ _

variable [NormedSpace Real E] [NormedRing 𝕜] [Module 𝕜 E] [IsBoundedSMul 𝕜 E] [SMulCommClass Real 𝕜 E]
  [CompleteSpace E]

section IntegrationInL1

attribute [local instance] simpleFunc.isBoundedSMul simpleFunc.module

open ContinuousLinearMap

variable (𝕜) in
/-- The Bochner integral in L1 space as a continuous linear map. -/
nonrec def integralCLM' : (α ->₁[μ] E) ->L[𝕜] E :=
  (integralCLM' α E 𝕜 μ).extend (coeToLp α E 𝕜)

/--
Definition of `integralCLM` / `integralCLM` 的定义

English:
definition integralCLM
  signature: : (α ->₁[μ] E) ->L[Real] E
  body: integralCLM' Real

中文:
定义 integralCLM
  签名: : (α ->₁[μ] E) ->L[实数] E
  定义体: integralCLM' Real

Depends on / 依赖: integralCLM
-/
def integralCLM : (α ->₁[μ] E) ->L[Real] E :=
  integralCLM' Real

/-- The Bochner integral in L1 space -/
irreducible_def integral : (α ->₁[μ] E) -> E :=
  integralCLM

/--
theorem `integral_eq` / 定理 `integral_eq`

English:
theorem integral_eq
  given: (f : α ->₁[μ] E)
  statement: integral f = integralCLM f
  proof: by
  simp only [integral]

中文:
定理 integral_eq
  条件: (f : α ->₁[μ] E)
  结论: integral f = integralCLM f
  证明: by
  simp only [integral]

Depends on / 依赖: integral
-/
theorem integral_eq (f : α ->₁[μ] E) : integral f = integralCLM f := by
  simp only [integral]

/--
theorem `integral_eq_setToL1` / 定理 `integral_eq_setToL1`

English:
theorem integral_eq_setToL1
  given: (f : α ->₁[μ] E)
  proof: by
  simp only [integral]; rfl

@[norm_cast]

中文:
定理 integral_eq_setToL1
  条件: (f : α ->₁[μ] E)
  证明: by
  simp only [integral]; rfl

@[norm_cast]

Depends on / 依赖: integral
-/
theorem integral_eq_setToL1 (f : α ->₁[μ] E) :
    integral f = setToL1 (dominatedFinMeasAdditive_weightedSMul μ) f := by
  simp only [integral]; rfl

@[norm_cast]
/--
theorem `SimpleFunc.integral_L1_eq_integral` / 定理 `SimpleFunc.integral_L1_eq_integral`

English:
theorem SimpleFunc.integral_L1_eq_integral
  given: (f : α ->₁ₛ[μ] E)
  proof: by
  simp only [integral, L1.integral]
  exact setToL1_eq_setToL1SCLM (dominatedFinMeasAdditive_weightedSMul μ) f

@[norm_cast]

中文:
定理 SimpleFunc.integral_L1_eq_integral
  条件: (f : α ->₁ₛ[μ] E)
  证明: by
  simp only [integral, L1.integral]
  exact setToL1_eq_setToL1SCLM (dominatedFinMeasAdditive_weightedSMul μ) f

@[norm_cast]

Depends on / 依赖: L1.integral, dominatedFinMeasAdditive_weightedSMul, integral, setToL1_eq_setToL1SCLM
-/
theorem SimpleFunc.integral_L1_eq_integral (f : α ->₁ₛ[μ] E) :
    L1.integral (f : α ->₁[μ] E) = SimpleFunc.integral f := by
  simp only [integral, L1.integral]
  exact setToL1_eq_setToL1SCLM (dominatedFinMeasAdditive_weightedSMul μ) f

@[norm_cast]
/--
theorem `SimpleFunc.integralCLM'_L1_eq_integral` / 定理 `SimpleFunc.integralCLM'_L1_eq_integral`

English:
theorem SimpleFunc.integralCLM'_L1_eq_integral
  given: (f : α ->₁ₛ[μ] E)
  proof: by
  apply ContinuousLinearMap.extend_eq _ _ simpleFunc.isUniformInducing
  exact simpleFunc.denseRange one_ne_top

中文:
定理 SimpleFunc.integralCLM'_L1_eq_integral
  条件: (f : α ->₁ₛ[μ] E)
  证明: by
  apply ContinuousLinearMap.extend_eq _ _ simpleFunc.isUniformInducing
  exact simpleFunc.denseRange one_ne_top

Depends on / 依赖: ContinuousLinearMap, ContinuousLinearMap.extend_eq, denseRange, extend_eq, isUniformInducing, one_ne_top, simpleFunc, simpleFunc.denseRange, simpleFunc.isUniformInducing
-/
theorem SimpleFunc.integralCLM'_L1_eq_integral (f : α ->₁ₛ[μ] E) :
    L1.integralCLM' 𝕜 (f : α ->₁[μ] E) = SimpleFunc.integral f := by
  apply ContinuousLinearMap.extend_eq _ _ simpleFunc.isUniformInducing
  exact simpleFunc.denseRange one_ne_top

variable (𝕜) in
/--
theorem `integral_eq'` / 定理 `integral_eq'`

English:
theorem integral_eq'
  given: (f : α ->₁[μ] E)
  statement: integral f = integralCLM' 𝕜 f
  proof: by
  apply isClosed_property (simpleFunc.denseRange one_ne_top)
    (isClosed_eq _ (integralCLM' 𝕜).continuous) _ f
  · simp_rw [integral_def]
    exact (integralCLM (E := E)).continuous
  intro f
  norm_cast

中文:
定理 integral_eq'
  条件: (f : α ->₁[μ] E)
  结论: integral f = integralCLM' 𝕜 f
  证明: by
  apply isClosed_property (simpleFunc.denseRange one_ne_top)
    (isClosed_eq _ (integralCLM' 𝕜).continuous) _ f
  · simp_rw [integral_def]
    exact (integralCLM (E := E)).continuous
  intro f
  norm_cast

Depends on / 依赖: continuous, denseRange, integralCLM, integral_def, isClosed_eq, isClosed_property, one_ne_top, simp_rw, simpleFunc, simpleFunc.denseRange
-/
theorem integral_eq' (f : α ->₁[μ] E) : integral f = integralCLM' 𝕜 f := by
  apply isClosed_property (simpleFunc.denseRange one_ne_top)
    (isClosed_eq _ (integralCLM' 𝕜).continuous) _ f
  · simp_rw [integral_def]
    exact (integralCLM (E := E)).continuous
  intro f
  norm_cast

variable (α E)

@[simp]
/--
theorem `integral_zero` / 定理 `integral_zero`

English:
theorem integral_zero
  statement: integral (0 : α ->₁[μ] E) = 0
  proof: by
  simp only [integral]
  exact map_zero integralCLM

中文:
定理 integral_zero
  结论: integral (0 : α ->₁[μ] E) = 0
  证明: by
  simp only [integral]
  exact map_zero integralCLM

Depends on / 依赖: integral, integralCLM, map_zero
-/
theorem integral_zero : integral (0 : α ->₁[μ] E) = 0 := by
  simp only [integral]
  exact map_zero integralCLM

variable {α E}

@[integral_simps]
/--
theorem `integral_add` / 定理 `integral_add`

English:
theorem integral_add
  given: (f g : α ->₁[μ] E)
  statement: integral (f + g) = integral f + integral g
  proof: by
  simp only [integral]
  exact map_add integralCLM f g

@[integral_simps]

中文:
定理 integral_add
  条件: (f g : α ->₁[μ] E)
  结论: integral (f + g) = integral f + integral g
  证明: by
  simp only [integral]
  exact map_add integralCLM f g

@[integral_simps]

Depends on / 依赖: integral, integralCLM, map_add
-/
theorem integral_add (f g : α ->₁[μ] E) : integral (f + g) = integral f + integral g := by
  simp only [integral]
  exact map_add integralCLM f g

@[integral_simps]
/--
theorem `integral_neg` / 定理 `integral_neg`

English:
theorem integral_neg
  given: (f : α ->₁[μ] E)
  statement: integral (-f) = -integral f
  proof: by
  simp only [integral]
  exact map_neg integralCLM f

@[integral_simps]

中文:
定理 integral_neg
  条件: (f : α ->₁[μ] E)
  结论: integral (-f) = -integral f
  证明: by
  simp only [integral]
  exact map_neg integralCLM f

@[integral_simps]

Depends on / 依赖: integral, integralCLM, map_neg
-/
theorem integral_neg (f : α ->₁[μ] E) : integral (-f) = -integral f := by
  simp only [integral]
  exact map_neg integralCLM f

@[integral_simps]
/--
theorem `integral_sub` / 定理 `integral_sub`

English:
theorem integral_sub
  given: (f g : α ->₁[μ] E)
  statement: integral (f - g) = integral f - integral g
  proof: by
  simp only [integral]
  exact map_sub integralCLM f g

@[integral_simps]

中文:
定理 integral_sub
  条件: (f g : α ->₁[μ] E)
  结论: integral (f - g) = integral f - integral g
  证明: by
  simp only [integral]
  exact map_sub integralCLM f g

@[integral_simps]

Depends on / 依赖: integral, integralCLM, map_sub
-/
theorem integral_sub (f g : α ->₁[μ] E) : integral (f - g) = integral f - integral g := by
  simp only [integral]
  exact map_sub integralCLM f g

@[integral_simps]
/--
theorem `integral_smul` / 定理 `integral_smul`

English:
theorem integral_smul
  given: (c : 𝕜) (f : α ->₁[μ] E)
  statement: integral (c • f) = c • integral f
  proof: by
  rw [integral_eq' 𝕜 f]; rw [integral_eq' 𝕜 (c • f)]; rw [map_smul (integralCLM' 𝕜) c f]

中文:
定理 integral_smul
  条件: (c : 𝕜) (f : α ->₁[μ] E)
  结论: integral (c • f) = c • integral f
  证明: by
  rw [integral_eq' 𝕜 f]; rw [integral_eq' 𝕜 (c • f)]; rw [map_smul (integralCLM' 𝕜) c f]

Depends on / 依赖: integralCLM, integral_eq, map_smul
-/
theorem integral_smul (c : 𝕜) (f : α ->₁[μ] E) : integral (c • f) = c • integral f := by
  rw [integral_eq' 𝕜 f]; rw [integral_eq' 𝕜 (c • f)]; rw [map_smul (integralCLM' 𝕜) c f]

/--
theorem `norm_Integral_le_one` / 定理 `norm_Integral_le_one`

English:
theorem norm_Integral_le_one
  statement: ‖integralCLM (α := α) (E := E) (μ := μ)‖ <= 1
  proof: norm_setToL1_le (dominatedFinMeasAdditive_weightedSMul μ) zero_le_one

中文:
定理 norm_整数egral_le_one
  结论: ‖integralCLM (α := α) (E := E) (μ := μ)‖ <= 1
  证明: norm_setToL1_le (dominatedFinMeasAdditive_weightedSMul μ) zero_le_one
-/
theorem norm_Integral_le_one : ‖integralCLM (α := α) (E := E) (μ := μ)‖ <= 1 :=
  norm_setToL1_le (dominatedFinMeasAdditive_weightedSMul μ) zero_le_one

/--
theorem `nnnorm_Integral_le_one` / 定理 `nnnorm_Integral_le_one`

English:
theorem nnnorm_Integral_le_one
  statement: ‖integralCLM (α := α) (E := E) (μ := μ)‖₊ <= 1
  proof: norm_Integral_le_one

中文:
定理 nnnorm_整数egral_le_one
  结论: ‖integralCLM (α := α) (E := E) (μ := μ)‖₊ <= 1
  证明: norm_Integral_le_one
-/
theorem nnnorm_Integral_le_one : ‖integralCLM (α := α) (E := E) (μ := μ)‖₊ <= 1 :=
  norm_Integral_le_one

/--
theorem `norm_integral_le` / 定理 `norm_integral_le`

English:
theorem norm_integral_le
  given: (f : α ->₁[μ] E)
  statement: ‖integral f‖ <= ‖f‖
  proof: calc
    ‖integral f‖ = ‖integralCLM f‖ := by simp only [integral]
    _ <= ‖integralCLM (α := α) (μ := μ)‖ * ‖f‖ := le_opNorm _ _
_ <= 1 * ‖f‖ := mul_le_mul_of_nonneg_right norm_Integral_le_one norm_nonneg _
    _ = ‖f‖ := one_mul _

中文:
定理 norm_integral_le
  条件: (f : α ->₁[μ] E)
  结论: ‖integral f‖ <= ‖f‖
  证明: calc
    ‖integral f‖ = ‖integralCLM f‖ := by simp only [integral]
    _ <= ‖integralCLM (α := α) (μ := μ)‖ * ‖f‖ := le_opNorm _ _
_ <= 1 * ‖f‖ := mul_le_mul_of_nonneg_right norm_Integral_le_one norm_nonneg _
    _ = ‖f‖ := one_mul _

Depends on / 依赖: integral, integralCLM, le_opNorm, mul_le_mul_of_nonneg_right, norm_Integral_le_one, norm_nonneg, one_mul
-/
theorem norm_integral_le (f : α ->₁[μ] E) : ‖integral f‖ <= ‖f‖ :=
  calc
    ‖integral f‖ = ‖integralCLM f‖ := by simp only [integral]
    _ <= ‖integralCLM (α := α) (μ := μ)‖ * ‖f‖ := le_opNorm _ _
_ <= 1 * ‖f‖ := mul_le_mul_of_nonneg_right norm_Integral_le_one norm_nonneg _
    _ = ‖f‖ := one_mul _

/--
theorem `nnnorm_integral_le` / 定理 `nnnorm_integral_le`

English:
theorem nnnorm_integral_le
  given: (f : α ->₁[μ] E)
  statement: ‖integral f‖₊ <= ‖f‖₊
  proof: norm_integral_le f

@[continuity]

中文:
定理 nnnorm_integral_le
  条件: (f : α ->₁[μ] E)
  结论: ‖integral f‖₊ <= ‖f‖₊
  证明: norm_integral_le f

@[continuity]

Depends on / 依赖: norm_integral_le
-/
theorem nnnorm_integral_le (f : α ->₁[μ] E) : ‖integral f‖₊ <= ‖f‖₊ :=
  norm_integral_le f

@[continuity]
/--
theorem `continuous_integral` / 定理 `continuous_integral`

English:
theorem continuous_integral
  statement: Continuous fun f : α ->₁[μ] E => integral f
  proof: by
  simp only [integral]
  exact L1.integralCLM.continuous

中文:
定理 continuous_integral
  结论: 连续 fun f : α ->₁[μ] E => integral f
  证明: by
  simp only [integral]
  exact L1.integralCLM.continuous

Depends on / 依赖: L1.integralCLM.continuous, continuous, integral, integralCLM
-/
theorem continuous_integral : Continuous fun f : α ->₁[μ] E => integral f := by
  simp only [integral]
  exact L1.integralCLM.continuous

section PosPart

/--
theorem `integral_eq_norm_posPart_sub` / 定理 `integral_eq_norm_posPart_sub`

English:
theorem integral_eq_norm_posPart_sub
  given: (f : α ->₁[μ] Real)
  proof: by
  -- Use `isClosed_property` and `isClosed_eq`
  refine @isClosed_property _ _ _ ((↑) : (α ->₁ₛ[μ] Real) -> α ->₁[μ] Real)
      (fun f : α ->₁[μ] Real => integral f = ‖Lp.posPart f‖ - ‖Lp.negPart f‖)
      (simpleFunc.denseRange one_ne_top) (isClosed_eq ?_ ?_) ?_ f
  · simp only [integral]
    e

中文:
定理 integral_eq_norm_posPart_sub
  条件: (f : α ->₁[μ] 实数)
  证明: by
  -- Use `isClosed_property` and `isClosed_eq`
  refine @isClosed_property _ _ _ ((↑) : (α ->₁ₛ[μ] Real) -> α ->₁[μ] Real)
      (fun f : α ->₁[μ] Real => integral f = ‖Lp.posPart f‖ - ‖Lp.negPart f‖)
      (simpleFunc.denseRange one_ne_top) (isClosed_eq ?_ ?_) ?_ f
  · simp only [integral]
    e
-/
theorem integral_eq_norm_posPart_sub (f : α ->₁[μ] Real) :
    integral f = ‖Lp.posPart f‖ - ‖Lp.negPart f‖ := by
  -- Use `isClosed_property` and `isClosed_eq`
  refine @isClosed_property _ _ _ ((↑) : (α ->₁ₛ[μ] Real) -> α ->₁[μ] Real)
      (fun f : α ->₁[μ] Real => integral f = ‖Lp.posPart f‖ - ‖Lp.negPart f‖)
      (simpleFunc.denseRange one_ne_top) (isClosed_eq ?_ ?_) ?_ f
  · simp only [integral]
    exact cont _
  · refine Continuous.sub (continuous_norm.comp Lp.continuous_posPart)
      (continuous_norm.comp Lp.continuous_negPart)
  -- Show that the property holds for all simple functions in the `L¹` space.
  · intro s
    norm_cast
    exact SimpleFunc.integral_eq_norm_posPart_sub _

end PosPart

end IntegrationInL1

end L1

end MeasureTheory
