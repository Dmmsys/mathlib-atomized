...
/--
theorem `exists_eq_polynomial` / 定理 `exists_eq_polynomial`

English:
theorem exists_eq_polynomial
  statement: [Semiring Fq] {d : Nat} {m : Nat} (hm : Fintype.card Fq ^ d <= m)
  proof: by
...
定理 exists_eq_polynomial
  ...
  set f : Fin m.succ -> Fin d -> Fq 
-/
theorem exists_eq_polynomial [Semiring Fq] {d : Nat} {m : Nat} (hm : Fintype.card Fq ^ d <= m)
    (b : Fq[X]) (hb : natDegree b <= d) (A : Fin m.succ -> Fq[X])
    (hA : forall i, degree (A i) < degree b) : exists i₀ i₁, i₀ != i₁ ∧ A i₁ = A i₀ := by
  -- Since there are > q^d elements of A, and only q^d choices for the highest `d` coefficients,
  -- there must be two elements of A with the same coefficients at
  -- `0`, ... `degree b - 1` ≤ `d - 1`.
  -- In other words, the following map is not injective:
...
/--
theorem `exists_approx_polynomial_aux` / 定理 `exists_approx_polynomial_aux`

English:
theorem exists_approx_polynomial_aux
  statement: [Ring Fq] {d : Nat} {m : Nat} (hm : Fintype.card Fq ^ d <= m)
  proof: by
  have hb : b != 0 := by
    rintro rfl
    specialize hA 0
...
Depends on / 依赖: bot_le, degree_zero, not_lt_of_ge, specialize
-/
theorem exists_approx_polynomial_aux [Ring Fq] {d : Nat} {m : Nat} (hm : Fintype.card Fq ^ d <= m)
    (b : Fq[X]) (A : Fin m.succ -> Fq[X]) (hA : forall i, degree (A i) < degree b) :
    exists i₀ i₁, i₀ != i₁ ∧ degree (A i₁ - A i₀) < ↑(natDegree b - d) := by
  have hb : b != 0 := by
    rintro rfl
    specialize hA 0
    rw [degree_zero] at hA
...
/--
theorem `exists_approx_polynomial` / 定理 `exists_approx_polynomial`

English:
theorem exists_approx_polynomial
  statement: {b : Fq[X]} (hb : b != 0) {ε : Real} (hε : 0 < ε)
  proof: by
  ...
  have one_lt_q' : (1 : Real) < Fintype.card Fq := by assumption_mod_cast
  have q_po
...
Depends on / 依赖: AbsoluteValue, AbsoluteValue.pos, Algebra, Algebra.smul_def, DistribLattice, Fintype, Fintype.card, Fintype.one_lt_card, GeneralizedCoheytingAlgebra, GeneralizedCoheytingAlgebra.toDistribLattice, Int.cast_pos.mpr, assumption_mod_cast, cardPowDegree, cast_pos, eq_intCast, mul_pos, one_lt_card, one_lt_q, q_pos, smul_def
-/
theorem exists_approx_polynomial {b : Fq[X]} (hb : b != 0) {ε : Real} (hε : 0 < ε)
    (A : Fin (Fintype.card Fq ^ ⌈-log ε / log (Fintype.card Fq)⌉₊).succ -> Fq[X]) :
    exists i₀ i₁, i₀ != i₁ ∧ (cardPowDegree (A i₁ % b - A i₀ % b) : Real) < cardPowDegree b • ε := by
  have hbε : 0 < cardPowDegree b • ε := by
    rw [Algebra.smul_def]; rw [eq_intCast]
    exact mul_pos (Int.cast_pos.mpr (AbsoluteValue.pos _ hb)) hε
  have one_lt_q : 1 < Fintype.card Fq := Fintype.one_lt_card
  ...
  have q_pos' : (0 : Real) < Fintype.card Fq := by assumption_mod_cast
  ...
  suffices (natDegree (A i₁ % b - A i₀ % b) : Real) < b.natDegree + log ε / log (Fintype.card Fq) by
  ...
  rw [← sub_neg_eq_add]; rw [← neg_div]; rw [Nat.cast_sub le_b.le]
  grw [← Nat.le_ceil]

/--
theorem `cardPowDegree_anti_archimedean` / 定理 `cardPowDegree_anti_archimedean`

English:
theorem cardPowDegree_anti_archimedean
  statement: {x y z : Fq[X]} {a : Int} (hxy : cardPowDegree (x - y) < a)
  proof: by
  ...
  rw [← Ne]; rw [← sub_ne_zero] at hxy' hyz' hxz'
  refine lt_of_le_of_lt ?_ (max_lt hxy hy

中文:
定理 cardPowDegree_anti_archimedean
  结论: {x y z : Fq[X]} {a : 整数} (hxy : cardPowDegree (x - y) < a)
  证明: by
  have ha : 0 < a := lt_of_le_of_lt (AbsoluteValue.nonneg _ _) hxy
  by_cases hxy' : x = y
  · rwa [hxy']
...
Depends on / 依赖: AbsoluteValue, AbsoluteValue.nonneg, Fintype, Fintype.card, Fintype.one_lt_card, cardPowDegree_nonzero, le_max_iff, lt_of_le_of_lt, map_zero, max_lt, mod_cast, nonneg, one_lt_card, sub_ne_zero, sub_self
-/
theorem cardPowDegree_anti_archimedean {x y z : Fq[X]} {a : Int} (hxy : cardPowDegree (x - y) < a)
  ...
  have : (1 : Int) <= Fintype.card Fq := mod_cast (@Fintype.one_lt_card Fq _ _).le
...
/--
theorem `exists_partition_polynomial_aux` / 定理 `exists_partition_polynomial_aux`

English:
theorem exists_partition_polynomial_aux
  statement: (n : Nat) {ε : Real} (hε : 0 < ε) {b : Fq[X]} (hb : b != 0)
  proof: by
  have hbε : 0 < cardPowDegree b • ε := by
    rw [Algebra.smul_def]; rw [eq_intCast]
    exact mul_pos (Int.cast_pos.mpr (AbsoluteValue.pos _ hb)) hε
...
Depends on / 依赖: AbsoluteValue, AbsoluteValue.pos, Algebra, Algebra.smul_def, Int.cast_pos.mpr, cardPowDegree, cast_pos, eq_intCast, mul_pos, smul_def
-/
theorem exists_partition_polynomial_aux (n : Nat) {ε : Real} (hε : 0 < ε) {b : Fq[X]} (hb : b != 0)
    (A : Fin n -> Fq[X]) : exists t : Fin n -> Fin (Fintype.card Fq ^ ⌈-log ε / log (Fintype.card Fq)⌉₊),
      forall i₀ i₁ : Fin n, t i₀ = t i₁ ↔
        (cardPowDegree (A i₁ % b - A i₀ % b) : Real) < cardPowDegree b • ε := by
  have hbε : 0 < cardPowDegree b • ε := by
    rw [Algebra.smul_def]; rw [eq_intCast]
    exact mul_pos (Int.cast_pos.mpr (AbsoluteValue.pos _ hb)) hε
  -- We go by induction on the size `A`.
  ...
  have anti_archim' : forall {i j k} {ε : Real},
    (cardPowDegree (A i % b - A j % b) : Real) < ε ->
      (cardPowDegree (A j % b - A k % b) : Real) < ε ->
        (cardPowDegree (A i % b - A k % b) : Real) < ε := by
    intro i j k ε
    simp_rw [← Int.lt_ceil]
  ...
  rsuffices ⟨j, hj⟩ :
    exists j, forall i, t' i = j ↔ (cardPowDegree (A 0 % b - A i.succ % b) : Real) < cardPowDegree b • ε
  ...
  obtain ⟨j, hj⟩ : exists j, forall i : Fin n,
      t' i = j -> (cardPowDegree (A 0 % b - A i.succ % b) : Real) < cardPowDegree b • ε := by
    by_contra! hg
  ...
  by_cases exists_nonempty_j : exists j, (exists i, t' i = j) ∧
      forall i, t' i = j -> (cardPowDegree (A 0 % b - A i.succ % b) : Real) < cardPowDegree b • ε
...
/--
theorem `exists_partition_polynomial` / 定理 `exists_partition_polynomial`

English:
theorem exists_partition_polynomial
  statement: (n : Nat) {ε : Real} (hε : 0 < ε) {b : Fq[X]} (hb : b != 0)
  proof: by
...
Depends on / 依赖: exists_partition_polynomial_aux
-/
theorem exists_partition_polynomial (n : Nat) {ε : Real} (hε : 0 < ε) {b : Fq[X]} (hb : b != 0)
    (A : Fin n -> Fq[X]) : exists t : Fin n -> Fin (Fintype.card Fq ^ ⌈-log ε / log (Fintype.card Fq)⌉₊),
      forall i₀ i₁ : Fin n, t i₀ = t i₁ ->
        (cardPowDegree (A i₁ % b - A i₀ % b) : Real) < cardPowDegree b • ε := by
  obtain ⟨t, ht⟩ := exists_partition_polynomial_aux n hε hb A
...
/--
Definition of `cardPowDegreeIsAdmissible` / `cardPowDegreeIsAdmissible` 的定义

English:
definition cardPowDegreeIsAdmissible
  signature: :
  body: { @cardPowDegree_isEuclidean Fq _
      _ with
    card := fun ε => Fintype.card Fq ^ ⌈-log ε / log (Fintype.card Fq)⌉₊
    exists_partition' := fun n _ hε _ hb => exists_partition_polynomial n hε hb }

中文:
定义 cardPowDegreeIsAdmissible
  签名: :
  定义体: { @cardPowDegree_isEuclidean Fq _
      _ with
    card := fun ε => Fintype.card Fq ^ ⌈-log ε / log (Fintype.card Fq)⌉₊
    exists_partition' := fun n _ hε _ hb => exists_partition_polynomial n hε hb }

Depends on / 依赖: Fintype, Fintype.card, cardPowDegree_isEuclidean, exists_partition, exists_partition_polynomial
-/
noncomputable def cardPowDegreeIsAdmissible :
    IsAdmissible (cardPowDegree : AbsoluteValue Fq[X] Int) :=
