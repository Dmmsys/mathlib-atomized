...
/--
lemma `set_smul_eq_map` / 引理 `set_smul_eq_map`

English:
lemma set_smul_eq_map
  given: [SMulCommClass R R N]
  proof: by
...
/--
lemma `mem_set_smul` / 引理 `mem_set_smul`

English:
lemma mem_set_smul
  given: (x : M) [SMulCommClass R R N]
  proof: by
  fconstructor
  · intro h
...
lemma mem_set_smul (x : M) [SMulCommClass R R N] :
    x in sR • N ↔ exists (c : R ->₀ N), (c.support : Set R) subseteq sR ∧ x = c.sum fun r m => r • m := by
  fconstructor
  · intro h
    rw [set_smul_eq_map] at h
    obtain ⟨c, hc, rfl⟩ := h
...
definition noncomputable
  signature: def pointwiseSetMulAction [SMulCommClass R R M]
  body: show {(1 : R)} • x = x from SetLike.ext fun m =>
...
English:
definition noncomputable
  signature: def pointwiseSetDistribMulAction [SMulCommClass R R M]
  body: set_smul_bot s
