...
/--
Definition of `IsTutteViolator` / `IsTutteViolator` 的定义

English:
definition IsTutteViolator
  signature: (G : SimpleGraph V) (u : Set V)
  body: u.ncard < ((⊤ : G.Subgraph).deleteVerts u).coe.oddComponents.ncard
...
/--
lemma `tutte_exists_isAlternating_isCycles` / 引理 `tutte_exists_isAlternating_isCycles`

English:
lemma tutte_exists_isAlternating_isCycles
  statement: {x b a c : V} {M : Subgraph (G ⊔ edge a c)}
  proof: by
...
/--
lemma `IsTutteViolator.mono` / 引理 `IsTutteViolator.mono`

English:
lemma IsTutteViolator.mono
  given: {u : Set V} (h : G <= G') (ht : G'.IsTutteViolator u)
  proof: by
  ...
  (replacing grind's canonicalizer with a type-directed normal
...
Depends on / 依赖: Before, IsTutteViolator, Mathlib, Subgraph, Subgraph.deleteVerts_mono, adaptation_note, canonicalizer, closed, deleteVerts_mono, directed, github, github.com, leanprover, lt_of_lt, minimization, ncard_oddComponents_mono, normalizer, oddComponents, original, problem
-/
...
/--
lemma `Subgraph.IsMatching.exists_verts_compl_subset_universalVerts` / 引理 `Subgraph.IsMatching.exists_verts_compl_subset_universalVerts`

English:
lemma Subgraph.IsMatching.exists_verts_compl_subset_universalVerts
  proof: by
...
private lemma Subgraph.IsMatching.exists_verts_compl_subset_universalVerts
    ...
    (h' : forall (K : G.deleteUniversalVerts.coe.ConnectedComponent),
      G.deleteUniversalVerts.coe.IsClique K.supp) :
    exists M : Subgraph G, M.IsMatching ∧ M.vertsᶜ subseteq G.universalVerts := by
  have hrep := ConnectedComponent.Represents.image_out G.deleteUniversalVerts.coe.oddComponents
  -- First we match one node from each odd component to a universal vertex
  obtain ⟨t, ht, M1, hM1⟩ := Subgraph.IsMatching.exists_of_universalVerts
      (disjoint_image_val_universalVerts _).symm (by
  ...
  have exists_complMatch (K : G.deleteUniversalVerts.coe.ConnectedComponent) :
      exists M : Subgraph G, M.verts = Subtype.val '' K.supp \ M1.verts ∧ M.IsMatching := by
    have : G.IsClique (Subtype.val '' K.supp \ M1.verts) :=
      ((h' K).of_induce).subset Set.sdiff_subset
    rw [← this.even_iff_exists_isMatching (Set.toFinite _)]; rw [hM1.1]
  ...
  have hM2 : M2.IsMatching := by
    ...
    rw [(hcomplMatch_match i).support_eq_verts]; rw [hcomplMatch_compl i]; rw [(hcomplMatch_match j).support_eq_verts]; rw [hcomplMatch_compl j]
...
Set.disjoint_image_of_injective Subtype.val_injective
  ...
  have disjointM12 : Disjoint M1.support M2.support := by
    rw [hM1.2.support_eq_verts]; rw [hM2.support_eq_verts]; rw [Subgraph.verts_iSup]; rw [Set.disjoint_iUnion_right]
    exact fun K => hcomplMatch_compl K ▸ Set.disjoint_sdiff_right
  -- The only vertices left are indeed contained in universalVerts
  have : (M1.verts union M2.verts)ᶜ subseteq G.universalVerts := by
    rw [Set.compl_subset_comm]; rw [Set.compl_eq_univ_sdiff]
    intro v hv
    by_cases h : v in M1.verts
...
/--
theorem `Subgraph.IsPerfectMatching.exists_of_isClique_supp` / 定理 `Subgraph.IsPerfectMatching.exists_of_isClique_supp`
...
Depends on / 依赖: G.isClique_universalVerts.subset, IsMatching, IsMatching.exists_verts_compl_subset_universalVerts, Set.even_ncard_compl_iff, Set.ncard_eq_toFinset_card, Set.toFinite, Set.toFinset_card, Subgrap, Subgraph, Subgraph.isSpanning_iff, classical, disjoint_compl_right, even_card, even_iff_exists_isMatching, even_ncard_compl_iff, exists_verts_compl_subset_universalVerts, hM.even_card, hM.sup, hM.support_eq_verts, hveven
-/
theorem Subgraph.IsPerfectMatching.exists_of_isClique_supp
...
/--
theorem `IsTutteViolator.empty` / 定理 `IsTutteViolator.empty`

English:
theorem IsTutteViolator.empty
  given: (hodd : Odd (Nat.card V))
  statement: G.IsTutteViolator ∅
  proof: by
  rw [IsTutteViolator]; rw [Set.ncard_empty]
  exact ((odd_ncard_oddComponents _).mpr <| by simpa using hodd).pos

中文:
定理 IsTutteViolator.empty
  条件: (hodd : Odd (自然数.card V))
  结论: G.IsTutteViolator ∅
  证明: by
  rw [IsTutteViolator]; rw [Set.ncard_empty]
  exact ((odd_ncard_oddComponents _).mpr <| by simpa using hodd).pos

Depends on / 依赖: IsTutteViolator, Set.ncard_empty, ncard_empty, odd_ncard_oddComponents
-/
...
/--
lemma `not_isTutteViolator_of_isPerfectMatching` / 引理 `not_isTutteViolator_of_isPerfectMatching`

English:
lemma not_isTutteViolator_of_isPerfectMatching
  statement: {M : Subgraph G} (hM : M.IsPerfectMatching)
  proof: by
...
exact Subtype.val_injective ConnectedComponent.eq_of_common_vertex (hg c
...
Depends on / 依赖: ConnectedComponent, ConnectedComponent.eq_of_common_vertex, ConnectedComponent.odd_matches_node_outside, Injective, IsTutteViolator, Nat.card_le_card_of_injective, Subtype, Subtype.val_injective, card_le_card_of_injective, eq_iff, eq_of_adj_right, eq_of_common_vertex, f.Injective, hfinj.eq_iff, odd_matches_node_outside, replace, val_injective
-/
...
/--
theorem `tutte_exists_isPerfectMatching_of_near_matchings` / 定理 `tutte_exists_isPerfectMatching_of_near_matchings`

English:
theorem tutte_exists_isPerfectMatching_of_near_matchings
  statement: {x a b c : V}
  proof: by
  classical
...
private theorem tutte_exists_isPerfectMatching_of_near_matchings {x a b c : V}
  ...
  suffices exists (G' : SimpleGraph V), G'.IsAlternating M2.spanningCoe ∧ G'.IsCycles ∧ ¬G'.Adj x b ∧
  ...
  obtain ⟨x', hx', p, hp, hpac, hnpxb⟩ :
      exists x' in ({x, b} : Finset V), exists (p : cycles.Walk a x'), p.IsPath ∧
...
obtain ⟨x', hx', hx'p, htw⟩ := Walk.exists_mem_support_forall_mem_support_imp_eq {x, b} by
  ...
  have hle : p.toSubgraph.spanningCoe <= G ⊔ edge a c := by
    rw [← sdiff_edge _ (by simpa : ¬p.toSubgraph.spanningCoe.Adj x b)]; rw [sdiff_le_iff']
    intro v w hvw
  ...
  have aux {x' : V} (hx' : x' in ({x, b} : Set V)) (c' : V) (hc : c' != a)
...
/--
lemma `exists_isTutteViolator` / 引理 `exists_isTutteViolator`

English:
lemma exists_isTutteViolator
  statement: (h : forall (M : G.Subgraph), ¬M.IsPerfectMatching)
  proof: by
  classical
  cases nonempty_fintype V
...
-/
lemma exists_isTutteViolator (h : forall (M : G.Subgraph), ¬M.IsPerfectMatching)
    (hvEven : Even (Nat.card V)) :
    exists u, G.IsTutteViolator u := by
  classical
  cases nonempty_fintype V
  -- It suffices to consider the edge-maximal case
  obtain ⟨Gmax, hSubgraph, hMatchingFree, hMaximal⟩ := exists_maximal_isMatchingFree h
  ...
  by_cases! h' : forall (K : ConnectedComponent Gmax.deleteUniversalVerts.coe),
  ...
  · -- Deleting universal vertices does not result in only cliques
    ...
    simp only [ConnectedComponent.toSimpleGraph, deleteUniversalVerts, universalVerts,
      ...
      at hxa hxb hnadjxb
    obtain ⟨c, hc⟩ : exists (c : V), (a : V) != c ∧ ¬ Gmax.Adj a c := by
...
/--
theorem `tutte` / 定理 `tutte`

English:
theorem tutte
  statement: (exists M : Subgraph G, M.IsPerfectMatching) ↔ forall u, ¬ G.IsTutteViolator u
  proof: by
...
Depends on / 依赖: Nat.card, Nat.not_odd_iff_even.mp, contrapose, exists_isTutteViolator, not_isTutteViolator_of_isPerfectMatching, not_odd_iff_even
-/
theorem tutte : (exists M : Subgraph G, M.IsPerfectMatching) ↔ forall u, ¬ G.IsTutteViolator u := by
  refine ⟨by rintro ⟨M, hM⟩; apply not_isTutteViolator_of_isPerfectMatching hM, ?_⟩
  contrapose!
  intro h
  by_cases hvOdd : Odd (Nat.card V)
