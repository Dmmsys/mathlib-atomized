...
instance [IsLocalRing
  signature: R] [IsLocalRing S] [IsLocalHom (algebraMap R S)] :
  body: let eSp : ResidueField R otimes[R] S ≃ₐ[R] S ⧸ (maximalIdeal R).map (algebraMap R S) :=
...
instance [IsLocalRing R] [IsLocalRing S] [IsLocalHom (algebraMap R S)] :
    IsLocalRing (ResidueField R otimes[R] S) :=
  let eSp : ResidueField R otimes[R] S ≃ₐ[R] S ⧸ (maximalIdeal R).map (algebraMap R S) :=
    (Algebra.TensorProduct.comm _ _ _).trans
      ((TensorProduct.quotIdealMapEquivTensorQuot S (maximalIdeal R)).symm.restrictScalars _)
  have : Nontrivial (IsLocalRing.ResidueField R otimes[R] S) := by
...
/--
lemma `ResidueField.exists_smul_eq_tmul_one` / 引理 `ResidueField.exists_smul_eq_tmul_one`
...
-/
lemma ResidueField.exists_smul_eq_tmul_one
    (x : S otimes[R] p.ResidueField) : exists r ∉ p, exists s, r • x = s otimesₜ[R] 1 := by
  obtain ⟨t, r, a, hrt, e⟩ := RingHom.SurjectiveOnStalks.exists_mul_eq_tmul
    p.surjectiveOnStalks_residueField x ⊥ isPrime_bot
  obtain ⟨t, rfl⟩ := IsLocalRing.residue_surjective t
  obtain ⟨⟨y, t⟩, rfl⟩ := IsLocalization.mk'_surjective p.primeCompl t
...
English:
abbreviation Fiber
  signature: (p : Ideal R) [p.IsPrime] (S : Type*) [AddCommGroup S] [Module R S]
  body: p.ResidueField otimes[R] S
...
/--
lemma `Fiber.exists_smul_eq_one_tmul` / 引理 `Fiber.exists_smul_eq_one_tmul`

English:
lemma Fiber.exists_smul_eq_one_tmul
  given: (x : p.Fiber S)
  statement: exists r ∉ p, exists s, r • x = 1 otimesₜ[R] s
  proof: by
...
Depends on / 依赖: Algebra, Algebra.TensorProduct.comm, Ideal.ResidueField.exists_smul_eq_tmul_one, ResidueField, TensorProduct, exists_smul_eq_tmul_one
-/
lemma Fiber.exists_smul_eq_one_tmul (x : p.Fiber S) : exists r ∉ p, exists s, r • x = 1 otimesₜ[R] s := by
  obtain ⟨r, hr, s, e⟩ := Ideal.ResidueField.exists_smul_eq_tmul_one _
    (Algebra.TensorProduct.comm _ _ _ x)
  refine ⟨r, hr, s, by simpa using congr((Algebra.TensorProduct.comm _ _ _).symm $e)⟩
...
/--
Definition of `Fiber.algEquivQuotient` / `Fiber.algEquivQuotient` 的定义

English:
definition Fiber.algEquivQuotient
  signature: :
...
/--
Definition of `Fiber.algEquivAux₁` / `Fiber.algEquivAux₁` 的定义

English:
definition Fiber.algEquivAux₁
  signature: :
  body: Localization (algebraMapSubmonoid S p.primeCompl)
...
定义 Fiber.algEquivAux₁
  签名: :
  定义体: Localization (algebraMapSubmonoid S p.primeCompl)
    letI pS := p.map (algebraMap R S)
    letI : Algebra S (p.Fiber S) := rightAlgebra
    p.Fiber S ≃ₐ[S] Sp ⧸ pS.map (algebraMap S Sp) :=
  letI : Algebra S (p.Fiber S) := rightAlgebra
...
English:
definition Fiber.algEquivAux₂
  signature: (q : Ideal (p.Fiber S)) [q.IsPrime]
  body: q.comap includeRight
...
/--
Definition of `Fiber.localizationAlgEquivQuotient` / `Fiber.localizationAlgEquivQuotient` 的定义

English:
definition Fiber.localizationAlgEquivQuotient
  signature: (q : Ideal (p.Fiber S)) [q.IsPrime]
  body: q.comap includeRight
    letI Sr := Localization.AtPrime r
...
@[simps]
/--
Definition of `PrimeSpectrum.preimageEquivFiber` / `PrimeSpectrum.preimageEquivFiber` 的定义

English:
definition PrimeSpectrum.preimageEquivFiber
...
@[simps!]
/--
Definition of `PrimeSpectrum.preimageOrderIsoFiber` / `PrimeSpectrum.preimageOrderIsoFiber` 的定义

English:
definition PrimeSpectrum.preimageOrderIsoFiber
  signature: (p : PrimeSpectrum R)
  body: preimageEquivFiber R S p
  map_rel_iff' {q₁ q₂} := by
    constructor
...
@[simps!]
/--
Definition of `PrimeSpectrum.primesOverOrderIsoFiber` / `PrimeSpectrum.primesOverOrderIsoFiber` 的定义

English:
definition PrimeSpectrum.primesOverOrderIsoFiber
  signature: (R S : Type*) [CommRing R]
  body: .trans ⟨⟨fun q => ⟨⟨q, q.2.1⟩, PrimeSpectrum.ext q.2.2.1.symm⟩,
...
Depends on / 依赖: PrimeSpectrum, PrimeSpectrum.ext, PrimeSpectrum.preimageOrderIsoFiber, asIdeal, preimageOrderIsoFiber
-/
...
@[simps!]
/--
Definition of `PrimeSpectrum.preimageHomeomorphFiber` / `PrimeSpectrum.preimageHomeomorphFiber` 的定义

English:
definition PrimeSpectrum.preimageHomeomorphFiber
  signature: (R S : Type*) [CommRing R]
  body: by
...
@[simp]
/--
theorem `PrimeSpectrum.coe_primesOverOrderIsoFiber_symm_apply` / 定理 `PrimeSpectrum.coe_primesOverOrderIsoFiber_symm_apply`

English:
theorem PrimeSpectrum.coe_primesOverOrderIsoFiber_symm_apply
  given: (q : PrimeSpectrum (p.Fiber S))
  proof: rfl
