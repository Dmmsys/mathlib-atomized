...
/--
theorem `abc_subcall` / 定理 `abc_subcall`

English:
theorem abc_subcall
  statement: {a b c w : k[X]} {hw : w != 0} (wab : w = wronskian a b) (ha : a != 0)
  proof: by
  ...
  have abc_dr_ndeg_lt : abc_dr.natDegree < a.natDegree + b.natDegree := by
    calc
      abc_dr.natDegree <= w.natD

中文:
定理 abc_subcall
  结论: {a b c w : k[X]} {hw : w != 0} (wab : w = wronskian a b) (ha : a != 0)
  证明: by
  have ab_nz := mul_ne_zero ha hb
  have abc_nz := mul_ne_zero ab_nz hc
  -- bound the degree of `divRadical (a * b * c)` using Wronskian `w`
...
private theorem abc_subcall {a b c w : k[X]} {hw : w != 0} (wab : w = wronskian a b) (ha : a != 0)
  ...
  calc
    a.natDegree + b.natDegree + c.natDegree = (a * b * c).natDegree := by
      rw [Polynomial.natDegree_mul ab_nz hc]; rw [Polynomial.natDegree_mul ha hb]
    _ = ((divRadical (a * b * c)) * (radical (a * b * c))).natDegree := by
      rw [mul_comm _ (radical _)]; rw [radical_mul_divRadical]
...
/--
theorem `Polynomial.abc` / 定理 `Polynomial.abc`
...
theorem Polynomial.abc
  ...
  have hbc : IsCoprime b c := by
    ...
    rw [← hsum]; rw [IsCoprime.neg_right_iff]
...
protected theorem Polynomial.abc
  ...
  have abc_dr_dvd_w : divRadical (a * b * c) ∣ w := by
    ...
    rw [← wbc] at cdr_dvd_w
    rw [divRadical_mul (hca.symm.mul_left hbc)]; rw [divRadical_mul hab]
