...
/--
lemma `extensiveTopology.mem_sieves_iff_contains_colimit_cofan` / 引理 `extensiveTopology.mem_sieves_iff_contains_colimit_cofan`

English:
lemma extensiveTopology.mem_sieves_iff_contains_colimit_cofan
  given: {X : C} (S : Sieve X)
  proof: by
  constructor
  · intro h
...
Depends on / 依赖: Cofan.isColimitOfIsIsoSigmaDesc, Cofan.mk, Presieve, Presieve.ofArrows.mk, Sigma.desc, isColimitOfIsIsoSigmaDesc, ofArrows
-/
lemma extensiveTopology.mem_sieves_iff_contains_colimit_cofan {X : C} (S : Sieve X) :
    S in (extensiveTopology C) X ↔
      (exists (α : Type) (_ : Finite α) (Y : α -> C) (π : (a : α) -> (Y a ⟶ X)),
        Nonempty (IsColimit (Cofan.mk X π)) ∧ (forall a : α, (S.arrows) (π a))) := by
  constructor
  · intro h
    induction h with
    | of X S hS =>
