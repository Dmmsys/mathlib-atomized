...
lemma F_BddAbove
  ...
  simp only [mem_image, forall_exists_index, and_imp, forall
...
lemma F_edge_le_one
  ...
  · simp only [hz0, zero_sub, Real.rpow_neg_one, neg_zero, Real.rpow_zero, mul_one,
    ...
    rw [← hz0]
    apply le_of_lt (norm_lt_sSupNormIm_eps f ε hε
...
lemma norm_le_interpStrip_of_mem_verticalClosedStrip₀₁
  ...
  rwa [verticalClosedStrip, ← closure_Ioo zero_ne_one, ← closu
...
lemma scale_id_mem_verticalStrip_of_mem_verticalStrip
  ...
  obtain ⟨hz₁, hz₂⟩ := hz
  simp only [hz₁, mul_pos_iff_of_pos_left, sub_pos, h
...
lemma mem_verticalClosedStrip_of_scale_id_mem_verticalClosedStrip
  ...
  norm_cast
  simp_rw [Complex.normSq_ofReal, mul_div_assoc, div_mu
...
lemma norm_le_interpStrip_of_mem_verticalClosedStrip
  ...
  rw [fun_arg_eq hul]; rw [div_sub_div_same]; r
...
lemma norm_le_interp_of_mem_verticalClosedStrip'
  ...
  simp only [scale, smul_eq_mul, sub_re] at hgoa
