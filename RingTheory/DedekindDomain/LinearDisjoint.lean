...
/--
theorem `Submodule.traceDual_le_span_map_traceDual` / 定理 `Submodule.traceDual_le_span_map_traceDual`
...
/--
theorem `differentIdeal_dvd_map_differentIdeal` / 定理 `differentIdeal_dvd_map_differentIdeal`

English:
theorem differentIdeal_dvd_map_differentIdeal
...
/--
theorem `map_differentIdeal_dvd_differentIdeal` / 定理 `map_differentIdeal_dvd_differentIdeal`

English:
theorem map_differentIdeal_dvd_differentIdeal
  proof: have := (differentIdeal_eq_differentIdeal_mul_differentIdeal A R₂ B).symm.trans
    (differentIdeal_eq_differentIdeal_mul_differentIdeal A R₁ B)
  h.symm.dvd_of_dvd_mul_right (dvd_of_mul_left_eq _ this)

中文:
定理 map_differentIdeal_dvd_differentIdeal
  证明: have := (differentIdeal_eq_differentIdeal_mul_differentIdeal A R₂ B).symm.trans
    (differentIdeal_eq_differentIdeal_mul_differentIdeal A R₁ B)
  h.symm.dvd_of_dvd_mul_right (dvd_of_mul_left_eq _ this)

Depends on / 依赖: differentIdeal_eq_differentIdeal_mul_differentIdeal, dvd_of_dvd_mul_right, dvd_of_mul_left_eq, h.symm.dvd_of_dvd_mul_right, symm.trans
-/
theorem map_differentIdeal_dvd_differentIdeal
...
/--
theorem `differentIdeal_eq_map_differentIdeal` / 定理 `differentIdeal_eq_map_differentIdeal`

English:
theorem differentIdeal_eq_map_differentIdeal
  statement: [Module.Free A R₂] (h₁ : F₁.LinearDisjoint F₂)
  proof: by
  apply dvd_antisymm
...
Depends on / 依赖: differentIdeal_dvd_map_differentIdeal, dvd_antisymm, map_differentIdeal_dvd_differentIdeal
-/
...
/--
theorem `differentIdeal_eq_differentIdeal_mul_differentIdeal_of_isCoprime` / 定理 `differentIdeal_eq_differentIdeal_mul_differentIdeal_of_isCoprime`
...
中文:
定理 differentIdeal_eq_differentIdeal_mul_differentIdeal_of_isCoprime
  证明: by
  have := differentIdeal_eq_differentIdeal_mul_differentIdeal A R₂ B
  rwa [← differentIdeal_eq_map_differentIdeal A B R₁ R₂ h₁ h₂ h₃,
    mul_comm] at this

Depends on / 依赖: differentIdeal_eq_differentIdeal_mul_differentIdeal, differentIdeal_eq_map_differentIdeal, mul_comm
-/
theorem differentIdeal_eq_differentIdeal_mul_differentIdeal_of_isCoprime
...
/--
theorem `Submodule.traceDual_eq_span_map_traceDual_of_linearDisjoint` / 定理 `Submodule.traceDual_eq_span_map_traceDual_of_linearDisjoint`
...
/--
theorem `ofIsCoprimeDifferentIdeal_aux` / 定理 `ofIsCoprimeDifferentIdeal_aux`

English:
theorem ofIsCoprimeDifferentIdeal_aux
  statement: [Module.Free A R₂]
  proof: by
  classical
  have h₂' : F₁ ⊔ F₂ = ⊤ := by
...
/--
Definition of `ofIsCoprimeDifferentIdeal` / `ofIsCoprimeDifferentIdeal` 的定义

English:
definition ofIsCoprimeDifferentIdeal
  signature: (h₁ : F₁.LinearDisjoint F₂)
  body: have : Module.Free A R₂ := Free.of_basis b
...
定义 ofIsCoprimeDifferentIdeal
  签名: (h₁ : F₁.LinearDisjoint F₂)
  定义体: have : Module.Free A R₂ := Free.of_basis b
  let v := fun i : ι => algebraMap R₂ B (b i)
  let b₂ : Basis ι K F₂ := b.localizationLocalization K A⁰ F₂
  have P₁ : LinearIndependent R₁ v := by
    rw [← LinearMap.linearIndependent_iff (IsScalarTower.toAlgHom R₁ B L).toLinearMap
...
@[simp]
/--
theorem `ofIsCoprimeDifferentIdeal_apply` / 定理 `ofIsCoprimeDifferentIdeal_apply`

English:
theorem ofIsCoprimeDifferentIdeal_apply
  statement: (h₁ : F₁.LinearDisjoint F₂)
  proof: by
  simp [Module.Basis.ofIsCoprimeDifferentIdeal]

中文:
定理 ofIsCoprimeDifferentIdeal_apply
  结论: (h₁ : F₁.LinearDisjoint F₂)
  证明: by
  simp [Module.Basis.ofIsCoprimeDifferentIdeal]

Depends on / 依赖: Module, Module.Basis.ofIsCoprimeDifferentIdeal, ofIsCoprimeDifferentIdeal
-/
...
/--
theorem `range_sup_range_eq_top_of_isCoprime_differentIdeal` / 定理 `range_sup_range_eq_top_of_isCoprime_differentIdeal`
...
/--
theorem `adjoin_union_eq_top_of_isCoprime_differentialIdeal` / 定理 `adjoin_union_eq_top_of_isCoprime_differentialIdeal`

English:
theorem adjoin_union_eq_top_of_isCoprime_differentialIdeal
  statement: [Module.Free A R₂]
  proof: by
...
theorem adjoin_union_eq_top_of_isCoprime_differentialIdeal [Module.Free A R₂]
    ...
    Algebra.adjoin A (algebraMap R₁ B '' s union algebraMap R₂ B '' t) = ⊤ := by
  rw [Algebra.adjoin_union]; rw [← IsScalarTower.coe_toAlgHom' A R₁]; rw [← IsScalarTower.coe_toAlgHom' A R₂]; rw [← AlgHom.map_adjoin]; rw [hs]; rw [← AlgHom.map_adjoin]; rw [ht]; rw [Algebra.map_top]; rw [Algebra.map_top]
