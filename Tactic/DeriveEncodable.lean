...
/-
**Mathlib.Deriving.Encodable.S** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathlib.Deriving.Enc
odable`。
...
The idea is that every encodable inductive type can be represented as a tree of 
natural numbers.
Inspiration for this is s-expressions used in Lisp/Scheme.
```lean
inductive S : Type where
...
def encodableT_toS {α} [Encodable α] (x : T α) : S :=
  ...
  | T.a a a_1 a_2 =>
    S.cons (S.nat 0)
      (S.cons (S.nat (Encodable.encode a))
        (S.cons (S.nat (Encodable.encode a_1)) (S.cons (encodableT_toS a_2) (S.n
at 0))))
  | T.b => S.cons (S.nat 1) (S.nat 0)
...
private theorem encodableT {α} [Encodable α] (x : @T α) :
    encodableT_fromS (encodableT_toS x) = some x := by
  cases x <;> (unfold encodableT_toS encodableT_fromS; simp only [Encodable.enco
dek, encodableT])

instance {α} [Encodable α] : Encodable (@T α) :=
  Encodable.ofLeftInjection encodableT_toS encodableT_fromS encodableT
```
The idea is that each constructor gets encoded as a linked list made of `S.cons`
 constructors
that is tagged with the constructor index.
-/
private inductive S : Type where
  ...
  | cons (a b : S)
/-
**Mathlib.Deriving.Encodable.S.encode** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Derivin
g.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Deriving.Encodable.nat_unpair_lt_2** 是 Mathlib 中的一个引理，位于命名空间 `Mathlib.
Deriving.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
private lemma nat_unpair_lt_2 {n : ℕ} (h : (Nat.unpair n).1 ≠ 0) : (Nat.unpair n).2 < n := by
  ...
  split <;> lia
/-
**Mathlib.Deriving.Encodable.S.decode** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Derivin
g.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Deriving.Encodable.S_equiv** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Deriving
.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
    ...
    next h =>
      ...
      · obtain _ | n' := n
        ...
        · have := Nat.unpair_lt (by lia : 1 ≤ n' + 1)
          lia
/-
**Mathlib.Deriving.Encodable.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Deriving.Encoda
ble`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Mathlib.Deriving.Encodable.mkToSMatch** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Deriv
ing.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Encodable.mkToSFuns** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Derivi
ng.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Encodable.mkFromSMatch** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Der
iving.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Encodable.mkFromSFuns** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Deri
ving.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Deriving.Encodable.mkInjThms** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Derivi
ng.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Deriving.Encodable.mkEncodableInstanceCmds** 是 Mathlib 中的一个定义，位于命名空间 `
Mathlib.Deriving.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
private def mkEncodableInstanceCmds (ctx : Deriving.Context) (typeNames : Array Name)
  ...
  return instances
/-
**Mathlib.Deriving.Encodable.mkEncodableCmds** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Deriving.Encodable`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
-/
/-
**Mathlib.Deriving.Encodable.mkEncodableInstance** 是 Mathlib 中的一个定义，位于命名空间 `Math
lib.Deriving.Encodable`。
形式化陈述：mkEncodableInstance (declNames : Array Name) : CommandElabM Bool
