...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.eq_zero_of_hom_shift_pos** 是 Ma
thlib 中的一个引理，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：eq_zero_of_hom_shift_pos {X Y : A} {n : Int} (f : (ι.obj X)⟦n⟧ ⟶ ι.obj Y) 
(hn : 0 < n) : f = 0
参数：f : (ι.obj X)⟦n⟧ ⟶ ι.obj Y；hn : 0 < n。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.** 是 Mathlib 中的一个定义，位于命名空间 `Cat
egoryTheory.Triangulated.AbelianSubcategory`。
...
@[simp, reassoc]
/-
**CategoryTheory.Triangulated.AbelianSubcategory.shift_** 是 Mathlib 中的一个引理，位于命名空
间 `CategoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp, reassoc]
/-
**CategoryTheory.Triangulated.AbelianSubcategory.** 是 Mathlib 中的一个引理，位于命名空间 `Cat
egoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[reassoc]
/-
**CategoryTheory.Triangulated.AbelianSubcategory.mor** 是 Mathlib 中的一个引理，位于命名空间 `
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.mono_** 是 Mathlib 中的一个引理，位于命名空间
 `CategoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.epi_** 是 Mathlib 中的一个引理，位于命名空间 
`CategoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.exists_lift_** 是 Mathlib 中的一个引理
，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-- `ιK` is a kernel. -/
/-
**CategoryTheory.Triangulated.AbelianSubcategory.isLimitKernelFork** 是 Mathlib 中
的一个定义，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：isLimitKernelFork : IsLimit (KernelFork.ofι _ (ιK_mor₁ hT α))
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.ιK_mor₁`：ιK_mor₁ : ιK f₃ 
α ≫ f₁ = 0
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.exists_lift_ιK`：exists_li
ft_ιK {B : A} (x₁ : B ⟶ X₁) (hx₁ : x₁ ≫ f₁ = 0) : exists (k : B ⟶ K), k ≫ ιK f₃ 
α = x₁
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.exists_desc_** 是 Mathlib 中的一个引理
，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-- `πQ` is a cokernel. -/
/-
**CategoryTheory.Triangulated.AbelianSubcategory.isColimitCokernelCofork** 是 Mat
hlib 中的一个定义，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：isColimitCokernelCofork : IsColimit (CokernelCofork.ofπ _ (mor₁_πQ hT β))
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.mor₁_πQ`：mor₁_πQ : f₁ ≫ π
Q f₂ β = 0
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.exists_desc_πQ`：exists_de
sc_πQ {B : A} (x₂ : X₂ ⟶ B) (hx₂ : f₁ ≫ x₂ = 0) : exists (k : Q ⟶ B), πQ f₂ β ≫ 
k = x₂
...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.hasKernel** 是 Mathlib 中的一个引理，位于
命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：hasKernel : HasKernel f₁
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.ιK_mor₁`：ιK_mor₁ : ιK f₃ 
α ≫ f₁ = 0
...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.hasCokernel** 是 Mathlib 中的一个引理，
位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：hasCokernel : HasCokernel f₁
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.mor₁_πQ`：mor₁_πQ : f₁ ≫ π
Q f₂ β = 0
...
`(ι.obj K)⟦1⟧ ⟶ X₃ ⟶ (ι.obj Q) ⟶ ...`. -/
/-
**CategoryTheory.Triangulated.AbelianSubcategory.admissibleMorphism** 是 Mathlib 
中的一个定义，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：admissibleMorphism : MorphismProperty A
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a functor `ι : A ⥤ C` from a preadditive category to a triangulated catego
ry,
a morphism `X₁ ⟶ X₂` in `A` is admissible if, when we complete `ι.obj f₁` in
a distinguished triangle `ι.obj X₁ ⟶ ι.obj X₂ ⟶ X₃ ⟶ (ι.obj X₁)⟦1⟧`,
there exists objects `K` and `Q`, and a distinguished triangle
...
include hι in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.hasKernel_of_admissibleMorphism
** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：hasKernel_of_admissibleMorphism {X₁ X₂ : A} (f₁ : X₁ ⟶ X₂) (hf₁ : admissib
leMorphism ι f₁) : HasKernel f₁
参数：f₁ : X₁ ⟶ X₂；hf₁ : admissibleMorphism ι f₁。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.hasKernel`：hasKernel : Ha
sKernel f₁
-/
...
include hι in
/-
**CategoryTheory.Triangulated.AbelianSubcategory.hasCokernel_of_admissibleMorphi
sm** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：hasCokernel_of_admissibleMorphism {X₁ X₂ : A} (f₁ : X₁ ⟶ X₂) (hf₁ : admiss
ibleMorphism ι f₁) : HasCokernel f₁
参数：f₁ : X₁ ⟶ X₂；hf₁ : admissibleMorphism ι f₁。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Pretriangulated.distinguished_cocone_triangle`：∀ {C : Typ
e u} {inst : CategoryTheory.Category.{v, u} C} {inst_1 : CategoryTheory.Limits.H
asZeroObject C}   {inst_2 : CategoryTheory.HasShif…
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.hasCokernel`：hasCokernel 
: HasCokernel f₁
-/
...
**CategoryTheory.Triangulated.AbelianSubcategory.isLimitKernelForkOfDistTriang**
 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：isLimitKernelForkOfDistTriang {X₁ X₂ X₃ : A} (f₁ : X₁ ⟶ X₂) (f₂ : X₂ ⟶ X₃)
 (f₃ : ι.obj X₃ ⟶ (ι.obj X₁)⟦(1 : Int)⟧) (hT : Triangle.mk (ι.map f₁) (ι.map f₂)
 f₃ in distTriang C) : IsLimit (KernelFork.ofι f₁ (show f₁ ≫ f₂ = 0 from ι.map_i
njective (by have
参数：f₁ : X₁ ⟶ X₂；f₂ : X₂ ⟶ X₃；f₃ : ι.obj X₃ ⟶ (ι.obj X₁)⟦(1 : Int)⟧；hT : Triangle
.mk (ι.map f₁) (ι.map f₂) f₃ in distTriang C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.isColimitCokernelCoforkOfDistTr
iang** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：isColimitCokernelCoforkOfDistTriang {X₁ X₂ X₃ : A} (f₁ : X₁ ⟶ X₂) (f₂ : X₂
 ⟶ X₃) (f₃ : ι.obj X₃ ⟶ (ι.obj X₁)⟦(1 : Int)⟧) (hT : Triangle.mk (ι.map f₁) (ι.m
ap f₂) f₃ in distTriang C) : IsColimit (CokernelCofork.ofπ f₂ (show f₁ ≫ f₂ = 0 
from ι.map_injective (by have
参数：f₁ : X₁ ⟶ X₂；f₂ : X₂ ⟶ X₃；f₃ : ι.obj X₃ ⟶ (ι.obj X₁)⟦(1 : Int)⟧；hT : Triangle
.mk (ι.map f₁) (ι.map f₂) f₃ in distTriang C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CategoryTheory.Triangulated.AbelianSubcategory.exists_distinguished_triangle_o
f_epi** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory.Triangulated.AbelianSubcategory`
。
形式化陈述：exists_distinguished_triangle_of_epi {X₂ X₃ : A} (π : X₂ ⟶ X₃) [Epi π] : e
xists (X₁ : A) (i : X₁ ⟶ X₂) (δ : ι.obj X₃ ⟶ (ι.obj X₁)⟦(1 : Int)⟧), Triangle.mk
 (ι.map i) (ι.map π) δ in distTriang C
参数：π : X₂ ⟶ X₃。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Pretriangulated.distinguished_cocone_triangle`：∀ {C : Typ
...
f) = -F.map f
· 使用引理 `CategoryTheory.Triangulated.AbelianSubcategory.shift_ι_map_ιK`：shift_ι_m
ap_ιK : (ι.map (ιK f₃ α))⟦(1 : Int)⟧' = α ≫ f₃
...
@[instance_reducible]
/-
**CategoryTheory.Triangulated.AbelianSubcategory.abelian** 是 Mathlib 中的一个定义，位于命名
空间 `CategoryTheory.Triangulated.AbelianSubcategory`。
形式化陈述：abelian [IsTriangulated C] : Abelian A
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
