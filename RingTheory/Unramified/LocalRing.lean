...
lemma isUnramifiedAt_iff_map_eq
  ...
  · exact ⟨fun _ => .of_restrictScalars R _ _,
      fun _ => Algebra.FormallyUnramified.comp _ (Localization.AtPrime p) 
...
lemma finite_of_primesOver_eq_singleton
  ...
  rw [Finset.coe_image]; rw [← Submodule.span_span_of_tower R]; rw [← Submodule.map_span]; rw [hs]; rw [Submodule.map_top]; rw [LinearMap.coe_range]; rw [AlgHom
