...
/--
Definition of `BarrelledSpace` / `BarrelledSpace` 的定义

English:
class BarrelledSpace
  parameters: (𝕜 E : Type*) [SeminormedRing 𝕜] [AddGroup E] [SMul 𝕜 E]
...
/--
theorem `Seminorm.continuous_of_lowerSemicontinuous` / 定理 `Seminorm.continuous_of_lowerSemicontinuous`

English:
theorem Seminorm.continuous_of_lowerSemicontinuous
  statement: {𝕜 E : Type*} [AddGroup E] [SMul 𝕜 E]
...
/--
theorem `Seminorm.continuous_iSup` / 定理 `Seminorm.continuous_iSup`
...
Depends on / 依赖: Seminorm, Seminorm.bddAbove_range_iff, Seminorm.coe_iSup_eq, Seminorm.continuous_of_lowerSemicontinuous, bddAbove_range_iff, coe_iSup_eq, continuous_of_lowerSemicontinuous, convert, iSup_apply, lowerSemicontinuous, lowerSemicontinuous_ciSup
-/
theorem Seminorm.continuous_iSup
    {ι : Sort*} {𝕜 E : Type*} [NormedField 𝕜] [AddCommGroup E] [Module 𝕜 E]
    [TopologicalSpace E] [BarrelledSpace 𝕜 E] (p : ι -> Seminorm 𝕜 E)
    (hp : forall i, Continuous (p i)) (bdd : BddAbove (range p)) :
    Continuous (⨆ i, p i) := by
  rw [← Seminorm.coe_iSup_eq bdd]
  refine Seminorm.continuous_of_lowerSemicontinuous _ ?_
  rw [Seminorm.coe_iSup_eq bdd]
...
/--
Instance `BaireSpace.instBarrelledSpace` / 实例 `BaireSpace.instBarrelledSpace`

English:
instance BaireSpace.instBarrelledSpace
  signature: [TopologicalSpace E] [IsTopologicalAddGroup E]
  body: by
...
theorem banach_steinhaus
  given: (H : forall k x, BddAbove (range fun i => q k (𝓕 i x)))
  proof: by
...
中文:
定理 banach_steinhaus
  条件: (H : 对任意 k x, BddAbove (range fun i => q k (𝓕 i x)))
  证明: by
  -- We just have to prove that `⊔ i, (q k) ∘ (𝓕 i)` is a (well-defined) continuous seminorm
  -- for all `k`.
  refine (hq.uniformEquicontinuous_iff_bddAbove_and_continuous_iSup (toLinearMap ∘ 𝓕)).mpr ?_
...
/--
theorem `PolynormableSpace.banach_steinhaus` / 定理 `PolynormableSpace.banach_steinhaus`

English:
theorem PolynormableSpace.banach_steinhaus
  proof: by
...
protected theorem PolynormableSpace.banach_steinhaus
    (H : forall x, IsVonNBounded 𝕜₂ (range fun i => 𝓕 i x)) :
    UniformEquicontinuous ((↑) ∘ 𝓕) := by
  have hp := PolynormableSpace.withSeminorms 𝕜₂ F
  refine hp.banach_steinhaus ?_
  simp_rw [hp.isVonNBounded_iff_seminorm_bddAbove, ← range_comp] at H
...
/--
Definition of `continuousLinearMapOfTendsto` / `continuousLinearMapOfTendsto` 的定义

English:
definition continuousLinearMapOfTendsto
  body: linearMapOfTendsto _ _ h
  cont := by
...
/--
Definition of `WithSeminorms.continuousLinearMapOfTendsto` / `WithSeminorms.continuousLinearMapOfTendsto` 的定义

English:
abbreviation WithSeminorms.continuousLinearMapOfTendsto
  signature: [T2Space F] {l : Filter α}
  body: haveI : PolynormableSpace 𝕜₂ F := hq.toPolynormableSpace
