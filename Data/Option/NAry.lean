...
/-
**Option.map** 是 Mathlib 中的一个定义，位于命名空间 `Option`。
形式化陈述：{α : Type u_1} → {β : Type u_2} → (α → β) → Option α → Option β
参数：α → β。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The image of a binary function `f : α → β → γ` as a function `Option α → Option 
β → Option γ`.
...
--- 原说明 ---
`Option.map₂` in terms of monadic operations. Note that this can't be taken as t
he definition
because of the lack of universe polymorphism.
-/
...
/-
**Option.map_map** 是 Mathlib 中的一个定理，位于命名空间 `Option`。
形式化陈述：∀ {β : Type u_1} {γ : Type u_2} {α : Type u_3} (h : β → γ) (g : α → β) (x 
: Option α),   Option.map h (Option.map g x) = Option.map (h ∘ g) x
参数：h : β → γ；g : α → β；x : Option α；Option.map g x；h ∘ g。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `of_eq_true`：∀ {p : Prop}, p = True → p
...
@[simp]
/-
**Option.map_uncurry** 是 Mathlib 中的一个定理，位于命名空间 `Option`。
形式化陈述：map_uncurry (f : α -> β -> γ) (x : Option (α × β)) : x.map (uncurry f) = m
ap₂ f (x.map Prod.fst) (x.map Prod.snd)
参数：f : α -> β -> γ；x : Option (α × β)。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
--- 原说明 ---
If `a` is a left identity for a binary operation `f`, then `some a` is a left id
entity for
`Option.map₂ f`.
-/
...
--- 原说明 ---
If `b` is a right identity for a binary operation `f`, then `some b` is a right 
identity for
`Option.map₂ f`.
-/
