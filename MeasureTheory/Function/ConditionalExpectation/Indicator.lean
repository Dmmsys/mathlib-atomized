...
theorem condExp_indicator_aux
  ...
  refine ((hsf_zero (μ[f | m]) (condExp_ae_eq_restr
...
theorem condExp_indicator
  ...
  have : SigmaFinite (μ.trim hm) := hμm
  -- use `have` to perform what should be the
...
theorem condExp_restrict_ae_eq_restrict
  ...
  refine ae_eq_condExp_of_forall_setIntegral_eq hm (hf_int.indicator (hm _ hs_m))
...
theorem condExp_ae_eq_restrict_of_measurableSpace_eq_on
  ...
  refine ((condExp_indicator hf_int hs_m).symm.trans 
