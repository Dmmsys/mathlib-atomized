...
@[simps!]
/-
**CategoryTheory.Triangulated.TStructure.triangle** 是 Mathlib 中的一个定义，位于命名空间 `Cat
egoryTheory.Triangulated.TStructure`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The functorial (distinguished) triangles that are part of the spectral
object attached to objects in a triangulated category equipped with a t-structur
e.
-/
...
--- 原说明 ---
The triangle `(t.triangleω₁δ a b c hab hbc).obj X` is isomorphic to
the (distinguished) triangle obtained by applying the functor `t.eTriangleLTGE.o
bj b`
to the object `(t.eTruncGE.obj a).obj ((t.eTruncLT.obj c).obj X)`.
-/
...
/-
**CategoryTheory.Triangulated.TStructure.triangle** 是 Mathlib 中的一个引理，位于命名空间 `Cat
egoryTheory.Triangulated.TStructure`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps ω₁]
/-
**CategoryTheory.Triangulated.TStructure.spectralObject** 是 Mathlib 中的一个定义，位于命名空
间 `CategoryTheory.Triangulated.TStructure`。
形式化陈述：spectralObject (X : C) : SpectralObject C EInt where ω₁
参数：X : C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simp]
/-
**CategoryTheory.Triangulated.TStructure.spectralObject_** 是 Mathlib 中的一个引理，位于命名
...
@[simps]
/-
**CategoryTheory.Triangulated.TStructure.spectralObjectFunctor** 是 Mathlib 中的一个定
义，位于命名空间 `CategoryTheory.Triangulated.TStructure`。
形式化陈述：spectralObjectFunctor : C ⥤ SpectralObject C EInt where obj
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
