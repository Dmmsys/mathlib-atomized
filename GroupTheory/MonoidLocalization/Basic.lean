/-
Copyright (c) 2019 Amelia Livingston. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Amelia Livingston
-/
module

public import Mathlib.Algebra.BigOperators.Group.Finset.Basic
public import Mathlib.Algebra.Group.Submonoid.Operations
public import Mathlib.Algebra.Regular.Basic
public import Mathlib.GroupTheory.Congruence.Hom
public import Mathlib.GroupTheory.OreLocalization.Basic

/-!
# Localizations of commutative monoids

Localizing a commutative ring at one of its submonoids does not rely on the ring's addition, so
we can generalize localizations to commutative monoids.

We characterize the localization of a commutative monoid `M` at a submonoid `S` up to
isomorphism; that is, a commutative monoid `N` is the localization of `M` at `S` iff we can find a
monoid homomorphism `f : M →* N` satisfying 3 properties:
1. For all `y ∈ S`, `f y` is a unit;
2. For all `z : N`, there exists `(x, y) : M × S` such that `z * f y = f x`;
3. For all `x, y : M` such that `f x = f y`, there exists `c ∈ S` such that `x * c = y * c`.
   (The converse is a consequence of 1.)

Given such a localization map `f : M →* N`, we can define the surjection
`Submonoid.LocalizationMap.mk'` sending `(x, y) : M × S` to `f x * (f y)⁻¹`. Mapping properties
of the localization (e.g. extending a map from `M → P` to `N` if the image of `S` is contained in
the units) are treated in a later file `Mathlib.GroupTheory.MonoidLocalization.Maps`.

We also define the quotient of `M × S` by the unique congruence relation (equivalence relation
preserving a binary operation) `r` such that for any other congruence relation `s` on `M × S`
satisfying '`∀ y ∈ S`, `(1, 1) ∼ (y, y)` under `s`', we have that `(x₁, y₁) ∼ (x₂, y₂)` by `s`
whenever `(x₁, y₁) ∼ (x₂, y₂)` by `r`. We show this relation is equivalent to the standard
localization relation.
This defines the localization as a quotient type, `Localization`, but the majority of
subsequent lemmas in the file are given in terms of localizations up to isomorphism, using maps
which satisfy the characteristic predicate.

The Grothendieck group construction corresponds to localizing at the top submonoid, namely making
every element invertible.

## Implementation notes

In maths it is natural to reason up to isomorphism, but in Lean we cannot naturally `rewrite` one
structure with an isomorphic one; one way around this is to isolate a predicate characterizing
a structure up to isomorphism, and reason about things that satisfy the predicate.

The infimum form of the localization congruence relation is chosen as 'canonical' here, since it
shortens some proofs.

To reason about the localization as a quotient type, use `mk_eq_monoidOf_mk'` and associated
lemmas. These show the quotient map `mk : M → S → Localization S` equals the
surjection `LocalizationMap.mk'` induced by the map
`Localization.monoidOf : Submonoid.LocalizationMap S (Localization S)` (where `of` establishes the
localization as a quotient type satisfies the characteristic predicate). The lemma
`mk_eq_monoidOf_mk'` hence gives you access to the results in the rest of the file, which are about
the `LocalizationMap.mk'` induced by any localization map.

## TODO

* Show that the localization at the top monoid is a group.
* Generalise to (nonempty) subsemigroups.
* If we acquire more bundlings, we can make `Localization.mkOrderEmbedding` be an ordered monoid
  embedding.

## Tags
localization, monoid localization, quotient monoid, congruence relation, characteristic predicate,
commutative monoid, grothendieck group
-/

@[expose] public section

assert_not_exists MonoidWithZero Ring

open Function
namespace AddSubmonoid

variable {M : Type*} [AddCommMonoid M] (S : AddSubmonoid M) (N : Type*) [AddCommMonoid N]

variable {N} in
/--
Definition of `IsLocalizationMap` / `IsLocalizationMap` 的定义

English:
structure IsLocalizationMap
  parameters: (S : AddSubmonoid M) (f : M -> N)
  axioms and operations (3):
    - map_addUnits((y : S)) : IsAddUnit (f y)
    - surj((z : N)) : exists x : M × S, z + f x.2 = f x.1
    - exists_of_eq({x y}) : f x = f y -> exists c : S, c + x = c + y

中文:
结构 是Localization映射
  参数: (S : 加法子幺半群 M) (f : M -> N)
  公理与运算 (3 个):
    - map_addUnits((y : S)) : IsAddUnit (f y)
    - surj((z : N)) : 存在 x : M × S, z + f x.2 = f x.1
    - exists_of_eq({x y}) : f x = f y -> 存在 c : S, c + x = c + y
-/
structure IsLocalizationMap (S : AddSubmonoid M) (f : M -> N) where
  map_addUnits (y : S) : IsAddUnit (f y)
  surj (z : N) : exists x : M × S, z + f x.2 = f x.1
  exists_of_eq {x y} : f x = f y -> exists c : S, c + x = c + y

/--
Definition of `LocalizationMap` / `LocalizationMap` 的定义

English:
structure LocalizationMap
  parameters: extends M ->ₙ+ N
  extends: M ->ₙ+ N
  axioms and operations (1):
    - isLocalizationMap : IsLocalizationMap S toFun

中文:
结构 Localization映射
  参数: extends M ->ₙ+ N
  继承: M ->ₙ+ N
  公理与运算 (1 个):
    - isLocalizationMap : 是Localization映射 S toFun

Depends on / 依赖: P.flip.root
-/
structure LocalizationMap extends M ->ₙ+ N where
  isLocalizationMap : IsLocalizationMap S toFun

/-- The additive homomorphism underlying a `LocalizationMap` of `AddCommMonoid`s. -/
add_decl_doc LocalizationMap.toAddHom

end AddSubmonoid

section CommMonoid

variable {M : Type*} [CommMonoid M] (S : Submonoid M) (N : Type*) [CommMonoid N] {P : Type*}
  [CommMonoid P]

namespace Submonoid

variable {N} in
/-- A predicate characterizing homomorphisms between monoids `M` and `N` that form a
commutative triangle with the canonical map from `M` to its localization at `S` and
some isomorphism between `N` and the localization. -/
@[to_additive (attr := mk_iff)]
/--
Definition of `IsLocalizationMap` / `IsLocalizationMap` 的定义

English:
structure IsLocalizationMap
  parameters: (S : Submonoid M) (f : M -> N)
  axioms and operations (3):
    - map_units((y : S)) : IsUnit (f y)
    - surj((z : N)) : exists x : M × S, z * f x.2 = f x.1
    - exists_of_eq({x y}) : f x = f y -> exists c : S, c * x = c * y

中文:
结构 是Localization映射
  参数: (S : 子幺半群 M) (f : M -> N)
  公理与运算 (3 个):
    - map_units((y : S)) : 是单位 (f y)
    - surj((z : N)) : 存在 x : M × S, z * f x.2 = f x.1
    - exists_of_eq({x y}) : f x = f y -> 存在 c : S, c * x = c * y
-/
structure IsLocalizationMap (S : Submonoid M) (f : M -> N) where
  map_units (y : S) : IsUnit (f y)
  surj (z : N) : exists x : M × S, z * f x.2 = f x.1
  exists_of_eq {x y} : f x = f y -> exists c : S, c * x = c * y

/--
Definition of `LocalizationMap` / `LocalizationMap` 的定义

English:
structure LocalizationMap
  parameters: extends M ->ₙ* N
  extends: M ->ₙ* N
  axioms and operations (1):
    - isLocalizationMap : IsLocalizationMap S toFun

中文:
结构 Localization映射
  参数: extends M ->ₙ* N
  继承: M ->ₙ* N
  公理与运算 (1 个):
    - isLocalizationMap : 是Localization映射 S toFun
-/
@[to_additive] structure LocalizationMap extends M ->ₙ* N where
  isLocalizationMap : IsLocalizationMap S toFun

/-- The multiplicative homomorphism underlying a `LocalizationMap`. -/
add_decl_doc LocalizationMap.toMulHom

end Submonoid

namespace Localization

/- Ensure that `@[to_additive]` uses the right namespace before the definition of `Localization`. -/
insert_to_additive_translation Localization AddLocalization

/-- The congruence relation on `M × S`, `M` a `CommMonoid` and `S` a submonoid of `M`, whose
quotient is the localization of `M` at `S`, defined as the unique congruence relation on
`M × S` such that for any other congruence relation `s` on `M × S` where for all `y ∈ S`,
`(1, 1) ∼ (y, y)` under `s`, we have that `(x₁, y₁) ∼ (x₂, y₂)` by `r` implies
`(x₁, y₁) ∼ (x₂, y₂)` by `s`. -/
@[to_additive
/-- The congruence relation on `M × S`, `M` an `AddCommMonoid` and `S` an `AddSubmonoid` of `M`,
whose quotient is the localization of `M` at `S`, defined as the unique congruence relation on
`M × S` such that for any other congruence relation `s` on `M × S` where for all `y ∈ S`,
`(0, 0) ∼ (y, y)` under `s`, we have that `(x₁, y₁) ∼ (x₂, y₂)` by `r` implies
`(x₁, y₁) ∼ (x₂, y₂)` by `s`. -/]
/--
Definition of `r` / `r` 的定义

English:
definition r
  signature: (S : Submonoid M)
  body: sInf { c | forall y : S, c 1 (y, y) }

中文:
定义 r
  签名: (S : 子幺半群 M)
  定义体: sInf { c | forall y : S, c 1 (y, y) }
-/
def r (S : Submonoid M) : Con (M × S) :=
  sInf { c | forall y : S, c 1 (y, y) }

/-- An alternate form of the congruence relation on `M × S`, `M` a `CommMonoid` and `S` a
submonoid of `M`, whose quotient is the localization of `M` at `S`. -/
@[to_additive
/-- An alternate form of the congruence relation on `M × S`, `M` a `CommMonoid` and `S` a
submonoid of `M`, whose quotient is the localization of `M` at `S`. -/]
/--
Definition of `r'` / `r'` 的定义

English:
definition r'
  signature: : Con (M × S)
  body: by
  -- note we multiply by `c` on the left so that we can later generalize to `•`
  refine
    { r := fun a b : M × S => exists c : S, ↑c * (↑b.2 * a.1) = c * (a.2 * b.1)
      iseqv := ⟨fun a => ⟨1, rfl⟩, fun ⟨c, hc⟩ => ⟨c, hc.symm⟩, ?_⟩
      mul' := ?_ }
  · rintro a b c ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁ * b.2
    simp only [Submonoid.coe_mul]
    calc
      (t₂ * t₁ * b.2 : M) * (c.2 * a.1) = t₂ * c.2 * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = t₁ * a.2 * (t₂ * (c.2 * b.1)) := by rw [ht₁]; ac_rfl
      _ = t₂ * t₁ * b.2 * (a.2 * c.1) := by rw [ht₂]; ac_rfl
  · rintro a b c d ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁
    calc
      (t₂ * t₁ : M) * (b.2 * d.2 * (a.1 * c.1)) = t₂ * (d.2 * c.1) * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = (t₂ * t₁ : M) * (a.2 * c.2 * (b.1 * d.1)) := by rw [ht₁, ht₂]; ac_rfl

中文:
定义 r'
  签名: : Con (M × S)
  定义体: by
  -- note we multiply by `c` on the left so that we can later generalize to `•`
  refine
    { r := fun a b : M × S => exists c : S, ↑c * (↑b.2 * a.1) = c * (a.2 * b.1)
      iseqv := ⟨fun a => ⟨1, rfl⟩, fun ⟨c, hc⟩ => ⟨c, hc.symm⟩, ?_⟩
      mul' := ?_ }
  · rintro a b c ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁ * b.2
    simp only [Submonoid.coe_mul]
    calc
      (t₂ * t₁ * b.2 : M) * (c.2 * a.1) = t₂ * c.2 * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = t₁ * a.2 * (t₂ * (c.2 * b.1)) := by rw [ht₁]; ac_rfl
      _ = t₂ * t₁ * b.2 * (a.2 * c.1) := by rw [ht₂]; ac_rfl
  · rintro a b c d ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁
    calc
      (t₂ * t₁ : M) * (b.2 * d.2 * (a.1 * c.1)) = t₂ * (d.2 * c.1) * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = (t₂ * t₁ : M) * (a.2 * c.2 * (b.1 * d.1)) := by rw [ht₁, ht₂]; ac_rfl
-/
def r' : Con (M × S) := by
  -- note we multiply by `c` on the left so that we can later generalize to `•`
  refine
    { r := fun a b : M × S => exists c : S, ↑c * (↑b.2 * a.1) = c * (a.2 * b.1)
      iseqv := ⟨fun a => ⟨1, rfl⟩, fun ⟨c, hc⟩ => ⟨c, hc.symm⟩, ?_⟩
      mul' := ?_ }
  · rintro a b c ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁ * b.2
    simp only [Submonoid.coe_mul]
    calc
      (t₂ * t₁ * b.2 : M) * (c.2 * a.1) = t₂ * c.2 * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = t₁ * a.2 * (t₂ * (c.2 * b.1)) := by rw [ht₁]; ac_rfl
      _ = t₂ * t₁ * b.2 * (a.2 * c.1) := by rw [ht₂]; ac_rfl
  · rintro a b c d ⟨t₁, ht₁⟩ ⟨t₂, ht₂⟩
    use t₂ * t₁
    calc
      (t₂ * t₁ : M) * (b.2 * d.2 * (a.1 * c.1)) = t₂ * (d.2 * c.1) * (t₁ * (b.2 * a.1)) := by ac_rfl
      _ = (t₂ * t₁ : M) * (a.2 * c.2 * (b.1 * d.1)) := by rw [ht₁, ht₂]; ac_rfl

/-- The congruence relation used to localize a `CommMonoid` at a submonoid can be expressed
equivalently as an infimum (see `Localization.r`) or explicitly
(see `Localization.r'`). -/
@[to_additive
/-- The additive congruence relation used to localize an `AddCommMonoid` at a submonoid can be
expressed equivalently as an infimum (see `AddLocalization.r`) or explicitly
(see `AddLocalization.r'`). -/]
/--
theorem `r_eq_r'` / 定理 `r_eq_r'`

English:
theorem r_eq_r'
  statement: r S = r' S
  proof: le_antisymm (sInf_le fun _ => ⟨1, by simp⟩)
    le_sInf fun b H ⟨p, q⟩ ⟨x, y⟩ ⟨t, ht⟩ => by
      rw [← one_mul (p]; rw [q)]; rw [← one_mul (x]; rw [y)]
      refine b.trans (b.mul (H (t * y)) (b.refl _)) ?_
      convert! b.symm (b.mul (H (t * q)) (b.refl (x, y))) using 1
      dsimp only [Prod.mk_mul_mk, Submonoid.coe_mul] at ht ⊢
      simp_rw [mul_assoc, ht, mul_comm y q]

中文:
定理 r_eq_r'
  结论: r S = r' S
  证明: le_antisymm (sInf_le fun _ => ⟨1, by simp⟩)
    le_sInf fun b H ⟨p, q⟩ ⟨x, y⟩ ⟨t, ht⟩ => by
      rw [← one_mul (p]; rw [q)]; rw [← one_mul (x]; rw [y)]
      refine b.trans (b.mul (H (t * y)) (b.refl _)) ?_
      convert! b.symm (b.mul (H (t * q)) (b.refl (x, y))) using 1
      dsimp only [Prod.mk_mul_mk, Submonoid.coe_mul] at ht ⊢
      simp_rw [mul_assoc, ht, mul_comm y q]

Depends on / 依赖: Prod.mk_mul_mk, Submonoid, Submonoid.coe_mul, b.mul, b.refl, b.symm, b.trans, coe_mul, convert, le_antisymm, le_sInf, mk_mul_mk, mul_assoc, mul_comm, one_mul, sInf_le, simp_rw
-/
theorem r_eq_r' : r S = r' S :=
le_antisymm (sInf_le fun _ => ⟨1, by simp⟩)
    le_sInf fun b H ⟨p, q⟩ ⟨x, y⟩ ⟨t, ht⟩ => by
      rw [← one_mul (p]; rw [q)]; rw [← one_mul (x]; rw [y)]
      refine b.trans (b.mul (H (t * y)) (b.refl _)) ?_
      convert! b.symm (b.mul (H (t * q)) (b.refl (x, y))) using 1
      dsimp only [Prod.mk_mul_mk, Submonoid.coe_mul] at ht ⊢
      simp_rw [mul_assoc, ht, mul_comm y q]

variable {S}

@[to_additive]
/--
theorem `r_iff_exists` / 定理 `r_iff_exists`

English:
theorem r_iff_exists
  given: {x y : M × S}
  statement: r S x y ↔ exists c : S, ↑c * (↑y.2 * x.1) = c * (x.2 * y.1)
  proof: by
  simp only [r_eq_r' S, r', Con.rel_mk]

@[to_additive]

中文:
定理 r_iff_存在
  条件: {x y : M × S}
  结论: r S x y ↔ 存在 c : S, ↑c * (↑y.2 * x.1) = c * (x.2 * y.1)
  证明: by
  simp only [r_eq_r' S, r', Con.rel_mk]

@[to_additive]

Depends on / 依赖: Con.rel_mk, r_eq_r, rel_mk
-/
theorem r_iff_exists {x y : M × S} : r S x y ↔ exists c : S, ↑c * (↑y.2 * x.1) = c * (x.2 * y.1) := by
  simp only [r_eq_r' S, r', Con.rel_mk]

@[to_additive]
/--
theorem `r_iff_oreEqv_r` / 定理 `r_iff_oreEqv_r`

English:
theorem r_iff_oreEqv_r
  given: {x y : M × S}
  statement: r S x y ↔ (OreLocalization.oreEqv S M).r x y
  proof: by
  simp +instances only [r_iff_exists, Subtype.exists, exists_prop, OreLocalization.oreEqv,
    smul_eq_mul, Submonoid.mk_smul]
  constructor
  · rintro ⟨u, hu, e⟩
    exact ⟨_, mul_mem hu x.2.2, u * y.2, by rw [mul_assoc, mul_assoc, ← e], mul_right_comm _ _ _⟩
  · rintro ⟨u, hu, v, e₁, e₂⟩
    exact ⟨u, hu, by rw [← mul_assoc, e₂, mul_right_comm, ← e₁, mul_assoc, mul_comm y.1]⟩

中文:
定理 r_iff_oreEqv_r
  条件: {x y : M × S}
  结论: r S x y ↔ (OreLocalization.oreEqv S M).r x y
  证明: by
  simp +instances only [r_iff_exists, Subtype.exists, exists_prop, OreLocalization.oreEqv,
    smul_eq_mul, Submonoid.mk_smul]
  constructor
  · rintro ⟨u, hu, e⟩
    exact ⟨_, mul_mem hu x.2.2, u * y.2, by rw [mul_assoc, mul_assoc, ← e], mul_right_comm _ _ _⟩
  · rintro ⟨u, hu, v, e₁, e₂⟩
    exact ⟨u, hu, by rw [← mul_assoc, e₂, mul_right_comm, ← e₁, mul_assoc, mul_comm y.1]⟩

Depends on / 依赖: OreLocalization, OreLocalization.oreEqv, Submonoid, Submonoid.mk_smul, Subtype, Subtype.exists, exists_prop, instances, mk_smul, mul_assoc, mul_comm, mul_mem, mul_right_comm, oreEqv, r_iff_exists, smul_eq_mul
-/
theorem r_iff_oreEqv_r {x y : M × S} : r S x y ↔ (OreLocalization.oreEqv S M).r x y := by
  simp +instances only [r_iff_exists, Subtype.exists, exists_prop, OreLocalization.oreEqv,
    smul_eq_mul, Submonoid.mk_smul]
  constructor
  · rintro ⟨u, hu, e⟩
    exact ⟨_, mul_mem hu x.2.2, u * y.2, by rw [mul_assoc, mul_assoc, ← e], mul_right_comm _ _ _⟩
  · rintro ⟨u, hu, v, e₁, e₂⟩
    exact ⟨u, hu, by rw [← mul_assoc, e₂, mul_right_comm, ← e₁, mul_assoc, mul_comm y.1]⟩

end Localization

set_option linter.translateOverwrite false in
/-- The localization of a `CommMonoid` at one of its submonoids (as a quotient type). -/
@[to_additive AddLocalization
/-- The localization of an `AddCommMonoid` at one of its submonoids (as a quotient type). -/]
/--
Definition of `Localization` / `Localization` 的定义

English:
abbreviation Localization
  body: OreLocalization S M

中文:
缩写 Localization
  定义体: OreLocalization S M

Depends on / 依赖: OreLocalization
-/
abbrev Localization := OreLocalization S M

namespace Localization

variable {S}

/-- Given a `CommMonoid` `M` and submonoid `S`, `mk` sends `x : M`, `y ∈ S` to the equivalence
class of `(x, y)` in the localization of `M` at `S`. -/
@[to_additive
/-- Given an `AddCommMonoid` `M` and submonoid `S`, `mk` sends `x : M`, `y ∈ S` to
the equivalence class of `(x, y)` in the localization of `M` at `S`. -/]
/--
Definition of `mk` / `mk` 的定义

English:
definition mk
  signature: (x : M) (y : S)
  body: x /ₒ y

@[to_additive]

中文:
定义 mk
  签名: (x : M) (y : S)
  定义体: x /ₒ y

@[to_additive]
-/
def mk (x : M) (y : S) : Localization S := x /ₒ y

@[to_additive]
/--
theorem `mk_eq_mk_iff` / 定理 `mk_eq_mk_iff`

English:
theorem mk_eq_mk_iff
  given: {a c : M} {b d : S}
  statement: mk a b = mk c d ↔ r S ⟨a, b⟩ ⟨c, d⟩
  proof: by
  simp +instances only [mk, OreLocalization.oreDiv_eq_iff, r_iff_oreEqv_r, OreLocalization.oreEqv]

universe u

中文:
定理 mk_eq_mk_iff
  条件: {a c : M} {b d : S}
  结论: mk a b = mk c d ↔ r S ⟨a, b⟩ ⟨c, d⟩
  证明: by
  simp +instances only [mk, OreLocalization.oreDiv_eq_iff, r_iff_oreEqv_r, OreLocalization.oreEqv]

universe u

Depends on / 依赖: OreLocalization, OreLocalization.oreDiv_eq_iff, OreLocalization.oreEqv, instances, oreDiv_eq_iff, oreEqv, r_iff_oreEqv_r
-/
theorem mk_eq_mk_iff {a c : M} {b d : S} : mk a b = mk c d ↔ r S ⟨a, b⟩ ⟨c, d⟩ := by
  simp +instances only [mk, OreLocalization.oreDiv_eq_iff, r_iff_oreEqv_r, OreLocalization.oreEqv]

universe u

/-- Dependent recursion principle for `Localizations`: given elements `f a b : p (mk a b)`
for all `a b`, such that `r S (a, b) (c, d)` implies `f a b = f c d` (with the correct coercions),
then `f` is defined on the whole `Localization S`. -/
@[to_additive (attr := elab_as_elim)
/-- Dependent recursion principle for `AddLocalizations`: given elements `f a b : p (mk a b)`
for all `a b`, such that `r S (a, b) (c, d)` implies `f a b = f c d` (with the correct coercions),
then `f` is defined on the whole `AddLocalization S`. -/]
/--
Definition of `rec` / `rec` 的定义

English:
definition rec
  signature: {p : Localization S -> Sort u} (f : forall (a : M) (b : S), p (mk a b))
  body: Quot.rec (fun y => f y.1 y.2)
    (fun y z h => by cases y; cases z; exact H (r_iff_oreEqv_r.mpr h)) x

中文:
定义 rec
  签名: {p : Localization S -> 类型层 u} (f : 对任意 (a : M) (b : S), p (mk a b))
  定义体: Quot.rec (fun y => f y.1 y.2)
    (fun y z h => by cases y; cases z; exact H (r_iff_oreEqv_r.mpr h)) x

Depends on / 依赖: Quot.rec, r_iff_oreEqv_r, r_iff_oreEqv_r.mpr
-/
def rec {p : Localization S -> Sort u} (f : forall (a : M) (b : S), p (mk a b))
    (H : forall {a c : M} {b d : S} (h : r S (a, b) (c, d)),
      (Eq.ndrec (f a b) (mk_eq_mk_iff.mpr h) : p (mk c d)) = f c d) (x) : p x :=
  Quot.rec (fun y => f y.1 y.2)
    (fun y z h => by cases y; cases z; exact H (r_iff_oreEqv_r.mpr h)) x

/-- Copy of `Quotient.recOnSubsingleton₂` for `Localization` -/
@[to_additive (attr := elab_as_elim)
/-- Copy of `Quotient.recOnSubsingleton₂` for `AddLocalization` -/]
/--
Definition of `recOnSubsingleton₂` / `recOnSubsingleton₂` 的定义

English:
definition recOnSubsingleton₂
  signature: {r : Localization S -> Localization S -> Sort u}
  body: @Quotient.recOnSubsingleton₂' _ _ _ _ r (Prod.rec fun _ _ => Prod.rec fun _ _ => h _ _ _ _) x y
    (Prod.rec fun _ _ => Prod.rec fun _ _ => f _ _ _ _)

@[to_additive]

中文:
定义 recOnSubsingleton₂
  签名: {r : Localization S -> Localization S -> 类型层 u}
  定义体: @Quotient.recOnSubsingleton₂' _ _ _ _ r (Prod.rec fun _ _ => Prod.rec fun _ _ => h _ _ _ _) x y
    (Prod.rec fun _ _ => Prod.rec fun _ _ => f _ _ _ _)

@[to_additive]

Depends on / 依赖: Prod.rec, Quotient, Quotient.recOnSubsingleton
-/
def recOnSubsingleton₂ {r : Localization S -> Localization S -> Sort u}
    [h : forall (a c : M) (b d : S), Subsingleton (r (mk a b) (mk c d))] (x y : Localization S)
    (f : forall (a c : M) (b d : S), r (mk a b) (mk c d)) : r x y :=
  @Quotient.recOnSubsingleton₂' _ _ _ _ r (Prod.rec fun _ _ => Prod.rec fun _ _ => h _ _ _ _) x y
    (Prod.rec fun _ _ => Prod.rec fun _ _ => f _ _ _ _)

@[to_additive]
/--
theorem `mk_mul` / 定理 `mk_mul`

English:
theorem mk_mul
  given: (a c : M) (b d : S)
  statement: mk a b * mk c d = mk (a * c) (b * d)
  proof: mul_comm b d ▸ OreLocalization.oreDiv_mul_oreDiv

unseal OreLocalization.one in
@[to_additive]

中文:
定理 mk_mul
  条件: (a c : M) (b d : S)
  结论: mk a b * mk c d = mk (a * c) (b * d)
  证明: mul_comm b d ▸ OreLocalization.oreDiv_mul_oreDiv

unseal OreLocalization.one in
@[to_additive]

Depends on / 依赖: OreLocalization, OreLocalization.oreDiv_mul_oreDiv, mul_comm, oreDiv_mul_oreDiv
-/
theorem mk_mul (a c : M) (b d : S) : mk a b * mk c d = mk (a * c) (b * d) :=
  mul_comm b d ▸ OreLocalization.oreDiv_mul_oreDiv

unseal OreLocalization.one in
@[to_additive]
/--
theorem `mk_one` / 定理 `mk_one`

English:
theorem mk_one
  statement: mk 1 (1 : S) = 1
  proof: OreLocalization.one_def

@[to_additive]

中文:
定理 mk_one
  结论: mk 1 (1 : S) = 1
  证明: OreLocalization.one_def

@[to_additive]

Depends on / 依赖: OreLocalization, OreLocalization.one_def, one_def
-/
theorem mk_one : mk 1 (1 : S) = 1 := OreLocalization.one_def

@[to_additive]
/--
theorem `mk_pow` / 定理 `mk_pow`

English:
theorem mk_pow
  given: (n : Nat) (a : M) (b : S)
  statement: mk a b ^ n = mk (a ^ n) (b ^ n)
  proof: OreLocalization.oreDiv_pow _ _ _ .all _ _

@[to_additive]

中文:
定理 mk_pow
  条件: (n : 自然数) (a : M) (b : S)
  结论: mk a b ^ n = mk (a ^ n) (b ^ n)
  证明: OreLocalization.oreDiv_pow _ _ _ .all _ _

@[to_additive]

Depends on / 依赖: OreLocalization, OreLocalization.oreDiv_pow, oreDiv_pow
-/
theorem mk_pow (n : Nat) (a : M) (b : S) : mk a b ^ n = mk (a ^ n) (b ^ n) :=
OreLocalization.oreDiv_pow _ _ _ .all _ _

@[to_additive]
/--
theorem `mk_prod` / 定理 `mk_prod`

English:
theorem mk_prod
  given: {ι} (t : Finset ι) (f : ι -> M) (s : ι -> S)
  proof: by
  classical
  induction t using Finset.induction_on <;> simp [mk_one, Finset.prod_insert, *, mk_mul]

@[to_additive (attr := simp)]

中文:
定理 mk_prod
  条件: {ι} (t : 有限集 ι) (f : ι -> M) (s : ι -> S)
  证明: by
  classical
  induction t using Finset.induction_on <;> simp [mk_one, Finset.prod_insert, *, mk_mul]

@[to_additive (attr := simp)]

Depends on / 依赖: Finset, Finset.induction_on, Finset.prod_insert, classical, induction_on, mk_mul, mk_one, prod_insert
-/
theorem mk_prod {ι} (t : Finset ι) (f : ι -> M) (s : ι -> S) :
    ∏ i in t, mk (f i) (s i) = mk (∏ i in t, f i) (∏ i in t, s i) := by
  classical
  induction t using Finset.induction_on <;> simp [mk_one, Finset.prod_insert, *, mk_mul]

@[to_additive (attr := simp)]
/--
theorem `ndrec_mk` / 定理 `ndrec_mk`

English:
theorem ndrec_mk
  statement: {p : Localization S -> Sort u} (f : forall (a : M) (b : S), p (mk a b)) (H) (a : M)
  proof: rfl

中文:
定理 ndrec_mk
  结论: {p : Localization S -> 类型层 u} (f : 对任意 (a : M) (b : S), p (mk a b)) (H) (a : M)
  证明: rfl
-/
theorem ndrec_mk {p : Localization S -> Sort u} (f : forall (a : M) (b : S), p (mk a b)) (H) (a : M)
    (b : S) : (rec f H (mk a b) : p (mk a b)) = f a b := rfl

/-- Non-dependent recursion principle for localizations: given elements `f a b : p`
for all `a b`, such that `r S (a, b) (c, d)` implies `f a b = f c d`,
then `f` is defined on the whole `Localization S`. -/
@[to_additive
/-- Non-dependent recursion principle for `AddLocalization`s: given elements `f a b : p`
for all `a b`, such that `r S (a, b) (c, d)` implies `f a b = f c d`,
then `f` is defined on the whole `Localization S`. -/]
/--
Definition of `liftOn` / `liftOn` 的定义

English:
definition liftOn
  signature: {p : Sort u} (x : Localization S) (f : M -> S -> p)
  body: rec f (fun h => (by simpa only [eq_rec_constant] using H h)) x

@[to_additive]

中文:
定义 liftOn
  签名: {p : 类型层 u} (x : Localization S) (f : M -> S -> p)
  定义体: rec f (fun h => (by simpa only [eq_rec_constant] using H h)) x

@[to_additive]

Depends on / 依赖: eq_rec_constant
-/
def liftOn {p : Sort u} (x : Localization S) (f : M -> S -> p)
    (H : forall {a c : M} {b d : S}, r S (a, b) (c, d) -> f a b = f c d) : p :=
  rec f (fun h => (by simpa only [eq_rec_constant] using H h)) x

@[to_additive]
/--
theorem `liftOn_mk` / 定理 `liftOn_mk`

English:
theorem liftOn_mk
  given: {p : Sort u} (f : M -> S -> p) (H) (a : M) (b : S)
  proof: rfl

@[to_additive (attr := elab_as_elim, induction_eliminator, cases_eliminator)]

中文:
定理 liftOn_mk
  条件: {p : 类型层 u} (f : M -> S -> p) (H) (a : M) (b : S)
  证明: rfl

@[to_additive (attr := elab_as_elim, induction_eliminator, cases_eliminator)]
-/
theorem liftOn_mk {p : Sort u} (f : M -> S -> p) (H) (a : M) (b : S) :
    liftOn (mk a b) f H = f a b := rfl

@[to_additive (attr := elab_as_elim, induction_eliminator, cases_eliminator)]
/--
theorem `ind` / 定理 `ind`

English:
theorem ind
  given: {p : Localization S -> Prop} (H : forall y : M × S, p (mk y.1 y.2)) (x)
  statement: p x
  proof: rec (fun a b => H (a, b)) (fun _ => rfl) x

@[to_additive (attr := elab_as_elim)]

中文:
定理 ind
  条件: {p : Localization S -> 命题} (H : 对任意 y : M × S, p (mk y.1 y.2)) (x)
  结论: p x
  证明: rec (fun a b => H (a, b)) (fun _ => rfl) x

@[to_additive (attr := elab_as_elim)]
-/
theorem ind {p : Localization S -> Prop} (H : forall y : M × S, p (mk y.1 y.2)) (x) : p x :=
  rec (fun a b => H (a, b)) (fun _ => rfl) x

@[to_additive (attr := elab_as_elim)]
/--
theorem `induction_on` / 定理 `induction_on`

English:
theorem induction_on
  given: {p : Localization S -> Prop} (x) (H : forall y : M × S, p (mk y.1 y.2))
  statement: p x
  proof: ind H x

中文:
定理 induction_on
  条件: {p : Localization S -> 命题} (x) (H : 对任意 y : M × S, p (mk y.1 y.2))
  结论: p x
  证明: ind H x
-/
theorem induction_on {p : Localization S -> Prop} (x) (H : forall y : M × S, p (mk y.1 y.2)) : p x :=
  ind H x

/-- Non-dependent recursion principle for localizations: given elements `f x y : p`
for all `x` and `y`, such that `r S x x'` and `r S y y'` implies `f x y = f x' y'`,
then `f` is defined on the whole `Localization S`. -/
@[to_additive
/-- Non-dependent recursion principle for localizations: given elements `f x y : p`
for all `x` and `y`, such that `r S x x'` and `r S y y'` implies `f x y = f x' y'`,
then `f` is defined on the whole `Localization S`. -/]
/--
Definition of `liftOn₂` / `liftOn₂` 的定义

English:
definition liftOn₂
  signature: {p : Sort u} (x y : Localization S) (f : M -> S -> M -> S -> p)
  body: liftOn x (fun a b => liftOn y (f a b) fun hy => H ((r S).refl _) hy) fun hx =>
    induction_on y fun ⟨_, _⟩ => H hx ((r S).refl _)

@[to_additive]

中文:
定义 liftOn₂
  签名: {p : 类型层 u} (x y : Localization S) (f : M -> S -> M -> S -> p)
  定义体: liftOn x (fun a b => liftOn y (f a b) fun hy => H ((r S).refl _) hy) fun hx =>
    induction_on y fun ⟨_, _⟩ => H hx ((r S).refl _)

@[to_additive]

Depends on / 依赖: induction_on, liftOn
-/
def liftOn₂ {p : Sort u} (x y : Localization S) (f : M -> S -> M -> S -> p)
    (H : forall {a a' b b' c c' d d'}, r S (a, b) (a', b') -> r S (c, d) (c', d') ->
      f a b c d = f a' b' c' d') : p :=
  liftOn x (fun a b => liftOn y (f a b) fun hy => H ((r S).refl _) hy) fun hx =>
    induction_on y fun ⟨_, _⟩ => H hx ((r S).refl _)

@[to_additive]
/--
theorem `liftOn₂_mk` / 定理 `liftOn₂_mk`

English:
theorem liftOn₂_mk
  given: {p : Sort*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S)
  proof: rfl

@[to_additive (attr := elab_as_elim)]

中文:
定理 liftOn₂_mk
  条件: {p : 类型层*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S)
  证明: rfl

@[to_additive (attr := elab_as_elim)]
-/
theorem liftOn₂_mk {p : Sort*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S) :
    liftOn₂ (mk a b) (mk c d) f H = f a b c d := rfl

@[to_additive (attr := elab_as_elim)]
/--
theorem `induction_on₂` / 定理 `induction_on₂`

English:
theorem induction_on₂
  statement: {p : Localization S -> Localization S -> Prop} (x y)
  proof: induction_on x fun x => induction_on y H x

@[to_additive (attr := elab_as_elim)]

中文:
定理 induction_on₂
  结论: {p : Localization S -> Localization S -> 命题} (x y)
  证明: induction_on x fun x => induction_on y H x

@[to_additive (attr := elab_as_elim)]

Depends on / 依赖: induction_on
-/
theorem induction_on₂ {p : Localization S -> Localization S -> Prop} (x y)
    (H : forall x y : M × S, p (mk x.1 x.2) (mk y.1 y.2)) : p x y :=
induction_on x fun x => induction_on y H x

@[to_additive (attr := elab_as_elim)]
/--
theorem `induction_on₃` / 定理 `induction_on₃`

English:
theorem induction_on₃
  statement: {p : Localization S -> Localization S -> Localization S -> Prop} (x y z)
  proof: induction_on₂ x y fun x y => induction_on z H x y

@[to_additive]

中文:
定理 induction_on₃
  结论: {p : Localization S -> Localization S -> Localization S -> 命题} (x y z)
  证明: induction_on₂ x y fun x y => induction_on z H x y

@[to_additive]

Depends on / 依赖: induction_on
-/
theorem induction_on₃ {p : Localization S -> Localization S -> Localization S -> Prop} (x y z)
    (H : forall x y z : M × S, p (mk x.1 x.2) (mk y.1 y.2) (mk z.1 z.2)) : p x y z :=
induction_on₂ x y fun x y => induction_on z H x y

@[to_additive]
/--
theorem `one_rel` / 定理 `one_rel`

English:
theorem one_rel
  given: (y : S)
  statement: r S 1 (y, y)
  proof: fun _ hb => hb y

@[to_additive]

中文:
定理 one_rel
  条件: (y : S)
  结论: r S 1 (y, y)
  证明: fun _ hb => hb y

@[to_additive]
-/
theorem one_rel (y : S) : r S 1 (y, y) := fun _ hb => hb y

@[to_additive]
/--
theorem `r_of_eq` / 定理 `r_of_eq`

English:
theorem r_of_eq
  given: {x y : M × S} (h : ↑y.2 * x.1 = ↑x.2 * y.1)
  statement: r S x y
  proof: r_iff_exists.2 ⟨1, by rw [h]⟩

@[to_additive]

中文:
定理 r_of_eq
  条件: {x y : M × S} (h : ↑y.2 * x.1 = ↑x.2 * y.1)
  结论: r S x y
  证明: r_iff_exists.2 ⟨1, by rw [h]⟩

@[to_additive]

Depends on / 依赖: r_iff_exists
-/
theorem r_of_eq {x y : M × S} (h : ↑y.2 * x.1 = ↑x.2 * y.1) : r S x y :=
  r_iff_exists.2 ⟨1, by rw [h]⟩

@[to_additive]
/--
theorem `mk_self` / 定理 `mk_self`

English:
theorem mk_self
  given: (a : S)
  statement: mk (a : M) a = 1
  proof: by
  symm
  rw [← mk_one]; rw [mk_eq_mk_iff]
  exact one_rel a

@[to_additive (attr := simp)]

中文:
定理 mk_self
  条件: (a : S)
  结论: mk (a : M) a = 1
  证明: by
  symm
  rw [← mk_one]; rw [mk_eq_mk_iff]
  exact one_rel a

@[to_additive (attr := simp)]

Depends on / 依赖: mk_eq_mk_iff, mk_one, one_rel
-/
theorem mk_self (a : S) : mk (a : M) a = 1 := by
  symm
  rw [← mk_one]; rw [mk_eq_mk_iff]
  exact one_rel a

@[to_additive (attr := simp)]
/--
lemma `mk_self_mk` / 引理 `mk_self_mk`

English:
lemma mk_self_mk
  given: (a : M) (haS : a in S)
  statement: mk a ⟨a, haS⟩ = 1
  proof: mk_self ⟨a, haS⟩

中文:
引理 mk_self_mk
  条件: (a : M) (haS : a in S)
  结论: mk a ⟨a, haS⟩ = 1
  证明: mk_self ⟨a, haS⟩

Depends on / 依赖: mk_self
-/
lemma mk_self_mk (a : M) (haS : a in S) : mk a ⟨a, haS⟩ = 1 :=
  mk_self ⟨a, haS⟩

/-- `Localization.mk` as a monoid hom. -/
@[to_additive (attr := simps) /-- `Localization.mk` as a monoid hom. -/]
/--
Definition of `mkHom` / `mkHom` 的定义

English:
definition mkHom
  signature: : M × S ->* Localization S where
  body: mk x.1 x.2
  map_one' := mk_one
  map_mul' _ _ := (mk_mul ..).symm

@[to_additive]

中文:
定义 mkHom
  签名: : M × S ->* Localization S where
  定义体: mk x.1 x.2
  map_one' := mk_one
  map_mul' _ _ := (mk_mul ..).symm

@[to_additive]
-/
def mkHom : M × S ->* Localization S where
  toFun x := mk x.1 x.2
  map_one' := mk_one
  map_mul' _ _ := (mk_mul ..).symm

@[to_additive]
/--
lemma `mkHom_surjective` / 引理 `mkHom_surjective`

English:
lemma mkHom_surjective
  statement: Surjective (mkHom (S := S))
  proof: by rintro ⟨x, y⟩; exact ⟨⟨x, y⟩, rfl⟩

中文:
引理 mkHom_surjective
  结论: 满射 (mkHom (S := S))
  证明: by rintro ⟨x, y⟩; exact ⟨⟨x, y⟩, rfl⟩
-/
lemma mkHom_surjective : Surjective (mkHom (S := S)) := by rintro ⟨x, y⟩; exact ⟨⟨x, y⟩, rfl⟩

section Scalar

variable {R R₁ R₂ : Type*}

/--
theorem `smul_mk` / 定理 `smul_mk`

English:
theorem smul_mk
  given: [SMul R M] [IsScalarTower R M M] (c : R) (a b)
  proof: by
  rw [mk]; rw [mk]; rw [← OreLocalization.smul_one_oreDiv_one_smul]; rw [OreLocalization.oreDiv_smul_oreDiv]
  change (c • 1) • a /ₒ (b * 1) = _
  rw [smul_assoc]; rw [one_smul]; rw [mul_one]

中文:
定理 smul_mk
  条件: [标量乘法 R M] [标量塔 R M M] (c : R) (a b)
  证明: by
  rw [mk]; rw [mk]; rw [← OreLocalization.smul_one_oreDiv_one_smul]; rw [OreLocalization.oreDiv_smul_oreDiv]
  change (c • 1) • a /ₒ (b * 1) = _
  rw [smul_assoc]; rw [one_smul]; rw [mul_one]

Depends on / 依赖: OreLocalization, OreLocalization.oreDiv_smul_oreDiv, OreLocalization.smul_one_oreDiv_one_smul, mul_one, one_smul, oreDiv_smul_oreDiv, smul_assoc, smul_one_oreDiv_one_smul
-/
theorem smul_mk [SMul R M] [IsScalarTower R M M] (c : R) (a b) :
    c • (mk a b : Localization S) = mk (c • a) b := by
  rw [mk]; rw [mk]; rw [← OreLocalization.smul_one_oreDiv_one_smul]; rw [OreLocalization.oreDiv_smul_oreDiv]
  change (c • 1) • a /ₒ (b * 1) = _
  rw [smul_assoc]; rw [one_smul]; rw [mul_one]

-- Note: Previously there was a `MulDistribMulAction R (Localization S)`.
-- It was removed as it is not the correct action.

end Scalar

end Localization

variable {S N}

namespace MonoidHom

/-- Makes a localization map from a `CommMonoid` hom satisfying the characteristic predicate. -/
@[to_additive /-- Makes a localization map from an `AddCommMonoid` hom satisfying the
characteristic predicate. -/]
/--
Definition of `toLocalizationMap` / `toLocalizationMap` 的定义

English:
definition toLocalizationMap
  signature: (f : M ->* N) (H1 : forall y : S, IsUnit (f y))
  body: f
  isLocalizationMap :=
  { map_units := H1
    surj := H2
    exists_of_eq := H3 _ _ }

中文:
定义 toLocalizationMap
  签名: (f : M ->* N) (H1 : 对任意 y : S, 是单位 (f y))
  定义体: f
  isLocalizationMap :=
  { map_units := H1
    surj := H2
    exists_of_eq := H3 _ _ }
-/
def toLocalizationMap (f : M ->* N) (H1 : forall y : S, IsUnit (f y))
    (H2 : forall z, exists x : M × S, z * f x.2 = f x.1) (H3 : forall x y, f x = f y -> exists c : S, ↑c * x = ↑c * y) :
    Submonoid.LocalizationMap S N where
  __ := f
  isLocalizationMap :=
  { map_units := H1
    surj := H2
    exists_of_eq := H3 _ _ }

end MonoidHom

namespace Submonoid

namespace IsLocalizationMap

/--
theorem `map_one` / 定理 `map_one`

English:
theorem map_one
  statement: {F : Type*} [FunLike F M N] [MulHomClass F M N]
  proof: by
  rw [← (hf.map_units 1).mul_right_inj]; rw [mul_one]
  exact (map_mul ..).symm.trans congr(f $(mul_one _))

中文:
定理 map_one
  结论: {F : 类型} [函数状 F M N] [乘法态射类 F M N]
  证明: by
  rw [← (hf.map_units 1).mul_right_inj]; rw [mul_one]
  exact (map_mul ..).symm.trans congr(f $(mul_one _))
-/
@[to_additive] protected theorem map_one {F : Type*} [FunLike F M N] [MulHomClass F M N]
    {f : F} (hf : IsLocalizationMap S f) : f 1 = 1 := by
  rw [← (hf.map_units 1).mul_right_inj]; rw [mul_one]
  exact (map_mul ..).symm.trans congr(f $(mul_one _))

/--
theorem `mulEquiv_comp` / 定理 `mulEquiv_comp`

English:
theorem mulEquiv_comp
  statement: {f : M -> N} (hf : IsLocalizationMap S f)
  proof: (hf.map_units x).map e
  surj y := let e : N ≃* P := e
    have ⟨x, eq⟩ := hf.surj (e.symm y)
    ⟨x, e.symm.injective (by simpa [e])⟩
  exists_of_eq eq := hf.exists_of_eq (EquivLike.injective e eq)

中文:
定理 mulEquiv_comp
  结论: {f : M -> N} (hf : 是Localization映射 S f)
  证明: (hf.map_units x).map e
  surj y := let e : N ≃* P := e
    have ⟨x, eq⟩ := hf.surj (e.symm y)
    ⟨x, e.symm.injective (by simpa [e])⟩
  exists_of_eq eq := hf.exists_of_eq (EquivLike.injective e eq)
-/
@[to_additive] theorem mulEquiv_comp {f : M -> N} (hf : IsLocalizationMap S f)
    {E} [EquivLike E N P] [MulEquivClass E N P] (e : E) : IsLocalizationMap S (e ∘ f) where
  map_units x := (hf.map_units x).map e
  surj y := let e : N ≃* P := e
    have ⟨x, eq⟩ := hf.surj (e.symm y)
    ⟨x, e.symm.injective (by simpa [e])⟩
  exists_of_eq eq := hf.exists_of_eq (EquivLike.injective e eq)

end IsLocalizationMap

namespace LocalizationMap

/-- A localization map between monoids automatically preserves 1 and therefore
is a monoid homomorphism. -/
@[to_additive /-- A localization map between additive monoids automatically preserves 0 and
therefore is an additive monoid homomorphism. -/]
/--
Definition of `toMonoidHom` / `toMonoidHom` 的定义

English:
abbreviation toMonoidHom
  signature: (f : LocalizationMap S N)
  body: f
  map_one' := f.isLocalizationMap.map_one (f := f.toMulHom)

@[to_additive]

中文:
缩写 toMonoidHom
  签名: (f : Localization映射 S N)
  定义体: f
  map_one' := f.isLocalizationMap.map_one (f := f.toMulHom)

@[to_additive]
-/
abbrev toMonoidHom (f : LocalizationMap S N) : M ->* N where
  __ := f
  map_one' := f.isLocalizationMap.map_one (f := f.toMulHom)

@[to_additive]
/--
theorem `toMonoidHom_injective` / 定理 `toMonoidHom_injective`

English:
theorem toMonoidHom_injective
  statement: Injective (toMonoidHom : LocalizationMap S N -> M ->* N)
  proof: fun f g => by cases f; congr! with eq; ext; exact congr($eq _)

中文:
定理 toMonoidHom_injective
  结论: 单射 (toMonoidHom : Localization映射 S N -> M ->* N)
  证明: fun f g => by cases f; congr! with eq; ext; exact congr($eq _)
-/
theorem toMonoidHom_injective : Injective (toMonoidHom : LocalizationMap S N -> M ->* N) :=
  fun f g => by cases f; congr! with eq; ext; exact congr($eq _)

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: FunLike (LocalizationMap S N) M N
  body: f.toMonoidHom
  coe_injective := DFunLike.coe_injective.comp toMonoidHom_injective

中文:
实例 :
  签名: 函数状 (Localization映射 S N) M N
  定义体: f.toMonoidHom
  coe_injective := DFunLike.coe_injective.comp toMonoidHom_injective
-/
@[to_additive] instance : FunLike (LocalizationMap S N) M N where
  coe f := f.toMonoidHom
  coe_injective := DFunLike.coe_injective.comp toMonoidHom_injective

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: MonoidHomClass (LocalizationMap S N) M N
  body: f.toMonoidHom.map_one
  map_mul f := f.map_mul

中文:
实例 :
  签名: 幺半群态射类 (Localization映射 S N) M N
  定义体: f.toMonoidHom.map_one
  map_mul f := f.map_mul
-/
@[to_additive] instance : MonoidHomClass (LocalizationMap S N) M N where
  map_one f := f.toMonoidHom.map_one
  map_mul f := f.map_mul

/--
lemma `toMonoidHom_apply` / 引理 `toMonoidHom_apply`

English:
lemma toMonoidHom_apply
  given: (f : LocalizationMap S N) (x : M)
  proof: rfl

@[to_additive (attr := ext)]

中文:
引理 toMonoidHom_apply
  条件: (f : Localization映射 S N) (x : M)
  证明: rfl

@[to_additive (attr := ext)]
-/
@[to_additive (attr := simp)] lemma toMonoidHom_apply (f : LocalizationMap S N) (x : M) :
    f.toMonoidHom x = f x := rfl

@[to_additive (attr := ext)]
/--
theorem `ext` / 定理 `ext`

English:
theorem ext
  given: {f g : LocalizationMap S N} (h : forall x, f x = g x)
  statement: f = g
  proof: DFunLike.ext _ _ h

@[to_additive]

中文:
定理 ext
  条件: {f g : Localization映射 S N} (h : 对任意 x, f x = g x)
  结论: f = g
  证明: DFunLike.ext _ _ h

@[to_additive]

Depends on / 依赖: DFunLike, DFunLike.ext
-/
theorem ext {f g : LocalizationMap S N} (h : forall x, f x = g x) : f = g := DFunLike.ext _ _ h

@[to_additive]
/--
theorem `map_units` / 定理 `map_units`

English:
theorem map_units
  given: (f : LocalizationMap S N) (y : S)
  statement: IsUnit (f y)
  proof: f.2.1 y

@[to_additive]

中文:
定理 map_units
  条件: (f : Localization映射 S N) (y : S)
  结论: 是单位 (f y)
  证明: f.2.1 y

@[to_additive]
-/
theorem map_units (f : LocalizationMap S N) (y : S) : IsUnit (f y) :=
  f.2.1 y

@[to_additive]
/--
theorem `surj` / 定理 `surj`

English:
theorem surj
  given: (f : LocalizationMap S N) (z : N)
  statement: exists x : M × S, z * f x.2 = f x.1
  proof: f.2.2 z

@[to_additive]

中文:
定理 surj
  条件: (f : Localization映射 S N) (z : N)
  结论: 存在 x : M × S, z * f x.2 = f x.1
  证明: f.2.2 z

@[to_additive]
-/
theorem surj (f : LocalizationMap S N) (z : N) : exists x : M × S, z * f x.2 = f x.1 :=
  f.2.2 z

@[to_additive]
/--
theorem `exists_of_eq` / 定理 `exists_of_eq`

English:
theorem exists_of_eq
  given: (f : LocalizationMap S N) {x y : M}
  statement: f x = f y -> exists c : S, c * x = c * y
  proof: f.2.3

中文:
定理 存在_of_eq
  条件: (f : Localization映射 S N) {x y : M}
  结论: f x = f y -> 存在 c : S, c * x = c * y
  证明: f.2.3
-/
theorem exists_of_eq (f : LocalizationMap S N) {x y : M} : f x = f y -> exists c : S, c * x = c * y :=
  f.2.3

/-- Given a localization map `f : M →* N`, and `z w : N`, there exist `z' w' : M` and `d : S`
such that `f z' / f d = z` and `f w' / f d = w`. -/
@[to_additive
/-- Given a localization map `f : M →+ N`, and `z w : N`, there exist `z' w' : M` and `d : S`
such that `f z' - f d = z` and `f w' - f d = w`. -/]
/--
theorem `surj₂` / 定理 `surj₂`

English:
theorem surj₂
  given: (f : LocalizationMap S N) (z w : N)
  statement: exists z' w' : M, exists d : S,
  proof: by
  let ⟨a, ha⟩ := surj f z
  let ⟨b, hb⟩ := surj f w
  refine ⟨a.1 * b.2, a.2 * b.1, a.2 * b.2, ?_, ?_⟩
  · simp_rw [mul_def, map_mul, ← ha]
    exact (mul_assoc z _ _).symm
  · simp_rw [mul_def, map_mul, ← hb]
    exact mul_left_comm w _ _

@[to_additive]

中文:
定理 surj₂
  条件: (f : Localization映射 S N) (z w : N)
  结论: 存在 z' w' : M, 存在 d : S,
  证明: by
  let ⟨a, ha⟩ := surj f z
  let ⟨b, hb⟩ := surj f w
  refine ⟨a.1 * b.2, a.2 * b.1, a.2 * b.2, ?_, ?_⟩
  · simp_rw [mul_def, map_mul, ← ha]
    exact (mul_assoc z _ _).symm
  · simp_rw [mul_def, map_mul, ← hb]
    exact mul_left_comm w _ _

@[to_additive]

Depends on / 依赖: map_mul, mul_assoc, mul_def, mul_left_comm, simp_rw
-/
theorem surj₂ (f : LocalizationMap S N) (z w : N) : exists z' w' : M, exists d : S,
    (z * f d = f z') ∧ (w * f d = f w') := by
  let ⟨a, ha⟩ := surj f z
  let ⟨b, hb⟩ := surj f w
  refine ⟨a.1 * b.2, a.2 * b.1, a.2 * b.2, ?_, ?_⟩
  · simp_rw [mul_def, map_mul, ← ha]
    exact (mul_assoc z _ _).symm
  · simp_rw [mul_def, map_mul, ← hb]
    exact mul_left_comm w _ _

@[to_additive]
/--
theorem `eq_iff_exists` / 定理 `eq_iff_exists`

English:
theorem eq_iff_exists
  given: (f : LocalizationMap S N) {x y}
  proof: Iff.intro f.2.3
  fun ⟨c, h⟩ => by
    replace h := congr_arg f h
    rw [map_mul]; rw [map_mul] at h
    exact (f.map_units c).mul_right_inj.mp h

中文:
定理 eq_iff_存在
  条件: (f : Localization映射 S N) {x y}
  证明: Iff.intro f.2.3
  fun ⟨c, h⟩ => by
    replace h := congr_arg f h
    rw [map_mul]; rw [map_mul] at h
    exact (f.map_units c).mul_right_inj.mp h

Depends on / 依赖: Iff.intro
-/
theorem eq_iff_exists (f : LocalizationMap S N) {x y} :
    f x = f y ↔ exists c : S, c * x = c * y := Iff.intro f.2.3
  fun ⟨c, h⟩ => by
    replace h := congr_arg f h
    rw [map_mul]; rw [map_mul] at h
    exact (f.map_units c).mul_right_inj.mp h

/-- Given a localization map `f : M →* N`, a section function sending `z : N` to some
`(x, y) : M × S` such that `f x * (f y)⁻¹ = z`. -/
@[to_additive
/-- Given a localization map `f : M →+ N`, a section function sending `z : N`
to some `(x, y) : M × S` such that `f x - f y = z`. -/]
/--
Definition of `sec` / `sec` 的定义

English:
definition sec
  signature: (f : LocalizationMap S N) (z : N)
  body: Classical.choose f.surj z

@[to_additive]

中文:
定义 sec
  签名: (f : Localization映射 S N) (z : N)
  定义体: Classical.choose f.surj z

@[to_additive]

Depends on / 依赖: Classical, Classical.choose, f.surj
-/
noncomputable def sec (f : LocalizationMap S N) (z : N) : M × S := Classical.choose f.surj z

@[to_additive]
/--
theorem `sec_spec` / 定理 `sec_spec`

English:
theorem sec_spec
  given: {f : LocalizationMap S N} (z : N)
  proof: Classical.choose_spec f.surj z

@[to_additive]

中文:
定理 sec_spec
  条件: {f : Localization映射 S N} (z : N)
  证明: Classical.choose_spec f.surj z

@[to_additive]

Depends on / 依赖: Classical, Classical.choose_spec, choose_spec, f.surj
-/
theorem sec_spec {f : LocalizationMap S N} (z : N) :
z * f (f.sec z).2 = f (f.sec z).1 := Classical.choose_spec f.surj z

@[to_additive]
/--
theorem `sec_spec'` / 定理 `sec_spec'`

English:
theorem sec_spec'
  given: {f : LocalizationMap S N} (z : N)
  proof: by rw [mul_comm, sec_spec]

中文:
定理 sec_spec'
  条件: {f : Localization映射 S N} (z : N)
  证明: by rw [mul_comm, sec_spec]

Depends on / 依赖: mul_comm, sec_spec
-/
theorem sec_spec' {f : LocalizationMap S N} (z : N) :
    f (f.sec z).1 = f (f.sec z).2 * z := by rw [mul_comm, sec_spec]

/-- Given a MonoidHom `f : M →* N` and Submonoid `S ⊆ M` such that `f(S) ⊆ Nˣ`, for all
`w, z : N` and `y ∈ S`, we have `w * (f y)⁻¹ = z ↔ w = f y * z`. -/
@[to_additive
/-- Given an AddMonoidHom `f : M →+ N` and Submonoid `S ⊆ M` such that `f(S) ⊆ AddUnits N`, for all
`w, z : N` and `y ∈ S`, we have `w - f y = z ↔ w = f y + z`. -/]
/--
theorem `mul_inv_left` / 定理 `mul_inv_left`

English:
theorem mul_inv_left
  given: {f : M ->* N} (h : forall y : S, IsUnit (f y)) (y : S) (w z : N)
  proof: by
  rw [mul_comm]
  exact Units.inv_mul_eq_iff_eq_mul (IsUnit.liftRight (f.domRestrict S) h y)

中文:
定理 mul_inv_left
  条件: {f : M ->* N} (h : 对任意 y : S, 是单位 (f y)) (y : S) (w z : N)
  证明: by
  rw [mul_comm]
  exact Units.inv_mul_eq_iff_eq_mul (IsUnit.liftRight (f.domRestrict S) h y)

Depends on / 依赖: IsUnit, IsUnit.liftRight, Units.inv_mul_eq_iff_eq_mul, domRestrict, f.domRestrict, inv_mul_eq_iff_eq_mul, liftRight, mul_comm
-/
theorem mul_inv_left {f : M ->* N} (h : forall y : S, IsUnit (f y)) (y : S) (w z : N) :
    w * (IsUnit.liftRight (f.domRestrict S) h y)⁻¹ = z ↔ w = f y * z := by
  rw [mul_comm]
  exact Units.inv_mul_eq_iff_eq_mul (IsUnit.liftRight (f.domRestrict S) h y)

/-- Given a MonoidHom `f : M →* N` and Submonoid `S ⊆ M` such that `f(S) ⊆ Nˣ`, for all
`w, z : N` and `y ∈ S`, we have `z = w * (f y)⁻¹ ↔ z * f y = w`. -/
@[to_additive
/-- Given an AddMonoidHom `f : M →+ N` and Submonoid `S ⊆ M` such that `f(S) ⊆ AddUnits N`, for all
`w, z : N` and `y ∈ S`, we have `z = w - f y ↔ z + f y = w`. -/]
/--
theorem `mul_inv_right` / 定理 `mul_inv_right`

English:
theorem mul_inv_right
  given: {f : M ->* N} (h : forall y : S, IsUnit (f y)) (y : S) (w z : N)
  proof: by
  rw [eq_comm]; rw [mul_inv_left h]; rw [mul_comm]; rw [eq_comm]

中文:
定理 mul_inv_right
  条件: {f : M ->* N} (h : 对任意 y : S, 是单位 (f y)) (y : S) (w z : N)
  证明: by
  rw [eq_comm]; rw [mul_inv_left h]; rw [mul_comm]; rw [eq_comm]

Depends on / 依赖: eq_comm, mul_comm, mul_inv_left
-/
theorem mul_inv_right {f : M ->* N} (h : forall y : S, IsUnit (f y)) (y : S) (w z : N) :
    z = w * (IsUnit.liftRight (f.domRestrict S) h y)⁻¹ ↔ z * f y = w := by
  rw [eq_comm]; rw [mul_inv_left h]; rw [mul_comm]; rw [eq_comm]

/-- Given a MonoidHom `f : M →* N` and Submonoid `S ⊆ M` such that
`f(S) ⊆ Nˣ`, for all `x₁ x₂ : M` and `y₁, y₂ ∈ S`, we have
`f x₁ * (f y₁)⁻¹ = f x₂ * (f y₂)⁻¹ ↔ f (x₁ * y₂) = f (x₂ * y₁)`. -/
@[to_additive (attr := simp)
/-- Given an AddMonoidHom `f : M →+ N` and Submonoid `S ⊆ M` such that
`f(S) ⊆ AddUnits N`, for all `x₁ x₂ : M` and `y₁, y₂ ∈ S`, we have
`f x₁ - f y₁ = f x₂ - f y₂ ↔ f (x₁ + y₂) = f (x₂ + y₁)`. -/]
/--
theorem `mul_inv` / 定理 `mul_inv`

English:
theorem mul_inv
  given: {f : M ->* N} (h : forall y : S, IsUnit (f y)) {x₁ x₂} {y₁ y₂ : S}
  proof: by
  rw [mul_inv_right h]; rw [mul_assoc]; rw [mul_comm _ (f y₂)]; rw [← mul_assoc]; rw [mul_inv_left h]; rw [mul_comm x₂]; rw [f.map_mul]; rw [f.map_mul]

中文:
定理 mul_inv
  条件: {f : M ->* N} (h : 对任意 y : S, 是单位 (f y)) {x₁ x₂} {y₁ y₂ : S}
  证明: by
  rw [mul_inv_right h]; rw [mul_assoc]; rw [mul_comm _ (f y₂)]; rw [← mul_assoc]; rw [mul_inv_left h]; rw [mul_comm x₂]; rw [f.map_mul]; rw [f.map_mul]

Depends on / 依赖: f.map_mul, map_mul, mul_assoc, mul_comm, mul_inv_left, mul_inv_right
-/
theorem mul_inv {f : M ->* N} (h : forall y : S, IsUnit (f y)) {x₁ x₂} {y₁ y₂ : S} :
    f x₁ * (IsUnit.liftRight (f.domRestrict S) h y₁)⁻¹ =
        f x₂ * (IsUnit.liftRight (f.domRestrict S) h y₂)⁻¹ ↔
      f (x₁ * y₂) = f (x₂ * y₁) := by
  rw [mul_inv_right h]; rw [mul_assoc]; rw [mul_comm _ (f y₂)]; rw [← mul_assoc]; rw [mul_inv_left h]; rw [mul_comm x₂]; rw [f.map_mul]; rw [f.map_mul]

/-- Given a MonoidHom `f : M →* N` and Submonoid `S ⊆ M` such that `f(S) ⊆ Nˣ`, for all
`y, z ∈ S`, we have `(f y)⁻¹ = (f z)⁻¹ → f y = f z`. -/
@[to_additive
/-- Given an AddMonoidHom `f : M →+ N` and Submonoid `S ⊆ M` such that
`f(S) ⊆ AddUnits N`, for all `y, z ∈ S`, we have `- (f y) = - (f z) → f y = f z`. -/]
/--
theorem `inv_inj` / 定理 `inv_inj`

English:
theorem inv_inj
  statement: {f : M ->* N} (hf : forall y : S, IsUnit (f y)) {y z : S}
  proof: by
  rw [← mul_one (f y)]; rw [eq_comm]; rw [← mul_inv_left hf y (f z) 1]; rw [h]
  exact Units.inv_mul (IsUnit.liftRight (f.domRestrict S) hf z)⁻¹

中文:
定理 inv_inj
  结论: {f : M ->* N} (hf : 对任意 y : S, 是单位 (f y)) {y z : S}
  证明: by
  rw [← mul_one (f y)]; rw [eq_comm]; rw [← mul_inv_left hf y (f z) 1]; rw [h]
  exact Units.inv_mul (IsUnit.liftRight (f.domRestrict S) hf z)⁻¹

Depends on / 依赖: IsUnit, IsUnit.liftRight, Units.inv_mul, domRestrict, eq_comm, f.domRestrict, inv_mul, liftRight, mul_inv_left, mul_one
-/
theorem inv_inj {f : M ->* N} (hf : forall y : S, IsUnit (f y)) {y z : S}
    (h : (IsUnit.liftRight (f.domRestrict S) hf y)⁻¹ =
      (IsUnit.liftRight (f.domRestrict S) hf z)⁻¹) : f y = f z := by
  rw [← mul_one (f y)]; rw [eq_comm]; rw [← mul_inv_left hf y (f z) 1]; rw [h]
  exact Units.inv_mul (IsUnit.liftRight (f.domRestrict S) hf z)⁻¹

/-- Given a MonoidHom `f : M →* N` and Submonoid `S ⊆ M` such that `f(S) ⊆ Nˣ`, for all
`y ∈ S`, `(f y)⁻¹` is unique. -/
@[to_additive
/-- Given an AddMonoidHom `f : M →+ N` and Submonoid `S ⊆ M` such that
`f(S) ⊆ AddUnits N`, for all `y ∈ S`, `- (f y)` is unique. -/]
/--
theorem `inv_unique` / 定理 `inv_unique`

English:
theorem inv_unique
  given: {f : M ->* N} (h : forall y : S, IsUnit (f y)) {y : S} {z : N} (H : f y * z = 1)
  proof: by
  rw [← one_mul _⁻¹]; rw [Units.val_mul]; rw [mul_inv_left]
  exact H.symm

中文:
定理 inv_unique
  条件: {f : M ->* N} (h : 对任意 y : S, 是单位 (f y)) {y : S} {z : N} (H : f y * z = 1)
  证明: by
  rw [← one_mul _⁻¹]; rw [Units.val_mul]; rw [mul_inv_left]
  exact H.symm

Depends on / 依赖: H.symm, Units.val_mul, mul_inv_left, one_mul, val_mul
-/
theorem inv_unique {f : M ->* N} (h : forall y : S, IsUnit (f y)) {y : S} {z : N} (H : f y * z = 1) :
    (IsUnit.liftRight (f.domRestrict S) h y)⁻¹ = z := by
  rw [← one_mul _⁻¹]; rw [Units.val_mul]; rw [mul_inv_left]
  exact H.symm

variable (f : LocalizationMap S N)

@[to_additive]
/--
theorem `map_right_cancel` / 定理 `map_right_cancel`

English:
theorem map_right_cancel
  given: {x y} {c : S} (h : f (c * x) = f (c * y))
  proof: by
  rw [map_mul]; rw [map_mul] at h
  let ⟨u, hu⟩ := f.map_units c
  rw [← hu] at h
  exact (Units.mul_right_inj u).1 h

@[to_additive]

中文:
定理 map_right_cancel
  条件: {x y} {c : S} (h : f (c * x) = f (c * y))
  证明: by
  rw [map_mul]; rw [map_mul] at h
  let ⟨u, hu⟩ := f.map_units c
  rw [← hu] at h
  exact (Units.mul_right_inj u).1 h

@[to_additive]

Depends on / 依赖: Units.mul_right_inj, f.map_units, map_mul, map_units, mul_right_inj
-/
theorem map_right_cancel {x y} {c : S} (h : f (c * x) = f (c * y)) :
    f x = f y := by
  rw [map_mul]; rw [map_mul] at h
  let ⟨u, hu⟩ := f.map_units c
  rw [← hu] at h
  exact (Units.mul_right_inj u).1 h

@[to_additive]
/--
theorem `map_left_cancel` / 定理 `map_left_cancel`

English:
theorem map_left_cancel
  given: {x y} {c : S} (h : f (x * c) = f (y * c))
  proof: f.map_right_cancel (c := c) by rw [mul_comm _ x, mul_comm _ y, h]

中文:
定理 map_left_cancel
  条件: {x y} {c : S} (h : f (x * c) = f (y * c))
  证明: f.map_right_cancel (c := c) by rw [mul_comm _ x, mul_comm _ y, h]

Depends on / 依赖: f.map_right_cancel, map_right_cancel, mul_comm
-/
theorem map_left_cancel {x y} {c : S} (h : f (x * c) = f (y * c)) :
    f x = f y :=
f.map_right_cancel (c := c) by rw [mul_comm _ x, mul_comm _ y, h]

/-- Given a localization map `f : M →* N`, the surjection sending `(x, y) : M × S` to
`f x * (f y)⁻¹`. -/
@[to_additive
/-- Given a localization map `f : M →+ N`, the surjection sending `(x, y) : M × S` to
`f x - f y`. -/]
/--
Definition of `mk'` / `mk'` 的定义

English:
definition mk'
  signature: (f : LocalizationMap S N) (x : M) (y : S)
  body: f x * ↑(IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y)⁻¹

@[to_additive]

中文:
定义 mk'
  签名: (f : Localization映射 S N) (x : M) (y : S)
  定义体: f x * ↑(IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y)⁻¹

@[to_additive]

Depends on / 依赖: IsUnit, IsUnit.liftRight, domRestrict, f.map_units, f.toMonoidHom.domRestrict, liftRight, map_units, toMonoidHom
-/
noncomputable def mk' (f : LocalizationMap S N) (x : M) (y : S) : N :=
  f x * ↑(IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y)⁻¹

@[to_additive]
/--
lemma `mk'_mul` / 引理 `mk'_mul`

English:
lemma mk'_mul
  given: (x₁ x₂ : M) (y₁ y₂ : S)
  statement: f.mk' (x₁ * x₂) (y₁ * y₂) = f.mk' x₁ y₁ * f.mk' x₂ y₂
  proof: by
  refine (mul_inv_left f.map_units _ _ _).2 ?_
  simp only [map_mul, coe_mul, toMonoidHom_apply, mk', IsUnit.liftRight, Units.liftRight,
    MonoidHom.domRestrict_apply, MonoidHom.coe_mk, OneHom.coe_mk]
  rw [mul_mul_mul_comm (f x₁)]; rw [mul_left_comm]; rw [mul_mul_mul_comm (f y₁)]
  simp

@[to_additive]

中文:
引理 mk'_mul
  条件: (x₁ x₂ : M) (y₁ y₂ : S)
  结论: f.mk' (x₁ * x₂) (y₁ * y₂) = f.mk' x₁ y₁ * f.mk' x₂ y₂
  证明: by
  refine (mul_inv_left f.map_units _ _ _).2 ?_
  simp only [map_mul, coe_mul, toMonoidHom_apply, mk', IsUnit.liftRight, Units.liftRight,
    MonoidHom.domRestrict_apply, MonoidHom.coe_mk, OneHom.coe_mk]
  rw [mul_mul_mul_comm (f x₁)]; rw [mul_left_comm]; rw [mul_mul_mul_comm (f y₁)]
  simp

@[to_additive]
-/
lemma mk'_mul (x₁ x₂ : M) (y₁ y₂ : S) : f.mk' (x₁ * x₂) (y₁ * y₂) = f.mk' x₁ y₁ * f.mk' x₂ y₂ := by
  refine (mul_inv_left f.map_units _ _ _).2 ?_
  simp only [map_mul, coe_mul, toMonoidHom_apply, mk', IsUnit.liftRight, Units.liftRight,
    MonoidHom.domRestrict_apply, MonoidHom.coe_mk, OneHom.coe_mk]
  rw [mul_mul_mul_comm (f x₁)]; rw [mul_left_comm]; rw [mul_mul_mul_comm (f y₁)]
  simp

@[to_additive]
/--
theorem `mk'_one` / 定理 `mk'_one`

English:
theorem mk'_one
  given: (x)
  statement: f.mk' x (1 : S) = f x
  proof: by
  rw [mk']; rw [map_one]
  exact mul_one _

中文:
定理 mk'_one
  条件: (x)
  结论: f.mk' x (1 : S) = f x
  证明: by
  rw [mk']; rw [map_one]
  exact mul_one _
-/
theorem mk'_one (x) : f.mk' x (1 : S) = f x := by
  rw [mk']; rw [map_one]
  exact mul_one _

/-- Given a localization map `f : M →* N` for a submonoid `S ⊆ M`, for all `z : N` we have that if
`x : M, y ∈ S` are such that `z * f y = f x`, then `f x * (f y)⁻¹ = z`. -/
@[to_additive (attr := simp)
/-- Given a localization map `f : M →+ N` for an AddSubmonoid `S ⊆ M`, for all `z : N`
we have that if `x : M, y ∈ S` are such that `z + f y = f x`, then `f x - f y = z`. -/]
/--
theorem `mk'_sec` / 定理 `mk'_sec`

English:
theorem mk'_sec
  given: (z : N)
  statement: f.mk' (f.sec z).1 (f.sec z).2 = z
  proof: show _ * _ = _ by rw [← sec_spec, mul_inv_left, mul_comm]; dsimp

@[to_additive]

中文:
定理 mk'_sec
  条件: (z : N)
  结论: f.mk' (f.sec z).1 (f.sec z).2 = z
  证明: show _ * _ = _ by rw [← sec_spec, mul_inv_left, mul_comm]; dsimp

@[to_additive]
-/
theorem mk'_sec (z : N) : f.mk' (f.sec z).1 (f.sec z).2 = z :=
  show _ * _ = _ by rw [← sec_spec, mul_inv_left, mul_comm]; dsimp

@[to_additive]
/--
theorem `mk'_surjective` / 定理 `mk'_surjective`

English:
theorem mk'_surjective
  given: (z : N)
  statement: exists (x : _) (y : S), f.mk' x y = z
  proof: ⟨(f.sec z).1, (f.sec z).2, f.mk'_sec z⟩

@[to_additive]

中文:
定理 mk'_surjective
  条件: (z : N)
  结论: 存在 (x : _) (y : S), f.mk' x y = z
  证明: ⟨(f.sec z).1, (f.sec z).2, f.mk'_sec z⟩

@[to_additive]
-/
theorem mk'_surjective (z : N) : exists (x : _) (y : S), f.mk' x y = z :=
  ⟨(f.sec z).1, (f.sec z).2, f.mk'_sec z⟩

@[to_additive]
/--
theorem `mk'_spec` / 定理 `mk'_spec`

English:
theorem mk'_spec
  given: (x) (y : S)
  statement: f.mk' x y * f y = f x
  proof: show _ * _ * _ = _ by rw [mul_assoc, mul_comm _ (f y), ← mul_assoc, mul_inv_left, mul_comm]; dsimp

@[to_additive]

中文:
定理 mk'_spec
  条件: (x) (y : S)
  结论: f.mk' x y * f y = f x
  证明: show _ * _ * _ = _ by rw [mul_assoc, mul_comm _ (f y), ← mul_assoc, mul_inv_left, mul_comm]; dsimp

@[to_additive]
-/
theorem mk'_spec (x) (y : S) : f.mk' x y * f y = f x :=
  show _ * _ * _ = _ by rw [mul_assoc, mul_comm _ (f y), ← mul_assoc, mul_inv_left, mul_comm]; dsimp

@[to_additive]
/--
theorem `mk'_spec'` / 定理 `mk'_spec'`

English:
theorem mk'_spec'
  given: (x) (y : S)
  statement: f y * f.mk' x y = f x
  proof: by rw [mul_comm, mk'_spec]

@[to_additive]

中文:
定理 mk'_spec'
  条件: (x) (y : S)
  结论: f y * f.mk' x y = f x
  证明: by rw [mul_comm, mk'_spec]

@[to_additive]
-/
theorem mk'_spec' (x) (y : S) : f y * f.mk' x y = f x := by rw [mul_comm, mk'_spec]

@[to_additive]
/--
theorem `eq_mk'_iff_mul_eq` / 定理 `eq_mk'_iff_mul_eq`

English:
theorem eq_mk'_iff_mul_eq
  given: {x} {y : S} {z}
  statement: z = f.mk' x y ↔ z * f y = f x
  proof: ⟨fun H => by rw [H, mk'_spec], fun H => by rwa [mk', mul_inv_right]⟩

@[to_additive]

中文:
定理 eq_mk'_iff_mul_eq
  条件: {x} {y : S} {z}
  结论: z = f.mk' x y ↔ z * f y = f x
  证明: ⟨fun H => by rw [H, mk'_spec], fun H => by rwa [mk', mul_inv_right]⟩

@[to_additive]

Depends on / 依赖: _spec, mul_inv_right
-/
theorem eq_mk'_iff_mul_eq {x} {y : S} {z} : z = f.mk' x y ↔ z * f y = f x :=
  ⟨fun H => by rw [H, mk'_spec], fun H => by rwa [mk', mul_inv_right]⟩

@[to_additive]
/--
theorem `mk'_eq_iff_eq_mul` / 定理 `mk'_eq_iff_eq_mul`

English:
theorem mk'_eq_iff_eq_mul
  given: {x} {y : S} {z}
  statement: f.mk' x y = z ↔ f x = z * f y
  proof: by
  rw [eq_comm]; rw [eq_mk'_iff_mul_eq]; rw [eq_comm]

@[to_additive]

中文:
定理 mk'_eq_iff_eq_mul
  条件: {x} {y : S} {z}
  结论: f.mk' x y = z ↔ f x = z * f y
  证明: by
  rw [eq_comm]; rw [eq_mk'_iff_mul_eq]; rw [eq_comm]

@[to_additive]
-/
theorem mk'_eq_iff_eq_mul {x} {y : S} {z} : f.mk' x y = z ↔ f x = z * f y := by
  rw [eq_comm]; rw [eq_mk'_iff_mul_eq]; rw [eq_comm]

@[to_additive]
/--
theorem `mk'_eq_iff_eq` / 定理 `mk'_eq_iff_eq`

English:
theorem mk'_eq_iff_eq
  given: {x₁ x₂} {y₁ y₂ : S}
  proof: by
    rw [map_mul f]; rw [map_mul f]; rw [f.mk'_eq_iff_eq_mul.1 H]; rw [← mul_assoc]; rw [mk'_spec']; rw [mul_comm (f x₂)]
  mpr H := by
    rw [mk'_eq_iff_eq_mul]; rw [mk']; rw [mul_assoc]; rw [mul_comm _ (f y₁)]; rw [← mul_assoc]; rw [← map_mul f]; rw [mul_comm x₂]; rw [← H]; rw [← mul_comm x₁]; rw [map_mul f]; rw [mul_inv_right f.map_units]; rw [toMonoidHom_apply]

@[to_additive]

中文:
定理 mk'_eq_iff_eq
  条件: {x₁ x₂} {y₁ y₂ : S}
  证明: by
    rw [map_mul f]; rw [map_mul f]; rw [f.mk'_eq_iff_eq_mul.1 H]; rw [← mul_assoc]; rw [mk'_spec']; rw [mul_comm (f x₂)]
  mpr H := by
    rw [mk'_eq_iff_eq_mul]; rw [mk']; rw [mul_assoc]; rw [mul_comm _ (f y₁)]; rw [← mul_assoc]; rw [← map_mul f]; rw [mul_comm x₂]; rw [← H]; rw [← mul_comm x₁]; rw [map_mul f]; rw [mul_inv_right f.map_units]; rw [toMonoidHom_apply]

@[to_additive]
-/
theorem mk'_eq_iff_eq {x₁ x₂} {y₁ y₂ : S} :
    f.mk' x₁ y₁ = f.mk' x₂ y₂ ↔ f (y₂ * x₁) = f (y₁ * x₂) where
  mp H := by
    rw [map_mul f]; rw [map_mul f]; rw [f.mk'_eq_iff_eq_mul.1 H]; rw [← mul_assoc]; rw [mk'_spec']; rw [mul_comm (f x₂)]
  mpr H := by
    rw [mk'_eq_iff_eq_mul]; rw [mk']; rw [mul_assoc]; rw [mul_comm _ (f y₁)]; rw [← mul_assoc]; rw [← map_mul f]; rw [mul_comm x₂]; rw [← H]; rw [← mul_comm x₁]; rw [map_mul f]; rw [mul_inv_right f.map_units]; rw [toMonoidHom_apply]

@[to_additive]
/--
theorem `mk'_eq_iff_eq'` / 定理 `mk'_eq_iff_eq'`

English:
theorem mk'_eq_iff_eq'
  given: {x₁ x₂} {y₁ y₂ : S}
  proof: by
  simp only [f.mk'_eq_iff_eq, mul_comm]

@[to_additive]

中文:
定理 mk'_eq_iff_eq'
  条件: {x₁ x₂} {y₁ y₂ : S}
  证明: by
  simp only [f.mk'_eq_iff_eq, mul_comm]

@[to_additive]
-/
theorem mk'_eq_iff_eq' {x₁ x₂} {y₁ y₂ : S} :
    f.mk' x₁ y₁ = f.mk' x₂ y₂ ↔ f (x₁ * y₂) = f (x₂ * y₁) := by
  simp only [f.mk'_eq_iff_eq, mul_comm]

@[to_additive]
/--
theorem `eq` / 定理 `eq`

English:
theorem eq
  given: {a₁ b₁} {a₂ b₂ : S}
  proof: f.mk'_eq_iff_eq.trans f.eq_iff_exists

@[to_additive]

中文:
定理 eq
  条件: {a₁ b₁} {a₂ b₂ : S}
  证明: f.mk'_eq_iff_eq.trans f.eq_iff_exists

@[to_additive]
-/
protected theorem eq {a₁ b₁} {a₂ b₂ : S} :
    f.mk' a₁ a₂ = f.mk' b₁ b₂ ↔ exists c : S, ↑c * (↑b₂ * a₁) = c * (a₂ * b₁) :=
f.mk'_eq_iff_eq.trans f.eq_iff_exists

@[to_additive]
/--
theorem `eq'` / 定理 `eq'`

English:
theorem eq'
  given: {a₁ b₁} {a₂ b₂ : S}
  proof: by
  rw [f.eq]; rw [Localization.r_iff_exists]

@[to_additive]

中文:
定理 eq'
  条件: {a₁ b₁} {a₂ b₂ : S}
  证明: by
  rw [f.eq]; rw [Localization.r_iff_exists]

@[to_additive]
-/
protected theorem eq' {a₁ b₁} {a₂ b₂ : S} :
    f.mk' a₁ a₂ = f.mk' b₁ b₂ ↔ Localization.r S (a₁, a₂) (b₁, b₂) := by
  rw [f.eq]; rw [Localization.r_iff_exists]

@[to_additive]
/--
theorem `eq_iff_eq` / 定理 `eq_iff_eq`

English:
theorem eq_iff_eq
  given: (g : LocalizationMap S P) {x y}
  statement: f x = f y ↔ g x = g y
  proof: f.eq_iff_exists.trans g.eq_iff_exists.symm

@[to_additive]

中文:
定理 eq_iff_eq
  条件: (g : Localization映射 S P) {x y}
  结论: f x = f y ↔ g x = g y
  证明: f.eq_iff_exists.trans g.eq_iff_exists.symm

@[to_additive]

Depends on / 依赖: eq_iff_exists, f.eq_iff_exists.trans, g.eq_iff_exists.symm
-/
theorem eq_iff_eq (g : LocalizationMap S P) {x y} : f x = f y ↔ g x = g y :=
  f.eq_iff_exists.trans g.eq_iff_exists.symm

@[to_additive]
/--
theorem `mk'_eq_iff_mk'_eq` / 定理 `mk'_eq_iff_mk'_eq`

English:
theorem mk'_eq_iff_mk'_eq
  given: (g : LocalizationMap S P) {x₁ x₂} {y₁ y₂ : S}
  proof: f.eq'.trans g.eq'.symm

中文:
定理 mk'_eq_iff_mk'_eq
  条件: (g : Localization映射 S P) {x₁ x₂} {y₁ y₂ : S}
  证明: f.eq'.trans g.eq'.symm
-/
theorem mk'_eq_iff_mk'_eq (g : LocalizationMap S P) {x₁ x₂} {y₁ y₂ : S} :
    f.mk' x₁ y₁ = f.mk' x₂ y₂ ↔ g.mk' x₁ y₁ = g.mk' x₂ y₂ :=
  f.eq'.trans g.eq'.symm

/-- Given a Localization map `f : M →* N` for a Submonoid `S ⊆ M`, for all `x₁ : M` and `y₁ ∈ S`,
if `x₂ : M, y₂ ∈ S` are such that `f x₁ * (f y₁)⁻¹ * f y₂ = f x₂`, then there exists `c ∈ S`
such that `x₁ * y₂ * c = x₂ * y₁ * c`. -/
@[to_additive
/-- Given a Localization map `f : M →+ N` for a Submonoid `S ⊆ M`, for all `x₁ : M`
and `y₁ ∈ S`, if `x₂ : M, y₂ ∈ S` are such that `(f x₁ - f y₁) + f y₂ = f x₂`, then there exists
`c ∈ S` such that `x₁ + y₂ + c = x₂ + y₁ + c`. -/]
/--
theorem `exists_of_sec_mk'` / 定理 `exists_of_sec_mk'`

English:
theorem exists_of_sec_mk'
  given: (x) (y : S)
  proof: f.eq_iff_exists.1 f.mk'_eq_iff_eq.1 (mk'_sec _ _).symm

@[to_additive]

中文:
定理 存在_of_sec_mk'
  条件: (x) (y : S)
  证明: f.eq_iff_exists.1 f.mk'_eq_iff_eq.1 (mk'_sec _ _).symm

@[to_additive]

Depends on / 依赖: _eq_iff_eq, _sec, eq_iff_exists, f.eq_iff_exists, f.mk
-/
theorem exists_of_sec_mk' (x) (y : S) :
    exists c : S, ↑c * (↑(f.sec <| f.mk' x y).2 * x) = c * (y * (f.sec <| f.mk' x y).1) :=
f.eq_iff_exists.1 f.mk'_eq_iff_eq.1 (mk'_sec _ _).symm

@[to_additive]
/--
theorem `mk'_eq_of_eq` / 定理 `mk'_eq_of_eq`

English:
theorem mk'_eq_of_eq
  given: {a₁ b₁ : M} {a₂ b₂ : S} (H : ↑a₂ * b₁ = ↑b₂ * a₁)
  proof: f.mk'_eq_iff_eq.2 H ▸ rfl

@[to_additive]

中文:
定理 mk'_eq_of_eq
  条件: {a₁ b₁ : M} {a₂ b₂ : S} (H : ↑a₂ * b₁ = ↑b₂ * a₁)
  证明: f.mk'_eq_iff_eq.2 H ▸ rfl

@[to_additive]
-/
theorem mk'_eq_of_eq {a₁ b₁ : M} {a₂ b₂ : S} (H : ↑a₂ * b₁ = ↑b₂ * a₁) :
    f.mk' a₁ a₂ = f.mk' b₁ b₂ :=
f.mk'_eq_iff_eq.2 H ▸ rfl

@[to_additive]
/--
theorem `mk'_eq_of_eq'` / 定理 `mk'_eq_of_eq'`

English:
theorem mk'_eq_of_eq'
  given: {a₁ b₁ : M} {a₂ b₂ : S} (H : b₁ * ↑a₂ = a₁ * ↑b₂)
  proof: f.mk'_eq_of_eq by simpa only [mul_comm] using H

@[to_additive]

中文:
定理 mk'_eq_of_eq'
  条件: {a₁ b₁ : M} {a₂ b₂ : S} (H : b₁ * ↑a₂ = a₁ * ↑b₂)
  证明: f.mk'_eq_of_eq by simpa only [mul_comm] using H

@[to_additive]
-/
theorem mk'_eq_of_eq' {a₁ b₁ : M} {a₂ b₂ : S} (H : b₁ * ↑a₂ = a₁ * ↑b₂) :
    f.mk' a₁ a₂ = f.mk' b₁ b₂ :=
f.mk'_eq_of_eq by simpa only [mul_comm] using H

@[to_additive]
/--
theorem `mk'_cancel` / 定理 `mk'_cancel`

English:
theorem mk'_cancel
  given: (a : M) (b c : S)
  proof: mk'_eq_of_eq' f (by rw [Submonoid.coe_mul, mul_comm (b : M), mul_assoc])

@[to_additive]

中文:
定理 mk'_cancel
  条件: (a : M) (b c : S)
  证明: mk'_eq_of_eq' f (by rw [Submonoid.coe_mul, mul_comm (b : M), mul_assoc])

@[to_additive]
-/
theorem mk'_cancel (a : M) (b c : S) :
    f.mk' (a * c) (b * c) = f.mk' a b :=
  mk'_eq_of_eq' f (by rw [Submonoid.coe_mul, mul_comm (b : M), mul_assoc])

@[to_additive]
/--
theorem `mk'_eq_of_same` / 定理 `mk'_eq_of_same`

English:
theorem mk'_eq_of_same
  given: {a b} {d : S}
  proof: by
  rw [mk'_eq_iff_eq']; rw [map_mul]; rw [map_mul]; rw [← eq_iff_exists f]
  exact (map_units f d).mul_left_inj

@[to_additive (attr := simp)]

中文:
定理 mk'_eq_of_same
  条件: {a b} {d : S}
  证明: by
  rw [mk'_eq_iff_eq']; rw [map_mul]; rw [map_mul]; rw [← eq_iff_exists f]
  exact (map_units f d).mul_left_inj

@[to_additive (attr := simp)]
-/
theorem mk'_eq_of_same {a b} {d : S} :
    f.mk' a d = f.mk' b d ↔ exists c : S, c * a = c * b := by
  rw [mk'_eq_iff_eq']; rw [map_mul]; rw [map_mul]; rw [← eq_iff_exists f]
  exact (map_units f d).mul_left_inj

@[to_additive (attr := simp)]
/--
theorem `mk'_self'` / 定理 `mk'_self'`

English:
theorem mk'_self'
  given: (y : S)
  statement: f.mk' (y : M) y = 1
  proof: show _ * _ = _ by rw [mul_inv_left, mul_one]; dsimp

@[to_additive (attr := simp)]

中文:
定理 mk'_self'
  条件: (y : S)
  结论: f.mk' (y : M) y = 1
  证明: show _ * _ = _ by rw [mul_inv_left, mul_one]; dsimp

@[to_additive (attr := simp)]
-/
theorem mk'_self' (y : S) : f.mk' (y : M) y = 1 :=
  show _ * _ = _ by rw [mul_inv_left, mul_one]; dsimp

@[to_additive (attr := simp)]
/--
theorem `mk'_self` / 定理 `mk'_self`

English:
theorem mk'_self
  given: (x) (H : x in S)
  statement: f.mk' x ⟨x, H⟩ = 1
  proof: mk'_self' f ⟨x, H⟩

@[to_additive]

中文:
定理 mk'_self
  条件: (x) (H : x in S)
  结论: f.mk' x ⟨x, H⟩ = 1
  证明: mk'_self' f ⟨x, H⟩

@[to_additive]
-/
theorem mk'_self (x) (H : x in S) : f.mk' x ⟨x, H⟩ = 1 := mk'_self' f ⟨x, H⟩

@[to_additive]
/--
theorem `mul_mk'_eq_mk'_of_mul` / 定理 `mul_mk'_eq_mk'_of_mul`

English:
theorem mul_mk'_eq_mk'_of_mul
  given: (x₁ x₂) (y : S)
  statement: f x₁ * f.mk' x₂ y = f.mk' (x₁ * x₂) y
  proof: by
  rw [← mk'_one]; rw [← mk'_mul]; rw [one_mul]

@[to_additive]

中文:
定理 mul_mk'_eq_mk'_of_mul
  条件: (x₁ x₂) (y : S)
  结论: f x₁ * f.mk' x₂ y = f.mk' (x₁ * x₂) y
  证明: by
  rw [← mk'_one]; rw [← mk'_mul]; rw [one_mul]

@[to_additive]

Depends on / 依赖: _mul, _one, one_mul
-/
theorem mul_mk'_eq_mk'_of_mul (x₁ x₂) (y : S) : f x₁ * f.mk' x₂ y = f.mk' (x₁ * x₂) y := by
  rw [← mk'_one]; rw [← mk'_mul]; rw [one_mul]

@[to_additive]
/--
theorem `mk'_mul_eq_mk'_of_mul` / 定理 `mk'_mul_eq_mk'_of_mul`

English:
theorem mk'_mul_eq_mk'_of_mul
  given: (x₁ x₂) (y : S)
  statement: f.mk' x₂ y * f x₁ = f.mk' (x₁ * x₂) y
  proof: by
  rw [mul_comm]; rw [mul_mk'_eq_mk'_of_mul]

@[to_additive]

中文:
定理 mk'_mul_eq_mk'_of_mul
  条件: (x₁ x₂) (y : S)
  结论: f.mk' x₂ y * f x₁ = f.mk' (x₁ * x₂) y
  证明: by
  rw [mul_comm]; rw [mul_mk'_eq_mk'_of_mul]

@[to_additive]
-/
theorem mk'_mul_eq_mk'_of_mul (x₁ x₂) (y : S) : f.mk' x₂ y * f x₁ = f.mk' (x₁ * x₂) y := by
  rw [mul_comm]; rw [mul_mk'_eq_mk'_of_mul]

@[to_additive]
/--
theorem `mul_mk'_one_eq_mk'` / 定理 `mul_mk'_one_eq_mk'`

English:
theorem mul_mk'_one_eq_mk'
  given: (x) (y : S)
  statement: f x * f.mk' 1 y = f.mk' x y
  proof: by
  rw [mul_mk'_eq_mk'_of_mul]; rw [mul_one]

@[to_additive (attr := simp)]

中文:
定理 mul_mk'_one_eq_mk'
  条件: (x) (y : S)
  结论: f x * f.mk' 1 y = f.mk' x y
  证明: by
  rw [mul_mk'_eq_mk'_of_mul]; rw [mul_one]

@[to_additive (attr := simp)]
-/
theorem mul_mk'_one_eq_mk' (x) (y : S) : f x * f.mk' 1 y = f.mk' x y := by
  rw [mul_mk'_eq_mk'_of_mul]; rw [mul_one]

@[to_additive (attr := simp)]
/--
theorem `mk'_mul_cancel_right` / 定理 `mk'_mul_cancel_right`

English:
theorem mk'_mul_cancel_right
  given: (x : M) (y : S)
  statement: f.mk' (x * y) y = f x
  proof: by
  rw [← mul_mk'_one_eq_mk']; rw [map_mul]; rw [mul_assoc]; rw [mul_mk'_one_eq_mk']; rw [mk'_self']; rw [mul_one]

@[to_additive]

中文:
定理 mk'_mul_cancel_right
  条件: (x : M) (y : S)
  结论: f.mk' (x * y) y = f x
  证明: by
  rw [← mul_mk'_one_eq_mk']; rw [map_mul]; rw [mul_assoc]; rw [mul_mk'_one_eq_mk']; rw [mk'_self']; rw [mul_one]

@[to_additive]
-/
theorem mk'_mul_cancel_right (x : M) (y : S) : f.mk' (x * y) y = f x := by
  rw [← mul_mk'_one_eq_mk']; rw [map_mul]; rw [mul_assoc]; rw [mul_mk'_one_eq_mk']; rw [mk'_self']; rw [mul_one]

@[to_additive]
/--
theorem `mk'_mul_cancel_left` / 定理 `mk'_mul_cancel_left`

English:
theorem mk'_mul_cancel_left
  given: (x) (y : S)
  statement: f.mk' ((y : M) * x) y = f x
  proof: by
  rw [mul_comm]; rw [mk'_mul_cancel_right]

@[to_additive]

中文:
定理 mk'_mul_cancel_left
  条件: (x) (y : S)
  结论: f.mk' ((y : M) * x) y = f x
  证明: by
  rw [mul_comm]; rw [mk'_mul_cancel_right]

@[to_additive]
-/
theorem mk'_mul_cancel_left (x) (y : S) : f.mk' ((y : M) * x) y = f x := by
  rw [mul_comm]; rw [mk'_mul_cancel_right]

@[to_additive]
/--
theorem `isUnit_comp` / 定理 `isUnit_comp`

English:
theorem isUnit_comp
  given: (j : N ->* P) (y : S)
  statement: IsUnit (j.comp f.toMonoidHom y)
  proof: ⟨Units.map j IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y, show j _ = j _ from
      congr_arg j (IsUnit.coe_liftRight (f.toMonoidHom.domRestrict S) f.map_units _)⟩

@[to_additive]

中文:
定理 isUnit_comp
  条件: (j : N ->* P) (y : S)
  结论: 是单位 (j.comp f.toMonoidHom y)
  证明: ⟨Units.map j IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y, show j _ = j _ from
      congr_arg j (IsUnit.coe_liftRight (f.toMonoidHom.domRestrict S) f.map_units _)⟩

@[to_additive]

Depends on / 依赖: IsUnit, IsUnit.coe_liftRight, IsUnit.liftRight, Units.map, coe_liftRight, congr_arg, domRestrict, f.map_units, f.toMonoidHom.domRestrict, liftRight, map_units, toMonoidHom
-/
theorem isUnit_comp (j : N ->* P) (y : S) : IsUnit (j.comp f.toMonoidHom y) :=
⟨Units.map j IsUnit.liftRight (f.toMonoidHom.domRestrict S) f.map_units y, show j _ = j _ from
      congr_arg j (IsUnit.coe_liftRight (f.toMonoidHom.domRestrict S) f.map_units _)⟩

@[to_additive]
/--
theorem `epic_of_localizationMap` / 定理 `epic_of_localizationMap`

English:
theorem epic_of_localizationMap
  statement: {P : Type*} [Monoid P] {j k : N ->* P}
  proof: by
  ext n
  obtain ⟨⟨m, s⟩, hn : n * f s = f m⟩ := f.surj n
  replace h (a) : j (f a) = k (f a) := congr($h a)
exact ((f.map_units s).map j).mul_left_inj.mp by rw [← j.map_mul, h, ← k.map_mul, hn, h m]

中文:
定理 epic_of_localizationMap
  结论: {P : 类型} [幺半群 P] {j k : N ->* P}
  证明: by
  ext n
  obtain ⟨⟨m, s⟩, hn : n * f s = f m⟩ := f.surj n
  replace h (a) : j (f a) = k (f a) := congr($h a)
exact ((f.map_units s).map j).mul_left_inj.mp by rw [← j.map_mul, h, ← k.map_mul, hn, h m]

Depends on / 依赖: f.map_units, f.surj, j.map_mul, k.map_mul, map_mul, map_units, mul_left_inj, mul_left_inj.mp, replace
-/
theorem epic_of_localizationMap {P : Type*} [Monoid P] {j k : N ->* P}
    (h : j.comp f.toMonoidHom = k.comp f.toMonoidHom) : j = k := by
  ext n
  obtain ⟨⟨m, s⟩, hn : n * f s = f m⟩ := f.surj n
  replace h (a) : j (f a) = k (f a) := congr($h a)
exact ((f.map_units s).map j).mul_left_inj.mp by rw [← j.map_mul, h, ← k.map_mul, hn, h m]

end LocalizationMap

end Submonoid

namespace Localization

variable (S) in
/-- Natural homomorphism sending `x : M`, `M` a `CommMonoid`, to the equivalence class of
`(x, 1)` in the Localization of `M` at a Submonoid. -/
@[to_additive
/-- Natural homomorphism sending `x : M`, `M` an `AddCommMonoid`, to the equivalence class of
`(x, 0)` in the Localization of `M` at a Submonoid. -/]
/--
Definition of `monoidOf` / `monoidOf` 的定义

English:
definition monoidOf
  signature: : Submonoid.LocalizationMap S (Localization S)
  body: { (r S).mk'.comp <| MonoidHom.inl M S with
    toFun := fun x => mk x 1
    map_mul' := fun x y => by rw [mk_mul, mul_one]
    isLocalizationMap :=
    { map_units y :=
        isUnit_iff_exists_inv.2 ⟨mk 1 y, by rw [mk_mul, mul_one, one_mul, mk_self]⟩
      surj z := induction_on z fun x =>
        ⟨x, by rw [mk_mul, mul_comm x.fst, ← mk_mul, mk_self, one_mul]⟩
exists_of_eq := Iff.mp mk_eq_mk_iff.trans r_iff_exists.trans by simp } }

@[to_additive]

中文:
定义 monoidOf
  签名: : 子幺半群.Localization映射 S (Localization S)
  定义体: { (r S).mk'.comp <| MonoidHom.inl M S with
    toFun := fun x => mk x 1
    map_mul' := fun x y => by rw [mk_mul, mul_one]
    isLocalizationMap :=
    { map_units y :=
        isUnit_iff_exists_inv.2 ⟨mk 1 y, by rw [mk_mul, mul_one, one_mul, mk_self]⟩
      surj z := induction_on z fun x =>
        ⟨x, by rw [mk_mul, mul_comm x.fst, ← mk_mul, mk_self, one_mul]⟩
exists_of_eq := Iff.mp mk_eq_mk_iff.trans r_iff_exists.trans by simp } }

@[to_additive]

Depends on / 依赖: Iff.mp, MonoidHom, MonoidHom.inl, exists_of_eq, induction_on, isLocalizationMap, isUnit_iff_exists_inv, map_mul, map_units, mk_eq_mk_iff, mk_eq_mk_iff.trans, mk_mul, mk_self, mul_comm, mul_one, one_mul, r_iff_exists, r_iff_exists.trans, x.fst
-/
def monoidOf : Submonoid.LocalizationMap S (Localization S) :=
  { (r S).mk'.comp <| MonoidHom.inl M S with
    toFun := fun x => mk x 1
    map_mul' := fun x y => by rw [mk_mul, mul_one]
    isLocalizationMap :=
    { map_units y :=
        isUnit_iff_exists_inv.2 ⟨mk 1 y, by rw [mk_mul, mul_one, one_mul, mk_self]⟩
      surj z := induction_on z fun x =>
        ⟨x, by rw [mk_mul, mul_comm x.fst, ← mk_mul, mk_self, one_mul]⟩
exists_of_eq := Iff.mp mk_eq_mk_iff.trans r_iff_exists.trans by simp } }

@[to_additive]
/--
theorem `mk_one_eq_monoidOf_mk` / 定理 `mk_one_eq_monoidOf_mk`

English:
theorem mk_one_eq_monoidOf_mk
  given: (x)
  statement: mk x 1 = monoidOf S x
  proof: rfl

@[to_additive]

中文:
定理 mk_one_eq_monoidOf_mk
  条件: (x)
  结论: mk x 1 = monoidOf S x
  证明: rfl

@[to_additive]
-/
theorem mk_one_eq_monoidOf_mk (x) : mk x 1 = monoidOf S x := rfl

@[to_additive]
/--
theorem `mk_eq_monoidOf_mk'_apply` / 定理 `mk_eq_monoidOf_mk'_apply`

English:
theorem mk_eq_monoidOf_mk'_apply
  given: (x y)
  statement: mk x y = (monoidOf S).mk' x y
  proof: show _ = _ * _ from
(Submonoid.LocalizationMap.mul_inv_right (monoidOf S).map_units _ _ _).2 by
      dsimp
      rw [← mk_one_eq_monoidOf_mk]; rw [← mk_one_eq_monoidOf_mk]; rw [mk_mul x y y 1]; rw [mul_comm y 1]
      conv => rhs; rw [← mul_one 1]; rw [← mul_one x]
      exact mk_eq_mk_iff.2 (Con.symm _ <| (Localization.r S).mul (Con.refl _ (x, 1)) <| one_rel _)

@[to_additive]

中文:
定理 mk_eq_monoidOf_mk'_apply
  条件: (x y)
  结论: mk x y = (monoidOf S).mk' x y
  证明: show _ = _ * _ from
(Submonoid.LocalizationMap.mul_inv_right (monoidOf S).map_units _ _ _).2 by
      dsimp
      rw [← mk_one_eq_monoidOf_mk]; rw [← mk_one_eq_monoidOf_mk]; rw [mk_mul x y y 1]; rw [mul_comm y 1]
      conv => rhs; rw [← mul_one 1]; rw [← mul_one x]
      exact mk_eq_mk_iff.2 (Con.symm _ <| (Localization.r S).mul (Con.refl _ (x, 1)) <| one_rel _)

@[to_additive]

Depends on / 依赖: Con.refl, Con.symm, Localization, Localization.r, LocalizationMap, Submonoid, Submonoid.LocalizationMap.mul_inv_right, map_units, mk_eq_mk_iff, mk_mul, mk_one_eq_monoidOf_mk, monoidOf, mul_comm, mul_inv_right, mul_one, one_rel
-/
theorem mk_eq_monoidOf_mk'_apply (x y) : mk x y = (monoidOf S).mk' x y :=
  show _ = _ * _ from
(Submonoid.LocalizationMap.mul_inv_right (monoidOf S).map_units _ _ _).2 by
      dsimp
      rw [← mk_one_eq_monoidOf_mk]; rw [← mk_one_eq_monoidOf_mk]; rw [mk_mul x y y 1]; rw [mul_comm y 1]
      conv => rhs; rw [← mul_one 1]; rw [← mul_one x]
      exact mk_eq_mk_iff.2 (Con.symm _ <| (Localization.r S).mul (Con.refl _ (x, 1)) <| one_rel _)

@[to_additive]
/--
theorem `mk_eq_monoidOf_mk'` / 定理 `mk_eq_monoidOf_mk'`

English:
theorem mk_eq_monoidOf_mk'
  statement: mk = (monoidOf S).mk'
  proof: funext fun _ => funext fun _ => mk_eq_monoidOf_mk'_apply _ _

universe u

@[to_additive (attr := simp)]

中文:
定理 mk_eq_monoidOf_mk'
  结论: mk = (monoidOf S).mk'
  证明: funext fun _ => funext fun _ => mk_eq_monoidOf_mk'_apply _ _

universe u

@[to_additive (attr := simp)]
-/
theorem mk_eq_monoidOf_mk' : mk = (monoidOf S).mk' :=
  funext fun _ => funext fun _ => mk_eq_monoidOf_mk'_apply _ _

universe u

@[to_additive (attr := simp)]
/--
theorem `liftOn_mk'` / 定理 `liftOn_mk'`

English:
theorem liftOn_mk'
  given: {p : Sort u} (f : M -> S -> p) (H) (a : M) (b : S)
  proof: by rw [← mk_eq_monoidOf_mk', liftOn_mk]

@[to_additive (attr := simp)]

中文:
定理 liftOn_mk'
  条件: {p : 类型层 u} (f : M -> S -> p) (H) (a : M) (b : S)
  证明: by rw [← mk_eq_monoidOf_mk', liftOn_mk]

@[to_additive (attr := simp)]

Depends on / 依赖: liftOn_mk, mk_eq_monoidOf_mk
-/
theorem liftOn_mk' {p : Sort u} (f : M -> S -> p) (H) (a : M) (b : S) :
    liftOn ((monoidOf S).mk' a b) f H = f a b := by rw [← mk_eq_monoidOf_mk', liftOn_mk]

@[to_additive (attr := simp)]
/--
theorem `liftOn₂_mk'` / 定理 `liftOn₂_mk'`

English:
theorem liftOn₂_mk'
  given: {p : Sort*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S)
  proof: by
  rw [← mk_eq_monoidOf_mk']; rw [liftOn₂_mk]

中文:
定理 liftOn₂_mk'
  条件: {p : 类型层*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S)
  证明: by
  rw [← mk_eq_monoidOf_mk']; rw [liftOn₂_mk]

Depends on / 依赖: mk_eq_monoidOf_mk
-/
theorem liftOn₂_mk' {p : Sort*} (f : M -> S -> M -> S -> p) (H) (a c : M) (b d : S) :
    liftOn₂ ((monoidOf S).mk' a b) ((monoidOf S).mk' c d) f H = f a b c d := by
  rw [← mk_eq_monoidOf_mk']; rw [liftOn₂_mk]

/-- The localization of a torsion-free monoid is torsion-free. -/
@[to_additive /-- The localization of a torsion-free monoid is torsion-free. -/]
/--
Instance `instIsMulTorsionFree` / 实例 `instIsMulTorsionFree`

English:
instance instIsMulTorsionFree
  signature: [IsMulTorsionFree M]
  body: by
    rintro ⟨a⟩ ⟨b⟩ (hab : mk a.1 a.2 ^ n = mk b.1 b.2 ^ n)
    change mk a.1 a.2 = mk b.1 b.2
    simp only [mk_pow, mk_eq_mk_iff, r_iff_exists, SubmonoidClass.coe_pow, Subtype.exists,
      exists_prop] at hab ⊢
    obtain ⟨c, hc, hab⟩ := hab
    refine ⟨c, hc, pow_left_injective hn ?_⟩
    obtain _ | n := n
    · simp
    · simp [mul_pow, pow_succ c, mul_assoc, hab]

中文:
实例 instIsMulTorsionFree
  签名: [是MulTorsionFree M]
  定义体: by
    rintro ⟨a⟩ ⟨b⟩ (hab : mk a.1 a.2 ^ n = mk b.1 b.2 ^ n)
    change mk a.1 a.2 = mk b.1 b.2
    simp only [mk_pow, mk_eq_mk_iff, r_iff_exists, SubmonoidClass.coe_pow, Subtype.exists,
      exists_prop] at hab ⊢
    obtain ⟨c, hc, hab⟩ := hab
    refine ⟨c, hc, pow_left_injective hn ?_⟩
    obtain _ | n := n
    · simp
    · simp [mul_pow, pow_succ c, mul_assoc, hab]

Depends on / 依赖: SubmonoidClass, SubmonoidClass.coe_pow, Subtype, Subtype.exists, coe_pow, exists_prop, mk_eq_mk_iff, mk_pow, mul_assoc, mul_pow, pow_left_injective, pow_succ, r_iff_exists
-/
instance instIsMulTorsionFree [IsMulTorsionFree M] : IsMulTorsionFree Localization S where
  pow_left_injective n hn := by
    rintro ⟨a⟩ ⟨b⟩ (hab : mk a.1 a.2 ^ n = mk b.1 b.2 ^ n)
    change mk a.1 a.2 = mk b.1 b.2
    simp only [mk_pow, mk_eq_mk_iff, r_iff_exists, SubmonoidClass.coe_pow, Subtype.exists,
      exists_prop] at hab ⊢
    obtain ⟨c, hc, hab⟩ := hab
    refine ⟨c, hc, pow_left_injective hn ?_⟩
    obtain _ | n := n
    · simp
    · simp [mul_pow, pow_succ c, mul_assoc, hab]

end Localization

end CommMonoid

namespace Localization

variable {α : Type*} [CommMonoid α] [IsCancelMul α] {s : Submonoid α} {a₁ b₁ : α} {a₂ b₂ : s}

@[to_additive]
/--
theorem `mk_left_injective` / 定理 `mk_left_injective`

English:
theorem mk_left_injective
  given: (b : s)
  statement: Injective fun a => mk a b
  proof: fun c d h => by
  simpa [mk_eq_mk_iff, r_iff_exists] using h

@[to_additive]

中文:
定理 mk_left_injective
  条件: (b : s)
  结论: 单射 fun a => mk a b
  证明: fun c d h => by
  simpa [mk_eq_mk_iff, r_iff_exists] using h

@[to_additive]

Depends on / 依赖: mk_eq_mk_iff, r_iff_exists
-/
theorem mk_left_injective (b : s) : Injective fun a => mk a b := fun c d h => by
  simpa [mk_eq_mk_iff, r_iff_exists] using h

@[to_additive]
/--
theorem `mk_eq_mk_iff'` / 定理 `mk_eq_mk_iff'`

English:
theorem mk_eq_mk_iff'
  statement: mk a₁ a₂ = mk b₁ b₂ ↔ ↑b₂ * a₁ = a₂ * b₁
  proof: by
  simp_rw [mk_eq_mk_iff, r_iff_exists, mul_left_cancel_iff, exists_const]

@[to_additive]

中文:
定理 mk_eq_mk_iff'
  结论: mk a₁ a₂ = mk b₁ b₂ ↔ ↑b₂ * a₁ = a₂ * b₁
  证明: by
  simp_rw [mk_eq_mk_iff, r_iff_exists, mul_left_cancel_iff, exists_const]

@[to_additive]

Depends on / 依赖: exists_const, mk_eq_mk_iff, mul_left_cancel_iff, r_iff_exists, simp_rw
-/
theorem mk_eq_mk_iff' : mk a₁ a₂ = mk b₁ b₂ ↔ ↑b₂ * a₁ = a₂ * b₁ := by
  simp_rw [mk_eq_mk_iff, r_iff_exists, mul_left_cancel_iff, exists_const]

@[to_additive]
/--
Instance `decidableEq` / 实例 `decidableEq`

English:
instance decidableEq
  signature: [DecidableEq α]
  body: fun a b =>
  Localization.recOnSubsingleton₂ a b fun _ _ _ _ => decidable_of_iff' _ mk_eq_mk_iff'

中文:
实例 decidableEq
  签名: [DecidableEq α]
  定义体: fun a b =>
  Localization.recOnSubsingleton₂ a b fun _ _ _ _ => decidable_of_iff' _ mk_eq_mk_iff'
-/
instance decidableEq [DecidableEq α] : DecidableEq (Localization s) := fun a b =>
  Localization.recOnSubsingleton₂ a b fun _ _ _ _ => decidable_of_iff' _ mk_eq_mk_iff'

end Localization

namespace OreLocalization

variable (R) [CommMonoid R] (S : Submonoid R)

/--
Definition of `localizationMap` / `localizationMap` 的定义

English:
definition localizationMap
  signature: : S.LocalizationMap R[S⁻¹]
  body: Localization.monoidOf S

中文:
定义 localizationMap
  签名: : S.Localization映射 R[S⁻¹]
  定义体: Localization.monoidOf S
-/
protected def localizationMap : S.LocalizationMap R[S⁻¹] := Localization.monoidOf S

/--
Definition of `noncomputable` / `noncomputable` 的定义

English:
definition noncomputable
  signature: def equivMonoidLocalization
  body: MulEquiv.refl _

中文:
定义 noncomputable
  签名: def equivMonoidLocalization
  定义体: MulEquiv.refl _
-/
protected noncomputable def equivMonoidLocalization : Localization S ≃* R[S⁻¹] := MulEquiv.refl _

end OreLocalization

section Group

variable {M G F : Type*} [CommMonoid M] [CommGroup G]

/--
theorem `Submonoid.isLocalizationMap_iff_bijective` / 定理 `Submonoid.isLocalizationMap_iff_bijective`

English:
theorem Submonoid.isLocalizationMap_iff_bijective
  statement: {S : Submonoid G}
  proof: by
    refine ⟨fun g g' eq => ?_, fun m => ?_⟩
    · have ⟨c, eq⟩ := h.exists_of_eq eq
      exact mul_left_cancel eq
    · have ⟨x, eq⟩ := h.surj m
      use x.1 / x.2
      rw [div_eq_mul_inv]; rw [map_mul]; rw [← eq]; rw [mul_assoc]; rw [← map_mul]; rw [mul_inv_cancel]; rw [h.map_one]; rw [mul_one]
  mpr h := let e : G ≃* M := ⟨Equiv.ofBijective f h, map_mul f⟩
  { map_units _ := (Group.isUnit _).map e
    surj m := have ⟨g, eq⟩ := h.2 m
⟨⟨g, 1⟩, congr(m * $e.map_one).trans (mul_one _).trans eq.symm⟩
    exists_of_eq eq := by simp [h.1 eq] }

中文:
定理 子幺半群.isLocalizationMap_iff_bijective
  结论: {S : 子幺半群 G}
  证明: by
    refine ⟨fun g g' eq => ?_, fun m => ?_⟩
    · have ⟨c, eq⟩ := h.exists_of_eq eq
      exact mul_left_cancel eq
    · have ⟨x, eq⟩ := h.surj m
      use x.1 / x.2
      rw [div_eq_mul_inv]; rw [map_mul]; rw [← eq]; rw [mul_assoc]; rw [← map_mul]; rw [mul_inv_cancel]; rw [h.map_one]; rw [mul_one]
  mpr h := let e : G ≃* M := ⟨Equiv.ofBijective f h, map_mul f⟩
  { map_units _ := (Group.isUnit _).map e
    surj m := have ⟨g, eq⟩ := h.2 m
⟨⟨g, 1⟩, congr(m * $e.map_one).trans (mul_one _).trans eq.symm⟩
    exists_of_eq eq := by simp [h.1 eq] }
-/
@[to_additive] theorem Submonoid.isLocalizationMap_iff_bijective {S : Submonoid G}
    [FunLike F G M] [MulHomClass F G M] {f : F} :
    S.IsLocalizationMap f ↔ Bijective f where
  mp h := by
    refine ⟨fun g g' eq => ?_, fun m => ?_⟩
    · have ⟨c, eq⟩ := h.exists_of_eq eq
      exact mul_left_cancel eq
    · have ⟨x, eq⟩ := h.surj m
      use x.1 / x.2
      rw [div_eq_mul_inv]; rw [map_mul]; rw [← eq]; rw [mul_assoc]; rw [← map_mul]; rw [mul_inv_cancel]; rw [h.map_one]; rw [mul_one]
  mpr h := let e : G ≃* M := ⟨Equiv.ofBijective f h, map_mul f⟩
  { map_units _ := (Group.isUnit _).map e
    surj m := have ⟨g, eq⟩ := h.2 m
⟨⟨g, 1⟩, congr(m * $e.map_one).trans (mul_one _).trans eq.symm⟩
    exists_of_eq eq := by simp [h.1 eq] }

/--
theorem `Submonoid.isLocalizationMap_id` / 定理 `Submonoid.isLocalizationMap_id`

English:
theorem Submonoid.isLocalizationMap_id
  given: (S : Submonoid G)
  proof: S.isLocalizationMap_iff_bijective (f := MulHom.id _).mpr bijective_id

中文:
定理 子幺半群.isLocalizationMap_id
  条件: (S : 子幺半群 G)
  证明: S.isLocalizationMap_iff_bijective (f := MulHom.id _).mpr bijective_id
-/
@[to_additive] theorem Submonoid.isLocalizationMap_id (S : Submonoid G) :
    S.IsLocalizationMap (@id G) :=
  S.isLocalizationMap_iff_bijective (f := MulHom.id _).mpr bijective_id

/--
theorem `Submonoid.isLocalizationMap_of_group` / 定理 `Submonoid.isLocalizationMap_of_group`

English:
theorem Submonoid.isLocalizationMap_of_group
  statement: {S : Submonoid M}
  proof: Group.isUnit _
  surj g := have ⟨x, y, hy, eq⟩ := surj g; ⟨⟨x, y, hy⟩, by simp [eq]⟩
  exists_of_eq eq := by simp [hf eq]

中文:
定理 子幺半群.isLocalizationMap_of_group
  结论: {S : 子幺半群 M}
  证明: Group.isUnit _
  surj g := have ⟨x, y, hy, eq⟩ := surj g; ⟨⟨x, y, hy⟩, by simp [eq]⟩
  exists_of_eq eq := by simp [hf eq]
-/
@[to_additive] theorem Submonoid.isLocalizationMap_of_group {S : Submonoid M}
    {f : M -> G} (hf : f.Injective) (surj : forall g : G, exists x : M, exists y in S, g = f x / f y) :
    S.IsLocalizationMap f where
  map_units _ := Group.isUnit _
  surj g := have ⟨x, y, hy, eq⟩ := surj g; ⟨⟨x, y, hy⟩, by simp [eq]⟩
  exists_of_eq eq := by simp [hf eq]

/--
theorem `AddSubmonoid.isLocalizationMap_nat_int` / 定理 `AddSubmonoid.isLocalizationMap_nat_int`

English:
theorem AddSubmonoid.isLocalizationMap_nat_int
  given: (S : AddSubmonoid Nat) (hS : S != ⊥)
  proof: S.isLocalizationMap_of_addGroup (fun _ _ => Int.natCast_inj.mp) fun z => by
    obtain ⟨z, rfl | rfl⟩ := z.eq_nat_or_neg
    · exact ⟨z, 0, zero_mem _, by lia⟩
    have ⟨n, hnS, hn0⟩ := S.bot_or_exists_ne_zero.resolve_left hS
have key : z < n * (z / n + 1) := Nat.lt_mul_div_succ _ Nat.pos_of_ne_zero hn0
    exact ⟨(z / n + 1) * n - z, (z / n + 1) * n, nsmul_mem hnS _, by lia⟩

中文:
定理 加法子幺半群.isLocalizationMap_nat_int
  条件: (S : 加法子幺半群 自然数) (hS : S != ⊥)
  证明: S.isLocalizationMap_of_addGroup (fun _ _ => Int.natCast_inj.mp) fun z => by
    obtain ⟨z, rfl | rfl⟩ := z.eq_nat_or_neg
    · exact ⟨z, 0, zero_mem _, by lia⟩
    have ⟨n, hnS, hn0⟩ := S.bot_or_exists_ne_zero.resolve_left hS
have key : z < n * (z / n + 1) := Nat.lt_mul_div_succ _ Nat.pos_of_ne_zero hn0
    exact ⟨(z / n + 1) * n - z, (z / n + 1) * n, nsmul_mem hnS _, by lia⟩

Depends on / 依赖: Int.natCast_inj.mp, Nat.lt_mul_div_succ, Nat.pos_of_ne_zero, S.bot_or_exists_ne_zero.resolve_left, S.isLocalizationMap_of_addGroup, bot_or_exists_ne_zero, eq_nat_or_neg, isLocalizationMap_of_addGroup, lt_mul_div_succ, natCast_inj, nsmul_mem, pos_of_ne_zero, resolve_left, z.eq_nat_or_neg, zero_mem
-/
theorem AddSubmonoid.isLocalizationMap_nat_int (S : AddSubmonoid Nat) (hS : S != ⊥) :
    S.IsLocalizationMap ((↑) : Nat -> Int) :=
  S.isLocalizationMap_of_addGroup (fun _ _ => Int.natCast_inj.mp) fun z => by
    obtain ⟨z, rfl | rfl⟩ := z.eq_nat_or_neg
    · exact ⟨z, 0, zero_mem _, by lia⟩
    have ⟨n, hnS, hn0⟩ := S.bot_or_exists_ne_zero.resolve_left hS
have key : z < n * (z / n + 1) := Nat.lt_mul_div_succ _ Nat.pos_of_ne_zero hn0
    exact ⟨(z / n + 1) * n - z, (z / n + 1) * n, nsmul_mem hnS _, by lia⟩

/--
theorem `AddSubmonoid.isLocalizationMap_top_nat_int` / 定理 `AddSubmonoid.isLocalizationMap_top_nat_int`

English:
theorem AddSubmonoid.isLocalizationMap_top_nat_int
  proof: AddSubmonoid.isLocalizationMap_nat_int _ top_ne_bot

中文:
定理 加法子幺半群.isLocalizationMap_top_nat_int
  证明: AddSubmonoid.isLocalizationMap_nat_int _ top_ne_bot

Depends on / 依赖: AddSubmonoid, AddSubmonoid.isLocalizationMap_nat_int, isLocalizationMap_nat_int, top_ne_bot
-/
theorem AddSubmonoid.isLocalizationMap_top_nat_int :
    (⊤ : AddSubmonoid Nat).IsLocalizationMap ((↑) : Nat -> Int) :=
  AddSubmonoid.isLocalizationMap_nat_int _ top_ne_bot

end Group

namespace Submonoid.LocalizationMap

variable {M N : Type*} [CommMonoid M] {S : Submonoid M} [CommMonoid N]

/--
theorem `injective_iff` / 定理 `injective_iff`

English:
theorem injective_iff
  given: (f : LocalizationMap S N)
  proof: by
  simp_rw [Commute.isRegular_iff (Commute.all _), IsLeftRegular,
    Injective, LocalizationMap.eq_iff_exists, exists_imp, Subtype.forall]
  exact forall₂_comm

中文:
定理 injective_iff
  条件: (f : Localization映射 S N)
  证明: by
  simp_rw [Commute.isRegular_iff (Commute.all _), IsLeftRegular,
    Injective, LocalizationMap.eq_iff_exists, exists_imp, Subtype.forall]
  exact forall₂_comm
-/
@[to_additive] theorem injective_iff (f : LocalizationMap S N) :
    Injective f ↔ forall ⦃x⦄, x in S -> IsRegular x := by
  simp_rw [Commute.isRegular_iff (Commute.all _), IsLeftRegular,
    Injective, LocalizationMap.eq_iff_exists, exists_imp, Subtype.forall]
  exact forall₂_comm

/--
theorem `top_injective_iff` / 定理 `top_injective_iff`

English:
theorem top_injective_iff
  given: (f : (⊤ : Submonoid M).LocalizationMap N)
  proof: by
  simp [injective_iff, isCancelMul_iff_forall_isRegular]

中文:
定理 top_injective_iff
  条件: (f : (⊤ : 子幺半群 M).Localization映射 N)
  证明: by
  simp [injective_iff, isCancelMul_iff_forall_isRegular]
-/
@[to_additive] theorem top_injective_iff (f : (⊤ : Submonoid M).LocalizationMap N) :
    Injective f ↔ IsCancelMul M := by
  simp [injective_iff, isCancelMul_iff_forall_isRegular]

/--
theorem `map_isRegular` / 定理 `map_isRegular`

English:
theorem map_isRegular
  statement: (f : LocalizationMap S N) {m : M}
  proof: by
  refine (Commute.isRegular_iff (Commute.all _)).mpr fun n₁ n₂ eq => ?_
  have ⟨ms₁, eq₁⟩ := f.surj n₁
  have ⟨ms₂, eq₂⟩ := f.surj n₂
  rw [← (f.map_units (ms₁.2 * ms₂.2)).mul_left_inj]; rw [Submonoid.coe_mul]
  replace eq := congr($eq * f (ms₁.2 * ms₂.2))
  simp_rw [mul_assoc] at eq
  rw [map_mul]; rw [← mul_assoc n₁]; rw [eq₁]; rw [← mul_assoc n₂]; rw [mul_right_comm n₂]; rw [eq₂] at eq ⊢
  simp_rw [← map_mul, eq_iff_exists] at eq ⊢
  simp_rw [mul_left_comm _ m] at eq
  exact eq.imp fun _ => (hm.1 ·)

中文:
定理 map_isRegular
  结论: (f : Localization映射 S N) {m : M}
  证明: by
  refine (Commute.isRegular_iff (Commute.all _)).mpr fun n₁ n₂ eq => ?_
  have ⟨ms₁, eq₁⟩ := f.surj n₁
  have ⟨ms₂, eq₂⟩ := f.surj n₂
  rw [← (f.map_units (ms₁.2 * ms₂.2)).mul_left_inj]; rw [Submonoid.coe_mul]
  replace eq := congr($eq * f (ms₁.2 * ms₂.2))
  simp_rw [mul_assoc] at eq
  rw [map_mul]; rw [← mul_assoc n₁]; rw [eq₁]; rw [← mul_assoc n₂]; rw [mul_right_comm n₂]; rw [eq₂] at eq ⊢
  simp_rw [← map_mul, eq_iff_exists] at eq ⊢
  simp_rw [mul_left_comm _ m] at eq
  exact eq.imp fun _ => (hm.1 ·)
-/
@[to_additive] theorem map_isRegular (f : LocalizationMap S N) {m : M}
    (hm : IsRegular m) : IsRegular (f m) := by
  refine (Commute.isRegular_iff (Commute.all _)).mpr fun n₁ n₂ eq => ?_
  have ⟨ms₁, eq₁⟩ := f.surj n₁
  have ⟨ms₂, eq₂⟩ := f.surj n₂
  rw [← (f.map_units (ms₁.2 * ms₂.2)).mul_left_inj]; rw [Submonoid.coe_mul]
  replace eq := congr($eq * f (ms₁.2 * ms₂.2))
  simp_rw [mul_assoc] at eq
  rw [map_mul]; rw [← mul_assoc n₁]; rw [eq₁]; rw [← mul_assoc n₂]; rw [mul_right_comm n₂]; rw [eq₂] at eq ⊢
  simp_rw [← map_mul, eq_iff_exists] at eq ⊢
  simp_rw [mul_left_comm _ m] at eq
  exact eq.imp fun _ => (hm.1 ·)

/--
theorem `isCancelMul` / 定理 `isCancelMul`

English:
theorem isCancelMul
  given: (f : LocalizationMap S N) [IsCancelMul M]
  statement: IsCancelMul N
  proof: by
  simp_rw [isCancelMul_iff_forall_isRegular, Commute.isRegular_iff (Commute.all _),
    ← Commute.isRightRegular_iff (Commute.all _)]
  intro n
  have ⟨ms, eq⟩ := f.surj n
  exact (eq ▸ f.map_isRegular (isCancelMul_iff_forall_isRegular.mp ‹_› ms.1)).2.of_mul

中文:
定理 isCancelMul
  条件: (f : Localization映射 S N) [是消去乘法 M]
  结论: 是消去乘法 N
  证明: by
  simp_rw [isCancelMul_iff_forall_isRegular, Commute.isRegular_iff (Commute.all _),
    ← Commute.isRightRegular_iff (Commute.all _)]
  intro n
  have ⟨ms, eq⟩ := f.surj n
  exact (eq ▸ f.map_isRegular (isCancelMul_iff_forall_isRegular.mp ‹_› ms.1)).2.of_mul
-/
@[to_additive] theorem isCancelMul (f : LocalizationMap S N) [IsCancelMul M] : IsCancelMul N := by
  simp_rw [isCancelMul_iff_forall_isRegular, Commute.isRegular_iff (Commute.all _),
    ← Commute.isRightRegular_iff (Commute.all _)]
  intro n
  have ⟨ms, eq⟩ := f.surj n
  exact (eq ▸ f.map_isRegular (isCancelMul_iff_forall_isRegular.mp ‹_› ms.1)).2.of_mul

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [IsCancelMul
  signature: M] : IsCancelMul (Localization S)
  body: (Localization.monoidOf S).isCancelMul

中文:
实例 [是消去乘法
  签名: M] : 是消去乘法 (Localization S)
  定义体: (Localization.monoidOf S).isCancelMul
-/
@[to_additive] instance [IsCancelMul M] : IsCancelMul (Localization S) :=
  (Localization.monoidOf S).isCancelMul

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [IsCancelMul
  signature: M] [Nontrivial M] : Nontrivial (Localization S)
  body: .nontrivial (injective_iff <| Localization.monoidOf S).mpr (fun _ _ => .all _)

中文:
实例 [是消去乘法
  签名: M] [非平凡 M] : 非平凡 (Localization S)
  定义体: .nontrivial (injective_iff <| Localization.monoidOf S).mpr (fun _ _ => .all _)
-/
@[to_additive] instance [IsCancelMul M] [Nontrivial M] : Nontrivial (Localization S) :=
.nontrivial (injective_iff <| Localization.monoidOf S).mpr (fun _ _ => .all _)

/-- Any localization of a cancellative commutative monoid is cancellative. -/
@[to_additive
/-- Any localization of a cancellative commutative additive monoid is cancellative. -/]
/--
Definition of `cancelCommMonoid` / `cancelCommMonoid` 的定义

English:
abbreviation cancelCommMonoid
  signature: {M N} [CancelCommMonoid M] {S : Submonoid M}
  body: f.isCancelMul.mul_left_cancel

中文:
缩写 cancelCommMonoid
  签名: {M N} [消去交换幺半群 M] {S : 子幺半群 M}
  定义体: f.isCancelMul.mul_left_cancel

Depends on / 依赖: f.isCancelMul.mul_left_cancel, isCancelMul, mul_left_cancel
-/
abbrev cancelCommMonoid {M N} [CancelCommMonoid M] {S : Submonoid M}
    [CommMonoid N] (f : S.LocalizationMap N) : CancelCommMonoid N where
  mul_left_cancel := f.isCancelMul.mul_left_cancel

@[to_additive] instance {M} [CancelCommMonoid M] (S : Submonoid M) :
    CancelCommMonoid (Localization S) :=
  (Localization.monoidOf S).cancelCommMonoid

/--
theorem `subsingleton_of_subsingleton` / 定理 `subsingleton_of_subsingleton`

English:
theorem subsingleton_of_subsingleton
  given: (f : LocalizationMap S N) [Subsingleton M]
  proof: by
    obtain ⟨mx, sx, rfl⟩ := f.mk'_surjective x
    obtain ⟨my, sy, rfl⟩ := f.mk'_surjective y
    exact congr(f.mk' $(Subsingleton.elim ..) $(Subsingleton.elim ..))

中文:
定理 subsingleton_of_subsingleton
  条件: (f : Localization映射 S N) [子单例 M]
  证明: by
    obtain ⟨mx, sx, rfl⟩ := f.mk'_surjective x
    obtain ⟨my, sy, rfl⟩ := f.mk'_surjective y
    exact congr(f.mk' $(Subsingleton.elim ..) $(Subsingleton.elim ..))
-/
@[to_additive] theorem subsingleton_of_subsingleton (f : LocalizationMap S N) [Subsingleton M] :
    Subsingleton N where
  allEq x y := by
    obtain ⟨mx, sx, rfl⟩ := f.mk'_surjective x
    obtain ⟨my, sy, rfl⟩ := f.mk'_surjective y
    exact congr(f.mk' $(Subsingleton.elim ..) $(Subsingleton.elim ..))

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [Subsingleton
  signature: M] : Subsingleton (Localization S)
  body: (Localization.monoidOf S).subsingleton_of_subsingleton

中文:
实例 [子单例
  签名: M] : 子单例 (Localization S)
  定义体: (Localization.monoidOf S).subsingleton_of_subsingleton

Depends on / 依赖: Localization, Localization.monoidOf, monoidOf, subsingleton_of_subsingleton
-/
instance [Subsingleton M] : Subsingleton (Localization S) :=
  (Localization.monoidOf S).subsingleton_of_subsingleton

end Submonoid.LocalizationMap
