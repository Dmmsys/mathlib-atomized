...
@[simps! obj_obj map]
/-
**CategoryTheory.PreGaloisCategory.functorToContAction** 是 Mathlib 中的一个定义，位于命名空间
 `CategoryTheory.PreGaloisCategory`。
形式化陈述：functorToContAction : C ⥤ ContAction FintypeCat (Aut F)
该定义给出了上述对象。
...
--- 原说明 ---
Any fiber functor `F` induces an equivalence of categories with the category of 
finite and
discrete `Aut F`-sets.
-/
