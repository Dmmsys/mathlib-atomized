...
/--
Definition of `toFunImpl` / `toFunImpl` 的定义

English:
definition toFunImpl
  signature: (src : Name) (stx : Syntax) (kind : AttributeKind)
  body: do
  ...
  let name := match id with
    | some name =>
      -- A provided name starting with `_root_` disables the namespac

中文:
定义 toFunImpl
  签名: (src : Name) (stx : Syntax) (kind : AttributeKind)
  定义体: do
  let `(attr| to_fun%$tk $optAttr $[$id]?) := stx | throwUnsupportedSyntax
  if (kind != AttributeKind.global) then
    throwError "`to_fun` can only be used as a global attribute"
