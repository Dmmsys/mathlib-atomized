...
中文:
定理 isLittleO_ofReal_left
  条件: {f : α -> 实数} {g : α -> E}
  结论: (f · : α -> Complex) =o[l] g ↔ f =o[l] g
...
中文:
定理 isLittleO_ofReal_right
  条件: {f : α -> E} {g : α -> 实数}
  结论: f =o[l] (g · : α -> Complex) ↔ f =o[l] g
...
中文:
定理 isTheta_ofReal_left
  条件: {f : α -> 实数} {g : α -> E}
  结论: (f · : α -> Complex) =Θ[l] g ↔ f =Θ[l] g
...
中文:
定理 isTheta_ofReal_right
  条件: {f : α -> E} {g : α -> 实数}
  结论: f =Θ[l] (g · : α -> Complex) ↔ f =Θ[l] g
...
中文:
引理 isBigO_comp_ofReal_nhds
  条件: {f g : Complex -> Complex} {x : 实数} (h : f =O[𝓝 (x : Complex)] g)
...
引理 isBigO_re_sub_re
  条件: {z : Complex}
  结论: (fun (w : Complex) => w.re - z.re) =O[𝓝 z] fun w => w - z
...
引理 isBigO_im_sub_im
  条件: {z : Complex}
  结论: (fun (w : Complex) => w.im - z.im) =O[𝓝 z] fun w => w - z
...
中文:
引理 Int.cast_complex_isTheta_cast_real
  结论: 整数.cast (R := Complex) =Θ[cofinite] 整数.cast (R := 实数)
  证明: by
  apply Asymptotics.IsTheta.of_norm_eventuallyEq_norm
  filter_upwards with n using by simp
