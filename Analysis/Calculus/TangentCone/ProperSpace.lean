...
/--
theorem `tangentConeAt_nonempty_of_properSpace` / 定理 `tangentConeAt_nonempty_of_properSpace`

English:
theorem tangentConeAt_nonempty_of_properSpace
  ...
  obtain ⟨u, -, u_pos, u_lim⟩ :
      exists u, StrictAnti u ∧ (forall (n : Nat), 0 < u n) ∧ Tendsto u atTop (𝓝 (0 : Real)) :=
    exists_seq_strictAnti_tendsto (0 : Real)
  have A n : exists y in closedBall x (u n) inter s, y != x :=
    accPt_iff_nhds.mp hx _ (closedBall_mem_nhds _ (u_pos n))
  choose v hv hvx using A
  choose hvu hvs using hv
  let d := fun n => v n - x
  have M n : x + d n in s \ {x} := by simp [d, hvs, hvx]
  ...
  obtain ⟨l, l_mem, φ, φ_strict, hφ⟩ :
      exists l in Metric.closedBall (0 : E) 1 \ Metric.ball (0 : E) (1 / ‖r‖),
      exists (φ : Nat -> Nat), StrictMono φ ∧ Tendsto ((fun n => c n • d n) ∘ φ) atTop (𝓝 l) := by
...
theorem tangentConeAt_nonempty_of_properSpace [ProperSpace E]
    ...
    (tangentConeAt 𝕜 s x inter {0}ᶜ).Nonempty := by
  /- Take a sequence `d n` tending to `0` such that `x + d n ∈ s`. Taking `c n` of the order
  of `1 / d n`. Then `c n • d n` belongs to a fixed annulus. By compactness, one can extract
  a subsequence converging to a limit `l`. Then `l` is nonzero, and by definition it belongs to
  the tangent cone. -/
