...
@[simps]
/-
**CategoryTheory.Presheaf.truth** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Preshe
af`。
形式化陈述：(C : Type u) →   [inst : CategoryTheory.Category.{v, u} C] →     (Category
Theory.Functor.const Cᵒᵖ).obj PUnit.{(max u v) + 1} ⟶ CategoryTheory.Functor.sie
ves C
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The truth morphism in the category of presheaves. At each component `X : C`, it 
is the constant
map returning `⊤ : Sieve X`.
-/
...
@[simps app]
/-
**CategoryTheory.Presheaf.** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The characteristic map of an inclusion of presheaves.
Given a monomorphism of sheaves `m : F ⟶ G`, an object X of the site, map an ele
ment `x : G(X)`
to the (closed) sieve on X where `f : Y → X` is in the sieve iff
  `∃ a ∈ F(Y), G(f)(x) = m_Y(a)`
-/
...
/-
**CategoryTheory.Presheaf.comp_** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.Presheaf.isPullback_** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory`
。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Presheaf.** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps! Ω truth Ω₀ χ χ₀]
/-
**CategoryTheory.Presheaf.classifier** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.P
resheaf`。
形式化陈述：(C : Type u) →   [inst : CategoryTheory.Category.{v, u} C] →     CategoryT
heory.Subobject.Classifier (CategoryTheory.Functor Cᵒᵖ (Type (max u v)))
参数：Type (max u v)。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.Presheaf.isPullback_χ_truth`：∀ {C : Type u} [inst : Categ
oryTheory.Category.{v, u} C] {F G : CategoryTheory.Functor Cᵒᵖ (Type (max u v))}
 (m : F ⟶ G)   [CategoryTheory.M…
· 使用定理 `CategoryTheory.Presheaf.χ_unique`：∀ {C : Type u} [inst : CategoryTheory.
Category.{v, u} C] {F G : CategoryTheory.Functor Cᵒᵖ (Type (max u v))} (m : F ⟶ 
G)   (χ' : G ⟶ Categor…
...
open Presheaf in
/-
**CategoryTheory.GrothendieckTopology.isClosed_** 是 Mathlib 中的一个引理，位于命名空间 `Categ
oryTheory`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps]
/-
**CategoryTheory.Sheaf.** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Sheaf`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The sheaf of closed sieves w/r/t `J`. See also `Functor.closedSieves` and `Sheaf
.classifier`
-/
...
@[simps]
/-
**CategoryTheory.Sheaf.truth** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Sheaf`。
形式化陈述：truth (J : GrothendieckTopology C) : Sheaf.terminal J (Types.isTerminalPUn
it) ⟶ Sheaf.Ω J where hom
...
--- 原说明 ---
Given a monomorphism of sheaves `η : F ⟶ G`, an object X of the site, map an ele
ment `x : G(X)`
to the (closed) sieve on X where `f : Y → X` is in the sieve iff
  `∃ a ∈ F(Y), G(f)(x) = η_Y(a)`
-/
...
set_option backward.defeqAttrib.useBackward true in
/-
**CategoryTheory.Sheaf.isPullback_** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory.She
af`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**CategoryTheory.Sheaf.** 是 Mathlib 中的一个引理，位于命名空间 `CategoryTheory.Sheaf`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simps! Ω truth Ω₀ χ χ₀]
/-
**CategoryTheory.Sheaf.classifier** 是 Mathlib 中的一个定义，位于命名空间 `CategoryTheory.Shea
f`。
形式化陈述：classifier (J : GrothendieckTopology C) : Subobject.Classifier (Sheaf J (T
ype max u v))
参数：J : GrothendieckTopology C。
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用引理 `CategoryTheory.Sheaf.isPullback_χ_truth`：isPullback_χ_truth (m : F ⟶ G) 
[Mono m] : IsPullback m ((isTerminalTerminal J _).from F) (Sheaf.χ m) (Sheaf.tru
th J)
· 使用引理 `CategoryTheory.Sheaf.χ_unique`：χ_unique (m : F ⟶ G) [Mono m] (χ' : G ⟶ S
heaf.Ω J) (hχ' : IsPullback m ((isTerminalTerminal J _).from F) χ' (Sheaf.truth 
J)) : χ' = Sheaf.χ …

--- 原说明 ---
A construction of a subobject classifier for sheaf categories. `Ω` is the sheaf 
of closed sieves,
and `truth` maps for each object `X : C`, an element of `PUnit` to the maximal `
Sieve X`, which is
always closed.
-/
