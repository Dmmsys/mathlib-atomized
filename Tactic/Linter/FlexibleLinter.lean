...
/-
**Mathlib.Linter.flexible** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter`。
...
/-
**Lean.Elab.TacticInfo.goalsTargetedBy** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Elab.Tact
icInfo`。
形式化陈述：goalsTargetedBy (t : TacticInfo) : List MVarId
参数：t : TacticInfo。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`goalsTargetedBy t` are the `MVarId`s before the `TacticInfo` `t` that "disappea
r" after it.
They should correspond to the goals in which the tactic `t` performs some action
.
-/
...
/-
**Lean.Elab.TacticInfo.goalsCreatedBy** 是 Mathlib 中的一个定义，位于命名空间 `Lean.Elab.Tacti
cInfo`。
形式化陈述：goalsCreatedBy (t : TacticInfo) : List MVarId
参数：t : TacticInfo。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`goalsCreatedBy t` are the `MVarId`s after the `TacticInfo` `t` that were not pr
esent before.
They should correspond to the goals created or changed by the tactic `t`.
-/
...
/-
**Mathlib.Linter.Flexible.TacticData** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Linter.F
lexible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.Flexible.extractTacticData** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.Flexible`。
形式化陈述：extractTacticData (tree : InfoTree) : Array TacticData
参数：tree : InfoTree。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.Flexible.Stained** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathlib.Linter.Fl
exible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.Flexible.** 是 Mathlib 中的一个实例，位于命名空间 `Mathlib.Linter.Flexible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.Flexible.toStained** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Fl
exible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`toStained stx` scans the input `Syntax` `stx` extracting identifiers and atoms,
 making an effort
to convert them to `Stained`.
The function is used to extract "location" information about `stx`: either expli
cit locations as in
`rw [] at locations` or implicit ones as `rw [h]`.

Whether or not what this function extracts really is a location will be determin
ed by the linter
using data embedded in the `InfoTree`s.
-/
...
partial
/-
**Mathlib.Linter.Flexible.getStained** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.F
lexible`。
形式化陈述：getStained (stx : Syntax) (all? : Syntax -> Bool
参数：stx : Syntax。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
This means that tactics that do not have an explicit "`at`" in their syntax will
 be treated as
acting on the main goal.
-/
def getStained! (stx : Syntax) (all? : Syntax → Bool := fun _ ↦ false) : Std.HashSet Stained :=
...
-/
/-
**Mathlib.Linter.Flexible.Stained.toFMVarId** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.L
inter.Flexible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`Stained.toFMVarId mv lctx st` takes a metavariable `mv`, a local context `lctx`
 and
a `Stained` `st` and returns the array of pairs `(FVarId, mv)`s that `lctx` assi
gns to `st`
(the second component is always `mv`):
* if `st` "is" a `Name`, returns the singleton of the `FVarId` with the name car
ried by `st`;
* if `st` is `.goal`, returns the singleton `#[default]`;
* if `st` is `.wildcard`, returns the array of all the `FVarId`s in `lctx` with 
also `default`
  (to keep track of the `goal`).
-/
...
-/
/-
**Mathlib.Linter.Flexible.stoppers** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Fle
xible`。
形式化陈述：stoppers : Std.HashSet Name
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Linter.Flexible.flexible** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Fle
xible`。
形式化陈述：flexible : Std.HashSet Name
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`SyntaxNodeKind`s that are allowed to follow a flexible tactic:
  `simp`, `simp_all`, `simpa`, `dsimp`, `grind`, `constructor`, `congr`, `done`,
 `rfl`, `ac_rfl`,
  `omega` and `lia`, `grobner`
  `abel` and `abel!`, `group`, `ring` and `ring!`, `module`, `field_simp` and `f
ield`, `norm_num`,
  `linarith`, `nlinarith` and `nlinarith!`, `norm_cast`, `tauto`,
  `aesop`, `cfc_tac` (and `cfc_zero_tac` and `cfc_cont_tac`),
  `continuity` and `measurability`, `finiteness`, `finiteness?`,
...
/-
**Mathlib.Linter.Flexible.usesGoal** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter.Fle
xible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
By default, if a `SyntaxNodeKind` is not special-cased here, then the linter ass
umes that
the tactic will use the goal as well: this heuristic works well with `exact`, `r
efine`, `apply`.
For tactics such as `cases` this is not true: for these tactics, `usesGoal?` yie
lds `false`.
-/
...
/-
**Mathlib.Linter.Flexible.getFVarIdCandidates** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Linter.Flexible`。
形式化陈述：getFVarIdCandidates (fv : FVarId) (name : Name) (lctx : LocalContext) : Ar
ray FVarId
参数：fv : FVarId；name : Name；lctx : LocalContext。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`getFVarIdCandidates fv name lctx` takes an input an `FVarId`, a `Name` and a `L
ocalContext`.
It returns an array of guesses for a "best fit" `FVarId` in the given `LocalCont
ext`.
The first entry of the array is the input `FVarId` `fv`, if it is present.
The next entry of the array is the `FVarId` with the given `Name`, if present.

Usually, the first entry of the returned array is "the best approximation" to `(
fv, name)`.
-/
...
/-
**Mathlib.Linter.Flexible.persistFVars** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linter
.Flexible`。
形式化陈述：persistFVars (fv : FVarId) (before after : LocalContext) : FVarId
参数：fv : FVarId；before after : LocalContext。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`persistFVars` is one step in persisting: track a single `FVarId` between two `L
ocalContext`s.
If an `FVarId` with the same unique name exists in the new context, use it.
Otherwise, if an `FVarId` with the same `userName` exists in the new context, us
e it.
If both of these fail, return `default` (i.e. "fail").
-/
...
/-
**Mathlib.Linter.Flexible.reallyPersist** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Linte
r.Flexible`。
形式化陈述：reallyPersist (fmvars : Array (FVarId × MVarId)) (mvs0 mvs1 : List MVarId)
 (ctx0 ctx1 : MetavarContext) : Array (FVarId × MVarId)
参数：fmvars : Array (FVarId × MVarId)；mvs0 mvs1 : List MVarId；ctx0 ctx1 : MetavarC
ontext。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`reallyPersist` converts an array of pairs `(fvar, mvar)` to another array of th
e same type.
-/
def reallyPersist
...
/-
**Mathlib.Linter.Flexible.StainData** 是 Mathlib 中的一个结构，位于命名空间 `Mathlib.Linter.Fl
exible`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.Flexible.generateSimpSuggestion** 是 Mathlib 中的一个定义，位于命名空间 `Math
lib.Linter.Flexible`。
形式化陈述：generateSimpSuggestion (stainData : StainData) (stainStx : Syntax) : CoreM
 (Option Syntax)
参数：stainData : StainData；stainStx : Syntax。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Generate a "simp only [...]" suggestion for a simp/simpAll tactic.
Returns `none` if the tactic is not simp/simpAll or if suggestion generation fai
ls.
-/
...
/-
**Mathlib.Linter.Flexible.flexibleLinter** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Lint
er.Flexible`。
形式化陈述：flexibleLinter : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
