...
/-
**LinearMap.liftQ** 是 Mathlib 中的一个定义，位于命名空间 `LinearMap`。
...
@[simp]
/-
**LinearMap.liftQ** 是 Mathlib 中的一个引理，位于命名空间 `LinearMap`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**LinearMap.IsRefl.liftQ** 是 Mathlib 中的一个缩写定义，位于命名空间 `LinearMap`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Special case of `LinearMap.liftQ₂` with left and right spaces the same. Reducibl
e so
that simp lemmas about `LinearMap.liftQ₂` apply to it.
-/
