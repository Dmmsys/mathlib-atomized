...
lemma term_welldef
  ...
  · exact (rpow_pos_of_pos ((Nat.cast_pos.mpr hn).tran
...
lemma termSum_one
  ...
  | succ N hN =>
    ...
    rw [Finset.sum_range_succ]; rw [hN]; rw [harmonic_
...
lemma tendsto_riemannZeta_sub_one_div_nhds_right
  ...
  suffices Tendsto (fun s : Real => (∑' n : Nat, 1 / (n + 1 : Real) ^ s) - 1 / (s - 1))
    ...
    simp only [Function.comp_apply, Complex.ofReal_sub, Complex.ofRea
...
lemma tendsto_Gamma_term_aux
  ...
  rw [GammaReal_one]; rw [div_one] at t
...
lemma differentiable_riemannZeta₀
  ...
  constructor
  · refine .congr (f := fun s => riemannZeta s - (s - 1)⁻¹) ?_ (by simp +contextual [r
...
lemma deriv_riemannZeta_eq_neg_inv_sub_sq_add
  ...
  · rw [deriv_fun_add (by fun_prop) (by fun_prop), deriv_fun_inv'' (by fun_prop) (by exact this)]
    simp [field]
  · filter_upwards [compl_singleton_mem_nhds hs] using by grind [riemannZ
...
lemma log_deriv_riemannZeta_add_inv_sub_sub_isBigO
  ...
  suffices (fun s => (deriv riemannZeta₁ s) / (riemannZeta₁ s) - γ) =O[𝓝 1] (· - 1) by
    ...
    simp +contextual
  suffices DifferentiableAt Complex (fun s => (deriv riemannZeta₁ s) /
