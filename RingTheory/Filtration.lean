/-
Copyright (c) 2022 Andrew Yang. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Andrew Yang
-/
module

public import Mathlib.Algebra.Polynomial.Module.Basic
public import Mathlib.RingTheory.Finiteness.Nakayama
public import Mathlib.RingTheory.LocalRing.MaximalIdeal.Basic
public import Mathlib.RingTheory.ReesAlgebra

/-!

# `I`-filtrations of modules

This file contains the definitions and basic results around (stable) `I`-filtrations of modules.

## Main results

- `Ideal.Filtration`:
  An `I`-filtration on the module `M` is a sequence of decreasing submodules `N i` such that
  `∀ i, I • (N i) ≤ N (i + 1)`. Note that we do not require the filtration to start from `⊤`.
- `Ideal.Filtration.Stable`: An `I`-filtration is stable if `I • (N i) = N (i + 1)` for large
  enough `i`.
- `Ideal.Filtration.submodule`: The associated module `⨁ Nᵢ` of a filtration, implemented as a
  submodule of `M[X]`.
- `Ideal.Filtration.submodule_fg_iff_stable`: If `F.N i` are all finitely generated, then
  `F.Stable` iff `F.submodule.FG`.
- `Ideal.Filtration.Stable.of_le`: In a finite module over a Noetherian ring,
  if `F' ≤ F`, then `F.Stable → F'.Stable`.
- `Ideal.exists_pow_inf_eq_pow_smul`: **Artin-Rees lemma**.
  given `N ≤ M`, there exists a `k` such that `IⁿM ⊓ N = Iⁿ⁻ᵏ(IᵏM ⊓ N)` for all `n ≥ k`.
- `Ideal.iInf_pow_eq_bot_of_isLocalRing`:
  **Krull's intersection theorem** (`⨅ i, I ^ i = ⊥`) for Noetherian local rings.
- `Ideal.iInf_pow_eq_bot_of_isDomain`:
  **Krull's intersection theorem** (`⨅ i, I ^ i = ⊥`) for Noetherian domains.

-/

@[expose] public section

variable {R M : Type*} [CommRing R] [AddCommGroup M] [Module R M] (I : Ideal R)

open Polynomial

open scoped Polynomial

/-- An `I`-filtration on the module `M` is a sequence of decreasing submodules `N i` such that
`I • (N i) ≤ N (i + 1)`. Note that we do not require the filtration to start from `⊤`. -/
@[ext]
/--
Definition of `Ideal.Filtration` / `Ideal.Filtration` 的定义

English:
structure Ideal.Filtration
  parameters: (M : Type*) [AddCommGroup M] [Module R M]
  axioms and operations (3):
    - N : Nat -> Submodule R M
    - mono : forall i, N (i + 1) <= N i
    - smul_le : forall i, I • N i <= N (i + 1)

中文:
结构 理想.滤子
  参数: (M : 类型) [加法交换群 M] [模 R M]
  公理与运算 (3 个):
    - N : 自然数 -> 子模 R M
    - mono : 对任意 i, N (i + 1) <= N i
    - smul_le : 对任意 i, I • N i <= N (i + 1)
-/
structure Ideal.Filtration (M : Type*) [AddCommGroup M] [Module R M] where
  N : Nat -> Submodule R M
  mono : forall i, N (i + 1) <= N i
  smul_le : forall i, I • N i <= N (i + 1)

variable (F F' : I.Filtration M) {I}

namespace Ideal.Filtration

/--
theorem `pow_smul_le` / 定理 `pow_smul_le`

English:
theorem pow_smul_le
  given: (i j : Nat)
  statement: I ^ i • F.N j <= F.N (i + j)
  proof: by
  induction i with
  | zero => simp
  | succ _ ih =>
    rw [pow_succ']; rw [mul_smul]; rw [add_assoc]; rw [add_comm 1]; rw [← add_assoc]
    exact (smul_mono_right _ ih).trans (F.smul_le _)

中文:
定理 pow_smul_le
  条件: (i j : 自然数)
  结论: I ^ i • F.N j <= F.N (i + j)
  证明: by
  induction i with
  | zero => simp
  | succ _ ih =>
    rw [pow_succ']; rw [mul_smul]; rw [add_assoc]; rw [add_comm 1]; rw [← add_assoc]
    exact (smul_mono_right _ ih).trans (F.smul_le _)

Depends on / 依赖: F.smul_le, add_assoc, add_comm, mul_smul, pow_succ, smul_le, smul_mono_right
-/
theorem pow_smul_le (i j : Nat) : I ^ i • F.N j <= F.N (i + j) := by
  induction i with
  | zero => simp
  | succ _ ih =>
    rw [pow_succ']; rw [mul_smul]; rw [add_assoc]; rw [add_comm 1]; rw [← add_assoc]
    exact (smul_mono_right _ ih).trans (F.smul_le _)

/--
theorem `pow_smul_le_pow_smul` / 定理 `pow_smul_le_pow_smul`

English:
theorem pow_smul_le_pow_smul
  given: (i j k : Nat)
  statement: I ^ (i + k) • F.N j <= I ^ k • F.N (i + j)
  proof: by
  rw [add_comm]; rw [pow_add]; rw [mul_smul]
  exact smul_mono_right _ (F.pow_smul_le i j)

中文:
定理 pow_smul_le_pow_smul
  条件: (i j k : 自然数)
  结论: I ^ (i + k) • F.N j <= I ^ k • F.N (i + j)
  证明: by
  rw [add_comm]; rw [pow_add]; rw [mul_smul]
  exact smul_mono_right _ (F.pow_smul_le i j)

Depends on / 依赖: F.pow_smul_le, add_comm, mul_smul, pow_add, pow_smul_le, smul_mono_right
-/
theorem pow_smul_le_pow_smul (i j k : Nat) : I ^ (i + k) • F.N j <= I ^ k • F.N (i + j) := by
  rw [add_comm]; rw [pow_add]; rw [mul_smul]
  exact smul_mono_right _ (F.pow_smul_le i j)

/--
theorem `antitone` / 定理 `antitone`

English:
theorem antitone
  statement: Antitone F.N
  proof: antitone_nat_of_succ_le F.mono

中文:
定理 antitone
  结论: 递减 F.N
  证明: antitone_nat_of_succ_le F.mono
-/
protected theorem antitone : Antitone F.N :=
  antitone_nat_of_succ_le F.mono

/-- The trivial `I`-filtration of `N`. -/
@[simps]
/--
Definition of `_root_.Ideal.trivialFiltration` / `_root_.Ideal.trivialFiltration` 的定义

English:
definition _root_.Ideal.trivialFiltration
  signature: (I : Ideal R) (N : Submodule R M)
  body: N
  mono _ := le_rfl
  smul_le _ := Submodule.smul_le_right

中文:
定义 _root_.理想.trivialFiltration
  签名: (I : 理想 R) (N : 子模 R M)
  定义体: N
  mono _ := le_rfl
  smul_le _ := Submodule.smul_le_right
-/
def _root_.Ideal.trivialFiltration (I : Ideal R) (N : Submodule R M) : I.Filtration M where
  N _ := N
  mono _ := le_rfl
  smul_le _ := Submodule.smul_le_right

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: Max (I.Filtration M)
  body: ⟨fun F F' =>
    ⟨F.N ⊔ F'.N, fun i => sup_le_sup (F.mono i) (F'.mono i), fun i =>
(Submodule.smul_sup _ _ _).trans_le sup_le_sup (F.smul_le i) (F'.smul_le i)⟩⟩

中文:
实例 :
  签名: 最大值 (I.滤子 M)
  定义体: ⟨fun F F' =>
    ⟨F.N ⊔ F'.N, fun i => sup_le_sup (F.mono i) (F'.mono i), fun i =>
(Submodule.smul_sup _ _ _).trans_le sup_le_sup (F.smul_le i) (F'.smul_le i)⟩⟩

Depends on / 依赖: F.mono, F.smul_le, Submodule, Submodule.smul_sup, smul_le, smul_sup, sup_le_sup, trans_le
-/
instance : Max (I.Filtration M) :=
  ⟨fun F F' =>
    ⟨F.N ⊔ F'.N, fun i => sup_le_sup (F.mono i) (F'.mono i), fun i =>
(Submodule.smul_sup _ _ _).trans_le sup_le_sup (F.smul_le i) (F'.smul_le i)⟩⟩

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: SupSet (I.Filtration M)
  body: ⟨fun S =>
    { N := sSup (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sSup_le_sSup_of_isCofinalFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sSup_eq_iSup']; rw [iSup_apply]; rw [Submodu

中文:
实例 :
  签名: 上确界集 (I.滤子 M)
  定义体: ⟨fun S =>
    { N := sSup (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sSup_le_sSup_of_isCofinalFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sSup_eq_iSup']; rw [iSup_apply]; rw [Submodu

Depends on / 依赖: F.mono, F.smul_le, Filtration, Ideal.Filtration.N, Submodule, Submodule.smul_iSup, iSup_apply, iSup_mono, sSup_eq_iSup, sSup_le_sSup_of_isCofinalFor, smul_iSup, smul_le
-/
instance : SupSet (I.Filtration M) :=
  ⟨fun S =>
    { N := sSup (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sSup_le_sSup_of_isCofinalFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sSup_eq_iSup']; rw [iSup_apply]; rw [Submodule.smul_iSup]; rw [iSup_apply]
        apply iSup_mono _
        rintro ⟨_, F, hF, rfl⟩
        exact F.smul_le i }⟩

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: Min (I.Filtration M)
  body: ⟨fun F F' =>
    ⟨F.N ⊓ F'.N, fun i => inf_le_inf (F.mono i) (F'.mono i), fun i =>
(smul_inf_le _ _ _).trans inf_le_inf (F.smul_le i) (F'.smul_le i)⟩⟩

中文:
实例 :
  签名: 最小值 (I.滤子 M)
  定义体: ⟨fun F F' =>
    ⟨F.N ⊓ F'.N, fun i => inf_le_inf (F.mono i) (F'.mono i), fun i =>
(smul_inf_le _ _ _).trans inf_le_inf (F.smul_le i) (F'.smul_le i)⟩⟩

Depends on / 依赖: F.mono, F.smul_le, inf_le_inf, smul_inf_le, smul_le
-/
instance : Min (I.Filtration M) :=
  ⟨fun F F' =>
    ⟨F.N ⊓ F'.N, fun i => inf_le_inf (F.mono i) (F'.mono i), fun i =>
(smul_inf_le _ _ _).trans inf_le_inf (F.smul_le i) (F'.smul_le i)⟩⟩

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: InfSet (I.Filtration M)
  body: ⟨fun S =>
    { N := sInf (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sInf_le_sInf_of_isCoinitialFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sInf_eq_iInf']; rw [iInf_apply]; rw [iInf_

中文:
实例 :
  签名: 下确界集 (I.滤子 M)
  定义体: ⟨fun S =>
    { N := sInf (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sInf_le_sInf_of_isCoinitialFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sInf_eq_iInf']; rw [iInf_apply]; rw [iInf_

Depends on / 依赖: F.mono, F.smul_le, Filtration, Ideal.Filtration.N, iInf_apply, iInf_mono, sInf_eq_iInf, sInf_le_sInf_of_isCoinitialFor, smul_iInf_le, smul_iInf_le.trans, smul_le
-/
instance : InfSet (I.Filtration M) :=
  ⟨fun S =>
    { N := sInf (Ideal.Filtration.N '' S)
      mono := fun i => by
        apply sInf_le_sInf_of_isCoinitialFor _
        rintro _ ⟨⟨_, F, hF, rfl⟩, rfl⟩
        exact ⟨_, ⟨⟨_, F, hF, rfl⟩, rfl⟩, F.mono i⟩
      smul_le := fun i => by
        rw [sInf_eq_iInf']; rw [iInf_apply]; rw [iInf_apply]
        refine smul_iInf_le.trans ?_
        apply iInf_mono _
        rintro ⟨_, F, hF, rfl⟩
        exact F.smul_le i }⟩

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: Top (I.Filtration M)
  body: ⟨I.trivialFiltration ⊤⟩

中文:
实例 :
  签名: 顶元素 (I.滤子 M)
  定义体: ⟨I.trivialFiltration ⊤⟩

Depends on / 依赖: I.trivialFiltration, trivialFiltration
-/
instance : Top (I.Filtration M) :=
  ⟨I.trivialFiltration ⊤⟩

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: Bot (I.Filtration M)
  body: ⟨I.trivialFiltration ⊥⟩

@[simp]

中文:
实例 :
  签名: 底元素 (I.滤子 M)
  定义体: ⟨I.trivialFiltration ⊥⟩

@[simp]

Depends on / 依赖: I.trivialFiltration, trivialFiltration
-/
instance : Bot (I.Filtration M) :=
  ⟨I.trivialFiltration ⊥⟩

@[simp]
/--
theorem `sup_N` / 定理 `sup_N`

English:
theorem sup_N
  statement: (F ⊔ F').N = F.N ⊔ F'.N
  proof: rfl

@[simp]

中文:
定理 sup_N
  结论: (F ⊔ F').N = F.N ⊔ F'.N
  证明: rfl

@[simp]
-/
theorem sup_N : (F ⊔ F').N = F.N ⊔ F'.N :=
  rfl

@[simp]
/--
theorem `sSup_N` / 定理 `sSup_N`

English:
theorem sSup_N
  given: (S : Set (I.Filtration M))
  statement: (sSup S).N = sSup (Ideal.Filtration.N '' S)
  proof: rfl

@[simp]

中文:
定理 sSup_N
  条件: (S : 集合 (I.滤子 M))
  结论: (sSup S).N = sSup (理想.滤子.N '' S)
  证明: rfl

@[simp]
-/
theorem sSup_N (S : Set (I.Filtration M)) : (sSup S).N = sSup (Ideal.Filtration.N '' S) :=
  rfl

@[simp]
/--
theorem `inf_N` / 定理 `inf_N`

English:
theorem inf_N
  statement: (F ⊓ F').N = F.N ⊓ F'.N
  proof: rfl

@[simp]

中文:
定理 inf_N
  结论: (F ⊓ F').N = F.N ⊓ F'.N
  证明: rfl

@[simp]
-/
theorem inf_N : (F ⊓ F').N = F.N ⊓ F'.N :=
  rfl

@[simp]
/--
theorem `sInf_N` / 定理 `sInf_N`

English:
theorem sInf_N
  given: (S : Set (I.Filtration M))
  statement: (sInf S).N = sInf (Ideal.Filtration.N '' S)
  proof: rfl

@[simp]

中文:
定理 sInf_N
  条件: (S : 集合 (I.滤子 M))
  结论: (sInf S).N = sInf (理想.滤子.N '' S)
  证明: rfl

@[simp]
-/
theorem sInf_N (S : Set (I.Filtration M)) : (sInf S).N = sInf (Ideal.Filtration.N '' S) :=
  rfl

@[simp]
/--
theorem `top_N` / 定理 `top_N`

English:
theorem top_N
  statement: (⊤ : I.Filtration M).N = ⊤
  proof: rfl

@[simp]

中文:
定理 top_N
  结论: (⊤ : I.滤子 M).N = ⊤
  证明: rfl

@[simp]
-/
theorem top_N : (⊤ : I.Filtration M).N = ⊤ :=
  rfl

@[simp]
/--
theorem `bot_N` / 定理 `bot_N`

English:
theorem bot_N
  statement: (⊥ : I.Filtration M).N = ⊥
  proof: rfl

@[simp]

中文:
定理 bot_N
  结论: (⊥ : I.滤子 M).N = ⊥
  证明: rfl

@[simp]
-/
theorem bot_N : (⊥ : I.Filtration M).N = ⊥ :=
  rfl

@[simp]
/--
theorem `iSup_N` / 定理 `iSup_N`

English:
theorem iSup_N
  given: {ι : Sort*} (f : ι -> I.Filtration M)
  statement: (iSup f).N = ⨆ i, (f i).N
  proof: congr_arg sSup (Set.range_comp _ _).symm

@[simp]

中文:
定理 iSup_N
  条件: {ι : 类型层*} (f : ι -> I.滤子 M)
  结论: (iSup f).N = ⨆ i, (f i).N
  证明: congr_arg sSup (Set.range_comp _ _).symm

@[simp]

Depends on / 依赖: Set.range_comp, congr_arg, range_comp
-/
theorem iSup_N {ι : Sort*} (f : ι -> I.Filtration M) : (iSup f).N = ⨆ i, (f i).N :=
  congr_arg sSup (Set.range_comp _ _).symm

@[simp]
/--
theorem `iInf_N` / 定理 `iInf_N`

English:
theorem iInf_N
  given: {ι : Sort*} (f : ι -> I.Filtration M)
  statement: (iInf f).N = ⨅ i, (f i).N
  proof: congr_arg sInf (Set.range_comp _ _).symm

中文:
定理 iInf_N
  条件: {ι : 类型层*} (f : ι -> I.滤子 M)
  结论: (iInf f).N = ⨅ i, (f i).N
  证明: congr_arg sInf (Set.range_comp _ _).symm

Depends on / 依赖: Set.range_comp, congr_arg, range_comp
-/
theorem iInf_N {ι : Sort*} (f : ι -> I.Filtration M) : (iInf f).N = ⨅ i, (f i).N :=
  congr_arg sInf (Set.range_comp _ _).symm

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: PartialOrder (I.Filtration M)
  body: PartialOrder.lift _ fun _ _ => Ideal.Filtration.ext

中文:
实例 :
  签名: 偏序 (I.滤子 M)
  定义体: PartialOrder.lift _ fun _ _ => Ideal.Filtration.ext

Depends on / 依赖: Filtration, Ideal.Filtration.ext, PartialOrder, PartialOrder.lift
-/
instance : PartialOrder (I.Filtration M) :=
  PartialOrder.lift _ fun _ _ => Ideal.Filtration.ext

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: CompleteLattice (I.Filtration M)
  body: Function.Injective.completeLattice Ideal.Filtration.N
    (fun _ _ => Ideal.Filtration.ext) .rfl .rfl sup_N inf_N
    (fun _ => sSup_image) (fun _ => sInf_image) top_N bot_N

中文:
实例 :
  签名: 完备格 (I.滤子 M)
  定义体: Function.Injective.completeLattice Ideal.Filtration.N
    (fun _ _ => Ideal.Filtration.ext) .rfl .rfl sup_N inf_N
    (fun _ => sSup_image) (fun _ => sInf_image) top_N bot_N

Depends on / 依赖: Filtration, Function, Function.Injective.completeLattice, Ideal.Filtration.N, Ideal.Filtration.ext, Injective, bot_N, completeLattice, inf_N, sInf_image, sSup_image, sup_N, top_N
-/
instance : CompleteLattice (I.Filtration M) :=
  Function.Injective.completeLattice Ideal.Filtration.N
    (fun _ _ => Ideal.Filtration.ext) .rfl .rfl sup_N inf_N
    (fun _ => sSup_image) (fun _ => sInf_image) top_N bot_N

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: Inhabited (I.Filtration M)
  body: ⟨⊥⟩

中文:
实例 :
  签名: 可居 (I.滤子 M)
  定义体: ⟨⊥⟩
-/
instance : Inhabited (I.Filtration M) :=
  ⟨⊥⟩

/--
Definition of `Stable` / `Stable` 的定义

English:
definition Stable
  signature: : Prop
  body: exists n₀, forall n >= n₀, I • F.N n = F.N (n + 1)

中文:
定义 稳定
  签名: : 命题
  定义体: exists n₀, forall n >= n₀, I • F.N n = F.N (n + 1)
-/
def Stable : Prop :=
  exists n₀, forall n >= n₀, I • F.N n = F.N (n + 1)

/-- The trivial stable `I`-filtration of `N`. -/
@[simps]
/--
Definition of `_root_.Ideal.stableFiltration` / `_root_.Ideal.stableFiltration` 的定义

English:
definition _root_.Ideal.stableFiltration
  signature: (I : Ideal R) (N : Submodule R M)
  body: I ^ i • N
  mono i := by rw [add_comm, pow_add, mul_smul]; exact Submodule.smul_le_right
  smul_le i := by rw [add_comm, pow_add, mul_smul, pow_one]

中文:
定义 _root_.理想.stableFiltration
  签名: (I : 理想 R) (N : 子模 R M)
  定义体: I ^ i • N
  mono i := by rw [add_comm, pow_add, mul_smul]; exact Submodule.smul_le_right
  smul_le i := by rw [add_comm, pow_add, mul_smul, pow_one]
-/
def _root_.Ideal.stableFiltration (I : Ideal R) (N : Submodule R M) : I.Filtration M where
  N i := I ^ i • N
  mono i := by rw [add_comm, pow_add, mul_smul]; exact Submodule.smul_le_right
  smul_le i := by rw [add_comm, pow_add, mul_smul, pow_one]

set_option backward.defeqAttrib.useBackward true in
/--
theorem `_root_.Ideal.stableFiltration_stable` / 定理 `_root_.Ideal.stableFiltration_stable`

English:
theorem _root_.Ideal.stableFiltration_stable
  given: (I : Ideal R) (N : Submodule R M)
  proof: by
  use 0
  intro n _
  dsimp
  rw [add_comm]; rw [pow_add]; rw [mul_smul]; rw [pow_one]

中文:
定理 _root_.理想.stableFiltration_stable
  条件: (I : 理想 R) (N : 子模 R M)
  证明: by
  use 0
  intro n _
  dsimp
  rw [add_comm]; rw [pow_add]; rw [mul_smul]; rw [pow_one]

Depends on / 依赖: add_comm, mul_smul, pow_add, pow_one
-/
theorem _root_.Ideal.stableFiltration_stable (I : Ideal R) (N : Submodule R M) :
    (I.stableFiltration N).Stable := by
  use 0
  intro n _
  dsimp
  rw [add_comm]; rw [pow_add]; rw [mul_smul]; rw [pow_one]

variable {F F'}

/--
theorem `Stable.exists_pow_smul_eq` / 定理 `Stable.exists_pow_smul_eq`

English:
theorem Stable.exists_pow_smul_eq
  given: (h : F.Stable)
  statement: exists n₀, forall k, F.N (n₀ + k) = I ^ k • F.N n₀
  proof: by
  obtain ⟨n₀, hn⟩ := h
  use n₀
  intro k
  induction k with
  | zero => simp
  | succ _ ih => rw [← add_assoc, ← hn, ih, add_comm, pow_add, mul_smul, pow_one]; lia

中文:
定理 稳定.存在_pow_smul_eq
  条件: (h : F.稳定)
  结论: 存在 n₀, 对任意 k, F.N (n₀ + k) = I ^ k • F.N n₀
  证明: by
  obtain ⟨n₀, hn⟩ := h
  use n₀
  intro k
  induction k with
  | zero => simp
  | succ _ ih => rw [← add_assoc, ← hn, ih, add_comm, pow_add, mul_smul, pow_one]; lia

Depends on / 依赖: add_assoc, add_comm, mul_smul, pow_add, pow_one
-/
theorem Stable.exists_pow_smul_eq (h : F.Stable) : exists n₀, forall k, F.N (n₀ + k) = I ^ k • F.N n₀ := by
  obtain ⟨n₀, hn⟩ := h
  use n₀
  intro k
  induction k with
  | zero => simp
  | succ _ ih => rw [← add_assoc, ← hn, ih, add_comm, pow_add, mul_smul, pow_one]; lia

/--
theorem `Stable.exists_pow_smul_eq_of_ge` / 定理 `Stable.exists_pow_smul_eq_of_ge`

English:
theorem Stable.exists_pow_smul_eq_of_ge
  given: (h : F.Stable)
  proof: by
  obtain ⟨n₀, hn₀⟩ := h.exists_pow_smul_eq
  use n₀
  intro n hn
  convert! hn₀ (n - n₀)
  rw [add_comm]; rw [tsub_add_cancel_of_le hn]

中文:
定理 稳定.存在_pow_smul_eq_of_ge
  条件: (h : F.稳定)
  证明: by
  obtain ⟨n₀, hn₀⟩ := h.exists_pow_smul_eq
  use n₀
  intro n hn
  convert! hn₀ (n - n₀)
  rw [add_comm]; rw [tsub_add_cancel_of_le hn]

Depends on / 依赖: add_comm, convert, exists_pow_smul_eq, h.exists_pow_smul_eq, tsub_add_cancel_of_le
-/
theorem Stable.exists_pow_smul_eq_of_ge (h : F.Stable) :
    exists n₀, forall n >= n₀, F.N n = I ^ (n - n₀) • F.N n₀ := by
  obtain ⟨n₀, hn₀⟩ := h.exists_pow_smul_eq
  use n₀
  intro n hn
  convert! hn₀ (n - n₀)
  rw [add_comm]; rw [tsub_add_cancel_of_le hn]

/--
theorem `stable_iff_exists_pow_smul_eq_of_ge` / 定理 `stable_iff_exists_pow_smul_eq_of_ge`

English:
theorem stable_iff_exists_pow_smul_eq_of_ge
  proof: by
  refine ⟨Stable.exists_pow_smul_eq_of_ge, fun h => ⟨h.choose, fun n hn => ?_⟩⟩
  rw [h.choose_spec n hn]; rw [h.choose_spec (n + 1) (by lia)]; rw [smul_smul]; rw [← pow_succ']; rw [tsub_add_eq_add_tsub hn]

中文:
定理 stable_iff_存在_pow_smul_eq_of_ge
  证明: by
  refine ⟨Stable.exists_pow_smul_eq_of_ge, fun h => ⟨h.choose, fun n hn => ?_⟩⟩
  rw [h.choose_spec n hn]; rw [h.choose_spec (n + 1) (by lia)]; rw [smul_smul]; rw [← pow_succ']; rw [tsub_add_eq_add_tsub hn]

Depends on / 依赖: Stable, Stable.exists_pow_smul_eq_of_ge, choose_spec, exists_pow_smul_eq_of_ge, h.choose, h.choose_spec, pow_succ, smul_smul, tsub_add_eq_add_tsub
-/
theorem stable_iff_exists_pow_smul_eq_of_ge :
    F.Stable ↔ exists n₀, forall n >= n₀, F.N n = I ^ (n - n₀) • F.N n₀ := by
  refine ⟨Stable.exists_pow_smul_eq_of_ge, fun h => ⟨h.choose, fun n hn => ?_⟩⟩
  rw [h.choose_spec n hn]; rw [h.choose_spec (n + 1) (by lia)]; rw [smul_smul]; rw [← pow_succ']; rw [tsub_add_eq_add_tsub hn]

/--
theorem `Stable.exists_forall_le` / 定理 `Stable.exists_forall_le`

English:
theorem Stable.exists_forall_le
  given: (h : F.Stable) (e : F.N 0 <= F'.N 0)
  proof: by
  obtain ⟨n₀, hF⟩ := h
  use n₀
  intro n
  induction n with
  | zero => refine (F.antitone ?_).trans e; simp
  | succ n hn =>
    rw [add_right_comm]; rw [← hF]
    · exact (smul_mono_right _ hn).trans (F'.smul_le _)
    simp

中文:
定理 稳定.存在_对任意_le
  条件: (h : F.稳定) (e : F.N 0 <= F'.N 0)
  证明: by
  obtain ⟨n₀, hF⟩ := h
  use n₀
  intro n
  induction n with
  | zero => refine (F.antitone ?_).trans e; simp
  | succ n hn =>
    rw [add_right_comm]; rw [← hF]
    · exact (smul_mono_right _ hn).trans (F'.smul_le _)
    simp

Depends on / 依赖: F.antitone, add_right_comm, antitone, smul_le, smul_mono_right
-/
theorem Stable.exists_forall_le (h : F.Stable) (e : F.N 0 <= F'.N 0) :
    exists n₀, forall n, F.N (n + n₀) <= F'.N n := by
  obtain ⟨n₀, hF⟩ := h
  use n₀
  intro n
  induction n with
  | zero => refine (F.antitone ?_).trans e; simp
  | succ n hn =>
    rw [add_right_comm]; rw [← hF]
    · exact (smul_mono_right _ hn).trans (F'.smul_le _)
    simp

/--
theorem `Stable.bounded_difference` / 定理 `Stable.bounded_difference`

English:
theorem Stable.bounded_difference
  given: (h : F.Stable) (h' : F'.Stable) (e : F.N 0 = F'.N 0)
  proof: by
  obtain ⟨n₁, h₁⟩ := h.exists_forall_le (le_of_eq e)
  obtain ⟨n₂, h₂⟩ := h'.exists_forall_le (le_of_eq e.symm)
  use max n₁ n₂
  intro n
  refine ⟨(F.antitone ?_).trans (h₁ n), (F'.antitone ?_).trans (h₂ n)⟩ <;> simp

中文:
定理 稳定.bounded_difference
  条件: (h : F.稳定) (h' : F'.稳定) (e : F.N 0 = F'.N 0)
  证明: by
  obtain ⟨n₁, h₁⟩ := h.exists_forall_le (le_of_eq e)
  obtain ⟨n₂, h₂⟩ := h'.exists_forall_le (le_of_eq e.symm)
  use max n₁ n₂
  intro n
  refine ⟨(F.antitone ?_).trans (h₁ n), (F'.antitone ?_).trans (h₂ n)⟩ <;> simp

Depends on / 依赖: F.antitone, antitone, e.symm, exists_forall_le, h.exists_forall_le, le_of_eq
-/
theorem Stable.bounded_difference (h : F.Stable) (h' : F'.Stable) (e : F.N 0 = F'.N 0) :
    exists n₀, forall n, F.N (n + n₀) <= F'.N n ∧ F'.N (n + n₀) <= F.N n := by
  obtain ⟨n₁, h₁⟩ := h.exists_forall_le (le_of_eq e)
  obtain ⟨n₂, h₂⟩ := h'.exists_forall_le (le_of_eq e.symm)
  use max n₁ n₂
  intro n
  refine ⟨(F.antitone ?_).trans (h₁ n), (F'.antitone ?_).trans (h₂ n)⟩ <;> simp

open PolynomialModule

variable (F F')

/--
Definition of `noncomputable` / `noncomputable` 的定义

English:
definition noncomputable
  signature: def submodule
  body: { f | forall i, f.coeff i in F.N i }
  add_mem' hf hg i := Submodule.add_mem _ (hf i) (hg i)
  zero_mem' _ := Submodule.zero_mem _
  smul_mem' r f hf i := by
    rw [Subalgebra.smul_def]; rw [PolynomialModule.smul_apply]
    apply Submodule.sum_mem
    rintro ⟨j, k⟩ e
    rw [Finset.mem_antidiagonal

中文:
定义 noncomputable
  签名: def submodule
  定义体: { f | forall i, f.coeff i in F.N i }
  add_mem' hf hg i := Submodule.add_mem _ (hf i) (hg i)
  zero_mem' _ := Submodule.zero_mem _
  smul_mem' r f hf i := by
    rw [Subalgebra.smul_def]; rw [PolynomialModule.smul_apply]
    apply Submodule.sum_mem
    rintro ⟨j, k⟩ e
    rw [Finset.mem_antidiagonal
-/
protected noncomputable def submodule : Submodule (reesAlgebra I) (PolynomialModule R M) where
  carrier := { f | forall i, f.coeff i in F.N i }
  add_mem' hf hg i := Submodule.add_mem _ (hf i) (hg i)
  zero_mem' _ := Submodule.zero_mem _
  smul_mem' r f hf i := by
    rw [Subalgebra.smul_def]; rw [PolynomialModule.smul_apply]
    apply Submodule.sum_mem
    rintro ⟨j, k⟩ e
    rw [Finset.mem_antidiagonal] at e
    subst e
    exact F.pow_smul_le j k (Submodule.smul_mem_smul (r.2 j) (hf k))

@[simp]
/--
theorem `mem_submodule` / 定理 `mem_submodule`

English:
theorem mem_submodule
  given: (f : PolynomialModule R M)
  statement: f in F.submodule ↔ forall i, f.coeff i in F.N i
  proof: Iff.rfl

中文:
定理 mem_submodule
  条件: (f : 多项式模 R M)
  结论: f in F.submodule ↔ 对任意 i, f.coeff i in F.N i
  证明: Iff.rfl

Depends on / 依赖: Iff.rfl
-/
theorem mem_submodule (f : PolynomialModule R M) : f in F.submodule ↔ forall i, f.coeff i in F.N i :=
  Iff.rfl

/--
theorem `inf_submodule` / 定理 `inf_submodule`

English:
theorem inf_submodule
  statement: (F ⊓ F').submodule = F.submodule ⊓ F'.submodule
  proof: by
  ext
  exact forall_and

中文:
定理 inf_submodule
  结论: (F ⊓ F').submodule = F.submodule ⊓ F'.submodule
  证明: by
  ext
  exact forall_and

Depends on / 依赖: forall_and
-/
theorem inf_submodule : (F ⊓ F').submodule = F.submodule ⊓ F'.submodule := by
  ext
  exact forall_and

variable (I M)

/--
Definition of `submoduleInfHom` / `submoduleInfHom` 的定义

English:
definition submoduleInfHom
  signature: :
  body: Ideal.Filtration.submodule
  map_inf' := inf_submodule

中文:
定义 submoduleInfHom
  签名: :
  定义体: Ideal.Filtration.submodule
  map_inf' := inf_submodule

Depends on / 依赖: Filtration, Ideal.Filtration.submodule, submodule
-/
noncomputable def submoduleInfHom :
    InfHom (I.Filtration M) (Submodule (reesAlgebra I) (PolynomialModule R M)) where
  toFun := Ideal.Filtration.submodule
  map_inf' := inf_submodule

variable {I M}

/--
theorem `submodule_closure_single` / 定理 `submodule_closure_single`

English:
theorem submodule_closure_single
  proof: by
  apply le_antisymm
  · rw [AddSubmonoid.closure_le, Set.iUnion_subset_iff]
    rintro i _ ⟨m, hm, rfl⟩ j
    rw [coeff_single]; rw [Finsupp.single_apply]
    split_ifs with h
    · rwa [← h]
    · exact (F.N j).zero_mem
  · intro f hf
    rw [← f.ofCoeff_coeff]; rw [← f.coeff.sum_single]; rw [of

中文:
定理 submodule_closure_single
  证明: by
  apply le_antisymm
  · rw [AddSubmonoid.closure_le, Set.iUnion_subset_iff]
    rintro i _ ⟨m, hm, rfl⟩ j
    rw [coeff_single]; rw [Finsupp.single_apply]
    split_ifs with h
    · rwa [← h]
    · exact (F.N j).zero_mem
  · intro f hf
    rw [← f.ofCoeff_coeff]; rw [← f.coeff.sum_single]; rw [of

Depends on / 依赖: AddSubmonoid, AddSubmonoid.closure_le, AddSubmonoid.subset_closure, AddSubmonoid.sum_mem, Finsupp, Finsupp.single_apply, Set.iUnion_subset_iff, Set.mem_image_of_mem, Set.subset_iUnion, closure_le, coeff_single, f.coeff.sum_single, f.ofCoeff_coeff, iUnion_subset_iff, le_antisymm, mem_image_of_mem, ofCoeff_coeff, ofCoeff_finsuppSum, single_apply, split_ifs
-/
theorem submodule_closure_single :
    AddSubmonoid.closure (⋃ i, single R i '' (F.N i : Set M)) = F.submodule.toAddSubmonoid := by
  apply le_antisymm
  · rw [AddSubmonoid.closure_le, Set.iUnion_subset_iff]
    rintro i _ ⟨m, hm, rfl⟩ j
    rw [coeff_single]; rw [Finsupp.single_apply]
    split_ifs with h
    · rwa [← h]
    · exact (F.N j).zero_mem
  · intro f hf
    rw [← f.ofCoeff_coeff]; rw [← f.coeff.sum_single]; rw [ofCoeff_finsuppSum]
    apply AddSubmonoid.sum_mem _ _
    rintro c -
    exact AddSubmonoid.subset_closure (Set.subset_iUnion _ c <| Set.mem_image_of_mem _ (hf c))

/--
theorem `submodule_span_single` / 定理 `submodule_span_single`

English:
theorem submodule_span_single
  proof: by
  rw [← Submodule.span_closure]; rw [submodule_closure_single]; rw [Submodule.coe_toAddSubmonoid]
  exact Submodule.span_eq (Filtration.submodule F)

中文:
定理 submodule_span_single
  证明: by
  rw [← Submodule.span_closure]; rw [submodule_closure_single]; rw [Submodule.coe_toAddSubmonoid]
  exact Submodule.span_eq (Filtration.submodule F)

Depends on / 依赖: Filtration, Filtration.submodule, Submodule, Submodule.coe_toAddSubmonoid, Submodule.span_closure, Submodule.span_eq, coe_toAddSubmonoid, span_closure, span_eq, submodule, submodule_closure_single
-/
theorem submodule_span_single :
    Submodule.span (reesAlgebra I) (⋃ i, single R i '' (F.N i : Set M)) = F.submodule := by
  rw [← Submodule.span_closure]; rw [submodule_closure_single]; rw [Submodule.coe_toAddSubmonoid]
  exact Submodule.span_eq (Filtration.submodule F)

set_option backward.isDefEq.respectTransparency false in
/--
theorem `submodule_eq_span_le_iff_stable_ge` / 定理 `submodule_eq_span_le_iff_stable_ge`

English:
theorem submodule_eq_span_le_iff_stable_ge
  given: (n₀ : Nat)
  proof: by
  rw [← submodule_span_single]; rw [← (Submodule.span_mono (Set.iUnion₂_subset_iUnion _ _)).ge_iff_eq']; rw [Submodule.span_le]; rw [Set.iUnion_subset_iff]
  constructor
  · intro H n hn
    refine (F.smul_le n).antisymm ?_
    intro x hx
    obtain ⟨l, hl⟩ := (Finsupp.mem_span_iff_linearCombinat

中文:
定理 submodule_eq_span_le_iff_stable_ge
  条件: (n₀ : 自然数)
  证明: by
  rw [← submodule_span_single]; rw [← (Submodule.span_mono (Set.iUnion₂_subset_iUnion _ _)).ge_iff_eq']; rw [Submodule.span_le]; rw [Set.iUnion_subset_iff]
  constructor
  · intro H n hn
    refine (F.smul_le n).antisymm ?_
    intro x hx
    obtain ⟨l, hl⟩ := (Finsupp.mem_span_iff_linearCombinat

Depends on / 依赖: F.smul_le, Finsupp, Finsupp.mem_span_iff_linearCombination, Finsupp.single_apply, PolynomialModule, PolynomialModule.coeff_single, Set.iUnion, Set.iUnion_subset_iff, Submodule, Submodule.span_le, Submodule.span_mono, antisymm, coeff_single, congr_arg, f.coeff, ge_iff_eq, iUnion_subset_iff, if_pos, mem_span_iff_linearCombination, replace
-/
theorem submodule_eq_span_le_iff_stable_ge (n₀ : Nat) :
    F.submodule = Submodule.span _ (⋃ i <= n₀, single R i '' (F.N i : Set M)) ↔
      forall n >= n₀, I • F.N n = F.N (n + 1) := by
  rw [← submodule_span_single]; rw [← (Submodule.span_mono (Set.iUnion₂_subset_iUnion _ _)).ge_iff_eq']; rw [Submodule.span_le]; rw [Set.iUnion_subset_iff]
  constructor
  · intro H n hn
    refine (F.smul_le n).antisymm ?_
    intro x hx
    obtain ⟨l, hl⟩ := (Finsupp.mem_span_iff_linearCombination _ _ _).mp (H _ ⟨x, hx, rfl⟩)
    replace hl := congr_arg (fun f : PolynomialModule R M => f.coeff (n + 1)) hl
    rw [PolynomialModule.coeff_single]; rw [Finsupp.single_apply]; rw [if_pos rfl] at hl
    rw [← hl]; rw [Finsupp.linearCombination_apply]; rw [PolynomialModule.coeff_finsuppSum]; rw [Finsupp.sum_apply]
    apply Submodule.sum_mem _ _
    rintro ⟨_, _, ⟨n', rfl⟩, _, ⟨hn', rfl⟩, m, hm, rfl⟩ -
    dsimp only [Subtype.coe_mk]
    rw [Subalgebra.smul_def]; rw [smul_single_apply]; rw [if_pos (show n' <= n + 1 by lia)]
    have e : n' <= n := by lia
    have := F.pow_smul_le_pow_smul (n - n') n' 1
    rw [tsub_add_cancel_of_le e]; rw [pow_one]; rw [add_comm _ 1]; rw [← add_tsub_assoc_of_le e]; rw [add_comm] at this
    exact this (Submodule.smul_mem_smul ((l _).2 <| n + 1 - n') hm)
  · let F' := Submodule.span (reesAlgebra I) (⋃ i <= n₀, single R i '' (F.N i : Set M))
    intro hF i
    have : forall i <= n₀, single R i '' (F.N i : Set M) subseteq F' := fun i hi =>
      -- Porting note: need to add hint for `s`
      (Set.subset_iUnion₂ (s := fun i _ => (single R i '' (N F i : Set M))) i hi).trans
        Submodule.subset_span
    induction i with
    | zero => exact this _ zero_le
    | succ j hj => ?_
    by_cases hj' : j.succ <= n₀
    · exact this _ hj'
    simp only [not_le, Nat.lt_succ_iff] at hj'
    rw [← hF _ hj']
    rintro _ ⟨m, hm, rfl⟩
    refine Submodule.smul_induction_on hm (fun r hr m' hm' => ?_) (fun x y hx hy => ?_)
    · rw [add_comm, ← monomial_smul_single]
      exact F'.smul_mem
        ⟨_, reesAlgebra.monomial_mem.mpr (by rwa [pow_one])⟩ (hj <| Set.mem_image_of_mem _ hm')
    · rw [PolynomialModule.single_add]
      exact F'.add_mem hx hy

set_option backward.isDefEq.respectTransparency.types false in
/--
theorem `submodule_fg_iff_stable` / 定理 `submodule_fg_iff_stable`

English:
theorem submodule_fg_iff_stable
  given: (hF' : forall i, (F.N i).FG)
  statement: F.submodule.FG ↔ F.Stable
  proof: by
  classical
  delta Ideal.Filtration.Stable
  simp_rw [← F.submodule_eq_span_le_iff_stable_ge]
  constructor
  · rintro H
    refine H.stabilizes_of_iSup_eq
        ⟨fun n₀ => Submodule.span _ (⋃ (i : Nat) (_ : i <= n₀), single R i '' ↑(F.N i)), ?_⟩ ?_
    · intro n m e
      rw [Submodule.span_l

中文:
定理 submodule_fg_iff_stable
  条件: (hF' : 对任意 i, (F.N i).FG)
  结论: F.submodule.FG ↔ F.稳定
  证明: by
  classical
  delta Ideal.Filtration.Stable
  simp_rw [← F.submodule_eq_span_le_iff_stable_ge]
  constructor
  · rintro H
    refine H.stabilizes_of_iSup_eq
        ⟨fun n₀ => Submodule.span _ (⋃ (i : Nat) (_ : i <= n₀), single R i '' ↑(F.N i)), ?_⟩ ?_
    · intro n m e
      rw [Submodule.span_l

Depends on / 依赖: F.submodule_eq_span_le_iff_stable_ge, Filtration, H.stabilizes_of_iSup_eq, Ideal.Filtration.Stable, Set.Subset.trans, Set.iUnion, Set.subset_iUnion, Stable, Submodule, Submodule.span, Submodule.span_iUnion, Submodule.span_le, Submodule.subset_span, Subset, classical, hi.trans, simp_rw, single, span_iUnion, span_le
-/
theorem submodule_fg_iff_stable (hF' : forall i, (F.N i).FG) : F.submodule.FG ↔ F.Stable := by
  classical
  delta Ideal.Filtration.Stable
  simp_rw [← F.submodule_eq_span_le_iff_stable_ge]
  constructor
  · rintro H
    refine H.stabilizes_of_iSup_eq
        ⟨fun n₀ => Submodule.span _ (⋃ (i : Nat) (_ : i <= n₀), single R i '' ↑(F.N i)), ?_⟩ ?_
    · intro n m e
      rw [Submodule.span_le]; rw [Set.iUnion₂_subset_iff]
      intro i hi
      refine Set.Subset.trans ?_ Submodule.subset_span
      refine @Set.subset_iUnion₂ _ _ _ (fun i _ => ↑((single R i) '' ((N F i) : Set M))) i ?_
      exact hi.trans e
    · dsimp
      rw [← Submodule.span_iUnion]; rw [← submodule_span_single]
      simp [Set.biUnion_le_eq_iUnion]
  · rintro ⟨n, hn⟩
    rw [hn]
    simp_rw [Submodule.span_iUnion₂, ← Finset.mem_range_succ_iff, iSup_subtype']
    apply Submodule.fg_iSup
    rintro ⟨i, hi⟩
    obtain ⟨s, hs⟩ := hF' i
    have : Submodule.span (reesAlgebra I) (s.image (lsingle R i) : Set (PolynomialModule R M)) =
        Submodule.span _ (single R i '' (F.N i : Set M)) := by
      rw [Finset.coe_image]; rw [← Submodule.span_span_of_tower R]; rw [← Submodule.map_span]; rw [hs]; rfl
    rw [Subtype.coe_mk]; rw [← this]
    exact ⟨_, rfl⟩

variable {F}

/--
theorem `Stable.of_le` / 定理 `Stable.of_le`

English:
theorem Stable.of_le
  statement: [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable)
  proof: by
  rw [← submodule_fg_iff_stable] at hF ⊢
  any_goals intro i; exact IsNoetherian.noetherian _
  have := isNoetherian_of_fg_of_noetherian _ hF
  rw [isNoetherian_submodule] at this
  exact this _ (OrderHomClass.mono (submoduleInfHom M I) hf)

中文:
定理 稳定.of_le
  结论: [是Noether环 R] [模.有限 R M] (hF : F.稳定)
  证明: by
  rw [← submodule_fg_iff_stable] at hF ⊢
  any_goals intro i; exact IsNoetherian.noetherian _
  have := isNoetherian_of_fg_of_noetherian _ hF
  rw [isNoetherian_submodule] at this
  exact this _ (OrderHomClass.mono (submoduleInfHom M I) hf)

Depends on / 依赖: IsNoetherian, IsNoetherian.noetherian, OrderHomClass, OrderHomClass.mono, any_goals, isNoetherian_of_fg_of_noetherian, isNoetherian_submodule, noetherian, submoduleInfHom, submodule_fg_iff_stable
-/
theorem Stable.of_le [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable)
    {F' : I.Filtration M} (hf : F' <= F) : F'.Stable := by
  rw [← submodule_fg_iff_stable] at hF ⊢
  any_goals intro i; exact IsNoetherian.noetherian _
  have := isNoetherian_of_fg_of_noetherian _ hF
  rw [isNoetherian_submodule] at this
  exact this _ (OrderHomClass.mono (submoduleInfHom M I) hf)

/--
theorem `Stable.inter_right` / 定理 `Stable.inter_right`

English:
theorem Stable.inter_right
  given: [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable)
  proof: hF.of_le inf_le_left

中文:
定理 稳定.inter_right
  条件: [是Noether环 R] [模.有限 R M] (hF : F.稳定)
  证明: hF.of_le inf_le_left

Depends on / 依赖: hF.of_le, inf_le_left, of_le
-/
theorem Stable.inter_right [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable) :
    (F ⊓ F').Stable :=
  hF.of_le inf_le_left

/--
theorem `Stable.inter_left` / 定理 `Stable.inter_left`

English:
theorem Stable.inter_left
  given: [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable)
  proof: hF.of_le inf_le_right

中文:
定理 稳定.inter_left
  条件: [是Noether环 R] [模.有限 R M] (hF : F.稳定)
  证明: hF.of_le inf_le_right

Depends on / 依赖: hF.of_le, inf_le_right, of_le
-/
theorem Stable.inter_left [IsNoetherianRing R] [Module.Finite R M] (hF : F.Stable) :
    (F' ⊓ F).Stable :=
  hF.of_le inf_le_right

end Ideal.Filtration

variable (I)

/--
theorem `Ideal.exists_pow_inf_eq_pow_smul` / 定理 `Ideal.exists_pow_inf_eq_pow_smul`

English:
theorem Ideal.exists_pow_inf_eq_pow_smul
  statement: [IsNoetherianRing R] [Module.Finite R M]
  proof: ((I.stableFiltration_stable ⊤).inter_right (I.trivialFiltration N)).exists_pow_smul_eq_of_ge

中文:
定理 理想.存在_pow_inf_eq_pow_smul
  结论: [是Noether环 R] [模.有限 R M]
  证明: ((I.stableFiltration_stable ⊤).inter_right (I.trivialFiltration N)).exists_pow_smul_eq_of_ge

Depends on / 依赖: I.stableFiltration_stable, I.trivialFiltration, exists_pow_smul_eq_of_ge, inter_right, stableFiltration_stable, trivialFiltration
-/
theorem Ideal.exists_pow_inf_eq_pow_smul [IsNoetherianRing R] [Module.Finite R M]
    (N : Submodule R M) : exists k : Nat, forall n >= k, I ^ n • ⊤ ⊓ N = I ^ (n - k) • (I ^ k • ⊤ ⊓ N) :=
  ((I.stableFiltration_stable ⊤).inter_right (I.trivialFiltration N)).exists_pow_smul_eq_of_ge

/--
theorem `Ideal.mem_iInf_smul_pow_eq_bot_iff` / 定理 `Ideal.mem_iInf_smul_pow_eq_bot_iff`

English:
theorem Ideal.mem_iInf_smul_pow_eq_bot_iff
  given: [IsNoetherianRing R] [Module.Finite R M] (x : M)
  proof: by
  let N := (⨅ i : Nat, I ^ i • ⊤ : Submodule R M)
  have hN : forall k, (I.stableFiltration ⊤ ⊓ I.trivialFiltration N).N k = N :=
    fun k => inf_eq_right.mpr ((iInf_le _ k).trans <| le_of_eq <| by simp)
  constructor
  · obtain ⟨r, hr₁, hr₂⟩ :=
      Submodule.exists_mem_and_smul_eq_self_of_fg_

中文:
定理 理想.mem_iInf_smul_pow_eq_bot_iff
  条件: [是Noether环 R] [模.有限 R M] (x : M)
  证明: by
  let N := (⨅ i : Nat, I ^ i • ⊤ : Submodule R M)
  have hN : forall k, (I.stableFiltration ⊤ ⊓ I.trivialFiltration N).N k = N :=
    fun k => inf_eq_right.mpr ((iInf_le _ k).trans <| le_of_eq <| by simp)
  constructor
  · obtain ⟨r, hr₁, hr₂⟩ :=
      Submodule.exists_mem_and_smul_eq_self_of_fg_

Depends on / 依赖: I.stableFiltration, I.stableFiltration_stable, I.trivialFiltration, IsNoetherian, IsNoetherian.noetherian, Submodule, Submodule.exists_mem_and_smul_eq_self_of_fg_of_le_smul, exists_mem_and_smul_eq_self_of_fg_of_le_smul, iInf_le, inf_eq_right, inf_eq_right.mpr, inter_right, le_of_eq, le_refl, noetherian, stableFiltration, stableFiltration_stable, this.symm, trivialFiltration
-/
theorem Ideal.mem_iInf_smul_pow_eq_bot_iff [IsNoetherianRing R] [Module.Finite R M] (x : M) :
    x in (⨅ i : Nat, I ^ i • ⊤ : Submodule R M) ↔ exists r : I, (r : R) • x = x := by
  let N := (⨅ i : Nat, I ^ i • ⊤ : Submodule R M)
  have hN : forall k, (I.stableFiltration ⊤ ⊓ I.trivialFiltration N).N k = N :=
    fun k => inf_eq_right.mpr ((iInf_le _ k).trans <| le_of_eq <| by simp)
  constructor
  · obtain ⟨r, hr₁, hr₂⟩ :=
      Submodule.exists_mem_and_smul_eq_self_of_fg_of_le_smul I N (IsNoetherian.noetherian N) (by
        obtain ⟨k, hk⟩ := (I.stableFiltration_stable ⊤).inter_right (I.trivialFiltration N)
        have := hk k (le_refl _)
        rw [hN]; rw [hN] at this
        exact le_of_eq this.symm)
    intro H
    exact ⟨⟨r, hr₁⟩, hr₂ _ H⟩
  · rintro ⟨r, eq⟩
    rw [Submodule.mem_iInf]
    intro i
    induction i with
    | zero => simp
    | succ i hi =>
      rw [add_comm]; rw [pow_add]; rw [← smul_smul]; rw [pow_one]; rw [← eq]
      exact Submodule.smul_mem_smul r.prop hi

/--
theorem `Ideal.iInf_pow_smul_eq_bot_of_le_jacobson` / 定理 `Ideal.iInf_pow_smul_eq_bot_of_le_jacobson`

English:
theorem Ideal.iInf_pow_smul_eq_bot_of_le_jacobson
  statement: [IsNoetherianRing R]
  proof: by
  rw [eq_bot_iff]
  intro x hx
  obtain ⟨r, hr⟩ := (I.mem_iInf_smul_pow_eq_bot_iff x).mp hx
  have := isUnit_of_sub_one_mem_jacobson_bot (1 - r.1) (by simpa using h r.2)
  apply this.smul_left_cancel.mp
  simp [sub_smul, hr]

中文:
定理 理想.iInf_pow_smul_eq_bot_of_le_jacobson
  结论: [是Noether环 R]
  证明: by
  rw [eq_bot_iff]
  intro x hx
  obtain ⟨r, hr⟩ := (I.mem_iInf_smul_pow_eq_bot_iff x).mp hx
  have := isUnit_of_sub_one_mem_jacobson_bot (1 - r.1) (by simpa using h r.2)
  apply this.smul_left_cancel.mp
  simp [sub_smul, hr]

Depends on / 依赖: I.mem_iInf_smul_pow_eq_bot_iff, eq_bot_iff, isUnit_of_sub_one_mem_jacobson_bot, mem_iInf_smul_pow_eq_bot_iff, smul_left_cancel, sub_smul, this.smul_left_cancel.mp
-/
theorem Ideal.iInf_pow_smul_eq_bot_of_le_jacobson [IsNoetherianRing R]
    [Module.Finite R M] (h : I <= Ideal.jacobson ⊥) : (⨅ i : Nat, I ^ i • ⊤ : Submodule R M) = ⊥ := by
  rw [eq_bot_iff]
  intro x hx
  obtain ⟨r, hr⟩ := (I.mem_iInf_smul_pow_eq_bot_iff x).mp hx
  have := isUnit_of_sub_one_mem_jacobson_bot (1 - r.1) (by simpa using h r.2)
  apply this.smul_left_cancel.mp
  simp [sub_smul, hr]

open IsLocalRing in
/--
theorem `Ideal.iInf_pow_smul_eq_bot_of_isLocalRing` / 定理 `Ideal.iInf_pow_smul_eq_bot_of_isLocalRing`

English:
theorem Ideal.iInf_pow_smul_eq_bot_of_isLocalRing
  statement: [IsNoetherianRing R] [IsLocalRing R]
  proof: Ideal.iInf_pow_smul_eq_bot_of_le_jacobson _
    ((le_maximalIdeal h).trans (maximalIdeal_le_jacobson _))

中文:
定理 理想.iInf_pow_smul_eq_bot_of_isLocalRing
  结论: [是Noether环 R] [是局部环 R]
  证明: Ideal.iInf_pow_smul_eq_bot_of_le_jacobson _
    ((le_maximalIdeal h).trans (maximalIdeal_le_jacobson _))

Depends on / 依赖: Ideal.iInf_pow_smul_eq_bot_of_le_jacobson, iInf_pow_smul_eq_bot_of_le_jacobson, le_maximalIdeal, maximalIdeal_le_jacobson
-/
theorem Ideal.iInf_pow_smul_eq_bot_of_isLocalRing [IsNoetherianRing R] [IsLocalRing R]
    [Module.Finite R M] (h : I != ⊤) : (⨅ i : Nat, I ^ i • ⊤ : Submodule R M) = ⊥ :=
  Ideal.iInf_pow_smul_eq_bot_of_le_jacobson _
    ((le_maximalIdeal h).trans (maximalIdeal_le_jacobson _))

/--
theorem `Ideal.iInf_pow_eq_bot_of_isLocalRing` / 定理 `Ideal.iInf_pow_eq_bot_of_isLocalRing`

English:
theorem Ideal.iInf_pow_eq_bot_of_isLocalRing
  given: [IsNoetherianRing R] [IsLocalRing R] (h : I != ⊤)
  proof: by
  convert! I.iInf_pow_smul_eq_bot_of_isLocalRing (M := R) h
  ext i
  rw [smul_eq_mul]; rw [← Ideal.one_eq_top]; rw [mul_one]

中文:
定理 理想.iInf_pow_eq_bot_of_isLocalRing
  条件: [是Noether环 R] [是局部环 R] (h : I != ⊤)
  证明: by
  convert! I.iInf_pow_smul_eq_bot_of_isLocalRing (M := R) h
  ext i
  rw [smul_eq_mul]; rw [← Ideal.one_eq_top]; rw [mul_one]

Depends on / 依赖: I.iInf_pow_smul_eq_bot_of_isLocalRing, Ideal.one_eq_top, convert, iInf_pow_smul_eq_bot_of_isLocalRing, mul_one, one_eq_top, smul_eq_mul
-/
theorem Ideal.iInf_pow_eq_bot_of_isLocalRing [IsNoetherianRing R] [IsLocalRing R] (h : I != ⊤) :
    ⨅ i : Nat, I ^ i = ⊥ := by
  convert! I.iInf_pow_smul_eq_bot_of_isLocalRing (M := R) h
  ext i
  rw [smul_eq_mul]; rw [← Ideal.one_eq_top]; rw [mul_one]

/--
theorem `Ideal.isIdempotentElem_iff_eq_bot_or_top_of_isLocalRing` / 定理 `Ideal.isIdempotentElem_iff_eq_bot_or_top_of_isLocalRing`

English:
theorem Ideal.isIdempotentElem_iff_eq_bot_or_top_of_isLocalRing
  statement: {R} [CommRing R]
  proof: by
  constructor
  · intro H
    by_cases I = ⊤; · exact Or.inr ‹_›
    refine Or.inl (eq_bot_iff.mpr ?_)
    rw [← Ideal.iInf_pow_eq_bot_of_isLocalRing I ‹_›]
    apply le_iInf
    rintro (_ | n) <;> simp [H.pow_succ_eq]
  · rintro (rfl | rfl) <;> simp [IsIdempotentElem]

中文:
定理 理想.isIdempotentElem_iff_eq_bot_or_top_of_isLocalRing
  结论: {R} [交换环 R]
  证明: by
  constructor
  · intro H
    by_cases I = ⊤; · exact Or.inr ‹_›
    refine Or.inl (eq_bot_iff.mpr ?_)
    rw [← Ideal.iInf_pow_eq_bot_of_isLocalRing I ‹_›]
    apply le_iInf
    rintro (_ | n) <;> simp [H.pow_succ_eq]
  · rintro (rfl | rfl) <;> simp [IsIdempotentElem]

Depends on / 依赖: H.pow_succ_eq, Ideal.iInf_pow_eq_bot_of_isLocalRing, IsIdempotentElem, Or.inl, Or.inr, eq_bot_iff, eq_bot_iff.mpr, iInf_pow_eq_bot_of_isLocalRing, le_iInf, pow_succ_eq
-/
theorem Ideal.isIdempotentElem_iff_eq_bot_or_top_of_isLocalRing {R} [CommRing R]
    [IsNoetherianRing R] [IsLocalRing R] (I : Ideal R) :
    IsIdempotentElem I ↔ I = ⊥ ∨ I = ⊤ := by
  constructor
  · intro H
    by_cases I = ⊤; · exact Or.inr ‹_›
    refine Or.inl (eq_bot_iff.mpr ?_)
    rw [← Ideal.iInf_pow_eq_bot_of_isLocalRing I ‹_›]
    apply le_iInf
    rintro (_ | n) <;> simp [H.pow_succ_eq]
  · rintro (rfl | rfl) <;> simp [IsIdempotentElem]

open IsLocalRing in
/--
theorem `Ideal.iInf_pow_smul_eq_bot_of_isTorsionFree` / 定理 `Ideal.iInf_pow_smul_eq_bot_of_isTorsionFree`

English:
theorem Ideal.iInf_pow_smul_eq_bot_of_isTorsionFree
  statement: [IsDomain R]
  proof: by
  rw [eq_bot_iff]
  intro x hx
  by_contra hx'
  have := Ideal.mem_iInf_smul_pow_eq_bot_iff I x
  obtain ⟨r, hr⟩ := this.mp hx
  have := smul_left_injective _ hx' (hr.trans (one_smul _ x).symm)
  exact I.eq_top_iff_one.not.mp h (this ▸ r.prop)

@[deprecated (since := "2026-01-17")]
alias Ideal.iI

中文:
定理 理想.iInf_pow_smul_eq_bot_of_isTorsionFree
  结论: [是整环 R]
  证明: by
  rw [eq_bot_iff]
  intro x hx
  by_contra hx'
  have := Ideal.mem_iInf_smul_pow_eq_bot_iff I x
  obtain ⟨r, hr⟩ := this.mp hx
  have := smul_left_injective _ hx' (hr.trans (one_smul _ x).symm)
  exact I.eq_top_iff_one.not.mp h (this ▸ r.prop)

@[deprecated (since := "2026-01-17")]
alias Ideal.iI

Depends on / 依赖: I.eq_top_iff_one.not.mp, Ideal.mem_iInf_smul_pow_eq_bot_iff, eq_bot_iff, eq_top_iff_one, hr.trans, mem_iInf_smul_pow_eq_bot_iff, one_smul, r.prop, smul_left_injective, this.mp
-/
theorem Ideal.iInf_pow_smul_eq_bot_of_isTorsionFree [IsDomain R]
    [IsNoetherianRing R] [Module.IsTorsionFree R M]
    [Module.Finite R M] (h : I != ⊤) : (⨅ i : Nat, I ^ i • ⊤ : Submodule R M) = ⊥ := by
  rw [eq_bot_iff]
  intro x hx
  by_contra hx'
  have := Ideal.mem_iInf_smul_pow_eq_bot_iff I x
  obtain ⟨r, hr⟩ := this.mp hx
  have := smul_left_injective _ hx' (hr.trans (one_smul _ x).symm)
  exact I.eq_top_iff_one.not.mp h (this ▸ r.prop)

@[deprecated (since := "2026-01-17")]
alias Ideal.iInf_pow_smul_eq_bot_of_noZeroSMulDivisors :=
  Ideal.iInf_pow_smul_eq_bot_of_isTorsionFree

/--
theorem `Ideal.iInf_pow_eq_bot_of_isDomain` / 定理 `Ideal.iInf_pow_eq_bot_of_isDomain`

English:
theorem Ideal.iInf_pow_eq_bot_of_isDomain
  given: [IsNoetherianRing R] [IsDomain R] (h : I != ⊤)
  proof: by
  convert! I.iInf_pow_smul_eq_bot_of_isTorsionFree (M := R) h
  simp

中文:
定理 理想.iInf_pow_eq_bot_of_isDomain
  条件: [是Noether环 R] [是整环 R] (h : I != ⊤)
  证明: by
  convert! I.iInf_pow_smul_eq_bot_of_isTorsionFree (M := R) h
  simp

Depends on / 依赖: I.iInf_pow_smul_eq_bot_of_isTorsionFree, convert, iInf_pow_smul_eq_bot_of_isTorsionFree
-/
theorem Ideal.iInf_pow_eq_bot_of_isDomain [IsNoetherianRing R] [IsDomain R] (h : I != ⊤) :
    ⨅ i : Nat, I ^ i = ⊥ := by
  convert! I.iInf_pow_smul_eq_bot_of_isTorsionFree (M := R) h
  simp
