...
/-
**Mathlib.Tactic.FieldSimp.qNF** 是 Mathlib 中的一个缩写定义，位于命名空间 `Mathlib.Tactic.Field
Simp`。
形式化陈述：qNF (M : Q(Type v))
参数：M : Q(Type v)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Basic meta-code "normal form" object of the `field_simp` tactic: a type synonym
for a list of ordered triples comprising an expression representing a term of a 
type `M` (where
typically `M` is a field), together with an integer "power" and a natural number
 "index".
...
a₂, x₂), k)`
appear in two different `FieldSimp.qNF` objects (i.e. with the same `ℕ`-index `k
`), it is expected
that the expressions `x₁` and `x₂` are the same.  It is also expected that the i
tems in a
`FieldSimp.qNF` list are in strictly decreasing order by natural-number index.

By forgetting the natural number indices, an expression representing a `Mathlib.
Tactic.FieldSimp.NF`
object can be built from a `FieldSimp.qNF` object; this construction is provided
 as
`Mathlib.Tactic.FieldSimp.qNF.toNF`.
-/
...
-/
/-
**Mathlib.Tactic.FieldSimp.qNF.toNF** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Fi
eldSimp.qNF`。
形式化陈述：toNF (l : qNF q($M)) : Q(NF $M)
参数：l : qNF q($M)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given `l` of type `qNF M`, i.e. a list of `(ℤ × Q($M)) × ℕ`s (two `Expr`s and a 
natural
number), build an `Expr` representing an object of type `NF M` (i.e. `List (ℤ × 
M)`) in the
in the obvious way: by forgetting the natural numbers and gluing together the in
tegers and `Expr`s.
-/
...
components. -/
/-
**Mathlib.Tactic.FieldSimp.qNF.onExponent** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.FieldSimp.qNF`。
形式化陈述：onExponent (l : qNF M) (f : Int -> Int) : qNF M
参数：l : qNF M；f : Int -> Int。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Given `l` of type `qNF M`, i.e. a list of `(ℤ × Q($M)) × ℕ`s (two `Expr`s and a 
natural
number), apply an expression representing a function with domain `ℤ` to each of 
the `ℤ`
components.
-/
...
/-
**Mathlib.Tactic.FieldSimp.qNF.evalPrettyMonomial** 是 Mathlib 中的一个定义，位于命名空间 `Mat
hlib.Tactic.FieldSimp.qNF`。
形式化陈述：evalPrettyMonomial (iM : Q(GroupWithZero $M)) (r : Int) (x : Q($M)) : Meta
M (Σ e : Q($M), Q(zpow' $x $r = $e))
参数：iM : Q(GroupWithZero $M)；r : Int；x : Q($M)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Build a transparent expression for the product of powers represented by `l : qNF
 M`.
-/
...
/-
**Mathlib.Tactic.FieldSimp.qNF.tryClearZero** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.T
actic.FieldSimp.qNF`。
形式化陈述：tryClearZero (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type
)) (iM : Q(CommGroupWithZero $M)) (r : Int) (x : Q($M)) (i : Nat) (l : qNF M) : 
MetaM Σ l' : qNF M, Q(NF.eval $(qNF.toNF (((r, x), i) :: l)) = NF.eval $(l'.toNF
))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；r : Int；x : Q($M)；i : Nat；l : qNF M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Try to drop an expression `zpow' x r` from the beginning of a product. If `r ≠ 0
` this of course
can't be done. If `r = 0`, then `zpow' x r` is equal to `x / x`, so it can be si
mplified to 1 (hence
dropped from the beginning of the product) if we can find a proof that `x ≠ 0`.
-/
def tryClearZero
...
are equal. -/
/-
**Mathlib.Tactic.FieldSimp.qNF.removeZeros** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Ta
ctic.FieldSimp.qNF`。
形式化陈述：removeZeros (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)
) (iM : Q(CommGroupWithZero $M)) (l : qNF M) : MetaM Σ l' : qNF M, Q(NF.eval $(l
.toNF) = NF.eval $(l'.toNF))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；l : qNF M。
该定义给出了上述对象。
...
--- 原说明 ---
Given `l : qNF M`, obtain `l' : qNF M` by removing all `l`'s exponent-zero entri
es where the
corresponding atom can be proved nonzero, and construct a proof that their assoc
iated expressions
are equal.
-/
def removeZeros
...
/-
**Mathlib.Tactic.FieldSimp.qNF.split** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.F
ieldSimp.qNF`。
形式化陈述：split (iM : Q(CommGroupWithZero $M)) (l : qNF M) : MetaM (Σ l_n l_d : qNF 
M, Q(NF.eval $(l.toNF) = NF.eval (l_n.toNF) / NF.eval (l_d.toNF)))
参数：iM : Q(CommGroupWithZero $M)；l : qNF M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a product of powers, split as a quotient: the positive powers divided by (
the negations
of) the negative powers.
-/
...
/-
**Mathlib.Tactic.FieldSimp.qNF.evalPrettyAux** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.
Tactic.FieldSimp.qNF`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-- Build a transparent expression for the product of powers represented by `l : qNF M`. -/
/-
**Mathlib.Tactic.FieldSimp.qNF.evalPretty** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.FieldSimp.qNF`。
形式化陈述：evalPretty (iM : Q(CommGroupWithZero $M)) (l : qNF M) : MetaM (Σ e : Q($M)
, Q(NF.eval $(l.toNF) = $e))
参数：iM : Q(CommGroupWithZero $M)；l : qNF M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.FieldSimp.qNF.mul** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Fie
ldSimp.qNF`。
形式化陈述：{v : Level} →   {M : Q(Type v)} →     Mathlib.Tactic.FieldSimp.qNF q(«$M»)
 → Mathlib.Tactic.FieldSimp.qNF q(«$M») → Mathlib.Tactic.FieldSimp.qNF q(«$M»)
参数：Type v；«$M»；«$M»；«$M»。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given two terms `l₁`, `l₂` of type `qNF M`, i.e. lists of `(ℤ × Q($M)) × ℕ`s (an
 integer, an
`Expr` and a natural number), construct another such term `l`, which will have t
he property that in
the field `$M`, the product of the "multiplicative linear combinations" represen
ted by `l₁` and
`l₂` is the multiplicative linear combination represented by `l`.

The construction assumes, to be valid, that the lists `l₁` and `l₂` are in stric
tly decreasing order
by `ℕ`-component, and that if pairs `(a₁, x₁)` and `(a₂, x₂)` appear in `l₁`, `l
₂` respectively with
the same `ℕ`-component `k`, then the expressions `x₁` and `x₂` are equal.

The construction is as follows: merge the two lists, except that if pairs `(a₁, 
x₁)` and `(a₂, x₂)`
appear in `l₁`, `l₂` respectively with the same `ℕ`-component `k`, then contribu
te a term
`(a₁ + a₂, x₁)` to the output list with `ℕ`-component `k`.
-/
...
/-
**Mathlib.Tactic.FieldSimp.qNF.mkMulProof** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.FieldSimp.qNF`。
形式化陈述：mkMulProof (iM : Q(CommGroupWithZero $M)) (l₁ l₂ : qNF M) : Q((NF.eval $(l
₁.toNF)) * NF.eval $(l₂.toNF) = NF.eval $((qNF.mul l₁ l₂).toNF))
参数：iM : Q(CommGroupWithZero $M)；l₁ l₂ : qNF M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Given two terms `l₁`, `l₂` of type `qNF M`, i.e. lists of `(ℤ × Q($M)) × ℕ`s (an
 integer, an
`Expr` and a natural number), recursively construct a proof that in the field `$
M`, the product of
the "multiplicative linear combinations" represented by `l₁` and `l₂` is the mul
tiplicative linear
combination represented by `FieldSimp.qNF.mul l₁ l₁`.
-/
...
-/
/-
**Mathlib.Tactic.FieldSimp.qNF.div** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Fie
ldSimp.qNF`。
形式化陈述：{v : Level} →   {M : Q(Type v)} → Mathlib.Tactic.FieldSimp.qNF M → Mathlib
.Tactic.FieldSimp.qNF M → Mathlib.Tactic.FieldSimp.qNF M
参数：Type v。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`Expr` and a natural number), construct another such term `l`, which will have t
he property that in
the field `$M`, the quotient of the "multiplicative linear combinations" represe
nted by `l₁` and
`l₂` is the multiplicative linear combination represented by `l`.
...
/-
**Mathlib.Tactic.FieldSimp.qNF.mkDivProof** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.FieldSimp.qNF`。
形式化陈述：mkDivProof (iM : Q(CommGroupWithZero $M)) (l₁ l₂ : qNF M) : Q(NF.eval $(l₁
.toNF) / NF.eval $(l₂.toNF) = NF.eval $((qNF.div l₁ l₂).toNF))
参数：iM : Q(CommGroupWithZero $M)；l₁ l₂ : qNF M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`Expr` and a natural number), recursively construct a proof that in the field `$
M`, the quotient
of the "multiplicative linear combinations" represented by `l₁` and `l₂` is the 
multiplicative
linear combination represented by `FieldSimp.qNF.div l₁ l₁`.
-/
...
/-
**Mathlib.Tactic.FieldSimp.DenomCondition** 是 Mathlib 中的一个归纳类型，位于命名空间 `Mathlib.T
actic.FieldSimp`。
形式化陈述：{v : Level} → {M : Q(Type v)} → Q(GroupWithZero «$M») → Type
参数：Type v；GroupWithZero «$M»。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Constraints on denominators which may need to be considered in `field_simp`: no 
condition,
nonzeroness, or strict positivity.
-/
...
/-
**Mathlib.Tactic.FieldSimp.DenomCondition.proof** 是 Mathlib 中的一个定义，位于命名空间 `Mathl
ib.Tactic.FieldSimp.DenomCondition`。
形式化陈述：{v : Level} →   {M : Q(Type v)} →     {iM : Q(GroupWithZero «$M»)} → Mathl
ib.Tactic.FieldSimp.qNF M → Mathlib.Tactic.FieldSimp.DenomCondition iM → Type
参数：Type v；GroupWithZero «$M»。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a field-simp-normal-form expression `L` (a product of powers of atoms), a 
proof (according
to the value of `DenomCondition`) of that expression's nonzeroness, strict posit
ivity, etc.
-/
...
/-
**Mathlib.Tactic.FieldSimp.DenomCondition.proofZero** 是 Mathlib 中的一个定义，位于命名空间 `M
athlib.Tactic.FieldSimp.DenomCondition`。
形式化陈述：proofZero {iM : Q(CommGroupWithZero $M)} : forall cond : DenomCondition (M
参数：CommGroupWithZero $M。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The empty field-simp-normal-form expression `[]` (representing `1` as an empty p
roduct of powers
of atoms) can be proved to be nonzero, strict positivity, etc., as needed, as sp
ecified by the
value of `DenomCondition`.
-/
...
/-
**Mathlib.Tactic.FieldSimp.mkDenomConditionProofSucc** 是 Mathlib 中的一个定义，位于命名空间 `
Mathlib.Tactic.FieldSimp`。
形式化陈述：mkDenomConditionProofSucc {iM : Q(CommGroupWithZero $M)} (disch : forall {
u : Level} (type : Q(Sort u)), MetaM Q($type)) {cond : DenomCondition (M
参数：CommGroupWithZero $M；disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($
type)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a proof of the nonzeroness, strict positivity, etc. (as specified by the v
alue of
`DenomCondition`) of a field-simp-normal-form expression `L` (a product of power
s of atoms),
construct a corresponding proof for `((r, e), i) :: L`.
...
/-
**Mathlib.Tactic.FieldSimp.qNF.gcd** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Tactic.
FieldSimp.qNF`。
形式化陈述：{v : Level} →   {M : Q(Type v)} →     (iM : Q(CommGroupWithZero «$M»)) →  
     (l₁ l₂ : Mathlib.Tactic.FieldSimp.qNF M) →         ({u : Level} → (type : Q
(Sort u)) → MetaM Q(«$type»)) →           (cond : Mathlib.Tactic.FieldSimp.Denom
Condition q(inferInstance)) →             MetaM               ((L : Mathlib.Tact
ic.FieldSimp.qNF M) ×                 (l₁' : Mathlib.Tactic.FieldSimp.qNF M) ×  
                 (l₂' : Mathlib.Tactic.FieldSimp.qNF M) ×                     (h
ave a := l₁.toNF;                       have a_1 := l₁'.toNF;                   
    have a_2 := L.toNF;                       Q(«$a_2».eval * «$a_1».eval = «$a»
.eval)) ×                       (have a := l₂.toNF;                         have
 a_1 := l₂'.toNF;                         have a_2 := L.toNF;                   
      Q(«$a_2».eval * «$a_1».eval = «$a».eval)) ×                         Mathli
b.Tactic.FieldSimp.DenomCondition.proof L cond)
参数：Type v；iM : Q(CommGroupWithZero «$M»)；l₁ l₂ : Mathlib.Tactic.FieldSimp.qNF M；
{u : Level} → (type : Q(Sort u)) → MetaM Q(«$type»)；cond : Mathlib.Tactic.FieldS
imp.DenomCondition q(inferInstance)；(L : Mathlib.Tactic.FieldSimp.qNF M) ×      
           (l₁' : Mathlib.Tactic.FieldSimp.qNF M) ×                   (l₂' : Mat
hlib.Tactic.FieldSimp.qNF M) ×                     (have a := l₁.toNF;          
             have a_1 := l₁'.toNF;                       have a_2 := L.toNF;    
                   Q(«$a_2».eval * «$a_1».eval = «$a».eval)) ×                  
     (have a := l₂.toNF;                         have a_1 := l₂'.toNF;          
               have a_2 := L.toNF;                         Q(«$a_2».eval * «$a_1
».eval = «$a».eval)) ×                         Mathlib.Tactic.FieldSimp.DenomCon
dition.proof L cond。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Extract a common factor `L` of two products-of-powers `l₁` and `l₂` in `M`, in t
he sense that
both `l₁` and `l₂` are quotients by `L` of products of *positive* powers.

The variable `cond` specifies whether we extract a *certified nonzero(/positive)
* (and therefore
potentially smaller) common factor. If so, the metaprogram returns a "proof" tha
t this common factor
is nonzero/positive, i.e. an expression `Q(NF.eval $(L.toNF) ≠ 0)` / `Q(0 < NF.e
val $(L.toNF))`.
-/
...
/-
**Mathlib.Tactic.FieldSimp.normalize** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Tacti
c.FieldSimp`。
形式化陈述：{v : Level} →   {M : Q(Type v)} →     ({u : Level} → (type : Q(Sort u)) → 
MetaM Q(«$type»)) →       (iM : Q(CommGroupWithZero «$M»)) →         (x : Q(«$M»
)) →           Mathlib.Tactic.AtomM             ((y : Q(«$M»)) ×               (
(g : Mathlib.Tactic.FieldSimp.Sign M) ×                   have a := g.expr y;   
                Q(«$x» = «$a»)) ×                 (l : Mathlib.Tactic.FieldSimp.
qNF M) ×                   have a := l.toNF;                   Q(«$y» = «$a».eva
l))
参数：Type v；{u : Level} → (type : Q(Sort u)) → MetaM Q(«$type»)；iM : Q(CommGroupWi
thZero «$M»)；x : Q(«$M»)；(y : Q(«$M»)) ×               ((g : Mathlib.Tactic.Fiel
dSimp.Sign M) ×                   have a := g.expr y;                   Q(«$x» =
 «$a»)) ×                 (l : Mathlib.Tactic.FieldSimp.qNF M) ×                
   have a := l.toNF;                   Q(«$y» = «$a».eval)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.FieldSimp.reduceExprQ** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.FieldSimp`。
形式化陈述：reduceExprQ (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)
) (iM : Q(CommGroupWithZero $M)) (x : Q($M)) : AtomM (Σ x' : Q($M), Q($x = $x'))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；x : Q($M)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
Given `x` in a commutative group-with-zero, construct a new expression in the st
andard form
*** / *** (all denominators at the end) which is equal to `x`.
-/
...
equivalent to `e₁ = e₂`. -/
/-
**Mathlib.Tactic.FieldSimp.reduceEqQ** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.F
ieldSimp`。
形式化陈述：reduceEqQ (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)) 
(iM : Q(CommGroupWithZero $M)) (e₁ e₂ : Q($M)) : AtomM (Σ f₁ f₂ : Q($M), Q(($e₁ 
= $e₂) = ($f₁ = $f₂)))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；e₁ e₂ : Q($M)。
该定义给出了上述对象。
...
--- 原说明 ---
Given `e₁` and `e₂`, cancel nonzero factors to construct a new equality which is
 logically
equivalent to `e₁ = e₂`.
-/
...
equivalent to `e₁ ≤ e₂`. -/
/-
**Mathlib.Tactic.FieldSimp.reduceLeQ** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.F
ieldSimp`。
形式化陈述：reduceLeQ (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)) 
(iM : Q(CommGroupWithZero $M)) (iM' : Q(PartialOrder $M)) (iM'' : Q(PosMulStrict
Mono $M)) (iM''' : Q(PosMulReflectLE $M)) (iM'''' : Q(ZeroLEOneClass $M)) (e₁ e₂
 : Q($M)) : AtomM (Σ f₁ f₂ : Q($M), Q(($e₁ <= $e₂) = ($f₁ <= $f₂)))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；iM' : Q(PartialOrder $M)；iM'' : Q(PosMulStrictMono $M)；iM''' : Q(P
osMulReflectLE $M)；iM'''' : Q(ZeroLEOneClass $M)；e₁ e₂ : Q($M)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given `e₁` and `e₂`, cancel positive factors to construct a new inequality which
 is logically
equivalent to `e₁ ≤ e₂`.
-/
...
equivalent to `e₁ < e₂`. -/
/-
**Mathlib.Tactic.FieldSimp.reduceLtQ** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.F
ieldSimp`。
形式化陈述：reduceLtQ (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)) 
(iM : Q(CommGroupWithZero $M)) (iM' : Q(PartialOrder $M)) (iM'' : Q(PosMulStrict
Mono $M)) (iM''' : Q(PosMulReflectLT $M)) (iM'''' : Q(ZeroLEOneClass $M)) (e₁ e₂
 : Q($M)) : AtomM (Σ f₁ f₂ : Q($M), Q(($e₁ < $e₂) = ($f₁ < $f₂)))
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；iM : Q(CommGrou
pWithZero $M)；iM' : Q(PartialOrder $M)；iM'' : Q(PosMulStrictMono $M)；iM''' : Q(P
osMulReflectLT $M)；iM'''' : Q(ZeroLEOneClass $M)；e₁ e₂ : Q($M)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
*** / *** (all denominators at the end) which is equal to `x`. -/
/-
**Mathlib.Tactic.FieldSimp.reduceExpr** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.
FieldSimp`。
形式化陈述：reduceExpr (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type))
 (x : Expr) : AtomM Simp.Result
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；x : Expr。
该定义给出了上述对象。
...
`a ≤ b`, `a < b`). -/
/-
**Mathlib.Tactic.FieldSimp.reduceProp** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.
FieldSimp`。
形式化陈述：reduceProp (disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type))
 (t : Expr) : AtomM Simp.Result
参数：disch : forall {u : Level} (type : Q(Sort u)), MetaM Q($type)；t : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given an (in)equality `a = b` (respectively, `a ≤ b`, `a < b`), cancel nonzero (
resp. positive)
...
/-
**Mathlib.Tactic.FieldSimp.parseDischarger** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Ta
ctic.FieldSimp`。
形式化陈述：parseDischarger (d : Option (TSyntax ``discharger)) (args : Option (TSynta
x ``simpArgs)) : TacticM (forall {u : Level} (type : Q(Sort u)), MetaM Q($type))
参数：d : Option (TSyntax ``discharger)；args : Option (TSyntax ``simpArgs)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If the user provided a discharger, elaborate it. If not, we will use the `field_
simp_discharge`
default discharger, which (among other things) includes a simp-run for the speci
fied argument list,
so we elaborate those arguments.
-/
...
-/
/-
**Mathlib.Tactic.FieldSimp.proc** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.FieldS
imp`。
形式化陈述：proc : Simp.Simproc
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
The `field` simp set is a variant of the `field_simp` tactic. The latter operate
s recursively on
subexpressions, bringing every field-expression encountered to the form `n / d`,
 and then attempts
to clear the denominator. (For confluence reasons, the `field` simprocs also hav
e a slightly
different normal form from `field_simp`'s.)

The tactic will try discharge proofs of nonzeroness of denominators, and skip st
eps if discharging
fails. These denominators are made out of denominators appearing in the input ex
pression,
by repeatedly taking products or divisors. The discharger can be non-universal, 
i.e. can be specific
to the field at hand (order properties, explicit `≠ 0` hypotheses, `CharZero` if
 that is known,
etc). See `field_simp_discharge` for full details of the discharger algorithm.

* `simp [field, t₁, ..., tₙ]` provides terms `t₁`, ..., `tₙ` to the discharger f
or
  nonzeroness/positivity proofs.
