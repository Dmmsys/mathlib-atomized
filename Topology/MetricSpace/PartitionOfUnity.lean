/-
Copyright (c) 2022 Yury Kudryashov. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yury Kudryashov
-/
module

public import Mathlib.Topology.EMetricSpace.Paracompact
public import Mathlib.Topology.Instances.ENNReal.Lemmas
public import Mathlib.Analysis.Convex.PartitionOfUnity

/-!
# Lemmas about (e)metric spaces that need partition of unity

The main lemma in this file (see `Metric.exists_continuous_real_forall_closedBall_subset`) says the
following. Let `X` be a metric space. Let `K : ι → Set X` be a locally finite family of closed sets,
let `U : ι → Set X` be a family of open sets such that `K i ⊆ U i` for all `i`. Then there exists a
positive continuous function `δ : C(X, ℝ)` such that for any `i` and `x ∈ K i`, we have
`Metric.closedBall x (δ x) ⊆ U i`. We also formulate versions of this lemma for extended metric
spaces and for different codomains (`ℝ`, `ℝ≥0`, and `ℝ≥0∞`).

We also prove a few auxiliary lemmas to be used later in a proof of the smooth version of this
lemma.

## Tags

metric space, partition of unity, locally finite
-/

public section

open Topology ENNReal NNReal Filter Set Function TopologicalSpace Metric

variable {ι X : Type*}

namespace Metric

variable [EMetricSpace X] {K : ι -> Set X} {U : ι -> Set X}

/--
theorem `eventually_nhds_zero_forall_closedEBall_subset` / 定理 `eventually_nhds_zero_forall_closedEBall_subset`

English:
theorem eventually_nhds_zero_forall_closedEBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: by
  suffices forall i, x in K i -> forallᶠ p : Real>=0∞ × X in 𝓝 0 ×ˢ 𝓝 x, closedEBall p.2 p.1 subseteq U i by
    apply mp_mem ((eventually_all_finite (hfin.point_finite x)).2 this)
      (mp_mem (@tendsto_snd Real>=0∞ _ (𝓝 0) _ _ (hfin.iInter_compl_mem_nhds hK x)) _)
    apply univ_mem'
    rintro ⟨r, y⟩ hxy hyU i hi
    simp only [mem_iInter, mem_compl_iff, not_imp_not, mem_preimage] at hxy
    exact hyU _ (hxy _ hi)
  intro i hi
  rcases nhds_basis_closedEBall.mem_iff.1 ((hU i).mem_nhds <| hKU i hi) with ⟨R, hR₀, hR⟩
  rcases ENNReal.lt_iff_exists_nnreal_btwn.mp hR₀ with ⟨r, hr₀, hrR⟩
  filter_upwards [prod_mem_prod (eventually_lt_nhds hr₀)
      (closedEBall_mem_nhds x (tsub_pos_iff_lt.2 hrR))] with p hp z hz
  apply hR
  calc
    edist z x <= edist z p.2 + edist p.2 x := edist_triangle _ _ _
_ <= p.1 + (R - p.1) := add_le_add hz le_trans hp.2 tsub_le_tsub_left hp.1.out.le _
    _ = R := add_tsub_cancel_of_le (lt_trans (by exact hp.1) hrR).le

中文:
定理 eventually_nhds_zero_对任意_closedEBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: by
  suffices forall i, x in K i -> forallᶠ p : Real>=0∞ × X in 𝓝 0 ×ˢ 𝓝 x, closedEBall p.2 p.1 subseteq U i by
    apply mp_mem ((eventually_all_finite (hfin.point_finite x)).2 this)
      (mp_mem (@tendsto_snd Real>=0∞ _ (𝓝 0) _ _ (hfin.iInter_compl_mem_nhds hK x)) _)
    apply univ_mem'
    rintro ⟨r, y⟩ hxy hyU i hi
    simp only [mem_iInter, mem_compl_iff, not_imp_not, mem_preimage] at hxy
    exact hyU _ (hxy _ hi)
  intro i hi
  rcases nhds_basis_closedEBall.mem_iff.1 ((hU i).mem_nhds <| hKU i hi) with ⟨R, hR₀, hR⟩
  rcases ENNReal.lt_iff_exists_nnreal_btwn.mp hR₀ with ⟨r, hr₀, hrR⟩
  filter_upwards [prod_mem_prod (eventually_lt_nhds hr₀)
      (closedEBall_mem_nhds x (tsub_pos_iff_lt.2 hrR))] with p hp z hz
  apply hR
  calc
    edist z x <= edist z p.2 + edist p.2 x := edist_triangle _ _ _
_ <= p.1 + (R - p.1) := add_le_add hz le_trans hp.2 tsub_le_tsub_left hp.1.out.le _
    _ = R := add_tsub_cancel_of_le (lt_trans (by exact hp.1) hrR).le

Depends on / 依赖: closedEBall, eventually_all_finite, hfin.iInter_compl_mem_nhds, hfin.point_finite, iInter_compl_mem_nhds, mem_compl_iff, mem_iInter, mem_iff, mem_nhds, mem_preimage, mp_mem, nhds_basis_closedEBall, nhds_basis_closedEBall.mem_iff, not_imp_not, point_finite, subseteq, tendsto_snd, univ_mem
-/
theorem eventually_nhds_zero_forall_closedEBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) (x : X) :
    forallᶠ p : Real>=0∞ × X in 𝓝 0 ×ˢ 𝓝 x, forall i, p.2 in K i -> closedEBall p.2 p.1 subseteq U i := by
  suffices forall i, x in K i -> forallᶠ p : Real>=0∞ × X in 𝓝 0 ×ˢ 𝓝 x, closedEBall p.2 p.1 subseteq U i by
    apply mp_mem ((eventually_all_finite (hfin.point_finite x)).2 this)
      (mp_mem (@tendsto_snd Real>=0∞ _ (𝓝 0) _ _ (hfin.iInter_compl_mem_nhds hK x)) _)
    apply univ_mem'
    rintro ⟨r, y⟩ hxy hyU i hi
    simp only [mem_iInter, mem_compl_iff, not_imp_not, mem_preimage] at hxy
    exact hyU _ (hxy _ hi)
  intro i hi
  rcases nhds_basis_closedEBall.mem_iff.1 ((hU i).mem_nhds <| hKU i hi) with ⟨R, hR₀, hR⟩
  rcases ENNReal.lt_iff_exists_nnreal_btwn.mp hR₀ with ⟨r, hr₀, hrR⟩
  filter_upwards [prod_mem_prod (eventually_lt_nhds hr₀)
      (closedEBall_mem_nhds x (tsub_pos_iff_lt.2 hrR))] with p hp z hz
  apply hR
  calc
    edist z x <= edist z p.2 + edist p.2 x := edist_triangle _ _ _
_ <= p.1 + (R - p.1) := add_le_add hz le_trans hp.2 tsub_le_tsub_left hp.1.out.le _
    _ = R := add_tsub_cancel_of_le (lt_trans (by exact hp.1) hrR).le

/--
theorem `exists_forall_closedEBall_subset_aux₁` / 定理 `exists_forall_closedEBall_subset_aux₁`

English:
theorem exists_forall_closedEBall_subset_aux₁
  statement: (hK : forall i, IsClosed (K i))
  proof: by
  have := (ENNReal.continuous_ofReal.tendsto' 0 0 ENNReal.ofReal_zero).eventually
    (eventually_nhds_zero_forall_closedEBall_subset hK hU hKU hfin x).curry
  rcases this.exists_gt with ⟨r, hr0, hr⟩
  refine ⟨r, hr.mono fun y hy => ⟨hr0, ?_⟩⟩
  rwa [mem_preimage, mem_iInter₂]

中文:
定理 存在_对任意_closedEBall_subset_aux₁
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: by
  have := (ENNReal.continuous_ofReal.tendsto' 0 0 ENNReal.ofReal_zero).eventually
    (eventually_nhds_zero_forall_closedEBall_subset hK hU hKU hfin x).curry
  rcases this.exists_gt with ⟨r, hr0, hr⟩
  refine ⟨r, hr.mono fun y hy => ⟨hr0, ?_⟩⟩
  rwa [mem_preimage, mem_iInter₂]

Depends on / 依赖: ENNReal, ENNReal.continuous_ofReal.tendsto, ENNReal.ofReal_zero, continuous_ofReal, eventually, eventually_nhds_zero_forall_closedEBall_subset, exists_gt, hr.mono, mem_preimage, ofReal_zero, tendsto, this.exists_gt
-/
theorem exists_forall_closedEBall_subset_aux₁ (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) (x : X) :
    exists r : Real, forallᶠ y in 𝓝 x,
      r in Ioi (0 : Real) inter ENNReal.ofReal ⁻¹' ⋂ (i) (_ : y in K i), { r | closedEBall y r subseteq U i } := by
  have := (ENNReal.continuous_ofReal.tendsto' 0 0 ENNReal.ofReal_zero).eventually
    (eventually_nhds_zero_forall_closedEBall_subset hK hU hKU hfin x).curry
  rcases this.exists_gt with ⟨r, hr0, hr⟩
  refine ⟨r, hr.mono fun y hy => ⟨hr0, ?_⟩⟩
  rwa [mem_preimage, mem_iInter₂]

/--
theorem `exists_forall_closedEBall_subset_aux₂` / 定理 `exists_forall_closedEBall_subset_aux₂`

English:
theorem exists_forall_closedEBall_subset_aux₂
  given: (y : X)
  proof: (convex_Ioi _).inter OrdConnected.convex OrdConnected.preimage_ennreal_ofReal
    ordConnected_iInter fun i => ordConnected_iInter fun (_ : y in K i) =>
      ordConnected_setOfPred_closedEBall_subset y (U i)

中文:
定理 存在_对任意_closedEBall_subset_aux₂
  条件: (y : X)
  证明: (convex_Ioi _).inter OrdConnected.convex OrdConnected.preimage_ennreal_ofReal
    ordConnected_iInter fun i => ordConnected_iInter fun (_ : y in K i) =>
      ordConnected_setOfPred_closedEBall_subset y (U i)

Depends on / 依赖: OrdConnected, OrdConnected.convex, OrdConnected.preimage_ennreal_ofReal, convex, convex_Ioi, ordConnected_iInter, ordConnected_setOfPred_closedEBall_subset, preimage_ennreal_ofReal
-/
theorem exists_forall_closedEBall_subset_aux₂ (y : X) :
    Convex Real
      (Ioi (0 : Real) inter ENNReal.ofReal ⁻¹' ⋂ (i) (_ : y in K i), { r | closedEBall y r subseteq U i }) :=
(convex_Ioi _).inter OrdConnected.convex OrdConnected.preimage_ennreal_ofReal
    ordConnected_iInter fun i => ordConnected_iInter fun (_ : y in K i) =>
      ordConnected_setOfPred_closedEBall_subset y (U i)

/--
theorem `exists_continuous_real_forall_closedEBall_subset` / 定理 `exists_continuous_real_forall_closedEBall_subset`

English:
theorem exists_continuous_real_forall_closedEBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: by
  simpa only [mem_inter_iff, forall_and, mem_preimage, mem_iInter, @forall_comm ι X] using!
    exists_continuous_forall_mem_convex_of_local_const exists_forall_closedEBall_subset_aux₂
      (exists_forall_closedEBall_subset_aux₁ hK hU hKU hfin)

中文:
定理 存在_continuous_real_对任意_closedEBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: by
  simpa only [mem_inter_iff, forall_and, mem_preimage, mem_iInter, @forall_comm ι X] using!
    exists_continuous_forall_mem_convex_of_local_const exists_forall_closedEBall_subset_aux₂
      (exists_forall_closedEBall_subset_aux₁ hK hU hKU hfin)

Depends on / 依赖: exists_continuous_forall_mem_convex_of_local_const, forall_and, forall_comm, mem_iInter, mem_inter_iff, mem_preimage
-/
theorem exists_continuous_real_forall_closedEBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) :
    exists δ : C(X, Real), (forall x, 0 < δ x) ∧
      forall (i), forall x in K i, closedEBall x (ENNReal.ofReal <| δ x) subseteq U i := by
  simpa only [mem_inter_iff, forall_and, mem_preimage, mem_iInter, @forall_comm ι X] using!
    exists_continuous_forall_mem_convex_of_local_const exists_forall_closedEBall_subset_aux₂
      (exists_forall_closedEBall_subset_aux₁ hK hU hKU hfin)

/--
theorem `exists_continuous_nnreal_forall_closedEBall_subset` / 定理 `exists_continuous_nnreal_forall_closedEBall_subset`

English:
theorem exists_continuous_nnreal_forall_closedEBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: by
  rcases exists_continuous_real_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ₀, hδ⟩
  lift δ to C(X, Real>=0) using fun x => (hδ₀ x).le
  refine ⟨δ, hδ₀, fun i x hi => ?_⟩
  simpa only [← ENNReal.ofReal_coe_nnreal] using! hδ i x hi

中文:
定理 存在_continuous_nnreal_对任意_closedEBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: by
  rcases exists_continuous_real_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ₀, hδ⟩
  lift δ to C(X, Real>=0) using fun x => (hδ₀ x).le
  refine ⟨δ, hδ₀, fun i x hi => ?_⟩
  simpa only [← ENNReal.ofReal_coe_nnreal] using! hδ i x hi

Depends on / 依赖: ENNReal, ENNReal.ofReal_coe_nnreal, exists_continuous_real_forall_closedEBall_subset, ofReal_coe_nnreal
-/
theorem exists_continuous_nnreal_forall_closedEBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) :
    exists δ : C(X, Real>=0), (forall x, 0 < δ x) ∧ forall (i), forall x in K i, closedEBall x (δ x) subseteq U i := by
  rcases exists_continuous_real_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ₀, hδ⟩
  lift δ to C(X, Real>=0) using fun x => (hδ₀ x).le
  refine ⟨δ, hδ₀, fun i x hi => ?_⟩
  simpa only [← ENNReal.ofReal_coe_nnreal] using! hδ i x hi

/--
theorem `exists_continuous_ennreal_forall_closedEBall_subset` / 定理 `exists_continuous_ennreal_forall_closedEBall_subset`

English:
theorem exists_continuous_ennreal_forall_closedEBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, ENNReal.continuous_coe⟩ δ, fun x => ENNReal.coe_pos.2 (hδ₀ x), hδ⟩

中文:
定理 存在_continuous_ennreal_对任意_closedEBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, ENNReal.continuous_coe⟩ δ, fun x => ENNReal.coe_pos.2 (hδ₀ x), hδ⟩

Depends on / 依赖: Coe.coe, ContinuousMap, ContinuousMap.comp, ENNReal, ENNReal.coe_pos, ENNReal.continuous_coe, coe_pos, continuous_coe, exists_continuous_nnreal_forall_closedEBall_subset
-/
theorem exists_continuous_ennreal_forall_closedEBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) :
    exists δ : C(X, Real>=0∞), (forall x, 0 < δ x) ∧ forall (i), forall x in K i, closedEBall x (δ x) subseteq U i :=
  let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, ENNReal.continuous_coe⟩ δ, fun x => ENNReal.coe_pos.2 (hδ₀ x), hδ⟩

end Metric

namespace EMetric
open Metric

@[deprecated (since := "2026-01-24")]
alias eventually_nhds_zero_forall_closedBall_subset :=
  eventually_nhds_zero_forall_closedEBall_subset

@[deprecated (since := "2026-01-24")]
alias exists_forall_closedBall_subset_aux₁ := exists_forall_closedEBall_subset_aux₁

@[deprecated (since := "2026-01-24")]
alias exists_forall_closedBall_subset_aux₂ := exists_forall_closedEBall_subset_aux₂

@[deprecated (since := "2026-01-24")]
alias exists_continuous_real_forall_closedBall_subset :=
  exists_continuous_real_forall_closedEBall_subset

@[deprecated (since := "2026-01-24")]
alias exists_continuous_nnreal_forall_closedBall_subset :=
  exists_continuous_nnreal_forall_closedEBall_subset

@[deprecated (since := "2026-01-24")]
alias exists_continuous_eNNReal_forall_closedBall_subset :=
  exists_continuous_ennreal_forall_closedEBall_subset

end EMetric

namespace Metric

variable [MetricSpace X] {K : ι -> Set X} {U : ι -> Set X}

/--
theorem `exists_continuous_nnreal_forall_closedBall_subset` / 定理 `exists_continuous_nnreal_forall_closedBall_subset`

English:
theorem exists_continuous_nnreal_forall_closedBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: by
  rcases Metric.exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ0, hδ⟩
  refine ⟨δ, hδ0, fun i x hx => ?_⟩
  rw [← Metric.closedEBall_coe]
  exact hδ i x hx

中文:
定理 存在_continuous_nnreal_对任意_closedBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: by
  rcases Metric.exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ0, hδ⟩
  refine ⟨δ, hδ0, fun i x hx => ?_⟩
  rw [← Metric.closedEBall_coe]
  exact hδ i x hx

Depends on / 依赖: Metric, Metric.closedEBall_coe, Metric.exists_continuous_nnreal_forall_closedEBall_subset, closedEBall_coe, exists_continuous_nnreal_forall_closedEBall_subset
-/
theorem exists_continuous_nnreal_forall_closedBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) :
    exists δ : C(X, Real>=0), (forall x, 0 < δ x) ∧ forall (i), forall x in K i, closedBall x (δ x) subseteq U i := by
  rcases Metric.exists_continuous_nnreal_forall_closedEBall_subset hK hU hKU hfin with ⟨δ, hδ0, hδ⟩
  refine ⟨δ, hδ0, fun i x hx => ?_⟩
  rw [← Metric.closedEBall_coe]
  exact hδ i x hx

/--
theorem `exists_continuous_real_forall_closedBall_subset` / 定理 `exists_continuous_real_forall_closedBall_subset`

English:
theorem exists_continuous_real_forall_closedBall_subset
  statement: (hK : forall i, IsClosed (K i))
  proof: let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, NNReal.continuous_coe⟩ δ, hδ₀, hδ⟩

中文:
定理 存在_continuous_real_对任意_closedBall_subset
  结论: (hK : 对任意 i, 是闭集 (K i))
  证明: let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, NNReal.continuous_coe⟩ δ, hδ₀, hδ⟩

Depends on / 依赖: Coe.coe, ContinuousMap, ContinuousMap.comp, NNReal, NNReal.continuous_coe, continuous_coe, exists_continuous_nnreal_forall_closedBall_subset
-/
theorem exists_continuous_real_forall_closedBall_subset (hK : forall i, IsClosed (K i))
    (hU : forall i, IsOpen (U i)) (hKU : forall i, K i subseteq U i) (hfin : LocallyFinite K) :
    exists δ : C(X, Real), (forall x, 0 < δ x) ∧ forall (i), forall x in K i, closedBall x (δ x) subseteq U i :=
  let ⟨δ, hδ₀, hδ⟩ := exists_continuous_nnreal_forall_closedBall_subset hK hU hKU hfin
  ⟨ContinuousMap.comp ⟨Coe.coe, NNReal.continuous_coe⟩ δ, hδ₀, hδ⟩

end Metric
