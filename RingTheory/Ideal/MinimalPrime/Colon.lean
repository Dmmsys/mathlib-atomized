...
/--
theorem `exists_eq_colon_of_mem_minimalPrimes` / 定理 `exists_eq_colon_of_mem_minimalPrimes`

English:
theorem exists_eq_colon_of_mem_minimalPrimes
...
theorem exists_eq_colon_of_mem_minimalPrimes [IsNoetherianRing R]
    (hI : I in (N.colon {x}).minimalPrimes) : exists x' : M, I = N.colon {x'} := by
  by_cases hx : x in N
  · simp [show (colon N {x}) = ⊤ by simpa, Ideal.minimalPrimes_top] at hI
  classical
  -- `I` is a minimal prime over `ann = colon N {x}`
