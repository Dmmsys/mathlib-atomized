...
/--
theorem `mul_dist_add_mul_dist_eq_mul_dist_of_cospherical` / 定理 `mul_dist_add_mul_dist_eq_mul_dist_of_cospherical`
...
theorem mul_dist_add_mul_dist_eq_mul_dist_of_cospherical
  ...
  have h₄ : forall x y : Real, x * (y * x) = x * x * y := fun x y => by rw [mul_left_comm, mul_comm]
  simp [field, h₁, h₂, dist_eq_add_dist_of_angle_eq_pi hbpd, h₃, dist_comm a b, h₄, ← sq,
    dist_sq_mul_dist_add_dist_sq_mul_dist b, hapc]

中文:
定理 mul_dist_add_mul_dist_eq_mul_dist_of_cospherical
  结论: {a b c d p : P}
  证明: by
  have h' : Cospherical ({a, c, b, d} : Set P) := by rwa [Set.insert_comm c b {d}]
  have hmul := mul_dist_eq_mul_dist_of_cospherical_of_angle_eq_pi h' hapc hbpd
...
Depends on / 依赖: Cospherical, Set.insert_comm, all_goals, angle_comm, angle_eq_angle_of_angle_eq_pi_of_angle_eq_pi, dist_comm, dist_mul_of_eq_angle_of_dist_mul, insert_comm, left_dist_ne_zero_of_angle_eq_pi, mul_comm, mul_dist_eq_mul_dist_of_cospherical_of_angle_eq_pi
-/
