...
/--
lemma `foldl_cons_nil` / 引理 `foldl_cons_nil`

English:
lemma foldl_cons_nil
  statement: l.foldl (flip cons) [] = l.reverse
  proof: by
  induction l <;> simp [flip, foldl_eq_foldr_reverse, -foldr_reverse]

中文:
引理 foldl_cons_nil
  结论: l.foldl (flip cons) [] = l.reverse
  证明: by
  induction l <;> simp [flip, foldl_eq_foldr_reverse, -foldr_reverse]

Depends on / 依赖: foldl_eq_foldr_reverse, foldr_reverse
-/
lemma foldl_cons_nil : l.foldl (flip cons) [] = l.reverse := by
  induction l <;> simp [flip, foldl_eq_foldr_reverse, -foldr_reverse]

/--
lemma `foldl_cons_eq_apply_foldl` / 引理 `foldl_cons_eq_apply_foldl`

English:
lemma foldl_cons_eq_apply_foldl
  given: [hv : RightCommutative v]
  proof: by
  rw [foldl_cons]
...
Depends on / 依赖: foldl_cons, generalizing, hv.right_comm, right_comm
-/
...
/--
lemma `foldr_cons_eq_foldr_apply` / 引理 `foldr_cons_eq_foldr_apply`

English:
lemma foldr_cons_eq_foldr_apply
  given: [hf : LeftCommutative f]
  proof: by
  rw [foldr_cons]
...
Depends on / 依赖: foldr_cons, generalizing, hf.left_comm, left_comm
-/
...
/--
lemma `foldl1_eq_foldr1` / 引理 `foldl1_eq_foldr1`

English:
lemma foldl1_eq_foldr1
  given: {f : α -> α -> α} [ha : Std.Associative f] {a b : α}
  proof: by
...
Depends on / 依赖: generalizing, ha.assoc
-/
...
/--
theorem `foldl_eq_foldr_of_commute` / 定理 `foldl_eq_foldr_of_commute`

English:
theorem foldl_eq_foldr_of_commute
  given: {f : α -> α -> α} [Std.Associative f] (ha : forall x, f a x = f x a)
  proof: by
...
Depends on / 依赖: foldl_assoc
-/
theorem foldl_eq_foldr_of_commute {f : α -> α -> α} [Std.Associative f] (ha : forall x, f a x = f x a) :
    l.foldl f a = l.foldr f a := by
  induction l <;> simp [*, foldl_assoc]

/--
theorem `foldl_eq_foldr` / 定理 `foldl_eq_foldr`

English:
theorem foldl_eq_foldr
  given: {f : α -> α -> α} [hf : Std.Commutative f] [Std.Associative f]
  proof: foldl_eq_foldr_of_commute (hf.comm a)
...
/--
theorem `foldl_flip_eq_foldr` / 定理 `foldl_flip_eq_foldr`

English:
theorem foldl_flip_eq_foldr
  given: [LeftCommutative f]
  statement: l.foldl (flip f) b = l.foldr f b
  proof: by
...
Depends on / 依赖: foldr_cons, foldr_cons_eq_foldr_apply, generalizing
-/
theorem foldl_flip_eq_foldr [LeftCommutative f] : l.foldl (flip f) b = l.foldr f b := by
  induction l generalizing b <;> simp [*, flip, foldr_cons_eq_foldr_apply, -foldr_cons]

/--
theorem `foldr_flip_eq_foldl` / 定理 `foldr_flip_eq_foldl`

English:
theorem foldr_flip_eq_foldl
  given: [RightCommutative v]
  statement: l.foldr (flip v) b = l.foldl v b
  proof: by
