...
/--
lemma `isTightMeasureSet_of_tendsto_charFun` / 引理 `isTightMeasureSet_of_tendsto_charFun`

English:
lemma isTightMeasureSet_of_tendsto_charFun
  statement: {μ : Nat -> Measure E} [forall i, IsProbabilityMeasure (μ i)]
  proof: by
...
引理 isTightMeasureSet_of_tendsto_charFun
  ...
  have h_le_4 n r (hr : 0
-/
lemma isTightMeasureSet_of_tendsto_charFun {μ : Nat -> Measure E} [forall i, IsProbabilityMeasure (μ i)]
    {f : E -> Complex} (hf : ContinuousAt f 0)
    (h : forall t, Tendsto (fun n => charFun (μ n) t) atTop (𝓝 (f t))) :
  ...
  have h_le_4 n r (hr : 0 < r) :
    ...
    _ <= 2⁻¹ * r * ∫ t in -(2 * r⁻¹)..2 * r⁻¹, 2 := by
      gcongr
      rw [intervalIntegral.integral_of_le hr']; rw [intervalIntegral.integral_of_le hr']
...
· exact fun _ => Measurable.aestronglyMeasurable by fun_prop
  ...
  obtain ⟨δ, hδ, hδ_lt⟩ : exists δ, 0 < δ ∧ forall ⦃x : E⦄, ‖x‖ < δ -> ‖1 - f x‖ < ε / 4 :=
    hf_tendsto (ε / 4) (by positivity)
  refine ⟨4 * δ⁻¹, fun r hrδ => ?_⟩
  have hr : 0 < r := lt_of_lt_of_le (by positivity) hrδ
  have hr' : -(2 * r⁻¹) <= 2 * r⁻¹ := by rw [neg_le_self_iff]; positivity
  have h_le_Ioc x (hx : x in Set.Ioc (-(2 * r⁻¹)) (2 * r⁻¹)) : ‖1 - f (x • z)‖ <= ε / 4 := by
    refine (hδ_lt ?_).le
    simp only [norm_smul, Real.norm_eq_abs, mul_one, hz]
    calc |x|
    _ <= 2 * r⁻¹ := by grind
    _ < δ := by
      rw [← lt_div_iff₀' (by positivity)]; rw [inv_lt_comm₀ hr (by positivity)]
...
/--
lemma `ProbabilityMeasure.tendsto_of_tight_of_separatesPoints` / 引理 `ProbabilityMeasure.tendsto_of_tight_of_separatesPoints`

English:
lemma ProbabilityMeasure.tendsto_of_tight_of_separatesPoints
...
Depends on / 依赖: Filter, Filter.tendsto_iff_ultrafilter, IsCompact, Measure, Tendsto, TopologicalSpace, TopologicalSpace.upgradeIsCompletelyMetrizable, U.map, closure, convert, eq_or_neBot, ext_of_forall_mem_sub, h_compact, h_compact.ultrafilter_le_nhds, h_tight, isCompact_closure_of_isTightMeasureSet, monotone_principal, subset_closure, tendsto_iff_ultrafilter, ultrafilter_le_nhds
-/
lemma ProbabilityMeasure.tendsto_of_tight_of_separatesPoints (𝕜 : Type*) [RCLike 𝕜]
    ...
    (hμ : forall g in A, Tendsto (fun n => ∫ x, g x ∂(μ n)) 𝓕 (𝓝 (∫ x, g x ∂μ₀))) :
    Tendsto μ 𝓕 (𝓝 μ₀) := by
  let := TopologicalSpace.upgradeIsCompletelyMetrizable E
  obtain rfl | _ := 𝓕.eq_or_neBot
  · simp
...
/--
lemma `ProbabilityMeasure.tendsto_charPoly_of_tendsto_charFun` / 引理 `ProbabilityMeasure.tendsto_charPoly_of_tendsto_charFun`

English:
lemma ProbabilityMeasure.tendsto_charPoly_of_tendsto_charFun
  statement: {μ : ι -> ProbabilityMeasure E}
  proof: by
  rw [mem_charPoly] at hg
  obtain ⟨w, hw⟩ := hg
  have h_eq (μ : Measure E) (hμ : IsProbabilityMeasure μ) :
      ∫ x, g x ∂μ = w.coeff.sum (fun a z => z * ∫ x, (probChar (innerₗ E x a) : Complex) ∂μ) := by
...
lemma ProbabilityMeasure.tendsto_charPoly_of_tendsto_charFun {μ : ι -> ProbabilityMeasure E}
    (h : forall t : E, Tendsto (fun n => charFun (μ n) t) 𝓕 (𝓝 (charFun μ₀ t)))
    {g : E ->ᵇ Complex} (hg : g in charPoly continuous_probChar (L := innerₗ E) continuous_inner) :
...
variable {μ : Nat -> ProbabilityMeasure E}

/--
lemma `ProbabilityMeasure.tendsto_of_tendsto_charFun` / 引理 `ProbabilityMeasure.tendsto_of_tendsto_charFun`
...
lemma ProbabilityMeasure.tendsto_of_tendsto_charFun
  ...
  refine tendsto_of_tight_of_separatesPoints h_tight (𝕜 := Complex)
...
Depends on / 依赖: DFunLike, DFunLike.ne_iff.mpr, IsTightMeasureSet, charPoly, continuous_inner, continuous_probChar, fun_prop, h_tight, inner_self_ne_zero, inner_self_ne_zero.mpr, isTightMeasureSet_of_tendsto_charFun, ne_iff, probChar_ne_one, separatesPoints_charPoly, tendsto_charPoly_of_tendsto_charFun, tendsto_of_tight_of_separatesPoints
-/
lemma ProbabilityMeasure.tendsto_of_tendsto_charFun
    (h : forall t : E, Tendsto (fun n => charFun (μ n) t) atTop (𝓝 (charFun μ₀ t))) :
...
/--
theorem `ProbabilityMeasure.tendsto_iff_tendsto_charFun` / 定理 `ProbabilityMeasure.tendsto_iff_tendsto_charFun`
...
Depends on / 依赖: ProbabilityMeasure, ProbabilityMeasure.tendsto_iff_forall_integral_rclike_tendsto, charFun_eq_integral_innerProbChar, innerProbChar, simp_rw, tendsto_iff_forall_integral_rclike_tendsto, tendsto_of_tendsto_charFun
-/
theorem ProbabilityMeasure.tendsto_iff_tendsto_charFun :
    Tendsto μ atTop (𝓝 μ₀) ↔
      forall t : E, Tendsto (fun n => charFun (μ n) t) atTop (𝓝 (charFun μ₀ t)) := by
  refine ⟨fun h t => ?_, tendsto_of_tendsto_charFun⟩
  rw [ProbabilityMeasure.tendsto_iff_forall_integral_rclike_tendsto Complex] at h
  simp_rw [charFun_eq_integral_innerProbChar]
  exact h (innerProbChar t)
