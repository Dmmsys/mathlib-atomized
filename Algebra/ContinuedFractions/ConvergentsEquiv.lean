...
/--
Definition of `squashSeq` / `squashSeq` 的定义

English:
definition squashSeq
  signature: (s : Stream'.Seq <| Pair K) (n : Nat)
  body: match Prod.mk (s.get? n) (s.get? (n + 1)) with
...
Depends on / 依赖: Prod.mk, Seq.nats.zipWith, Stream, gp_n, gp_succ_n, s.get, zipWith
-/
def squashSeq (s : Stream'.Seq <| Pair K) (n : Nat) : Stream'.Seq (Pair K) :=
...
/--
theorem `squashSeq_eq_self_of_terminated` / 定理 `squashSeq_eq_self_of_terminated`

English:
theorem squashSeq_eq_self_of_terminated
  given: (terminatedAt_succ_n : s.TerminatedAt (n + 1))
  proof: by
  change s.get? (n + 1) = none at terminatedAt_succ_n
  cases s_nth_eq : s.get? n <;> simp only [*, squashSeq]

中文:
定理 squashSeq_eq_self_of_terminated
  条件: (terminatedAt_succ_n : s.TerminatedAt (n + 1))
  证明: by
  change s.get? (n + 1) = none at terminatedAt_succ_n
  cases s_nth_eq : s.get? n <;> simp only [*, squashSeq]

Depends on / 依赖: s.get, s_nth_eq, squashSeq, terminatedAt_succ_n
-/
...
/--
theorem `squashSeq_nth_of_not_terminated` / 定理 `squashSeq_nth_of_not_terminated`

English:
theorem squashSeq_nth_of_not_terminated
  statement: {gp_n gp_succ_n : Pair K} (s_nth_eq : s.get? n = some gp_n)
  proof: by
  simp [*, squashSeq]
...
/--
theorem `squashSeq_nth_of_lt` / 定理 `squashSeq_nth_of_lt`

English:
theorem squashSeq_nth_of_lt
  given: {m : Nat} (m_lt_n : m < n)
  statement: (squashSeq s n).get? m = s.get? m
  proof: by
  cases s_succ_nth_eq : s.get? (n + 1) with
  | none => rw [squashSeq_eq_self_of_terminated s_succ_nth_eq]
  | some =>
    obtain ⟨gp_n, s_nth_eq⟩ : exists gp_n, s.get? n = some gp_n :=
      s.ge_stable n.le_succ s_succ_nth_eq
    simp [*, squashSeq, m_lt_n.ne]

中文:
定理 squashSeq_nth_of_lt
  条件: {m : 自然数} (m_lt_n : m < n)
  结论: (squashSeq s n).get? m = s.get? m
  证明: by
  cases s_succ_nth_eq : s.get? (n + 1) with
  | none => rw [squashSeq_eq_self_of_terminated s_succ_nth_eq]
  | some =>
...
Depends on / 依赖: ge_stable, gp_n, le_succ, m_lt_n, m_lt_n.ne, n.le_succ, s.ge_stable, s.get, s_nth_eq, s_succ_nth_eq, squashSeq, squashSeq_eq_self_of_terminated
-/
theorem squashSeq_nth_of_lt {m : Nat} (m_lt_n : m < n) : (squashSeq s n).get? m = s.get? m := by
  cases s_succ_nth_eq : s.get? (n + 1) with
  | none => rw [squashSeq_eq_self_of_terminated s_succ_nth_eq]
  | some =>
    obtain ⟨gp_n, s_nth_eq⟩ : exists gp_n, s.get? n = some gp_n :=
...
/--
theorem `squashSeq_succ_n_tail_eq_squashSeq_tail_n` / 定理 `squashSeq_succ_n_tail_eq_squashSeq_tail_n`
...
theorem squashSeq_succ_n_tail_eq_squashSeq_tail_n
  ...
  | some gp_succ_succ_n =>
    obtain ⟨gp_succ_n, s_succ_nth_eq⟩ : exists gp_succ_n, s.get? (n + 1) = som

中文:
定理 squashSeq_succ_n_tail_eq_squashSeq_tail_n
  证明: by
  cases s_succ_succ_nth_eq : s.get? (n + 2) with
  | none =>
    cases s_succ_nth_eq : s.get? (n + 1) <;>
...
Depends on / 依赖: Seq.get, Stream, _tail, ge_stable, gp_succ_n, gp_succ_succ_n, le_succ, s.ge_stable, s.get, s_succ_nth_eq, s_succ_succ_nth_eq, squashSeq
-/
theorem squashSeq_succ_n_tail_eq_squashSeq_tail_n :
  ...
  | none =>
    cases s_succ_nth_eq : s.get? (n + 1) <;>
      simp only [squashSeq, Stream'.Seq.get?_tail, s_succ_nth_eq, s_succ_succ_nth_eq]
  | some gp_succ_succ_n =>
    obtain ⟨gp_succ_n, s_succ_nth_eq⟩ : exists gp_succ_n, s.get? (n + 1) = some gp_succ_n :=
...
/--
theorem `succ_succ_nth_conv'Aux_eq_succ_nth_conv'Aux_squashSeq` / 定理 `succ_succ_nth_conv'Aux_eq_succ_nth_conv'Aux_squashSeq`
...
cases s_succ_nth_eq : s.get? n + 1 with
  | none =>
    rw [squashSeq_eq_self_of_terminated s_succ_nth_eq]; rw [convs'Aux_stable_step_of_terminated s_succ_nth_eq]
  | some gp_succ_n =>
    induction n generalizing s gp_succ_n with
    | zero =>
      obtain ⟨gp_head, s_head_eq⟩ : exists gp_head, 

中文:
定理 succ_succ_nth_conv'Aux_eq_succ_nth_conv'Aux_squashSeq
  证明: by
cases s_succ_nth_eq : s.get? n + 1 with
  | none =>
    rw [squashSeq_eq_self_of_terminated s_succ_nth_eq]; rw [convs'Aux_stable_step_of_terminated s_succ_nth_eq]
...
Depends on / 依赖: Aux_stable_step_of_terminated, ge_stable, generalizing, gp_head, gp_head.a, gp_head.b, gp_succ_n, gp_succ_n.a, gp_succ_n.b, s.ge_stable, s.get, s.head, s_head_eq, s_succ_nth_eq, squashSeq, squashSeq_eq_self_of_terminated, squashSeq_nth_of_not_terminated, zero_le_one
-/
theorem succ_succ_nth_conv'Aux_eq_succ_nth_conv'Aux_squashSeq :
...
cases s_succ_nth_eq : s.get? n + 1 with
  ...
  | some gp_succ_n =>
    induction n generalizing s gp_succ_n with
    | zero =>
      obtain ⟨gp_head, s_head_eq⟩ : exists gp_head, s.head = some gp_head :=
...
/--
Definition of `squashGCF` / `squashGCF` 的定义

English:
definition squashGCF
...
定义 squashGCF
  签名: (g : GenContFract K)
-/
def squashGCF (g : GenContFract K) : Nat -> GenContFract K
  | 0 =>
...
/--
theorem `squashGCF_eq_self_of_terminated` / 定理 `squashGCF_eq_self_of_terminated`

English:
theorem squashGCF_eq_self_of_terminated
  given: (terminatedAt_n : TerminatedAt g n)
  proof: by
  cases n with
  | zero =>
...
中文:
定理 squashGCF_eq_self_of_terminated
  条件: (terminatedAt_n : TerminatedAt g n)
  证明: by
  cases n with
  | zero =>
    change g.s.get? 0 = none at terminatedAt_n
...
Depends on / 依赖: Algebra, Algebra.lmul, Subring, Subring.center, center, charP_of_injective_ringHom, g.s.get, injective, mk.injEq, squashGCF, squashSeq_eq_self_of_terminated, terminatedAt_n, toRingHom, toRingHom.injective, true_and
-/
...
/--
theorem `squashGCF_nth_of_lt` / 定理 `squashGCF_nth_of_lt`

English:
theorem squashGCF_nth_of_lt
  given: {m : Nat} (m_lt_n : m < n)
  proof: by
  simp only [squashGCF, squashSeq_nth_of_lt m_lt_n]

中文:
定理 squashGCF_nth_of_lt
  条件: {m : 自然数} (m_lt_n : m < n)
  证明: by
  simp only [squashGCF, squashSeq_nth_of_lt m_lt_n]

Depends on / 依赖: Algebra, Algebra.lmul, Subring, Subring.center, center, expChar_of_injective_ringHom, injective, m_lt_n, squashGCF, squashSeq_nth_of_lt, toRingHom, toRingHom.injective
-/
theorem squashGCF_nth_of_lt {m : Nat} (m_lt_n : m < n) :
    (squashGCF g (n + 1)).s.get? m = g.s.get? m := by
  simp only [squashGCF, squashSeq_nth_of_lt m_lt_n]

/--
theorem `succ_nth_conv'_eq_squashGCF_nth_conv'` / 定理 `succ_nth_conv'_eq_squashGCF_nth_conv'`
...
中文:
定理 succ_nth_conv'_eq_squashGCF_nth_conv'
  证明: by
  cases n with
  | zero =>
    cases g_s_head_eq : g.s.get? 0 <;>
...
Depends on / 依赖: Aux_eq_succ_nth_conv, Aux_squashSeq, Seq.head, Stream, g.s.get, g_s_head_eq, squashGCF, succ_succ_nth_conv
-/
theorem succ_nth_conv'_eq_squashGCF_nth_conv' :
...
/--
theorem `contsAux_eq_contsAux_squashGCF_of_le` / 定理 `contsAux_eq_contsAux_squashGCF_of_le`

English:
theorem contsAux_eq_contsAux_squashGCF_of_le
  given: {m : Nat}
  proof: Nat.strong_induction_on m
    (by
      clear m
      intro m IH m_le_n
      rcases m with - | m'
      ...
      · rcases n with - | n'
        ...
        · rcases m' with - | m''
          · rfl
          · -- get some inequalities to inst

中文:
定理 contsAux_eq_contsAux_squashGCF_of_le
  条件: {m : 自然数}
  证明: Nat.strong_induction_on m
    (by
      clear m
      intro m IH m_le_n
      rcases m with - | m'
...
Depends on / 依赖: Nat.strong_induction_on, m_le_n, not_succ_le_zero, strong_induction_on
-/
theorem contsAux_eq_contsAux_squashGCF_of_le {m : Nat} :
...
/--
theorem `succ_nth_conv_eq_squashGCF_nth_conv` / 定理 `succ_nth_conv_eq_squashGCF_nth_conv`

English:
theorem succ_nth_conv_eq_squashGCF_nth_conv
  statement: [Field K]
  proof: by
  ...
  · obtain ⟨⟨a, b⟩, s_nth_eq⟩ : exists gp_n, g.s.get? n = some
...
Depends on / 依赖: Decidable, Decidable.em, Option.ne_none_iff_exists, TerminatedAt, b_ne_zero, convs_stable_of_terminated, g.TerminatedAt, g.s.get, gp_n, le_succ, n.le_succ, ne_none_iff_exists, not_terminatedAt_n, nth_partDen_ne_zero, partDen_eq_s_b, s_nth_eq, squashGCF, squashGCF_eq_self_of_terminated, terminatedAt_n
-/
theorem succ_nth_conv_eq_squashGCF_nth_conv [Field K]
    (nth_partDen_ne_zero : forall {b : K}, g.partDens.get? n = some b -> b != 0) :
    g.convs (n + 1) = (squashGCF g n).convs n := by
  rcases Decidable.em (g.TerminatedAt n) with terminatedAt_n | not_terminatedAt_n
  · have : squashGCF g n = g := squashGCF_eq_self_of_terminated terminatedAt_n
    simp only [this, convs_stable_of_terminated n.le_succ terminatedAt_n]
  · obtain ⟨⟨a, b⟩, s_nth_eq⟩ : exists gp_n, g.s.get? n = some gp_n :=
    ...
    | zero =>
      ...
      grind
    | succ n' =>
      obtain ⟨⟨pa, pb⟩, s_n'th_eq⟩ : exists gp_n', g.s.get? n' = some gp_n' :=
      ...
      have : g'.convs (n' + 1) =
        ...
        rw [conv_eq_conts_a_div_conts_b]; rw [conts_recurrenceAux this n'th_contsAux_eq'.symm succ_n'th_contsAux_eq'.symm]
      rw [this]
      ...
      have : g.convs (n' + 2) =
        ...
        -- and a second time
        rw [conv_eq_conts_a_div_conts_b]; rw [conts_recurrenceAux s_nth_eq succ_n'th_contsAux_eq.symm this]
      rw [this]
      suffices
...
/--
theorem `convs_eq_convs'` / 定理 `convs_eq_convs'`

English:
theorem convs_eq_convs'
  statement: [Field K] [LinearOrder K] [IsStrictOrderedRing K]
  proof: by
  ...
  | succ n IH =>
    ...
    rcases Decidable.em (TerminatedAt g n) wit
...
Depends on / 依赖: generalizing, squashGCF
-/
theorem convs_eq_convs' [Field K] [LinearOrder K] [IsStrictOrderedRing K]
    (s_pos : forall {gp : Pair K} {m : Nat}, m < n -> g.s.get? m = some gp -> 0 < gp.a ∧ 0 < gp.b) :
  ...
  | succ n IH =>
    ...
    · have g'_eq_g : g' = g := squashGCF_eq_self_of_terminated terminatedAt_n
      rw [convs_stable_of_terminated n.le_succ terminatedAt_n]; rw [g'_eq_g]; rw [IH _]
    ...
    · suffices g.convs (n + 1) = g'.convs n by
        ...
        · -- the difficult case at the squashed position: we first obtain the values from
          -- the sequence
          obtain ⟨gp_succ_m, s_succ_mth_eq⟩ : exists gp_succ_m, g.s.get? (m + 1) = some gp_succ_m :=
            Option.ne_none_iff_exists'.mp not_terminatedAt_n
          obtain ⟨gp_m, mth_s_eq⟩ : exists gp_m, g.s.get? m = some gp_m :=
      ...
      have : forall ⦃b⦄, g.partDens.get? n = some b -> b != 0 := by
...
Depends on / 依赖: GenContFract, GenContFract.convs_eq_convs, SimpContFract, c.property, convs_eq_convs, gp.a, gp.b, partDen_eq_s_b, partNum_eq_s_a, property, s_nth_eq, symm.le, trans_le, zero_lt_one, zero_lt_one.trans_le
-/
theorem convs_eq_convs' [Field K] [LinearOrder K] [IsStrictOrderedRing K]
    {c : ContFract K} :
