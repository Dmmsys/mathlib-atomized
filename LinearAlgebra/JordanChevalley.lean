...
/--
theorem `exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow` / 定理 `exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow`

English:
theorem exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow
  statement: {P : K[X]} {k : Nat}
  proof: by
...
-/
theorem exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow {P : K[X]} {k : Nat}
    (sep : P.Separable) (nil : minpoly K f ∣ P ^ k) :
    existsᵉ (n in adjoin K {f}) (s in adjoin K {f}), IsNilpotent n ∧ IsSemisimple s ∧ f = n + s := by
  set ff : adjoin K {f} := ⟨f, self_mem_adjoin_singleton K f⟩
  set P' := derivative P
  have nil' : IsNilpotent (aeval ff P) := by
    use k
...
/--
theorem `exists_isNilpotent_isSemisimple` / 定理 `exists_isNilpotent_isSemisimple`

English:
theorem exists_isNilpotent_isSemisimple
  given: [PerfectField K]
  proof: by
...
Depends on / 依赖: PerfectField, PerfectField.separable_iff_squarefree, exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow, exists_squarefree_dvd_pow_of_ne_zero, minpoly, minpoly.ne_zero_of_finite, ne_zero_of_finite, separable_iff_squarefree
-/
...
/--
theorem `isNilpotent_isSemisimple_unique` / 定理 `isNilpotent_isSemisimple_unique`

English:
theorem isNilpotent_isSemisimple_unique
  statement: [PerfectField K]
  proof: by
