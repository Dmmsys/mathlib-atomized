/-
Copyright (c) 2017 Johannes Hölzl. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Johannes Hölzl
-/
module

public import Mathlib.Data.Finset.Preimage
public import Mathlib.Algebra.BigOperators.Group.Finset.Basic

/-!
# Sums and products over preimages of finite sets.
-/

public section

assert_not_exists MonoidWithZero MulAction IsOrderedMonoid

variable {ι κ β : Type*}

open Fin Function

namespace Finset

variable [CommMonoid β]

@[to_additive]
/--
lemma `prod_preimage'` / 引理 `prod_preimage'`

English:
lemma prod_preimage'
  given: (f : ι -> κ) [DecidablePred (· in Set.range f)] (s : Finset κ) (hf) (g : κ -> β)
  proof: by
  classical
  calc
    ∏ x in preimage s f hf, g (f x) = ∏ x in image f (preimage s f hf), g x :=
Eq.symm prod_image by simpa [mem_preimage, Set.InjOn] using hf
    _ = ∏ x in s with x in Set.range f, g x := by rw [image_preimage]

@[to_additive]

中文:
引理 prod_preimage'
  条件: (f : ι -> κ) [DecidablePred (· in Set.range f)] (s : Finset κ) (hf) (g : κ -> β)
  证明: by
  classical
  calc
    ∏ x in preimage s f hf, g (f x) = ∏ x in image f (preimage s f hf), g x :=
Eq.symm prod_image by simpa [mem_preimage, Set.InjOn] using hf
    _ = ∏ x in s with x in Set.range f, g x := by rw [image_preimage]

@[to_additive]

Depends on / 依赖: Eq.symm, Set.InjOn, Set.range, classical, image_preimage, mem_preimage, preimage, prod_image
-/
lemma prod_preimage' (f : ι -> κ) [DecidablePred (· in Set.range f)] (s : Finset κ) (hf) (g : κ -> β) :
    ∏ x in s.preimage f hf, g (f x) = ∏ x in s with x in Set.range f, g x := by
  classical
  calc
    ∏ x in preimage s f hf, g (f x) = ∏ x in image f (preimage s f hf), g x :=
Eq.symm prod_image by simpa [mem_preimage, Set.InjOn] using hf
    _ = ∏ x in s with x in Set.range f, g x := by rw [image_preimage]

@[to_additive]
/--
lemma `prod_preimage` / 引理 `prod_preimage`

English:
lemma prod_preimage
  statement: (f : ι -> κ) (s : Finset κ) (hf) (g : κ -> β)
  proof: by
  classical rw [prod_preimage', prod_filter_of_ne]; exact fun x hx => Not.imp_symm (hg x hx)

@[to_additive]

中文:
引理 prod_preimage
  结论: (f : ι -> κ) (s : Finset κ) (hf) (g : κ -> β)
  证明: by
  classical rw [prod_preimage', prod_filter_of_ne]; exact fun x hx => Not.imp_symm (hg x hx)

@[to_additive]

Depends on / 依赖: Not.imp_symm, classical, imp_symm, prod_filter_of_ne, prod_preimage
-/
lemma prod_preimage (f : ι -> κ) (s : Finset κ) (hf) (g : κ -> β)
    (hg : forall x in s, x ∉ Set.range f -> g x = 1) :
    ∏ x in s.preimage f hf, g (f x) = ∏ x in s, g x := by
  classical rw [prod_preimage', prod_filter_of_ne]; exact fun x hx => Not.imp_symm (hg x hx)

@[to_additive]
/--
lemma `prod_preimage_of_bij` / 引理 `prod_preimage_of_bij`

English:
lemma prod_preimage_of_bij
  given: (f : ι -> κ) (s : Finset κ) (hf : Set.BijOn f (f ⁻¹' ↑s) ↑s) (g : κ -> β)
  proof: prod_preimage _ _ hf.injOn g fun _ hs h_f => (h_f <| hf.subset_range hs).elim

中文:
引理 prod_preimage_of_bij
  条件: (f : ι -> κ) (s : Finset κ) (hf : Set.BijOn f (f ⁻¹' ↑s) ↑s) (g : κ -> β)
  证明: prod_preimage _ _ hf.injOn g fun _ hs h_f => (h_f <| hf.subset_range hs).elim

Depends on / 依赖: hf.injOn, hf.subset_range, prod_preimage, subset_range
-/
lemma prod_preimage_of_bij (f : ι -> κ) (s : Finset κ) (hf : Set.BijOn f (f ⁻¹' ↑s) ↑s) (g : κ -> β) :
    ∏ x in s.preimage f hf.injOn, g (f x) = ∏ x in s, g x :=
  prod_preimage _ _ hf.injOn g fun _ hs h_f => (h_f <| hf.subset_range hs).elim

end Finset
