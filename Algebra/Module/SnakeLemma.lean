...
/-
**SnakeLemma.** 是 Mathlib 中的一个引理，位于命名空间 ``。
...
/-
**SnakeLemma.eq_of_eq** 是 Mathlib 中的一个引理，位于命名空间 ``。
形式化陈述：SnakeLemma.eq_of_eq (x : K₃) (y₁) (hy₁ : f₂ y₁ = ι₃ x) (z₁) (hz₁ : g₁ z₁ =
 i₂ y₁) (y₂) (hy₂ : f₂ y₂ = ι₃ x) (z₂) (hz₂ : g₁ z₂ = i₂ y₂) : π₁ z₁ = π₁ z₂
参数：x : K₃；y₁；hy₁ : f₂ y₁ = ι₃ x；z₁；hz₁ : g₁ z₁ = i₂ y₁；y₂；hy₂ : f₂ y₂ = ι₃ x；z₂；
hz₂ : g₁ z₂ = i₂ y₂。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
-/
/-
**SnakeLemma.** 是 Mathlib 中的一个定义，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
```
such that `f₂` is surjective with a (set-theoretic) section `σ`, `g₁` is injecti
ve with a
(set-theoretic) retraction `ρ`,
then the map `π₁ ∘ ρ ∘ i₂ ∘ σ ∘ ι₃` is a linear map from `K₃` to `C₁`.
...
-/
/-
**SnakeLemma.exact_** 是 Mathlib 中的一个引理，位于命名空间 ``。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
such that `f₂` is surjective with a (set-theoretic) section `σ`, `g₁` is injecti
ve with a
(set-theoretic) retraction `ρ`, and `ι₃` is injective, then `K₂ -F→ K₃ -δ→ C₁` i
s exact.
-/
...
such that `f₂` is surjective with a (set-theoretic) section `σ`, `g₁` is injecti
ve with a
(set-theoretic) retraction `ρ`, and `π₁` is surjective, then `K₃ -δ→ C₁ -G→ C₂` 
is exact.
-/
