...
/-
**MeasureTheory.SignedMeasure.withDensity** 是 Mathlib 中的一个定理，位于命名空间 `MeasureTheo
ry.SignedMeasure`。
...
/-
**MeasureTheory.SignedMeasure.absolutelyContinuous_iff_withDensity** 是 Mathlib 中
的一个定理，位于命名空间 `MeasureTheory.SignedMeasure`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
