...
/--
lemma `has_decomp_quotients` / 引理 `has_decomp_quotients`

English:
lemma has_decomp_quotients
...
Depends on / 依赖: Concrete, ContinuousSMul, ContinuousSMul.mk, DiscreteTopology, TopologicalSpace, has_decomp_connected_components, r.hom, u.hom
-/
lemma has_decomp_quotients (X : Action FintypeCat G)
    [TopologicalSpace X.V] [DiscreteTopology X.V] [ContinuousSMul G X.V] :
    exists (ι : Type) (_ : Finite ι) (f : ι -> OpenSubgroup (G)),
...
(ConcreteCategory.mono_iff_injective_of_preservesPullback r).mp mono_comp _ _
    ...
    rw [← this]; rw [continuous_induced_rng]; rw [← heq]
    exact Continuous.comp continuous_smul (by fun_prop)
  have (i : ι) : exists (U : OpenSubgroup (G)), (Nonempty ((f i) ≅ G ⧸ₐ U.toSubgroup)) := by
...
/--
Definition of `fiberIsoQuotientStabilizer` / `fiberIsoQuotientStabilizer` 的定义

English:
definition fiberIsoQuotientStabilizer
  signature: (X : C) [IsConnected X] (x : F.obj X)
  body: haveI : IsConnected ((functorToAction F).obj X) := PreservesIsConnected.preserves
...
Depends on / 依赖: Fintype, FintypeCat, FintypeCat.isoQuotientStabilizerOfIsConnected, IsConnected, MulAction, MulAction.stabilizer, PreservesIsConnected, PreservesIsConnected.preserves, fintypeQuotientStabilizer, functorToAction, isoQuotientStabilizerOfIsConnected, preserves, stabilizer
-/
...
/--
Definition of `quotientToEndObjectHom` / `quotientToEndObjectHom` 的定义

English:
definition quotientToEndObjectHom
  signature: :
  body: let ff : (functorToAction F).FullyFaithful := FullyFaithful.ofFullyFaithful (functorToAction F)
...
中文:
定义 quotientToEndObjectHom
  签名: :
  定义体: let ff : (functorToAction F).FullyFaithful := FullyFaithful.ofFullyFaithful (functorToAction F)
  let e : End A ≃* End (Aut F ⧸ₐ U.toSubgroup) := (ff.mulEquivEnd A).trans (Iso.conj u)
  e.symm.toMonoidHom.comp (quotientToEndHom V.toSubgroup U.toSubgroup)
-/
...
/--
lemma `functorToAction_map_quotientToEndObjectHom` / 引理 `functorToAction_map_quotientToEndObjectHom`

English:
lemma functorToAction_map_quotientToEndObjectHom
  proof: by
...
中文:
引理 functorToAction_map_quotientToEndObjectHom
  证明: by
  simp [← cancel_epi u.inv, ← cancel_mono u.hom, ← Iso.conj_apply, quotientToEndObjectHom]
...
@[simps!]
/--
Definition of `quotientDiag` / `quotientDiag` 的定义

English:
definition quotientDiag
  signature: : SingleObj (V.toSubgroup ⧸ Subgroup.subgroupOf U V) ⥤ C
  body: SingleObj.functor (quotientToEndObjectHom V h u)
...
@[simps]
/--
Definition of `coconeQuotientDiag` / `coconeQuotientDiag` 的定义

English:
definition coconeQuotientDiag
  signature: :
  body: Aut F ⧸ₐ V.toSubgroup
ι := SingleObj.natTrans (u.hom ≫ quotientToQuotientOfLE V.toSubgroup U.toSubgroup hUinV) by
    ...
    rw [← cancel_epi (u.inv)]; rw [Iso.inv_hom_id_assoc]
    

中文:
定义 coconeQuotientDiag
  签名: :
  定义体: Aut F ⧸ₐ V.toSubgroup
ι := SingleObj.natTrans (u.hom ≫ quotientToQuotientOfLE V.toSubgroup U.toSubgroup hUinV) by
    intro (m : V ⧸ Subgroup.subgroupOf U V)
    simp only [const_obj_obj, Functor.comp_map, const_obj_map, Category.comp_id]
    rw [← cancel_epi (u.inv)]; rw [Iso.inv_hom_id_assoc]
...
@[simps]
/--
Definition of `coconeQuotientDiagDesc` / `coconeQuotientDiagDesc` 的定义

English:
definition coconeQuotientDiagDesc
  body: FintypeCat.homMk
    (Quotient.lift (fun σ => (u.inv ≫ s.ι.app (SingleObj.star _)).hom ⟦σ⟧) <| fun σ τ hst => by
      ...
      let m : End (SingleObj.star (V.toSubgroup ⧸ Subgroup.subgroupOf U V)) :=
        ⟦⟨σ⁻¹ * τ, (QuotientGroup.leftRel_apply).mp h

中文:
定义 coconeQuotientDiagDesc
  定义体: FintypeCat.homMk
    (Quotient.lift (fun σ => (u.inv ≫ s.ι.app (SingleObj.star _)).hom ⟦σ⟧) <| fun σ τ hst => by
      let J' := quotientDiag V h u ⋙ functorToAction F
      let m : End (SingleObj.star (V.toSubgroup ⧸ Subgroup.subgroupOf U V)) :=
        ⟦⟨σ⁻¹ * τ, (QuotientGroup.leftRel_apply).mp h
...
private def coconeQuotientDiagDesc
  ...
  comm g := by
    ...
    rw [← this]; rw [u.inv.comm g]
    rfl

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
Definition of `coconeQuotientDiagIsColimit` / `coconeQuotientDiagIsColimit` 的定义

English:
definition coconeQuotientDiagIsColimit
  signature: :
  body: coconeQuotientDiagDesc h u hUinV
  fac s j := by
...
中文:
定义 coconeQuotientDiagIsColimit
  签名: :
  定义体: coconeQuotientDiagDesc h u hUinV
  fac s j := by
    apply (cancel_epi u.inv).mp
    apply Action.hom_ext
    ext (x : Aut F ⧸ U.toSubgroup)
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `exists_lift_of_quotient_openSubgroup` / 引理 `exists_lift_of_quotient_openSubgroup`

English:
lemma exists_lift_of_quotient_openSubgroup
  given: (V : OpenSubgroup (Aut F))
  proof: by
...
Depends on / 依赖: F.obj, IsConnected, MulAction, Nonempty, OpenSubgroup, V.isOpen, X.property, X.val, exists_hom_from_galois_of_fiber_nonempty, exists_set_ker_evaluation_subset_of_isOpen, isOpen, nonempty_fiber_of_isConnected, nonempty_fiber_pi_of_nonempty_of_finite, one_mem, property
-/
lemma exists_lift_of_quotient_openSubgroup (V : OpenSubgroup (Aut F)) :
    exists (X : C), Nonempty ((functorToAction F).obj X ≅ Aut F ⧸ₐ V.toSubgroup) := by
  obtain ⟨I, hf, hc, hi⟩ := exists_set_ker_evaluation_subset_of_isOpen F (one_mem V) V.isOpen'
  have (X : I) : IsConnected X.val := hc X X.property
  have (X : I) : Nonempty (F.obj X.val) := nonempty_fiber_of_isConnected F X
  ...
  have h1 (σ : Aut F) (σinU : σ in U) : σ.hom.app A = 𝟙 (F.obj A) := by
  ...
  have h2 (σ : Aut F) (σinU : σ in U) : forall X : I, σ.hom.app X = 𝟙 (F.obj X) := by
    intro ⟨X, hX⟩
    ext (x : F.obj X)
...
/--
theorem `exists_lift_of_continuous` / 定理 `exists_lift_of_continuous`

English:
theorem exists_lift_of_continuous
  statement: (X : Action FintypeCat (Aut F))
  proof: by
...
Depends on / 依赖: PreservesCoproduct, PreservesCoproduct.iso, Sigma.mapIso, exists_lift_of_quotient_openSubgroup, functorToAction, has_decomp_quotients, mapIso
-/
theorem exists_lift_of_continuous (X : Action FintypeCat (Aut F))
    ...
    exists A, Nonempty ((functorToAction F).obj A ≅ X) := by
  obtain ⟨ι, hfin, f, ⟨u⟩⟩ := has_decomp_quotients X
  choose g gu using (fun i => exists_lift_of_quotient_openSubgroup (f i))
  exact ⟨∐ g, ⟨PreservesCoproduct.iso (functorToAction F) g ≪≫
    Sigma.mapIso (fun i => (gu i).some) ≪≫ u⟩⟩
