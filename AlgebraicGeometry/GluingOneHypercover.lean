...
definition oneHypercover
  signature: : Scheme.zariskiTopology.OneHypercover D.glued where
  body: D.J
  X := D.U
  f := D.ι
  I₁ _ _ := PUnit
  ...
  mem₀ := by
    refine zariskiTopology.superset_covering ?_ D.openCover.mem_grothendie
...
variable {F : Sheaf Scheme.zariskiTopology (Type v)}
  (s : forall (j : D.J), F.obj.obj (op (D.U j)))
  (h : forall (i j : D.J), F.obj.map (D.f i j).op (s i) =
    F.obj.map ((D.f j i).op ≫ (D.t i j).op) (s j))

/--
Definition of `sheafValGluedMk` / `sheafValGluedMk` 的定义

English:
definition sheafValGluedMk
  signature: : F.obj.obj (op D.glued)
  body: Multifork.IsLimit.sectionsEquiv (D.oneHypercover.isLimitMultifork F)
    { val := s
...
中文:
定义 sheafValGluedMk
  签名: : F.obj.obj (op D.glued)
  定义体: Multifork.IsLimit.sectionsEquiv (D.oneHypercover.isLimitMultifork F)
    { val := s
      property := fun _ => h _ _ }

@[simp]

Depends on / 依赖: D.oneHypercover.isLimitMultifork, IsLimit, Multifork, Multifork.IsLimit.sectionsEquiv, isLimitMultifork, oneHypercover, property, sectionsEquiv
-/
...
@[simp]
/--
lemma `sheafValGluedMk_val` / 引理 `sheafValGluedMk_val`

English:
lemma sheafValGluedMk_val
  given: (j : D.J)
  statement: F.obj.map (D.ι j).op (D.sheafValGluedMk s h) = s j
  proof: Multifork.IsLimit.sectionsEquiv_apply_val (D.oneHypercover.isLimitMultifork F) _ _

中文:
引理 sheafValGluedMk_val
  条件: (j : D.J)
  结论: F.obj.map (D.ι j).op (D.sheafValGluedMk s h) = s j
  证明: Multifork.IsLimit.sectionsEquiv_apply_val (D.oneHypercover.isLimitMultifork F) _ _

Depends on / 依赖: D.oneHypercover.isLimitMultifork, IsLimit, Multifork, Multifork.IsLimit.sectionsEquiv_apply_val, isLimitMultifork, oneHypercover, sectionsEquiv_apply_val
-/
