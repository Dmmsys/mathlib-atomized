...
English:
definition bicone
  signature: [HasFiniteBiproducts C] {J : Type} [Finite J] (F : J -> Karoubi C)
  body: { X := biproduct fun j => (F j).X
...
/--
theorem `karoubi_hasFiniteBiproducts` / 定理 `karoubi_hasFiniteBiproducts`

English:
theorem karoubi_hasFiniteBiproducts
  given: [HasFiniteBiproducts C]
  statement: HasFiniteBiproducts (Karoubi C)
  proof: { out := fun n =>
...
Depends on / 依赖: Biproducts, Biproducts.bicone, F.isColimitOfIsWellOrderContinuous, bicone, biproduct, biproduct.map_eq.symm, hasBiproduct_of_total, has_biproduct, isColimitOfIsWellOrderContinuous, map_eq
-/
...
Depends on / 依赖: IsWellOrderContinuous, e.toInitialSeg.monotone.functor, functor, monotone, toInitialSeg
-/
...
definition decomposition
  signature: (P : Karoubi C)
  body: biprod.desc P.decompId_i P.complement.decompId_i
...
定义 decomposition
  签名: (P : Karoubi C)
  定义体: biprod.desc P.decompId_i P.complement.decompId_i
  inv := biprod.lift P.decompId_p P.complement.decompId_p
  hom_inv_id := by
    apply biprod.hom_ext'
    · rw [biprod.inl_desc_assoc, comp_id, biprod.lift_eq, comp_add, ← decompId_assoc,
