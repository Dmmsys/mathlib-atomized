...
/--
theorem `IsStandardSmooth.of_basis_kaehlerDifferential` / 定理 `IsStandardSmooth.of_basis_kaehlerDifferential`

English:
theorem IsStandardSmooth.of_basis_kaehlerDifferential
  statement: [FinitePresentation R S]
  proof: by
  nontriviality S
...
theorem IsStandardSmooth.of_basis_kaehlerDifferential [FinitePresentation R S]
    ...
    {I : Type*} (b : Module.Basis I S (Ω[S⁄R])) (hb : Set.range b subseteq Set.range (D R S)) :
...
/--
theorem `IsStandardSmooth.iff_exists_basis_kaehlerDifferential` / 定理 `IsStandardSmooth.iff_exists_basis_kaehlerDifferential`

English:
theorem IsStandardSmooth.iff_exists_basis_kaehlerDifferential
...
Depends on / 依赖: Algebra, Algebra.IsStandardSmooth.out, IsStandardSmooth, P.basisKaehler, Set.range_subset_iff, basisKaehler, of_basis_kaehlerDifferential, range_subset_iff
-/
theorem IsStandardSmooth.iff_exists_basis_kaehlerDifferential [FinitePresentation R S] :
    IsStandardSmooth R S ↔ Subsingleton (H1Cotangent R S) ∧
      exists (I : Type) (b : Module.Basis I S (Ω[S⁄R])), Set.range b subseteq Set.range (D R S) := by
  refine ⟨fun h => ⟨inferInstance, ?_⟩, fun ⟨h, ⟨_, b, hb⟩⟩ => .of_basis_kaehlerDifferential b hb⟩
  obtain ⟨ι, σ, _, _, ⟨P⟩⟩ := Algebra.IsStandardSmooth.out (R := R) (S := S)
  exact ⟨_, P.basisKaehler, by simp [Set.range_subset_iff]⟩

/--
theorem `Etale.iff_isStandardSmoothOfRelativeDimension_zero` / 定理 `Etale.iff_isStandardSmoothOfRelativeDimension_zero`
...
variable (R) in
/--
theorem `IsSmoothAt.exists_notMem_isStandardSmooth` / 定理 `IsSmoothAt.exists_notMem_isStandardSmooth`

English:
theorem IsSmoothAt.exists_notMem_isStandardSmooth
  statement: [FinitePresentation R S] (p : Ideal S) [p.IsPrime]
  proof: by
...
theorem IsSmoothAt.exists_notMem_isStandardSmooth [FinitePresentation R S] (p : Ideal S) [p.IsPrime]
    [IsSmoothAt R p] :
    exists (f : S), f ∉ p ∧ IsStandardSmooth R (Localization.Away f) := by
  -- By replacing `S` by some `S[1/g]` we may assume `S` is globally smooth.
  wlog h : Smooth R S
  · obtain ⟨g, hg, hsm⟩ := IsSmoothAt.exists_notMem_smooth R p
    have _ : (Ideal.map (algebraMap S (Localization.Away g)) p).IsPrime := by
...
variable (R S) in
/--
theorem `Smooth.exists_span_eq_top_isStandardSmooth` / 定理 `Smooth.exists_span_eq_top_isStandardSmooth`

English:
theorem Smooth.exists_span_eq_top_isStandardSmooth
  given: [Smooth R S]
  proof: by
...
theorem Smooth.exists_span_eq_top_isStandardSmooth [Smooth R S] :
    exists (s : Set S), Ideal.span s = ⊤ ∧ forall x in s, IsStandardSmooth R (Localization.Away x) := by
  choose f hf₁ hf₂ using IsSmoothAt.exists_notMem_isStandardSmooth R (S := S)
  /- #adaptation_note Before https://github.com/leanprover/lean4/pull/13166
  (replacing grind's canonicalizer with a type-directed normalizer), the original proof was:
