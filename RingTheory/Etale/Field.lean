...
/--
theorem `of_isSeparable_aux` / 定理 `of_isSeparable_aux`

English:
theorem of_isSeparable_aux
  given: [Algebra.IsSeparable K L] [EssFiniteType K L]
  proof: by
...
/--
lemma `of_isSeparable` / 引理 `of_isSeparable`

English:
lemma of_isSeparable
  given: [Algebra.IsSeparable K L]
  statement: FormallyEtale K L
  proof: by
  -- We shall show that any `f : L → B/I` can be lifted to `L → B` if `I^2 = ⊥`.
  refine FormallyEtale.iff_comp_bijective.mpr fun B _ _ I h => ?_
...
/--
theorem `iff_isSeparable` / 定理 `iff_isSeparable`

English:
theorem iff_isSeparable
  given: [EssFiniteType K L]
  proof: ⟨fun _ => FormallyUnramified.isSeparable K L, fun _ => of_isSeparable K L⟩

中文:
定理 iff_isSeparable
  条件: [EssFiniteType K L]
  证明: ⟨fun _ => FormallyUnramified.isSeparable K L, fun _ => of_isSeparable K L⟩

Depends on / 依赖: FormallyUnramified, FormallyUnramified.isSeparable, isSeparable, of_isSeparable
-/
...
instance [EssFiniteType
  signature: K A] [FormallyEtale K A] (p
  body: by
...
/--
lemma `of_formallyUnramified_of_field` / 引理 `of_formallyUnramified_of_field`

English:
lemma of_formallyUnramified_of_field
  given: [EssFiniteType K A] [FormallyUnramified K A]
  proof: by
...
variable {K A} in
/--
lemma `iff_formallyUnramified_of_field` / 引理 `iff_formallyUnramified_of_field`

English:
lemma iff_formallyUnramified_of_field
  given: [EssFiniteType K A]
  proof: ⟨fun _ => inferInstance, fun _ => .of_formallyUnramified_of_field K A⟩

中文:
引理 iff_formallyUnramified_of_field
  条件: [EssFiniteType K A]
  证明: ⟨fun _ => inferInstance, fun _ => .of_formallyUnramified_of_field K A⟩

Depends on / 依赖: of_formallyUnramified_of_field
-/
...
/--
theorem `iff_exists_algEquiv_prod` / 定理 `iff_exists_algEquiv_prod`
...
theorem iff_exists_algEquiv_prod [EssFiniteType K A] :
    FormallyEtale K A ↔
      exists (I : Type u) (_ : Finite I) (Ai : I -> Type u) (_ : forall i, Field (Ai i))
        (_ : forall i, Algebra K (Ai i)) (_ : A ≃ₐ[K] Π i, Ai i),
        forall i, Algebra.IsSeparable K (Ai i) := by
  classical
  constructor
  · intro H
    have := FormallyUnramified.finite_of_free K A
...
noncomputable
/--
Definition of `equivPiOfIsSepClosed` / `equivPiOfIsSepClosed` 的定义

English:
definition equivPiOfIsSepClosed
  signature: [EssFiniteType K A] [FormallyEtale K A] [IsSepClosed K]
  body: haveI := Algebra.FormallyUnramified.finite_of_free K A
...
/--
lemma `equivPiOfIsSepClosed_self_apply` / 引理 `equivPiOfIsSepClosed_self_apply`

English:
lemma equivPiOfIsSepClosed_self_apply
  given: [IsSepClosed K] (x : K) (p : PrimeSpectrum K)
  proof: by
...
@[simp]
/--
lemma `equivPiOfIsSepClosed_comap` / 引理 `equivPiOfIsSepClosed_comap`

English:
lemma equivPiOfIsSepClosed_comap
  statement: {B : Type*} [CommRing B] [EssFiniteType K A] [FormallyEtale K A]
  proof: by
  dsimp [equivPiOfIsSepClosed]
  simp only [Equiv.piCongrLeft_symm_apply, AlgEquiv.piCongrRight_apply,
...
/--
theorem `Algebra.Etale.iff_exists_algEquiv_prod` / 定理 `Algebra.Etale.iff_exists_algEquiv_prod`
...
theorem Algebra.Etale.iff_exists_algEquiv_prod :
    Etale K A ↔
      exists (I : Type u) (_ : Finite I) (Ai : I -> Type u) (_ : forall i, Field (Ai i))
        (_ : forall i, Algebra K (Ai i)) (_ : A ≃ₐ[K] Π i, Ai i),
        forall i, Module.Finite K (Ai i) ∧ Algebra.IsSeparable K (Ai i) := by
  constructor
  · intro H
    obtain ⟨I, _, Ai, _, _, e, _⟩ := (FormallyEtale.iff_exists_algEquiv_prod K A).mp inferInstance
    have := FormallyUnramified.finite_of_free K A
