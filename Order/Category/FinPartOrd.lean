...
中文:
缩写 of
  签名: (α : 类型) [PartialOrder α] [Fintype α]
  定义体: α
-/
...
中文:
实例 concreteCategory
  签名: : ConcreteCategory FinPartOrd (· ->o ·)
...
实例 hasForgetToPartOrd
  签名: : HasForget₂ FinPartOrd PartOrd
...
实例 hasForgetToFintype
  签名: : HasForget₂ FinPartOrd FintypeCat where
  定义体: .of X
  forget₂.map f := FintypeCat.homMk f.hom
-/
...
中文:
缩写 ofHom
  签名: {X Y : 类型u} [PartialOrder X] [Fintype X] [PartialOrder Y] [Fintype Y] (f : X ->o Y)
...
中文:
引理 hom_hom_comp
  条件: {X Y Z : FinPartOrd} (f : X ⟶ Y) (g : Y ⟶ Z)
  证明: rfl
-/
...
中文:
引理 comp_apply
  条件: {X Y Z : FinPartOrd} (f : X ⟶ Y) (g : Y ⟶ Z) (x : X)
  证明: by simp
...
中文:
引理 hom_ext
  条件: {X Y : FinPartOrd} {f g : X ⟶ Y} (hf : f.hom.hom = g.hom.hom)
  结论: f = g
...
中文:
引理 hom_hom_ofHom
  结论: {X Y : 类型u} [PartialOrder X] [Fintype X] [PartialOrder Y] [Fintype Y]
  证明: rfl
...
中文:
定义 dual
  签名: : FinPartOrd ⥤ FinPartOrd where
  定义体: of Xᵒᵈ
  map f := ofHom f.hom.hom.dual
-/
