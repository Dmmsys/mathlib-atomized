...
/--
Definition of `IsKrasner` / `IsKrasner` 的定义

English:
class IsKrasner
  parameters: [Field K] [Algebra K L]
...
class IsKrasner [Field K] [Algebra K L] : Prop where
  krasner' {x y : L} : IsSeparable K x -> ((minpoly K x).map (algebraMap K L)).Splits ->
    IsIntegral K y -> (forall x' : L, IsConjRoot K x x' -> x != x' -> ‖x - y‖ < ‖x - x'‖) -> x in K⟮y⟯

namespace IsKrasner

variable {K L} in
/--
theorem `krasner` / 定理 `krasner`

English:
theorem krasner
  statement: [Field K] [Algebra K L]
  proof: IsKrasner.krasner' hx sp hy h
...
theorem krasner [Field K] [Algebra K L]
    ...
    (h : (forall x' : L, IsConjRoot K x x' -> x != x' -> ‖x - y‖ < ‖x - x'‖)) : x in K⟮y⟯ :=
...
/--
theorem `of_completeSpace_of_normal` / 定理 `of_completeSpace_of_normal`

English:
theorem of_completeSpace_of_normal
  given: [Normal K L]
  statement: IsKrasner K L where
  proof: by
    let z := x - y
...
/--
Instance `of_completeSpace` / 实例 `of_completeSpace`

English:
instance of_completeSpace
