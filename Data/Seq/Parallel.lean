...
/--
Definition of `parallel.aux2` / `parallel.aux2` 的定义

English:
definition parallel.aux2
  signature: : List (Computation α) -> α oplus (List (Computation α))
  body: List.foldr
    (fun c o =>
      match o with
      | Sum.inl a => Sum.inl a
...
定义 parallel.aux2
  ...
  定义体: List.foldr
    ...
    (Sum.inr [])
-/
private def parallel.aux2 : List (Computation α) -> α oplus (List (Computation α)) :=
  List.foldr
    (fun c o =>
      match o with
      | Sum.inl a => Sum.inl a
...
/--
Definition of `parallel.aux1` / `parallel.aux1` 的定义

English:
definition parallel.aux1
  signature: :
...
private def parallel.aux1 :
    List (Computation α) × WSeq (Computation α) ->
      α oplus (List (Computation α) × WSeq (Computation α))
  | (l, S) =>
    rmap
      (fun l' =>
...
English:
definition parallel
  signature: (S : WSeq (Computation α))
  body: corec parallel.aux1 ([], S)

中文:
定义 parallel
  签名: (S : WSeq (Computation α))
  定义体: corec parallel.aux1 ([], S)

Depends on / 依赖: parallel, parallel.aux1
-/
...
set_option backward.privateInPublic.warn false in
/--
theorem `terminates_parallel.aux` / 定理 `terminates_parallel.aux`

English:
theorem terminates_parallel.aux
  proof: by
  have lem1 :
    forall l S, (exists a : α, parallel.aux2 l = Sum.inl a) -> Terminates (corec parallel.aux1 (l, S)) := by
    intro l S e
    obtain ⟨a, e⟩ := e
  ...
  · intro a l S m
    ...
    · obtain ⟨a', e⟩ := IH m
      ...
      exact ⟨a', rfl⟩
  · intro s IH l S m
    have H1 (l') (e' : parallel.aux2 l = Sum.inr l') : s in l' := by
...
Depends on / 依赖: List.mem_cons, Sum.inl, Terminates, corec_eq, destruct_eq_pure, mem_cons, parallel, parallel.aux1, parallel.aux2, ret_terminates, return, revert, terminatesRecOn
-/
theorem terminates_parallel.aux :
    forall {l : List (Computation α)} {S c},
      c in l -> Terminates c -> Terminates (corec parallel.aux1 (l, S)) := by
  have lem1 :
    forall l S, (exists a : α, parallel.aux2 l = Sum.inl a) -> Terminates (corec parallel.aux1 (l, S)) := by
    intro l S e
    obtain ⟨a, e⟩ := e
...
/--
theorem `terminates_parallel` / 定理 `terminates_parallel`

English:
theorem terminates_parallel
  given: {S : WSeq (Computation α)} {c} (h : c in S) [T : Terminates c]
  proof: by
  suffices
    forall (n) (l : List (Computation α)) (S c),
      c in l ∨ some (some c) = Seq.get? S n -> Terminates c -> Terminates (corec parallel.aux1 (l, S))
    from
    let ⟨n, h⟩ := h
  ...
  case succ n IH =>
    ...
    · have C : corec parallel.aux1 (l, S) = _ := destruct_eq_think (by
      ...
      rw [C]
      refine @Computation.think_terminates _ _ ?_
      have TT : forall l', Terminates (corec parallel.aux1 (l', S.tail)) := by
        intro
        apply IH _ _ _ (Or.inr _) T
        rw [a]; rw [Seq.get?_tail]
...
中文:
定理 terminates_parallel
  条件: {S : WSeq (Computation α)} {c} (h : c in S) [T : Terminates c]
  证明: by
  suffices
    forall (n) (l : List (Computation α)) (S c),
      c in l ∨ some (some c) = Seq.get? S n -> Terminates c -> Terminates (corec parallel.aux1 (l, S))
...
Depends on / 依赖: Computation, Or.inr, Seq.destruct, Seq.get, Seq.tail, Terminates, destruct, parallel, parallel.aux1, parallel.aux2, terminates_parallel, terminates_parallel.aux
-/
theorem terminates_parallel {S : WSeq (Computation α)} {c} (h : c in S) [T : Terminates c] :
...
/--
theorem `exists_of_mem_parallel` / 定理 `exists_of_mem_parallel`

English:
theorem exists_of_mem_parallel
  given: {S : WSeq (Computation α)} {a} (h : a in parallel S)
  proof: by
  suffices
    forall C, a in C -> forall (l : List (Computation α)) (S),
      corec parallel.aux1 (l, S) = C -> exists c, (c in l ∨ c in S) ∧ a in c from
    let ⟨c, h1, h2⟩ := this _ h [] S rfl
⟨c, h1.resolve_left List.not_mem_nil, h2⟩
  let F : List (Computation α) -> α oplus (List (Computation α)) -> Prop := by
    intro l a
    rcases a with a | l'
    · exact exists c in l, a in c
    · exact forall a', (exists c in l', a' in c) -> exists c in l, a' in c
...
Depends on / 依赖: Computation, List.not_mem_nil, h1.resolve_left, not_mem_nil, parallel, parallel.aux1, parallel.aux2, resolve_left
-/
theorem exists_of_mem_parallel {S : WSeq (Computation α)} {a} (h : a in parallel S) :
    exists c in S, a in c := by
  suffices
    forall C, a in C -> forall (l : List (Computation α)) (S),
      corec parallel.aux1 (l, S) = C -> exists c, (c in l ∨ c in S) ∧ a in c from
    let ⟨c, h1, h2⟩ := this _ h [] S rfl
...
/--
theorem `map_parallel` / 定理 `map_parallel`

English:
theorem map_parallel
  given: (f : α -> β) (S)
  statement: map f (parallel S) = parallel (S.map (map f))
  proof: by
  refine
    eq_of_bisim
      (fun c1 c2 =>
...
定理 map_parallel
  条件: (f : α -> β) (S)
  结论: map f (parallel S) = parallel (S.map (map f))
  证明: by
  refine
    eq_of_bisim
      (fun c1 c2 =>
...
Depends on / 依赖: BisimO, List.map, S.map, continuousPregroupoid, corec_, destruct_map, eq_of_bisim, l.map, parallel, parallel.aux1, parallel.aux2
-/
...
/--
theorem `parallel_empty` / 定理 `parallel_empty`

English:
theorem parallel_empty
  given: (S : WSeq (Computation α)) (h : S.head ~> none)
  statement: parallel S = empty _
  proof: eq_empty_of_not_terminates fun ⟨⟨a, m⟩⟩ => by
...
中文:
定理 parallel_empty
  条件: (S : WSeq (Computation α)) (h : S.head ~> none)
  结论: parallel S = empty _
  证明: eq_empty_of_not_terminates fun ⟨⟨a, m⟩⟩ => by
    let ⟨c, cs, _⟩ := exists_of_mem_parallel m
    let ⟨n, nm⟩ := WSeq.exists_get?_of_mem cs
    let ⟨c', h'⟩ := WSeq.head_some_of_get?_some nm
    injection h h'

Depends on / 依赖: WSeq.exists_get, WSeq.head_some_of_get, _of_mem, _some, eq_empty_of_not_terminates, exists_get, exists_of_mem_parallel, head_some_of_get, injection
-/
...
/--
Definition of `parallelRec` / `parallelRec` 的定义

English:
definition parallelRec
  signature: {S : WSeq (Computation α)} (C : α -> Sort v) (H : forall s in S, forall a in s, C a) {a}
  body: by
...
Depends on / 依赖: Computation, Function, Function.comp_def, S.map, T.map, Terminates, WSeq.map, WSeq.map_comp, WSeq.map_id, c.map, comp_def, congr_arg, map_comp, map_id, map_parallel, parallel, symm.trans, terminates_map_iff
-/
def parallelRec {S : WSeq (Computation α)} (C : α -> Sort v) (H : forall s in S, forall a in s, C a) {a}
    (h : a in parallel S) : C a := by
  let T : WSeq (Computation (α × Computation α)) := S.map fun c => c.map fun a => (a, c)
  have : S = T.map (map fun c => c.1) := by
    rw [← WSeq.map_comp]
    refine (WSeq.map_id _).symm.trans (congr_arg (fun f => WSeq.map f S) ?_)
...
/--
theorem `parallel_promises` / 定理 `parallel_promises`

English:
theorem parallel_promises
  given: {S : WSeq (Computation α)} {a} (H : forall s in S, s ~> a)
  statement: parallel S ~> a
  proof: fun _ ma' =>
  let ⟨_, cs, ac⟩ := exists_of_mem_parallel ma'
  H _ cs ac

中文:
定理 parallel_promises
  条件: {S : WSeq (Computation α)} {a} (H : 对任意 s in S, s ~> a)
  结论: parallel S ~> a
  证明: fun _ ma' =>
  let ⟨_, cs, ac⟩ := exists_of_mem_parallel ma'
  H _ cs ac

Depends on / 依赖: exists_of_mem_parallel
-/
theorem parallel_promises {S : WSeq (Computation α)} {a} (H : forall s in S, s ~> a) : parallel S ~> a :=
  fun _ ma' =>
  let ⟨_, cs, ac⟩ := exists_of_mem_parallel ma'
  H _ cs ac

/--
theorem `mem_parallel` / 定理 `mem_parallel`

English:
theorem mem_parallel
  statement: {S : WSeq (Computation α)} {a} (H : forall s in S, s ~> a) {c} (cs : c in S)
  proof: by
...
中文:
定理 mem_parallel
  结论: {S : WSeq (Computation α)} {a} (H : 对任意 s in S, s ~> a) {c} (cs : c in S)
  证明: by
  have := terminates_of_mem ac
  have := terminates_parallel cs
  exact mem_of_promises _ (parallel_promises H)

Depends on / 依赖: mem_of_promises, parallel_promises, terminates_of_mem, terminates_parallel
-/
theorem mem_parallel {S : WSeq (Computation α)} {a} (H : forall s in S, s ~> a) {c} (cs : c in S)
    (ac : a in c) : a in parallel S := by
  have := terminates_of_mem ac
  have := terminates_parallel cs
  exact mem_of_promises _ (parallel_promises H)

/--
theorem `parallel_congr_lem` / 定理 `parallel_congr_lem`

English:
theorem parallel_congr_lem
  given: {S T : WSeq (Computation α)} {a} (H : S.LiftRel Equiv T)
  proof: ⟨fun h1 _ tT =>
...
Depends on / 依赖: WSeq.exists_of_liftRel_left, WSeq.exists_of_liftRel_right, exists_of_liftRel_left, exists_of_liftRel_right, promises_congr
-/
...
/--
theorem `parallel_congr_left` / 定理 `parallel_congr_left`

English:
theorem parallel_congr_left
  statement: {S T : WSeq (Computation α)} {a} (h1 : forall s in S, s ~> a)
  proof: let h2 := (parallel_congr_lem H).1 h1
  fun a' =>
  ⟨fun h => by
...
中文:
定理 parallel_congr_left
  结论: {S T : WSeq (Computation α)} {a} (h1 : 对任意 s in S, s ~> a)
  证明: let h2 := (parallel_congr_lem H).1 h1
  fun a' =>
  ⟨fun h => by
    have aa := parallel_promises h1 h
    rw [← aa]
...
Depends on / 依赖: WSeq.exists_of_liftRel_left, WSeq.exists_of_liftRel_right, exists_of_liftRel_left, exists_of_liftRel_right, exists_of_mem_parallel, mem_parallel, parallel_congr_lem, parallel_promises
-/
theorem parallel_congr_left {S T : WSeq (Computation α)} {a} (h1 : forall s in S, s ~> a)
...
/--
theorem `parallel_congr_right` / 定理 `parallel_congr_right`

English:
theorem parallel_congr_right
  statement: {S T : WSeq (Computation α)} {a} (h2 : forall t in T, t ~> a)
  proof: parallel_congr_left ((parallel_congr_lem H).2 h2) H

中文:
定理 parallel_congr_right
  结论: {S T : WSeq (Computation α)} {a} (h2 : 对任意 t in T, t ~> a)
  证明: parallel_congr_left ((parallel_congr_lem H).2 h2) H

Depends on / 依赖: parallel_congr_left, parallel_congr_lem
-/
theorem parallel_congr_right {S T : WSeq (Computation α)} {a} (h2 : forall t in T, t ~> a)
    (H : S.LiftRel Equiv T) : parallel S ~ parallel T :=
