...
variable {f : E -> F} {x y : E} {t : 𝕜} {n : Nat}

/--
theorem `DifferentiableAt.deriv_comp_add_smul` / 定理 `DifferentiableAt.deriv_comp_add_smul`

English:
theorem DifferentiableAt.deriv_comp_add_smul
  given: (hf : DifferentiableAt 𝕜 f (x + t • y))
  proof: by
...
中文:
定理 DifferentiableAt.deriv_comp_add_smul
  条件: (hf : DifferentiableAt 𝕜 f (x + t • y))
  证明: by
  have hg : Differentiable 𝕜 (fun (s : 𝕜) => (x + s • y)) := by fun_prop
  convert fderiv_comp_deriv t hf hg.differentiableAt
  · simp
...
Depends on / 依赖: Differentiable, convert, deriv_smul_const, differentiableAt, differentiableAt_id, fderiv_comp_deriv, fun_prop, hg.differentiableAt
-/
...
/--
theorem `ContDiffAt.deriv_fderiv_add_smul` / 定理 `ContDiffAt.deriv_fderiv_add_smul`

English:
theorem ContDiffAt.deriv_fderiv_add_smul
  given: (hf : ContDiffAt 𝕜 (n + 1) f (x + t • y))
  proof: by
...
中文:
定理 ContDiffAt.deriv_fderiv_add_smul
  条件: (hf : ContDiffAt 𝕜 (n + 1) f (x + t • y))
  证明: by
  have hf' : DifferentiableAt 𝕜 (iteratedFDeriv 𝕜 n f) (x + t • y) := by
    apply hf.differentiableAt_iteratedFDeriv
    norm_cast
...
Depends on / 依赖: DifferentiableAt, continuousMultilinear_apply_const, convert, deriv_comp_add_smul, differentiableAt_iteratedFDeriv, hf.differentiableAt_iteratedFDeriv, iteratedFDeriv, iteratedFDeriv_succ_apply_left, lt_add_one
-/
...
variable {f : E -> F} {x y : E} {n : Nat}

variable [CompleteSpace F]

/--
theorem `map_add_eq_sum_add_integral_iteratedFDeriv` / 定理 `map_add_eq_sum_add_integral_iteratedFDeriv`

English:
theorem map_add_eq_sum_add_integral_iteratedFDeriv
  statement: (hf : forall (t : Real) (_ht : t in Set.Icc 0 1),
  proof: by
...
-/
theorem map_add_eq_sum_add_integral_iteratedFDeriv (hf : forall (t : Real) (_ht : t in Set.Icc 0 1),
    ContDiffAt Real (n + 1) f (x + t • y)) :
    f (x + y) = ∑ k in Finset.range (n + 1), (k ! : Real)⁻¹ • (iteratedFDeriv Real k f x (fun _ => y)) +
    (n ! : Real)⁻¹ • ∫ t in 0..1, (1 - t)^n • iteratedFDeriv Real (n + 1) f (x + t • y) (fun _ => y) := by
  simp_rw [← Set.uIcc_of_le zero_le_one] at hf
  induction n with
  | zero =>
    -- The base case follows from the fundamental theorem of calculus
