...
/-
**Mathlib.Linter.globalAttributeInLinter.getLinterGlobalAttributeIn** 是 Mathlib 
中的一个定义，位于命名空间 `Mathlib.Linter.globalAttributeInLinter`。
形式化陈述：getLinterGlobalAttributeIn (o : LinterOptions) : Bool
参数：o : LinterOptions。
该定义给出了上述对象。
...
-/
/-
**Mathlib.Linter.globalAttributeInLinter.getGlobalAttributesIn** 是 Mathlib 中的一个定
义，位于命名空间 `Mathlib.Linter.globalAttributeInLinter`。
形式化陈述：getGlobalAttributesIn? : Syntax -> Option (Ident × Array (TSyntax `attr)) 
| `(attribute [$x,*] $id in $_) => let xs
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`getGlobalAttributesIn? cmd` assumes that `cmd` represents a `attribute [...] id
 in ...` command.
If this is the case, then it returns `(id, #[non-local nor scoped attributes])`.
Otherwise, it returns `default`.
-/
...
-/
/-
**Mathlib.Linter.globalAttributeInLinter.globalAttributeIn** 是 Mathlib 中的一个定义，位于
命名空间 `Mathlib.Linter.globalAttributeInLinter`。
形式化陈述：globalAttributeIn : Linter where run
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The `globalAttributeInLinter` linter flags any global attributes generated by an
`attribute [...] in` declaration. (This includes the `instance`, `simp` and `ext
` attributes.)

Despite the `in`, these define *global* instances, which can be rather misleadin
g.
Instead, remove the `in` or mark them with `local`.
-/
