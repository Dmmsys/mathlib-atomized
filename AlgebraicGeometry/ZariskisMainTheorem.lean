...
/--
theorem `exists_etale_isCompl_of_quasiFiniteAt` / 定理 `exists_etale_isCompl_of_quasiFiniteAt`

English:
theorem exists_etale_isCompl_of_quasiFiniteAt
...
theorem exists_etale_isCompl_of_quasiFiniteAt [IsSeparated f]
    ...
    exists (U : Scheme) (g : U ⟶ S), Etale g ∧ s in Set.range g ∧
    exists (V W : (pullback f g).Opens) (v : V), IsCompl V W ∧ IsFinite (V.ι ≫ pullback.snd f g) ∧
...
set_option backward.isDefEq.respectTransparency false in
/--
lemma `Scheme.Hom.exists_mem_and_isIso_morphismRestrict_toNormalization` / 引理 `Scheme.Hom.exists_mem_and_isIso_morphismRestrict_toNormalization`
...
lemma Scheme.Hom.exists_mem_and_isIso_morphismRestrict_toNormalization
    ...
    exists V, f.toNormalization x in V ∧ IsIso (f.toNormalization ∣_ V) := by
  obtain ⟨T, fT, _, ⟨u, hu⟩, V, W, v, hVW, _, hv₂⟩ := exists_etale_isCompl_of_quasiFiniteAt _ rfl hx
  obtain ⟨U, hU, _⟩ : exists U, (pullback.snd f fT).toNormalization v.1 in U ∧
      IsIso ((pullback.snd f fT).toNormalization ∣_ U) := by
    have hVW' : (W : Set ↑(pullback f fT)) = (↑V)ᶜ :=
...
@[stacks 03GW]
/--
lemma `Scheme.Hom.exists_isIso_morphismRestrict_toNormalization` / 引理 `Scheme.Hom.exists_isIso_morphismRestrict_toNormalization`
...
lemma Scheme.Hom.exists_isIso_morphismRestrict_toNormalization
    [LocallyOfFiniteType f] [IsSeparated f] [QuasiCompact f] :
    exists U : f.normalization.Opens, IsIso (f.toNormalization ∣_ U) ∧
...
set_option backward.isDefEq.respectTransparency.types false in
/--
lemma `Scheme.Hom.isOpen_quasiFiniteAt` / 引理 `Scheme.Hom.isOpen_quasiFiniteAt`

English:
lemma Scheme.Hom.isOpen_quasiFiniteAt
  given: [LocallyOfFiniteType f]
  proof: by
...
/--
Definition of `Scheme.Hom.quasiFiniteLocus` / `Scheme.Hom.quasiFiniteLocus` 的定义

English:
definition Scheme.Hom.quasiFiniteLocus
  signature: [LocallyOfFiniteType f]
...
@[simp]
/--
lemma `Scheme.Hom.mem_quasiFiniteLocus` / 引理 `Scheme.Hom.mem_quasiFiniteLocus`

English:
lemma Scheme.Hom.mem_quasiFiniteLocus
  statement: [LocallyOfFiniteType f]
  proof: .rfl
...
lemma Scheme.Hom.mem_quasiFiniteLocus [LocallyOfFiniteType f]
    {x : X} : x in f.quasiFiniteLocus ↔ f.QuasiFiniteAt x := .rfl
...
instance [LocallyOfFiniteType
  signature: f] [IsSeparated f] [QuasiCompact f] :
  body: by
  obtain ⟨U, hU, e⟩ := Scheme.Hom.exists_isIso_morphismRestrict_toNormalization f
  convert!
    (inferInstance :
...
Depends on / 依赖: IsOpenImmersion, Scheme, Scheme.Hom.exists_isIso_morphismRestrict_toNormalization, SetLike, SetLike.coe_injective, X.isoOfEq, coe_injective, convert, e.symm, exists_isIso_morphismRestrict_toNormalization, f.quasiFiniteLocus, f.toNormalization, isoOfEq, quasiFiniteLocus, toNormalization
-/
...
/--
lemma `Scheme.Hom.quasiFiniteLocus_eq_top` / 引理 `Scheme.Hom.quasiFiniteLocus_eq_top`

English:
lemma Scheme.Hom.quasiFiniteLocus_eq_top
  given: [LocallyQuasiFinite f] [LocallyOfFiniteType f]
...
/--
lemma `Scheme.Hom.quasiFiniteLocus_comp` / 引理 `Scheme.Hom.quasiFiniteLocus_comp`

English:
lemma Scheme.Hom.quasiFiniteLocus_comp
  statement: {Z : Scheme} [IsOpenImmersion f]
  proof: by
  ext
...
Depends on / 依赖: quasiFiniteAt_comp_iff_of_isOpenImmersion
-/
...
/--
lemma `Scheme.Hom.quasiFiniteLocus_eq_top_iff` / 引理 `Scheme.Hom.quasiFiniteLocus_eq_top_iff`

English:
lemma Scheme.Hom.quasiFiniteLocus_eq_top_iff
  given: [LocallyOfFiniteType f]
  proof: by
...
Depends on / 依赖: Scheme, Scheme.Hom.quasiFiniteLocus_comp, Scheme.Hom.quasiFiniteLocus_eq_top_iff, Scheme.Opens, quasiFiniteLocus_comp, quasiFiniteLocus_eq_top_iff
-/
...
instance [LocallyQuasiFinite
  signature: f] [LocallyOfFiniteType f] [IsSeparated f] [QuasiCompact f] :
  body: by
  convert!
    (inferInstance :
      IsOpenImmersion
...
Depends on / 依赖: IsOpenImmersion, X.isoOfEq, X.topIso.inv, convert, f.quasiFiniteLocus, f.quasiFiniteLocus_eq_top, f.toNormalization, isoOfEq, quasiFiniteLocus, quasiFiniteLocus_eq_top, toNormalization, topIso
-/
...
English:
instance [QuasiSeparated
  signature: f] [UniversallyClosed f] : UniversallyClosed f.toNormalization
  body: have : UniversallyClosed (f.toNormalization ≫ f.fromNormalization) := by simpa
...
Depends on / 依赖: UniversallyClosed, f.fromNormalization, f.toNormalization, fromNormalization, of_comp_of_isSeparated, toNormalization
-/
...
/--
lemma `IsFinite.of_isProper_of_locallyQuasiFinite` / 引理 `IsFinite.of_isProper_of_locallyQuasiFinite`
...
@[stacks 02LS "(1) <=> (3)"]
/--
lemma `IsFinite.iff_isProper_and_locallyQuasiFinite` / 引理 `IsFinite.iff_isProper_and_locallyQuasiFinite`
...
Depends on / 依赖: of_isProper_of_locallyQuasiFinite
-/
lemma IsFinite.iff_isProper_and_locallyQuasiFinite :
...
/--
lemma `IsFinite.eq_proper_inf_locallyQuasiFinite` / 引理 `IsFinite.eq_proper_inf_locallyQuasiFinite`
...
Depends on / 依赖: IsFinite, IsFinite.iff_isProper_and_locallyQuasiFinite, iff_isProper_and_locallyQuasiFinite
-/
lemma IsFinite.eq_proper_inf_locallyQuasiFinite :
...
@[stacks 04XV "(1) <=> (2)"]
/--
lemma `IsClosedImmersion.iff_isProper_and_mono` / 引理 `IsClosedImmersion.iff_isProper_and_mono`
...
Depends on / 依赖: IsClosedImmersion, IsClosedImmersion.iff_isFinite_and_mono, IsFinite, IsFinite.iff_isProper_and_locallyQuasiFinite, IsProper, LocallyQuasiFinite, iff_isFinite_and_mono, iff_isProper_and_locallyQuasiFinite
-/
lemma IsClosedImmersion.iff_isProper_and_mono :
...
/--
lemma `IsClosedImmersion.eq_proper_inf_monomorphisms` / 引理 `IsClosedImmersion.eq_proper_inf_monomorphisms`
...
Depends on / 依赖: IsClosedImmersion, IsClosedImmersion.iff_isProper_and_mono, iff_isProper_and_mono
-/
lemma IsClosedImmersion.eq_proper_inf_monomorphisms :
...
@[stacks 02UP]
/--
lemma `exists_isFinite_morphismRestrict_of_finite_preimage_singleton` / 引理 `exists_isFinite_morphismRestrict_of_finite_preimage_singleton`
...
lemma exists_isFinite_morphismRestrict_of_finite_preimage_singleton
    ...
    exists V : Y.Opens, y in V ∧ IsFinite (f ∣_ V) := by
  let V : Y.Opens := ⟨_, (f.isClosedMap _ f.quasiFiniteLocus.isOpen.isClosed_compl).isOpen_compl⟩
  refine ⟨V, ?_, ?_⟩
  · suffices forall x, f x = y -> f.QuasiFiniteAt x by simpa [V, not_imp_not]
    rintro x rfl
...
@[stacks 0AH8]
/--
lemma `exists_finite_imageι_comp_morphismRestrict_of_finite_image_preimage` / 引理 `exists_finite_imageι_comp_morphismRestrict_of_finite_image_preimage`
...
lemma exists_finite_imageι_comp_morphismRestrict_of_finite_image_preimage
    ...
    exists U : S.Opens, s in U ∧ IsFinite ((f.imageι ≫ g) ∣_ U) := by
  have : IsProper f := .of_comp f g
  have : IsProper (f.imageι ≫ g) := by
    suffices UniversallyClosed (f.imageι ≫ g) from ⟨⟩
    have : UniversallyClosed (f.toImage ≫ f.imageι ≫ g) := by
