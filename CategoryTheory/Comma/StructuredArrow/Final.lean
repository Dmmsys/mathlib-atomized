...
/--
lemma `final_of_final_costructuredArrowToOver_small` / 引理 `final_of_final_costructuredArrowToOver_small`

English:
lemma final_of_final_costructuredArrowToOver_small
  statement: (L : A ⥤ T) (R : B ⥤ T) [Final R]
  proof: by
  rw [final_iff_isIso_colimit_pre]
  intro G
  have : forall (b : B), Final ((whiskerLeft R (preFunctor L (𝟭 T))).app b).toFunctor := fun b =>
    inferInstanceAs (Final (CostructuredArrow.toOver L (R.obj b)))
  let i : colimit (L ⋙ G) ≅ colimit G :=
calc colimit (L ⋙ G) ≅ colimit grothendieckPro
...
private lemma final_of_final_costructuredArrowToOver_small (L : A ⥤ T) (R : B ⥤ T) [Final R]
    [forall b : B, Final (CostructuredArrow.toOver L (R.obj b))] : Final L := by
  rw [final_iff_isIso_colimit_pre]
  intro G
  have : forall (b : B), Final ((whiskerLeft R (preFunctor L (𝟭 T))).app b).toFunctor := fun b =>
    inferInstanceAs (Final (CostructuredArrow.toOver L (R.obj b)))
...
_ ≅ colimit grothendieckProj (𝟭 T) ⋙ G :=
  ...
  rw [← Iso.inv_comp_eq]; rw [Iso.eq_inv_comp]
...
set_option backward.isDefEq.respectTransparency false in
/--
theorem `final_of_final_costructuredArrowToOver` / 定理 `final_of_final_costructuredArrowToOver`

English:
theorem final_of_final_costructuredArrowToOver
  statement: (L : A ⥤ T) (R : B ⥤ T) [Final R]
  proof: by
...
Depends on / 依赖: AsSmall, AsSmall.equiv, CostructuredArrow, CostructuredArrow.toOver, functor, inverse, sA.inverse, sB.inverse, sT.functor, toOver
-/
theorem final_of_final_costructuredArrowToOver (L : A ⥤ T) (R : B ⥤ T) [Final R]
    [hB : forall b : B, Final (CostructuredArrow.toOver L (R.obj b))] : Final L := by
  let sA : A ≌ AsSmall.{max u₁ u₂ u₃ v₁ v₂ v₃} A := AsSmall.equiv
  let sB : B ≌ AsSmall.{max u₁ u₂ u₃ v₁ v₂ v₃} B := AsSmall.equiv
  let sT : T ≌ AsSmall.{max u₁ u₂ u₃ v₁ v₂ v₃} T := AsSmall.equiv
  let L' := sA.inverse ⋙ L ⋙ sT.functor
