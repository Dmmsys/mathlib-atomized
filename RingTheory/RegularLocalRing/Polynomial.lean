...
/--
lemma `Polynomial.isRegularLocalRing_localization_atPrime_of_comap_eq_maximalIdeal` / 引理 `Polynomial.isRegularLocalRing_localization_atPrime_of_comap_eq_maximalIdeal`
...
Depends on / 依赖: Ideal.span, IsRegularLocalRing, IsRegularLocalRing.of_spanFinrank_maximalIdeal_le, Submodule, Submodule.FG.finite_generators, fg_of_isNoetherianRing, finite_generators, height, height_map_C, isRegularLocalRing_iff, le_of_eq, map_comap_le, maximalIdeal, of_spanFinrank_maximalIdeal_le, p.height, q.height
-/
lemma Polynomial.isRegularLocalRing_localization_atPrime_of_comap_eq_maximalIdeal
  ...
  · have lt : q < p := lt_of_le_of_ne qle (Ne.symm eq)
    ...
    have peq : p = Ideal.span (((algebraMap R R[X]) '' (maximalIdeal R).generators) union {y}) := by
    ...
    rw [← maximalIdeal_height_eq_ringKrullDim]; rw [← IsLocalization.height_under p.primeCompl]; rw [IsLocalization.AtPrime.under_maximalIdeal _ p]
    ...
    rw [WithBot.coe_add]; rw [maximalIdeal_height_eq_ringKrullDim]; rw [WithBot.coe_one]; rw [← reg]; rw [← Nat.cast_one]; rw [← Nat.cast_add]; rw [Nat.cast_le]
    ...
    rw [Set.ncard_singleton]; rw [add_le_add_iff_right]; rw [← Submodule.FG.generators_ncard fg']
    exact Set.ncard_image_le fg

/--
Instance `Polynomial.isRegularRing_of_isRegularRing` / 实例 `Polynomial.isRegularRing_of_isRegularRing`

English:
instance Polynomial.isRegularRing_of_isRegularRing
  signature: [IsRegularRing R]
  body: by
...
Depends on / 依赖: Algebra, AtPrime, Disjoint, IsLocalization, IsPrim, Localization, Localization.AtPrime, Polynomial, Polynomial.C.toMonoidHom, Polynomial.isLocalization, Set.disjoint_compl_left_iff_subset.mpr, Set.disjoint_image_left.mpr, Submonoid, Submonoid.map, algebra, algebraMap, disjoint_compl_left_iff_subset, disjoint_image_left, isLocalization, isRegularRing_iff
-/
instance Polynomial.isRegularRing_of_isRegularRing [IsRegularRing R] : IsRegularRing R[X] := by
  ...
  have eq : comap C pS = maximalIdeal (Localization.AtPrime q) := by
    rw [← IsLocalization.map_under q.primeCompl _ (comap C pS)]; rw [← IsLocalization.map_under q.primeCompl _ (maximalIdeal (Localization.AtPrime q))]
    simp only [comap_comap, S, pS]
    rw [← Polynomial.algebraMap_eq (R := Localization.AtPrime q)]; rw [← IsScalarTower.algebraMap_eq R (Localization.AtPrime q) (Localization.AtPrime q)[X],
...
/--
Instance `MvPolynomial.isRegularRing_of_isRegularRing` / 实例 `MvPolynomial.isRegularRing_of_isRegularRing`

English:
instance MvPolynomial.isRegularRing_of_isRegularRing
  signature: [IsRegularRing R] {ι : Type*} [Finite ι]
  body: by
...
Depends on / 依赖: Finite, Finite.induction_empty_option, IsRegularRing, IsRegularRing.of_ringEquiv, MvPolynomial, MvPolynomial.optionEquivLeft, h_empty, h_option, induction_empty_option, isEmptyRingEquiv, of_equiv, of_ringEquiv, optionEquivLeft, renameEquiv, toRingEquiv, toRingEquiv.symm
-/
