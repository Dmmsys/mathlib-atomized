...
/--
Definition of `sylvester` / `sylvester` 的定义

English:
definition sylvester
  signature: (f g : R[X]) (m n : Nat)
  body: .of fun i j => j.addCases
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + n) then g.coeff (i - j₁) else 0)
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + m) then f.coeff (i - j₁) else 0)

中文:
定义 sylvester
  签名: (f g : R[X]) (m n : 自然数)
  定义体: .of fun i j => j.addCases
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + n) then g.coeff (i - j₁) else 0)
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + m) then f.coeff (i - j₁) else 0)

Depends on / 依赖: Set.Icc, addCases, f.coeff, g.coeff, j.addCases
-/
def sylvester (f g : R[X]) (m n : Nat) : Matrix (Fin (m + n)) (Fin (m + n)) R :=
  .of fun i j => j.addCases
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + n) then g.coeff (i - j₁) else 0)
    (fun j₁ => if (i : Nat) in Set.Icc (j₁ : Nat) (j₁ + m) then f.coeff (i - j₁) else 0)

variable (f g : R[X]) (m n : Nat)

/--
theorem `sylvester_zero_left_deg` / 定理 `sylvester_zero_left_deg`

English:
theorem sylvester_zero_left_deg
  proof: Matrix.ext fun i j => j.addCases nofun fun j => by
    rw [sylvester]; rw [Matrix.of_apply]; rw [Fin.addCases_right]; rw [Matrix.diagonal_apply]
    split_ifs <;> simp_all [Fin.ext_iff]

中文:
定理 sylvester_zero_left_deg
  证明: Matrix.ext fun i j => j.addCases nofun fun j => by
    rw [sylvester]; rw [Matrix.of_apply]; rw [Fin.addCases_right]; rw [Matrix.diagonal_apply]
    split_ifs <;> simp_all [Fin.ext_iff]
-/
...
/--
lemma `sylvester_comm` / 引理 `sylvester_comm`
...
Depends on / 依赖: Fin.addCases, addCases, sylvester
-/
lemma sylvester_comm :
...
/--
lemma `sylvester_map_map` / 引理 `sylvester_map_map`

English:
lemma sylvester_map_map
  given: (φ : R ->+* S)
  proof: by
  ext i j; induction j using Fin.addCases <;> simp [sylvester, apply_ite φ]

中文:
引理 sylvester_map_map
  条件: (φ : R ->+* S)
  证明: by
  ext i j; induction j using Fin.addCases <;> simp [sylvester, apply_ite φ]

Depends on / 依赖: Fin.addCases, addCases, apply_ite, sylvester
-/
...
/--
Definition of `sylvesterDeriv` / `sylvesterDeriv` 的定义

English:
definition sylvesterDeriv
  signature: (f : R[X])
  body: letI n := f.natDegree
  if hn : n = 0 then 0
...
定义 sylvesterDeriv
  签名: (f : R[X])
  定义体: letI n := f.natDegree
  if hn : n = 0 then 0
  else (f.derivative.sylvester f (n - 1) n).updateRow ⟨2 * n - 2, by lia⟩
    (fun j => if ↑j = n - 2 then 1 else (if ↑j = 2 * n - 2 then n else 0))

Depends on / 依赖: derivative, f.derivative.sylvester, f.natDegree, natDegree, sylvester, updateRow
-/
...
/--
lemma `sylvesterDeriv_updateRow` / 引理 `sylvesterDeriv_updateRow`

English:
lemma sylvesterDeriv_updateRow
  given: (f : R[X]) (hf : 0 < f.natDegree)
  proof: by
  by_cases hn : f.natDegree = 0
  · ext ⟨i, hi⟩; lia
  ext ⟨i, hi⟩ ⟨j, hj⟩
  rw [sylvesterDeriv]; rw [dif_neg hn]
...
/--
Definition of `resultant` / `resultant` 的定义

English:
definition resultant
  signature: (f g : R[X]) (m : Nat := f.natDegree) (n : Nat := g.natDegree)
  body: (sylvester f g m n).det

中文:
定义 resultant
  签名: (f g : R[X]) (m : 自然数 := f.natDegree) (n : 自然数 := g.natDegree)
  定义体: (sylvester f g m n).det

Depends on / 依赖: f.natDegree, g.natDegree, natDegree
-/
def resultant (f g : R[X]) (m : Nat := f.natDegree) (n : Nat := g.natDegree) : R :=
  (sylvester f g m n).det

variable (f g p : R[X]) (r : R) (m n k : Nat)

@[simp]
/--
lemma `resultant_map_map` / 引理 `resultant_map_map`

English:
lemma resultant_map_map
  given: (φ : R ->+* S)
  proof: by
  simp [resultant, Polynomial.sylvester_map_map, RingHom.map_det]

中文:
引理 resultant_map_map
  条件: (φ : R ->+* S)
  证明: by
  simp [resultant, Polynomial.sylvester_map_map, RingHom.map_det]

Depends on / 依赖: Polynomial, Polynomial.sylvester_map_map, RingHom, RingHom.map_det, map_det, resultant, sylvester_map_map
-/
...
/--
theorem `resultant_zero_left_deg` / 定理 `resultant_zero_left_deg`

English:
theorem resultant_zero_left_deg
  statement: resultant f g 0 m = f.coeff 0 ^ m
  proof: by
  simp [resultant]

中文:
定理 resultant_zero_left_deg
  结论: resultant f g 0 m = f.coeff 0 ^ m
  证明: by
  simp [resultant]
-/
...
/--
theorem `resultant_C_zero_left` / 定理 `resultant_C_zero_left`

English:
theorem resultant_C_zero_left
  statement: resultant (C r) g 0 m = r ^ m
  proof: by simp

中文:
定理 resultant_C_zero_left
  结论: resultant (C r) g 0 m = r ^ m
  证明: by simp

Depends on / 依赖: AddMonoid, ContinuousAdd, instNSMul
-/
theorem resultant_C_zero_left : resultant (C r) g 0 m = r ^ m := by simp

set_option backward.defeqAttrib.useBackward true in
/--
lemma `resultant_comm` / 引理 `resultant_comm`

English:
lemma resultant_comm
  statement: resultant f g m n = (-1) ^ (m * n) * resultant g f n m
  proof: by
  rw [resultant]; rw [resultant]; rw [sylvester_comm]; rw [Matrix.det_reindex]; rw [Equiv.Perm.sign_eq_prod_prod_Ioi]
  congr 1
  dsimp
...
中文:
引理 resultant_comm
  结论: resultant f g m n = (-1) ^ (m * n) * resultant g f n m
  证明: by
  rw [resultant]; rw [resultant]; rw [sylvester_comm]; rw [Matrix.det_reindex]; rw [Equiv.Perm.sign_eq_prod_prod_Ioi]
  congr 1
  dsimp
...
/--
theorem `resultant_zero_right_deg` / 定理 `resultant_zero_right_deg`

English:
theorem resultant_zero_right_deg
  statement: resultant f g m 0 = g.coeff 0 ^ m
  proof: by
  rw [resultant_comm]; simp

中文:
定理 resultant_zero_right_deg
  结论: resultant f g m 0 = g.coeff 0 ^ m
  证明: by
  rw [resultant_comm]; simp
-/
...
/--
theorem `resultant_C_zero_right` / 定理 `resultant_C_zero_right`

English:
theorem resultant_C_zero_right
  given: (r : R)
  statement: resultant f (C r) m 0 = r ^ m
  proof: by simp

@[simp]

中文:
定理 resultant_C_zero_right
  条件: (r : R)
  结论: resultant f (C r) m 0 = r ^ m
  证明: by simp
...
@[simp]
/--
theorem `resultant_zero_right` / 定理 `resultant_zero_right`

English:
theorem resultant_zero_right
  statement: resultant f 0 m n = 0 ^ m * f.coeff 0 ^ n
  proof: by
  obtain _ | m := m; · simp
...
中文:
定理 resultant_zero_right
  结论: resultant f 0 m n = 0 ^ m * f.coeff 0 ^ n
  证明: by
  obtain _ | m := m; · simp
  have (i : Fin (m + 1 + n)) : sylvester f 0 (m + 1) n i ⟨0, by lia⟩ = 0 := by
    simp [sylvester, show (0 : Fin (m + 1 + n)) = Fin.castAdd _ 0 from rfl, Fin.addCases_left]
...
Depends on / 依赖: Fin.addCases_left, Fin.castAdd, Matrix, Matrix.det_eq_zero_of_column_eq_zero, addCases_left, castAdd, det_eq_zero_of_column_eq_zero, resultant, sylvester
-/
...
@[simp]
/--
theorem `resultant_zero_left` / 定理 `resultant_zero_left`

English:
theorem resultant_zero_left
  statement: resultant 0 g m n = 0 ^ n * g.coeff 0 ^ m
  proof: by
  rw [resultant_comm]; rw [resultant_zero_right]
  cases n <;> simp

中文:
定理 resultant_zero_left
  结论: resultant 0 g m n = 0 ^ n * g.coeff 0 ^ m
  证明: by
  rw [resultant_comm]; rw [resultant_zero_right]
  cases n <;> simp

Depends on / 依赖: resultant_comm, resultant_zero_right
-/
...
/--
theorem `resultant_zero_zero` / 定理 `resultant_zero_zero`

English:
theorem resultant_zero_zero
  statement: resultant (0 : R[X]) 0 m n = 0 ^ (m + n)
  proof: by simp [pow_add]

中文:
定理 resultant_zero_zero
  结论: resultant (0 : R[X]) 0 m n = 0 ^ (m + n)
  证明: by simp [pow_add]
...
/--
lemma `resultant_add_mul_monomial_right` / 引理 `resultant_add_mul_monomial_right`

English:
lemma resultant_add_mul_monomial_right
  given: (hk : k + m <= n) (hf : f.natDegree <= m)
  proof: by
...
set_option backward.defeqAttrib.useBackward true in
/--
lemma `resultant_add_mul_right` / 引理 `resultant_add_mul_right`

English:
lemma resultant_add_mul_right
  given: (hp : p.natDegree + m <= n) (hf : f.natDegree <= m)
  proof: by
  have H : p.support subseteq Finset.range (n - m + 1) := by
...
引理 resultant_add_mul_right
  条件: (hp : p.natDegree + m <= n) (hf : f.natDegree <= m)
  证明: by
  have H : p.support subseteq Finset.range (n - m + 1) := by
    simp only [Finset.subset_iff, Finset.mem_range]
    exact fun x hx => (le_natDegree_of_mem_supp _ hx).trans_lt (by lia)
...
/--
lemma `resultant_add_mul_left` / 引理 `resultant_add_mul_left`

English:
lemma resultant_add_mul_left
  given: (hk : p.natDegree + n <= m) (hg : g.natDegree <= n)
  proof: by
  rw [resultant_comm]; rw [resultant_add_mul_right _ _ _ _ _ hk hg]; rw [resultant_comm]; rw [← mul_assoc]; rw [← pow_add]; rw [mul_comm m n]; rw [← two_mul]; rw [pow_mul]
  ring

中文:
引理 resultant_add_mul_left
  条件: (hk : p.natDegree + n <= m) (hg : g.natDegree <= n)
  证明: by
  rw [resultant_comm]; rw [resultant_add_mul_right _ _ _ _ _ hk hg]; rw [resultant_comm]; rw [← mul_assoc]; rw [← pow_add]; rw [mul_comm m n]; rw [← two_mul]; rw [pow_mul]
  ring

Depends on / 依赖: mul_assoc, mul_comm, pow_add, pow_mul, resultant_add_mul_right, resultant_comm, two_mul
-/
...
/--
lemma `resultant_C_mul_right` / 引理 `resultant_C_mul_right`

English:
lemma resultant_C_mul_right
  given: (r : R)
  proof: by
...
/--
lemma `resultant_C_mul_left` / 引理 `resultant_C_mul_left`

English:
lemma resultant_C_mul_left
  given: (r : R)
  proof: by
  rw [resultant_comm]; rw [resultant_C_mul_right]; rw [resultant_comm]; rw [mul_left_comm]; rw [← mul_assoc ((-1) ^ _)]; rw [mul_comm n m]; rw [← mul_pow]; rw [neg_one_mul]; rw [neg_neg]; rw [one_pow]; rw [one_mul]

中文:
引理 resultant_C_mul_left
  条件: (r : R)
  证明: by
  rw [resultant_comm]; rw [resultant_C_mul_right]; rw [resultant_comm]; rw [mul_left_comm]; rw [← mul_assoc ((-1) ^ _)]; rw [mul_comm n m]; rw [← mul_pow]; rw [neg_one_mul]; rw [neg_neg]; rw [one_pow]; rw [one_mul]

Depends on / 依赖: mul_assoc, mul_comm, mul_left_comm, mul_pow, neg_neg, neg_one_mul, one_mul, one_pow, resultant_C_mul_right, resultant_comm
-/
...
/--
lemma `resultant_succ_left_deg` / 引理 `resultant_succ_left_deg`

English:
lemma resultant_succ_left_deg
  given: (hf : f.natDegree <= m)
  proof: by
  obtain _ | n := n
  · simp [pow_succ']
...
/--
lemma `resultant_add_left_deg` / 引理 `resultant_add_left_deg`

English:
lemma resultant_add_left_deg
  given: (hf : f.natDegree <= m)
  proof: by
  induction k with
  | zero => simp
...
Depends on / 依赖: add_assoc, hf.trans, resultant_succ_left_deg
-/
...
/--
lemma `resultant_add_right_deg` / 引理 `resultant_add_right_deg`

English:
lemma resultant_add_right_deg
  given: (k : Nat) (hg : g.natDegree <= n)
  proof: by
  rw [resultant_comm]; rw [resultant_add_left_deg _ _ _ _ _ hg]; rw [resultant_comm]; rw [mul_assoc]; rw [mul_left_comm (f.coeff m ^ k)]
  simp only [← mul_assoc, ← pow_add, ← mul_add, mul_comm _ m]
  ring_nf
  simp [mul_comm]

中文:
引理 resultant_add_right_deg
  条件: (k : 自然数) (hg : g.natDegree <= n)
  证明: by
  rw [resultant_comm]; rw [resultant_add_left_deg _ _ _ _ _ hg]; rw [resultant_comm]; rw [mul_assoc]; rw [mul_left_comm (f.coeff m ^ k)]
  simp only [← mul_assoc, ← pow_add, ← mul_add, mul_comm _ m]
  ring_nf
...
Depends on / 依赖: f.coeff, mul_add, mul_assoc, mul_comm, mul_left_comm, pow_add, resultant_add_left_deg, resultant_comm, ring_nf
-/
lemma resultant_add_right_deg (k : Nat) (hg : g.natDegree <= n) :
...
/--
lemma `resultant_eq_zero_of_lt_lt` / 引理 `resultant_eq_zero_of_lt_lt`

English:
lemma resultant_eq_zero_of_lt_lt
  given: (hf : f.natDegree < m) (hg : g.natDegree < n)
  proof: by
  obtain _ | m := m; · lia
  obtain _ | n := n; · lia
  rw [resultant_add_left_deg _ _ _ _ _ (by lia)]; rw [resultant_add_right_deg _ _ _ _ _ (by lia)]
  simp [coeff_eq_zero_of_natDegree_lt hg]

@[simp]

中文:
引理 resultant_eq_zero_of_lt_lt
  条件: (hf : f.natDegree < m) (hg : g.natDegree < n)
  证明: by
  obtain _ | m := m; · lia
  obtain _ | n := n; · lia
  rw [resultant_add_left_deg _ _ _ _ _ (by lia)]; rw [resultant_add_right_deg _ _ _ _ _ (by lia)]
...
Depends on / 依赖: coeff_eq_zero_of_natDegree_lt, resultant_add_left_deg, resultant_add_right_deg
-/
...
@[simp]
/--
theorem `resultant_C_left` / 定理 `resultant_C_left`

English:
theorem resultant_C_left
  given: (r : R)
  proof: by
  rw [← zero_add m]; rw [resultant_add_left_deg _ _ _ _ _ (by simp)]; rw [mul_comm n m]
  simp
...
Depends on / 依赖: mul_comm, resultant_add_left_deg, zero_add
-/
...
/--
theorem `resultant_C_right` / 定理 `resultant_C_right`

English:
theorem resultant_C_right
  given: (r : R)
  statement: resultant f (C r) m n = f.coeff m ^ n * r ^ m
  proof: by
  rw [← zero_add n]; rw [resultant_add_right_deg _ _ _ _ _ (by simp)]
  simp

中文:
定理 resultant_C_right
  条件: (r : R)
  结论: resultant f (C r) m n = f.coeff m ^ n * r ^ m
  证明: by
  rw [← zero_add n]; rw [resultant_add_right_deg _ _ _ _ _ (by simp)]
  simp
-/
...
/--
theorem `resultant_one_left` / 定理 `resultant_one_left`

English:
theorem resultant_one_left
  statement: resultant 1 g m n = (-1) ^ (m * n) * g.coeff n ^ m
  proof: by
  simpa [-resultant_C_left] using! resultant_C_left g m n 1

中文:
定理 resultant_one_left
  结论: resultant 1 g m n = (-1) ^ (m * n) * g.coeff n ^ m
  证明: by
  simpa [-resultant_C_left] using! resultant_C_left g m n 1
-/
...
/--
theorem `resultant_one_right` / 定理 `resultant_one_right`

English:
theorem resultant_one_right
  statement: resultant f 1 m n = f.coeff m ^ n
  proof: by
  simpa [-resultant_C_right] using! resultant_C_right f m n 1

中文:
定理 resultant_one_right
  结论: resultant f 1 m n = f.coeff m ^ n
  证明: by
  simpa [-resultant_C_right] using! resultant_C_right f m n 1
-/
...
/--
lemma `resultant_X_sub_C_left` / 引理 `resultant_X_sub_C_left`

English:
lemma resultant_X_sub_C_left
  given: (r : R) (hg : g.natDegree <= n)
  proof: by
  nontriviality R
...
/--
lemma `resultant_X_add_C_left` / 引理 `resultant_X_add_C_left`

English:
lemma resultant_X_add_C_left
  given: (r : R) (hg : g.natDegree <= n)
  proof: by
  rw [← resultant_X_sub_C_left g n (-r) hg]; rw [map_neg]; rw [sub_neg_eq_add]

中文:
引理 resultant_X_add_C_left
  条件: (r : R) (hg : g.natDegree <= n)
  证明: by
  rw [← resultant_X_sub_C_left g n (-r) hg]; rw [map_neg]; rw [sub_neg_eq_add]
-/
...
/--
lemma `resultant_X_sub_C_right` / 引理 `resultant_X_sub_C_right`

English:
lemma resultant_X_sub_C_right
  given: (r : R) (hf : f.natDegree <= m)
  proof: by
  rw [resultant_comm]; rw [resultant_X_sub_C_left _ _ _ hf]
  simp

中文:
引理 resultant_X_sub_C_right
  条件: (r : R) (hf : f.natDegree <= m)
  证明: by
  rw [resultant_comm]; rw [resultant_X_sub_C_left _ _ _ hf]
  simp
-/
...
/--
lemma `resultant_X_add_C_right` / 引理 `resultant_X_add_C_right`

English:
lemma resultant_X_add_C_right
  given: (r : R) (hf : f.natDegree <= m)
  proof: by
  rw [← resultant_X_sub_C_right f m (-r) hf]; rw [map_neg]; rw [sub_neg_eq_add]

中文:
引理 resultant_X_add_C_right
  条件: (r : R) (hf : f.natDegree <= m)
  证明: by
  rw [← resultant_X_sub_C_right f m (-r) hf]; rw [map_neg]; rw [sub_neg_eq_add]
-/
...
/--
lemma `resultant_eq_prod_roots_sub` / 引理 `resultant_eq_prod_roots_sub`
...
nonrec lemma resultant_eq_prod_eval [IsDomain R]
    (f g : R[X]) (n : Nat) (hg : g.natDegree <= n) (hf : f.Splits) :
  ...
  · let inst := hR.toField
    have H : (C f.leadingCoeff⁻¹ * f).Monic := by
      rw [Monic]; rw [← coeff_natDegree]; rw [natDegree_C_mul (by simp [hf0]), coeff_C_mul]; simp [hf0]
  ...
  · let inst := hR.toField
    ...
    rw [resultant_C_mul_right]; rw [inv_pow]; rw [inv_mul_eq_iff_eq_mul₀ (by simp [hg0])] at this
  ...
  rw [← resultant_map_map]; rw [← Nat.add_sub_cancel' hg]; rw [resultant_add_right_deg _ _ _ _ _ (by simp)]; rw [this]; rw [coeff_map]; rw [coeff_natDegree]; rw [hfm.leadingCoeff]; rw [map_one]; rw [one_pow]; rw [one_mul]; rw [map_sub_sprod_roots_eq_prod_map_eval _ _ (hgm.map _) (SplittingField.splits _)]; rw [hf.roots_map]; rw [map_multiset_prod]; rw [Multiset.map_map]
...
nonrec lemma induction_of_Splits_of_injective_of_surjective.{u}
    ...
    (P : forall {R : Type u} [CommRing R], R[X] -> Prop)
    (Splits : forall (R : Type u) [Field R] (p : R[X]) (hp : p.Splits), P p)
    (injective : forall (R S : Type u) [CommRing R] [CommRing S]
      (φ : R ->+* S) (hφ : Function.Injective φ) (p : R[X]) (IH : P (p.map φ)), P p)
    (surjective : forall (R S : Type u) [CommRing R] [CommRing S]
      (φ : R ->+* S) (hφ : Function.Surjective φ) (p : S[X]) (IH : forall q : R[X], P q), P p) : P p := by
  wlog hR : IsDomain R generalizing R
  · exact surjective _ _ (MvPolynomial.eval₂Hom (algebraMap Int R) id)
...
nonrec lemma resultant_mul_right (f g₁ g₂ : R[X]) (m : Nat) (hm : f.natDegree <= m) :
  ...
  | surjective R S φ hφ f IH =>
    ...
    obtain ⟨g₂', hg₂, e₂⟩ := exists_degree_eq_of_mem_lifts (Polynomial.map_surjective φ hφ g₂)
    rw [← hg₁]; rw [← hg₂]; rw [← hf']; rw [← Polynomial.map_mul]
    simp_rw [resultant_map_map, hg₁, hg₂, hf', ← natDegree_eq_natDegree e₁,
      ← natDegree_eq_natDegree e₂, ← natDegree_eq_natDegree e, IH, map_mul]

/--
lemma `resultant_mul_left` / 引理 `resultant_mul_left`

English:
lemma resultant_mul_left
  given: (f₁ f₂ g : R[X]) (n : Nat) (hn : g.natDegree <= n)
  proof: by
  rw [resultant_comm]; rw [resultant_mul_right _ _ _ n hn]; rw [resultant_comm]; rw [resultant_comm f₂]
  ring_nf
  simp

中文:
引理 resultant_mul_left
  条件: (f₁ f₂ g : R[X]) (n : 自然数) (hn : g.natDegree <= n)
  证明: by
  rw [resultant_comm]; rw [resultant_mul_right _ _ _ n hn]; rw [resultant_comm]; rw [resultant_comm f₂]
  ring_nf
  simp

Depends on / 依赖: resultant_comm, resultant_mul_right, ring_nf
-/
lemma resultant_mul_left (f₁ f₂ g : R[X]) (n : Nat) (hn : g.natDegree <= n) :
...
@[simp] nonrec lemma resultant_self (f : R[X]) : resultant f f = 0 ^ f.natDegree := by
  ...
  | surjective R S φ hφ f IH =>
    obtain ⟨f', hf', e⟩ := exists_degree_eq_of_mem_lifts (Polynomial.map_surjective φ hφ f)
    rw [← hf']; rw [resultant_map_map]; rw [hf']; rw [← natDegree_eq_natDegree e]; rw [IH f']; rw [map_pow]; rw [map_zero]

/--
lemma `resultant_self_eq_zero` / 引理 `resultant_self_eq_zero`

English:
lemma resultant_self_eq_zero
  given: (f : R[X]) (h : f.natDegree != 0)
  proof: by
  simp [resultant_self, h]

中文:
引理 resultant_self_eq_zero
  条件: (f : R[X]) (h : f.natDegree != 0)
  证明: by
  simp [resultant_self, h]

Depends on / 依赖: resultant_self
-/
...
/--
lemma `resultant_dvd_leadingCoeff_pow` / 引理 `resultant_dvd_leadingCoeff_pow`

English:
lemma resultant_dvd_leadingCoeff_pow
  given: [IsDomain R] (f g : R[X]) (H : IsCoprime f g)
  proof: by
...
lemma resultant_dvd_leadingCoeff_pow [IsDomain R] (f g : R[X]) (H : IsCoprime f g) :
    exists n, resultant f g ∣ f.leadingCoeff ^ n := by
  obtain rfl | hf := eq_or_ne f 0
  · simp only [isCoprime_zero_left, isUnit_iff] at H
    aesop
  obtain rfl | hg := eq_or_ne g 0
...
/--
lemma `resultant_ne_zero` / 引理 `resultant_ne_zero`

English:
lemma resultant_ne_zero
  given: [IsDomain R] (f g : R[X]) (H : IsCoprime f g)
  proof: by
  obtain rfl | hf := eq_or_ne f 0
  · simp only [isCoprime_zero_left, isUnit_iff] at H
...
Depends on / 依赖: eq_or_ne, isCoprime_zero_left, isUnit_iff, resultant_dvd_leadingCoeff_pow
-/
...
@[simp]
/--
lemma `resultant_prod_left` / 引理 `resultant_prod_left`

English:
lemma resultant_prod_left
  statement: {ι : Type*} (s : Finset ι) (f : ι -> R[X]) (g : R[X])
  proof: by
  classical
...
lemma resultant_prod_left {ι : Type*} (s : Finset ι) (f : ι -> R[X]) (g : R[X])
    ...
    (∏ i in s, f i).resultant g (∏ i in s, f i).natDegree n =
      ∏ i in s, (f i).resultant g (f i).natDegree n := by
  classical
  induction s using Finset.induction with
  | empty => simp
  | insert a s has IH =>
...
@[simp]
/--
lemma `resultant_prod_right` / 引理 `resultant_prod_right`

English:
lemma resultant_prod_right
  statement: {ι : Type*} (s : Finset ι) (f : R[X]) (g : ι -> R[X])
  proof: by
...
Depends on / 依赖: Finset, Finset.mul_sum, Finset.prod_mul_distrib, Finset.prod_pow_eq_pow_sum, mul_sum, natDegree_prod, prod_mul_distrib, prod_pow_eq_pow_sum, resultant_comm, resultant_prod_left, simp_rw
-/
lemma resultant_prod_right {ι : Type*} (s : Finset ι) (f : R[X]) (g : ι -> R[X])
    (m : Nat) (hm : f.natDegree <= m) (hg : ∏ i in s, (g i).leadingCoeff != 0) :
    f.resultant (∏ i in s, g i) m = ∏ i in s, f.resultant (g i) m := by
  simp_rw [resultant_comm f]
  rw [resultant_prod_left _ _ _ _ hg hm]; rw [Finset.prod_mul_distrib]; rw [Finset.prod_pow_eq_pow_sum]; rw [← Finset.mul_sum]; rw [natDegree_prod' _ _ hg]

@[simp]
/--
lemma `resultant_pow_left` / 引理 `resultant_pow_left`

English:
lemma resultant_pow_left
  given: (hf : f.leadingCoeff ^ m != 0) (hn : g.natDegree <= n)
  proof: by
...
中文:
引理 resultant_pow_left
  条件: (hf : f.leadingCoeff ^ m != 0) (hn : g.natDegree <= n)
  证明: by
  convert! resultant_prod_left (Finset.range m) (fun _ => f) g n (by simpa) hn <;> simp

@[simp]

Depends on / 依赖: Finset, Finset.range, convert, resultant_prod_left
-/
...
@[simp]
/--
lemma `resultant_pow_right` / 引理 `resultant_pow_right`

English:
lemma resultant_pow_right
  given: (hm : f.natDegree <= m) (hg : g.leadingCoeff ^ n != 0)
  proof: by
  convert! resultant_prod_right (Finset.range n) f (fun _ => g) m hm (by simpa) <;> simp

中文:
引理 resultant_pow_right
  条件: (hm : f.natDegree <= m) (hg : g.leadingCoeff ^ n != 0)
  证明: by
  convert! resultant_prod_right (Finset.range n) f (fun _ => g) m hm (by simpa) <;> simp

Depends on / 依赖: Finset, Finset.range, convert, resultant_prod_right
-/
...
/--
lemma `resultant_X_sub_C_pow_left` / 引理 `resultant_X_sub_C_pow_left`

English:
lemma resultant_X_sub_C_pow_left
  given: (r : R) (g : R[X]) (m n : Nat) (hn : g.natDegree <= n)
  proof: by
  nontriviality R
  convert! resultant_pow_left _ _ _ _ _ _ <;> simp [natDegree_pow', hn]

中文:
引理 resultant_X_sub_C_pow_left
  条件: (r : R) (g : R[X]) (m n : 自然数) (hn : g.natDegree <= n)
  证明: by
  nontriviality R
  convert! resultant_pow_left _ _ _ _ _ _ <;> simp [natDegree_pow', hn]

Depends on / 依赖: convert, natDegree_pow, nontriviality, resultant_pow_left
-/
lemma resultant_X_sub_C_pow_left (r : R) (g : R[X]) (m n : Nat) (hn : g.natDegree <= n) :
    ((X - C r) ^ m).resultant g m n = eval r g ^ m := by
  nontriviality R
  convert! resultant_pow_left _ _ _ _ _ _ <;> simp [natDegree_pow', hn]

/--
lemma `resultant_X_sub_C_pow_right` / 引理 `resultant_X_sub_C_pow_right`

English:
lemma resultant_X_sub_C_pow_right
  given: (f : R[X]) (r : R) (m n : Nat) (hm : f.natDegree <= m)
  proof: by
  rw [resultant_comm]; rw [resultant_X_sub_C_pow_left _ _ _ _ hm]

中文:
引理 resultant_X_sub_C_pow_right
  条件: (f : R[X]) (r : R) (m n : 自然数) (hm : f.natDegree <= m)
  证明: by
  rw [resultant_comm]; rw [resultant_X_sub_C_pow_left _ _ _ _ hm]

Depends on / 依赖: resultant_X_sub_C_pow_left, resultant_comm
-/
lemma resultant_X_sub_C_pow_right (f : R[X]) (r : R) (m n : Nat) (hm : f.natDegree <= m) :
    f.resultant ((X - C r) ^ n) m n = (-1) ^ (m * n) * eval r f ^ n := by
  rw [resultant_comm]; rw [resultant_X_sub_C_pow_left _ _ _ _ hm]

/--
lemma `resultant_X_pow_left` / 引理 `resultant_X_pow_left`

English:
lemma resultant_X_pow_left
  given: (g : R[X]) (m n : Nat) (hn : g.natDegree <= n)
  proof: by
  convert! resultant_X_sub_C_pow_left 0 g m n hn <;> simp [coeff_zero_eq_eval_zero]

中文:
引理 resultant_X_pow_left
  条件: (g : R[X]) (m n : 自然数) (hn : g.natDegree <= n)
  证明: by
  convert! resultant_X_sub_C_pow_left 0 g m n hn <;> simp [coeff_zero_eq_eval_zero]

Depends on / 依赖: coeff_zero_eq_eval_zero, convert, resultant_X_sub_C_pow_left
-/
lemma resultant_X_pow_left (g : R[X]) (m n : Nat) (hn : g.natDegree <= n) :
    (X ^ m).resultant g m n = g.coeff 0 ^ m := by
  convert! resultant_X_sub_C_pow_left 0 g m n hn <;> simp [coeff_zero_eq_eval_zero]

/--
lemma `resultant_X_pow_right` / 引理 `resultant_X_pow_right`

English:
lemma resultant_X_pow_right
  given: (f : R[X]) (m n : Nat) (hm : f.natDegree <= m)
  proof: by
...
引理 resultant_X_pow_right
  条件: (f : R[X]) (m n : 自然数) (hm : f.natDegree <= m)
  证明: by
  convert! resultant_X_sub_C_pow_right f 0 m n hm <;> simp [coeff_zero_eq_eval_zero]
...
-/
lemma resultant_X_pow_right (f : R[X]) (m n : Nat) (hm : f.natDegree <= m) :
...
/--
lemma `resultant_integralNormalization` / 引理 `resultant_integralNormalization`

English:
lemma resultant_integralNormalization
  given: (f g : R[X]) (hg : g.natDegree != 0)
  proof: by
  rw [natDegree_scaleRoots]; rw [natDegree_integralNormalization]
  wlog hR : IsDomain R
...
nonrec lemma resultant_taylor (f g : R[X]) (r : R) :
  ...
  | Splits R f hf' =>
    ...
    | mem x h =>
      ...
      · rw [taylor_C]; simp
      · nontriviality R
        rw [map_add]; rw [taylor_X]; rw [taylor_C]; rw [add_assoc]; rw [← map_add]
        simp [-map_add, taylor_eval]
    | one => simp
    | mul x y hx hy hx' hy' =>
      by_cases hx0 : x = 0; · simp [hx0]
      by_cases hy0 : y = 0; · simp [hy0]
      rw [taylor_mul]; rw [natDegree_mul' (by simp [*]), resultant_mul_left _ _ _ _ le_rfl]
      simp [natDegree_mul', hx0, hy0, resultant_mul_left, ← hx', ← hy']
  | injective R S φ hφ f IH =>
    apply hφ
    have := IH (g.map φ) (φ r)
    rw [← map_taylor]; rw [← map_taylor] at this
    simpa [natDegree_map_eq_of_injective hφ] using this
  | surjective R S φ hφ f IH =>
    obtain ⟨f', hf', ef⟩ := exists_degree_eq_of_mem_lifts (Polynomial.map_surjective φ hφ f)
    obtain ⟨g', hg', eg⟩ := exists_degree_eq_of_mem_lifts (Polynomial.map_surjective φ hφ g)
    ...
    have hgl : g.leadingCoeff = φ g'.leadingCoeff := by
      simp_rw [← coeff_natDegree, ← natDegree_eq_natDegree eg, ← hg', coeff_map]
    rw [natDegree_taylor]; rw [natDegree_taylor]; rw [← hf']; rw [← map_taylor]; rw [← hg']; rw [← map_taylor]; rw [resultant_map_map]; rw [resultant_map_map]; rw [hf']; rw [hg']; rw [← natDegree_eq_natDegree ef]; rw [← natDegree_eq_natDegree eg]; rw [← IH f' g' r]; rw [natDegree_taylor]; rw [natDegree_taylor]
...
noncomputable
/--
Definition of `sylvesterMap` / `sylvesterMap` 的定义

English:
definition sylvesterMap
  signature: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  body: ⟨f * pq.2 + g * pq.1, by
    obtain ⟨⟨p, hp⟩, ⟨q, hq⟩⟩ := pq
...
中文:
定义 sylvesterMap
  签名: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  定义体: ⟨f * pq.2 + g * pq.1, by
    obtain ⟨⟨p, hp⟩, ⟨q, hq⟩⟩ := pq
    rw [Polynomial.mem_degreeLT]
    refine (degree_add_le _ _).trans_lt (max_lt ?_ ?_)
    · by_cases hf' : f = 0; · simp_all
...
/--
lemma `toMatrix_sylvesterMap` / 引理 `toMatrix_sylvesterMap`

English:
lemma toMatrix_sylvesterMap
  given: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  proof: by
  ext i (j | j)
...
引理 toMatrix_sylvesterMap
  条件: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  证明: by
  ext i (j | j)
  · suffices (if j.1 <= i then g.coeff (i - j) else 0) =
      if j <= i.1 ∧ ↑i <= j + n then g.coeff (i - j) else 0 by
...
Depends on / 依赖: LinearMap, LinearMap.toMatrix_apply, finSumFinEquiv, finSumFinEquiv.surjective, surjective, toMatrix_apply, toMatrix_sylvesterMap
-/
lemma toMatrix_sylvesterMap' (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n) :
    (sylvesterMap f g hf hg).toMatrix
...
noncomputable
/--
Definition of `adjSylvester` / `adjSylvester` 的定义

English:
definition adjSylvester
  signature: (f g : R[X])
  body: (f.sylvester g m n).adjugate.toLin (degreeLT.basis R (m + n))
    (((degreeLT.basis R m).prod (degreeLT.basis R n)).reindex finSumFinEquiv)

中文:
定义 adjSylvester
  签名: (f g : R[X])
  定义体: (f.sylvester g m n).adjugate.toLin (degreeLT.basis R (m + n))
    (((degreeLT.basis R m).prod (degreeLT.basis R n)).reindex finSumFinEquiv)

Depends on / 依赖: adjugate, adjugate.toLin, degreeLT, degreeLT.basis, f.sylvester, finSumFinEquiv, reindex, sylvester
-/
...
/--
lemma `sylveserMap_comp_adjSylvester` / 引理 `sylveserMap_comp_adjSylvester`

English:
lemma sylveserMap_comp_adjSylvester
  given: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  proof: by
...
/--
lemma `adjSylvester_comp_sylveserMap` / 引理 `adjSylvester_comp_sylveserMap`

English:
lemma adjSylvester_comp_sylveserMap
  given: (f g : R[X]) (hf : f.natDegree <= m) (hg : g.natDegree <= n)
  proof: by
  let b₁ := ((degreeLT.basis R m).prod (degreeLT.basis R n)).reindex finSumFinEquiv
  let b₂ := degreeLT.basis R (m + n)
...
/--
lemma `exists_mul_add_mul_eq_C_resultant` / 引理 `exists_mul_add_mul_eq_C_resultant`
...
lemma exists_mul_add_mul_eq_C_resultant
    ...
    exists p q, p.degree < ↑n ∧ q.degree < ↑m ∧ f * p + g * q = C (f.resultant g m n) := by
  nontriviality R
  let X := adjSylvester f g ⟨1, by simpa [Polynomial.mem_degreeLT,
    ← Nat.cast_add, Nat.pos_iff_ne_zero, not_and_or, -not_and] using H⟩
  have : ((sylvesterMap f g hf hg X)).1 = _ :=
...
/--
lemma `isUnit_resultant_iff_isCoprime` / 引理 `isUnit_resultant_iff_isCoprime`

English:
lemma isUnit_resultant_iff_isCoprime
  given: {f g : R[X]} (hf : f.Monic)
  proof: by
  by_cases hf0 : f.natDegree = 0
...
/--
lemma `resultant_eq_zero_iff` / 引理 `resultant_eq_zero_iff`

English:
lemma resultant_eq_zero_iff
  given: {K : Type*} [Field K] {f g : K[X]}
  proof: by
  obtain rfl | hf := eq_or_ne f 0
  · obtain rfl | hg := eq_or_ne g 0; · simp
...
definition discr
  signature: (f : R[X])
  body: f.sylvesterDeriv.det * (-1) ^ (f.natDegree * (f.natDegree - 1) / 2)

中文:
定义 discr
  签名: (f : R[X])
  定义体: f.sylvesterDeriv.det * (-1) ^ (f.natDegree * (f.natDegree - 1) / 2)

Depends on / 依赖: f.natDegree, f.sylvesterDeriv.det, natDegree, sylvesterDeriv
-/
...
set_option backward.isDefEq.respectTransparency.types false in
/--
lemma `discr_C` / 引理 `discr_C`

English:
lemma discr_C
  given: (r : R)
  statement: discr (C r) = 1
  proof: by
...
/--
lemma `discr_of_degree_eq_one` / 引理 `discr_of_degree_eq_one`

English:
lemma discr_of_degree_eq_one
  given: {f : R[X]} (hf : f.degree = 1)
  statement: discr f = 1
  proof: by
  rw [← Nat.cast_one]; rw [degree_eq_iff_natDegree_eq_of_pos one_pos] at hf
...
/--
lemma `discr_of_degree_eq_two` / 引理 `discr_of_degree_eq_two`

English:
lemma discr_of_degree_eq_two
  given: {f : R[X]} (hf : f.degree = 2)
  proof: by
  rw [← Nat.cast_two]; rw [degree_eq_iff_natDegree_eq_of_pos two_pos] at hf
...
/--
lemma `resultant_deriv` / 引理 `resultant_deriv`

English:
lemma resultant_deriv
  given: {f : R[X]} (hf : 0 < f.degree)
  proof: by
...
set_option linter.style.whitespace false in -- manual alignment is not recognised
/--
lemma `sylvesterDeriv_of_natDegree_eq_three` / 引理 `sylvesterDeriv_of_natDegree_eq_three`

English:
lemma sylvesterDeriv_of_natDegree_eq_three
  given: {f : R[X]} (hf : f.natDegree = 3)
  proof: by
  ext ⟨i, hi⟩ ⟨j, hj⟩
...
/--
lemma `discr_of_degree_eq_three` / 引理 `discr_of_degree_eq_three`

English:
lemma discr_of_degree_eq_three
  given: {f : R[X]} (hf : f.degree = 3)
  proof: by
