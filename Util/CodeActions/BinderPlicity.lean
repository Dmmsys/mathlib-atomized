...
/--
Definition of `binderPlicity` / `binderPlicity` 的定义

English:
definition binderPlicity
  signature: : CodeActionProvider
  body: fun params snap => do
...
中文:
定义 binderPlicity
  签名: : CodeActionProvider
  定义体: fun params snap => do
  let doc ← RequestM.readDoc
  let mkCodeAction kind range newText : LazyCodeAction := {
      eager.title := s!"Make {kind}: {newText}"
      eager.kind? := "refactor.rewrite"
