/-
Copyright (c) 2022 Jujian Zhang. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Jujian Zhang
-/
module

public import Mathlib.RingTheory.GradedAlgebra.Basic
public import Mathlib.Algebra.GradedMulAction
public import Mathlib.Algebra.DirectSum.Decomposition
public import Mathlib.Algebra.Module.BigOperators

/-!
# Graded Module

Given an `R`-algebra `A` graded by `𝓐`, a graded `A`-module `M` is expressed as
`DirectSum.Decomposition 𝓜` and `SetLike.GradedSMul 𝓐 𝓜`.
Then `⨁ i, 𝓜 i` is an `A`-module and is isomorphic to `M`.

## Tags

graded module
-/

@[expose] public section


section

open DirectSum

variable {ιA ιB : Type*} (A : ιA -> Type*) (M : ιB -> Type*)

namespace DirectSum

open GradedMonoid

/--
Definition of `GdistribMulAction` / `GdistribMulAction` 的定义

English:
class GdistribMulAction
  parameters: [AddMonoid ιA] [VAdd ιA ιB] [GMonoid A] [forall i, AddMonoid (M i)]
  extends: GMulAction A M
  axioms and operations (2):
    - smul_add({i j} (a : A i) (b c : M j)) : smul a (b + c) = smul a b + smul a c
    - smul_zero({i j} (a : A i)) : smul a (0 : M j) = 0

中文:
类 GdistribMulAction
  参数: [AddMonoid ιA] [VAdd ιA ιB] [GMonoid A] [对任意 i, AddMonoid (M i)]
  继承: GMulAction A M
  公理与运算 (2 个):
    - smul_add({i j} (a : A i) (b c : M j)) : smul a (b + c) = smul a b + smul a c
    - smul_zero({i j} (a : A i)) : smul a (0 : M j) = 0
-/
class GdistribMulAction [AddMonoid ιA] [VAdd ιA ιB] [GMonoid A] [forall i, AddMonoid (M i)]
    extends GMulAction A M where
  smul_add {i j} (a : A i) (b c : M j) : smul a (b + c) = smul a b + smul a c
  smul_zero {i j} (a : A i) : smul a (0 : M j) = 0

/--
Definition of `Gmodule` / `Gmodule` 的定义

English:
class Gmodule
  parameters: [AddMonoid ιA] [VAdd ιA ιB] [forall i, AddMonoid (A i)] [forall i, AddMonoid (M i)] [GMonoid A]
  extends: GdistribMulAction A M
  axioms and operations (2):
    - add_smul({i j} (a a' : A i) (b : M j)) : smul (a + a') b = smul a b + smul a' b
    - zero_smul({i j} (b : M j)) : smul (0 : A i) b = 0

中文:
类 Gmodule
  参数: [AddMonoid ιA] [VAdd ιA ιB] [对任意 i, AddMonoid (A i)] [对任意 i, AddMonoid (M i)] [GMonoid A]
  继承: GdistribMulAction A M
  公理与运算 (2 个):
    - add_smul({i j} (a a' : A i) (b : M j)) : smul (a + a') b = smul a b + smul a' b
    - zero_smul({i j} (b : M j)) : smul (0 : A i) b = 0
-/
class Gmodule [AddMonoid ιA] [VAdd ιA ιB] [forall i, AddMonoid (A i)] [forall i, AddMonoid (M i)] [GMonoid A]
    extends GdistribMulAction A M where
  add_smul {i j} (a a' : A i) (b : M j) : smul (a + a') b = smul a b + smul a' b
  zero_smul {i j} (b : M j) : smul (0 : A i) b = 0

/--
Instance `GSemiring.toGmodule` / 实例 `GSemiring.toGmodule`

English:
instance GSemiring.toGmodule
  signature: [AddMonoid ιA] [forall i : ιA, AddCommMonoid (A i)]
  body: { GMonoid.toGMulAction A with
    smul_add := fun _ _ _ => h.mul_add _ _ _
    smul_zero := fun _ => h.mul_zero _
    add_smul := fun _ _ => h.add_mul _ _
    zero_smul := fun _ => h.zero_mul _ }

中文:
实例 GSemiring.toGmodule
  签名: [AddMonoid ιA] [对任意 i : ιA, AddCommMonoid (A i)]
  定义体: { GMonoid.toGMulAction A with
    smul_add := fun _ _ _ => h.mul_add _ _ _
    smul_zero := fun _ => h.mul_zero _
    add_smul := fun _ _ => h.add_mul _ _
    zero_smul := fun _ => h.zero_mul _ }

Depends on / 依赖: GMonoid, GMonoid.toGMulAction, add_mul, add_smul, h.add_mul, h.mul_add, h.mul_zero, h.zero_mul, mul_add, mul_zero, smul_add, smul_zero, toGMulAction, zero_mul, zero_smul
-/
instance GSemiring.toGmodule [AddMonoid ιA] [forall i : ιA, AddCommMonoid (A i)]
    [h : GSemiring A] : Gmodule A A :=
  { GMonoid.toGMulAction A with
    smul_add := fun _ _ _ => h.mul_add _ _ _
    smul_zero := fun _ => h.mul_zero _
    add_smul := fun _ _ => h.add_mul _ _
    zero_smul := fun _ => h.zero_mul _ }

variable [AddMonoid ιA] [VAdd ιA ιB] [forall i : ιA, AddCommMonoid (A i)] [forall i, AddCommMonoid (M i)]

/-- The piecewise multiplication from the `Mul` instance, as a bundled homomorphism. -/
@[simps]
/--
Definition of `gsmulHom` / `gsmulHom` 的定义

English:
definition gsmulHom
  signature: [GMonoid A] [Gmodule A M] {i j}
  body: { toFun := fun b => GSMul.smul a b
      map_zero' := GdistribMulAction.smul_zero _
      map_add' := GdistribMulAction.smul_add _ }
  map_zero' := AddMonoidHom.ext fun a => Gmodule.zero_smul a
  map_add' _a₁ _a₂ := AddMonoidHom.ext fun _b => Gmodule.add_smul _ _ _

中文:
定义 gsmulHom
  签名: [GMonoid A] [Gmodule A M] {i j}
  定义体: { toFun := fun b => GSMul.smul a b
      map_zero' := GdistribMulAction.smul_zero _
      map_add' := GdistribMulAction.smul_add _ }
  map_zero' := AddMonoidHom.ext fun a => Gmodule.zero_smul a
  map_add' _a₁ _a₂ := AddMonoidHom.ext fun _b => Gmodule.add_smul _ _ _

Depends on / 依赖: AddMonoidHom, AddMonoidHom.ext, GSMul.smul, GdistribMulAction, GdistribMulAction.smul_add, GdistribMulAction.smul_zero, Gmodule, Gmodule.add_smul, Gmodule.zero_smul, add_smul, map_add, map_zero, smul_add, smul_zero, zero_smul
-/
def gsmulHom [GMonoid A] [Gmodule A M] {i j} : A i ->+ M j ->+ M (i +ᵥ j) where
  toFun a :=
    { toFun := fun b => GSMul.smul a b
      map_zero' := GdistribMulAction.smul_zero _
      map_add' := GdistribMulAction.smul_add _ }
  map_zero' := AddMonoidHom.ext fun a => Gmodule.zero_smul a
  map_add' _a₁ _a₂ := AddMonoidHom.ext fun _b => Gmodule.add_smul _ _ _

namespace Gmodule

/--
Definition of `smulAddMonoidHom` / `smulAddMonoidHom` 的定义

English:
definition smulAddMonoidHom
  signature: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  body: toAddMonoid fun _i =>
AddMonoidHom.flip
toAddMonoid fun _j => AddMonoidHom.flip (of M _).compHom.comp gsmulHom A M

中文:
定义 smulAddMonoidHom
  签名: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  定义体: toAddMonoid fun _i =>
AddMonoidHom.flip
toAddMonoid fun _j => AddMonoidHom.flip (of M _).compHom.comp gsmulHom A M

Depends on / 依赖: AddMonoidHom, AddMonoidHom.flip, compHom, compHom.comp, gsmulHom, toAddMonoid
-/
def smulAddMonoidHom [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M] :
    (⨁ i, A i) ->+ (⨁ i, M i) ->+ ⨁ i, M i :=
  toAddMonoid fun _i =>
AddMonoidHom.flip
toAddMonoid fun _j => AddMonoidHom.flip (of M _).compHom.comp gsmulHom A M

section

open GradedMonoid DirectSum Gmodule

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance [DecidableEq
  signature: ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M] :
  body: smulAddMonoidHom A M x y

@[simp]

中文:
实例 [DecidableEq
  签名: ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M] :
  定义体: smulAddMonoidHom A M x y

@[simp]

Depends on / 依赖: smulAddMonoidHom
-/
instance [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M] :
    SMul (⨁ i, A i) (⨁ i, M i) where
  smul x y := smulAddMonoidHom A M x y

@[simp]
/--
theorem `smul_def` / 定理 `smul_def`

English:
theorem smul_def
  statement: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  proof: rfl

@[simp]

中文:
定理 smul_def
  结论: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  证明: rfl

@[simp]
-/
theorem smul_def [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
    (x : ⨁ i, A i) (y : ⨁ i, M i) :
    x • y = smulAddMonoidHom _ _ x y := rfl

@[simp]
/--
theorem `smulAddMonoidHom_apply_of_of` / 定理 `smulAddMonoidHom_apply_of_of`

English:
theorem smulAddMonoidHom_apply_of_of
  statement: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  proof: by
  simp [smulAddMonoidHom]

中文:
定理 smulAddMonoidHom_apply_of_of
  结论: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  证明: by
  simp [smulAddMonoidHom]

Depends on / 依赖: smulAddMonoidHom
-/
theorem smulAddMonoidHom_apply_of_of [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
    {i j} (x : A i) (y : M j) :
    smulAddMonoidHom A M (DirectSum.of A i x) (of M j y) = of M (i +ᵥ j) (GSMul.smul x y) := by
  simp [smulAddMonoidHom]

/--
theorem `of_smul_of` / 定理 `of_smul_of`

English:
theorem of_smul_of
  statement: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  proof: by simp

中文:
定理 of_smul_of
  结论: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  证明: by simp
-/
theorem of_smul_of [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
    {i j} (x : A i) (y : M j) :
    DirectSum.of A i x • of M j y = of M (i +ᵥ j) (GSMul.smul x y) := by simp

open AddMonoidHom

-- Almost identical to the proof of `direct_sum.one_mul`
/--
theorem `one_smul'` / 定理 `one_smul'`

English:
theorem one_smul'
  statement: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  proof: by
  suffices smulAddMonoidHom A M 1 = AddMonoidHom.id (⨁ i, M i) from DFunLike.congr_fun this x
  apply DirectSum.addHom_ext; intro i xi
  rw [show (1 : DirectSum ιA fun i => A i) = (of A 0) GOne.one by rfl]
  rw [smulAddMonoidHom_apply_of_of]
  exact DirectSum.of_eq_of_gradedMonoid_eq (one_smul (G

中文:
定理 one_smul'
  结论: [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
  证明: by
  suffices smulAddMonoidHom A M 1 = AddMonoidHom.id (⨁ i, M i) from DFunLike.congr_fun this x
  apply DirectSum.addHom_ext; intro i xi
  rw [show (1 : DirectSum ιA fun i => A i) = (of A 0) GOne.one by rfl]
  rw [smulAddMonoidHom_apply_of_of]
  exact DirectSum.of_eq_of_gradedMonoid_eq (one_smul (G
-/
private theorem one_smul' [DecidableEq ιA] [DecidableEq ιB] [GMonoid A] [Gmodule A M]
    (x : ⨁ i, M i) :
    (1 : ⨁ i, A i) • x = x := by
  suffices smulAddMonoidHom A M 1 = AddMonoidHom.id (⨁ i, M i) from DFunLike.congr_fun this x
  apply DirectSum.addHom_ext; intro i xi
  rw [show (1 : DirectSum ιA fun i => A i) = (of A 0) GOne.one by rfl]
  rw [smulAddMonoidHom_apply_of_of]
  exact DirectSum.of_eq_of_gradedMonoid_eq (one_smul (GradedMonoid A) <| GradedMonoid.mk i xi)

set_option backward.defeqAttrib.useBackward true in
-- Almost identical to the proof of `direct_sum.mul_assoc`
/--
theorem `mul_smul'` / 定理 `mul_smul'`

English:
theorem mul_smul'
  statement: [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M]
  proof: by
  suffices
    (-- `fun a b c ↦ (a * b) • c` as a bundled hom
              smulAddMonoidHom
              A M).compHom.comp
        (DirectSum.mulHom A) =
      (AddMonoidHom.compHom AddMonoidHom.flipHom <|
(smulAddMonoidHom A M).flip.compHom.comp smulAddMonoidHom A M).flip
    from -- `fun a b 

中文:
定理 mul_smul'
  结论: [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M]
  证明: by
  suffices
    (-- `fun a b c ↦ (a * b) • c` as a bundled hom
              smulAddMonoidHom
              A M).compHom.comp
        (DirectSum.mulHom A) =
      (AddMonoidHom.compHom AddMonoidHom.flipHom <|
(smulAddMonoidHom A M).flip.compHom.comp smulAddMonoidHom A M).flip
    from -- `fun a b 
-/
private theorem mul_smul' [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M]
    (a b : ⨁ i, A i)
    (c : ⨁ i, M i) : (a * b) • c = a • b • c := by
  suffices
    (-- `fun a b c ↦ (a * b) • c` as a bundled hom
              smulAddMonoidHom
              A M).compHom.comp
        (DirectSum.mulHom A) =
      (AddMonoidHom.compHom AddMonoidHom.flipHom <|
(smulAddMonoidHom A M).flip.compHom.comp smulAddMonoidHom A M).flip
    from -- `fun a b c ↦ a • (b • c)` as a bundled hom
      DFunLike.congr_fun (DFunLike.congr_fun (DFunLike.congr_fun this a) b) c
  ext ai ax bi bx ci cx : 6
  dsimp only [coe_comp, Function.comp_apply, compHom_apply_apply, flip_apply, flipHom_apply]
  rw [smulAddMonoidHom_apply_of_of]; rw [smulAddMonoidHom_apply_of_of]; rw [DirectSum.mulHom_of_of]; rw [smulAddMonoidHom_apply_of_of]
  exact
    DirectSum.of_eq_of_gradedMonoid_eq
      (mul_smul (GradedMonoid.mk ai ax) (GradedMonoid.mk bi bx) (GradedMonoid.mk ci cx))

/--
Instance `module` / 实例 `module`

English:
instance module
  signature: [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M]
  body: by exact one_smul' _ _
  mul_smul := by exact mul_smul' _ _
  smul_add r := (smulAddMonoidHom A M r).map_add
  smul_zero r := (smulAddMonoidHom A M r).map_zero
  add_smul r s x := by simp only [smul_def, map_add, AddMonoidHom.add_apply]
  zero_smul x := by simp only [smul_def, map_zero, AddMonoidHom

中文:
实例 module
  签名: [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M]
  定义体: by exact one_smul' _ _
  mul_smul := by exact mul_smul' _ _
  smul_add r := (smulAddMonoidHom A M r).map_add
  smul_zero r := (smulAddMonoidHom A M r).map_zero
  add_smul r s x := by simp only [smul_def, map_add, AddMonoidHom.add_apply]
  zero_smul x := by simp only [smul_def, map_zero, AddMonoidHom

Depends on / 依赖: AddMonoidHom, AddMonoidHom.add_apply, AddMonoidHom.zero_apply, add_apply, add_smul, map_add, map_zero, mul_smul, one_smul, smulAddMonoidHom, smul_add, smul_def, smul_zero, zero_apply, zero_smul
-/
instance module [DecidableEq ιA] [DecidableEq ιB] [GSemiring A] [Gmodule A M] :
    Module (⨁ i, A i) (⨁ i, M i) where
  one_smul := by exact one_smul' _ _
  mul_smul := by exact mul_smul' _ _
  smul_add r := (smulAddMonoidHom A M r).map_add
  smul_zero r := (smulAddMonoidHom A M r).map_zero
  add_smul r s x := by simp only [smul_def, map_add, AddMonoidHom.add_apply]
  zero_smul x := by simp only [smul_def, map_zero, AddMonoidHom.zero_apply]

end

end Gmodule

end DirectSum

end

open DirectSum

variable {ιA ιM R A M σ σ' : Type*}
variable [AddMonoid ιA] [AddAction ιA ιM] [CommSemiring R] [Semiring A] [Algebra R A]
variable (𝓐 : ιA -> σ') [SetLike σ' A]
variable (𝓜 : ιM -> σ)

namespace SetLike

/--
Instance `gmulAction` / 实例 `gmulAction`

English:
instance gmulAction
  signature: [AddMonoid M] [DistribMulAction A M] [SetLike σ M] [SetLike.GradedMonoid 𝓐]
  body: { SetLike.toGSMul 𝓐 𝓜 with
    one_smul := fun ⟨_i, _m⟩ => Sigma.subtype_ext (zero_vadd _ _) (one_smul _ _)
    mul_smul := fun ⟨_i, _a⟩ ⟨_j, _a'⟩ ⟨_k, _b⟩ =>
      Sigma.subtype_ext (add_vadd _ _ _) (mul_smul _ _ _) }

中文:
实例 gmulAction
  签名: [AddMonoid M] [DistribMulAction A M] [SetLike σ M] [SetLike.GradedMonoid 𝓐]
  定义体: { SetLike.toGSMul 𝓐 𝓜 with
    one_smul := fun ⟨_i, _m⟩ => Sigma.subtype_ext (zero_vadd _ _) (one_smul _ _)
    mul_smul := fun ⟨_i, _a⟩ ⟨_j, _a'⟩ ⟨_k, _b⟩ =>
      Sigma.subtype_ext (add_vadd _ _ _) (mul_smul _ _ _) }

Depends on / 依赖: SetLike, SetLike.toGSMul, Sigma.subtype_ext, add_vadd, mul_smul, one_smul, subtype_ext, toGSMul, zero_vadd
-/
instance gmulAction [AddMonoid M] [DistribMulAction A M] [SetLike σ M] [SetLike.GradedMonoid 𝓐]
    [SetLike.GradedSMul 𝓐 𝓜] : GradedMonoid.GMulAction (fun i => 𝓐 i) fun i => 𝓜 i :=
  { SetLike.toGSMul 𝓐 𝓜 with
    one_smul := fun ⟨_i, _m⟩ => Sigma.subtype_ext (zero_vadd _ _) (one_smul _ _)
    mul_smul := fun ⟨_i, _a⟩ ⟨_j, _a'⟩ ⟨_k, _b⟩ =>
      Sigma.subtype_ext (add_vadd _ _ _) (mul_smul _ _ _) }

/--
Instance `gdistribMulAction` / 实例 `gdistribMulAction`

English:
instance gdistribMulAction
  signature: [AddMonoid M] [DistribMulAction A M] [SetLike σ M]
  body: { SetLike.gmulAction 𝓐 𝓜 with
smul_add := fun _a _b _c => Subtype.ext smul_add _ _ _
smul_zero := fun _a => Subtype.ext smul_zero _ }

中文:
实例 gdistribMulAction
  签名: [AddMonoid M] [DistribMulAction A M] [SetLike σ M]
  定义体: { SetLike.gmulAction 𝓐 𝓜 with
smul_add := fun _a _b _c => Subtype.ext smul_add _ _ _
smul_zero := fun _a => Subtype.ext smul_zero _ }

Depends on / 依赖: SetLike, SetLike.gmulAction, Subtype, Subtype.ext, gmulAction, smul_add, smul_zero
-/
instance gdistribMulAction [AddMonoid M] [DistribMulAction A M] [SetLike σ M]
    [AddSubmonoidClass σ M] [SetLike.GradedMonoid 𝓐] [SetLike.GradedSMul 𝓐 𝓜] :
    DirectSum.GdistribMulAction (fun i => 𝓐 i) fun i => 𝓜 i :=
  { SetLike.gmulAction 𝓐 𝓜 with
smul_add := fun _a _b _c => Subtype.ext smul_add _ _ _
smul_zero := fun _a => Subtype.ext smul_zero _ }

variable [AddCommMonoid M] [Module A M] [SetLike σ M] [AddSubmonoidClass σ' A]
  [AddSubmonoidClass σ M] [SetLike.GradedMonoid 𝓐] [SetLike.GradedSMul 𝓐 𝓜]

/--
Instance `gmodule` / 实例 `gmodule`

English:
instance gmodule
  signature: : DirectSum.Gmodule (fun i => 𝓐 i) fun i => 𝓜 i
  body: { SetLike.gdistribMulAction 𝓐 𝓜 with
    smul := fun x y => ⟨(x : A) • (y : M), SetLike.GradedSMul.smul_mem x.2 y.2⟩
add_smul := fun _a _a' _b => Subtype.ext add_smul _ _ _
zero_smul := fun _b => Subtype.ext zero_smul _ _ }

中文:
实例 gmodule
  签名: : DirectSum.Gmodule (fun i => 𝓐 i) fun i => 𝓜 i
  定义体: { SetLike.gdistribMulAction 𝓐 𝓜 with
    smul := fun x y => ⟨(x : A) • (y : M), SetLike.GradedSMul.smul_mem x.2 y.2⟩
add_smul := fun _a _a' _b => Subtype.ext add_smul _ _ _
zero_smul := fun _b => Subtype.ext zero_smul _ _ }

Depends on / 依赖: GradedSMul, SetLike, SetLike.GradedSMul.smul_mem, SetLike.gdistribMulAction, Subtype, Subtype.ext, add_smul, gdistribMulAction, smul_mem, zero_smul
-/
instance gmodule : DirectSum.Gmodule (fun i => 𝓐 i) fun i => 𝓜 i :=
  { SetLike.gdistribMulAction 𝓐 𝓜 with
    smul := fun x y => ⟨(x : A) • (y : M), SetLike.GradedSMul.smul_mem x.2 y.2⟩
add_smul := fun _a _a' _b => Subtype.ext add_smul _ _ _
zero_smul := fun _b => Subtype.ext zero_smul _ _ }

end SetLike

namespace GradedModule

variable [AddCommMonoid M] [Module A M] [SetLike σ M] [AddSubmonoidClass σ' A]
  [AddSubmonoidClass σ M] [SetLike.GradedSMul 𝓐 𝓜]
  [DecidableEq ιA] [DecidableEq ιM] [GradedRing 𝓐]

/-- The smul multiplication of `A` on `⨁ i, 𝓜 i` from `(⨁ i, 𝓐 i) →+ (⨁ i, 𝓜 i) →+ ⨁ i, 𝓜 i`
turns `⨁ i, 𝓜 i` into an `A`-module
-/
@[instance_reducible]
/--
Definition of `isModule` / `isModule` 的定义

English:
definition isModule
  signature: : Module A (⨁ i, 𝓜 i)
  body: { Module.compHom _ (DirectSum.decomposeRingEquiv 𝓐 : A ≃+* ⨁ i, 𝓐 i).toRingHom with
    smul := fun a b => DirectSum.decompose 𝓐 a • b }

中文:
定义 isModule
  签名: : Module A (⨁ i, 𝓜 i)
  定义体: { Module.compHom _ (DirectSum.decomposeRingEquiv 𝓐 : A ≃+* ⨁ i, 𝓐 i).toRingHom with
    smul := fun a b => DirectSum.decompose 𝓐 a • b }

Depends on / 依赖: DirectSum, DirectSum.decompose, DirectSum.decomposeRingEquiv, Module, Module.compHom, compHom, decompose, decomposeRingEquiv, toRingHom
-/
def isModule : Module A (⨁ i, 𝓜 i) :=
  { Module.compHom _ (DirectSum.decomposeRingEquiv 𝓐 : A ≃+* ⨁ i, 𝓐 i).toRingHom with
    smul := fun a b => DirectSum.decompose 𝓐 a • b }

/--
Definition of `linearEquiv` / `linearEquiv` 的定义

English:
definition linearEquiv
  signature: [DirectSum.Decomposition 𝓜]
  body: by
  letI h := isModule 𝓐 𝓜
  refine ⟨⟨(DirectSum.decomposeAddEquiv 𝓜).toAddHom, ?_⟩,
    (DirectSum.decomposeAddEquiv 𝓜).symm.toFun, (DirectSum.decomposeAddEquiv 𝓜).left_inv,
    (DirectSum.decomposeAddEquiv 𝓜).right_inv⟩
  intro x y
  classical
  rw [AddHom.toFun_eq_coe]; rw [← DirectSum.sum_suppo

中文:
定义 linearEquiv
  签名: [DirectSum.Decomposition 𝓜]
  定义体: by
  letI h := isModule 𝓐 𝓜
  refine ⟨⟨(DirectSum.decomposeAddEquiv 𝓜).toAddHom, ?_⟩,
    (DirectSum.decomposeAddEquiv 𝓜).symm.toFun, (DirectSum.decomposeAddEquiv 𝓜).left_inv,
    (DirectSum.decomposeAddEquiv 𝓜).right_inv⟩
  intro x y
  classical
  rw [AddHom.toFun_eq_coe]; rw [← DirectSum.sum_suppo

Depends on / 依赖: AddEquiv, AddEquiv.coe_toAddHom, AddHom, AddHom.toFun_eq_coe, DirectSum, DirectSum.decomposeAddEquiv, DirectSum.sum_support_decompose, Finset, Finset.sum_congr, Finset.sum_smul, RingHom, RingHom.id_apply, classical, coe_toAddHom, decomposeAddEquiv, id_apply, infer_instance, isModule, left_inv, map_sum
-/
def linearEquiv [DirectSum.Decomposition 𝓜] :
    @LinearEquiv A A _ _ (RingHom.id A) (RingHom.id A) _ _ M (⨁ i, 𝓜 i) _
    _ _ (by letI := isModule 𝓐 𝓜; infer_instance) := by
  letI h := isModule 𝓐 𝓜
  refine ⟨⟨(DirectSum.decomposeAddEquiv 𝓜).toAddHom, ?_⟩,
    (DirectSum.decomposeAddEquiv 𝓜).symm.toFun, (DirectSum.decomposeAddEquiv 𝓜).left_inv,
    (DirectSum.decomposeAddEquiv 𝓜).right_inv⟩
  intro x y
  classical
  rw [AddHom.toFun_eq_coe]; rw [← DirectSum.sum_support_decompose 𝓐 x]; rw [map_sum]; rw [Finset.sum_smul]; rw [AddEquiv.coe_toAddHom]; rw [map_sum]; rw [Finset.sum_smul]
  refine Finset.sum_congr rfl (fun i _hi => ?_)
  rw [RingHom.id_apply]; rw [← DirectSum.sum_support_decompose 𝓜 y]; rw [map_sum]; rw [Finset.smul_sum]; rw [map_sum]; rw [Finset.smul_sum]
  refine Finset.sum_congr rfl (fun j _hj => ?_)
  rw [show (decompose 𝓐 x i : A) • (decomposeAddEquiv 𝓜 ↑(decompose 𝓜 y j) : (⨁ i]; rw [𝓜 i)) =
    DirectSum.Gmodule.smulAddMonoidHom _ _ (decompose 𝓐 ↑(decompose 𝓐 x i))
    (decomposeAddEquiv 𝓜 ↑(decompose 𝓜 y j)) from DirectSum.Gmodule.smul_def _ _ _ _]
  simp only [decomposeAddEquiv_apply, decompose_coe, Gmodule.smulAddMonoidHom_apply_of_of]
  convert! DirectSum.decompose_coe 𝓜 _
  rfl

end GradedModule
