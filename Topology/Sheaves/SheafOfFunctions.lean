...
/--
theorem `toTypes_isSheaf` / 定理 `toTypes_isSheaf`

English:
theorem toTypes_isSheaf
  given: (T : X -> Type*)
  statement: (presheafToTypes X T).IsSheaf
  proof: isSheaf_of_isSheafUniqueGluing_types _ fun ι U sf hsf => by
...
/--
theorem `toType_isSheaf` / 定理 `toType_isSheaf`

English:
theorem toType_isSheaf
  given: (T : Type*)
  statement: (presheafToType X T).IsSheaf
  proof: toTypes_isSheaf X fun _ => T
...
/--
Definition of `sheafToTypes` / `sheafToTypes` 的定义

English:
definition sheafToTypes
  signature: (T : X -> Type*)
  body: ⟨presheafToTypes X T, Presheaf.toTypes_isSheaf _ _⟩

中文:
定义 sheafToTypes
  签名: (T : X -> 类型)
  定义体: ⟨presheafToTypes X T, Presheaf.toTypes_isSheaf _ _⟩

Depends on / 依赖: Presheaf, Presheaf.toTypes_isSheaf, presheafToTypes, toTypes_isSheaf
-/
def sheafToTypes (T : X -> Type*) : Sheaf (Type _) X :=
  ⟨presheafToTypes X T, Presheaf.toTypes_isSheaf _ _⟩

/--
Definition of `sheafToType` / `sheafToType` 的定义

English:
definition sheafToType
  signature: (T : Type*)
  body: ⟨presheafToType X T, Presheaf.toType_isSheaf _ _⟩

中文:
定义 sheafToType
  签名: (T : 类型)
  定义体: ⟨presheafToType X T, Presheaf.toType_isSheaf _ _⟩

Depends on / 依赖: Presheaf, Presheaf.toType_isSheaf, presheafToType, toType_isSheaf
-/
