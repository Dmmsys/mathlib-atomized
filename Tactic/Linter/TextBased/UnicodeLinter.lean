...
/-
**Mathlib.Linter.TextBased.UnicodeLinter.ASCII.allowedWhitespace** 是 Mathlib 中的一
个定义，位于命名空间 `Mathlib.Linter.TextBased.UnicodeLinter`。
...
/-
**Mathlib.Linter.TextBased.UnicodeLinter.ASCII.printable** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Linter.TextBased.UnicodeLinter`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Linter.TextBased.UnicodeLinter.ASCII.allowed** 是 Mathlib 中的一个定义，位于命名空间
...
-/
/-
**Mathlib.Linter.TextBased.UnicodeLinter.withVSCodeAbbrev** 是 Mathlib 中的一个定义，位于命
名空间 `Mathlib.Linter.TextBased.UnicodeLinter`。
形式化陈述：withVSCodeAbbrev : Array Char
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
Symbols with VSCode extension abbreviation, as of March 17, 2026.
Obtained using `scripts/extract-unique-nonascii.lean` from
https://github.com/leanprover/vscode-lean4/blob/132d329aa8afa3e8508ef77cfdcff112
d3b35c88
-/
...
-/
/-
**Mathlib.Linter.TextBased.UnicodeLinter.othersInMathlib** 是 Mathlib 中的一个定义，位于命名
空间 `Mathlib.Linter.TextBased.UnicodeLinter`。
形式化陈述：othersInMathlib : Array Char
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Other unicode characters present in Mathlib but not present in any of the above 
lists
(as of March 17, 2026).

This list may be extended by more characters for which no VSCode-extension abbre
viation exists.
Some guidelines:
- No invisible characters
- No characters affecting text directionality (Mathlib uses left-to-right text).
...
The list `withVSCodeAbbrev` contains characters with abbreviations; if an abbrev
iation
already has been defined, you might need to update the list `withVSCodeAbbrev`.
Further, consider proposing an abbreviation to [the extension](https://github.co
m/leanprover/vscode-lean4) for new symbols.

Empty lines for improved readability (otherwise some characters overlap in some 
fonts).
-/
