...
/--
theorem `Setoid.IsPartition.ncard_eq_finsum` / 定理 `Setoid.IsPartition.ncard_eq_finsum`

English:
theorem Setoid.IsPartition.ncard_eq_finsum
  statement: {α : Type*} {P : Set (Set α)}
  proof: by
  classical
  have hst (t : Set α) : (s inter t).Finite := hs.inter_of_left t
  have hst' (t : Set α) : Nat.card ↑(s inter t) = (hst t).toFinset.card :=
...
theorem Setoid.IsPartition.ncard_eq_finsum {α : Type*} {P : Set (Set α)}
    ...
    s.ncard = finsum fun t : P => (s inter t).ncard := by
  classical
  have hst (t : Set α) : (s inter t).Finite := hs.inter_of_left t
  have hst' (t : Set α) : Nat.card ↑(s inter t) = (hst t).toFinset.card :=
    Nat.card_eq_card_finite_toFinset (hst t)
