...
/--
Definition of `bracketedBinderType` / `bracketedBinderType` 的定义

English:
definition bracketedBinderType
  signature: : Syntax -> Option Term
...
/--
Definition of `pendingActionableSynthMVar` / `pendingActionableSynthMVar` 的定义

English:
definition pendingActionableSynthMVar
  signature: (binder : TSyntax ``bracketedBinder)
  body: do
...
中文:
定义 pendingActionableSynthMVar
  签名: (binder : TSyntax ``bracketedBinder)
  定义体: do
  let pendingMVars := (← get).pendingMVars
  if pendingMVars.isEmpty then
    return none
...
/--
Definition of `getSubproblem` / `getSubproblem` 的定义

English:
definition getSubproblem
  body: do
  let res : Term.TermElabResult (Option (MessageData × TSyntax ``bracketedBinder)) ←
    ...
    Term.withAutoBoundImplicit do
      _ ← Term.elabType ty
      Term.synthesizeSyntheticMVars (postpone 
...
/--
Definition of `completeBinders'` / `completeBinders'` 的定义

English:
definition completeBinders'
  signature: (maxSteps : Nat) (gas : Nat)
  body: do
...
English:
definition completeBinders
  signature: (maxSteps : Nat) (checkRedundant : Bool)
  body: completeBinders' maxSteps maxSteps checkRedundant binders #[] 0
...
/--
Definition of `cleanBinders` / `cleanBinders` 的定义

English:
definition cleanBinders
  signature: (binders : TSyntaxArray ``bracketedBinder)
  body: Id.run do
  let mut binders' := #[]
...
中文:
定义 cleanBinders
  签名: (binders : TSyntaxArray ``bracketedBinder)
  定义体: Id.run do
  let mut binders' := #[]
  for binder in binders do
binders' := binders'.push ⟨binder.raw.unsetTrailing⟩
...
/--
Definition of `ignorevariable?` / `ignorevariable?` 的定义

English:
definition ignorevariable?
  signature: : Linter.IgnoreFunction
  body: fun _ stack _ =>
