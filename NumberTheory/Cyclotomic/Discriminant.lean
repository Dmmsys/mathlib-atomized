/-
Copyright (c) 2022 Riccardo Brasca. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Riccardo Brasca
-/
module

public import Mathlib.NumberTheory.Cyclotomic.PrimitiveRoots
public import Mathlib.RingTheory.DedekindDomain.Dvr
public import Mathlib.NumberTheory.NumberField.Discriminant.Defs

/-!
# Discriminant of cyclotomic fields

We compute the discriminant of a `p ^ n`-th cyclotomic extension.

## Main results
* `IsCyclotomicExtension.discr_odd_prime` : if `p` is an odd prime such that
  `IsCyclotomicExtension {p} K L` and `Irreducible (cyclotomic p K)`, then
  `discr K (hζ.powerBasis K).basis = (-1) ^ ((p - 1) / 2) * p ^ (p - 2)` for any
  `hζ : IsPrimitiveRoot ζ p`.

-/

public section


universe u v

open Algebra Polynomial Nat IsPrimitiveRoot PowerBasis

open scoped Polynomial Cyclotomic

namespace IsPrimitiveRoot

variable {n : Nat} [NeZero n] {K : Type u} [Field K] [CharZero K] {ζ : K}
variable [ce : IsCyclotomicExtension {n} Rat K]

/--
theorem `discr_zeta_eq_discr_zeta_sub_one` / 定理 `discr_zeta_eq_discr_zeta_sub_one`

English:
theorem discr_zeta_eq_discr_zeta_sub_one
  given: (hζ : IsPrimitiveRoot ζ n)
  proof: by
  have : NumberField K := @NumberField.mk _ _ _ (IsCyclotomicExtension.finiteDimensional {n} Rat K)
  have H₁ : (aeval (hζ.powerBasis Rat).gen) (X - 1 : Int[X]) = (hζ.subOnePowerBasis Rat).gen := by simp
  have H₂ : (aeval (hζ.subOnePowerBasis Rat).gen) (X + 1 : Int[X]) = (hζ.powerBasis Rat).gen 

中文:
定理 discr_zeta_eq_discr_zeta_sub_one
  条件: (hζ : IsPrimitiveRoot ζ n)
  证明: by
  have : NumberField K := @NumberField.mk _ _ _ (IsCyclotomicExtension.finiteDimensional {n} Rat K)
  have H₁ : (aeval (hζ.powerBasis Rat).gen) (X - 1 : Int[X]) = (hζ.subOnePowerBasis Rat).gen := by simp
  have H₂ : (aeval (hζ.subOnePowerBasis Rat).gen) (X + 1 : Int[X]) = (hζ.powerBasis Rat).gen 

Depends on / 依赖: IsCyclotomicExtension, IsCyclotomicExtension.finiteDimensional, NeZero, NeZero.pos, NumberField, NumberField.mk, discr_eq_discr_of_toMatrix_coeff_isIntegral, finiteDimensional, isIntegral, minpol, powerBasis, subOnePowerBasis, toMatrix_isIntegral
-/
theorem discr_zeta_eq_discr_zeta_sub_one (hζ : IsPrimitiveRoot ζ n) :
    discr Rat (hζ.powerBasis Rat).basis = discr Rat (hζ.subOnePowerBasis Rat).basis := by
  have : NumberField K := @NumberField.mk _ _ _ (IsCyclotomicExtension.finiteDimensional {n} Rat K)
  have H₁ : (aeval (hζ.powerBasis Rat).gen) (X - 1 : Int[X]) = (hζ.subOnePowerBasis Rat).gen := by simp
  have H₂ : (aeval (hζ.subOnePowerBasis Rat).gen) (X + 1 : Int[X]) = (hζ.powerBasis Rat).gen := by simp
  refine discr_eq_discr_of_toMatrix_coeff_isIntegral _ (fun i j => toMatrix_isIntegral H₁ ?_ ?_ _ _)
    fun i j => toMatrix_isIntegral H₂ ?_ ?_ _ _
  · exact hζ.isIntegral (NeZero.pos _)
  · refine minpoly.isIntegrallyClosed_eq_field_fractions' (K := Rat) (hζ.isIntegral (NeZero.pos _))
  · exact (hζ.isIntegral (NeZero.pos _)).sub isIntegral_one
  · refine minpoly.isIntegrallyClosed_eq_field_fractions' (K := Rat) ?_
    exact (hζ.isIntegral (NeZero.pos _)).sub isIntegral_one

end IsPrimitiveRoot

namespace IsCyclotomicExtension

variable {p : Nat} {k : Nat} {K : Type u} {L : Type v} {ζ : L} [Field K] [Field L]
variable [Algebra K L]

/--
theorem `discr_prime_pow_ne_two` / 定理 `discr_prime_pow_ne_two`

English:
theorem discr_prime_pow_ne_two
  statement: [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
  proof: by
  have hne := IsCyclotomicExtension.neZero' (p ^ (k + 1)) K L
  have mf : Module.Finite K L := finiteDimensional {p ^ (k + 1)} K L
  have se : Algebra.IsSeparable K L := isSeparable {p ^ (k + 1)} K L
  rw [discr_powerBasis_eq_norm]; rw [finrank L hirr]; rw [hζ.powerBasis_gen _]; rw [← hζ.minpoly_

中文:
定理 discr_prime_pow_ne_two
  结论: [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
  证明: by
  have hne := IsCyclotomicExtension.neZero' (p ^ (k + 1)) K L
  have mf : Module.Finite K L := finiteDimensional {p ^ (k + 1)} K L
  have se : Algebra.IsSeparable K L := isSeparable {p ^ (k + 1)} K L
  rw [discr_powerBasis_eq_norm]; rw [finrank L hirr]; rw [hζ.powerBasis_gen _]; rw [← hζ.minpoly_

Depends on / 依赖: Algebra, Algebra.IsSeparable, Finite, IsCyclotomicExtension, IsCyclotomicExtension.neZero, IsSeparable, Module, Module.Finite, Nat.add_one_sub_one, absurd, add_one_sub_one, discr_powerBasis_eq_norm, eq_or_ne, finiteDimensional, finrank, hp.out, isSeparable, minpoly_eq_cyclotomic_of_irreducible, neZero, powerBasis_gen
-/
theorem discr_prime_pow_ne_two [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
    (hζ : IsPrimitiveRoot ζ (p ^ (k + 1))) (hirr : Irreducible (cyclotomic (p ^ (k + 1)) K))
    (hk : p ^ (k + 1) != 2) : discr K (hζ.powerBasis K).basis =
      (-1) ^ ((p ^ (k + 1)).totient / 2) * p ^ (p ^ k * ((p - 1) * (k + 1) - 1)) := by
  have hne := IsCyclotomicExtension.neZero' (p ^ (k + 1)) K L
  have mf : Module.Finite K L := finiteDimensional {p ^ (k + 1)} K L
  have se : Algebra.IsSeparable K L := isSeparable {p ^ (k + 1)} K L
  rw [discr_powerBasis_eq_norm]; rw [finrank L hirr]; rw [hζ.powerBasis_gen _]; rw [← hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rw [totient_prime_pow hp.out (succ_pos k)]; rw [Nat.add_one_sub_one]
  have hp2 : p = 2 -> k != 0 := by
    rintro rfl rfl
    exact absurd rfl hk
  congr 1
  · rcases eq_or_ne p 2 with (rfl | hp2)
    · rcases Nat.exists_eq_succ_of_ne_zero (hp2 rfl) with ⟨k, rfl⟩
      rw [succ_sub_succ_eq_sub]; rw [tsub_zero]; rw [mul_one]; simp only [_root_.pow_succ']
      rw [mul_assoc]; rw [Nat.mul_div_cancel_left _ zero_lt_two]; rw [Nat.mul_div_cancel_left _ zero_lt_two]
      cases k
      · simp
      · simp_rw [_root_.pow_succ', (even_two.mul_right _).neg_one_pow,
          ((even_two.mul_right _).mul_right _).neg_one_pow]
    · have hpo : Odd p := hp.out.odd_of_ne_two hp2
      obtain ⟨a, ha⟩ := (hp.out.even_sub_one hp2).two_dvd
      rw [ha]; rw [mul_left_comm]; rw [mul_assoc]; rw [Nat.mul_div_cancel_left _ two_pos]; rw [Nat.mul_div_cancel_left _ two_pos]; rw [mul_right_comm]; rw [pow_mul]; rw [(hpo.pow.mul _).neg_one_pow]; rw [pow_mul]; rw [hpo.pow.neg_one_pow]
      refine Nat.Even.sub_odd ?_ (even_two_mul _) odd_one
      rw [mul_left_comm]; rw [← ha]
      exact one_le_mul (one_le_pow _ _ hp.1.pos) (succ_le_iff.2 <| tsub_pos_of_lt hp.1.one_lt)
  · have H := congr_arg (@derivative K _) (cyclotomic_prime_pow_mul_X_pow_sub_one K p k)
    rw [derivative_mul]; rw [derivative_sub]; rw [derivative_one]; rw [sub_zero]; rw [derivative_X_pow]; rw [C_eq_natCast]; rw [derivative_sub]; rw [derivative_one]; rw [sub_zero]; rw [derivative_X_pow]; rw [C_eq_natCast]; rw [hζ.minpoly_eq_cyclotomic_of_irreducible hirr] at H
    replace H := congr_arg (fun P => aeval ζ P) H
    simp only [aeval_add, aeval_mul, minpoly.aeval, zero_mul, add_zero, aeval_natCast,
      map_sub, aeval_one, aeval_X_pow] at H
    replace H := congr_arg (Algebra.norm K) H
    have hnorm : (norm K) (ζ ^ p ^ k - 1) = (p : K) ^ p ^ k := by
      by_cases hp : p = 2
      · exact mod_cast hζ.norm_pow_sub_one_eq_prime_pow_of_ne_zero hirr le_rfl (hp2 hp)
      · exact mod_cast hζ.norm_pow_sub_one_of_prime_ne_two hirr le_rfl hp
    rw [map_mul]; rw [hnorm]; rw [map_mul]; rw [← map_natCast (algebraMap K L)]; rw [Algebra.norm_algebraMap]; rw [finrank L hirr]; rw [← succ_eq_add_one]; rw [totient_prime_pow hp.out (succ_pos k)]; rw [Nat.sub_one]; rw [Nat.pred_succ] at H
    rw [← hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rw [map_pow]; rw [hζ.norm_eq_one hk hirr]; rw [one_pow]; rw [mul_one]; rw [cast_pow]; rw [← pow_mul]; rw [← mul_assoc]; rw [mul_comm (k + 1)]; rw [mul_assoc] at H
    have := mul_pos (succ_pos k) (tsub_pos_of_lt hp.out.one_lt)
    rw [← succ_pred_eq_of_pos this]; rw [mul_succ]; rw [pow_add _ _ (p ^ k)] at H
    replace H := (mul_left_inj' fun h => ?_).1 H
    · simp only [H, mul_comm _ (k + 1)]; norm_cast
    · have := hne.1
      rw [Nat.cast_pow]; rw [Ne]; rw [pow_eq_zero_iff (by lia)] at this
      exact absurd (eq_zero_of_pow_eq_zero h) this

/--
theorem `discr_prime_pow_ne_two'` / 定理 `discr_prime_pow_ne_two'`

English:
theorem discr_prime_pow_ne_two'
  statement: [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
  proof: by
  simpa [totient_prime_pow hp.out (succ_pos k)] using discr_prime_pow_ne_two hζ hirr hk

中文:
定理 discr_prime_pow_ne_two'
  结论: [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
  证明: by
  simpa [totient_prime_pow hp.out (succ_pos k)] using discr_prime_pow_ne_two hζ hirr hk

Depends on / 依赖: discr_prime_pow_ne_two, hp.out, succ_pos, totient_prime_pow
-/
theorem discr_prime_pow_ne_two' [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact p.Prime]
    (hζ : IsPrimitiveRoot ζ (p ^ (k + 1))) (hirr : Irreducible (cyclotomic (p ^ (k + 1)) K))
    (hk : p ^ (k + 1) != 2) : discr K (hζ.powerBasis K).basis =
      (-1) ^ (p ^ k * (p - 1) / 2) * p ^ (p ^ k * ((p - 1) * (k + 1) - 1)) := by
  simpa [totient_prime_pow hp.out (succ_pos k)] using discr_prime_pow_ne_two hζ hirr hk

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
theorem `discr_prime_pow` / 定理 `discr_prime_pow`

English:
theorem discr_prime_pow
  statement: [hcycl : IsCyclotomicExtension {p ^ k} K L] [hp : Fact p.Prime]
  proof: by
  rcases k with - | k
  · simp only [coe_basis, _root_.pow_zero, powerBasis_gen _ hζ, totient_one, mul_zero,
      show 1 / 2 = 0 by rfl, discr, traceMatrix]
    have hζone : ζ = 1 := by simpa using hζ
    rw [hζ.powerBasis_dim _]; rw [hζone]; rw [← (algebraMap K L).map_one]; rw [minpoly.eq_X_sub

中文:
定理 discr_prime_pow
  结论: [hcycl : IsCyclotomicExtension {p ^ k} K L] [hp : Fact p.Prime]
  证明: by
  rcases k with - | k
  · simp only [coe_basis, _root_.pow_zero, powerBasis_gen _ hζ, totient_one, mul_zero,
      show 1 / 2 = 0 by rfl, discr, traceMatrix]
    have hζone : ζ = 1 := by simpa using hζ
    rw [hζ.powerBasis_dim _]; rw [hζone]; rw [← (algebraMap K L).map_one]; rw [minpoly.eq_X_sub

Depends on / 依赖: Matrix, Matrix.det_unique, _root_, _root_.pow_zero, algebraMap, coe_basis, det_unique, eq_X_sub_C_of_algebraMap_inj, finrank, injective, map_one, minpoly, minpoly.eq_X_sub_C_of_algebraMap_inj, mul_one, mul_zero, natDegree_X_sub_C, one_pow, pow_zero, powerBasis_dim, powerBasis_gen
-/
theorem discr_prime_pow [hcycl : IsCyclotomicExtension {p ^ k} K L] [hp : Fact p.Prime]
    (hζ : IsPrimitiveRoot ζ (p ^ k)) (hirr : Irreducible (cyclotomic (p ^ k) K)) :
    discr K (hζ.powerBasis K).basis =
      (-1) ^ ((p ^ k).totient / 2) * p ^ (p ^ (k - 1) * ((p - 1) * k - 1)) := by
  rcases k with - | k
  · simp only [coe_basis, _root_.pow_zero, powerBasis_gen _ hζ, totient_one, mul_zero,
      show 1 / 2 = 0 by rfl, discr, traceMatrix]
    have hζone : ζ = 1 := by simpa using hζ
    rw [hζ.powerBasis_dim _]; rw [hζone]; rw [← (algebraMap K L).map_one]; rw [minpoly.eq_X_sub_C_of_algebraMap_inj _ (algebraMap K L).injective]; rw [natDegree_X_sub_C]
    simp only [map_one, one_pow, Matrix.det_unique, traceForm_apply, mul_one]
    rw [← (algebraMap K L).map_one]; rw [trace_algebraMap]; rw [finrank _ hirr]
    simp
  · by_cases hk : p ^ (k + 1) = 2
    · obtain rfl : p = 2 := by
        rw [← pow_one 2] at hk
        exact eq_of_prime_pow_eq (prime_iff.1 hp.out) (prime_iff.1 Nat.prime_two) (succ_pos _) hk
      nth_rw 2 [← pow_one 2] at hk
      replace hk := Nat.pow_right_injective rfl.le hk
      rw [add_eq_right] at hk
      subst hk
      rw [pow_one] at hζ hcycl
      have : natDegree (minpoly K ζ) = 1 := by
        rw [hζ.eq_neg_one_of_two_right]; rw [show (-1 : L) = algebraMap K L (-1) by simp]; rw [minpoly.eq_X_sub_C_of_algebraMap_inj _ (FaithfulSMul.algebraMap_injective K L)]
        exact natDegree_X_sub_C (-1)
      rcases Fin.equiv_iff_eq.2 this with ⟨e⟩
      rw [← Algebra.discr_reindex K (hζ.powerBasis K).basis e]; rw [coe_basis]; rw [powerBasis_gen]
      simp only [powerBasis_dim,
        zero_add, pow_one, totient_two, reduceDiv, pow_zero, cast_ofNat, tsub_self,
        Nat.add_one_sub_one, mul_one, mul_zero]
      simp_rw [hζ.eq_neg_one_of_two_right, show (-1 : L) = algebraMap K L (-1) by simp]
      convert_to (discr K fun i : Fin 1 => (algebraMap K L) (-1) ^ ↑i) = _
      · congr 1
        ext i
        simp only [map_neg, map_one, Function.comp_apply, Fin.val_eq_zero, _root_.pow_zero]
        suffices (e.symm i : Nat) = 0 by simp [this]
        rw [← Nat.lt_one_iff]
        convert! (e.symm i).2
        rw [this]
      · simp only [discr, traceMatrix_apply, Matrix.det_unique, Fin.default_eq_zero, Fin.val_zero,
          _root_.pow_zero, traceForm_apply, mul_one]
        rw [← (algebraMap K L).map_one]; rw [trace_algebraMap]; rw [finrank _ hirr]; simp
    · exact discr_prime_pow_ne_two hζ hirr hk

/--
theorem `discr_prime_pow_eq_unit_mul_pow` / 定理 `discr_prime_pow_eq_unit_mul_pow`

English:
theorem discr_prime_pow_eq_unit_mul_pow
  statement: [IsCyclotomicExtension {p ^ k} K L]
  proof: by
  rw [discr_prime_pow hζ hirr]
  by_cases heven : Even ((p ^ k).totient / 2)
  · exact ⟨1, p ^ (k - 1) * ((p - 1) * k - 1), by rw [heven.neg_one_pow]; simp⟩
  · exact ⟨-1, p ^ (k - 1) * ((p - 1) * k - 1), by
      rw [(not_even_iff_odd.1 heven).neg_one_pow]; simp⟩

中文:
定理 discr_prime_pow_eq_unit_mul_pow
  结论: [IsCyclotomicExtension {p ^ k} K L]
  证明: by
  rw [discr_prime_pow hζ hirr]
  by_cases heven : Even ((p ^ k).totient / 2)
  · exact ⟨1, p ^ (k - 1) * ((p - 1) * k - 1), by rw [heven.neg_one_pow]; simp⟩
  · exact ⟨-1, p ^ (k - 1) * ((p - 1) * k - 1), by
      rw [(not_even_iff_odd.1 heven).neg_one_pow]; simp⟩

Depends on / 依赖: discr_prime_pow, heven.neg_one_pow, neg_one_pow, not_even_iff_odd, totient
-/
theorem discr_prime_pow_eq_unit_mul_pow [IsCyclotomicExtension {p ^ k} K L]
    [hp : Fact p.Prime] (hζ : IsPrimitiveRoot ζ (p ^ k))
    (hirr : Irreducible (cyclotomic (p ^ k) K)) :
    exists (u : Intˣ) (n : Nat), discr K (hζ.powerBasis K).basis = u * p ^ n := by
  rw [discr_prime_pow hζ hirr]
  by_cases heven : Even ((p ^ k).totient / 2)
  · exact ⟨1, p ^ (k - 1) * ((p - 1) * k - 1), by rw [heven.neg_one_pow]; simp⟩
  · exact ⟨-1, p ^ (k - 1) * ((p - 1) * k - 1), by
      rw [(not_even_iff_odd.1 heven).neg_one_pow]; simp⟩

/--
theorem `discr_odd_prime` / 定理 `discr_odd_prime`

English:
theorem discr_odd_prime
  statement: [IsCyclotomicExtension {p} K L] [hp : Fact p.Prime]
  proof: by
  have : IsCyclotomicExtension {p ^ (0 + 1)} K L := by
    rw [zero_add]; rw [pow_one]
    infer_instance
  have hζ' : IsPrimitiveRoot ζ (p ^ (0 + 1)) := by simpa using hζ
  convert! discr_prime_pow_ne_two hζ' (by simpa [hirr]) (by simp [hodd]) using 2
  · rw [zero_add, pow_one, totient_prime hp.

中文:
定理 discr_odd_prime
  结论: [IsCyclotomicExtension {p} K L] [hp : Fact p.Prime]
  证明: by
  have : IsCyclotomicExtension {p ^ (0 + 1)} K L := by
    rw [zero_add]; rw [pow_one]
    infer_instance
  have hζ' : IsPrimitiveRoot ζ (p ^ (0 + 1)) := by simpa using hζ
  convert! discr_prime_pow_ne_two hζ' (by simpa [hirr]) (by simp [hodd]) using 2
  · rw [zero_add, pow_one, totient_prime hp.

Depends on / 依赖: IsCyclotomicExtension, IsPrimitiveRoot, Nat.sub_sub, _root_, _root_.pow_zero, convert, discr_prime_pow_ne_two, hp.out, infer_instance, mul_one, one_mul, pow_one, pow_zero, sub_sub, totient_prime, zero_add
-/
theorem discr_odd_prime [IsCyclotomicExtension {p} K L] [hp : Fact p.Prime]
    (hζ : IsPrimitiveRoot ζ p) (hirr : Irreducible (cyclotomic p K)) (hodd : p != 2) :
    discr K (hζ.powerBasis K).basis = (-1) ^ ((p - 1) / 2) * p ^ (p - 2) := by
  have : IsCyclotomicExtension {p ^ (0 + 1)} K L := by
    rw [zero_add]; rw [pow_one]
    infer_instance
  have hζ' : IsPrimitiveRoot ζ (p ^ (0 + 1)) := by simpa using hζ
  convert! discr_prime_pow_ne_two hζ' (by simpa [hirr]) (by simp [hodd]) using 2
  · rw [zero_add, pow_one, totient_prime hp.out]
  · rw [_root_.pow_zero, one_mul, zero_add, mul_one, Nat.sub_sub]

end IsCyclotomicExtension
