...
* or `|s₁ + t₁| = |s₂ + t₂|` and `|s₁| + |t₁| = |s₂| + |t₂|` and `|s₁| < |s₂|`. -/]
/--
Definition of `DevosMulRel` / `DevosMulRel` 的定义

English:
definition DevosMulRel
  signature: : Finset α × Finset α -> Finset α × Finset α -> Prop
  body: Prod.Lex (· < ·) (Prod.Lex (· > ·) (· < ·)) on fun x => (#(x.1 * x.2), #x.1 + #x.2, #x.1)
...
@[to_additive]
/--
lemma `devosMulRel_iff` / 引理 `devosMulRel_iff`
...
@[to_additive]
/--
lemma `devosMulRel_of_le` / 引理 `devosMulRel_of_le`

English:
lemma devosMulRel_of_le
  statement: (mul : #(x.1 * x.2) <= #(y.1 * y.2))
  proof: devosMulRel_iff.2 mul.lt_or_eq.imp_right fun h => Or.inl ⟨h, hadd⟩

@[to_additive]

中文:
引理 devosMulRel_of_le
  结论: (mul : #(x.1 * x.2) <= #(y.1 * y.2))
  证明: devosMulRel_iff.2 mul.lt_or_eq.imp_right fun h => Or.inl ⟨h, hadd⟩
...
@[to_additive]
/--
lemma `devosMulRel_of_le_of_le` / 引理 `devosMulRel_of_le_of_le`

English:
lemma devosMulRel_of_le_of_le
  statement: (mul : #(x.1 * x.2) <= #(y.1 * y.2))
  proof: devosMulRel_iff.2
    mul.lt_or_eq.imp_right fun h => hadd.lt_or_eq'.imp (And.intro h) fun h' => ⟨h, h', hone⟩

@[to_additive]

中文:
引理 devosMulRel_of_le_of_le
  结论: (mul : #(x.1 * x.2) <= #(y.1 * y.2))
  证明: devosMulRel_iff.2
    mul.lt_or_eq.imp_right fun h => hadd.lt_or_eq'.imp (And.intro h) fun h' => ⟨h, h', hone⟩
...
@[to_additive]
/--
lemma `wellFoundedOn_devosMulRel` / 引理 `wellFoundedOn_devosMulRel`
...
/--
lemma `cauchy_davenport_minOrder_mul` / 引理 `cauchy_davenport_minOrder_mul`

English:
lemma cauchy_davenport_minOrder_mul
...
lemma cauchy_davenport_minOrder_mul (hs : s.Nonempty) (ht : t.Nonempty) :
  ...
  obtain ⟨g, hg, hgs⟩ : exists g : α, g != 1 ∧ (s inter op g • s).Nonempty :=
  ...
  · have hS : (zpowers g : Set α) subseteq a⁻¹ • (s : Set α) := by
refine forall_mem_zpowers.2 @zpow_induction_right _ _ _ (· in a⁻¹ • (s : Set α))
...
((Nat.card_mono s.finite_toSet.smul_set hS).trans_eq <| ?_).trans
  ...
  replace hsg : #(s inter op g • s) < #s := card_lt_card ⟨inter_subset_left, fun h =>
...
by `|s| + |t| - 1`. -/]
/--
lemma `cauchy_davenport_of_isMulTorsionFree` / 引理 `cauchy_davenport_of_isMulTorsionFree`

English:
lemma cauchy_davenport_of_isMulTorsionFree
  statement: [DecidableEq G] [Group G] [IsMulTorsionFree G]
  proof: by
...
Depends on / 依赖: Monoid, Monoid.minOrder_eq_top, Nat.cast_le, cast_le, cauchy_davenport_minOrder_mul, le_top, minOrder_eq_top, min_eq_right
-/
...
/--
lemma `ZMod.cauchy_davenport` / 引理 `ZMod.cauchy_davenport`

English:
lemma ZMod.cauchy_davenport
  statement: {p : Nat} (hp : p.Prime) {s t : Finset (ZMod p)} (hs : s.Nonempty)
  proof: by
...
Depends on / 依赖: Nat.cast_le, ZMod.minOrder_of_prime, cast_le, cauchy_davenport_minOrder_add, minOrder_of_prime, min_le_iff
-/
lemma ZMod.cauchy_davenport {p : Nat} (hp : p.Prime) {s t : Finset (ZMod p)} (hs : s.Nonempty)
...
/--
lemma `cauchy_davenport_mul_of_linearOrder_isCancelMul` / 引理 `cauchy_davenport_mul_of_linearOrder_isCancelMul`

English:
lemma cauchy_davenport_mul_of_linearOrder_isCancelMul
  statement: [LinearOrder α] [Mul α] [IsCancelMul α]
  proof: by
...
Depends on / 依赖: Nat.add_sub_cancel, _mem, add_sub_cancel, card_mono, card_mul_singleton, card_singleton, card_singleton_mul, card_union_add_card_inter, eq_singleton_iff_unique_mem, mem_inter, mem_sing, mul_mem_mul, mul_subset_mul_left, mul_subset_mul_right, s.max, singleton_subset_iff, t.min, union_subset
-/
