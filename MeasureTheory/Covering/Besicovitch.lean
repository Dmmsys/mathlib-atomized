...
-/
/-
**Besicovitch.SatelliteConfig** 是 Mathlib 中的一个归纳类型，位于命名空间 `Besicovitch`。
形式化陈述：(α : Type u_1) → [MetricSpace α] → ℕ → ℝ → Type u_1
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A satellite configuration is a configuration of `N+1` points that shows up in th
e inductive
construction for the Besicovitch covering theorem. It depends on some parameter 
`τ ≥ 1`.

This is a family of balls (indexed by `i : Fin N.succ`, with center `c i` and ra
dius `r i`) such
that the last ball intersects all the other balls (condition `inter`),
and given any two balls there is an order between them, ensuring that the first 
ball does not
contain the center of the other one, and the radius of the second ball cannot be
 larger than
the radius of the first ball (up to a factor `τ`). This order corresponds to the
 order of choice
in the inductive construction: otherwise, the second ball would have been chosen
 before.
This is the condition `h`.

Finally, the last ball is chosen after all the other ones, meaning that `h` can 
be strengthened
by keeping only one side of the alternative in `hlast`.
-/
...
/-
**HasBesicovitchCovering** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：(α : Type u_1) → [MetricSpace α] → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A metric space has the Besicovitch covering property if there exist `N` and `τ >
 1` such that
there are no satellite configurations of parameter `τ` with `N+1` points. This i
s the condition that
guarantees that the measurable Besicovitch covering theorem holds. It is satisfi
ed by
finite-dimensional real vector spaces.
-/
...
/-
**Besicovitch.SatelliteConfig.instInhabited** 是 Mathlib 中的一个实例，位于命名空间 ``。
形式化陈述：Besicovitch.SatelliteConfig.instInhabited {α : Type*} {τ : Real} [Inhabite
d α] [MetricSpace α] : Inhabited (Besicovitch.SatelliteConfig α 0 τ)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Besicovitch.SatelliteConfig.inter'** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitch.Sate
lliteConfig`。
形式化陈述：inter' (i : Fin N.succ) : dist (a.c i) (a.c (last N)) <= a.r i + a.r (last
 N)
参数：i : Fin N.succ。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `lt_or_ge`：∀ {α : Type u_1} [inst : LinearOrder α] (a b : α), a < b ∨ b ≤
 a
· 使用定理 `Besicovitch.SatelliteConfig.inter`：∀ {α : Type u_1} [inst : MetricSpace 
α] {N : ℕ} {τ : ℝ} (self : Besicovitch.SatelliteConfig α N τ),   ∀ i < Fin.last 
N, dist (self.c i) (sel…
...
/-
**Besicovitch.SatelliteConfig.hlast'** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitch.Sate
lliteConfig`。
形式化陈述：hlast' (i : Fin N.succ) (h : 1 <= τ) : a.r (last N) <= τ * a.r i
参数：i : Fin N.succ；h : 1 <= τ。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `lt_or_ge`：∀ {α : Type u_1} [inst : LinearOrder α] (a b : α), a < b ∨ b ≤
 a
· 使用定理 `And.right`：∀ {a b : Prop}, a ∧ b → b
· 使用定理 `Besicovitch.SatelliteConfig.hlast`：∀ {α : Type u_1} [inst : MetricSpace 
α] {N : ℕ} {τ : ℝ} (self : Besicovitch.SatelliteConfig α N τ),   ∀ i < Fin.last 
N, self.r i ≤ dist (sel…
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
· 使用定理 `Nat.instNeZeroSucc`：∀ {n : ℕ}, NeZero (n + 1)
...
/-
**Besicovitch.BallPackage** 是 Mathlib 中的一个归纳类型，位于命名空间 `Besicovitch`。
形式化陈述：Type u_1 → Type u_2 → Type (max u_1 u_2)
参数：max u_1 u_2。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A ball package is a family of balls in a metric space with positive bounded radi
i.
-/
...
/-
**Besicovitch.unitBallPackage** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch`。
形式化陈述：unitBallPackage (α : Type*) : BallPackage α α where c
参数：α : Type*。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
def unitBallPackage (α : Type*) : BallPackage α α where
  ...
  r_le _ := le_rfl
/-
**Besicovitch.BallPackage.instInhabited** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.B
allPackage`。
形式化陈述：(α : Type u_1) → Inhabited (Besicovitch.BallPackage α α)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
one variable. -/
/-
**Besicovitch.TauPackage** 是 Mathlib 中的一个归纳类型，位于命名空间 `Besicovitch`。
形式化陈述：Type u_1 → Type u_2 → Type (max u_1 u_2)
参数：max u_1 u_2。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A Besicovitch tau-package is a family of balls in a metric space with positive b
ounded radii,
together with enough data to proceed with the Besicovitch greedy algorithm. We r
egister this in
a single structure to make sure that all our constructions in this algorithm onl
y depend on
one variable.
-/
structure TauPackage (β : Type*) (α : Type*) extends BallPackage β α where
  ...
  one_lt_tau : 1 < τ
/-
**Besicovitch.TauPackage.instInhabited** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.Ta
uPackage`。
形式化陈述：(α : Type u_1) → Inhabited (Besicovitch.TauPackage α α)
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**Besicovitch.TauPackage.index** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.TauPackage
`。
形式化陈述：index : Ordinal.{u} -> β | i => -- `Z` is the set of points that are cover
ed by already constructed balls let Z
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Choose inductively large balls with centers that are not contained in the union 
of already
chosen balls. This is a transfinite induction.
-/
...
/-
**Besicovitch.TauPackage.iUnionUpTo** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.TauPa
ckage`。
形式化陈述：iUnionUpTo (i : Ordinal.{u}) : Set α
参数：i : Ordinal.{u}。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Besicovitch.TauPackage.monotone_iUnionUpTo** 是 Mathlib 中的一个定理，位于命名空间 `Besicovi
tch.TauPackage`。
形式化陈述：monotone_iUnionUpTo : Monotone p.iUnionUpTo
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Besicovitch.TauPackage.R** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.TauPackage`。
形式化陈述：R (i : Ordinal.{u}) : Real
参数：i : Ordinal.{u}。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Besicovitch.TauPackage.color** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.TauPackage
`。
形式化陈述：color : Ordinal.{u} -> Nat | i => let A : Set Nat
该定义给出了一等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Group the balls into disjoint families, by assigning to a ball the smallest colo
r for which
it does not intersect any already chosen ball of this color.
-/
...
/-
**Besicovitch.TauPackage.lastStep** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch.TauPack
age`。
形式化陈述：lastStep : Ordinal.{u}
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`p.lastStep` is the first ordinal where the construction stops making sense, i.e
., `f` returns
garbage since there is no point left to be chosen. We will only use ordinals bef
ore this step.
-/
def lastStep : Ordinal.{u} :=
  sInf {i | ¬∃ b : β, p.c b ∉ p.iUnionUpTo i ∧ p.R i ≤ p.τ * p.r b}
/-
**Besicovitch.TauPackage.lastStep_nonempty** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitc
h.TauPackage`。
形式化陈述：lastStep_nonempty : {i | ¬exists b : β, p.c b ∉ p.iUnionUpTo i ∧ p.R i <= 
p.τ * p.r b}.Nonempty
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Besicovitch.TauPackage.index.eq_1`：∀ {α : Type u_1} [inst : MetricSpace 
α] {β : Type u} [inst_1 : Nonempty β] (p : Besicovitch.TauPackage β α)   (x : Or
dinal.{u}),   p.index x…
...
· 使用定理 `LT.lt.trans_le`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a < b 
→ b ≤ c → a < c
· 使用定理 `Besicovitch.BallPackage.rpos`：∀ {β : Type u_1} {α : Type u_2} (self : Be
sicovitch.BallPackage β α) (b : β), 0 < self.r b
...
/-
**Besicovitch.TauPackage.mem_iUnionUpTo_lastStep** 是 Mathlib 中的一个定理，位于命名空间 `Besi
covitch.TauPackage`。
形式化陈述：mem_iUnionUpTo_lastStep (x : β) : p.c x in p.iUnionUpTo p.lastStep
参数：x : β。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `csInf_mem`：csInf_mem (hs : s.Nonempty) : sInf s in s
· 使用定理 `Besicovitch.TauPackage.lastStep_nonempty`：lastStep_nonempty : {i | ¬exis
ts b : β, p.c b ∉ p.iUnionUpTo i ∧ p.R i <= p.τ * p.r b}.Nonempty
· 使用定理 `congrFun'`：∀ {α : Sort u} {β : Sort v} {f g : α → β}, f = g → ∀ (a : α),
 f a = g a
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
...
· 使用定理 `Besicovitch.TauPackage.one_lt_tau`：∀ {β : Type u_1} {α : Type u_2} (self
 : Besicovitch.TauPackage β α), 1 < self.τ
· 使用定理 `Besicovitch.BallPackage.rpos`：∀ {β : Type u_1} {α : Type u_2} (self : Be
sicovitch.BallPackage β α) (b : β), 0 < self.r b
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
...
/-
**Besicovitch.TauPackage.color_lt** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitch.TauPack
age`。
形式化陈述：color_lt {i : Ordinal.{u}} (hi : i < p.lastStep) {N : Nat} (hN : IsEmpty (
SatelliteConfig α N p.τ)) : p.color i < N
参数：hi : i < p.lastStep；hN : IsEmpty (SatelliteConfig α N p.τ)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `Besicovitch.TauPackage.color.eq_1`：∀ {α : Type u_1} [inst : MetricSpace 
α] {β : Type u} [inst_1 : Nonempty β] (p : Besicovitch.TauPackage β α)   (x : Or
dinal.{u}),   p.color x…
· 使用定理 `Eq.trans`：∀ {α : Sort u} {a b c : α}, a = b → b = c → a = c
· 使用定理 `congr`：∀ {α : Sort u} {β : Sort v} {f₁ f₂ : α → β} {a₁ a₂ : α}, f₁ = f₂ 
...
b → a = b ∨ a < b
· 使用定理 `Besicovitch.TauPackage.color.congr_simp`：∀ {α : Type u_1} [inst : Metric
Space α] {β : Type u} [inst_1 : Nonempty β] (p p_1 : Besicovitch.TauPackage β α)
,   p = p_1 → ∀ (a a_1 : Ordi…
...
--- 原说明 ---
If there are no configurations of satellites with `N+1` points, one never uses m
ore than `N`
distinct families in the Besicovitch inductive construction.
-/
...
/-
**Besicovitch.exist_disjoint_covering_families** 是 Mathlib 中的一个定理，位于命名空间 `Besico
vitch`。
形式化陈述：exist_disjoint_covering_families {N : Nat} {τ : Real} (hτ : 1 < τ) (hN : I
sEmpty (SatelliteConfig α N τ)) (q : BallPackage β α) : exists s : Fin N -> Set 
β, (forall i : Fin N, (s i).PairwiseDisjoint fun j => closedBall (q.c j) (q.r j)
) ∧ range q.c subseteq ⋃ i : Fin N, ⋃ j in s i, ball (q.c j) (q.r j)
参数：hτ : 1 < τ；hN : IsEmpty (SatelliteConfig α N τ)；q : BallPackage β α。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `true_and`：∀ (p : Prop), (True ∧ p) = p
· 使用定理 `LT.lt.ne'`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, b < a → a ≠ b
· 使用定理 `Besicovitch.TauPackage.color_lt`：color_lt {i : Ordinal.{u}} (hi : i < p.
lastStep) {N : Nat} (hN : IsEmpty (SatelliteConfig α N p.τ)) : p.color i < N
· 使用定理 `LT.lt.trans`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a < b → b
 < c → a < c
...
--- 原说明 ---
The topological Besicovitch covering theorem: there exist finitely many families
 of disjoint
...
-/
/-
**Besicovitch.exist_finset_disjoint_balls_large_measure** 是 Mathlib 中的一个定理，位于命名空
间 `Besicovitch`。
形式化陈述：exist_finset_disjoint_balls_large_measure (μ : Measure α) [IsFiniteMeasure
 μ] {N : Nat} {τ : Real} (hτ : 1 < τ) (hN : IsEmpty (SatelliteConfig α N τ)) (s 
: Set α) (r : α -> Real) (rpos : forall x in s, 0 < r x) (rle : forall x in s, r
 x <= 1) : exists t : Finset α, ↑t subseteq s ∧ μ (s \ ⋃ x in t, closedBall x (r
 x)) <= N / (N + 1) * μ s ∧ (t : Set α).PairwiseDisjoint fun x => closedBall x (
r x)
参数：μ : Measure α；hτ : 1 < τ；hN : IsEmpty (SatelliteConfig α N τ)；s : Set α；r : α
 -> Real；rpos : forall x in s, 0 < r x；rle : forall x in s, r x <= 1。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
--- 原说明 ---
Consider, for each `x` in a set `s`, a radius `r x ∈ (0, 1]`. Then one can find 
finitely
many disjoint balls of the form `closedBall x (r x)` covering a proportion `1/(N
+1)` of `s`, if
there are no satellite configurations with `N+1` points.
-/
...
-/
/-
**Besicovitch.exists_disjoint_closedBall_covering_ae_of_finiteMeasure_aux** 是 Ma
thlib 中的一个定理，位于命名空间 `Besicovitch`。
形式化陈述：exists_disjoint_closedBall_covering_ae_of_finiteMeasure_aux (μ : Measure α
) [IsFiniteMeasure μ] (f : α -> Set Real) (s : Set α) (hf : forall x in s, foral
l δ > 0, (f x inter Ioo 0 δ).Nonempty) : exists t : Set (α × Real), t.Countable 
∧ (forall p in t, p.1 in s) ∧ (forall p in t, p.2 in f p.1) ∧ μ (s \ ⋃ (p : α × 
Real) (_ : p in t), closedBall p.1 p.2) = 0 ∧ t.PairwiseDisjoint fun p => closed
Ball p.1 p.2
参数：μ : Measure α；f : α -> Set Real；s : Set α；hf : forall x in s, forall δ > 0, (
f x inter Ioo 0 δ).Nonempty。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `HasBesicovitchCovering.no_satelliteConfig`：∀ {α : Type u_1} {inst : Metr
icSpace α} [self : HasBesicovitchCovering α],   ∃ N τ, 1 < τ ∧ IsEmpty (Besicovi
tch.SatelliteConfig α N τ)
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用引理 `isClosed_biUnion_finset`：isClosed_biUnion_finset {s : Finset α} {f : α -
> Set X} (h : forall i in s, IsClosed (f i)) : IsClosed (⋃ i in s, f i)
...
· 使用定理 `Metric.disjoint_closedBall_of_lt_infDist`：disjoint_closedBall_of_lt_infD
ist {r : Real} (h : r < infDist x s) : Disjoint (closedBall x r) s
· 使用引理 `min_le_left`：min_le_left (a b : α) : min a b <= a
· 使用定理 `Besicovitch.exist_finset_disjoint_balls_large_measure`：exist_finset_disj
oint_balls_large_measure (μ : Measure α) [IsFiniteMeasure μ] {N : Nat} {τ : Real
} (hτ : 1 < τ) (hN : IsEmpty (SatelliteConf…
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
...
--- 原说明 ---
The **measurable Besicovitch covering theorem**. Assume that, for any `x` in a s
et `s`,
one is given a set of admissible closed balls centered at `x`, with arbitrarily 
small radii.
Then there exists a disjoint covering of almost all `s` by admissible closed bal
ls centered at some
points of `s`.
This version requires that the underlying measure is finite, and that the space 
has the Besicovitch
covering property (which is satisfied for instance by finite-dimensional normed 
real vector spaces).
...
For a version assuming that the measure is s-finite,
see `exists_disjoint_closedBall_covering_ae_aux`.
For a version giving the conclusion in a nicer form, see `exists_disjoint_closed
Ball_covering_ae`.
-/
...
-/
/-
**Besicovitch.exists_disjoint_closedBall_covering_ae_aux** 是 Mathlib 中的一个定理，位于命名
空间 `Besicovitch`。
形式化陈述：exists_disjoint_closedBall_covering_ae_aux (μ : Measure α) [SFinite μ] (f 
: α -> Set Real) (s : Set α) (hf : forall x in s, forall δ > 0, (f x inter Ioo 0
 δ).Nonempty) : exists t : Set (α × Real), t.Countable ∧ (forall p in t, p.1 in 
s) ∧ (forall p in t, p.2 in f p.1) ∧ μ (s \ ⋃ (p : α × Real) (_ : p in t), close
dBall p.1 p.2) = 0 ∧ t.PairwiseDisjoint fun p => closedBall p.1 p.2
参数：μ : Measure α；f : α -> Set Real；s : Set α；hf : forall x in s, forall δ > 0, (
f x inter Ioo 0 δ).Nonempty。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
re ν ∧ μ ≪ ν ∧ ν ≪ μ
· 使用定理 `Besicovitch.exists_disjoint_closedBall_covering_ae_of_finiteMeasure_aux`
：exists_disjoint_closedBall_covering_ae_of_finiteMeasure_aux (μ : Measure α) [Is
FiniteMeasure μ] (f : α -> Set Real) (s : Set α) (hf : forall…

--- 原说明 ---
The measurable **Besicovitch covering theorem**.

Assume that, for any `x` in a set `s`, one is given a set of admissible closed b
alls centered at
`x`, with arbitrarily small radii. Then there exists a disjoint covering of almo
st all `s` by
admissible closed balls centered at some points of `s`.

This version requires the underlying measure to be s-finite, and the space to ha
ve the
Besicovitch covering property (which is satisfied for instance by finite-dimensi
onal
normed real vector spaces).
It expresses the conclusion in a slightly awkward form (with a subset of `α × ℝ`
) coming from the
...
-/
/-
**Besicovitch.exists_disjoint_closedBall_covering_ae** 是 Mathlib 中的一个定理，位于命名空间 `
Besicovitch`。
形式化陈述：exists_disjoint_closedBall_covering_ae (μ : Measure α) [SFinite μ] (f : α 
-> Set Real) (s : Set α) (hf : forall x in s, forall δ > 0, (f x inter Ioo 0 δ).
Nonempty) (R : α -> Real) (hR : forall x in s, 0 < R x) : exists (t : Set α) (r 
: α -> Real), t.Countable ∧ t subseteq s ∧ (forall x in t, r x in f x inter Ioo 
0 (R x)) ∧ μ (s \ ⋃ x in t, closedBall x (r x)) = 0 ∧ t.PairwiseDisjoint fun x =
> closedBall x (r x)
参数：μ : Measure α；f : α -> Set Real；s : Set α；hf : forall x in s, forall δ > 0, (
f x inter Ioo 0 δ).Nonempty；R : α -> Real；hR : forall x in s, 0 < R x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `lt_min`：lt_min (h₁ : a < b) (h₂ : a < c) : a < min b c
· 使用定理 `And.left`：∀ {a b : Prop}, a ∧ b → a
...
· 使用引理 `min_le_right`：min_le_right (a b : α) : min a b <= b
· 使用引理 `min_le_left`：min_le_left (a b : α) : min a b <= a
· 使用定理 `Besicovitch.exists_disjoint_closedBall_covering_ae_aux`：exists_disjoint_
closedBall_covering_ae_aux (μ : Measure α) [SFinite μ] (f : α -> Set Real) (s : 
Set α) (hf : forall x in s, forall δ > 0, (f…
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用引理 `Set.exists_eq_graphOn`：exists_eq_graphOn [Nonempty β] {s : Set (α × β)} 
...
`x`, with arbitrarily small radii. Then there exists a disjoint covering of almo
st all `s` by
admissible closed balls centered at some points of `s`. We can even require that
 the radius at `x`
is bounded by a given function `R x`. (Take `R = 1` if you don't need this addit
ional feature).

This version requires the underlying measure to be sigma-finite, and the space t
o have the
Besicovitch covering property (which is satisfied for instance by normed real ve
ctor spaces).
-/
...
/-
**Besicovitch.exists_closedBall_covering_tsum_measure_le** 是 Mathlib 中的一个定理，位于命名
空间 `Besicovitch`。
形式化陈述：exists_closedBall_covering_tsum_measure_le (μ : Measure α) [SFinite μ] [Me
asure.OuterRegular μ] {ε : Real>=0∞} (hε : ε != 0) (f : α -> Set Real) (s : Set 
α) (hf : forall x in s, forall δ > 0, (f x inter Ioo 0 δ).Nonempty) : exists (t 
: Set α) (r : α -> Real), t.Countable ∧ t subseteq s ∧ (forall x in t, r x in f 
x) ∧ (s subseteq ⋃ x in t, closedBall x (r x)) ∧ (∑' x : t, μ (closedBall x (r x
))) <= μ s + ε
参数：μ : Measure α；hε : ε != 0；f : α -> Set Real；s : Set α；hf : forall x in s, for
all δ > 0, (f x inter Ioo 0 δ).Nonempty。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
x
· 使用定理 `Besicovitch.exists_disjoint_closedBall_covering_ae`：exists_disjoint_clos
edBall_covering_ae (μ : Measure α) [SFinite μ] (f : α -> Set Real) (s : Set α) (
hf : forall x in s, forall δ > 0, (f x i…
· 使用定理 `And.left`：∀ {a b : Prop}, a ∧ b → a
...
elf
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `Besicovitch.exist_disjoint_covering_families`：exist_disjoint_covering_fa
milies {N : Nat} {τ : Real} (hτ : 1 < τ) (hN : IsEmpty (SatelliteConfig α N τ)) 
(q : BallPackage β α) : exists s :…
（共 82 条，此处仅展示前 30 条）

--- 原说明 ---
In a space with the Besicovitch property, any set `s` can be covered with balls 
whose measures
add up to at most `μ s + ε`, for any positive `ε`. This works even if one restri
cts the set of
allowed radii around a point `x` to a set `f x` which accumulates at `0`.
-/
...
/-
**Besicovitch.vitaliFamily** 是 Mathlib 中的一个定义，位于命名空间 `Besicovitch`。
形式化陈述：{α : Type u_1} →   [inst : MetricSpace α] →     [SecondCountableTopology α
] →       [inst_2 : MeasurableSpace α] →         [OpensMeasurableSpace α] →     
      [HasBesicovitchCovering α] → (μ : MeasureTheory.Measure α) → [MeasureTheor
y.SFinite μ] → VitaliFamily μ
参数：μ : MeasureTheory.Measure α。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
In a space with the Besicovitch covering property, the set of closed balls with 
positive radius
forms a Vitali family. This is essentially a restatement of the measurable Besic
ovitch theorem.
-/
...
versions. -/
/-
**Besicovitch.tendsto_filterAt** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitch`。
形式化陈述：tendsto_filterAt (μ : Measure α) [SFinite μ] (x : α) : Tendsto (fun r => c
losedBall x r) (𝓝[>] 0) ((Besicovitch.vitaliFamily μ).filterAt x)
参数：μ : Measure α；x : α。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.mp`：∀ {a b : Prop}, (a ↔ b) → a → b
...
--- 原说明 ---
The main feature of the Besicovitch Vitali family is that its filter at a point 
`x` corresponds
to convergence along closed balls. We record one of the two implications here, w
hich will enable us
to deduce specific statements on differentiation of measures in this context fro
m the general
versions.
-/
...
/-
**Besicovitch.ae_tendsto_rnDeriv** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitch`。
形式化陈述：ae_tendsto_rnDeriv (ρ μ : Measure β) [IsLocallyFiniteMeasure μ] [IsLocally
FiniteMeasure ρ] : forallᵐ x ∂μ, Tendsto (fun r => ρ (closedBall x r) / μ (close
dBall x r)) (𝓝[>] 0) (𝓝 (ρ.rnDeriv μ x))
参数：ρ μ : Measure β。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Filter.mp_mem`：mp_mem (hs : s in f) (h : { x | x in s -> x in t } in f) 
...
to g y z …
· 使用定理 `Besicovitch.tendsto_filterAt`：tendsto_filterAt (μ : Measure α) [SFinite 
μ] (x : α) : Tendsto (fun r => closedBall x r) (𝓝[>] 0) ((Besicovitch.vitaliFami
ly μ).filterAt x)

--- 原说明 ---
In a space with the Besicovitch covering property, the ratio of the measure of b
alls converges
almost surely to the Radon-Nikodym derivative.
-/
...
/-
**Besicovitch.ae_tendsto_measure_inter_div_of_measurableSet** 是 Mathlib 中的一个定理，位
于命名空间 `Besicovitch`。
形式化陈述：ae_tendsto_measure_inter_div_of_measurableSet (μ : Measure β) [IsLocallyFi
niteMeasure μ] {s : Set β} (hs : MeasurableSet s) : forallᵐ x ∂μ, Tendsto (fun r
 => μ (s inter closedBall x r) / μ (closedBall x r)) (𝓝[>] 0) (𝓝 (s.indicator 1 
x))
参数：μ : Measure β；hs : MeasurableSet s。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Filter.mp_mem`：mp_mem (hs : s in f) (h : { x | x in s -> x in t } in f) 
: t in f
...
--- 原说明 ---
Given a measurable set `s`, then `μ (s ∩ closedBall x r) / μ (closedBall x r)` c
onverges when
`r` tends to `0`, for almost every `x`. The limit is `1` for `x ∈ s` and `0` for
 `x ∉ s`.
This shows that almost every point of `s` is a Lebesgue density point for `s`.
A version for non-measurable sets holds, but it only gives the first conclusion,
see `ae_tendsto_measure_inter_div`.
...
/-
**Besicovitch.ae_tendsto_measure_inter_div** 是 Mathlib 中的一个定理，位于命名空间 `Besicovitc
h`。
形式化陈述：ae_tendsto_measure_inter_div (μ : Measure β) [IsLocallyFiniteMeasure μ] (s
 : Set β) : forallᵐ x ∂μ.restrict s, Tendsto (fun r => μ (s inter closedBall x r
) / μ (closedBall x r)) (𝓝[>] 0) (𝓝 1)
...
--- 原说明 ---
Given an arbitrary set `s`, then `μ (s ∩ closedBall x r) / μ (closedBall x r)` c
onverges
to `1` when `r` tends to `0`, for almost every `x` in `s`.
This shows that almost every point of `s` is a Lebesgue density point for `s`.
A stronger version holds for measurable sets, see `ae_tendsto_measure_inter_div_
of_measurableSet`.
