...
/--
theorem `mem_sieves_of_hasEffectiveEpi` / 定理 `mem_sieves_of_hasEffectiveEpi`

English:
theorem mem_sieves_of_hasEffectiveEpi
  given: (S : Sieve X)
  proof: by
  rintro ⟨Y, π, h⟩
...
theorem mem_sieves_of_hasEffectiveEpi (S : Sieve X) :
    (exists (Y : C) (π : Y ⟶ X), EffectiveEpi π ∧ S.arrows π) -> (S in (regularTopology C) X) := by
  rintro ⟨Y, π, h⟩
  have h_le : Sieve.generate (Presieve.ofArrows (fun () => Y) (fun _ => π)) <= S := by
    rw [Sieve.generate_le_iff (Presieve.ofArrows _ _) S]
    apply Presieve.le_of_factorsThru_sieve (Presieve.ofArrows _ _) S _
...
instance {Y Y' : C} (π : Y ⟶ X) [EffectiveEpi π]
  ...
  rw [effectiveEpi_iff_effectiveEpiFamily]; rw [← Sieve.effectiveEpimorphic_family]
  suffices h₂ : (Sieve.generate (Presieve.ofArrows _ _)) in (regularTopology C) X by
    change Nonempty _
...
/--
theorem `mem_sieves_iff_hasEffectiveEpi` / 定理 `mem_sieves_iff_hasEffectiveEpi`

English:
theorem mem_sieves_iff_hasEffectiveEpi
  given: (S : Sieve X)
  proof: by
  constructor
  · intro h
...
theorem mem_sieves_iff_hasEffectiveEpi (S : Sieve X) :
    (S in (regularTopology C) X) ↔
    exists (Y : C) (π : Y ⟶ X), EffectiveEpi π ∧ (S.arrows π) := by
  constructor
  · intro h
    induction h with
    | of Y T hS =>
