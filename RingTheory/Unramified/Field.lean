...
/--
theorem `of_isSeparable` / 定理 `of_isSeparable`

English:
theorem of_isSeparable
  given: [Algebra.IsSeparable K L]
  statement: FormallyUnramified K L
  proof: by
...
/--
theorem `bijective_of_isAlgClosed_of_isLocalRing` / 定理 `bijective_of_isAlgClosed_of_isLocalRing`
...
/--
theorem `isField_of_isAlgClosed_of_isLocalRing` / 定理 `isField_of_isAlgClosed_of_isLocalRing`

English:
theorem isField_of_isAlgClosed_of_isLocalRing
  proof: by
  rw [IsLocalRing.isField_iff_maximalIdeal_eq]; rw [eq_bot_iff]
  intro x hx
...
include K in
/--
theorem `isReduced_of_field` / 定理 `isReduced_of_field`
...
/--
theorem `isRadical_map_isMaximal` / 定理 `isRadical_map_isMaximal`

English:
theorem isRadical_map_isMaximal
...
Depends on / 依赖: Algebra, Algebra.FormallyUnramified.isReduced_of_field, FormallyUnramified, Ideal.Quotient.field, Ideal.isRadical_iff_quotient_reduced, Quotient, algebraMap, isRadical_iff_quotient_reduced, isReduced_of_field, p.map
-/
...
/--
theorem `range_eq_top_of_isPurelyInseparable` / 定理 `range_eq_top_of_isPurelyInseparable`

English:
theorem range_eq_top_of_isPurelyInseparable
  proof: by
  classical
  have : Nontrivial (L otimes[K] L) := by
    rw [← not_subsingleton_iff_nontrivial]; rw [← rank_zero_iff (R := K)]; rw [rank_tensorProduct']; rw [mul_eq_zero]; rw [or_self]; rw [rank_zero_iff]; rw [not_subsingleton_iff_nontrivial]
    infer_instance
  rw [← top_le_iff]
  intro x _
...
theorem iff_isSeparable
  given: (L : Type u) [Field L] [Algebra K L] [EssFiniteType K L]
  proof: ⟨fun _ => isSeparable K L, fun _ => of_isSeparable K L⟩
...
variable {K A} in
/--
theorem `Algebra.IsUnramifiedAt.not_minpoly_sq_dvd` / 定理 `Algebra.IsUnramifiedAt.not_minpoly_sq_dvd`
