...
/-
**Polynomial.X_pow_sub_X_sub_one_irreducible_aux** 是 Mathlib 中的一个定理，位于命名空间 `Poly
nomial`。
形式化陈述：X_pow_sub_X_sub_one_irreducible_aux (z : Complex) : ¬(z ^ n = z + 1 ∧ z ^ 
n + z ^ 2 = 0)
参数：z : Complex。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Polynomial.X_pow_sub_X_sub_one_irreducible** 是 Mathlib 中的一个定理，位于命名空间 `Polynomi
al`。
形式化陈述：X_pow_sub_X_sub_one_irreducible (hn1 : n != 1) : Irreducible (X ^ n - X - 
1 : Int[X])
参数：hn1 : n != 1。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**Polynomial.X_pow_sub_X_sub_one_irreducible_rat** 是 Mathlib 中的一个定理，位于命名空间 `Poly
nomial`。
形式化陈述：X_pow_sub_X_sub_one_irreducible_rat (hn1 : n != 1) : Irreducible (X ^ n - 
X - 1 : Rat[X])
参数：hn1 : n != 1。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
