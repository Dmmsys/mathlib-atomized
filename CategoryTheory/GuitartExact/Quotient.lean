...
/--
lemma `quotient_of_nonempty_leftHomotopy` / 引理 `quotient_of_nonempty_leftHomotopy`

English:
lemma quotient_of_nonempty_leftHomotopy
  statement: (e : T ⋙ R ≅ L ⋙ B)
  proof: by
...
lemma quotient_of_nonempty_leftHomotopy (e : T ⋙ R ≅ L ⋙ B)
    (he : forall ⦃X₀ : C₀⦄ ⦃Y : C⦄ (f₀ f₁ : L.obj X₀ ⟶ Y) (_ : B.map f₀ = B.map f₁),
      exists (P : Precylinder X₀), T.map P.i₀ = T.map P.i₁ ∧
...
/--
lemma `quotient_of_nonempty_rightHomotopy` / 引理 `quotient_of_nonempty_rightHomotopy`

English:
lemma quotient_of_nonempty_rightHomotopy
  statement: (e : T ⋙ R ≅ L ⋙ B)
  proof: by
...
中文:
引理 quotient_of_nonempty_rightHomotopy
  结论: (e : T ⋙ R ≅ L ⋙ B)
  证明: by
  rw [← guitartExact_op_iff]
  let e' : T.op ⋙ R.op ≅ L.op ⋙ B.op := NatIso.op e.symm
...
Depends on / 依赖: B.op, L.op, NatIso, NatIso.op, P.op, Quiver, Quiver.Hom.op_inj, Quiver.Hom.unop_inj, R.op, T.op, e.symm, guitartExact_op_iff, h.op, op_inj, quotient_of_nonempty_leftHomotopy, unop_inj
-/
lemma quotient_of_nonempty_rightHomotopy (e : T ⋙ R ≅ L ⋙ B)
    (he : forall ⦃X : C⦄ ⦃Y₀ : C₀⦄ (f₀ f₁ : X ⟶ L.obj Y₀) (_ : B.map f₀ = B.map f₁),
      exists (P : PrepathObject Y₀), T.map P.p₀ = T.map P.p₁ ∧
