/-
Copyright (c) 2024 Yury Kudryashov. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Yury Kudryashov
-/
module

public import Mathlib.MeasureTheory.Group.Action

/-!
# A.e. stabilizer of a set

In this file we define the a.e. stabilizer of a set under a measure-preserving group action.

The a.e. stabilizer `MulAction.aestabilizer G μ s` of a set `s`
is the set of the elements `g : G` such that `s` is a.e.-invariant under `(g • ·)`.

For a measure-preserving group action, this set is a subgroup of `G`.
If the set is null or conull, then this subgroup is the whole group.
The converse is true for an ergodic action and a null-measurable set.

## Implementation notes

We define the a.e. stabilizer as a bundled `Subgroup`,
thus we do not deal with monoid actions.

Also, many lemmas in this file are true for a *quasi-measure-preserving* action,
but we don't have the corresponding typeclass.
-/

@[expose] public section

open Filter Set MeasureTheory
open scoped Pointwise

variable (G : Type*) {α : Type*} [Group G] [MulAction G α]
  {_ : MeasurableSpace α} (μ : Measure α) [SMulInvariantMeasure G α μ]

namespace MulAction

set_option backward.isDefEq.respectTransparency false in
/-- A.e. stabilizer of a set under a group action. -/
@[to_additive (attr := simps) /-- A.e. stabilizer of a set under an additive group action. -/]
/--
Definition of `aestabilizer` / `aestabilizer` 的定义

English:
definition aestabilizer
  signature: (s : Set α)
  body: {g | g • s =ᵐ[μ] s}
  one_mem' := by simp
  -- TODO: `calc` would be more readable but fails because of defeq abuse
  mul_mem' {g₁ g₂} h₁ h₂ := by simpa only [smul_smul] using! ((smul_set_ae_eq g₁).2 h₂).trans h₁
  inv_mem' {g} h := by simpa using! (smul_set_ae_eq g⁻¹).2 h.out.symm

中文:
定义 aestabilizer
  签名: (s : Set α)
  定义体: {g | g • s =ᵐ[μ] s}
  one_mem' := by simp
  -- TODO: `calc` would be more readable but fails because of defeq abuse
  mul_mem' {g₁ g₂} h₁ h₂ := by simpa only [smul_smul] using! ((smul_set_ae_eq g₁).2 h₂).trans h₁
  inv_mem' {g} h := by simpa using! (smul_set_ae_eq g⁻¹).2 h.out.symm
-/
def aestabilizer (s : Set α) : Subgroup G where
  carrier := {g | g • s =ᵐ[μ] s}
  one_mem' := by simp
  -- TODO: `calc` would be more readable but fails because of defeq abuse
  mul_mem' {g₁ g₂} h₁ h₂ := by simpa only [smul_smul] using! ((smul_set_ae_eq g₁).2 h₂).trans h₁
  inv_mem' {g} h := by simpa using! (smul_set_ae_eq g⁻¹).2 h.out.symm

variable {G μ}
variable {g : G} {s t : Set α}

@[to_additive (attr := simp)]
/--
lemma `mem_aestabilizer` / 引理 `mem_aestabilizer`

English:
lemma mem_aestabilizer
  statement: g in aestabilizer G μ s ↔ g • s =ᵐ[μ] s
  proof: .rfl

中文:
引理 mem_aestabilizer
  结论: g in aestabilizer G μ s ↔ g • s =ᵐ[μ] s
  证明: .rfl
-/
lemma mem_aestabilizer : g in aestabilizer G μ s ↔ g • s =ᵐ[μ] s := .rfl

set_option backward.isDefEq.respectTransparency false in
@[to_additive]
/--
lemma `stabilizer_le_aestabilizer` / 引理 `stabilizer_le_aestabilizer`

English:
lemma stabilizer_le_aestabilizer
  given: (s : Set α)
  statement: stabilizer G s <= aestabilizer G μ s
  proof: by
  intro g hg
  simp_all

@[to_additive (attr := simp)]

中文:
引理 stabilizer_le_aestabilizer
  条件: (s : Set α)
  结论: stabilizer G s <= aestabilizer G μ s
  证明: by
  intro g hg
  simp_all

@[to_additive (attr := simp)]
-/
lemma stabilizer_le_aestabilizer (s : Set α) : stabilizer G s <= aestabilizer G μ s := by
  intro g hg
  simp_all

@[to_additive (attr := simp)]
/--
lemma `aestabilizer_empty` / 引理 `aestabilizer_empty`

English:
lemma aestabilizer_empty
  statement: aestabilizer G μ ∅ = ⊤
  proof: top_unique fun _ _ => by simp

@[to_additive (attr := simp)]

中文:
引理 aestabilizer_empty
  结论: aestabilizer G μ ∅ = ⊤
  证明: top_unique fun _ _ => by simp

@[to_additive (attr := simp)]

Depends on / 依赖: top_unique
-/
lemma aestabilizer_empty : aestabilizer G μ ∅ = ⊤ := top_unique fun _ _ => by simp

@[to_additive (attr := simp)]
/--
lemma `aestabilizer_univ` / 引理 `aestabilizer_univ`

English:
lemma aestabilizer_univ
  statement: aestabilizer G μ univ = ⊤
  proof: top_unique fun _ _ => by simp

中文:
引理 aestabilizer_univ
  结论: aestabilizer G μ univ = ⊤
  证明: top_unique fun _ _ => by simp

Depends on / 依赖: top_unique
-/
lemma aestabilizer_univ : aestabilizer G μ univ = ⊤ := top_unique fun _ _ => by simp

set_option backward.isDefEq.respectTransparency false in
@[to_additive]
/--
lemma `aestabilizer_congr` / 引理 `aestabilizer_congr`

English:
lemma aestabilizer_congr
  given: (h : s =ᵐ[μ] t)
  statement: aestabilizer G μ s = aestabilizer G μ t
  proof: by
  ext g
  rw [mem_aestabilizer]; rw [mem_aestabilizer]; rw [h.congr_right]; rw [((smul_set_ae_eq g).2 h).congr_left]

中文:
引理 aestabilizer_congr
  条件: (h : s =ᵐ[μ] t)
  结论: aestabilizer G μ s = aestabilizer G μ t
  证明: by
  ext g
  rw [mem_aestabilizer]; rw [mem_aestabilizer]; rw [h.congr_right]; rw [((smul_set_ae_eq g).2 h).congr_left]

Depends on / 依赖: congr_left, congr_right, h.congr_right, mem_aestabilizer, smul_set_ae_eq
-/
lemma aestabilizer_congr (h : s =ᵐ[μ] t) : aestabilizer G μ s = aestabilizer G μ t := by
  ext g
  rw [mem_aestabilizer]; rw [mem_aestabilizer]; rw [h.congr_right]; rw [((smul_set_ae_eq g).2 h).congr_left]

/--
lemma `aestabilizer_of_aeconst` / 引理 `aestabilizer_of_aeconst`

English:
lemma aestabilizer_of_aeconst
  given: (hs : EventuallyConst s (ae μ))
  statement: aestabilizer G μ s = ⊤
  proof: by
  refine top_unique fun g _ => ?_
  cases eventuallyConst_set'.mp hs with
  | inl h => simp [aestabilizer_congr h]
  | inr h => simp [aestabilizer_congr h]

中文:
引理 aestabilizer_of_aeconst
  条件: (hs : EventuallyConst s (ae μ))
  结论: aestabilizer G μ s = ⊤
  证明: by
  refine top_unique fun g _ => ?_
  cases eventuallyConst_set'.mp hs with
  | inl h => simp [aestabilizer_congr h]
  | inr h => simp [aestabilizer_congr h]

Depends on / 依赖: aestabilizer_congr, eventuallyConst_set, top_unique
-/
lemma aestabilizer_of_aeconst (hs : EventuallyConst s (ae μ)) : aestabilizer G μ s = ⊤ := by
  refine top_unique fun g _ => ?_
  cases eventuallyConst_set'.mp hs with
  | inl h => simp [aestabilizer_congr h]
  | inr h => simp [aestabilizer_congr h]

end MulAction

variable {G μ}
variable {x y : G} {s : Set α}

namespace MeasureTheory

@[to_additive]
/--
theorem `smul_ae_eq_self_of_mem_zpowers` / 定理 `smul_ae_eq_self_of_mem_zpowers`

English:
theorem smul_ae_eq_self_of_mem_zpowers
  statement: (hs : (x • s : Set α) =ᵐ[μ] s)
  proof: by
  rw [← MulAction.mem_aestabilizer]; rw [← Subgroup.zpowers_le] at hs
  exact hs hy

@[to_additive]

中文:
定理 smul_ae_eq_self_of_mem_zpowers
  结论: (hs : (x • s : Set α) =ᵐ[μ] s)
  证明: by
  rw [← MulAction.mem_aestabilizer]; rw [← Subgroup.zpowers_le] at hs
  exact hs hy

@[to_additive]

Depends on / 依赖: MulAction, MulAction.mem_aestabilizer, Subgroup, Subgroup.zpowers_le, mem_aestabilizer, zpowers_le
-/
theorem smul_ae_eq_self_of_mem_zpowers (hs : (x • s : Set α) =ᵐ[μ] s)
    (hy : y in Subgroup.zpowers x) : (y • s : Set α) =ᵐ[μ] s := by
  rw [← MulAction.mem_aestabilizer]; rw [← Subgroup.zpowers_le] at hs
  exact hs hy

@[to_additive]
/--
theorem `inv_smul_ae_eq_self` / 定理 `inv_smul_ae_eq_self`

English:
theorem inv_smul_ae_eq_self
  given: (hs : (x • s : Set α) =ᵐ[μ] s)
  statement: (x⁻¹ • s : Set α) =ᵐ[μ] s
  proof: inv_mem (s := MulAction.aestabilizer G μ s) hs

中文:
定理 inv_smul_ae_eq_self
  条件: (hs : (x • s : Set α) =ᵐ[μ] s)
  结论: (x⁻¹ • s : Set α) =ᵐ[μ] s
  证明: inv_mem (s := MulAction.aestabilizer G μ s) hs

Depends on / 依赖: MulAction, MulAction.aestabilizer, aestabilizer, inv_mem
-/
theorem inv_smul_ae_eq_self (hs : (x • s : Set α) =ᵐ[μ] s) : (x⁻¹ • s : Set α) =ᵐ[μ] s :=
  inv_mem (s := MulAction.aestabilizer G μ s) hs

end MeasureTheory
