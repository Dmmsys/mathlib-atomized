...
/--
theorem `exists_mem_stabilizer_isThreeCycle_of_two_lt_ncard` / 定理 `exists_mem_stabilizer_isThreeCycle_of_two_lt_ncard`
...
theorem exists_mem_stabilizer_isThreeCycle_of_two_lt_ncard
  ...
  rw [mem_stabilizer_set_iff_subset_smul_set s.toFinite]; rw [subset_smul_set_iff]
  rintro _ ⟨x, hx, rfl⟩
  aesop
...
Depends on / 依赖: isThreeCycle_swap_mul_swap_same, mem_stabilizer_set_iff_subset_smul_set, s.toFinite, subset_smul_set_iff, toFinite, two_lt_ncard_iff
-/
theorem exists_mem_stabilizer_isThreeCycle_of_two_lt_ncard
    {s : Set α} (hs : 2 < ncard s) :
    exists g in stabilizer (Perm α) s, g.IsThreeCycle := by
  rw [two_lt_ncard_iff] at hs
  obtain ⟨a, b, c, ha, hb, hc, hab, hac, hbc⟩ := hs
  refine ⟨swap a b * swap a c, ?_, isThreeCycle_swap_mul_swap_same hab hac hbc⟩
  rw [mem_stabilizer_set_iff_subset_smul_set s.toFinite]; rw [subset_smul_set_iff]
...
/--
theorem `exists_mem_stabilizer_isThreeCycle` / 定理 `exists_mem_stabilizer_isThreeCycle`
...
Depends on / 依赖: Nat.lt_or_ge, exists_mem_stabilizer_isThreeCycle_of_two_lt_ncard, lt_or_ge, ncard_add_ncard_compl, stabilizer_compl
-/
theorem exists_mem_stabilizer_isThreeCycle
...
/--
theorem `alternatingGroup_le_of_isPreprimitive` / 定理 `alternatingGroup_le_of_isPreprimitive`

English:
theorem alternatingGroup_le_of_isPreprimitive
  statement: (h4 : 4 < Nat.card α)
  proof: by
...
中文:
定理 alternatingGroup_le_of_isPreprimitive
  结论: (h4 : 4 < 自然数.card α)
  证明: by
  -- G contains a three_cycle
  obtain ⟨g, hg, hg3⟩ := exists_mem_stabilizer_isThreeCycle s h4
...
theorem stabilizer.surjective_toPerm
  given: {s : Set α} (hs : sᶜ.Nontrivial)
  proof: by
  classical
...
/--
theorem `stabilizer_isPreprimitive` / 定理 `stabilizer_isPreprimitive`

English:
theorem stabilizer_isPreprimitive
  given: {s : Set α} (hs : (sᶜ : Set α).Nontrivial)
  proof: isPreprimitive_stabilizer_of_surjective s (stabilizer.surjective_toPerm hs)
...
/--
theorem `stabilizer_subgroup_isPreprimitive` / 定理 `stabilizer_subgroup_isPreprimitive`

English:
theorem stabilizer_subgroup_isPreprimitive
  statement: {s : Set α} (hsc : sᶜ.Nontrivial)
  proof: have := stabilizer_isPreprimitive hsc
...
Depends on / 依赖: Function, Function.surjective_id, IsPreprimitive, IsPreprimitive.of_surjective, alternatingGroup, g.prop, map_smul, of_surjective, stabilizer, stabilizer_isPreprimitive, surjective_id
-/
...
/--
theorem `stabilizer_ne_top` / 定理 `stabilizer_ne_top`

English:
theorem stabilizer_ne_top
  given: {s : Set α} (hs : s.Nonempty) (hsc : sᶜ.Nontrivial)
  proof: by
  obtain ⟨a, ha⟩ := hs
  obtain ⟨b, hb, c, hc, hbc⟩ := hsc
  suffices exists g, g ∉ stabilizer (alternatingGroup α) s by contrapose! this; simp [this]
...
Depends on / 依赖: Equiv.swap, Perm.smul_def, Subgroup, Subgroup.mk_smul, alternatingGroup, contrapose, mem_stabilizer_set, mk_smul, mul_smul, simp_rw, smul_def, stabilizer
-/
...
theorem exists_mem_stabilizer_smul_eq (hα : 4 <= Nat.card α) {t : Set α} :
    forall a in t, forall b in t, exists g in stabilizer (alternatingGroup α) t, g • a = b := by
  intro a ha b hb
  by_cases hab : a = b
  · use 1
    simpa
...
/--
theorem `subgroup_eq_top_of_isPreprimitive` / 定理 `subgroup_eq_top_of_isPreprimitive`

English:
theorem subgroup_eq_top_of_isPreprimitive
  statement: (h4 : 4 < Nat.card α)
  proof: by
  obtain ⟨g, hg, hg3⟩ := exists_mem_stabilizer_isThreeCycle s h4
...
/--
theorem `subsingleton_of_ssubset_compl_of_stabilizer_alternatingGroup_le` / 定理 `subsingleton_of_ssubset_compl_of_stabilizer_alternatingGroup_le`
...
中文:
定理 subsingleton_of_ssubset_compl_of_stabilizer_alternatingGroup_le
  证明: by
  apply hB.subsingleton_of_ssubset_of_stabilizer_le hB_ss_sc
  intro g
  obtain ⟨⟨k, hk⟩, rfl⟩ := alternatingGroup.stabilizer.surjective_toPerm (by rwa [compl_compl]) g
...
Depends on / 依赖: alternatingGroup, alternatingGroup.stabilizer.surjective_toPerm, compl_compl, hB.subsingleton_of_ssubset_of_stabilizer_le, hB_ss_sc, stabilizer, stabilizer_compl, subsingleton_of_ssubset_of_stabilizer_le, surjective_toPerm
-/
theorem subsingleton_of_ssubset_compl_of_stabilizer_alternatingGroup_le
...
/--
theorem `isCoatom_stabilizer_singleton` / 定理 `isCoatom_stabilizer_singleton`

English:
theorem isCoatom_stabilizer_singleton
  statement: (h3 : 3 <= Nat.card α)
  proof: by
