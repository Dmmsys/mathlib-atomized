...
/-
**LieModule.exists_polynomial_eval_sub_aux** 是 Mathlib 中的一个引理，位于命名空间 `LieModule`
。
形式化陈述：exists_polynomial_eval_sub_aux {ι R K : Type*} [Finite ι] [CommRing R] [Fi
eld K] [Algebra R K] {E : Submodule R K} (a : ι -> K) (ha : forall i, a i in E) 
(f : E ->+ R) : exists r : K[X], forall i j, r.eval (a i - a j) = algebraMap R K
 (f ⟨a i, ha i⟩) - algebraMap R K (f ⟨a j, ha j⟩)
参数：a : ι -> K；ha : forall i, a i in E；f : E ->+ R。
该定理/引理给出了一组等式。
...
/-
**LieModule.isNilpotent_derivedSeries_of_traceForm_eq_zero_aux** 是 Mathlib 中的一个定
理，位于命名空间 `LieModule`。
形式化陈述：isNilpotent_derivedSeries_of_traceForm_eq_zero_aux {K : Type*} [Field K] [
CharZero K] [IsAlgClosed K] [LieAlgebra K L] [Module K M] [LieModule K L M] [Fin
iteDimensional K M] (h : traceForm K L M = 0) : IsNilpotent (derivedSeries K L 1
) M
参数：h : traceForm K L M = 0。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
An auxiliary lemma used to prove `LieModule.isNilpotent_derivedSeries_of_traceFo
rm_eq_zero`
which proves the same result except without the algebraically closed assumption.
-/
...
/-
**LieAlgebra.** 是 Mathlib 中的一个示例，位于命名空间 `LieAlgebra`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
