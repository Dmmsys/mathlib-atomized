...
@[simps]
/-
**HomologicalComplex.HomologySequence.mapSnakeInput** 是 Mathlib 中的一个定义，位于命名空间 `H
omologicalComplex.HomologySequence`。
形式化陈述：mapSnakeInput (i j : ι) (hij : c.Rel i j) : snakeInput hS₁ i j hij ⟶ snake
Input hS₂ i j hij where f₀
...
@[reassoc]
/-
**HomologicalComplex.HomologySequence.** 是 Mathlib 中的一个引理，位于命名空间 `HomologicalCom
...
--- 原说明 ---
The (exact) sequence
`H_i(S.X₁) ⟶ H_i(S.X₂) ⟶ H_i(S.X₃) ⟶ H_j(S.X₁) ⟶ H_j(S.X₂) ⟶ H_j(S.X₃)` when `c.
Rel i j`
and `S` is a short exact short complex of homological complexes in an abelian ca
tegory.
-/
...
@[simp]
/-
**HomologicalComplex.HomologySequence.mapComposableArrows** 是 Mathlib 中的一个定义，位于命
名空间 `HomologicalComplex.HomologySequence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The map between the exact sequences `S₁.X₁.homology i ⟶ S₁.X₂.homology i ⟶ S₁.X₃
.homology i`
and `S₂.X₁.homology i ⟶ S₂.X₂.homology i ⟶ S₂.X₃.homology i` that is induced by 
`φ : S₁ ⟶ S₂`.
-/
...
--- 原说明 ---
The map `composableArrows₅ hS₁ i j hij ⟶ composableArrows₅ hS₂ i j hij` of exact
sequences induced by a morphism `φ : S₁ ⟶ S₂` between short exact short complexe
s of
homological complexes.
-/
...
include hS₁ hS₂
/-
**HomologicalComplex.HomologySequence.mono_homologyMap_** 是 Mathlib 中的一个引理，位于命名空
间 `HomologicalComplex.HomologySequence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
set_option backward.defeqAttrib.useBackward true in
/-
**HomologicalComplex.HomologySequence.epi_homologyMap_** 是 Mathlib 中的一个引理，位于命名空间
 `HomologicalComplex.HomologySequence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**HomologicalComplex.HomologySequence.isIso_homologyMap_** 是 Mathlib 中的一个引理，位于命名
空间 `HomologicalComplex.HomologySequence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**HomologicalComplex.HomologySequence.quasiIso_** 是 Mathlib 中的一个引理，位于命名空间 `Homol
ogicalComplex.HomologySequence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.ShortComplex.ShortExact.exactAt_X** 是 Mathlib 中的一个引理，位于命名空间 `Ca
...
lemma exactAt_X₃ (hS : S.ShortExact) (i : ι)
  ...
  · have := hS.epi_g
    ...
    simp [← HomologicalComplex.homologyMap_comp]
/-
**CategoryTheory.ShortComplex.ShortExact.acyclic_X** 是 Mathlib 中的一个引理，位于命名空间 `Ca
tegoryTheory.ShortComplex.ShortExact`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
