...
/-
**CategoryTheory.MonoidalCategory.externalProductBifunctorCurried** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.MonoidalCategory`。
形式化陈述：externalProductBifunctorCurried : (J₁ ⥤ C) ⥤ (J₂ ⥤ C) ⥤ J₁ ⥤ J₂ ⥤ C
该定义给出了上述对象。
...
@[simps!, implicit_reducible]
/-
**CategoryTheory.MonoidalCategory.externalProductBifunctor** 是 Mathlib 中的一个定义，位于
命名空间 `CategoryTheory.MonoidalCategory`。
形式化陈述：externalProductBifunctor : ((J₁ ⥤ C) × (J₂ ⥤ C)) ⥤ J₁ × J₂ ⥤ C
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**CategoryTheory.MonoidalCategory.externalProduct** 是 Mathlib 中的一个缩写定义，位于命名空间 `C
ategoryTheory.MonoidalCategory`。
形式化陈述：externalProduct (F₁ : J₁ ⥤ C) (F₂ : J₂ ⥤ C)
参数：F₁ : J₁ ⥤ C；F₂ : J₂ ⥤ C。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.MonoidalCategory.externalProductCompDiagIso** 是 Mathlib 中的一个定义，
位于命名空间 `CategoryTheory.MonoidalCategory`。
形式化陈述：externalProductCompDiagIso : externalProductBifunctor J₁ J₁ C ⋙ (whiskerin
gLeft _ _ _ |>.obj <| Functor.diag J₁) ≅ tensor (J₁ ⥤ C)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
When both diagrams have the same source category, composing the external product
 with
the diagonal gives the pointwise functor tensor product.
Note that `(externalProductCompDiagIso _ _).app (F₁, F₂) : Functor.diag J₁ ⋙ F₁ 
⊠ F₂ ≅ F₁ ⊗ F₂`
type checks.
-/
...
@[simps!]
/-
**CategoryTheory.MonoidalCategory.externalProductSwap** 是 Mathlib 中的一个定义，位于命名空间 
`CategoryTheory.MonoidalCategory`。
形式化陈述：externalProductSwap [BraidedCategory C] : externalProductBifunctor J₁ J₂ C
 ⋙ (whiskeringLeft _ _ _ |>.obj <| Prod.swap _ _) ≅ Prod.swap _ _ ⋙ externalProd
uctBifunctor J₂ J₁ C
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
When `C` is braided, there is an isomorphism `Prod.swap _ _ ⋙ F₁ ⊠ F₂ ≅ F₂ ⊠ F₁`
, natural
in both `F₁` and `F₂`.
Note that `(externalProductSwap _ _ _).app (F₁, F₂) : Prod.swap _ _ ⋙ F₁ ⊠ F₂ ≅ 
F₂ ⊠ F₁`
type checks.
-/
...
@[simps!]
/-
**CategoryTheory.MonoidalCategory.externalProductFlip** 是 Mathlib 中的一个定义，位于命名空间 
`CategoryTheory.MonoidalCategory`。
形式化陈述：externalProductFlip [BraidedCategory C] : (Functor.postcompose₂.obj <| fli
pFunctor _ _ _).obj (externalProductBifunctorCurried J₁ J₂ C) ≅ (externalProduct
BifunctorCurried J₂ J₁ C).flip
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.MonoidalCategory.prodCompExternalProduct** 是 Mathlib 中的一个定义，位于命
名空间 `CategoryTheory.MonoidalCategory`。
形式化陈述：prodCompExternalProduct (F₁ : I₁ ⥤ J₁) (G₁ : J₁ ⥤ C) (F₂ : I₂ ⥤ J₂) (G₂ : 
J₂ ⥤ C) : F₁.prod F₂ ⋙ G₁ ⊠ G₂ ≅ (F₁ ⋙ G₁) ⊠ (F₂ ⋙ G₂)
参数：F₁ : I₁ ⥤ J₁；G₁ : J₁ ⥤ C；F₂ : I₂ ⥤ J₂；G₂ : J₂ ⥤ C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
