...
/--
theorem `Polynomial.Monic.irreducible_of_irreducible_map_of_isPrime_nilradical` / 定理 `Polynomial.Monic.irreducible_of_irreducible_map_of_isPrime_nilradical`
...
theorem Polynomial.Monic.irreducible_of_irreducible_map_of_isPrime_nilradical
  ...
  rw [show φ = ψ.comp ι from rfl]; rw [← map_map] at hi
.irreducible_of_irreducible_map _ _ hi replace hi := hm.map ι
refine ⟨fun h => hi.1 (mapRingHom ι).isUnit_map h, fun a b h => ?_⟩
  ...
  have hn (i : Nat) (hi : i != 0) : IsNilpotent (b.coeff i) := by
...
(x := a.coeff f.natDegree) (IsUnit.neg_iff _).1 ?_) hn
  ...
  rw [hm]; rw [coeff_mul]; rw [Finset.Nat.sum_antidiagonal_eq_sum_range_succ fun i j => a.coeff i * b.coeff j]; rw [Finset.sum_range_succ]; rw [← sub_eq_iff_eq_add]; rw [Nat.sub_self] at hc
  rw [← add_sub_cancel_left 1 (-(_ * _))]; rw [← sub_eq_add_neg]; rw [hc]
exact IsNilpotent.isUnit_sub_one show _ in nilradical R from sum_mem fun i hi =>
...
Depends on / 依赖: Ideal.Quotient.lift, IsUnit, Polynomial, Polynomial.map_mul, Quotient, RingHom, RingHom.ker, RingHom.ker_isPrime, algebraMap, b.map, generalizing, hm.map, irreducible_of_irreducible_map, isUnit_map, ker_isPrime, mapRingHom, map_map, map_mul, mul_comm, nilradical
-/
theorem Polynomial.Monic.irreducible_of_irreducible_map_of_isPrime_nilradical
