...
/-
**CategoryTheory.ObjectProperty.TensorLE** 是 Mathlib 中的一个归纳类型，位于命名空间 `CategoryTh
eory.ObjectProperty`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     [Category
Theory.MonoidalCategory C] →       CategoryTheory.ObjectProperty C → CategoryThe
ory.ObjectProperty C → CategoryTheory.ObjectProperty C → Prop
...
/-
**CategoryTheory.ObjectProperty.prop_tensor** 是 Mathlib 中的一个引理，位于命名空间 `CategoryT
heory.ObjectProperty`。
形式化陈述：prop_tensor {P₁ P₂ Q : ObjectProperty C} [TensorLE P₁ P₂ Q] {X₁ X₂ : C} (h
₁ : P₁ X₁) (h₂ : P₂ X₂) : Q (X₁ otimes X₂)
参数：h₁ : P₁ X₁；h₂ : P₂ X₂。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.ObjectProperty.TensorLE.prop_tensor`：∀ {C : Type u} {inst
 : CategoryTheory.Category.{v, u} C} {inst_1 : CategoryTheory.MonoidalCategory C
}   {P₁ P₂ Q : CategoryTheory.ObjectProp…
-/
...
/-
**CategoryTheory.ObjectProperty.ContainsUnit** 是 Mathlib 中的一个归纳类型，位于命名空间 `Catego
ryTheory.ObjectProperty`。
形式化陈述：{C : Type u} →   [inst : CategoryTheory.Category.{v, u} C] →     [Category
Theory.MonoidalCategory C] → CategoryTheory.ObjectProperty C → Prop
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
class ContainsUnit (P : ObjectProperty C) : Prop where
  prop_unit : P (𝟙_ C)
/-
**CategoryTheory.ObjectProperty.prop_unit** 是 Mathlib 中的一个引理，位于命名空间 `CategoryThe
ory.ObjectProperty`。
形式化陈述：prop_unit (P : ObjectProperty C) [ContainsUnit P] : P (𝟙_ C)
参数：P : ObjectProperty C。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.ObjectProperty.ContainsUnit.prop_unit`：∀ {C : Type u} {in
st : CategoryTheory.Category.{v, u} C} {inst_1 : CategoryTheory.MonoidalCategory
 C}   {P : CategoryTheory.ObjectProperty C…
-/
...
/-
**CategoryTheory.ObjectProperty.IsMonoidal** 是 Mathlib 中的一个归纳类型，位于命名空间 `Category
...
-/
/-
**CategoryTheory.ObjectProperty.IsMonoidalClosed** 是 Mathlib 中的一个类，位于命名空间 `Categ
oryTheory.ObjectProperty`。
形式化陈述：IsMonoidalClosed (P : ObjectProperty C) [MonoidalClosed C] : Prop where pr
op_ihom (X Y : C) : P X -> P Y -> P ((ihom X).obj Y)
参数：P : ObjectProperty C。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A property of objects is a monoidal closed if it is closed under taking internal
 homs
-/
class IsMonoidalClosed (P : ObjectProperty C) [MonoidalClosed C] : Prop where
  prop_ihom (X Y : C) : P X → P Y → P ((ihom X).obj Y) := by cat_disch
/-
**CategoryTheory.ObjectProperty.prop_ihom** 是 Mathlib 中的一个引理，位于命名空间 `CategoryThe
ory.ObjectProperty`。
形式化陈述：prop_ihom (P : ObjectProperty C) [MonoidalClosed C] [P.IsMonoidalClosed] {
X Y : C} (hX : P X) (hY : P Y) : P ((ihom X).obj Y)
参数：P : ObjectProperty C；hX : P X；hY : P Y。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `CategoryTheory.ObjectProperty.IsMonoidalClosed.prop_ihom`：∀ {C : Type u}
 {inst : CategoryTheory.Category.{v, u} C} {inst_1 : CategoryTheory.MonoidalCate
gory C}   {P : CategoryTheory.ObjectProperty C…
-/
...
-/
/-
**CategoryTheory.ObjectProperty.fullMonoidalSubcategory** 是 Mathlib 中的一个实例，位于命名空
间 `CategoryTheory.ObjectProperty`。
形式化陈述：fullMonoidalSubcategory : MonoidalCategory (FullSubcategory P)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
When `P : ObjectProperty C` is monoidal, the full subcategory for `P` inherits t
he
monoidal structure of `C`.
-/
...
-/
/-
**CategoryTheory.ObjectProperty.monoidal** 是 Mathlib 中的一个实例，位于命名空间 `CategoryTheo
ry.ObjectProperty`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The forgetful monoidal functor from a full monoidal subcategory into the origina
l category
("forgetting" the condition).
-/
...
-/
/-
**CategoryTheory.ObjectProperty.fullBraidedSubcategory** 是 Mathlib 中的一个实例，位于命名空间
 `CategoryTheory.ObjectProperty`。
形式化陈述：fullBraidedSubcategory : BraidedCategory (FullSubcategory P)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The braided structure on a full subcategory inherited by the braided structure o
n `C`.
-/
...
--- 原说明 ---
The forgetful braided functor from a full braided subcategory into the original 
category
("forgetting" the condition).
-/
...
/-
**CategoryTheory.ObjectProperty.fullSymmetricSubcategory** 是 Mathlib 中的一个实例，位于命名
空间 `CategoryTheory.ObjectProperty`。
形式化陈述：fullSymmetricSubcategory : SymmetricCategory P.FullSubcategory
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**CategoryTheory.ObjectProperty.fullMonoidalClosedSubcategory** 是 Mathlib 中的一个实例
，位于命名空间 `CategoryTheory.ObjectProperty`。
形式化陈述：fullMonoidalClosedSubcategory : MonoidalClosed (FullSubcategory P) where c
losed X
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.ObjectProperty.ihom_obj** 是 Mathlib 中的一个定理，位于命名空间 `CategoryTheo
ry.ObjectProperty`。
形式化陈述：ihom_obj (X Y : P.FullSubcategory) : ((ihom X).obj Y).obj = (ihom X.obj).o
bj Y.obj
参数：X Y : P.FullSubcategory。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**CategoryTheory.ObjectProperty.ihom_map_hom** 是 Mathlib 中的一个定理，位于命名空间 `Category
Theory.ObjectProperty`。
形式化陈述：ihom_map_hom (X : P.FullSubcategory) {Y Z : P.FullSubcategory} (f : Y ⟶ Z)
 : ((ihom X).map f).hom = (ihom X.obj).map f.hom
参数：X : P.FullSubcategory；f : Y ⟶ Z。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
