...
/-
**Mathlib.Tactic.applyFunHyp** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic`。
形式化陈述：applyFunHyp (f : Term) (using? : Option Term) (h : FVarId) (g : MVarId) : 
TacticM (List MVarId)
参数：f : Term；using? : Option Term；h : FVarId；g : MVarId。
该定义给出了上述对象。
...
/-
**Mathlib.Tactic.applyFunTargetFailure** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
`。
形式化陈述：applyFunTargetFailure (f : Term) : MetaM (List MVarId)
参数：f : Term。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.maybeProveInjective** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic`。
形式化陈述：maybeProveInjective (ginj : Expr) (using? : Option Expr) : MetaM Bool
参数：ginj : Expr；using? : Option Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.applyFunTarget** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic`。
形式化陈述：applyFunTarget (f : Term) (using? : Option Term) (g : MVarId) : TacticM (L
ist MVarId)
参数：f : Term；using? : Option Term；g : MVarId。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
