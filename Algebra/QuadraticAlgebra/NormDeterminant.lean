...
@[simp]
/-
**QuadraticAlgebra.det_toLinearMap_eq_norm** 是 Mathlib 中的一个定理，位于命名空间 `QuadraticA
lgebra`。
形式化陈述：det_toLinearMap_eq_norm (z : QuadraticAlgebra R a b) : (DistribSMul.toLine
arMap R (QuadraticAlgebra R a b) z).det = z.norm
...
--- 原说明 ---
The norm of an element in a quadratic algebra is the determinant of the endomorp
hism defined by
left multiplication by that element.
-/
