...
irreducible_def vitaliFamily (K : Real) : VitaliFamily μ := by
  ...
  have A : forall x : α, existsᶠ r in 𝓝[>] (0 : Real),
...
/--
theorem `closedBall_mem_vitaliFamily_of_dist_le_mul` / 定理 `closedBall_mem_vitaliFamily_of_dist_le_mul`

English:
theorem closedBall_mem_vitaliFamily_of_dist_le_mul
  statement: {K : Real} {x y : α} {r : Real} (h : dist x y <= K * r)
  proof: by
  let R := scalingScaleOf μ (max (4 * K + 3) 3)
...
定理 closedBall_mem_vitaliFamily_of_dist_le_mul
  结论: {K : 实数} {x y : α} {r : 实数} (h : dist x y <= K * r)
  证明: by
  let R := scalingScaleOf μ (max (4 * K + 3) 3)
  simp only [vitaliFamily, VitaliFamily.enlarge, Vitali.vitaliFamily, mem_union, mem_ofPred_eq,
    isClosed_closedBall, true_and, (nonempty_ball.2 rpos).mono ball_subset_interior_closedBall,
...
-/
theorem closedBall_mem_vitaliFamily_of_dist_le_mul {K : Real} {x y : α} {r : Real} (h : dist x y <= K * r)
    (rpos : 0 < r) : closedBall y r in (vitaliFamily μ K).setsAt x := by
  let R := scalingScaleOf μ (max (4 * K + 3) 3)
  simp only [vitaliFamily, VitaliFamily.enlarge, Vitali.vitaliFamily, mem_union, mem_ofPred_eq,
    isClosed_closedBall, true_and, (nonempty_ball.2 rpos).mono ball_subset_interior_closedBall,
    measurableSet_closedBall]
...
/--
theorem `tendsto_closedBall_filterAt` / 定理 `tendsto_closedBall_filterAt`

English:
theorem tendsto_closedBall_filterAt
  statement: {K : Real} {x : α} {ι : Type*} {l : Filter ι} (w : ι -> α)
  proof: by
...
-/
theorem tendsto_closedBall_filterAt {K : Real} {x : α} {ι : Type*} {l : Filter ι} (w : ι -> α)
    (δ : ι -> Real) (δlim : Tendsto δ l (𝓝[>] 0)) (xmem : forallᶠ j in l, x in closedBall (w j) (K * δ j)) :
...
theorem ae_tendsto_measure_inter_div
  given: (S : Set α) (K : Real)
  statement: forallᵐ x ∂μ.restrict S,
  proof: by
...
Depends on / 依赖: ae_tendsto_measure_inter_div, filter_upwards, hx.comp, tendsto_closedBall_filterAt, vitaliFamily
-/
theorem ae_tendsto_measure_inter_div (S : Set α) (K : Real) : forallᵐ x ∂μ.restrict S,
    forall {ι : Type*} {l : Filter ι} (w : ι -> α) (δ : ι -> Real) (_ : Tendsto δ l (𝓝[>] 0))
      (_ : forallᶠ j in l, x in closedBall (w j) (K * δ j)),
      Tendsto (fun j => μ (S inter closedBall (w j) (δ j)) / μ (closedBall (w j) (δ j))) l (𝓝 1) := by
  filter_upwards [(vitaliFamily μ K).ae_tendsto_measure_inter_div S] with x hx ι l w δ δlim
    xmem using hx.comp (tendsto_closedBall_filterAt μ _ _ δlim xmem)
...
theorem ae_tendsto_average_norm_sub
  given: {f : α -> E} (hf : LocallyIntegrable f μ) (K : Real)
  statement: forallᵐ x ∂μ,
  proof: by
...
定理 ae_tendsto_average_norm_sub
  条件: {f : α -> E} (hf : Locally整数egrable f μ) (K : 实数)
  结论: 对任意ᵐ x ∂μ,
  证明: by
  filter_upwards [(vitaliFamily μ K).ae_tendsto_average_norm_sub hf] with x hx ι l w δ δlim
    xmem using hx.comp (tendsto_closedBall_filterAt μ _ _ δlim xmem)

Depends on / 依赖: ae_tendsto_average_norm_sub, filter_upwards, hx.comp, tendsto_closedBall_filterAt, vitaliFamily
-/
theorem ae_tendsto_average_norm_sub {f : α -> E} (hf : LocallyIntegrable f μ) (K : Real) : forallᵐ x ∂μ,
    forall {ι : Type*} {l : Filter ι} (w : ι -> α) (δ : ι -> Real) (_ : Tendsto δ l (𝓝[>] 0))
      (_ : forallᶠ j in l, x in closedBall (w j) (K * δ j)),
...
theorem ae_tendsto_average
  statement: [NormedSpace Real E] [CompleteSpace E]
  proof: by
...
Depends on / 依赖: ae_tendsto_average, filter_upwards, hx.comp, tendsto_closedBall_filterAt, vitaliFamily
-/
theorem ae_tendsto_average [NormedSpace Real E] [CompleteSpace E]
    {f : α -> E} (hf : LocallyIntegrable f μ) (K : Real) : forallᵐ x ∂μ,
      forall {ι : Type*} {l : Filter ι} (w : ι -> α) (δ : ι -> Real) (_ : Tendsto δ l (𝓝[>] 0))
        (_ : forallᶠ j in l, x in closedBall (w j) (K * δ j)),
