...
@[simp]
/-
**ModuleCat.extendsScalars_map_leftUnitor_inv_one_tmul** 是 Mathlib 中的一个引理，位于命名空间
 `ModuleCat`。
形式化陈述：extendsScalars_map_leftUnitor_inv_one_tmul (M : ModuleCat R) (m : M) : let
I
...
@[simp]
/-
**ModuleCat.extendsScalars_map_rightUnitor_inv_one_tmul** 是 Mathlib 中的一个引理，位于命名空
间 `ModuleCat`。
形式化陈述：extendsScalars_map_rightUnitor_inv_one_tmul (M : ModuleCat R) (m : M) : le
tI
参数：M : ModuleCat R；m : M。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**ModuleCat.extendScalars_** 是 Mathlib 中的一个引理，位于命名空间 `ModuleCat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**ModuleCat.restrictScalars_** 是 Mathlib 中的一个引理，位于命名空间 `ModuleCat`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
