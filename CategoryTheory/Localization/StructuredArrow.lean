...
definition structuredArrowEquiv
  signature: :
  body: StructuredArrow.mk (homEquiv W L L' f.hom)
...
定义 structuredArrowEquiv
  签名: :
  定义体: StructuredArrow.mk (homEquiv W L L' f.hom)
  invFun f := StructuredArrow.mk (homEquiv W L' L f.hom)
  left_inv f := by
    obtain ⟨Y, f, rfl⟩ := f.mk_surjective
    dsimp
...
/--
lemma `induction_structuredArrow'` / 引理 `induction_structuredArrow'`

English:
lemma induction_structuredArrow'
  proof: by
  let X₀ : Paths (LocQuiver W) := ⟨X⟩
  suffices forall ⦃Y₀ : Paths (LocQuiver W)⦄ (f : X₀ ⟶ Y₀),
...
private lemma induction_structuredArrow'
    ...
    (hP₁ : forall ⦃Y₁ Y₂ : C⦄ (f : Y₁ ⟶ Y₂) (φ : W.Q.obj X ⟶ W.Q.obj Y₁),
      P (StructuredArrow.mk φ) -> P (StructuredArrow.mk (φ ≫ W.Q.map f)))
    (hP₂ : forall ⦃Y₁ Y₂ : C⦄ (w : Y₁ ⟶ Y₂) (hw : W w) (φ : W.Q.obj X ⟶ W.Q.obj Y₂),
...
lemma induction_structuredArrow
    ...
    (hP₁ : forall ⦃Y₁ Y₂ : C⦄ (f : Y₁ ⟶ Y₂) (φ : L.obj X ⟶ L.obj Y₁),
      P (StructuredArrow.mk φ) -> P (StructuredArrow.mk (φ ≫ L.map f)))
    (hP₂ : forall ⦃Y₁ Y₂ : C⦄ (w : Y₁ ⟶ Y₂) (hw : W w) (φ : L.obj X ⟶ L.obj Y₂),
...
@[elab_as_elim]
/--
lemma `induction_costructuredArrow` / 引理 `induction_costructuredArrow`
...
lemma induction_costructuredArrow
    ...
    (hP₁ : forall ⦃X₁ X₂ : C⦄ (f : X₁ ⟶ X₂) (φ : L.obj X₂ ⟶ L.obj Y),
      P (CostructuredArrow.mk φ) -> P (CostructuredArrow.mk (L.map f ≫ φ)))
    (hP₂ : forall ⦃X₁ X₂ : C⦄ (w : X₁ ⟶ X₂) (hw : W w) (φ : L.obj X₁ ⟶ L.obj Y),
