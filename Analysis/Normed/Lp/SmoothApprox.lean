...
variable [NormedAddCommGroup E] [NormedSpace Real E] [FiniteDimensional Real E] [BorelSpace E]
  [NormedSpace Real F]

/--
theorem `exist_eLpNorm_sub_le_of_continuous` / 定理 `exist_eLpNorm_sub_le_of_continuous`

English:
theorem exist_eLpNorm_sub_le_of_continuous
...
theorem exist_eLpNorm_sub_le_of_continuous (μ : Measure E := by volume_tac)
    [IsFiniteMeasureOnCompacts μ] {p : Real>=0∞} {ε : Real} (hε : 0 < ε) {f : E -> F}
    (h₁ : HasCompactSupport f) (h₂ : Continuous f) :
    exists (g : E -> F), HasCompactSupport g ∧ ContDiff Real ∞ g ∧
...
/--
theorem `exist_eLpNorm_sub_le` / 定理 `exist_eLpNorm_sub_le`

English:
theorem exist_eLpNorm_sub_le
  statement: {p : Real>=0∞} (hp : p != ⊤) (hp₂ : 1 <= p) {f : E -> F} (hf : MemLp f p μ)
  proof: by
...
定理 exist_eLpNorm_sub_le
  结论: {p : 实数>=0∞} (hp : p != ⊤) (hp₂ : 1 <= p) {f : E -> F} (hf : MemLp f p μ)
  证明: by
  -- We use a standard ε / 2 argument to deduce the result from the approximation for
  -- continuous compactly supported functions.
  have hε₂ : 0 < ε / 2 := by positivity
...
theorem exist_eLpNorm_sub_le {p : Real>=0∞} (hp : p != ⊤) (hp₂ : 1 <= p) {f : E -> F} (hf : MemLp f p μ)
    {ε : Real} (hε : 0 < ε) :
    exists g, HasCompactSupport g ∧ ContDiff Real ∞ g ∧ eLpNorm (f - g) p μ <= ENNReal.ofReal ε := by
  -- We use a standard ε / 2 argument to deduce the result from the approximation for
  -- continuous compactly supported functions.
  have hε₂ : 0 < ε / 2 := by positivity
  have hε₂' : 0 < ENNReal.ofReal (ε / 2) := by positivity
...
/--
theorem `_root_.MeasureTheory.Lp.dense_hasCompactSupport_contDiff` / 定理 `_root_.MeasureTheory.Lp.dense_hasCompactSupport_contDiff`

English:
theorem _root_.MeasureTheory.Lp.dense_hasCompactSupport_contDiff
  statement: {p : Real>=0∞} (hp : p != ⊤)
  proof: by
  intro f
...
-/
theorem _root_.MeasureTheory.Lp.dense_hasCompactSupport_contDiff {p : Real>=0∞} (hp : p != ⊤)
    [hp₂ : Fact (1 <= p)] :
    Dense {f : Lp F p μ | exists (g : E -> F), f =ᵐ[μ] g ∧ HasCompactSupport g ∧ ContDiff Real ∞ g} := by
  intro f
  refine (mem_closure_iff_nhds_basis Metric.nhds_basis_closedBall).2 fun ε hε => ?_
  obtain ⟨g, hg₁, hg₂, hg₃⟩ := exist_eLpNorm_sub_le hp hp₂.out (Lp.memLp f) hε
  have hg₄ : MemLp g p μ := hg₂.continuous.memLp_of_hasCompactSupport hg₁
