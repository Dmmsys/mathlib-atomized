...
/-
**BirdDet.sumFrom_eq_sum_Ico** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：sumFrom_eq_sum_Ico {lo : Nat} (f : Nat -> R) : BirdDet.sumFrom n lo f = ∑ 
k in Finset.Ico lo n, f k
参数：f : Nat -> R。
该定理/引理给出了一组等式。
...
/-
**BirdDet.sumFrom_fin_tail** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：sumFrom_fin_tail (i : Fin n) (f : Nat -> R) : BirdDet.sumFrom n (i.val + 1
) f = ∑ k in Finset.Ioi i, f k.val
参数：i : Fin n；f : Nat -> R。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**BirdDet.iterate_stepEntry_get_eq_spec** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：iterate_stepEntry_get_eq_spec (A : Array R) (hA : A.size = n * n) (t : Nat
) (i j : Fin n) : ((stepEntry n A)^[t] (BirdDet.get n A)) i.val j.val = (Spec.st
epEntry (.ofArray A hA))^[t] (.ofArray A hA) i j
参数：A : Array R；hA : A.size = n * n；t : Nat；i j : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.birdDet_eq_birdDetSpec** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：birdDet_eq_birdDetSpec (A : Array R) (hA : A.size = n * n) : birdDet n A =
 Spec.birdDet (.ofArray A hA)
参数：A : Array R；hA : A.size = n * n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.bminor** 是 Mathlib 中的一个缩写定义，位于命名空间 `BirdDet`。
形式化陈述：bminor (i j : Fin n) (α : Fin p -> Fin n) : R
参数：i j : Fin n；α : Fin p -> Fin n。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.pminor** 是 Mathlib 中的一个缩写定义，位于命名空间 `BirdDet`。
形式化陈述：pminor (α : Fin p -> Fin n) : R
参数：α : Fin p -> Fin n。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
abbrev pminor (α : Fin p → Fin n) : R :=
  (A.submatrix α α).det
/-
**BirdDet.det_submatrix_removeNth_eq_sign_mul_bminor** 是 Mathlib 中的一个引理，位于命名空间 `
BirdDet`。
形式化陈述：det_submatrix_removeNth_eq_sign_mul_bminor (α : Fin (p + 1) -> Fin n) (i :
 Fin n) (s : Fin (p + 1)) : (A.submatrix (Fin.cons i (s.removeNth α)) α).det = (
-1 : R) ^ s.val * bminor A i (α s) (s.removeNth α)
参数：α : Fin (p + 1) -> Fin n；i : Fin n；s : Fin (p + 1)。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**BirdDet.det_bordered_expand** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：det_bordered_expand (α : Fin (p + 1) -> Fin n) (i j : Fin n) : bminor A i 
j α = pminor A α * A i j - ∑ s : Fin (p + 1), bminor A i (α s) (s.removeNth α) *
 A (α s) j
参数：α : Fin (p + 1) -> Fin n；i j : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.bminor_eq_zero_of_mem_range** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：bminor_eq_zero_of_mem_range {k : Fin n} (α : Fin p -> Fin n) (i : Fin n) (
hk : k in Set.range α) : bminor A i k α = 0
参数：α : Fin p -> Fin n；i : Fin n；hk : k in Set.range α。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.S** 是 Mathlib 中的一个定义，位于命名空间 `BirdDet`。
形式化陈述：S (p : Nat) (i : Fin n) : Finset (Fin p -> Fin n)
参数：p : Nat；i : Fin n。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.mem_S_iff** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：mem_S_iff {p : Nat} {i : Fin n} {α : Fin p -> Fin n} : α in S p i ↔ Strict
Mono (Fin.cons i α)
该定理/引理刻画了左右两侧的等价关系。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
word `ε`. -/
/-
**BirdDet.S_zero** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：S_zero (i : Fin n) : S 0 i = {![]}
参数：i : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.S_zero_eq_singleton** 是 Mathlib 中的一个引理，位于命名空间 `BirdDet`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.S_succ_eq_biUnion** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：S_succ_eq_biUnion {p : Nat} (i : Fin n) : S (p + 1) i = (Finset.Ioi i).biU
nion fun k => (S p k).image (Fin.cons k)
参数：i : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.exists_insertNth_mem_S** 是 Mathlib 中的一个引理，位于命名空间 `BirdDet`。
形式化陈述：exists_insertNth_mem_S {p : Nat} {i : Fin n} {α : Fin p -> Fin n} {k : Fin
 n} (hα : α in S p i) (hik : i < k) (hk : k ∉ Set.range α) : exists t : Fin (p +
 1), t.insertNth k α in S (p + 1) i
参数：hα : α in S p i；hik : i < k；hk : k ∉ Set.range α。
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A symbol above `i` that does not occur in a word in `S p i` can be inserted whil
e
preserving strict monotonicity.
-/
...
/-
**BirdDet.Eq1** 是 Mathlib 中的一个缩写定义，位于命名空间 `BirdDet`。
形式化陈述：Eq1 : Prop
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.paper_eq2** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：paper_eq2 (i : Fin n) (hEq1 : Eq1 A p) : (-∑ k in Finset.Ioi i, (Spec.step
Entry A)^[p] A k k) = (-1) ^ (p + 1) * ∑ α in S (p + 1) i, pminor A α
参数：i : Fin n；hEq1 : Eq1 A p。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.paper_eq3** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：paper_eq3 (i j : Fin n) (hEq1 : Eq1 A p) : ((Spec.stepEntry A)^[p + 1] A) 
i j = (-1) ^ (p + 1) * (∑ α in S (p + 1) i, pminor A α * A i j - ∑ k in Finset.I
oi i, ∑ α in S p i, bminor A i k α * A k j)
参数：i j : Fin n；hEq1 : Eq1 A p。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-- Bird's equation (5) -/
/-
**BirdDet.paper_eq5** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：paper_eq5 (i j : Fin n) : ∑ α in S (p + 1) i, bminor A i j α = ∑ α in S (p
 + 1) i, pminor A α * A i j - ∑ α in S (p + 1) i, ∑ t : Fin (p + 1), bminor A i 
(α t) (t.removeNth α) * A (α t) j
参数：i j : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BirdDet.paper_eq3_eq5_off_diag** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：paper_eq3_eq5_off_diag (i j : Fin n) : ∑ k in Finset.Ioi i, ∑ α in S p i, 
bminor A i k α * A k j = ∑ α' in S (p + 1) i, ∑ t : Fin (p + 1), bminor A i (α' 
t) (t.removeNth α') * A (α' t) j
参数：i j : Fin n。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-! ## Bird's Equation (1) -/
/-
**BirdDet.paper_eq1** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：paper_eq1 : Eq1 A p
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-- Bird's Theorem 1 -/
/-
**BirdDet.birdDetSpec_eq_det** 是 Mathlib 中的一个定理，位于命名空间 `BirdDet`。
形式化陈述：birdDetSpec_eq_det (A : Matrix (Fin n) (Fin n) R) : Matrix.det A = Spec.bi
rdDet A
