/-
Copyright (c) 2025 Joël Riou. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Joël Riou
-/
module

public import Mathlib.Algebra.Homology.Embedding.CochainComplex
public import Mathlib.CategoryTheory.Preadditive.Injective.Resolution

/-!
# Injective resolutions as cochain complexes indexed by the integers

Given an injective resolution `R` of an object `X` in an abelian category `C`,
we define `R.cochainComplex : CochainComplex C ℤ`, which is the extension
of `R.cocomplex : CochainComplex C ℕ`, and the quasi-isomorphism
`R.ι' : (CochainComplex.singleFunctor C 0).obj X ⟶ R.cochainComplex`.

-/

@[expose] public section

universe v u

namespace CategoryTheory

open Limits

variable {C : Type u} [Category.{v} C]

namespace InjectiveResolution

section

variable [HasZeroObject C] [Preadditive C] {X : C}
  (R : InjectiveResolution X)

/--
Definition of `cochainComplex` / `cochainComplex` 的定义

English:
definition cochainComplex
  signature: : CochainComplex C Int
  body: R.cocomplex.extend ComplexShape.embeddingUpNat

中文:
定义 cochainComplex
  签名: : CochainComplex C 整数
  定义体: R.cocomplex.extend ComplexShape.embeddingUpNat

Depends on / 依赖: ComplexShape, ComplexShape.embeddingUpNat, R.cocomplex.extend, cocomplex, embeddingUpNat, extend
-/
noncomputable def cochainComplex : CochainComplex C Int :=
  R.cocomplex.extend ComplexShape.embeddingUpNat

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: R.cochainComplex.IsStrictlyGE 0
  body: by
  dsimp [cochainComplex]
  infer_instance

中文:
实例 :
  签名: R.cochainComplex.IsStrictlyGE 0
  定义体: by
  dsimp [cochainComplex]
  infer_instance

Depends on / 依赖: cochainComplex, infer_instance
-/
instance : R.cochainComplex.IsStrictlyGE 0 := by
  dsimp [cochainComplex]
  infer_instance

/--
Definition of `cochainComplexXIso` / `cochainComplexXIso` 的定义

English:
definition cochainComplexXIso
  signature: (n : Int) (k : Nat) (h : k = n)
  body: HomologicalComplex.extendXIso _ _ h

@[reassoc]

中文:
定义 cochainComplexXIso
  签名: (n : 整数) (k : 自然数) (h : k = n)
  定义体: HomologicalComplex.extendXIso _ _ h

@[reassoc]

Depends on / 依赖: HomologicalComplex, HomologicalComplex.extendXIso, extendXIso
-/
noncomputable def cochainComplexXIso (n : Int) (k : Nat) (h : k = n) :
    R.cochainComplex.X n ≅ R.cocomplex.X k :=
  HomologicalComplex.extendXIso _ _ h

@[reassoc]
/--
lemma `cochainComplex_d` / 引理 `cochainComplex_d`

English:
lemma cochainComplex_d
  given: (n₁ n₂ : Int) (k₁ k₂ : Nat) (h₁ : k₁ = n₁) (h₂ : k₂ = n₂)
  proof: HomologicalComplex.extend_d_eq _ _ h₁ h₂

中文:
引理 cochainComplex_d
  条件: (n₁ n₂ : 整数) (k₁ k₂ : 自然数) (h₁ : k₁ = n₁) (h₂ : k₂ = n₂)
  证明: HomologicalComplex.extend_d_eq _ _ h₁ h₂

Depends on / 依赖: HomologicalComplex, HomologicalComplex.extend_d_eq, extend_d_eq
-/
lemma cochainComplex_d (n₁ n₂ : Int) (k₁ k₂ : Nat) (h₁ : k₁ = n₁) (h₂ : k₂ = n₂) :
    R.cochainComplex.d n₁ n₂ = (cochainComplexXIso _ _ _ h₁).hom ≫
      R.cocomplex.d k₁ k₂ ≫ (cochainComplexXIso _ _ _ h₂).inv :=
  HomologicalComplex.extend_d_eq _ _ h₁ h₂

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: R.cochainComplex.IsStrictlyGE 0
  body: by
  dsimp [cochainComplex]
  infer_instance

中文:
实例 :
  签名: R.cochainComplex.IsStrictlyGE 0
  定义体: by
  dsimp [cochainComplex]
  infer_instance

Depends on / 依赖: cochainComplex, infer_instance
-/
instance : R.cochainComplex.IsStrictlyGE 0 := by
  dsimp [cochainComplex]
  infer_instance

instance (n : Int) : Injective (R.cochainComplex.X n) := by
  by_cases hn : 0 <= n
  · obtain ⟨k, rfl⟩ := Int.eq_ofNat_of_zero_le hn
    exact Injective.of_iso (R.cochainComplexXIso _ _ rfl).symm inferInstance
  · exact IsZero.injective (CochainComplex.isZero_of_isStrictlyGE _ 0 _ (by lia))

/--
Definition of `ι'` / `ι'` 的定义

English:
definition ι'
  signature: : (CochainComplex.singleFunctor C 0).obj X ⟶ R.cochainComplex
  body: (HomologicalComplex.extendSingleIso _ _ _ _ (by simp)).inv ≫
    (ComplexShape.embeddingUpNat.extendFunctor C).map R.ι

中文:
定义 ι'
  签名: : (CochainComplex.singleFunctor C 0).obj X ⟶ R.cochainComplex
  定义体: (HomologicalComplex.extendSingleIso _ _ _ _ (by simp)).inv ≫
    (ComplexShape.embeddingUpNat.extendFunctor C).map R.ι

Depends on / 依赖: ComplexShape, ComplexShape.embeddingUpNat.extendFunctor, HomologicalComplex, HomologicalComplex.extendSingleIso, embeddingUpNat, extendFunctor, extendSingleIso
-/
noncomputable def ι' : (CochainComplex.singleFunctor C 0).obj X ⟶ R.cochainComplex :=
  (HomologicalComplex.extendSingleIso _ _ _ _ (by simp)).inv ≫
    (ComplexShape.embeddingUpNat.extendFunctor C).map R.ι

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
@[reassoc]
/--
lemma `ι'_f_zero` / 引理 `ι'_f_zero`

English:
lemma ι'_f_zero
  proof: by
  dsimp [ι']
  rw [HomologicalComplex.extendMap_f _ _ (i := 0) (by simp)]; rw [HomologicalComplex.extendSingleIso_inv_f]
  cat_disch

中文:
引理 ι'_f_zero
  证明: by
  dsimp [ι']
  rw [HomologicalComplex.extendMap_f _ _ (i := 0) (by simp)]; rw [HomologicalComplex.extendSingleIso_inv_f]
  cat_disch
-/
lemma ι'_f_zero :
    R.ι'.f 0 = (HomologicalComplex.singleObjXSelf (.up Int) 0 X).hom ≫ R.ι.f 0 ≫
      (R.cochainComplexXIso _ _ (by simp)).inv := by
  dsimp [ι']
  rw [HomologicalComplex.extendMap_f _ _ (i := 0) (by simp)]; rw [HomologicalComplex.extendSingleIso_inv_f]
  cat_disch

end

variable [Abelian C] {X : C} (R : InjectiveResolution X)

set_option backward.defeqAttrib.useBackward true in
set_option backward.isDefEq.respectTransparency false in
/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: QuasiIso R.ι'
  body: by dsimp [ι']; infer_instance

中文:
实例 :
  签名: QuasiIso R.ι'
  定义体: by dsimp [ι']; infer_instance

Depends on / 依赖: infer_instance
-/
instance : QuasiIso R.ι' := by dsimp [ι']; infer_instance

/--
Instance `_anonymous_` / 实例 `_anonymous_`

English:
instance :
  signature: R.cochainComplex.IsLE 0
  body: by
  simp only [← HomologicalComplex.isSupported_iff_of_quasiIso R.ι']
  infer_instance

中文:
实例 :
  签名: R.cochainComplex.IsLE 0
  定义体: by
  simp only [← HomologicalComplex.isSupported_iff_of_quasiIso R.ι']
  infer_instance

Depends on / 依赖: HomologicalComplex, HomologicalComplex.isSupported_iff_of_quasiIso, infer_instance, isSupported_iff_of_quasiIso
-/
instance : R.cochainComplex.IsLE 0 := by
  simp only [← HomologicalComplex.isSupported_iff_of_quasiIso R.ι']
  infer_instance

namespace Hom

variable {R} {X' : C} {R' : InjectiveResolution X'} {f : X ⟶ X'}
  (φ : Hom R R' f)

/--
Definition of `hom'` / `hom'` 的定义

English:
definition hom'
  signature: : R.cochainComplex ⟶ R'.cochainComplex
  body: HomologicalComplex.extendMap φ.hom _

中文:
定义 hom'
  签名: : R.cochainComplex ⟶ R'.cochainComplex
  定义体: HomologicalComplex.extendMap φ.hom _

Depends on / 依赖: HomologicalComplex, HomologicalComplex.extendMap, extendMap
-/
noncomputable def hom' : R.cochainComplex ⟶ R'.cochainComplex :=
  HomologicalComplex.extendMap φ.hom _

set_option backward.isDefEq.respectTransparency false in
@[reassoc]
/--
lemma `hom'_f` / 引理 `hom'_f`

English:
lemma hom'_f
  given: (n : Int) (m : Nat) (h : m = n)
  proof: by
  simp [hom',
    HomologicalComplex.extendMap_f _ ComplexShape.embeddingUpNat (i := m) (i' := n) (by simpa),
    cochainComplexXIso]

中文:
引理 hom'_f
  条件: (n : 整数) (m : 自然数) (h : m = n)
  证明: by
  simp [hom',
    HomologicalComplex.extendMap_f _ ComplexShape.embeddingUpNat (i := m) (i' := n) (by simpa),
    cochainComplexXIso]
-/
lemma hom'_f (n : Int) (m : Nat) (h : m = n) :
    φ.hom'.f n =
    (R.cochainComplexXIso n m h).hom ≫ φ.hom.f m ≫ (R'.cochainComplexXIso n m h).inv := by
  simp [hom',
    HomologicalComplex.extendMap_f _ ComplexShape.embeddingUpNat (i := m) (i' := n) (by simpa),
    cochainComplexXIso]

set_option backward.isDefEq.respectTransparency false in
@[reassoc (attr := simp)]
/--
lemma `ι'_comp_hom'` / 引理 `ι'_comp_hom'`

English:
lemma ι'_comp_hom'
  proof: HomologicalComplex.from_single_hom_ext (by
    simp [hom'_f _ 0 0 rfl, ι'_f_zero, CochainComplex.singleFunctor,
      CochainComplex.singleFunctors,
      HomologicalComplex.single, HomologicalComplex.singleObjXSelf,
      HomologicalComplex.singleObjXIsoOfEq])

中文:
引理 ι'_comp_hom'
  证明: HomologicalComplex.from_single_hom_ext (by
    simp [hom'_f _ 0 0 rfl, ι'_f_zero, CochainComplex.singleFunctor,
      CochainComplex.singleFunctors,
      HomologicalComplex.single, HomologicalComplex.singleObjXSelf,
      HomologicalComplex.singleObjXIsoOfEq])

Depends on / 依赖: CochainComplex, CochainComplex.singleFunctor, CochainComplex.singleFunctors, HomologicalComplex, HomologicalComplex.from_single_hom_ext, HomologicalComplex.single, HomologicalComplex.singleObjXIsoOfEq, HomologicalComplex.singleObjXSelf, _f_zero, from_single_hom_ext, single, singleFunctor, singleFunctors, singleObjXIsoOfEq, singleObjXSelf
-/
lemma ι'_comp_hom' :
    R.ι' ≫ φ.hom' = (CochainComplex.singleFunctor C 0).map f ≫ R'.ι' :=
  HomologicalComplex.from_single_hom_ext (by
    simp [hom'_f _ 0 0 rfl, ι'_f_zero, CochainComplex.singleFunctor,
      CochainComplex.singleFunctors,
      HomologicalComplex.single, HomologicalComplex.singleObjXSelf,
      HomologicalComplex.singleObjXIsoOfEq])

end Hom

end InjectiveResolution

end CategoryTheory
