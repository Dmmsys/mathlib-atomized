/-
Copyright (c) 2022 Anne Baanen. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Junyan Xu, Anne Baanen
-/
module

public import Mathlib.Algebra.Module.LocalizedModule.IsLocalization
public import Mathlib.LinearAlgebra.Basis.Basic
public import Mathlib.RingTheory.Localization.FractionRing
public import Mathlib.RingTheory.Localization.Integer

/-!
# Modules / vector spaces over localizations / fraction fields

This file contains some results about vector spaces over the field of fractions of a ring.

## Main results

* `LinearIndependent.localization`: `b` is linear independent over a localization of `R`
  if it is linear independent over `R` itself
* `Basis.ofIsLocalizedModule` / `Basis.localizationLocalization`: promote an `R`-basis `b` of `A`
  to an `Rₛ`-basis of `Aₛ`, where `Rₛ` and `Aₛ` are localizations of `R` and `A` at `s`
  respectively
* `LinearIndependent.iff_fractionRing`: `b` is linear independent over `R` iff it is
  linear independent over `Frac(R)`
-/

@[expose] public section


open nonZeroDivisors

section Localization
variable {R : Type*} (Rₛ : Type*)

section IsLocalizedModule

open Submodule

variable [CommSemiring R] (S : Submonoid R) [CommSemiring Rₛ] [Algebra R Rₛ] [IsLocalization S Rₛ]
  {M Mₛ : Type*} [AddCommMonoid M] [Module R M] [AddCommMonoid Mₛ] [Module R Mₛ]
  [Module Rₛ Mₛ] [IsScalarTower R Rₛ Mₛ] (f : M ->ₗ[R] Mₛ) [IsLocalizedModule S f]

include S

/--
theorem `span_eq_top_of_isLocalizedModule` / 定理 `span_eq_top_of_isLocalizedModule`

English:
theorem span_eq_top_of_isLocalizedModule
  given: {v : Set M} (hv : span R v = ⊤)
  proof: top_unique fun x _ => by
  obtain ⟨⟨m, s⟩, h⟩ := IsLocalizedModule.surj S f x
  rw [Submonoid.smul_def]; rw [← algebraMap_smul Rₛ]; rw [← Units.smul_isUnit (IsLocalization.map_units Rₛ s)]; rw [eq_comm]; rw [← inv_smul_eq_iff] at h
  refine h ▸ smul_mem _ _ (span_subset_span R Rₛ _ ?_)
  rw [← LinearMap.coe_restrictScalars R]; rw [← LinearMap.map_span]; rw [hv]
  exact mem_map_of_mem mem_top

中文:
定理 span_eq_top_of_isLocalizedModule
  条件: {v : 集合 M} (hv : span R v = ⊤)
  证明: top_unique fun x _ => by
  obtain ⟨⟨m, s⟩, h⟩ := IsLocalizedModule.surj S f x
  rw [Submonoid.smul_def]; rw [← algebraMap_smul Rₛ]; rw [← Units.smul_isUnit (IsLocalization.map_units Rₛ s)]; rw [eq_comm]; rw [← inv_smul_eq_iff] at h
  refine h ▸ smul_mem _ _ (span_subset_span R Rₛ _ ?_)
  rw [← LinearMap.coe_restrictScalars R]; rw [← LinearMap.map_span]; rw [hv]
  exact mem_map_of_mem mem_top

Depends on / 依赖: IsLocalization, IsLocalization.map_units, IsLocalizedModule, IsLocalizedModule.surj, LinearMap, LinearMap.coe_restrictScalars, LinearMap.map_span, Submonoid, Submonoid.smul_def, Units.smul_isUnit, algebraMap_smul, coe_restrictScalars, eq_comm, inv_smul_eq_iff, map_span, map_units, mem_map_of_mem, mem_top, smul_def, smul_isUnit
-/
theorem span_eq_top_of_isLocalizedModule {v : Set M} (hv : span R v = ⊤) :
    span Rₛ (f '' v) = ⊤ := top_unique fun x _ => by
  obtain ⟨⟨m, s⟩, h⟩ := IsLocalizedModule.surj S f x
  rw [Submonoid.smul_def]; rw [← algebraMap_smul Rₛ]; rw [← Units.smul_isUnit (IsLocalization.map_units Rₛ s)]; rw [eq_comm]; rw [← inv_smul_eq_iff] at h
  refine h ▸ smul_mem _ _ (span_subset_span R Rₛ _ ?_)
  rw [← LinearMap.coe_restrictScalars R]; rw [← LinearMap.map_span]; rw [hv]
  exact mem_map_of_mem mem_top

set_option backward.isDefEq.respectTransparency false in
/--
theorem `LinearIndependent.of_isLocalizedModule` / 定理 `LinearIndependent.of_isLocalizedModule`

English:
theorem LinearIndependent.of_isLocalizedModule
  statement: {ι : Type*} {v : ι -> M}
  proof: by
  rw [linearIndependent_iff'ₛ] at hv ⊢
  intro t g₁ g₂ eq i hi
  choose! a fg hfg using IsLocalization.exist_integer_multiples S (t.disjSum t) (Sum.elim g₁ g₂)
  simp_rw [Sum.forall, Finset.inl_mem_disjSum, Sum.elim_inl, Finset.inr_mem_disjSum, Sum.elim_inr,
    Subtype.forall'] at hfg
  apply_fun ((a : R) • ·) at eq
  simp_rw [← t.sum_coe_sort, Finset.smul_sum, ← smul_assoc, ← hfg,
    algebraMap_smul, Function.comp_def, ← map_smul, ← map_sum,
    t.sum_coe_sort (f := fun x => fg (Sum.inl x) • v x),
    t.sum_coe_sort (f := fun x => fg (Sum.inr x) • v x)] at eq
  have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
  simp_rw [Finset.smul_sum, Submonoid.smul_def, smul_smul] at eq
  have := congr(algebraMap R Rₛ $(hv t _ _ eq i hi))
  simpa only [map_mul, (IsLocalization.map_units Rₛ s).mul_right_inj, hfg.1 ⟨i, hi⟩, hfg.2 ⟨i, hi⟩,
    Algebra.smul_def, (IsLocalization.map_units Rₛ a).mul_right_inj] using this

中文:
定理 LinearIndependent.of_isLocalizedModule
  结论: {ι : 类型} {v : ι -> M}
  证明: by
  rw [linearIndependent_iff'ₛ] at hv ⊢
  intro t g₁ g₂ eq i hi
  choose! a fg hfg using IsLocalization.exist_integer_multiples S (t.disjSum t) (Sum.elim g₁ g₂)
  simp_rw [Sum.forall, Finset.inl_mem_disjSum, Sum.elim_inl, Finset.inr_mem_disjSum, Sum.elim_inr,
    Subtype.forall'] at hfg
  apply_fun ((a : R) • ·) at eq
  simp_rw [← t.sum_coe_sort, Finset.smul_sum, ← smul_assoc, ← hfg,
    algebraMap_smul, Function.comp_def, ← map_smul, ← map_sum,
    t.sum_coe_sort (f := fun x => fg (Sum.inl x) • v x),
    t.sum_coe_sort (f := fun x => fg (Sum.inr x) • v x)] at eq
  have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
  simp_rw [Finset.smul_sum, Submonoid.smul_def, smul_smul] at eq
  have := congr(algebraMap R Rₛ $(hv t _ _ eq i hi))
  simpa only [map_mul, (IsLocalization.map_units Rₛ s).mul_right_inj, hfg.1 ⟨i, hi⟩, hfg.2 ⟨i, hi⟩,
    Algebra.smul_def, (IsLocalization.map_units Rₛ a).mul_right_inj] using this

Depends on / 依赖: Finset, Finset.inl_mem_disjSum, Finset.inr_mem_disjSum, Finset.smul_sum, Function, Function.comp_def, IsLocalization, IsLocalization.exist_integer_multiples, Subtype, Subtype.forall, Sum.elim, Sum.elim_inl, Sum.elim_inr, Sum.forall, Sum.inl, algebraMap_smul, apply_fun, comp_def, disjSum, elim_inl
-/
theorem LinearIndependent.of_isLocalizedModule {ι : Type*} {v : ι -> M}
    (hv : LinearIndependent R v) : LinearIndependent Rₛ (f ∘ v) := by
  rw [linearIndependent_iff'ₛ] at hv ⊢
  intro t g₁ g₂ eq i hi
  choose! a fg hfg using IsLocalization.exist_integer_multiples S (t.disjSum t) (Sum.elim g₁ g₂)
  simp_rw [Sum.forall, Finset.inl_mem_disjSum, Sum.elim_inl, Finset.inr_mem_disjSum, Sum.elim_inr,
    Subtype.forall'] at hfg
  apply_fun ((a : R) • ·) at eq
  simp_rw [← t.sum_coe_sort, Finset.smul_sum, ← smul_assoc, ← hfg,
    algebraMap_smul, Function.comp_def, ← map_smul, ← map_sum,
    t.sum_coe_sort (f := fun x => fg (Sum.inl x) • v x),
    t.sum_coe_sort (f := fun x => fg (Sum.inr x) • v x)] at eq
  have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
  simp_rw [Finset.smul_sum, Submonoid.smul_def, smul_smul] at eq
  have := congr(algebraMap R Rₛ $(hv t _ _ eq i hi))
  simpa only [map_mul, (IsLocalization.map_units Rₛ s).mul_right_inj, hfg.1 ⟨i, hi⟩, hfg.2 ⟨i, hi⟩,
    Algebra.smul_def, (IsLocalization.map_units Rₛ a).mul_right_inj] using this

set_option backward.isDefEq.respectTransparency false in
/--
theorem `LinearIndependent.of_isLocalizedModule_of_isRegular` / 定理 `LinearIndependent.of_isLocalizedModule_of_isRegular`

English:
theorem LinearIndependent.of_isLocalizedModule_of_isRegular
  statement: {ι : Type*} {v : ι -> M}
  proof: hv.map_injOn _ by
    rw [← Finsupp.range_linearCombination]
    rintro _ ⟨_, r, rfl⟩ _ ⟨_, r', rfl⟩ eq
    congr; ext i
    have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
    simp_rw [Submonoid.smul_def, ← map_smul] at eq
    exact (h s).1 (DFunLike.congr_fun (hv eq) i)

中文:
定理 LinearIndependent.of_isLocalizedModule_of_isRegular
  结论: {ι : 类型} {v : ι -> M}
  证明: hv.map_injOn _ by
    rw [← Finsupp.range_linearCombination]
    rintro _ ⟨_, r, rfl⟩ _ ⟨_, r', rfl⟩ eq
    congr; ext i
    have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
    simp_rw [Submonoid.smul_def, ← map_smul] at eq
    exact (h s).1 (DFunLike.congr_fun (hv eq) i)

Depends on / 依赖: DFunLike, DFunLike.congr_fun, Finsupp, Finsupp.range_linearCombination, IsLocalizedModule, IsLocalizedModule.exists_of_eq, Submonoid, Submonoid.smul_def, congr_fun, exists_of_eq, hv.map_injOn, map_injOn, map_smul, range_linearCombination, simp_rw, smul_def
-/
theorem LinearIndependent.of_isLocalizedModule_of_isRegular {ι : Type*} {v : ι -> M}
    (hv : LinearIndependent R v) (h : forall s : S, IsRegular (s : R)) : LinearIndependent R (f ∘ v) :=
hv.map_injOn _ by
    rw [← Finsupp.range_linearCombination]
    rintro _ ⟨_, r, rfl⟩ _ ⟨_, r', rfl⟩ eq
    congr; ext i
    have ⟨s, eq⟩ := IsLocalizedModule.exists_of_eq (S := S) eq
    simp_rw [Submonoid.smul_def, ← map_smul] at eq
    exact (h s).1 (DFunLike.congr_fun (hv eq) i)

/--
theorem `LinearIndependent.localization` / 定理 `LinearIndependent.localization`

English:
theorem LinearIndependent.localization
  statement: [Module Rₛ M] [IsScalarTower R Rₛ M]
  proof: by
  have := isLocalizedModule_id S M Rₛ
  exact hli.of_isLocalizedModule Rₛ S .id

中文:
定理 LinearIndependent.localization
  结论: [模 Rₛ M] [标量塔 R Rₛ M]
  证明: by
  have := isLocalizedModule_id S M Rₛ
  exact hli.of_isLocalizedModule Rₛ S .id

Depends on / 依赖: hli.of_isLocalizedModule, isLocalizedModule_id, of_isLocalizedModule
-/
theorem LinearIndependent.localization [Module Rₛ M] [IsScalarTower R Rₛ M]
    {ι : Type*} {b : ι -> M} (hli : LinearIndependent R b) :
    LinearIndependent Rₛ b := by
  have := isLocalizedModule_id S M Rₛ
  exact hli.of_isLocalizedModule Rₛ S .id

set_option backward.isDefEq.respectTransparency false in
include f in
/--
lemma `IsLocalizedModule.linearIndependent_lift` / 引理 `IsLocalizedModule.linearIndependent_lift`

English:
lemma IsLocalizedModule.linearIndependent_lift
  given: {ι} {v : ι -> Mₛ} (hf : LinearIndependent R v)
  proof: by
  cases isEmpty_or_nonempty ι
  · exact ⟨isEmptyElim, linearIndependent_empty_type⟩
  have inj := hf.smul_left_injective (Classical.arbitrary ι)
  choose sec hsec using surj S f
  use fun i => (sec (v i)).1
  rw [linearIndependent_iff'ₛ] at hf ⊢
  intro t g g' eq i hit
refine (isRegular_of_smul_left_injective f inj (sec (v i)).2).2
    hf t (fun i => _ * (sec (v i)).2) (fun i => _ * (sec (v i)).2) ?_ i hit
  simp_rw [mul_smul, ← Submonoid.smul_def, hsec, ← map_smul, ← map_sum, eq]

中文:
引理 是Localized模.linearIndependent_lift
  条件: {ι} {v : ι -> Mₛ} (hf : LinearIndependent R v)
  证明: by
  cases isEmpty_or_nonempty ι
  · exact ⟨isEmptyElim, linearIndependent_empty_type⟩
  have inj := hf.smul_left_injective (Classical.arbitrary ι)
  choose sec hsec using surj S f
  use fun i => (sec (v i)).1
  rw [linearIndependent_iff'ₛ] at hf ⊢
  intro t g g' eq i hit
refine (isRegular_of_smul_left_injective f inj (sec (v i)).2).2
    hf t (fun i => _ * (sec (v i)).2) (fun i => _ * (sec (v i)).2) ?_ i hit
  simp_rw [mul_smul, ← Submonoid.smul_def, hsec, ← map_smul, ← map_sum, eq]

Depends on / 依赖: Classical, Classical.arbitrary, Submonoid, Submonoid.smul_def, arbitrary, hf.smul_left_injective, isEmptyElim, isEmpty_or_nonempty, isRegular_of_smul_left_injective, linearIndependent_empty_type, linearIndependent_iff, map_smul, map_sum, mul_smul, simp_rw, smul_def, smul_left_injective
-/
lemma IsLocalizedModule.linearIndependent_lift {ι} {v : ι -> Mₛ} (hf : LinearIndependent R v) :
    exists w : ι -> M, LinearIndependent R w := by
  cases isEmpty_or_nonempty ι
  · exact ⟨isEmptyElim, linearIndependent_empty_type⟩
  have inj := hf.smul_left_injective (Classical.arbitrary ι)
  choose sec hsec using surj S f
  use fun i => (sec (v i)).1
  rw [linearIndependent_iff'ₛ] at hf ⊢
  intro t g g' eq i hit
refine (isRegular_of_smul_left_injective f inj (sec (v i)).2).2
    hf t (fun i => _ * (sec (v i)).2) (fun i => _ * (sec (v i)).2) ?_ i hit
  simp_rw [mul_smul, ← Submonoid.smul_def, hsec, ← map_smul, ← map_sum, eq]

namespace Module.Basis

variable {ι : Type*} (b : Basis ι R M)

/--
Definition of `ofIsLocalizedModule` / `ofIsLocalizedModule` 的定义

English:
definition ofIsLocalizedModule
  signature: : Basis ι Rₛ Mₛ
  body: .mk (b.linearIndependent.of_isLocalizedModule Rₛ S f) by
    rw [Set.range_comp]; rw [span_eq_top_of_isLocalizedModule Rₛ S _ b.span_eq]

@[simp]

中文:
定义 ofIsLocalizedModule
  签名: : 基 ι Rₛ Mₛ
  定义体: .mk (b.linearIndependent.of_isLocalizedModule Rₛ S f) by
    rw [Set.range_comp]; rw [span_eq_top_of_isLocalizedModule Rₛ S _ b.span_eq]

@[simp]

Depends on / 依赖: Set.range_comp, b.linearIndependent.of_isLocalizedModule, b.span_eq, linearIndependent, of_isLocalizedModule, range_comp, span_eq, span_eq_top_of_isLocalizedModule
-/
noncomputable def ofIsLocalizedModule : Basis ι Rₛ Mₛ :=
.mk (b.linearIndependent.of_isLocalizedModule Rₛ S f) by
    rw [Set.range_comp]; rw [span_eq_top_of_isLocalizedModule Rₛ S _ b.span_eq]

@[simp]
/--
theorem `ofIsLocalizedModule_apply` / 定理 `ofIsLocalizedModule_apply`

English:
theorem ofIsLocalizedModule_apply
  given: (i : ι)
  statement: b.ofIsLocalizedModule Rₛ S f i = f (b i)
  proof: by
  rw [ofIsLocalizedModule]; rw [coe_mk]; rw [Function.comp_apply]

@[simp]

中文:
定理 ofIsLocalizedModule_apply
  条件: (i : ι)
  结论: b.ofIsLocalizedModule Rₛ S f i = f (b i)
  证明: by
  rw [ofIsLocalizedModule]; rw [coe_mk]; rw [Function.comp_apply]

@[simp]

Depends on / 依赖: Function, Function.comp_apply, coe_mk, comp_apply, ofIsLocalizedModule
-/
theorem ofIsLocalizedModule_apply (i : ι) : b.ofIsLocalizedModule Rₛ S f i = f (b i) := by
  rw [ofIsLocalizedModule]; rw [coe_mk]; rw [Function.comp_apply]

@[simp]
/--
theorem `ofIsLocalizedModule_repr_apply` / 定理 `ofIsLocalizedModule_repr_apply`

English:
theorem ofIsLocalizedModule_repr_apply
  given: (m : M) (i : ι)
  proof: by
  suffices ((b.ofIsLocalizedModule Rₛ S f).repr.toLinearMap.restrictScalars R) ∘ₗ f =
      Finsupp.mapRange.linearMap (Algebra.linearMap R Rₛ) ∘ₗ b.repr.toLinearMap by
    exact DFunLike.congr_fun (LinearMap.congr_fun this m) i
  refine ext b fun i => ?_
  rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearMap.coe_restrictScalars]; rw [LinearEquiv.coe_coe]; rw [← b.ofIsLocalizedModule_apply Rₛ S f]; rw [repr_self]; rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearEquiv.coe_coe]; rw [repr_self]; rw [Finsupp.mapRange.linearMap_apply]; rw [Finsupp.mapRange_single]; rw [Algebra.linearMap_apply]; rw [map_one]

中文:
定理 ofIsLocalizedModule_repr_apply
  条件: (m : M) (i : ι)
  证明: by
  suffices ((b.ofIsLocalizedModule Rₛ S f).repr.toLinearMap.restrictScalars R) ∘ₗ f =
      Finsupp.mapRange.linearMap (Algebra.linearMap R Rₛ) ∘ₗ b.repr.toLinearMap by
    exact DFunLike.congr_fun (LinearMap.congr_fun this m) i
  refine ext b fun i => ?_
  rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearMap.coe_restrictScalars]; rw [LinearEquiv.coe_coe]; rw [← b.ofIsLocalizedModule_apply Rₛ S f]; rw [repr_self]; rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearEquiv.coe_coe]; rw [repr_self]; rw [Finsupp.mapRange.linearMap_apply]; rw [Finsupp.mapRange_single]; rw [Algebra.linearMap_apply]; rw [map_one]

Depends on / 依赖: Algebra, Algebra.linearMap, DFunLike, DFunLike.congr_fun, Finsupp, Finsupp.mapRange.linearMap, Function, Function.comp_apply, LinearEquiv, LinearEquiv.coe_coe, LinearMap, LinearMap.coe_comp, LinearMap.coe_restrictScalars, LinearMap.congr_fun, b.ofIsLocalizedModule, b.ofIsLocalizedModule_apply, b.repr.toLinearMap, coe_coe, coe_comp, coe_restrictScalars
-/
theorem ofIsLocalizedModule_repr_apply (m : M) (i : ι) :
    ((b.ofIsLocalizedModule Rₛ S f).repr (f m)) i = algebraMap R Rₛ (b.repr m i) := by
  suffices ((b.ofIsLocalizedModule Rₛ S f).repr.toLinearMap.restrictScalars R) ∘ₗ f =
      Finsupp.mapRange.linearMap (Algebra.linearMap R Rₛ) ∘ₗ b.repr.toLinearMap by
    exact DFunLike.congr_fun (LinearMap.congr_fun this m) i
  refine ext b fun i => ?_
  rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearMap.coe_restrictScalars]; rw [LinearEquiv.coe_coe]; rw [← b.ofIsLocalizedModule_apply Rₛ S f]; rw [repr_self]; rw [LinearMap.coe_comp]; rw [Function.comp_apply]; rw [LinearEquiv.coe_coe]; rw [repr_self]; rw [Finsupp.mapRange.linearMap_apply]; rw [Finsupp.mapRange_single]; rw [Algebra.linearMap_apply]; rw [map_one]

/--
theorem `ofIsLocalizedModule_span` / 定理 `ofIsLocalizedModule_span`

English:
theorem ofIsLocalizedModule_span
  proof: by
  calc span R (Set.range (b.ofIsLocalizedModule Rₛ S f))
    _ = span R (f '' (Set.range b)) := by congr; ext; simp
    _ = map f (span R (Set.range b)) := by rw [Submodule.map_span]
    _ = LinearMap.range f := by rw [b.span_eq, Submodule.map_top]

中文:
定理 ofIsLocalizedModule_span
  证明: by
  calc span R (Set.range (b.ofIsLocalizedModule Rₛ S f))
    _ = span R (f '' (Set.range b)) := by congr; ext; simp
    _ = map f (span R (Set.range b)) := by rw [Submodule.map_span]
    _ = LinearMap.range f := by rw [b.span_eq, Submodule.map_top]

Depends on / 依赖: LinearMap, LinearMap.range, Set.range, Submodule, Submodule.map_span, Submodule.map_top, b.ofIsLocalizedModule, b.span_eq, map_span, map_top, ofIsLocalizedModule, span_eq
-/
theorem ofIsLocalizedModule_span :
    span R (Set.range (b.ofIsLocalizedModule Rₛ S f)) = LinearMap.range f := by
  calc span R (Set.range (b.ofIsLocalizedModule Rₛ S f))
    _ = span R (f '' (Set.range b)) := by congr; ext; simp
    _ = map f (span R (Set.range b)) := by rw [Submodule.map_span]
    _ = LinearMap.range f := by rw [b.span_eq, Submodule.map_top]

end Module.Basis

end IsLocalizedModule

section LocalizationLocalization

variable [CommSemiring R] (S : Submonoid R) [CommSemiring Rₛ] [Algebra R Rₛ]
variable [IsLocalization S Rₛ]
variable {A : Type*} [CommSemiring A] [Algebra R A]
variable (Aₛ : Type*) [CommSemiring Aₛ] [Algebra A Aₛ]
variable [Algebra Rₛ Aₛ] [Algebra R Aₛ] [IsScalarTower R Rₛ Aₛ] [IsScalarTower R A Aₛ]
variable [IsLocalization (Algebra.algebraMapSubmonoid A S) Aₛ]

open Submodule

include S

/--
theorem `LinearIndependent.localization_localization` / 定理 `LinearIndependent.localization_localization`

English:
theorem LinearIndependent.localization_localization
  statement: {ι : Type*} {v : ι -> A}
  proof: hv.of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

中文:
定理 LinearIndependent.localization_localization
  结论: {ι : 类型} {v : ι -> A}
  证明: hv.of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

Depends on / 依赖: IsScalarTower, IsScalarTower.toAlgHom, hv.of_isLocalizedModule, of_isLocalizedModule, toAlgHom, toLinearMap
-/
theorem LinearIndependent.localization_localization {ι : Type*} {v : ι -> A}
    (hv : LinearIndependent R v) : LinearIndependent Rₛ (algebraMap A Aₛ ∘ v) :=
  hv.of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

/--
theorem `span_eq_top_localization_localization` / 定理 `span_eq_top_localization_localization`

English:
theorem span_eq_top_localization_localization
  given: {v : Set A} (hv : span R v = ⊤)
  proof: span_eq_top_of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap hv

中文:
定理 span_eq_top_localization_localization
  条件: {v : 集合 A} (hv : span R v = ⊤)
  证明: span_eq_top_of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap hv

Depends on / 依赖: IsScalarTower, IsScalarTower.toAlgHom, span_eq_top_of_isLocalizedModule, toAlgHom, toLinearMap
-/
theorem span_eq_top_localization_localization {v : Set A} (hv : span R v = ⊤) :
    span Rₛ (algebraMap A Aₛ '' v) = ⊤ :=
  span_eq_top_of_isLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap hv

namespace Module.Basis

/--
Definition of `localizationLocalization` / `localizationLocalization` 的定义

English:
definition localizationLocalization
  signature: {ι : Type*} (b : Basis ι R A)
  body: b.ofIsLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

@[simp]

中文:
定义 localizationLocalization
  签名: {ι : 类型} (b : 基 ι R A)
  定义体: b.ofIsLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

@[simp]

Depends on / 依赖: IsScalarTower, IsScalarTower.toAlgHom, b.ofIsLocalizedModule, ofIsLocalizedModule, toAlgHom, toLinearMap
-/
noncomputable def localizationLocalization {ι : Type*} (b : Basis ι R A) : Basis ι Rₛ Aₛ :=
  b.ofIsLocalizedModule Rₛ S (IsScalarTower.toAlgHom R A Aₛ).toLinearMap

@[simp]
/--
theorem `localizationLocalization_apply` / 定理 `localizationLocalization_apply`

English:
theorem localizationLocalization_apply
  given: {ι : Type*} (b : Basis ι R A) (i)
  proof: b.ofIsLocalizedModule_apply Rₛ S _ i

@[simp]

中文:
定理 localizationLocalization_apply
  条件: {ι : 类型} (b : 基 ι R A) (i)
  证明: b.ofIsLocalizedModule_apply Rₛ S _ i

@[simp]

Depends on / 依赖: b.ofIsLocalizedModule_apply, ofIsLocalizedModule_apply
-/
theorem localizationLocalization_apply {ι : Type*} (b : Basis ι R A) (i) :
    b.localizationLocalization Rₛ S Aₛ i = algebraMap A Aₛ (b i) :=
  b.ofIsLocalizedModule_apply Rₛ S _ i

@[simp]
/--
theorem `localizationLocalization_repr_algebraMap` / 定理 `localizationLocalization_repr_algebraMap`

English:
theorem localizationLocalization_repr_algebraMap
  given: {ι : Type*} (b : Basis ι R A) (x i)
  proof: b.ofIsLocalizedModule_repr_apply Rₛ S _ _ i

中文:
定理 localizationLocalization_repr_algebraMap
  条件: {ι : 类型} (b : 基 ι R A) (x i)
  证明: b.ofIsLocalizedModule_repr_apply Rₛ S _ _ i

Depends on / 依赖: b.ofIsLocalizedModule_repr_apply, ofIsLocalizedModule_repr_apply
-/
theorem localizationLocalization_repr_algebraMap {ι : Type*} (b : Basis ι R A) (x i) :
    (b.localizationLocalization Rₛ S Aₛ).repr (algebraMap A Aₛ x) i =
      algebraMap R Rₛ (b.repr x i) := b.ofIsLocalizedModule_repr_apply Rₛ S _ _ i

/--
theorem `localizationLocalization_span` / 定理 `localizationLocalization_span`

English:
theorem localizationLocalization_span
  given: {ι : Type*} (b : Basis ι R A)
  proof: b.ofIsLocalizedModule_span Rₛ S _

中文:
定理 localizationLocalization_span
  条件: {ι : 类型} (b : 基 ι R A)
  证明: b.ofIsLocalizedModule_span Rₛ S _

Depends on / 依赖: b.ofIsLocalizedModule_span, ofIsLocalizedModule_span
-/
theorem localizationLocalization_span {ι : Type*} (b : Basis ι R A) :
    Submodule.span R (Set.range (b.localizationLocalization Rₛ S Aₛ)) =
      LinearMap.range (IsScalarTower.toAlgHom R A Aₛ : A ->ₗ[R] Aₛ) :=
  b.ofIsLocalizedModule_span Rₛ S _

end Module.Basis
end LocalizationLocalization


section FractionRing

variable (R K : Type*) [CommRing R] [CommRing K] [Algebra R K] [IsFractionRing R K]
variable {V : Type*} [AddCommGroup V] [Module R V] [Module K V] [IsScalarTower R K V]

/--
theorem `LinearIndependent.iff_fractionRing` / 定理 `LinearIndependent.iff_fractionRing`

English:
theorem LinearIndependent.iff_fractionRing
  given: {ι : Type*} {b : ι -> V}
  proof: ⟨.localization K R⁰,
.restrict_scalars (faithfulSMul_iff_injective_smul_one ..).mp inferInstance⟩

中文:
定理 LinearIndependent.iff_fractionRing
  条件: {ι : 类型} {b : ι -> V}
  证明: ⟨.localization K R⁰,
.restrict_scalars (faithfulSMul_iff_injective_smul_one ..).mp inferInstance⟩

Depends on / 依赖: faithfulSMul_iff_injective_smul_one, localization, restrict_scalars
-/
theorem LinearIndependent.iff_fractionRing {ι : Type*} {b : ι -> V} :
    LinearIndependent R b ↔ LinearIndependent K b :=
  ⟨.localization K R⁰,
.restrict_scalars (faithfulSMul_iff_injective_smul_one ..).mp inferInstance⟩

end FractionRing

section

variable {R : Type*} [CommSemiring R] (S : Submonoid R)
variable (A : Type*) [CommSemiring A] [Algebra R A] [IsLocalization S A]
variable {M N : Type*}
  [AddCommMonoid M] [Module R M] [Module A M] [IsScalarTower R A M]
  [AddCommMonoid N] [Module R N] [Module A N] [IsScalarTower R A N]

open IsLocalization

/--
Definition of `LinearMap.extendScalarsOfIsLocalization` / `LinearMap.extendScalarsOfIsLocalization` 的定义

English:
definition LinearMap.extendScalarsOfIsLocalization
  signature: (f : M ->ₗ[R] N)
  body: f
  map_add' := f.map_add
  map_smul' := (IsLocalization.linearMap_compatibleSMul S A M N).map_smul _

中文:
定义 线性映射.extendScalarsOfIsLocalization
  签名: (f : M ->ₗ[R] N)
  定义体: f
  map_add' := f.map_add
  map_smul' := (IsLocalization.linearMap_compatibleSMul S A M N).map_smul _
-/
def LinearMap.extendScalarsOfIsLocalization (f : M ->ₗ[R] N) : M ->ₗ[A] N where
  toFun := f
  map_add' := f.map_add
  map_smul' := (IsLocalization.linearMap_compatibleSMul S A M N).map_smul _

/--
lemma `LinearMap.restrictScalars_extendScalarsOfIsLocalization` / 引理 `LinearMap.restrictScalars_extendScalarsOfIsLocalization`

English:
lemma LinearMap.restrictScalars_extendScalarsOfIsLocalization
  given: (f : M ->ₗ[R] N)
  proof: rfl

中文:
引理 线性映射.restrictScalars_extendScalarsOfIsLocalization
  条件: (f : M ->ₗ[R] N)
  证明: rfl
-/
@[simp] lemma LinearMap.restrictScalars_extendScalarsOfIsLocalization (f : M ->ₗ[R] N) :
    (f.extendScalarsOfIsLocalization S A).restrictScalars R = f := rfl

/--
lemma `LinearMap.extendScalarsOfIsLocalization_apply` / 引理 `LinearMap.extendScalarsOfIsLocalization_apply`

English:
lemma LinearMap.extendScalarsOfIsLocalization_apply
  given: (f : M ->ₗ[A] N)
  proof: rfl

中文:
引理 线性映射.extendScalarsOfIsLocalization_apply
  条件: (f : M ->ₗ[A] N)
  证明: rfl
-/
@[simp] lemma LinearMap.extendScalarsOfIsLocalization_apply (f : M ->ₗ[A] N) :
    f.extendScalarsOfIsLocalization S A = f := rfl

/--
lemma `LinearMap.extendScalarsOfIsLocalization_apply'` / 引理 `LinearMap.extendScalarsOfIsLocalization_apply'`

English:
lemma LinearMap.extendScalarsOfIsLocalization_apply'
  given: (f : M ->ₗ[R] N) (x : M)
  proof: rfl

中文:
引理 线性映射.extendScalarsOfIsLocalization_apply'
  条件: (f : M ->ₗ[R] N) (x : M)
  证明: rfl
-/
@[simp] lemma LinearMap.extendScalarsOfIsLocalization_apply' (f : M ->ₗ[R] N) (x : M) :
    (f.extendScalarsOfIsLocalization S A) x = f x := rfl

/-- The `S⁻¹R`-linear maps between two `S⁻¹R`-modules are exactly the `R`-linear maps. -/
@[simps]
/--
Definition of `LinearMap.extendScalarsOfIsLocalizationEquiv` / `LinearMap.extendScalarsOfIsLocalizationEquiv` 的定义

English:
definition LinearMap.extendScalarsOfIsLocalizationEquiv
  signature: : (M ->ₗ[R] N) ≃ₗ[A] (M ->ₗ[A] N) where
  body: LinearMap.extendScalarsOfIsLocalization S A
  invFun := LinearMap.restrictScalars R
  map_add' := by intros; ext; simp
  map_smul' := by intros; ext; simp
  left_inv := by intro _; ext; simp
  right_inv := by intro _; ext; simp

中文:
定义 线性映射.extendScalarsOfIsLocalizationEquiv
  签名: : (M ->ₗ[R] N) ≃ₗ[A] (M ->ₗ[A] N) where
  定义体: LinearMap.extendScalarsOfIsLocalization S A
  invFun := LinearMap.restrictScalars R
  map_add' := by intros; ext; simp
  map_smul' := by intros; ext; simp
  left_inv := by intro _; ext; simp
  right_inv := by intro _; ext; simp

Depends on / 依赖: LinearMap, LinearMap.extendScalarsOfIsLocalization, extendScalarsOfIsLocalization
-/
def LinearMap.extendScalarsOfIsLocalizationEquiv : (M ->ₗ[R] N) ≃ₗ[A] (M ->ₗ[A] N) where
  toFun := LinearMap.extendScalarsOfIsLocalization S A
  invFun := LinearMap.restrictScalars R
  map_add' := by intros; ext; simp
  map_smul' := by intros; ext; simp
  left_inv := by intro _; ext; simp
  right_inv := by intro _; ext; simp

/-- An `R`-linear isomorphism between `S⁻¹R`-modules is actually `S⁻¹R`-linear. -/
@[simps!]
/--
Definition of `LinearEquiv.extendScalarsOfIsLocalization` / `LinearEquiv.extendScalarsOfIsLocalization` 的定义

English:
definition LinearEquiv.extendScalarsOfIsLocalization
  signature: (f : M ≃ₗ[R] N)
  body: .ofLinearMap (LinearMap.extendScalarsOfIsLocalization S A f)
    (LinearMap.extendScalarsOfIsLocalization S A f.symm)
    (by ext; simp) (by ext; simp)

中文:
定义 线性等价.extendScalarsOfIsLocalization
  签名: (f : M ≃ₗ[R] N)
  定义体: .ofLinearMap (LinearMap.extendScalarsOfIsLocalization S A f)
    (LinearMap.extendScalarsOfIsLocalization S A f.symm)
    (by ext; simp) (by ext; simp)

Depends on / 依赖: LinearMap, LinearMap.extendScalarsOfIsLocalization, extendScalarsOfIsLocalization, f.symm, ofLinearMap
-/
def LinearEquiv.extendScalarsOfIsLocalization (f : M ≃ₗ[R] N) : M ≃ₗ[A] N :=
  .ofLinearMap (LinearMap.extendScalarsOfIsLocalization S A f)
    (LinearMap.extendScalarsOfIsLocalization S A f.symm)
    (by ext; simp) (by ext; simp)

/-- The `S⁻¹R`-linear isomorphisms between two `S⁻¹R`-modules are exactly the `R`-linear
isomorphisms. -/
@[simps]
/--
Definition of `LinearEquiv.extendScalarsOfIsLocalizationEquiv` / `LinearEquiv.extendScalarsOfIsLocalizationEquiv` 的定义

English:
definition LinearEquiv.extendScalarsOfIsLocalizationEquiv
  signature: : (M ≃ₗ[R] N) ≃ M ≃ₗ[A] N where
  body: e.extendScalarsOfIsLocalization S A
  invFun e := e.restrictScalars R
  left_inv e := by ext; simp
  right_inv e := by ext; simp

中文:
定义 线性等价.extendScalarsOfIsLocalizationEquiv
  签名: : (M ≃ₗ[R] N) ≃ M ≃ₗ[A] N where
  定义体: e.extendScalarsOfIsLocalization S A
  invFun e := e.restrictScalars R
  left_inv e := by ext; simp
  right_inv e := by ext; simp

Depends on / 依赖: e.extendScalarsOfIsLocalization, extendScalarsOfIsLocalization
-/
def LinearEquiv.extendScalarsOfIsLocalizationEquiv : (M ≃ₗ[R] N) ≃ M ≃ₗ[A] N where
  toFun e := e.extendScalarsOfIsLocalization S A
  invFun e := e.restrictScalars R
  left_inv e := by ext; simp
  right_inv e := by ext; simp

end

end Localization

namespace IsLocalizedModule

variable {R : Type*} [CommSemiring R] (S : Submonoid R)
variable {M M' : Type*} [AddCommMonoid M] [AddCommMonoid M']
variable [Module R M] [Module R M']
variable (f : M ->ₗ[R] M') [IsLocalizedModule S f]
variable {N N'} [AddCommMonoid N] [AddCommMonoid N'] [Module R N] [Module R N']
variable (g : N ->ₗ[R] N') [IsLocalizedModule S g]
variable (Rₛ) [CommSemiring Rₛ] [Algebra R Rₛ] [Module Rₛ M'] [Module Rₛ N']
variable [IsScalarTower R Rₛ M'] [IsScalarTower R Rₛ N'] [IsLocalization S Rₛ]

/-- A linear map `M →ₗ[R] N` gives a map between localized modules `Mₛ →ₗ[Rₛ] Nₛ`. -/
@[simps!]
noncomputable
/--
Definition of `mapExtendScalars` / `mapExtendScalars` 的定义

English:
definition mapExtendScalars
  signature: : (M ->ₗ[R] N) ->ₗ[R] (M' ->ₗ[Rₛ] N')
  body: ((LinearMap.extendScalarsOfIsLocalizationEquiv S Rₛ).restrictScalars R).toLinearMap ∘ₗ map S f g

中文:
定义 mapExtendScalars
  签名: : (M ->ₗ[R] N) ->ₗ[R] (M' ->ₗ[Rₛ] N')
  定义体: ((LinearMap.extendScalarsOfIsLocalizationEquiv S Rₛ).restrictScalars R).toLinearMap ∘ₗ map S f g

Depends on / 依赖: LinearMap, LinearMap.extendScalarsOfIsLocalizationEquiv, extendScalarsOfIsLocalizationEquiv, restrictScalars, toLinearMap
-/
def mapExtendScalars : (M ->ₗ[R] N) ->ₗ[R] (M' ->ₗ[Rₛ] N') :=
  ((LinearMap.extendScalarsOfIsLocalizationEquiv S Rₛ).restrictScalars R).toLinearMap ∘ₗ map S f g

/-- An `R`-module isomorphism `M ≃ₗ[R] N` gives an `Rₛ`-module isomorphism `Mₛ ≃ₗ[Rₛ] Nₛ`. -/
@[simps!]
noncomputable
/--
Definition of `mapEquiv` / `mapEquiv` 的定义

English:
definition mapEquiv
  signature: (e : M ≃ₗ[R] N)
  body: LinearEquiv.ofLinearMap
    (IsLocalizedModule.mapExtendScalars S f g Rₛ e)
    (IsLocalizedModule.mapExtendScalars S g f Rₛ e.symm)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S g g
      ext; simp)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S f f
      ext; simp)

中文:
定义 mapEquiv
  签名: (e : M ≃ₗ[R] N)
  定义体: LinearEquiv.ofLinearMap
    (IsLocalizedModule.mapExtendScalars S f g Rₛ e)
    (IsLocalizedModule.mapExtendScalars S g f Rₛ e.symm)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S g g
      ext; simp)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S f f
      ext; simp)

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.linearMap_ext, IsLocalizedModule.mapExtendScalars, LinearEquiv, LinearEquiv.ofLinearMap, LinearMap, LinearMap.restrictScalars_injective, e.symm, linearMap_ext, mapExtendScalars, ofLinearMap, restrictScalars_injective
-/
def mapEquiv (e : M ≃ₗ[R] N) : M' ≃ₗ[Rₛ] N' :=
  LinearEquiv.ofLinearMap
    (IsLocalizedModule.mapExtendScalars S f g Rₛ e)
    (IsLocalizedModule.mapExtendScalars S g f Rₛ e.symm)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S g g
      ext; simp)
    (by
      apply LinearMap.restrictScalars_injective R
      apply IsLocalizedModule.linearMap_ext S f f
      ext; simp)

end IsLocalizedModule

section LocalizedModule

variable {R : Type*} [CommSemiring R] (S : Submonoid R)
variable {M : Type*} [AddCommMonoid M] [Module R M]
variable {N} [AddCommMonoid N] [Module R N]

/-- A linear map `M →ₗ[R] N` gives a map between localized modules `Mₛ →ₗ[Rₛ] Nₛ`. -/
noncomputable
/--
Definition of `LocalizedModule.map` / `LocalizedModule.map` 的定义

English:
definition LocalizedModule.map
  signature: :
  body: IsLocalizedModule.mapExtendScalars S (LocalizedModule.mkLinearMap S M)
    (LocalizedModule.mkLinearMap S N) (Localization S)

@[simp]

中文:
定义 LocalizedModule.map
  签名: :
  定义体: IsLocalizedModule.mapExtendScalars S (LocalizedModule.mkLinearMap S M)
    (LocalizedModule.mkLinearMap S N) (Localization S)

@[simp]

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.mapExtendScalars, Localization, LocalizedModule, LocalizedModule.mkLinearMap, mapExtendScalars, mkLinearMap
-/
def LocalizedModule.map :
    (M ->ₗ[R] N) ->ₗ[R] (LocalizedModule S M ->ₗ[Localization S] LocalizedModule S N) :=
  IsLocalizedModule.mapExtendScalars S (LocalizedModule.mkLinearMap S M)
    (LocalizedModule.mkLinearMap S N) (Localization S)

@[simp]
/--
lemma `LocalizedModule.map_mk` / 引理 `LocalizedModule.map_mk`

English:
lemma LocalizedModule.map_mk
  given: (f : M ->ₗ[R] N) (x y)
  proof: by
  rw [IsLocalizedModule.mk_eq_mk']; rw [IsLocalizedModule.mk_eq_mk']
  exact IsLocalizedModule.map_mk' _ _ _ _ _ _

@[simp]

中文:
引理 LocalizedModule.map_mk
  条件: (f : M ->ₗ[R] N) (x y)
  证明: by
  rw [IsLocalizedModule.mk_eq_mk']; rw [IsLocalizedModule.mk_eq_mk']
  exact IsLocalizedModule.map_mk' _ _ _ _ _ _

@[simp]

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.map_mk, IsLocalizedModule.mk_eq_mk, map_mk, mk_eq_mk
-/
lemma LocalizedModule.map_mk (f : M ->ₗ[R] N) (x y) :
    map S f (.mk x y) = LocalizedModule.mk (f x) y := by
  rw [IsLocalizedModule.mk_eq_mk']; rw [IsLocalizedModule.mk_eq_mk']
  exact IsLocalizedModule.map_mk' _ _ _ _ _ _

@[simp]
/--
lemma `LocalizedModule.map_id` / 引理 `LocalizedModule.map_id`

English:
lemma LocalizedModule.map_id
  proof: LinearMap.ext fun x => LinearMap.congr_fun (IsLocalizedModule.map_id S (mkLinearMap S M)) x

中文:
引理 LocalizedModule.map_id
  证明: LinearMap.ext fun x => LinearMap.congr_fun (IsLocalizedModule.map_id S (mkLinearMap S M)) x

Depends on / 依赖: LinearMap, LinearMap.id
-/
lemma LocalizedModule.map_id :
    LocalizedModule.map S (.id (R := R) (M := M)) = LinearMap.id :=
  LinearMap.ext fun x => LinearMap.congr_fun (IsLocalizedModule.map_id S (mkLinearMap S M)) x

/--
lemma `LocalizedModule.map_injective` / 引理 `LocalizedModule.map_injective`

English:
lemma LocalizedModule.map_injective
  given: (l : M ->ₗ[R] N) (hl : Function.Injective l)
  proof: IsLocalizedModule.map_injective S (mkLinearMap S M) (mkLinearMap S N) l hl

中文:
引理 LocalizedModule.map_injective
  条件: (l : M ->ₗ[R] N) (hl : 函数.单射 l)
  证明: IsLocalizedModule.map_injective S (mkLinearMap S M) (mkLinearMap S N) l hl

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.map_injective, map_injective, mkLinearMap
-/
lemma LocalizedModule.map_injective (l : M ->ₗ[R] N) (hl : Function.Injective l) :
    Function.Injective (map S l) :=
  IsLocalizedModule.map_injective S (mkLinearMap S M) (mkLinearMap S N) l hl

/--
lemma `LocalizedModule.map_surjective` / 引理 `LocalizedModule.map_surjective`

English:
lemma LocalizedModule.map_surjective
  given: (l : M ->ₗ[R] N) (hl : Function.Surjective l)
  proof: IsLocalizedModule.map_surjective S (mkLinearMap S M) (mkLinearMap S N) l hl

中文:
引理 LocalizedModule.map_surjective
  条件: (l : M ->ₗ[R] N) (hl : 函数.满射 l)
  证明: IsLocalizedModule.map_surjective S (mkLinearMap S M) (mkLinearMap S N) l hl

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.map_surjective, map_surjective, mkLinearMap
-/
lemma LocalizedModule.map_surjective (l : M ->ₗ[R] N) (hl : Function.Surjective l) :
    Function.Surjective (map S l) :=
  IsLocalizedModule.map_surjective S (mkLinearMap S M) (mkLinearMap S N) l hl

/--
lemma `LocalizedModule.restrictScalars_map_eq` / 引理 `LocalizedModule.restrictScalars_map_eq`

English:
lemma LocalizedModule.restrictScalars_map_eq
  statement: {M' N' : Type*} [AddCommMonoid M'] [AddCommMonoid N']
  proof: by
  rw [LinearEquiv.eq_toLinearMap_symm_comp]; rw [← LinearEquiv.comp_toLinearMap_symm_eq]
  apply IsLocalizedModule.linearMap_ext S g₁ g₂
  rw [LinearMap.comp_assoc]; rw [IsLocalizedModule.iso_symm_comp]
  ext
  simp

中文:
引理 LocalizedModule.restrictScalars_map_eq
  结论: {M' N' : 类型} [加法交换幺半群 M'] [加法交换幺半群 N']
  证明: by
  rw [LinearEquiv.eq_toLinearMap_symm_comp]; rw [← LinearEquiv.comp_toLinearMap_symm_eq]
  apply IsLocalizedModule.linearMap_ext S g₁ g₂
  rw [LinearMap.comp_assoc]; rw [IsLocalizedModule.iso_symm_comp]
  ext
  simp

Depends on / 依赖: IsLocalizedModule, IsLocalizedModule.iso_symm_comp, IsLocalizedModule.linearMap_ext, LinearEquiv, LinearEquiv.comp_toLinearMap_symm_eq, LinearEquiv.eq_toLinearMap_symm_comp, LinearMap, LinearMap.comp_assoc, comp_assoc, comp_toLinearMap_symm_eq, eq_toLinearMap_symm_comp, iso_symm_comp, linearMap_ext
-/
lemma LocalizedModule.restrictScalars_map_eq {M' N' : Type*} [AddCommMonoid M'] [AddCommMonoid N']
    [Module R M'] [Module R N'] (g₁ : M ->ₗ[R] M') (g₂ : N ->ₗ[R] N')
    [IsLocalizedModule S g₁] [IsLocalizedModule S g₂]
    (l : M ->ₗ[R] N) :
    (map S l).restrictScalars R = (IsLocalizedModule.iso S g₂).symm ∘ₗ
      IsLocalizedModule.map S g₁ g₂ l ∘ₗ IsLocalizedModule.iso S g₁ := by
  rw [LinearEquiv.eq_toLinearMap_symm_comp]; rw [← LinearEquiv.comp_toLinearMap_symm_eq]
  apply IsLocalizedModule.linearMap_ext S g₁ g₂
  rw [LinearMap.comp_assoc]; rw [IsLocalizedModule.iso_symm_comp]
  ext
  simp

variable {S} in
/--
lemma `LocalizedModule.coe_map_eq` / 引理 `LocalizedModule.coe_map_eq`

English:
lemma LocalizedModule.coe_map_eq
  statement: {M' N' : Type*} [AddCommMonoid M'] [AddCommMonoid N']
  proof: by
  rw [← LinearMap.coe_restrictScalars R]; rw [restrictScalars_map_eq _ g₁ g₂ l]
  simp

中文:
引理 LocalizedModule.coe_map_eq
  结论: {M' N' : 类型} [加法交换幺半群 M'] [加法交换幺半群 N']
  证明: by
  rw [← LinearMap.coe_restrictScalars R]; rw [restrictScalars_map_eq _ g₁ g₂ l]
  simp

Depends on / 依赖: LinearMap, LinearMap.coe_restrictScalars, coe_restrictScalars, restrictScalars_map_eq
-/
lemma LocalizedModule.coe_map_eq {M' N' : Type*} [AddCommMonoid M'] [AddCommMonoid N']
    [Module R M'] [Module R N'] (g₁ : M ->ₗ[R] M') (g₂ : N ->ₗ[R] N')
    [IsLocalizedModule S g₁] [IsLocalizedModule S g₂] (l : M ->ₗ[R] N) :
    ⇑(map S l) = (IsLocalizedModule.iso S g₂).symm ∘
      IsLocalizedModule.map S g₁ g₂ l ∘ IsLocalizedModule.iso S g₁ := by
  rw [← LinearMap.coe_restrictScalars R]; rw [restrictScalars_map_eq _ g₁ g₂ l]
  simp

end LocalizedModule

namespace IsLocalizedModule

variable {R M N M' N' : Type*} [CommSemiring R] {S : Submonoid R}
  [AddCommMonoid M] [Module R M] [AddCommMonoid N] [Module R N]
  [AddCommMonoid M'] [Module R M'] [AddCommMonoid N'] [Module R N']
  (g₁ : M ->ₗ[R] M') (g₂ : N ->ₗ[R] N')
  [IsLocalizedModule S g₁] [IsLocalizedModule S g₂] {l : M ->ₗ[R] N}

/--
lemma `map_injective_iff_localizedModuleMap_injective` / 引理 `map_injective_iff_localizedModuleMap_injective`

English:
lemma map_injective_iff_localizedModuleMap_injective
  proof: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

中文:
引理 map_injective_iff_localizedModuleMap_injective
  证明: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

Depends on / 依赖: LocalizedModule, LocalizedModule.coe_map_eq, coe_map_eq
-/
lemma map_injective_iff_localizedModuleMap_injective :
    Function.Injective (IsLocalizedModule.map S g₁ g₂ l) ↔
      Function.Injective (LocalizedModule.map S l) := by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

/--
lemma `map_surjective_iff_localizedModuleMap_surjective` / 引理 `map_surjective_iff_localizedModuleMap_surjective`

English:
lemma map_surjective_iff_localizedModuleMap_surjective
  proof: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

中文:
引理 map_surjective_iff_localizedModuleMap_surjective
  证明: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

Depends on / 依赖: LocalizedModule, LocalizedModule.coe_map_eq, coe_map_eq
-/
lemma map_surjective_iff_localizedModuleMap_surjective :
    Function.Surjective (IsLocalizedModule.map S g₁ g₂ l) ↔
      Function.Surjective (LocalizedModule.map S l) := by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

/--
lemma `map_bijective_iff_localizedModuleMap_bijective` / 引理 `map_bijective_iff_localizedModuleMap_bijective`

English:
lemma map_bijective_iff_localizedModuleMap_bijective
  proof: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

中文:
引理 map_bijective_iff_localizedModuleMap_bijective
  证明: by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

Depends on / 依赖: LocalizedModule, LocalizedModule.coe_map_eq, coe_map_eq
-/
lemma map_bijective_iff_localizedModuleMap_bijective :
    Function.Bijective (IsLocalizedModule.map S g₁ g₂ l) ↔
      Function.Bijective (LocalizedModule.map S l) := by
  simp [LocalizedModule.coe_map_eq g₁ g₂]

end IsLocalizedModule
