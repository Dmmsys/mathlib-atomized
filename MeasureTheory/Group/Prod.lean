...
/--
Definition of `MeasurableEquiv.shearMulRight` / `MeasurableEquiv.shearMulRight` 的定义

English:
definition MeasurableEquiv.shearMulRight
  signature: [MeasurableInv G]
...
/--
Definition of `MeasurableEquiv.shearDivRight` / `MeasurableEquiv.shearDivRight` 的定义

English:
definition MeasurableEquiv.shearDivRight
  signature: [MeasurableInv G]
...
/--
theorem `measurePreserving_prod_mul` / 定理 `measurePreserving_prod_mul`

English:
theorem measurePreserving_prod_mul
  given: [IsMulLeftInvariant ν]
  proof: (MeasurePreserving.id μ).skew_product measurable_mul
...
Depends on / 依赖: Eventually, Filter, Filter.Eventually.of_forall, MeasurePreserving, MeasurePreserving.id, map_mul_left_eq_self, measurable_mul, of_forall, skew_product
-/
...
/--
theorem `measurePreserving_prod_mul_swap` / 定理 `measurePreserving_prod_mul_swap`

English:
theorem measurePreserving_prod_mul_swap
  given: [IsMulLeftInvariant μ]
  proof: (measurePreserving_prod_mul ν μ).comp measurePreserving_swap
...
Depends on / 依赖: measurePreserving_prod_mul, measurePreserving_swap
-/
...
@[to_additive]
/--
theorem `measurable_measure_mul_right` / 定理 `measurable_measure_mul_right`
...
/-- The map `(x, y) ↦ (x, - x + y)` is measure-preserving. -/]
/--
theorem `measurePreserving_prod_inv_mul` / 定理 `measurePreserving_prod_inv_mul`

English:
theorem measurePreserving_prod_inv_mul
  given: [IsMulLeftInvariant ν]
  proof: (measurePreserving_prod_mul μ ν).symm MeasurableEquiv.shearMulRight G
...
/-- The map `(x, y) ↦ (y, - y + x)` sends `μ × ν` to `ν × μ`. -/]
/--
theorem `measurePreserving_prod_inv_mul_swap` / 定理 `measurePreserving_prod_inv_mul_swap`

English:
theorem measurePreserving_prod_inv_mul_swap
  proof: (measurePreserving_prod_inv_mul ν μ).comp measurePreserving_swap

中文:
定理 measurePreserving_prod_inv_mul_swap
  证明: (measurePreserving_prod_inv_mul ν μ).comp measurePreserving_swap

Depends on / 依赖: measurePreserving_prod_inv_mul, measurePreserving_swap
-/
theorem measurePreserving_prod_inv_mul_swap :
    MeasurePreserving (fun z : G × G => (z.2, z.2⁻¹ * z.1)) (μ.prod ν) (ν.prod μ) :=
...
/--
theorem `measurePreserving_mul_prod_inv` / 定理 `measurePreserving_mul_prod_inv`

English:
theorem measurePreserving_mul_prod_inv
  given: [IsMulLeftInvariant ν]
  proof: by
  convert!
...
Depends on / 依赖: Function, Function.comp_apply, comp_apply, convert, inv_mul_cancel_right, measurePreserving_prod_inv_mul_swap, measurePreserving_prod_mul_swap, mul_inv_rev, simp_rw
-/
...
@[to_additive (attr := fun_prop)]
/--
theorem `quasiMeasurePreserving_inv` / 定理 `quasiMeasurePreserving_inv`

English:
theorem quasiMeasurePreserving_inv
  statement: QuasiMeasurePreserving (Inv.inv : G -> G) μ μ
  proof: by
...
/--
theorem `measure_inv_null` / 定理 `measure_inv_null`

English:
theorem measure_inv_null
  statement: μ s⁻¹ = 0 ↔ μ s = 0
  proof: by
...
Depends on / 依赖: inv_inv, preimage_null, quasiMeasurePreserving_inv
-/
...
Depends on / 依赖: Filter, Filter.map_mono, inv_inv, le_antisymm, map_mono, nth_rewrite, quasiMeasurePreserving_inv, tendsto_ae
-/
...
@[to_additive (attr := simp)]
/--
theorem `eventuallyConst_inv_set_ae` / 定理 `eventuallyConst_inv_set_ae`

English:
theorem eventuallyConst_inv_set_ae
  proof: by
  rw [← inv_preimage]; rw [eventuallyConst_preimage]; rw [Filter.map_inv]; rw [inv_ae]

@[to_additive]

中文:
定理 eventuallyConst_inv_set_ae
  证明: by
  rw [← inv_preimage]; rw [eventuallyConst_preimage]; rw [Filter.map_inv]; rw [inv_ae]

@[to_additive]

Depends on / 依赖: Filter, Filter.map_inv, eventuallyConst_preimage, inv_ae, inv_preimage, map_inv
-/
theorem eventuallyConst_inv_set_ae :
...
@[to_additive]
/--
theorem `inv_absolutelyContinuous` / 定理 `inv_absolutelyContinuous`

English:
theorem inv_absolutelyContinuous
  statement: μ.inv ≪ μ
  proof: (quasiMeasurePreserving_inv μ).absolutelyContinuous

@[to_additive]

中文:
定理 inv_absolutelyContinuous
  结论: μ.inv ≪ μ
  证明: (quasiMeasurePreserving_inv μ).absolutelyContinuous

@[to_additive]

Depends on / 依赖: absolutelyContinuous, quasiMeasurePreserving_inv
-/
...
@[to_additive]
/--
theorem `absolutelyContinuous_inv` / 定理 `absolutelyContinuous_inv`

English:
theorem absolutelyContinuous_inv
  statement: μ ≪ μ.inv
  proof: by
...
Depends on / 依赖: AbsolutelyContinuous, AbsolutelyContinuous.mk, imp_self, inv_apply, measure_inv_null, simp_rw
-/
...
@[to_additive]
/--
theorem `lintegral_lintegral_mul_inv` / 定理 `lintegral_lintegral_mul_inv`

English:
theorem lintegral_lintegral_mul_inv
  statement: [IsMulLeftInvariant ν] (f : G -> G -> Real>=0∞)
  proof: by
...
-/
theorem lintegral_lintegral_mul_inv [IsMulLeftInvariant ν] (f : G -> G -> Real>=0∞)
...
@[to_additive]
/--
theorem `measure_mul_right_null` / 定理 `measure_mul_right_null`

English:
theorem measure_mul_right_null
  given: (y : G)
  statement: μ ((fun x => x * y) ⁻¹' s) = 0 ↔ μ s = 0
  proof: calc
...
中文:
定理 measure_mul_right_null
  条件: (y : G)
  结论: μ ((fun x => x * y) ⁻¹' s) = 0 ↔ μ s = 0
  证明: calc
    μ ((fun x => x * y) ⁻¹' s) = 0 ↔ μ ((fun x => y⁻¹ * x) ⁻¹' s⁻¹)⁻¹ = 0 := by
      simp_rw [← inv_preimage, preimage_preimage, mul_inv_rev, inv_inv]
    _ ↔ μ s = 0 := by simp only [measure_inv_null μ, measure_preimage_mul]
...
Depends on / 依赖: decidable_of_iff, inv_inv, inv_preimage, irrational_sqrt_ofNat_iff, measure_inv_null, measure_preimage_mul, mul_inv_rev, preimage_preimage, simp_rw
-/
...
@[to_additive]
/--
theorem `measure_mul_right_ne_zero` / 定理 `measure_mul_right_ne_zero`

English:
theorem measure_mul_right_ne_zero
  given: (h2s : μ s != 0) (y : G)
  statement: μ ((fun x => x * y) ⁻¹' s) != 0
  proof: (not_congr (measure_mul_right_null μ y)).mpr h2s

@[to_additive]

中文:
定理 measure_mul_right_ne_zero
  条件: (h2s : μ s != 0) (y : G)
  结论: μ ((fun x => x * y) ⁻¹' s) != 0
  证明: (not_congr (measure_mul_right_null μ y)).mpr h2s

@[to_additive]

Depends on / 依赖: decidable_of_iff, irrational_sqrt_natCast_iff, measure_mul_right_null, not_congr
-/
...
@[to_additive]
/--
theorem `absolutelyContinuous_map_mul_right` / 定理 `absolutelyContinuous_map_mul_right`

English:
theorem absolutelyContinuous_map_mul_right
  given: (g : G)
  statement: μ ≪ map (· * g) μ
  proof: by
...
中文:
定理 absolutelyContinuous_map_mul_right
  条件: (g : G)
  结论: μ ≪ map (· * g) μ
  证明: by
  refine AbsolutelyContinuous.mk fun s hs => ?_
...
Depends on / 依赖: AbsolutelyContinuous, AbsolutelyContinuous.mk, decidable_of_iff, irrational_sqrt_intCast_iff, map_apply, measurable_mul_const, measure_mul_right_null
-/
...
@[to_additive]
/--
theorem `absolutelyContinuous_map_div_left` / 定理 `absolutelyContinuous_map_div_left`

English:
theorem absolutelyContinuous_map_div_left
  given: (g : G)
  statement: μ ≪ map (fun h => g / h) μ
  proof: by
...
中文:
定理 absolutelyContinuous_map_div_left
  条件: (g : G)
  结论: μ ≪ map (fun h => g / h) μ
  证明: by
  simp_rw [div_eq_mul_inv]
  have := map_map (μ := μ) (measurable_const_mul g) measurable_inv
  simp only [Function.comp_def] at this
...
Depends on / 依赖: Function, Function.comp_def, absolutelyContinuous_inv, comp_def, conv_lhs, decidable_of_iff, div_eq_mul_inv, irrational_sqrt_ratCast_iff, map_map, map_mul_left_eq_self, measurable_const_mul, measurable_inv, simp_rw
-/
...
/--
theorem `measure_mul_lintegral_eq` / 定理 `measure_mul_lintegral_eq`

English:
theorem measure_mul_lintegral_eq
  statement: [IsMulLeftInvariant ν] (sm : MeasurableSet s) (f : G -> Real>=0∞)
  proof: by
...
-/
theorem measure_mul_lintegral_eq [IsMulLeftInvariant ν] (sm : MeasurableSet s) (f : G -> Real>=0∞)
...
/-- Any two nonzero left-invariant measures are absolutely continuous w.r.t. each other. -/]
/--
theorem `absolutelyContinuous_of_isMulLeftInvariant` / 定理 `absolutelyContinuous_of_isMulLeftInvariant`

English:
theorem absolutelyContinuous_of_isMulLeftInvariant
  given: [IsMulLeftInvariant ν] (hν : ν != 0)
  statement: μ ≪ ν
  proof: by
...
@[to_additive]
/--
theorem `ae_measure_preimage_mul_right_lt_top` / 定理 `ae_measure_preimage_mul_right_lt_top`

English:
theorem ae_measure_preimage_mul_right_lt_top
  given: (hμs : μ' s != ∞)
  proof: by
...
@[to_additive]
/--
theorem `ae_measure_preimage_mul_right_lt_top_of_ne_zero` / 定理 `ae_measure_preimage_mul_right_lt_top_of_ne_zero`

English:
theorem ae_measure_preimage_mul_right_lt_top_of_ne_zero
  given: (h2s : ν' s != 0) (h3s : ν' s != ∞)
  proof: by
...
中文:
定理 ae_measure_preimage_mul_right_lt_top_of_ne_zero
  条件: (h2s : ν' s != 0) (h3s : ν' s != ∞)
  证明: by
  refine (ae_measure_preimage_mul_right_lt_top ν' ν' h3s).filter_mono ?_
  refine (absolutelyContinuous_of_isMulLeftInvariant μ' ν' ?_).ae_le
  refine mt ?_ h2s
...
Depends on / 依赖: Measure, Measure.coe_zero, Pi.zero_apply, absolutelyContinuous_of_isMulLeftInvariant, ae_le, ae_measure_preimage_mul_right_lt_top, coe_zero, filter_mono, zero_apply
-/
theorem ae_measure_preimage_mul_right_lt_top_of_ne_zero (h2s : ν' s != 0) (h3s : ν' s != ∞) :
    forallᵐ x ∂μ', ν' ((fun y => y * x) ⁻¹' s) < ∞ := by
  refine (ae_measure_preimage_mul_right_lt_top ν' ν' h3s).filter_mono ?_
  refine (absolutelyContinuous_of_isMulLeftInvariant μ' ν' ?_).ae_le
  refine mt ?_ h2s
  intro hν
...
/--
theorem `measure_lintegral_div_measure` / 定理 `measure_lintegral_div_measure`

English:
theorem measure_lintegral_div_measure
  statement: (sm : MeasurableSet s) (h2s : ν' s != 0) (h3s : ν' s != ∞)
  proof: by
...
@[to_additive]
/--
theorem `measure_mul_measure_eq` / 定理 `measure_mul_measure_eq`

English:
theorem measure_mul_measure_eq
  given: (s t : Set G) (h2s : ν' s != 0) (h3s : ν' s != ∞)
  proof: by
...
/--
theorem `measure_eq_div_smul` / 定理 `measure_eq_div_smul`

English:
theorem measure_eq_div_smul
  given: (h2s : ν' s != 0) (h3s : ν' s != ∞)
  proof: by
  ext1 t -
  rw [Measure.smul_apply]; rw [smul_eq_mul]; rw [mul_comm]; rw [← mul_div_assoc]; rw [mul_comm]; rw [measure_mul_measure_eq μ' ν' s t h2s h3s]; rw [mul_div_assoc]; rw [ENNReal.mul_div_cancel h2s h3s]

中文:
定理 measure_eq_div_smul
  条件: (h2s : ν' s != 0) (h3s : ν' s != ∞)
  证明: by
  ext1 t -
  rw [Measure.smul_apply]; rw [smul_eq_mul]; rw [mul_comm]; rw [← mul_div_assoc]; rw [mul_comm]; rw [measure_mul_measure_eq μ' ν' s t h2s h3s]; rw [mul_div_assoc]; rw [ENNReal.mul_div_cancel h2s h3s]

Depends on / 依赖: ENNReal, ENNReal.mul_div_cancel, Measure, Measure.smul_apply, measure_mul_measure_eq, mul_comm, mul_div_assoc, mul_div_cancel, smul_apply, smul_eq_mul
-/
...
/--
theorem `measurePreserving_prod_mul_right` / 定理 `measurePreserving_prod_mul_right`

English:
theorem measurePreserving_prod_mul_right
  given: [IsMulRightInvariant ν]
  proof: MeasurePreserving.skew_product (g := fun x y => y * x) (MeasurePreserving.id μ)
...
Depends on / 依赖: Eventually, Filter, Filter.Eventually.of_forall, MeasurePreserving, MeasurePreserving.id, MeasurePreserving.skew_product, map_mul_right_eq_self, measurable_fst, measurable_snd, measurable_snd.mul, of_forall, skew_product
-/
...
/--
theorem `measurePreserving_prod_mul_swap_right` / 定理 `measurePreserving_prod_mul_swap_right`

English:
theorem measurePreserving_prod_mul_swap_right
  given: [IsMulRightInvariant μ]
  proof: (measurePreserving_prod_mul_right ν μ).comp measurePreserving_swap
...
/--
theorem `measurePreserving_mul_prod` / 定理 `measurePreserving_mul_prod`

English:
theorem measurePreserving_mul_prod
  given: [IsMulRightInvariant μ]
  proof: measurePreserving_swap.comp (measurePreserving_prod_mul_swap_right μ ν)
...
/-- The map `(x, y) ↦ (x, y - x)` is measure-preserving. -/]
/--
theorem `measurePreserving_prod_div` / 定理 `measurePreserving_prod_div`

English:
theorem measurePreserving_prod_div
  given: [IsMulRightInvariant ν]
  proof: (measurePreserving_prod_mul_right μ ν).symm (MeasurableEquiv.shearDivRight G).symm
...
/-- The map `(x, y) ↦ (y, x - y)` sends `μ × ν` to `ν × μ`. -/]
/--
theorem `measurePreserving_prod_div_swap` / 定理 `measurePreserving_prod_div_swap`

English:
theorem measurePreserving_prod_div_swap
  given: [IsMulRightInvariant μ]
  proof: (measurePreserving_prod_div ν μ).comp measurePreserving_swap
...
/-- The map `(x, y) ↦ (x - y, y)` preserves the measure `μ × ν`. -/]
/--
theorem `measurePreserving_div_prod` / 定理 `measurePreserving_div_prod`

English:
theorem measurePreserving_div_prod
  given: [IsMulRightInvariant μ]
  proof: measurePreserving_swap.comp (measurePreserving_prod_div_swap μ ν)
...
/-- The map `(x, y) ↦ (x + y, - x)` is measure-preserving. -/]
/--
theorem `measurePreserving_mul_prod_inv_right` / 定理 `measurePreserving_mul_prod_inv_right`

English:
theorem measurePreserving_mul_prod_inv_right
  given: [IsMulRightInvariant μ] [IsMulRightInvariant ν]
  proof: by
  convert!
...
Depends on / 依赖: Function, Function.comp_apply, comp_apply, convert, div_mul_eq_div_div_swap, div_self, measurePreserving_prod_div_swap, measurePreserving_prod_mul_swap_right, one_div, simp_rw
-/
...
/--
theorem `quasiMeasurePreserving_mul` / 定理 `quasiMeasurePreserving_mul`

English:
theorem quasiMeasurePreserving_mul
  given: [IsMulLeftInvariant ν]
  proof: quasiMeasurePreserving_snd.comp (measurePreserving_prod_mul _ _).quasiMeasurePreserving
...
/--
theorem `quasiMeasurePreserving_mul_swap` / 定理 `quasiMeasurePreserving_mul_swap`

English:
theorem quasiMeasurePreserving_mul_swap
  given: [IsMulLeftInvariant μ]
  proof: quasiMeasurePreserving_snd.comp (measurePreserving_prod_mul_swap _ _).quasiMeasurePreserving
...
@[to_additive (attr := fun_prop) /-- The map `(x, y) ↦ -x + y` is quasi-measure-preserving. -/]
/--
theorem `quasiMeasurePreserving_inv_mul` / 定理 `quasiMeasurePreserving_inv_mul`

English:
theorem quasiMeasurePreserving_inv_mul
  given: [IsMulLeftInvariant ν]
  proof: quasiMeasurePreserving_snd.comp (measurePreserving_prod_inv_mul _ _).quasiMeasurePreserving
...
@[to_additive (attr := fun_prop) /-- The map `(x, y) ↦ -y + x` is quasi-measure-preserving. -/]
/--
theorem `quasiMeasurePreserving_inv_mul_swap` / 定理 `quasiMeasurePreserving_inv_mul_swap`

English:
theorem quasiMeasurePreserving_inv_mul_swap
  given: [IsMulLeftInvariant μ]
  proof: quasiMeasurePreserving_snd.comp (measurePreserving_prod_inv_mul_swap _ _).quasiMeasurePreserving
...
Depends on / 依赖: measurePreserving_prod_inv_mul_swap, quasiMeasurePreserving, quasiMeasurePreserving_snd, quasiMeasurePreserving_snd.comp
-/
...
@[to_additive (attr := fun_prop)]
/--
theorem `quasiMeasurePreserving_inv_of_right_invariant` / 定理 `quasiMeasurePreserving_inv_of_right_invariant`

English:
theorem quasiMeasurePreserving_inv_of_right_invariant
  given: [IsMulRightInvariant μ]
  proof: by
  rw [← μ.inv_inv]
  exact
...
Depends on / 依赖: absolutelyContinuous_inv, inv_absolutelyContinuous, inv_inv, quasiMeasurePreserving_inv
-/
...
@[to_additive]
/--
theorem `quasiMeasurePreserving_div_left` / 定理 `quasiMeasurePreserving_div_left`

English:
theorem quasiMeasurePreserving_div_left
  given: [IsMulLeftInvariant μ] (g : G)
  proof: by
  simp_rw [div_eq_mul_inv]
  exact
...
Depends on / 依赖: div_eq_mul_inv, measurePreserving_mul_left, quasiMeasurePreserving, quasiMeasurePreserving.comp, quasiMeasurePreserving_inv, simp_rw
-/
...
@[to_additive]
/--
theorem `quasiMeasurePreserving_div_left_of_right_invariant` / 定理 `quasiMeasurePreserving_div_left_of_right_invariant`

English:
theorem quasiMeasurePreserving_div_left_of_right_invariant
  given: [IsMulRightInvariant μ] (g : G)
  proof: by
  rw [← μ.inv_inv]
  exact
...
Depends on / 依赖: absolutelyContinuous_inv, inv_absolutelyContinuous, inv_inv, quasiMeasurePreserving_div_left
-/
...
@[to_additive]
/--
theorem `quasiMeasurePreserving_div_of_right_invariant` / 定理 `quasiMeasurePreserving_div_of_right_invariant`

English:
theorem quasiMeasurePreserving_div_of_right_invariant
  given: [IsMulRightInvariant μ]
  proof: by
...
Depends on / 依赖: Eventually, Eventually.of_forall, QuasiMeasurePreserving, QuasiMeasurePreserving.prod_of_left, measurable_div, measurePreserving_div_right, of_forall, prod_of_left, quasiMeasurePreserving
-/
...
@[to_additive]
/--
theorem `quasiMeasurePreserving_div` / 定理 `quasiMeasurePreserving_div`

English:
theorem quasiMeasurePreserving_div
  given: [IsMulLeftInvariant μ]
  proof: (quasiMeasurePreserving_div_of_right_invariant μ.inv ν).mono
...
Depends on / 依赖: AbsolutelyContinuous, AbsolutelyContinuous.rfl, absolutelyContinuous_inv, inv_absolutelyContinuous, quasiMeasurePreserving_div_of_right_invariant
-/
...
/--
theorem `quasiMeasurePreserving_mul_right` / 定理 `quasiMeasurePreserving_mul_right`

English:
theorem quasiMeasurePreserving_mul_right
  given: [IsMulLeftInvariant μ] (g : G)
  proof: by
...
Depends on / 依赖: AbsolutelyContinuous, AbsolutelyContinuous.mk, map_apply, measurable_mul_const, measure_mul_right_null
-/
...
/--
theorem `quasiMeasurePreserving_mul_left` / 定理 `quasiMeasurePreserving_mul_left`

English:
theorem quasiMeasurePreserving_mul_left
  given: [IsMulRightInvariant μ] (g : G)
  proof: by
  have :=
