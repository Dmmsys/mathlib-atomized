...
/-
**Mathlib.Tactic.Linarith.splitConjunctions** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.T
actic.Linarith`。
形式化陈述：Mathlib.Tactic.Linarith.Preprocessor
...
-/
/-
**Mathlib.Tactic.Linarith.filterComparisons** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.T
actic.Linarith`。
形式化陈述：Mathlib.Tactic.Linarith.Preprocessor
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Removes any expressions that are not proofs of inequalities, equalities, or nega
tions thereof.
Negations are pushed into inequalities when possible.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.isNatProp** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Li
narith`。
形式化陈述：Expr → MetaM Bool
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`isNatProp tp` is true iff `tp` is an inequality or equality between natural num
bers
or the negation thereof.
-/
...
/-
**Mathlib.Tactic.Linarith.isNatCoe** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Lin
arith`。
形式化陈述：isNatCoe (e : Expr) : Option (Expr × Expr)
参数：e : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.Linarith.getNatComparisons** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathli
b.Tactic.Linarith`。
形式化陈述：Expr → List (Expr × Expr)
参数：Expr × Expr。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`getNatComparisons e` returns a list of all subexpressions of `e` of the form `(
(t : ℕ) : C)`.
-/
...
/-
**Mathlib.Tactic.Linarith.mkNatCastNonnegProof** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Tactic.Linarith`。
形式化陈述：mkNatCastNonnegProof? (p : Expr × Expr) : MetaM (Option Expr)
参数：p : Expr × Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.Linarith.natToInt** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Lin
arith`。
形式化陈述：natToInt : GlobalBranchingPreprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`natToInt` lifts this inequality to the integers.
It also adds the facts that the integers involved are nonnegative.
To avoid adding the same nonnegativity facts many times, it is a global preproce
ssor.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.mkNonstrictIntProof** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Tactic.Linarith`。
形式化陈述：mkNonstrictIntProof? (pf : Expr) : MetaM (Option Expr)
参数：pf : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**Mathlib.Tactic.Linarith.strengthenStrictInt** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Tactic.Linarith`。
形式化陈述：strengthenStrictInt : Preprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`strengthenStrictInt h` turns a proof `h` of a strict integer inequality `t1 < t
2`
into a proof of `t1 ≤ t2 + 1`.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.rearrangeComparison** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib
.Tactic.Linarith`。
形式化陈述：Expr → MetaM (Option Expr)
参数：Option Expr。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`rearrangeComparison e` takes a proof `e` of an equality, inequality, or negatio
n thereof,
and turns it into a proof of a comparison `_ R 0`, where `R ∈ {=, ≤, <}`.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.compWithZero** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.Linarith`。
形式化陈述：compWithZero : Preprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`compWithZero h` takes a proof `h` of an equality, inequality, or negation there
of,
and turns it into a proof of a comparison `_ R 0`, where `R ∈ {=, ≤, <}`.
-/
...
/-
**Mathlib.Tactic.Linarith.without_one_mul** 是 Mathlib 中的一个定理，位于命名空间 `Mathlib.Tac
tic.Linarith`。
形式化陈述：without_one_mul {M : Type*} [MulOneClass M] {a b : M} (h : 1 * a = b) : a 
= b
参数：h : 1 * a = b。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
-/
/-
**Mathlib.Tactic.Linarith.normalizeDenominatorsLHS** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Tactic.Linarith`。
形式化陈述：normalizeDenominatorsLHS (h lhs : Expr) : MetaM Expr
参数：h lhs : Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
--- 原说明 ---
`normalizeDenominatorsLHS h lhs` assumes that `h` is a proof of `lhs R 0`.
It creates a proof of `lhs' R 0`, where all numeric division in `lhs` has been c
ancelled.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.cancelDenoms** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.Linarith`。
形式化陈述：cancelDenoms : Preprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`cancelDenoms pf` assumes `pf` is a proof of `t R 0`. If `t` contains the divisi
on symbol `/`,
it tries to scale `t` to cancel out division by numerals.
-/
...
/-
**Mathlib.Tactic.Linarith.findSquares** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Tact
ic.Linarith`。
形式化陈述：Std.TreeSet (ℕ × Bool) compare → Expr → Mathlib.Tactic.AtomM (Std.TreeSet 
(ℕ × Bool) compare)
参数：ℕ × Bool；Std.TreeSet (ℕ × Bool) compare。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`findSquares s e` collects all terms of the form `a ^ 2` and `a * a` that appear
 in `e`
and adds them to the set `s`.
A pair `(i, true)` is added to `s` when `atoms[i]^2` appears in `e`,
and `(i, false)` is added to `s` when `atoms[i]*atoms[i]` appears in `e`.
...
/-
**Mathlib.Tactic.Linarith.nlinarithGetSquareProofs** 是 Mathlib 中的一个定义，位于命名空间 `Ma
thlib.Tactic.Linarith`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.Linarith.nlinarithGetProductsProofs** 是 Mathlib 中的一个定义，位于命名空间 `
...
Note that the length of the resulting list is proportional to `ls.length^2`, whi
ch can make a large
amount of work for the linarith oracle.
-/
...
-/
/-
**Mathlib.Tactic.Linarith.nlinarithExtras** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tac
tic.Linarith`。
形式化陈述：nlinarithExtras : GlobalPreprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
* For every term `t` such that `t^2` or `t*t` appears in the input, adds a proof
 of `t^2 ≥ 0`
  or `t*t ≥ 0`.
* For every pair of comparisons `t1 R1 0` and `t2 R2 0`, adds a proof of `t1*t2 
R 0`.
...
-/
/-
**Mathlib.Tactic.Linarith.removeNeAux** 是 Mathlib 中的一个不透明定义，位于命名空间 `Mathlib.Tact
ic.Linarith`。
形式化陈述：MVarId → List Expr → MetaM (List Mathlib.Tactic.Linarith.Branch)
...
-/
/-
**Mathlib.Tactic.Linarith.removeNe** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.Lin
arith`。
形式化陈述：removeNe : GlobalBranchingPreprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
`removeNe` case splits on any proof `h : a ≠ b` in the input, turning it into `a
 < b ∨ a > b`,
by calling `linarith.removeNeAux`, provided the type has a `LinearOrder` instanc
e.
This produces `2^n` branches when there are `n` such hypotheses in the input.
-/
...
/-
**Mathlib.Tactic.Linarith.nnrealToReal** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic
.Linarith`。
形式化陈述：nnrealToReal : GlobalPreprocessor where description
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
If `h` is an equality or inequality between NNReals, `nnrealToReal` lifts this i
nequality to the
Reals. It also adds the facts that the reals involved are nonnegative. To avoid 
adding the same
nonnegativity facts many times, it is a global preprocessor. This preprocessor d
oes nothing unless
`Mathlib.Tactic.Linarith.NNRealPreprocessor` is imported
-/
...
-/
/-
**Mathlib.Tactic.Linarith.defaultPreprocessors** 是 Mathlib 中的一个定义，位于命名空间 `Mathli
b.Tactic.Linarith`。
形式化陈述：defaultPreprocessors : List GlobalBranchingPreprocessor
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
-/
/-
**Mathlib.Tactic.Linarith.preprocess** 是 Mathlib 中的一个定义，位于命名空间 `Mathlib.Tactic.L
inarith`。
形式化陈述：preprocess (pps : List GlobalBranchingPreprocessor) (g : MVarId) (l : List
 Expr) : MetaM (List Branch)
参数：pps : List GlobalBranchingPreprocessor；g : MVarId；l : List Expr。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
`preprocess pps l` takes a list `l` of proofs of propositions.
It maps each preprocessor `pp ∈ pps` over this list.
The preprocessors are run sequentially: each receives the output of the previous
 one.
Note that a preprocessor may produce multiple or no expressions from each input 
expression,
so the size of the list may change.
-/
