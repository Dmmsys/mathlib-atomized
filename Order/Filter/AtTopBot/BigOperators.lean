...
/--
theorem `Filter.map_atTop_finsetProd_le_of_prod_eq` / 定理 `Filter.map_atTop_finsetProd_le_of_prod_eq`

English:
theorem Filter.map_atTop_finsetProd_le_of_prod_eq
...
theorem Filter.map_atTop_finsetProd_le_of_prod_eq {f : α -> M} {g : β -> M}
    (h_eq : forall u : Finset β,
      exists v : Finset α, forall v', v subseteq v' -> exists u', u subseteq u' ∧ ∏ x in u', g x = ∏ b in v', f b) :
    (atTop.map fun s : Finset α => ∏ b in s, f b) <=
      atTop.map fun s : Finset β => ∏ x in s, g x := by
  refine ((atTop_basis.map _).le_basis_iff (atTop_basis.map _)).2 fun b _ => ?_
  let ⟨v, hv⟩ := h_eq b
  refine ⟨v, trivial, ?_⟩
  simpa [Finset.image_subset_iff] using! hv
...
@[to_additive]
/--
theorem `Function.Injective.map_atTop_finsetProd_eq` / 定理 `Function.Injective.map_atTop_finsetProd_eq`

English:
theorem Function.Injective.map_atTop_finsetProd_eq
  statement: {g : α -> β}
  proof: by
...
theorem Function.Injective.map_atTop_finsetProd_eq {g : α -> β}
    (hg : Function.Injective g) {f : β -> M} (hf : forall x, x ∉ Set.range g -> f x = 1) :
