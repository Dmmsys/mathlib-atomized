...
namespace Finset
variable {G : Type*} [Group G] [DecidableEq G] {H : Subgroup G} [DecidablePred (· in H)] [H.Normal]
  {A : Finset G} {m n : Nat}

@[to_additive]
/--
lemma `card_pow_quotient_mul_pow_inter_subgroup_le` / 引理 `card_pow_quotient_mul_pow_inter_subgroup_le`
...
lemma card_pow_quotient_mul_pow_inter_subgroup_le
  ...
  have hφA {a} (ha : a in π '' (A ^ m)) : φ a in A ^ m := by
...
中文:
引理 card_pow_quotient_mul_pow_inter_subgroup_le
  证明: by
  set π := QuotientGroup.mk' H
  let φ := invFunOn π (A ^ m)
  have hφ : Set.InjOn φ (π '' (A ^ m)) := invFunOn_injOn_image ..
...
Depends on / 依赖: Finset, Finset.card_image_of_injOn, QuotientGroup, QuotientGroup.mk, Set.InjOn, card_image_of_injOn, invFunOn, invFunOn_eq, invFunOn_injOn_image, invFunOn_mem
-/
lemma card_pow_quotient_mul_pow_inter_subgroup_le :
    #((A ^ m).image <| QuotientGroup.mk' H) * #{x in A ^ n | x in H} <= #(A ^ (m + n)) := by
  set π := QuotientGroup.mk' H
  let φ := invFunOn π (A ^ m)
  have hφ : Set.InjOn φ (π '' (A ^ m)) := invFunOn_injOn_image ..
  have hφA {a} (ha : a in π '' (A ^ m)) : φ a in A ^ m := by
    ...
    norm_cast at this
    simpa using this
  have hπφ {a} (ha : a in π '' (A ^ m)) : π (φ a) = a := invFunOn_eq (by simpa using ha)
  calc
    #((A ^ m).image π) * #{x in A ^ n | x in H}
    _ = #(((A ^ m).image π).image φ) * #{x in A ^ n | x in H} := by
...
@[to_additive]
/--
lemma `le_card_quotient_mul_sq_inter_subgroup` / 引理 `le_card_quotient_mul_sq_inter_subgroup`

English:
lemma le_card_quotient_mul_sq_inter_subgroup
  given: (hAsymm : A⁻¹ = A)
  proof: by
  set π := QuotientGroup.mk' H
...
refine sum_le_card_nsmul _ _ _ forall_mem_image.2 fun a ha => ?_
  calc
    #{a' in A | π a' = π a}
    _ <= #({a' in A | π a' = π a}⁻¹ * {a' in A | π a' = π a}) :=
      card_le_card_mul_left ⟨a⁻¹, by simpa⟩
    _ <= #{x in A⁻¹ * A |

中文:
引理 le_card_quotient_mul_sq_inter_subgroup
  条件: (hAsymm : A⁻¹ = A)
  证明: by
  set π := QuotientGroup.mk' H
  rw [card_eq_sum_card_image π]
...
Depends on / 依赖: QuotientGroup, QuotientGroup.eq_one_iff, QuotientGroup.mk, and_imp, card_eq_sum_card_image, card_le_card_mul_left, eq_one_iff, forall_mem_image, map_inv, mem_filter, mem_inv, mul_mem_mul, mul_subset_iff, sum_le_card_nsmul
-/
lemma le_card_quotient_mul_sq_inter_subgroup (hAsymm : A⁻¹ = A) :
    #A <= #(A.image <| QuotientGroup.mk' H) * #{x in A ^ 2 | x in H} := by
  set π := QuotientGroup.mk' H
  rw [card_eq_sum_card_image π]
refine sum_le_card_nsmul _ _ _ forall_mem_image.2 fun a ha => ?_
  calc
    ...
    _ <= #{x in A⁻¹ * A | x in H} := by
      ...
      simp [hya, ← hxa]
    _ = #{x in A ^ 2 | x in H} := by simp [hAsymm, sq]
