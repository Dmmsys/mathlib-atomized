...
@[simps]
/-
**BialgCat.instMonoidalCategoryStruct** 是 Mathlib 中的一个实例，位于命名空间 `BialgCat`。
形式化陈述：instMonoidalCategoryStruct : MonoidalCategoryStruct.{u} (BialgCat R) where
 tensorObj X Y
该定义给出了上述对象。
...
@[simps]
/-
**BialgCat.MonoidalCategory.inducingFunctorData** 是 Mathlib 中的一个定义，位于命名空间 `Bialg
Cat.MonoidalCategory`。
形式化陈述：(R : Type u) →   [inst : CommRing R] → CategoryTheory.Monoidal.InducingFun
ctorData (CategoryTheory.forget₂ (BialgCat R) (AlgCat R))
参数：BialgCat R；AlgCat R。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**BialgCat.instMonoidalCategory** 是 Mathlib 中的一个实例，位于命名空间 `BialgCat`。
形式化陈述：instMonoidalCategory : MonoidalCategory (BialgCat R)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
