...
lemma measure_le_mul_measure_gt_le_of_map_rotation_eq_self
  ...
  _ = ((μ.prod μ).map (ContinuousLinearMap.rotation
...
lemma measure_gt_normThreshold_le_rpow
  ...
  | zero =>
    ...
    rw [ENNReal.mul_div_cancel hc_pos.ne' h
...
lemma logRatio_pos
  ...
  · refine (ENNReal.toReal_lt_toReal (by finiteness) (by finiteness)).mpr ?_
    refine ENNReal.sub_lt_of_lt_ad
...
lemma logRatio_mono
  ...
  · simp only [ENNReal.toReal_pos_iff, tsu
