...
/--
theorem `MvPolynomial.sum_eval_eq_zero` / 定理 `MvPolynomial.sum_eval_eq_zero`

English:
theorem MvPolynomial.sum_eval_eq_zero
  statement: (f : MvPolynomial σ K)
  proof: by
...
variable [DecidableEq K] (p : Nat) [CharP K p]

/--
theorem `char_dvd_card_solutions_of_sum_lt` / 定理 `char_dvd_card_solutions_of_sum_lt`

English:
theorem char_dvd_card_solutions_of_sum_lt
  statement: {s : Finset ι} {f : ι -> MvPolynomial σ K}
  proof: by
...
theorem char_dvd_card_solutions_of_sum_lt {s : Finset ι} {f : ι -> MvPolynomial σ K}
    ...
    p ∣ Fintype.card { x : σ -> K // forall i in s, eval x (f i) = 0 } := by
  have hq : 0 < q - 1 := by rw [← Fintype.card_units, Fintype.card_pos_iff]; exact ⟨1⟩
  let S : Finset (σ -> K) := {x | forall i in s, eval x (f i) = 0}
  have hS (x : σ -> K) : x in S ↔ forall i in s, eval x (f i) = 0 := by simp [S]
...
/--
theorem `char_dvd_card_solutions_of_fintype_sum_lt` / 定理 `char_dvd_card_solutions_of_fintype_sum_lt`

English:
theorem char_dvd_card_solutions_of_fintype_sum_lt
  statement: [Fintype ι] {f : ι -> MvPolynomial σ K}
  proof: by
...
Depends on / 依赖: a.toFun, char_dvd_card_solutions_of_sum_lt
-/
theorem char_dvd_card_solutions_of_fintype_sum_lt [Fintype ι] {f : ι -> MvPolynomial σ K}
    (h : (∑ i, (f i).totalDegree) < Fintype.card σ) :
    p ∣ Fintype.card { x : σ -> K // forall i, eval x (f i) = 0 } := by
  simpa using char_dvd_card_solutions_of_sum_lt p h

/--
theorem `char_dvd_card_solutions` / 定理 `char_dvd_card_solutions`

English:
theorem char_dvd_card_solutions
  given: {f : MvPolynomial σ K} (h : f.totalDegree < Fintype.card σ)
  proof: by
...
Depends on / 依赖: Fintype, Fintype.card, MvPolynomial, char_dvd_card_solutions_of_sum_lt, convert, totalDegree
-/
...
/--
theorem `char_dvd_card_solutions_of_add_lt` / 定理 `char_dvd_card_solutions_of_add_lt`

English:
theorem char_dvd_card_solutions_of_add_lt
  statement: {f₁ f₂ : MvPolynomial σ K}
  proof: by
...
Depends on / 依赖: Bool.forall_bool, Fintype, Fintype.card, MvPolynomial, add_comm, char_dvd_card_solutions_of_fintype_sum_lt, forall_bool, totalDegree, trans_lt
-/
