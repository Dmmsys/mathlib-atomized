...
@[simps]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.functorObjObj** 是 Mathli
b 中的一个定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：functorObjObj (A : C ⥤ D) [MonObj A] (X : C) : Mon D where X
参数：A : C ⥤ D；X : C。
该定义给出了上述对象。
...
@[simps]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.functorObj** 是 Mathlib 中
的一个定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：functorObj (A : C ⥤ D) [MonObj A] : C ⥤ Mon D where obj
参数：A : C ⥤ D。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A monoid object in a functor category induces a functor to the category of monoi
d objects.
-/
...
@[simps]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.functor** 是 Mathlib 中的一个
定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：functor : Mon (C ⥤ D) ⥤ C ⥤ Mon D where obj A
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.inverseObj** 是 Mathlib 中
的一个定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：inverseObj (F : C ⥤ Mon D) : Mon (C ⥤ D) where X
参数：F : C ⥤ Mon D。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.inverse** 是 Mathlib 中的一个
定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：inverse : (C ⥤ Mon D) ⥤ Mon (C ⥤ D) where obj
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.unitIso** 是 Mathlib 中的一个
定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：unitIso : 𝟭 (Mon (C ⥤ D)) ≅ functor ⋙ inverse
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.MonFunctorCategoryEquivalence.counitIso** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.Monoidal.MonFunctorCategoryEquivalence`。
形式化陈述：counitIso : inverse ⋙ functor ≅ 𝟭 (C ⥤ Mon D)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.monFunctorCategoryEquivalence** 是 Mathlib 中的一个定义，位于命名空
间 `CategoryTheory.Monoidal`。
形式化陈述：monFunctorCategoryEquivalence : Mon (C ⥤ D) ≌ C ⥤ Mon D where functor
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.functorObjObj** 是 Math
lib 中的一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
形式化陈述：functorObjObj (A : C ⥤ D) [ComonObj A] (X : C) : Comon D where X
参数：A : C ⥤ D；X : C。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.functorObj** 是 Mathlib
 中的一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
形式化陈述：functorObj (A : (C ⥤ D)) [ComonObj A] : C ⥤ Comon D where obj
参数：A : (C ⥤ D)。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A comonoid object in a functor category induces a functor to the category of com
onoid objects.
-/
...
@[simps]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.functor** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
形式化陈述：functor : Comon (C ⥤ D) ⥤ C ⥤ Comon D where obj A
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.inverseObj** 是 Mathlib
 中的一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
形式化陈述：inverseObj (F : C ⥤ Comon D) : Comon (C ⥤ D) where X
参数：F : C ⥤ Comon D。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A functor to the category of comonoid objects can be translated as a comonoid ob
ject
in the functor category.
-/
...
@[simps]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.inverse** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.unitIso** 是 Mathlib 中的
一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence.counitIso** 是 Mathlib 
中的一个定义，位于命名空间 `CategoryTheory.Monoidal.ComonFunctorCategoryEquivalence`。
形式化陈述：counitIso : inverse ⋙ functor ≅ 𝟭 (C ⥤ Comon D)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.comonFunctorCategoryEquivalence** 是 Mathlib 中的一个定义，位于命
名空间 `CategoryTheory.Monoidal`。
形式化陈述：comonFunctorCategoryEquivalence : Comon (C ⥤ D) ≌ C ⥤ Comon D where functo
r
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence.functor** 是 Mathlib 
中的一个定义，位于命名空间 `CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence`。
形式化陈述：functor : CommMon (C ⥤ D) ⥤ C ⥤ CommMon D where obj A
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence.inverse** 是 Mathlib 
中的一个定义，位于命名空间 `CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence`。
形式化陈述：inverse : (C ⥤ CommMon D) ⥤ CommMon (C ⥤ D) where obj F
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence.unitIso** 是 Mathlib 
中的一个定义，位于命名空间 `CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence`。
形式化陈述：unitIso : 𝟭 (CommMon (C ⥤ D)) ≅ functor ⋙ inverse
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps!]
/-
**CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence.counitIso** 是 Mathli
b 中的一个定义，位于命名空间 `CategoryTheory.Monoidal.CommMonFunctorCategoryEquivalence`。
形式化陈述：counitIso : inverse ⋙ functor ≅ 𝟭 (C ⥤ CommMon D)
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps]
/-
**CategoryTheory.Monoidal.commMonFunctorCategoryEquivalence** 是 Mathlib 中的一个定义，位
于命名空间 `CategoryTheory.Monoidal`。
形式化陈述：commMonFunctorCategoryEquivalence : CommMon (C ⥤ D) ≌ C ⥤ CommMon D where 
functor
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
