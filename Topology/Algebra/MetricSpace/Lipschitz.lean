...
variable {α β : Type*} [PseudoMetricSpace α] [PseudoMetricSpace β] {K : Real>=0}

/--
lemma `LipschitzWith.cauchySeq_comp` / 引理 `LipschitzWith.cauchySeq_comp`

English:
lemma LipschitzWith.cauchySeq_comp
  statement: {f : α -> β} (hf : LipschitzWith K f) {u : Nat -> α}
  proof: by
...
引理 LipschitzWith.cauchySeq_comp
  结论: {f : α -> β} (hf : LipschitzWith K f) {u : 自然数 -> α}
  证明: by
  rcases cauchySeq_iff_le_tendsto_0.1 hu with ⟨b, b_nonneg, hb, blim⟩
  refine cauchySeq_iff_le_tendsto_0.2 ⟨fun n => K * b n, ?_, ?_, ?_⟩
  · exact fun n => mul_nonneg (by positivity) (b_nonneg n)
...
-/
lemma LipschitzWith.cauchySeq_comp {f : α -> β} (hf : LipschitzWith K f) {u : Nat -> α}
    (hu : CauchySeq u) :
    CauchySeq (f ∘ u) := by
  rcases cauchySeq_iff_le_tendsto_0.1 hu with ⟨b, b_nonneg, hb, blim⟩
  refine cauchySeq_iff_le_tendsto_0.2 ⟨fun n => K * b n, ?_, ?_, ?_⟩
...
/--
lemma `LipschitzOnWith.cauchySeq_comp` / 引理 `LipschitzOnWith.cauchySeq_comp`

English:
lemma LipschitzOnWith.cauchySeq_comp
  statement: {s : Set α} {f : α -> β} (hf : LipschitzOnWith K f s)
  proof: by
  rcases cauchySeq_iff_le_tendsto_0.1 hu with ⟨b, b_nonneg, hb, blim⟩
  refine cauchySeq_iff_le_tendsto_0.2 ⟨fun n => K * b n, ?_, ?_, ?_⟩
  · exact fun n => mul_nonneg (by positivity) (b_nonneg n)
...
lemma LipschitzOnWith.cauchySeq_comp {s : Set α} {f : α -> β} (hf : LipschitzOnWith K f s)
    {u : Nat -> α} (hu : CauchySeq u) (h'u : range u subseteq s) :
    CauchySeq (f ∘ u) := by
  rcases cauchySeq_iff_le_tendsto_0.1 hu with ⟨b, b_nonneg, hb, blim⟩
  refine cauchySeq_iff_le_tendsto_0.2 ⟨fun n => K * b n, ?_, ?_, ?_⟩
  · exact fun n => mul_nonneg (by positivity) (b_nonneg n)
...
/--
theorem `continuousAt_of_locally_lipschitz` / 定理 `continuousAt_of_locally_lipschitz`

English:
theorem continuousAt_of_locally_lipschitz
  statement: {f : α -> β} {x : α} {r : Real} (hr : 0 < r) (K : Real)
  proof: by
...
/--
lemma `LocallyLipschitzOn.exists_lipschitzOnWith_of_compact` / 引理 `LocallyLipschitzOn.exists_lipschitzOnWith_of_compact`

English:
lemma LocallyLipschitzOn.exists_lipschitzOnWith_of_compact
