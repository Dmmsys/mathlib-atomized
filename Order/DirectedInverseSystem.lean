...
/-
**DirectedSystem** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：{ι : Type u_1} → [inst : Preorder ι] → (F : ι → Type u_4) → (⦃i j : ι⦄ → i
 ≤ j → F i → F j) → Prop
参数：F : ι → Type u_4；⦃i j : ι⦄ → i ≤ j → F i → F j。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A directed system is a functor from a category (directed poset) to another categ
ory.
-/
...
/-
**DirectedSystem.map_self'** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：DirectedSystem.map_self' ⦃i⦄ (x) : f i i le_rfl x = x
参数：x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
`fun i j h ↦ f i j h` can confuse the simplifier. -/
/-
**DirectedSystem.map_map'** 是 Mathlib 中的一个定理，位于命名空间 ``。
形式化陈述：DirectedSystem.map_map' ⦃i j k⦄ (hij hjk x) : f j k hjk (f i j hij x) = f 
i k (hij.trans hjk) x
参数：hij hjk x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
@[instance_reducible]
/-
**DirectLimit.setoid** 是 Mathlib 中的一个定义，位于命名空间 `DirectLimit`。
形式化陈述：setoid : Setoid (Σ i, F i) where r x y
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**DirectLimit.r_of_le** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：r_of_le (x : Σ i, F i) (i : ι) (h : x.1 <= i) : (setoid f).r x ⟨i, f _ _ h
 x.2⟩
参数：x : Σ i, F i；i : ι；h : x.1 <= i。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `LE.le.trans`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a ≤ b → b
 ≤ c → a ≤ c
· 使用定理 `DirectedSystem.map_map'`：DirectedSystem.map_map' ⦃i j k⦄ (hij hjk x) : f
 j k hjk (f i j hij x) = f i k (hij.trans hjk) x
-/
...
/-
**DirectLimit._root_.DirectLimit** 是 Mathlib 中的一个缩写定义，位于命名空间 `DirectLimit`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
variable {f} in
/-
**DirectLimit.eq_of_le** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：eq_of_le (x : Σ i, F i) (i : ι) (h : x.1 <= i) : (⟦x⟧ : DirectLimit F f) =
 ⟦⟨i, f _ _ h x.2⟩⟧
参数：x : Σ i, F i；i : ι；h : x.1 <= i。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
b⟧
· 使用定理 `DirectLimit.r_of_le`：r_of_le (x : Σ i, F i) (i : ι) (h : x.1 <= i) : (se
toid f).r x ⟨i, f _ _ h x.2⟩
-/
...
@[simp]
/-
**DirectLimit.mk_apply** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：mk_apply (i j : ι) (x : F i) (h : i <= j) : ⟦⟨j, f _ _ h x⟩⟧ = (⟦⟨i, x⟩⟧ :
 DirectLimit F f)
参数：i j : ι；x : F i；h : i <= j。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
...
/-
**DirectLimit.induction** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：∀ {ι : Type u_1} [inst : Preorder ι] {F : ι → Type u_4} {T : ⦃i j : ι⦄ → i
 ≤ j → Sort u_8}   (f : (i j : ι) → (h : i ≤ j) → T h) [inst_1 : ⦃i j : ι⦄ → (h 
: i ≤ j) → FunLike (T h) (F i) (F j)]   [inst_2 : DirectedSystem F fun x1 x2 x3 
=> ⇑(f x1 x2 x3)] [inst_3 : IsDirectedOrder ι] {C : DirectLimit F f → Prop},   (
∀ (i : ι) (x : F i), C ⟦⟨i, x⟩⟧) → ∀ (x : DirectLimit F f), C x
参数：f : (i j : ι) → (h : i ≤ j) → T h；h : i ≤ j；T h；F i；F j；f x1 x2 x3；∀ (i : ι) 
(x : F i), C ⟦⟨i, x⟩⟧；x : DirectLimit F f。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DirectLimit.exists_eq_mk** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：exists_eq_mk (z : DirectLimit F f) : exists i x, z = ⟦⟨i, x⟩⟧
...
/-
**DirectLimit.mk_injective** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：mk_injective (h : forall i j hij, Function.Injective (f i j hij)) (i) : Fu
nction.Injective fun x => (⟦⟨i, x⟩⟧ : DirectLimit F f)
参数：h : forall i j hij, Function.Injective (f i j hij)；i。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DirectLimit.lift** 是 Mathlib 中的一个定义，位于命名空间 `DirectLimit`。
形式化陈述：{ι : Type u_1} →   [inst : Preorder ι] →     {F : ι → Type u_4} →       {T
 : ⦃i j : ι⦄ → i ≤ j → Sort u_8} →         (f : (i j : ι) → (h : i ≤ j) → T h) →
           [inst_1 : ⦃i j : ι⦄ → (h : i ≤ j) → FunLike (T h) (F i) (F j)] →     
        [inst_2 : DirectedSystem F fun x1 x2 x3 => ⇑(f x1 x2 x3)] →             
  [inst_3 : IsDirectedOrder ι] →                 {C : Sort u_9} →               
    (ih : (i : ι) → F i → C) →                     (∀ (i j : ι) (h : i ≤ j) (x :
 F i), ih i x = ih j ((f i j h) x)) → DirectLimit F f → C
参数：f : (i j : ι) → (h : i ≤ j) → T h；h : i ≤ j；T h；F i；F j；f x1 x2 x3；ih : (i : 
ι) → F i → C；∀ (i j : ι) (h : i ≤ j) (x : F i), ih i x = ih j ((f i j h) x)。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
To define a function from the direct limit, it suffices to provide one function 
from each
component subject to a compatibility condition.
-/
...
@[simp]
/-
**DirectLimit.lift_def** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：lift_def (x) : DirectLimit.lift f ih compat ⟦x⟧ = ih x.1 x.2
参数：x。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**DirectLimit.lift_injective** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：lift_injective (h : forall i, Function.Injective (ih i)) : Function.Inject
ive (DirectLimit.lift f ih compat)
参数：h : forall i, Function.Injective (ih i)。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DirectLimit.map_def** 是 Mathlib 中的一个定理，位于命名空间 `DirectLimit`。
形式化陈述：map_def (x) : map f₁ f₂ ih compat ⟦x⟧ = ⟦⟨x.1, ih x.1 x.2⟩⟧
参数：x。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
--- 原说明 ---
To define a binary function from the direct limit, it suffices to provide one bi
nary function
from each component subject to a compatibility condition.
-/
...
/-
**InverseSystem** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：{ι : Type u_1} → [inst : Preorder ι] → {F : ι → Type u_4} → (⦃i j : ι⦄ → i
 ≤ j → F j → F i) → Prop
参数：⦃i j : ι⦄ → i ≤ j → F j → F i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
An inverse system indexed by a preorder is a contravariant functor from the preo
rder
to another category. It is dual to `DirectedSystem`.
-/
...
/-
**InverseSystem.limit** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：limit (i : ι) : Set (forall l : Iio i, F l)
参数：i : ι。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.piLT** 是 Mathlib 中的一个缩写定义，位于命名空间 `InverseSystem`。
形式化陈述：piLT (X : ι -> Type*) (i : ι)
参数：X : ι -> Type*；i : ι。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.piLTProj** 是 Mathlib 中的一个缩写定义，位于命名空间 `InverseSystem`。
形式化陈述：piLTProj (f : piLT X j) : piLT X i
参数：f : piLT X j。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
The projection from a Pi type to the Pi type over an initial segment of its inde
xing type.
-/
abbrev piLTProj (f : piLT X j) : piLT X i := fun l ↦ f ⟨l, l.2.trans_le h⟩
/-
**InverseSystem.piLTProj_intro** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：piLTProj_intro {l : Iio j} {f : piLT X j} (hl : l < i) : f l = piLTProj h 
f ⟨l, hl⟩
参数：hl : l < i。
该定理/引理给出了一组等式。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.IsNatEquiv** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：IsNatEquiv {s : Set ι} (equiv : forall j : s, F j ≃ piLT X j) : Prop
参数：equiv : forall j : s, F j ≃ piLT X j。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.piLTLim** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：{ι : Type u_6} →   [inst : LinearOrder ι] →     {X : ι → Type u_7} →      
 {i : ι} → Order.IsSuccPrelimit i → InverseSystem.piLT X i ≃ ↑(InverseSystem.lim
it InverseSystem.piLTProj i)
参数：InverseSystem.limit InverseSystem.piLTProj i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.piLTLim_symm_apply** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：piLTLim_symm_apply {f} (k : Iio i) {l : Iio i} (hl : l.1 < k.1) : (piLTLim
 (X
参数：k : Iio i；hl : l.1 < k.1。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**InverseSystem.piSplitLE** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：piSplitLE : piLT X i × X i ≃ forall j : Iic i, X j where toFun f j
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.piSplitLE_eq** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：∀ {ι : Type u_6} {X : ι → Type u_8} {i : ι} [inst : PartialOrder ι] [inst_
1 : DecidableEq ι]   {f : InverseSystem.piLT X i × X i}, InverseSystem.piSplitLE
 f ⟨i, ⋯⟩ = f.2
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**InverseSystem.piSplitLE_lt** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：piSplitLE_lt {f : piLT X i × X i} {j} (hj : j < i) : piSplitLE f ⟨j, hj.le
⟩ = f.1 ⟨j, hj⟩
参数：hj : j < i。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**InverseSystem.piEquivSucc** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：piEquivSucc : forall j : Iic i⁺, F j ≃ piLT X j
该定义给出了上述对象。
...
/-
**InverseSystem.piEquivSucc_self** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：piEquivSucc_self {x} : piEquivSucc equiv e hi ⟨_, le_rfl⟩ x ⟨i, lt_succ_of
_not_isMax hi⟩ = (e x).2
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用引理 `le_rfl`：le_rfl : a <= a
...
· 使用定理 `congrFun`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, f = g →
 ∀ (a : α), f a = g a
· 使用定理 `InverseSystem.piSplitLE_eq`：∀ {ι : Type u_6} {X : ι → Type u_8} {i : ι} 
[inst : PartialOrder ι] [inst_1 : DecidableEq ι]   {f : InverseSystem.piLT X i ×
 X i}, InverseSy…
· 使用定理 `Equiv.prodCongr_apply`：∀ {α₁ : Type u_9} {α₂ : Type u_10} {β₁ : Type u_1
1} {β₂ : Type u_12} (e₁ : α₁ ≃ α₂) (e₂ : β₁ ≃ β₂),   ⇑(e₁.prodCongr e₂) = Prod.m
ap ⇑e₁ ⇑e₂
· 使用定理 `Equiv.piCongrSet_apply`：∀ {α : Type u_11} {W : α → Sort w} {s t : Set α}
 (h : s = t) (f : (i : { i // i ∈ s }) → W ↑i) (i : { i // i ∈ t }),   (Equiv.pi
CongrSet h) …
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
-/
...
/-
**InverseSystem.isNatEquiv_piEquivSucc** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`
。
形式化陈述：isNatEquiv_piEquivSucc [InverseSystem f] (H : forall x, (e x).1 = f (le_su
cc i) x) (nat : IsNatEquiv f equiv) : IsNatEquiv f (piEquivSucc equiv e hi)
参数：H : forall x, (e x).1 = f (le_succ i) x；nat : IsNatEquiv f equiv。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `InverseSystem.map_self`：∀ {ι : Type u_1} {inst : Preorder ι} {F : ι → Ty
pe u_4} {f : ⦃i j : ι⦄ → i ≤ j → F j → F i} [self : InverseSystem f]   ⦃i : ι⦄ (
x : F i), f …
· 使用定理 `eq_self`：∀ {α : Sort u_1} (a : α), (a = a) = True
· 使用定理 `Eq.symm`：∀ {α : Sort u} {a b : α}, a = b → b = a
...
· 使用定理 `Equiv.trans`：Equiv.trans {s t u : Computation α} : s ~ t -> t ~ u -> s ~
 u
· 使用定理 `Equiv.refl`：Equiv.refl (s : Computation α) : s ~ s
· 使用定理 `InverseSystem.piEquivSucc.eq_1`：∀ {ι : Type u_6} {F : ι → Type u_7} {X :
 ι → Type u_8} {i : ι} [inst : LinearOrder ι] [inst_1 : SuccOrder ι]   (equiv : 
(j : ↑(Set.Iic i)) →…
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `InverseSystem.piSplitLE_lt`：piSplitLE_lt {f : piLT X i × X i} {j} (hj : 
j < i) : piSplitLE f ⟨j, hj.le⟩ = f.1 ⟨j, hj⟩
· 使用定理 `LE.le.trans`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a ≤ b → b
 ≤ c → a ≤ c
· 使用定理 `InverseSystem.map_map`：∀ {ι : Type u_1} {inst : Preorder ι} {F : ι → Typ
e u_4} {f : ⦃i j : ι⦄ → i ≤ j → F j → F i} [self : InverseSystem f]   ⦃k j i : ι
⦄ (hkj : k …
· 使用定理 `InverseSystem.piLTProj.eq_1`：∀ {ι : Type u_1} [inst : Preorder ι] {X : ι
 → Type u_5} ⦃i j : ι⦄ (h : i ≤ j) (f : InverseSystem.piLT X j)   (l : ↑(Set.Iio
 i)), InverseSyst…
· 使用引理 `le_rfl`：le_rfl : a <= a
...
/-
**InverseSystem.invLimEquiv** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：{ι : Type u_6} →   {F : ι → Type u_7} →     {X : ι → Type u_8} →       {i 
: ι} →         [inst : LinearOrder ι] →           {f : ⦃i j : ι⦄ → i ≤ j → F j →
 F i} →             {equiv : (j : ↑(Set.Iio i)) → F ↑j ≃ InverseSystem.piLT X ↑j
} →               InverseSystem.IsNatEquiv f equiv →                 ↑(InverseSy
stem.limit f i) ≃ ↑(InverseSystem.limit InverseSystem.piLTProj i)
参数：j : ↑(Set.Iio i)；InverseSystem.limit f i；InverseSystem.limit InverseSystem.pi
LTProj i。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
...
/-
**InverseSystem.piEquivLim** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：piEquivLim : forall j : Iic i, F j ≃ piLT X j
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.trans`：Equiv.trans {s t u : Computation α} : s ~ t -> t ~ u -> s ~
 u
...
variable {equivLim}
/-
**InverseSystem.isNatEquiv_piEquivLim** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：isNatEquiv_piEquivLim [InverseSystem f] (H : forall x l, (equivLim x).1 l 
= f l.2.le x) : IsNatEquiv f (piEquivLim nat equivLim hi)
参数：H : forall x l, (equivLim x).1 l = f l.2.le x。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
· 使用定理 `Subtype.property`：∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑s
...
· 使用定理 `InverseSystem.piSplitLE_lt`：piSplitLE_lt {f : piLT X i × X i} {j} (hj : 
j < i) : piSplitLE f ⟨j, hj.le⟩ = f.1 ⟨j, hj⟩
· 使用定理 `InverseSystem.piLTProj.congr_simp`：∀ {ι : Type u_1} [inst : Preorder ι] 
{X : ι → Type u_5} ⦃i j : ι⦄ (h : i ≤ j) (f f_1 : InverseSystem.piLT X j),   f =
 f_1 → ∀ (l : ↑(Set.Iio…
· 使用定理 `InverseSystem.piSplitLE_eq`：∀ {ι : Type u_6} {X : ι → Type u_8} {i : ι} 
[inst : PartialOrder ι] [inst_1 : DecidableEq ι]   {f : InverseSystem.piLT X i ×
 X i}, InverseSy…
· 使用定理 `InverseSystem.piLTProj.eq_1`：∀ {ι : Type u_1} [inst : Preorder ι] {X : ι
 → Type u_5} ⦃i j : ι⦄ (h : i ≤ j) (f : InverseSystem.piLT X j)   (l : ↑(Set.Iio
 i)), InverseSyst…
· 使用定理 `InverseSystem.piLTLim_symm_apply`：piLTLim_symm_apply {f} (k : Iio i) {l 
: Iio i} (hl : l.1 < k.1) : (piLTLim (X
· 使用定理 `InverseSystem.invLimEquiv_apply_coe`：∀ {ι : Type u_6} {F : ι → Type u_7}
 {X : ι → Type u_8} {i : ι} [inst : LinearOrder ι]   {f : ⦃i j : ι⦄ → i ≤ j → F 
j → F i} {equiv : (j : ↑(…
· 使用定理 `InverseSystem.piEquivLim.eq_1`：∀ {ι : Type u_6} {F : ι → Type u_7} {X : 
ι → Type u_8} {i : ι} [inst : LinearOrder ι]   {f : ⦃i j : ι⦄ → i ≤ j → F j → F 
i} {equiv : (j : ↑(…
· 使用定理 `LE.le.trans_lt`：∀ {α : Type u_1} [inst : Preorder α] {a b c : α}, a ≤ b 
→ b < c → a < c
-/
...
/-
**InverseSystem.PEquivOn** 是 Mathlib 中的一个归纳类型，位于命名空间 `InverseSystem`。
形式化陈述：{ι : Type u_6} →   {F : ι → Type u_7} →     {X : ι → Type u_8} →       [in
st : LinearOrder ι] →         (⦃i j : ι⦄ → i ≤ j → F j → F i) →           [inst_
1 : SuccOrder ι] →             (⦃i : ι⦄ → ¬IsMax i → F (Order.succ i) ≃ F i × X 
i) → Set ι → Type (max (max u_6 u_7) u_8)
参数：⦃i j : ι⦄ → i ≤ j → F j → F i；⦃i : ι⦄ → ¬IsMax i → F (Order.succ i) ≃ F i × X
 i；max (max u_6 u_7) u_8。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
A natural partial family of bijections to `piLT` satisfying a compatibility cond
ition.
-/
...
/-
**InverseSystem.PEquivOn.restrict** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem.PEqui
vOn`。
形式化陈述：{ι : Type u_6} →   {F : ι → Type u_7} →     {X : ι → Type u_8} →       [in
st : LinearOrder ι] →         {f : ⦃i j : ι⦄ → i ≤ j → F j → F i} →           [i
nst_1 : SuccOrder ι] →             {equivSucc : ⦃i : ι⦄ → ¬IsMax i → F (Order.su
cc i) ≃ F i × X i} →               {s t : Set ι} → InverseSystem.PEquivOn f equi
vSucc t → s ⊆ t → InverseSystem.PEquivOn f equivSucc s
参数：Order.succ i。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
@[simps] def PEquivOn.restrict (e : PEquivOn f equivSucc t) (h : s ⊆ t) :
  ...
  compat _ := e.compat _
/-
**InverseSystem.unique_pEquivOn** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：unique_pEquivOn (hs : IsLowerSet s) {e₁ e₂ : PEquivOn f equivSucc s} : e₁ 
= e₂
参数：hs : IsLowerSet s。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Order.lt_succ_of_not_isMax`：∀ {α : Type u_1} [inst : Preorder α] [inst_1
 : SuccOrder α] {a : α}, ¬IsMax a → a < Order.succ a
· 使用定理 `InverseSystem.PEquivOn.ext`：∀ {ι : Type u_6} {F : ι → Type u_7} {X : ι →
 Type u_8} {inst : LinearOrder ι} {f : ⦃i j : ι⦄ → i ≤ j → F j → F i}   {inst_1 
: SuccOrder ι} {…
· 使用定理 `funext`：∀ {α : Sort u} {β : α → Sort v} {f g : (x : α) → β x}, (∀ (x : α
), f x = g x) → f = g
...
· 使用定理 `congrArg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ →
 f a₁ = f a₂
· 使用定理 `InverseSystem.piLTProj_intro`：piLTProj_intro {l : Iio j} {f : piLT X j} 
(hl : l < i) : f l = piLTProj h f ⟨l, hl⟩
· 使用定理 `Subtype.property`：∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑s
elf
· 使用定理 `LT.lt.le`：∀ {α : Type u_1} [inst : Preorder α] {a b : α}, a < b → a ≤ b
-/
...
/-
**InverseSystem.pEquivOn_apply_eq** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`。
形式化陈述：pEquivOn_apply_eq (h : IsLowerSet (s inter t)) {e₁ : PEquivOn f equivSucc 
s} {e₂ : PEquivOn f equivSucc t} {i} {his : i in s} {hit : i in t} : e₁.equiv ⟨i
, his⟩ = e₂.equiv ⟨i, hit⟩
参数：h : IsLowerSet (s inter t)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
→ f a₁ = f a₂
· 使用定理 `InverseSystem.unique_pEquivOn`：unique_pEquivOn (hs : IsLowerSet s) {e₁ e
₂ : PEquivOn f equivSucc s} : e₁ = e₂
-/
...
/-
**InverseSystem.pEquivOnSucc** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：pEquivOnSucc [InverseSystem f] (hi : ¬IsMax i) (e : PEquivOn f equivSucc (
Iic i)) (H : forall ⦃i⦄ (hi : ¬ IsMax i) x, (equivSucc hi x).1 = f (le_succ i) x
) : PEquivOn f equivSucc (Iic i⁺) where equiv
参数：hi : ¬IsMax i；e : PEquivOn f equivSucc (Iic i)；H : forall ⦃i⦄ (hi : ¬ IsMax i
) x, (equivSucc hi x).1 = f (le_succ i) x。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**InverseSystem.pEquivOnGlue** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：pEquivOnGlue : PEquivOn f equivSucc (Iio i) where equiv
该定义给出了上述对象。
本定义的构造引用了以下数学事实（定理与引理）：
· 使用定理 `Equiv.symm`：Equiv.symm {s t : Computation α} : s ~ t -> t ~ s
...
/-
**InverseSystem.pEquivOnLim** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：pEquivOnLim [InverseSystem f] (equivLim : F i ≃ limit f i) (H : forall x l
, (equivLim x).1 l = f l.2.le x) : PEquivOn f equivSucc (Iic i) where equiv
参数：equivLim : F i ≃ limit f i；H : forall x l, (equivLim x).1 l = f l.2.le x。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Extend `pEquivOnGlue` by one step, obtaining a partial family over a right-close
d interval.
-/
...
set_option backward.privateInPublic true in
/-
**InverseSystem.globalEquivAux** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**InverseSystem.globalEquiv** 是 Mathlib 中的一个定义，位于命名空间 `InverseSystem`。
形式化陈述：globalEquiv (i : ι) : F i ≃ piLT X i
参数：i : ι。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Over a well-ordered type, construct a family of bijections by transfinite recurs
ion.
-/
noncomputable def globalEquiv (i : ι) : F i ≃ piLT X i :=
  (globalEquivAux equivSucc equivLim i).equiv ⟨i, le_rfl⟩
/-
**InverseSystem.globalEquiv_naturality** 是 Mathlib 中的一个定理，位于命名空间 `InverseSystem`
。
形式化陈述：globalEquiv_naturality ⦃i j⦄ (h : i <= j) (x : F j) : letI e
参数：h : i <= j；x : F j。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Order.le_succ`：le_succ : forall a : α, a <= succ a
...
· 使用定理 `InverseSystem.pEquivOn_apply_eq`：pEquivOn_apply_eq (h : IsLowerSet (s in
ter t)) {e₁ : PEquivOn f equivSucc s} {e₂ : PEquivOn f equivSucc t} {i} {his : i
 in s} {hit : i in t}…
...
· 使用定理 `isLowerSet_Iic`：∀ {α : Type u_1} [inst : Preorder α] (a : α), IsLowerSet
 (Set.Iic a)
· 使用定理 `InverseSystem.PEquivOn.nat`：∀ {ι : Type u_6} {F : ι → Type u_7} {X : ι →
 Type u_8} [inst : LinearOrder ι] {f : ⦃i j : ι⦄ → i ≤ j → F j → F i}   [inst_1 
: SuccOrder ι] {…
· 使用引理 `le_rfl`：le_rfl : a <= a
-/
...
/-
**InverseSystem.globalEquiv_compatibility** 是 Mathlib 中的一个定理，位于命名空间 `InverseSyst
em`。
形式化陈述：globalEquiv_compatibility ⦃i⦄ (hi : ¬IsMax i) (x) : globalEquiv equivSucc 
equivLim i⁺ x ⟨i, lt_succ_of_not_isMax hi⟩ = ((equivSucc i hi).1 x).2
参数：hi : ¬IsMax i；x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Order.le_succ`：le_succ : forall a : α, a <= succ a
...
· 使用定理 `Subtype.property`：∀ {α : Sort u} {p : α → Prop} (self : Subtype p), p ↑s
elf
· 使用定理 `InverseSystem.PEquivOn.compat`：∀ {ι : Type u_6} {F : ι → Type u_7} {X : 
ι → Type u_8} [inst : LinearOrder ι] {f : ⦃i j : ι⦄ → i ≤ j → F j → F i}   [inst
_1 : SuccOrder ι] {…
· 使用引理 `le_rfl`：le_rfl : a <= a
-/
