...
@[simps!]
/-
**Rep.FiniteCyclicGroup.coinvariantsTensorResolutionIso** 是 Mathlib 中的一个定义，位于命名空
间 `Rep.FiniteCyclicGroup`。
形式化陈述：coinvariantsTensorResolutionIso (hg : forall x, x in Subgroup.zpowers g) :
 (resolution k g⁻¹ ((@Subgroup.zpowers_inv G ..).symm ▸ hg)).complex.coinvariant
sTensorObj A ≅ moduleCatChainComplex A g
...
entation `A`,
the period chain complex
`... ⟶ (A ⊗ₖ k[G])_G --⟦Id ⊗ N⟧--> (A ⊗ₖ k[G])_G --⟦Id ⊗ (ρ(g⁻¹) - 𝟙)⟧--> (A ⊗ₖ 
k[G])_G ⟶ 0`
is isomorphic as a complex in `ModuleCat k` to
`... ⟶ A --N--> A --(ρ(g) - 𝟙)--> A --N--> A --(ρ(g) - 𝟙)--> A ⟶ 0`.
-/
...
/-
**Rep.FiniteCyclicGroup.groupHomologyIso** 是 Mathlib 中的一个缩写定义，位于命名空间 `Rep.Finite
CyclicGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a finite cyclic group `G` generated by `g` and `A : Rep k G`, `H₀(G, A)` i
s isomorphic
to the cokernel of `ρ(g) - Id(A)`.
-/
...
nonzero and even. -/
/-
**Rep.FiniteCyclicGroup.groupHomologyIsoEven** 是 Mathlib 中的一个定义，位于命名空间 `Rep.Fini
teCyclicGroup`。
形式化陈述：groupHomologyIsoEven (hg : forall x, x in Subgroup.zpowers g) (i : Nat) [h
₀ : NeZero i] (hi : Even i) : groupHomology A i ≅ (subCompNormHom A g).homology
参数：hg : forall x, x in Subgroup.zpowers g；i : Nat；hi : Even i。
该定义给出了上述对象。
...
--- 原说明 ---
Given a finite cyclic group `G` generated by `g` and `A : Rep k G`, `Hᵢ(G, A)` i
s isomorphic
to the homology of the short complex of `k`-modules `A --(ρ(g) - 𝟙)--> A --N--> 
A` when `i` is
nonzero and even.
-/
...
/-
**Rep.FiniteCyclicGroup.groupHomology** 是 Mathlib 中的一个缩写定义，位于命名空间 `Rep.FiniteCyc
licGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a finite cyclic group `G` generated by `g` and `A : Rep k G`, this is the 
quotient map
`Ker(N) ⟶ Ker(N)/Im(ρ(g) - Id(A)) ≅ Hᵢ(G, A)` for any nonzero even `i`.
-/
...
set_option backward.isDefEq.respectTransparency false in
/-
**Rep.FiniteCyclicGroup.groupHomology** 是 Mathlib 中的一个引理，位于命名空间 `Rep.FiniteCycli
cGroup`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
odd. -/
/-
**Rep.FiniteCyclicGroup.groupHomologyIsoOdd** 是 Mathlib 中的一个定义，位于命名空间 `Rep.Finit
eCyclicGroup`。
形式化陈述：groupHomologyIsoOdd (hg : forall x, x in Subgroup.zpowers g) (i : Nat) (hi
 : Odd i) : groupHomology A i ≅ (normHomCompSub A g).homology
参数：hg : forall x, x in Subgroup.zpowers g；i : Nat；hi : Odd i。
该定义给出了上述对象。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。

--- 原说明 ---
Given a finite cyclic group `G` generated by `g` and `A : Rep k G`, `Hⁱ(G, A)` i
s isomorphic
to the homology of the short complex of `k`-modules `A --N--> A --(ρ(g) - 𝟙)--> 
A` when `i` is
odd.
-/
