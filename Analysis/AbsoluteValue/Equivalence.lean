...
theorem isEquiv_iff_lt_one_iff
  ...
  · simp_all
  rw [le_iff_le_iff_lt_iff_lt]; rw [← one_mul (v x)]; rw [← mul_inv_lt_iff₀ (by simp_all)]; rw [← one_mul (w x)]; rw [← mul_inv_lt_iff₀ (by simp_all)]; rw [← map_inv₀]; rw [← map_mul]; rw [← 
...
theorem isEquiv_of_lt_one_imp
  ...
  have hpow (n : Nat) (hv : 1 <= v a) : w x₀ < w a ^ n := by
    rw [← one_mul (_ ^ _)]; rw [← mul_inv_lt_iff₀ (pow_pos (by simp_all) 
...
theorem denseRange_algebraMap_pi
  ...
  have htend : atTop.Tendsto (fun n i => (WithAbs.equiv (
