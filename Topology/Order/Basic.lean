...
中文:
实例 [t
  签名: : OrderTopology α] : OrderTopology αᵒᵈ
  定义体: by
  constructor
  rcases t.topology_eq_generate_intervals with rfl
  simp_rw [Preorder.topology, or_comm]
...
中文:
定理 OrderTopology.continuous_iff
  条件: [OrderTopology α] [TopologicalSpace β] {f : β -> α}
  证明: by
  simp_rw +instances [OrderTopology.topology_eq_generate_intervals, continuous_generateFrom_iff]
  aesop
...
中文:
定理 lt_mem_nhds
  条件: [OrderTopology α] {a b : α} (h : a < b)
...
中文:
定理 tendsto_order
  条件: [OrderTopology α] {f : β -> α} {a : α} {x : Filter β}
  证明: by
  simp only [nhds_eq_order a, tendsto_inf, tendsto_iInf, tendsto_principal]; rfl
...
中文:
引理 exists_countable_generateFrom_Ioi_Iio
  证明: by
  rcases isEmpty_or_nonempty α with hα | hα
  · exact ⟨∅, by simp, Subsingleton.elim _ _⟩
  obtain ⟨t, t_subs, t_count, ht⟩ : exists t subseteq { s | exists a, s = Ioi a ∨ s = Iio a },
...
中文:
引理 isTopologicalBasis_biInter_Ioi_Iio_of_generateFrom
  结论: (c : Set α)
  证明: by
  refine IsTopologicalBasis.of_isOpen_of_subset ?_ (isTopologicalBasis_of_subbasis h) ?_
  · rintro u ⟨f, g, hfc, hgc, hf_fin, hg_fin, rfl⟩
...
定理 tendsto_of_tendsto_of_tendsto_of_le_of_le'
  结论: [OrderTopology α] {f g h : β -> α} {b : Filter β}
...
定理 nhds_order_unbounded
  条件: [OrderTopology α] {a : α} (hu : 存在 u, a < u) (hl : 存在 l, l < a)
  证明: by
  simp only [nhds_eq_order, ← inf_biInf, ← biInf_inf, *, ← inf_principal, ← Ioi_inter_Iio]; rfl
...
定理 tendsto_order_unbounded
  结论: [OrderTopology α] {f : β -> α} {a : α} {x : Filter β}
  证明: by
  simp only [nhds_order_unbounded hu hl, tendsto_iInf, tendsto_principal]
  exact fun l hl u => h l u hl
...
实例 tendstoIxxNhdsWithin
  签名: {α : 类型} [TopologicalSpace α] (a : α) {s t : Set α}
...
定理 induced_topology_le_preorder
  结论: [Preorder α] [Preorder β] [TopologicalSpace β]
  证明: by
  let _ := Preorder.topology α; have : OrderTopology α := ⟨rfl⟩
  refine le_of_nhds_le_nhds fun x => ?_
  simp only [nhds_eq_order, nhds_induced, comap_inf, comap_iInf, comap_principal, Ioi, Iio, ← hf]
...
定理 induced_orderTopology'
  结论: {α β : 类型} [Preorder α] [ta : TopologicalSpace β]
...
中文:
定理 StrictMono.isEmbedding_of_ordConnected
  结论: {α β : 类型} [LinearOrder α] [LinearOrder β]
...
实例 orderTopology_of_ordConnected
  签名: {α : 类型u} [TopologicalSpace α] [LinearOrder α]
...
定理 nhdsGE_eq_iInf_inf_principal
  条件: [TopologicalSpace α] [Preorder α] [OrderTopology α] (a : α)
  证明: by
  rw [nhdsWithin]; rw [nhds_eq_order]
  refine le_antisymm (inf_le_inf_right _ inf_le_right) (le_inf (le_inf ?_ inf_le_left) inf_le_right)
  exact inf_le_right.trans (le_iInf₂ fun l hl => principal_mono.2 <| Ici_subset_Ioi.2 hl)
...
定理 nhdsGE_eq_iInf_principal
  结论: [TopologicalSpace α] [Preorder α] [OrderTopology α] {a : α}
  证明: by
  simp only [nhdsGE_eq_iInf_inf_principal, biInf_inf ha, inf_principal, Iio_inter_Ici]
...
中文:
定理 nhdsGE_basis_of_exists_gt
  结论: [TopologicalSpace α] [LinearOrder α] [OrderTopology α] {a : α}
...
中文:
定理 nhdsGE_basis
  条件: [TopologicalSpace α] [LinearOrder α] [OrderTopology α] [NoMaxOrder α] (a : α)
...
中文:
定理 nhds_top_order
  条件: [TopologicalSpace α] [Preorder α] [OrderTop α] [OrderTopology α]
...
中文:
定理 nhds_top_basis
  结论: [TopologicalSpace α] [LinearOrder α] [OrderTop α] [OrderTopology α]
  证明: by
  have : exists x : α, x < ⊤ := (exists_ne ⊤).imp fun x hx => hx.lt_top
  simpa only [Iic_top, nhdsWithin_univ, Ioc_top] using nhdsLE_basis_of_exists_lt this
...
定理 tendsto_nhds_top_mono
  结论: [TopologicalSpace β] [Preorder β] [OrderTop β] [OrderTopology β]
  证明: by
  simp only [nhds_top_order, tendsto_iInf, tendsto_principal] at hf ⊢
  intro x hx
  filter_upwards [hf x hx, hg] with _ using lt_of_lt_of_le
...
中文:
定理 exists_Ioc_subset_of_mem_nhds
  条件: {a : α} {s : Set α} (hs : s in 𝓝 a) (h : 存在 l, l < a)
...
中文:
定理 exists_Ioc_subset_of_mem_nhds'
  条件: {a : α} {s : Set α} (hs : s in 𝓝 a) {l : α} (hl : l < a)
...
中文:
定理 exists_Icc_mem_subset_of_mem_nhdsGE
  条件: {a : α} {s : Set α} (hs : s in 𝓝[>=] a)
  证明: by
  rcases (em (IsMax a)).imp_right not_isMax_iff.mp with (ha | ha)
  · use a
    simpa [ha.Ici_eq] using hs
...
中文:
定理 exists_Icc_mem_subset_of_mem_nhds
  条件: {a : α} {s : Set α} (hs : s in 𝓝 a)
  证明: by
  rcases exists_Icc_mem_subset_of_mem_nhdsLE (nhdsWithin_le_nhds hs) with
    ⟨b, hba, hb_nhds, hbs⟩
...
中文:
定理 IsOpen.exists_Ioo_subset
  条件: [Nontrivial α] {s : Set α} (hs : IsOpen s) (h : s.Nonempty)
  证明: by
  obtain ⟨x, hx⟩ : exists x, x in s := h
  obtain ⟨y, hy⟩ : exists y, y != x := exists_ne x
  rcases lt_trichotomy x y with (H | rfl | H)
...
中文:
定理 dense_of_exists_between
  结论: [Nontrivial α] {s : Set α}
  证明: by
  refine dense_iff_inter_open.2 fun U U_open U_nonempty => ?_
  obtain ⟨a, b, hab, H⟩ : exists a b : α, a < b ∧ Ioo a b subseteq U := U_open.exists_Ioo_subset U_nonempty
  obtain ⟨x, xs, hx⟩ : exists x in s, a < x ∧ x < b := h hab
...
中文:
定理 IsUpperSet.isClosed
  条件: [WellFoundedLT α] {s : Set α} (h : IsUpperSet s)
  结论: IsClosed s
  证明: by
  obtain rfl | ⟨a, rfl⟩ := h.eq_empty_or_Ici
  exacts [isClosed_empty, isClosed_Ici]
...
中文:
定理 IsLowerSet.isOpen
  条件: [WellFoundedLT α] {s : Set α} (h : IsLowerSet s)
  结论: IsOpen s
  证明: by
  simpa using h.compl.isClosed
...
中文:
定理 dense_iff_exists_between
  条件: [DenselyOrdered α] [Nontrivial α] {s : Set α}
...
中文:
定理 mem_nhds_iff_exists_Ioo_subset'
  结论: {a : α} {s : Set α} (hl : 存在 l, l < a)
  证明: by
  constructor
  · intro h
    rcases exists_Ico_subset_of_mem_nhds h hu with ⟨u, au, hu⟩
...
中文:
定理 mem_nhds_iff_exists_Ioo_subset
  结论: [NoMaxOrder α] [NoMinOrder α] {a : α}
...
中文:
定理 nhds_basis_Ioo
  条件: [NoMaxOrder α] [NoMinOrder α] (a : α)
...
中文:
定理 Filter.Eventually.exists_Ioo_subset
  结论: [NoMaxOrder α] [NoMinOrder α] {a : α}
...
中文:
定理 Dense.topology_eq_generateFrom
  结论: [DenselyOrdered α] {s : Set α}
  证明: by
  refine (OrderTopology.topology_eq_generate_intervals (α := α)).trans ?_
  refine le_antisymm (generateFrom_anti ?_) (le_generateFrom ?_)
  · simp only [union_subset_iff, image_subset_iff]
...
中文:
定理 SuccOrder.hasBasis_nhds_Ioc_of_exists_lt
  条件: [SuccOrder α] {a : α} (ha : 存在 l, l < a)
...
中文:
定理 SuccOrder.hasBasis_nhds_Ioc
  条件: [SuccOrder α] {a : α} [NoMinOrder α]
...
中文:
定理 SecondCountableTopology.of_separableSpace_orderTopology
  结论: [DenselyOrdered α]
  证明: by
  rcases exists_countable_dense α with ⟨s, hc, hd⟩
  refine ⟨⟨_, ?_, hd.topology_eq_generateFrom⟩⟩
...
中文:
定理 Set.PairwiseDisjoint.countable_of_Ioo
...
中文:
引理 Continuous.of_ordContinuous
