...
/-
**DFunLike** 是 Mathlib 中的一个归纳类型，位于命名空间 ``。
形式化陈述：Sort u_1 → (α : outParam (Sort u_2)) → outParam (α → Sort u_3) → Sort (max
 (max (max 1 u_1) u_2) u_3)
参数：α : outParam (Sort u_2)；α → Sort u_3；max (max (max 1 u_1) u_2) u_3。
...
-/
/-
**FunLike** 是 Mathlib 中的一个缩写定义，位于命名空间 ``。
形式化陈述：FunLike F α β
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
...
/-
**DFunLike.** 是 Mathlib 中的一个实例，位于命名空间 `DFunLike`。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
/-
**DFunLike.coe_eq_coe_fn** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：coe_eq_coe_fn : (DFunLike.coe (F
该定理/引理描述了相关对象所满足的性质。
黑盒内容：本声明未引用其他定理/引理；其成立仅依赖定义、结构与类型类实例。
-/
...
@[simp]
/-
**DFunLike.coe_fn_eq** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：coe_fn_eq {f g : F} : (f : forall a : α, β a) = (g : forall a : α, β a) ↔ 
f = g
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.ext'** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：ext' {f g : F} (h : (f : forall a : α, β a) = (g : forall a : α, β a)) : f
 = g
参数：h : (f : forall a : α, β a) = (g : forall a : α, β a)。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `DFunLike.coe_injective`：∀ {F : Sort u_1} {α : outParam (Sort u_2)} {β : 
outParam (α → Sort u_3)} [self : DFunLike F α β],   Function.Injective DFunLike.
...
/-
**DFunLike.ext'_iff** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：∀ {F : Sort u_1} {α : Sort u_2} {β : α → Sort u_3} [i : DFunLike F α β] {f
 g : F}, f = g ↔ ⇑f = ⇑g
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.ext** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：ext (f g : F) (h : forall x : α, f x = g x) : f = g
参数：f g : F；h : forall x : α, f x = g x。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `DFunLike.coe_injective`：∀ {F : Sort u_1} {α : outParam (Sort u_2)} {β : 
outParam (α → Sort u_3)} [self : DFunLike F α β],   Function.Injective DFunLike.
...
/-
**DFunLike.ext_iff** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：ext_iff {f g : F} : f = g ↔ forall x, f x = g x
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.congr_fun** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：∀ {F : Sort u_1} {α : Sort u_2} {β : α → Sort u_3} [i : DFunLike F α β] {f
 g : F}, f = g → ∀ (x : α), f x = g x
参数：x : α。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.ne_iff** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：ne_iff {f g : F} : f != g ↔ exists a, f a != g a
该定理/引理刻画了左右两侧的等价关系。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Iff.trans`：∀ {a b c : Prop}, (a ↔ b) → (b ↔ c) → (a ↔ c)
...
/-
**DFunLike.exists_ne** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：exists_ne {f g : F} (h : f != g) : exists x, f x != g x
参数：h : f != g。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.subsingleton_cod** 是 Mathlib 中的一个引理，位于命名空间 `DFunLike`。
形式化陈述：subsingleton_cod [forall a, Subsingleton (β a)] : Subsingleton F
参数：β a。
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
...
/-- This is not an instance to avoid slowing down every single `Subsingleton` typeclass search. -/
/-
**DFunLike.subsingleton_dom** 是 Mathlib 中的一个引理，位于命名空间 `DFunLike`。
形式化陈述：subsingleton_dom [IsEmpty α] : Subsingleton F
该定理/引理描述了相关对象所满足的性质。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `Function.Injective.subsingleton`：∀ {α : Sort u_1} {β : Sort u_2} {f : α 
→ β}, Function.Injective f → ∀ [Subsingleton β], Subsingleton α
...
/-
**DFunLike.congr** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：∀ {F : Sort u_1} {α : Sort u_2} {β : Sort u_3} [i : FunLike F α β] {f g : 
F} {x y : α}, f = g → x = y → f x = g y
黑盒证明引用了以下数学事实（定理与引理）：
...
/-
**DFunLike.congr_arg** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：∀ {F : Sort u_1} {α : Sort u_2} {β : Sort u_3} [i : FunLike F α β] (f : F)
 {x y : α}, x = y → f x = f y
参数：f : F。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `congr_arg`：∀ {α : Sort u} {β : Sort v} {a₁ a₂ : α} (f : α → β), a₁ = a₂ 
...
protected theorem congr_arg (f : F) {x y : α} (h₂ : x = y) : f x = f y :=
  congr_arg _ h₂
/-
**DFunLike.dite_apply** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：dite_apply {P : Prop} [Decidable P] (f : P -> F) (g : ¬P -> F) (x : α) : (
...
theorem dite_apply {P : Prop} [Decidable P] (f : P → F) (g : ¬P → F) (x : α) :
  ...
  split_ifs <;> rfl
/-
**DFunLike.ite_apply** 是 Mathlib 中的一个定理，位于命名空间 `DFunLike`。
形式化陈述：ite_apply {P : Prop} [Decidable P] (f g : F) (x : α) : (if P then f else g
) x = if P then f x else g x
参数：f g : F；x : α。
该定理/引理给出了一组等式。
黑盒证明引用了以下数学事实（定理与引理）：
· 使用定理 `DFunLike.dite_apply`：dite_apply {P : Prop} [Decidable P] (f : P -> F) (g
 : ¬P -> F) (x : α) : (if h : P then f h else g h) x = if h : P then f h x else 
g h x
-/
