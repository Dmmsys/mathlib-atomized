/-
Copyright (c) 2021 Sébastien Gouëzel. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Sébastien Gouëzel
-/
module

public import Mathlib.Order.Filter.AtTopBot.Archimedean
public import Mathlib.Order.Filter.AtTopBot.Finite
public import Mathlib.Order.Filter.AtTopBot.Prod
public import Mathlib.Topology.Algebra.Ring.Real

/-!
# Convergence of subadditive sequences

A subadditive sequence `u : ℕ → ℝ` is a sequence satisfying `u (m + n) ≤ u m + u n` for all `m, n`.
We define this notion as `Subadditive u`, and prove in `Subadditive.tendsto_lim` that, if `u n / n`
is bounded below, then it converges to a limit (that we denote by `Subadditive.lim` for
convenience). This result is known as Fekete's lemma in the literature.

## TODO

Define a bundled `SubadditiveHom`, use it.
-/

@[expose] public section

noncomputable section

open Set Filter Topology

/--
Definition of `Subadditive` / `Subadditive` 的定义

English:
definition Subadditive
  signature: (u : Nat -> Real)
  body: forall m n, u (m + n) <= u m + u n

中文:
定义 Subadditive
  签名: (u : 自然数 -> 实数)
  定义体: forall m n, u (m + n) <= u m + u n
-/
def Subadditive (u : Nat -> Real) : Prop :=
  forall m n, u (m + n) <= u m + u n

namespace Subadditive

variable {u : Nat -> Real} (h : Subadditive u)

/-- The limit of a bounded-below subadditive sequence. The fact that the sequence indeed tends to
this limit is given in `Subadditive.tendsto_lim` -/
@[nolint unusedArguments, irreducible]
/--
Definition of `lim` / `lim` 的定义

English:
definition lim
  signature: (_h : Subadditive u)
  body: sInf ((fun n : Nat => u n / n) '' Ici 1)

中文:
定义 lim
  签名: (_h : Subadditive u)
  定义体: sInf ((fun n : Nat => u n / n) '' Ici 1)
-/
protected def lim (_h : Subadditive u) :=
  sInf ((fun n : Nat => u n / n) '' Ici 1)

/--
theorem `lim_le_div` / 定理 `lim_le_div`

English:
theorem lim_le_div
  given: (hbdd : BddBelow (range fun n => u n / n)) {n : Nat} (hn : n != 0)
  proof: by
  rw [Subadditive.lim]
  exact csInf_le (hbdd.mono <| image_subset_range _ _) ⟨n, hn.bot_lt, rfl⟩

include h in

中文:
定理 lim_le_div
  条件: (hbdd : BddBelow (range fun n => u n / n)) {n : 自然数} (hn : n != 0)
  证明: by
  rw [Subadditive.lim]
  exact csInf_le (hbdd.mono <| image_subset_range _ _) ⟨n, hn.bot_lt, rfl⟩

include h in

Depends on / 依赖: Subadditive, Subadditive.lim, bot_lt, csInf_le, hbdd.mono, hn.bot_lt, image_subset_range
-/
theorem lim_le_div (hbdd : BddBelow (range fun n => u n / n)) {n : Nat} (hn : n != 0) :
    h.lim <= u n / n := by
  rw [Subadditive.lim]
  exact csInf_le (hbdd.mono <| image_subset_range _ _) ⟨n, hn.bot_lt, rfl⟩

include h in
/--
theorem `apply_mul_add_le` / 定理 `apply_mul_add_le`

English:
theorem apply_mul_add_le
  given: (k n r)
  statement: u (k * n + r) <= k * u n + u r
  proof: by
  induction k with
  | zero => simp only [Nat.cast_zero, zero_mul, zero_add]; rfl
  | succ k IH =>
    calc
      u ((k + 1) * n + r) = u (n + (k * n + r)) := by congr 1; ring
      _ <= u n + u (k * n + r) := h _ _
      _ <= u n + (k * u n + u r) := by grw [IH]
      _ = (k + 1 : Nat) * u n + u r := by simp; ring

include h in

中文:
定理 apply_mul_add_le
  条件: (k n r)
  结论: u (k * n + r) <= k * u n + u r
  证明: by
  induction k with
  | zero => simp only [Nat.cast_zero, zero_mul, zero_add]; rfl
  | succ k IH =>
    calc
      u ((k + 1) * n + r) = u (n + (k * n + r)) := by congr 1; ring
      _ <= u n + u (k * n + r) := h _ _
      _ <= u n + (k * u n + u r) := by grw [IH]
      _ = (k + 1 : Nat) * u n + u r := by simp; ring

include h in

Depends on / 依赖: Nat.cast_zero, cast_zero, zero_add, zero_mul
-/
theorem apply_mul_add_le (k n r) : u (k * n + r) <= k * u n + u r := by
  induction k with
  | zero => simp only [Nat.cast_zero, zero_mul, zero_add]; rfl
  | succ k IH =>
    calc
      u ((k + 1) * n + r) = u (n + (k * n + r)) := by congr 1; ring
      _ <= u n + u (k * n + r) := h _ _
      _ <= u n + (k * u n + u r) := by grw [IH]
      _ = (k + 1 : Nat) * u n + u r := by simp; ring

include h in
/--
theorem `eventually_div_lt_of_div_lt` / 定理 `eventually_div_lt_of_div_lt`

English:
theorem eventually_div_lt_of_div_lt
  given: {L : Real} {n : Nat} (hn : n != 0) (hL : u n / n < L)
  proof: by
  /- It suffices to prove the statement for each arithmetic progression `(n * · + r)`. -/
  refine .atTop_of_arithmetic hn fun r _ => ?_
  /- `(k * u n + u r) / (k * n + r)` tends to `u n / n < L`, hence
  `(k * u n + u r) / (k * n + r) < L` for sufficiently large `k`. -/
  have A : Tendsto (fun x : Real => (u n + u r / x) / (n + r / x)) atTop (𝓝 ((u n + 0) / (n + 0))) :=
    (tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id).div
(tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id) by simpa
  have B : Tendsto (fun x => (x * u n + u r) / (x * n + r)) atTop (𝓝 (u n / n)) := by
    rw [add_zero]; rw [add_zero] at A
refine A.congr' (eventually_ne_atTop 0).mono fun x hx => ?_
    simp only [add_div' _ _ _ hx, div_div_div_cancel_right₀ hx, mul_comm]
  refine ((B.comp tendsto_natCast_atTop_atTop).eventually (gt_mem_nhds hL)).mono fun k hk => ?_
  /- Finally, we use an upper estimate on `u (k * n + r)` to get an estimate on
  `u (k * n + r) / (k * n + r)`. -/
  rw [mul_comm]
  refine lt_of_le_of_lt ?_ hk
  simp only [(· ∘ ·), ← Nat.cast_add, ← Nat.cast_mul]
  gcongr
  apply h.apply_mul_add_le

中文:
定理 eventually_div_lt_of_div_lt
  条件: {L : 实数} {n : 自然数} (hn : n != 0) (hL : u n / n < L)
  证明: by
  /- It suffices to prove the statement for each arithmetic progression `(n * · + r)`. -/
  refine .atTop_of_arithmetic hn fun r _ => ?_
  /- `(k * u n + u r) / (k * n + r)` tends to `u n / n < L`, hence
  `(k * u n + u r) / (k * n + r) < L` for sufficiently large `k`. -/
  have A : Tendsto (fun x : Real => (u n + u r / x) / (n + r / x)) atTop (𝓝 ((u n + 0) / (n + 0))) :=
    (tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id).div
(tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id) by simpa
  have B : Tendsto (fun x => (x * u n + u r) / (x * n + r)) atTop (𝓝 (u n / n)) := by
    rw [add_zero]; rw [add_zero] at A
refine A.congr' (eventually_ne_atTop 0).mono fun x hx => ?_
    simp only [add_div' _ _ _ hx, div_div_div_cancel_right₀ hx, mul_comm]
  refine ((B.comp tendsto_natCast_atTop_atTop).eventually (gt_mem_nhds hL)).mono fun k hk => ?_
  /- Finally, we use an upper estimate on `u (k * n + r)` to get an estimate on
  `u (k * n + r) / (k * n + r)`. -/
  rw [mul_comm]
  refine lt_of_le_of_lt ?_ hk
  simp only [(· ∘ ·), ← Nat.cast_add, ← Nat.cast_mul]
  gcongr
  apply h.apply_mul_add_le
-/
theorem eventually_div_lt_of_div_lt {L : Real} {n : Nat} (hn : n != 0) (hL : u n / n < L) :
    forallᶠ p in atTop, u p / p < L := by
  /- It suffices to prove the statement for each arithmetic progression `(n * · + r)`. -/
  refine .atTop_of_arithmetic hn fun r _ => ?_
  /- `(k * u n + u r) / (k * n + r)` tends to `u n / n < L`, hence
  `(k * u n + u r) / (k * n + r) < L` for sufficiently large `k`. -/
  have A : Tendsto (fun x : Real => (u n + u r / x) / (n + r / x)) atTop (𝓝 ((u n + 0) / (n + 0))) :=
    (tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id).div
(tendsto_const_nhds.add <| tendsto_const_nhds.div_atTop tendsto_id) by simpa
  have B : Tendsto (fun x => (x * u n + u r) / (x * n + r)) atTop (𝓝 (u n / n)) := by
    rw [add_zero]; rw [add_zero] at A
refine A.congr' (eventually_ne_atTop 0).mono fun x hx => ?_
    simp only [add_div' _ _ _ hx, div_div_div_cancel_right₀ hx, mul_comm]
  refine ((B.comp tendsto_natCast_atTop_atTop).eventually (gt_mem_nhds hL)).mono fun k hk => ?_
  /- Finally, we use an upper estimate on `u (k * n + r)` to get an estimate on
  `u (k * n + r) / (k * n + r)`. -/
  rw [mul_comm]
  refine lt_of_le_of_lt ?_ hk
  simp only [(· ∘ ·), ← Nat.cast_add, ← Nat.cast_mul]
  gcongr
  apply h.apply_mul_add_le

/--
theorem `tendsto_lim` / 定理 `tendsto_lim`

English:
theorem tendsto_lim
  given: (hbdd : BddBelow (range fun n => u n / n))
  proof: by
  refine tendsto_order.2 ⟨fun l hl => ?_, fun L hL => ?_⟩
  · refine eventually_atTop.2
      ⟨1, fun n hn => hl.trans_le (h.lim_le_div hbdd (zero_lt_one.trans_le hn).ne')⟩
  · obtain ⟨n, npos, hn⟩ : exists n : Nat, 0 < n ∧ u n / n < L := by
      rw [Subadditive.lim] at hL
      rcases exists_lt_of_csInf_lt (by simp) hL with ⟨x, hx, xL⟩
      rcases (mem_image _ _ _).1 hx with ⟨n, hn, rfl⟩
      exact ⟨n, zero_lt_one.trans_le hn, xL⟩
    exact h.eventually_div_lt_of_div_lt npos.ne' hn

中文:
定理 tendsto_lim
  条件: (hbdd : BddBelow (range fun n => u n / n))
  证明: by
  refine tendsto_order.2 ⟨fun l hl => ?_, fun L hL => ?_⟩
  · refine eventually_atTop.2
      ⟨1, fun n hn => hl.trans_le (h.lim_le_div hbdd (zero_lt_one.trans_le hn).ne')⟩
  · obtain ⟨n, npos, hn⟩ : exists n : Nat, 0 < n ∧ u n / n < L := by
      rw [Subadditive.lim] at hL
      rcases exists_lt_of_csInf_lt (by simp) hL with ⟨x, hx, xL⟩
      rcases (mem_image _ _ _).1 hx with ⟨n, hn, rfl⟩
      exact ⟨n, zero_lt_one.trans_le hn, xL⟩
    exact h.eventually_div_lt_of_div_lt npos.ne' hn

Depends on / 依赖: Subadditive, Subadditive.lim, eventually_atTop, eventually_div_lt_of_div_lt, exists_lt_of_csInf_lt, h.eventually_div_lt_of_div_lt, h.lim_le_div, hl.trans_le, lim_le_div, mem_image, npos.ne, tendsto_order, trans_le, zero_lt_one, zero_lt_one.trans_le
-/
theorem tendsto_lim (hbdd : BddBelow (range fun n => u n / n)) :
    Tendsto (fun n => u n / n) atTop (𝓝 h.lim) := by
  refine tendsto_order.2 ⟨fun l hl => ?_, fun L hL => ?_⟩
  · refine eventually_atTop.2
      ⟨1, fun n hn => hl.trans_le (h.lim_le_div hbdd (zero_lt_one.trans_le hn).ne')⟩
  · obtain ⟨n, npos, hn⟩ : exists n : Nat, 0 < n ∧ u n / n < L := by
      rw [Subadditive.lim] at hL
      rcases exists_lt_of_csInf_lt (by simp) hL with ⟨x, hx, xL⟩
      rcases (mem_image _ _ _).1 hx with ⟨n, hn, rfl⟩
      exact ⟨n, zero_lt_one.trans_le hn, xL⟩
    exact h.eventually_div_lt_of_div_lt npos.ne' hn

end Subadditive
